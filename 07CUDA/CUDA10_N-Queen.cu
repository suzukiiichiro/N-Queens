...
13:        73712            9233  00:00:00:01.04
14:       365596           45752  00:00:00:01.34
15:      2279184          285053  00:00:00:04.99
16:     14772512         1846955  00:00:00:32.64
17:     95815104        11977939  00:00:04:09.09
*/
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
void NQueenG(int size,int steps)
{
  ...
  /***10 backtrack1,2を意識し１行目右端とそれ以外で処理を分ける*********************/
...
//
/***10 solve_nqueenrをbackTrack2に変更*********************/
...
//
/***10 solve_nqueenrをbackTrack1に変更*********************/
//void solve_nqueen(int size,int mask, int row,int h_left,int h_down,int h_right,unsigned int* aBoard)
...
void backTrackR2(int size,int mask, int row,int left,int down,int right,unsigned int* aBoard)
{
 ...
 if(row==sizeE){
    ...
    while(bitmap){
      ...
      backTrackR2(size,mask,row+1,(left|bit)<<1, down|bit,(right|bit)>>1,aBoard);
    }
  }
}
//
/***10 solve_nqueenrをbackTrackR1に変更*********************/
...
void backTrackR1(int size,int mask, int row,int left,int down,int right,unsigned int* aBoard)
{
 ...
 if(row==sizeE){
    ...
    while(bitmap){
      bitmap^=aBoard[row]=bit=(-bitmap&bitmap);
      backTrackR1(size,mask,row+1,(left|bit)<<1, down|bit,(right|bit)>>1,aBoard);
    }
  }
}  
//CPUR 再帰版 ロジックメソッド
...
void backTrack2D_NR(int size,int mask,int row,int left,int down,int right,unsigned int* aBoard){
  int bitmap,bit;
  int b[100], *p=b;
  int sizeE=size-1;
...
void NQueenD(int size,int mask){
  ...
  for(BOUND1=1,BOUND2=size-2;BOUND1<BOUND2;BOUND1++,BOUND2--){
    ...
    backTrack2D_NR(size,mask,1,bit<<1,bit,bit>>1,aBoard);
...
// 通常版
/***09 backTrack登場メソッド名だけ枝刈りはまだしない*****/  
void backTrack2D(int size,int mask,int row,int left,int down,int right,unsigned int* aBoard){
  int bit;
  ...
  if(row==size){
    ...
    while(bitmap){
      ...
      backTrack2D(size,mask,row+1,(left|bit)<<1,down|bit,(right|bit)>>1,aBoard);
    }
  }
}
//
...
void NQueenDR(int size,int mask)
{
 ...
 /***09 backTrack登場メソッド名だけ枝刈りはまだしない*****/  
  ...
  for(BOUND1=1,BOUND2=size-2;BOUND1<BOUND2;BOUND1++,BOUND2--){
    aBoard[0]=bit=(1<<BOUND1);
    backTrack2D(size,mask,1,bit<<1,bit,bit>>1,aBoard);
    LASTMASK|=LASTMASK>>1|LASTMASK<<1;
    ENDBIT>>=1;
  }
}
