...
12:        14200            1787  00:00:00:00.36
13:        73712            9233  00:00:00:01.63
14:       365596           45752  00:00:00:09.07
15:       278064           34758  00:00:00:09.27
...
__global__ void nqueen_cuda_backTrack2(long *d_results,int *d_aBoard,int *d_aT,int *d_aS,int size,int mask,int row,int left,int down,int right,int BOUND1,int BOUND2,int SIDEMASK,int LASTMASK);
__global__ void nqueen_cuda_backTrack1(long *d_results,int *d_aBoard,int *d_aT,int *d_aS,int size,int mask,int row,int left,int down,int right,int BOUND1);
...
//【通常版】
void _backTrack2(int si,int mask,int y,int l,int d,int r);
void _backTrack1(int si,int mask,int y,int l,int d,int r);
...
//GPU
__global__
void nqueen_cuda_backTrack2(long *d_results,int *d_aBoard,int *d_aT,int *d_aS,int size,int mask,int row,int left,int down,int right,int BOUND1,int BOUND2,int SIDEMASK,int LASTMASK){
  int bitmap,bit;
  int b[100], *p=b;
  int sizeE=size-1;
...
//GPU
__global__
void nqueen_cuda_backTrack1(long *d_results,int *d_aBoard,int *d_aT,int *d_aS,int size,int mask,int row,int left,int down,int right,int BOUND1){
  int bitmap,bit;
  int b[100], *p=b;
  int sizeE=size-1;
  int odd=size&1; //奇数:1 偶数:0
...
void solve_nqueen_cuda(int si,int mask,long results[2],int steps){
  ...
  for(BOUND1=2;BOUND1<si-1;BOUND1++){
    ...
    //実行
    nqueen_cuda_backTrack1<<<1,1>>>(d_results,d_aBoard,d_aT,d_aS,si,mask,2,(2|bit)<<1,(1|bit),(bit>>1),BOUND1);
    //device to host
  ...
  for(BOUND1=1,BOUND2=si-2;BOUND1<BOUND2;BOUND1++,BOUND2--){
    ...
    cudaMemcpy(d_results,h_results,
        sizeof(long)*steps,cudaMemcpyHostToDevice);
    //実行
    nqueen_cuda_backTrack2<<<1,1>>>(d_results,d_aBoard,d_aT,d_aS,si,mask,1,bit<<1,bit,bit>>1,BOUND1,BOUND2,SIDEMASK,LASTMASK);
    //device to host
    cudaMemcpy(h_results,d_results,
        sizeof(long)*steps,cudaMemcpyDeviceToHost);
    //
...
//
//【通常版】
void _backTrack2(int size,int mask,int row,int left,int down,int right){
  int bit;
...
//
//【通常版】
void _backTrack1(int size,int mask,int row,int left,int down,int right){
  int bit;
  int bitmap=mask&~(left|down|right);
  //【枝刈り】１行目角にクイーンがある場合回転対称チェックを省略
  if(row==size-1) {
