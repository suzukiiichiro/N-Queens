...
void NQueenR()
{
  ...
  for(int w=0;w<=(size/2)*(size-3);w++){
    if(pres_a[w]>0 && pres_a[w]>beforepres){
      LASTMASK|=LASTMASK>>1|LASTMASK<<1;
      BOUND2=size-1-pres_a[w];
      beforepres=pres_a[w];
    }
    printf("BOUND2:%d:pa:%d,sm:%d,lm:%d,tb:%d\n",BOUND2,pres_a[w],SIDEMASK,LASTMASK,TOPBIT);
    //
    B=wB;
    ...
    if(placement(1,pres_b[w])==false){ continue; }//1行目にクイーンを置く
    //
    //
    //
    //
