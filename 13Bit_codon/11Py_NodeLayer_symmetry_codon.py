...
class Local:
    ...
    def __init__(self, TOPBIT: int, ENDBIT: int, LASTMASK: int, SIDEMASK: int,
                 BOUND1: int, BOUND2: int, board: List[int]) -> None:
        self.TOPBIT = TOPBIT
        self.ENDBIT = ENDBIT
...
class NQueens11:
    ...
    # ------------------------------------------------------------
    def kLayer_nodeLayer_backtrack_corner(self, size: int, nodes: List[int], k: int,
                                          left: int, down: int, right: int,
                                          local: Local, local_list: List[Local]) -> None:
        mask: int = (1 << size) - 1
        bitmap: int = mask & ~(left | down | right)
        row: int = self.count_bits_nodeLayer(down)
        if row == k:
            nodes.extend((left, down, right))
            local_list.append(Local(TOPBIT=local.TOPBIT, ENDBIT=local.ENDBIT, LASTMASK=local.LASTMASK,
                                    SIDEMASK=local.SIDEMASK, BOUND1=local.BOUND1, BOUND2=local.BOUND2,
        ...
        while bitmap:
            ...
            local.board[row] = bit
            self.kLayer_nodeLayer_backtrack_corner(size, nodes, k, (left | bit) << 1, (down | bit), (right | bit) >> 1,
                                                   local, local_list)

    def kLayer_nodeLayer_backtrack(self, size: int, nodes: List[int], k: int,
                                   left: int, down: int, right: int,
                                   local: Local, local_list: List[Local]) -> None:
        mask: int = (1 << size) - 1
        bitmap: int = mask & ~(left | down | right)
        ...
        while bitmap:
            ...
            bitmap ^= bit
            local.board[row] = bit
            self.kLayer_nodeLayer_backtrack(size, nodes, k, (left | bit) << 1, (down | bit), (right | bit) >> 1,
                                            local, local_list)

    def kLayer_nodeLayer(self, size: int, nodes: List[int], k: int, local_list: List[Local]) -> None:
        local = Local(TOPBIT=1 << (size - 1), ENDBIT=0, LASTMASK=0, SIDEMASK=0,
        ...
        while local.BOUND1 > 1 and local.BOUND1 < size - 1:
            if local.BOUND1 < size - 1:
                bit = 1 << local.BOUND1
                local.board[1] = bit
                self.kLayer_nodeLayer_backtrack_corner(size, nodes, k, (2 | bit) << 1, (1 | bit), (2 | bit) >> 1,
                                                       local, local_list)
            local.BOUND1 += 1
        # 角なし
