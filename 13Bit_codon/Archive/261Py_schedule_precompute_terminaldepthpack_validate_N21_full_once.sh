...
# =============================================================================
# 261 N=21 full validation harness
# Parent: 256 terminaldepthpack JIT-ok / 244 r4 maskpack / 241 restore239.
...
SRC=${SRC:-./261Py_schedule_precompute_terminaldepthpack_probe.py}
CAND=${CAND:-./261Py_schedule_precompute_terminaldepthpack_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/261Py_schedule_precompute_terminaldepthpack_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/261Py_schedule_precompute_terminaldepthpack_logs_N21_full_once_${TS}"
...
echo "[start] 261 schedule-precompute-terminaldepthpack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 261 schedule-precompute-terminaldepthpack terminal-depth-only SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '261 schedule-precompute-terminaldepthpack probe' "$SRC"; then
  printf 'source_version_tag\t261 schedule-precompute-terminaldepthpack probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t261 schedule-precompute-terminaldepthpack probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split261_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s and 'split244_' not in s and 'split255_' not in s and 'split256_' not in s and 'split257_' not in s and 'split258_' not in s and 'split259_' not in s and 'split260_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split261 active; split240/241/242/243/244/255/256/257/258/259/260 runtime tags absent', 'split261', True))
else:
    checks.append(('source_split_tag', 'split261 active; split240/241/242/243/244/255/256/257/258/259/260 runtime tags absent', 'old split runtime tag present or split261 missing', False))
...
if not m14:
    ...
    ok = (
        ...
        'sched_terminal_depth_arr:Ptr[u32]' in body and
        ...
        'terminal_depth:int=int(sched_terminal_depth_arr[i]&u32(15))' in body and
    ...
    actual = 'terminaldepthpack precompute active' if ok else 'old schedule interpreter present or terminaldepthpack loads missing or root_action/action/ctrl pack still present'
    checks.append(('source_schedule_precompute_terminaldepthpack', 'MAXD14 precompute terminaldepthpack active', actual, ok))
...
checks.append(('source_host_precompute_terminaldepthpack_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
echo "[static-ok] 261 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 261 schedule-precompute-terminaldepthpack mode31 split145"
...
echo "[validation-ok] 261 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
