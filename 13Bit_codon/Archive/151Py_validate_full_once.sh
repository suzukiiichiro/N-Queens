...
# 151 single-pass validation harness
#
...
# Candidate 151 reuses loop-head avail/ctrl locals and removes only their duplicate stack reloads.
# A real flock guard prevents two copies of this harness from competing.

SRC=${SRC:-./151Py_frame_reload_elision_probe.py}
CAND=${CAND:-./151Py_frame_reload_elision_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/151Py_nqueens_gpu_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 150/149 full runs and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 151 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/151Py_logs_N21_full_once_${TS}"
...
echo "[validation-ok] 151 one full run reproduced cases 01-06"
