...
# =============================================================================
# NQ_UPDATE_MEMO
# 279: restore276-after-zeroconst-reject。278 zero-const-assign は
#      MAXD14 kernel内の単純な u32(0) 代入を定数変数参照へ置換したが、
#      N=21 full onceで正当性OKながら 0:07:09.603 と明確に退行したため不採用。
#      277 depth_u child/save共通化、278 zero-const変数化を撤回し、
#      276 restore274-after-bucketdiag-reject の本線へ戻す安全復帰版。
#      CUDA MAXD14/16/18/20/21 kernel本文、rootrestlate、futuremask、
#      no-sibling、split145/chunkshape148、broadmarktail、cache生成、
#      worker split、mode28/29/30/31、MAXD fallback、CPU dfs_iter pathは276相当で維持。
#      273 root0 direct kernel、275 bucket診断、277 depth_u、278 zero-const、
...
今回は **278を不採用にして276へ戻す安全復帰版**です。277の `depth_u`、278のzero-const変数化、275 bucket診断、273 root0 direct kernel は入れていません。

`STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [279Py_restore276_after_zeroconst_reject_probe.py](sandbox:/mnt/data/279Py_restore276_after_zeroconst_reject_probe.py)
* [279Py_restore276_after_zeroconst_reject_validate_N21_full_once.sh](sandbox:/mnt/data/279Py_restore276_after_zeroconst_reject_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [279Py_restore276_after_zeroconst_reject_sha256.txt](sandbox:/mnt/data/279Py_restore276_after_zeroconst_reject_sha256.txt)
* [279Py_restore276_after_zeroconst_reject_bundle_20260709.tar](sandbox:/mnt/data/279Py_restore276_after_zeroconst_reject_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./279Py_restore276_after_zeroconst_reject_probe

STATIC_ONLY=1 bash 279Py_restore276_after_zeroconst_reject_validate_N21_full_once.sh

bash 279Py_restore276_after_zeroconst_reject_validate_N21_full_once.sh
```

期待値は、276相当の `0:07:07.6xx〜0:07:07.7xx` 付近へ戻ることです。
...
VERSION_TAG:str="279 restore276-after-zeroconst-reject: 278 zero-const-assign rejected for speed; restored 276/274/271 final-fastest-complete baseline; generic N5..27; bare -g split145 mode31; unused recursive CPU dfs fallback remains removed; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved; CUDA kernels unchanged"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split279_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split279_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split279-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split279-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split279_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
