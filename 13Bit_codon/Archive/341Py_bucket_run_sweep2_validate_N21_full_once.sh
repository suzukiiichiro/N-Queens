...
# =============================================================================
# 341 N=21 full validation harness (bucket-run-sweep2)
# Base: 340/339/338/337/336/335/334/333/328-332 lineage.
#
# 341 CONTAINS NO SOURCE CODE CHANGE AT ALL. 341Py_bucket_run_sweep2.py is
# byte-for-byte identical to 340's and 339's source apart from VERSION_TAG
# and documentation.
#
# 340 SWEEP RESULT (all six points in one session, every point reproduced
# 314666222712 exactly, every point cache_state=reuse, zero failures):
#     RUN=1   450.183s   in-session control
#     RUN=16  448.931s   -0.278% vs RUN=1
#     RUN=32  447.141s / 447.213s   anchor, measured first and last
#     RUN=48  448.995s   -0.264% vs RUN=1
#     RUN=64  446.390s   -0.843% vs RUN=1   <-- best so far
#   Session drift (the two RUN=32 anchors) is 0.072s = 0.016%, so every gap
#   above is 4x to 50x the drift.
#
# THE MECHANISM IS SETTLED. RUN=48 is LARGER than 32 and still regressed to
# 448.995s, level with RUN=16's 448.931s. So what matters is not run length
# but that the run length is a MULTIPLE OF THE WARP WIDTH -- bucket
# boundaries aligning with warp boundaries. That was the branch criterion
# fixed in advance in 340, and it resolved to alignment.
#
# WHAT 341 ASKS. One residual is unexplained: RUN=64 is a further 0.176%
# faster than RUN=32 (about 11x drift, so real), yet every multiple of 32
# should give identical intra-warp homogeneity. So 341 extends upward
# through the multiples and re-tests alignment at a larger scale:
#     RUN=80   2.5x -- NON-MULTIPLE CONTROL. If alignment still governs at
#              larger run lengths, this must regress.
#     RUN=96   3x
#     RUN=128  4x
#     RUN=256  8x -- the CHUNKSHAPE148_BUCKET_RUN_MAX ceiling. If 256 is
#              best, raising that ceiling needs a source change in 342.
#   RUN=64 is the anchor, measured first and last so drift stays visible.
#
# THIS SCRIPT is the ordinary single-point validate harness; its default is
# now CHUNKSHAPE148_BUCKET_RUN=64 (340's best), so running it bare
# reproduces that point. The sweep itself is driven by:
#
#     bash 341_bucket_run_sweep2_driver.sh
#
# DRIVER FIX CARRIED IN FROM 340: 340's driver ran all nine passes
# correctly but its aggregation table came out empty, because the harness
# prints "[logdir]   <path>" with three spaces and the driver's sed stripped
# only one, leaving a path with leading whitespace. The 341 driver no longer
# depends on that capture at all -- it builds the table by scanning the log
# directories created during the session -- and it also fixes a PIPESTATUS
# read that was taken after a command substitution rather than a pipeline.
#
# ADOPTION IS DEFERRED TO 342. The source default CHUNKSHAPE148_BUCKET_RUN
# stays 1; 341 is still a pure measurement revision.
#
# MANDATORY VALIDATION ORDER:
#   1. STATIC_ONLY=1 first (checks unchanged from 339/340, including the
#      339 r2 quote-balance guard).
#   2. Full N=21 run; confirm 314666222712 FIRST, before any timing.
#   3. Then the sweep via 341_bucket_run_sweep2_driver.sh.
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./341Py_bucket_run_sweep2.py}
CAND=${CAND:-./341Py_bucket_run_sweep2}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/341Py_bucket_run_sweep2_N21_full.lock}
...
CHUNKSHAPE148_BUCKET_RUN=${CHUNKSHAPE148_BUCKET_RUN:-64}
...
LOGDIR="${LOG_ROOT%/}/341Py_bucket_run_sweep2_logs_N21_full_once_${TS}"
...
echo "[start] 341 bucket-run-sweep2 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 341 bucket-run-sweep2 SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '341 bucket-run-sweep2' "$SRC"; then
  printf 'source_version_tag	341 bucket-run-sweep2	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	341 bucket-run-sweep2	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN")
{
  ...
  echo "validation: one N=21 full run through 341 bucket-run-sweep2 (kernel-identical to 338/337/336/335/334/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN"
...
echo "[validation-ok] 341 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); 341 changes NO source code at all -- identical to 340/339 apart from VERSION_TAG and documentation. This run used CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN (340 measured RUN=64 at 446.390s, -0.843% vs the in-session RUN=1 control of 450.183s). CHECK chunkshape148_cache_state IN THE SUMMARY BEFORE COMPARING TIMING: only reuse rows are comparable. The 341 experiment proper is the sweep -- run bash 341_bucket_run_sweep2_driver.sh"
