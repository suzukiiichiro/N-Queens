...
# =============================================================================
# NQ_SH_UPDATE_HISTORY
# 180: 175相当kernelへ安全に戻したうえで、progress TSVからtail_top_chunks.tsv / tail_bucket_summary.tsvを生成する検証ハーネスを追加。
# 181: 更新メモ: 180の検証ハーネスを継承し、source側の更新履歴ヘッダー存在確認とtail_range_ms / tail_p95_over_p50_permilleを追加。
# =============================================================================

# 181 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 181 keeps the validated 180/175 nibble MAXD14 kernel after the 178/179
# block-switch experiments produced CUDA_ERROR_INVALID_PTX at PTX JIT time.
#
# Kernel arithmetic is unchanged from 175: register-held current frame,
# 13-slot ancestor stack, scalar 4-bit nibble schedule, schedule_words=0,
# and 208 local stack bytes/thread. The new experiment is deliberately safe: add a persistent update-history
# header to the source and extend the validation harness with tail-flatness
# metrics derived from the existing progress TSV. GPU arithmetic and task order
# remain the validated 180/175 path.
SRC=${SRC:-./181Py_kernel_history_tailflat_nibble_maxd14_probe.py}
CAND=${CAND:-./181Py_kernel_history_tailflat_nibble_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/181Py_kernel_history_tailflat_nibble_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 181 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/181Py_kernel_history_tailflat_nibble_maxd14_logs_N21_full_once_${TS}"
...
# ---------------- update-history header checks ----------------
assert 'NQ_PY_UPDATE_HISTORY' in s
assert '181: 更新メモ' in s
for hist_marker in ('168:', '175:', '180:', '181:'):
    assert hist_marker in s, hist_marker
...
assert '181 history-header tailflat GPU candidate supports N=5..27 only' in s
...
# schedule fields, the 175/180 register-held active frame, and the 181 history-tailflat nibble schedule hot path.
...
def nibble_from_byte(op):
    # 181 keeps the byte-opcode low nibble directly:
    #   bits0..2 = block_code, bit3 = future_check.
    nib=op&15
    code=nib&7
    future=(nib>>3)&1
    assert code==0 or future==0,(op,nib,code,future)
    assert code==0 or 1<=code<=5,(op,nib,code)
    return nib
...
def decode_nibble(nib):
    ...
    code=nib&7
    future=(nib>>3)&1
    assert code==0 or future==0,(nib,code,future)
    if code==0:
        return 0,1,0,0,0,future
    step=2+((OP_STEP3>>code)&1)
    add=(OP_ADD1>>code)&1
    bl=((OP_BL1>>code)&1)|(((OP_BL2>>code)&1)<<1)
    kt=((OP_KN3>>code)&1)|(((OP_KN4>>code)&1)<<1)
...
# Every mark fid must map to exactly one valid code, and nibble decode must match byte decode.
block_code_cases=0
nibble_cases=0
for f in range(28):
    if (IS_MARK>>f)&1:
...
for op in range(16):
    ...
    if code==0 or (1<=code<=5 and future==0):
        nib=nibble_from_byte(op)
        assert decode_byte_opcode(op)==decode_nibble(nib),(op,nib,decode_byte_opcode(op),decode_nibble(nib))
        nibble_cases+=1
assert nibble_cases==7,nibble_cases
            ...
            for e in vals:
                for m1 in vals:
                    for m2 in vals:
                        ...
                        if a['action']<2:
                            blocks,step,add,bl,kt,future=decode_nibble(nibble_from_byte(b['opcode']))
...
def build_scalar_schedule(raw,j,e,m1,m2,capacity=14):
    ...
    while True:
        ...
        if action>=2:
            break
        nib=nibble_from_byte(fr['opcode'])
        if depth<8:
            lo |= nib << (depth*4)
        else:
            hi |= nib << ((depth-8)*4)
...
def dfs_register(N,raw,ld0,rd0,col0,a0,j,e,m1,m2,schedule_capacity=14,ancestor_capacity=13,limit=120000):
    ...
    while True:
        ...
        if cur_avail==0:
            ...
            load_count+=1
            continue
        nib=scalar_op_at(sched,sp)
        blocks,step,add,bl,kt,future=decode_nibble(nib)
...
print('MAXD14_NIBBLE_SCHEDULE=1')
print(f'BLOCK_CODE_CASES={block_code_cases}')
print(f'NIBBLE_OPCODE_CASES={nibble_cases}')
...
PYMODEL
then
  printf 'source_nibble_schedule_shape\trequested layout\tmodel/static failure\tFAIL\n' >> "$SUMMARY"
...
printf 'source_parent_baseline\tvalidated fastest 180/175 nibble MAXD14 dispatch\tverified\tOK\n' >> "$SUMMARY"
printf 'source_update_history_header\tpersistent per-file update history + validated nibble kernel\tverified\tOK\n' >> "$SUMMARY"
printf 'source_update_history_181\t181 update memo present in header\tverified\tOK\n' >> "$SUMMARY"
...
printf 'maxd14_nibble_schedule\t1\t%s\tOK\n' "$MAXD14_NIBBLE_SCHEDULE" >> "$SUMMARY"
printf 'block_opcode_mapping\t19 mark fids\t%s cases\tOK\n' "$BLOCK_CODE_CASES" >> "$SUMMARY"
printf 'nibble_opcode_classes\t7 classes\t%s classes\tOK\n' "$NIBBLE_OPCODE_CASES" >> "$SUMMARY"
...
printf 'dfs_164_vs_181_nibble_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 181 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, validated 180/175 nibble kernel + update-history header + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 181 keeps the validated 180/175 kernel structure, then derives chunk-tail diagnostics from
...
# 181: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 181 history-tailflat nibble baseline reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
