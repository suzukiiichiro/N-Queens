...
# =============================================================================
# NQ_UPDATE_MEMO
# 255 r2: schedule-precompute actionpack commandfix。241 restore239本線を親に、244 r4 maskpack成立後、
#      terminal_base14/root_actionだけをsched_action_arrへpackするJIT切り分け版。r2は検証シェルのmode31引数固定。
...
"""
ログ上の実行コマンドが崩れています。今回の255初回は、actionpackそのもののJIT可否としてはまだ判定しない方がよいです。
...
```bash
-g 21 21 32 484 10 7 31 8 7 0 0 1 2
```

つまり、`log_level=1, sort_mode=0, preset=7, bench_mode=31` ではなく、`10, 7, 31, 8...` のように引数がずれており、mode31 split145 を正しく評価できていません。これは検証シェル側の commandfix が必要です。

**255 r2 commandfix** を作成しました。source/kernel探索ロジックは actionpack のまま維持し、検証シェルだけを修正しています。r2では command vector を固定し、summary に `command_arg_vector` check を追加しました。

実行は r2 名でお願いします。

```bash
rm -f ./255Py_schedule_precompute_actionpack_probe
rm -f ./255Py_schedule_precompute_actionpack_probe_r2

STATIC_ONLY=1 bash 255Py_schedule_precompute_actionpack_validate_N21_full_once_r2.sh

bash 255Py_schedule_precompute_actionpack_validate_N21_full_once_r2.sh
```
...
* [255Py_schedule_precompute_actionpack_probe_r2.py](sandbox:/mnt/data/255Py_schedule_precompute_actionpack_probe_r2.py)
...
* [255Py_schedule_precompute_actionpack_validate_N21_full_once_r2.sh](sandbox:/mnt/data/255Py_schedule_precompute_actionpack_validate_N21_full_once_r2.sh)
...
* [255Py_schedule_precompute_actionpack_bundle_20260707_r2.tar](sandbox:/mnt/data/255Py_schedule_precompute_actionpack_bundle_20260707_r2.tar)

sha256です。

```text
8d22c301c2f2b08058f12431efa77ff36354a499c316872dfc8ff1e38400b3df  255Py_schedule_precompute_actionpack_probe.py
8d22c301c2f2b08058f12431efa77ff36354a499c316872dfc8ff1e38400b3df  255Py_schedule_precompute_actionpack_probe_r2.py
62fc98235645638e6522b81ac913be55e06133868567519b4e820a1d643527b7  255Py_schedule_precompute_actionpack_validate_N21_full_once.sh
62fc98235645638e6522b81ac913be55e06133868567519b4e820a1d643527b7  255Py_schedule_precompute_actionpack_validate_N21_full_once_r2.sh
62906ed6ea7567869eb1db61e10a0cf4c1dabd9bef6d65124c0277679df7485b  README.md
```
