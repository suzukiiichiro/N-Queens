...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_bucket_mode:int=0)->None:
  """
  機能:
  ...
  if use_gpu:
    ...
    # constellation bucket 用。expanded task は作らず、19の1 constellation = 1 threadを維持する。
    bsoa:TaskSoA=TaskSoA(STEPS)
    bw_arr:List[u64]=[u64(0)]*STEPS
    bowner_arr:List[int]=[0]*STEPS
    bucket_totals:List[int]=[0]*64
    ...
    if gpu_bucket_mode>0:
      print(f"[gpu-config] N={N} const_bucket_mode={gpu_bucket_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS}")
    ...
    while off < m_all:
      ...
      if gpu_bucket_mode==0:
        ...
        # 29実験: CPU shallow expand はしない。元constellationだけを軽くbucket化する。
        # mode=1: funcptn/avail bucket、mode=2: functionid bucket
        bucket_count:int=12 if gpu_bucket_mode==1 else 28
        for b in range(bucket_count):
          bm:int=0
          for i in range(m):
            f:int=soa.funcid_arr[i]
            ptn:int=func_meta[f][1]
            af:int=func_meta[f][2]
            key:int=(ptn<<1)|af if gpu_bucket_mode==1 else f
            if key==b:
              bsoa.ld_arr[bm]=soa.ld_arr[i]
              bsoa.rd_arr[bm]=soa.rd_arr[i]
              bsoa.col_arr[bm]=soa.col_arr[i]
              bsoa.row_arr[bm]=soa.row_arr[i]
              bsoa.free_arr[bm]=soa.free_arr[i]
              bsoa.jmark_arr[bm]=soa.jmark_arr[i]
              bsoa.end_arr[bm]=soa.end_arr[i]
              bsoa.mark1_arr[bm]=soa.mark1_arr[i]
              bsoa.mark2_arr[bm]=soa.mark2_arr[i]
              bsoa.funcid_arr[bm]=soa.funcid_arr[i]
              bw_arr[bm]=w_arr[i]
              bowner_arr[bm]=i
              bm+=1
          if bm>0:
            bucket_totals[b]+=bm
            GRID = (bm + BLOCK - 1) // BLOCK
            kernel_dfs_iter_gpu(
              gpu.raw(bsoa.ld_arr), gpu.raw(bsoa.rd_arr), gpu.raw(bsoa.col_arr),
              gpu.raw(bsoa.row_arr), gpu.raw(bsoa.free_arr),
              gpu.raw(bsoa.jmark_arr), gpu.raw(bsoa.end_arr),
              gpu.raw(bsoa.mark1_arr), gpu.raw(bsoa.mark2_arr),
              gpu.raw(bsoa.funcid_arr), gpu.raw(bw_arr),
              gpu.raw(meta_next),
              gpu.raw(results),
              bm, board_mask,
              n3, n4,
            ...
            @par
            for j in range(bm):
              results_all[off + bowner_arr[j]] = results[j]
      off += m

    if gpu_bucket_mode>0:
      for b in range(len(bucket_totals)):
        if bucket_totals[b]>0:
          if gpu_bucket_mode==1:
            print(f"[gpu-summary-bucket] bucket={b:2d} total_constellations={bucket_totals[b]}")
          else:
            print(f"[gpu-summary-bucket] fid={b:2d} total_constellations={bucket_totals[b]}")
  ##########
  # CPU 
  ##########
  else:
...
def main()->None:
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_bucket_mode]")
      return
    ...
    if argc >= 5:
      gpu_bucket_mode=int(sys.argv[4])
    if argc > 5:
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        exec_solutions(N,constellations,use_gpu,gpu_bucket_mode)
