...
# =============================================================================
# 244 r3 N=21 full validation harness
# Parent: 241 restore239-after-fid14split-reject.
# Experiment: keep single launch and keep 243 schedule precompute, but pack
#             MAXD14 precomputed fields from 7 u32 arrays into 4 low-bit u32 arrays.
#             r1 3-array high-bit pack and r2 u32 terminal-depth decode triggered CUDA_ERROR_INVALID_PTX;
#             r3 decodes terminal_depth as int before comparisons.
...
SRC=${SRC:-./244Py_schedule_precompute_pack_probe_r3.py}
CAND=${CAND:-./244Py_schedule_precompute_pack_probe_r3}
AUTO_BUILD=${AUTO_BUILD:-1}
# 244: visible startup + force release rebuild by default so a previous non-release
# `codon build` executable is not reused and cannot trigger CUDA_ERROR_INVALID_PTX.
# 244: print start/status early and prevent static pycheck from failing silently under set -e.
FORCE_REBUILD=${FORCE_REBUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/244Py_schedule_precompute_pack_N21_full.lock}
...
BASELINE_243_REJECT_SECONDS=${BASELINE_243_REJECT_SECONDS:-430.524}
...
LOGDIR="${LOG_ROOT%/}/244Py_schedule_precompute_pack_logs_N21_full_once_${TS}"
...
echo "[start] 244 r3 schedule-precompute-pack4-intdecode validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 244 r3 schedule-precompute-pack4-intdecode SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '244 r3 schedule-precompute-lowpack-intdecode' "$SRC"; then
  printf 'source_version_tag\t244 r3 schedule-precompute-lowpack-intdecode\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t244 r3 schedule-precompute-lowpack-intdecode\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split244_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split244 active; split240/241/242/243 runtime tags absent', 'split244', True))
else:
    checks.append(('source_split_tag', 'split244 active; split240/241/242/243 runtime tags absent', 'old split runtime tag present or split244 missing', False))
...
if not m14:
    checks.append(('source_schedule_precompute_lowpack', 'MAXD14 packed precompute active', 'MAXD14 kernel not found', False))
else:
    body = m14.group(0)
    ok = (
        ...
        'sched_ctrl_arr:Ptr[u32]' in body and
        'schedule_lo:u32=sched_lo_arr[i]' in body and
        'schedule_hi:u32=sched_hi_arr[i]' in body and
        'mask_pack:u32=sched_mask_arr[i]' in body and
        'ctrl_pack:u32=sched_ctrl_arr[i]' in body and
        'root_action:u32=(ctrl_pack>>u32(5))&u32(3)' in body and
        'terminal_depth:int=int(ctrl_pack&u32(15))' in body and
        'schedule_raw:u32=ctrl0_arr[i]' not in body and
        'schedule_fu:u32=' not in body and
        'root_action_arr' not in body and
        'terminal_depth_arr' not in body and
        'future_check_mask_arr' not in body and
        'sched_hi_term_arr' not in body and
        'hi_term:u32=' not in body
    )
    actual = 'lowpack intdecode precompute active' if ok else 'old interpreter/highpack/unpacked/u32-depth arrays present'
    checks.append(('source_schedule_precompute_intdecode', 'MAXD14 lowpack intdecode precompute active', actual, ok))
...
host_ok = (
    ...
    'self.sched_ctrl_arr:List[u32]' in s and
    'self.sched_ctrl_arr:List[u32]' in s and
    'gpu.raw(soa.sched_lo_arr)' in s and
    'gpu.raw(soa.sched_hi_arr)' in s and
    'gpu.raw(soa.sched_mask_arr)' in s and
    'gpu.raw(soa.sched_ctrl_arr)' in s and
    'gpu.raw(soa.sched_ctrl_arr)' in s and
    'self.root_action_arr:List[u32]' not in s and
    'self.terminal_depth_arr:List[u32]' not in s and
    'self.sched_hi_term_arr:List[u32]' not in s
)
checks.append(('source_host_precompute_intdecode_arrays', '4 lowpack arrays present', 'present' if host_ok else 'missing/highpack/unpacked arrays present', host_ok))
fail = 0
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 244 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 244 r3 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 244 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 244 r3 schedule-precompute-pack4-intdecode mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "241parent:$BASELINE_241_SECONDS" "243rejected:$BASELINE_243_REJECT_SECONDS" "239:$BASELINE_239_SECONDS" "242rejected:$BASELINE_242_REJECT_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 244 r3 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
