...
# 147 locked validation harness
# - Candidate: 147Py_scorestripe_chunkshape_probe
# - Base: validated 146 compile-control, MAXD=21, five u32 DFS stacks, unchanged task order.
# - Sole kernel experiment: replace the mark selector's two sel branches with one markv comparison.
# - Independent repeat using the fixed 146 partial totals; no values are learned from the first 147 run.
# - All five subtotals and the full N=21 total must reproduce exactly.

BASE=${BASE:-./115Py_range_default_clean_cg_v2}
CAND=${CAND:-./147Py_scorestripe_chunkshape_probe}
RUN_FULL=${RUN_FULL:-1}
...
LOGDIR="147Py_logs_N21_locked_validation_${TS}"
...
echo "[CAND] ${CAND}"
echo "[RUN_FULL] ${RUN_FULL}"
echo "[146-fixed-values] 01=${EXP01} 02=${EXP02} 03=${EXP03} 04=${EXP04} 05=${EXP05} full=${FULL_TOTAL}"

run_case \
  "01_cand147_mode30_standard_7chunk" \
  "partial_total=${EXP01}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "0,20,40,60,80,100,120"

run_case \
  "02_cand147_mode30_heavy_band" \
  "partial_total=${EXP02}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "35,40,41,42,47,48,52,53"

run_case \
  "03_cand147_mode30_d2base14_density" \
  "partial_total=${EXP03}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "20,40,55,56,57,58,60"

run_case \
  "04_cand147_mode30_late_tail" \
  "partial_total=${EXP04}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "100,105,110,115,120,125,130"

run_case \
  "05_cand147_mode31_worker0of4_quick" \
...
if [ "${RUN_FULL}" = "1" ]; then
  run_case \
    "06_cand147_mode31_full_correctness" \
    "${FULL_TOTAL}.*ok" \
    "$CAND" \
