...
def kernel_dfs_iter_gpu(
    ...
    # u32 ビットボード系。
    # 31版: step/add1/nextf/use_blocks/future/inited を ctrl[int] へパックし、
    # GPU thread あたりのローカルスタック圧を下げる。
    # ctrl < 0  : 未初期化フレーム
    ...
    while sp>=0:
      ...
      if ctrl[sp]<0:
        ...
        if use_future==1 and (row[sp]+stepv)<endm:
          fcv=1
        ctrl[sp]=nextfv | (stepv<<5) | (addv<<7) | (use_blocks<<8) | (fcv<<9)
      #----------------
      # 1bit 展開
      #----------------
      a=avail[sp]
