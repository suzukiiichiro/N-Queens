...
# 172 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 172 inherits the
# validated 171 current-frame-register MAXD14 static dispatch.  Its single
# kernel experiment keeps the 170 scalar schedule and the 171 active-frame
# register cache, but shrinks MAXD14 local arrays from u32[14] to u32[13]
# because only ancestor frames are spilled.
#
# For N=21 all launches must remain required_maxd=14, select MAXD14, report
# schedule_words=0 and 208 local stack bytes/thread.

SRC=${SRC:-./172Py_kernel_ancestor13_stack_maxd14_probe.py}
CAND=${CAND:-./172Py_kernel_ancestor13_stack_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/172Py_kernel_ancestor13_stack_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 172 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/172Py_kernel_ancestor13_stack_maxd14_logs_N21_full_once_${TS}"
...
assert '172 ancestor13 stack GPU candidate supports N=5..27 only' in s
...
# schedule fields, and the 172 register-held active frame.
...
print('MAXD14_ANCESTOR13_STACK=1')
...
PYMODEL
then
  printf 'source_ancestor13_stack_shape\trequested layout\tmodel/static failure\tFAIL\n' >> "$SUMMARY"
...
printf 'source_parent_baseline\tvalidated 171 current-frame-register MAXD14 dispatch\tverified\tOK\n' >> "$SUMMARY"
printf 'source_ancestor13_stack_shape\tMAXD14 scalar schedule + register-held current frame + 13-slot ancestor stack\tverified\tOK\n' >> "$SUMMARY"
...
printf 'maxd14_ancestor13_stack_schedule\t1\t%s\tOK\n' "$MAXD14_ANCESTOR13_STACK" >> "$SUMMARY"
...
printf 'dfs_164_vs_172_ancestor13_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 172 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, current-frame register cache + ancestor13 stack, ancestor stack 13"
  echo "================================================================"
} | tee "$RUN_LOG"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "171:$BASELINE_171_SECONDS" "170:$BASELINE_170_SECONDS" "169:$BASELINE_169_SECONDS" "168:$BASELINE_168_SECONDS" "167:$BASELINE_167_SECONDS" "166:$BASELINE_166_SECONDS" "165:$BASELINE_165_SECONDS" "164:$BASELINE_164_SECONDS" "162:$BASELINE_162_SECONDS"; do
...
echo "[validation-ok] 172 current-frame register cache + ancestor13 stack reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
