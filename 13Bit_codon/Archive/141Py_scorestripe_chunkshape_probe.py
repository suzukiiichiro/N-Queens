...
確認しました。**136Py / 136Py locked はどちらも問題ありません。**

```text
136Py1-6.sh:
  01 OK  partial_total=16867221674  elapsed=0:01:10.635
  02 OK  partial_total=19226531218  elapsed=0:01:16.672
  03 OK  partial_total=16935383998  elapsed=0:01:07.613
  04 OK  partial_total=16231974232  elapsed=0:01:06.362
  05 OK  partial_total=79383407310  elapsed=0:05:23.586
  06 OK  total=314666222712        elapsed=0:21:03.345

136Py1-6_locked.sh:
  01 OK  partial_total=16867221674  elapsed=0:01:09.024
  02 OK  partial_total=19226531218  elapsed=0:01:16.564
  03 OK  partial_total=16935383998  elapsed=0:01:07.625
  04 OK  partial_total=16231974232  elapsed=0:01:06.355
  05 OK  partial_total=79383407310  elapsed=0:05:23.549
  06 OK  total=314666222712        elapsed=0:21:03.332
```

安全条件も維持されています。

```text
source_records=2025282
base_reordered_records=2025282
shaped_records=2025282
chunks=131
executed_chunks=131
chunkshape=scorestripe_v4_lanephase32
split_mode=0
specialized=0
generic-only path retained
```

作成しました。

[137Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/137Py_scorestripe_chunkshape_probe.py)
[137Py1-6.sh](sandbox:/mnt/data/137Py1-6.sh)
[137Py1-6_locked.sh](sandbox:/mnt/data/137Py1-6_locked.sh)
[143_136_validation_locked_review_memo.txt](sandbox:/mnt/data/143_136_validation_locked_review_memo.txt)

137 は予定どおり **136 からの最小差分実験**です。

```text
137Py:
  scorestripe_v5_lanephase32_phasejump

継承:
  kernel_dfs_iter_gpu 変更なし
  DFS / solution arithmetic 変更なし
  generic-only mode30 / mode31 維持
  score key 継承
  percentile 8 bucket 継承
  32-lane extraction 継承

変更:
  bucket 内 32-lane の start phase schedule のみ変更
  136:
    start_lane = out_ch*7 + b*5 + written_by_bucket*3
  137:
    phase_seed = out_ch*11 + (out_ch//4)*17 + b*13
    start_lane = phase_seed + written_by_bucket*5
```

一点だけ重要です。137 は host-side permutation を変えるため、**初回実行前には fixed partial_total がまだ分かりません**。そのため `137Py1-6_locked.sh` は、最新の `137Py_logs_N21_validation_*` から 01〜05 の `partial_total` を自動抽出して、それを固定値として locked validation を行う形にしています。

実行順は以下です。

```bash
codon build -release 137Py_scorestripe_chunkshape_probe.py
./137Py1-6.sh
./137Py1-6_locked.sh
```

特定の初回ログを locked seed にしたい場合は、以下のように指定できます。

```bash
LOCK_SOURCE_DIR=137Py_logs_N21_validation_YYYYMMDD_HHMMSS ./137Py1-6_locked.sh
```

`137Py1-6.sh` / `137Py1-6_locked.sh` は実行権限付き、`bash -n` 確認済みです。こちらでは Codon 実機ビルドは未実行です。




138 generation note:
  138Py_scorestripe_chunkshape_probe.py is the minimal successor to the validated 137 line.
  It keeps kernel_dfs_iter_gpu, DFS/solution arithmetic, score key, percentile buckets,
  32-lane extraction, quota logic, and mode30/mode31 generic-only execution unchanged.
  The only shaping change is an even-output-chunk +1 lane-phase bias:
    137 phase_seed = out_ch*11 + (out_ch//4)*17 + b*13
    138 phase_seed = out_ch*11 + (out_ch//4)*17 + b*13 + (1 if out_ch is even else 0)
  This targets the strong even/odd chunk timing stripe observed in the validated 137 full log.

# 138 build / validation
codon build -release 138Py_scorestripe_chunkshape_probe.py
./138Py1-6.sh
./138Py1-6_locked.sh


139 generation note:
  139Py_scorestripe_chunkshape_probe.py is the minimal successor to the validated 138 line.
  It keeps kernel_dfs_iter_gpu, DFS/solution arithmetic, score key, percentile buckets,
  32-lane extraction, quota logic, within-bucket stride, and mode30/mode31 generic-only
  execution unchanged.
  The only shaping change is the even-output-chunk phase bias magnitude:
    138 pair phase difference before bias = +11 lanes
    138 phase_seed = out_ch*11 + (out_ch//4)*17 + b*13 + (1 if out_ch is even else 0)
    139 phase_seed = out_ch*11 + (out_ch//4)*17 + b*13 + (11 if out_ch is even else 0)
  For every pair (2k, 2k+1), out_ch//4 is equal and the unmodified phase difference is 11.
  Therefore +11 phase-locks the even chunk seed to its following odd chunk seed, while
  preserving the validated record-permutation safety boundary.

# 139 build / validation
codon build -release 139Py_scorestripe_chunkshape_probe.py
./139Py1-6.sh
./139Py1-6_locked.sh

140 generation note:
  139 validation and locked validation both passed.  The +11 pair lock reduced the
  full-run even/odd mean gap, but the two-run full average was slower than the
  validated 138 +1 line.  Pair-position comparison against 138 was reproducible:
    pair (4k,   4k+1): 139 averaged about 61.8 ms faster per pair
    pair (4k+2, 4k+3): 139 averaged about 178.2 ms slower per pair
  140 therefore keeps all validated scorestripe machinery and changes only the
  even-output-chunk phase schedule:
    out_ch % 4 == 0: +11 bias (139 phase lock for the first pair)
    out_ch % 4 == 2:  +1 bias (138 schedule for the second pair)
    odd output chunks: 0 bias
  Kernel, DFS/solution arithmetic, score key, percentile buckets, quotas,
  32-lane extraction, within-bucket stride, and generic-only mode30/mode31 remain unchanged.

# 140 build / validation
codon build -release 140Py_scorestripe_chunkshape_probe.py
./140Py1-6.sh
./140Py1-6_locked.sh


141 generation note:
  140 validation and locked validation both passed, including fixed selected-chunk
  partial_total checks and the full N=21 total 314666222712.  Its two-run full average
  was 20:59.851: about 0.525 s slower than the validated 138 +1 baseline, but about
  3.147 s faster than the 139 all-pair +11 line.

  The 140 full logs exposed a reproducible quartet-parity stripe in the first pair of
  each eight-chunk group.  Pair (8k,8k+1) averaged about 397 ms slower than pair
  (8k+4,8k+5), while the corresponding second-pair gap was much smaller.

  141 keeps the validated 140 +11/+1 pair selector and adds only an octet-level
  first-pair phase lock.  For the same position inside adjacent quartets, the inherited seed
  advances by:
    4*11 + 17 = 61 = 29 (mod 32)
  Therefore an added +29 on the first pair of each complete even quartet phase-locks
  that pair to the first pair of its following odd quartet.  The second pair and the
  final incomplete, unpaired quartet remain on the 140 schedule.

  Unchanged:
    kernel_dfs_iter_gpu, DFS/solution arithmetic, score key, percentile buckets,
    quotas, 32-lane extraction, within-bucket stride, and generic-only mode30/mode31.

# 141 build / validation
codon build -release 141Py_scorestripe_chunkshape_probe.py
./141Py1-6.sh
./141Py1-6_locked.sh



はい。**現在は「流し込み方の改善」の終盤**にいます。

140までで、DFS・解数計算・`kernel_dfs_iter_gpu` は変えず、16バイトのコンステレーションレコードを欠落・重複なく並べ替える層だけを調整しています。141も同じ境界内で、残った8チャンク周期を狙う微調整です。

全体工程を分けると、現在地は次の位置です。

1. コンステレーション生成・正解性確立：完了
2. 汎用GPU DFS kernelの安定化：完了
3. stream化、SoA化、chunkサイズ、funcid・mark距離による大分類：完了
4. score bucket、32 lane、phaseによる静的流し込み最適化：**現在、ほぼ終盤**
5. kernel内部またはkernel実行モデルの最適化：次段階
6. N=22以上への一般化、最終固定：最後

特に138以降は、score、bucket、quota、32-lane構造を変えず、位相だけを数チャンク単位で調整しています。139では偶数・奇数チャンク差を約12%縮めたにもかかわらず、全体時間は138より平均0.292%遅くなりました。これは、流し込みの一部を改善すると別の場所が悪化する、**静的並べ替えの局所最適点に近づいている典型的な状態**です。

## 流し込み改善に終わりはあるか

理論上は、並べ方の組合せは非常に多いため絶対的な終点はありません。しかし、**実用上の終点はあります**。
...
一方で、各コンステレーションが持つDFS木そのもの、探索ノード数、DFSループ1回当たりの命令数は減りません。したがって、並べ替えが十分進むと、その先は「同じ総仕事量を別の順番で実行する」だけになります。

今回の141は、その終了判定にかなり近い実験です。140で残った大きな偏りではなく、8チャンク単位の先頭ペアという、より細かな残差を修正しています。

実務上は、次の状態になったら流し込み段階を終了してよいです。

* 連続した候補が、最速基準に対して改善・悪化を繰り返す
* 改善幅がvalidationとlockedの実行差と同程度になる
* チャンク差は縮むが、全体時間が縮まらない
* 位相を変えると、遅い場所が別のチャンクへ移動するだけになる
* N=21での改善がN=22の代表chunkで再現しない

141が138または140を明確に上回らず、差が1秒前後に収まるなら、**現在方式の静的流し込みは一度終了**と判断してよいと思います。

## 次は `@gpu.kernel` に手を加えるのか

**はい。次の大きな改善段階は、`@gpu.kernel` 本体、またはkernelとタスクの境界に手を加える段階です。**

ただし、最初からDFSの判定ロジックを書き換えるべきではありません。次の順序が安全です。

### 最初の対象は、1スレッド当たりのDFSスタック
...
という深さ32の配列を持っています。ソース上でも、`col/avail/ctrl` を `u32` 化してローカルメモリ圧を下げる工夫は既に入っています。

それでも、コンパイラがこれらを十分レジスタに置けずローカルメモリへ退避している場合、ここが大きな制約になります。次段階の第一候補は、

* 実際の最大到達深さに合わせたスタック縮小
* スタック要素のさらなるpacking
* `ld/rd` を必要な区間だけ保存する方式
* `ctrl` の再計算と保存量のトレードオフ
* N=21/N=22専用定数化

です。

これはDFSの意味を変えずにkernel内部だけを改善できます。

### 次に、粗いkernel分離

現在の140系は、追加のspecialized kernelを意図的に定義せず、全タスクを115由来の汎用kernelへ送っています。これは複数kernel起動のオーバーヘッドやPTX/JITリスクを避けるためです。

したがって、次に分離するとしても28個のfuncidごとではなく、
...
のような2〜3本の粗い分離が妥当です。小さすぎるkernelを多数起動すると、分岐削減より起動コストの方が大きくなる可能性があります。
...
DFS木の大きさには大きな差があるため、どれだけ静的に並べても、最後に長いDFSを担当したスレッドが残る問題は完全には消えません。

その限界を越える方法は、

* 重いコンステレーションだけ事前にもう一段分割する
* 途中状態をcontinuationタスクとして再投入する
* persistent kernelとwork queueを使用する
* blockまたはwarp単位で次の仕事を取得する

といった動的方式です。

これは`@gpu.kernel`本体だけでなく、**「1タスク1スレッド」というkernelの実行モデルを変える段階**です。最も難しい一方、静的並べ替えより大きな伸びが期待できる領域です。

## 141終了後の判断

141が完了したら、次の基準で決めるのがよいです。
...
141が同等または遅い:
  138または140を流し込み最終版として固定
  直ちにkernel系列へ移行
```

つまり、**141は流し込み研究の途中というより、流し込み段階を閉じるための最終確認に近い位置**です。

次に触るべきなのはDFSの数学的条件ではなく、まず `kernel_dfs_iter_gpu` のスタック使用量・レジスタ圧・ローカルメモリ・分岐発散です。その後に、重いタスクの粗い分離、最終的には動的work queueへ進むのが自然です。
...
VERSION_TAG:str="141 scorestripe v9 lanephase32 octetfirstpairlock29 - mode30/mode31 generic kernel, host-side 32-lane score-bucket shaping"
...
####################################################################################################
# 141 reorder chunkshape harness
#
# This build intentionally defines no additional specialized @gpu.kernel.
# It keeps only the proven 115 kernel_dfs_iter_gpu and uses the split/candidate
# host-side harness from 127 for mode30 selected-chunk probes and mode31 worker/full runs, plus 130 chunk-shape bin.
#
# Purpose:
#   - measure the overhead of the split/candidate harness itself
#   - provide a clean baseline before reorder/chunk-shaping candidates
#   - avoid PTX/JIT risk from extra specialized kernels
####################################################################################################
...
def gen_constellations(
  ...
  # which caused misleading logs such as preset_queens=13 or preset_queens=130.
  return ijkl_list, subconst_cache, constellations, preset_queens
...
####################################################################################################
# 141 score-aware chunk-shape layer
# 141 is a minimal octet-level first-pair phase-lock experiment after the validated 140
# scorestripe_v8_lanephase32_quartetpairmix result, with isolated cache/log labels.
#
# Safety boundary:
#   - kernel_dfs_iter_gpu is unchanged
#   - DFS / solution arithmetic is unchanged
#   - generic-only mode30 / mode31 path is retained
#   - score key, percentile thresholds, bucket quotas, 32-lane extraction,
#     within-bucket stride, and the 140 +11/+1 pair selector are unchanged
#   - the layer only permutes 16-byte constellation records on disk
#   - every input record is emitted exactly once
#
# The two validated 140 full logs showed a reproducible quartet-parity stripe:
#   first pair in even quartet q=2p: chunks (8p,   8p+1) were slower
#   first pair in odd  quartet q=2p+1: chunks (8p+4, 8p+5) were faster
# For the same r/b position, adjacent quartet seeds differ by:
#   4*11 + 17 = 61 = 29 (mod 32)
# 141 changes only the quartet phase selector:
#   first pair of even quartet with a following odd quartet -> +29 on both chunks
#   second pair / odd quartet / final unpaired quartet       ->  0
# This phase-locks only pair (8p,8p+1) to pair (8p+4,8p+5), while retaining
# every second-pair phase and the 140 +11/+1 selector unchanged.
####################################################################################################

CHUNKSHAPE141_REORDER_VERSION:str="scorestripe_v9_lanephase32_octetfirstpairlock29"
CHUNKSHAPE141_DEFAULT_REASON:str="scorestripe_v9_lanephase32_octetfirstpairlock29: keep validated 140 +11/+1 pair mix, score buckets, quotas, 32 lanes, and stride; add +29 only to the first pair of complete even quartets so it locks to the following odd-quartet first pair; second pairs and final unpaired quartet unchanged; generic kernel unchanged"


def chunkshape141_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape141_{CHUNKSHAPE141_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}.bin"


CHUNKSHAPE141_SCORE_KEY_MAX:int=32767
CHUNKSHAPE141_LANE_COUNT:int=32
CHUNKSHAPE141_LANE_MASK:int=31


def chunkshape141_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  141 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, and 140 +11/+1 pair selector
  are carried forward unchanged.  The 141 experiment adds only a +29 bias to the first pair
  of each complete even quartet so its two phase seeds match the following odd-quartet pair.

  The key is consumed only by the on-disk permutation builder.  It does not change DFS state,
  symmetry weights, GPU kernel inputs, or solution arithmetic.
  """
  fid:int=soa.funcid_arr[idx]
  pc:int=popcount_int(soa.free_arr[idx])
  ...
  # Coarse static cost estimate from scorestripe_v2.
  raw:int=0
  raw+=pc*12
  raw+=depth*7
  raw+=row_to_end*3

  # Risk hints from the existing broadmark / mark-distance line of work.
  if fid==26 or fid==27:
    raw+=96
  ...
  # Deterministic tie lane.  Percentile buckets use the full key; v4 bucket lanes use key&31.
  tie:int=(global_idx*13 + soa.ijkl_arr[idx]*7 + fid*5 + pc*3 + depth) & 31
  key:int=raw*32 + tie
  if key<0:
    key=0
  if key>CHUNKSHAPE141_SCORE_KEY_MAX:
    key=CHUNKSHAPE141_SCORE_KEY_MAX
  return key


def chunkshape141_build_thresholds_from_hist(hist:List[int],total_records:int)->List[int]:
  """Build 8 percentile thresholds from the score-key histogram."""
  thresholds:List[int]=[0]*8
  if total_records<=0:
    thresholds[7]=CHUNKSHAPE141_SCORE_KEY_MAX
    return thresholds

  b:int=0
  cum:int=0
  k:int=0
  while k<=CHUNKSHAPE141_SCORE_KEY_MAX:
    cum+=hist[k]
    while b<7:
  ...
  while b<7:
    thresholds[b]=CHUNKSHAPE141_SCORE_KEY_MAX
    b+=1
  thresholds[7]=CHUNKSHAPE141_SCORE_KEY_MAX
  return thresholds


def chunkshape141_bucket_from_key(key:int,thresholds:List[int])->int:
  """Map a score key to bucket 0..7 using percentile thresholds."""
  b:int=0
  while b<7:
    if key<=thresholds[b]:
      return b
...
def chunkshape141_lane_from_key(key:int)->int:
  """Map a score key to a deterministic lane 0..31 inside its score bucket."""
  lane:int=key&CHUNKSHAPE141_LANE_MASK
  if lane<0:
    lane=0
  if lane>=CHUNKSHAPE141_LANE_COUNT:
    lane=lane&CHUNKSHAPE141_LANE_MASK
  return lane


def chunkshape141_make_quotas(rem_counts:List[int],total_remaining:int,m_target:int)->List[int]:
  """Make 8 score-bucket quotas for one output chunk."""
  quotas:List[int]=[0]*8
  if total_remaining<=0 or m_target<=0:
    return quotas
  ...
  # Fill integer-division remainder from the buckets with the most room left.
  while quota_sum<m_target:
    best:int=-1
    best_room:int=-1
    b=0
...
def build_chunkshape141_reordered_bin(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  """
  141 host-side score-aware lanephase chunk shaping.

  Minimal octet-level first-pair phase-lock experiment after validated 140: no DFS, kernel,
  score-key, quota, bucket-count, lane-extraction, within-bucket stride, or +11/+1 pair-selector
  change.  Only the first pair of each complete even quartet receives a +29 phase bias.

  Input:
    fname is normally the already-built broadmarktail reorder bin.

  Output:
    A second bin whose records are a permutation of fname.  The builder first assigns each record
    to a percentile score bucket using the validated v2/v4 score key.  It then keeps the 128
    cross-stripe traversal order and the 32 deterministic lanes.  For each complete pair of
    adjacent quartets, +29 makes the first pair of the even quartet's phase seeds equal the first
    pair of the odd quartet.  All second pairs and the final incomplete/unpaired quartet retain
    the 140 schedule.

  Safety:
    - every input record is emitted exactly once
    - source_records == shaped_records is checked via .done and record count
    - kernel_dfs_iter_gpu and all DFS/solution logic are unchanged
  """
  BLOCK:int=gpu_block
  MAX_BLOCKS:int=gpu_max_blocks
  if BLOCK<=0:
  ...
  out_fname:str=chunkshape141_reorder_output_fname(N,preset_queens,BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_chunkshape141_{CHUNKSHAPE141_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)}_sim.tsv"

  if gpu_log_level>=1:
    print(f"[chunkshape141-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE141_DEFAULT_REASON}")

  with open(progress_fname,"w") as pf:
    lane_header:str=""
    lane_i:int=0
    while lane_i<CHUNKSHAPE141_LANE_COUNT:
  ...
  if len(data)<expected_bytes:
    print(f"[chunkshape141-warning] source byte count shorter than expected: bytes={len(data)} expected={expected_bytes}")
  ...
  # ------------------------------------------------------------------
  # Stage 1: classify every source record into a percentile score key.
  # ------------------------------------------------------------------
  score_key_by_idx:List[int]=[0]*total_records
  key_hist:List[int]=[0]*(CHUNKSHAPE141_SCORE_KEY_MAX+1)
  off:int=0
  classify_chunk:int=0
  key_min:int=CHUNKSHAPE141_SCORE_KEY_MAX
  key_max:int=0
  while off<total_records:
    ...
    i=0
    while i<m:
      key:int=chunkshape141_score_key_from_soa(soa,i,off+i)
      if key<0:
        key=0
      if key>CHUNKSHAPE141_SCORE_KEY_MAX:
        key=CHUNKSHAPE141_SCORE_KEY_MAX
    ...
    if gpu_log_level>=2:
      print(f"[chunkshape141-classify-chunk] N={N} chunk={classify_chunk} off={off} m={m}")
    off+=m
    classify_chunk+=1

  score_thresholds:List[int]=chunkshape141_build_thresholds_from_hist(key_hist,total_records)
  bucket_counts:List[int]=[0]*8
  lane_counts:List[int]=[0]*CHUNKSHAPE141_LANE_COUNT
  idx_count:int=0
  while idx_count<total_records:
    b0:int=chunkshape141_bucket_from_key(score_key_by_idx[idx_count],score_thresholds)
    lane0:int=chunkshape141_lane_from_key(score_key_by_idx[idx_count])
    bucket_counts[b0]+=1
    lane_counts[lane0]+=1
    idx_count+=1

  if gpu_log_level>=1:
    print(f"[chunkshape141-score-key] N={N} records={total_records} key_min={key_min} key_max={key_max} t0={score_thresholds[0]} t1={score_thresholds[1]} t2={score_thresholds[2]} t3={score_thresholds[3]} t4={score_thresholds[4]} t5={score_thresholds[5]} t6={score_thresholds[6]}")
    print(f"[chunkshape141-score-buckets] N={N} records={total_records} b0={bucket_counts[0]} b1={bucket_counts[1]} b2={bucket_counts[2]} b3={bucket_counts[3]} b4={bucket_counts[4]} b5={bucket_counts[5]} b6={bucket_counts[6]} b7={bucket_counts[7]}")
    lane_counts_text:str=""
    lane_i:int=0
    while lane_i<CHUNKSHAPE141_LANE_COUNT:
      lane_counts_text+=f" l{lane_i}={lane_counts[lane_i]}"
      lane_i+=1
    print(f"[chunkshape141-lanes] N={N} records={total_records}{lane_counts_text}")

  # ------------------------------------------------------------------
  # Stage 2: keep the proven 128 cross-stripe order, but split each score bucket into 32 lanes.
  # ------------------------------------------------------------------
  bucket_lane_indices:List[List[int]]=[]
  flat:int=0
  while flat<(8*CHUNKSHAPE141_LANE_COUNT):
    one:List[int]=[]
      ...
      while src_ch<n_chunks:
        ...
        if within<STEPS:
          ...
          if idx<total_records:
            bucket:int=chunkshape141_bucket_from_key(score_key_by_idx[idx],score_thresholds)
            lane_val:int=chunkshape141_lane_from_key(score_key_by_idx[idx])
            bucket_lane_indices[bucket*CHUNKSHAPE141_LANE_COUNT+lane_val].append(idx)
            staged_total+=1
        src_ch+=1
      base+=1
    out_ch+=1
  ...
  if staged_total!=total_records:
    print(f"[chunkshape141-warning] staged count mismatch: staged={staged_total} source={total_records}")

  # ------------------------------------------------------------------
  # Stage 3: write output chunks with proportional score-bucket quotas.
  #          Inside each bucket, retain the 140 +11/+1 pair bias and add the 141 complete-even-quartet first-pair +29 lock.
  # ------------------------------------------------------------------
  bucket_lane_pos:List[int]=[0]*(8*CHUNKSHAPE141_LANE_COUNT)
  bucket_lane_rem:List[int]=[0]*(8*CHUNKSHAPE141_LANE_COUNT)
  bucket_rem:List[int]=[0]*8
  b:int=0
  while b<8:
    lane:int=0
    while lane<CHUNKSHAPE141_LANE_COUNT:
      flat=b*CHUNKSHAPE141_LANE_COUNT+lane
  ...
  # Heavy/low interleave.  Rotate by out_ch to avoid identical inter-bucket phase.
  ...
  with open(out_fname,"ab") as out:
    while out_ch<n_chunks:
      ...
      quotas:List[int]=chunkshape141_make_quotas(bucket_rem,total_remaining,m_target)
      written_by_bucket:List[int]=[0]*8
      written_by_lane:List[int]=[0]*CHUNKSHAPE141_LANE_COUNT
      written_chunk:int=0

      # 141 octetfirstpairlock29:
      # For the same position in adjacent quartets, the inherited seed advances by
      # 4*11 + 17 = 61 = 29 (mod 32).  Add +29 to both chunks in the first pair
      # of a complete even quartet so they phase-lock to the following odd-quartet pair.
      # Do not shift second pairs or the final incomplete/unpaired quartet.
      ...
      while written_chunk<m_target:
        ...
        while oi<8:
          ...
          if quotas[b]>0:
            # Retain the validated 140 pair selector exactly:
            #   chunk 4k -> +11, chunk 4k+2 -> +1, odd chunks -> 0.
            pair_phase_bias:int=0
            if (out_ch&1)==0:
              pair_phase_bias=11 if (out_ch&3)==0 else 1
            phase_seed:int=(out_ch*11 + quartet_index*17 + b*13 + pair_phase_bias + octet_first_pair_phase_bias) & CHUNKSHAPE141_LANE_MASK
            start_lane:int=(phase_seed + written_by_bucket[b]*5) & CHUNKSHAPE141_LANE_MASK
            found:int=-1
            scan:int=0
            while scan<CHUNKSHAPE141_LANE_COUNT:
              lane=(start_lane+scan) & CHUNKSHAPE141_LANE_MASK
              flat=b*CHUNKSHAPE141_LANE_COUNT+lane
              ...
              scan+=1
            if found>=0:
              flat=b*CHUNKSHAPE141_LANE_COUNT+found
      ...
      if gpu_log_level>=2:
        ...
        while lane_i<CHUNKSHAPE141_LANE_COUNT:
          ...
          lane_i+=1
        print(f"[chunkshape141-build-chunk] N={N} chunk={out_ch} written={written_chunk} total_written={written_total} b0={written_by_bucket[0]} b1={written_by_bucket[1]} b2={written_by_bucket[2]} b3={written_by_bucket[3]} b4={written_by_bucket[4]} b5={written_by_bucket[5]} b6={written_by_bucket[6]} b7={written_by_bucket[7]}{lane_debug}")
      out_ch+=1
  ...
  if written_total!=total_records:
    print(f"[chunkshape141-warning] record count mismatch: written={written_total} source={total_records}")
  if reordered_records!=written_total:
    print(f"[chunkshape141-warning] output count mismatch: file_records={reordered_records} written={written_total}")
  if gpu_log_level>=1:
    print(f"[chunkshape141-build-summary] N={N} records={reordered_records} chunks={n_chunks} output={out_fname} progress={progress_fname} valid={1 if validate_bin_file(out_fname) else 0}")
...
"""130: reorder/chunkshape generic-only split harness。special kernel は使わず、全 task を 115 汎用 kernel に流す。"""
def exec_solutions_gpu_chunk_split141(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  """
  130 reorder/chunkshape generic-only harness.

  Host-side behavior:
    - keep the 127-style mode30 selected chunk probe
    - keep the 127-style mode31 full/worker stream runner
    - consume the 130 cross-chunk shaped bin
    - keep per-chunk stats/progress logging

  GPU behavior:
    - do not split into specialized buckets
    - do not launch any specialized kernel
    - run every task through the proven 115 kernel_dfs_iter_gpu

  This keeps the generic-only kernel path and measures host-side cross-chunk shaping without adding specialized kernels.
  """
  board_mask:int=(1<<N)-1
  ...
  # Count diagnostic buckets only.  No records are moved and no specialized launch is used.
  d2base14_m:int=0
  d0_m:int=0
  i:int=0
  while i<m:
  ...
  if gpu_log_level>=1:
    print(f"[split141-buckets] N={N} m={m} generic={m} d2base14={d2base14_m} d0={d0_m} rest=0 split_mode={split_mode} specialized=0")
...
def exec_solutions_gpu_bin_stream_split141(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split141_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split141_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split141_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split141_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split141-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split141-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split141-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split141-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split141(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split141-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split141-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split141 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split141 full run / optional multi-GPU worker
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 30 8 7 0 1222 5 "" 2  # 130 shaped generic-only probe chunks 1222..1226
      #   -g 22 22 32 484 1 0 7 31 8 7 0 0 1 2  # 130 shaped generic-only full GPU single worker
      if argc >= 11:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split141-generic-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split141-generic worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split141_generic_harness_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split141_generic_harness_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  if gpu_log_level>=1 and (bench_mode==30 or bench_mode==31):
    print(f"chunkshape141_params: version={CHUNKSHAPE141_REORDER_VERSION} reason={CHUNKSHAPE141_DEFAULT_REASON}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split141-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split141-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      shaped_fname:str=chunkshape141_reorder_output_fname(N,preset_queens,gpu_block,gpu_max_blocks)
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape141-probe-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape141-probe-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape141_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split141_probe_{CHUNKSHAPE141_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split141(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split141-probe-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE141_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split141-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split141-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split141-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape141-full-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape141-full-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape141_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split141_full_{CHUNKSHAPE141_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split141(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split141-full-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE141_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
