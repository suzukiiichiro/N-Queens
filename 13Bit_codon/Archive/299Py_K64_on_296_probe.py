...
**298 = 416.429s — 大幅悪化、即不採用。** vs 296比で −62.758s (−17.745%) 退行、292より遅い。

`next_depth` の排除は完全に逆効果でした。この結果と 297 の結果を合わせて重要な知見が得られました。

**`save_sp`・`next_depth` はともに「削除すると大幅退行する」。** これらはコンパイラの命令スケジューラが依存チェーンを分散させるためのアンカー変数として機能していると考えられます。ホットループの Stall Wait (48%) の正体がここにある可能性があります — **変数が増えることで命令間の距離が広がり、レイテンシが隠れる**。

この発見を踏まえ、方針を転換します。## 298 結果まとめと 299 の考え方

**298 不採用**: 416.429s — vs 296比で **−62.758s (−17.745%) 大幅悪化**。292より遅い。

これで 297/298 の連続大失敗から重要な教訓が確立しました：

> **ホットループの一時変数（`save_sp`・`next_depth`）は命令スケジューリングの"緩衝材"として機能しており、削除すると nf 依存チェーンの後にスロットがなくなって Stall Wait が激増する。変数削除路線は終了。**

---

## 299 の着眼点: K=64 再挑戦

292 では K=32 と K=64 がほぼ同値（差 0.2s）でした。しかし 296 カーネルは **3.74% 速い**（タスクあたりの所要時間が短縮）。これによりスレッドごとの "active window" が変わり、K=64 が有利になった可能性があります。

変更は定数1行のみ：`K_PER_THREAD_MAXD14: 32 → 64`。chunk 数は 5→3 に減ります。

```bash
STATIC_ONLY=1 bash 299Py_K64_on_296_validate_N21_full_once.sh
bash 299Py_K64_on_296_validate_N21_full_once.sh
```
"""
...
K_PER_THREAD_MAXD14:Static[int]=64


VERSION_TAG:str="299 K64-on-296 probe: parent 298 next-depth-elim (rejected, -62.758s regression); reverted to 296 stack-ptr base (353.671s); single change: K_PER_THREAD_MAXD14 32->64 to test whether the 296 faster kernel shifts the K sweet spot; EXPECTED_CHUNKS=ceil(2025282/991232)=3 (vs 296 K=32 CHUNKS=5); all kernel logic, stack_ptr/save_sp, stack array unchanged from 296; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
