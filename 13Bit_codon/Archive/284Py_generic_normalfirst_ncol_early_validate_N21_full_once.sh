...
# =============================================================================
# 284 N=21 full validation harness
# Parent: 283 generic-normalfirst validated at 0:07:07.698.
# Experiment: keep 283 MAXD14 generic normal-path-first branch order and move ncol=cur_col|bit before the block_code branch in the generic DFS loop. Root-preroll and host shaping/cache/dispatch features are unchanged.
...
SRC=${SRC:-./284Py_generic_normalfirst_ncol_early_probe.py}
CAND=${CAND:-./284Py_generic_normalfirst_ncol_early_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/284Py_generic_normalfirst_ncol_early_N21_full.lock}
...
BASELINE_282_RESTORE_SECONDS=${BASELINE_282_RESTORE_SECONDS:-428.066}
...
LOGDIR="${LOG_ROOT%/}/284Py_generic_normalfirst_ncol_early_logs_N21_full_once_${TS}"
...
echo "[start] 284 generic-normalfirst-ncol-early validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 284 generic-normalfirst-ncol-early SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '284 generic-normalfirst-ncol-early' "$SRC"; then
  printf 'source_version_tag\t284 generic-normalfirst-ncol-early\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t284 generic-normalfirst-ncol-early\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
old_split_markers = ['split283', 'split282', 'split281', 'split280', 'split278', 'split277', 'split275', 'split273', 'split270_', 'split240', 'split239_', 'kernel-blockdiag', 'block_count_diag', 'rootaction0-direct']
old_present = [x for x in old_split_markers if x in s]
if 'split284' in s and not old_present:
    checks.append(('source_split_tag', 'split284 active; rejected runtime tags absent', 'split284', True))
else:
    checks.append(('source_split_tag', 'split284 active; rejected runtime tags absent', 'present=' + ','.join(old_present) if old_present else 'split284 missing', False))
...
# Check MAXD14 generic DFS loop normal-first + ncol early branch. Keep root-preroll branch untouched.
...
if not m14:
    checks.append(('source_generic_normalfirst_ncolearly', 'MAXD14 generic normal-first with ncol early', 'MAXD14 not found', False))
else:
    body = m14.group(0)
    idx = body.find('block_code:u32=nibble_op&u32(7)')
    tail = body[idx:] if idx >= 0 else ''
    expected_snippet = '''ncol:u32=cur_col|bit
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if block_code==u32(0):
    ...
    old_late = '''nld:u32=u32(0)
      nrd:u32=u32(0)
      if block_code==u32(0):'''
    old_late_present = False
    if old_late in tail:
        if 'ncol:u32=cur_col|bit' not in tail or tail.find(old_late) < tail.find('ncol:u32=cur_col|bit'):
            old_late_present = True
    checks.append(('source_generic_normalfirst_ncolearly', 'MAXD14 generic normal path first and ncol before branch', 'present' if ok and not old_late_present else 'missing-or-late', ok and not old_late_present))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 284 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 284 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 284 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 284 generic-normalfirst-ncol-early mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "283normalfirst:$BASELINE_283_NORMALFIRST_SECONDS" "282restore:$BASELINE_282_RESTORE_SECONDS" "281diag:574.374" "279restore:427.728" "276current:427.672" "271fastest:427.705" "227normalfirst:427.808" "239:427.703" "217:427.709"; do
...
echo "[validation-ok] 284 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
