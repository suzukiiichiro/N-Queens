...
# =============================================================================
# NQ_UPDATE_MEMO
# 221: 更新メモ: 220Py rootrestlate_roottail は正当性OKだが217最速を上回らなかったため不採用。221Pyは217Py rootrestlate新最速基準を親に、MAXD14 kernelのみroot<=2判定を root_after_second scalar から inline XOR predicate へ変える再結合実験。rootrestlate / root_first late extraction / futuremask / no-sibling / root one-two preroll は維持する。
...
220Pyは正当性OKですが、217Pyより `0.250秒` 遅いため**不採用**です。予定通り、221Pyは **217 + inline xor predicate** にしました。
...
```text
bash -n 221Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 221Py...sh OK
source_rootrestlate_inlinexor         OK
source_split_tag split145 split221    OK
```

ファイルはこちらです。

* [221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_probe.py)
* [221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_validate_full_once.sh

bash 221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 221 rootrestlate-inlinexor memo over validated 211 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="221 roottwo-rootrestlate-inlinexor: 217 rootrestlate fastest baseline with inline XOR root<=2 predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # correct but slower.  221 keeps 217's delayed cur_avail=root_rest assignment
    # and late root_first extraction, but removes the root_after_second temporary
    # scalar and tests the XOR predicate directly.  If the first child dies or is
    # terminal, the generic loop still sees root_rest as the active root remainder.
    # If the first child descends, the same root_rest is saved as a sibling.
...
CHUNKSHAPE148_DEFAULT_REASON:str="221 rootrestlate-inlinexor reuses validated 217 scorestripe_v9 task order/cache; MAXD14 inline XOR predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split221_roottwo_rootrestlate_inlinexor_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split221_roottwo_rootrestlate_inlinexor_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split221-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split221-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split221_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split221-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split221-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split221_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
