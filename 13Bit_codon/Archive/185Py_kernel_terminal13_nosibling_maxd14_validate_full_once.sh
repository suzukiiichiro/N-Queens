...
# =============================================================================
# NQ_UPDATE_MEMO
# 185: 更新メモ: 184のN=21 full検証OK・0:07:15.635を受け、runtime-only検証を継承。MAXD14 kernelは184のno-sibling spill elisionを維持し、N=21向け terminal_depth==13 fast path を単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 185 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 185 starts from validated 184 and changes only the MAXD14 terminal-depth test after the 178/179
...
# schedule_words=0, and 208 local stack bytes/thread. The experiment is deliberately focused: for schedules whose terminal parent depth is 13, compare
# cur_depth against the literal 13 in the hot loop. Host task order, dispatch,
# cache, progress verification, and README-managed history remain unchanged.
SRC=${SRC:-./185Py_kernel_terminal13_nosibling_maxd14_probe.py}
CAND=${CAND:-./185Py_kernel_terminal13_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/185Py_kernel_terminal13_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 185 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/185Py_kernel_terminal13_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 185Py source
#   has a harmless header/string/layout difference.  This 185 harness validates the
...
printf 'source_parent_baseline\tvalidated 184 no-sibling MAXD14 dispatch + terminal-depth-13 experiment\truntime-only validation\tINFO\n' >> "$SUMMARY"
printf 'kernel_shape_expectation\tMAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + terminal13 fast path\truntime dispatch/progress checked\tINFO\n' >> "$SUMMARY"
printf 'schedule_words_MAXD14\t0\truntime dispatch checked\tINFO\n' >> "$SUMMARY"
printf 'stack_bytes_MAXD14\t208\truntime dispatch checked\tINFO\n' >> "$SUMMARY"

static_failures=0
if grep -q 'if terminal_parent_depth==13:' "$SRC"; then
  printf 'source_terminal13_fastpath\tpresent\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_terminal13_fastpath\tpresent\tmissing\tFAIL\n' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
if grep -q 'if cur_depth==13:' "$SRC"; then
  printf 'source_terminal_literal_compare\tpresent\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_terminal_literal_compare\tpresent\tmissing\tFAIL\n' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
if grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag\tsplit185 only\tsplit185 only\tOK\n' >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit185 only\tcheck source\tFAIL\n' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 185 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source terminal13 static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 185 terminal13 no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 185 keeps the validated host/chunk structure, then derives chunk-tail diagnostics from
...
# 185: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 185 terminal13 nosibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
