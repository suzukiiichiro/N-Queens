...
# =============================================================================
# 353 N=21 work-census harness (MEASUREMENT ONLY -- NOT A CANDIDATE)
#
# WHAT 353 IS. One per task counter inside kernel_dfs_iter_gpu_maxd14, and a
# dump of one u64 per task, in shaped-bin record order, for every one of the
# 2025282 tasks of N=21. Three lines in one kernel:
#     trips:u64=u64(0)          declared beside total, once per task
#     trips+=u64(1)             first statement of the DFS while loop
#     census_arr[idx]=trips     stored just before the grid-stride advance
# maxd16/18/20/21 are UNTOUCHED. That is sufficient because dispatch_non_MAXD14
# is 0 at N=21; a chunk that selected another kernel writes NO file and prints
# [census-error] rather than emitting a file full of zeros.
#
# WHY THIS AND NOT ITEM 11. The 352 handover itself records that wait and
# branch_resolving are a question about the machine and not a lever, that
# SourceCounters cannot finish on N=21 (5-pass replay, 146s per launch), and
# that N=18 stall ratios do not represent production (K=1 vs K=48). Item 11 is
# expensive, low resolution and inside the blast radius of item 12.
#
# WHY THE CENSUS CANNOT BE A LOTTERY DRAW. A loop trip count is independent of
# scheduling. Each thread runs a private DFS over a private stack with no shared
# pruning and no early termination, so instruction cache alignment, register
# bank conflicts, and any occupancy change caused by the extra u64 leave the
# recorded numbers bit for bit unchanged. This build being slower than 352 does
# not damage the measurement in any way. That is the whole point of doing this
# before spending weeks on a port.
#
# WHAT IT BOUNDS. BLOCK=32, so one block is exactly one warp, and all 484 blocks
# are simultaneously resident at 33.33% occupancy (80 SMs x 16 warp slots =
# 1280 >> 484). Launch time is therefore set by the heaviest warp.
#   cross-warp imbalance      -> ceiling for host-side reordering / work stealing
#   within-warp lane tail     -> ceiling for warp compaction, i.e. the item 6 port
#   rank correlation          -> grades the CURRENT sort key, no extra instrumentation
#                                (the shaped bin is already in sort order, so task
#                                 position IS the rank under the current key)
#   LPT lower bound           -> the makespan floor of ANY static reordering
#
# WHAT IT DOES NOT MEASURE. Control-flow divergence, the 2-3 threads per warp
# figure, is a property of branch paths and not of work length. It stays an ncu
# quantity. 353 pins down the share of the loss that is workload imbalance; what
# is left over is divergence.
#
# BY-PRODUCT, possibly larger than the result. census + the cached shaped bin
# turns evaluation of a new sort key from a 398s run into an offline makespan
# computation. That is a four-order-of-magnitude shorter loop on the most
# productive axis this project has (K-batching -4.4%, bucket_run -4.0%,
# sort scope -6.8%).
#
# WALL CLOCK FROM THIS BUILD IS NOT A TIMING ROW. It is INFO only. This binary
# is not a candidate and its elapsed must never be quoted as a result.
# 354 RESTARTS FROM THE 352 KERNEL. Carrying the counter forward would re-draw
# the item 12 lottery.
#
# PRE-REGISTERED DECISION RULE (fixed here, before the data is seen):
#   lane tail  >= 6x   -> the CUDA C port is justified as the main axis
#   lane tail  <  2x   -> warp intrinsics alone do not justify the port
#   LPT bound  <= -3%  -> host-side reordering still has live ammunition; that is
#                         the cheapest next move and it stays inside Codon
#   rank corr  >= 0.85 -> the key-redesign axis is closed
#
# RUN ORDER. The 352 lesson was: put a few-second check in front of every long
# command. Here that is three steps, in this order.
#
#     STATIC_ONLY=1 bash 353Py_work_census_validate_N21_full_once.sh
#     CENSUS_SMOKE=1 bash 353Py_work_census_validate_N21_full_once.sh   # ~146s, chunk0 only
#     bash 353Py_work_census_validate_N21_full_once.sh                  # full N=21
#
# The smoke uses bench_mode=30 (split145 probe, chunk 0 only) and proves the
# census plumbing end to end for the price of one chunk. Do not skip it: a full
# run that produces a file of zeros costs 400s and tells you nothing.
#
# Cache is reused (..._isort9.bin); there is no build run in this revision.
# EXPECTED_CHUNKS=3, K=48, stride=15488 (unchanged).
# =============================================================================

SRC=${SRC:-./353Py_work_census.py}
CAND=${CAND:-./353Py_work_census}
...
# 353: CENSUS_SMOKE=1 runs bench_mode 30 (split145 probe) over chunk 0 only,
# about 146s, and validates the census plumbing end to end before the full run
# is committed to. The 352 lesson was that a long command must be preceded by a
# short check; a full run that emits a file of zeros costs 400s and teaches
# nothing. The smoke exits before the N=21 full run.
CENSUS_SMOKE=${CENSUS_SMOKE:-0}
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/353Py_work_census_N21_full.lock}
...
# 353 is NOT a candidate, so none of these are a verdict on it. They are kept
# only so that the log records how far the instrumented build sits from the
# production lineage; the counter costs what it costs and that number is not a
# regression. 352 is listed first because it is the code 354 restarts from.
BASELINE_352_SECONDS=${BASELINE_352_SECONDS:-398.055}
...
LOGDIR="${LOG_ROOT%/}/353Py_work_census_logs_N21_full_once_${TS}"
...
echo "[start] 353 work-census validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 353 work-census SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '353 work-census' "$SRC"; then
  printf 'source_version_tag	353 work-census	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	353 work-census	missing	FAIL
...
# ---- 353: prove the code differs from 352 ONLY by the census lines ----
# The 352 revision proved byte identity with 351. 353 cannot claim identity --
# it adds a counter -- so it proves the NEXT strongest thing: that the code diff
# against 352, after docstrings and the three prose constants are stripped, is
# EXACTLY the census edit and nothing else. The fingerprint below is the sha256
# of the sorted added and removed lines of that diff. If a single unrelated
# character moved, the hash changes and this FAILS. It FAILS rather than skips
# when the 352 source is absent, for the same reason 352 did: a check that
# silently skips is worse than no check.
#
# The five removed lines are all in-place modifications, listed here so the
# failure is diagnosable without re-running the diff by hand:
#   1  k_per_thread:int=1  ...            -> gains a comma, census_out_fname added after
#   2  dispatcher call in exec_solutions (sorted path)     -> gains census_scratch
#   3  dispatcher call in exec_solutions (unsorted path)   -> gains census_scratch
#   4  dispatcher call in exec_solutions_gpu_chunk_split145 -> gains census
#   5  exec_solutions_gpu_chunk_split145 call in the driver -> gains census_fname
# exec_solutions is on the non-split145 route and never writes a file; it takes
# a scratch array purely so the module still compiles.
REF352=${REF352:-./352Py_record_fix.py}
EXPECTED_CENSUS_DIFF="added=62 removed=5 sha256=7feddf169a60ff8f64e4be760004b090a309f2210928efeacc37a2f6b4824b50"
if [[ -f "$REF352" ]]; then
  CENSUS_DIFF=$(python3 - "$REF352" "$SRC" <<'PYCENSUSDIFF'
import re, sys, difflib, hashlib
def strip(p):
    s = open(p, encoding='utf-8').read()
    s = re.sub(r'"""[\s\S]*?"""', '', s)
    s = re.sub(r'^(VERSION_TAG|WHI_ELIM_REASON|CENSUS_REASON):str="[^"]*"$', '', s, flags=re.M)
    return s.split('\n')
...
print("added=%d removed=%d sha256=%s" % (len(adds), len(dels), hashlib.sha256(blob).hexdigest()))
PYCENSUSDIFF
)
  record_check source_code_identical_to_352_except_census "$EXPECTED_CENSUS_DIFF" "${CENSUS_DIFF:-compare_failed}" || static_failures=$((static_failures+1))
else
  printf 'source_code_identical_to_352_except_census\t%s\t352 source not found at %s\tFAIL\n' "$EXPECTED_CENSUS_DIFF" "$REF352" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi

# ---- 353: the census edit itself, checked site by site ----
# THE NEGATIVE TEST. Feed 352's source to this script and source_census_kernel_sites
# must read 0 and FAIL:
#     SRC=./352Py_record_fix.py STATIC_ONLY=1 bash 353Py_work_census_validate_N21_full_once.sh
# That is the 351/352 procedure and it is the only thing that proves these checks
# are actually looking at the change rather than at prose. All greps read the
# docstring-stripped copy, per the 328 r4 / 345 r2 fix, because this project's
# workflow pastes chat text into the module docstring every revision.
CENSUS_DECL=$(grep -c '^      trips:u64=u64(0)$' "$SRC_NODOC" || true)
CENSUS_INC=$(grep -c '^        trips+=u64(1)$' "$SRC_NODOC" || true)
CENSUS_STORE=$(grep -c '^      census_arr\[idx\]=trips$' "$SRC_NODOC" || true)
CENSUS_PARAM=$(grep -c '^    census_arr:Ptr\[u64\],$' "$SRC_NODOC" || true)
CENSUS_SITES="decl=$CENSUS_DECL inc=$CENSUS_INC store=$CENSUS_STORE param=$CENSUS_PARAM"
record_check source_census_kernel_sites "decl=1 inc=1 store=1 param=1" "$CENSUS_SITES" || static_failures=$((static_failures+1))

# Scope: the counter must exist in maxd14 ONLY. If a future edit copies it into
# the fallback kernels this fires, because the whole-file count would exceed the
# maxd14-scoped count.
CENSUS_SCOPE=$(python3 - "$SRC_NODOC" <<'PYCENSUSSCOPE'
import sys
s = open(sys.argv[1], encoding='utf-8').read()
a = s.index('def kernel_dfs_iter_gpu_maxd14(')
b = s.index('def kernel_dfs_iter_gpu_maxd16(')
k = s[a:b]
inside = k.count('trips') + k.count('census_arr')
whole = 0
for name in ('kernel_dfs_iter_gpu_maxd16(', 'kernel_dfs_iter_gpu_maxd18(',
             'kernel_dfs_iter_gpu_maxd20(', 'kernel_dfs_iter_gpu_maxd21('):
    i = s.index('def ' + name)
    j = s.find('\n@gpu.kernel', i)
    if j < 0:
        j = s.find('\ndef launch_kernel_dfs_iter_gpu_static_maxd(', i)
    body = s[i:j]
    whole += body.count('trips') + body.count('census_arr')
print('maxd14=%d others=%d' % (inside, whole))
PYCENSUSSCOPE
)
record_check source_census_maxd14_only "maxd14=5 others=0" "${CENSUS_SCOPE:-scope_probe_failed}" || static_failures=$((static_failures+1))

# The counter must be u64. A u32 would cost one instruction instead of two but a
# silent wrap would lie, and the speed of a measurement build is worth nothing.
if grep -q '^      trips:u64=u64(0)$' "$SRC_NODOC" && ! grep -q 'trips:u32' "$SRC_NODOC"; then
  printf 'source_census_counter_width\tu64\tu64\tOK\n' >> "$SUMMARY"
else
  printf 'source_census_counter_width\tu64\tnot u64\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi

# The dump must sit AFTER t4, so a ~6MB write cannot contaminate
# stage_kernel_reduce_ms. Checked by line order, not by presence.
CENSUS_ORDER=$(python3 - "$SRC_NODOC" <<'PYCENSUSORDER'
import sys
lines = open(sys.argv[1], encoding='utf-8').read().split('\n')
t4 = dump = -1
for i, l in enumerate(lines):
    if l == '  t4=datetime.now()':
        t4 = i
    if l == '  if census_out_fname!="":' and t4 >= 0 and dump < 0:
        dump = i
print('t4=%d dump=%d order=%s' % (t4, dump, 'ok' if 0 <= t4 < dump else 'bad'))
PYCENSUSORDER
)
case "$CENSUS_ORDER" in
  *order=ok*) printf 'source_census_dump_after_t4\torder=ok\t%s\tOK\n' "$CENSUS_ORDER" >> "$SUMMARY" ;;
  *) printf 'source_census_dump_after_t4\torder=ok\t%s\tFAIL\n' "${CENSUS_ORDER:-probe_failed}" >> "$SUMMARY"; static_failures=$((static_failures+1)) ;;
esac

# No atomics anywhere: one task owns one slot, so the dump needs none. Codon has
# no atomics on this backend in the first place; this check exists so that a
# future edit cannot quietly assume otherwise.
CENSUS_ATOMIC=$(grep -c 'atomic' "$SRC_NODOC" || true)
record_check source_census_no_atomic 0 "$CENSUS_ATOMIC" || static_failures=$((static_failures+1))

# The writer must buffer. A per-record write of eight characters is the slow path
# in this runtime and would add minutes to a 400s run for no reason.
if grep -q 'if len(buf)>=65536:' "$SRC_NODOC"; then
  printf 'source_census_writer_buffered\t65536-record blocks\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_census_writer_buffered\t65536-record blocks\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
# ---- 353: census smoke (chunk 0 only, bench_mode 30) ----
if [[ "$CENSUS_SMOKE" == "1" ]]; then
  echo "[census-smoke] bench_mode=30 chunk 0 only; this proves the census plumbing for the price of one chunk"
  SMOKE_LOG="$LOGDIR/census_smoke.log"
  # argv layout for bench_mode 30 differs from 31: [13]=chunk_start [14]=chunk_count
  # [15]=chunk_list [16]=variant, and bucket_run/iter_sort are NOT parsed, so the
  # module defaults (2048 / 9) apply and the same shaped bin is reused.
  SMOKE_CMD=("$CAND" -g 21 21 "$BLOCK" "$MAX_BLOCKS" 1 "$SORT_MODE" "$PRESET_QUEENS" 30 "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" 0 1 "" "$BROADMARK_VARIANT")
  echo "[census-smoke] ${SMOKE_CMD[*]}" | tee "$SMOKE_LOG"
  set +e; stdbuf -oL -eL "${SMOKE_CMD[@]}" 2>&1 | tee -a "$SMOKE_LOG"; smoke_rc=${PIPESTATUS[0]}; set -e
  record_check census_smoke_exit 0 "$smoke_rc" || failures=$((failures+1))
  SMOKE_ERR=$(grep -c '\[census-error\]' "$SMOKE_LOG" || true)
  record_check census_smoke_error_hits 0 "$SMOKE_ERR" || failures=$((failures+1))
  SMOKE_FILE=$(sed -n 's/.*\[census-write\].* file=\([^ ]*\.bin\).*/\1/p' "$SMOKE_LOG" | tail -n1)
  SMOKE_RECS=$(sed -n 's/.*\[census-write\] m=[0-9]* records=\([0-9]*\) .*/\1/p' "$SMOKE_LOG" | tail -n1)
  record_check census_smoke_records 743424 "${SMOKE_RECS:-none}" || failures=$((failures+1))
  if [[ -n "$SMOKE_FILE" && -s "$SMOKE_FILE" ]]; then
    SMOKE_BYTES=$(wc -c < "$SMOKE_FILE")
    record_check census_smoke_bytes 5947392 "$SMOKE_BYTES" || failures=$((failures+1))
    SMOKE_STATS=$(python3 - "$SMOKE_FILE" <<'PYSMOKE'
import array, sys
a = array.array('Q')
with open(sys.argv[1], 'rb') as f:
    a.frombytes(f.read())
n = len(a); nz = sum(1 for v in a if v)
print("n=%d nonzero=%d nonzero_pct=%.2f max=%d sum=%d" % (n, nz, 100.0*nz/n if n else 0.0, max(a) if n else 0, sum(a)))
PYSMOKE
)
    printf 'census_smoke_stats\t(for eyeballing; nonzero_pct well above 0 and a max far above the mean is the expected shape)\t%s\tINFO\n' "$SMOKE_STATS" >> "$SUMMARY"
    case "$SMOKE_STATS" in
      *"nonzero=0 "*) printf 'census_smoke_nonzero\tnonzero>0\tALL ZERO -- the counter did not reach the device\tFAIL\n' >> "$SUMMARY"; failures=$((failures+1)) ;;
      *) printf 'census_smoke_nonzero\tnonzero>0\tpresent\tOK\n' >> "$SUMMARY" ;;
    esac
  else
    printf 'census_smoke_file\tpresent and non-empty\t%s\tFAIL\n' "${SMOKE_FILE:-missing}" >> "$SUMMARY"; failures=$((failures+1))
  fi
  echo
  echo "================================================================"
  echo "[census-smoke summary]"
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "================================================================"
  if (( failures != 0 || static_failures != 0 )); then
    echo "[census-smoke-failed] failures=$failures static_failures=$static_failures" >&2
    exit 1
  fi
  echo "[census-smoke-ok] plumbing verified on chunk 0; now run the full N=21 pass without CENSUS_SMOKE"
  exit 0
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 353 work-census. MEASUREMENT ONLY, NOT A CANDIDATE. The kernel carries a per-task DFS loop trip counter and the run writes one u64 per task, in shaped-bin record order, for all 2025282 tasks. mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT, all unchanged from 352, and the shaped bin is reused. The counter is deterministic and scheduling independent, so the census is exact even though this build is slower than 352; the elapsed is recorded as INFO and must not be quoted as a result. Expect 314666222712 and cache_state reuse"
...
# ---- 353: census output checks and the offline analysis ----
# The census files stay in the working directory, not in LOGDIR: three chunks is
# about 18MB and the log tar should stay small. Only the report is archived.
CENSUS_WRITES=$(grep -c '\[census-write\]' "$RUN_LOG" || true)
record_check census_write_rows "$EXPECTED_CHUNKS" "$CENSUS_WRITES" || failures=$((failures+1))
CENSUS_RECS=$(sed -n 's/.*\[census-write\] m=[0-9]* records=\([0-9]*\) .*/\1/p' "$RUN_LOG" | paste -sd, -)
record_check census_record_counts "743424,743424,538434" "${CENSUS_RECS:-none}" || failures=$((failures+1))
CENSUS_ERRS=$(grep -c '\[census-error\]' "$RUN_LOG" || true)
record_check census_error_rows 0 "$CENSUS_ERRS" || failures=$((failures+1))
CENSUS_FILES=$(sed -n 's/.*\[census-write\].* file=\([^ ]*\.bin\).*/\1/p' "$RUN_LOG" | paste -sd' ' -)
printf 'census_files\t(kept in the working directory, not archived)\t%s\tINFO\n' "${CENSUS_FILES:-none}" >> "$SUMMARY"

CENSUS_REPORT="$LOGDIR/census_report.txt"
CENSUS_STRIDE=$((BLOCK * MAX_BLOCKS))
if [[ "$CENSUS_WRITES" == "$EXPECTED_CHUNKS" ]]; then
  echo "[census] analysing $CENSUS_WRITES chunks at stride=$CENSUS_STRIDE (this takes a few seconds)"
  set +e
  python3 - "$CENSUS_STRIDE" 'census353_*_chunk*.bin' > "$CENSUS_REPORT" 2>&1 <<'PYCENSUSANALYZE'
# 353 offline census analysis.
# Input : census353_*_chunk<i>.bin, one u64 per task, shaped-bin record order.
# Output: a report on stdout. Every number here is scheduling independent, so it
#         does not matter that the instrumented build is not the fastest build.
import sys, os, array, glob, heapq

STRIDE = int(sys.argv[1]) if len(sys.argv) > 1 else 15488
WARP = 32
PAT = sys.argv[2] if len(sys.argv) > 2 else 'census353_*_chunk*.bin'

def spearman(y):
    # rank correlation between position (0..n-1, already the rank under the
    # current sort key, because the shaped bin is written in sort order) and the
    # measured trip count. Average ranks for ties.
    n = len(y)
...
def lpt(loads_source, bins):
    # longest processing time first: the standard 4/3-approximation, used here as
    # a practical stand-in for the makespan floor of any static assignment.
    heap = [0] * bins
    heapq.heapify(heap)
    for v in sorted(loads_source, reverse=True):
...
files = sorted(glob.glob(PAT), key=lambda p: int(p.rsplit('chunk', 1)[1].split('.')[0]))
if not files:
    print('CENSUS_ANALYSIS status=no_files pattern=%s' % PAT)
    sys.exit(0)

grand = {'S_make': 0, 'S_ideal': 0.0, 'P_make': 0, 'P_ideal': 0.0,
         'lpt_thread': 0, 'obs_thread': 0, 'lpt_warp': 0, 'total': 0, 'tasks': 0}

for path in files:
    ci = int(path.rsplit('chunk', 1)[1].split('.')[0])
    a = array.array('Q')
    ...
    S_make = max(B); S_ideal = total / float(warps)
    P_make = max(Mx); P_ideal = total / float(STRIDE)
    obs_thread = max(T)
    lane_tail = [WARP * Mx[w] / float(B[w]) for w in range(warps) if B[w] > 0]
    lt_mean = sum(lane_tail) / len(lane_tail)
    lt_max = max(lane_tail)
    lpt_thread = lpt(a, STRIDE)
    lpt_warp = lpt(a, warps)
    rho = spearman(a)

    print('CENSUS_CHUNK chunk=%d tasks=%d total_trips=%d zero_tasks=%d zero_pct=%.2f'
          % (ci, m, total, m - nz, 100.0 * (m - nz) / m))
    print('CENSUS_CHUNK chunk=%d task_trips mean=%.1f p50=%d p90=%d p99=%d p999=%d max=%d max_over_mean=%.1f'
          % (ci, total / float(m), pct(srt, .50), pct(srt, .90), pct(srt, .99), pct(srt, .999),
             srt[-1], srt[-1] / (total / float(m))))
    print('CENSUS_CHUNK chunk=%d modelS warp_sum_max=%d warp_sum_mean=%.0f imbalance=%.4f headroom_pct=%.3f'
          % (ci, S_make, S_ideal, S_make / S_ideal, 100.0 * (1.0 - S_ideal / S_make)))
    print('CENSUS_CHUNK chunk=%d modelP warp_maxlane_max=%d thread_mean=%.0f imbalance=%.4f headroom_pct=%.3f'
          % (ci, P_make, P_ideal, P_make / P_ideal, 100.0 * (1.0 - P_ideal / P_make)))
    print('CENSUS_CHUNK chunk=%d lane_tail mean=%.3fx max=%.3fx  (1.0 = every lane in the warp does equal work; 32.0 = one lane does all of it)'
          % (ci, lt_mean, lt_max))
    print('CENSUS_CHUNK chunk=%d lpt thread_obs=%d thread_lpt=%d thread_gain_pct=%.3f warp_obs=%d warp_lpt=%d warp_gain_pct=%.3f'
          % (ci, obs_thread, lpt_thread, 100.0 * (1.0 - lpt_thread / float(obs_thread)),
             S_make, lpt_warp, 100.0 * (1.0 - lpt_warp / float(S_make))))
    print('CENSUS_CHUNK chunk=%d sortkey rank_corr_position_vs_trips=%.4f  (the shaped bin is already in sort order, so position IS the rank under the current key)'
          % (ci, rho))

    grand['S_make'] += S_make; grand['S_ideal'] += S_ideal
    grand['P_make'] += P_make; grand['P_ideal'] += P_ideal
    grand['lpt_thread'] += lpt_thread; grand['obs_thread'] += obs_thread
    grand['lpt_warp'] += lpt_warp; grand['total'] += total; grand['tasks'] += m

print('CENSUS_TOTAL chunks=%d tasks=%d total_trips=%d' % (len(files), grand['tasks'], grand['total']))
print('CENSUS_TOTAL modelS headroom_pct=%.3f   (ceiling for perfect balancing across warps)'
      % (100.0 * (1.0 - grand['S_ideal'] / grand['S_make'])))
print('CENSUS_TOTAL modelP headroom_pct=%.3f   (ceiling for perfect balancing across threads)'
      % (100.0 * (1.0 - grand['P_ideal'] / grand['P_make'])))
print('CENSUS_TOTAL lpt_thread_gain_pct=%.3f lpt_warp_gain_pct=%.3f   (floor of any STATIC reordering; this is the host-side axis)'
      % (100.0 * (1.0 - grand['lpt_thread'] / float(grand['obs_thread'])),
         100.0 * (1.0 - grand['lpt_warp'] / float(grand['S_make']))))
PYCENSUSANALYZE
  census_rc=$?
  set -e
  record_check census_analysis_exit 0 "$census_rc" || failures=$((failures+1))
  if [[ -s "$CENSUS_REPORT" ]]; then
    echo
    echo "---------------- census report ----------------"
    cat "$CENSUS_REPORT"
    echo "-----------------------------------------------"
    # Promote the headline numbers into the summary so a future revision can
    # diff them without re-reading the report. All INFO: these are measurements
    # of the workload, not verdicts on a candidate.
    while IFS= read -r line; do
      case "$line" in
        CENSUS_TOTAL*) printf 'census_%s\t(measurement of the workload, not a verdict)\t%s\tINFO\n' "$(echo "$line" | awk '{print $2}')" "$line" >> "$SUMMARY" ;;
      esac
    done < "$CENSUS_REPORT"
    # The pre-registered decision rule, applied mechanically so that the reading
    # is fixed before anyone gets a chance to argue with the number.
    LANE_TAIL=$(sed -n 's/.*lane_tail mean=\([0-9.]*\)x.*/\1/p' "$CENSUS_REPORT" | tail -n1)
    LPT_WARP=$(sed -n 's/.*lpt_thread_gain_pct=[0-9.-]* lpt_warp_gain_pct=\([0-9.-]*\).*/\1/p' "$CENSUS_REPORT" | tail -n1)
    RANK_CORR=$(sed -n 's/.*rank_corr_position_vs_trips=\([0-9.-]*\).*/\1/p' "$CENSUS_REPORT" | tail -n1)
    VERDICT=$(awk -v lt="${LANE_TAIL:-0}" -v lw="${LPT_WARP:-0}" -v rc="${RANK_CORR:-0}" 'BEGIN{
      p="";
      if (lt >= 6.0) p = p "port_justified(lane_tail>=6x) ";
      else if (lt < 2.0) p = p "port_not_justified_on_this_evidence(lane_tail<2x) ";
      else p = p "port_inconclusive(2x<=lane_tail<6x) ";
      if (lw >= 3.0) p = p "host_reorder_has_headroom(lpt_warp>=3%) ";
      else p = p "host_reorder_near_exhausted(lpt_warp<3%) ";
      if (rc >= 0.85) p = p "key_axis_closed(rank_corr>=0.85)";
      else p = p "key_axis_open(rank_corr<0.85)";
      print p }')
    printf 'census_preregistered_verdict\t(rule fixed in the header BEFORE the data was seen)\t%s\tINFO\n' "$VERDICT" >> "$SUMMARY"
  else
    printf 'census_report\tnon-empty\tempty or missing\tFAIL\n' >> "$SUMMARY"; failures=$((failures+1))
  fi
else
  printf 'census_analysis_exit\t0\tskipped: expected %s census files, saw %s\tFAIL\n' "$EXPECTED_CHUNKS" "$CENSUS_WRITES" >> "$SUMMARY"
  failures=$((failures+1))
fi

printf 'timing_not_a_candidate\telapsed from this build is INFO only\t353 carries a per-task counter in the hot loop; its wall clock is not comparable to 352 and must never be quoted as a result\tINFO\n' >> "$SUMMARY"
...
echo "[validation-ok] 353 N=21 mode31 split145 full run reproduced 314666222712 with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2, CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN and CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT, all unchanged from 352, and wrote a per-task DFS trip census for all 2025282 tasks. 353 IS NOT A CANDIDATE: its elapsed is INFO only and 354 restarts from the 352 kernel. Read census_report.txt and census_preregistered_verdict, not the timing rows"
