...
# =============================================================================
# NQ_UPDATE_MEMO
# 201: 更新メモ: 200は正当性OKだが197比で遅いため不採用。197のroot<=2 direct-if preroll/future_check_mask guard/no-siblingを親に戻し、MAXD14 kernelのみroot-preroll内のchild_jmark bit0判定から一時scalarを削って直接判定する微差実験を導入。
...
201Py note:
  200Py was correct but slower than 197, so this branch returns to the
  validated 197 MAXD14 root fast-start scope: only post-root-action
  root_avail with one or two bits is prerolled, and 3-bit or wider roots stay
  on the generic loop.  The only kernel micro-experiment is inside that root
  preroll body: remove the temporary pr_child_jmark scalar and test
  (child_jmark_mask & 1) directly.
...
"""
200Pyログを確認しました。**正当性・実行結果は問題ありません。**
...
```text
200Py  0:07:09.520
197Py  0:07:08.202  → 200が 1.318秒 / 0.308% 遅い
199Py  0:07:08.293  → 200が 1.227秒 / 0.286% 遅い
198Py  0:07:08.239  → 200が 1.281秒 / 0.299% 遅い
196Py  0:07:08.239  → 200が 1.281秒 / 0.299% 遅い
194Py  0:07:08.288  → 200が 1.232秒 / 0.288% 遅い
188Py  0:07:10.137  → 200が 0.617秒 / 0.143% 高速
```

そのため、**200Pyは正当性OKだが197Pyを上回らないため不採用**としました。
新最速基準は引き続き **197Py** です。

201Pyは200を親にせず、197Pyへ戻しています。root fast-startの対象は197/196/194と同じく **root_avail 1bit/2bitのみ**です。200のroot-preroll future-bit直接判定、199の `root_rest==root_second`、198のtail-test、195の3bit root prerollは採用していません。

201Pyの差分はMAXD14 kernel内だけです。

```text
197Py:
  if pr_descend != 0:
      pr_child_jmark = child_jmark_mask & 1
      if pr_child_jmark != 0:
          jmark処理

201Py:
  if pr_descend != 0:
      if (child_jmark_mask & 1) != 0:
          jmark処理
```

意味は同じです。root-preroll内だけ `pr_child_jmark` 一時scalarを削除し、`child_jmark_mask` のbit0を直接判定します。generic DFS loop側の `child_jmark` 処理は197相当のままです。

手元確認結果です。

```text
bash -n 201Py...sh        OK
STATIC_ONLY=1 201Py...sh  OK

source_future_check_mask_guard   OK
source_nosibling_parent          OK
source_rootpr_jmarkdirect        OK
...
こちらではCodon/CUDA実機build/full実行はしていません。cudacodon側では、以下でN=21 fullのcases 01-06、dispatch、final total、warning/error、特に197/200/199/198/196比のtimingを確認してください。

```bash
STATIC_ONLY=1 bash 201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_validate_full_once.sh
bash 201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物はこちらです。

* [201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_probe.py)
* [201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_bundle_20260701.tar)
"""

# 201 root-preroll child_jmark-direct memo over validated 197 root-two futuremask/no-sibling MAXD14:
#   200 was correct but 1.318 seconds slower than the 197 numerical baseline,
#   so this branch returns to the validated 197 root one/two-candidate direct-if
#   preroll, 188 future_check_mask guard, and 184 no-sibling tailcall build.
...
#   only restorable ancestor frames.  201 keeps the narrow root fast-start only when
#   post-root-action root availability has one or two bits, matching the validated
#   197/196/194 behavior. Three-candidate roots explicitly stay on the generic
#   loop.  The root<=2 predicate is still the 197 second-lowbit direct-if test.
#   The only new kernel micro-experiment is inside the root-preroll body:
#   child_jmark_mask bit0 is tested directly instead of materializing a
#   pr_child_jmark scalar. Generic DFS child_jmark handling is unchanged.
#   The no-sibling rule and depth packing in avail[] bits 27..31 are unchanged,
#   so MAXD14 remains four u32[13] arrays = 208 bytes/thread.  The supported GPU
#   range remains N=5..27.
...
# 201 keeps the validated 197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 201 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="201 roottwo-prjmarkdirect: 197 root<=2 direct-if futuremask/no-sibling MAXD14 baseline with direct child_jmark bit0 test in root preroll - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """201 MAXD14 nibble schedule plus futuremask/no-sibling, root one/two-candidate direct-if preroll, and direct root-preroll child_jmark bit0 test."""
    ...
    # 201 keeps the validated 197 current-frame register cache, 13-slot
    ...
    # was correct but slower.  The predicate is still the 197 second-lowbit
    # direct-if form. Only the root-preroll child_jmark handling changes: bit0
    # of child_jmark_mask is tested directly instead of materializing a
    # pr_child_jmark scalar. Therefore 3-bit and wider roots stay on the generic loop.
    ...
    if root_after_second==u32(0):
      ...
      if pr_descend!=u32(0):
        if (child_jmark_mask&u32(1))!=u32(0):
          pr_nf&=~u32(1)
          if pr_nf==u32(0):
            pr_descend=u32(0)
          else:
...
CHUNKSHAPE148_DEFAULT_REASON:str="201 roottwo-prjmarkdirect reuses validated 197 scorestripe_v9 task order/cache and changes only MAXD14 root-preroll child_jmark bit0 test shape"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split201_roottwo_prjmarkdirect_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split201_roottwo_prjmarkdirect_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split201-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split201-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split201_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      ...
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split201-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split201-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split201-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split201_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
