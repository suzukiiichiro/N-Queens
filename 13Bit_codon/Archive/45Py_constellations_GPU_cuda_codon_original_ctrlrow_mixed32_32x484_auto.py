...
Python/codon Ｎクイーン コンステレーション版 CUDA 高速ソルバ 45 trunk候補 32x484 auto-sort + chunk内sort診断（ctrlrow mixed32 tuned）
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  # sort_mode auto:
  #   N=20 は実測で sort_mode=2 が単体/範囲とも N=20 を最速化。
  #   N<=19 は範囲実行で sort_mode=0 の方が速いため conservative に 0。
  if gpu_sort_mode < 0:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk45={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
