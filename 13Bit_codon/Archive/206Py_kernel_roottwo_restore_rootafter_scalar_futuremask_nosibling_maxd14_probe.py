...
# =============================================================================
# NQ_UPDATE_MEMO
# 206: 更新メモ: 205Py rootafterinlineは正当性OKながら204Pyより0.279秒遅いため不採用。206Pyは204Py相当へ戻し、MAXD14 kernelのみroot_after_second一時scalarを復元する安全復帰版。root_rest clear-lowbit / futuremask / no-sibling / root one-two preroll は維持する。
...
206Py note:
  204Py was correct and numerically faster than 197, so this branch adopts
  204 as the new numerical baseline.  It keeps the same validated MAXD14
  root fast-start scope: only post-root-action root_avail with one or two
  bits is prerolled, and 3-bit or wider roots stay on the generic loop.
  This is a safety rollback from 205: keep the 204 clear-lowbit root_rest
  expression and restore the validated root_after_second temporary scalar
  before the root<=2 if condition.
...
"""
204Pyログを確認しました。**正当性・実行結果ともに問題ありません。**
...
```text
204Py  0:07:07.795
197Py  0:07:08.202  → 204が 0.407秒 / 0.095% 高速
203Py  0:07:08.344  → 204が 0.549秒 / 0.128% 高速
202Py  0:07:08.482  → 204が 0.687秒 / 0.160% 高速
201Py  0:07:08.383  → 204が 0.588秒 / 0.137% 高速
199Py  0:07:08.293  → 204が 0.498秒 / 0.116% 高速
198Py  0:07:08.239  → 204が 0.444秒 / 0.104% 高速
194Py  0:07:08.288  → 204が 0.493秒 / 0.115% 高速
188Py  0:07:10.137  → 204が 2.342秒 / 0.544% 高速
```

したがって、**204Pyを新最速基準として採用**しました。

206Pyは204Pyを親にしています。差分はMAXD14 kernel内だけです。
...
204Py:
  ...
  if root_after_second == u32(0):
      root 1bit/2bit とみなしてpreroll

206Py:
  root_after_second 一時scalarを作らず、

  if (root_rest ^ root_second) == u32(0):
      root 1bit/2bit とみなしてpreroll
```

意味は同じです。root fast-startの対象は204/197/196/194と同じく **root_avail 1bit/2bitのみ**です。195の3bit root preroll、198〜203の不採用差分は入れていません。

手元確認結果です。

```text
bash -n 206Py...sh        OK
STATIC_ONLY=1 206Py...sh  OK

source_future_check_mask_guard   OK
source_nosibling_parent          OK
source_restore_rootafter_scalar           OK
...
```text
204 root_after_second scalar predicate
206 inline predicate
equivalence OK: 65535 exhaustive masks
```

こちらではCodon/CUDA実機build/full実行はしていません。cudacodon側では以下で確認してください。

```bash
STATIC_ONLY=1 bash 206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_validate_full_once.sh
bash 206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物はこちらです。

* [206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_probe.py)
* [206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_bundle_20260701.tar)

"""

# 206 restore_rootafter_scalar memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  206 keeps the narrow root fast-start only when
...
# 206 keeps the validated 204/197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 206 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="206 roottwo-restore_rootafter_scalar: 204 rootclearrest futuremask/no-sibling MAXD14 baseline with root_after_second scalar predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """206 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate restored root_after_second predicate."""
    ...
    # 206 restore_rootafter_scalar:
    # If the root frame has one or two candidates, process the first root
    # candidate before the generic DFS loop, matching the validated 204/197/196/194 scope.
    # 195's three-candidate case remains intentionally removed because it was
    # correct but slower.  206 keeps 204's clear-lowbit root_rest construction and restores
    # the root_after_second temporary scalar because 205 inline predicate
    # was correct but slower. Therefore 3-bit and wider roots stay on the generic loop.
...
CHUNKSHAPE148_DEFAULT_REASON:str="206 restore_rootafter_scalar reuses validated 204 scorestripe_v9 task order/cache; MAXD14 root_after_second scalar shape restored after 205 was not adopted"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split206_roottwo_restore_rootafter_scalar_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split206_roottwo_restore_rootafter_scalar_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split206-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split206-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split206_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split206-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split206-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split206_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
