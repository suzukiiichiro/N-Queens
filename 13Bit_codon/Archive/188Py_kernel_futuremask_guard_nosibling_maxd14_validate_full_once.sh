...
# =============================================================================
# NQ_UPDATE_MEMO
# 188: 更新メモ: 187は正当性OKだが184比で微小退行したため不採用。親を184のno-sibling spill elisionへ戻し、MAXD14 kernelのみ future_check_mask==0 の軽量guardを単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 188 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 188 starts from
# validated 184 because 185 terminal-depth-13, 186 terminal-base14 split, and
# 187 child_jmark_mask guard were correct but slower/not faster. 188 changes
# only the MAXD14 future-prune zero-mask guard.
#
# Kernel arithmetic keeps 184 bitboard/schedule semantics: register-held current
# frame, 13-slot ancestor stack, scalar 4-bit nibble schedule, no-sibling spill
# elision, schedule_words=0, and 208 local stack bytes/thread. The experiment is
# deliberately focused: when future_check_mask==0, skip the per-candidate future
# prune branch; otherwise keep the 184 generic behavior. Host task order,
# dispatch, cache, progress verification, and README-managed history remain
# unchanged.
SRC=${SRC:-./188Py_kernel_futuremask_guard_nosibling_maxd14_probe.py}
CAND=${CAND:-./188Py_kernel_futuremask_guard_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/188Py_kernel_futuremask_guard_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 188 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/188Py_kernel_futuremask_guard_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 188Py source
#   has a harmless header/string/layout difference.  This 188 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 184 no-sibling MAXD14 dispatch; 185/186/187 correct but slower/not faster	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split188 only	split188 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split188 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 188 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source child_future_mask guard static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 188 futuremask guard no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 188 keeps the validated host/chunk structure, then derives chunk-tail diagnostics from
...
# 188: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 188 futuremask guard nosibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
