...
CPU
$ codon build -release 63Py_constellations_GPU_cuda_codon_balanced_stripe_32x484.py
$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -c

GPU
$ codon build -release 63Py_constellations_GPU_cuda_codon_balanced_stripe_32x484.py
$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -g 5 20 32 484 0 5

N=20 単体ベンチ
$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -g 20 20 32 484 0 5

ログ確認
$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -g 20 20 32 484 2 5
...
auto:
  N=20      -> sort_mode=6   # striped chunks only
  otherwise -> sort_mode=0
...
suzuki@cudacodon$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -g 5 20 32 484 0 5
GPU mode selected
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  # sort_mode auto:
  #   63 BALANCED STRIPE:
  #     N=20 は sort_mode=6（striped chunks only）を採用。
  #     N!=20 は従来どおり sort_mode=0。
  #   sort_mode=5（stripe + sort4）は実測で悪化したため auto には入れない。
  if gpu_sort_mode < 0:
    gpu_sort_mode = 6 if N == 20 else 0
  ...
  if use_gpu:
    ...
    #   sort_mode=7: balanced stripe only。各出力chunkが within 全域をまたぐように再配置。
    #   sort_mode=8: balanced stripe + sort4（比較用）
    #
    #   解数は加算なので、direct_total では元 index への scatter は不要。
    stripe_chunks:bool=(gpu_sort_mode==5 or gpu_sort_mode==6 or gpu_sort_mode==7 or gpu_sort_mode==8)
    ...
    if stripe_chunks:
      ...
      if gpu_sort_mode==5 or gpu_sort_mode==8:
        local_sort_mode=4
      else:
        local_sort_mode=0
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk63={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} local_sort={local_sort_mode} stripe={1 if stripe_chunks else 0} balanced={1 if balanced_stripe else 0} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    ...
    if gpu_log_level>=1:
      print(f"[gpu-summary] N={N} constellations={m_all} chunks={chunks} direct_total=1 stripe={1 if stripe_chunks else 0} balanced={1 if balanced_stripe else 0}")
    return
  ##########
  # CPU 
  ##########
...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  # 63 BALANCED STRIPE: preset_queens は 5 固定。preset=6 は 48 で正解には直せたが速度面で不採用。
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 63 BALANCED STRIPE; using 5")
      preset_queens_arg=5
    if argc > 9:
