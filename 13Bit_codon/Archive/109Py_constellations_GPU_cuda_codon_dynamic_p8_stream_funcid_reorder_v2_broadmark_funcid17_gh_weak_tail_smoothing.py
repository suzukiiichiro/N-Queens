...
codon build -release \
  109Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_weak_tail_smoothing.py

#CPU
./109Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_weak_tail_smoothing \
  -c 5 22
#GPU
./109Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_weak_tail_smoothing \
...
作成しました。
今回は **送付いただいた 107Py / 108Py を正**として、109Py を生成しています。

[109Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_weak_tail_smoothing.py](sandbox:/mnt/data/109Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_weak_tail_smoothing.py)

109Py の方針は以下です。

```text
ベース:
  107Py broad funcid primary + mark-distance secondary
  broad bucket balance は 107Py を維持

108Py から継承:
  broadmarktail の 25 x 3 subcell 構造
  mode 28 / 29
  GPU kernel / DFS 本体は変更なし

109Py で変更:
  tail 分離対象を fid7 から funcid17 に変更
  cell_G_H は引き続き弱く分離
  tail_window_boost は 2 → 1 に弱化
  broadmarktail reorder version は v2 に変更
```

はい、**N21 でデバッグしたほうが効率的**です。
109Py はまず **N21 full run** で、コンパイル・mode 29・`.bin` 生成・progress TSV・集計スクリプト接続・解数一致を確認するのが良いです。

重要な注意点として、**N21 は preset=6** で実行してください。ソース内の dynamic preset は `N=18..21 → 6`, `N=22..24 → 7` になっています。

過去ログでも N21 は `0:25:38.459`、N22 は `3:18:42.963` の例があり、N21 は N22 の約 1/8 程度で確認できます。N21 の期待 total は `314666222712` です。

## 109Py N21 デバッグ実行コマンド
...
./109Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_weak_tail_smoothing \
  ...
  2>&1 | tee 109Py_N21_p6_broadmarktail_gpu_debug.log
```

これは **N21 の full run** です。
N22 full run ではなく、N21 だけを全 chunk 実行するデバッグ用 full run です。
...
N21 なので、ファイル名は `N21_6` になるはずです。

```text
progress_N21_6_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu.tsv
progress_N21_6_stream_broadmarktail_reorder_v2_w8_j7_sim.tsv
constellations_N21_6_broadmarktail_reorder_v2_w8_j7.bin
constellations_N21_6_broadmarktail_reorder_v2_w8_j7.bin.done
109Py_N21_p6_broadmarktail_gpu_debug.log
```

## N21 集計コマンド

まずは baseline なしで十分です。

```bash
python3 aggregate_107py_broadmark_gpu.py \
  --gpu progress_N21_6_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu.tsv \
  --sim progress_N21_6_stream_broadmarktail_reorder_v2_w8_j7_sim.tsv \
  --out 109Py_N21_p6_broadmarktail \
  --top 100 \
  --run-threshold p95 \
  --log 109Py_N21_p6_broadmarktail_gpu_debug.log
```
...
109Py_N21_p6_broadmarktail_report.md
109Py_N21_p6_broadmarktail_summary.tsv
109Py_N21_p6_broadmarktail_correlation.tsv
109Py_N21_p6_broadmarktail_top_elapsed.tsv
109Py_N21_p6_broadmarktail_elapsed_runs.tsv
109Py_N21_p6_broadmarktail_bucket_balance.tsv
109Py_N21_p6_broadmarktail_cell_balance.tsv
109Py_N21_p6_broadmarktail_slow_feature_ratio.tsv
109Py_N21_p6_broadmarktail_joined.tsv
```

## 判定方針

N21 で見るべきことは、主に **正しさと実装健全性** です。
...
N21で確認:
  ...
  status = ok
  mode 29 が通る
  broadmarktail v2 の sim/gpu TSV が出る
  aggregate_107py_broadmark_gpu.py が通る
  funcid17/G_H の列が joined.tsv に出る

N21では最終判断しない:
  N22 の最終 elapsed 優劣
  N22 の slow island 66-70, 1633-1650 付近の改善可否
```

N21 が `ok` なら、その後に N22 full run へ進む流れが一番効率的です。


N21 デバッグ結果、確認しました。**109Py は N21 では正常です。N22 full run に進んで大丈夫です。**

集計結果では、`final_gpu_total=314666222712` が `expected_total=314666222712` と一致し、`status=ok`、`log_status=ok` です。chunk 数は 131、records は 2,025,282、kernel elapsed 合計は **1443.523 sec = 24:03.523**、ログ上の最終 elapsed は **0:24:16.615** でした。
...
109Py N21:
  正しさ        : ok
  mode 29       : ok
  broadmarktail v2 sim/gpu TSV : ok
  aggregate 接続: ok
  N22 full run  : 実行してよい
```

N21 の elapsed 分布も、致命的な崩れはありません。

```text
elapsed_mean_ms : 11019.260
elapsed_p95_ms  : 12195.500
elapsed_p99_ms  : 13101.900
elapsed_max_ms  : 14199.000
elapsed_cv      : 0.077628
slow_p95_count  : 7 / 131 chunks
```

`elapsed_vs_chunk_total_pearson=0.239611` なので、N21 では chunk_total 依存も強すぎません。

## 気になる点

N21 では slow chunk がかなり局所化しています。最大は **chunk 96 = 14199 ms**、次が **chunk 90 = 13386 ms** です。p95 slow run は `89-90`, `96`, `104`, `98`, `83`, `106` のように短い island で、長い連続 slow island にはなっていません。

相関上位は N22 の 108Py と少し違います。N21 では `score_sum`, `cell_A_H_count`, `risky_a_count`, `markrisk_h_count`, `cell_O_X_count`, `funcid_6_count`, `funcid_4_count` が上位です。109Py で狙った `funcid_17` は N21 では主役ではありません。これは N21 と N22 で tail 構造が違うため、**N21 は正しさ確認用、性能採否は N22 で判断**が妥当です。

また、slow p95 feature ratio では `funcid_6_count` が `all_mean=91.565 → slow_mean=190.286`、約 **2.08倍** になっています。N21 だけを見るなら次候補は `funcid_6` ですが、今回の最終目的は N22 なので、ここで 110Py に進む必要はありません。

## 次は N22 full run でお願いします
...
./109Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_weak_tail_smoothing \
  ...
  2>&1 | tee 109Py_N22_p7_broadmarktail_gpu_full.log
```

一点だけ補足です。`tee` で保存するのは標準出力ログです。raw progress TSV はプログラムが直接生成します。N22 実行後に添付いただきたいのは以下です。

```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu.tsv
progress_N22_7_stream_broadmarktail_reorder_v2_w8_j7_sim.tsv
109Py_N22_p7_broadmarktail_gpu_full.log
```
...
```bash
python3 aggregate_107py_broadmark_gpu.py \
  --gpu progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu.tsv \
  --sim progress_N22_7_stream_broadmarktail_reorder_v2_w8_j7_sim.tsv \
  --baseline-summary 107Py_N22_p7_broadmark_summary.tsv \
  --baseline-label 107Py \
  --out 109Py_N22_p7_broadmarktail \
  --top 100 \
  --run-threshold p95 \
  --log 109Py_N22_p7_broadmarktail_gpu_full.log
```

`107Py_N22_p7_broadmark_summary.tsv` が無い場合は、`--baseline-summary` と `--baseline-label` を外して実行してください。

N22 集計後は、この一式を送ってください。

```text
109Py_N22_p7_broadmarktail_report.md
109Py_N22_p7_broadmarktail_summary.tsv
109Py_N22_p7_broadmarktail_comparison.tsv
109Py_N22_p7_broadmarktail_correlation.tsv
109Py_N22_p7_broadmarktail_top_elapsed.tsv
109Py_N22_p7_broadmarktail_elapsed_runs.tsv
109Py_N22_p7_broadmarktail_bucket_balance.tsv
109Py_N22_p7_broadmarktail_cell_balance.tsv
109Py_N22_p7_broadmarktail_slow_feature_ratio.tsv
109Py_N22_p7_broadmarktail_joined.tsv
```

現時点の結論は、**109Py は N21 デバッグ通過。次は N22 full run で 107Py に勝つかどうかを見る段階**です。
...
VERSION_TAG:str="109 broadmark funcid17/G_H weak tertiary smoothing auto w8_j7 stream GPU runner from 107 type-explicit"
...
# 109 BROAD+MARKDIST+WEAK TAIL TERTIARY REORDER:
...
BROAD_MARKDIST_TAIL_REORDER_VERSION:str="v2"
BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON:str="109 broadmark + funcid17/G_H weak tertiary smoothing"
...
"""109 broadmark tail: tertiary tail bucket label。0=funcid17, 1=cell_G_H, 2=rest。"""
...
"""109 broadmark tail: 25x3 subcell bin 名。"""
...
"""109 broadmark tail: reorder 済み bin 名。"""
...
"""109 broadmark tail: subcell index を 0..74 に pack。"""
def broad_markdist_tail_subcell_index(broad:int,risk:int,tail:int)->int:
  return (broad*5+risk)*3+tail

"""109 broadmark tail: 75 個の constellation buffer を作る。"""
...
"""109 broadmark tail: fid / broad / risk から weak tertiary tail bucket を返す。"""
...
"""109 broadmark tail: 75 subcell stats。subcell=(broad*5+risk)*3+tail。"""
...
"""109 broadmark tail: summary stats。0=funcid17,1=cell_G_H,2=markrisk_H,3=proxy_sum。"""
...
"""109 broadmark tail: chunk constellations から tail summary を作る。"""
...
"""109 broadmark tail: tail summary suffix。"""
...
"""109 broadmark tail: 元 bin を broad x markdist x tail 75 subcell bin へ分配する。"""
...
"""109 broadmark tail: 75 subcell rem_counts から broad primary / markrisk secondary / weak tail tertiary quota を作る。"""
...
"""109 broadmark tail: one 107 cell の F17/GH/R parts を弱く interleave する。"""
def interleave_broad_markdist_tail_subparts(part_f17:List[Dict[str,int]],part_gh:List[Dict[str,int]],part_r:List[Dict[str,int]],m_target:int)->List[Dict[str,int]]:
  out:List[Dict[str,int]]=[]
  i17:int=0
  ig:int=0
  ir:int=0
...
"""109 broadmark tail: 107 broadmark base + funcid17/G_H weak tertiary smoothing で reordered bin を作る。"""
