...
# =============================================================================
# 298 N=21 full validation harness (next-depth-elim probe)
...
# Note: Registers/Thread may decrease by 1 vs 296 if next_depth was register-allocated.
...
SRC=${SRC:-./298Py_next_depth_elim_probe.py}
CAND=${CAND:-./298Py_next_depth_elim_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/298Py_next_depth_elim_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/298Py_next_depth_elim_logs_N21_full_once_${TS}"
...
echo "[start] 298 next-depth-elim validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 298 next-depth-elim SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '298 next-depth-elim probe' "$SRC"; then
  printf 'source_version_tag	298 next-depth-elim probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	298 next-depth-elim probe	missing	FAIL
...
# 298: next-depth-elim shape checks (based on 296, save_sp retained).
    ok_stride_param = 'stride:int,' in body14.split(')->None:')[0]
    ok_stack_array = 'stack=__array__[u64](MAXD14_ANCESTOR*2)' in body14
    ok_no_next_depth = 'next_depth' not in body14
    ok_cur_depth_incr = 'cur_depth+=1' in body14
    ok_save_sp_retained = 'if save_sp==0:' in body14
    ok_stack_ptr_incr = body14.count('stack_ptr+=2') == 2
    ok_stack_ptr_decr = body14.count('stack_ptr-=2') == 1
    ...
    ok_stack_shape = (ok_stride_param and ok_stack_array and ok_no_next_depth and ok_cur_depth_incr
                       and ok_save_sp_retained and ok_stack_ptr_incr and ok_stack_ptr_decr
                       and ok_gridstride_loop and ok_single_writeback and ok_no_percall_writeback
                       and ok_push and ok_pop)
    checks.append(('source_next_depth_elim_shape',
                    'no next_depth + cur_depth+=1 + save_sp==0 check retained + stack_ptr+2/-2 + stride + single writeback',
                    'present' if ok_stack_shape else (
                        'missing (stride=%s stack=%s no_nd=%s cdepth_incr=%s save_sp=%s ptr_incr=%s ptr_decr=%s gs=%s wb=%s push=%s pop=%s)'
                        % (ok_stride_param, ok_stack_array, ok_no_next_depth, ok_cur_depth_incr, ok_save_sp_retained,
                           ok_stack_ptr_incr, ok_stack_ptr_decr, ok_gridstride_loop, ok_single_writeback, ok_push, ok_pop)
                    ),
                    ok_stack_shape))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 298 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 298 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 298 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 298 next-depth-elim mode31 split145"
...
echo "[validation-ok] 298 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
