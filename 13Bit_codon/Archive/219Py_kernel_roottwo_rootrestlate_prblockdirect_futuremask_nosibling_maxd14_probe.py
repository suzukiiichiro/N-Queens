...
# =============================================================================
# NQ_UPDATE_MEMO
# 217: 更新メモ: 216Py rootprblockdirect は正当性OKだが204最速を上回らなかったため不採用。219Pyは211Pyのroot_first遅延形を親に、MAXD14 kernelのみroot-preroll内の cur_avail=root_rest 代入をfirst-child検証後へ遅延する最後のroot-preroll微差実験。root_rest clear-lowbit / root_after_second XOR / futuremask / no-sibling / root one-two preroll は維持する。
...
219Py note:
  218Py was correct but did not beat the 217 rootrestlate fastest baseline.
  This branch returns to 217 as parent and tests only one MAXD14 kernel
  micro-change: keep pr_nibble_op = schedule_lo & 15 for the future bit,
  but derive pr_block_code directly as schedule_lo & 7 instead of
  pr_nibble_op & 7.  This preserves the 217 rootrestlate timing shape,
  root_after_second XOR predicate, futuremask guard, no-sibling spill
  elision, and root one/two-candidate preroll scope.
...
218Pyは正当性OKですが、217Pyより `0.116秒` 遅いため**不採用**です。予定通り、219Pyは **217 + pr_block_code direct** にしました。
...
```text
bash -n 219Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 219Py...sh OK
source_rootrestlate_prblockdirect     OK
source_split_tag split145 split219    OK
```

ファイルはこちらです。

* [219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_probe.py)
* [219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_validate_full_once.sh

bash 219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 219 rootrestlate_prblockdirect memo over validated 217 rootrestlate futuremask/no-sibling MAXD14:
#   217 became the new fastest N=21 full-once baseline at 0:07:07.709.
#   218 rootrestlate_andnot was correct but slower, so 219 returns to 217
#   and tests only one narrow MAXD14 root-preroll codegen shape.
...
#
#   219 keeps 217 rootrestlate: root_first is computed only inside the
#   root<=2 branch, and cur_avail=root_rest is delayed until after the
#   first-child nf/future/terminal/jmark checks.  The only new change is
#   root-preroll block-code extraction:
#       217: pr_block_code = pr_nibble_op & 7
#       219: pr_block_code = schedule_lo & 7
#   pr_nibble_op is still schedule_lo & 15 and is still used for the future
#   bit.  The supported GPU range remains N=5..27.
...
VERSION_TAG:str="219 roottwo-rootrestlate_prblockdirect: 217 rootrestlate fastest baseline with root-preroll pr_block_code schedule_lo direct extraction - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """219 MAXD14 nibble schedule plus futuremask/no-sibling and rootrestlate + direct root-preroll block-code extraction."""
    ...
    # 219 rootrestlate-prblockdirect:
...
CHUNKSHAPE148_DEFAULT_REASON:str="219 rootrestlate-prblockdirect reuses validated 204 scorestripe_v9 task order/cache; MAXD14 delays cur_avail=root_rest assignment within root-preroll"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split219_roottwo_rootrestlate_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split219_roottwo_rootrestlate_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split219-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split219-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split219_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split219-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split219-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split219_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
