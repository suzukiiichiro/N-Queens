...
# =============================================================================
# 265 N=21 full validation harness
...
SRC=${SRC:-./265Py_schedule_precompute_reusemask_probe.py}
CAND=${CAND:-./265Py_schedule_precompute_reusemask_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/265Py_schedule_precompute_reusemask_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/265Py_schedule_precompute_reusemask_logs_N21_full_once_${TS}"
...
echo "[start] 265 schedule-precompute-reusemask validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 265 schedule-precompute-reusemask ctrl0-markctrl-free-repack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '265 schedule-precompute-reusemask probe' "$SRC"; then
  printf 'source_version_tag\t265 schedule-precompute-reusemask probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t265 schedule-precompute-reusemask probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
old_splits = ['split240_','split241_','split242_','split243_','split244_','split255_','split256_','split257_','split258_','split259_','split260_','split261_','split262_','split263_','split264_']
runtime_split_ok = ('split265_' in s and not any(x in s for x in old_splits))
if runtime_split_ok:
    checks.append(('source_split_tag', 'split265 active; old split runtime tags absent', 'split265', True))
else:
    checks.append(('source_split_tag', 'split265 active; old split runtime tags absent', 'old split runtime tag present or split265 missing', False))
...
if not m14:
    ...
    actual = 'reusemask MAXD14 precompute active' if ok else 'old schedule interpreter present or free_arr mask reuse missing or unsafe pack present'
    checks.append(('source_schedule_precompute_reusemask', 'MAXD14 precompute reusemask active', actual, ok))
...
checks.append(('source_host_precompute_reusemask_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 265 schedule-precompute-reusemask mode31 split145"
...
echo "[validation-ok] 265 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
