...
# =============================================================================
# NQ_UPDATE_MEMO
# 220: 更新メモ: 219Py rootrestlate_prblockdirect は正当性OKだが217最速を上回らなかったため不採用。221Pyは217Py rootrestlate新最速基準を親に、MAXD14 kernelのみroot<=2判定をroot_tail clear-lowbit判定へ置き換える再結合実験を検証する。
# Full update history: see README.md
# =============================================================================

# 221 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 221 keeps the
# 217 rootrestlate fastest baseline because 218 rootrestlate_andnot and 219
# rootrestlate_prblockdirect were correct but did not beat 217. 221 is the
...
#   - rootrestlate_inlinexor source check is whitespace/comment robust.
SRC=${SRC:-./221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
LOGDIR="${LOG_ROOT%/}/221Py_kernel_roottwo_rootrestlate_inlinexor_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 217 rootrestlate futuremask/no-sibling MAXD14 dispatch; 217 is new fastest over 204; 218 rootrestlate_andnot, 219 rootrestlate_prblockdirect, and 220 rootrestlate_roottail were correct but slower than 217; 221 tests 217 plus inline XOR predicate; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + inline XOR predicate + rootrestlate	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
marker='# 221 rootrestlate-inlinexor:'
...
if_pos=root_compact.find('if(root_rest^root_second)==u32(0):')
...
checks = {
    ...
    'root_second_present': 'root_second:u32=root_rest&(u32(0)-root_rest)' in root_compact,
    'inline_xor_if_present': if_pos >= 0,
    ...
    'root_tail_absent': 'root_tail:u32=' not in root_compact and 'ifroot_tail==u32(0):' not in root_compact,
...
if all(checks.values()):
    sys.exit(0)
print('source_rootrestlate_inlinexor_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootrestlate_inlinexor	present_rootrestlate_inlinexor	present_rootrestlate_inlinexor	OK
' >> "$SUMMARY"
else
  printf 'source_rootrestlate_inlinexor	present_rootrestlate_inlinexor	missing_or_old_rootrestlate_inlinexor_shape	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split221'}
bad=sorted(tags-allowed)
if 'split221' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split221 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split221 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 221 runtime-only harness disabled source/model assertions"
