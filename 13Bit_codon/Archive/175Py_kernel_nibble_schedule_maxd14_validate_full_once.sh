...
# 175 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 175 inherits the
...
# the MAXD14 hot loop uses a 4-bit nibble op: direct block_code plus future bit.
#
# For N=21 all launches must remain required_maxd=14, select MAXD14, report
# schedule_words=0 and 208 local stack bytes/thread.

SRC=${SRC:-./175Py_kernel_nibble_schedule_maxd14_probe.py}
CAND=${CAND:-./175Py_kernel_nibble_schedule_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/175Py_kernel_nibble_schedule_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 175 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/175Py_kernel_nibble_schedule_maxd14_logs_N21_full_once_${TS}"
...
assert '175 nibble-schedule GPU candidate supports N=5..27 only' in s
...
# schedule fields, the 172 register-held active frame, and the 175 nibble schedule hot path.
...
printf 'source_nibble_schedule_shape\tMAXD14 scalar nibble schedule + register-held current frame + 13-slot ancestor stack\tverified\tOK\n' >> "$SUMMARY"
...
printf 'dfs_164_vs_175_nibble_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 175 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 172 register cache + ancestor13 stack + 175 nibble schedule"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 175 keeps the validated 172 kernel structure, then derives chunk-tail diagnostics from
...
echo "[validation-ok] 175 nibble schedule reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
