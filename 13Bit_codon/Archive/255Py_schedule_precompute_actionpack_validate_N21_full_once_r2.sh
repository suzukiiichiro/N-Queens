...
# =============================================================================
# 255 N=21 full validation harness
# Parent: 241 restore239-after-fid14split-reject.
# Experiment: keep single launch and retain 244 r4 maskpack, then pack only
#             terminal_base14/root_action into sched_action_arr while keeping
#             terminal_depth separate. This isolates whether action-pack is JIT-safe.
#             Keep -c/-g, N5..27, cache generation, broadmarktail 28/29,
#             split145 30/31, MAXD fallbacks, and worker_id/worker_count.
...
SRC=${SRC:-./255Py_schedule_precompute_actionpack_probe_r2.py}
CAND=${CAND:-./255Py_schedule_precompute_actionpack_probe_r2}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/255Py_schedule_precompute_actionpack_r2_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/255Py_schedule_precompute_actionpack_r2_logs_N21_full_once_${TS}"
...
echo "[start] 255 r2 schedule-precompute-actionpack commandfix validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 255 r2 schedule-precompute-actionpack commandfix SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '255 r2 schedule-precompute-actionpack commandfix probe' "$SRC"; then
  printf 'source_version_tag\t255 r2 schedule-precompute-actionpack commandfix probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t255 r2 schedule-precompute-actionpack commandfix probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split255_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s and 'split244_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split255 active; split240/241/242/243/244 runtime tags absent', 'split255', True))
else:
    checks.append(('source_split_tag', 'split255 active; split240/241/242/243/244 runtime tags absent', 'old split runtime tag present or split255 missing', False))
...
if not m14:
    checks.append(('source_schedule_precompute_actionpack', 'MAXD14 precompute actionpack active', 'MAXD14 kernel not found', False))
else:
    body = m14.group(0)
    ok = (
        'sched_lo_arr:Ptr[u32]' in body and
        'sched_hi_arr:Ptr[u32]' in body and
        'sched_mask_arr:Ptr[u32]' in body and
        'sched_action_arr:Ptr[u32]' in body and
        ...
        'sched_action:u32=sched_action_arr[i]' in body and
        'terminal_base14:u32=sched_action&u32(1)' in body and
        'root_action:u32=(sched_action>>u32(1))&u32(3)' in body and
        'terminal_base14_arr:Ptr[u32]' not in body and
        'root_action_arr:Ptr[u32]' not in body and
        'schedule_raw:u32=ctrl0_arr[i]' not in body and
        'schedule_fu:u32=' not in body and
        'sched_ctrl_arr:Ptr[u32]' not in body
    )
    actual = 'actionpack precompute active' if ok else 'old schedule interpreter present or actionpack loads missing or old action arrays still present'
    checks.append(('source_schedule_precompute_actionpack', 'MAXD14 precompute actionpack active', actual, ok))
...
host_ok = (
    ...
    'self.sched_action_arr:List[u32]' in s and
    'gpu.raw(soa.sched_mask_arr)' in s and
    'gpu.raw(soa.sched_action_arr)' in s and
    'sched_action:u32=(terminal_is_base14&u32(1))|((root_action&u32(3))<<u32(1))' in s and
    'self.child_jmark_mask_arr' not in s and
    'self.future_check_mask_arr' not in s and
    'self.terminal_base14_arr' not in s and
    'self.root_action_arr' not in s and
    'sched_ctrl_arr' not in s
)
checks.append(('source_host_precompute_actionpack_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 255 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 255 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 255 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
# 255 r2 commandfix: this validation is intentionally fixed to the known N21 mode31
# argument vector.  A previous shell/package emitted `... 32 484 10 7 31 ...`,
# which shifted sort/preset/bench arguments and did not test split145 mode31.
CMD=("$CAND" -g 21 21 32 484 1 0 7 31 8 7 0 0 1 2)
CMD_EXPECTED=("$CAND" -g 21 21 32 484 1 0 7 31 8 7 0 0 1 2)
if [[ "${CMD[*]}" == "${CMD_EXPECTED[*]}" ]]; then
  printf 'command_arg_vector\t-g 21 21 32 484 1 0 7 31 8 7 0 0 1 2\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'command_arg_vector\t-g 21 21 32 484 1 0 7 31 8 7 0 0 1 2\t%s\tFAIL\n' "${CMD[*]}" >> "$SUMMARY"
  failures=$((failures+1))
fi
{
  echo "================================================================"
  ...
  echo "validation: one N=21 full run through 255 r2 schedule-precompute-actionpack mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "241parent:$BASELINE_241_SECONDS" "243precompute:$BASELINE_243_SECONDS" "239:$BASELINE_239_SECONDS" "242rejected:$BASELINE_242_REJECT_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 255 r2 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
