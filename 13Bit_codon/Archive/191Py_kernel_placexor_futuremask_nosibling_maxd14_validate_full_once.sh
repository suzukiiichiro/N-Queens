...
# =============================================================================
# NQ_UPDATE_MEMO
# 191: 更新メモ: 190は正当性OKだが188比で退行したため不採用。188を親に戻し、MAXD14 kernelのみfree候補bitがld/rd/colとdisjointである前提を使い、queen配置のORをXORへ置換する単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 191 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 191 starts from
# validated 188 because 188 remains the current fastest N=21 baseline: 189
# forced-chain and 190 block_check_mask guard were both correct but slower.
#
# Kernel arithmetic keeps 188 bitboard/schedule semantics: register-held current
...
# bytes/thread. The experiment is deliberately small: since each selected bit is
# taken from cur_avail = bm & ~(cur_ld|cur_rd|cur_col), it is disjoint from the
# active ld/rd/col board bits. Therefore (state|bit) and (state^bit) are
# equivalent for queen placement. 191 replaces only the MAXD14 placement ORs
# with XORs and keeps host task order, dispatch, cache, progress verification,
# and README-managed history unchanged.
SRC=${SRC:-./191Py_kernel_placexor_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./191Py_kernel_placexor_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/191Py_kernel_placexor_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 191 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/191Py_kernel_placexor_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 191Py source
#   has a harmless header/string/layout difference. This 191 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 188 futuremask/no-sibling MAXD14 dispatch; 189 forced-chain and 190 blockmask guard not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + disjoint placement XOR	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -Fq 'nld=((cur_ld^bit)<<stepu)|addvu|bLiu' "$SRC" \
  && grep -Fq 'nrd=((cur_rd^bit)>>stepu)|bKu' "$SRC" \
  && grep -Fq 'nld=(cur_ld^bit)<<u32(1)' "$SRC" \
  && grep -Fq 'nrd=(cur_rd^bit)>>u32(1)' "$SRC" \
  && grep -Fq 'ncol:u32=cur_col^bit' "$SRC"; then
  printf 'source_disjoint_place_xor	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_disjoint_place_xor	present	missing	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
if ! grep -q 'block_check_mask:u32' "$SRC" && ! grep -q 'if block_check_mask==u32(0):' "$SRC"; then
  printf 'source_190_block_guard_removed	absent	absent	OK
' >> "$SUMMARY"
else
  printf 'source_190_block_guard_removed	absent	present	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
if ! grep -q 'fast_nibble_op:u32=u32(0)' "$SRC" && ! grep -q 'fast_child_jmark:u32=' "$SRC" && ! grep -q 'if (cur_avail&(cur_avail-u32(1)))==u32(0):' "$SRC"; then
...
if grep -q 'split191' "$SRC" && ! grep -q 'split190' "$SRC" && ! grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split191 only	split191 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split191 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 191 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source place-xor static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 191 place-xor over 188 futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 191 keeps the validated 188 host/chunk structure, then derives chunk-tail diagnostics from
...
# 191: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 191 place-xor futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
