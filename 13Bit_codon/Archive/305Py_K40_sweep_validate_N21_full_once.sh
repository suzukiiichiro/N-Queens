...
# =============================================================================
# 305 N=21 full validation harness (K40-sweep probe)
...
# EXPECTED_CHUNKS=4, K=32 (unchanged from 296).
...
SRC=${SRC:-./305Py_K40_sweep_probe.py}
CAND=${CAND:-./305Py_K40_sweep_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/305Py_K40_sweep_probe_N21_full.lock}
...
EXPECTED_K_PER_THREAD_MAXD14=40
...
LOGDIR="${LOG_ROOT%/}/305Py_K40_sweep_logs_N21_full_once_${TS}"
...
echo "[start] 305 K40-sweep validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 305 K40-sweep SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '305 K40-sweep probe' "$SRC"; then
  printf 'source_version_tag	305 K40-sweep probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	305 K40-sweep probe	missing	FAIL
...
# 304: K48-sweep shape checks (296 kernel logic, K=48).
    ...
    checks.append(('source_K40_sweep_shape',
                    '296 kernel shape (stack/save_sp/next_depth/cur_depth/stack_ptr) + K=40',
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 305 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 305 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 305 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 305 K40-sweep mode31 split145"
...
echo "[validation-ok] 305 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=40 (4 dispatch/progress rows)"
