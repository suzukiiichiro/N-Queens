...
# 202 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 202 starts from
# validated 197 because 198/199/200/201 were correct but did not beat the
...
# root sibling changes: the saved avail payload uses root_second rather than
# cur_avail. Host task order, dispatch, cache, progress verification, and
...
#   - root-second-save source check is whitespace/comment robust.
SRC=${SRC:-./202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 202 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 202Py source
#   has a harmless header/string/layout difference. This 202 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 197 root one/two-candidate direct-if futuremask/no-sibling MAXD14 dispatch; 198 tailtest, 199 eqsecond, 200 prfuturebit, and 201 prjmarkdirect correct but not adopted; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root-preroll root_second save payload	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
checks = {
    ...
    'root_second_active_payload': 'cur_avail=root_second' in compact,
    'root_second_save_guard': 'ifroot_second!=u32(0):' in compact,
    'root_second_save_payload': 'avail[save_sp]=root_second|(u32(cur_depth)<<u32(27))' in compact,
    # 202 returns to 197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 202 returns to 197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_rootsecond_save_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootsecond_save	present_root_second_save_payload	present_root_second_save_payload	OK
' >> "$SUMMARY"
else
  printf 'source_rootsecond_save	present_root_second_save_payload	missing_or_old_payload	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split202'}
bad=sorted(tags-allowed)
if 'split202' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split202 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split202 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 202 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 202 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootsecondsave static checks failed" >&2
  exit 66
fi
...
echo "[validation-ok] 201 roottwo-prjmarkdirect futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
