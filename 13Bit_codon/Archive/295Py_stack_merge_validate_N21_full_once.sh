...
# =============================================================================
# 295 N=21 full validation harness (stack-merge probe)
...
#           schedule_words=0, stack=208 bytes/thread (unchanged: MAXD14_ANCESTOR*2
#           x 8B = 208 bytes), K_PER_THREAD_MAXD14=32 -> 5 dispatch/progress rows.
...
SRC=${SRC:-./295Py_stack_merge_probe.py}
CAND=${CAND:-./295Py_stack_merge_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/295Py_stack_merge_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/295Py_stack_merge_logs_N21_full_once_${TS}"
...
echo "[start] 295 stack-merge validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 295 stack-merge SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '295 stack-merge probe' "$SRC"; then
  printf 'source_version_tag	295 stack-merge probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	295 stack-merge probe	missing	FAIL
...
# 295: stack-merge shape checks (replaces 294 colav-ldrd checks).
    ok_stride_param = 'stride:int,' in body14.split(')->None:')[0]
    ok_stack_array = 'stack=__array__[u64](MAXD14_ANCESTOR*2)' in body14
    ok_no_ldrd_array = 'ldrd=__array__' not in body14
    ok_no_colav_array = 'colav=__array__' not in body14
    ...
    ok_push = body14.count('stack[sp2]=u64(cur_ld)') >= 1 and body14.count('stack[sp2+1]=u64(cur_col)') >= 1
    ok_pop = 'packed_ldrd:u64=stack[sp2]' in body14 and 'packed_colav:u64=stack[sp2+1]' in body14
    ok_sp2 = body14.count('sp2:int=save_sp*2') >= 1
    ok_stack_shape = (ok_stride_param and ok_stack_array and ok_no_ldrd_array and ok_no_colav_array
                       and ok_no_u32_stack and ok_gridstride_loop and ok_single_writeback
                       and ok_no_percall_writeback and ok_push and ok_pop and ok_sp2)
    checks.append(('source_stack_merge_shape',
                    'single stack u64 array (MAXD14_ANCESTOR*2) + interleaved sp2 indexing + stride + single writeback',
                    'present' if ok_stack_shape else (
                        'missing (stride=%s stack=%s no_ldrd=%s no_colav=%s no_u32=%s gs=%s wb=%s npc=%s push=%s pop=%s sp2=%s)'
                        % (ok_stride_param, ok_stack_array, ok_no_ldrd_array, ok_no_colav_array, ok_no_u32_stack,
                           ok_gridstride_loop, ok_single_writeback, ok_no_percall_writeback, ok_push, ok_pop, ok_sp2)
                    ),
                    ok_stack_shape))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 295 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 295 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 295 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 295 stack-merge mode31 split145"
...
echo "[validation-ok] 295 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
