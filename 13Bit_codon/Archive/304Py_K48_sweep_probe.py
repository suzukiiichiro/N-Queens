...
まず結果を確認します。**303 = 658.105s — さらに悪化。** レジスタ中立置換でも −304s (−86%) 退行。

この結果は非常に重要です。`cur_depth` を `cur_depth_x4` に置換しても変わらず壊滅的で、むしろ 302 より悪い。これは **レジスタ圧迫が原因ではなかった** ことを示しています。`cur_depth*4` は NVPTX では既に `SHL 2` にコンパイルされており、真のボトルネックではなかった。むしろ `cur_depth_x4-32` の計算（符号付き引き算 → u32 変換）か、`cur_depth_x4>>2`（child_jmark の extra shift）が新たなレイテンシ源になっています。

**結論: cur_depth_x4 方向は完全に終了。296 が現在の最良点。**

次は根本的に異なる角度を試みます。## 303 結果まとめと 304 の考え方

**303 不採用**: 658.105s — **−304s (−86%) さらに悪化**。これで 301〜303 の3連続実験から `cur_depth*4` はすでに NVPTX で `SHL 2` にコンパイルされており真の MUL ではないことが確定しました。**cur_depth_x4 方向は完全終了。**
...
296 の成功は `save_sp*2` が **真の IMUL** だったことによるもので、他の `*4` や `*2` はすでに SHL としてコンパイル済みでした。

## 304 の着眼点: K=48 未測定点

K=32(353.671s)と K=64(353.896s, flat)の間の **K=48** がまだ未測定です。296 カーネルロジックは完全無変更、定数1行のみ変更。CHUNKS=3。

```bash
STATIC_ONLY=1 bash 304Py_K48_sweep_validate_N21_full_once.sh
bash 304Py_K48_sweep_validate_N21_full_once.sh
```
"""
...
VERSION_TAG:str="304 K48-sweep probe: parent 303 cur-depth-x4-neutral (rejected -304s; cur_depth*4 is already SHL on GPU, no multiply to remove); back to 296 base (353.671s); single change: K_PER_THREAD_MAXD14 32->48; K=32 and K=64 were both measured at ~353.7s (flat at K=64 in 299), K=48 as an intermediate point has not been tested; EXPECTED_CHUNKS=ceil(2025282/(32*484*48))=ceil(2025282/743424)=3; all kernel logic unchanged from 296; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
