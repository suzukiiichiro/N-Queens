...
# =============================================================================
# NQ_UPDATE_MEMO
# 186: 更新メモ: 185は正当性OKだが184比で低下したため不採用。最速184へ戻し、MAXD14 kernelのみ terminal_base14==0/1 でDFS loopを外側分岐し、terminal count hot pathからbase14分岐を外す単独実験。
...
185Pyログを確認しました。**正当性は問題ありません**。N=21 fullで `314666222712` 一致、131チャンク完走、全dispatchが `required_maxd=14 / selected_MAXD=14 / schedule_words=0 / stack_bytes_per_thread=208`、warning/errorなしです。

ただし速度は `0:07:24.682` で、184Pyの `0:07:15.635` より `9.047秒`、`2.077%` 遅くなっています。したがって、185Pyは「正当性OKだが不採用」とし、186Pyは185を親にせず、**184Pyを親に戻して**作成しました。

作成物はこちらです。

* [186Py_kernel_terminalbase14_split_nosibling_maxd14_probe.py](sandbox:/mnt/data/186Py_kernel_terminalbase14_split_nosibling_maxd14_probe.py)
* [186Py_kernel_terminalbase14_split_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/186Py_kernel_terminalbase14_split_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/186Py_kernel_terminalbase14_split_nosibling_maxd14_bundle_20260630.tar)

186Pyの変更点は、184Pyの no-sibling spill elision / tail-call descent を維持したまま、MAXD14 kernelの `terminal_base14` 判定をDFS loop外側へ分離した点です。

```text
184Py:
  terminal hitごとに terminal_base14 を判定

186Py:
  loop外側で terminal_base14==0 / !=0 に分岐
  通常base path: terminal hit は total += 1
  base14 path  : terminal hit は nf & ~1 を見る
```

185Pyで入れた `cur_depth==13` literal fast pathは、速度退行したため186Pyには入れていません。186Pyは184Py親です。

手元では以下を確認済みです。

```text
bash -n 186Py...sh                          OK
STATIC_ONLY=1 186Py...sh                    OK
source_terminalbase14_outer_split            OK
source_terminal_paths normal/base14          OK
source_terminal13_literal_removed            OK
source_split_tag split186 only               OK
abstract terminalbase14 split equivalence    10000 cases OK
```

こちらではCodon/CUDA実機build/full実行はしていません。186Pyの採用可否は、添付の `186Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に184比のtimingを見て判断してください。



"""

# 186 terminal-base14 split revision memo over validated 184 no-sibling MAXD14 baseline:
#   Branch from the validated 184 no-sibling tail-call build.  Host-side depth
...
#   only restorable ancestor frames.  184 changed only the MAXD14 descend policy:
#   after selecting a bit, if the parent has no remaining sibling candidates,
...
#   four u32[13] arrays = 208 bytes/thread.
#
#   186 changes only the terminal count shape: after deterministic schedule
#   generation, branch once on terminal_base14 and run a base or base14 DFS loop.
#   This removes the per-terminal-hit base/base14 branch from the hot path while
#   leaving the 184 no-sibling descend semantics unchanged.  The supported GPU
...
# 186 keeps the validated 184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 186 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="186 terminal-base14 split: 184 no-sibling MAXD14 baseline plus terminal count loop split - README-managed history"
...
# 186 inherits the exact 184/183/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """186 MAXD14 nibble schedule plus no-sibling spill elision and terminal_base14 split."""
    ...
    # 186 keeps the validated 184 current-frame register cache, no-sibling descend, and 13-slot
    # ancestor storage, but separates logical schedule depth from the saved
    ...
    # 186 splits the terminal counting loop by terminal_base14.  The 184
    # no-sibling descend policy is unchanged, but the hottest terminal-hit path
    # no longer branches on terminal_base14 for every leaf.  The outer branch is
    # per task after deterministic schedule generation.
...
####################################################################################################
# 186 terminal_base14-split MAXD14 + scalar-u32pair dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="186 terminal-base14 split reuses validated 184 scorestripe_v9 task order/cache and changes only MAXD14 terminal count loop shape"
...
def main()->None:
  ...
  # 186 branches from validated 184.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 186 terminal-base14 split GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split186_terminalbase14_split_nosibling_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split186_terminalbase14_split_nosibling_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split186-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split186-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split186_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split186-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split186-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split186_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
