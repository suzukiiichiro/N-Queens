...
# =============================================================================
# 296 N=21 full validation harness (stack-ptr probe)
...
SRC=${SRC:-./296Py_stack_ptr_probe.py}
CAND=${CAND:-./296Py_stack_ptr_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/296Py_stack_ptr_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/296Py_stack_ptr_logs_N21_full_once_${TS}"
...
echo "[start] 296 stack-ptr validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 296 stack-ptr SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '296 stack-ptr probe' "$SRC"; then
  printf 'source_version_tag	296 stack-ptr probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	296 stack-ptr probe	missing	FAIL
...
# 296: stack-ptr shape checks.
    ...
    ok_no_sp2 = 'sp2' not in body14
    ok_stack_ptr_init = 'stack_ptr:int=0' in body14
    ...
    ok_stack_shape = (ok_stride_param and ok_stack_array and ok_no_ldrd_array and ok_no_colav_array
                       and ok_no_sp2 and ok_stack_ptr_init and ok_stack_ptr_incr and ok_stack_ptr_decr
                       and ok_gridstride_loop and ok_single_writeback and ok_no_percall_writeback
                       and ok_push and ok_pop)
    checks.append(('source_stack_ptr_shape',
                    'stack array + no sp2 multiply + stack_ptr counter (+2/-2) + stride + single writeback',
                    'present' if ok_stack_shape else (
                        'missing (stride=%s stack=%s no_sp2=%s ptr_init=%s ptr_incr=%s ptr_decr=%s gs=%s wb=%s push=%s pop=%s)'
                        % (ok_stride_param, ok_stack_array, ok_no_sp2, ok_stack_ptr_init, ok_stack_ptr_incr,
                           ok_stack_ptr_decr, ok_gridstride_loop, ok_single_writeback, ok_push, ok_pop)
                    ),
                    ok_stack_shape))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 296 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 296 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 296 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 296 stack-ptr mode31 split145"
...
echo "[validation-ok] 296 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
