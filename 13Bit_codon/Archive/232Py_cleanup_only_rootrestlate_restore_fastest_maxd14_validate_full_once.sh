...
# =============================================================================
# NQ_UPDATE_MEMO
# 232 validation: cleanup-only版。231/217相当のkernel探索ロジックを維持したまま、
#                 source履歴コメントと検証パラメータを整理した版を検証する。
# Expected: N=21 full once, split145/split232, required=14, MAXD14, schedule_words=0, stack=208.
# =============================================================================

SRC=${SRC:-./232Py_cleanup_only_rootrestlate_restore_fastest_maxd14_probe.py}
CAND=${CAND:-./232Py_cleanup_only_rootrestlate_restore_fastest_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/232Py_cleanup_only_rootrestlate_restore_fastest_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 232 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/232Py_cleanup_only_rootrestlate_restore_fastest_maxd14_logs_N21_full_once_${TS}"
...
static_failures=0
if grep -q 'VERSION_TAG:str="232 cleanup-only:' "$SRC"; then
  printf 'source_version_tag\t232 cleanup-only\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t232 cleanup-only\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
PYCHECK
then
  printf 'source_217_231_restore_shape\trootrestlate_restore_cleanup\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_217_231_restore_shape\trootrestlate_restore_cleanup\tmissing_or_changed\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split232'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split232' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit145 split232 active\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit145 split232 active\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if (( static_failures != 0 )); then
  echo "================================================================"; echo "[static-summary]"; cat "$SUMMARY"; echo "[logdir] $LOGDIR"
  echo "[error] 232 source static checks failed" >&2
  exit 66
fi
...
echo "[validation-ok] 232 cleanup-only reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
