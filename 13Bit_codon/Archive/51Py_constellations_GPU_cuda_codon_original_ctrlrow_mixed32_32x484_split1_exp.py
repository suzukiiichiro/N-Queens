...
childlow
-g -c g(GPU)  c(CPU)
Start/End     = 18 19 (Nを18と19の両方処理,18 18 は18のみ処理)
block      = 32
max_blocks = 484
steps      = 15488
log level  = 0(no log)  1(info)  2(debug)
sort_mode  = -1: auto（N=20 は 2、それ以外は 0）
             0 : sortなし
             1 : chunk内 functionid sort
             2 : chunk内 funcptn sort
             3 : chunk内 funcptn + work bucket sort（free popcount / depth）
preset_q   = 5 fixed. preset_queens=6 is disabled for public/default use.

$ codon build -release <filename>.py
$ ./<filename> -g 18 18 32 484 0 -1
$ ./<filename> -g 20 20 32 484 2 3      # sort_mode=3 診断
または
$ ./<filename> -g 18 19  N18とN19の両方を処理 32 484 0 -1 
======
...
amazon AWS g5.16xlarge x 1
suzuki@cudacodon$ date
2026年  4月 22日 水曜日 11:24:05 UTC
...
suzuki@cudacodon$ codon build -release 45Py_constellations_GPU_cuda_codon_original_ctrlrow_mixed32_32x512_sort.py
suzuki@cudacodon$ ./45Py_constellations_GPU_cuda_codon_original_ctrlrow_mixed32_32x512_sort -c
CPU mode selected
 N:             Total           Unique         hh:mm:ss.ms
 5:                10                0          0:00:00.000
 6:                 4                0          0:00:00.047    ok
 7:                40                0          0:00:00.000    ok
 8:                92                0          0:00:00.001    ok
 9:               352                0          0:00:00.018    ok
10:               724                0          0:00:00.004    ok
11:              2680                0          0:00:00.013    ok
12:             14200                0          0:00:00.022    ok
13:             73712                0          0:00:00.047    ok
14:            365596                0          0:00:00.090    ok
15:           2279184                0          0:00:00.170    ok
16:          14772512                0          0:00:00.299    ok
17:          95815104                0          0:00:00.468    ok
18:         666090624                0          0:00:02.216    ok
19:        4968057848                0          0:00:15.750    ok

suzuki@cudacodon$ codon build -release 45Py_constellations_GPU_cuda_codon_original_ctrlrow_mixed32_32x512_sort.py
suzuki@cudacodon$ ./45Py_constellations_GPU_cuda_codon_original_ctrlrow_mixed32_32x512_sort -g 5 19 32 512 0 0
GPU mode selected
 N:             Total           Unique         hh:mm:ss.ms
 5:                10                0          0:00:00.000
 6:                 4                0          0:00:00.053    ok
 7:                40                0          0:00:00.005    ok
 8:                92                0          0:00:00.005    ok
 9:               352                0          0:00:00.004    ok
10:               724                0          0:00:00.008    ok
11:              2680                0          0:00:00.011    ok
12:             14200                0          0:00:00.018    ok
13:             73712                0          0:00:00.022    ok
14:            365596                0          0:00:00.035    ok
15:           2279184                0          0:00:00.063    ok
16:          14772512                0          0:00:00.140    ok
17:          95815104                0          0:00:00.516    ok
18:         666090624                0          0:00:02.901    ok
19:        4968057848                0          0:00:21.792    ok
...
def should_split_task_gpu(split_mode:int, fid:int, ptn:int, free:int, row:int, endm:int)->bool:
  """GPU split experiment selector.
  split_mode=0: off
  split_mode=1: split funcptn=2 only
  split_mode=2: split funcptn=0 or 2
  """
  if split_mode<=0:
    return False
  depth:int=endm-row
  ...
  if split_mode==1:
    return ptn==2
  if split_mode==2:
    return ptn==0 or ptn==2
  return False

"""1段だけCPU側でfrontier splitする実験版。
split対象外はそのままコピーし、split対象はkernel_dfs_iter_gpu()の最初の1bit展開だけを
host側で行って子タスクとしてdstへ詰める。parent_mapは元constellation indexへ戻すために使う。
"""
def expand_split1_for_gpu(
  N:int,
  src:TaskSoA,
  src_w:List[u64],
  m:int,
  ...
  dst_w:List[u64],
  parent_map:List[int],
  funcptn_by_fid:List[int],
  meta_next_i:List[int],
  board_mask:int,
  n3:int,
  n4:int,
  split_mode:int,
  ...
  MASK_K_N3:int=185471169
  MASK_K_N4:int=4227088
  MASK_L_1:int=12689458
  MASK_L_2:int=17039488
  out:int=0
  split_tasks:int=0
  split_children:int=0
  overflow:int=0
  for i in range(m):
    fid:int=src.funcid_arr[i]
    ptn:int=funcptn_by_fid[fid]
    free:int=src.free_arr[i]&board_mask
    row:int=src.row_arr[i]
    endm:int=src.end_arr[i]
    do_split:bool=should_split_task_gpu(split_mode,fid,ptn,free,row,endm)
    if not do_split:
      if out<task_cap:
        dst.ld_arr[out]=src.ld_arr[i]
        dst.rd_arr[out]=src.rd_arr[i]
        dst.col_arr[out]=src.col_arr[i]
        dst.row_arr[out]=src.row_arr[i]
        dst.free_arr[out]=src.free_arr[i]
        dst.jmark_arr[out]=src.jmark_arr[i]
        dst.end_arr[out]=src.end_arr[i]
        dst.mark1_arr[out]=src.mark1_arr[i]
        dst.mark2_arr[out]=src.mark2_arr[i]
        dst.funcid_arr[out]=src.funcid_arr[i]
        dst.ijkl_arr[out]=src.ijkl_arr[i]
        dst_w[out]=src_w[i]
        parent_map[out]=base_off+i
        out+=1
      else:
        overflow+=1
      continue

    # kernel_dfs_iter_gpu() の初回 frame 初期化と同じことを host 側で1段だけ行う。
    ld:int=src.ld_arr[i]
    rd:int=src.rd_arr[i]
    col:int=src.col_arr[i]
    avail:int=free
    nextfv:int=fid
    stepv:int=1
    addv:int=0
    ...
    # split対象は funcptn 0/2 なので mark系だけだが、念のため汎用にしておく。
    if ptn==0 or ptn==1 or ptn==2:
      at_mark:int=0
      if ptn==1:
        if row==src.mark2_arr[i]:
          at_mark=1
      else:
        if row==src.mark1_arr[i]:
          at_mark=1
      if at_mark==1 and avail!=0:
        use_blocks=1
        stepv=3 if ptn==2 else 2
        addv=1 if fid==20 else 0
        nextfv=meta_next_i[fid]
    elif ptn==3:
      if row==src.jmark_arr[i]:
        avail&=~1
        ld|=1
        nextfv=meta_next_i[fid]
    elif ptn==5:
      # baseはsplit対象にしない。ここへ来た場合はそのままコピーにフォールバック。
      if out<task_cap:
        dst.ld_arr[out]=src.ld_arr[i]
        dst.rd_arr[out]=src.rd_arr[i]
        dst.col_arr[out]=src.col_arr[i]
    ...
    # 子タスクが作業領域を超えそうな場合は、部分的にsplitせず親タスクのまま流す。
    # 部分splitは二重計上/欠落の原因になるため禁止。
    need_est:int=popcount_int(avail)
    remain:int=m-i-1
    if out+need_est+remain>=task_cap:
      if out<task_cap:
        dst.ld_arr[out]=src.ld_arr[i]
        dst.rd_arr[out]=src.rd_arr[i]
        dst.col_arr[out]=src.col_arr[i]
    ...
    child_row:int=row+stepv
    if ptn==4:
      # availptn=1の通常系。split対象外のはずだが、保険としてfutureを維持。
      if child_row<endm:
        use_future=1
    ...
    while avail:
      ...
      nrd:int=0
      if use_blocks==1:
        bL:int=((MASK_L_1>>fid)&1)|(((MASK_L_2>>fid)&1)<<1)
        bK:int=0
        if ((MASK_K_N3>>fid)&1)==1:
          bK=n3
        elif ((MASK_K_N4>>fid)&1)==1:
          bK=n4
        nld=((ld|bit)<<stepv)|addv|bL
        nrd=((rd|bit)>>stepv)|bK
      else:
        nld=(ld|bit)<<1
        nrd=(rd|bit)>>1
      ...
      if out<task_cap:
        dst.ld_arr[out]=nld
        dst.rd_arr[out]=nrd
        dst.col_arr[out]=ncol
        dst.row_arr[out]=child_row
        dst.free_arr[out]=nf
        dst.jmark_arr[out]=src.jmark_arr[i]
        dst.end_arr[out]=src.end_arr[i]
        dst.mark1_arr[out]=src.mark1_arr[i]
        dst.mark2_arr[out]=src.mark2_arr[i]
        dst.funcid_arr[out]=nextfv
        dst.ijkl_arr[out]=src.ijkl_arr[i]
        dst_w[out]=src_w[i]
        parent_map[out]=base_off+i
        out+=1
    ...
    split_tasks+=1
    split_children+=made
  return out,split_tasks,split_children,overflow
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,gpu_split_mode:int=0)->None:
  """
  機能:
  ...
  # sort_mode auto:
  #   互換性重視で従来値を維持。N=20 は sort_mode=2、それ以外は 0。
  #   新規診断の sort_mode=3 は明示指定で試す。
  if gpu_sort_mode < 0:
    gpu_sort_mode = 2 if N == 20 else 0
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk51={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} split_mode={gpu_split_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    soa:TaskSoA=TaskSoA(STEPS)
    # split実験では1 chunk内の子タスクが増えるため、作業領域だけ大きく取る。
    TASK_CAP:int=STEPS
    if gpu_split_mode>0:
      TASK_CAP=STEPS*8
    split_soa:TaskSoA=TaskSoA(TASK_CAP)
    split_w_arr:List[u64]=[u64(0)]*TASK_CAP
    parent_map:List[int]=[0]*TASK_CAP
    results:List[u64]=[u64(0)]*TASK_CAP
    # results_allはm_all
    ...
    # split_mode > 0 の場合は split 後の子タスク列に対して sort する。
    sort_soa:TaskSoA=TaskSoA(TASK_CAP)
    sort_w_arr:List[u64]=[u64(0)]*TASK_CAP
    idx_map:List[int]=[0]*TASK_CAP
    order:List[int]=[0]*TASK_CAP

    meta_next_i: List[int] = [1,2,3,3,2,6,2,2,0,4,5,7,13,14,14,14,17,14,14,20,21,21,21,25,21,21,26,26]
    ...
    while off < m_all:
      ...
      if gpu_log_level>=2:
        t1=datetime.now()
      src_soa:TaskSoA=soa
      src_w_arr:List[u64]=w_arr
      m_run:int=m
      split_tasks:int=0
      split_children:int=0
      split_overflow:int=0
      if gpu_split_mode>0:
        m_run,split_tasks,split_children,split_overflow=expand_split1_for_gpu(
          N,soa,w_arr,m,off,split_soa,split_w_arr,parent_map,
          funcptn_by_fid,meta_next_i,board_mask,n3,n4,gpu_split_mode,TASK_CAP
        )
        src_soa=split_soa
        src_w_arr=split_w_arr
      ...
      if use_sorted:
        ...
        for i in range(m_run):
          fid0:int=src_soa.funcid_arr[i]
          key:int=fid0
          if gpu_sort_mode==2:
            key=funcptn_by_fid[fid0]
          if gpu_sort_mode==3:
            ptn:int=funcptn_by_fid[fid0]
            depth:int=src_soa.end_arr[i]-src_soa.row_arr[i]
            if depth<0:
              depth=0
            pc:int=popcount_int(src_soa.free_arr[i])
            wb:int=0
            if pc>=3:
              wb+=1
            if depth>=12:
        ...
        for i in range(m_run):
          ...
          order[p]=i
          cur[key]+=1
        for p in range(m_run):
          q:int=order[p]
          sort_soa.ld_arr[p]=src_soa.ld_arr[q]
          sort_soa.rd_arr[p]=src_soa.rd_arr[q]
          sort_soa.col_arr[p]=src_soa.col_arr[q]
          sort_soa.row_arr[p]=src_soa.row_arr[q]
          sort_soa.free_arr[p]=src_soa.free_arr[q]
          sort_soa.jmark_arr[p]=src_soa.jmark_arr[q]
          sort_soa.end_arr[p]=src_soa.end_arr[q]
          sort_soa.mark1_arr[p]=src_soa.mark1_arr[q]
          sort_soa.mark2_arr[p]=src_soa.mark2_arr[q]
          sort_soa.funcid_arr[p]=src_soa.funcid_arr[q]
          sort_soa.ijkl_arr[p]=src_soa.ijkl_arr[q]
          sort_w_arr[p]=src_w_arr[q]
          if gpu_split_mode>0:
            idx_map[p]=parent_map[q]
          else:
            idx_map[p]=off+q
      if gpu_log_level>=2:
        ts1=datetime.now()
      GRID = (m_run + BLOCK - 1) // BLOCK
      if use_sorted:
        kernel_dfs_iter_gpu(
        ...
        kernel_dfs_iter_gpu(
          gpu.raw(src_soa.ld_arr), gpu.raw(src_soa.rd_arr), gpu.raw(src_soa.col_arr),
          gpu.raw(src_soa.row_arr), gpu.raw(src_soa.free_arr),
          gpu.raw(src_soa.jmark_arr), gpu.raw(src_soa.end_arr),
          gpu.raw(src_soa.mark1_arr), gpu.raw(src_soa.mark2_arr),
          gpu.raw(src_soa.funcid_arr), gpu.raw(src_w_arr),
          gpu.raw(meta_next),
          gpu.raw(results),
          m_run, board_mask,
          n3, n4,
      ...
      if gpu_split_mode>0:
        if use_sorted:
          ...
          for i in range(m_run):
            results_all[parent_map[i]] += results[i]
      else:
        if use_sorted:
          @par
          for i in range(m_run):
      ...
      if gpu_log_level>=2:
        t3=datetime.now()
        print(f"[gpu-chunk] N={N} chunk={chunks} off={off} m={m} run={m_run} grid={GRID} sort={gpu_sort_mode} split={gpu_split_mode} split_tasks={split_tasks} split_children={split_children} overflow={split_overflow} build={str(t1-t0)[:-3]} sort_time={str(ts1-ts0)[:-3]} kernel={str(t2-ts1)[:-3]} copy={str(t3-t2)[:-3]}")
      off += m
...
def gen_constellations(
  ...
  # 51 SPLIT1 EXP: return preset_queens itself.
...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  gpu_split_mode:int=0
  # 51 SPLIT1 EXP: preset_queens は 5 固定。46で 6 を試したところ N=20 が ng になったため、
  # 互換目的で末尾引数を受けても 5 に丸めます。
  ...
  if argc == 1:
    ...
    if argc >= 9:
      requested_preset:int=int(sys.argv[8])
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 51 SPLIT1 EXP; using 5")
      preset_queens_arg=5
    if argc >= 10:
      gpu_split_mode=int(sys.argv[9])
    if argc > 10:
      print("Too many arguments")
      print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_block gpu_max_blocks log_level sort_mode preset_queens split_mode]")
      return
  else:
    print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_block gpu_max_blocks log_level sort_mode]")
    return
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        exec_solutions(N,constellations,use_gpu,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,gpu_split_mode)
