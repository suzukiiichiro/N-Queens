...
# =============================================================================
# NQ_UPDATE_MEMO
# 204: 更新メモ: 203は正当性OKだが197比で遅いため不採用。197 root<=2 direct-if prerollを親に戻し、MAXD14 kernelのみroot_rest生成をXOR式からclear-lowbit式へ置き換える微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 204 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 204 starts from
# validated 197 because 198/199/200/201/202/203 were correct but did not beat the
...
# second-lowbit direct-if form. Only root_rest construction changes: instead of
# cur_avail ^ root_first, 204 uses cur_avail & (cur_avail - 1), which clears the
# same low bit. Host task order, dispatch, cache, progress verification, and
...
#   - root-clearrest source check is whitespace/comment robust.
SRC=${SRC:-./204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 204 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 204Py source
#   has a harmless header/string/layout difference. This 204 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 197 root one/two-candidate direct-if futuremask/no-sibling MAXD14 dispatch; 198 tailtest, 199 eqsecond, 200 prfuturebit, 201 prjmarkdirect, 202 rootsecondsave, and 203 rootdepth0save correct but not adopted; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit expression	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
checks = {
    ...
    # 204 returns to 197 save payload; 203 depth0-save is not adopted.
    ...
    # 204 returns to 197 payload source; 202 root_second payload is not adopted.
    ...
    # 204 returns to 197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 204 returns to 197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_rootclearrest_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootclearrest	present_rootclearrest_expression	present_rootclearrest_expression	OK
' >> "$SUMMARY"
else
  printf 'source_rootclearrest	present_rootclearrest_expression	missing_or_old_xor_rest	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split204'}
bad=sorted(tags-allowed)
if 'split204' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split204 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split204 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 204 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 204 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootclearrest static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 204 roottwo-rootclearrest futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 204 roottwo-rootclearrest futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
