...
作成しました。164Pyからの単独変更として、深さ別`ctrl_schedule`をDFS開始前に生成し、DFS中のchild分類と`ctrl`書込みを除去しています。

* [165Py_kernel_ctrl_schedule_precompute_probe.py](sandbox:/mnt/data/165Py_kernel_ctrl_schedule_precompute_probe.py)
* [165Py_kernel_ctrl_schedule_precompute_validate_full_once.sh](sandbox:/mnt/data/165Py_kernel_ctrl_schedule_precompute_validate_full_once.sh)

静的検査、schedule pack 1,032,192ケース、164対165のDFSモデル26,202ケース、検証シェルの131チャンク再生試験は通過済みです。Codon/CUDA実機実行は未実施です。


"""

# 165 GPU depth-indexed ctrl-schedule precomputation experiment:
#   Branch from the fully validated 164 child-frame push-initialization baseline.
#   Retain its u32 stack layout, root-pop termination, absent overflow guard,
#   host/order/cache pipeline, bitboard arithmetic, and solution arithmetic.
#   The sole structural experiment observes that function-id, row, mark/end
#   transitions are independent of the selected queen bit.  Each GPU thread
#   therefore builds one depth-indexed ctrl_schedule before entering DFS.
#   Child entry actions (ordinary, jmark, base, special base14) are encoded in
#   the parent schedule word.  The DFS hot loop reads ctrl_schedule[sp], never
#   classifies/repacks a child frame, and never writes a ctrl value on push.
#   ctrl_schedule remains u32[MAXD], so the five-array 420-byte static footprint
#   and the proven N=5..27 safety envelope are unchanged from validated 164.
...
# MAXD=21 exposes indices 0..20 and retains zero spare frames.  165 replaces
# the ctrl stack with a same-sized depth schedule and inherits 164/161's absent
# overflow guard, so GPU execution remains limited to this proven range.
MAXD:Static[int]=21

VERSION_TAG:str="165 depth-indexed ctrl schedule precompute over 164 child frame push initialization - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    引数（抜粋）:
      ...
      ctrl0_arr:
        ...
        165ではdepth schedule生成のseedとして一度だけ読みます。
      markctrl_arr:
    ...
    前提/不変条件:
      ...
      - 現行 dynamic preset の N<=27 では最大 stored frame index=20。
      - function-id/row/mark/endの制御遷移はqueenの選択bitに依存しない。
      - ctrl_schedule[d]はdepth dの展開metadataと、そのchild entry actionを保持する。
      - 165は164を親版とし、制御metadataの生成時点だけを変更する。
    ホットパス（ソース引用）:
      bit = a & -a
      avail[sp] = a ^ bit
    """
    ...
    # ctrl_schedule word layout:
    #   bits 0..4   : child/next fid for this frame expansion
    ...
    #   bits 19..20 : child entry action
    #                  0=ordinary, 1=jmark, 2=base, 3=base14
    # The child action is attached to its parent, so a terminal child never
    # needs ctrl_schedule[terminal_depth] and MAXD still indexes 0..20 only.
    # 165 keeps the same five u32[MAXD] arrays as 164 (420 bytes/thread), but
    # schedule words are generated once and are never written during DFS push.
    ld=__array__[u32](MAXD)
    rd=__array__[u32](MAXD)
    col=__array__[u32](MAXD)
    avail=__array__[u32](MAXD)
    ctrl_schedule=__array__[u32](MAXD)
    bm:u32=board_mask
    ...
    # ------------------------------------------------------------------
    # One-time depth schedule generation.
    #
    # frame_action describes entry into the frame currently being classified.
    # For depth>0 it is stored in the preceding (parent) schedule word.  A base
    # action terminates generation without writing a terminal schedule entry.
    # ------------------------------------------------------------------
    ...
    while True:
      ...
      # P5 is a same-row function transition and is folded into this frame.
      ...
      if schedule_isbu!=u32(0) and schedule_rowv==endm:
        ...
        schedule_addu:u32=u32(0)
        schedule_blocksu:u32=u32(0)
        schedule_bLiu:u32=u32(0)
        schedule_ktu:u32=u32(0)
        ...
        if schedule_ismu!=u32(0):
          ...
          if schedule_rowv==schedule_markv:
            schedule_blocksu=u32(1)
            schedule_bLiu=(
              ((MASK_L_1>>schedule_fu)&u32(1))
              |(((MASK_L_2>>schedule_fu)&u32(1))<<u32(1))
            )
            schedule_ktu=(
              ((MASK_K_N3>>schedule_fu)&u32(1))
              |(((MASK_K_N4>>schedule_fu)&u32(1))<<u32(1))
            )
            schedule_use_futureu=u32(0)
            schedule_stepv=u32(3) if ((STP3_MASK>>schedule_fu)&u32(1))!=u32(0) else u32(2)
            schedule_addu=u32(1) if schedule_fu==u32(20) else u32(0)
        ...
        frame_cv=(
          schedule_nextfidu
          |(schedule_child_rowu<<u32(5))
          |(schedule_stepv<<u32(10))
          |(schedule_addu<<u32(12))
          |(schedule_blocksu<<u32(13))
          |(schedule_fcvu<<u32(14))
          |(schedule_bLiu<<u32(15))
          |(schedule_ktu<<u32(17))
        )
    ...
    # Root has no parent schedule word, so apply its entry action once here.
    ...
    while True:
      ...
      # Child classification was completed in the one-time schedule pass.
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164/165: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 165 ctrl-schedule precomputation over validated 164 and the 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 164; 165 changes only GPU control-metadata generation placement
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 164 scorestripe_v9 task order/cache unchanged; 165 precomputes depth-indexed ctrl schedule"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  165 reuses the validated 164/162/161/160/159/158/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 165 experiment changes only GPU ctrl-schedule generation placement.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  165 host-side score-aware lanephase chunk shaping is reused unchanged from validated 164/162/161/160/159/158/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 165 experiment changes only GPU ctrl-schedule generation; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/165: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 165 branches from 164 and inherits the 161 safety envelope: the removed
  # kernel guard was proven unreachable only for dynamic-preset N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 165 ctrl-schedule-precompute GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split165_ctrl_schedule_precompute_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split165_ctrl_schedule_precompute_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split165-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split165-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split165_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split165-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split165-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split165_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
