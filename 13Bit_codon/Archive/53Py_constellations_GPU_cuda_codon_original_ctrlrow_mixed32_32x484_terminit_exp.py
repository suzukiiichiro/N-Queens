...
def kernel_dfs_iter_gpu(
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        # terminal init-count EXP:
        # 子フレームが base かつ endmark 到達なら、このフレームの残り候補を
        # ここで直接数える。52 のように hot path へ毎回 branch を足さず、
        # 初期化時だけ判定する。
        if child_row==endm and ((IS_BASE_MASK>>nextfv)&1)==1:
          a2:u32=avail[sp]
          bK2:int=0
          if use_blocks==1:
            if ktype==1:
              ...
              bK2=n4
          while a2!=u32(0):
            bit2:u32=a2&(u32(0)-a2)
            a2=a2^bit2
            bit2_i:int=int(bit2)
            if use_blocks==1:
              nld2:int=((ld[sp]|bit2_i)<<stepv)|addv|bLv
              nrd2:int=((rd[sp]|bit2_i)>>stepv)|bK2
            else:
              nld2:int=(ld[sp]|bit2_i)<<1
              nrd2:int=(rd[sp]|bit2_i)>>1
            ncol2:u32=col[sp]|bit2
            nf2:u32=bm&~(u32(nld2)|u32(nrd2)|ncol2)
            if nf2!=u32(0):
              if nextfv==14:
                if (nf2&~u32(1))!=u32(0):
                  total+=u64(1)
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk53={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 53 TERMINIT EXP; using 5")
      preset_queens_arg=5
    if argc > 9:
