...
# =============================================================================
# NQ_UPDATE_MEMO
# 230: 更新メモ: 229Py root-preroll normal-first は正当性OKだが217/225を上回らなかったため不採用。230Pyは225Py generic ncol-earlyを親に、MAXD14 root-preroll内だけ (cur_ld|pr_bit)/(cur_rd|pr_bit) を pr_placed_ld/pr_placed_rd として共通化する rootpr place-or 微差実験。generic loop、rootrestlate、futuremask、no-sibling、root one-two preroll は維持する。
...
230Py note:
  229Py was correct but did not beat 217/225, so this branch returns to the
  225 generic ncol-early shape and changes only the MAXD14 root-preroll
  placement expressions.  `cur_ld|pr_bit` and `cur_rd|pr_bit` are computed
  once as pr_placed_ld/pr_placed_rd and reused by both root-preroll block and
  normal paths.  The generic DFS loop, rootrestlate predicate, futuremask guard,
  no-sibling stack behavior, and host task order are unchanged.
...
229Pyは正当性OKですが、225Py/223Py/217Pyを上回らないため**不採用**です。

230Pyは229のroot-preroll normal-firstを撤回して225Py相当へ戻し、root-preroll内だけで `cur_ld|pr_bit` / `cur_rd|pr_bit` の共通化を試す版にしました。
...
# 230Py
pr_placed_ld = cur_ld | pr_bit
pr_placed_rd = cur_rd | pr_bit

if pr_block_code != u32(0):
    pr_nld = (pr_placed_ld << pr_stepu) | pr_addvu | pr_bLiu
    pr_nrd = (pr_placed_rd >> pr_stepu) | pr_bKu
else:
    pr_nld = pr_placed_ld << u32(1)
    pr_nrd = pr_placed_rd >> u32(1)
```

こちらでは `STATIC_ONLY=1` の静的確認までOKです。

```bash id="ur8l2x"
cd /home/suzuki/Github/N-Queens/13Bit_codon/Archive

STATIC_ONLY=1 bash 230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物です。

* [230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_probe.py)
* [230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_bundle_20260703.tar)

"""

# 230 generic-ncol-early/rootpr-placeor memo over validated 217 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="230 rootpr-placeor: 225 generic ncol-early plus root-preroll placed ld/rd common scalars - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    if root_after_second==u32(0):
      ...
      # 230 rootpr-placeor: root-preroll placement OR is common to the
      # block-specific path and the normal step1 path.  Keep 225 generic
      # ncol-early unchanged and only reuse these two root-preroll scalars.
      pr_placed_ld:u32=cur_ld|pr_bit
      pr_placed_rd:u32=cur_rd|pr_bit
      pr_nld:u32=u32(0)
      pr_nrd:u32=u32(0)
      if pr_block_code!=u32(0):
    ...
    while True:
      ...
      # 230 rootpr-placeor: ncol is independent of block_code and nld/nrd.
      # Compute it before the block-specific diagonal update to test whether
      # shorter scheduling/dependency shape helps without changing semantics.
...
CHUNKSHAPE148_DEFAULT_REASON:str="230 rootpr-placeor reuses validated 217 scorestripe_v9 task order/cache; MAXD14 scalar root_after_second predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split230_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split230_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split230-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split230-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split230_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split230-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split230-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split230_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
