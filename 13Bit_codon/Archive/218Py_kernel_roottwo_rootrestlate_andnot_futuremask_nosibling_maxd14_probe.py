...
# =============================================================================
# NQ_UPDATE_MEMO
# 217: 更新メモ: 216Py rootprblockdirect は正当性OKだが204最速を上回らなかったため不採用。218Pyは211Pyのroot_first遅延形を親に、MAXD14 kernelのみroot-preroll内の cur_avail=root_rest 代入をfirst-child検証後へ遅延する最後のroot-preroll微差実験。root_rest clear-lowbit / root_after_second XOR / futuremask / no-sibling / root one-two preroll は維持する。
...
218Py note:
  ...
  217Py became the new fastest baseline by delaying cur_avail=root_rest
  until the first root child has passed nf/future/terminal/jmark checks.
  218 keeps that rootrestlate structure and changes only the root_after_second
  scalar from XOR to and-not: root_rest & (~root_second).
...
"""
218Pyを作成しました。
方針通り、**217Py + and-not predicate** です。

```text id="5ziwka"
Parent: 217Py rootrestlate 新最速基準
218Py差分:
...
```text id="l2e18j"
bash -n 218Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 218Py...sh OK
source_rootrestlate_andnot            OK
source_split_tag split145 split218    OK
```

ファイルはこちらです。

* [218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_probe.py)
* [218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash id="qfmdnv"
STATIC_ONLY=1 AUTO_BUILD=0 bash 218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_validate_full_once.sh

bash 218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 218 rootrestlate_andnot memo over validated 217 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="218 roottwo-rootrestlate-andnot: 217 rootrestlate/204 rootclearrest futuremask/no-sibling MAXD14 baseline with root_after_second and-not predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """218 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate root-rest-late + and-not predicate preroll."""
    ...
    # correct but slower.  218 keeps 217's rootrestlate structure and late
    # root_first extraction, but changes only root_after_second from XOR to
    # and-not: root_rest & (~root_second).  If the first child dies or is
    # terminal, the generic loop still sees root_rest as the active root
    # remainder.  If the first child descends, the same root_rest is saved as
    # a sibling exactly as before.
...
CHUNKSHAPE148_DEFAULT_REASON:str="218 rootrestlate_andnot reuses validated 217/204 scorestripe_v9 task order/cache; MAXD14 keeps rootrestlate and uses root_after_second and-not predicate"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split218_roottwo_rootrestlate_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split218_roottwo_rootrestlate_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split218-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split218-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split218_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split218-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split218-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split218_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
