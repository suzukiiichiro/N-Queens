...
[93Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder.py](sandbox:/mnt/data/93Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder.py)

今回の93は、**92 measure2 ベース**で、GPU kernel / DFS / `exec_solutions()` の中身は触っていません。

追加した内容は以下です。

```text
bench_mode=14:
  funcid reorder simulation only
  GPUなし
  .bin を funcid risk bucket に分ける
  balanced reorder 後の .bin を作る
  progress_N21_6_stream_funcid_reorder_sim.tsv を出す

bench_mode=15:
  funcid reordered stream GPU
  GPU実行あり
  reorder後の .bin を既存GPU kernelへ流す
  progress_N21_6_stream_funcid_reorder.tsv を出す
```
...
まずは simulation-only からお願いします。

```bash
codon build -release 93Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder.py

./93Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder \
  -g 21 21 32 484 1 0 5 14
```

次に、simulation の偏りが薄まっていれば GPU 実行です。

```bash
./93Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder \
  -g 21 21 32 484 1 0 5 15
```
...
```text
constellations_N21_6_funcid_reorder_B.bin
constellations_N21_6_funcid_reorder_A.bin
constellations_N21_6_funcid_reorder_C.bin
constellations_N21_6_funcid_reorder_G.bin
constellations_N21_6_funcid_reorder_O.bin
constellations_N21_6_funcid_reorder.bin
```

確認ポイントはまずこれです。

```bash
head -1 progress_N21_6_stream_funcid_reorder_sim.tsv

awk -F'\t' '
NR==1 {
  for (i=1; i<=NF; i++) c[$i]=i
  print "chunk\tm\trisky_a\trisky_b\trisky_c\tgood\tother"
  next
}
{
  print $3, $5, $(c["risky_a_ratio"]), $(c["risky_b_ratio"]), $(c["risky_c_ratio"]), $(c["good_ratio"]), $(c["other_ratio"])
}
' progress_N21_6_stream_funcid_reorder_sim.tsv | head -30
```
...
こちらの環境ではCodon実ビルドはできませんでしたが、字句レベルの確認は通しています。過去添付の一部は期限切れ表示になっていますが、手元にある92ファイルをベースに作成しています。
...
VERSION_TAG:str="93 stream funcid risk balanced reorder GPU runner from 92 measure2"
CROSS_STRIPE_SAFE_DEFAULT:bool=False

# 93 FUNCID REORDER:
#   Based on 92 stream measure2. Kernel / DFS logic is intentionally unchanged.
#   Changes are limited to stream-side measurement/reordering:
#     - keep 92 measure2 normal path intact
#     - add funcid risk columns for A/B/C/good groups
#     - add bench_mode=14 simulation-only mode: build balanced reordered .bin and TSV, no GPU
#     - add bench_mode=15 reordered stream GPU mode: run GPU on balanced reordered .bin
#
#   Risk groups used by 93:
...
def funcid_reorder_bucket_fname(N:int,preset_queens:int,g:int)->str:
  return f"constellations_N{N}_{preset_queens}_funcid_reorder_{funcid_reorder_bucket_label(g)}.bin"

"""93 funcid reorder: reorder 済み bin 名。"""
def funcid_reorder_output_fname(N:int,preset_queens:int)->str:
  return f"constellations_N{N}_{preset_queens}_funcid_reorder.bin"
...
def build_funcid_reorder_bucket_bins(N:int,fname:str,preset_queens:int,BLOCK:int,MAX_BLOCKS:int,gpu_log_level:int=0)->List[int]:
  ...
  if gpu_log_level>=1:
    print(f"[funcid-reorder-bucket-config] N={N} bin={fname} steps={STEPS}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if gpu_log_level>=2:
        print(f"[funcid-reorder-bucket-chunk] chunk={chunk_index} m={m} B={len(bucket_b)} A={len(bucket_a)} C={len(bucket_c)} G={len(bucket_g)} O={len(bucket_o)}")
      chunk_index+=1

  if gpu_log_level>=1:
    total:int=counts[0]+counts[1]+counts[2]+counts[3]+counts[4]
    print(f"[funcid-reorder-bucket-summary] N={N} records={total} B={counts[0]} A={counts[1]} C={counts[2]} G={counts[3]} O={counts[4]}")
...
"""93 funcid reorder: bucket bin から balanced reorder bin を作り、simulation TSV を出す。"""
def build_funcid_reordered_bin(
  ...
  if counted_records!=total_records:
    print(f"[funcid-reorder-warning] bucket count mismatch: counted={counted_records} total_records={total_records}")
  ...
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_funcid_reorder_sim.tsv"
  ...
  if gpu_log_level>=1:
    print(f"[funcid-reorder-build-config] N={N} records={total_records} steps={STEPS} output={reorder_fname} progress={progress_fname}")
  ...
  while total_remaining>0:
    ...
    while len(chunk_constellations)<m_target:
      progressed:bool=False
      if qb>0:
        if append_one_constellation_from_file(fb,chunk_constellations):
          qb-=1
          rem_counts[0]-=1
          progressed=True
        else:
          qb=0
      if qg>0:
        if append_one_constellation_from_file(fg,chunk_constellations):
          qg-=1
          rem_counts[3]-=1
          progressed=True
        else:
          qg=0
      if qa>0:
        if append_one_constellation_from_file(fa,chunk_constellations):
          qa-=1
          rem_counts[1]-=1
          progressed=True
        else:
          qa=0
      if qo>0:
        if append_one_constellation_from_file(fo,chunk_constellations):
          qo-=1
          rem_counts[4]-=1
          progressed=True
        else:
          qo=0
      if qc>0:
        if append_one_constellation_from_file(fc,chunk_constellations):
          qc-=1
          rem_counts[2]-=1
          progressed=True
        else:
    ...
    if gpu_log_level>=2:
      print(f"[funcid-reorder-build-chunk] chunk={chunk_index} off={off} m={m} B={stats[18+26]+stats[18+27]} A={stats[18+19]+stats[18+22]+stats[18+23]+stats[18+24]} C={stats[18+20]+stats[18+21]} G={stats[18+0]+stats[18+4]+stats[18+5]+stats[18+12]+stats[18+16]+stats[18+17]+stats[18+18]}")
  ...
  if gpu_log_level>=1:
    print(f"[funcid-reorder-build-summary] N={N} records={reordered_records} chunks={chunk_index} output={reorder_fname} progress={progress_fname} valid={1 if validate_bin_file(reorder_fname) else 0}")
...
def exec_solutions_gpu_bin_stream_funcid_reorder(
  ...
  total_records:int=count_constellations_bin_records(fname)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_funcid_reorder.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[funcid-reorder-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if chunk_only else 0} progress={progress_fname} inner_log_level=0")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[funcid-reorder-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m
          chunk_index+=1
          continue
      ...
      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[funcid-reorder-gpu-chunk-start] N={N} chunk={chunk_index} off={off} m={m}")
      ...
      if gpu_log_level>=1:
        print(f"[funcid-reorder-gpu-chunk-end] N={N} chunk={chunk_index} off={off} m={m} elapsed={elapsed_text} elapsed_ms={elapsed_ms} chunk_total={chunk_total} gpu_total={gpu_total}")

      off+=m
      chunk_index+=1

  if gpu_log_level>=1:
    print(f"[funcid-reorder-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} progress={progress_fname}")
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if bench_mode<0 or (bench_mode>11 and bench_mode!=13 and bench_mode!=14 and bench_mode!=15):
  ...
  if use_gpu:
    ...
    if bench_mode==14:
      print(f"funcid_reorder_sim: mode={bench_mode} preset={preset_queens_arg}")
    if bench_mode==15:
      print(f"funcid_reorder_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if bench_mode==14:
      ...
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[funcid-reorder-sim-only] N={N} preset_queens={preset_queens} records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} valid={1 if validate_bin_file(reorder_fname) else 0}")
      print(f"{N:2d}:{0:18d}{0:17d}{text:>21s}    funcid-reorder-sim-only")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==15:
      ...
      print(f"[funcid-reorder-gpu-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue

    if use_gpu and N>=21 and not (bench_mode==8 or bench_mode==9 or bench_mode==10 or bench_mode==14 or bench_mode==15):
      ijkl_list,subconst_cache,stream_records,preset_queens,stream_fname=ensure_constellations_bin_stream(N,ijkl_list,subconst_cache,preset_queens,gpu_log_level)
