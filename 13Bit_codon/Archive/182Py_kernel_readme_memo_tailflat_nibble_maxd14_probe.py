...
# =============================================================================
# NQ_UPDATE_MEMO
# 182: 更新メモ: 181のN=21 full検証OK・0:07:51.068を受け、GPU kernelは181/180/175相当の4-bit nibble MAXD14のまま維持。ソース冒頭は当該版メモのみに戻し、全履歴はREADME.mdへ集約・追記する運用へ変更。
...
181Pyログを確認しました。**問題ありません。かなり良い結果です。**

```text
181Py : 0:07:51.068
175Py : 0:07:51.106
差分  : 181Pyが 0.038秒高速、約0.008%改善
180Py : 0:07:51.191
差分  : 181Pyが 0.123秒高速
```
...
```text
min / max       : 3368 ms @ chunk99 / 3965 ms @ chunk13
p50 / p90 / p95 : 3575 / 3732 / 3806 ms
top5 share      : 41/1000
top10 share     : 81/1000
```

したがって、**181Pyは実測上の最速値**として記録してよいです。ただしkernel本体は175/180相当なので、175・180・181は「同一系統の最速kernel基準」として扱うのが自然です。

今回の182Pyは、合意した運用に合わせて作りました。**GPU kernelは変更していません。**
182Pyの主目的は、ソース冒頭の履歴運用を整理することです。

```text
182Py:
  GPU kernel        : 181/180/175相当のまま
  MAXD14構成        : 4-bit nibble schedule
  current frame     : register保持
  ancestor stack    : 13段
  schedule_words    : 0
  stack bytes/thread: 208

運用変更:
  .py 冒頭には NQ_UPDATE_MEMO の当該版メモのみ
  .py 冒頭に過去一覧 NQ_PY_UPDATE_HISTORY は置かない
  全履歴一覧は README.md に追記・集約
```

作成したファイルはこちらです。

* [182Py_kernel_readme_memo_tailflat_nibble_maxd14_probe.py](sandbox:/mnt/data/182Py_kernel_readme_memo_tailflat_nibble_maxd14_probe.py)
* [182Py_kernel_readme_memo_tailflat_nibble_maxd14_validate_full_once.sh](sandbox:/mnt/data/182Py_kernel_readme_memo_tailflat_nibble_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

こちらでは `STATIC_ONLY=1` の静的・モデル検証を通しています。

```text
source_parent_baseline              validated 181/180/175 nibble MAXD14 dispatch OK
source_revision_memo                short per-version NQ_UPDATE_MEMO OK
source_no_full_history_header        NQ_PY_UPDATE_HISTORY absent OK
kernel_variant_count                5 / 5 OK
maxd14_nibble_schedule              OK
frame_opcode_equivalence            506604 cases OK
dfs_164_vs_182_nibble_equivalence   12000 cases OK
...
```bash
chmod +x 182Py_kernel_readme_memo_tailflat_nibble_maxd14_validate_full_once.sh
./182Py_kernel_readme_memo_tailflat_nibble_maxd14_validate_full_once.sh
```

182Pyはkernel不変なので、速度は181/180/175近辺になる想定です。今回から、履歴一覧はREADME.mdへ追記し、ソース冒頭は当該版の短い更新メモだけにします。


"""

# 182 README-managed revision memo over validated 175/180 nibble-schedule MAXD14:
...
# 182 keeps the validated 175/180 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 182 MAXD14 uses scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="182 README-memo tailflat: validated 175/180 nibble MAXD14 kernel unchanged - source header shortened, full history in README.md"
...
# 182 inherits the exact 172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """182 MAXD14 nibble schedule plus register-held current frame and 13-slot ancestor stack."""
    ...
    # 182 keeps the validated 172 current-frame register cache plus ancestor stack shrink.  The active
...
####################################################################################################
# 182 nibble-schedule MAXD14 + scalar-u32pair dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="182 README-memo tailflat reuses validated 175/180 nibble MAXD14 kernel and scorestripe_v9 task order/cache"
...
def main()->None:
  ...
  # 182 branches from validated 180/175.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 182 readme-memo tailflat GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split182_readme_memo_tailflat_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split182_readme_memo_tailflat_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split182-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split182-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split182_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split182-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split182-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split182_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
