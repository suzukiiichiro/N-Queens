...
# =============================================================================
# 244 r4 N=21 full validation harness
# Parent: 241 restore239-after-fid14split-reject.
# Experiment: keep single launch and retain 243 schedule precompute, but pack only
#             child_jmark_mask/future_check_mask into sched_mask_arr. Terminal/root_action stay separate
#             to isolate the PTX/JIT failure seen in 244 r1-r3 ctrl-pack attempts.
...
SRC=${SRC:-./244Py_schedule_precompute_maskpack_probe_r4.py}
CAND=${CAND:-./244Py_schedule_precompute_maskpack_probe_r4}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/244Py_schedule_precompute_maskpack_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/244Py_schedule_precompute_maskpack_logs_N21_full_once_${TS}"
...
echo "[start] 244 r4 schedule-precompute-maskpack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 244 r4 schedule-precompute-maskpack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '244 r4 schedule-precompute-maskpack probe' "$SRC"; then
  printf 'source_version_tag\t244 r4 schedule-precompute-maskpack probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t244 r4 schedule-precompute-maskpack probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if not m14:
    ...
    actual = 'maskpack precompute active' if ok else 'old schedule interpreter present or maskpack loads missing or ctrl-pack still present'
    checks.append(('source_schedule_precompute_maskpack', 'MAXD14 precompute maskpack active', actual, ok))
...
checks.append(('source_host_precompute_maskpack_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 243 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 243 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 244 r4 schedule-precompute-maskpack mode31 split145"
...
echo "[validation-ok] 243 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
