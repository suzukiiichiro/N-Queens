...
# =============================================================================
# 329 N=21 full validation harness (soa-adopt-sourcecounters-reanalysis)
# Base: 328 warr-soa-split-implement, run on real hardware and confirmed:
#   correctness 314666222712, timing 456.036s (-0.324% vs 327's 454.563s,
#   within noise -- no regression). 327 (warrloadsplit-verify) itself was
#   kernel-logic byte-for-byte identical to 319-325's unified hot loop.
#
# *** THIS REVISION CONTAINS NO KERNEL-LOGIC CHANGE. *** The kernel body,
#   the shared dispatcher, and every w_lo_arr/w_hi_arr read site are
#   BYTE-FOR-BYTE IDENTICAL to 328 (confirmed by diff scoped to only the
#   header docstring / VERSION_TAG / reason-string lines). This revision:
#   (1) formally ADOPTS 328 as the working baseline now that its N=21 run
#       has confirmed correctness and non-regressing timing on real
#       hardware, and
#   (2) reanalyzes 328's own `ncu --section SourceCounters` (sudo,
#       kernel_dfs_iter_gpu_maxd14, launch-skip/launch-count 1) dump
#       against the source, with no new hardware profiling required for
#       this analysis step (the existing 328_ncu_source.txt is reused).
#
# WHAT THE REANALYSIS FOUND:
#   (a) All 3 w_lo_arr/w_hi_arr LDG.E read sites in the real
#       kernel_dfs_iter_gpu_maxd14 kernel now carry L2 Theoretical
#       Sectors Global Excessive=0, matching 327's probe prediction
#       exactly, confirmed on real hardware for the first time.
#   (b) stall_branch_resolving (22.6% of all sampled stall cycles in this
#       profile, consistent with 319/322's ~19-23% figure) remains
#       concentrated at the SAME two PCs identified in 322 (an
#       unconditional BRA back-edge, and a BRA into a BSYNC reconvergence
#       point), accounting for 544629/831032 = 65.5% of
#       stall_branch_resolving samples (322's own figure: ~65.6%).
#       Expected and unremarkable: SoA touches no control flow.
#   (c) NEW characterization on top of 322's finding: both hotspot PCs
#       show Divergent Branches=0 (the branch instance itself is not
#       divergent) combined with Avg. Threads Executed of only 2-3 out
#       of 32 (one PC pair: 13/32). I.e. this is warp occupancy collapse
#       / tail effect (most lanes already finished their DFS subtree,
#       a few stragglers keep the warp alive at the shared back-edge/
#       BSYNC), not classic per-branch SIMT divergence. Does NOT change
#       the existing 保留 (on-hold) decision on Stall Branch Resolving --
#       a real fix would need warp-level dynamic work redistribution,
#       architecturally distinct from the 5 already-rejected kernel-
#       decomposition attempts (240/266-269/273/326), and is NOT
#       attempted in this revision.
#   (d) NEW minor finding, not previously noted: the kernel epilogue's 2
#       STG.E result-writeback instructions (thread_total -> results,
#       once per thread) carry L2 Theoretical Sectors Global
#       Excessive=1936 each (3872 total), unrelated to w_arr/SoA.
#       Executed only 484 times (15488 threads) vs the hot loop's
#       hundreds of billions of executions in this same profile --
#       judged negligible-impact, recorded only (Open Objectives item 4).
#
# THIS REVISION'S OWN FULL N=21 RUN IS A NO-CODE-CHANGE RECONFIRMATION,
#   not a test of new behavior; correctness (314666222712) and timing
#   within noise of 328's 456.036s are still checked per standard
#   discipline, but no regression is expected since nothing changed.
# MANDATORY VALIDATION ORDER (unchanged discipline):
#   1. STATIC_ONLY=1 first.
#   2. Full N=21 run. Confirm 314666222712 FIRST, before looking at
#      timing. Any deviation is an immediate abort (would indicate an
#      unintended diff crept in, since none is expected here).
#   3. Compare timing against 328's 456.036s / 327's 454.563s; expect
#      within noise given no kernel-logic change.
# EXPECTED_CHUNKS=3, K=48 (unchanged from 304).
# =============================================================================

SRC=${SRC:-./329Py_soa_adopt_sourcecounters_reanalysis.py}
CAND=${CAND:-./329Py_soa_adopt_sourcecounters_reanalysis}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/329Py_soa_adopt_sourcecounters_reanalysis_N21_full.lock}
...
# NOTE: 329 is kernel-logic identical to 328 (SoA w_lo_arr/w_hi_arr
# split across all 5 kernels + shared dispatcher, per 327's probe-
# verified fix). 328's own N=21 run confirmed correctness (314666222712)
# and 456.036s (-0.324% vs 327's 454.563s, within noise) -- 328 is
# hereby ADOPTED as the working baseline. 326 (future_check_mask
# in-kernel specialization) remains REJECTED: correctness OK but
# elapsed=517.563s, -13.7% slower than 319's 455.116s baseline -- the
# worst of 5 failed GPU-side specialization attempts. 328/456.036s is
# now the most recent confirmed-good timing baseline; this revision (329)
# makes no kernel-logic change and is expected to reproduce it within
# noise.
...
LOGDIR="${LOG_ROOT%/}/329Py_soa_adopt_sourcecounters_reanalysis_logs_N21_full_once_${TS}"
...
echo "[start] 329 soa-adopt-sourcecounters-reanalysis validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 329 soa-adopt-sourcecounters-reanalysis SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '329 soa-adopt-sourcecounters-reanalysis' "$SRC"; then
  printf 'source_version_tag	329 soa-adopt-sourcecounters-reanalysis	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	329 soa-adopt-sourcecounters-reanalysis	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 329 soa-adopt-sourcecounters-reanalysis (kernel-identical to 328) mode31 split145"
...
echo "[validation-ok] 329 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); kernel logic identical to 328 (w_lo_arr/w_hi_arr SoA split across all 5 kernels + shared dispatcher); 328 ADOPTED as baseline (314666222712 / 456.036s confirmed); this run is a no-code-change reconfirmation; if timing drifts materially outside noise of 328's 456.036s / 327's 454.563s, investigate before proceeding"
