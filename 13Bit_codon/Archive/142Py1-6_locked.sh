...
# 142 locked validation harness
# - Candidate: 142Py_scorestripe_chunkshape_probe
# - 142 keeps the validated 141 host-side permutation unchanged.
# - The sole experiment is kernel_dfs_iter_gpu local ld stack: int -> u32.
# - Independent repeat using the same fixed 141 partial totals.
# - No values are learned from the first 142 run: a repeatable kernel must reproduce all five
#   subtotals and the full N=21 total exactly.

BASE=${BASE:-./115Py_range_default_clean_cg_v2}
CAND=${CAND:-./142Py_scorestripe_chunkshape_probe}
RUN_FULL=${RUN_FULL:-1}
...
LOGDIR="142Py_logs_N21_locked_validation_${TS}"
...
echo "[CAND] ${CAND}"
echo "[RUN_FULL] ${RUN_FULL}"
echo "[141-fixed-values] 01=${EXP01} 02=${EXP02} 03=${EXP03} 04=${EXP04} 05=${EXP05} full=${FULL_TOTAL}"

run_case \
  "01_cand142_mode30_standard_7chunk" \
  "partial_total=${EXP01}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "0,20,40,60,80,100,120"

run_case \
  "02_cand142_mode30_heavy_band" \
  "partial_total=${EXP02}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "35,40,41,42,47,48,52,53"

run_case \
  "03_cand142_mode30_d2base14_density" \
  "partial_total=${EXP03}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "20,40,55,56,57,58,60"

run_case \
  "04_cand142_mode30_late_tail" \
  "partial_total=${EXP04}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "100,105,110,115,120,125,130"

run_case \
  "05_cand142_mode31_worker0of4_quick" \
...
if [ "${RUN_FULL}" = "1" ]; then
  run_case \
    "06_cand142_mode31_full_correctness" \
    "${FULL_TOTAL}.*ok" \
    "$CAND" \
