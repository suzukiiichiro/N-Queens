...
作成しました。99Py は **Step 4: chunk size 実験** 用です。
97Py の `bench_mode=17` は残し、98Py では新しく **`bench_mode=18`** を追加しました。97Py の microbench は `executed_chunks=20`、`partial_total=27298463716`、`sum_chunk_elapsed=153.150 sec` まで確認できているので、その同じ20 chunkを profile 対象にしています。

ソースはこちらです。

[98Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_profile.py](sandbox:/mnt/data/98Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_profile.py)

差分はこちらです。

[97_to_98_profile.patch](sandbox:/mnt/data/97_to_98_profile.patch)
...
```text
mode17 = funcid-reorder-v2-microbench
```

に加えて、

```text
mode18 = funcid-reorder-v2-profile
```

を追加しました。

`mode18` は、選択 chunk ごとに以下を TSV へ追加します。
...
出力 TSV は full / microbench と衝突しないように別名です。

```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_profile.tsv
```
...
codon build -release \
  98Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_profile.py
```

## まず実行するコマンド

```bash
./98Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_profile \
  -g 22 22 32 484 1 0 7 18
```
...
既存の reordered bin があれば再利用します。

```text
[funcid-reorder-v2-profile-reuse]
```

実行開始時は以下のように出ます。

```text
funcid_reorder_v2_profile: mode=18 preset=7 chunk_start=0 chunk_count=1 chunk_list=0,8,94,...
funcid_reorder_v2_params: window_mult=8 phase_jump=7 param=w8_j7 reason=N22 measured best baseline w8_j7
```

chunkごとに、例えば以下のような stage 付きログが出ます。

```text
[funcid-reorder-v2-profile-chunk-end] ...
read_ms=...
soa_ms=...
stats_ms=...
sort_ms=...
...
```bash
PROG=progress_N22_7_stream_funcid_reorder_v2_w8_j7_profile.tsv
...
{
  rows++
  sum_elapsed += $(c["elapsed_ms"])
  sum_read    += $(c["stage_read_ms"])
  sum_soa     += $(c["stage_soa_ms"])
  sum_stats   += $(c["stage_stats_ms"])
  sum_sort    += $(c["stage_sort_ms"])
  sum_kernel  += $(c["stage_kernel_ms"])
  sum_reduce  += $(c["stage_reduce_ms"])
  sum_compute += $(c["stage_compute_ms"])
  sum_total   += $(c["stage_total_ms"])
  last_chunk=$(c["chunk"])
  last_total=$(c["gpu_total"])
}
END {
  print "rows=" rows
  print "last_chunk=" last_chunk
  print "partial_gpu_total=" last_total
  printf "sum_elapsed_ms=%.3f sec\n", sum_elapsed/1000.0
  printf "sum_read_ms=%.3f sec\n", sum_read/1000.0
  printf "sum_soa_ms=%.3f sec\n", sum_soa/1000.0
  printf "sum_stats_ms=%.3f sec\n", sum_stats/1000.0
  printf "sum_sort_ms=%.3f sec\n", sum_sort/1000.0
  printf "sum_kernel_ms=%.3f sec\n", sum_kernel/1000.0
  printf "sum_reduce_ms=%.3f sec\n", sum_reduce/1000.0
  printf "sum_compute_ms=%.3f sec\n", sum_compute/1000.0
  printf "sum_total_ms=%.3f sec\n", sum_total/1000.0
}
' "$PROG"
```
...
}
{
  print $(c["elapsed_ms"]), \
        "chunk=" $(c["chunk"]), \
        "total=" $(c["chunk_total"]), \
        "kernel=" $(c["stage_kernel_ms"]), \
        "reduce=" $(c["stage_reduce_ms"]), \
        "soa=" $(c["stage_soa_ms"]), \
        "risky_c=" $(c["risky_c_count"]), \
        "other=" $(c["other_count"]), \
        "f5=" $(c["funcid_5_count"]), \
        "f7=" $(c["funcid_7_count"]), \
        "f20=" $(c["funcid_20_count"]), \
        "f23=" $(c["funcid_23_count"])
}
' "$PROG" | sort -nr | head -20
```
...
`stage_kernel_ms` と `stage_reduce_ms` は、Codon/CUDA 側の同期仕様によっては完全には分離できない可能性があります。GPU kernel が非同期で返る場合、実際の待ち時間が `results[]` を読む `stage_reduce_ms` 側に出ることがあります。なので最初は、

```text
stage_kernel_ms + stage_reduce_ms
```

を GPU 本体側の重さとして見てください。

今回の98Pyは kernel / DFS 本体は変えず、**計測ラッパーと mode18 追加だけ**にしています。実機ビルドは未確認なので、まず `codon build -release` の結果を見たいです。


原因はここです。

```python
f.seek(off_record*16)
```

Codon の `seek()` は Python のような1引数形式ではなく、明示的に `whence` が必要です。
したがって、
...
に直す必要があります。`0` はファイル先頭基準、つまり `SEEK_SET` です。99Pyで追加した `read_constellations_bin_range()` の範囲読み込み部分が原因でした。

修正版を作成しました。

[99Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_chunksize_fix1.py](sandbox:/mnt/data/99Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_chunksize_fix1.py)

差分はこちらです。

[99Py_chunksize_seek_fix1.patch](sandbox:/mnt/data/99Py_chunksize_seek_fix1.patch)

修正差分はこれだけです。

```diff
-    f.seek(off_record*16)
+    # Codon file.seek() requires explicit whence. 0 = SEEK_SET.
+    f.seek(off_record*16,0)
```

手元のサーバーで直接直すなら、以下でも大丈夫です。
...
p = Path("99Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_chunksize.py")
s = p.read_text()

old = "    f.seek(off_record*16)\n"
new = "    # Codon file.seek() requires explicit whence. 0 = SEEK_SET.\n    f.seek(off_record*16,0)\n"
...
```bash
codon build -release \
  99Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_chunksize.py
```

まず smoke test はこれでよいです。

```bash
./99Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_chunksize \
  -g 22 22 32 484 1 0 7 19 8 7 0 1222 1 "-" "1"
```

これが通れば、次に重い候補だけの `1,2,4` 比較へ進めます。

```bash
./99Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_chunksize \
  -g 22 22 32 484 1 0 7 19 8 7 0 0 1 "94,1587,1625,1644,1772,1854" "1,2,4"
```

確認しました。**Step 4 は完了でよいです。結論は「chunk size 拡大は採用しない」です。**

今回の 99Py は `factor_list=1,2,4`、対象 chunk は `94,1587,1625,1644,1772,1854`、合計18ケースで完走し、`partial_total=60707333966`、実行時間 `0:10:08.931` でした。
...
```text
factor=1  records=81723   elapsed= 58.408 sec  kernel= 58.402 sec  kernel_ms_per_record=0.714634
factor=2  records=159163  elapsed=155.028 sec  kernel=155.015 sec  kernel_ms_per_record=0.973939
factor=4  records=314043  elapsed=395.172 sec  kernel=395.145 sec  kernel_ms_per_record=1.258251
```

`factor=1 → 2 → 4` と増やすほど、**recordあたり kernel時間が悪化**しています。

## 結論

現時点では、chunk size はこのまま固定がよいです。
...
`m=30976` や `m=61952` は、少なくとも今回の重い代表chunkでは速くなりません。むしろ悪化しています。

## なぜ悪化したか

今回の結果を見る限り、chunk拡大で kernel launch 固定費を薄める効果はほぼ出ていません。
逆に、1回の kernel に複数chunkぶんの重い経路を混ぜることで、GPU内の tail latency が大きくなっているように見えます。

特に factor=4 では重いケースがかなり伸びています。

```text
factor=4 base_chunk=1772 range=1772-1775 kernel=85355 ms
factor=4 base_chunk=1587 range=1587-1590 kernel=78630 ms
factor=4 base_chunk=1644 range=1644-1647 kernel=77065 ms
factor=4 base_chunk=94   range=94-97   kernel=76183 ms
factor=4 base_chunk=1625 range=1625-1628 kernel=70689 ms
```

このあたりは、単に records 数が増えたというより、**重い funcid 群を同じ kernel に入れた時の待ち合わせ・分岐ばらつき・長いDFS thread の影響**が強く出ている可能性があります。

## 末尾 chunk 1854 の確認

chunk 1854 は factor を変えても実 m が 4283 のままなので、当然ほぼ同じです。

```text
factor=1  kernel=7221 ms
factor=2  kernel=7222 ms
factor=4  kernel=7223 ms
```

これは 99Py の range 処理が正常に動いている確認にもなります。`steps` は 15488 / 30976 / 61952 に変わっていますが、末尾では読み込める record が 4283 しかないため、実行対象は同じです。

## funcid 観点で見えたこと

Step 5 へ進む根拠がさらに強くなりました。

factor=4 の重いケースでは、`funcid_5` と `funcid_23` がかなり目立ちます。

```text
factor=4 base_chunk=1772  f5=12197  f7=4876  f20=0     f23=11583
factor=4 base_chunk=1587  f5=21597  f7=3245  f20=3656  f23=11347
factor=4 base_chunk=1644  f5=11673  f7=12    f20=3656  f23=10055
factor=4 base_chunk=94    f5=17056  f7=2749  f20=3652  f23=9939
factor=4 base_chunk=1625  f5=28036  f7=153   f20=3656  f23=2623
```

一方で、`f5` が最大の `base_chunk=1625` が factor=4 の最遅ではなく、`f23` や `f7` が多い `base_chunk=1772` が最遅です。したがって、**funcid_5 だけを犯人にするのは危険**で、次は `funcid_5 / funcid_7 / funcid_23` の組み合わせ、特に `funcid_23` をかなり重く見たいです。

## Step 5 の方針

次は chunk size ではなく、**funcid別の重い経路の分離**へ進むのが正解です。

まず 100Py では、kernel ロジックをいきなり変えず、診断専用で以下を入れるのがよいです。
...
目的:
  selected chunks を funcid group 別に抽出して実行時間を見る

候補 group:
  group=5       funcid_5 only
  group=7       funcid_7 only
  group=23      funcid_23 only
  group=5,23    funcid_5 + funcid_23
  group=7,23    funcid_7 + funcid_23
  group=other   上記以外
  group=all     baseline
```
...
kernel_ms_per_chunk_total
funcid別 count
partial_total
```

これで、`funcid_5` が本当に重いのか、`funcid_23` が支配的なのか、あるいは mixed kernel 内の分岐混在が悪いのかを切り分けられます。
...
Step 4:
  chunk size拡大は不採用。
  baseline m=15488 のまま。

Step 5:
  funcid_5 / funcid_7 / funcid_23 を中心に診断。
  特に funcid_23 を重視。
```

次は **100Py: funcid別 profile** に進むのがよいです。
...
VERSION_TAG:str="99 chunksize auto w8_j7 stream funcid risk balanced reorder v2 GPU runner from 98"
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if bench_mode<0 or (bench_mode>11 and bench_mode!=13 and bench_mode!=14 and bench_mode!=15 and bench_mode!=16 and bench_mode!=17 and bench_mode!=18 and bench_mode!=19):
    ...
    if bench_mode>=8:
      ...
      preset_queens_arg=5
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19:
        if argc >= 14:
  ...
  if bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and not (bench_mode==8 or bench_mode==9 or bench_mode==10 or bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19):
      ijkl_list,subconst_cache,stream_records,preset_queens,stream_fname=ensure_constellations_bin_stream(N,ijkl_list,subconst_cache,preset_queens,gpu_log_level)
