...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 333 (2026-07-23)
...
5. [クローズ・333で正式採用] funcid_reorder w/jのN21再調整 → w3_j7採用
   ...
   不変。333の単発フルvalidateで確定する。

このセクション自体は通常カーネルロジックとは独立しています。
333はカーネルロジックの変更を含みません(カーネル・ディスパッチャは
328-332と同一)。ソースの変更は、ホスト側並べ替えパラメータの
デフォルト定数2箇所(WINDOW_MULT 8→3)とreason文字列、および
このドキュメント/VERSION_TAGのみです。
================================================================================
...
**333の内容(w3_j7正式採用)**
- 328以来初のソース変更ですが、**ホスト側パラメータのみ**: WINDOW_MULT両定数を8→3、reason文字列を「N21 measured best w3_j7」に更新。カーネル・ディスパッチャは328-332とバイト単位で同一(diffで確認済み)
- 検証は通常の単発フルvalidate形式に戻し、静的チェックに**w3/j7定数チェック3種を追加**(=3が2箇所/=8残存ゼロ/PHASE_JUMP=7/reason文字列)
- w/jはタスク集合を並べ替えるだけなので正当性は不変のはず — **もし総数がずれたら実バグとして即中断**、とスクリプトヘッダーに明記しました
...
VERSION_TAG:str="333 w3j7-adopt: FIRST source change since 328, but HOST-SIDE PARAMETER ONLY -- kernel logic and dispatcher remain BYTE-FOR-BYTE IDENTICAL to 328-332 (adopted SoA baseline). This revision formally adopts w3_j7 as the funcid_reorder default, replacing the rev94-99-era N22-tuned w8_j7: A10G_FINAL_DEFAULT_REORDER_WINDOW_MULT and FUNCID_REORDER_V2_WINDOW_MULT change 8->3 (PHASE_JUMP stays 7), plus the FUNCID_REORDER_V2_DEFAULT_REASON string. Basis: the three-stage sweep 330-332 (buried-idea C-3 from the 2026-07-23 archive survey). 330 (W in {4,6,8,10,12,16}, J=7) found W=4 as candidate (-0.523%); 331 (W in {2,3,4,5,8}) found W=3 as the interior optimum BRACKETED by worse neighbors W=2 (450840) and W=4 (452288), at kernel_reduce_ms 448789 = -1.294% vs W=8; 332 confirmed the J side ((3,3)/(3,5)/(3,11) all clearly worse than (3,7): 462440/458032/458036) and ruled out a low-side second minimum ((1,7)=454315, -0.08%, within noise). Metric reliability: the (8,7) control reproduced across FOUR sessions at 454672/454674/454673/454677 (spread 5ms = 0.001%), the (3,7) anchor reproduced to 2ms (448789 vs 448791), and the cached-bin elapsed predictions hit exactly (W=4: predicted ~453.6s, measured 453.623s; W=3: predicted ~450.1s, measured 450.113s). Expected result of this revision's own full N=21 validate: correctness 314666222712 unchanged (task REORDERING only -- the task set is identical, so any total deviation means a real bug and is an immediate abort), elapsed ~450.1s (~-1.3% vs the 456.0s w8_j7 baseline), kernel_reduce_ms sum ~448.8s. The w3_j7 reorder bins are already cached from 331/332, so no bin-generation cost applies. Unchanged: Codon-atomic question CLOSED (no device atomics; rev292 plan-C-1 impossible in pure Codon), rev84 CUDA C runner remains the mid-term route to the tail-effect fix (stall_branch_resolving 65.5% concentration, occupancy collapse per 329)"
