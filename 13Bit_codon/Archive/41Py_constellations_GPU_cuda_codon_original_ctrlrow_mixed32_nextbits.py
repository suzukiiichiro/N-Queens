...
def kernel_dfs_iter_gpu(
    ...
    NEXT0:Static[int]=257971887225801793
    NEXT1:Static[int]=925126487791057357
    NEXT2:Static[int]=879285
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=256,gpu_max_blocks:int=32,gpu_log_level:int=0)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 childlow=1 nextbits=1 block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
