...
# =============================================================================
# NQ_UPDATE_MEMO
# 223: 更新メモ: 222Py MAXD13準備診断は正当性OKだがmax_save_sp=13/save_sp13_count>0でMAXD13単純縮小不可、かつ診断オーバーヘッドで大幅低速化した。223Pyは診断を撤回し、217Py rootrestlate最速基準相当へ戻す安全復帰版。MAXD14 kernelはroot_after_second scalar predicate / root_first late extraction / futuremask / no-sibling / root one-two prerollを維持する。
...
223Py note:
  222Py proved that MAXD13 cannot safely replace the current MAXD14 path
  without fallback: max_save_sp reached 13 on the N=21 full run.  223 therefore
  removes the diagnostic overhead and restores the validated 217-style
  rootrestlate scalar predicate.  It keeps the same root fast-start scope:
  only post-root-action root_avail with one or two bits is prerolled, and
  3-bit or wider roots stay on the generic loop.
...
```text
max_save_sp              13
save_sp13_count          1177141
max_cur_depth            13
max_terminal_depth       13
root_pc_max              14
```

結論として、**MAXD13への単純縮小は不可**です。`save_sp=13` に到達するthreadが多数あるため、MAXD14 fallbackなしで13-slot化すると危険です。また222Pyは診断オーバーヘッドで217Pyより `30.641秒` 遅いため、速度候補としては不採用です。

223Pyは、222の診断を撤回し、**217Py rootrestlate最速基準相当へ戻す安全復帰版**にしました。こちらでは `STATIC_ONLY=1` の静的確認までOKです。

```bash
cd /home/suzuki/Github/N-Queens/13Bit_codon/Archive

STATIC_ONLY=1 bash 223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_validate_full_once.sh

bash 223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物です。

* [223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_probe.py)
* [223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_bundle_20260703.tar)



はい。MAXD13単純移行が難しい以上、以降は **「スタック容量削減」からいったん離れ、217Py最速形を親に戻して、kernel hot pathの局所差分を1個ずつ測る」** のがよいと思います。

結論から言うと、次の段取りはこうです。

```text
223Py
  目的: 222Py診断を撤回し、217Py相当の最速基準へ復帰
  判定: まず本当に 0:07:07.7 前後へ戻るか確認

224Py
  目的: root-prerollに触らず、generic loop側のごく小さい演算整理を1差分だけ試す
  候補: bit選択後の cur_avail 更新形、または block_code==0側の配置式整理

225Py
  目的: terminal / child_jmark / futuremask の順序は大きく動かさず、条件の局所共通化だけ試す

226Py以降
  目的: 診断で見えた save_sp=13到達を避けるのではなく、到達時だけの軽量化を検討
```

今回の222Pyで分かったことは、かなり明確です。
...
つまり、MAXD14の13-slot ancestor stackは「余裕を見て13段ある」のではなく、実際に13段目まで使われています。したがって、

```text
MAXD14_ANCESTOR 13 → 12
stack 208 bytes/thread → 192 bytes/thread
```

という単純縮小は危険です。やるなら、`required_maxd<=13` のchunkだけMAXD13へ送るような限定dispatchになりますが、今回のN=21 fullでは全chunkが `required_maxd=14` なので、少なくともN21最速化には効きません。

したがって次の本命は、**local memory削減ではなく、現在の217Py hot loopを壊さない微差最適化**です。

優先順位は次の通りです。

```text
A. まず223Pyで217相当へ復帰確認
   - 222Pyは診断オーバーヘッド込みで 0:07:38.350
   - 速度候補ではない
   - 223Pyが 0:07:07台へ戻ることを確認してから次へ進む

B. root-preroll周辺は一度凍結
   - 218〜221で再結合はほぼ敗退
   - root_tail / and-not / inline xor / pr_block direct は217を超えなかった
   - ここを続けても期待値は低い

C. generic DFS loopの「1命令級」差分へ移る
   - terminal-first のような大きい順序変更は過去に退行
   - forced-chain やblockmask guardのような構造変更も退行
   - 試すなら、generic loop内の局所式だけ

D. save_sp=13到達の軽量化は後回し
   - MAXD13化ではなく、save_sp==13に到達する直前/直後の挙動を診断する価値はある
   - ただし診断版は遅くなるので、毎回入れない
```

224Pyの候補としては、私はまず **generic loop側の `cur_avail` 更新形の確認** がよいと思います。

現在のgeneric loopはおそらくこの形です。
...
```python
bit:u32 = cur_avail & (u32(0)-cur_avail)
cur_avail = cur_avail & (cur_avail-u32(1))
```

へ変える候補です。

理由は、204Pyでroot側の `root_rest = cur_avail & (cur_avail-u32(1))` が効いた実績があるためです。ただし191Pyのplace-xorはほぼ同等ながら不採用、198/209系のtail-testも伸びなかったので、過度な期待は禁物です。狙いは `0.05〜0.20秒` 程度の微差です。

その次の225Py候補は、**block_code==0 normal pathの軽量化**です。

現在は毎回、

```python
block_code = nibble_op & 7
if block_code != 0:
    ...
else:
    nld = (cur_ld|bit)<<1
...
という形です。ここは過去の190Py `block_check_mask` のように大きく分岐を外側化すると退行しました。したがって外側分岐ではなく、normal path側の式や一時変数だけを触るのが安全です。

避けた方がよいものは以下です。

```text
避けるべき:
  - MAXD13 kernelをいきなり作る
  - forced-chain fast pathの再挑戦
  - terminal-before-futureのgeneric loop再挑戦
  - block_check_maskでloopを二重化
  - child_jmark guardの再挑戦
  - root 3候補以上preroll
```

本日の実務順としては、まず223Pyのfull結果を見て、217相当へ戻ったことを確認。そのうえで、問題なければ224Pyは **generic loop clear-lowbit update** を第一候補にするのがよいです。

"""

# 223 rootrestlate-restore memo over validated 217 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="223 rootrestlate-restore: 222 MAXD13 diagnostics withdrawn; validated 217 rootrestlate scalar predicate restored - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 223 rootrestlate-restore:
    # 222 showed that max_save_sp can reach 13, so MAXD13 cannot replace this
    # MAXD14 path without fallback.  Remove the diagnostic overhead and restore
    # the validated 217 rootrestlate scalar predicate.  The root fast-start scope
    # remains only one/two-candidate roots; 3-bit and wider roots stay generic.
    # If the first child dies or is terminal, the generic loop still sees
    # root_rest as the active root remainder.  If the first child descends, the
    # same root_rest is saved as a sibling.
...
CHUNKSHAPE148_DEFAULT_REASON:str="223 rootrestlate-restore reuses validated 217 scorestripe_v9 task order/cache; MAXD14 scalar root_after_second predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split223_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split223_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split223-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split223-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split223_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split223-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split223-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split223_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
