...
全項目 OK。では README.md を更新します。おはようございます。こちらこそよろしくお願いします。ChatGPT Pro解約、期待に応えられるよう全力でやります。
...
### 今回の実験の着眼点
292で K-batching (K=32) が −13.4% を達成しましたが、ncu の **`Stall Wait` ~48%** は K 数によらず不変でした。これは「warpレーン不均一（constellation間）」とは別の問題 — **1constellation内のホットループ命令依存チェーン** です。189番の教訓(内部ロジック書き換えで +108%)があるため、ホットループ内部には触れず、「**同一スレッドが2つのconstellationを同一スコープで保持し、コンパイラ/スケジューラに独立した命令ストリームを与える**」dual-lane アプローチで対処します。

### 293 の変更点（ソース構造）
- `kernel_dfs_iter_gpu_maxd14` に `DUAL_LANE_MAXD14:Static[int]=1` フラグ追加
- `DUAL_LANE_MAXD14!=0` のブランチ: 各パスで `idx`(laneA)と `idx+stride`(laneB)を処理、スタック配列は完全独立(ld_b / rd_b / col_b / avail_b)、`idx+=stride+stride` で2つ分進む
- `DUAL_LANE_MAXD14==0` のフォールバックブランチ: 292 の single-lane body を変数名リネームのみで完全保持（回帰時即切替可能）
- DFSロジック・nibble_op decode・root-preroll は**1行も変更なし**

### 既知リスク（実行前に把握）
| リスク | 根拠 |
|---|---|
| スタック footprint 倍増 (208→~416 bytes/thread) による L1/L2 hit率低下 | 292は near-100% hit率だったが per-thread 配列が倍に |
| コンパイラが2ストリームを直列扱いする可能性 | PTX/ncu 計測まで不明 |

### 今日の手順

**ステップ 1（静的チェック）**
```bash
STATIC_ONLY=1 bash 293Py_dual_lane_maxd14_validate_N21_full_once.sh
```
全項目 OK を机上では確認済みです。実機でも通ることを確認してください。

**ステップ 2（本番実行）**
```bash
bash 293Py_dual_lane_maxd14_validate_N21_full_once.sh
```
- `final total: 314666222712`、`error_or_mismatch_hits: 0` を確認
- `timing_vs_292k32confirmed` (ベースライン 367.413s) の delta/% を確認

**ステップ 3（採否分岐）**
- **改善の場合** → `ncu`(chunk 40, SpeedOfLight/WarpStateStats/SchedulerStats) で `Stall Wait` と L1/L2 hit率を計測し、設計仮説を定量記録
- **回帰の場合** → `DUAL_LANE_MAXD14=0` でビルドして292との等価性を確認してから不採用へ
- **正当性NG** → laneAのみ(`idx_b<m`ブランチを常に skip)するデバッグモードを追加して切り分け

結果が出たらログをお送りください。採否いずれも次の一手を一緒に考えます。
"""
...
# 293: single tuning knob for the dual-lane probe. 1 = new behavior (each thread
# processes 2 constellations per outer-loop pass, idx and idx+stride, back to
# back in program order, to give the compiler/scheduler independent instruction
# streams to interleave across -- targeting the ~48% "Stall Wait" (dependency-chain)
# cost measured on 292/279 ncu profiles). 0 = fall back to the byte-identical 292
# single-lane path (verified-safe baseline; use this if 293 regresses or misbehaves).
DUAL_LANE_MAXD14:Static[int]=1

VERSION_TAG:str="293 dual-lane-maxd14 probe: parent 292 kbatch-gridstride (K_PER_THREAD_MAXD14=32); kernel_dfs_iter_gpu_maxd14 outer grid-stride loop now processes 2 constellations per pass (idx and idx+stride) back to back in program order via a DUAL_LANE_MAXD14 static flag, each lane using its own independent stack arrays (ld/rd/col/avail vs ld_b/rd_b/col_b/avail_b), to expose independent instruction streams for the scheduler/compiler to interleave and hide the ~48% Stall Wait dependency-chain cost measured on 292 ncu profiles; DUAL_LANE_MAXD14=0 preserves a byte-identical copy of 292's single-lane body as a verified-safe fallback; hot-loop internals (schedule decode + iterative DFS logic itself) are NOT rewritten, only duplicated verbatim per lane via mechanical continue->break transform (see 293 notes); kernel_dfs_iter_gpu_maxd16/18/20/21, root-preroll, dispatch/task order, CPU dfs_iter path unchanged from 292"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 293 DUAL_LANE_MAXD14 probe: a second, independent set of stack arrays for
    # "lane B" so two constellations (idx and idx+stride) can be carried by the
    # same thread in the same outer-loop pass. Declared unconditionally so the
    # DUAL_LANE_MAXD14=0 fallback path below is a byte-identical copy of 292;
    # -release should dead-code-eliminate these when unused.
    ld_b=__array__[u32](MAXD14_ANCESTOR)
    rd_b=__array__[u32](MAXD14_ANCESTOR)
    col_b=__array__[u32](MAXD14_ANCESTOR)
    avail_b=__array__[u32](MAXD14_ANCESTOR)
    bm:u32=board_mask
    ...
    if DUAL_LANE_MAXD14!=0:
      thread_total:u64=u64(0)
      idx:int=tid
      while idx<m:
        idx_b:int=idx+stride
        lane_total:u64=u64(0)
        while True:
          ...
          lane_total=total*w_arr[idx]
          break
        thread_total+=lane_total
        if idx_b<m:
          lane_total_b:u64=u64(0)
          while True:
            markctrl_b:u32=markctrl_arr[idx_b]
            jmark_b:u32=markctrl_b&u32(31)
            endm_b:u32=(markctrl_b>>u32(5))&u32(31)
            mark1_b:u32=(markctrl_b>>u32(10))&u32(31)
            mark2_b:u32=(markctrl_b>>u32(15))&u32(31)
            total_b:u64=u64(0)

            root_ld_b:u32=ld_arr[idx_b]
            root_rd_b:u32=rd_arr[idx_b]
            root_col_b:u32=col_arr[idx_b]
            root_a_b:u32=free_arr[idx_b]&bm
            if root_a_b==u32(0):
              break

            schedule_raw_b:u32=ctrl0_arr[idx_b]
            schedule_depth_b:int=0
            schedule_lo_b:u32=u32(0)
            schedule_hi_b:u32=u32(0)
            child_jmark_mask_b:u32=u32(0)
            future_check_mask_b:u32=u32(0)
            terminal_parent_depth_b:int=0
            terminal_is_base14_b:u32=u32(0)
            root_action_b:u32=u32(0)
            while True:
              schedule_fu_b:u32=schedule_raw_b&u32(31)
              schedule_rowv_b:u32=(schedule_raw_b>>u32(5))&u32(31)

              if ((IS_P5_MASK>>schedule_fu_b)&u32(1))!=u32(0):
                if schedule_rowv_b==mark1_b:
                  schedule_fu_b=u32(meta_next[int(schedule_fu_b)])

              frame_action_b:u32=u32(0)
              frame_nibble_b:u32=u32(0)
              frame_raw_b:u32=u32(0)
              schedule_isbu_b:u32=(IS_BASE_MASK>>schedule_fu_b)&u32(1)
              if schedule_isbu_b!=u32(0) and schedule_rowv_b==endm_b:
                frame_action_b=u32(3) if schedule_fu_b==u32(14) else u32(2)
              else:
                schedule_ismu_b:u32=(IS_MARK_MASK>>schedule_fu_b)&u32(1)
                schedule_block_code_b:u32=u32(0)
                schedule_stepv_b:u32=u32(1)
                schedule_use_futureu_b:u32=u32(1)-schedule_ismu_b
                schedule_nextfidu_b:u32=schedule_fu_b

                if schedule_ismu_b!=u32(0):
                  schedule_markv_b:u32=mark2_b if ((SEL2_MASK>>schedule_fu_b)&u32(1))!=u32(0) else mark1_b
                  if schedule_rowv_b==schedule_markv_b:
                    schedule_block_code_b=(
                      ((BLOCK_CODE_B0_MASK>>schedule_fu_b)&u32(1))
                      |(((BLOCK_CODE_B1_MASK>>schedule_fu_b)&u32(1))<<u32(1))
                      |(((BLOCK_CODE_B2_MASK>>schedule_fu_b)&u32(1))<<u32(2))
                    )
                    schedule_stepv_b=u32(2)+((OP_STEP3_MASK>>schedule_block_code_b)&u32(1))
                    schedule_use_futureu_b=u32(0)
                    schedule_nextfidu_b=u32(meta_next[int(schedule_fu_b)])

                schedule_isju_b:u32=(IS_JMARK_MASK>>schedule_fu_b)&u32(1)
                if schedule_isju_b!=u32(0):
                  if schedule_rowv_b==jmark_b:
                    frame_action_b=u32(1)
                    schedule_nextfidu_b=u32(meta_next[int(schedule_fu_b)])

                schedule_child_rowu_b:u32=schedule_rowv_b+schedule_stepv_b
                schedule_fcvu_b:u32=u32(0)
                if schedule_use_futureu_b!=u32(0) and schedule_child_rowu_b<endm_b:
                  schedule_fcvu_b=u32(1)
                frame_nibble_b=schedule_block_code_b|(schedule_fcvu_b<<u32(3))
                frame_raw_b=schedule_nextfidu_b|(schedule_child_rowu_b<<u32(5))

              if schedule_depth_b==0:
                root_action_b=frame_action_b
              else:
                parent_depth_b:int=schedule_depth_b-1
                if frame_action_b==u32(1):
                  child_jmark_mask_b|=u32(1)<<u32(parent_depth_b)
                elif frame_action_b>=u32(2):
                  terminal_parent_depth_b=parent_depth_b
                  terminal_is_base14_b=u32(1) if frame_action_b==u32(3) else u32(0)

              if frame_action_b>=u32(2):
                break

              if schedule_fcvu_b!=u32(0):
                future_check_mask_b|=u32(1)<<u32(schedule_depth_b)

              if schedule_depth_b<8:
                schedule_lo_b|=frame_nibble_b<<u32(schedule_depth_b*4)
              else:
                schedule_hi_b|=frame_nibble_b<<u32((schedule_depth_b-8)*4)
              schedule_raw_b=frame_raw_b
              schedule_depth_b+=1

            if root_action_b==u32(2):
              lane_total_b=w_arr[idx_b]
              break
            if root_action_b==u32(3):
              total_b+=u64(1) if ((root_a_b&~u32(1))!=u32(0)) else u64(0)
              lane_total_b=total_b*w_arr[idx_b]
              break
            if root_action_b==u32(1):
              root_a_b&=~u32(1)
              if root_a_b==u32(0):
                break
              root_ld_b|=u32(1)

            terminal_depth_b:int=terminal_parent_depth_b
            terminal_base14_b:u32=terminal_is_base14_b

            save_sp_b:int=0
            cur_depth_b:int=0
            cur_ld_b:u32=root_ld_b
            cur_rd_b:u32=root_rd_b
            cur_col_b:u32=root_col_b
            cur_avail_b:u32=root_a_b

            root_rest_b:u32=cur_avail_b&(cur_avail_b-u32(1))
            root_second_b:u32=root_rest_b&(u32(0)-root_rest_b)
            root_after_second_b:u32=root_rest_b^root_second_b

            if root_after_second_b==u32(0):
              root_first_b:u32=cur_avail_b&(u32(0)-cur_avail_b)
              pr_nibble_op_b:u32=schedule_lo_b&u32(15)
              pr_block_code_b:u32=pr_nibble_op_b&u32(7)
              pr_bit_b:u32=root_first_b

              pr_nld_b:u32=u32(0)
              pr_nrd_b:u32=u32(0)
              if pr_block_code_b!=u32(0):
                pr_stepu_b:u32=u32(2)+((OP_STEP3_MASK>>pr_block_code_b)&u32(1))
                pr_addvu_b:u32=(OP_ADD1_MASK>>pr_block_code_b)&u32(1)
                pr_bLiu_b:u32=(
                  ((OP_BL1_MASK>>pr_block_code_b)&u32(1))
                  |(((OP_BL2_MASK>>pr_block_code_b)&u32(1))<<u32(1))
                )
                pr_ktu_b:u32=(
                  ((OP_KN3_MASK>>pr_block_code_b)&u32(1))
                  |(((OP_KN4_MASK>>pr_block_code_b)&u32(1))<<u32(1))
                )
                pr_bKu_b:u32=(n3&(u32(0)-(pr_ktu_b&u32(1))))|(n4&(u32(0)-(pr_ktu_b>>u32(1))))
                pr_nld_b=((cur_ld_b|pr_bit_b)<<pr_stepu_b)|pr_addvu_b|pr_bLiu_b
                pr_nrd_b=((cur_rd_b|pr_bit_b)>>pr_stepu_b)|pr_bKu_b
              else:
                pr_nld_b=(cur_ld_b|pr_bit_b)<<u32(1)
                pr_nrd_b=(cur_rd_b|pr_bit_b)>>u32(1)
              pr_ncol_b:u32=cur_col_b|pr_bit_b
              pr_nf_b:u32=bm&~(pr_nld_b|pr_nrd_b|pr_ncol_b)
              pr_descend_b:u32=u32(1)
              if pr_nf_b==u32(0):
                pr_descend_b=u32(0)
              if pr_descend_b!=u32(0):
                if future_check_mask_b!=u32(0):
                  if (pr_nibble_op_b&u32(8))!=u32(0):
                    if (bm&~((pr_nld_b<<u32(1))|(pr_nrd_b>>u32(1))|pr_ncol_b))==u32(0):
                      pr_descend_b=u32(0)

              if pr_descend_b!=u32(0):
                if terminal_depth_b==0:
                  if terminal_base14_b==u32(0):
                    total_b+=u64(1)
                  else:
                    total_b+=u64(1) if ((pr_nf_b&~u32(1))!=u32(0)) else u64(0)
                  pr_descend_b=u32(0)

              if pr_descend_b!=u32(0):
                pr_child_jmark_b:u32=child_jmark_mask_b&u32(1)
                if pr_child_jmark_b!=u32(0):
                  pr_nf_b&=~u32(1)
                  if pr_nf_b==u32(0):
                    pr_descend_b=u32(0)
                  else:
              ...
              cur_avail_b=root_rest_b
              if pr_descend_b!=u32(0):
                if cur_avail_b!=u32(0):
                  ld_b[save_sp_b]=cur_ld_b
                  rd_b[save_sp_b]=cur_rd_b
                  col_b[save_sp_b]=cur_col_b
                  avail_b[save_sp_b]=cur_avail_b|(u32(cur_depth_b)<<u32(27))
                  save_sp_b+=1
                cur_ld_b=pr_nld_b
                cur_rd_b=pr_nrd_b
                cur_col_b=pr_ncol_b
            ...
            while True:
              if cur_avail_b==u32(0):
                if save_sp_b==0:
                  break
                save_sp_b-=1
                cur_ld_b=ld_b[save_sp_b]
                cur_rd_b=rd_b[save_sp_b]
                cur_col_b=col_b[save_sp_b]
                saved_avail_b:u32=avail_b[save_sp_b]
                cur_avail_b=saved_avail_b&bm
                cur_depth_b=int(saved_avail_b>>u32(27))
                continue

              nibble_op_b:u32=u32(0)
              if cur_depth_b<8:
                nibble_op_b=(schedule_lo_b>>u32(cur_depth_b*4))&u32(15)
              else:
                nibble_op_b=(schedule_hi_b>>u32((cur_depth_b-8)*4))&u32(15)
              bit_b:u32=cur_avail_b&(u32(0)-cur_avail_b)
              cur_avail_b=cur_avail_b^bit_b

              # 291: keep_b 289 normal_b-default_b nld_b/nrd_b + ncol_b-only_b early_b, but_b delay_b
              # block_code_b scalar_b creation_b to_b the_b special_b branch_b only_b.
              # nf_b is_b still_b computed_b once_b after_b the_b branch_b; 288 nf_b-default_b is_b intentionally_b not used_b.
              nld_b:u32=(cur_ld_b|bit_b)<<u32(1)
              nrd_b:u32=(cur_rd_b|bit_b)>>u32(1)
              ncol_b:u32=cur_col_b|bit_b
              if (nibble_op_b&u32(7))!=u32(0):
                block_code_b:u32=nibble_op_b&u32(7)
                stepu_b:u32=u32(2)+((OP_STEP3_MASK>>block_code_b)&u32(1))
                addvu_b:u32=(OP_ADD1_MASK>>block_code_b)&u32(1)
                bLiu_b:u32=(
                  ((OP_BL1_MASK>>block_code_b)&u32(1))
                  |(((OP_BL2_MASK>>block_code_b)&u32(1))<<u32(1))
                )
                ktu_b:u32=(
                  ((OP_KN3_MASK>>block_code_b)&u32(1))
                  |(((OP_KN4_MASK>>block_code_b)&u32(1))<<u32(1))
                )
                bKu_b:u32=(n3&(u32(0)-(ktu_b&u32(1))))|(n4&(u32(0)-(ktu_b>>u32(1))))
                nld_b=((cur_ld_b|bit_b)<<stepu_b)|addvu_b|bLiu_b
                nrd_b=((cur_rd_b|bit_b)>>stepu_b)|bKu_b
              nf_b:u32=bm&~(nld_b|nrd_b|ncol_b)
              if nf_b==u32(0):
                continue
              if future_check_mask_b!=u32(0):
                if (nibble_op_b&u32(8))!=u32(0):
                  if (bm&~((nld_b<<u32(1))|(nrd_b>>u32(1))|ncol_b))==u32(0):
                    continue

              if cur_depth_b==terminal_depth_b:
                if terminal_base14_b==u32(0):
                  total_b+=u64(1)
                else:
                  total_b+=u64(1) if ((nf_b&~u32(1))!=u32(0)) else u64(0)
                continue

              child_jmark_b:u32=(child_jmark_mask_b>>u32(cur_depth_b))&u32(1)
              if child_jmark_b!=u32(0):
                nf_b&=~u32(1)
                if nf_b==u32(0):
                  continue
                nld_b|=u32(1)

              next_depth_b:int=cur_depth_b+1
              if cur_avail_b!=u32(0):
                ld_b[save_sp_b]=cur_ld_b
                rd_b[save_sp_b]=cur_rd_b
                col_b[save_sp_b]=cur_col_b
              ...
              cur_col_b=ncol_b
              cur_avail_b=nf_b
              cur_depth_b=next_depth_b
            lane_total_b=total_b*w_arr[idx_b]
            break
          thread_total+=lane_total_b
          idx+=stride+stride
        else:
          idx+=stride
      results[tid]=thread_total
    else:
      thread_total_f:u64=u64(0)
      idx_f:int=tid
      while idx_f<m:
        markctrl:u32=markctrl_arr[idx_f]
        jmark:u32=markctrl&u32(31)
        endm:u32=(markctrl>>u32(5))&u32(31)
        mark1:u32=(markctrl>>u32(10))&u32(31)
        mark2:u32=(markctrl>>u32(15))&u32(31)
        ...
        root_ld:u32=ld_arr[idx_f]
        root_rd:u32=rd_arr[idx_f]
        root_col:u32=col_arr[idx_f]
        root_a:u32=free_arr[idx_f]&bm
        if root_a==u32(0):
          idx_f+=stride
          continue

        schedule_raw:u32=ctrl0_arr[idx_f]
        schedule_depth:int=0
        schedule_lo:u32=u32(0)
        schedule_hi:u32=u32(0)
        child_jmark_mask:u32=u32(0)
        ...
        if root_action==u32(2):
          thread_total_f+=w_arr[idx_f]
          idx_f+=stride
          continue
        if root_action==u32(3):
          total+=u64(1) if ((root_a&~u32(1))!=u32(0)) else u64(0)
          thread_total_f+=total*w_arr[idx_f]
          idx_f+=stride
          continue
        if root_action==u32(1):
          root_a&=~u32(1)
