...
def kernel_dfs_iter_gpu(
    ...
    # mixed32+ldu32 ビットボード系。
    # rd は盤面外の高位ビットが後続の >> で盤面内へ入る可能性があるため int のまま保持する。
    # ld は左シフト方向にしか進まず盤面内へ戻らないため u32 化する。
    # col/avail/ctrl も盤面幅内または小さい制御値なので u32 化してローカルメモリ圧を下げる.
    ...
    #   bit  7      : add1
    #   bit  8      : use_blocks
    #   bit  9      : future check enabled
    #   bits 10..11 : blockL encoded value
    #   bits 12..13 : blockK type: 0=0, 1=n3, 2=n4
    #   bits 14..18 : child row after this frame transition
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        ctrl[sp]=u32(524288 | nextfv | (stepv<<5) | (addv<<7) | (use_blocks<<8) | (fcv<<9) | (bLv<<10) | (ktype<<12) | (child_row<<14))
      #----------------
      # 1bit 展開
      #----------------
      a=avail[sp]
      ...
      # push
      #----------------
      next_row:int=(cvi>>14)&31
      sp+=1
      if sp>=MAXD:
        results[i]=total*w_arr[i]
        return
      ctrl[sp]=u32((cvi&31) | (next_row<<5))
      ld[sp]=u32(nld)
      rd[sp]=nrd
      col[sp]=ncol
      avail[sp]=nf
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=256,gpu_max_blocks:int=32,gpu_log_level:int=0)->None:
  """
  機能:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 ldu32=1 block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    ...
    while off < m_all:
      ...
      if gpu_log_level>=2:
        t3=datetime.now()
        print(f"[gpu-chunk] N={N} chunk={chunks} off={off} m={m} grid={GRID} build={str(t1-t0)[:-3]} kernel={str(t2-t1)[:-3]} copy={str(t3-t2)[:-3]}")
      off += m
    if gpu_log_level>=1:
...
def main()->None:
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_block gpu_max_blocks log_level]")
      return
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        exec_solutions(N,constellations,use_gpu,gpu_block,gpu_max_blocks,gpu_log_level)
