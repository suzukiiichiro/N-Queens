...
# =============================================================================
# NQ_UPDATE_MEMO
# 197: 更新メモ: 196は正当性OKかつ194比0.049秒高速で数値上の新最速基準。196のroot<=2 second-lowbit preroll/future_check_mask guard/no-siblingを親に、MAXD14 kernelのみroot<=2判定のroot_preroll flagを削り、root_after_second==0 direct-ifでpreroll bodyへ入る微差実験を導入。
...
197Py note:
  196Py was correct and numerically faster than both 194 and 195.  This branch
  keeps the same validated MAXD14 root fast-start scope as 196: only
  ...
  predicate/control-flow shape: remove the scalar root_preroll flag and enter
  the preroll body directly under root_after_second==0.
...
"""
原因は確認できました。添付ログでは、197本体の主要チェックはすべてOKで、GPU実行前に `source_split_tag` だけがFAILして停止しています。

これは **197Py本体のkernel差分ではなく、`.sh` 側の静的検査が厳しすぎた問題**です。
197の `.py` には実行時に使う `split197` 以外にも、継承している `split145` 系のdispatch/progressスコープ文字列が残ります。これは正常です。r2の検査は、過去版・派生tag文字列の扱いがまだ硬く、cudacodon側で `source_split_tag` だけ落ちていました。
...
```text
source_split_tag 静的検査を修正

旧:
  split197 があり、split184..split196 がソース全体に無いことを要求

新:
  コメント・履歴文・ローカルメモ中の旧tagは無視
  progress_suffix / runtime print など、実行時に出るactive split tagだけを見る
  split197 がactiveならOK
  split184..split196 がactiveでなければOK
  継承元の split145 dispatch/progress scope は許可
```
...
`source_split_tag` はr3で以下になります。

```text
source_split_tag    split197 active preferred    split145 split197    OK
```

`split145` は既存のgeneric/full dispatch scope由来なので許可しています。197の実験tagは `split197` です。

こちらではCodon/CUDA full実行はしていません。cudacodon側では、3点とも上書きしてから再実行してください。

```bash
tar xf 197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_r3_bundle_20260701.tar

STATIC_ONLY=1 bash 197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_validate_full_once.sh
bash 197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_validate_full_once.sh
```

修正版はこちらです。

* [197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_probe.py)
* [197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [197 r3 3点まとめtar](sandbox:/mnt/data/197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_r3_bundle_20260701.tar)

"""


# 197 root<=2 direct-if memo over validated 196 root-two futuremask/no-sibling MAXD14:
#   196 was correct and 0.049 seconds faster than 194, so this branch adopts
#   196 as the new numerical baseline.  It keeps the validated root one/two-
...
#   only restorable ancestor frames.  197 keeps the narrow root fast-start only when
#   post-root-action root availability has one or two bits, matching the validated
#   196/194 behavior.  Three-candidate roots explicitly stay on the generic loop.
#   The root<=2 predicate is still the second-lowbit test, but 197 removes the
#   intermediate root_preroll flag and uses a direct if on root_after_second==0.
...
# 197 keeps the validated 196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 197 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="197 roottwo-directif: 196 root<=2 second-lowbit futuremask/no-sibling MAXD14 baseline with direct-if preroll predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 197 root_two_direct_if:
    # If the root frame has one or two candidates, process the first root
    # candidate before the generic DFS loop, matching the validated 196/194 scope.
    # 195's three-candidate case remains intentionally removed because it was
    # correct but slower.  The predicate is still the 196 second-lowbit test, but
    # 197 removes the root_preroll flag and enters the preroll body directly when
    # root_after_second==0.  Therefore 3-bit and wider roots stay on the generic loop.
...
CHUNKSHAPE148_DEFAULT_REASON:str="197 roottwo-directif reuses validated 196/194 scorestripe_v9 task order/cache; only MAXD14 root<=2 predicate control-flow shape changes"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split197_roottwo_directif_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split197_roottwo_directif_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split197-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split197-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split197_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      ...
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split197-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split197-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split197-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split197_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
