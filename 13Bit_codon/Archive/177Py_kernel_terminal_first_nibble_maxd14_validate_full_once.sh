...
# 177 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 177 inherits the
# validated fastest 175 nibble-schedule MAXD14 kernel.  It does not inherit the slower 176 current-nibble-register path.  Its single kernel
# experiment keeps the 172 register-held active frame, 13-slot ancestor stack,
# and 4-bit nibble op, but checks the terminal-parent frame before the
# future-prune branch in the hot loop.
#
# For N=21 all launches must remain required_maxd=14, select MAXD14, report
# schedule_words=0 and 208 local stack bytes/thread.

SRC=${SRC:-./177Py_kernel_terminal_first_nibble_maxd14_probe.py}
CAND=${CAND:-./177Py_kernel_terminal_first_nibble_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/177Py_kernel_terminal_first_nibble_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
BASELINE_176_SECONDS=${BASELINE_176_SECONDS:-539.402}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 177 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/177Py_kernel_terminal_first_nibble_maxd14_logs_N21_full_once_${TS}"
...
assert '177 terminal-first nibble-schedule GPU candidate supports N=5..27 only' in s
...
# schedule fields, the 172 register-held active frame, and the 177 terminal-first nibble schedule hot path.
...
terminal_pos=b14.index('if sp==terminal_depth:')
future_pos=b14.index('if (nibble_op&u32(8))!=u32(0):')
assert terminal_pos < future_pos, (terminal_pos, future_pos)
assert '177 terminal-first' in b14
...
def nibble_from_byte(op):
    # 177 keeps the byte-opcode low nibble directly:
            ...
            for l in range(N):
                ...
                if d<=21:
                    ...
                    if d==14:
                        ...
                        terminal_depth_for_case=scalar_terminal_at(sched)
                        assert terminal_depth_for_case==13,terminal_depth_for_case
                        terminal_nib=scalar_op_at(sched,terminal_depth_for_case)
                        assert decode_nibble(terminal_nib)[5]==0,(terminal_nib,decode_nibble(terminal_nib))
...
PYMODEL
then
  printf 'source_terminal_first_nibble_shape\trequested layout\tmodel/static failure\tFAIL\n' >> "$SUMMARY"
...
printf 'source_parent_baseline\tvalidated fastest 175 nibble-schedule MAXD14 dispatch\tverified\tOK\n' >> "$SUMMARY"
printf 'source_terminal_first_nibble_shape\tMAXD14 terminal-first scalar nibble schedule + register-held current frame + 13-slot ancestor stack\tverified\tOK\n' >> "$SUMMARY"
...
printf 'maxd14_terminal_first_nibble_schedule\t1\t%s\tOK\n' "$MAXD14_NIBBLE_SCHEDULE" >> "$SUMMARY"
printf 'maxd14_terminal_first_order\t1\t%s\tOK\n' "$MAXD14_TERMINAL_FIRST" >> "$SUMMARY"
...
printf 'dfs_164_vs_177_terminal_first_nibble_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 177 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 175 nibble schedule + 177 terminal-first over 172 register cache + ancestor13 stack"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 177 keeps the validated 175 kernel structure, then derives chunk-tail diagnostics from
...
echo "[validation-ok] 177 terminal-first nibble schedule reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
