...
# =============================================================================
# NQ_UPDATE_MEMO
# 193: 更新メモ: 192は正当性OKかつ188比で0.019秒のごく小幅改善。192のroot-singleton preroll/future_check_mask guard/no-sibling spill elisionを親に、MAXD14 terminal hitでfuture-prune判定を先に見ない terminal-before-future ordering を単独実験として導入。
...
速度は `0:07:10.118` でした。
188Pyの `0:07:10.137` より `0.019秒`、`0.004%` 高速です。差はかなり誤差級ですが、数値上は最速更新なので、READMEでは**192Pyを数値上の新最速基準**として反映しました。

作成物はこちらです。

* [193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_probe.py)
* [193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_bundle_20260630.tar)

193Pyは192Pyを親にしています。192の `root-singleton preroll`、188の `future_check_mask guard`、184の `no-sibling spill elision` は維持し、MAXD14 kernelだけ **terminal-before-future ordering** に変更しました。

```text
192Py:
  nf != 0
  future_check_mask / nibble_op&8 を確認
  terminal_depthならcount

193Py:
  nf != 0
  terminal_depthなら先にcount
  非terminalだけ future_check_mask / nibble_op&8 を確認
```

terminal parent frameでは、schedule生成上 `child_row == endmark` になるため、terminal parentのnibbleにfuture-prune bitは立たない、という不変条件を使っています。狙いは、terminal hitで `future_check_mask` 分岐を通らず即countすることです。

手元では以下を確認済みです。

```text
bash -n 193Py...sh                         OK
STATIC_ONLY=1 193Py...sh                   OK
source_future_check_mask_guard             OK
source_nosibling_parent                    OK
source_root_singleton_preroll              OK
source_terminal_before_future_order        OK
...
source_split_tag split193 only             OK
abstract terminal future-bit invariant     250000 random cases OK
```

こちらではCodon/CUDA実機build/full実行はしていません。193Pyの採用可否は、添付の `193Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に192比と188比のtimingを見て判断してください。


"""

# 193 terminal-before-future revision memo over validated 192 rootsingle/futuremask/no-sibling MAXD14:
#   191 place-xor was correct but marginally slower than 188, so this branch returns to
#   the validated 188 future_check_mask guard and 184 no-sibling tailcall build.
...
#   only restorable ancestor frames.  193 inherits the 192 narrow fast-start: when the
#   post-root-action root availability has exactly one bit, process depth 0 once
#   before entering the generic DFS loop, then continue at depth 1.  Multi-root
#   candidate threads use the exact 188 loop.  The no-sibling rule and depth
#   packing in avail[] bits 27..31 are unchanged, so MAXD14 remains four
...
# 193 keeps the validated 192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 193 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="193 terminal-prefuture: 192 rootsingle/futuremask/no-sibling MAXD14 plus terminal-before-future ordering - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """193 MAXD14 nibble schedule plus rootsingle/futuremask/no-sibling and terminal-before-future ordering."""
    ...
    # 193 keeps the validated 192/188 current-frame register cache, 13-slot
    # ancestor storage, no-sibling tail-call descent, and future-prune guard.
    # It adds only a root-singleton preroll: when the root active frame has a
    # single available bit after root-action handling, process that first frame
    # once before entering the generic DFS loop. This removes one round of loop
    # backedge/restore checks for single-root threads while preserving the 188
    # generic loop for every multi-candidate root.
    #
    ...
    # 193 inherits 192 root_singleton_preroll:
    # If the root frame has exactly one candidate, no ancestor can ever be
    # restored above it. Process depth 0 once here, then enter the same 188
    # generic DFS loop at depth 1. Multi-candidate roots bypass this block.
    if (cur_avail&(cur_avail-u32(1)))==u32(0):
      pr_nibble_op:u32=schedule_lo&u32(15)
      pr_block_code:u32=pr_nibble_op&u32(7)
      pr_bit:u32=cur_avail
      cur_avail=u32(0)
      ...
      # 193 terminal-before-future: at a terminal parent the generated
      # nibble cannot carry the future-prune bit because child_row==endmark.
      # Count the terminal hit before testing future_check_mask so terminal
      # leaves avoid the extra future branch.
      if terminal_depth==0:
    ...
    while True:
      ...
      # 193 terminal-before-future: terminal-parent nibbles have no future
      # prune bit by construction, so terminal hits can count immediately and
      # skip the schedule-level future_check_mask branch.  Non-terminal frames
      # retain the validated 188 future-prune condition below.
...
CHUNKSHAPE148_DEFAULT_REASON:str="193 terminal-prefuture reuses validated 192/188 scorestripe_v9 task order/cache and changes only MAXD14 terminal/future ordering"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split193_terminal_prefuture_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split193_terminal_prefuture_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split193-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split193-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split193_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split193-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split193-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split193_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
