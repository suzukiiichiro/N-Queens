...
# =============================================================================
# 297 N=21 full validation harness (save-sp-elim probe)
...
# Note: Registers/Thread may decrease by 1 vs 296 if save_sp was register-allocated.
...
SRC=${SRC:-./297Py_save_sp_elim_probe.py}
CAND=${CAND:-./297Py_save_sp_elim_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/297Py_save_sp_elim_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/297Py_save_sp_elim_logs_N21_full_once_${TS}"
...
echo "[start] 297 save-sp-elim validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 297 save-sp-elim SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '297 save-sp-elim probe' "$SRC"; then
  printf 'source_version_tag	297 save-sp-elim probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	297 save-sp-elim probe	missing	FAIL
...
# 297: save-sp-elim shape checks.
    ok_stride_param = 'stride:int,' in body14.split(')->None:')[0]
    ok_stack_array = 'stack=__array__[u64](MAXD14_ANCESTOR*2)' in body14
    code_no_comments = re.sub(r'#[^\n]*', '', body14)
    ok_no_save_sp = 'save_sp' not in code_no_comments
    ok_no_sp2 = 'sp2' not in code_no_comments
    ok_stack_ptr_init = 'stack_ptr:int=0' in code_no_comments
    ok_stack_ptr_zero_check = 'if stack_ptr==0:' in code_no_comments
    ok_stack_ptr_incr = code_no_comments.count('stack_ptr+=2') == 2
    ok_stack_ptr_decr = code_no_comments.count('stack_ptr-=2') == 1
    ...
    ok_push = code_no_comments.count('stack[stack_ptr]=u64(cur_ld)') == 2
    ok_pop = 'packed_ldrd:u64=stack[stack_ptr]' in code_no_comments
    ok_stack_shape = (ok_stride_param and ok_stack_array and ok_no_save_sp and ok_no_sp2
                       and ok_stack_ptr_init and ok_stack_ptr_zero_check
                       and ok_stack_ptr_incr and ok_stack_ptr_decr
                       and ok_gridstride_loop and ok_single_writeback and ok_no_percall_writeback
                       and ok_push and ok_pop)
    checks.append(('source_save_sp_elim_shape',
                    'stack array + no save_sp in code + stack_ptr==0 check + stack_ptr+2/-2 + stride + single writeback',
                    'present' if ok_stack_shape else (
                        'missing (stride=%s stack=%s no_sp=%s no_sp2=%s ptr_init=%s ptr_zero=%s ptr_incr=%s ptr_decr=%s push=%s pop=%s)'
                        % (ok_stride_param, ok_stack_array, ok_no_save_sp, ok_no_sp2, ok_stack_ptr_init,
                           ok_stack_ptr_zero_check, ok_stack_ptr_incr, ok_stack_ptr_decr, ok_push, ok_pop)
                    ),
                    ok_stack_shape))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 297 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 297 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 297 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 297 save-sp-elim mode31 split145"
...
echo "[validation-ok] 297 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
