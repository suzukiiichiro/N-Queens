...
# =============================================================================
# 270 N=21 full validation harness
# Parent: 267 root0countcache precompute-best / 241 restore239.
# Experiment: keep 270 rootpre2flag and host-precompute root_pre2_flag
#             into sched_mask bit28, removing root_second/root_after_second
#             calculation from MAXD14/root0 kernels.
...
SRC=${SRC:-./270Py_schedule_precompute_rootpre2flag_probe.py}
CAND=${CAND:-./270Py_schedule_precompute_rootpre2flag_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/270Py_schedule_precompute_rootpre2flag_N21_full.lock}
...
BASELINE_268_REJECT_SECONDS=${BASELINE_268_REJECT_SECONDS:-428.525}
BASELINE_269_REJECT_SECONDS=${BASELINE_269_REJECT_SECONDS:-428.454}
...
LOGDIR="${LOG_ROOT%/}/270Py_schedule_precompute_rootpre2flag_logs_N21_full_once_${TS}"
...
echo "[start] 270 schedule-precompute-rootpre2flag validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 270 schedule-precompute-rootpre2flag root-pre2-flag SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '270 schedule-precompute-rootpre2flag' "$SRC"; then
  printf 'source_version_tag\t270 schedule-precompute-rootpre2flag\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t270 schedule-precompute-rootpre2flag\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
old_splits = ['split240_','split241_','split242_','split243_','split244_','split255_','split256_','split257_','split258_','split259_','split260_','split261_','split262_','split263_','split264_','split265_','split266_','split267_','split268_','split269_']
runtime_split_ok = ('split270_' in s and not any(x in s for x in old_splits))
if runtime_split_ok:
    checks.append(('source_split_tag', 'split270 active; old split runtime tags absent', 'split270', True))
else:
    checks.append(('source_split_tag', 'split270 active; old split runtime tags absent', 'old split runtime tag present or split270 missing', False))
...
if not m14:
    ...
    actual = 'rootpre2flag MAXD14 precompute active' if ok else 'rootpre2flag missing or old schedule interpreter present or free_arr mask reuse missing or unsafe pack present'
    checks.append(('source_schedule_precompute_rootpre2flag', 'MAXD14 precompute rootpre2flag active', actual, ok))
...
checks.append(('source_host_precompute_rootpre2flag_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 270 schedule-precompute-rootpre2flag mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "241parent:$BASELINE_241_SECONDS" "267:$BASELINE_267_SECONDS" "269rejected:$BASELINE_269_REJECT_SECONDS" "268rejected:$BASELINE_268_REJECT_SECONDS" "266:$BASELINE_266_SECONDS" "265:$BASELINE_265_SECONDS" "263:$BASELINE_263_SECONDS" "264rejected:$BASELINE_264_SECONDS" "262:$BASELINE_262_SECONDS" "239:$BASELINE_239_SECONDS" "242rejected:$BASELINE_242_REJECT_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 270 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
