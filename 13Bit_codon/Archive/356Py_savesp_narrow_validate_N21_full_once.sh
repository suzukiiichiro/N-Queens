...
# =============================================================================
# 356 N=21 full validation harness (save_sp width narrowing -- FIRST KERNEL EDIT SINCE 352)
#
# WHAT 356 IS. 355's ncu SourceCounters at N=18 (K=1) identified two
# unconditional BRA instructions, both adjacent to BSYNC reconvergence
# markers, accounting for 60.2% of measured stall_branch_resolving (121083 +
# 41143 of 269490 samples), and confirmed the aggregate stall composition
# matches previously known figures exactly (wait 45.44%, selected 22.47%,
# branch_resolving 20.10%, long_sb 5.01%, no_inst 4.81%). Disassembly context
# traces both BRAs to the stack push/pop machinery; the 64-bit not-equal-zero
# compare feeding the pop-loop's conditional BREAK is `save_sp==0`, and the
# single instruction right after it, @!P2 BREAK B2, alone accounts for 6.4%
# of the ENTIRE measured wait total -- the single largest individual
# contributor 355 found.
#
# save_sp is declared as Codon's 64-bit int though it only ever ranges 0 to
# MAXD14_ANCESTOR. 356 narrows it to u32: declaration, the ==0 test, the
# decrement, and both increments -- four sites, five lines of diff, verified
# by source_code_identical_to_352_except_savesp below.
#
...
# 326's hot-loop duplication, cost -13.7%). Control flow shape is completely
# unchanged; only one scalar's bit width changes. stack_ptr is deliberately
# left as int (it indexes the stack array; Codon's acceptance of a u32 index
# was not verified, so risk is avoided there even though comment 296 notes
# stack_ptr always equals save_sp*2).
#
# maxd16, maxd18, maxd20, maxd21 and the dispatcher are untouched, proven by
# the same kernel-region byte-identity technique 353/354/355 established.
#
# EXPECTATIONS ARE MODEST. Item 12 (kernel micro-edits are a lottery) still
# applies -- a hot-path instruction-count reduction this small may land
# inside the ~0.2% band 351/352 established as unattributable noise. This is
# still worth measuring: it is the first edit aimed at a SASS instruction 355
# actually named as the single largest contributor to a stall reason, not a
# guess.
#
# JUDGE ON CHUNK0/CHUNK1, NOT SESSION ELAPSED -- 352's own noise-floor lesson:
# session-to-session elapsed comparisons misjudge 0.1%-class effects.
#
...
#     bash 356Py_savesp_narrow_validate_N21_full_once.sh   # candidate
#
# DECISION RULE (fixed before running): within +0.10% of the 352 anchor on
# chunk0/chunk1 (improvement or neutral) -> ADOPT, proceed to 357. Beyond
# +0.10% regression -> REVERT, return to 355's SASS work. Correctness
# 314666222712 is checked before any timing is read, in either case.
#
# EXPECTED_CHUNKS=3, K=48 (unchanged). Cache is reused (..._isort9.bin).
# =============================================================================

SRC=${SRC:-./356Py_savesp_narrow.py}
CAND=${CAND:-./356Py_savesp_narrow}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/356Py_savesp_narrow_N21_full.lock}
...
# 356 judges chunk0/chunk1 against the 352 anchor per the noise-floor lesson;
...
LOGDIR="${LOG_ROOT%/}/356Py_savesp_narrow_logs_N21_full_once_${TS}"
...
echo "[start] 356 savesp-narrow validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 356 savesp-narrow SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '356 savesp-narrow' "$SRC"; then
  printf 'source_version_tag	356 savesp-narrow	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	356 savesp-narrow	missing	FAIL
...
# ---- 356: prove the diff against 352 is EXACTLY the save_sp width change ----
# Two proofs, protecting against two different mistakes.
#
# PROOF 1: maxd16/maxd18/maxd20/maxd21 and the dispatcher's launch signatures
# are BYTE IDENTICAL to 352 -- sliced from "def kernel_dfs_iter_gpu_maxd16("
# to "def launch_kernel_dfs_iter_gpu_static_maxd(" and compared verbatim. No
# docstring stripping needed; this slice has none. Any difference FAILs.
#
# PROOF 2: after docstrings and the two prose constants are stripped, the
# rest of the file (including maxd14) differs from 352 by EXACTLY 5 lines --
# the sha256 fingerprint of the sorted diff. Any unrelated change anywhere
# FAILs this.
#
# Both FAIL rather than skip when 352 is absent, per every prior identity
# check in this project.
REF352=${REF352:-./352Py_record_fix.py}
EXPECTED_SAVESP_DIFF="added=5 removed=5 sha256=aa3e06aced2dc161f84d6a9c1cfdfca06651944e8a46d70dc95448b6ad9a0f15"
if [[ -f "$REF352" ]]; then
  SAVESP_IDENT_AND_DIFF=$(python3 - "$REF352" "$SRC" <<'PYSAVESPIDENT'
...
PYSAVESPIDENT
  ...
  OTHER_STATUS=$(echo "$SAVESP_IDENT_AND_DIFF" | sed -n '1p')
  DIFF_STATUS=$(echo "$SAVESP_IDENT_AND_DIFF" | sed -n '2p')
  record_check source_other_kernels_identical_to_352 "other_kernels=identical" "${OTHER_STATUS:-compare_failed}" || static_failures=$((static_failures+1))
  record_check source_code_identical_to_352_except_savesp "$EXPECTED_SAVESP_DIFF" "${DIFF_STATUS:-compare_failed}" || static_failures=$((static_failures+1))
else
  printf 'source_other_kernels_identical_to_352\tother_kernels=identical\t352 source not found at %s\tFAIL\n' "$REF352" >> "$SUMMARY"
  printf 'source_code_identical_to_352_except_savesp\t%s\t352 source not found at %s\tFAIL\n' "$EXPECTED_SAVESP_DIFF" "$REF352" >> "$SUMMARY"
  static_failures=$((static_failures+2))
fi

# ---- 356: the save_sp edit itself, checked site by site ----
# NEGATIVE TEST (the established procedure): feed 352's source to this script
# and this must read 0 and FAIL:
#     SRC=./352Py_record_fix.py STATIC_ONLY=1 bash 356Py_savesp_narrow_validate_N21_full_once.sh
SAVESP_DECL=$(grep -c '^      save_sp:u32=u32(0)$' "$SRC_NODOC" || true)
SAVESP_CMP=$(grep -c '^          if save_sp==u32(0):$' "$SRC_NODOC" || true)
SAVESP_DEC=$(grep -c '^          save_sp-=u32(1)$' "$SRC_NODOC" || true)
SAVESP_INC=$(grep -c '^            save_sp+=u32(1)$' "$SRC_NODOC" || true)
SAVESP_INC2=$(grep -c '^          save_sp+=u32(1)$' "$SRC_NODOC" || true)
SAVESP_SITES="decl=$SAVESP_DECL cmp=$SAVESP_CMP dec=$SAVESP_DEC inc_prologue=$SAVESP_INC inc_mainloop=$SAVESP_INC2"
record_check source_savesp_sites "decl=1 cmp=1 dec=1 inc_prologue=1 inc_mainloop=1" "$SAVESP_SITES" || static_failures=$((static_failures+1))

# stack_ptr must remain untouched (still plain int, no u32 cast anywhere on it)
if grep -q 'stack_ptr:int=0' "$SRC_NODOC" && ! grep -q 'stack_ptr:u32' "$SRC_NODOC" && ! grep -q 'stack_ptr+=u32' "$SRC_NODOC" && ! grep -q 'stack_ptr-=u32' "$SRC_NODOC"; then
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 356 savesp-narrow. FIRST KERNEL EDIT SINCE 352: save_sp narrowed from i64 to u32 at declaration, its ==0 test, its decrement, and both increments (4 sites, 5 lines of diff, source_code_identical_to_352_except_savesp proves it statically), targeting the single largest individual wait-stall contributor 355 measured (@!P2 BREAK B2, 6.4 percent of the total). stack_ptr, maxd16/18/20/21 and the dispatcher are unchanged. mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT, all unchanged from 352, shaped bin reused. Judge on chunk0/chunk1 against the 352 anchor (398.055s), not session elapsed. Adopt within +0.10 percent, revert beyond it; correctness 314666222712 checked first either way"
...
echo "[validation-ok] 356 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2, CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN and CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT, all unchanged from 352. save_sp is u32 at all 4 sites, stack_ptr and the other 4 kernels are byte identical to 352. Read the chunk0/chunk1 comparison against the 352 anchor, not session elapsed, before deciding adopt vs revert per the fixed +0.10 percent rule"
