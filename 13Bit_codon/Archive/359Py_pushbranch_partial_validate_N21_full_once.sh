...
# =============================================================================
# 359 N=21 full validation harness (hot-loop push guard, PARTIAL version:
# only the ldrd store is hoisted unconditional -- SECOND CONTROL-FLOW-SHAPE
# ATTEMPT AFTER 358's REJECTION)
#
# WHAT 359 IS. 358 hoisted BOTH stack stores unconditional in the hot-loop
# push guard and was REVERTED: all 3 chunks regressed +3.47% to +5.51%
# against the 356 anchor despite correctness holding and every static check
# passing. 358's post-mortem N=18 SourceCounters rerun showed the
# branch-elimination hypothesis was PARTIALLY correct: BSYNC reconvergence
# for this guard did shrink (old culprit-1's BRA addresses dropped out of
# the top branch_resolving contributors, BSYNC itself fell to 5.85% of it),
# but stall_long_sb (memory-latency scoreboard waits) exploded from ~5.6%
# to 17.77%, because BOTH stores became unconditional and every lane now
# issues them regardless of whether it used to skip the guard.
#
# THE CHANGE (one site only, smaller than 358's): only the ldrd store is
# hoisted unconditional; the colav store and the stack_ptr/save_sp updates
# stay inside a (3-statement, down from 356's 4) `if`:
#     stack[stack_ptr]=...                                   # unconditional
#     if cur_avail!=u32(0):
#       stack[stack_ptr+1]=...
#       stack_ptr+=2; save_sp+=u32(1)
#
# THE `if` SHAPE REMAINS -- unlike 358 -- so it is NOT certain the compiler
# generates less branch cost than 356's baseline at all; that uncertainty
# is exactly what this revision measures, honestly acknowledged rather than
# assumed away.
#
# MEMORY SAFETY: identical argument to 358's. stack is sized
# MAXD14_ANCESTOR*2 = 26 u64 words; save_sp never reaches MAXD14_ANCESTOR
# while correctness holds at 314666222712, so the pre-increment stack_ptr
# here is always <=24; the unconditional ldrd store, when cur_avail==0, is
# simply discarded and never read back.
#
# JUDGE ON CHUNK0/CHUNK1 AGAINST THE 356 ANCHOR (144590ms / 144473ms), NOT
# SESSION ELAPSED -- 357 made no code change and 358 was reverted, so 356
# remains the anchor.
#
...
#     bash 359Py_pushbranch_partial_validate_N21_full_once.sh   # candidate
#
# DECISION RULE (fixed before running): within +0.10% of the 356 anchor on
# chunk0/chunk1 (improvement or neutral) -> ADOPT, proceed to 360. Beyond
# +0.10% regression -> REVERT, close this axis (push-guard branch removal)
# in favor of item 6 (the CUDA C runner). Correctness 314666222712 is
# checked before any timing is read, in either case.
#
# FOLLOW-UP (regardless of adoption): rerun N=18 SourceCounters to compare
# branch_resolving and long_sb against both 356 and 358, to see whether
# halving the unconditional-store count roughly halves long_sb's growth.
#
# EXPECTED_CHUNKS=3, K=48 (unchanged). Cache is reused (..._isort9.bin).
# =============================================================================

SRC=${SRC:-./359Py_pushbranch_partial.py}
CAND=${CAND:-./359Py_pushbranch_partial}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/359Py_pushbranch_partial_N21_full.lock}
...
BASELINE_358_REJECTED_SECONDS=${BASELINE_358_REJECTED_SECONDS:-412.028}
...
LOGDIR="${LOG_ROOT%/}/359Py_pushbranch_partial_logs_N21_full_once_${TS}"
...
echo "[start] 359 pushbranch-partial validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 359 pushbranch-partial SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '359 pushbranch-partial' "$SRC"; then
  printf 'source_version_tag	359 pushbranch-partial	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	359 pushbranch-partial	missing	FAIL
...
EXPECTED_PUSHBRANCH_DIFF="added=20 removed=1 sha256=87464856638913ed9a90e67875cb5d95c7a49c525faa84ad387c7eb1b1354ec4"
if [[ -f "$REF356" ]]; then
...
# ---- 359: the partial push-guard rewrite itself, checked site by site ----
# NEGATIVE TEST (the established procedure): feed 356's own source to this
# script (SRC=REF=356) and these checks correctly FAIL: source_version_tag
# (missing), source_hotloop_ldrd_store_unconditional (356's ldrd store is
# still inside the if, not hoisted), source_K48_sweep_shape (old literal),
# and source_code_identical_to_356_except_pushbranch (356 vs itself diffs
# by 0/0, which does not match the expected 20/1):
#     SRC=./356Py_savesp_narrow.py STATIC_ONLY=1 bash 359Py_pushbranch_partial_validate_N21_full_once.sh
#
# 1) the hot-loop sequence must read EXACTLY: next_depth line, then the
# unconditional ldrd store (8-space indent), then `if cur_avail!=u32(0):`
# (8-space, STILL PRESENT -- unlike 358), then the colav store (10-space).
# Comment-only lines in between (this revision's explanatory note) are
# skipped so they cannot cause a false negative.
HOTLOOP_SEQ=$(python3 - "$SRC_NODOC" <<'PYHOTLOOPSEQ'
import sys
lines = open(sys.argv[1], encoding='utf-8').read().split('\n')
for i, l in enumerate(lines):
    if l == '        next_depth:int=cur_depth+1':
        rest = [x for x in lines[i+1:i+40] if not x.strip().startswith('#')]
        l1, l2, l3 = (rest + ['', '', ''])[:3]
        expect1 = '        stack[stack_ptr]=u64(cur_ld)|(u64(cur_rd)<<u64(32))'
        expect2 = '        if cur_avail!=u32(0):'
        expect3 = '          stack[stack_ptr+1]=u64(cur_col)|(u64(cur_avail|(u32(cur_depth)<<u32(27)))<<u64(32))'
        if l1 == expect1 and l2 == expect2 and l3 == expect3:
            print('MATCH')
        else:
            print('NO_MATCH l1=[%s] l2=[%s] l3=[%s]' % (l1, l2, l3))
        break
else:
    print('NO_MATCH next_depth line not found')
PYHOTLOOPSEQ
)
record_check source_hotloop_ldrd_store_unconditional MATCH "${HOTLOOP_SEQ:-CHECK_FAILED}" || static_failures=$((static_failures+1))
# 2) the preamble guard (structurally identical, untouched) must still have
# BOTH stores inside its own if, at its own (12-space) indent.
...
PREAMBLE_BOTH_STORES=$(awk '/^          if cur_avail!=u32\(0\):$/{getline l1; getline l2; if (l1=="            stack[stack_ptr]=u64(cur_ld)|(u64(cur_rd)<<u64(32))" && l2=="            stack[stack_ptr+1]=u64(cur_col)|(u64(cur_avail|(u32(cur_depth)<<u32(27)))<<u64(32))") print "MATCH"; else print "NO_MATCH"; exit}' "$SRC_NODOC")
record_check source_preamble_push_both_stores_guarded MATCH "${PREAMBLE_BOTH_STORES:-CHECK_FAILED}" || static_failures=$((static_failures+1))
...
# 4) save_sp stays u32 everywhere (356's narrowing is not reverted by 359)
...
# 304: K48-sweep shape checks (296 kernel logic, K=48).
    ...
    # 359: unlike 358, the stack_ptr/save_sp increment logic is UNCHANGED --
    # only the ldrd store was hoisted out of the if. `stack_ptr+=2` remains
    # a literal, still exactly twice (preamble + hot loop), same as 356.
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 359 pushbranch-partial. SECOND CONTROL-FLOW-SHAPE ATTEMPT AFTER 358's REJECTION: only the ldrd store in the hot-loop push guard is hoisted unconditional; the colav store and stack_ptr/save_sp updates stay inside a smaller (3-statement) if (20 added / 1 removed lines vs 356, source_code_identical_to_356_except_pushbranch_partial proves it statically), halving 358's unconditional-store count to curb the stall_long_sb growth 358's post-mortem measured (5.6 to 17.77 percent) while still targeting some of culprit-1's branch_resolving cost. Preamble push and pop path (culprit-2) are untouched; stack_ptr, maxd16/18/20/21 and the dispatcher are unchanged. mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT, all unchanged from 356, shaped bin reused. Judge on chunk0/chunk1 against the 356 anchor (144590ms/144473ms), not session elapsed. Adopt within +0.10 percent, revert and close this axis otherwise; correctness 314666222712 checked first either way"
...
echo "[validation-ok] 359 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2, CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN and CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT, all unchanged from 356. Hot-loop push guard's ldrd store is unconditional, colav store + stack_ptr/save_sp updates stay guarded (3-statement if) at the one site 355/357/358 measured; preamble push, pop path, stack_ptr and the other 4 kernels are byte identical to 356. Read the chunk0/chunk1 comparison against the 356 anchor, not session elapsed, before deciding adopt vs revert per the fixed +0.10 percent rule"
