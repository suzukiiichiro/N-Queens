...
# - -c 単独も CPU N22 単体実行
# - -g 単独は A10G 1枚向けの GPU 最終設定で N22 を実行
#   ./115Py... -g
#   は以下と同等:
...
codon build -release \
  115Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_tail_final.py

# 既定実行: CPU N22
./115Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_tail_final

# CPU N22 明示
./115Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_tail_final -c

# A10G 1枚 GPU N22 最終設定
./115Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_tail_final -g

# A10G 1枚 GPU N22 最終設定の明示形
./115Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_tail_final \
...
CUDA_VISIBLE_DEVICES=0 ./115Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_tail_final \
  -g 22 22 32 484 1 0 7 29 8 7 0 0 4 2 \
  2>&1 | tee 115Py_N22_worker0of4_rotate_only.log &

  
修正版を作成しました。ご提示の表に合わせて、`-c` 単独・`-g` 単独では **N22単体ではなく N=5..23** を流すように変更しています。

* [更新版 115Py ソース](sandbox:/mnt/data/115Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_tail_final.py)
* [更新版 README](sandbox:/mnt/data/115Py_README.md)
* [今回の変更パッチ](sandbox:/mnt/data/115Py_bare_c_g_range_default.patch)
...
実装上は、`main()` の既定範囲を以下として明示しました。

```python
MAIN_DEFAULT_NMIN:int=5
MAIN_DEFAULT_NMAX_EXCLUSIVE:int=24
```

Codon/Python の `range(nmin, nmax)` は上限を含まないので、これで **5〜23** までになります。添付ログにも、以前の `-g` 実行例として N=5 から N=23 までの表形式出力が残っていたため、その形に合わせています。

なお、**引数なし**だけは前回の方針を残してあります。

```bash
./115Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_tail_final
```

これは引き続き **CPU N22 単体**です。

A10G 1枚向けの N22 broadmarktail 最終設定は、`-g` 単独ではなく、明示指定で残しています。
...
今回触ったのは **`main()` の起動引数デフォルト、表示抑制、コメント/README** だけです。DFS / GPU kernel / 解数計算ロジックには触れていません。114Py 側でも broadmarktail v4 は stream ordering / `.bin` layout の変更で、GPU kernel と DFS logic は変更しない前提でした。

Codon はこの sandbox には入っていないため、こちらでは実機ビルドまでは実行していません。
...
VERSION_TAG:str="115 final CPU-default + A10G single-GPU broadmarktail v4 rotate_only runner from 114"
...
#   Bare "-g" runs the final A10G single-GPU N=22 challenge.
...
A10G_FINAL_DEFAULT_LOG_LEVEL:int=1
...
def main()->None:
  ...
  if argc == 1:
    ...
    # A10G single-GPU remains available with bare -g.
    use_gpu=False
    ...
    print("[115-default] no arguments: CPU N22 default; use -g for A10G single-GPU final mode")
  elif argc >= 2:
    arg = sys.argv[1]
    if arg == "-c":
      use_gpu = False
      ...
      if argc == 2:
        # Bare -c is kept practical: run the same N=22 challenge on CPU.
        # Full ranges can still be requested explicitly, e.g. -c 5 22.
        nmin=CPU_FINAL_DEFAULT_N
        nmax=CPU_FINAL_DEFAULT_N+1
        requested_preset_arg=5
        preset_queens_arg=5
        bench_mode=0
        print("[115-default] bare -c: CPU N22 default")
    elif arg == "-g":
      use_gpu = True
      print("GPU mode selected")
      if argc == 2:
        # Bare -g uses the final A10G single-GPU broadmarktail flow.
        nmin=A10G_FINAL_DEFAULT_N
        nmax=A10G_FINAL_DEFAULT_N+1
        ...
        print("[115-default] bare -g: A10G single-GPU final N22 mode29 w8_j7 variant=2 rotate_only")
    else:
