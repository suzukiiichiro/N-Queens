...
# =============================================================================
# NQ_UPDATE_MEMO
# 284: generic-normalfirst-ncol-early。283 generic-normalfirst が N=21 full once
#      で 0:07:07.698 を記録し271/239/217をわずかに上回ったため、283を親に
#      MAXD14 generic DFS loopだけをさらに1差分確認する。283の normal path先行は
#      維持し、`ncol:u32=cur_col|bit` を block_code 分岐の前へ前倒しして、normal/
#      special の両pathで共通に使う。root-preroll、rootrestlate、futuremask、
#      no-sibling、split145/chunkshape148、broadmarktail、cache生成、worker split、
...
284一式を作成しました。
ご指摘通り、`.py`、`.sh`、README.md の3点セットです。

今回は **283を親にした微差実験版**です。283の `generic normal-first` を維持しつつ、MAXD14 generic DFS loop内だけ、

```python
ncol:u32 = cur_col | bit
```

を `block_code` 分岐の前へ前倒しします。`placed_ld/placed_rd` のような `cur_ld|bit` / `cur_rd|bit` 共通化は、過去226で退行しているため入れていません。

`STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [284Py_generic_normalfirst_ncol_early_probe.py](sandbox:/mnt/data/284Py_generic_normalfirst_ncol_early_probe.py)
* [284Py_generic_normalfirst_ncol_early_validate_N21_full_once.sh](sandbox:/mnt/data/284Py_generic_normalfirst_ncol_early_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [284Py_generic_normalfirst_ncol_early_sha256.txt](sandbox:/mnt/data/284Py_generic_normalfirst_ncol_early_sha256.txt)
* [284Py_generic_normalfirst_ncol_early_bundle_20260709.tar](sandbox:/mnt/data/284Py_generic_normalfirst_ncol_early_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./284Py_generic_normalfirst_ncol_early_probe

STATIC_ONLY=1 bash 284Py_generic_normalfirst_ncol_early_validate_N21_full_once.sh

bash 284Py_generic_normalfirst_ncol_early_validate_N21_full_once.sh
```

採否は、まず正当性OK、そのうえで **283の `0:07:07.698`、276の `0:07:07.672`、271の `0:07:07.705`** と比較して判断するのがよいです。
...
VERSION_TAG:str="284 generic-normalfirst-ncol-early: parent 283 generic-normalfirst validated fast; MAXD14 generic DFS loop keeps normal path first and moves ncol=cur_col|bit before block_code branch; root-preroll and all host shaping/cache/dispatch features preserved"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split284_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split284_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split284-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split284-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split284_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
