...
"""
**353の設計です。** 353は採否のあるリビジョンではありません。
**カーネルに計数器を1本入れて、N=21の全2,025,282タスクについて
実際のDFSループ試行回数を悉皆調査する、計測専用ビルド**です。

## なぜ項目11(wait/branch_resolving)ではないのか

352の引継ぎメモは353を項目11に充てる予定にしていました。しかしその
メモ自身が論点1で「これはレバーではなく機械についての問い、性能が動く
保証はない」と書き、論点3で「N=21では SourceCounters が現実的な時間で
終わらない」と書いています。つまり項目11は**コストが高く、解像度が低く、
しかも項目12(くじ引き)の射程内**にあります。0.2%の壁時計変動が帰属
できない世界で、N=18のストール率からN=21の設計判断を下すのは危険です。

353はその手前に一段挟みます。**このプロジェクトで唯一「くじ引きになら
ない計測」**だからです。理由は一つです。

  **ループ試行回数はスケジューリング非依存の決定的な量である。**

各スレッドのDFSは完全に独立で、共有枝刈りも早期打ち切りもありません
(ソース上で確認済み: カーネルはタスクごとに専用スタックを持ち、他
レーンの状態を一切参照しません)。したがって計測ビルドが命令キャッシュ
整列で遅くなろうが、レジスタが45から47に増えて占有率が動こうが、
**得られる数値は1ビットも変わりません**。ncuのストール率がK値・N値・
replayで動くのとは、測定物の性質が根本的に違います。

## 何が測れるか

census(タスクindex → 試行回数)と、grid-strideの決定的な写像

  tid = idx mod 15488 、 warp = tid div 32 、 lane = tid mod 32

だけで、以下がすべてオフラインで出ます。BLOCK=32なので**1ブロック=1
ワープ**であり、warp境界とblock境界が一致します。

1. **ワープ間不均衡** max/mean。全484ブロックが同時常駐する
   (占有率33.33% = SMあたり16ワープ、80SMで1280枠 >> 484)ので、
   launch時間は最も重いワープで決まります。これが**ホスト側並べ替えと
   work stealingの天井**です。
2. **ワープ内レーン退場テール** ワープ内で最も長いレーンと平均の比。
   これが**warp compaction(CUDA C移植)の天井**です。
3. **現行ソートキーの成績表** shaped binは既にソート済みなので、
   タスクの位置がそのまま現行キーの順位です。位置と試行回数の
   スピアマン順位相関を取れば、**追加の計装なしでキーを採点**できます。
4. **LPT下界** 真の試行回数を使った最長ジョブ優先割当のmakespanと実測
   割当のmakespanの差。これが**あらゆる静的並べ替えの残りheadroom**です。

**測れないものも明記します。** 制御フロー発散(Avg Threads Executed
2〜3/32)はcensusでは測れません。あれは経路の分岐であって仕事量の
長短ではないからです。そこはncuの領分のまま残ります。353が確定させる
のは**仕事量分布に起因する損失**であり、それを引いた残りが発散由来です。

## 副産物

census binと既存のshaped binが揃うと、**新しいソートキーの評価にGPU実行
が不要になります**。キー案 → binを作り直して398秒回して判定、という
サイクルが、キー案 → censusに対するオフラインmakespan計算、に変わります。
このプロジェクトで最も当たりの多い軸(K-batching -4.4%、bucket_run
-4.0%、sort scope -6.8%)の実験サイクルが4桁縮みます。353自体はレバー
ではありませんが、**レバー製造機**です。

## 変更点(352からの単一変数)

`kernel_dfs_iter_gpu_maxd14` のみに3行、ディスパッチャに1引数、ホスト側
に確保・受け取り・ダンプです。**maxd16/18/20/21は1バイトも触りません。**
N=21では dispatch_non_MAXD14=0 が示すとおり全チャンクがmaxd14を通るので、
それで十分であり、既存の `source_maxd16_unmodified` 等4検査もそのまま
通ります。maxd14以外が選ばれたチャンクは、ゼロで埋めたファイルを書く
のではなく**書かずに [census-error] を出します**。

計数器はu64です。u32なら1命令で済みますが、桁溢れが静かに嘘をつきます。
計測ビルドの速度には価値がないので、正しさを取ります。

ダンプは t4(全ステージタイマを閉じた後)の直後に置きました。約6MBの
書き出しが `stage_kernel_reduce_ms` を汚さないためです。

## 静的チェック

**`source_code_identical_to_352_except_census` を新設しました。**
docstringとVERSION_TAG/WHI_ELIM_REASON/CENSUS_REASONを除去し、さらに
353が追加した行(ホワイトリスト)を除去したうえで、352のソースと
**バイト単位で完全一致**することを検査します。352のソースが同じ
ディレクトリに無い場合はSKIPではなくFAILです。

**負のテスト**も入れてあります。352のソースを食わせると
`source_census_kernel_sites` がFAILすることを、351/352で確立した手順の
とおり確認できます。

## 実行手順

1. `STATIC_ONLY=1`
2. **N=18スモーク(3秒)**。長時間コマンドの前に数秒の確認を置く、という
   352の教訓をここで使います。censusファイルのレコード数と非ゼロ率、
   N=18の解の一致を見ます。
3. **N=21フル1回**。正解 314666222712 は厳守です。
4. オフライン解析。`.sh` 内のCPythonヒアドキュメントが走ります。

**壁時計はsummary.tsvにtiming行として記録しません。** 比較対象では
ないからです。INFO行として残すだけにしてあります。

## 判定基準(データを見る前に固定します)

事後の理屈付けを封じるため、閾値を先に置きます。

- ワープ内テール比が6倍以上 → warp compactionの天井が大きい。
  項目6のCUDA Cランナーが主軸として正当化されます。
- ワープ内テール比が2倍未満 → warp intrinsics単独では移植を
  正当化できません。別の根拠が要ります。
- LPT下界が実測比3%以上 → ホスト側並べ替えに実弾が残っています。
  Codonのまま、最も安い次の一手です。
- スピアマン相関が0.85以上 → 現行キーはコストをほぼ説明済み。
  キー再設計軸はクローズします。

## 354はどこから始めるか

**353は系譜の枝です。** 354のカーネルは352から再開します
(計数器を載せたまま先へ進めてはいけません。項目12の引き直しになります)。
...
VERSION_TAG:str="353 work-census: MEASUREMENT ONLY REVISION, NOT A CANDIDATE FOR ADOPTION. 353 adds one per task counter to kernel_dfs_iter_gpu_maxd14 and dumps, for every one of the 2025282 tasks of N=21, the exact number of trips taken by the iterative DFS loop. THREE LINES IN ONE KERNEL: trips is declared beside total, incremented as the first statement of the DFS while loop, and stored to census_arr at the task index just before the grid stride advance. The array is zero initialised on the host so the tasks that exit before the DFS loop keep a count of zero without a store on those paths. maxd16, maxd18, maxd20 and maxd21 are untouched, which is sufficient because dispatch_non_MAXD14 is 0 at N=21, and a chunk that selected any other kernel writes no file and prints census-error rather than emitting zeros. WHY THIS AND NOT ITEM 11: the 352 handover itself records that wait and branch resolving are a question about the machine rather than a lever, that SourceCounters cannot finish on N=21, and that N=18 stall ratios do not represent production. Item 11 is therefore expensive, low resolution, and inside the blast radius of item 12. THE CENSUS IS THE ONE MEASUREMENT IN THIS PROJECT THAT CANNOT BE A LOTTERY DRAW, because a loop trip count is independent of scheduling: each thread runs a private DFS over a private stack with no shared pruning and no early termination, so instruction cache alignment, register bank conflicts and any occupancy change caused by the extra u64 leave the recorded numbers bit for bit unchanged. WHAT IT BOUNDS: BLOCK is 32 so one block is exactly one warp, and all 484 blocks are simultaneously resident at 33.33 percent occupancy, so launch time is set by the heaviest warp. Cross warp imbalance therefore bounds host side reordering and work stealing; within warp lane retirement tail bounds warp compaction and hence the CUDA C port of item 6; the shaped bin is already in sort order so task position is the rank under the current key and the rank correlation between position and trips grades that key with no extra instrumentation; and a longest processing time first assignment over the true counts gives the lower bound on the makespan of any static reordering. WHAT IT DOES NOT MEASURE: control flow divergence, the 2 to 3 threads per warp figure, is a property of branch paths rather than of work length and remains an ncu quantity. 353 fixes the share of the loss that is workload imbalance; what is left over is divergence. BY PRODUCT: census plus the cached shaped bin turns evaluation of a new sort key from a 398 second run into an offline makespan computation, which is a four order of magnitude shorter loop on the most productive axis this project has, K batching at minus 4.4 percent, bucket run at minus 4.0 percent and sort scope at minus 6.8 percent. THE COUNTER IS u64 ON PURPOSE: u32 would cost one instruction instead of two but a silent wrap would lie, and the speed of a measurement build is worth nothing. THE DUMP SITS AFTER t4 ON PURPOSE, so that a six megabyte write cannot contaminate stage_kernel_reduce_ms. WALL CLOCK FROM THIS BUILD IS NOT A TIMING ROW: it is recorded as INFO only, because this binary is not a candidate. 354 RESTARTS FROM THE 352 KERNEL; carrying the counter forward would re-draw the item 12 lottery. PRE REGISTERED DECISION RULE, fixed before the data is seen: lane retirement tail at or above 6 times justifies the CUDA C port as the main axis, below 2 times does not; an LPT lower bound 3 percent or better below the observed makespan means host side reordering still has live ammunition and is the cheapest next move; rank correlation at or above 0.85 closes the key redesign axis."
...
CENSUS_REASON:str="353 instruments kernel_dfs_iter_gpu_maxd14 with a per task DFS loop trip counter and writes one u64 per task, in shaped bin record order, to census353_N21_<preset>_<param>_k48_chunk<i>.bin. Record i of that file joins to record i of the shaped bin by position, so no key is stored. The counter is deterministic and independent of scheduling, so the numbers are valid even though this build is not the fastest build. Tasks that exit before the DFS loop keep the zero the host wrote. Only maxd14 carries the counter; any chunk that selects another kernel writes no file and prints census-error. This revision is diagnostic and is not adopted; 354 restarts from the 352 kernel."
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 353: scratch census target for the non-split145 route. The dispatcher now
    # takes the array unconditionally; only mode31/split145 ever writes a file.
    census_scratch:List[u64]=[u64(0)]*STEPS
    gpu_total:int=0
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        if not launch_kernel_dfs_iter_gpu_static_maxd(selected_maxd,sort_soa,sort_w_arr,meta_next,results,census_scratch,m,board_mask_gpu,n3_gpu,n4_gpu,GRID,BLOCK):
        ...
        if gpu_log_level>=2:
          print(f"[maxd-dispatch] N={N} scope=exec chunk={chunk_index} m={m} required_maxd={required_maxd} selected_MAXD={selected_maxd} schedule_words={packed_schedule_words_for_maxd(selected_maxd)} stack_bytes_per_thread={packed_stack_bytes_per_thread(selected_maxd)} capacity_check=OK")
        if not launch_kernel_dfs_iter_gpu_static_maxd(selected_maxd,soa,w_arr,meta_next,results,census_scratch,m,board_mask_gpu,n3_gpu,n4_gpu,GRID,BLOCK):
          print(f"[maxd-error] unsupported required_maxd={required_maxd}; supported maximum is 21")
          return
...
def write_census_bin(fname:str,census:List[u64],m:int)->int:
  # 353: one u64 per task, written in shaped-bin record order, little endian.
  # Record i of this file is the DFS loop trip count of record i of the shaped
  # bin, so the file joins to the bin by position and needs no key of its own.
  # Written once per chunk AFTER the timed kernel stage, so it cannot move
  # stage_kernel_reduce_ms. Buffered in 65536-record blocks because a per
  # record write of eight characters is the slow path in this runtime.
  wrote:int=0
  buf:List[str]=[]
  with open(fname,"wb") as f:
    i:int=0
    while i<m:
      b=int_to_le_bytes8(int(census[i]))
      buf.append("".join(chr(c) for c in b))
      wrote+=1
      i+=1
      if len(buf)>=65536:
        f.write("".join(buf))
...
def exec_solutions_gpu_chunk_split145(
  ...
  census_out_fname:str=""  # 353: per task DFS trip count dump; empty disables the dump
  ...
  w_arr:List[u64]=[u64(0)]*STEPS
  results:List[u64]=[u64(0)]*STEPS
  # 353: per task DFS trip counter. Zero initialised, so the tasks that exit
  # before the DFS loop (root_action 1/2/3, or an empty root) correctly keep a
  # count of zero without needing a store on those paths.
  census:List[u64]=[u64(0)]*STEPS
  ...
  if m>0:
    required_maxd:int=max_schedule_depth_of_tasks(soa,m,meta_next)
    selected_maxd:int=select_static_maxd(required_maxd)
    census_selected_maxd=selected_maxd
    if gpu_log_level>=1:
    ...
    if selected_maxd==14:
      ...
      GRID=(m+BLOCK-1)//BLOCK
      kbatch_stride=0
    if not launch_kernel_dfs_iter_gpu_static_maxd(selected_maxd,soa,w_arr,meta_next,results,census,m,board_mask_gpu,n3_gpu,n4_gpu,GRID,BLOCK,kbatch_stride):
      print(f"[maxd-error] unsupported required_maxd={required_maxd}; supported maximum is 21")
  ...
  t4=datetime.now()
  # 353: the census dump sits AFTER t4 on purpose. Every stage timer in this
  # function is closed by then, so the ~6MB write lands only in the outer chunk
  # elapsed and cannot contaminate stage_kernel_reduce_ms. Only maxd14 carries
  # the counter; the four fallback kernels are unmodified, so a chunk that
  # selected any other kernel must produce no file rather than a file of zeros.
  if census_out_fname!="":
    if census_selected_maxd==14:
      census_written:int=write_census_bin(census_out_fname,census,m)
      print(f"[census-write] m={m} records={census_written} bytes={census_written*8} selected_maxd={census_selected_maxd} file={census_out_fname}")
    else:
      print(f"[census-error] selected_maxd={census_selected_maxd} is not 14; the census counter exists only in kernel_dfs_iter_gpu_maxd14, so no file was written for this chunk (file={census_out_fname})")
...
def exec_solutions_gpu_bin_stream_split145(
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      elapsed_ms:int=0
      census_fname=f"census353_N{N}_{preset_queens}_{run_param_tag}_k{k_per_thread}_chunk{chunk_index}.bin"
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split145(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level,k_per_thread,census_fname)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30 or bench_mode==31:
      ...
      print(f"census_params: census=on counter=dfs_loop_trips width=u64 scope=maxd14_only reason={CENSUS_REASON}")
