...
# =============================================================================
# 326 N=21 full validation harness (futurecheck-specialize-axis1)
# Parent: 325 inlineprobe-prep. 325's standalone probe confirmed Codon
#   DOES inline plain functions called from a @gpu.kernel (no CALL/RET,
#   both branch bodies appear directly inlined, converging at a single
#   BSYNC) -- see 325_inline_probe_ncu_source.txt. None of 320-325 have
#   their own confirmed N=21 timing; 319 (455.116s, 314666222712)
#   remains the most recent confirmed baseline.
#
# *** THIS IS THE FIRST KERNEL-LOGIC CHANGE SINCE 311. *** Every
#   revision from 311 through 325 kept kernel_dfs_iter_gpu_maxd14 (and
#   all other kernels) byte-for-byte identical while investigating Stall
#   Branch Resolving via profiling and manual review only. Treat this
#   revision with the same caution as the rev-189 regression precedent
#   (+108% slowdown from an earlier inner-loop restructuring attempt)
#   and the four rejected GPU kernel-decomposition attempts
#   (240/266-269/273). If this does not clearly help, REVERT
#   IMMEDIATELY, same as every prior rejected probe.
#
# WHAT CHANGED: given 325 confirmed Codon inlines plain functions, this
#   revision does NOT introduce a separate device function at all --
#   doing so would add unverified risk (passing the local `stack` array
#   across a function boundary has no precedent anywhere in this
#   codebase) for no proven benefit over the simpler option. Instead,
#   the SAME while-True hot loop body (kernel_dfs_iter_gpu_maxd14 only,
#   ~line 783 in prior revisions) is now duplicated in place inside an
#   `if future_check_mask==u32(0): ... else: ...` that is evaluated ONCE
#   per task, using future_check_mask which is already a per-task scalar
#   computed by the root-preroll before this point (unchanged elsewhere).
#   - `future_check_mask==0` branch: the future-check block (the
#     `if future_check_mask!=u32(0): if (nibble_op&8)!=u32(0): if
#     (bm&~(...))==u32(0): continue` 4-line block) is entirely ABSENT --
#     not skipped at runtime, structurally not present in this compiled
#     path.
#   - `future_check_mask!=0` branch: the same block is present, with
#     only the now-always-true outer `if future_check_mask!=u32(0):`
#     wrapper removed (the inner nibble_op/lookahead check is unchanged
#     and still evaluated per-node as before).
#   Both branches were verified line-by-line via automated diff to be
#   byte-for-byte identical outside this one block (see 326's docstring
#   for the diff). MAXD16/18/20/21 kernels, root-preroll, chunkshape148,
#   broadmarktail, cache generation, worker split, dispatch, and every
#   other part of the source are UNCHANGED.
#
# THIS HAS NOT BEEN BUILT OR RUN ON REAL HARDWARE. Codon build errors
#   are plausible on the first attempt (this codebase's own history has
#   many buildfix sub-revisions for far smaller changes, e.g. 240 r4/r7,
#   257 r2, 259). Expect possible iteration.
# MANDATORY VALIDATION ORDER:
#   1. `STATIC_ONLY=1 bash 326Py_futurecheck_specialize_axis1_validate_N21_full_once.sh`
#      -- confirms the source parses/builds statically before any GPU time is spent.
#   2. Full N=21 run. Confirm final total == 314666222712 FIRST, before
#      looking at timing at all. Any correctness deviation is an
#      immediate abort -- do not proceed to timing comparison.
#   3. Only after correctness is confirmed, compare elapsed time against
#      319's 455.116s baseline. If not clearly faster, and especially if
#      slower, REVERT to 325 immediately (do not keep iterating on this
#      branch without a clear win, per 240/268/269/273 precedent).
#   4. If a real timing improvement appears, re-profile with the same
#      `sudo ncu --section SourceCounters` command from 318-319 to
#      confirm stall_branch_resolving/stall_wait actually decreased --
#      a wall-clock change without a matching profiled-mechanism change
#      would be suspicious and should be understood before adopting.
# EXPECTED_CHUNKS=3, K=48 (unchanged from 304).
# =============================================================================

SRC=${SRC:-./326Py_futurecheck_specialize_axis1.py}
CAND=${CAND:-./326Py_futurecheck_specialize_axis1}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/326Py_futurecheck_specialize_axis1_N21_full.lock}
...
# NOTE: none of 320/321/322/323/324/325's own N=21 full runs have been
# executed yet (320 went straight to `codon build --help`; 321's -debug
# build crashed with CUDA_ERROR_INVALID_PTX before any N=21 run was
# attempted; 322 was a documentation-only manual source review; 323 was
# a documentation-only ncu re-analysis; 324 was a documentation-only
# design writeup; 325 added an unrelated standalone probe with no main
# solver build/run requested). No fabricated BASELINE_320_* through
# BASELINE_325_* values are added here; 319 remains the most recent
# confirmed timing baseline until this 326 run (or a future one)
# produces a newer one. IMPORTANT: 326 is the FIRST kernel-LOGIC change
# since 311 -- every revision from 311 through 325 kept kernel logic
# byte-for-byte identical (variant=2, K=48) while investigating Stall
# Branch Resolving via profiling/diagnostics only. This run's timing
# comparison against 319 is the real correctness+performance test.
...
LOGDIR="${LOG_ROOT%/}/326Py_futurecheck_specialize_axis1_logs_N21_full_once_${TS}"
...
echo "[start] 326 futurecheck-specialize-axis1 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 326 futurecheck-specialize-axis1 SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '326 futurecheck-specialize-axis1' "$SRC"; then
  printf 'source_version_tag	326 futurecheck-specialize-axis1	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	326 futurecheck-specialize-axis1	missing	FAIL
...
# 304: K48-sweep shape checks (296 kernel logic, K=48).
    ...
    # 326: the hot loop is now duplicated in-place (future_check_mask==0 /
    # !=0 branches), so the push pattern legitimately appears 3 times:
    # root-preroll's own push (1) + loop A's push (1) + loop B's push (1).
    # Prior revisions (304-325) expected 2 (preroll + single hot loop).
    ok_stack_ptr_incr = body14.count('stack_ptr+=2') == 3
    ok_gridstride_loop = 'while idx<m:' in body14
    ok_single_writeback = 'results[tid]=thread_total\n' in body14
    ok_push = body14.count('stack[stack_ptr]=u64(cur_ld)') == 3
...
# 326: MAXD14 hot loop must now be specialized into two copies keyed on
# future_check_mask==0, and MAXD16/18/20/21 must NOT have this split
# (they were untouched -- checked separately above via source_maxdNN_unmodified).
if m14:
    body14_326 = m14.group(0)
    ok_split_present = ('if future_check_mask==u32(0):' in body14_326
                         and body14_326.count('while True:') >= 2)
    # the future_check_mask!=0 branch must keep the inner lookahead check
    # unconditionally; the ==0 branch must not contain it at all.
    else_idx = body14_326.find('if future_check_mask==u32(0):')
    ok_two_bodies = False
    if else_idx >= 0:
        after = body14_326[else_idx:]
        ok_two_bodies = after.count('(nibble_op&u32(8))!=u32(0)') == 1
    checks.append(('source_futurecheck_axis1_split', 'present, one copy each side', 'present' if (ok_split_present and ok_two_bodies) else 'missing/malformed', ok_split_present and ok_two_bodies))
else:
    checks.append(('source_futurecheck_axis1_split', 'present, one copy each side', 'MAXD14 body not found', False))
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 326 futurecheck-specialize-axis1 mode31 split145"
...
echo "[validation-ok] 326 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); FIRST kernel-logic change since 311: kernel_dfs_iter_gpu_maxd14's hot loop is now specialized into two in-place copies keyed on future_check_mask==0, chosen once per task (325 confirmed Codon inlines, so this avoids an unverified separate-function/pointer-passing risk); MAXD16/18/20/21 and everything else unchanged; if this run's elapsed time is not clearly better than 319's 455.116s, REVERT to 325 immediately per the header comment"
