...
# =============================================================================
# NQ_UPDATE_MEMO
# 190: 更新メモ: 189は正当性OKだが188比で大幅退行したため不採用。188のfuture_check_mask guard/no-sibling spill elisionへ戻し、MAXD14 kernelのみ block_check_mask==0 のscheduleでblock分岐を省く軽量guardを単独実験として導入。
...
189Pyログを確認しました。**正当性は問題ありません**。

N=21 fullで以下はすべてOKです。
...
ただし速度は `0:14:57.665` で、188Pyの `0:07:10.137` より `467.528秒`、`108.693%` 遅くなっています。したがって、189Pyは**正当性OKだが不採用**とし、190Pyは189を親にせず、**188Pyを親に戻して**作成しました。

作成物はこちらです。

* [190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_probe.py)
* [190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_bundle_20260630.tar)

190Pyの変更点は、188Pyの `future_check_mask` guard / no-sibling spill elision を維持したまま、MAXD14 kernelに **`block_check_mask==0` guard** を追加した点です。

```text
188Py:
  毎候補で block_code = nibble_op & 7
  block_code != 0 なら block metadata decode
  block_code == 0 なら通常1段diagonal更新

190Py:
  schedule生成時に block operation depth を block_check_mask へ集約

  block_check_mask == 0:
    hot loopで block_code分岐/decodeを省略
    通常1段diagonal更新へ直接進む

  block_check_mask != 0:
    188相当の per-depth block_code path を実行
```

189Pyの forced-chain fast path は外しています。187Pyの child_jmark guard、185Pyの terminal13 literal fast path、186Pyの terminal_base14 split も入れていません。190Pyは188Py親の単独実験です。

手元では以下を確認済みです。

```text
bash -n 190Py...sh                         OK
STATIC_ONLY=1 190Py...sh                   OK
source_future_check_mask_guard             OK
source_block_check_mask_guard              OK
source_189_forced_chain_removed            OK
source_nosibling_parent                    OK
source_split_tag split190 only             OK
abstract block_check_mask equivalence      100000 cases OK
```

こちらではCodon/CUDA実機build/full実行はしていません。190Pyの採用可否は、添付の `190Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に188比のtimingを見て判断してください。


"""

# 190 block-mask guard revision memo over validated 188 futuremask/no-sibling MAXD14:
#   Branch from validated 188 after 189 forced-chain was correct but much slower.
#   Host-side depth computation, scorestripe/chunkshape148 task order, cache naming,
#   dispatch range 14/16/18/20/21, bitboard arithmetic, and solution arithmetic are unchanged.
#
#   MAXD14 still uses PTX-safe scalar u32 schedule fields: schedule_lo/schedule_hi
#   store 14 x 4-bit nibble operations, plus scalar child-jmark mask, future-check
#   mask, terminal parent depth, terminal base14 bit, and root action.  MAXD16/18/20/21 keep the
...
#   only restorable ancestor frames.  190 keeps the 188 future_check_mask guard and
#   184 no-sibling descend policy, then adds only a block_check_mask guard:
#   when a deterministic schedule has no block-operation depth at all, skip the
#   per-candidate block_code branch/decode and use the normal one-row diagonal update directly.
#   Schedules with any block operation keep the 188 arithmetic.  The no-sibling
#   rule is unchanged: if the parent has no remaining sibling candidates after
#   selecting a bit, do not spill that parent to local memory.  N<=27 leaves
#   avail[] bits 27..31 free for packed logical depth, so MAXD14 remains
...
# 190 keeps the validated 188 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 190 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="190 blockmask-guard: 188 futuremask/no-sibling MAXD14 baseline plus block_check_mask zero guard - README-managed history"
...
# 190 inherits the exact 188/184/183/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """190 MAXD14 nibble schedule plus no-sibling/futuremask and block_check_mask zero guard."""
    ...
    while True:
      ...
      if schedule_block_code!=u32(0):
        block_check_mask|=u32(1)<<u32(schedule_depth)
    ...
    # 190 keeps the validated 188 current-frame register cache, 13-slot
    # ancestor storage, no-sibling tail-call descent, and future-prune guard.
    # It adds only a light guard around block-operation decoding: schedules with
    # block_check_mask==0 skip the per-candidate block_code branch/decode and
    # use the normal one-row diagonal update directly. The nonzero-mask path
    # keeps the exact 188 arithmetic.
    #
    ...
    while True:
      ...
      nld:u32=u32(0)
      nrd:u32=u32(0)
      # 190: if the schedule contains no block operation at any depth, avoid
      # the block_code branch/decode in the hot loop. Nonzero block schedules
      # retain the exact 188 per-depth block_code path.
      if block_check_mask==u32(0):
      ...
      if nf==u32(0):
        continue
      # 188/190: if no depth in this schedule carries the future-prune bit, avoid
...
CHUNKSHAPE148_DEFAULT_REASON:str="190 blockmask guard reuses validated 188 scorestripe_v9 task order/cache and changes only MAXD14 block-operation guard"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split190_blockmask_guard_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split190_blockmask_guard_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split190-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split190-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split190_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split190-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split190-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split190_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
