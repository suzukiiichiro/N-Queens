...
# =============================================================================
# NQ_UPDATE_MEMO
# 213: 更新メモ: 212Py rootfirstlate+andnot は正当性OKだが204最速を上回れなかったため未採用。213Pyは211のroot_first遅延を維持しつつ、205で試した root_after_second scalar削除を late-root-first 形に限定して再検証する。root_rest clear-lowbit / futuremask / no-sibling / root one-two preroll は維持する。
...
212Pyは204Pyより `0.033秒` 遅いため、**正当性OKだが不採用**です。

213Pyは、212の and-not predicate を撤回し、211の `root_first` 遅延を維持したまま、205で試した `root_after_second` scalar削除を late-root-first 形に限定して再検証します。
...
```text
bash -n 213Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 213Py...sh OK
source_rootfirstlate_inlinexor        OK
source_split_tag split145 split213    OK
```

ファイルはこちらです。

* [213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_probe.py)
* [213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_validate_full_once.sh

bash 213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 213 rootfirstlate_inlinexor memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  213 keeps the narrow root fast-start only when
...
VERSION_TAG:str="213 roottwo-rootfirstlate_inlinexor: 211 late root_first plus inline XOR root<=2 predicate over 204 rootclearrest futuremask/no-sibling MAXD14 baseline - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """213 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate late-root-first inline XOR predicate."""
    ...
    # correct but slower.  213 keeps 211's late root_first lowbit extraction and
    # changes only the root_after_second scalar shape from XOR to inline XOR:
    # if (root_rest ^ root_second) == 0.  3-bit and wider roots
    # stay on the generic loop.
    root_rest:u32=cur_avail&(cur_avail-u32(1))
    root_second:u32=root_rest&(u32(0)-root_rest)
    if (root_rest^root_second)==u32(0):
      root_first:u32=cur_avail&(u32(0)-cur_avail)
...
CHUNKSHAPE148_DEFAULT_REASON:str="213 rootfirstlate_inlinexor reuses validated 204 scorestripe_v9 task order/cache; MAXD14 delays root_first and tests inline XOR root_after_second"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split213_roottwo_rootfirstlate_inlinexor_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split213_roottwo_rootfirstlate_inlinexor_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split213-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split213-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split213_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split213-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split213-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split213_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
