...
# =============================================================================
# 328 N=21 full validation harness (warr-soa-split-implement)
# Base: 327 warrloadsplit-verify (kernel logic byte-for-byte identical
#   to 319-325's unified hot loop; 327's own N=21 run confirmed
#   454.563s, within 0.12% of 319's 455.116s). 326 (future_check_mask
#   in-kernel specialization) was tried and REJECTED (517.563s, -13.7%
#   slower than 319) -- unrelated to this revision's change.
#
# *** THIS IS A KERNEL-LOGIC CHANGE. *** Unlike 326 (which touched the
#   hot divergent DFS loop, executed per-node, billions of times), this
#   revision touches only the `w_arr` read that happens ONCE PER TASK
#   (not per DFS node) in all five kernels (kernel_dfs_iter_gpu_maxd14/
#   16/18/20/21), plus the shared dispatcher. This is a fundamentally
#   lower-risk change in kind, not just degree.
#
# WHAT CHANGED, and why: 327's w_arr_loadsplit probe (327_w_arr_
#   loadsplit_probe.py, 3-kernel comparison) confirmed the 323
#   hypothesis AND found the actual mechanism: w_arr (Ptr[u64], 8-byte
#   stride) compiles to two 32-bit LDG.E instructions (confirmed
#   general Codon/NVPTX behavior, not DFS-complexity-specific), and
#   BOTH loads individually carry non-zero L2 Theoretical Sectors
#   Global Excessive (25000 in the probe, matching 319's real-kernel
#   92,928-per-instruction finding) because the 8-byte stride puts
#   consecutive threads' addresses 8 bytes apart instead of 4 for a
#   given 32-bit half. The probe's w_probe_kernel_soa variant (w_arr
#   split into two SEPARATE densely-packed u32 arrays, each read with
#   plain contiguous indexing) achieved EXACTLY ZERO excessive sectors,
#   with the reconstructed values verified bit-identical to the
#   baseline (sum_base==sum_soa: True in the probe's own self-check).
#   A plain rephrasing-only variant (w_probe_kernel_tmpvar) made zero
#   difference, ruling out "just write it differently" -- the fix
#   requires an actual SoA data-layout change.
#
# This revision applies the SAME verified transformation to the real
#   kernels: w_arr:Ptr[u64] -> w_lo_arr:Ptr[u32],w_hi_arr:Ptr[u32] in
#   all 5 kernel signatures (14/16/18/20/21, for consistency -- 16/18/
#   20/21 are essentially never exercised for N=21 since
#   selected_maxd==14 always, but keeping all 5 in sync avoids a latent
#   signature mismatch). Every w_arr[idx]/w_arr[i] read site (3 per
#   kernel: two early-exit root_action paths plus the main post-loop
#   accumulation) is replaced with
#   (u64(w_lo_arr[X])|(u64(w_hi_arr[X])<<u64(32))), reconstructing the
#   identical u64 value from two u32 halves -- verified via the exact
#   same bit-combine formula the probe already confirmed correct.
#   launch_kernel_dfs_iter_gpu_static_maxd (the single shared dispatcher)
#   now derives w_lo_arr/w_hi_arr from the existing w_arr:List[u64]
#   ONCE per dispatch call and passes both to whichever kernel launches.
#
# WHAT DID NOT CHANGE: build_soa_for_range, w_arr's generation/storage
#   everywhere else in the file, the CPU verification path, and every
#   caller of launch_kernel_dfs_iter_gpu_static_maxd (its own signature
#   is unchanged -- it still accepts w_arr:List[u64] exactly as before;
#   only its internals now derive the SoA view before dispatching).
#   Every part of the DFS hot loop itself (the actual subject of 322's
#   Stall Branch Resolving finding) is completely untouched.
# THIS HAS NOT BEEN BUILT OR RUN ON REAL HARDWARE.
# MANDATORY VALIDATION ORDER (same discipline as 326, though this
#   change is lower-risk in kind):
#   1. STATIC_ONLY=1 first.
#   2. Full N=21 run. Confirm 314666222712 FIRST, before looking at
#      timing. Any deviation is an immediate abort.
#   3. Only after correctness is confirmed, compare timing against
#      327's 454.563s / 319's 455.116s. The OPT advisor's estimate was
#      11.79% for the whole kernel; since this touches only a
#      once-per-task read (not the billions-of-times-per-chunk hot
#      loop), the realistic improvement is likely far smaller than
#      11.79% of TOTAL runtime -- do not be surprised by a small delta,
#      but any regression should still trigger the same immediate
#      revert-to-327 discipline as every prior rejected change.
# EXPECTED_CHUNKS=3, K=48 (unchanged from 304).
# =============================================================================

SRC=${SRC:-./328Py_warr_soa_split_implement.py}
CAND=${CAND:-./328Py_warr_soa_split_implement}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/328Py_warr_soa_split_implement_N21_full.lock}
...
# NOTE: 328 is based on 327 (warrloadsplit-verify, itself kernel-logic
# identical to 319-325's unified hot loop). 326 (future_check_mask
# in-kernel specialization) was tried and REJECTED: correctness OK but
# elapsed=517.563s, -13.7% slower than 319's 455.116s baseline -- the
# worst of 5 failed GPU-side specialization attempts. Kept as a recorded
# baseline for reference, but this 328 lineage does not build on 326's
# code. 327's own N=21 run confirmed 454.563s (within 0.12% of 319),
# confirming the kernel-logic-identical claim. 319/327 (~454-455s)
# remain the most recent confirmed-good timing baselines; this revision
# is the first to change kernel logic since 326 (which was rejected).
...
LOGDIR="${LOG_ROOT%/}/328Py_warr_soa_split_implement_logs_N21_full_once_${TS}"
...
echo "[start] 328 warr-soa-split-implement validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 328 warr-soa-split-implement SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '328 warr-soa-split-implement' "$SRC"; then
  printf 'source_version_tag	328 warr-soa-split-implement	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	328 warr-soa-split-implement	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 328 warr-soa-split-implement mode31 split145"
...
echo "[validation-ok] 328 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); w_arr split into w_lo_arr/w_hi_arr (u32 SoA) across all 5 kernels + the shared dispatcher, per the 327 probe's verified fix (zero L2 Excessive sectors, sum_base==sum_soa); DFS hot loop itself untouched; if timing is not clearly better than 327's 454.563s / 319's 455.116s, revert to 327 per the header comment"
