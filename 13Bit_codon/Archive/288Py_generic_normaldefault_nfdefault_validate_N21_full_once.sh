...
# =============================================================================
# 288 N=21 full validation harness
# Parent: 287/286 normaldefault-nldnrd. Experiment: compute normal-path ncol/nf as defaults and recompute nf only for special block_code.
...
SRC=${SRC:-./288Py_generic_normaldefault_nfdefault_probe.py}
CAND=${CAND:-./288Py_generic_normaldefault_nfdefault_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/288Py_generic_normaldefault_nfdefault_probe_N21_full.lock}
...
BASELINE_287_ADOPT_SECONDS=${BASELINE_287_ADOPT_SECONDS:-424.033}
...
LOGDIR="${LOG_ROOT%/}/288Py_generic_normaldefault_nfdefault_logs_N21_full_once_${TS}"
...
echo "[start] 288 generic-normaldefault-nfdefault validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 288 generic-normaldefault-nfdefault SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '288 generic-normaldefault-nfdefault' "$SRC"; then
  printf 'source_version_tag	288 generic-normaldefault-nfdefault	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	288 generic-normaldefault-nfdefault	missing	FAIL
...
old_split_markers = ['split287', 'split286', 'split285', 'split284', 'split282', 'split281', 'split280', 'split278', 'split277', 'split275', 'split273', 'split270_', 'split240', 'split239_', 'kernel-blockdiag', 'block_count_diag', 'rootaction0-direct', 'ncol_early']
old_present = [x for x in old_split_markers if x in s]
if 'split288' in s and not old_present:
    checks.append(('source_split_tag', 'split288 active; rejected runtime tags absent', 'split288', True))
else:
    checks.append(('source_split_tag', 'split288 active; rejected runtime tags absent', 'present=' + ','.join(old_present) if old_present else 'split288 missing', False))
...
# Check MAXD14 generic DFS loop normal-default nld/nrd/ncol/nf shape. Keep root-preroll branch untouched.
...
if not m14:
    checks.append(('source_generic_nfdefault', 'MAXD14 generic normal-default nld/nrd/ncol/nf', 'MAXD14 not found', False))
else:
    body = m14.group(0)
    idx = body.find('block_code:u32=nibble_op&u32(7)')
    tail = body[idx:] if idx >= 0 else ''
    expected_snippet = '''nld:u32=(cur_ld|bit)<<u32(1)
      nrd:u32=(cur_rd|bit)>>u32(1)
      ncol:u32=cur_col|bit
      nf:u32=bm&~(nld|nrd|ncol)
      if block_code!=u32(0):'''
    expected_recompute = '''nld=((cur_ld|bit)<<stepu)|addvu|bLiu
        nrd=((cur_rd|bit)>>stepu)|bKu
        nf=bm&~(nld|nrd|ncol)'''
    bad_snippet = '''if block_code!=u32(0):
        stepu:u32='''
    ok = expected_snippet in tail and expected_recompute in tail
    checks.append(('source_generic_nfdefault', 'MAXD14 generic normal-default nld/nrd/ncol/nf', 'present' if ok else 'missing/old-form', ok))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 288 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 288 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 288 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 288 generic-normaldefault-nfdefault mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "287adopt:$BASELINE_287_ADOPT_SECONDS" "286adopt:$BASELINE_286_ADOPT_SECONDS" "285restore:$BASELINE_285_RESTORE_SECONDS" "284rejected:$BASELINE_284_REJECT_SECONDS" "283normalfirst:$BASELINE_283_NORMALFIRST_SECONDS" "276current:427.672" "271fastest:427.705" "239:427.703" "217:427.709"; do
...
echo "[validation-ok] 288 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
