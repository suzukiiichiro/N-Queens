...
# =============================================================================
# 343_bucket_run_cap_raise2_driver.sh
#
...
#   SWEEP="64 96 256 64" bash 343_bucket_run_cap_raise2_driver.sh   # quick version
#   DRY_RUN=1        bash 343_bucket_run_cap_raise2_driver.sh       # plan only
#   AGGREGATE_ONLY=1 SWEEPDIR=./343_bucket_run_cap_raise2_<ts> \
#                    bash 343_bucket_run_cap_raise2_driver.sh       # rebuild table
# =============================================================================

HARNESS=${HARNESS:-./343Py_bucket_run_cap_raise2_validate_N21_full_once.sh}
SRC=${SRC:-./343Py_bucket_run_cap_raise2.py}
LOGPREFIX=${LOGPREFIX:-./343Py_bucket_run_cap_raise2_logs_N21_full_once_}
SWEEP=${SWEEP:-"2048 4096 8192 16384 32768 2048"}
DRY_RUN=${DRY_RUN:-0}
...
REF_RUN64=${REF_RUN64:-431.684}  # 343: now the RUN=2048 reference

TS=$(date -u +%Y%m%d_%H%M%S)
SWEEPDIR=${SWEEPDIR:-./343_bucket_run_cap_raise2_${TS}}
mkdir -p "$SWEEPDIR"
...
echo "[start] 343 bucket-run cap-raise2 driver"
...
# ---------------------------------------------------------------------------
# 343: saturation check at the DATA level, no GPU time required.
# quotas[] sums to m_target = STEPS = BLOCK*MAX_BLOCKS = 15488, so any
# RUN >= 15488 emits every bucket's whole quota as one unbroken block and all
# such values MUST produce byte-identical shaped bins. Equal digests confirm
# the model; differing digests falsify it.
# ---------------------------------------------------------------------------
SATURATION_AT=$((BLOCK * MAX_BLOCKS))
echo
echo "[saturation-check] shaped-bin digests (RUN >= $SATURATION_AT must all match)"
DIGEST_TABLE="$SWEEPDIR/shaped_bin_digests.tsv"
printf 'bucket_run\tat_or_above_saturation\tdigest\tbytes\tfile\n' > "$DIGEST_TABLE"
for r in $SWEEP; do
  b=$(shaped_bin_for "$r")
  [[ -f "$b" ]] || continue
  grep -q "^$r	" "$DIGEST_TABLE" && continue
  dg=$(md5sum "$b" 2>/dev/null | awk '{print $1}')
  sz=$(wc -c < "$b" | tr -d ' ')
  flag=no; (( r >= SATURATION_AT )) && flag=yes
  printf '%s\t%s\t%s\t%s\t%s\n' "$r" "$flag" "${dg:-?}" "$sz" "$b" >> "$DIGEST_TABLE"
done
{ column -t -s $'\t' "$DIGEST_TABLE" 2>/dev/null || cat "$DIGEST_TABLE"; }
SAT_DISTINCT=$(awk -F'\t' 'NR>1 && $2=="yes"{print $3}' "$DIGEST_TABLE" | sort -u | wc -l | tr -d ' ')
SAT_COUNT=$(awk -F'\t' 'NR>1 && $2=="yes"' "$DIGEST_TABLE" | wc -l | tr -d ' ')
if (( SAT_COUNT >= 2 )); then
  if (( SAT_DISTINCT == 1 )); then
    echo "[saturation-check] CONFIRMED: $SAT_COUNT values at or above $SATURATION_AT share one digest."
  else
    echo "[saturation-check] FALSIFIED: $SAT_COUNT values at or above $SATURATION_AT produced $SAT_DISTINCT distinct digests." >&2
    echo "[saturation-check] The quota-saturation model is wrong; re-derive it before sweeping further." >&2
  fi
else
  echo "[saturation-check] inconclusive: fewer than two swept values are at or above $SATURATION_AT."
fi
