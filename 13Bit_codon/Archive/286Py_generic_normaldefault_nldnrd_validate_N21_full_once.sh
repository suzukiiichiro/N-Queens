...
# =============================================================================
# 286 N=21 full validation harness
# Parent: 285/283 generic-normalfirst. Experiment: compute normal-path nld/nrd as defaults and overwrite only for special block_code.
...
SRC=${SRC:-./286Py_generic_normaldefault_nldnrd_probe.py}
CAND=${CAND:-./286Py_generic_normaldefault_nldnrd_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/286Py_generic_normaldefault_nldnrd_probe_N21_full.lock}
...
BASELINE_285_RESTORE_SECONDS=${BASELINE_285_RESTORE_SECONDS:-427.698}
...
LOGDIR="${LOG_ROOT%/}/286Py_generic_normaldefault_nldnrd_logs_N21_full_once_${TS}"
...
echo "[start] 286 generic-normaldefault-nldnrd validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 286 generic-normaldefault-nldnrd SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '286 generic-normaldefault-nldnrd' "$SRC"; then
  printf 'source_version_tag	286 generic-normaldefault-nldnrd	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	286 generic-normaldefault-nldnrd	missing	FAIL
...
old_split_markers = ['split285', 'split284', 'split282', 'split281', 'split280', 'split278', 'split277', 'split275', 'split273', 'split270_', 'split240', 'split239_', 'kernel-blockdiag', 'block_count_diag', 'rootaction0-direct']
old_present = [x for x in old_split_markers if x in s]
if 'split286' in s and not old_present:
    checks.append(('source_split_tag', 'split286 active; rejected runtime tags absent', 'split286', True))
else:
    checks.append(('source_split_tag', 'split286 active; rejected runtime tags absent', 'present=' + ','.join(old_present) if old_present else 'split286 missing', False))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 286 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 286 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 286 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 286 generic-normaldefault-nldnrd mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "285restore:$BASELINE_285_RESTORE_SECONDS" "284rejected:$BASELINE_284_REJECT_SECONDS" "283normalfirst:$BASELINE_283_NORMALFIRST_SECONDS" "276current:427.672" "271fastest:427.705" "239:427.703" "217:427.709"; do
...
echo "[validation-ok] 286 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
