...
# =============================================================================
# NQ_UPDATE_MEMO
# 243: schedule-precompute probe。241 restore239本線を親に、別launchなし・単一launch維持のまま、
...
"""
243 schedule-precompute note: 241 restore239本線を親に、240のfid14/rest別launchと242のfuturemask-depthbit微差を採用せず、単一launchのままMAXD14 schedule生成をhost precomputeへ移す構造probe。


作成しました。243は **241親 / 単一launch維持 / MAXD14 schedule生成host precompute probe** です。

240のfid14/rest別launch、242のfuturemask-depthbit微差は採用せず、dispatch rowsは従来通り `131` 前提です。変更点は、MAXD14 kernel冒頭のschedule生成interpreterを `build_soa_for_range()` 側で事前生成し、kernelには配列で渡す構造変更です。
...
source_schedule_precompute      OK
source_host_precompute_arrays   OK
release_build_policy            OK
```

ファイルはこちらです。

* [243Py_schedule_precompute_probe.py](sandbox:/mnt/data/243Py_schedule_precompute_probe.py)
* [243Py_schedule_precompute_validate_N21_full_once.sh](sandbox:/mnt/data/243Py_schedule_precompute_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [243Py_schedule_precompute_sha256.txt](sandbox:/mnt/data/243Py_schedule_precompute_sha256.txt)
* [243Py_schedule_precompute_bundle_20260707.tar](sandbox:/mnt/data/243Py_schedule_precompute_bundle_20260707.tar)

sha256です。

```text
83baa0373988e11df5dab4db1dfc96829f30a7c42d05ef322f3926d34f45aa01  243Py_schedule_precompute_probe.py
14c5568445925f697d7c0c1a16c9bdf294c1e127011b70651e7ee9dd395fed56  243Py_schedule_precompute_validate_N21_full_once.sh
a124cdaa10be50927ec37095233661fbadd1c316e7ec882a4e9d9f4769ccd92e  README.md
```

実行はいつも通りです。

```bash
rm -f ./243Py_schedule_precompute_probe

STATIC_ONLY=1 bash 243Py_schedule_precompute_validate_N21_full_once.sh

bash 243Py_schedule_precompute_validate_N21_full_once.sh
```

この環境には Codon がないため、こちらでは `codon build -release` は実行できていません。cudacodon側ではまず `STATIC_ONLY=1` の一致を見てからfullへ進めてください。
...
VERSION_TAG:str="243 schedule-precompute probe: parent 241 restore239; single launch kept; MAXD14 schedule interpreter moved to host precomputed arrays; 240 fid14 split and 242 condition micro-diff rejected; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 243: bare -g stays split145 mode31; single-launch schedule-precompute probe
...
# 243: keep runtime globals required by broadmarktail/chunkshape148/split145
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 243: host-precomputed schedule fields for MAXD14. These mirror the
    # former per-thread schedule interpreter in kernel_dfs_iter_gpu_maxd14.
    self.sched_lo_arr:List[u32]=[u32(0)]*m
    self.sched_hi_arr:List[u32]=[u32(0)]*m
    self.child_jmark_mask_arr:List[u32]=[u32(0)]*m
    self.future_check_mask_arr:List[u32]=[u32(0)]*m
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32,u32]:
  # 243: host-side clone of the former MAXD14 kernel schedule builder.
  ...
  while True:
    ...
    raw=frame_raw
    schedule_depth+=1
  return schedule_lo,schedule_hi,child_jmark_mask,future_check_mask,terminal_parent_depth,terminal_is_base14,root_action
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    sched_lo_arr:Ptr[u32],sched_hi_arr:Ptr[u32],
    child_jmark_mask_arr:Ptr[u32],future_check_mask_arr:Ptr[u32],
    ...
    # 243: schedule fields are precomputed on the host in build_soa_for_range().
    # The former per-thread schedule interpreter is intentionally absent here.
    schedule_lo:u32=sched_lo_arr[i]
    schedule_hi:u32=sched_hi_arr[i]
    child_jmark_mask:u32=child_jmark_mask_arr[i]
    future_check_mask:u32=future_check_mask_arr[i]
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.sched_lo_arr),gpu.raw(soa.sched_hi_arr),
      gpu.raw(soa.child_jmark_mask_arr),gpu.raw(soa.future_check_mask_arr),
...
# 243: recursive CPU dfs fallback remains removed. CPU path uses dfs_iter only.
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        root_action:u32=u32(0)
        sched_lo,sched_hi,child_jmark_mask,future_check_mask,terminal_depth_u,terminal_base14,root_action=precompute_maxd14_schedule_fields(soa.ctrl0_arr[t],soa.markctrl_arr[t],meta_next_sched)
        soa.sched_lo_arr[t]=sched_lo
        soa.sched_hi_arr[t]=sched_hi
        soa.child_jmark_mask_arr[t]=child_jmark_mask
        soa.future_check_mask_arr[t]=future_check_mask
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        for p in range(m):
          ...
          sort_soa.child_jmark_mask_arr[p]=soa.child_jmark_mask_arr[q]
          sort_soa.future_check_mask_arr[p]=soa.future_check_mask_arr[q]
...
CHUNKSHAPE148_DEFAULT_REASON:str="243 schedule-precompute keeps validated 241/239/238 scorestripe_v9 task order/cache; single-launch MAXD14 schedule precompute probe"
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 243 schedule-precompute; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split243_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split243_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split243-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split243-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split243_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
