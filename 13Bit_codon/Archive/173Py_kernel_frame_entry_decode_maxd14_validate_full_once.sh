...
# 173 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 173 inherits the
# validated 172 ancestor13-stack MAXD14 static dispatch.  Its single kernel
# experiment keeps the 170 scalar schedule, 171 active-frame register cache,
# and 172 u32[13] ancestor stack, but decodes each current frame's compact
# operation once when that frame becomes active.
#
# For N=21 all launches must remain required_maxd=14, select MAXD14, report
# schedule_words=0 and 208 local stack bytes/thread.

SRC=${SRC:-./173Py_kernel_frame_entry_decode_maxd14_probe.py}
CAND=${CAND:-./173Py_kernel_frame_entry_decode_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/173Py_kernel_frame_entry_decode_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 173 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/173Py_kernel_frame_entry_decode_maxd14_logs_N21_full_once_${TS}"
...
assert '173 frame-entry decode GPU candidate supports N=5..27 only' in s
...
# schedule fields, the 172 register-held active frame, and 173 frame-entry metadata decode.
...
def decode_frame_metadata_from_compact(comp,n3,n4):
    blocks,step,add,bl,kt,future=decode_compact(comp)
    bk=0
    if blocks:
        bk=(n3&-(kt&1))|(n4&-((kt>>1)&1))
    return blocks,step,add,bl,bk,future

def dfs_entry_decode(N,raw,ld0,rd0,col0,a0,j,e,m1,m2,schedule_capacity=14,ancestor_capacity=13,limit=120000):
    bm=(1<<N)-1;n3=1<<max(0,N-3);n4=1<<max(0,N-4)
    a0&=bm
    if a0==0:return 0,0,-1,0,0
    sched,depth=build_scalar_schedule(raw,j,e,m1,m2,schedule_capacity)
    root=scalar_root_at(sched)
    if root==2:return 1,0,-1,0,0
    if root==3:return int((a0&~1)!=0),0,-1,0,0
    if root==1:
        a0&=~1
        if a0==0:return 0,0,-1,0,0
        ld0|=1
    ...
    # Mirrors 173: arrays contain only 13 ancestor frames; the active frame is scalar.
    # Operation metadata is decoded only when a frame becomes current (root, descend, restore),
    # not once for every candidate bit in the same frame.
    ...
    cur_blocks,cur_step,cur_add,cur_bl,cur_bk,cur_future=decode_frame_metadata_from_compact(scalar_op_at(sched,sp),n3,n4)
    decode_count=1
    total=0;maxframes=1;steps=0
    ...
    while True:
        ...
        candidate_count+=1
        if cur_blocks:
            nld=((cur_ld|bit)<<cur_step)|cur_add|cur_bl;nrd=((cur_rd|bit)>>cur_step)|cur_bk
        else:
            nld=(cur_ld|bit)<<1;nrd=(cur_rd|bit)>>1
        ncol=cur_col|bit;nf=bm&~(nld|nrd|ncol)
        if nf==0:continue
        if cur_future and bm&~((nld<<1)|(nrd>>1)|ncol)==0:continue
        if sp==terminal:
    ...
    assert decode_count==1+store_count+load_count
    return total,maxframes,max_saved_index,candidate_count,decode_count

random.seed(173172)
dfs_cases=0;attempts=0;max_observed=0;max_depth_tested=0;max_saved_index=-1;max_candidate_count=0;max_decode_count=0
while dfs_cases<12000 and attempts<220000:
    ...
    try:
        ...
        got,obs2,saved,cands,decodes=dfs_entry_decode(N,raw,ld,rd,col,a,j,e,m1,m2,14,13)
    except RuntimeError:
        continue
    ...
    max_candidate_count=max(max_candidate_count,cands)
    max_decode_count=max(max_decode_count,decodes)
    assert decodes<=2*cands+1 or cands==0,(decodes,cands)
    dfs_cases+=1
...
print('MAXD14_FRAME_ENTRY_DECODE=1')
...
print(f'DFS_MAX_CANDIDATE_COUNT={max_candidate_count}')
print(f'DFS_MAX_DECODE_COUNT={max_decode_count}')
...
PYMODEL
then
  printf 'source_frame_entry_decode_shape\trequested layout\tmodel/static failure\tFAIL\n' >> "$SUMMARY"
...
printf 'source_parent_baseline\tvalidated 172 ancestor13-stack MAXD14 dispatch\tverified\tOK\n' >> "$SUMMARY"
printf 'source_frame_entry_decode_shape\tMAXD14 scalar schedule + register-held current frame + 13-slot ancestor stack + entry decode\tverified\tOK\n' >> "$SUMMARY"
...
printf 'maxd14_frame_entry_decode_schedule\t1\t%s\tOK\n' "$MAXD14_FRAME_ENTRY_DECODE" >> "$SUMMARY"
...
printf 'dfs_164_vs_173_entrydecode_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
printf 'dfs_entrydecode_random_max_depth_tested\t>=7\t%s\tOK\n' "$DFS_MAX_DEPTH_TESTED" >> "$SUMMARY"
printf 'dfs_entrydecode_random_max_saved_index\t<=12\t%s\tOK\n' "$DFS_MAX_SAVED_INDEX" >> "$SUMMARY"
printf 'dfs_entrydecode_candidate_count_seen\t>0\t%s\tOK\n' "$DFS_MAX_CANDIDATE_COUNT" >> "$SUMMARY"
printf 'dfs_entrydecode_decode_count_seen\t>0\t%s\tOK\n' "$DFS_MAX_DECODE_COUNT" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 173 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, frame-entry decode + current-frame register cache + ancestor13 stack"
  echo "================================================================"
} | tee "$RUN_LOG"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "172:$BASELINE_172_SECONDS" "171:$BASELINE_171_SECONDS" "170:$BASELINE_170_SECONDS" "169:$BASELINE_169_SECONDS" "168:$BASELINE_168_SECONDS" "167:$BASELINE_167_SECONDS" "166:$BASELINE_166_SECONDS" "165:$BASELINE_165_SECONDS" "164:$BASELINE_164_SECONDS" "162:$BASELINE_162_SECONDS"; do
...
echo "[validation-ok] 173 frame-entry decode + ancestor13 stack reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
