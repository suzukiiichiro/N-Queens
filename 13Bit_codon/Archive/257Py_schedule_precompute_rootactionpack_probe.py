...
# =============================================================================
# NQ_UPDATE_MEMO
# 257: schedule-precompute-rootactionpack probe。256 base14pack JIT成立後の切り分け版。
...
"""
257 schedule-precompute note: 256 base14pack JIT成立結果を受け、255 actionpackのPTX/JIT不成立をさらに切り分けるため、root_actionだけを(root_action<<1)のsched_root_action_arrからdecodeし、terminal_depth/terminal_base14は別配列のまま保持するprobe。

256は正当性OKで、`terminal_base14` 単独low-bit decodeはJIT安全と判断できます。速度は241比で遅いので採用候補ではありませんが、切り分けとしては有効です。

次の257は、予定通り **root_action単独low-bit decode probe** にしました。

```text
257:
  親: 256 base14pack JIT-ok / 244 r4 maskpack / 241 restore239
  単一launch維持
  child/future maskpack維持
  terminal_base14 は sched_base14_arr のまま
  terminal_depth は別配列のまま
  root_action だけ sched_root_action_arr から low-bit decode
```

重要な修正点として、257では255 actionpack由来のshift decodeを混ぜず、以下の形に固定しています。

```python
sched_root_action = root_action & 3
root_action = sched_root_action_arr[i] & 3
```
...
source_schedule_precompute_rootactionpack  OK
source_host_precompute_rootactionpack_arrays OK
release_build_policy                       OK
```

ファイルはこちらです。

* [257Py_schedule_precompute_rootactionpack_probe.py](sandbox:/mnt/data/257Py_schedule_precompute_rootactionpack_probe.py)
* [257Py_schedule_precompute_rootactionpack_validate_N21_full_once.sh](sandbox:/mnt/data/257Py_schedule_precompute_rootactionpack_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [257Py_schedule_precompute_rootactionpack_sha256.txt](sandbox:/mnt/data/257Py_schedule_precompute_rootactionpack_sha256.txt)
* [257Py_schedule_precompute_rootactionpack_bundle_20260707.tar](sandbox:/mnt/data/257Py_schedule_precompute_rootactionpack_bundle_20260707.tar)

sha256です。

```text
423cd780f458914e6a80b1846ec9f5d904a83c08defbd365723503200c0bdb48  257Py_schedule_precompute_rootactionpack_probe.py
041c815ca73747716d7e79f70b666a9565c554c40d9215bc3082a2bdd04bce78  257Py_schedule_precompute_rootactionpack_validate_N21_full_once.sh
c4cbeb5bdf9b24a64f9930e3e18f57b507ebdea4873c3d0f040ac4bbd422ce8e  README.md
```

実行は以下でお願いします。

```bash
rm -f ./257Py_schedule_precompute_rootactionpack_probe

STATIC_ONLY=1 bash 257Py_schedule_precompute_rootactionpack_validate_N21_full_once.sh

bash 257Py_schedule_precompute_rootactionpack_validate_N21_full_once.sh
```
...
VERSION_TAG:str="257 schedule-precompute-rootactionpack probe: parent 256 base14pack JIT-ok / 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; child/future maskpack retained; 256 proved terminal_base14-only low-bit decode is JIT-safe; only root_action is decoded from shifted sched_root_action_arr while terminal_depth/terminal_base14 stay separate; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 257: bare -g stays split145 mode31; root_action-only pack probe
...
# 257: keep runtime globals required by broadmarktail/chunkshape148/split145
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 257: 256 rootactionpack JIT-ok shape plus root_action-only decode
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 257: host-side clone of the former MAXD14 kernel schedule builder.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 257: schedule fields are precomputed on the host in build_soa_for_range().
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split257_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split257_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      progress_suffix:str=f"split257_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
