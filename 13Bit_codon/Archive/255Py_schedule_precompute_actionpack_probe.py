...
# =============================================================================
# NQ_UPDATE_MEMO
# 244 r4: schedule-precompute probe。241 restore239本線を親に、別launchなし・単一launch維持のまま、
#      MAXD14 kernel冒頭のschedule生成interpreterをhost側precomputeへ逃がす構造変更。
#      kernelには schedule_lo/hi, future_check_mask, child_jmark_mask,
#      terminal_depth/base14, root_action を配列で渡す。
#      240 fid14/rest別launch、242 futuremask-depthbit微差は不採用として入れない。
...
"""
244 r4 schedule-precompute note: 241 restore239本線を親に、240のfid14/rest別launchと242のfuturemask-depthbit微差を採用せず、単一launchのままMAXD14 schedule生成をhost precomputeへ移す構造probe。

このログは **正当性OK、速度は不採用寄り**です。

ただし一点だけ注意です。貼っていただいた出力は、

```text
[progress] ./244Py_schedule_precompute_maskpack_logs...
[validation-ok] 243 N=21 ...
```

となっているので、**255ではなく244 r4 maskpack側の実行結果**に見えます。`[validation-ok] 243` はシェル末尾メッセージの旧タグ残りで、計算結果そのものには影響ありません。

結果としては：

```text
final total        314666222712 OK
full_chunk_sum     314666222712 OK
error/mismatch     0 OK
elapsed            0:07:10.666
```
...
なので、**maskpackはJIT成立・正当性OKだが、速度改善には至らない**という判定です。
ただし価値はあります。これで少なくとも、

```text
child_jmark_mask + future_check_mask の pack 自体は JIT 不成立要因ではない
```

と切り分けられました。

255 actionpack の評価をする場合は、次を実行してください。

```bash
rm -f ./255Py_schedule_precompute_actionpack_probe

STATIC_ONLY=1 bash 255Py_schedule_precompute_actionpack_validate_N21_full_once.sh

bash 255Py_schedule_precompute_actionpack_validate_N21_full_once.sh
```

もし255もJIT成立して同程度に遅いなら、次の結論になります。

```text
precompute自体は正しい
packも一部はJIT安全
しかし追加global loadとdecodeが重く、241を上回らない
```

その場合、次は「さらにpackを増やす」より、**既存配列の再利用**へ移る方が筋が良いです。たとえば `ctrl0_arr` / `markctrl_arr` を MAXD14 dispatch直前だけ `schedule_lo` / packed fields に上書きして、新規配列loadを減らす方向です。


作成しました。255は **244 r4 maskpack成立後の次段**として、`terminal_base14 + root_action` だけを低位bit packする切り分け版です。

255の狙いです。

```text
親:
  244 r4 maskpack / 241 restore239
...
維持:
  ...
  cache生成
  worker split
  MAXD14/16/18/20/21 fallback

追加:
  sched_action_arr = terminal_base14 | (root_action << 1)

維持して分ける:
  terminal_depth_arr は separate のまま
```

244 r1〜r3で `terminal_depth / terminal_base14 / root_action` をまとめたctrl-packが `CUDA_ERROR_INVALID_PTX` になったため、255ではまず **terminal_depthを混ぜずに、terminal_base14/root_actionだけ** をpackします。JIT成立すれば、次に terminal_depth 単独packや既存配列再利用へ進めます。
...
source_schedule_precompute_actionpack  OK
source_host_precompute_actionpack_arrays OK
release_build_policy                   OK
```

ファイルはこちらです。

* [255Py_schedule_precompute_actionpack_probe.py](sandbox:/mnt/data/255Py_schedule_precompute_actionpack_probe.py)
* [255Py_schedule_precompute_actionpack_validate_N21_full_once.sh](sandbox:/mnt/data/255Py_schedule_precompute_actionpack_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [255Py_schedule_precompute_actionpack_sha256.txt](sandbox:/mnt/data/255Py_schedule_precompute_actionpack_sha256.txt)
* [255Py_schedule_precompute_actionpack_bundle_20260707.tar](sandbox:/mnt/data/255Py_schedule_precompute_actionpack_bundle_20260707.tar)

sha256です。

```text
01cb83d20688c5b44d8876e6a4efb3c3b2ef5fffdd49a0c7a5126b8c6f84341d  255Py_schedule_precompute_actionpack_probe.py
e625ad5396b222dc2f031697530b4147e11ae6839170fb15566d944f7ee6b7d1  255Py_schedule_precompute_actionpack_validate_N21_full_once.sh
03f2c3c984884edac38bcfec72058d8f000c05e662dfbeccc9d7765852199b27  README.md
```
...
まず見るべき順番は、**JIT成立 → 正当性OK → 244 r4 / 243 / 241との速度比較**です。
...
VERSION_TAG:str="255 schedule-precompute-actionpack probe: parent 244 r4 maskpack / 241 restore239; single launch kept; child/future maskpack retained; terminal_base14+root_action packed into sched_action_arr; terminal_depth remains separate to continue PTX/JIT isolation; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 244 r4: bare -g stays split145 mode31; single-launch schedule-precompute probe
...
# 255: keep runtime globals required by broadmarktail/chunkshape148/split145
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 255: host-precomputed schedule fields for MAXD14. 244 r4 proved
    # child_jmark_mask/future_check_mask can be packed into sched_mask_arr.
    # 255 next packs only terminal_base14/root_action into sched_action_arr;
    # terminal_depth remains separate to avoid the previous ctrl-pack PTX/JIT risk.
    self.sched_lo_arr:List[u32]=[u32(0)]*m
    self.sched_hi_arr:List[u32]=[u32(0)]*m
    self.sched_mask_arr:List[u32]=[u32(0)]*m
    self.terminal_depth_arr:List[u32]=[u32(0)]*m
    self.sched_action_arr:List[u32]=[u32(0)]*m
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32]:
  # 255: host-side clone of the former MAXD14 kernel schedule builder.
  # Child/future masks remain packed as in 244 r4. This probe additionally packs
  # only terminal_base14/root_action into sched_action_arr, leaving terminal_depth separate.
  ...
  sched_action:u32=(terminal_is_base14&u32(1))|((root_action&u32(3))<<u32(1))
  return schedule_lo,schedule_hi,sched_mask,terminal_parent_depth,sched_action
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    sched_lo_arr:Ptr[u32],sched_hi_arr:Ptr[u32],sched_mask_arr:Ptr[u32],
    terminal_depth_arr:Ptr[u32],sched_action_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 255: schedule fields are precomputed on the host in build_soa_for_range().
    # Child/future masks use the 244 r4 JIT-established maskpack. This probe packs
    # only terminal_base14/root_action into a low-bit sched_action value.
    schedule_lo:u32=sched_lo_arr[i]
    ...
    terminal_depth:int=int(terminal_depth_arr[i])
    sched_action:u32=sched_action_arr[i]
    terminal_base14:u32=sched_action&u32(1)
    root_action:u32=(sched_action>>u32(1))&u32(3)
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.sched_lo_arr),gpu.raw(soa.sched_hi_arr),gpu.raw(soa.sched_mask_arr),
      gpu.raw(soa.terminal_depth_arr),gpu.raw(soa.sched_action_arr),
...
# 244 r4: recursive CPU dfs fallback remains removed. CPU path uses dfs_iter only.
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        sched_action:u32=u32(0)
        sched_lo,sched_hi,sched_mask,terminal_depth_u,sched_action=precompute_maxd14_schedule_fields(soa.ctrl0_arr[t],soa.markctrl_arr[t],meta_next_sched)
        soa.sched_lo_arr[t]=sched_lo
        soa.sched_hi_arr[t]=sched_hi
        soa.sched_mask_arr[t]=sched_mask
        soa.terminal_depth_arr[t]=terminal_depth_u
        soa.sched_action_arr[t]=sched_action
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        for p in range(m):
          ...
          sort_soa.terminal_depth_arr[p]=soa.terminal_depth_arr[q]
          sort_soa.sched_action_arr[p]=soa.sched_action_arr[q]
...
CHUNKSHAPE148_DEFAULT_REASON:str="244 r4 schedule-precompute keeps validated 241/239/238 scorestripe_v9 task order/cache; single-launch MAXD14 schedule precompute probe"
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 244 r4 schedule-precompute; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split255_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split255_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split255-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split255-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split255_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
