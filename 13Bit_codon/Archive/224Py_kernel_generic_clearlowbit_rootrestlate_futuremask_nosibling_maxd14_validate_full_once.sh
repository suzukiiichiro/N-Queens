...
# =============================================================================
# NQ_UPDATE_MEMO
# 223: 222Py MAXD13準備診断は正当性OKだが max_save_sp=13 / save_sp13_count>0 のため
#      MAXD13単純縮小は不可。診断オーバーヘッドも大きかったため、223Pyは診断を撤回し、
#      217Py rootrestlate最速基準相当のMAXD14 scalar predicateへ戻す安全復帰版。
# Full update history: see README.md
# =============================================================================

SRC=${SRC:-./224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 224 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
marker='# 224 rootrestlate parent + generic clear-lowbit update:'
...
checks={
  ...
  'root_after_scalar_present':'root_after_second:u32=root_rest^root_second' in root_compact,
  'root_after_if_present':'ifroot_after_second==u32(0):' in root_compact,
  'inline_xor_if_absent':'if(root_rest^root_second)==u32(0):' not in root_compact,
  'root_first_late_present':root_compact.find('root_first:u32=cur_avail&(u32(0)-cur_avail)') > root_compact.find('ifroot_after_second==u32(0):') >= 0,
  'cur_avail_rootrest_late':'cur_avail=root_rest' in root_compact and root_compact.find('cur_avail=root_rest') > root_compact.find('pr_child_jmark:u32=child_jmark_mask&u32(1)') >= 0,
  'root_tail_absent':'root_tail:u32=' not in root_compact and 'ifroot_tail==u32(0):' not in root_compact,
  'andnot_absent':'root_rest&(~root_second)' not in root_compact,
  'diag_kernel_absent':'diag_save_sp:Ptr[u32]' not in compact and '[maxd13-diag]' not in s,
  'generic_clear_lowbit_present':'cur_avail=cur_avail&(cur_avail-u32(1))' in compact,
  'generic_xor_clear_absent':'cur_avail=cur_avail^bit' not in compact,
}
if all(checks.values()):
    sys.exit(0)
print('source_generic_clearlowbit_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_generic_clearlowbit\trootrestlate_plus_generic_clear_lowbit\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_generic_clearlowbit\trootrestlate_plus_generic_clear_lowbit\tmissing_or_old_shape\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split224'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split224' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit224 active preferred\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit224 active preferred\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if (( static_failures != 0 )); then
  echo "================================================================"; echo "[static-summary]"; cat "$SUMMARY"; echo "[logdir] $LOGDIR"
  echo "[error] 224 source static checks failed" >&2
  exit 66
fi
...
echo "[validation-ok] 224 generic clear-lowbit reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
