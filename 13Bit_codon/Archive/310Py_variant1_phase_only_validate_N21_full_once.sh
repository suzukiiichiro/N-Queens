...
# =============================================================================
# 310 N=21 full validation harness (variant1-phase-only probe)
# Parent: 309 variant4-phase-rotate — REJECTED. 481.149s vs 304 K=48=351.070s,
#   delta -130.079s (-37.052%), severe regression far larger than anything
#   seen in the K sweep. Per-chunk breakdown (chunk0=169.369s, chunk1=172.626s,
#   chunk2=119.915s vs 304's ~117s/chunk average) showed all 3 chunks slower,
#   not a localized anomaly -- the reordered task stream made SIMT lane
#   imbalance worse, not better.
# Experiment: variant=4(phase_rotate) = phase_mix=1 (cell/risk-aware tail
#   phase) + rotate_interleave=1 (rotating interleave), on top of variant=2
#   (rotate_only, the validated-safe production default since 115) which is
#   phase_mix=0 + rotate_interleave=1. The only difference between variant=2
#   and variant=4 is phase_mix. This probe isolates that one factor by testing
#   variant=1 (phase_only: phase_mix=1, rotate_interleave=0) -- same branch
#   logic already in source (broad_markdist_tail_use_phase_mix() is True for
#   v==1,4,5; broad_markdist_tail_use_rotating_interleave() is True for
#   v==2,4,5), zero kernel code changes, pure CLI-argument switch.
#   - If variant=1 comes back near variant=2 (~351s): the 309 regression was
#     the phase_mix + rotate_interleave *combination*, not phase_mix alone.
#   - If variant=1 also regresses like 309: phase_mix (cell/risk-aware tail
#     phase) itself is the problem, and task-reorder-side approaches to the
#     ncu-identified warp imbalance should be reconsidered.
# Safety: kernel_dfs_iter_gpu_maxd14/16/18/20/21 bodies are byte-identical to
#   304/308/309; only two top-level int constants change. No hot-loop, stack,
#   or K-batching logic is touched.
...
#           from 304/308/309), BROADMARK_VARIANT=1 (phase_only).
...
SRC=${SRC:-./310Py_variant1_phase_only.py}
CAND=${CAND:-./310Py_variant1_phase_only}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/310Py_variant1_phase_only_N21_full.lock}
...
BROADMARK_VARIANT=${BROADMARK_VARIANT:-1}
EXPECTED_BROADMARK_VARIANT=1
EXPECTED_BROADMARK_VARIANT_TAG="phase_only"
...
LOGDIR="${LOG_ROOT%/}/310Py_variant1_phase_only_logs_N21_full_once_${TS}"
...
echo "[start] 310 variant1-phase-only validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 310 variant1-phase-only SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '310 variant1-phase-only' "$SRC"; then
  printf 'source_version_tag	310 variant1-phase-only	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	310 variant1-phase-only	missing	FAIL
...
if grep -q '^A10G_FINAL_DEFAULT_BROADMARK_VARIANT:int=1' "$SRC"; then
  printf 'source_a10g_default_variant1	1	1	OK\n' >> "$SUMMARY"
else
  printf 'source_a10g_default_variant1	1	missing/mismatch	FAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 310 variant1-phase-only mode31 split145"
...
echo "[validation-ok] 310 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=1 (phase_only)"
