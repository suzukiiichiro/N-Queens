...
154Pyは問題ありません。全131チャンク、部分和01〜05、最終合計`314666222712`が一致し、欠落・重複・警告・エラーはありません。実行時間は`0:16:03.903`で、153Py比`1.402秒（0.145%）`短縮しています。

### 155Py

[155Py_ctrl0_u32_prepack_probe.py](sandbox:/mnt/data/155Py_ctrl0_u32_prepack_probe.py)

`row`と`functionid`をホスト側で事前パックします。

```python
soa.ctrl0_arr[t]=u32(target)|(u32(start)<<u32(5))
```

GPUカーネル入口は次の1ロードになりました。

```python
ctrl[0]=ctrl0_arr[i]
```

`row_arr`と`funcid_arr`はCPU処理・診断・ソート用として残しますが、GPUカーネルには渡しません。全5か所のカーネル呼出しと、ソート時のコピー処理を対応済みです。

### 実行シェル

[155Py_validate_full_once.sh](sandbox:/mnt/data/155Py_validate_full_once.sh)

```bash
chmod +x 155Py_validate_full_once.sh
./155Py_validate_full_once.sh
```

全131チャンクを1回実行し、部分和01〜05、全体値、欠落・重複・エラーを検証します。`ctrl0`の全896組合せのpack/decode等価性検査と、154Pyとの時間比較も含みます。

シェル構文と模擬全件検証は通過しています。この環境ではCodon/CUDA実機コンパイルは未実施です。

はい。結論としては、**`INIT_MASK`単独のu32化は効果がほぼなく、`rowv`周辺はまとめてu32化する場合に効果が期待できます。**
...
となっています。`INIT_MASK`はコンパイル時定数で、使用時に明示的に`u32`へ変換されているため、通常は32bit即値へ定数畳み込みされます。したがって、宣言だけをu32へ変更しても生成命令は同じになる可能性が高く、単独実験にする価値は低いです。

そのまま維持するのが安全です。

```python
INIT_MASK:Static[int]=1<<19
```

### `nextfv`と`child_row`

この2つは負値を使わず、最終的に`ctrl:u32`へ格納するだけなので、**意味上はu32の方が適しています**。

```python
nextfvu:u32=u32(f)
stepu:u32=u32(1)

nextfvu=u32(meta_next[f])

child_rowu:u32=rowu+stepu
```

ただし、これらだけをu32化しても、現在のパック式には`int`の`addv`、`use_blocks`、`fcv`、`bLv`、`ktype`が混在しています。そのため、式全体が一度`int`で計算され、最後に`u32(...)`となる可能性があります。現在のパック処理はこれらをすべて`int`で組み立てています。

したがって、`nextfv`と`child_row`は**パック用変数一式と同時にu32化**するのが適切です。

### `rowv`と`stepv`

現在の155Pyでは、`rowv`と`stepv`を`int`にしていることには整合性があります。
...
のように同じ型で比較・加算されます。`rowv`と`stepv`だけをu32にすると、比較のたびに`int`との変換が必要となり、逆効果になり得ます。

Codonの通常の`int`は64bit符号付きで、`u32`は32bit符号なし整数です。そのため、関連変数をすべて32bitで閉じられる場合にはu32化の利点がありますが、部分的な変更では型変換が増えます。([docs.exaloop.io][1])

### 推奨する次の変更単位

次の実験では、`INIT_MASK`だけではなく、**ctrl生成側をまとめてu32で閉じる**のがよいです。

```python
fu:u32=cv&u32(31)
f:int=int(fu)  # meta_nextのindex用だけint

rowu:u32=(cv>>u32(5))&u32(31)

stepu:u32=u32(1)
addvu:u32=u32(0)
use_blocksu:u32=u32(0)
nextfvu:u32=fu
fcvu:u32=u32(0)
bLvu:u32=u32(0)
ktypeu:u32=u32(0)
...
ただし、`rowu`をu32化するには、比較対象の`jmark/endm/mark1/mark2`もGPU側ではu32へ揃える必要があります。ホスト側の共通SoAをそのままu32化するのは避けるべきです。`mark1`や`mark2`には`k-1`、`l-1`という生成式があり、型設計上は負値を取り得るほか、ホスト側診断では`mark-row`の符号付き差分も使用しています。

したがって優先順位は次のとおりです。

1. `INIT_MASK`は現状維持
2. `nextfv`単独ではなく、ctrlへ格納する全フィールドをu32化
3. `rowv/stepv`は、GPU用のmark/end情報も同時にu32化または事前パックする場合に変更
4. `f`、`sp`、`i`、配列indexは`int`を維持

特に、`jmark/endm/mark1/mark2`を1個の`markctrl_arr:u32`へ事前パックすると、4本の`Ptr[int]`入力を1本の`Ptr[u32]`へ統合できるため、`rowv/stepv`だけを変更するより効果が期待できます。

[1]: https://docs.exaloop.io/language/lowlevel/?utm_source=chatgpt.com "Low-Level Programming - Codon - Exaloop"

結論として、**速度向上を目的に `u32(0)`／`u32(1)`／`u64(0)`／`u64(1)`を変数化する効果は、基本的にありません**。むしろ変数がレジスタとして残る場合は逆効果になり得ます。
...
としても、`-release`では元の型付きリテラルと同じコードへ最適化される可能性が高いためです。CodonはLLVMへloweringし、LLVMは定数オペランドを定数畳み込みします。([docs.exaloop.io][1])

重要なのは、次の記述が**実行時に毎回変換関数を呼んでいるわけではない**点です。
...
`0`や`1`はコンパイル時に型が確定する即値です。現在のカーネルでも、ゼロ比較、low-bit抽出、シフト、解数加算にこの形式が使用されています。
...
の`u32(0)`を変数にしても、減算命令自体は必要なので命令数は減りません。

一方、変数化した定数がコンパイラによって完全除去されなかった場合、各GPUスレッドで値の生存期間が長くなり、レジスタ圧力を増やす可能性があります。NVIDIAも、値のキャッシュや共通部分式の保持は計算を減らす一方で、レジスタ圧力を増やして占有率へ悪影響を与える場合があると説明しています。([NVIDIA Docs][2])

### 変数化に効果があるケース

効果が期待できるのは、リテラルではなく**実行時の値を繰り返し変換する場合**です。
...
は意味があります。`n3`と`n4`はカーネル引数だったため、ループ内で繰り返し変換するより、入口で一度だけ変換して再利用する方がよいためです。
...
を何度も行う場合や、配列からロードした`int`を何度も`u32`化する場合は、型を最初から揃える価値があります。しかし、`u32(0)`や`u32(1)`はこのケースには該当しません。

したがって、**0・1の共通変数化は次の速度実験には採用せず、現在の型付き即値表記を維持するのが適切です。**

[1]: https://docs.exaloop.io/developers/compilation/?utm_source=chatgpt.com "Compilation Flow | Codon - Exaloop"
[2]: https://docs.nvidia.com/cuda/cuda-c-programming-guide/?utm_source=chatgpt.com "CUDA C++ Programming Guide (Legacy)"


**分けて実装します。**

* **156Py**：`ctrl`へ格納する全フィールドだけを`u32`化
* **157Py**：GPU用mark/endの事前パックを追加し、`rowv/stepv`も完全に`u32`化

現在はmark/endと`rowv/stepv/nextfv/child_row`が`int`で、最後にctrl式全体を`u32(...)`へ変換しています。

156では次のように、最終パックの各オペランドを個別に`u32`へ揃えます。
...
`mark1=k-1`の生成箇所があり、`-1`になり得るため、157では`31`を未使用markの番兵値として扱う検証が必要です。

この順序なら、

1. 156でctrl生成式の32bit化だけの効果を測定
2. 157で4本のmark/end配列を1本へ統合した効果を追加測定
3. 不一致や速度低下が出ても原因を特定可能

となります。なお、156が速度中立または微減でも、混在型が残る途中段階なので、正解であれば157まで試す価値があります。

"""

# 155 ctrl0-u32 prepack experiment:
#   Retain the fully validated 154 GPU-input-u32 representation, 153
#   future-from-mark complement, 152 lazy next-fid lookup, 151 frame-state
#   reload elision, 150 branchless blockK selector, 149 ctrl-u32 decode, 148
#   task order, MAXD=21, five u32 DFS stacks, and nld/nrd full-u32 arithmetic.
#   The sole GPU experiment pre-packs each task's initial function-id and row
#   into ctrl0_arr:List[u32] on the host.  kernel_dfs_iter_gpu() receives one
#   Ptr[u32] instead of row_arr:Ptr[int] plus funcid_arr:Ptr[int], and initializes
#   ctrl[0] with one direct u32 load.  The int row/funcid arrays remain as host
#   mirrors for CPU mode, diagnostics, and optional host-side sorting; they are
#   no longer transferred to this kernel.
#   Host-side scorestripe/chunkshape cache names intentionally remain at 148
#   so the validated reordered .bin can be reused without rebuilding.
...
VERSION_TAG:str="155 ctrl0-u32 prepack over 154 gpu-input-u32 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
class TaskSoA:
  ...
  def __init__(self,m:int)->None:
    ...
    # N<=27 keeps ld/rd/col/free and the 10-bit initial ctrl word within u32.
...
def kernel_dfs_iter_gpu(
    ...
    # 155: initial function-id/row already arrive as one packed u32 word.
    ...
    while sp>=0:
      ...
      if (cv&u32(INIT_MASK))==u32(0):
        ...
        f:int=cv0i&31
        rowv:int=(cv0i>>5)&31
        # 152: meta_next[f] is intentionally not loaded here.  Most frames do
        # not take a function transition; each transition site loads it lazily.
        ...
        # 通常状態設定 / is_mark分類
        #
        # valid funcid 0..27 では、future既定値を持つ型
        # (P4/P5/P6) が IS_MARK_MASK の厳密な補集合である。
        # 153 は mark判定を先に一度だけ行い、その同じ1bitから
        # use_future を作ることで META_AVAIL_MASK の別shiftを除去する。
        ism:int=(IS_MARK_MASK>>f)&1
        #######################################
        stepv:int=1
        addv:int=0
        use_blocks:int=0
        use_future:int=1-ism
        nextfv:int=f
        #######################################
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148/149/150/151/152/153/154/155
          markv:int=mark2 if ((SEL2_MASK>>f)&1)==1 else mark1
          ###################
          # mark
          ###################
        ...
        cv=u32(524288 | nextfv | (child_row<<5) | (stepv<<10) | (addv<<12) | (use_blocks<<13) | (fcv<<14) | (bLv<<15) | (ktype<<17))
        ctrl[sp]=cv
      #----------------
      # 1bit 展開
      #----------------
      # 151/152/153/154/155: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154/155: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        # 155: also pre-pack the initial function-id/row control word once.
        soa.ld_arr[t]=u32(ld)
        soa.rd_arr[t]=u32(rd)
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 155 ctrl0-u32 prepack over the validated 154/153/152/151/150/149/148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 154/153/152/151/150/149/148; 155 changes only the initial ctrl input representation
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 154/153/152/151/150/149/148 scorestripe_v9 task order/cache unchanged; 155 pre-packs initial row/function-id into ctrl0_arr:u32"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  155 reuses the validated 154/153/152/151/150/149/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 155 experiment keeps the validated 154 kernel logic and changes only the initial row/function-id input packing.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  155 host-side score-aware lanephase chunk shaping reused unchanged from validated 154/153/152/151/150/149/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 155 experiment changes only initial ctrl input packing; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split155_ctrl0_u32_prepack_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split155_ctrl0_u32_prepack_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split155-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split155-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split155_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split155-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split155-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split155_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
