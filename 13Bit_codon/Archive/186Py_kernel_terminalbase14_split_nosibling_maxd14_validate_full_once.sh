...
# =============================================================================
# NQ_UPDATE_MEMO
# 186: 更新メモ: 185は正当性OKだが184比で遅いため採用せず、184のno-sibling spill elisionへ戻す。MAXD14 kernelのみ terminal_base14 分岐をDFS loop外側へ分離する単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 186 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 186 starts from validated 184 and changes only the MAXD14 terminal-base14 loop shape after 185
# terminal-depth-13 fast path was correct but slower and the 178/179 block-switch experiments produced CUDA_ERROR_INVALID_PTX at PTX JIT time.
#
# Kernel arithmetic keeps 184 bitboard/schedule semantics: register-held current frame,
# 13-slot ancestor stack, scalar 4-bit nibble schedule, no-sibling spill elision,
# schedule_words=0, and 208 local stack bytes/thread. The experiment is deliberately focused: split the DFS loop once by terminal_base14 so terminal hits avoid the per-hit base/base14 branch. Host task order, dispatch, cache, progress verification, and README-managed history remain unchanged.
SRC=${SRC:-./186Py_kernel_terminalbase14_split_nosibling_maxd14_probe.py}
CAND=${CAND:-./186Py_kernel_terminalbase14_split_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/186Py_kernel_terminalbase14_split_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 186 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/186Py_kernel_terminalbase14_split_nosibling_maxd14_logs_N21_full_once_${TS}"
...
# 186 RUNTIME-ONLY VALIDATION:
#   The long Python source/model assertion block is intentionally disabled.
#   It was useful while editing the generated source, but on the cudacodon side it
#   can fail before the real Codon/CUDA validation starts if the local 186Py source
#   has a harmless header/string/layout difference.  This 186 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 184 no-sibling MAXD14 dispatch; 185 terminal13 correct but slower	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + terminal_base14 outer loop split	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
static_failures=0
if grep -q 'if terminal_base14==u32(0):' "$SRC"; then
  printf 'source_terminalbase14_outer_split	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_terminalbase14_outer_split	present	missing	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
if grep -q 'total+=u64(1)$' "$SRC" && grep -q 'total+=u64(1) if ((nf&~u32(1))!=u32(0)) else u64(0)' "$SRC"; then
  printf 'source_terminal_paths	normal_base_and_base14	normal_base_and_base14	OK
' >> "$SUMMARY"
else
  printf 'source_terminal_paths	normal_base_and_base14	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split186 only	split186 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split186 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 186 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source terminalbase14 split static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 186 terminalbase14 split no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 186 keeps the validated host/chunk structure, then derives chunk-tail diagnostics from
...
# 186: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 186 terminalbase14 split nosibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
