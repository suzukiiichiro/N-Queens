...
# =============================================================================
# NQ_UPDATE_MEMO
# 209: 更新メモ: 208Py root-preroll terminal-firstは正当性OKながら204/206比で遅いため不採用。209Pyは204/206相当へ戻し、MAXD14 kernelのみroot<=2判定をroot_tail clear-lowbit predicateへ置換する微差実験。
...
209Py note:
  208 root-preroll terminal-first was correct but slower than 204/206, so this
  branch returns to the 204/206 root-preroll body and generic ordering.  The
  only new MAXD14 kernel experiment is the root<=2 predicate: keep 204
  root_rest clear-lowbit, then test root_tail = root_rest & (root_rest - 1).
  Root availability with one or two candidates enters the preroll body;
  three-bit or wider roots stay on the generic loop.
...
したがって、208Pyは**正当性OKだが不採用**です。

209Pyは、208のroot-preroll terminal-firstを撤回し、204/206相当へ戻したうえで、MAXD14 kernel内だけの小差分にしました。
...
`root_rest` は204由来の `cur_avail & (cur_avail-u32(1))` のままです。root availabilityが1bit/2bitなら `root_tail==0`、3bit以上ならgeneric loopへ戻るので、対象範囲は204/206と同じです。

手元確認は以下です。

```text
bash -n 209Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 209Py...sh OK
source_roottailclear                  OK
source_split_tag split145 split209    OK
```

ファイルはこちらです。

* [209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_probe.py)
* [209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_validate_full_once.sh

bash 209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_validate_full_once.sh
```
...
VERSION_TAG:str="209 roottwo-roottailclear: 204/206 rootclearrest futuremask/no-sibling MAXD14 baseline with root_tail clear-lowbit root<=2 predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """209 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate root_tail clear-lowbit predicate."""
...
CHUNKSHAPE148_DEFAULT_REASON:str="209 roottailclear reuses validated 204/206 scorestripe_v9 task order/cache; MAXD14 root_tail clear-lowbit predicate after 208 was not adopted"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split209_roottwo_roottailclear_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split209_roottwo_roottailclear_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split209-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split209-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split209_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split209-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split209-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split209_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
