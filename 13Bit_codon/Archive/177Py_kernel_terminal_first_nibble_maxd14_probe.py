...
176Pyログを確認しました。**正当性は問題ありません**。

```text
176Py : 0:08:59.402
175Py : 0:07:51.106
差分  : 176Pyが +68.296秒遅い、約14.497%低下
```
...
したがって、176Pyの **current nibble register** は正当性OKですが、速度面では不採用とします。
177Pyは **最速基準の175Pyへ戻し**、176Pyの `cur_nibble` 追加は継承しない形にしました。

作成したファイルはこちらです。

* [177Py_kernel_terminal_first_nibble_maxd14_probe.py](sandbox:/mnt/data/177Py_kernel_terminal_first_nibble_maxd14_probe.py)
* [177Py_kernel_terminal_first_nibble_maxd14_validate_full_once.sh](sandbox:/mnt/data/177Py_kernel_terminal_first_nibble_maxd14_validate_full_once.sh)
...
不採用:
  ...
  cur_nibble scalar追加なし

177Py:
  terminal-parent frame の判定を future-prune より前に移動
...
狙いは、終端親frameでは `future-prune` を見る必要がないので、terminal count へ早く抜けることです。
`STATIC_ONLY=1` では、N=21/MAXD14の terminal parent がすべて `sp=13` で、かつ terminal parent の nibble に future bit が立っていないことを検証しています。
...
maxd14_terminal_first_nibble_schedule         1 / 1 OK
maxd14_terminal_first_order                   1 / 1 OK
block_opcode_mapping                         19 cases OK
nibble_opcode_classes                        7 classes OK
frame_opcode_equivalence                     506604 cases OK
dfs_164_vs_177_terminal_first_nibble_equivalence 12000 cases OK
...
```bash
chmod +x 177Py_kernel_terminal_first_nibble_maxd14_validate_full_once.sh
./177Py_kernel_terminal_first_nibble_maxd14_validate_full_once.sh
```

この177Pyが175Pyより遅い場合は、terminal-firstも不採用にして、175Py基準のまま次は **kernel hot loop微修正ではなく、task配置・chunk tail側の改善**へ戻すのがよいです。


"""

# 177 GPU terminal-first nibble experiment over validated fastest 175 MAXD14:
#   Branch from the validated fastest 175 nibble-schedule build; the rejected 176 current-nibble-register path is not inherited.  Host-side depth
...
# 177 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 177 MAXD14 uses scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="177 terminal-first nibble schedule for MAXD14 over validated fastest 175 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 177 inherits the exact 175/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """177 MAXD14 terminal-first nibble schedule plus register-held current frame and 13-slot ancestor stack."""
    ...
    # 177 then handles the terminal-parent frame before the future branch;
    # static/model checks verify terminal parents never carry the future bit.
    ...
    # 177 keeps the validated 175 nibble schedule and 172 current-frame register cache plus ancestor stack shrink.  The active
    ...
    while True:
      ...
      # 177 terminal-first: terminal-parent frames have no future bit in the
      # generated N=21/MAXD14 schedule, so count terminal children before the
      # future-prune test.  This keeps non-terminal frames on the 175 nibble path.
...
####################################################################################################
# 177 terminal-first nibble-schedule MAXD14 + scalar-u32pair dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 175 scorestripe_v9 task order/cache and nibble MAXD14; 177 checks terminal parent before future branch"
...
def main()->None:
  ...
  # 177 branches from validated 175.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 177 terminal-first nibble-schedule GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split177_terminal_first_nibble_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split177_terminal_first_nibble_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split177-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split177-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split177_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split177-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split177-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split177_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
