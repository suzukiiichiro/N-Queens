...
# =============================================================================
# NQ_UPDATE_MEMO
# 187: 更新メモ: 186は正当性OKだが184比で大幅退行したため不採用。親を184のno-sibling spill elisionへ戻し、MAXD14 kernelのみ child_jmark_mask==0 の軽量guardを単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 187 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 187 starts from validated 184 because both 185 terminal-depth-13 and 186
# terminal-base14 loop split were correct but slower. 187 changes only the MAXD14
# child-jmark decode guard after the 178/179 block-switch experiments produced CUDA_ERROR_INVALID_PTX at PTX JIT time.
#
...
# schedule_words=0, and 208 local stack bytes/thread. The experiment is deliberately focused: when child_jmark_mask==0, skip the per-depth jmark shift/mask in the hot loop; otherwise keep the 184 generic behavior. Host task order, dispatch, cache, progress verification, and README-managed history remain unchanged.
SRC=${SRC:-./187Py_kernel_jmarkmask_guard_nosibling_maxd14_probe.py}
CAND=${CAND:-./187Py_kernel_jmarkmask_guard_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/187Py_kernel_jmarkmask_guard_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 187 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/187Py_kernel_jmarkmask_guard_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 187Py source
#   has a harmless header/string/layout difference.  This 187 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 184 no-sibling MAXD14 dispatch; 185/186 correct but slower	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + child_jmark_mask zero guard	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'if child_jmark_mask!=u32(0):' "$SRC"; then
  printf 'source_child_jmark_mask_guard	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_child_jmark_mask_guard	present	missing	FAIL
' >> "$SUMMARY"
...
if grep -q 'no-sibling tail-call descent' "$SRC" && grep -q 'save_sp:int=0' "$SRC" && grep -q 'cur_depth:int=0' "$SRC"; then
  printf 'source_nosibling_parent	184-style save_sp/cur_depth	present	OK
' >> "$SUMMARY"
else
  printf 'source_nosibling_parent	184-style save_sp/cur_depth	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split187 only	split187 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split187 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 187 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source child_jmark_mask guard static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 187 jmarkmask guard no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 187 keeps the validated host/chunk structure, then derives chunk-tail diagnostics from
...
# 187: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 187 jmarkmask guard nosibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
