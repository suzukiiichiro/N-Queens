...
CPU
$ codon build -release 57Py_constellations_GPU_cuda_codon_basetail_fastpath_32x484.py
$ ./57Py_constellations_GPU_cuda_codon_basetail_fastpath_32x484 -c
...
suzuki@cudacodon$ ./57Py_constellations_GPU_cuda_codon_basetail_fastpath_32x484 -g 5 20 32 484 0 -1
GPU mode selected
...
def kernel_dfs_iter_gpu(
    ...
    # 57 BASETAIL FASTPATH
    # ------------------------------------------------------------
    # funcptn==5 の base 系 functionid は、mark/jmark/block 遷移を持たず、
    # row を +1 しながら endmark まで通常 DFS するだけでよい。
    # そのため汎用 ctrl デコードを通らず、軽い専用スタックで処理する。
    # 対象: SQB(3), SQd2B(14), SQd1B(21), SQd0B(26)
    f0:int=funcid_arr[i]
    if f0==3 or f0==14 or f0==21 or f0==26:
      bsp:int=0
      brow0:int=row_arr[i]
      avail[0]=free0
      btotal:u64=u64(0)
      ...
      while bsp>=0:
        ...
        if brow==endm:
          if f0==14:
            btotal+=u64(1) if ((ba&~u32(1))!=u32(0)) else u64(0)
          else:
            btotal+=u64(1)
          bsp-=1
          continue

        bbit:u32=ba&(u32(0)-ba)
        avail[bsp]=ba^bbit

        bbit_i:int=int(bbit)
        bnld:int=(ld[bsp]|bbit_i)<<1
        bnrd:int=(rd[bsp]|bbit_i)>>1
        bncol:u32=col[bsp]|bbit
        bnf:u32=bm&~(u32(bnld)|u32(bnrd)|bncol)
        if bnf==u32(0):
          continue

        # base 系は availptn=1。終端直前では先読みしない。
        if (brow+1)<endm:
          if (bm&~(u32(bnld<<1)|u32(bnrd>>1)|bncol))==u32(0):
            continue

        bsp+=1
        if bsp>=MAXD:
          results[i]=btotal*w_arr[i]
          return
        ld[bsp]=bnld
        rd[bsp]=bnrd
        col[bsp]=bncol
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk57={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  # 57 BASETAIL FASTPATH: preset_queens は 5 固定。preset=6 は 48 で正解には直せたが速度面で不採用。
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 57 BASETAIL FASTPATH; using 5")
      preset_queens_arg=5
    if argc > 9:
