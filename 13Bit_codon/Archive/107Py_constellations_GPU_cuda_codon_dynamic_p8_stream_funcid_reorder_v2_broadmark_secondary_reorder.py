...
codon build -release \
  107Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_secondary_reorder.py

#CPU
./107Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_secondary_reorder \
  -c 5 22
#GPU
./107Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_secondary_reorder \
  -g 5 22 32 484 1 0 7 29
...
107Py update, type-explicit clean header:
  bench_mode=26 : exact fid x gap x d1 risk reorder simulation/build-only.
  bench_mode=27 : run GPU on the 106 risk-reordered bin.
  bench_mode=28 : broad funcid primary + exact mark-distance secondary reorder simulation/build-only.
  bench_mode=29 : run GPU on the 107 broad+mark secondary reordered bin.

Python/Codon N-Queens constellation CUDA solver.
Kernel / DFS logic is unchanged from 105/106; 107 changes only stream-side reorder.
106 made exact mark-distance risk primary; GPU Full showed broad funcid imbalance.
107 keeps the broad funcid quota primary and uses mark-distance risk only inside each broad bucket.

107Py のソースコードを作成しました。

[107Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_secondary_reorder.py](sandbox:/mnt/data/107Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_secondary_reorder.py)

差分はこちらです。

[106_to_107_broadmark_secondary_reorder.patch](sandbox:/mnt/data/106_to_107_broadmark_secondary_reorder.patch)

今回の 107Py は、106Py の反省をそのまま反映しています。106Py GPU Full は正解性は `ok` でしたが、`3:44:49.084` で、bucket balance 上も `markrisk_x/t/h/m/o` はきれいに揃った一方、`risky_a/risky_b/risky_c/good/other` の broad funcid balance が大きく崩れていました。

107Py の方針は以下です。

```text
107Py:
  broad funcid balance primary
  exact mark-distance risk secondary

primary bucket:
  risky_b = fid 26,27
  risky_a = fid 19,22,23,24
  risky_c = fid 20,21
  good    = fid 0,4,5,12,16,17,18
  other   = others
...
quota:
  broad bucket × markrisk bucket = 5 × 5 = 25 cell
```

105Py で確認した `fid × gap × d1` の exact tail は残しつつ、106Py のようにそれを primary にしすぎない構造です。`d2` は引き続き `d2 = d1 + gap` なので reorder key には使っていません。

追加したモードは以下です。

```text
bench_mode=28
  broad funcid primary + exact mark-distance secondary reorder simulation/build-only

bench_mode=29
  107 broad+mark secondary reordered GPU full run
```
...
```text
constellations_N22_7_broadmark_reorder_v1_w8_j7.bin
progress_N22_7_stream_broadmark_reorder_v1_w8_j7_sim.tsv
```

GPU Full は sim が `valid=1` で通った後にこちらです。

```bash
./107Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_secondary_reorder \
  -g 22 22 32 484 1 0 7 29 \
  | tee 107Py_N22_p7_broadmark_gpu_full.log
```

GPU progress は以下になる想定です。

```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmark_gpu.tsv
```

107Py の sim TSV には、従来の broad bucket 列と 106Py の markrisk 列に加えて、25 cell の件数列を追加しています。

```text
cell_B_X_count ... cell_B_O_count
cell_A_X_count ... cell_A_O_count
cell_C_X_count ... cell_C_O_count
cell_G_X_count ... cell_G_O_count
cell_O_X_count ... cell_O_O_count
```

確認として、Codon 固有の `@par` 行を除外した Python AST 構文チェックは通しています。

```text
AST OK
```

ただし、こちらの環境には `codon` がないため、`codon build` は未実行です。


はい。**GPU Full 後は `progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmark_gpu.tsv` だけでなく、sim TSV と full log も添付**していただくのがよいです。

107Py の sim はすでに、

```text
records=28719035
reordered_records=28719035
chunks=1855
valid=1
```

で通っているので、この sim TSV と GPU Full TSV を chunk 番号で join して解析します。

107Py 用の集計スクリプトはこちらです。

[aggregate_107py_broadmark_gpu.py](sandbox:/mnt/data/aggregate_107py_broadmark_gpu.py)

GPU Full 実行はこの形でお願いします。
...
```bash
python3 aggregate_107py_broadmark_gpu.py \
  --gpu progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmark_gpu.tsv \
  --sim progress_N22_7_stream_broadmark_reorder_v1_w8_j7_sim.tsv \
  --log 107Py_N22_p7_broadmark_gpu_full.log \
  --baseline-summary 106Py_N22_p7_markdistrisk_summary.tsv \
  --baseline-label 106Py \
  --out 107Py_N22_p7_broadmark
```

`106Py_N22_p7_markdistrisk_summary.tsv` が同じディレクトリにない場合は、`--baseline-summary ...` と `--baseline-label ...` の 2 行を外して実行してください。

出力される主なファイルは以下です。

```text
107Py_N22_p7_broadmark_report.md
107Py_N22_p7_broadmark_summary.tsv
107Py_N22_p7_broadmark_comparison.tsv
107Py_N22_p7_broadmark_correlation.tsv
107Py_N22_p7_broadmark_top_elapsed.tsv
107Py_N22_p7_broadmark_elapsed_runs.tsv
107Py_N22_p7_broadmark_bucket_balance.tsv
107Py_N22_p7_broadmark_cell_balance.tsv
107Py_N22_p7_broadmark_slow_feature_ratio.tsv
107Py_N22_p7_broadmark_joined.tsv
```

添付していただきたい優先順はこれです。

```text
1. 107Py_N22_p7_broadmark_report.md
2. 107Py_N22_p7_broadmark_summary.tsv
3. 107Py_N22_p7_broadmark_comparison.tsv
4. 107Py_N22_p7_broadmark_correlation.tsv
5. 107Py_N22_p7_broadmark_top_elapsed.tsv
6. 107Py_N22_p7_broadmark_elapsed_runs.tsv
7. 107Py_N22_p7_broadmark_bucket_balance.tsv
8. 107Py_N22_p7_broadmark_cell_balance.tsv
9. 107Py_N22_p7_broadmark_slow_feature_ratio.tsv
10. progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmark_gpu.tsv
11. progress_N22_7_stream_broadmark_reorder_v1_w8_j7_sim.tsv
12. 107Py_N22_p7_broadmark_gpu_full.log
```

`107Py_N22_p7_broadmark_joined.tsv` は全 chunk の結合表なので、サイズが大きければ省略で大丈夫です。必要になった場合だけ追加で見ます。

今回の集計で特に見たいのは、次の 4 点です。

```text
1. final total が 2691008701644 で ok か
2. 106Py の 3:44:49.084 より elapsed_sum_sec が改善しているか
3. broad bucket の CV が 106Py より戻っているか
4. 25 cell、つまり broad × markrisk のどの cell が slow chunk と相関しているか
```

107Py は **broad funcid balance primary / mark-distance risk secondary** なので、`bucket_balance.tsv` と `cell_balance.tsv` が特に重要です。

承知しました。追加分も含めて見ます。

現時点で添付済みの範囲だけでの暫定判定は、次のとおりです。
...
107Py elapsed:
  report elapsed_sum_sec = 12954.626 sec
  log wall clock         = 3:36:12.231

106Py elapsed:
  elapsed_sum_sec = 13473.847 sec
  wall clock      = 3:44:49.084

107Py vs 106Py:
  -519.221 sec
  約 8分39秒 改善
  current_over_baseline = 0.961465
```

つまり、**107Py は 106Py よりは明確に改善**しています。
ログ上も `broadmark-reorder-gpu-done` で `ok` まで到達しており、正解性は問題ありません。

ただし、まだ最終採用とまでは言い切れません。理由は、107Py は平均・中央値・p95・p99 は 106Py より改善していますが、`elapsed_max_ms` と `elapsed_cv` は悪化しています。
...
```text
106Py:
  exact mark-distance primary は不採用

107Py:
  broad funcid primary へ戻した判断は正しい
  ただし tail 平準化はまだ不十分

次:
  108Py では broad balance を維持したまま、
  slow region / funcid_7 / G_H / chunk_total proxy をさらに見る
```

です。

追加ファイルがある場合は、特に `joined.tsv` か、旧 funcid reorder v2 の GPU progress があると、107Py が「84/旧方式に対して本当に改善しているか」まで比較できます。


はい、**`progress_N22_7_stream_funcid_reorder_v2.tsv` でよいです。**

ただし、そのファイルが **旧 funcid reorder v2 の GPU Full 実行時の progress TSV** であることが条件です。
比較したいのは、今回の 107Py broadmark GPU Full、

```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmark_gpu.tsv
```

に対する **旧方式の GPU elapsed/chunk_total** なので、`progress_N22_7_stream_funcid_reorder_v2.tsv` に `chunk`, `off`, `m`, `elapsed_ms`, `chunk_total` などが入っていれば十分です。107Py 側は `status=ok`, `final_gpu_total=2691008701644`, `elapsed_sum_sec=12954.626` まで確認済みです。

添付していただきたいのは、優先順で以下です。

```text
1. progress_N22_7_stream_funcid_reorder_v2.tsv
2. その実行ログ、または最後の ok 行が分かるテキスト
```

もし似た名前で、

```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_gpu.tsv
progress_N22_7_stream_funcid_reorder_v2_w8_j7.tsv
```

のように **`w8_j7` が付いた GPU progress** があるなら、そちらの方がより望ましいです。107Py/106Py はどちらも `window_mult=8 phase_jump=7 param=w8_j7` 条件で比較しているためです。

`progress_N22_7_stream_funcid_reorder_v2.tsv` だけでも、こちらで列を見て「旧 v2 GPU Full」として使えるか判定します。
...
で実行しているためです。比較対象も同じ `w8_j7` に揃えるのが一番きれいです。

添付していただきたい優先順は以下です。

```text
1. progress_N22_7_stream_funcid_reorder_v2_w8_j7.tsv
2. progress_N22_7_stream_funcid_reorder_v2_w8_j7_sim.tsv
3. 可能なら、その実行時のログまたは最後の ok 行
```

`progress_N22_7_stream_funcid_reorder_v2_w8_j7.tsv` が旧 funcid reorder v2 の GPU Full progress です。
`progress_N22_7_stream_funcid_reorder_v2_w8_j7_sim.tsv` は、旧方式側の bucket / funcid 配分を見るために使えます。
...
も参考にはなりますが、`w8_j7` 明示版があるので、今回は **`progress_N22_7_stream_funcid_reorder_v2_w8_j7.tsv` を優先**でお願いします。
...
106Py:
  progress_N22_7_stream_funcid_reorder_v2_w8_j7_markdistrisk_gpu.tsv

107Py:
  progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmark_gpu.tsv
```

これで、旧 v2 → 106Py → 107Py の同一 `w8_j7` 条件比較ができます。


添付分を再集計しました。結論は、**107Py は旧 v2 `w8_j7` よりも速く、106Py よりも明確に改善しています。採用候補は 107Py でよい**です。

ただし、**最大 chunk elapsed だけは旧 v2 より少し悪化**しているため、107Py は「現時点の最良版」ですが、108Py では tail chunk の追加対策を入れる価値があります。

## 同一 `w8_j7` 条件の比較

下表は `progress_*.tsv` の `elapsed_ms` を合計した比較です。ログ上の wall clock は入出力や集計オーバーヘッドが少し乗るため、reorder 比較ではこちらの方が揃った比較になります。107Py 側は `final_gpu_total=2691008701644`, `status=ok` まで確認済みです。

| version         |     elapsed sum |      mean/chunk |      median |           p95 |            p99 |      max |       CV |
| --------------- | --------------: | --------------: | ----------: | ------------: | -------------: | -------: | -------: |
| 旧 v2 `w8_j7`    |     3:40:32.062 |     7133.187 ms |     7138 ms |     9435.5 ms |    10027.14 ms | 10560 ms | 0.197100 |
| 106Py markrisk  |     3:44:33.847 |     7263.529 ms |     7250 ms |     8792.8 ms |      9497.6 ms | 10706 ms | 0.122540 |
| 107Py broadmark | **3:35:54.626** | **6983.626 ms** | **6920 ms** | **8671.3 ms** | **9352.46 ms** | 10743 ms | 0.143934 |

107Py は、ログ上の wall clock でも `3:36:12.231 ok` です。
...
106Py との比較は、107Py の集計レポート側でも `elapsed_sec_sum` が `13473.847 → 12954.626`、`current_over_baseline=0.961465` と出ています。

## 重要な観察

107Py は **旧 v2 の broad bucket balance を維持したまま、mark-distance risk を secondary に使う**方針でしたが、この判断は正しかったです。
...
つまり、**107Py は旧 v2 と同等の broad balance を完全に取り戻しています**。106Py の失速原因だった broad imbalance は解消されています。

一方で、107Py の `elapsed_max_ms=10743` は旧 v2 の `10560` より少し大きいです。ここだけは悪化しています。平均・中央値・p95・p99・総時間は改善しているので、全体としては 107Py 優位ですが、tail 対策はまだ残っています。

## なぜ 107Py がよくなったか

一番大きいのは、`chunk_total` の tail が下がったことです。

```text
chunk_total p95:
  old v2 = 1,903,893,804
  106Py  = 1,988,119,956
  107Py  = 1,751,124,190.8
```

また、elapsed と `chunk_total` の相関も下がっています。

```text
elapsed vs chunk_total pearson:
  old v2 = 0.656692
  106Py  = 0.565055
  107Py  = 0.424220
```

これは、107Py が「探索展開量の大きい chunk がそのまま遅くなる」状態をかなり緩和できている、という意味です。

## まだ残っている slow region

107Py の p95 以上の連続 slow run は主にここです。
...
107Py の上位相関では `funcid_7_count`, `cell_G_H_count`, `markrisk_h_ratio` が目立っています。特に `funcid_7_count` と `cell_G_H_count` は、108Py の追加制御対象として有力です。
...
106Py:
  不採用
  exact mark-distance primary は broad balance を壊しすぎた

107Py:
  採用候補
  旧 v2 w8_j7 より速い
  106Py より明確に速い
  broad balance は旧 v2 と同等に復元

残課題:
  max chunk elapsed が少し悪化
  slow island が 1644-1654, 66-72, 1574-1580 などに残る
```

次は **108Py: broadmark + fid7/G_H tail smoothing** がよいです。

方針は、

```text
primary:
  broad bucket quota は 107Py のまま維持

secondary:
  markrisk X/T/H/M/O も維持
...
[比較レポート md](sandbox:/mnt/data/N22_w8_j7_reorder_gpu_comparison_report.md)
[比較 summary TSV](sandbox:/mnt/data/N22_w8_j7_reorder_gpu_comparison_summary.tsv)
[pairwise comparison TSV](sandbox:/mnt/data/N22_w8_j7_reorder_gpu_pairwise_comparison.tsv)
[top30 elapsed TSV](sandbox:/mnt/data/N22_w8_j7_reorder_gpu_top30_elapsed.tsv)
[p95 slow runs TSV](sandbox:/mnt/data/N22_w8_j7_reorder_gpu_slow_runs_p95.tsv)
[bucket balance TSV](sandbox:/mnt/data/N22_w8_j7_reorder_gpu_bucket_balance.tsv)
...
VERSION_TAG:str="107 broad funcid primary mark-distance secondary reorder auto w8_j7 stream GPU runner from 106 type-explicit"
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29:
      ...
      #   mode28:    ... [preset_queens] 28   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe]  # broadmark sim/build only
      #   mode29:    ... [preset_queens] 29   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe]  # broadmark GPU full run
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 28        # broad funcid primary + mark-distance secondary reorder simulation/build only
      #   -g 22 22 32 484 1 0 7 29        # broad+mark secondary reordered GPU full run
      if argc >= 11:
  ...
  if use_gpu:
    ...
    if bench_mode==28:
      print(f"broadmark_reorder_sim: mode={bench_mode} preset={preset_queens_arg}")
    if bench_mode==29:
      print(f"broadmark_reorder_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if bench_mode==26:
      ...
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[markdist-risk-reorder-sim-only] N={N} preset_queens={preset_queens} records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} valid={1 if validate_bin_file(reorder_fname) else 0}")
    ...
    if use_gpu and N>=21 and bench_mode==27:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[markdist-risk-reorder-gpu-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()}")
      else:
        if gpu_log_level>=1:
    ...
    if bench_mode==28:
      ijkl_list,subconst_cache,stream_records,preset_queens,stream_fname=ensure_constellations_bin_stream(N,ijkl_list,subconst_cache,preset_queens,gpu_log_level)
      reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[broadmark-reorder-sim-only] N={N} preset_queens={preset_queens} records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} valid={1 if validate_bin_file(reorder_fname) else 0}")
      print(f"{N:2d}:{0:18d}{0:17d}{text:>21s}    broadmark-reorder-sim-only")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==29:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[broadmark-reorder-gpu-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()}")
      else:
        if gpu_log_level>=1:
          print(f"[broadmark-reorder-gpu-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      total:int=exec_solutions_gpu_bin_stream_funcid_reorder(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"","broadmark_gpu")
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      print(f"[broadmark-reorder-gpu-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==23:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[funcid-reorder-v2-funciddepth-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()}")
      else:
        if gpu_log_level>=1:
    ...
    if use_gpu and N>=21 and bench_mode==24:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[funcid-reorder-v2-funcidmark-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()}")
      else:
        if gpu_log_level>=1:
    ...
    if use_gpu and N>=21 and bench_mode==25:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[funcid-reorder-v2-funcidmarkdist-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()}")
      else:
        if gpu_log_level>=1:
    ...
    if use_gpu and N>=21 and bench_mode==22:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[funcid-reorder-v2-funcidsplit-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()}")
      else:
        if gpu_log_level>=1:
    ...
    if use_gpu and N>=21 and bench_mode==21:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[funcid-reorder-v2-funcidsingle-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()}")
      else:
        if gpu_log_level>=1:
