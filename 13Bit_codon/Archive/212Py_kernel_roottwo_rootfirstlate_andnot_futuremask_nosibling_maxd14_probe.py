...
# =============================================================================
# NQ_UPDATE_MEMO
# 212: 更新メモ: 211Py rootfirstlate は正当性OKで204とほぼ同等だが204最速を0.006秒上回れなかったため未採用。212Pyは211のroot_first遅延を維持しつつ、210で比較的良好だった root_after_second = root_rest & (~root_second) のand-not predicateを組み合わせる微差実験。root_rest clear-lowbit / futuremask / no-sibling / root one-two preroll は維持する。
...
211Pyは204Pyに **0.006秒差** まで近づきました。非常に良い結果ですが、204Pyを上回ってはいないため、**正当性OKだが新最速基準にはせず未採用**とします。

212Pyは、211の `root_first` 遅延を維持し、210で比較的良好だった and-not predicate を組み合わせる微差実験にしました。
...
```text
bash -n 212Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 212Py...sh OK
source_rootfirstlate_andnot           OK
source_split_tag split145 split212    OK
```

ファイルはこちらです。

* [212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_probe.py)
* [212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_validate_full_once.sh

bash 212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 212 rootfirstlate_andnot memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  212 keeps the narrow root fast-start only when
...
VERSION_TAG:str="212 roottwo-rootfirstlate_andnot: 211 late root_first plus 210 and-not root_after_second over 204 rootclearrest futuremask/no-sibling MAXD14 baseline - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # correct but slower.  212 keeps 211's late root_first lowbit extraction and
    # changes only the root_after_second scalar shape from XOR to and-not:
    # root_after_second = root_rest & (~root_second).  3-bit and wider roots
...
CHUNKSHAPE148_DEFAULT_REASON:str="212 rootfirstlate_andnot reuses validated 204 scorestripe_v9 task order/cache; MAXD14 delays root_first and tests and-not root_after_second"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split212_roottwo_rootfirstlate_andnot_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split212_roottwo_rootfirstlate_andnot_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split212-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split212-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split212_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split212-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split212-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split212_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
