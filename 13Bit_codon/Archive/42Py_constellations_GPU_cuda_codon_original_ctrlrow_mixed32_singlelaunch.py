...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=256,gpu_max_blocks:int=32,gpu_log_level:int=0)->None:
  ...
  # gpu_max_blocks <= 0 のときは全 constellation を 1 回の kernel で投げる。
  # 39 の N=19 ログでは最後の小 chunk でも kernel が約2.7秒かかっており、
  # chunk ごとの tail が支配的になっている可能性があるため、その診断用。
  if gpu_max_blocks<=0:
    MAX_BLOCKS=max(1,(m_all+BLOCK-1)//BLOCK)
    STEPS=max(1,m_all)
  else:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 singlelaunch_auto={(1 if gpu_max_blocks<=0 else 0)} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
...
def main()->None:
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_block gpu_max_blocks log_level]  # gpu_max_blocks<=0 means single-launch")
      return
