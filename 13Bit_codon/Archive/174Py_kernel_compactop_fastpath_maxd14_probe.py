...
173Pyログを確認しました。**正当性は問題ありませんが、速度は明確に低下**しています。

```text
173Py : 0:12:28.469
172Py : 0:08:11.190
差分  : 173Pyが +257.279秒遅い、約52.379%低下
```

したがって、174Pyでは **173Pyのframe-entry decode方針は不採用**にしました。
親は最速基準の **172Py** に戻し、173Pyで増えたlive scalarを避ける方向で、MAXD14 DFSループだけを軽く変更しています。

作成したファイルはこちらです。

* [174Py_kernel_compactop_fastpath_maxd14_probe.py](sandbox:/mnt/data/174Py_kernel_compactop_fastpath_maxd14_probe.py)
* [174Py_kernel_compactop_fastpath_maxd14_validate_full_once.sh](sandbox:/mnt/data/174Py_kernel_compactop_fastpath_maxd14_validate_full_once.sh)
...
172Py:
  compact_op を読み出した後、
    ...
    future_check
  を毎候補bitで作り、
  block_code != 0 で通常/ブロックを分岐

173Py:
  frame entry時にdecode済みmetadataをscalar保持
  → register圧が増えて大幅悪化

174Py:
  173Pyのentry decodeは撤回
  172Pyのcurrent-frame register + ancestor13 stackを維持

  compact_opを候補bit時点で読むが、
  compact_op 0/1 を通常step1 fast pathとして先に処理

    compact_op == 0 : normal step1, futureなし
    compact_op == 1 : normal step1, futureあり
    compact_op >= 2 : block path

  future_check scalarは廃止し、
  compact_op == 1 を直接判定
```

stack footprintは172Pyと同じです。
...
検証シェルには、通常のfull validationに加えて、既存progress TSVから以下のchunk-tail診断を出す処理も追加しました。

```text
chunk_elapsed_sum_ms
chunk_elapsed_min_ms
chunk_elapsed_max_ms
chunk_elapsed_p50_p90_p95_ms
```

こちらの環境では `STATIC_ONLY=1` の静的・モデル検証を通しています。

```text
kernel_variant_count                    5 / 5 OK
kernel_fallback_normalized_equivalence  1 / 1 OK
maxd14_compactop_fastpath_schedule       1 / 1 OK
frame_opcode_equivalence                506604 cases OK
dfs_164_vs_174_fastpath_equivalence     12000 cases OK
...
```bash
chmod +x 174Py_kernel_compactop_fastpath_maxd14_validate_full_once.sh
./174Py_kernel_compactop_fastpath_maxd14_validate_full_once.sh
```

今回の狙いは、173Pyで悪化した **register圧増加** を避けつつ、172Pyの候補bitループ内のdecodeを少し軽くできるかの確認です。速度が戻らない、または172Pyより遅い場合は、174Pyの方向も捨てて、次はkernel内変更ではなくtask配置・tail診断側へ進むのがよいです。


"""

# 174 GPU compact-op fast-path experiment over validated 172 ancestor13 MAXD14:
...
# 174 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 174 MAXD14 uses scalar u32 schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="174 compact-op fast path for MAXD14 over validated 172 ancestor13 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 174 inherits the exact 172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """174 MAXD14 scalar schedule plus register-held current frame, 13-slot ancestor stack, and compact-op fast path."""
    ...
    # 174 keeps the 172 current-frame register cache and 13-slot ancestor
    # stack, but backs out the 173 frame-entry decode.  Instead, compact_op is
    # decoded at the candidate point with an explicit fast path for the two
    # normal step1 forms (compact_op 0/1).  This avoids the extra live metadata
    # scalars that made 173 slower while removing the block_code/future_check
    # temporaries from the overwhelmingly common normal path.
    sp:int=0
    cur_ld:u32=root_ld
    cur_rd:u32=root_rd
    ...
    while True:
      ...
      nrd:u32=u32(0)
      if compact_op<u32(2):
        # compact_op 0: normal step1, no future check
        # compact_op 1: normal step1, with future check
        nld=(cur_ld|bit)<<u32(1)
        nrd=(cur_rd|bit)>>u32(1)
      else:
        block_code:u32=compact_op-u32(1)
...
def kernel_dfs_iter_gpu_maxd16(
    ...
    """174 inherited byte-opcode schedule fallback, static MAXD=16."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd18(
    ...
    """174 inherited byte-opcode schedule fallback, static MAXD=18."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd20(
    ...
    """174 inherited byte-opcode schedule fallback, static MAXD=20."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd21(
    ...
    """174 inherited byte-opcode schedule fallback, static MAXD=21."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
####################################################################################################
# 174 compact-op fast-path MAXD14 + scalar-u32pair schedule harness
#
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164/167/168/169/170/171/172/174: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 172 scorestripe_v9 task order/cache and ancestor13 MAXD14; 174 splits compact_op normal/block fast paths"
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/167/168/169/170/171/172/174: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 174 branches from validated fastest 172.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 174 compact-op fast-path GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split174_compactop_fastpath_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split174_compactop_fastpath_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split174-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split174-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split174_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split174-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split174-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split174_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
