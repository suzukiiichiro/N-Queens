...
"""bucket index 配列を作る。bucket=(funcptn<<2)|(avail_flag<<1)|(row>=endmark-1)"""
def make_bucket_indices()->List[List[int]]:
    ...
    while stack:
      ...
      if row>=target_row:
        append_expanded_task(tasks,bucket_indices,meta,
          ...
          jmark,endmark,mark1,mark2,weight,owner)
        continue
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_split_row:int=-1)->None:
  """
  機能:
  ...
  if use_gpu:
    ...
    GPU_BUCKETS:int=24
    print(f"[gpu-config] N={N} bucketized=1 split_row={GPU_SPLIT_ROW} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS}")
    ...
    while off < m_all:
      ...
      bucket_indices:List[List[int]]=make_bucket_indices()
      expand_iter_to_row(
        func_meta,
      ...
      for bucket_id in range(GPU_BUCKETS):
        ...
        pos:int=0
        while pos<bucket_m:
          cm:int=min(STEPS,bucket_m-pos)
          for ii in range(cm):
            t:int=idxs[pos+ii]
            soa.ld_arr[ii]=expanded.ld_arr[t]
            soa.rd_arr[ii]=expanded.rd_arr[t]
            soa.col_arr[ii]=expanded.col_arr[t]
            soa.row_arr[ii]=expanded.row_arr[t]
            soa.free_arr[ii]=expanded.free_arr[t]
            soa.jmark_arr[ii]=expanded.jmark_arr[t]
            soa.end_arr[ii]=expanded.end_arr[t]
            soa.mark1_arr[ii]=expanded.mark1_arr[t]
            soa.mark2_arr[ii]=expanded.mark2_arr[t]
            soa.funcid_arr[ii]=expanded.funcid_arr[t]
            w_arr[ii]=expanded.w_arr[t]
...
def main()->None:
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [N | N_start N_end] [split_row]")
      return
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        exec_solutions(N,constellations,use_gpu,gpu_split_row)
