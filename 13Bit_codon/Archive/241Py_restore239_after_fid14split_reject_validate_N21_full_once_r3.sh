...
# =============================================================================
# 241 N=21 full validation harness
# Parent: 239 n27coretrim-keepfeatures.
# Experiment: reject 240 fid14/rest launch split for speed and restore the
#             239 baseline while keeping -c/-g, N5..27, cache generation,
...
SRC=${SRC:-./241Py_restore239_after_fid14split_reject_probe_r3.py}
CAND=${CAND:-./241Py_restore239_after_fid14split_reject_probe_r3}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/241Py_restore239_after_fid14split_reject_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/241Py_restore239_after_fid14split_reject_logs_N21_full_once_${TS}"
...
echo "[start] 241 r3 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 241 restore239-after-fid14split-reject SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '241 restore239-after-fid14split-reject' "$SRC"; then
  printf 'source_version_tag\t241 restore239-after-fid14split-reject\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t241 restore239-after-fid14split-reject\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split241' in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split241 active; split240 rejected', 'split241', True))
else:
    checks.append(('source_split_tag', 'split241 active; split240 rejected', 'split240/split239 runtime present or split241 missing', False))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 241 source static checks failed" >&2
  exit 66
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 241 restore mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "239parent:$BASELINE_239_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 241 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
