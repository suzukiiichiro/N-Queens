...
**307 (K=44) = 351.240s** — vs 304(K=48) = 351.070s、差 −0.170s (−0.048%) — **誤差内、実質 flat**。

K=44、K=48、K=56 の3点が全て 351.0〜351.5s の誤差ゾーンに収まっています。K=44〜56 が flat 最適ゾーンであると確定です。
...
K=48: 351.070s  ← 測定上の最小値（304）
K=56: 351.534s  ← flat zone 内
K=64: 353.896s
```

K スイープはこれ以上絞り込む意味がありません。304 (K=48, 351.070s) を確定 BEST として、別方向へ移ります。K=52 は確実に flat。別方向を試みます。今日一日の試行から、スタック操作以外で改善できる場所を冷静に探します。## 307 結果まとめと 308 の考え方

**307 (K=44) = 351.240s — 誤差級、不採用**。K=48 との差 −0.170s は測定誤差内。
...
K=40: 353.587s  │
K=44: 351.240s ─┼─ flat zone 開始
K=48: 351.070s  │  ← 現 BEST（304）
K=52: (308で確認)│
K=56: 351.534s  │
K=64: 353.896s ─┘ 上り坂
```

K=44〜56 が flat ゾーン（約 351.0〜351.5s）。304 (K=48, 351.070s) が測定上の最小値として確定。

## 308 の目的

K=52 を測定して K カーブを完成させます。結果はほぼ確実に 351.x s (flat) です。

```bash
STATIC_ONLY=1 bash 308Py_K52_final_sweep_validate_N21_full_once.sh
bash 308Py_K52_final_sweep_validate_N21_full_once.sh
```

本日のセッションはここまで非常に充実した探索ができました。292から304まで累計 **+16.3s (+4.45%)** の改善を達成しています。お疲れ様でした。
