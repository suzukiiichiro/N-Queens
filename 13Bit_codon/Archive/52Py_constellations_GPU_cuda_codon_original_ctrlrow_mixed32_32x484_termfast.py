...
$ ./<filename> -g 20 20 32 484 2 3      # sort_mode=3 診断 / terminal fast-count
または
...
def kernel_dfs_iter_gpu(
    ...
    CTRL_ROW_MASK:Static[int]=992  # bits 5..9
    ...
    while sp>=0:
      ...
      # terminal fast-count
      #----------------
      # 子フレームが base かつ endmark 到達なら、push して次ループで数える代わりに
      # ここで直接 count する。N=20 のホットパスでは終端フレームの stack 書き込み/読み出しを減らす狙い。
      if (cv&u32(CTRL_ROW_MASK))==endm5:
        child_f:int=int(cv)&31
        if ((IS_BASE_MASK>>child_f)&1)==1:
          if child_f==14:
            if (nf&~u32(1))!=u32(0):
              total+=u64(1)
          else:
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk52={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  # 52 TERMFAST: preset_queens は 5 固定。46で 6 を試したところ N=20 が ng になったため、
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 52 TERMFAST; using 5")
      preset_queens_arg=5
    if argc > 9:
