...
Python/codon Ｎクイーン コンステレーション版 CUDA 高速ソルバ 50 trunk候補 32x484 ops diagnostic SAFE FIX（ctrlrow mixed32 tuned）
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk50={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    soa:TaskSoA=TaskSoA(STEPS)
    results:List[u64]=[u64(0)]*STEPS
    ops_arr:List[u64]=[u64(0)]*STEPS
    # results_allはm_all
    ...
    while off < m_all:
      ...
      if use_sorted:
        kernel_dfs_iter_gpu(
          ...
          gpu.raw(meta_next),
          gpu.raw(results), gpu.raw(ops_arr),
          m, board_mask,
          n3, n4,
      ...
      if gpu_log_level>=2:
        ...
        ops_max:u64=u64(0)
        ops_max_i:int=0
        ptn_max:List[u64]=[u64(0)]*6
        ptn_cnt:List[int]=[0]*6
        for oi in range(m):
          ov:u64=ops_arr[oi]
          ops_sum+=ov
          if ov>ops_max:
            ops_max=ov
            ops_max_i=oi
          fid_for_ops:int=soa.funcid_arr[oi]
          if use_sorted:
            fid_for_ops=sort_soa.funcid_arr[oi]
          ptn_for_ops:int=funcptn_by_fid[fid_for_ops]
          ptn_cnt[ptn_for_ops]+=1
          if ov>ptn_max[ptn_for_ops]:
            ptn_max[ptn_for_ops]=ov
        ops_avg:int=0
        if m>0:
          ops_avg=int(ops_sum)//m
        ops_max_global:int=off+ops_max_i
        if use_sorted:
          ops_max_global=idx_map[ops_max_i]
        hot_ptn:int=0
        for bi in range(6):
          if ptn_max[bi]>ptn_max[hot_ptn]:
            hot_ptn=bi
        print(f"[gpu-chunk] N={N} chunk={chunks} off={off} m={m} grid={GRID} sort={gpu_sort_mode} build={str(t1-t0)[:-3]} sort_time={str(ts1-ts0)[:-3]} kernel={str(t2-ts1)[:-3]} copy={str(t3-t2)[:-3]} ops_avg={ops_avg} ops_max={int(ops_max)} ops_max_idx={ops_max_global} ops_hot_ptn={hot_ptn} ops_hot_ptn_max={int(ptn_max[hot_ptn])} ops_hot_ptn_cnt={ptn_cnt[hot_ptn]}")
      off += m
...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  # 49 SAFE FIX: preset_queens は 5 固定。46で 6 を試したところ N=20 が ng になったため、
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 49 SAFE FIX; using 5")
      preset_queens_arg=5
    if argc > 9:
