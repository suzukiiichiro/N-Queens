...
# =============================================================================
# NQ_UPDATE_MEMO
# 269: schedule-precompute-root0child0 probe。268 root0future0 は正当性OKだが
#      267より遅かったため採用せず、267 root0countcacheへ戻す。
#      267の root_action==0 専用MAXD14 kernelを維持し、さらに
#      root_action==0 かつ child_jmark_mask==0 のchunkだけ、child_jmark
#      分岐も持たない root0child0 kernelへdispatchする。
#      root_action非zero chunkは265互換precompute kernelへfallback、
#      root_action==0だがchild_jmarkありchunkは267 root0 kernelへfallbackする。
#      単一launch per chunk、mode28/29/30/31、cache生成、MAXD14/16/18/20/21
#      fallback、worker split、CPU dfs_iter pathは維持。
...
"""
269 schedule-precompute-root0child0 note: 269 root0countcacheを親に、root_action==0 かつ child_jmark_mask==0 のchunkだけ、root_action load/branchに加えて child_jmark branchも持たない MAXD14 root0child0 kernelへdispatchする。root_action非zero chunkは269 fallback、root0だがchild_jmarkありchunkは269 root0 kernelへfallbackする。単一launch per chunk、mode28/29/30/31、cache生成、MAXD fallback、worker splitは保持。268 root0future0 は正当性OKだが速度退行のため採用しない。


268ログは問題ありません。**正当性OKですが、速度は267より遅いため不採用**です。
...
```text
267 root0countcache      0:07:08.056
268 root0future0         0:07:08.525
差分                     +0.469秒
```

したがって269は、268を採用せず267へ戻し、次の切り分けとして **root_action==0 かつ child_jmark_mask==0 chunk専用kernel** を追加しました。

```text
269:
  root_action==0 and child_jmark_mask==0:
      root0child0 kernel

  root_action==0 but child_jmark_maskあり:
      267 root0 kernel

  root_action非zero:
      265互換 fallback kernel
```
...
source_schedule_precompute_root0child0 OK
source_rootaction_zero_kernel          OK
source_rootaction_zero_dispatch        OK
source_host_precompute_root0child0_arrays OK
release_build_policy                   OK
```

ファイルはこちらです。

* [269Py_schedule_precompute_root0child0_probe.py](sandbox:/mnt/data/269Py_schedule_precompute_root0child0_probe.py)
* [269Py_schedule_precompute_root0child0_validate_N21_full_once.sh](sandbox:/mnt/data/269Py_schedule_precompute_root0child0_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [269Py_schedule_precompute_root0child0_sha256.txt](sandbox:/mnt/data/269Py_schedule_precompute_root0child0_sha256.txt)
* [269Py_schedule_precompute_root0child0_bundle_20260707.tar](sandbox:/mnt/data/269Py_schedule_precompute_root0child0_bundle_20260707.tar)

sha256です。

```text
547280f23509c2315635bf479097d59ab5f4cc636fb6c997c2d1e77910cb920c  269Py_schedule_precompute_root0child0_probe.py
a0060f6bfc954b545dbc407300e47b7a47591726722c2d3b2f6c0848ce4639e2  269Py_schedule_precompute_root0child0_validate_N21_full_once.sh
40034b9e55306206ff3cb75a5ab0ca0e09748900382d61d6e24315fe189eda80  README.md
```

実行は以下でお願いします。

```bash
rm -f ./269Py_schedule_precompute_root0child0_probe

STATIC_ONLY=1 bash 269Py_schedule_precompute_root0child0_validate_N21_full_once.sh

bash 269Py_schedule_precompute_root0child0_validate_N21_full_once.sh
```
...
VERSION_TAG:str="269 schedule-precompute-root0child0: parent 269 root0countcache precompute-best; single launch per chunk kept; TaskSoA also caches child_jmark_nonzero_count; root_action==0 and child_jmark_mask==0 chunks use a root0child0 MAXD14 kernel without root_action load/branch or child_jmark branch; root0/fallback dispatch preserved; 268 root0future0 rejected; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 267: keep 262's safe terminal_depth+terminal_base14 pack and 244 r4 maskpack.
    ...
    self.child_jmark_nonzero_count:int=0
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 267: host-side clone of the former MAXD14 kernel schedule builder.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 267: free_arr is repacked to sched_mask for selected MAXD14 launch.
    ...
    # 267: schedule fields are precomputed on host. For MAXD14 launch,
...
@gpu.kernel
def kernel_dfs_iter_gpu_maxd14_root0_child0(
    ld_arr:Ptr[u32],rd_arr:Ptr[u32],col_arr:Ptr[u32],ctrl0_arr:Ptr[u32],free_arr:Ptr[u32],
    markctrl_arr:Ptr[u32],
    ...
    # 267: schedule fields are precomputed on host. For MAXD14 launch,
    # ctrl0_arr has been repacked to schedule_lo, markctrl_arr to sched_terminal,
    # and free_arr to child/future sched_mask. root_action and child_jmark_mask are known zero for this kernel.
    schedule_lo:u32=ctrl0_arr[i]
    schedule_hi:u32=sched_hi_arr[i]
    sched_mask:u32=free_arr[i]
    future_check_mask:u32=(sched_mask>>u32(14))&u32(16383)
    ...
    terminal_depth:int=int(sched_terminal&u32(15))
    terminal_base14:u32=(sched_terminal>>u32(4))&u32(1)
    # 269 root0child0 kernel: root_action and child_jmark_mask are statically zero for all tasks in this launch.
...
def kernel_dfs_iter_gpu_maxd14_root0(
    ...
    terminal_depth:int=int(sched_terminal&u32(15))
    terminal_base14:u32=(sched_terminal>>u32(4))&u32(1)
    # 269 root0 kernel: root_action is statically zero for all tasks in this launch.
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    # 269: selected_MAXD14 is known here, so original ctrl0/markctrl/free
    ...
    # If all root_action values in this chunk are zero and child_jmark_mask is
    # also zero, launch a root0child0 variant that has no root_action load/branch
    # and no child_jmark branch. If only root_action is zero, use the 269 root0
    # kernel. Nonzero root_action chunks fall back to the 265-compatible kernel.
    # This keeps exactly one launch per chunk.
    root_action_nonzero_count:int=soa.root_action_nonzero_count
    child_jmark_nonzero_count:int=soa.child_jmark_nonzero_count
    ri:int=0
    while ri<m:
      ...
      ri+=1
    if root_action_nonzero_count==0 and child_jmark_nonzero_count==0:
      kernel_dfs_iter_gpu_maxd14_root0_child0(
...
def build_soa_for_range(
    ...
    soa.child_jmark_nonzero_count=0
    ...
    for t in range(m):
        ...
        if (sched_mask&u32(16383))!=u32(0):
          soa.child_jmark_nonzero_count+=1
  ...
  if use_gpu:
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        sort_soa.child_jmark_nonzero_count=soa.child_jmark_nonzero_count
      if gpu_log_level>=2:
        ts1=datetime.now()
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split269_root0child0_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split269_root0child0_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split269-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split269-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split269_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
