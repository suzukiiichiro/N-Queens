...
はい、良い案です。153の次の単独実験として採用する価値があります。

現在はカーネル入口で、`board_mask`、`n3`、`n4`、`ld/rd/col/free`を各スレッドごとに`u32`へ変換しています。

ただし、最初から全`*.arr[]`を一括変更するより、154Pyでは次に限定するのが安全です。
...
これにより、明示的な`u32(...)`を少なくとも7か所除去できます。

一方、以下は154では変更しない方がよいです。

* `jmark_arr/end_arr/mark1_arr/mark2_arr`：`rowv:int`との比較で逆にキャストが必要になる可能性があります。
* `row_arr/funcid_arr`：将来的には`ctrl0_arr:u32`へ事前パックする方が効果的です。
* `ijkl_arr`：カーネルへ渡していないため変更してもGPU側の効果がありません。
* `w_arr/results`：解数処理のため`u64`を維持します。
* `meta_next`：既に`u8`です。

また、共通の`TaskSoA`を全面的に`u32`化すると、CPU処理や`depth=end-row`などの符号付き診断処理へ影響します。そのため、GPU用のビットボード配列だけを`u32`化するか、GPU専用SoAを設けるのが安全です。

この変更はDFSループ内ではなく各スレッドの開始時に一度だけ行われる変換の除去なので、大幅な高速化よりも小幅な改善を想定しています。ただし、入力配列の要素型も明確に32bit化されるため、キャスト削減だけでなく入力ロード面にも改善余地があります。

**153Pyの結果確認後、154Pyは「GPU入力ビットボード配列＋`board_mask/n3/n4`の完全u32化」を単独変更として作成する方針が適切です。**


153Pyは問題ありません。全131チャンク、部分和01〜05、最終合計`314666222712`が一致し、欠落・重複・警告・エラーはありません。実行時間は`0:16:05.305`でした。

### 154Py

[154Py_gpu_input_u32_probe.py](sandbox:/mnt/data/154Py_gpu_input_u32_probe.py)

以下を`u32`化しました。

* `ld_arr / rd_arr / col_arr / free_arr`：`List[u32]`
* カーネル引数：`Ptr[u32]`
* `board_mask / n3 / n4`：`u32`
* カーネル入口の7個の`u32(...)`変換を除去
* `row/mark/functionid`配列は`int`のまま維持
* CPU経路ではDFS呼出し境界で明示的に`int`へ戻す
* 148の並び替えbinと探索順序は変更なし

### 実行シェル

[154Py_validate_full_once.sh](sandbox:/mnt/data/154Py_validate_full_once.sh)

```bash
chmod +x 154Py_validate_full_once.sh
./154Py_validate_full_once.sh
```

全131チャンクを1回だけ実行し、01〜05の部分和と全件結果を再検証します。153Pyの`965.305秒`との速度比較も自動表示します。

シェル構文、u32変更箇所の静的検査、153Pyの131チャンクTSVを使った模擬全件検証は通過しています。この環境ではCodon/CUDA実機コンパイルは未実施です。

"""

# 154 GPU input-u32 experiment:
#   Retain the fully validated 153 future-from-mark complement, 152 lazy next-fid
#   lookup, 151 frame-state reload elision, 150 branchless blockK selector,
#   149 ctrl-u32 decode, 148 task order, MAXD=21, five u32 DFS stacks, and
#   nld/nrd full-u32 arithmetic.  The sole experiment changes the four GPU
#   bitboard SoA inputs (ld/rd/col/free) from List[int]/Ptr[int] to
#   List[u32]/Ptr[u32], and passes board_mask/n3/n4 as u32 scalars.  Therefore
#   kernel entry can load these values directly without seven per-thread u32
#   conversions.  Row/mark/function-id arrays remain int so their comparisons
#   and mask-table indexing are unchanged.
...
VERSION_TAG:str="154 gpu-input-u32 over 153 future-from-mark complement - scorestripe v9 lanephase32 octetfirstpairlock29"
...
class TaskSoA:
  ...
  def __init__(self,m:int)->None:
    ...
    # N<=27 keeps ld/rd/col/free within u32 (rd may retain future bits up to bit30).
...
def kernel_dfs_iter_gpu(
    ld_arr:Ptr[u32],rd_arr:Ptr[u32],col_arr:Ptr[u32],row_arr:Ptr[int],free_arr:Ptr[u32],
    ...
    while sp>=0:
      ...
      if (cv&u32(INIT_MASK))==u32(0):
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148/149/150/151/152/153/154
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 154 GPU-input-u32 over the validated 153/152/151/150/149/148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 153/152/151/150/149/148; 154 changes only GPU input element/scalar types
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 153/152/151/150/149/148 scorestripe_v9 task order/cache unchanged; 154 passes bitboard SoA and mask scalars to the GPU as u32"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  154 reuses the validated 153/152/151/150/149/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 154 experiment keeps the validated 153 kernel logic and changes only GPU input storage/scalar types.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  154 host-side score-aware lanephase chunk shaping reused unchanged from validated 153/152/151/150/149/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 154 experiment changes only GPU input storage/scalar types; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split154_gpu_input_u32_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split154_gpu_input_u32_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split154-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split154-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split154_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split154-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split154-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split154_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
