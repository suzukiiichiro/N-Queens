...
現時点の本線:
  94 funcid_reorder_v2
  - 84 stream 32x484 sort_mode=0 はバックアップ本線として残す
  - 実用本線は94へ昇格でよい

94の狙い:
  93 funcid_reorder の risk均一化を維持しつつ、bucket内の元順序・位相の揃いを崩す。
  phase/stripe sampling により、重い領域が同じchunkに集まるのを防ぐ。
...
v2 parameters:
  FUNCID_REORDER_V2_WINDOW_MULT = 16
  FUNCID_REORDER_V2_PHASE_JUMP  = 7

N21確認済み結果:
  94 first:
    21:      314666222712                0          0:24:37.680    ok

  94 rerun:
    21:      314666222712                0          0:24:37.597    ok

比較:
  84 best 32x484 N21 : 0:25:38.459  ok
  91 measure N21     : 0:26:00.782  ok
  92 measure2 N21    : 0:26:01.180  ok
  93 reorder N21     : 0:25:34.752  ok
  94 reorder_v2 N21  : 0:24:37.597〜0:24:37.680  ok
...
基本コマンド:
  codon build -release 94Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2.py

simulation-only:
  ./94Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2 \
...
N22へ進める場合:
  ./94Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2 \
    -g 22 22 32 484 1 0 5 15

N22期待値:
  22:     2691008701644                0      ...    ok

封印・注意:
  sort_mode=4 は chunk_total=0 になったため封印。
  heavy split 系 88/89/90 は正解一致したが84/94より遅いので検証ブランチ扱い。
  32x968 も正解一致したが32x484より遅いので本線にしない。
  94では bench_mode=15 が reordered stream GPU 実行。
  bench_mode=14 は simulation-only でGPUを回さない。

次回の作業候補:
  1. 94ソースを添付して、まずN21ログ確認。
  2. 94を正式本線としてN22へ進める。
  3. 95を作る場合は大改造ではなく、94のparameter sweep版にする。
...
VERSION_TAG:str="94 stream funcid risk balanced reorder v2 phase GPU runner from 93"
CROSS_STRIPE_SAFE_DEFAULT:bool=False

# 94 FUNCID REORDER V2:
#   Based on 93 funcid risk reorder. Kernel / DFS logic is intentionally unchanged.
...
def funcid_reorder_output_fname(N:int,preset_queens:int)->str:
  return f"constellations_N{N}_{preset_queens}_funcid_reorder_v2.bin"
...
"""94 funcid reorder v2: bucket 内を phase/stripe サンプリングするための window 倍率。"""
...
def build_funcid_reordered_bin(
  ...
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_funcid_reorder_v2_sim.tsv"
  ...
  if gpu_log_level>=1:
    print(f"[funcid-reorder-v2-build-config] N={N} records={total_records} steps={STEPS} output={reorder_fname} progress={progress_fname} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP}")
...
def exec_solutions_gpu_bin_stream_funcid_reorder(
  ...
  total_records:int=count_constellations_bin_records(fname)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_funcid_reorder_v2.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[funcid-reorder-v2-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if chunk_only else 0} progress={progress_fname} inner_log_level=0")
...
def main()->None:
  ...
  for N in range(nmin,nmax):
    ...
    if bench_mode==14:
      ...
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[funcid-reorder-v2-sim-only] N={N} preset_queens={preset_queens} records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} valid={1 if validate_bin_file(reorder_fname) else 0}")
    ...
    if use_gpu and N>=21 and bench_mode==15:
      ...
      print(f"[funcid-reorder-v2-gpu-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname}")
