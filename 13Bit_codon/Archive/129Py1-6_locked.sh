...
# 129 locked validation harness
# - Candidate: 129Py_scorestripe_chunkshape_probe
# - Kernel path remains generic-only; 129 adds host-side chunkshape129_scorestripe_v1.
# - Locked after the first N21 validation run on 2026-06-15.
# - This script checks fixed partial_total values plus full correctness.

BASE=${BASE:-./115Py_range_default_clean_cg_v2}
CAND=${CAND:-./129Py_scorestripe_chunkshape_probe}
RUN_FULL=${RUN_FULL:-1}

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="logs_N21_129_locked_validation_${TS}"
...
# ================================================================
# 1) 129 selected-chunk smoke: standard spread
# locked partial_total = 16770333494
# ================================================================
run_case \
  "01_cand129_mode30_standard_7chunk" \
  "partial_total=16770333494" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "0,20,40,60,80,100,120"

# ================================================================
# 2) 129 selected-chunk stress: heavy band
# locked partial_total = 19239439100
# ================================================================
run_case \
  "02_cand129_mode30_heavy_band" \
  "partial_total=19239439100" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "35,40,41,42,47,48,52,53"

# ================================================================
# 3) 129 selected-chunk stress: d2base14-density band
# locked partial_total = 16823602934
# ================================================================
run_case \
  "03_cand129_mode30_d2base14_density" \
  "partial_total=16823602934" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "20,40,55,56,57,58,60"

# ================================================================
# 4) 129 selected-chunk stress: late/tail band
# locked partial_total = 16316779188
# ================================================================
run_case \
  "04_cand129_mode30_late_tail" \
...
# worker 0/4 locked partial_total = 79339432716
# ================================================================
run_case \
  "05_cand129_mode31_worker0of4_quick" \
  "partial_total=79339432716 expected_total=314666222712" \
  "$CAND" \
...
if [ "${RUN_FULL}" = "1" ]; then
  run_case \
    "06_cand129_mode31_full_correctness" \
    "314666222712.*ok" \
    "$CAND" \
