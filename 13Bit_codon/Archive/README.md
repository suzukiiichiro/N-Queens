...
## 現在の未解決課題 (Open Objectives) -- 最終更新: 350 (2026-07-28)
...
7. **[新規発見・338・未着手・低リスク] `w_hi_arr`は恒等的にゼロ**
   338の仕様化作業中に確認した事実: `symmetry()`(2519行)の戻り値は`u64(2)`/`u64(4)`/`u64(8)`の3値のみであり、w_arrの値域は{2,4,8}に限られる。したがって328で分割した`w_hi_arr`は**全要素が恒等的に0**であり、maxd14カーネルが読む3箇所(576/581/750行)のロードは常に0を返す純粋な無駄である。さらに`w = 1<<e`(e∈{1,2,3})なので、`markctrl`の未使用ビット20-21にeを詰めれば`w_lo_arr`も不要になり、エピローグの64bit乗算はシフトに退化する(`markctrl`はbit0-19のみ使用: jmark(0-4)/endmark(5-9)/mark1(10-14)/mark2(15-19))。プロローグ/エピローグのみの変更でホットループには一切触れないため、5戦5敗のカーネル分解とは別軸。期待効果は小(内側DFSループ支配のため<0.5%と見込む)だがリスクとコストも小。339以降の単一変数実験候補として記録する。
