...
# =============================================================================
# NQ_UPDATE_MEMO
# 244: schedule-precompute-lowpack probe。241 restore239本線を親に、240別launchと
#      242小手先条件整理を採用せず、243のschedule precompute構造だけを継承する。
#      243の7本追加配列loadを、sched_lo / sched_hi/sched_ctrl / sched_mask の4本へ低位packし、
#      単一launch維持のままMAXD14 kernel冒頭のschedule生成interpreter撤去効果を再評価する。
...
"""
244 r2 schedule-precompute-lowpack-lowbitsafe note: 241 restore239本線を親に、240のfid14/rest別launchと242のfuturemask-depthbit微差を採用せず、単一launchのままMAXD14 schedule precompute情報を3 u32へpackする構造probe。

244初回は **PTX/JIT不成立** なので、計算不一致ではなく **pack表現の撤回対象**として扱います。

原因候補は、`sched_hi_term` に `terminal_depth / terminal_base14 / root_action` を高位bitへ詰めた形です。`codon build -release` は通っていますが、GPU module load時に `CUDA_ERROR_INVALID_PTX` になっているため、初回244の **3配列 high-bit pack** は危険です。

そこで **244 r2 lowpack buildfix** を作りました。方針は同じく schedule precompute pack ですが、高位bit packをやめて、低位bitだけの4配列にしています。

```text
244初回:
  sched_lo_arr
  sched_hi_term_arr   # 高位bitに terminal/root_action をpack
  sched_mask_arr
...
244 r2:
  ...
  sched_hi_arr
  sched_mask_arr
  sched_ctrl_arr      # 低位bitだけに terminal/root_action をpack
```
...
sched_mask_arr:
  bits  0..13  child_jmark_mask
  bits 14..27  future_check_mask

sched_ctrl_arr:
  bits  0..3   terminal_depth
  bit      4   terminal_base14
  bits  5..6   root_action
```
...
source_schedule_precompute_lowpack     OK
source_host_precompute_lowpack_arrays  OK
release_build_policy                   OK
```

古い244を掴まないよう、r2名で実行してください。

```bash
rm -f ./244Py_schedule_precompute_pack_probe
rm -f ./244Py_schedule_precompute_pack_probe_r2

STATIC_ONLY=1 bash 244Py_schedule_precompute_pack_validate_N21_full_once_r2.sh

bash 244Py_schedule_precompute_pack_validate_N21_full_once_r2.sh
```

ファイルはこちらです。

* [244Py_schedule_precompute_pack_probe.py](sandbox:/mnt/data/244Py_schedule_precompute_pack_probe.py)
* [244Py_schedule_precompute_pack_probe_r2.py](sandbox:/mnt/data/244Py_schedule_precompute_pack_probe_r2.py)
* [244Py_schedule_precompute_pack_validate_N21_full_once.sh](sandbox:/mnt/data/244Py_schedule_precompute_pack_validate_N21_full_once.sh)
* [244Py_schedule_precompute_pack_validate_N21_full_once_r2.sh](sandbox:/mnt/data/244Py_schedule_precompute_pack_validate_N21_full_once_r2.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [244Py_schedule_precompute_pack_sha256.txt](sandbox:/mnt/data/244Py_schedule_precompute_pack_sha256.txt)
* [244Py_schedule_precompute_pack_bundle_20260707_r2.tar](sandbox:/mnt/data/244Py_schedule_precompute_pack_bundle_20260707_r2.tar)

今回の source sha256 です。

```text
edaa6ed770eaf43ff7f51eb4bb213231f03b80a4cf038a2706022633eb8962f5  244Py_schedule_precompute_pack_probe.py
edaa6ed770eaf43ff7f51eb4bb213231f03b80a4cf038a2706022633eb8962f5  244Py_schedule_precompute_pack_probe_r2.py
```
...
VERSION_TAG:str="244 r2 schedule-precompute-lowpack-lowbitsafe probe: parent 241 restore239; single launch kept; MAXD14 host precompute fields packed into 4 u32 arrays; 240 fid14 split and 242 condition micro-diff rejected; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 244: bare -g stays split145 mode31; single-launch schedule-precompute-lowpack probe
...
# 244: keep runtime globals required by broadmarktail/chunkshape148/split145
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 244: packed host-precomputed schedule fields for MAXD14.
    # 243 used 7 arrays; 244 r2 packs the same information into 4 u32 arrays
    # to reduce added global-load pressure while keeping single-launch dispatch.
    self.sched_lo_arr:List[u32]=[u32(0)]*m
    self.sched_hi_arr:List[u32]=[u32(0)]*m
    self.sched_mask_arr:List[u32]=[u32(0)]*m
    self.sched_ctrl_arr:List[u32]=[u32(0)]*m
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32]:
  # 244: host-side clone of the former MAXD14 kernel schedule builder.
  # This removes the schedule interpreter from the hot GPU kernel entry path.
  ...
  sched_ctrl:u32=(terminal_parent_depth&u32(15))|((terminal_is_base14&u32(1))<<u32(4))|((root_action&u32(3))<<u32(5))
  return schedule_lo,schedule_hi,sched_mask,sched_ctrl
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    sched_lo_arr:Ptr[u32],sched_hi_arr:Ptr[u32],sched_mask_arr:Ptr[u32],sched_ctrl_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 244 r2: packed schedule fields are precomputed on the host in build_soa_for_range().
    # 243 loaded 7 separate arrays. 244 r2 uses 4 low-bit fields to avoid the
    # high-bit pack form that triggered CUDA_ERROR_INVALID_PTX on cudacodon.
    schedule_lo:u32=sched_lo_arr[i]
    schedule_hi:u32=sched_hi_arr[i]
    mask_pack:u32=sched_mask_arr[i]
    ctrl_pack:u32=sched_ctrl_arr[i]
    child_jmark_mask:u32=mask_pack&u32(16383)
    future_check_mask:u32=(mask_pack>>u32(14))&u32(16383)
    terminal_depth_u:u32=ctrl_pack&u32(15)
    terminal_base14:u32=(ctrl_pack>>u32(4))&u32(1)
    root_action:u32=(ctrl_pack>>u32(5))&u32(3)
    ...
    while True:
      ...
      if u32(cur_depth)==terminal_depth_u:
        if terminal_base14==u32(0):
          total+=u64(1)
        else:
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.sched_lo_arr),gpu.raw(soa.sched_hi_arr),gpu.raw(soa.sched_mask_arr),gpu.raw(soa.sched_ctrl_arr),
...
# 244: recursive CPU dfs fallback remains removed. CPU path uses dfs_iter only.
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        sched_ctrl:u32=u32(0)
        sched_lo,sched_hi,sched_mask,sched_ctrl=precompute_maxd14_schedule_fields(soa.ctrl0_arr[t],soa.markctrl_arr[t],meta_next_sched)
        soa.sched_lo_arr[t]=sched_lo
        soa.sched_hi_arr[t]=sched_hi
        soa.sched_mask_arr[t]=sched_mask
        soa.sched_ctrl_arr[t]=sched_ctrl
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        for p in range(m):
          ...
          sort_soa.sched_ctrl_arr[p]=soa.sched_ctrl_arr[q]
...
CHUNKSHAPE148_DEFAULT_REASON:str="244 r2 schedule-precompute-lowpack-lowbitsafe keeps validated 241/239/238 scorestripe_v9 task order/cache; single-launch MAXD14 packed schedule precompute probe"
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 244 r2 schedule-precompute-lowpack-lowbitsafe; using 0")
        bench_mode=0
