...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  """
  機能:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk55={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    ...
    while off < m_all:
      ...
      if gpu_log_level>=2:
        t3=datetime.now()
        print(f"[gpu-chunk] N={N} chunk={chunks} off={off} m={m} grid={GRID} sort={gpu_sort_mode} build={str(t1-t0)[:-3]} sort_time={str(ts1-ts0)[:-3]} kernel={str(t2-ts1)[:-3]} copy={str(t3-t2)[:-3]}")
      off += m
...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  # 55 CHUNKSWEEP: preset_queens は 5 固定。46で 6 を試したところ N=20 が ng になったため、
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_block gpu_max_blocks log_level sort_mode] [preset]\n       gpu_max_blocks=-1 enables chunk-size sweep")
      return
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 55 CHUNKSWEEP; using 5")
      preset_queens_arg=5
    if argc > 9:
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        # 55 CHUNKSWEEP:
        #   gpu_max_blocks=-1 のとき、kernel ロジックは変えずに chunk サイズだけを
        #   小さめ中心に自動比較する。54 で大きい chunk が大幅悪化したため、
        #   32x484 近辺と小さめだけを見る診断モード。
        if use_gpu and gpu_max_blocks<0:
          candidates:List[int]=[128,192,256,320,384,448,484,512,640]
          print(f"[sweep-config] N={N} block={gpu_block} sort_mode={gpu_sort_mode} candidates=128,192,256,320,384,448,484,512,640")
          for mb in candidates:
            for ci in range(len(constellations)):
              constellations[ci]["solutions"]=0
            sweep_start=datetime.now()
            exec_solutions(N,constellations,use_gpu,gpu_block,mb,gpu_log_level,gpu_sort_mode)
            sweep_total:int=sum(c['solutions'] for c in constellations if c['solutions']>0)
            sweep_dt=datetime.now()-sweep_start
            sweep_text=str(sweep_dt)[:-3]
            sweep_status:str="ok" if expected[N]==sweep_total else f"ng({sweep_total}!={expected[N]})"
            print(f"[sweep-result] N={N} block={gpu_block} max_blocks={mb} steps={gpu_block*mb} total={sweep_total} time={sweep_text} {sweep_status}")
          # sweep mode は通常の1本表示を出さず、次のNへ進む
          continue

        if use_gpu and gpu_max_blocks==0:
          print("[warning] gpu_max_blocks=0 is disabled in 55 CHUNKSWEEP; using 484")
          gpu_max_blocks=484
