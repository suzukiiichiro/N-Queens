...
# =============================================================================
# 344 N=21 full validation harness (bucket-run-adopt)
#
# 344 IS THE ADOPTION REVISION for the 339-343 campaign, in the same role
# that 333 played for w3_j7 after the 330-332 sweeps.
#
...
# Kernels, dispatcher, K=48, the SoA w_lo/w_hi split, w3_j7 and the host-side
# run-length emit loop added in 339 are byte-for-byte identical to 343.
#
# WHY 2048: 343 confirmed SATURATION at the data level, not by inference from
# timings. The shaped bins produced by 2048, 4096, 8192, 16384 and 32768 are
# BYTE-IDENTICAL -- md5 283b038c2573848f6a5b945b20ec6102, 32404512 bytes, all
# five -- so no larger value can differ. Timings agreed: 431.677 to 431.764s,
# a spread of 0.020%. 2048 is adopted as the smallest saturating value; its
# cache already exists, so nothing is rebuilt.
#
# FINAL STANDING (N=21, against the in-session RUN=1 control of 450.183s):
#     adopted RUN=2048   431.677s   -4.111%
#     chunk0 -3.853%   chunk1 -4.529%   chunk2 -3.836%
#     cumulative from 328's 456.036s (when SoA was adopted): -5.341%
#   The whole campaign was host-side reordering. The kernel was never touched.
#
# THE THREE RULES ESTABLISHED (340-343):
#   1. Alignment to warp width is necessary. Non-multiples of 32 -- 16, 48,
#      80 -- all land in a -0.26% to -0.34% band regardless of magnitude.
#   2. Being a power of two matters on top of alignment. 96 and 384 are
#      multiples of 32 yet sit at the bottom of the aligned group (-0.609%,
#      -0.690%) while 64, 128 and 512 reach -0.832%, -0.818%, -2.210%.
#   3. The axis saturates at 2048, confirmed by byte-identical shaped bins.
#
# A CORRECTION TO THE RECORD: 343 stated that 342's saturation reasoning was
# wrong. That statement was itself overstated. 342 estimated saturation near
# quotas[b] ~ STEPS/8 ~ 1936 and the measured point is 2048, so the estimate
# was right; the 15488 figure in 343 is a SUFFICIENT bound, not a necessary
# one, because the quota distribution is close to uniform. 342's ceiling of
# 8192 also sat above its own best point of 2048, so unlike 341 -- where the
# ceiling of 256 WAS the best point -- it never constrained anything. The
# static check added in 343 requiring the cap to clear BLOCK*MAX_BLOCKS is
# harmless and is kept as insurance against future configuration changes.
#
# REACHING THE OLD BASELINE: passing CHUNKSHAPE148_BUCKET_RUN=1 explicitly
# still reproduces the 276-338 emit order AND its cached shaped-bin filename,
# so the pre-339 baseline remains one command away:
#
#     CHUNKSHAPE148_BUCKET_RUN=1 bash 344Py_bucket_run_adopt_validate_N21_full_once.sh
#
# There is no sweep driver in 344; adoption is a single-point revision.
#
# NEXT AXIS (345): the saturated output is one grid-stride iteration arranged
# as 8 contiguous score-bucket blocks, lane-diffused within each block. The
# open question is whether a FINER sort helps. Two routes: (a) generalise the
# bucket count 8 -> 16 -> 32, which requires touching interleave_order (fixed
# 8 elements), chunkshape148_build_thresholds_from_hist (7 thresholds) and
# bucket_lane_indices (8*32); or (b) sort each iteration directly by score
# key, which is the limit of (a) and is simpler to implement. (b) answers the
# question in one experiment and should come first -- if a full sort does not
# beat 8 saturated buckets, parameterising the bucket count is not worth it.
# Note that (b) destroys the lane diffusion that currently spreads equal-score
# records across the 32 lanes, so the effect on tied scores must be looked at
# separately.
#
# MANDATORY VALIDATION ORDER:
#   1. STATIC_ONLY=1 first. Four gating checks cover the adoption: the module
#      default is 2048, the old value 1 is gone, the A10G default is 2048,
#      and the two defaults agree.
#   2. Full N=21 run; confirm 314666222712 FIRST, before any timing.
#   3. Expect ~431.7s, reproducing 343's saturated measurement.
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./344Py_bucket_run_adopt.py}
CAND=${CAND:-./344Py_bucket_run_adopt}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/344Py_bucket_run_adopt_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/344Py_bucket_run_adopt_logs_N21_full_once_${TS}"
...
echo "[start] 344 bucket-run-adopt validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 344 bucket-run-adopt SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '344 bucket-run-adopt' "$SRC"; then
  printf 'source_version_tag	344 bucket-run-adopt	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	344 bucket-run-adopt	missing	FAIL
