...
#      226Pyは223/217相当のgeneric cur_avail^bit更新へ戻し、MAXD14 generic loopのみ
#      ncol=cur_col|bit を nld/nrd 計算より前へ移動する微差実験を検証する。
# Full update history: see README.md
# =============================================================================

SRC=${SRC:-./226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 226 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
if root_pos < 0:
    root_pos=s.find('# 226 generic-placeor:')
if root_pos < 0:
...
# Locate the first generic-loop lowbit extraction after block_code decode.
...
checks={
    ...
    'generic_placed_ld_present': 'placed_ld:u32=cur_ld|bit' in generic_compact,
    'generic_placed_rd_present': 'placed_rd:u32=cur_rd|bit' in generic_compact,
    'generic_placed_before_nld': 0 <= generic_compact.find('placed_ld:u32=cur_ld|bit', pos) < nld_pos and 0 <= generic_compact.find('placed_rd:u32=cur_rd|bit', pos) < nld_pos,
    'generic_block_uses_placed': 'nld=(placed_ld<<stepu)|addvu|bLiu' in generic_compact and 'nrd=(placed_rd>>stepu)|bKu' in generic_compact,
    'generic_normal_uses_placed': 'nld=placed_ld<<u32(1)' in generic_compact and 'nrd=placed_rd>>u32(1)' in generic_compact,
    'generic_old_or_absent': 'nld=((cur_ld|bit)<<stepu)|addvu|bLiu' not in generic_compact and 'nld=(cur_ld|bit)<<u32(1)' not in generic_compact,
...
if all(checks.values()):
    sys.exit(0)
print('source_generic_placeor_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_generic_placeor\trootrestlate_plus_generic_placeor\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_generic_placeor\trootrestlate_plus_generic_placeor\tmissing_or_old_shape\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split226'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split226' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit226 active preferred\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit226 active preferred\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if (( static_failures != 0 )); then
  echo "================================================================"; echo "[static-summary]"; cat "$SUMMARY"; echo "[logdir] $LOGDIR"
  echo "[error] 226 source static checks failed" >&2
  exit 66
fi
...
echo "[validation-ok] 226 generic place-or reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
