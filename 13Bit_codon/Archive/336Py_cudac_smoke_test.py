...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 336 (2026-07-24)
...
6. [設計中・336で最初のコード成果物] CUDA Cランナー(案C-2)スパイク設計
   ...
   形式を読む最小構成とする。
   **334-335の実機プローブ結果(確定)**: nvccはcudacodon実機の
   `/usr/local/cuda/bin/nvcc`にCUDA 13.0(V13.0.88)として存在し、
   `.cu`ビルドは可能。A10GのCompute Capabilityは
   `nvidia-smi --query-gpu=compute_cap`で**8.6**と実測確定
   (`-arch=sm_86`が正しいビルドフラグ)。一方`cuobjdump`はPATH検索
   でもnvccと同一ディレクトリ(`/usr/local/cuda/bin`)内検索でも
   検出されず、**このCUDA 13.0ツールキットインストールには
   cuobjdumpが同梱されていない**ことが確定した(325/327での過去の
   成功は別環境、または実際にはncuフォールバック経由だった可能性が
   高い)。SASS静的逆アセンブルが必要な場面では、318以降確立済みの
   `sudo ncu --section SourceCounters --page source`経路を使う。
   **336で最初の具体的なコード成果物に着手**: メインソルバのカーネル・
   ディスパッチャには一切触れず、`.cu`ツールチェーンが実際に
   ビルド・実行できることを確認する独立した最小スモークテスト
   (`336_cudac_smoke_test.cu`)を追加した。DFSロジックは一切含まず、
   `-arch=sm_86`でのビルド・GPU実行・既知の期待値との自己検証のみを
   行う。325(Codon inline probe)・327(w_arr load-split probe)と
   同じ方法論(メインソルバから独立したファイルで先にコード生成/
   実行環境の挙動を確認してから本体に手を付ける)を、Codonではなく
   CUDA Cツールチェーン側に適用したもの。成功すれば、次のマイルストーンは
   bin形式のC側reader仕様化とN=18限定maxd14移植の設計。

このセクション自体は通常カーネルロジックとは独立しています。
336はカーネルロジックの変更を含みません(カーネル・ディスパッチャは
328-335と同一、funcid_reorderもw3_j7のまま)。ソースの変更は、
このドキュメント/VERSION_TAGと項目6のみです。336で追加された
唯一のコードは、メインソルバとは独立した`336_cudac_smoke_test.cu`
...
**335の実行結果(確認)**
- 静的チェック全OK(34項目+nvcc/cuobjdumpプローブ2項目+新規2項目、FAILゼロ)。正当性314666222712一致。elapsed 450.218s(334の450.181s比-0.008%、ノイズ内)で完全に再現しました
- `nvcc`のフルパス: `/usr/local/cuda/bin/nvcc`(CUDA 13.0、V13.0.88)。`.cu`ビルドは可能と確認済みです
- `cuobjdump`: PATH検索でも、nvccと同じディレクトリ(`/usr/local/cuda/bin`)内の直接検索でも見つかりませんでした。**このCUDA 13.0インストールにはcuobjdumpが同梱されていない**ことが確定しました(325/327での過去の成功実績とは別環境か、実際にはncuフォールバック経由だった可能性が高いです)
- `nvidia-smi --query-gpu=compute_cap`: **8.6**を実測。A10Gのビルドフラグは`-arch=sm_86`で確定です

**336の内容(CUDA C案C-2の最初の具体的なコード成果物)**
- カーネル・ディスパッチャは328-335と一切変更なし。ソース変更はOpen Objectives項目6(更新)とVERSION_TAGのみです
- SASS静的逆アセンブルが今後必要になった場合は、cuobjdumpが使えないため318以降確立済みの`sudo ncu --section SourceCounters --page source`経路に一本化します
- メインソルバとは完全に独立した最小スモークテスト`336_cudac_smoke_test.cu`を追加しました。DFSロジックは一切含まず、`nvcc -arch=sm_86`でのビルド・GPU実行・既知の期待値との自己検証のみを行います(325のCodon inline probe、327のw_arr load-split probeと同じ「本体に触れる前に独立ファイルで確認する」方法論を、CUDA Cツールチェーン側に適用したものです)
- 対応するビルド/実行チェックスクリプト`336_cudac_smoke_test_check.sh`も同梱しています(こちらはGPUを実際に使用します。既存のN=21フルvalidateとは完全に独立しています)
- 次の一手は、このスモークテストの実行結果(ビルドログ・GPU出力)を確認し、成功すればbin形式のC側reader仕様化とN=18限定maxd14移植の設計に進むことです
"""
...
VERSION_TAG:str="336 cudac-smoke-test: NO kernel or dispatcher change -- byte-for-byte identical to 335/334/333/328-332 (w3_j7 remains the funcid_reorder default). 335's own full N=21 validate is now CONFIRMED: correctness 314666222712, elapsed 450.218s (vs 334's 450.181s, -0.008%, within noise), all static checks OK plus four toolchain INFO probes. 335's toolchain recon results (now settled): nvcc resolves to /usr/local/cuda/bin/nvcc, CUDA 13.0 (V13.0.88); cuobjdump was searched both on PATH and directly inside nvcc's own directory (/usr/local/cuda/bin) and found in NEITHER -- this CUDA 13.0 toolkit install genuinely does not ship cuobjdump (325/327's past cuobjdump successes were presumably a different environment, or actually routed through the ncu fallback both probes documented at the time). A10G Compute Capability measured via nvidia-smi --query-gpu=compute_cap: 8.6, so -arch=sm_86 is the confirmed build flag for any future nvcc invocation (this codebase's only prior sm_XX reference, sm_61, was for a different, older GPU and was never applicable to the A10G). 336 writes the first concrete buried-idea C-2 (rev84 CUDA C runner) artifact: a standalone smoke test (336_cudac_smoke_test.cu + 336_cudac_smoke_test_check.sh) that builds with nvcc -arch=sm_86, launches a trivial self-verifying GPU kernel (no DFS logic, no bin-format interaction), and confirms basic compile/link/execute round-trip works on cudacodon before any bin-format reader or maxd14 port work begins. This mirrors the 325 (Codon inline probe) / 327 (w_arr load-split probe) methodology of validating toolchain/codegen behavior in an isolated file before touching the main solver, applied here to the CUDA C toolchain instead of Codon. Since cuobjdump is unavailable, any future SASS-level inspection of the .cu spike will route through the already-established sudo ncu --section SourceCounters --page source path (318+) rather than cuobjdump --dump-sass. The main solver kernel/dispatcher/host params (w3_j7, K=48, SoA w_lo_arr/w_hi_arr) remain completely unchanged from 335/334/333."
