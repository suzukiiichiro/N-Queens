...
# =============================================================================
# NQ_UPDATE_MEMO
# 196: 更新メモ: 195は正当性OKだが194比0.450秒遅く不採用。194のroot one/two-candidate preroll/future_check_mask guard/no-siblingを親へ戻し、MAXD14 kernelのみroot<=2判定をsecond-lowbit方式へ置き換える復帰・微差実験を導入。
...
196Py note:
  195Py was correct but slower than the 194 fastest baseline.  This branch
  returns the MAXD14 root fast-start policy to the validated 194 scope
  (post-root-action root_avail has one or two bits only) while keeping the
  188 future_check_mask guard and 184 no-sibling ancestor stack rule.  The
  only new micro-experiment is the root<=2 predicate shape: use a second-lowbit
  test instead of the 195 root<=3 tail test, so 3-bit roots go back to the
  generic 194 loop.
...
ただし速度は `0:07:08.738` でした。
194Pyの `0:07:08.288` より `0.450秒`、`0.105%` 遅いため、**195Pyは正当性OKだが不採用**としました。新最速基準は引き続き **194Py** です。

そのため196Pyは、195の3bit root prerollを採用せず、194の `root one/two-candidate preroll` へ戻しています。差分はMAXD14 kernel内だけで、root<=2判定を second-lowbit 方式へ置き換える復帰・微差実験です。

```text
195Py:
  root_avail 1bit/2bit/3bit をpreroll
  → 正当性OKだが194より遅い

196Py:
  root_avail 1bit/2bit のみpreroll
  3bit以上は194 generic loopへ戻す
...
判定:
  ...
  root_after_second == 0 のときだけpreroll
```
...
```text
bash -n 196Py...sh                         OK
STATIC_ONLY=1 196Py...sh                   OK
source_future_check_mask_guard             OK
source_nosibling_parent                    OK
source_root_two_predicate_rebase           OK
...
source_split_tag split196 only             OK
```

こちらではCodon/CUDA実機build/full実行はしていません。cudacodon側では添付の `196Py...sh` を実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に194/195/192/188比のtimingを見て採用可否を判断してください。

作成物はこちらです。

* [196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_probe.py)
* [196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_bundle_20260701.tar)

"""

# 196 root<=2 rebase memo over validated 194 root-two futuremask/no-sibling MAXD14:
#   195 was correct but 0.450 seconds slower than 194, so this branch does not
#   adopt the root three-candidate fast-start. It returns to the validated 194
#   root one/two-candidate preroll, 188 future_check_mask guard, and 184
#   no-sibling tailcall build.
...
#   only restorable ancestor frames.  196 keeps the narrow root fast-start only when
#   post-root-action root availability has one or two bits, matching the validated
#   194 behavior.  Three-candidate roots explicitly return to the generic 194 loop.
...
# 196 keeps the validated 194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 196 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="196 roottwo-rebase: 195 rootthree correct but slower; return to 194 root one/two-candidate futuremask/no-sibling MAXD14 with second-lowbit root<=2 predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """196 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate preroll rebase."""
    ...
    # 196 keeps the validated 194 current-frame register cache, 13-slot
    # ancestor storage, no-sibling tail-call descent, future-prune guard, and
    # root one/two-candidate preroll. It removes the 195 three-candidate expansion: when the root
    ...
    # 196 root_two_predicate_rebase:
    # If the root frame has one or two candidates, process the first root
    # candidate before the generic DFS loop, matching the validated 194 scope.
    # 195's three-candidate case is intentionally removed because it was correct
    # but slower than 194.  The predicate uses a second-lowbit test: after removing
    # the first candidate, remove one more lowbit and preroll only when nothing
    # remains.  Therefore 3-bit and wider roots stay on the generic loop.
...
CHUNKSHAPE148_DEFAULT_REASON:str="196 roottwo-rebase reuses validated 194 scorestripe_v9 task order/cache; 195 root three-candidate fast-start was correct but not adopted"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split196_roottwo_rebase_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split196_roottwo_rebase_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split196-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split196-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split196_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split196-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split196-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split196_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
