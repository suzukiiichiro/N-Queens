...
# =============================================================================
# NQ_UPDATE_MEMO
# 204: 更新メモ: 203は正当性OKだが197比で遅いため不採用。197 root<=2 direct-if preroll/future_check_mask guard/no-siblingを親に戻し、MAXD14 kernelのみroot_first除去後のroot_rest生成を XOR ではなく clear-lowbit 式へ置き換える微差実験を導入。
...
"""
203Pyログを確認しました。**正当性・実行結果は問題ありません。**
...
```text
203Py  0:07:08.344
197Py  0:07:08.202  → 203が 0.142秒 / 0.033% 遅い
202Py  0:07:08.482  → 203が 0.138秒 高速
201Py  0:07:08.383  → 203が 0.039秒 高速
199Py  0:07:08.293  → 203が 0.051秒 遅い
198Py  0:07:08.239  → 203が 0.105秒 遅い
196Py  0:07:08.239  → 203が 0.105秒 遅い
194Py  0:07:08.288  → 203が 0.056秒 遅い
```

そのため、**203Pyは正当性OKだが197Pyを上回らないため不採用**としました。
新最速基準は引き続き **197Py** です。

204Pyは203を親にせず、197Pyへ戻しています。

204Pyの差分はMAXD14 kernel内だけです。

```text
197Py:
  root_first = cur_avail & -cur_avail
  root_rest  = cur_avail ^ root_first

204Py:
  root_first = cur_avail & -cur_avail
  root_rest  = cur_avail & (cur_avail - 1)
```

意味は同じです。どちらも `cur_avail` からlowbit、つまり `root_first` を除去して `root_rest` を作ります。

```text
root<=2 predicate:
  root_second       = root_rest & -root_rest
...
```bash
STATIC_ONLY=1 bash 204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_validate_full_once.sh
bash 204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物はこちらです。

* [204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_probe.py)
* [204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/204Py_kernel_roottwo_rootclearrest_futuremask_nosibling_maxd14_bundle_20260701.tar)
...
VERSION_TAG:str="204 roottwo-rootclearrest: 197 root<=2 direct-if futuremask/no-sibling MAXD14 baseline with root_rest clear-lowbit expression - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # correct but slower.  The predicate remains the 197 second-lowbit direct-if
    # shape, but root_rest is built by clearing the low bit directly instead of
    # XORing root_first.  Therefore 3-bit and wider roots stay on the generic loop.
...
CHUNKSHAPE148_DEFAULT_REASON:str="204 rootclearrest reuses validated 197 scorestripe_v9 task order/cache; only MAXD14 root_rest expression changes"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split204_roottwo_rootclearrest_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split204_roottwo_rootclearrest_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split204-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split204-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split204_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split204-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split204-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split204_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
