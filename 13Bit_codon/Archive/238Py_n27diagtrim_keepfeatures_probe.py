...
"""
原因はご指摘どおり、**N27へ不要な診断を削る作業の中で、mode28/29/30/31 本線でも使う runtime parameter まで削ってしまった**ことです。

今回の r2 では、N21専用化や機能削除はせず、238の方針を保ったまま以下を復元しました。

```text
復元したruntime global:
  FUNCID_REORDER_V2_WINDOW_MULT=8
  FUNCID_REORDER_V2_PHASE_JUMP=7
  FUNCID_REORDER_V2_DEFAULT_REASON
  BROAD_MARKDIST_TAIL_REORDER_VERSION="v4"
  BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON
  BROAD_MARKDIST_TAIL_VARIANT=2
  BROAD_MARKDIST_TAIL_PHASE_SALT=53
  BROAD_MARKDIST_TAIL_CELL_SALT=17
  BROAD_MARKDIST_TAIL_RISK_SALT=11

復元したruntime helper:
  stream_funcid_reorder_risk_suffix
  funcid_reorder_make_quotas
  interleave_funcid_reorder_parts
```

これらは旧診断ではなく、**broadmarktail / chunkshape148 / split145 の cache-miss時生成やcache名、shaping順序に必要な本線部品**でした。

こちらでは以下を確認済みです。

```text
STATIC_ONLY=1                         OK
preprocessed AST missing-name scan     OK

source_runtime_globals                 OK
required_runtime_defs                  OK
removed_diag_defs                      OK
removed_diag_modes 17..27              OK
kept_core_modes 28/29/30/31            OK
source_split_tag split238              OK
worker_split_args                      OK
```

237親版との正規化比較では、以下は同一です。

```text
MAXD14/16/18/20/21 kernels
launch_kernel_dfs_iter_gpu_static_maxd
build_soa_for_range
ensure_constellations_bin_stream
build_broad_markdist_tail_reordered_bin
build_chunkshape148_reordered_bin
exec_solutions_gpu_bin_stream_funcid_reorder
exec_solutions_gpu_bin_stream_split145
exec_solutions_gpu_chunk_split145
```

cudacodon側では古い238を上書きしてから、念のためbinaryを消して実行してください。

```bash
rm -f ./238Py_n27diagtrim_keepfeatures_probe
...
* [238_n27diagtrim_r2_buildfix_report.txt](sandbox:/mnt/data/238_n27diagtrim_r2_buildfix_report.txt)
* [238_n27diagtrim_r2_sha256.txt](sandbox:/mnt/data/238_n27diagtrim_r2_sha256.txt)
* [238 r2 buildfix bundle](sandbox:/mnt/data/238Py_n27diagtrim_keepfeatures_r2_buildfix_bundle_20260706.tar)
...
19f6118a04db74004bea11e64d547003a3272e4722f11bdf5d976c5066675a84  README.md
0914805091ff745620e379e59732a8c302fdc179d5f665a3342ab6fb67a02162  238Py_n27diagtrim_keepfeatures_r2_buildfix_bundle_20260706.tar
```
