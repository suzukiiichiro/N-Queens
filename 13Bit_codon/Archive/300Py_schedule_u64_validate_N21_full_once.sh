...
# =============================================================================
# 300 N=21 full validation harness (schedule-u64 probe)
...
# EXPECTED_CHUNKS unchanged at 5 (K=32 as in 296).
...
SRC=${SRC:-./300Py_schedule_u64_probe.py}
CAND=${CAND:-./300Py_schedule_u64_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/300Py_schedule_u64_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/300Py_schedule_u64_logs_N21_full_once_${TS}"
...
echo "[start] 300 schedule-u64 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 300 schedule-u64 SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '300 schedule-u64 probe' "$SRC"; then
  printf 'source_version_tag	300 schedule-u64 probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	300 schedule-u64 probe	missing	FAIL
...
# 300: schedule-u64 shape checks.
    ok_stride_param = 'stride:int,' in body14.split(')->None:')[0]
    ok_stack_array = 'stack=__array__[u64](MAXD14_ANCESTOR*2)' in body14
    ok_no_schedule_lo = 'schedule_lo' not in body14
    ok_no_schedule_hi = 'schedule_hi' not in body14
    ok_schedule_u64 = 'schedule:u64=u64(0)' in body14
    ok_schedule_build = body14.count('schedule|=u64(frame_nibble)') == 2
    ok_hot_decode = 'nibble_op:u32=u32((schedule>>u64(cur_depth*4))&u64(15))' in body14
    ok_no_depth_branch = 'if cur_depth<8:' not in body14
    ...
    ok_shape = (ok_stride_param and ok_stack_array and ok_no_schedule_lo and ok_no_schedule_hi
                 and ok_schedule_u64 and ok_schedule_build and ok_hot_decode and ok_no_depth_branch
                 and ok_save_sp and ok_next_depth and ok_stack_ptr_incr and ok_gridstride_loop
                 and ok_single_writeback and ok_push)
    checks.append(('source_schedule_u64_shape',
                    'schedule:u64 + no schedule_lo/hi + branch-free hot decode + 296 stack unchanged',
                    'present' if ok_shape else (
                        'missing (stride=%s stack=%s no_lo=%s no_hi=%s sched_u64=%s build=%s hot=%s no_br=%s sp=%s nd=%s ptr=%s gs=%s wb=%s push=%s)'
                        % (ok_stride_param, ok_stack_array, ok_no_schedule_lo, ok_no_schedule_hi,
                           ok_schedule_u64, ok_schedule_build, ok_hot_decode, ok_no_depth_branch,
                           ok_save_sp, ok_next_depth, ok_stack_ptr_incr, ok_gridstride_loop,
                           ok_single_writeback, ok_push)
                    ),
                    ok_shape))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 300 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 300 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 300 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 300 schedule-u64 mode31 split145"
...
echo "[validation-ok] 300 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
