...
# =============================================================================
# NQ_UPDATE_MEMO
# 276: restore274-after-bucketdiag-reject。275 split145 bucket診断で
#      d2base14 は 8214 tasks と小さく、d0 は 293733 tasks あるが
#      全131chunkに混在し、chunk単位専用dispatchには向かないと判断。
#      速度候補ではなく、275診断差分を撤回して274/271本線へ戻す安全復帰版。
#      CUDA MAXD14/16/18/20/21 kernel本文、rootrestlate、futuremask、
#      no-sibling、split145/chunkshape148、broadmarktail、cache生成、
#      worker split、mode28/29/30/31、MAXD fallback、CPU dfs_iter pathは維持。
...
"""
275ログを確認しました。計算自体はOKですが、診断結果から **d2base14/d0 のchunk単位専用化は見送り**が妥当です。
...
d2base14_tasks : 8214
d0_tasks       : 293733
d0_or_d2_tasks : 301947
other_tasks    : 1723335
both_chunks    : 131 / 131
```

d2base14は少なく、d0は全chunkに混在しているため、chunk単位dispatchには向きません。
そのため276は **275診断を撤回して274/271本線へ戻す安全復帰版**にしました。

`STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [276Py_restore274_after_bucketdiag_reject_probe.py](sandbox:/mnt/data/276Py_restore274_after_bucketdiag_reject_probe.py)
* [276Py_restore274_after_bucketdiag_reject_validate_N21_full_once.sh](sandbox:/mnt/data/276Py_restore274_after_bucketdiag_reject_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [276Py_restore274_after_bucketdiag_reject_sha256.txt](sandbox:/mnt/data/276Py_restore274_after_bucketdiag_reject_sha256.txt)
* [276Py_restore274_after_bucketdiag_reject_bundle_20260709.tar](sandbox:/mnt/data/276Py_restore274_after_bucketdiag_reject_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./276Py_restore274_after_bucketdiag_reject_probe

STATIC_ONLY=1 bash 276Py_restore274_after_bucketdiag_reject_validate_N21_full_once.sh

bash 276Py_restore274_after_bucketdiag_reject_validate_N21_full_once.sh
```

276で `0:07:07.7xx` 付近へ戻れば、bucket分解系もいったん終了してよいと思います。
...
VERSION_TAG:str="276 restore274-after-bucketdiag-reject: 275 split145 bucket diagnostic showed d2base14 tiny and d0 mixed across all chunks, so no chunk-level d0/d2 specialization is adopted; restored 274/271 final-fastest-complete baseline; generic N5..27; bare -g split145 mode31; unused recursive CPU dfs fallback remains removed; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved; CUDA kernels unchanged"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split276_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split276_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split276-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split276-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split276_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
