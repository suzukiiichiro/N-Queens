...
# =============================================================================
# 290 N=21 full validation harness
# Parent: 290 nibble0-normal-fastpath. Experiment: nibble_op==0 normal/no-future fast path in MAXD14 generic loop.
...
SRC=${SRC:-./290Py_nibble0_normal_fastpath_probe.py}
CAND=${CAND:-./290Py_nibble0_normal_fastpath_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/290Py_nibble0_normal_fastpath_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/290Py_nibble0_normal_fastpath_logs_N21_full_once_${TS}"
...
echo "[start] 290 nibble0-normal-fastpath validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 290 nibble0-normal-fastpath SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '290 nibble0-normal-fastpath' "$SRC"; then
  printf 'source_version_tag	290 nibble0-normal-fastpath	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	290 nibble0-normal-fastpath	missing	FAIL
...
# Keep this check intentionally runtime-tag focused; explanatory comments may mention older split tags.
if 'split290' in s:
    checks.append(('source_split_tag', 'split290 active', 'split290', True))
else:
    checks.append(('source_split_tag', 'split290 active', 'split290 missing', False))
...
# Check MAXD14 generic DFS loop 290 nibble_op==0 fast path shape. Keep root-preroll branch untouched.
...
if not m14:
    checks.append(('source_generic_nibble0_fastpath', 'MAXD14 nibble_op==0 normal fast path', 'MAXD14 not found', False))
else:
    body = m14.group(0)
    idx = body.find('while True:')
    tail = body[idx:] if idx >= 0 else body
    expected_default = '''nld:u32=(cur_ld|bit)<<u32(1)
      nrd:u32=(cur_rd|bit)>>u32(1)
      ncol:u32=cur_col|bit'''
    fast_snippet = '''if nibble_op==u32(0):
        fast_nf:u32=bm&~(nld|nrd|ncol)'''
    block_after_fast = tail.find('if nibble_op==u32(0):') >= 0 and tail.find('block_code:u32=nibble_op&u32(7)') > tail.find('if nibble_op==u32(0):')
    nf_default_bad = '''nf:u32=bm&~(nld|nrd|ncol)
      if block_code!=u32(0):''' in tail
    ok = expected_default in tail and fast_snippet in tail and block_after_fast and not nf_default_bad and 'fast_next_depth:int=cur_depth+1' in tail
    checks.append(('source_generic_nibble0_fastpath', 'MAXD14 nibble_op==0 normal fast path; nf default absent', 'present' if ok else 'missing/old-form-or-nfdefault', ok))
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 290 nibble0-normal-fastpath mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "289ncolonly:$BASELINE_289_NCOLONLY_SECONDS" "288rejected:$BASELINE_288_REJECT_SECONDS" "287adopt:$BASELINE_287_ADOPT_SECONDS" "286adopt:$BASELINE_286_ADOPT_SECONDS" "285restore:$BASELINE_285_RESTORE_SECONDS" "284rejected:$BASELINE_284_REJECT_SECONDS" "283normalfirst:$BASELINE_283_NORMALFIRST_SECONDS" "276current:427.672" "271fastest:427.705" "239:427.703" "217:427.709"; do
...
echo "[validation-ok] 290 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
