...
# =============================================================================
# 277 N=21 full validation harness
# Parent: 276 restore274-after-bucketdiag-reject / 271 final-fastest-complete.
# Experiment: MAXD14 generic DFS loop only: create depth_u:u32=u32(cur_depth)
#             once after terminal check and reuse it for child_jmark shift and
#             save payload depth pack. Keep nibble extraction, root-preroll,
#             dispatch, cache generation, worker split, and one launch per chunk.
...
SRC=${SRC:-./277Py_depthu_childsave_probe.py}
CAND=${CAND:-./277Py_depthu_childsave_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
# 277: visible startup + force release rebuild by default so a previous non-release
# `codon build` executable is not reused and cannot trigger CUDA_ERROR_INVALID_PTX.
# 277: print start/status early and prevent static pycheck from failing silently under set -e.
FORCE_REBUILD=${FORCE_REBUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/277Py_depthu_childsave_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/277Py_depthu_childsave_logs_N21_full_once_${TS}"
...
echo "[start] 277 depthu-childsave validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 277 depthu-childsave SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '277 depthu-childsave' "$SRC"; then
  printf 'source_version_tag\t277 depthu-childsave\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t277 depthu-childsave\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split277' in s and 'split276' not in s and 'split275' not in s and 'split274' not in s and 'split273' not in s and 'rootaction0-direct' not in s and 'kernel_dfs_iter_gpu_maxd14_root0' not in s and 'split270_' not in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split277 active; 276 parent restored; 275/273/270/240 rejected', 'split277', True))
else:
    checks.append(('source_split_tag', 'split277 active; 275/273/270/240 rejected', 'old/bucket/root0 split runtime present or split277 missing', False))
...
# 277: verify the intended MAXD14 generic-loop depth_u commonization is active
...
if not m14:
    checks.append(('source_depthu_childsave', 'present in MAXD14', 'MAXD14 kernel not found', False))
else:
    body = m14.group(0)
    ok = (
        'depth_u:u32=u32(cur_depth)' in body and
        'child_jmark:u32=(child_jmark_mask>>depth_u)&u32(1)' in body and
        'avail[save_sp]=cur_avail|(depth_u<<u32(27))' in body and
        'child_jmark:u32=(child_jmark_mask>>u32(cur_depth))&u32(1)' not in body and
        body.count('avail[save_sp]=cur_avail|(u32(cur_depth)<<u32(27))') == 1
    )
    checks.append(('source_depthu_childsave', 'present in MAXD14 generic loop', 'present' if ok else 'missing/generic old pattern remains', ok))
fail = 0
...
echo "[static-ok] 277 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 277 depthu-childsave mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "276current:$BASELINE_276_CURRENT_SECONDS" "276parent274:$BASELINE_276_PARENT_274_SECONDS" "275diag:$BASELINE_275_DIAG_SECONDS" "273rejected:$BASELINE_273_REJECT_SECONDS" "272diag:$BASELINE_272_DIAG_SECONDS" "271fastest:$BASELINE_271_FASTEST_SECONDS" "271parent241:$BASELINE_271_PARENT_SECONDS" "270rejected:$BASELINE_270_REJECT_SECONDS" "267:$BASELINE_267_SECONDS" "239:$BASELINE_239_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 277 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
