...
# 139 locked validation harness
# - Candidate: 139Py_scorestripe_chunkshape_probe
# - Kernel path remains generic-only.
# - 139 is scorestripe_v7_lanephase32_pairphase11 from the validated 138 scorestripe_v6_lanephase32_oddphasebias line.
# - Because 139 changes host-side permutation, fixed selected-chunk partial_total values are learned
#   from the latest 139Py1-6.sh validation log directory before this script runs.
# - Set LOCK_SOURCE_DIR=/path/to/139Py_logs_N21_validation_YYYYMMDD_HHMMSS to pin a specific seed run.

BASE=${BASE:-./115Py_range_default_clean_cg_v2}
CAND=${CAND:-./139Py_scorestripe_chunkshape_probe}
RUN_FULL=${RUN_FULL:-1}
LOCK_SOURCE_DIR=${LOCK_SOURCE_DIR:-}

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="139Py_logs_N21_locked_validation_${TS}"
...
find_lock_source_dir() {
  ...
  ls -dt 139Py_logs_N21_validation_* 2>/dev/null | head -n 1
}
...
if [ -z "$LOCK_DIR" ] || [ ! -d "$LOCK_DIR" ]; then
  echo "[lock-error] 139 validation log directory not found. Run ./139Py1-6.sh first, or set LOCK_SOURCE_DIR." >&2
  echo -e "lock_source\tmissing\tFAIL_LOCK_SOURCE\t2\t-" >> "$SUMMARY"
  exit 2
fi

EXP01=$(extract_partial_total "${LOCK_DIR}/01_cand139_mode30_standard_7chunk.log")
EXP02=$(extract_partial_total "${LOCK_DIR}/02_cand139_mode30_heavy_band.log")
EXP03=$(extract_partial_total "${LOCK_DIR}/03_cand139_mode30_d2base14_density.log")
EXP04=$(extract_partial_total "${LOCK_DIR}/04_cand139_mode30_late_tail.log")
EXP05=$(extract_partial_total "${LOCK_DIR}/05_cand139_mode31_worker0of4_quick.log")

if [ -z "$EXP01" ] || [ -z "$EXP02" ] || [ -z "$EXP03" ] || [ -z "$EXP04" ] || [ -z "$EXP05" ]; then
  echo "[lock-error] could not extract all 139 fixed partial_total values from ${LOCK_DIR}" >&2
...
# ================================================================
# 1) 139 selected-chunk smoke: standard spread
# ================================================================
run_case \
  "01_cand139_mode30_standard_7chunk" \
  "partial_total=${EXP01}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "0,20,40,60,80,100,120"

# ================================================================
# 2) 139 selected-chunk stress: heavy band
# ================================================================
run_case \
  "02_cand139_mode30_heavy_band" \
  "partial_total=${EXP02}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "35,40,41,42,47,48,52,53"

# ================================================================
# 3) 139 selected-chunk stress: d2base14-density band
# ================================================================
run_case \
  "03_cand139_mode30_d2base14_density" \
  "partial_total=${EXP03}" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "20,40,55,56,57,58,60"

# ================================================================
# 4) 139 selected-chunk stress: late/tail band
# ================================================================
run_case \
  "04_cand139_mode30_late_tail" \
  "partial_total=${EXP04}" \
  "$CAND" \
...
# ================================================================
run_case \
  "05_cand139_mode31_worker0of4_quick" \
...
if [ "${RUN_FULL}" = "1" ]; then
  run_case \
    "06_cand139_mode31_full_correctness" \
    "314666222712.*ok" \
    "$CAND" \
