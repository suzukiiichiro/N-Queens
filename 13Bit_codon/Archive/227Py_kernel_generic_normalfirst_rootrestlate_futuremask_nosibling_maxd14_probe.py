...
# =============================================================================
# NQ_UPDATE_MEMO
# 227: 更新メモ: 226Py generic place-or は正当性OKだが225/217最速を上回らなかったため不採用。227Pyは225Pyのncol-earlyを親に、MAXD14 generic loopのみ block_code==0 のnormal pathを先に書く分岐順序微差実験。rootrestlate / root_after_second scalar predicate / root_first late extraction / futuremask / no-sibling / root one-two preroll は維持する。
...
warning/error            0 / 0
N=21 full                0:07:07.998
```

ただし速度は225Pyより遅くなりました。
...
したがって、226Pyの `placed_ld` / `placed_rd` 共通化は**不採用**です。

227Pyは226の差分を撤回して225Pyの `ncol` early へ戻し、MAXD14 generic loopだけで **normal path first** を試す版にしました。
...
`rootrestlate`、`root_after_second` scalar predicate、futuremask、no-sibling、root one/two-candidate preroll、dispatch、host task orderは変更していません。こちらでは `STATIC_ONLY=1` の静的確認までOKです。

```bash id="2drmul"
cd /home/suzuki/Github/N-Queens/13Bit_codon/Archive

STATIC_ONLY=1 bash 227Py_kernel_generic_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 227Py_kernel_generic_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物です。

* [227Py_kernel_generic_normalfirst_rootrestlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/227Py_kernel_generic_normalfirst_rootrestlate_futuremask_nosibling_maxd14_probe.py)
* [227Py_kernel_generic_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/227Py_kernel_generic_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/227Py_kernel_generic_normalfirst_rootrestlate_futuremask_nosibling_maxd14_bundle_20260703.tar)

"""

# 227 generic-normalfirst memo over validated 217 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="227 generic-normalfirst: 222 MAXD13 diagnostics withdrawn; validated 217 rootrestlate scalar predicate restored - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while True:
      ...
      # 227 generic-normalfirst: keep 225's ncol-early shape, but test the
      # common normal path first.  Semantics are unchanged because block_code==0
      # is exactly the old else branch, and nonzero block codes keep the same
      # block-specific diagonal update.
...
CHUNKSHAPE148_DEFAULT_REASON:str="227 generic-normalfirst reuses validated 217 scorestripe_v9 task order/cache; MAXD14 scalar root_after_second predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split227_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split227_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split227-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split227-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split227_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split227-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split227-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split227_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
