...
# =============================================================================
# NQ_UPDATE_MEMO
# 184: 更新メモ: 183のN=21 full検証OK・0:07:51.091を受け、host shaping/dispatchは183のまま維持。MAXD14 kernelのみ、親に残候補がないdescendでancestor保存を省く no-sibling spill elision / tail-call descent を単独実験として導入。
...
おはようございます。
183ログは、N=21 full `0:07:51.091`、`required_maxd=14 / selected_MAXD=14 / schedule_words=0 / stack_bytes_per_thread=208`、tail平坦という前提で問題なさそうでした。184Pyは段取りメモ通り、N=22へ進まず、183を親にしてMAXD14 kernelだけを no-sibling spill elision / tail-call descent にした単独実験版にしています。

作成しました。

* [184Py_kernel_nosibling_tailcall_maxd14_probe.py](sandbox:/mnt/data/184Py_kernel_nosibling_tailcall_maxd14_probe.py)
* [184Py_kernel_nosibling_tailcall_maxd14_validate_full_once.sh](sandbox:/mnt/data/184Py_kernel_nosibling_tailcall_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/184Py_kernel_nosibling_tailcall_maxd14_bundle_20260630.tar)

主な差分は、183Pyの `sp` を `cur_depth` と `save_sp` に分離し、親に残候補がある場合だけancestor stackへ保存する形です。残候補がない場合は保存せず、childをそのままactive frameにします。復元用の論理depthは `avail` 上位5bitへpackする方式にしました。N<=27ではbitboardが下位27bitに収まるため、`stack_bytes_per_thread=208` は維持しています。

手元では以下を確認済みです。

```text
bash -n 184Py...sh             OK
STATIC_ONLY=1 184Py...sh       OK
split184 tag                   OK
split183残存                   なし
abstract stack policy model    2000 cases OK
```

こちらの環境ではCodon/CUDA実機でのbuild/full実行はできていません。184の採用可否は、添付した `184Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/errorを確認して判断してください。


"""

# 184 no-sibling tail-call revision memo over validated 183/182/181/180/175 nibble-schedule MAXD14:
...
#   only restorable ancestor frames.  184 changes only the MAXD14 descend policy:
...
# 184 keeps the validated 183 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 184 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="184 no-sibling tailcall: 183 MAXD14 nibble kernel plus spill-elision experiment - README-managed history"
...
# 184 inherits the exact 183/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """184 MAXD14 nibble schedule plus no-sibling spill elision / tail-call descent."""
    ...
    # 184 keeps the validated 183 current-frame register cache and 13-slot
...
CHUNKSHAPE148_DEFAULT_REASON:str="184 no-sibling tailcall reuses validated 183 scorestripe_v9 task order/cache and changes only MAXD14 descend spill policy"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split184_nosibling_tailcall_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split184_nosibling_tailcall_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split184-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split184-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split184_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split184-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split184-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split184_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
