...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 340 (2026-07-27)
...
8. [実装・339で効果を確認・340でスイープ中] warp内コスト不均質(tail effect)
   329で確定した「Divergent Branches=0、かつAvg Threads Executed 2〜3/32」
   は、分岐発散ではなくwarp内32レーンのDFS試行回数(trip count)の
   ばらつきを意味する。338で仮説化、339でソース精読により実装上の
   根拠を特定し対策を実装、**339のN=21フル実行で効果を確認した**。

   (a) インデクシング: grid-strideにより、同一warpの32レーンが同一
       イテレーションで処理するのは配列上で連続する32レコード
       (idx = n*stride + 32w + j、j=0..31)。
   (b) 現状(276以来): `build_chunkshape148_reordered_bin`は
       `interleave_order=[7,0,6,1,5,2,4,3]`を**1レコードずつ**回すため、
       連続32レコード=1 warp-iterationは8バケット全部を4回ずつ含む、
       意図的に最も不均質な構成だった。
   (c) ただしそれはwarp「間」の均衡設計でもあり、単純な全体ソートは
       別の不均衡と交換するだけになる。
   (d) 両立可能: 作業単位`STEPS=BLOCK*MAX_BLOCKS=15488`はgrid-stride
       1イテレーションそのものであり、バケット順は`(oi+out_ch)%8`で
       イテレーションごとに1回転する。よって1バケットから32件連続で
       出すと、warp wはイテレーションout_chで
       `interleave_order[(w+out_ch)%8]`を受け取り、131イテレーション
       全体で全8バケットをほぼ一様に経験する。warp内は均質化、
       warp間の均衡は維持。

   **339の実測結果(確定)**: `CHUNKSHAPE148_BUCKET_RUN=32`で
   正当性314666222712一致、elapsed **447.116s**。338(RUN=1相当)の
   450.329s比 **-0.713%**、333以降のベースライン群(450.056〜450.667s、
   全幅0.136%)に対しても**-0.65%以上**で、ノイズ全幅の約4.8倍。
   `chunkshape148_cache_state=reuse`のためタイミングは比較可能。
   チャンク別kernel elapsedは chunk0 +0.244% / chunk1 -0.745% /
   chunk2 -1.856%、合計 -0.666%。**仮説(d)は支持された。**

   **340でスイープする**。単なる単調スイープではなく、モデルを識別する
   3点を選んだ:
     - RUN=16 (warp幅未満。1 warp-iterationが2バケットにまたがる)
       → モデルが正しければ32より悪いはず
     - RUN=48 (32の倍数でない。warp境界とバケット境界がずれる)
       → **値が大きいのに悪化するなら「大きいほど良い」ではなく
         「warp幅に整列していることが本質」だと確定できる**
     - RUN=64 (32の倍数。warp内均質性は維持され、warp間の混合が粗くなる)
       → モデルが正しければ32とほぼ同等のはず
   RUN=1(対照)とRUN=32(アンカー)も同一セッション内で測り直し、
   セッション間ドリフトを排除する。RUN=32はスイープの最初と最後の
   2回測り、ドリフト自体も検出する。

   **340はソースコードを一切変更しない**(340のソースは339と
   VERSION_TAG/ドキュメント以外バイト一致)。スイープは既存ノブへの
   引数だけで実行できるため、`340_bucket_run_sweep_driver.sh`という
   独立したドライバスクリプトで行う。

   次の軸(341候補、340の結果次第): バケット数そのものの細分化。
   現在スコアバケットは8個固定であり、RUN=32でwarp内は「同一バケット」
   にはなるが、バケット内のばらつきは残る。バケット数を16/32に増やせば
   warp内の残存分散をさらに削れる。ただし`interleave_order`(8要素固定)、
   `chunkshape148_build_thresholds_from_hist`(8閾値)、
   `bucket_lane_indices`(8*32)に手が入るため339より侵襲的であり、
   340でRUN=32の最適性が確認できてから着手するのが順序として正しい。

このセクション自体は通常カーネルロジックとは独立しています。
340は**ソースコードを一切変更しません**。339のソースに対する差分は
このドキュメントとVERSION_TAGのみで、カーネル・ディスパッチャ
(5つの@gpu.kernel、launch_kernel_dfs_iter_gpu_static_maxd、K=48、
SoA w_lo_arr/w_hi_arr、w3_j7)もホスト側の
build_chunkshape148_reordered_bin もすべて339とバイト一致です。
340のスイープは`CHUNKSHAPE148_BUCKET_RUN`への引数だけで実行でき、
独立したドライバ`340_bucket_run_sweep_driver.sh`が担当します。
ソース既定値は引き続き1(=276以来の現行動作)であり、339で確認された
RUN=32はハーネス/ドライバが明示的に渡します。採用の可否は340の
スイープ結果を見てから341で判断します。
================================================================================
...
"""
339、確認しました。**改善しています。****339 (RUN=32): 447.116s** — 338比 **−0.713%**、ノイズ全幅の約4.8倍の差です。`cache_state=reuse` なので比較可能、正当性も完全一致、FAILゼロ。仮説(d)が支持されました。340の設計に入ります。Ç¥åâ��Ç¥å��Ç¥åÆ��Ç¥å��Ñ��Ç¥å��Ñ��Ç¥å��Ç¥åÆ��Ç¥åÆ��Ç¥åâ��Ç¥å��Ç¥å��Ñ��Ç¥å��Ñ��Ñ��Ç¥å��Ç¥åâ��Ç¥å��Ç¥å��Ñ��Ç¥åÆ��Ç¥å��Ñ��Ç¥å��Ñ��Ç¥åâ��Ñ��Ç¥å��Ç¥å��Ç¥åÆ��Ç¥åÆ��Ñ��Ç¥åâ��Ç¥å��Ç¥åâ��Ç¥åâ��Ç¥åÆ��Ç¥å��Ç¥åÆ��Ç¥å��Ñ��Ç¥å��Ç¥å��Ç¥åÆ��2点ほど堅牢性を上げます(プリセット番号の推定と `grep -P` 依存の除去)。## 339の結果 — 効きました

| 比較対象 | ベースライン | 339 (RUN=32) |
|---|---|---|
...
**447.116s**、正当性 `314666222712` 一致、FAILゼロ、`cache_state=reuse` で比較可能。

333以降のベースライン群は450.056〜450.667s(全幅0.136%)。339の差分2.94sは**その約4.8倍**で、ノイズでは説明がつきません。チャンク別は chunk0 +0.244% / chunk1 −0.745% / chunk2 −1.856%、合計 −0.666%(chunk0だけわずかに悪化している点は、サンプル1点なので記録にとどめます)。

329で特定された tail effect に対する**最初の実測上の改善**であり、しかも**カーネルには一切触れていません**。カーネル側が5戦5敗なのに対し、ホスト側並べ替え軸は K-batching(−4.4%)、w3_j7(−1.3%)に続く3勝目です。

---

## 340 — ソース変更ゼロの測定リビジョン

`340Py_bucket_run_sweep.py` は339とVERSION_TAG・ドキュメント以外**バイト一致**です(`import gpu` 以降5,306行の完全一致を機械検証済み)。スイープはノブへの引数だけで済むので、コードを触る必要がありません。

### スイープ点は単調スイープにしていません

問うべきは「大きいほど良いのか」ではなく「**なぜ32が効くのか**」です。3点で識別します。

| RUN | 意味 | モデルの予測 |
|---|---|---|
| 16 | warp幅未満。1 warp-iterationが2バケットにまたがる | 32より**悪い** |
| **48** | **32より大きいが32の倍数でない。warp境界とバケット境界がずれる** | **32より悪い ← 決定的** |
| 64 | 32の倍数。warp内均質性は維持、warp間の混合が粗くなる | 32と**ほぼ同等** |

**RUN=48 が要です。** 値が大きいのに悪化するなら、機構は「run長」ではなく「**warp幅への整列**」だと確定できます。逆に48が32と64の中間に落ちたら、機構は run長 ということになり、341はバケット粒度ではなく上方向(96/128)のスイープに切り替えます。**この分岐条件を実測前に固定しておきました。**

RUN=1(対照)と RUN=32(アンカー)も同一セッション内で測り直します。**RUN=32は最初と最後の2回**測り、その差をそのままセッションドリフトの実測値として使います(332で対照点を4セッション測ったのと同じ規律です)。

### ドライバの動作

キャッシュ `..._run{R}.bin` が無い値だけ「ビルドパス(タイミング破棄)+計測パス」の2回、有る値は計測1回。Codonバイナリのビルドは初回のみ。**全点で正当性を検査**し、失敗点があっても残りを続行して最後に非ゼロ終了します。`cache_state=reuse` でない行には `NOT-COMPARABLE` を付けます。

所要は計測6回(約45分)+ 新規3値のキャッシュ構築。ディスクは新規1値あたり約32MBです。

## 実行手順

```
STATIC_ONLY=1 bash 340Py_bucket_run_sweep_validate_N21_full_once.sh
DRY_RUN=1 bash 340_bucket_run_sweep_driver.sh    # 実行計画の確認(すぐ終わります)
bash 340_bucket_run_sweep_driver.sh
```

時間が取れなければ `SWEEP="16 64" bash 340_bucket_run_sweep_driver.sh` と絞れます(ただし48を落とすと決定的な点が失われます)。

`bash -n` 3本、静的チェック(39項目 FAILゼロ)、ドライラン、キャッシュ検出のファイル名一致(339のログ実物と照合)まで手元で確認済みです。結果の `sweep_table.tsv` をいただければ、341で採用判断に進みます。
"""
...
VERSION_TAG:str="340 bucket-run-sweep: NO SOURCE CODE CHANGE AT ALL -- byte-for-byte identical to 339 except this VERSION_TAG and the documentation block. All five @gpu.kernel bodies, launch_kernel_dfs_iter_gpu_static_maxd, K_PER_THREAD_MAXD14=48, the SoA w_lo_arr/w_hi_arr split, the w3_j7 funcid_reorder defaults AND the host-side build_chunkshape148_reordered_bin emit loop introduced in 339 are all unchanged. 339 RESULT IS CONFIRMED AND POSITIVE: with CHUNKSHAPE148_BUCKET_RUN=32 the N=21 full run reproduced 314666222712 exactly and took 447.116s against 338s 450.329s, i.e. -0.713%, with chunkshape148_cache_state=reuse so the timing is directly comparable. Every baseline since 333 sits in 450.056-450.667s (full width 0.136%), so the -0.65% to -0.79% improvement is roughly 4.8x the entire historical noise band. Per-chunk kernel elapsed moved chunk0 +0.244%, chunk1 -0.745%, chunk2 -1.856%, total -0.666%. This is the first measured win against the tail effect that 329 identified, and it was obtained purely by host-side reordering with the kernel untouched. 340 does not change anything; it sweeps the existing knob to establish WHY it works, using three discriminating points rather than a plain monotone scan: RUN=16 (below warp width, so one warp-iteration straddles two buckets -- the model predicts worse than 32), RUN=48 (larger than 32 but NOT a multiple of it, so warp and bucket boundaries misalign -- if a larger value still regresses, that proves the mechanism is warp-width ALIGNMENT and not simply longer runs), and RUN=64 (a multiple of 32, so intra-warp homogeneity is preserved while inter-warp mixing coarsens -- the model predicts roughly equal to 32). RUN=1 (control) and RUN=32 (anchor) are re-measured in the same session to remove cross-session drift, and RUN=32 is measured both first and last so drift itself is visible. The sweep needs no rebuild between points because the knob is a runtime argument, so it is driven by a standalone script, 340_bucket_run_sweep_driver.sh, in the same spirit as 337s standalone bin reader check. Adoption is deliberately deferred to 341 so that 340 remains a pure measurement revision. If RUN=32 is confirmed as a bracketed optimum, the natural next axis is the number of score buckets itself: with 8 fixed buckets, RUN=32 makes a warp-iteration draw from a single bucket but the spread WITHIN that bucket remains, and raising the bucket count to 16 or 32 would cut the residual intra-warp variance further -- at the cost of touching interleave_order (fixed 8 elements), chunkshape148_build_thresholds_from_hist (8 thresholds) and bucket_lane_indices (8*32), which is more invasive than 339 and should wait for 340s confirmation."
