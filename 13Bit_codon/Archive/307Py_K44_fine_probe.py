...
**306 (K=56) = 351.534s** — vs 304(K=48)=351.070s 比で −0.464s (−0.132%) と **誤差級**。K=48 と K=56 はほぼ同等で、flat ゾーンに入っています。
...
| 40 | 353.587s | −2.517s |
| **48** | **351.070s** | **基準** |
| 56 | 351.534s | −0.464s (誤差) |
| 64 | 353.896s | −2.826s |

K=48〜56 のゾーンが最適、その中でも K=48 がわずかに最良。304 (K=48) を確定採用として、次のステップへ。## 306 結果まとめと 307 の考え方

**306 (K=56) = 351.534s — 誤差級、不採用**。K=48 との差 −0.464s は測定誤差の範囲内。
...
```
K=32: 353.671s  ┐
K=40: 353.587s  ┘  steep drop →
K=48: 351.070s  ← 現 BEST（確定）
K=56: 351.534s  ←  flat zone（誤差内）
K=64: 353.896s  ┐
               ┘  rising
```

K=48 が測定上の最良点。304 (K=48, 351.070s) が現確定ベスト。

## 307 の着眼点: K=44（左側の境界確認）

K=40→K=48 の区間は 2.517s も下がっており傾斜が急です。K=44 がどのくらいの値になるかで、最適ゾーンの左端が明確になります。CHUNKS=3（K=48 と同じ）。

```bash
STATIC_ONLY=1 bash 307Py_K44_fine_validate_N21_full_once.sh
bash 307Py_K44_fine_validate_N21_full_once.sh
```
"""
...
K_PER_THREAD_MAXD14:Static[int]=44


VERSION_TAG:str="307 K44-fine-probe: parent 306 K56-sweep (351.534s, flat vs 304 K48=351.070s); K sweep complete: K=32(353.671s) K=40(353.587s) K=48(351.070s BEST) K=56(351.534s flat) K=64(353.896s); K=48-56 is a flat optimum zone; probing K=44 to check if peak is between 40-48 or exactly at 48; EXPECTED_CHUNKS=ceil(2025282/(32*484*44))=ceil(2025282/681472)=3; all kernel logic unchanged from 296/304; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
