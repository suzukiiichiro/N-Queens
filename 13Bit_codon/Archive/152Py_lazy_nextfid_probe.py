...
151Pyは問題ありません。全131チャンク、部分和01〜05、最終合計`314666222712`が一致し、警告・エラー・欠落・重複はありません。実行時間は`0:16:16.854`です。

### 152Py

[152Py_lazy_nextfid_probe.py](sandbox:/mnt/data/152Py_lazy_nextfid_probe.py)

151を維持し、各新規DFSフレームで無条件に行っていた`meta_next[f]`の読込みを、P5・mark・jmark遷移が実際に成立した場合だけ行うよう変更しました。探索順序、計算式、キャッシュ形式は変更していません。

### 実行シェル

[152Py_validate_full_once.sh](sandbox:/mnt/data/152Py_validate_full_once.sh)

```bash
chmod +x 152Py_validate_full_once.sh
./152Py_validate_full_once.sh
```

全131チャンクを1回だけ実行し、同じprogress TSVから01〜05と全件結果を検証します。`flock`、自動ビルド、欠落・重複・エラー検査、151との時間比較を含みます。

シェル構文、模擬ログによる全検証処理、旧処理との遷移等価性71,680ケースは確認済みです。Codon/CUDA実機コンパイルはこの環境では未実施です。


"""

# 152 GPU kernel experiment:
#   Retain the fully validated 151 frame-state reload elision, 150 branchless
#   blockK selector, 149 ctrl-u32 decode, 148 task order, MAXD=21, five u32
#   DFS stacks, and nld/nrd full-u32 arithmetic.  The sole kernel change is
#   lazy next-fid lookup: meta_next[f] is read only when a P5, mark, or jmark
#   transition is actually taken, rather than once for every new DFS frame.
...
VERSION_TAG:str="152 lazy next-fid lookup over 151 frame reload elision - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    # 152: fixed-width operands retained for the u32 diagonal, ctrl-decode, and branchless blockK hot path.
    n3u:u32=u32(n3)
    n4u:u32=u32(n4)
    ...
    while sp>=0:
      ...
      if (cv&u32(INIT_MASK))==u32(0):
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148/149/150/151/152
      ...
      # 1bit 展開
      #----------------
      # 151/152: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
####################################################################################################
# 152 lazy next-fid lookup over the validated 151/150/149/148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 151/150/149/148; 152 changes only meta_next lookup timing inside kernel_dfs_iter_gpu
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 151/150/149/148 scorestripe_v9 task order/cache unchanged; 152 kernel experiment loads meta_next only at taken transitions"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  152 reuses the validated 151/150/149/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 152 experiment keeps the 151 frame-local path and changes only meta_next lookup timing inside the GPU kernel.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  152 host-side score-aware lanephase chunk shaping reused unchanged from validated 151/150/149/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 152 experiment changes only lazy meta_next lookup; the 148 host-shaping cache is reused unchanged.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split152_lazy_nextfid_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split152_lazy_nextfid_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split152-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split152-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split152_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split152-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split152-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split152_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
