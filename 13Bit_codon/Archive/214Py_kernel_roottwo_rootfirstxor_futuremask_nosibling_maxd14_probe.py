...
# =============================================================================
# NQ_UPDATE_MEMO
# 214: 更新メモ: 213Py rootfirstlate inlinexor は正当性OKだが204最速を上回らなかったため不採用。214Pyは211Pyの良好なlate-root-first形を親に、MAXD14 kernelのみroot_firstを cur_avail & -cur_avail ではなく cur_avail ^ root_rest で復元する微差実験。root_rest clear-lowbit / root_after_second XOR scalar / futuremask / no-sibling / root one-two preroll は維持する。
...
213Pyは197よりわずかに速いものの、204/211/212/210/206を上回らないため、**正当性OKだが不採用**です。

214Pyは、最も204に近かった211Pyの `root_first` 遅延形を親にし、MAXD14 kernel内だけの微差分にしました。
...
`root_rest` は `cur_avail` からlowbitを除いた値なので、`cur_avail ^ root_rest` は除去されたlowbit、つまり `root_first` と等価です。root fast-start対象は204/206/211と同じく1bit/2bit rootのみです。

手元確認は以下です。

```text
bash -n 214Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 214Py...sh OK
source_rootfirstxor                   OK
source_split_tag split145 split214    OK
```

ファイルはこちらです。

* [214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_probe.py)
* [214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_validate_full_once.sh

bash 214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 214 rootfirstxor memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  214 keeps the narrow root fast-start only when
...
VERSION_TAG:str="214 roottwo-rootfirstxor: 211 late-root-first shape with root_first restored as cur_avail xor root_rest inside root<=2 preroll - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # correct but slower.  214 keeps 204's clear-lowbit root_rest construction and restores
...
CHUNKSHAPE148_DEFAULT_REASON:str="214 rootfirstxor reuses validated 204 scorestripe_v9 task order/cache; MAXD14 restores root_first as cur_avail xor root_rest inside root<=2 predicate"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split214_roottwo_rootfirstxor_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split214_roottwo_rootfirstxor_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split214-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split214-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split214_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split214-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split214-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split214_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
