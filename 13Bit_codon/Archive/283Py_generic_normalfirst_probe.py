...
# =============================================================================
# NQ_UPDATE_MEMO
# 283: generic-normalfirst-reprobe。281 block count診断でgeneric loopは
#      normal_block_count が special_block_count より大きいことを確認したため、
#      282復帰版を親に、MAXD14 generic DFS loopだけを `block_code==0` の
#      normal path先行へ入れ替える最小速度候補。root-prerollの分岐順序、
#      rootrestlate、futuremask、no-sibling、split145/chunkshape148、broadmarktail、
#      cache生成、worker split、mode28/29/30/31、MAXD fallback、CPU dfs_iter pathは
#      282/279/276相当で維持する。227 generic normal-first は当時不採用だったが、
#      281診断結果を受け、現行276/279系で再確認する。
...
282復帰版と283 normal-first再確認版を作成しました。

282は **281診断撤回 → 279/276本線へ戻す安全復帰版**です。
283は **282を親に、MAXD14 generic loopだけ `block_code==0` normal path先行へ入れ替える最小速度候補**です。root-preroll側の分岐順序は触っていません。

どちらも `STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

## 282

* [282Py_restore279_after_blockdiag_reject_probe.py](sandbox:/mnt/data/282Py_restore279_after_blockdiag_reject_probe.py)
* [282Py_restore279_after_blockdiag_reject_validate_N21_full_once.sh](sandbox:/mnt/data/282Py_restore279_after_blockdiag_reject_validate_N21_full_once.sh)
* [282Py_restore279_after_blockdiag_reject_sha256.txt](sandbox:/mnt/data/282Py_restore279_after_blockdiag_reject_sha256.txt)
* [282Py_restore279_after_blockdiag_reject_bundle_20260709.tar](sandbox:/mnt/data/282Py_restore279_after_blockdiag_reject_bundle_20260709.tar)

```bash
rm -f ./282Py_restore279_after_blockdiag_reject_probe

STATIC_ONLY=1 bash 282Py_restore279_after_blockdiag_reject_validate_N21_full_once.sh

bash 282Py_restore279_after_blockdiag_reject_validate_N21_full_once.sh
```

## 283

* [283Py_generic_normalfirst_probe.py](sandbox:/mnt/data/283Py_generic_normalfirst_probe.py)
* [283Py_generic_normalfirst_validate_N21_full_once.sh](sandbox:/mnt/data/283Py_generic_normalfirst_validate_N21_full_once.sh)
* [283Py_generic_normalfirst_sha256.txt](sandbox:/mnt/data/283Py_generic_normalfirst_sha256.txt)
* [283Py_generic_normalfirst_bundle_20260709.tar](sandbox:/mnt/data/283Py_generic_normalfirst_bundle_20260709.tar)

```bash
rm -f ./283Py_generic_normalfirst_probe

STATIC_ONLY=1 bash 283Py_generic_normalfirst_validate_N21_full_once.sh

bash 283Py_generic_normalfirst_validate_N21_full_once.sh
```
...
実行順は、まず282で `0:07:07.7xx` 近辺へ戻ることを確認し、その後に283を回すのが安全です。283の採否は、276 `0:07:07.672`、279 `0:07:07.728`、271 `0:07:07.705`、過去227 `0:07:07.808` と比較して判断します。
...
VERSION_TAG:str="283 generic-normalfirst-reprobe: parent 282 restore279 baseline; MAXD14 generic DFS loop only changes block_code branch from special-first to normal-first based on 281 block count diagnostics; root-preroll branch order and all host shaping/cache/dispatch features preserved"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split283_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split283_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split283-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split283-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split283_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
