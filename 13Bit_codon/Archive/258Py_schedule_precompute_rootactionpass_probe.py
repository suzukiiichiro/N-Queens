...
# =============================================================================
# NQ_UPDATE_MEMO
# 258: schedule-precompute-rootaction-pass probe。257 root_action low-bit
#      decode が CUDA_ERROR_INVALID_PTX になったため、同じ sched_root_action_arr
#      を使いつつ kernel 側の `&u32(3)` decode を外して pass-through する切り分け版。
#      256 base14pack JIT成立、244 r4 maskpack JIT成立を親に、単一launchを維持する。
#      child/future maskpack、terminal_base14単独配列、terminal_depth別配列は維持し、
#      root_action は sched_root_action_arr[i] をそのまま使う。
#      240 fid14/rest別launch、242 futuremask-depthbit微差、255 actionpack、257 root_action decode は入れない。
...
"""
258 schedule-precompute note: 257 root_action low-bit decode が CUDA_ERROR_INVALID_PTX になったため、同じ sched_root_action_arr を使いつつ kernel側では sched_root_action_arr[i] をそのまま root_action として使う pass-through probe。


257は、正しい引数でも `CUDA_ERROR_INVALID_PTX` なので、**root_action単独の kernel側 low-bit decode がJIT不成立源**と見て切り分けを進めます。
...
256では `root_action_arr[i]` の素通しはJIT成立済みなので、258では同じ `sched_root_action_arr` を使いながら、kernel側の `&u32(3)` だけを外しました。

```python
root_action:u32 = sched_root_action_arr[i]
```

host precompute側では `0..3` に収めているので、意味は変えない想定です。
...
source_schedule_precompute_rootactionpass  OK
source_host_precompute_rootactionpass_arrays OK
release_build_policy                       OK
```

ファイルはこちらです。

* [258Py_schedule_precompute_rootactionpass_probe.py](sandbox:/mnt/data/258Py_schedule_precompute_rootactionpass_probe.py)
* [258Py_schedule_precompute_rootactionpass_validate_N21_full_once.sh](sandbox:/mnt/data/258Py_schedule_precompute_rootactionpass_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [258Py_schedule_precompute_rootactionpass_sha256.txt](sandbox:/mnt/data/258Py_schedule_precompute_rootactionpass_sha256.txt)
* [258Py_schedule_precompute_rootactionpass_bundle_20260707.tar](sandbox:/mnt/data/258Py_schedule_precompute_rootactionpass_bundle_20260707.tar)

sha256です。

```text
d26dc3137f265a2331991127e2a0e2a48b47bb9024412fcf68dc66ad61a7a338  258Py_schedule_precompute_rootactionpass_probe.py
b37e30a13273c4b1491663b69ca0d4e5fee31af058df7e665f0a22e0fb29bb51  258Py_schedule_precompute_rootactionpass_validate_N21_full_once.sh
b9248006883fd36946cdfb88a8b0e39f0b18f4a73d12aeeafa149dc4113ceda5  README.md
```

実行は以下でお願いします。

```bash
rm -f ./258Py_schedule_precompute_rootactionpass_probe

STATIC_ONLY=1 bash 258Py_schedule_precompute_rootactionpass_validate_N21_full_once.sh

bash 258Py_schedule_precompute_rootactionpass_validate_N21_full_once.sh
```

258がJIT成立すれば、257の不成立源は `root_action` 配列ではなく、kernel内の `&u32(3)` decode形にほぼ絞れます。
...
VERSION_TAG:str="258 schedule-precompute-rootactionpass probe: parent 256 base14pack JIT-ok / 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; child/future maskpack retained; terminal_base14 separate; root_action loaded from sched_root_action_arr without kernel-side & mask to isolate 257 CUDA_ERROR_INVALID_PTX; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 257: 256 rootactionpass JIT-ok shape plus root_action-only decode
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 258: schedule fields are precomputed on the host in build_soa_for_range().
    # child/future masks are packed; terminal_base14 remains separate; root_action
    # is loaded from sched_root_action_arr without kernel-side bitmask.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split258_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split258_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      progress_suffix:str=f"split258_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
