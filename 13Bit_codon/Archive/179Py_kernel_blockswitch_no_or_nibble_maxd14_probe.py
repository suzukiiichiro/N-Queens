...
178Pyの2本目も **CUDA JIT段階の `CUDA_ERROR_INVALID_PTX`** なので、178Pyは不採用にします。

今回の178Pyで一番怪しい差分は、MAXD14 kernel内のこの条件です。

```text
elif block_code==u32(2) or block_code==u32(5):
```

Codon/CUDAのGPU kernel内で、`u32`比較をPython/Codonの `or` でつないだ形がPTX生成上かなり危険そうです。
そこで次は **179Py** として、175PyのJIT済み構造を維持しつつ、178Pyのblock switchを **no-or形式** に直した版を作りました。

ファイルはこちらです。

* [179Py_kernel_blockswitch_no_or_nibble_maxd14_probe.py](sandbox:/mnt/data/179Py_kernel_blockswitch_no_or_nibble_maxd14_probe.py)
* [179Py_kernel_blockswitch_no_or_nibble_maxd14_validate_full_once.sh](sandbox:/mnt/data/179Py_kernel_blockswitch_no_or_nibble_maxd14_validate_full_once.sh)
...
撤回:
  旧178 host-precomputed schedule
  旧178 block-switch の `or` 条件

179Py:
  175Pyの4-bit nibble scheduleを維持
  current frame registerを維持
  ancestor13 stackを維持
  schedule_words=0
  stack_bytes_per_thread=208

変更:
  block_code 2/5 の共通処理を `or` でまとめず、
  個別の elif に分離
```
...
maxd14_no_or_blockswitch_nibble_schedule      1 / 1 OK
block_opcode_mapping                         19 cases OK
nibble_opcode_classes                        7 classes OK
frame_opcode_equivalence                     506604 cases OK
dfs_164_vs_179_no_or_blockswitch_nibble_equivalence 12000 cases OK
...
```bash
chmod +x 179Py_kernel_blockswitch_no_or_nibble_maxd14_validate_full_once.sh
./179Py_kernel_blockswitch_no_or_nibble_maxd14_validate_full_once.sh
```

この179PyでもJITが落ちる場合は、block-switch系そのものを捨てて、最速基準を **175Py** に戻し、次はkernel hot loopではなく **task配置・chunk tail側** に進むのが安全です。


"""

# 179 GPU PTX-safe no-or block-switch nibble experiment over validated 175 MAXD14:
...
#
#   179 keeps the fastest 175 layout:
...
#   block metadata with several mask shifts from block_code.  179 instead uses
...
# 179 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 179 MAXD14 uses scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="179 PTX-safe no-or block-switch nibble schedule for MAXD14 over validated 175 nibble baseline - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 179 inherits the exact 175/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """179 MAXD14 nibble schedule with PTX-safe no-or direct block-code switch plus register-held current frame and 13-slot ancestor stack."""
    ...
    # 179 keeps the 175 layout but replaces block-code mask decoding in the
    # MAXD14 hot loop with direct constant cases for codes 1..5.  The
    # code2/code5 case is intentionally written without Python/Codon `or`,
    # because the previous 178 block-switch variant JIT-failed in CUDA PTX.
    ...
    # 179 keeps the validated 175 current-frame register cache plus ancestor stack shrink.  The active
    ...
    while True:
      ...
      nld:u32=u32(0)
      nrd:u32=u32(0)
      # 179: MAXD14 block-code switch, PTX-safe no-or form.  175 decoded
      # block metadata with several bit-mask shifts for every block candidate.
      # The nibble already carries the direct block code, so use constant cases
      # in the hot loop.  Code 2 and code 5 are arithmetically identical, but
      # they are deliberately kept as two separate elif cases to avoid the
      # Codon/CUDA invalid-PTX pattern seen with `elif a==2 or a==5`.
      if block_code==u32(0):
...
####################################################################################################
# 179 PTX-safe no-or block-switch nibble MAXD14 + static dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 175 scorestripe_v9 task order/cache and MAXD14 nibble schedule; 179 switches block-code decode to PTX-safe no-or direct constants"
...
def main()->None:
  ...
  # 179 branches from validated 175.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 179 no-or block-switch nibble GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split179_no_or_blockswitch_nibble_schedule_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split179_no_or_blockswitch_nibble_schedule_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split179-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split179-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split179_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split179-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split179-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split179_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
