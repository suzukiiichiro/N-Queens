...
# =============================================================================
# NQ_UPDATE_MEMO
# 208: 更新メモ: 207Py rootprdescendnfは正当性OKながら204/206/205/197より遅いため不採用。208Pyは206/204相当へ戻し、MAXD14 kernelのみroot-preroll内のterminal_depth==0判定をfuture_check前へ移動する微差実験。root_after_second scalar / root_rest clear-lowbit / futuremask / no-sibling / root one-two preroll は維持する。
...
したがって、207Pyは**正当性OKだが不採用**です。

208Pyは、207の `pr_descend:u32=pr_nf` を採用せず、206/204相当へ戻したうえで、MAXD14 kernel内だけの小差分にしました。
...
# 208の追加差分
# root-preroll内だけ terminal_depth==0 判定を future_check_mask guard より前へ移動
```

generic DFS loopのterminal/future順序は変更していません。193Pyのような広い terminal-before-future ではなく、root-preroll内だけに限定しています。

手元確認は以下です。

```text
bash -n 208Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 208Py...sh OK
source_prterminalfirst                OK
source_split_tag split145 split208    OK
```

ファイルはこちらです。

* [208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_probe.py)
* [208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_validate_full_once.sh

bash 208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 208 restore_rootafter_scalar memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  208 keeps the narrow root fast-start only when
...
# 208 keeps the validated 204/197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 208 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="208 roottwo-restore_rootafter_scalar: 204 rootclearrest futuremask/no-sibling MAXD14 baseline with root_after_second scalar predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """208 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate restored root_after_second predicate."""
    ...
    # correct but slower.  208 keeps 204's clear-lowbit root_rest construction and restores
    ...
    if root_after_second==u32(0):
      ...
      if pr_nf==u32(0):
        pr_descend=u32(0)
      # 208 prterminalfirst:
      # For root-preroll only, handle terminal child depth before the
      # future-prune guard. In the validated schedule model terminal children
      # do not need the future check; generic loop ordering is unchanged.
      if pr_descend!=u32(0):
        if terminal_depth==0:
...
CHUNKSHAPE148_DEFAULT_REASON:str="208 restore_rootafter_scalar reuses validated 204 scorestripe_v9 task order/cache; MAXD14 root_after_second scalar shape restored after 205 was not adopted"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split208_roottwo_prterminalfirst_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split208_roottwo_prterminalfirst_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split208-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split208-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split208_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split208-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split208-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split208_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
