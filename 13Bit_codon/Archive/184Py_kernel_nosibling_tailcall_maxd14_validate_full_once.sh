...
# =============================================================================
# NQ_UPDATE_MEMO
# 184: 更新メモ: 183のN=21 full検証OK・0:07:51.091を受け、runtime-only検証を継承。MAXD14 kernelのみ no-sibling spill elision / tail-call descent を単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 184 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 184 starts from validated 183 and changes only the MAXD14 descend spill policy after the 178/179
# block-switch experiments produced CUDA_ERROR_INVALID_PTX at PTX JIT time.
#
# Kernel arithmetic keeps 183 bitboard/schedule semantics: register-held current frame,
# 13-slot ancestor stack, scalar 4-bit nibble schedule, schedule_words=0,
# and 208 local stack bytes/thread. The experiment is deliberately focused: skip ancestor spill when the parent has no remaining siblings, keep
# update history in README.md, and retain tail-flatness metrics derived from the
# existing progress TSV. The long source/model assertion block remains disabled.
# GPU task order and dispatch remain the validated 183 path.
SRC=${SRC:-./184Py_kernel_nosibling_tailcall_maxd14_probe.py}
CAND=${CAND:-./184Py_kernel_nosibling_tailcall_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/184Py_kernel_nosibling_tailcall_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 184 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/184Py_kernel_nosibling_tailcall_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 184Py source
#   has a harmless header/string/layout difference.  This 184 harness validates the
...
printf 'source_parent_baseline\tvalidated 183 nibble MAXD14 dispatch + no-sibling spill-elision experiment\truntime-only validation\tINFO\n' >> "$SUMMARY"
printf 'kernel_shape_expectation\tMAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision\truntime dispatch/progress checked\tINFO\n' >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 184 runtime-only harness disabled source/model assertions"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 184 no-sibling spill-elision kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 184 keeps the validated host/chunk structure, then derives chunk-tail diagnostics from
...
# 184: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 184 no-sibling tailcall reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
