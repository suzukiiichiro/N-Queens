...
Python/codon Ｎクイーン コンステレーション版 CUDA 高速ソルバ（ctrlrow packed MAXD28 tuned）
...
def kernel_dfs_iter_gpu(
    ...
    # u32 ビットボード系。
    # 33版: fid[MAXD] と row[MAXD] を ctrl[int] に吸収する。
    ...
    mark2=mark2_arr[i]
    sp=0
    ctrl[0]=funcid_arr[i] | (row_arr[i]<<5)
    ld[0]=ld_arr[i]
    rd[0]=rd_arr[i]
    col[0]=col_arr[i]
    ...
    while sp>=0:
      ...
      if (cv0&INIT_MASK)==0:
        ...
        if use_blocks==1:
          ...
          if ((MASK_K_N4>>f)&1)==1:
            ktype=2
        ctrl[sp]=INIT_MASK | nextfv | (stepv<<5) | (addv<<7) | (use_blocks<<8) | (fcv<<9) | (bLv<<10) | (ktype<<12) | (rowv<<14)
      #----------------
      # 1bit 展開
      #----------------
      a=avail[sp]
      ...
      if ((cv>>8)&1)==1:
        ...
        if kt==1:
          ...
          bK=n4
        nld=((ld[sp]|bit)<<stepv)|addv|bL
        nrd=((rd[sp]|bit)>>stepv)|bK
      else:
        nld=(ld[sp]|bit)<<1
        nrd=(rd[sp]|bit)>>1
      ncol=col[sp]|bit
      ...
      # push
      #----------------
      next_row=((cv>>14)&31)+stepv
      sp+=1
      if sp>=MAXD:
        results[i]=total*w_arr[i]
        return
      ctrl[sp]=(cv&31) | (next_row<<5)
      ld[sp]=nld
      rd[sp]=nrd
      col[sp]=ncol
      avail[sp]=nf
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=256,gpu_max_blocks:int=32,gpu_log_level:int=0)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 maxd28=1 block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
