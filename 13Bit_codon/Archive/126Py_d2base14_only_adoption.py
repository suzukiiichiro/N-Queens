...
おはようございます。実行結果、確認しました。

**結論として、125 は「正しいが速度採用は見送り」です。**
3 chunk は `partial_total = 7287470230` で一致していますが、時間は `0:03:13.522`。7 chunk も `partial_total = 16337097758` で一致していますが、時間は `0:07:34.078` でした。

比較対象では、124 が 3 chunk `0:01:29.805`、7 chunk `0:03:24.788`、123 の 7 chunk が `0:03:19.346` なので、125 は 123/124 よりかなり遅くなっています。

原因はほぼ **d0 を special kernel 側に入れたこと**だと思います。添付ソース上も 125 は `fid14, fid26, fid27` を同じ special bucket に入れる設計で、ログでは各 chunk の special の大半が d0 です。たとえば chunk 40 は `special=2261` のうち `d2base14=15`、chunk 120 は `special=2351` のうち `d2base14=104` で、ほとんどが d0 です。

そのため、次は **d0 を 115 汎用 kernel に戻し、119 相当の fid14 / SQd2B だけを専用化する 126** に戻すのがよいです。作成しました。

[126Py_d2base14_only_adoption.py](sandbox:/mnt/data/126Py_d2base14_only_adoption.py)
[126_next_probe_memo.txt](sandbox:/mnt/data/126_next_probe_memo.txt)

実行はまずこちらでお願いします。

```bash
codon build -release 126Py_d2base14_only_adoption.py
```

3 chunk probe:

```bash
./126Py_d2base14_only_adoption \
...
```text
[split126-buckets] N=21 m=15488 special_d2base14=... d0_rest=... rest=...
```

3 chunk が通ったら 7 chunk です。
...
```text
PTX 通過 + total 一致 + 123/124 より速い
  → 126 を d2base14-only 採用ベースにする

total 一致だが遅い
  → 126 も採用見送り。115/119 の既存ベースへ戻す

total 不一致
  → fid14 の終端条件 free & ~1 を再確認
```

こちらでは Codon/CUDA 実機ビルドまでは実行していません。変更内容は 125 からの最小差分で、**special 対象を fid14 のみに戻し、fid26/fid27 は rest 側へ戻す**ものです。

確認しました。**N21 軽量版は全項目 PASS です。**
`CAND=./126Py_d2base14_only_adoption` の mode30 / mode31 と、`BASE=./115Py_range_default_clean_cg_v2` の mode28 は、すべて期待値一致・exit code 0 で終わっています。

| case                   | 判定 |                                                                                     実測 |
| ---------------------- | -: | -------------------------------------------------------------------------------------: |
| #1 standard 7-chunk    | OK |                                             `partial_total=16337097758`, `0:01:23.737` |
| #2 heavy-band          | OK |                                             `partial_total=22253483640`, `0:01:36.945` |
| #3 d2base14-density    | OK |                                             `partial_total=16871798494`, `0:01:24.924` |
| #4 late/tail           | OK |                                             `partial_total=16202368694`, `0:01:20.794` |
| #5 CAND worker 0/4     | OK |                                             `partial_total=79322573818`, `0:06:36.423` |
| #6 BASE reorder mode28 | OK | `records=2025282`, `reordered_records=2025282`, `chunks=131`, `valid=1`, `0:00:11.391` |

#1 は 7 chunk で `total=16337097758`、`partial_total=16337097758`、`0:01:23.737`、exit code 0 です。
#2 は heavy-band 8 chunk で `partial_total=22253483640`、`0:01:36.945`、exit code 0 です。
#3 は d2base14-density 7 chunk で `partial_total=16871798494`、`0:01:24.924`、exit code 0 です。
#4 は late/tail 7 chunk で `partial_total=16202368694`、`0:01:20.794`、exit code 0 です。chunk 130 も `m=11842` の short chunk として処理されています。
#5 は candidate の mode31 worker 0/4 が `partial_total=79322573818`、期待値 `79322573818` と一致し、`expected_total=314666222712` も表示されています。
#6 は reorder build/reuse が `records=2025282`、`reordered_records=2025282`、`chunks=131`、`valid=1` で通っています。
...
ログの開始・終了時刻ベースでも、`06:56:10` 開始から `07:08:46` 終了なので、壁時計で約 **12分36秒** です。軽量版の所要時間見積もりとして使ってよいです。

重要な点として、#3 の d2base14-density では意図どおり `special_d2base14` が濃い chunk を踏めています。chunk 55 が 217、chunk 56 が 294、chunk 57 が 330、chunk 58 が 374、chunk 60 が 310 です。
つまり、この検証メニューは **fid14 / SQd2B の変更を短時間で踏む**目的にちゃんと使えます。

判断はこうです。

```text
N21 light validation menu:
  PASS
  → 今後の candidate 一次検査として採用してよい

126:
  correctness probe としては継続 OK
  ただし N21 full で 115 より遅かったため、
  default 採用見送り判断は変更なし

次の candidate:
  まず今回の軽量版 #1〜#4
  ↓
  全部 OK なら #5 worker 0/4
  ↓
  selected chunk で明確に速い場合だけ N21 full
```

記録用メモも作成しました。

[131_N21_light_validation_result_memo.txt](sandbox:/mnt/data/131_N21_light_validation_result_memo.txt)
...
VERSION_TAG:str="126 d2base14-only adoption - fid14 SQd2B specialized, d0 stays on proven 115 generic kernel"
...
"""126: fid14 / SQd2B だけを専用 kernel に流し、d0(fid26/27) は 115 汎用 kernel に戻す。"""
def exec_solutions_gpu_chunk_split126(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  """
  126 d2base14-only adoption split:
    - bucket only funcid 14 / SQd2B
    - run that bucket through the small 119-style specialized kernel
    - run all other tasks, including d0 fid26/27, through the proven 115 generic kernel

  125 proved that the compact fid14+fid26+fid27 kernel is correct, but the N21 probe became much
  slower because d0 dominates the special bucket.  This build intentionally returns d0 to the generic
  path and keeps only the previously best d2base14 minimum kernel.
  """
  board_mask:int=(1<<N)-1
  ...
  if gpu_log_level>=1:
    print(f"[split126-buckets] N={N} m={m} special_d2base14={special_m} d0_rest={d0_m} rest={rest_m} split_mode={split_mode} specialized={1 if use_specialized else 0}")
...
def exec_solutions_gpu_bin_stream_split126(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split126_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split126_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split126_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split126_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split126-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split126-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split126-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split126-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split126(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split126-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split126-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split126 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split126 full run / optional multi-GPU worker
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 30 8 7 0 1222 5 "" 2  # 126 d2base14-only probe chunks 1222..1226
      #   -g 22 22 32 484 1 0 7 31 8 7 0 0 1 2  # 126 d2base14-only full GPU single worker
      if argc >= 11:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split126-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split126 worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split126_d2base14_only_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split126_d2base14_only_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split126-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split126-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split126_probe_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split126(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split126-probe-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split126-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split126-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split126-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split126_full_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split126(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split126-full-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
