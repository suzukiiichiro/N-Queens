...
165Pyは正解性に問題ありませんが、`11:07.471`で164Pyより`33.945秒（5.358%）`低速でした。164Pyを親に166Pyを作成しました。

* [166Py_kernel_terminal_parent_batch_probe.py](sandbox:/mnt/data/166Py_kernel_terminal_parent_batch_probe.py)
* [166Py_kernel_terminal_parent_batch_validate_full_once.sh](sandbox:/mnt/data/166Py_kernel_terminal_parent_batch_validate_full_once.sh)

166Pyはterminal直前frameの候補をレジスタ内ループで一括処理します。静的検査、164対166のDFSモデル25,000ケース、検証シェルの模擬131チャンク再生は通過済みです。Codon/CUDA実機実行は未実施です。


"""

# 166 GPU terminal-parent batch experiment:
#   Branch from the fastest fully validated 164 child-frame push-initialization
#   baseline; the slower 165 ctrl-schedule experiment is intentionally not
#   inherited.  Retain 164's u32 stack layout, pre-push frame initialization,
#   root-pop termination, absent overflow guard, host/order/cache pipeline,
#   bitboard arithmetic, DFS order, and solution arithmetic.
#   The sole kernel experiment uses ctrl bits19..20 to encode whether the next
#   child is an ordinary base or the fid14 special base.  A frame immediately
#   preceding a base consumes all remaining candidate bits in a register-local
#   inner loop, then pops once.  This removes one outer DFS-loop re-entry and
#   child-base classification per candidate while leaving nonterminal pushes
#   unchanged.  The five u32[MAXD] arrays and N=5..27 safety envelope remain
#   exactly as in validated 164.
...
# MAXD=21 exposes indices 0..20 and retains zero spare frames.  166 branches
# from validated 164 and inherits 161's guard removal, so GPU execution remains
# limited to this proven range.
MAXD:Static[int]=21

VERSION_TAG:str="166 terminal parent batch over 164 child frame push initialization - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    引数（抜粋）:
      ...
      ctrl0_arr:
        ...
        166でもroot初期化専用で、raw値をctrl stackへ保存しない。
      markctrl_arr:
    ...
    前提/不変条件:
      ...
      - ctrl stack上の全frameは常に初期化済みmetadataである。
      - ctrl bits19..20は、そのframeが生成するchildのterminal kindを保持する。
      - terminal kind!=0のframeではfuture bit14は必ず0である。
      - 166は最速の検証済み164を親版とし、terminal直前frameだけを一括処理する。
    ホットパス（ソース引用）:
      bit = a & -a
      avail[sp] = a ^ bit
    """
    # GPUではlist/tuple参照を避け、function分類を32bit maskとして焼き込む。
    # 166は164の初期化済みctrlへterminal kindを追加し、base直前frameを一括処理する。
    ...
    # 145: MAXD=21、154以降: u32入力/stack、155: ctrl0 prepack、157: markctrl prepack。
    # 158/159: function分類とmask演算をu32化、160: root-pop終了、161: 到達不能guard除去。
    # 162: mark成立箇所でblockL/blockK metadataを直接生成。
    # 164: rootとchildをstack保存前に初期化。
    # 166: 同じctrlへchild terminal kindを追加し、terminal直前frameを一括展開する。
    ...
    #   bits 19..20 : child terminal kind: 0=nonterminal, 1=ordinary base, 2=fid14 base
    # low 10 bits remain the raw identity of the child frame, so ordinary child initialization starts from cv&1023.
    # P5 targets are never base fids in the fixed 28-function table; terminal kind can therefore be derived from nextfid/child row directly.
    # GPUは161由来のN=5..27制限を維持する。N>27ではMAXD境界を再証明すること。
    ...
    root_raw:u32=ctrl0_arr[i]
    root_fu:u32=root_raw&u32(31)
    root_rowv:u32=(root_raw>>u32(5))&u32(31)

    # P5 is a same-row function transition.  Initialize the transitioned frame
    # directly; no raw ctrl value is ever stored in the stack.
    if ((IS_P5_MASK>>root_fu)&u32(1))!=u32(0):
      if root_rowv==mark1:
        root_fu=u32(meta_next[int(root_fu)])
    ...
    root_stepv:u32=u32(1)
    root_addu:u32=u32(0)
    root_blocksu:u32=u32(0)
    root_bLiu:u32=u32(0)
    root_ktu:u32=u32(0)
    root_use_futureu:u32=u32(1)-root_ismu
    root_nextfidu:u32=root_fu

    if root_ismu!=u32(0):
      root_markv:u32=mark2 if ((SEL2_MASK>>root_fu)&u32(1))!=u32(0) else mark1
      if root_rowv==root_markv:
        root_blocksu=u32(1)
        root_bLiu=(
          ((MASK_L_1>>root_fu)&u32(1))
          |(((MASK_L_2>>root_fu)&u32(1))<<u32(1))
        )
        root_ktu=(
          ((MASK_K_N3>>root_fu)&u32(1))
          |(((MASK_K_N4>>root_fu)&u32(1))<<u32(1))
        )
        root_use_futureu=u32(0)
        root_stepv=u32(3) if ((STP3_MASK>>root_fu)&u32(1))!=u32(0) else u32(2)
        root_addu=u32(1) if root_fu==u32(20) else u32(0)
        root_nextfidu=u32(meta_next[int(root_fu)])
    ...
    if ((IS_BASE_MASK>>root_nextfidu)&u32(1))!=u32(0) and root_child_rowu==endm:
      root_terminal_kind=u32(2) if root_nextfidu==u32(14) else u32(1)
    root_cv:u32=(
      root_nextfidu
      |(root_child_rowu<<u32(5))
      |(root_stepv<<u32(10))
      |(root_addu<<u32(12))
      |(root_blocksu<<u32(13))
      |(root_fcvu<<u32(14))
    ...
    while True:
      ...
      # Every stack entry is initialized before it is stored; no initialization test.
      cv:u32=ctrl[sp]

      # A terminal-parent frame has no recursive child to push.  Consume every
      # remaining candidate in registers, count the base child, and pop once.
      ...
      if terminal_kind!=u32(0):
        ...
        if (cv&u32(8192))!=u32(0):
          terminal_stepu:u32=(cv>>u32(10))&u32(3)
          terminal_addvu:u32=(cv>>u32(12))&u32(1)
          terminal_bLiu:u32=(cv>>u32(15))&u32(3)
          terminal_ktu:u32=(cv>>u32(17))&u32(3)
          terminal_bKu:u32=(n3&(u32(0)-(terminal_ktu&u32(1))))|(n4&(u32(0)-(terminal_ktu>>u32(1))))
          while a!=u32(0):
            terminal_bit:u32=a&(u32(0)-a)
            a=a^terminal_bit
            terminal_nld:u32=((terminal_ld|terminal_bit)<<terminal_stepu)|terminal_addvu|terminal_bLiu
            terminal_nrd:u32=((terminal_rd|terminal_bit)>>terminal_stepu)|terminal_bKu
            terminal_nf:u32=bm&~(terminal_nld|terminal_nrd|(terminal_col|terminal_bit))
          ...
          while a!=u32(0):
            terminal_bit:u32=a&(u32(0)-a)
            a=a^terminal_bit
            terminal_nld:u32=(terminal_ld|terminal_bit)<<u32(1)
            terminal_nrd:u32=(terminal_rd|terminal_bit)>>u32(1)
            terminal_nf:u32=bm&~(terminal_nld|terminal_nrd|(terminal_col|terminal_bit))
            if (terminal_nf&~terminal_exclude)!=u32(0):
              total+=u64(1)
        if sp==0:
      ...
      # ----------------------------------------------------------------
      # child frame initialization before push
      # ----------------------------------------------------------------
      ...
      if ((IS_P5_MASK>>child_fu)&u32(1))!=u32(0):
        if child_rowv==mark1:
          child_fu=u32(meta_next[int(child_fu)])

      # Base children were encoded in the parent ctrl and consumed by the
      # terminal batch above, so the ordinary path cannot enter a base frame.
      child_ismu:u32=(IS_MARK_MASK>>child_fu)&u32(1)
      child_stepv:u32=u32(1)
      child_addu:u32=u32(0)
      child_blocksu:u32=u32(0)
      child_bLiu:u32=u32(0)
      child_ktu:u32=u32(0)
      child_use_futureu:u32=u32(1)-child_ismu
      child_nextfidu:u32=child_fu

      if child_ismu!=u32(0):
        child_markv:u32=mark2 if ((SEL2_MASK>>child_fu)&u32(1))!=u32(0) else mark1
        if child_rowv==child_markv:
          child_blocksu=u32(1)
          child_bLiu=(
            ((MASK_L_1>>child_fu)&u32(1))
            |(((MASK_L_2>>child_fu)&u32(1))<<u32(1))
          )
          child_ktu=(
            ((MASK_K_N3>>child_fu)&u32(1))
            |(((MASK_K_N4>>child_fu)&u32(1))<<u32(1))
          )
          child_use_futureu=u32(0)
          child_stepv=u32(3) if ((STP3_MASK>>child_fu)&u32(1))!=u32(0) else u32(2)
          child_addu=u32(1) if child_fu==u32(20) else u32(0)
          child_nextfidu=u32(meta_next[int(child_fu)])
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164/166: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 166 terminal-parent batching over validated 164 and the 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 164; 166 changes only terminal-parent GPU execution
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 164 scorestripe_v9 task order/cache unchanged; 166 batches terminal-parent candidates"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  166 reuses the validated 164/162/161/160/159/158/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 166 experiment changes only terminal-parent GPU execution.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  166 host-side score-aware lanephase chunk shaping is reused unchanged from validated 164/162/161/160/159/158/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 166 experiment changes only terminal-parent GPU execution; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/166: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 166 branches from validated 164 and inherits the 161 safety envelope: the removed
  # kernel guard was proven unreachable only for dynamic-preset N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 166 terminal-parent-batch GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split166_terminal_parent_batch_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split166_terminal_parent_batch_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split166-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split166-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split166_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split166-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split166-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split166_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
