...
# 133 validation harness
# - Candidate: 133Py_scorestripe_chunkshape_probe
# - Kernel path remains generic-only; 133 adds scorestripe_v3_lanephase host-side bucket-internal lane rotation over the validated 132 carry-forward.
# - The selected-chunk partial totals are intentionally not hard-coded for the first run,
#   because scorestripe_v3_lanephase changes which records belong to each chunk.
# - Full correctness is checked in case 6.

BASE=${BASE:-./115Py_range_default_clean_cg_v2}
CAND=${CAND:-./133Py_scorestripe_chunkshape_probe}
RUN_FULL=${RUN_FULL:-1}

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="133Py_logs_N21_validation_${TS}"
...
# ================================================================
# 1) 133 selected-chunk smoke: standard spread
# chunkshape133 scorestripe_v3_lanephase changes chunk composition, so only partial_total emission is checked.
# ================================================================
run_case \
  "01_cand133_mode30_standard_7chunk" \
  "partial_total=" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "0,20,40,60,80,100,120"

# ================================================================
# 2) 133 selected-chunk stress: former heavy band
# ================================================================
run_case \
  "02_cand133_mode30_heavy_band" \
  "partial_total=" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "35,40,41,42,47,48,52,53"

# ================================================================
# 3) 133 selected-chunk stress: former d2base14-density band
# ================================================================
run_case \
  "03_cand133_mode30_d2base14_density" \
  "partial_total=" \
  "$CAND" \
    -g 21 21 32 484 1 0 7 30 8 7 0 0 1 "20,40,55,56,57,58,60"

# ================================================================
# 4) 133 selected-chunk stress: late/tail band
# ================================================================
run_case \
  "04_cand133_mode30_late_tail" \
  "partial_total=" \
  "$CAND" \
...
# ================================================================
run_case \
  "05_cand133_mode31_worker0of4_quick" \
...
if [ "${RUN_FULL}" = "1" ]; then
  run_case \
    "06_cand133_mode31_full_correctness" \
    "314666222712.*ok" \
    "$CAND" \
