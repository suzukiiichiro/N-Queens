...
# =============================================================================
# 338 N=21 full validation harness (maxd14-port-design)
# Base: 337/336/335/334/333/328-332 lineage (adopted SoA baseline + w3_j7
#   reorder default; kernel logic and dispatcher byte-for-byte identical
#   throughout). 338 makes NO source change to the main solver at all --
#   kernel, dispatcher, and host reorder parameters (w3_j7, K=48) are all
#   unchanged from 337/336/335/334/333. The only .py content change is an
#   Open Objectives update (#6 rewritten, #7 and #8 added) plus VERSION_TAG.
#
# 338 IS A DESIGN-ONLY REVISION: it adds NO code whatsoever, not even a
# standalone artifact. Its single deliverable is the specification
# document 338_maxd14_port_spec.md, which is prose and is never built,
# run, or checked by this harness. Same discipline as 324 (design) before
# 325 (implementation).
#
# 337 RESULTS (both confirmed, used as this run's expected-match baseline):
#   N=21 harness: correctness 314666222712, elapsed 450.056s (vs 336's
#   450.432s, +0.083%, within noise). All static checks OK.
#   Standalone bin reader (337_bin_format_reader.cu via
#   337_bin_format_reader_check.sh, host-only, no GPU): size_mod16_ok=True,
#   records_ok=True, records_read=2025282 (exactly EXPECTED_TASKS),
#   checksum_u64=13342728758502 -- the header-less 16-byte-record .bin
#   spec is now confirmed against real data, not only by source reading.
#
# WHAT 338 SPECIFIES (in 338_maxd14_port_spec.md, not here):
#   (1) the complete derivation from one 16-byte bin record to every SoA
#       array the kernel consumes (ld/rd/col/ctrl0/free/markctrl and w),
#       i.e. a C-language spec of build_soa_for_range (28-leaf funcid
#       decision tree) and symmetry();
#   (2) the C-side memory layout for K-batching (K=48, grid-stride with
#       stride = grid*block = 484*32 = 15488) and the per-thread stack
#       (__array__[u64](MAXD14_ANCESTOR*2) = 208 bytes/thread, with
#       stack_ptr / save_sp / cur_depth / next_depth);
#   (3) the N=18-only cross-check scope from rev84's original proposal.
#
# TWO NEW FINDINGS recorded while writing the spec (Open Objectives 7/8,
# both NOT acted on in 338):
#   #7 symmetry() returns only u64(2)/u64(4)/u64(8), so w_hi_arr is
#      identically zero for every task; and since markctrl only occupies
#      bits 0-19, w's exponent could be packed into bits 20-21, removing
#      both w arrays. Prologue/epilogue only -- does NOT touch the hot
#      loop that failed 5/5 times.
#   #8 a warp's 32 lanes process 32 CONSECUTIVE records per grid-stride
#      iteration, so 329's tail effect is intra-warp trip-count variance
#      and is addressable by host-side reordering alone; Sum(max over 32)
#      / Sum(mean over 32) is a directly measurable upper bound.
#
# MANDATORY VALIDATION ORDER (unchanged discipline):
#   1. STATIC_ONLY=1 first (no new gating source checks in 338; only the
#      source_version_tag string moves to 338).
#   2. Full N=21 run; confirm 314666222712 FIRST, before timing.
#   3. Compare timing against 337 (450.056s) -- expect reproduction
#      within noise, since nothing in the solver changed.
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./338Py_maxd14_port_design.py}
CAND=${CAND:-./338Py_maxd14_port_design}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/338Py_maxd14_port_design_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/338Py_maxd14_port_design_logs_N21_full_once_${TS}"
...
echo "[start] 338 maxd14-port-design validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 338 maxd14-port-design SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '338 maxd14-port-design' "$SRC"; then
  printf 'source_version_tag	338 maxd14-port-design	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	338 maxd14-port-design	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 338 maxd14-port-design (kernel-identical to 337/336/335/334/333/328) mode31 split145"
...
echo "[validation-ok] 338 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); NO solver source change in 338 (kernel/dispatcher/host params identical to 337/336/335/334/333/328-332, w3_j7 remains default); 338 is DESIGN-ONLY and adds no code at all -- its sole deliverable is the prose spec 338_maxd14_port_spec.md, which this harness neither builds nor runs; expected elapsed ~450.1s matching 337s own measurement 450.056s"
