...
174Pyログを確認しました。**正当性は問題ありません**。

```text
174Py : 0:08:21.387
172Py : 0:08:11.190
差分  : 174Pyが +10.197秒遅い、約2.076%低下
```
...
したがって、174Pyの `compact_op fast path` は**正当性OKだが速度面では不採用**とし、175Pyは最速基準の **172Pyへ戻して**作りました。

作成したファイルはこちらです。

* [175Py_kernel_nibble_schedule_maxd14_probe.py](sandbox:/mnt/data/175Py_kernel_nibble_schedule_maxd14_probe.py)
* [175Py_kernel_nibble_schedule_maxd14_validate_full_once.sh](sandbox:/mnt/data/175Py_kernel_nibble_schedule_maxd14_validate_full_once.sh)

175Pyの変更点です。

```text
親:
  172Py ancestor13 stack + current-frame register MAXD14

不採用:
  173Py frame-entry decode
  174Py compact_op fast path

175Py:
  MAXD14 scheduleを 3bit compact op から 4bit nibble op へ変更
...
175Py nibble op:
  bits0..2 : block_code, 0=normal, 1..5=block
  bit3     : future_check
...
schedule_hi:
  depth 8..13 の 6個 × 4bit
```

狙いは、174Pyで悪化した分岐形を避けつつ、172Py hot loop内の以下を軽くすることです。

```text
172Pyで必要だった処理:
  compact_op == 1
  compact_op >= 2
  block_code = compact_op - 1
  depth * 3 shift
  depth - 10 shift
...
dfs_164_vs_175_nibble_equivalence      12000 cases OK
...
```bash
chmod +x 175Py_kernel_nibble_schedule_maxd14_validate_full_once.sh
./175Py_kernel_nibble_schedule_maxd14_validate_full_once.sh
```

この175Pyが172Pyより速ければ、次はこのnibble scheduleを基準化できます。遅ければ、175Pyも不採用にして、172Py基準のまま **kernel内の小変更ではなくtask配置・split側**へ進むのがよいです。


"""

# 175 GPU nibble-schedule experiment over validated 172 ancestor13-stack MAXD14:
...
# 175 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 175 MAXD14 uses scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="175 nibble scalar schedule for MAXD14 over validated 172 ancestor13 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 175 inherits the exact 172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """175 MAXD14 nibble schedule plus register-held current frame and 13-slot ancestor stack."""
    ...
    # 175 keeps the validated 172 current-frame register cache plus ancestor stack shrink.  The active
...
####################################################################################################
# 175 nibble-schedule MAXD14 + scalar-u32pair dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 172 scorestripe_v9 task order/cache and ancestor13 MAXD14; 175 switches MAXD14 nibble schedule to 4-bit nibble op"
...
def main()->None:
  ...
  # 175 branches from validated 172.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 175 nibble-schedule GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split175_nibble_schedule_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split175_nibble_schedule_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split175-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split175-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split175_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split175-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split175-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split175_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
