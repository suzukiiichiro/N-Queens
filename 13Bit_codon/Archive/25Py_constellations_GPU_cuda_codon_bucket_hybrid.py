...
def exec_solutions(
  ...
  if use_gpu:
    ...
    GPU_LOG_BUCKETS:bool=True
    GPU_BUCKET_MODE:int=gpu_bucket_mode
    GPU_BUCKETS:int=gpu_bucket_count(GPU_BUCKET_MODE)
    print(f"[gpu-config] N={N} bucketized=1 split_row={GPU_SPLIT_ROW} bucket_mode={GPU_BUCKET_MODE} block={BLOCK} owner_blocks={MAX_BLOCKS} owner_steps={STEPS} task_blocks={GPU_TASK_BLOCKS} task_steps={GPU_TASK_STEPS}")
...
def main()->None:
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [N | N_start N_end] [split_row] [bucket_mode] [owner_blocks] [task_blocks]")
      return
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        exec_solutions(N,constellations,use_gpu,gpu_split_row,gpu_bucket_mode,gpu_owner_blocks,gpu_task_blocks)
