...
# =============================================================================
# NQ_UPDATE_MEMO
# 225: 更新メモ: 224Py generic clear-lowbit update は正当性OKだが大幅に低速化したため不採用。225Pyは225/223/217相当の generic cur_avail^bit 更新へ戻し、MAXD14 generic loopのみ ncol=cur_col|bit を nld/nrd 計算より前へ移動する微差実験。rootrestlate / root_after_second scalar predicate / root_first late extraction / futuremask / no-sibling / root one-two preroll は維持する。
...
したがって、224Pyの generic loop clear-lowbit update は**不採用**です。generic loopでは `cur_avail=cur_avail^bit` のままがよいです。

225Pyは224の差分を撤回し、223/217相当の `cur_avail^bit` へ戻したうえで、次の微差だけを試す版にしました。
...
`ncol` は `block_code` に依存しないため、意味は同じです。こちらでは `STATIC_ONLY=1` の静的確認までOKです。

```bash id="7dyh3b"
cd /home/suzuki/Github/N-Queens/13Bit_codon/Archive

STATIC_ONLY=1 bash 225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物です。

* [225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_probe.py)
* [225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_bundle_20260703.tar)

"""

# 225 generic-ncol-early memo over validated 217 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="225 generic-ncol-early: 222 MAXD13 diagnostics withdrawn; validated 217 rootrestlate scalar predicate restored - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while True:
      ...
      # 225 generic-ncol-early: ncol is independent of block_code and nld/nrd.
...
CHUNKSHAPE148_DEFAULT_REASON:str="225 generic-ncol-early reuses validated 217 scorestripe_v9 task order/cache; MAXD14 scalar root_after_second predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split225_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split225_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split225-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split225-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split225_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split225-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split225-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split225_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
