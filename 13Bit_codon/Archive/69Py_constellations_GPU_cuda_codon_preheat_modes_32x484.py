...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  bench_mode:int=0  # 0:normal, 1:N=20 warmup repeat, 2:N19 preheat, 3:N18+N19 preheat
  # 69 PREHEAT MODES: preset_queens は 5 固定。preset=6 は 48 で正解には直せたが速度面で不採用。
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 69 PREHEAT MODES; using 5")
      preset_queens_arg=5
    if argc >= 10:
      bench_mode=int(sys.argv[9])
      if bench_mode<0 or bench_mode>3:
