...
# =============================================================================
# NQ_UPDATE_MEMO
# 188: 更新メモ: 187は正当性OKだが184比で微小退行したため不採用。184のno-sibling spill elisionを親に戻し、MAXD14 kernelのみ future_check_mask==0 のとき future prune 判定を省く軽量guardを単独実験として導入。
...
ただし速度は `0:07:16.241` で、184Pyの `0:07:15.635` より `0.606秒`、`0.139%` 遅くなっています。差は小さいですが、複雑化に対して184を上回っていないため、187Pyは**正当性OKだが不採用**としました。

文面では187Pyとありましたが、187Pyは今回実行済みなので、次の単独実験として **188Py** を作成しました。188Pyも187を親にせず、**184Pyを親に戻しています**。

作成物はこちらです。

* [188Py_kernel_futuremask_guard_nosibling_maxd14_probe.py](sandbox:/mnt/data/188Py_kernel_futuremask_guard_nosibling_maxd14_probe.py)
* [188Py_kernel_futuremask_guard_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/188Py_kernel_futuremask_guard_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/188Py_kernel_futuremask_guard_nosibling_maxd14_bundle_20260630.tar)

188Pyの変更点は、184Pyの no-sibling spill elision / tail-call descent を維持したまま、MAXD14 kernelに `future_check_mask` の軽量guardを追加した点です。
...
188Py:
  schedule生成時に future bit を future_check_mask へ集約
  future_check_mask == 0 のschedule:
    hot loopで nibble_op & 8 判定を省略
  future_check_mask != 0 のschedule:
    184相当のfuture-prune判定を実行
```

187Pyの `child_jmark_mask` guard、185Pyの `cur_depth==13` literal fast path、186Pyの `terminal_base14` outer split は入れていません。188Pyは184Py親の単独実験です。

手元では以下を確認済みです。

```text
bash -n 188Py...sh                         OK
STATIC_ONLY=1 188Py...sh                   OK
source_future_check_mask_guard             OK
source_nosibling_parent                    OK
source_187_jmark_guard_removed             OK
source_split_tag split188 only             OK
abstract future_check_mask equivalence     50000 cases OK
```

こちらではCodon/CUDA実機build/full実行はしていません。188Pyの採用可否は、添付の `188Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に184比のtimingを見て判断してください。



"""

# 188 future-mask guard revision memo over validated 184 no-sibling MAXD14:
#   Branch from the validated 184 no-sibling tailcall build after 187 child-jmark guard was correct but not faster.  Host-side depth
...
#   only restorable ancestor frames.  188 keeps the 184 descend policy and changes only the future-check guard:
#   when a schedule has no future-prune frame at all, skip the per-candidate future branch; the existing no-sibling rule still applies after selecting a bit: if the parent has no remaining sibling candidates,
...
# 188 keeps the validated 184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 188 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="188 futuremask-guard: 184 no-sibling MAXD14 baseline plus future_check_mask zero guard - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """188 MAXD14 nibble schedule plus no-sibling spill elision and future_check_mask zero guard."""
    ...
    # 188 keeps the validated 184 current-frame register cache, 13-slot
    # ancestor storage, and no-sibling tail-call descent. It adds only a light
    # guard around the future-prune check: schedules with future_check_mask==0
    # skip the per-candidate nibble bit3 branch entirely. The nonzero-mask path
...
CHUNKSHAPE148_DEFAULT_REASON:str="188 futuremask guard reuses validated 184 scorestripe_v9 task order/cache and changes only MAXD14 future-prune guard"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split188_futuremask_guard_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split188_futuremask_guard_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split188-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split188-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split188_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split188-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split188-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split188_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
