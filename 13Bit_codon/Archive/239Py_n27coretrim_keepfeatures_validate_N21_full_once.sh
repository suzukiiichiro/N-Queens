...
# =============================================================================
# 239 N=21 full validation harness
# Parent: 238 n27diagtrim-keepfeatures r2.
# Experiment: cleanup-only; remove unused recursive CPU dfs fallback and
#             unreachable use_itter branch while keeping -c/-g, N5..27,
#             cache generation, broadmarktail 28/29, split145 30/31,
#             MAXD fallbacks, and worker_id/worker_count.
# Expected: mode31 split145+chunkshape148, required=14, selected MAXD14,
#           schedule_words=0, stack=208 bytes/thread.
# =============================================================================

SRC=${SRC:-./239Py_n27coretrim_keepfeatures_probe.py}
CAND=${CAND:-./239Py_n27coretrim_keepfeatures_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/239Py_n27coretrim_keepfeatures_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/239Py_n27coretrim_keepfeatures_logs_N21_full_once_${TS}"
...
if grep -q '239 n27coretrim-keepfeatures' "$SRC"; then
  printf 'source_version_tag\t239 n27coretrim-keepfeatures\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t239 n27coretrim-keepfeatures\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split239' in s and 'split238' not in s:
    checks.append(('source_split_tag', 'split239 active', 'split239', True))
else:
    checks.append(('source_split_tag', 'split239 active', 'split238 present or split239 missing', False))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 239 source static checks failed" >&2
  exit 66
fi
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 239 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 239 coretrim mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 239 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
