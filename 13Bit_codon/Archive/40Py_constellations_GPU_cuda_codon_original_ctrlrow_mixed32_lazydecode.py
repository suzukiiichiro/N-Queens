...
def kernel_dfs_iter_gpu(
    ...
    while sp>=0:
      ...
      cv=ctrl[sp]
      bit_i:int=int(bit)
      if (cv&u32(256))!=u32(0):  # use_blocks bit8
        cvi:int=int(cv)
        stepv:int=(cvi>>5)&3
        addv:int=(cvi>>7)&1
        bLi:int=(cvi>>10)&3
      ...
      if nf==u32(0):
        continue
      if (cv&u32(512))!=u32(0):  # future bit9
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=256,gpu_max_blocks:int=32,gpu_log_level:int=0)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
