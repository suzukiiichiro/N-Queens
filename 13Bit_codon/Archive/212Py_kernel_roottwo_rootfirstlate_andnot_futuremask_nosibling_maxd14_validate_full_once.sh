...
# =============================================================================
# NQ_UPDATE_MEMO
# 211: 更新メモ: 211Py rootfirstlate は正当性OKで204最速に0.006秒差まで戻したが上回らなかったため未採用。212Pyは211のroot_first lateを維持し、210で良好だった root_after_second and-not predicate を組み合わせる微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 210 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 212 returns to
# validated 204 because 205 rootafterinline was correct but did not beat 204.
# 198/199/200/201/202/203/205 were correct but not adopted.
#
...
# 204 clear-lowbit root_rest plus root_after_second=root_rest^root_second scalar predicate, with root_first computed only inside the preroll branch.
...
#   - rootfirstlate_andnot source check is whitespace/comment robust.
SRC=${SRC:-./212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 212 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/212Py_kernel_roottwo_rootfirstlate_andnot_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 212Py source
#   has a harmless header/string/layout difference. This 212 harness validates the
...
marker='# 212 rootfirstlate_andnot:'
...
if all(checks.values()):
    sys.exit(0)
print('source_rootfirstlate_andnot_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootfirstlate_andnot\tpresent_late_root_first_andnot_predicate\tpresent_late_root_first_andnot_predicate\tOK
' >> "$SUMMARY"
else
  printf 'source_rootfirstlate_andnot\tpresent_late_root_first_andnot_predicate\tmissing_or_old_rootfirst_andnot_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split212'}
bad=sorted(tags-allowed)
if 'split212' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split212 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split212 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 212 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 212 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootfirstlate_andnot static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 212 roottwo-rootfirstlate_andnot futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 212 roottwo-rootfirstlate_andnot futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
