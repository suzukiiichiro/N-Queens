...
* [昨日のメモ](sandbox:/mnt/data/20260623ChatGPT.txt)
* [159Py 最終ソース](sandbox:/mnt/data/159Py_kernel_mask_constants_u32_probe.py)
* [全件検証シェル](sandbox:/mnt/data/159Py_kernel_mask_constants_u32_validate_full_once.sh)
* [159Py 実行ログ一式](sandbox:/mnt/data/159Py_kernel_mask_constants_u32_logs_N21_full_once_20260622_101435.tar)

**159Pyを検証済み基準版**として扱い、次は160Pyの単独実験「`while sp>=0`を廃止し、終了判定をroot pop時へ移動」から継続する認識です。

作成しました。

* [160Py_kernel_root_pop_termination_probe.py](sandbox:/mnt/data/160Py_kernel_root_pop_termination_probe.py)
* [160Py_kernel_root_pop_termination_validate_full_once.sh](sandbox:/mnt/data/160Py_kernel_root_pop_termination_validate_full_once.sh)

シェル構文と160Py固有の静的検査は通過済みです。Codon/CUDA実機検証は未実施です。

"""

# 160 GPU root-pop loop-termination experiment:
#   Retain the fully validated 159 kernel-local u32 mask constants, 158
#   function-id/classification-u32 arithmetic, 157 markctrl-u32 prepack, 156
#   initialized-ctrl-u32 packing, 155 ctrl0-u32 prepack, 154 GPU-input-u32
#   representation, and all earlier DFS/order changes.  The sole kernel
#   experiment replaces `while sp>=0` with `while True` and performs the root
#   termination check only when a frame is popped.  Candidate exhaustion, base
#   completion, and jmark exhaustion use the same guarded pop.  MAXD and its
#   overflow guard remain unchanged.  Host data layout, task order, cache
#   format, DFS transitions, and solution arithmetic remain unchanged from
#   validated 159.
...
VERSION_TAG:str="160 root-pop loop termination over 159 kernel mask constants native-u32 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    #       160はDFS終了判定だけをroot pop時へ移す単独実験とする。
    ...
    # 158/159/160: function-id, flags, and mask operands stay in u32.
    ...
    while True:
      ...
      if (cv&INIT_MASK)==u32(0):
        # 158/159/160: function-id and every classification-mask operand stay in u32.
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154/155/156/157/158/159/160: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155/156/157/158/159/160: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154/155/156/157/158/159/160: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 160 root-pop termination over validated 159 native-u32 masks and the 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 159; 160 changes only GPU DFS loop termination at pop sites
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 159 scorestripe_v9 task order/cache unchanged; 160 moves DFS termination to root pop"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  160 reuses the validated 159/158/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 160 experiment changes only GPU DFS loop termination at the three pop sites.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  160 host-side score-aware lanephase chunk shaping is reused unchanged from validated 159/158/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 160 experiment changes only GPU DFS loop termination at pop sites; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split160_root_pop_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split160_root_pop_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split160-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split160-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split160_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split160-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split160-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split160_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
