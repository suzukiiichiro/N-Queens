...
"""
**355の設計です。** 355はソースコードを一切変更しない。352の引継ぎ
メモが最初に予定していた**項目11**(`wait`45.6%/`branch_resolving`
20.1%)に、353・354を経て戻る。
...
現行キーに勝てなかった(`no_candidate_beats_current`)。残るのは
**発散そのもの**——`wait`と`branch_resolving`で66%。353の設計時に
自分たちで明記した通り、これは「レバーではなく機械についての問い」
であり、性能が動く保証はない。だが254行のコード変更で埋まる余地は
既に350〜354で使い切っており、残る大きな未知数はここしかない。

## 既知の資産(352で確定、持ち越す)

- registers/threadはコンパイル時属性でN非依存。N=18とN=21で45で一致
  することを実証済み
- N=18 launch 0は同じ`launch_kernel_dfs_iter_gpu_static_maxd`を通り、
  同じコンパイル済み`kernel_dfs_iter_gpu_maxd14`を起動する。構造・
  SASS単位の帰属はN=18で取ってN=21に転用してよい
- **ただし動的なストール比率は転用できない。** N=18の小規模テストは
  タスク数が少なくK=1(1スレッド1タスク)で流れるが、N=21本番は
  K=48(1スレッドが48タスクを連続処理)。K-batchingがレーン間の
  不均衡を平均化する(353で確認済み)のと同じ理屈で、ストールの
  発生パターンもK依存で変わりうる
- `--section SourceCounters`(5パスreplay)はN=18なら約9秒。N=21では
  1launchあたり約146秒×5パスで現実的な時間に収まらない
- `sudo`は起動メタデータだけでも要る。**権限確認は必ず数秒のコマンドを
  先に置くこと**(352で14分を無駄にした教訓)

## 355でやること

1. **権限確認**(数秒)。`sudo`が通ることを最初に確認する
2. **構造調査(N=18、`--section SourceCounters --page source`)**:
   `wait`スケールの内訳をSASS単位で見る。304-319が`branch_resolving`
   の65.6%を「BSYNC隣接の2つの無条件BRA」に帰属させたのと同じ手法を、
   今度は`wait`(より大きく、手つかずの方)に適用する。コンパイル時
   属性なのでN=18の約9秒で完結し、N=21にそのまま転用できる
3. **動的調査(N=21、`--section SchedulerStats --section WarpStateStats`、
   `--launch-count 1`)**: chunk0のlaunch 0だけに絞る。この2セクションは
   単発のハードウェアカウンタで完結し、SourceCountersのような5パス
   replayを要求しない。目的は2つ:
   - `wait`/`branch_resolving`の比率がK=48の本番形状でも66%前後を
     維持するか
   - Eligible Warps Per CycleとActive Warps Per Schedulerを比較し、
     「命令発行の余地はあるのに適格ワープが足りない」なら発散そのものが
     律速していると確信できる(Active十分・Eligible不足のパターン)

## このリビジョンにオラクルはない

コードを変えていないので、正解値も壁時計比較も存在しない。**唯一の
正しさの基準は、352とのコード差分がゼロであることそのもの**である。

## ncu出力のパースについて

**このリビジョンは、ncuのテキスト出力を数値として自動抽出しない。**
出力フォーマットは実機で一度も検証していないため、フォーマットへの
依存を「知っているつもり」で書くのは、345 r1・349 r1・354で実際に
踏んだ「地の文をコードと誤認する」失敗と同じ種類の危険を持つ。
`.sh`はセクション見出しの存在だけを緩く確認し、**生ログを丸ごと保存
する**。数値の読み取りと解釈は、実機で取れたログを見てから行う。

## 356はどこから始めるか

**355の結果次第で分岐する。** 発散が真に律速していると確認できれば、
項目6(CUDA Cランナー、warp intrinsics)の優先度が上がる。そうでなければ、
残る対策候補(BRA/BSYNC配置の見直しなど)をSASS/PTXレベルで検討する。
いずれの場合も356のカーネルは352から再開する(355はカーネルを
一切変更していないため)。
...
VERSION_TAG:str="355 branch-wait-probe: MEASUREMENT ONLY REVISION, NOT A CANDIDATE FOR ADOPTION, NO CODE CHANGE. Executable code is byte for byte identical to 352 and to 354s kernel (both already proven identical to 352); 355 is a pure ncu orchestration exercise, item 11 from the 352 handoff, deferred through 353 and 354. THE PATH HERE: 353 measured lane_tail at 1.14 to 1.16x, ruling out workload imbalance as the CUDA C port justification; 354 tried seven reweightings of the current sort key and none beat it, closing the cheap end of the key-redesign axis. What is left is the divergence itself: wait at 45.6 percent and branch_resolving at 20.1 percent of N18 K1 stall samples, together 66 percent, property of the divergent DFS and not of the epilogue. 304-319 already attributed about 65.6 percent of stall_branch_resolving to two unconditional BRA instructions adjacent to BSYNC reconvergence markers; wait, the larger reason, is untouched. THE N18 VS N21 CAVEAT, unchanged from 352: N18 launch 0 in this workload runs the same compiled kernel_dfs_iter_gpu_maxd14 through the same launch_kernel_dfs_iter_gpu_static_maxd dispatcher, so SASS level structure, per instruction attribution and any compile time property (e.g. registers per thread, confirmed 45 on both N18 and N21) transfer exactly; but N18s small task count means K is effectively 1, one constellation per thread through the grid stride loop, while N21 production runs K48, forty eight constellations per thread back to back, so DYNAMIC stall ratios measured at N18 do not represent N21 production and must be retaken there. SourceCounters 5 pass replay does not finish at N21 in reasonable time, about 146 seconds per pass times five per launch; SchedulerStats and WarpStateStats are single pass hardware counter sections and are the ones taken at N21, scoped to launch 0 of chunk 0 only via launch-count 1. 355s plan: a seconds long sudo permission check first, per the 352 lesson that skipping it cost fourteen minutes; SourceCounters at N18 for the STRUCTURAL question, which SASS instructions the wait stall attributes to, cheap at about 9 seconds and transferable because it is compile time; SchedulerStats plus WarpStateStats at N21 chunk 0 launch 0 for the DYNAMIC question, whether the 66 percent holds at true K48 shape and whether Eligible Warps Per Cycle is low while Active Warps Per Scheduler is not, which would mean divergence itself is the limiter rather than instruction scheduling. This revision does not parse ncu text output into the summary programmatically beyond checking that the expected section headers are present: ncu output formatting was not independently verified against a real run before this script was written, so raw logs are captured in full for manual reading rather than trusting a fragile scrape."
