...
* 実行時間`0:14:51.060`
* 161Py比`14.897秒（1.644%）`高速

163Pyでは`ctrl`静的スタックを`u32`から`u16`へ変更し、420→378 bytes/threadへ縮小しました。

* [163Py_kernel_ctrl_stack_u16_probe.py](sandbox:/mnt/data/163Py_kernel_ctrl_stack_u16_probe.py)
* [163Py_kernel_ctrl_stack_u16_validate_full_once.sh](sandbox:/mnt/data/163Py_kernel_ctrl_stack_u16_validate_full_once.sh)

シェル構文、block code等価性56ケース、u16 pack/decode 7,168ケース、模擬131チャンク再生は通過済みです。Codon/CUDA実機実行は未実施です。

なお、ログ記載の162PyソースSHA-256と添付された162PyソースのSHA-256は一致していないため、バイト完全同一性のみ未確認です。


"""

# 163 GPU ctrl-stack-u16 experiment:
#   Retain the fully validated 162 block-metadata branch cleanup, 161 no-stack-
#   overflow-guard kernel, 160 root-pop loop termination, 159 kernel-local u32
#   mask constants, and all earlier DFS/order changes.  The sole structural
#   experiment changes only the per-thread ctrl static stack from u32 to u16.
#   The low ten fid/row bits and the use_blocks/future flags are preserved;
#   initialized moves from bit19 to bit15, and the five reachable block metadata
#   combinations are encoded as a 3-bit block_code in bits10..12.  Arithmetic is
#   widened to u32 after each ctrl load, so bitboard and solution calculations
#   are unchanged.  The four other u32 stacks remain unchanged, reducing static
#   stack storage from 420 to 378 bytes/thread.  Host data layout, task order,
#   cache format, DFS transitions, and solution arithmetic remain unchanged
#   from validated 162.  The inherited GPU safety envelope remains N=5..27.
...
# MAXD=21 exposes indices 0..20 and retains zero spare frames.  163 inherits
# 161's guard removal, so GPU execution remains host-limited to this proven range.
MAXD:Static[int]=21

VERSION_TAG:str="163 ctrl stack u16 over 162 block metadata branch cleanup - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    前提/不変条件:
      ...
      - 163は162の探索論理を維持し、ctrl静的stackだけをu16へ縮小する。
    ホットパス（ソース引用）:
      bit = a & -a
      avail[sp] = a ^ bit
    """
    # NOTE: GPU では list/tuple 参照が遅くなりがちなので、分岐テーブルを
    #       32bit mask として焼き込む。159で各maskを入口からu32に固定済み。
    #       160でDFS終了判定をroot pop時へ移動し、161で到達不能なpush guardを除去済み。
    #       162でblock metadata生成分岐を整理済み。163はctrl stackだけをu16化する。
    ...
    # 159 は10個の分類maskとINIT_MASK自体をkernel-local u32として宣言し、
    #      使用箇所のu32(MASK)変換表記を除去する。mask値と分岐論理は変更しない。
    # 160 は while sp>=0 を while True へ変更し、終了確認を3つのpop経路の
    #      sp==0 判定へ移す。stack overflow guard とDFS状態遷移は変更しない。
    # 161 はN<=27で最大sp=20という既検証境界を利用し、push後の
    #      if sp>=MAXD branchだけを除去する。MAXDとstack配置は変更しない。
    # 162 はmark成立箇所でblockL/blockK metadataを直接生成し、後段の
    #      frame_blocksu判定とblockK用2分岐を削除する。pack layoutは変更しない。
    # 163 はctrlの到達可能状態が「通常2種 + block 5種」に限られることを利用し、
    #      block metadataを3bit codeへ圧縮してctrl static stackをu16化する。
    # Codon の UInt[32] shift は右辺も UInt[32] を要求するため、ctrlをloadした後の
    # metadata演算はu32で行い、static stackへのstore時だけu16へ狭める。
    # 現行 dynamic preset と endmark<=N-2 から、expected table の N<=27 では最大 sp=20。
    # MAXD=21 は index 0..20 を持つため0段の余裕がある。4本のu32 stackは336 bytes、
    # ctrl u16 stackは42 bytesで、静的stack量は420 -> 378 bytes/threadになる。
    # 163は161以降と同じくGPU N=5..27に限定する。N>27を有効化する場合は深さ境界を再証明し、
    # MAXDを見直してruntime guardまたは別の安全策を復帰させる。
    # 未初期化 ctrl: bit15=0, bits0..4=current fid, bits5..9=current row
    # 初期化済 ctrl: bit15=1
    #   bits 0..4   : child/next fid
    #   bits 5..9   : child row after this frame transition
    #   bits 10..12 : block_code (use_blocks時のみ)
    #   bit  13     : use_blocks
    #   bit  14     : future check enabled
    #   bit  15     : initialized
    # block_code:
    #   0 = step2/add0/blockL0/K=n3
    #   1 = step2/add0/blockL1/K=0
    #   2 = step3/add0/blockL2/K=n3
    #   3 = step3/add0/blockL1/K=n4
    #   4 = step2/add1/blockL0/K=0
    # child ctrl は低10bitそのものなので、通常pushでは
    #   ctrl[child] = u16(ctrl[parent] & 1023)
    # とできる。
    INIT_MASK:u32=u32(32768)  # 1<<15
    ...
    # 158/159/160/161/162/163: function-id, flags, and mask operands stay in u32.
    ...
    while True:
      ...
      if (cv&INIT_MASK)==u32(0):
        # 158/159/160/161/162/163: function-id and every classification-mask operand stay in u32.
        ...
        if ismu!=u32(0):
          ...
          if rowv==markv:
            ...
            ###################
            # 163 compact block code.  For every reachable mark fid, the tuple
            # (MASK_L_1 bit, STP3 bit, add1 bit) uniquely identifies the five
            # block metadata combinations listed above.
            frame_blockcodeu=(
              ((MASK_L_1>>fu)&u32(1))
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154/155/156/157/158/159/160/161/162/163: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155/156/157/158/159/160/161/162/163: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 163 decodes the compact 3-bit block code after widening the u16 ctrl
        # load to u32.  The code bits are: bit0=L1 selector, bit1=step3,
        # bit2=add1.  The five reachable combinations reconstruct the original
        # step/add/blockL/blockK tuple without a lookup table or K branch.
        ...
        bKu:u32=(n3&(u32(0)-n3selu))|(n4&(u32(0)-n4selu))
      ...
      #----------------
      sp+=1
      ctrl[sp]=u16(cv&u32(1023))  # child fid + child row
      ld[sp]=nld
      rd[sp]=nrd
      col[sp]=ncol
      avail[sp]=nf
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/163: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 163 ctrl-stack-u16 over validated 162 block-metadata cleanup and the 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 162; 163 changes only the kernel ctrl static stack representation
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 162 scorestripe_v9 task order/cache unchanged; 163 shrinks ctrl stack to u16"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  163 reuses the validated 162/161/160/159/158/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 163 experiment changes only the GPU ctrl static stack representation.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  163 host-side score-aware lanephase chunk shaping is reused unchanged from validated 162/161/160/159/158/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 163 experiment changes only the GPU ctrl static stack representation; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/163: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 163 inherits the 161 safety envelope: the removed kernel guard was proven
  # unreachable only for the existing dynamic-preset range N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 163 ctrl-stack-u16 GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split163_ctrl_u16_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split163_ctrl_u16_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split163-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split163-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split163_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split163-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split163-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split163_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
