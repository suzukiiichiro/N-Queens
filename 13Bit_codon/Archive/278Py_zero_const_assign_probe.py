...
# =============================================================================
# NQ_UPDATE_MEMO
# 278: zero-const-assign probe。277 depthu-childsave は正当性OKだが
#      276/271を上回らなかったため採用せず、276 restore274本線を親に戻す。
#      MAXD14 kernel内だけで ZERO:u32=u32(0) を1回作り、u32(0)を代入
#      している局所初期化・代入箇所をZERO参照へ置換する微差実験版。
#      u32(0)-x のlowbit抽出、clear-lowbit、mask/shift、比較条件、
#      u64(0)/u64(1)、MAXD16/18/20/21 fallback は触らない。
#      root-preroll、rootrestlate、futuremask、no-sibling、split145/chunkshape148、
#      broadmarktail、cache生成、worker split、mode28/29/30/31、CPU dfs_iter pathは維持。
#      273 root0 direct kernel、275 bucket診断、277 depth_u、270 schedule precompute、
...
277ログを確認しました。正当性はOKですが、276/271を上回らなかったため不採用にしました。

```text
277Py depthu_childsave
...
278は、ご提案の `u32(0)` 再利用を試す微差実験版として作成しました。**277は採用せず276本線へ戻し、MAXD14 kernel内だけ `ZERO:u32=u32(0)` を作って、単純な `u32(0)` 初期化・代入だけを `ZERO` 参照に置換**しています。`u32(0)-x` のlowbit抽出、比較条件、mask、root-preroll、dispatchは触っていません。

`STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [278Py_zero_const_assign_probe.py](sandbox:/mnt/data/278Py_zero_const_assign_probe.py)
* [278Py_zero_const_assign_validate_N21_full_once.sh](sandbox:/mnt/data/278Py_zero_const_assign_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [278Py_zero_const_assign_sha256.txt](sandbox:/mnt/data/278Py_zero_const_assign_sha256.txt)
* [278Py_zero_const_assign_bundle_20260709.tar](sandbox:/mnt/data/278Py_zero_const_assign_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./278Py_zero_const_assign_probe

STATIC_ONLY=1 bash 278Py_zero_const_assign_validate_N21_full_once.sh

bash 278Py_zero_const_assign_validate_N21_full_once.sh
```

採否は、まず正当性OK、そのうえで **276の `0:07:07.672`、271の `0:07:07.705`、277の `0:07:07.717`** と比較して判断するのがよいです。
...
VERSION_TAG:str="278 zero-const-assign: parent 278 zero-const-assign; rejected 277 depthu-childsave for speed; MAXD14 kernel defines ZERO:u32=u32(0) and reuses it only for simple u32 zero initializations/assignments, without changing lowbit extraction, comparisons, masks, root-preroll, rootrestlate, futuremask, no-sibling, split145/chunkshape148, cache generation, MAXD fallbacks, or worker split; generic N5..27; bare -g split145 mode31"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    bm:u32=board_mask
    # 278: reuse a local typed zero only for simple u32 zero initializers/assignments.
    # Keep u32(0)-x lowbit extraction, comparisons, and masks as immediate forms.
    ZERO:u32=u32(0)
    ...
    schedule_lo:u32=ZERO
    schedule_hi:u32=ZERO
    child_jmark_mask:u32=ZERO
    future_check_mask:u32=ZERO
    terminal_parent_depth:int=0
    terminal_is_base14:u32=ZERO
    root_action:u32=ZERO
    while True:
      ...
      frame_action:u32=ZERO
      frame_nibble:u32=ZERO
      frame_raw:u32=ZERO
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split278_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split278_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split278-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split278-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split278_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
