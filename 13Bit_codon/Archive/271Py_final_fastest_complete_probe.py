...
おはようございます。271一式を作成しました。

今回は、270は正当性OKながら `0:07:08.824` で241より遅く、添付メモ上も「不採用」とされているため、**271は241 r3を親にした完成版パッケージ**にしました。241は `0:07:07.788`、final total `314666222712` OK、MAXD14 / schedule_words=0 / stack 208 bytes/thread の採用基準です。

こちらで `STATIC_ONLY=1` は通しました。Codon release build と N=21 full run は、この環境では実行していません。

ファイルはこちらです。

* [271Py_final_fastest_complete_probe.py](sandbox:/mnt/data/271Py_final_fastest_complete_probe.py)
* [271Py_final_fastest_complete_validate_N21_full_once.sh](sandbox:/mnt/data/271Py_final_fastest_complete_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [271Py_final_fastest_complete_sha256.txt](sandbox:/mnt/data/271Py_final_fastest_complete_sha256.txt)
* [271Py_final_fastest_complete_bundle_20260709.tar](sandbox:/mnt/data/271Py_final_fastest_complete_bundle_20260709.tar)

実行手順は以下でお願いします。

```bash
rm -f ./271Py_final_fastest_complete_probe

STATIC_ONLY=1 bash 271Py_final_fastest_complete_validate_N21_full_once.sh

bash 271Py_final_fastest_complete_validate_N21_full_once.sh
```

今後の優先順位は、READMEにも追記しましたが、整理すると次の順です。

| 優先 | 候補                              | 効果見込み | 難易度 | コメント                                                             |
| -- | ------------------------------- | ----: | --: | ---------------------------------------------------------------- |
| A  | root_action分布の軽量診断              |   中〜高 | 低〜中 | 267ログだけで不足なら、root0/fallback/task数/非ゼロaction種別を出す診断版を作る           |
| A  | root_action非ゼロ側のchunk単位専用kernel |     中 | 中〜高 | `root_action==1/2/3` に偏りがある場合だけ。別launchではなくchunk単位dispatch       |
| B  | root0 kernelの不要load/decode削減    |   小〜中 |   中 | root_action load/branch削減は有効と確認済み。小手先のif変更ではなく不要仕事削除として扱う        |
| B  | MAXD14 hot loopの構造再検討           |     中 |   高 | generic loop単発置換は退行が多いので、保存規則・load/store削減として設計する                |
| C  | host precompute/pack系の再挑戦       |     小 |   高 | 267がprecompute系最良だが241に届かず、270も不採用。root_action pack系はJIT危険       |
| C  | MAXD13 / stack縮小                |    不明 |   高 | 222診断で単純MAXD13化は不可。限定dispatchには追加診断が必要                           |
| D  | 小手先微差                           |     低 |   低 | if順序、nibble/future bit微差、root-preroll小整理、別launch分割だけのprobeは当面避ける |

次の実験に進むなら、**272は速度候補ではなく root_action 分布の軽量診断版**が一番筋がよいと思います。266/267で `root_action==0` 専用kernelと `root_action_nonzero_count` cache は効果があり、root_action load/branchを消す方向が有効だと分かっています。
...
VERSION_TAG:str="271 final-fastest-complete: 270 rootpre2flag rejected for speed; fixed on 241 r3 restore239-after-fid14split-reject baseline; generic N5..27; bare -g split145 mode31; unused recursive CPU dfs fallback remains removed; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved; CUDA kernels unchanged"
...
CHUNKSHAPE148_DEFAULT_REASON:str="271 final keeps validated 241/239/238/231/217 scorestripe_v9 task order/cache; CUDA kernels unchanged; 270 rootpre2flag and 240 fid14 launch split rejected"
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 271 final/coretrim; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split271_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split271_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split271-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split271-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split271_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
