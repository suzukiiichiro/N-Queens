...
def kernel_dfs_iter_gpu(
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        f:int=int(cv0&u32(31))
        rowv:int=int((cv0>>5)&u32(31))
        nfid=meta_next[f]
        #######################################
        # 基底 is_base
        ...
        if isb==1 and rowv==endm:
          if f==14:# SQd2B 特例
            total+=u64(1) if ((a>>1)!=u32(0)) else u64(0)
          else:
            total+=u64(1)
          sp-=1
          continue
        ...
        if use_blocks==1:
          ...
          if ((MASK_K_N4>>f)&1)==1:
            ktype=2
        ctrl[sp]=u32(524288 | nextfv | (stepv<<5) | (addv<<7) | (use_blocks<<8) | (fcv<<9) | (bLv<<10) | (ktype<<12) | (rowv<<14))
      #----------------
      # 1bit 展開
      #----------------
      a=avail[sp]
      ...
      #----------------
      cv=ctrl[sp]
      stepv:int=int((cv>>5)&u32(3))
      addv:int=int((cv>>7)&u32(1))
      if ((cv>>8)&u32(1))==u32(1):
        bL=(cv>>10)&u32(3)
        kt:int=int((cv>>12)&u32(3))
        bK=0
        if kt==1:
          bK=n3
        elif kt==2:
          bK=n4
        bit_i:int=int(bit)
        nld=((ld[sp]|bit_i)<<stepv)|addv|int(bL)
      ...
      # push
      #----------------
      next_row:int=int((cv>>14)&u32(31))+stepv
      sp+=1
      if sp>=MAXD:
        results[i]=total*w_arr[i]
        return
      ctrl[sp]=u32(int(cv&u32(31)) | (next_row<<5))
      ld[sp]=nld
      rd[sp]=nrd
      col[sp]=ncol
      avail[sp]=nf
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=256,gpu_max_blocks:int=32,gpu_log_level:int=0)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 maxd28=1 block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
