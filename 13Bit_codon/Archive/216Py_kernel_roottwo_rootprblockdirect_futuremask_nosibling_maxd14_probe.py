...
# =============================================================================
# NQ_UPDATE_MEMO
# 216: 更新メモ: 215Py rootbitdirect は正当性OKだが204/211より遅いため不採用。216Pyは211Py rootprblockdirect形へ戻し、MAXD14 kernelのみroot-preroll内の pr_block_code を pr_nibble_op&7 ではなく schedule_lo&7 から直接取得する微差実験。
...
warning/error            0 / 0
N=21 full                0:07:08.061
```

ただし速度は204/211には届きませんでした。
...
したがって、215Pyは**正当性OKだが不採用**です。

216Pyは215の `pr_bit` alias削除を撤回し、204に最も近かった211Py形へ戻したうえで、MAXD14 kernel内だけ次の小差分にしました。
...
```text
bash -n 216Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 216Py...sh OK
source_rootprblockdirect              OK
source_split_tag split145 split216    OK
```

ファイルはこちらです。

* [216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_probe.py)
* [216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_validate_full_once.sh

bash 216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 216 rootprblockdirect memo over validated 211 rootprblockdirect futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="216 roottwo-rootprblockdirect: 211 late-root_first with root-preroll block_code read as schedule_lo&7 - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """216 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate root-preroll block-code direct read."""
    ...
    # correct but slower.  216 keeps 211's late root_first and 204's clear-lowbit
    # root_rest construction/root_after_second XOR scalar.  The micro-experiment
    # is only pr_block_code=schedule_lo&7, avoiding pr_nibble_op&7 dependency.
...
CHUNKSHAPE148_DEFAULT_REASON:str="216 rootprblockdirect reuses validated 211/204 scorestripe_v9 task order/cache; MAXD14 reads root-preroll block code directly from schedule_lo&7"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split216_roottwo_rootprblockdirect_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split216_roottwo_rootprblockdirect_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split216-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split216-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split216_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split216-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split216-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split216_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
