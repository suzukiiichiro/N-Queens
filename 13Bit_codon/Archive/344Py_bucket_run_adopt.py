...
8. [解決・344で採用] warp内コスト不均質(tail effect) -- CHUNKSHAPE148_BUCKET_RUN=2048
   ...
   **次の軸(345候補): イテレーション内のスコア整列を細かくする。**
   飽和状態の出力は「1イテレーション(15488件)を8バケットの連続ブロック
   に並べ、ブロック内はlane拡散順」である。粒度を上げる方向は2つある:
     (a) バケット数を8->16->32に増やす。`interleave_order`(8要素固定)、
         `chunkshape148_build_thresholds_from_hist`(7閾値)、
         `bucket_lane_indices`(8*32)の一般化が必要。
     (b) **イテレーション内をスコアキーで直接フルソートする**(=バケット
         数を極限まで上げた場合と等価)。実装は(a)より単純であり、
         「粒度を上げれば良くなるのか」という問いに一発で答えられる。
         ただしlane拡散(同一スコアを32レーンに散らす仕組み)を壊すため、
         同点集合が固まる影響は別途見る必要がある。
   **345は(b)を先に測るのが合理的**。効果がなければ(a)に進む意味は薄く、
   効果があれば(a)で粒度を調整する価値が確定する。

このセクション自体は通常カーネルロジックとは独立しています。
344は**採用リビジョン**です(330-332でスイープし333で採用したのと同じ
位置づけ)。ソース変更は既定値2個のみ:
...
w3_j7)も、339で入れたホスト側のrun-lengthループ本体も、343とバイト
一致です。`CHUNKSHAPE148_BUCKET_RUN=1`を明示的に渡せば276-338の順序と
キャッシュファイル名をそのまま再現できるため、旧ベースラインは
いつでも到達可能です。
================================================================================
...
**飽和が確定しました。ダイジェストが5値すべて一致しています。**決着しました。**ダイジェストが5値すべて一致しています。**

## 343の結果 — 飽和がデータレベルで確定

| RUN | elapsed | vs RUN=1 | |
|---:|---:|---:|---|
...
2048 / 4096 / 8192 / 16384 / 32768
  → md5 283b038c2573848f6a5b945b20ec6102
  → 32,404,512 bytes
  → 5値すべて完全一致
```

**タイミングの解釈に頼らず、2048より大きい値は原理的に何も変えられないことが確定しました。** 事前登録した予測の検定として、これ以上ない形で決着です。

最初の1点(432.171s)だけやや外れていますが、残り5点は **0.020%幅**。初回実行のウォームアップと見るのが自然です。

---

## そして、343での私の自己批判は不正確でした

343で「342の飽和点推定(≈1936)は誤り」「341と同じ誤りを二度繰り返した」と書きました。**どちらも事実に反します。**

- 実測の飽和点は **2048**。342の推定(≈1936)の方が正しかった
- 343で挙げた15488は**十分条件**であって必要条件ではない。quota分布がほぼ均等なので2048で飽和する
- 342の上限8192は最良点2048より**上**にあり、341(上限256が最良点そのもの)と違って**一度も制約になっていなかった**

**過度な自己批判も記録の不正確さであり、控えめな誤りではありません。** READMEで撤回・訂正しました。343で追加した「上限が `BLOCK*MAX_BLOCKS` 以上」の静的チェックは、無害かつ将来の設定変更への保険として残します。

---

## 344 — 採用

330-332でスイープし333で採用したのと同じ位置づけです。ソース変更は既定値2個のみ(`import gpu` 以降の実行コード差分がこの2行だけであることを機械検証済み):
...
新規ゲーティング静的チェック4件(既定値2048、旧値1の不在、A10G既定値2048、**2つの既定値の一致**)。

**旧ベースラインへの到達手段は維持しています。** `CHUNKSHAPE148_BUCKET_RUN=1` を明示すれば276-338の順序と**キャッシュファイル名まで**再現できます。

### キャンペーン総括(338-344)

| | |
|---|---|
| 最終成績 | **431.677s = −4.111%**(対照 RUN=1 = 450.183s) |
| チャンク別 | −3.853% / −4.529% / −3.836%(3チャンク同時) |
| 328(SoA採用時)から | **−5.341%** |
| カーネル変更 | **一行もなし** |

329で特定されたtail effectは、カーネル分解の5戦5敗を経て手つかずでした。ホスト側の並べ替えだけで −4.1% です。この軸の通算成績は K-batching(−4.4%)、w3_j7(−1.3%)、bucket_run(−4.1%)。

### 345の候補

飽和状態の出力は「1イテレーションを8バケットの連続ブロックに並べ、ブロック内はlane拡散順」です。粒度を上げる方向は2つ:

- **(a)** バケット数を8→16→32に一般化(`interleave_order`・閾値ビルダ・`bucket_lane_indices`に手が入る)
- **(b)** イテレーション内をスコアキーで**直接フルソート**((a)の極限、実装はむしろ単純)

**(b)を先に測るのが合理的**です。効果がなければ(a)に進む意味は薄く、効果があれば(a)で粒度を詰める価値が確定します。ただし(b)はlane拡散を壊すので、同点集合が固まる影響は別途見る必要があります。

```
STATIC_ONLY=1 bash 344Py_bucket_run_adopt_validate_N21_full_once.sh
bash 344Py_bucket_run_adopt_validate_N21_full_once.sh
CHUNKSHAPE148_BUCKET_RUN=1 bash 344Py_bucket_run_adopt_validate_N21_full_once.sh  # 任意: 対照点
```

静的チェック45項目 FAILゼロ、RUN=1 上書きの動作確認、ソース差分2行の機械確認まで済んでいます。
"""
...
VERSION_TAG:str="344 bucket-run-adopt: ADOPTION REVISION. The source change is two default constants only -- CHUNKSHAPE148_BUCKET_RUN and A10G_FINAL_DEFAULT_CHUNKSHAPE148_BUCKET_RUN both go from 1 to 2048. Kernels, dispatcher, K=48, the SoA w_lo_arr/w_hi_arr split, w3_j7 and the host-side run-length emit loop added in 339 are all byte-for-byte identical to 343/342/341/340. Passing CHUNKSHAPE148_BUCKET_RUN=1 explicitly still reproduces the 276-338 emit order and its cached shaped-bin filename, so the old baseline stays reachable. 343 CLOSED THE AXIS: the sweep of 2048, 4096, 8192, 16384 and 32768 came in at 431.677 to 431.764s, a spread of 0.020%, and the shaped bins for all five values are BYTE-IDENTICAL -- md5 283b038c2573848f6a5b945b20ec6102, 32404512 bytes -- which is the pre-registered saturation prediction confirmed at the data level rather than inferred from timings. Saturation therefore sits at 2048, and 2048 is adopted as the smallest saturating value. Final standing on N=21 against the in-session RUN=1 control of 450.183s: 431.677s, -4.111%, with all three chunks improving together (-3.853%, -4.529%, -3.836%), and -5.341% cumulatively from the 456.036s baseline at the time SoA was adopted in 328. The campaign never touched the kernel. Three rules were established along the way: alignment to warp width is necessary (16, 48 and 80 all sit in a -0.26% to -0.34% band regardless of magnitude); being a power of two matters on top of that (96 and 384 are multiples of 32 yet land at the bottom of the aligned group); and the axis saturates at 2048. A CORRECTION TO THE RECORD: 343 asserted that 342 reasoning about saturation was wrong, and that assertion was itself overstated. 342 estimated saturation near quotas[b] ~ STEPS/8 ~ 1936 and the measured saturation point is 2048, so that estimate was right; the 15488 figure given in 343 is a sufficient bound, not a necessary one, because the quota distribution is close to uniform. 342 ceiling of 8192 was also above its own best point of 2048, so unlike 341 -- where the ceiling of 256 was exactly the best point -- it never constrained anything. The static check added in 343 requiring the cap to clear BLOCK*MAX_BLOCKS is harmless and is kept as insurance. NEXT AXIS for 345: the saturated output is one grid-stride iteration arranged as 8 contiguous score-bucket blocks with lane-diffused order inside each block, so the open question is whether a finer sort helps. The cheaper way to answer it is to sort each iteration directly by score key, which is equivalent to taking the bucket count to its limit and is simpler to implement than generalising interleave_order, the threshold builder and bucket_lane_indices to a variable bucket count."
