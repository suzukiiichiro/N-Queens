...
作成しました。164Pyは162Pyから派生し、`ctrl:u32`を維持したまま`INIT_MASK`判定を廃止しています。

* [164Py_kernel_child_frame_push_init_probe.py](sandbox:/mnt/data/164Py_kernel_child_frame_push_init_probe.py)
* [164Py_kernel_child_frame_push_init_validate_full_once.sh](sandbox:/mnt/data/164Py_kernel_child_frame_push_init_validate_full_once.sh)

rootをループ前、childをpush直前に初期化します。base childと候補消滅jmark childはpushしません。静的検査、ctrl pack 258,048ケース、162対164のDFSモデル25,000ケースは通過済みです。Codon/CUDA実機実行は未実施です。


"""

# 164 GPU child-frame push-initialization experiment:
#   Branch from the validated 162 block-metadata branch-cleanup baseline; the
#   rejected 163 ctrl-u16 experiment is intentionally not inherited.  Retain
#   the validated 161 no-stack-overflow-guard kernel, 160 root-pop termination,
#   159 kernel-local u32 masks, and every earlier host/order/cache change.
#   The sole structural experiment initializes the root frame before entering
#   the DFS loop and initializes every child frame immediately before its push.
#   Therefore every ctrl stack entry is already expanded metadata, INIT_MASK
#   and the per-iteration initialized test disappear, and base/dead-jmark child
#   frames are consumed without stack stores or a later pop.  ctrl remains u32;
#   stack depth/layout, host data, task order, cache format, bitboard arithmetic,
#   and solution arithmetic remain inherited from validated 162.  The GPU
#   safety envelope remains N=5..27.
...
# MAXD=21 exposes indices 0..20 and retains zero spare frames.  164 branches
# from 162 and inherits 161's guard removal, so GPU execution remains limited
# to this proven range.
MAXD:Static[int]=21

VERSION_TAG:str="164 child frame push initialization over 162 block metadata branch cleanup - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 162 scorestripe_v9 task order/cache unchanged; 164 initializes child frames before push"
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 164 branches from 162 and inherits the 161 safety envelope: the removed
  # kernel guard was proven unreachable only for dynamic-preset N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 164 child-frame-push-init GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split164_child_push_init_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split164_child_push_init_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split164-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split164-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split164_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split164-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split164-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split164_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
