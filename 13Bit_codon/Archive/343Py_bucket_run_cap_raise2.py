...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 343 (2026-07-27)
...
8. [339効果確認・340機構確定・341/342で上限に二度到達・343で飽和点へ] warp内コスト不均質

   **340+341+342の全測定**(同一マシン、対照RUN=1=450.183s、全点で正当性
   314666222712・tasks=2025282・cache_state=reuse・FAILゼロ。ドリフトは
   340 +0.0161% / 341 +0.0027% / 342 -0.0195%):

     RUN   x32   2冪   elapsed     vs RUN=1    chunk0    chunk1    chunk2
     ...
     256    OK    OK   439.875s    -2.290%    -2.331%   -2.450%   -1.937%
     384    OK    -    447.077s    -0.690%    +0.423%   -0.745%   -2.079%
     512    OK    OK   440.235s    -2.210%    -3.256%   -1.163%   -2.121%
    1024    OK    OK   438.211s    -2.659%    -2.719%   -3.562%   -1.309%
    2048    OK    OK **431.684s**  **-4.109%** -3.851%  -4.530%   -3.835%

   **確定事項1: 32整列則(340)。** 非整列の16/48/80はすべて-0.26〜-0.34%の
   同一群に落ちる。342の対照点80も予測どおりだった。

   **確定事項2: 2冪則(342で確定)。** 32整列だが2冪でない96(-0.609%)と
   384(-0.690%)は、いずれも整列群の底(-0.6〜-0.7%)に留まり、隣接する
   2冪の64(-0.832%)・128(-0.818%)・512(-2.210%)に及ばない。
   342の対照点384は事前予測どおり悪化し、2冪則が確定した。

   **確定事項3: RUN=2048で全チャンクが同時に改善した。** 32〜128の間は
   chunk0が+0.27%に固定されchunk2だけが動いていた。256で初めてchunk0/1が
   動き、2048では**3チャンクすべてが-3.8〜-4.5%**で揃った。256〜1024で
   見えていたchunk間の綱引き(chunk2はRUN=64を好み、chunk0/1はRUN=256を
   好む)は2048で解消しており、**チャンク別RUNという案は不要になった**。
   曲線は単調ではない(512は256よりわずかに劣る)が、傾向は明確に下降。

   **343で修正する私の誤り**: 342で上限を8192にしたのは「飽和は
   `quotas[b] ≈ STEPS/8 = 1936`付近で来る」という読みによるが、**これは
   誤りだった**。1936は平均であり、`quotas[]`は各バケットの残存数に比例
   して`m_target = STEPS = 15488`に合計されるだけなので、単一バケットの
   quotaは最大15488になりうる。runは`min(RUN, quotas[b], 残り)`を出すため、
   **保証された飽和点は RUN >= 15488** であり、8192はまだその下だった。
   341で256を上限にして最良点を上限に張り付かせた失敗と同じ誤りを、
   規模を変えて繰り返したことになる。343は上限を65536にして飽和点を
   確実に跨ぐ。

   **343で検定する事前予測**: RUN >= 15488 では、どのバケットも必ず
   quota全量を一塊で出す。したがって**RUN >= 15488 のすべての値は
   バイト同一のshaped binを生成しなければならない**。343は16384と32768の
   ダイジェストを比較してこれを検定する(GPU実行を要さない)。異なれば
   飽和モデルが誤りである。

   **採用は飽和確認後**。上限に張り付いた点を最適と呼ばない方針は維持する。
   次の軸: 飽和後の最良値が「1イテレーションを8バケットの連続ブロックに
   並べ替えたもの」に相当するなら、次はバケット数そのものの細分化
   (8→16→32)が自然な延長になる。

このセクション自体は通常カーネルロジックとは独立しています。
343のソース変更は**定数1個のみ**です:
`CHUNKSHAPE148_BUCKET_RUN_MAX` を 8192 から 65536 に引き上げます
(付随してその上のコメントを、342で誤っていた飽和点の説明から
正しい説明に書き換えています)。カーネル・ディスパッチャも、339で
入れたホスト側のrun-lengthループ本体も、`CHUNKSHAPE148_BUCKET_RUN`の
ソース既定値1も、すべて342とバイト一致です。上限の引き上げは
RUN<=8192の挙動を一切変えないため、339-342で構築したshaped bin
キャッシュはすべて有効なまま使えます。
================================================================================
...
**成果1 — 2冪則が確定。** 事前登録した対照点 RUN=384(32整列だが2冪でない)は **−0.690%** で、96(−0.609%)と同じく整列群の底に落ちました。隣接する2冪(128 −0.818%、512 −2.210%)には遠く及びません。**32整列に加えて2冪であることが必要**です。

**成果2 — RUN=2048 で −4.109%。** そして**初めて3チャンクが揃って改善**しました。256〜1024で見えていたchunk間の逆方向の選好が2048で解消しています。したがって私が有望視していた**「チャンク別RUN」案は不要**になりました。chunk2の112,303msは全測定を通じた最小値です。

---

## そして、私は同じ誤りを二度繰り返しました

342で上限を8192にしたのは「飽和は `quotas[b] ≈ STEPS/8 = 15488/8 ≈ 1936` 付近で来る」という読みによるものでした。**これは誤りです。**

1936は**平均**です。`quotas[]` は各バケットの残存数に比例して `m_target = STEPS = 15488` に合計されるだけなので、**単一バケットのquotaは最大15488になりえます**。runが出すのは `min(RUN, quotas[b], 残り)` なので:

**保証された飽和点は RUN ≥ 15488。8192はまだその下でした。**

341で上限を256にして最良点(256)を上限に張り付かせたのと、まったく同じ誤りです。規模を変えて繰り返しました。二度目は言い訳が立ちません。

## 343 — 上限を跨ぎ、再発を機械的に防ぐ

ソース変更は定数1行のみ(`import gpu` 以降の実行コード差分がこの1行だけであることを機械検証済み):

```python
CHUNKSHAPE148_BUCKET_RUN_MAX:int=8192  →  65536
```

**再発防止として、上限が `BLOCK*MAX_BLOCKS`(=15488)以上であることをゲーティング静的チェックにしました**(`source_bucket_run_cap_clears_saturation`)。以後、飽和点を跨がない上限を設定すると**ビルド前に停止します**。私の判断ミスを仕組みで塞ぎます。

### GPU実行を要さない飽和検定

事前予測を明確にしておきます:

> RUN ≥ 15488 では、どのバケットも必ずquota全量を一塊で出す。したがって **RUN ≥ 15488 のすべての値はバイト同一のshaped binを生成しなければならない。**

ドライバはスイープ後に各shaped binのmd5を表にします。16384と32768が**同一ダイジェストなら飽和モデルは確認**、**異なればその場で反証**されます。タイミングの解釈を待たずに結論が出るので、これが343の主要な判定材料です。

スイープ点(既定 `2048 4096 8192 16384 32768 2048`):
- 2048をアンカーとして最初と最後
- 4096 / 8192 — 飽和点未満の2冪
- **16384 と 32768 — 両方15488以上。この2点が飽和検定**

任意で3072(=1.5×2048、非2冪)を足せばこの規模での2冪則も再確認できます。

**採用は343でも見送ります。** 二度誤った以上、上限に張り付いた点を最適と呼ばない方針はいっそう厳格に守ります。

```
STATIC_ONLY=1 bash 343Py_bucket_run_cap_raise2_validate_N21_full_once.sh
DRY_RUN=1 bash 343_bucket_run_cap_raise2_driver.sh
bash 343_bucket_run_cap_raise2_driver.sh
```

静的チェック42項目 FAILゼロ(新規の飽和点チェックを含む)、`bash -n` 2本、ドライラン、ソース差分1行の確認まで済んでいます。飽和が確認できれば344で採用し、その後はバケット数の細分化(8→16→32)に進みます。
"""
...
VERSION_TAG:str="343 bucket-run-cap-raise2: the ONLY source change is one constant, CHUNKSHAPE148_BUCKET_RUN_MAX raised from 8192 to 65536 (plus a corrected comment above it). Kernels, dispatcher, K=48, the SoA split, w3_j7, the 339 run-length emit loop and the source default CHUNKSHAPE148_BUCKET_RUN=1 are all byte-for-byte identical to 342/341/340/339, and the raise cannot change behaviour for any RUN <= 8192, so all shaped-bin caches from 339-342 remain valid. 342 RESULTS: RUN=2048 reached 431.684s, -4.109% against the in-session RUN=1 control of 450.183s, with all points correct at 314666222712 and session drift of -0.0195%. Three things are now established. The warp-alignment rule holds everywhere: 16, 48 and 80 all land in a tight -0.26% to -0.34% band. The power-of-two rule is confirmed: 96 and 384 are multiples of 32 but not powers of two, and both sit at the bottom of the aligned group (-0.609% and -0.690%) while their power-of-two neighbours 64, 128 and 512 reach -0.832%, -0.818% and -2.210%; 384 was the pre-registered control for this and it regressed exactly as predicted. And at RUN=2048 all three chunks improved together (-3.851%, -4.530%, -3.835%) for the first time, dissolving the tug-of-war seen from 256 to 1024 where chunk2 preferred RUN=64 while chunk0 and chunk1 preferred RUN=256 -- which retires the chunk-specific-RUN idea. The curve is not monotone (512 is marginally worse than 256) but the trend is clear. 343 CORRECTS AN ERROR OF MINE: 342 set the ceiling at 8192 on the reasoning that saturation would arrive near quotas[b] ~ STEPS/8 ~ 1936, but 1936 is only the AVERAGE quota. quotas[] is proportional to each bucket remaining population and merely sums to m_target = STEPS = 15488, so one bucket can hold a quota as large as 15488, and since a run emits min(RUN, quotas[b], remaining) the guaranteed saturation point is RUN >= 15488 -- above the 8192 ceiling. That is the same mistake as 341, which capped at 256 and then found 256 to be the best point; I repeated it one scale up. 65536 clears 15488 with room. The saturation claim is now hard and falsifiable: every RUN >= 15488 must emit each bucket whole quota as one unbroken block, so all such values must produce BYTE-IDENTICAL shaped bins, and 343 tests this by comparing file digests for 16384 and 32768 at no GPU cost. Adoption stays deferred until saturation is confirmed, since a best point pinned to an artificial ceiling is not an optimum. If the saturated configuration is effectively one grid-stride iteration sorted into eight contiguous score blocks, the natural next axis is the number of buckets itself, 8 to 16 to 32."
