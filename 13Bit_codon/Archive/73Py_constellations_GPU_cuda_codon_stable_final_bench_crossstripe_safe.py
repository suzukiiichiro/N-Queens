...
CPU
$ ./73Py_constellations_GPU_cuda_codon_stable_final_bench_crossstripe_safe -c

GPU
$ codon build -release 73Py_constellations_GPU_cuda_codon_stable_final_bench_crossstripe_safe.py
$ ./73Py_constellations_GPU_cuda_codon_stable_final_bench_crossstripe_safe -g 5 20 32 484 0 -1

N=20 単体ベンチ
$ ./73Py_constellations_GPU_cuda_codon_stable_final_bench_crossstripe_safe -g 20 20 32 484 0 -1

N=20 repeat2 benchmark（run1 warmup / run2 measured）
$ ./73Py_constellations_GPU_cuda_codon_stable_final_bench_crossstripe_safe -g 20 20 32 484 0 -1 5 5

cross_stripe_safe 有効化（検証用。ベンチ測定には使わない）
$ ./73Py_constellations_GPU_cuda_codon_stable_final_bench_crossstripe_safe -g 20 20 32 484 2 -1 5 5 1

ログ確認
$ ./73Py_constellations_GPU_cuda_codon_stable_final_bench_crossstripe_safe -g 20 20 32 484 2 -1 5 5
...
VERSION_TAG:str="73 STABLE FINAL BENCH + CROSS STRIPE SAFE CLEANUP"
...
"""73 の auto sort 方針。N=20 だけ sort_mode=9、それ以外は安全側で sort_mode=0。"""
def auto_sort_mode(N:int)->int:
  if N==20:
    return 9
  return 0
...
"""cross_stripe_safe 用の reorder 件数検証。投入順だけを変え、件数は変えないことを確認する。"""
def validate_reordered_count(label:str,expected:int,actual:int)->bool:
  if expected!=actual:
    print(f"[cross-stripe-safe][error] {label}: reordered count mismatch expected={expected} actual={actual}")
    return False
  return True

"""cross_stripe_safe 用の permutation 検証。重複投入・欠落投入を検出する。"""
def validate_reordered_permutation(label:str,original:List[Dict[str,int]],reordered:List[Dict[str,int]])->bool:
  if not validate_reordered_count(label,len(original),len(reordered)):
    return False
  counts:Dict[Tuple[int,int,int,int],int]={}
  for c in original:
    key:Tuple[int,int,int,int]=(c["ld"],c["rd"],c["col"],c["startijkl"])
    if key in counts:
      counts[key]+=1
    else:
      counts[key]=1
  for c in reordered:
    ...
    if key not in counts:
      bad_start:int=c["startijkl"]
      print(f"[cross-stripe-safe][error] {label}: unknown constellation in reordered list startijkl={bad_start}")
      return False
    if counts[key]<=0:
      dup_start:int=c["startijkl"]
      print(f"[cross-stripe-safe][error] {label}: duplicated constellation startijkl={dup_start}")
      return False
    counts[key]-=1
  remaining:int=0
  for key,value in counts.items():
    remaining+=value
  if remaining!=0:
    print(f"[cross-stripe-safe][error] {label}: missing constellation count={remaining}")
    return False
  return True
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT)->None:
  """
  機能:
  ...
  # sort_mode auto:
  #   73 STABLE FINAL BENCH:
  #     N=20 は sort_mode=9（cross stripe only）を採用。
  #     N!=20 は従来どおり sort_mode=0。
  #   N=21 以降へ sort_mode=9 を自動展開せず、明示指定で比較する。
  if gpu_sort_mode < 0:
  ...
  if use_gpu:
    ...
    if stripe_chunks:
      ...
      if cross_stripe:
        # 73 STABLE FINAL:
        #   sort_mode=9 is the adopted N=20 ordering from 72.
        #   It changes constellation launch order only; kernel logic remains unchanged.
        #   Do not auto-apply this mode to N>=21 until each N is measured explicitly.
        #
        # 65 CROSS STRIPE:
        #   63/64 の balanced stripe は出力chunkごとに within % n_chunks_est が固定される。
        #   cross stripe では src_ch と within residue をずらして直交させ、
        #   各出力chunkが全 residue を含むようにする。
        out_ch:int=0
        while out_ch<n_chunks_est:
          ...
          while slot<STEPS:
            ...
            # 66 SAFE FIX:
            #   STEPS が n_chunks_est で割り切れない場合、
            #   within == STEPS 以上になる端数スロットが発生する。
            #   65 ではこの端数が次 chunk の先頭 index と衝突し、
            #   一部 constellation を二重加算していた。
            if within<STEPS:
      ...
      if cross_stripe_safe:
        if not validate_reordered_permutation("stripe_reorder",constellations,work_constellations):
          return
      if gpu_sort_mode==5 or gpu_sort_mode==8 or gpu_sort_mode==10:
        ...
        local_sort_mode=0
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk73={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} local_sort={local_sort_mode} stripe={1 if stripe_chunks else 0} balanced={1 if balanced_stripe else 0} cross={1 if cross_stripe else 0} cross_safe={1 if cross_stripe_safe else 0} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    ...
    while off < m_all:
      ...
      if gpu_log_level>=2:
        t3=datetime.now()
        print(f"[gpu-chunk] N={N} chunk={chunks} off={off} m={m} grid={GRID} sort={gpu_sort_mode}/{local_sort_mode} build={str(t1-t0)[:-3]} sort_time={str(ts1-ts0)[:-3]} kernel={str(t2-ts1)[:-3]} sum={str(t3-t2)[:-3]}")
      off += m

    if m_all>0:
      constellations[0]["solutions"] = gpu_total
    if gpu_log_level>=1:
      print(f"[gpu-summary] N={N} constellations={m_all} chunks={chunks} direct_total=1 stripe={1 if stripe_chunks else 0} balanced={1 if balanced_stripe else 0} cross={1 if cross_stripe else 0} cross_safe={1 if cross_stripe_safe else 0}")
    return
  ##########
  # CPU 
  ##########
...
def main()->None:
  ...
  bench_mode:int=0  # 0:normal, 1:N=20 warmup repeat, 2:N19 preheat, 3:N18+N19 preheat, 4:N20 repeat3 sweep, 5:N20 repeat2 benchmark
  # 72 STABLE FINAL BENCH: preset_queens は 5 固定。preset=6 は 48 で正解には直せたが速度面で不採用。
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_block gpu_max_blocks log_level sort_mode] [preset_queens] [bench_mode] [cross_stripe_safe]")
      return
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 73 STABLE FINAL BENCH; using 5")
      preset_queens_arg=5
    if argc >= 10:
      bench_mode=int(sys.argv[9])
      if bench_mode<0 or bench_mode>5:
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        if bench_mode==1 and use_gpu and N==20:
          if gpu_log_level>=1:
