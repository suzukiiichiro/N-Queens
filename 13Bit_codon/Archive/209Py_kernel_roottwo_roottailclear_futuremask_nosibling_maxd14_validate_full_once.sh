...
# =============================================================================
# NQ_UPDATE_MEMO
# 206: 更新メモ: 205Py rootafterinlineは正当性OKながら204Pyより0.279秒遅いため不採用。209Pyは204Py相当へ戻し、MAXD14 kernelのみroot_after_second一時scalarを復元する安全復帰版を検証する。
# Full update history: see README.md
# =============================================================================

# 209 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 209 returns to
# validated 204/206 because 208 root-preroll terminal-first was correct but
# slower. 207/208 and 198/199/200/201/202/203/205 were correct but not adopted.
#
...
# 204 clear-lowbit root_rest plus root_tail clear-lowbit predicate:
# root_tail = root_rest & (root_rest - 1); root_tail==0 means root has one or
# two candidates. Host task order, dispatch, cache, progress verification, and
# README-managed history remain unchanged.
#
# Shell policy:
#   - STATIC_ONLY=1 does not take the full-run lock.
#   - source static-check failure prints the summary before exiting.
#   - roottailclear source check is whitespace/comment robust.
SRC=${SRC:-./209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 209 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/209Py_kernel_roottwo_roottailclear_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 209Py source
#   has a harmless header/string/layout difference. This 209 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_tail clear-lowbit root<=2 predicate	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
checks = {
    'root_clearrest_present': 'root_rest:u32=cur_avail&(cur_avail-u32(1))' in root_block,
    'root_tail_present': 'root_tail:u32=root_rest&(root_rest-u32(1))' in root_block,
    'root_tail_if_present': 'ifroot_tail==u32(0):' in root_block,
    'root_xor_rest_removed': 'root_rest:u32=cur_avail^root_first' not in compact,
    'root_after_scalar_absent': 'root_after_second:u32=root_rest^root_second' not in root_block,
    'root_second_absent': 'root_second:u32=' not in root_block,
    'root_inline_if_absent': 'if(root_rest^root_second)==u32(0):' not in root_block,
    'root_preroll_flag_absent': 'root_preroll:u32' not in s,
    # 209 keeps 204/206 save payload; 203 depth0-save is not adopted.
    ...
    # 209 keeps 204/206 payload source; 202 root_second payload is not adopted.
    ...
    # 209 keeps 204/206 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 209 keeps 204/206 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
    ...
    'tailtest_old_name_absent': 'root_rest_more:u32' not in s,
    # 208 terminal-first and 207 pr_descend_nf are not adopted.
    'pr_terminal_after_future_or_absent': not (root_block.find('ifterminal_depth==0:') >= 0 and root_block.find('iffuture_check_mask!=u32(0):') >= 0 and root_block.find('ifterminal_depth==0:') < root_block.find('iffuture_check_mask!=u32(0):')),
    'pr_descend_nf_absent': 'pr_descend:u32=pr_nf' not in root_block,
    'pr_descend_one_present': 'pr_descend:u32=u32(1)' in root_block,
}
checks['eqsecond_active_absent'] = 'ifroot_rest==root_second:' not in compact
if all(checks.values()):
    sys.exit(0)
print('source_roottailclear_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_roottailclear	root_tail clear-lowbit predicate	root_tail clear-lowbit predicate	OK
' >> "$SUMMARY"
else
  actual_tail=$(python3 - "$SRC" <<'PYTAIL2'
import sys
...
root_block=compact[root_pos:gen_pos if gen_pos >= 0 else len(compact)]
items=[]
if 'root_tail:u32=root_rest&(root_rest-u32(1))' in root_block: items.append('root_tail')
if 'ifroot_tail==u32(0):' in root_block: items.append('if_root_tail')
if 'root_after_second:u32=root_rest^root_second' in root_block: items.append('root_after_second_still_present')
print(','.join(items) if items else 'missing')
PYTAIL2
)
  printf 'source_roottailclear\troot_tail clear-lowbit predicate\t%s\tFAIL
' "$actual_tail" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split209'}
bad=sorted(tags-allowed)
if 'split209' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split209 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split209 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 209 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 209 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-roottailclear static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 209 roottwo-roottailclear futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 209 roottwo-roottailclear futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
