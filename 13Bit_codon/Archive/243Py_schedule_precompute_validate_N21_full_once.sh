...
# =============================================================================
# 243 N=21 full validation harness
# Parent: 241 restore239-after-fid14split-reject.
# Experiment: keep single launch and move the MAXD14 schedule-generation
#             interpreter from gpu.kernel to host-side precomputed arrays.
...
SRC=${SRC:-./243Py_schedule_precompute_probe.py}
CAND=${CAND:-./243Py_schedule_precompute_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/243Py_schedule_precompute_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/243Py_schedule_precompute_logs_N21_full_once_${TS}"
...
echo "[start] 243 schedule-precompute validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 243 schedule-precompute SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '243 schedule-precompute probe' "$SRC"; then
  printf 'source_version_tag\t243 schedule-precompute probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t243 schedule-precompute probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split243_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split243 active; split240/241/242 runtime tags absent', 'split243', True))
else:
    checks.append(('source_split_tag', 'split243 active; split240/241/242 runtime tags absent', 'old split runtime tag present or split243 missing', False))
...
if not m14:
    ...
    ok = (
        ...
        'schedule_fu:u32=' not in body
    )
    actual = 'precompute active' if ok else 'old schedule interpreter present or precompute loads missing'
    checks.append(('source_schedule_precompute', 'MAXD14 precompute active', actual, ok))
...
host_ok = (
    ...
    'gpu.raw(soa.sched_lo_arr)' in s
)
checks.append(('source_host_precompute_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 243 schedule-precompute mode31 split145"
