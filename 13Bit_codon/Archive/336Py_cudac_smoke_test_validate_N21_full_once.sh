...
# =============================================================================
# 336 N=21 full validation harness (cudac-smoke-test)
# Base: 335/334/333/328-332 lineage (adopted SoA baseline + w3_j7 reorder
#   default; kernel logic and dispatcher byte-for-byte identical
#   throughout). 336 makes NO source change to the main solver at all --
#   kernel, dispatcher, and host reorder parameters (w3_j7, K=48) are all
#   unchanged from 335/334/333. The only .py content is an Open
#   Objectives #6 update. The new deliverable this revision is a
#   STANDALONE artifact outside this harness: 336_cudac_smoke_test.cu +
#   336_cudac_smoke_test_check.sh (see those files; not part of this
#   N=21 validate at all).
#
# 335 RESULT (now confirmed, used as this run's expected-match baseline):
#   correctness 314666222712, elapsed 450.218s (334's 450.181s, -0.008%,
#   within noise). All static checks OK. Toolchain recon now SETTLED:
#   nvcc = /usr/local/cuda/bin/nvcc, CUDA 13.0 (V13.0.88); cuobjdump
#   NOT found on PATH nor inside nvcc's own directory -- this CUDA 13.0
#   install genuinely lacks cuobjdump (325/327's past cuobjdump use was
#   presumably a different environment, or actually the ncu fallback
#   both of those probes also documented). A10G compute_cap = 8.6, so
#   -arch=sm_86 is the confirmed build flag.
#
# 336's NEW ARTIFACT (first concrete step of buried-idea C-2, the rev84
# CUDA C runner proposal -- the sole remaining route to Open Objective
# #3 / tail effect now that plan C-1 is closed per 330): a standalone,
# self-verifying .cu smoke test that builds with
# `nvcc -arch=sm_86 -o 336_cudac_smoke_test 336_cudac_smoke_test.cu`
# and runs on the GPU, containing NO DFS logic and NO bin-file
# interaction -- purely a compile/link/execute round-trip check before
# any bin-format reader or maxd14 port work begins. Run it via
# `bash 336_cudac_smoke_test_check.sh` (separate from this harness;
# it DOES touch the GPU, unlike 334/335's probes). If it fails, that is
# itself valuable design information and does not affect the N=21
# validation below in any way.
#
...
#   3. Compare timing against 335 (450.218s) -- expect reproduction
#      within noise, since nothing in the solver changed.
#   4. Separately (order independent, does not gate the above):
#      `bash 336_cudac_smoke_test_check.sh` to exercise the new .cu
#      toolchain smoke test.
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./336Py_cudac_smoke_test.py}
CAND=${CAND:-./336Py_cudac_smoke_test}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/336Py_cudac_smoke_test_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/336Py_cudac_smoke_test_logs_N21_full_once_${TS}"
...
echo "[start] 336 cudac-smoke-test validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 336 cudac-smoke-test SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '336 cudac-smoke-test' "$SRC"; then
  printf 'source_version_tag	336 cudac-smoke-test	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	336 cudac-smoke-test	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 336 cudac-smoke-test (kernel-identical to 335/334/333/328) mode31 split145"
...
echo "[validation-ok] 336 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); NO solver source change in 336 (kernel/dispatcher/host params identical to 335/334/333/328-332, w3_j7 remains default); expected elapsed ~450.2s matching 335s own measurement 450.218s -- see the standalone 336_cudac_smoke_test.cu / 336_cudac_smoke_test_check.sh (run separately, NOT part of this harness) for the first concrete CUDA C toolchain smoke test using the confirmed nvcc path and -arch=sm_86"
