...
$ suzuki@cudacodon$ codon build -release 62Py_constellations_GPU_cuda_codon_striped_only_auto_32x484.py
...
15:           2279184                0          0:00:00.040    ok
16:          14772512                0          0:00:00.106    ok
17:          95815104                0          0:00:00.469    ok
18:         666090624                0          0:00:02.786    ok
19:        4968057848                0          0:00:20.678    ok
20:       39029188884                0          0:02:30.981    ok
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  if use_gpu:
    # 61 STRIPED CHUNKS:
    #   60 のログでは chunk ごとの kernel 時間が 15.8〜20.0秒程度にばらついた。
    #   build/sort/sum はほぼ 0 秒なので、chunk 内外の仕事量偏りが支配的。
    #   sort_mode=5: 元の chunk 群を縦方向にストライプ化して、新しい各 chunk に
    #                元chunk 0..last の要素が混ざるようにする。さらに chunk 内 sort4 を適用。
    #   sort_mode=6: ストライプ化のみ。
    #   解数は加算なので、direct_total では元 index への scatter は不要。
    stripe_chunks:bool=(gpu_sort_mode==5 or gpu_sort_mode==6)
    ...
    if stripe_chunks:
      ...
      if gpu_sort_mode==5:
        ...
        local_sort_mode=0
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk62={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} local_sort={local_sort_mode} stripe={1 if stripe_chunks else 0} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    ...
    if gpu_log_level>=1:
      print(f"[gpu-summary] N={N} constellations={m_all} chunks={chunks} direct_total=1 stripe={1 if stripe_chunks else 0}")
    return
  ##########
  # CPU 
  ##########
...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  # 62 STRIPED ONLY AUTO: preset_queens は 5 固定。preset=6 は 48 で正解には直せたが速度面で不採用。
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 62 STRIPED ONLY AUTO; using 5")
      preset_queens_arg=5
    if argc > 9:
