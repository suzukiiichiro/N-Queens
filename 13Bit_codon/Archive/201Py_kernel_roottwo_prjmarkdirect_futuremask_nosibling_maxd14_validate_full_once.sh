...
# =============================================================================
# NQ_UPDATE_MEMO
# 201: 更新メモ: 200は正当性OKだが197比で遅いため不採用。197 root<=2 direct-if prerollを親に戻し、MAXD14 kernelのみroot-preroll内child_jmark bit0判定から一時scalarを削る微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 201 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 201 starts from
# validated 197 because 198/199/200 were correct but did not beat the
...
# second-lowbit direct-if form. Only root-preroll child_jmark handling changes:
# child_jmark_mask bit0 is tested directly instead of materializing a
# pr_child_jmark scalar. Host task order, dispatch, cache, progress
...
#   - root-preroll child_jmark-direct source check is whitespace/comment robust.
SRC=${SRC:-./201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 201 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/201Py_kernel_roottwo_prjmarkdirect_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 201Py source
#   has a harmless header/string/layout difference. This 201 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 197 root one/two-candidate direct-if futuremask/no-sibling MAXD14 dispatch; 198 tailtest, 199 eqsecond, and 200 prfuturebit correct but not adopted; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root-preroll child_jmark direct bit0 test	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
checks = {
    ...
    'direct_child_jmark_bit0': 'if(child_jmark_mask&u32(1))!=u32(0):' in compact,
    'pr_child_jmark_scalar_absent': 'pr_child_jmark:u32' not in s,
    # 201 returns to 197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
    ...
    'eqsecond_absent': 'root_rest==root_second' not in s,
...
PYCHECK
then
  printf 'source_rootpr_jmarkdirect	present_direct_child_jmark_bit0	present_direct_child_jmark_bit0	OK
' >> "$SUMMARY"
else
  printf 'source_rootpr_jmarkdirect	present_direct_child_jmark_bit0	missing_or_old_pr_child_jmark_scalar	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split201'}
bad=sorted(tags-allowed)
if 'split201' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split201 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split201 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 201 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 201 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-prjmarkdirect static checks failed" >&2
  exit 66
fi
