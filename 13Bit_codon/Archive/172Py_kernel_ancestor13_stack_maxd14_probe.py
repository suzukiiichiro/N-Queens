...
171Pyログを確認しました。**問題ありません**。

```text
171Py : 0:08:11.231
170Py : 0:09:18.483
差分  : 171Pyが 67.252秒高速、約12.042%改善
```
...
したがって、**171Pyを現時点の最速基準**として扱ってよいです。

次の172Pyは予定どおり、171Pyを親にして **MAXD14のancestor stackを1段縮小**しました。active/current frameは171Py同様にレジスタ保持のままです。

ファイルはこちらです。

* [172Py_kernel_ancestor13_stack_maxd14_probe.py](sandbox:/mnt/data/172Py_kernel_ancestor13_stack_maxd14_probe.py)
* [172Py_kernel_ancestor13_stack_maxd14_validate_full_once.sh](sandbox:/mnt/data/172Py_kernel_ancestor13_stack_maxd14_validate_full_once.sh)

172Pyの主な変更点です。

```text
171Py:
  MAXD14 active frame = register
  saved ancestor/state arrays = u32[14] x 4
  stack_bytes_per_thread = 224

172Py:
  MAXD14 active frame = register
  saved ancestor arrays = u32[13] x 4
  stack_bytes_per_thread = 208
```

理由は、N=21のMAXD14 scheduleでは active frame が最大 `sp=13` まで到達しますが、その `sp=13` は terminal parent frame であり、そこからさらに descend しないためです。したがって保存が必要な親frameは `sp=0..12` の13段だけになります。

```text
MAXD14 logical schedule depth : 14
active frame max sp           : 13
deepest saved ancestor index  : 12
local arrays                   : 13 slots
bytes/thread                   : 4 arrays * 13 * 4 = 208
```
...
maxd14_ancestor13_stack_schedule         1 / 1 OK
frame_opcode_equivalence                506604 cases OK
dfs_164_vs_172_ancestor13_equivalence   12000 cases OK
...
```bash
chmod +x 172Py_kernel_ancestor13_stack_maxd14_validate_full_once.sh
./172Py_kernel_ancestor13_stack_maxd14_validate_full_once.sh
```

この172Pyで速くなれば、次は **173Py = frame entry時metadataを一度だけdecode** に進むのが自然です。


"""

# 172 GPU ancestor-stack-shrink experiment over 171 current-frame register MAXD14:
...
# 172 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 172 MAXD14 uses scalar u32 schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="172 ancestor13 stack shrink for MAXD14 over 171 current-frame register - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 172 inherits the exact 171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """172 MAXD14 scalar schedule plus register-held current frame and 13-slot ancestor stack."""
    ...
    # 172 current-frame register cache plus ancestor stack shrink.  The active
...
####################################################################################################
# 172 ancestor-stack-shrink MAXD14 + scalar-u32pair schedule harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 171 scorestripe_v9 task order/cache and current-frame register MAXD14; 172 shrinks saved ancestor stack to 13"
...
def main()->None:
  ...
  # 172 branches from validated 171.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 172 ancestor13 stack GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split172_ancestor13_stack_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split172_ancestor13_stack_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split172-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split172-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split172_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split172-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split172-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split172_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
