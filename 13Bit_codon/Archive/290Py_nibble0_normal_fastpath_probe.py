...
# =============================================================================
# NQ_UPDATE_MEMO
# 290: nibble0-normal-fastpath。289のnormaldefault nld/nrd + ncol-only earlyを親に、MAXD14 generic loopで nibble_op==0 の normal/no-future 最頻pathを先頭fast path化する実験版。
#      nibble_op==0 では block_code作成・special分岐・future_check_mask判定を通らず、
#      nf/terminal/child_jmark/save-descend へ直接進む。nibble_op==8 normal future pathと
#      special pathは289相当を維持する。nf default、placed_ld/placed_rd共通化、
...
今回は **289を親にした `nibble_op==0` normal/no-future fast path 実験版**です。
MAXD14 generic loop内だけを変更し、`nibble_op==0` の場合は `block_code` 作成・special branch・future checkを通らず、`nf → terminal → child_jmark → save/descend` へ直接進む形にしました。`nibble_op==8` のnormal future pathとspecial pathは289相当を維持しています。

`STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [290Py_nibble0_normal_fastpath_probe.py](sandbox:/mnt/data/290Py_nibble0_normal_fastpath_probe.py)
* [290Py_nibble0_normal_fastpath_validate_N21_full_once.sh](sandbox:/mnt/data/290Py_nibble0_normal_fastpath_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [290Py_nibble0_normal_fastpath_sha256.txt](sandbox:/mnt/data/290Py_nibble0_normal_fastpath_sha256.txt)
* [290Py_nibble0_normal_fastpath_bundle_20260709.tar](sandbox:/mnt/data/290Py_nibble0_normal_fastpath_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./290Py_nibble0_normal_fastpath_probe

STATIC_ONLY=1 bash 290Py_nibble0_normal_fastpath_validate_N21_full_once.sh

bash 290Py_nibble0_normal_fastpath_validate_N21_full_once.sh
```

採否は、まず正当性OK、そのうえで **289の `0:07:04.097`、287の `0:07:04.486`、286単発最速の `0:07:04.033`** と比較するのがよいです。
...
VERSION_TAG:str="290 nibble0-normal-fastpath: parent 287/286 normaldefault-nldnrd; MAXD14 generic loop computes normal-path nld/nrd as defaults and also computes ncol before the block_code special branch; nf remains after the branch and is not defaulted; no nfdefault, no placed_ld/placed_rd commonization, root-preroll and host shaping/cache/dispatch preserved"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while True:
      ...
      # 290: keep 289 normal-default nld/nrd+ncol-only and fast-path nibble_op==0.
      # nibble_op==0 is normal/no-future, so skip block_code/special/future checks.
      nld:u32=(cur_ld|bit)<<u32(1)
      nrd:u32=(cur_rd|bit)>>u32(1)
      ncol:u32=cur_col|bit

      if nibble_op==u32(0):
        fast_nf:u32=bm&~(nld|nrd|ncol)
        if fast_nf==u32(0):
          continue
        ...
        fast_next_depth:int=cur_depth+1
        if cur_avail!=u32(0):
          ld[save_sp]=cur_ld
          rd[save_sp]=cur_rd
          col[save_sp]=cur_col
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split290_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split290_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split290-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split290-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split290_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
