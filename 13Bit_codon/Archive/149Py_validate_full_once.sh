...
# 149 single-pass validation harness
#
...
SRC=${SRC:-./149Py_ctrl_u32_decode_probe.py}
CAND=${CAND:-./149Py_ctrl_u32_decode_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/149Py_nqueens_gpu_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Historical mean of the two validated 148 full runs (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 149 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/149Py_logs_N21_full_once_${TS}"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  PERF=$(awk -v base="$BASELINE_148_SECONDS" -v now="$ELAPSED_SECONDS" 'BEGIN {
    delta=base-now
    pct=(base>0)?(delta/base*100):0
    printf "elapsed=%s baseline=%s delta=%+.3f percent=%+.3f%%", now, base, delta, pct
  }')
  printf 'timing_info\t148_mean=%ss\t%s\tINFO\n' "$BASELINE_148_SECONDS" "$PERF" >> "$SUMMARY"
fi
...
echo "[validation-ok] one full run reproduced cases 01-06"
