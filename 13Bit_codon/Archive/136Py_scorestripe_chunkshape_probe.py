...
確認しました。`153Py1-6_locked.sh` は、添付内容から **135Py1-6_locked.sh** のこととして確認しました。

**135Py / 135Py locked はどちらも問題ありません。**

```text
135Py1-6.sh:
  01 OK  partial_total=16867221674  elapsed=0:01:10.667
  02 OK  partial_total=19226531218  elapsed=0:01:16.670
  03 OK  partial_total=16935383998  elapsed=0:01:07.626
  04 OK  partial_total=16231974232  elapsed=0:01:06.354
  05 OK  partial_total=79383407310  elapsed=0:05:23.994
  06 OK  total=314666222712        elapsed=0:21:03.390

135Py1-6_locked.sh:
  01 OK  partial_total=16867221674  elapsed=0:01:08.797
  02 OK  partial_total=19226531218  elapsed=0:01:16.601
  03 OK  partial_total=16935383998  elapsed=0:01:07.621
  04 OK  partial_total=16231974232  elapsed=0:01:06.351
  05 OK  partial_total=79383407310  elapsed=0:05:23.642
  06 OK  total=314666222712        elapsed=0:21:02.492
```
...
[136Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/136Py_scorestripe_chunkshape_probe.py)
[136Py1-6.sh](sandbox:/mnt/data/136Py1-6.sh)
[136Py1-6_locked.sh](sandbox:/mnt/data/136Py1-6_locked.sh)

136 は **135 validated carry-forward** として切っています。

```text
136Py:
  135 から分岐
  scorestripe_v4_lanephase32 を継承
  kernel / DFS / generic-only path は変更なし
  chunkshape136 / split136 に分離
  136 用 cache 名に分離

LOGDIR:
  136Py_logs_N21_validation_${TS}
  136Py_logs_N21_locked_validation_${TS}

locked expected:
  135 locked の fixed partial_total を継承
```

実行順は以下です。

```bash
codon build -release 136Py_scorestripe_chunkshape_probe.py
./136Py1-6.sh
./136Py1-6_locked.sh
```

`136Py1-6.sh` / `136Py1-6_locked.sh` は実行権限付き、`bash -n` 確認済みです。Codon 実機ビルドは未実行です。


承知しました。136 実行中の間に、今後の段取りを整理しておきます。

まず、土台の考え方は **「安全な固定系」と「速度探索系」を分ける**のがよいです。130 は locked validation で 6 ケース PASS、full `314666222712 ok`、かつ 129 locked より 29.745 秒速いことが記録済みなので、引き続き **安全な基準点 / rollback anchor** として扱います。 130 系の locked script も、mode30 selected chunk の固定 `partial_total`、worker 0/4、full correctness を検査する形になっているため、今後の locked script もこの形式を踏襲するのが安全です。

## 直近: 136Py 実行後の処理

136 の `1-6.sh` と `1-6_locked.sh` が戻ってきたら、見る順番は以下です。
...
3. permutation / count safety
   - source_records=2025282
   - base_reordered_records=2025282
   - shaped_records=2025282

4. generic-only safety
   - split_mode=0
   - specialized=0
...
136 が PASS なら、次の判定はこうします。

```text
136 initial PASS + 136 locked PASS:
  136 を validated carry-forward として保存

136 locked full が 134/135 と同等:
  scorestripe_v4_lanephase32 系は安定と判断

136 locked full が 134/135 より明確に速い:
  136 を v4 系の代表値に昇格候補

136 locked full が少し遅い:
  correctness / reproducibility PASS なら問題なし
  実行揺れとして扱う
```

ここで重要なのは、**136 は carry-forward なので、速くても遅くてもアルゴリズム差とは見ない**ことです。134/135/136 は `scorestripe_v4_lanephase32` の再現性確認系列として扱い、3 本そろったところで v4 系の安定度を判断します。

## 次の大きな節目

現時点の流れでは、節目は次の 3 段階です。

```text
第1節目:
  130
  scorestripe_v2
  安全 baseline / rollback anchor

第2節目:
  133
  scorestripe_v3_lanephase16
  lanephase 導入の実験成功点

第3節目:
  134/135/136
  scorestripe_v4_lanephase32
  現在の高速 experimental baseline 候補
```

130 は、129 の bucket 退化を直して、score bucket をほぼ 8 等分にした点が重要でした。130 の主変更は kernel を触らず、host-side chunkshape で percentile score-aware shaping を追加したことです。
133 以降は、その host-side shaping の中で **bucket 内 lane phase** を触っている段階です。

したがって今後は、kernel 本体を変えるより先に、もう少し **host-side permutation / chunkshape** を詰めるのがよいです。

## 137Py の推奨方針

136 が問題なく通った場合、137 は carry-forward ではなく、**134/135/136 の v4_lanephase32 からの最小差分実験**にするのがよいです。
...
変更するもの:
  bucket 内 32-lane の phase rotation seed / phase jump だけ
...
変更しないもの:
  ...
  8 bucket quota
  mode30 / mode31 harness
  generic-only path
```

狙いは、134 の 32 lane 化で良くなった流れを維持しつつ、chunk ごとの lane 開始位置を少し変えて、重い lane の局所連続をさらに散らすことです。

イメージとしては以下です。

```text
134:
  lane = deterministic 32-lane key
  chunkごとに phase rotate

137:
  lane = same 32-lane key
  phase jump / start lane / bucket offset だけ変更
```

つまり、137 では **lane 数を増やさない**方がよいと思います。すぐ 64 lane にすると、改善しても「lane 数の効果」なのか「phase の効果」なのか分かりにくくなります。まず 32 lane のまま phase だけ変え、差分を小さく保つのが安全です。

## 138Py 以降の候補

137 が良ければ、次に試す順番はこうです。

```text
138Py:
  scorestripe_v6_lanephase32_quota_carry
  bucket quota の余り配分を chunk 間で持ち越す

139Py:
  scorestripe_v7_lanephase64
  32 lane から 64 lane へ拡張

140Py:
  scorestripe_v8_tail_smooth
  終盤 chunk 120〜130 付近の偏りを軽くならす

141Py:
  best-of carry-forward
  137〜140 の最良版をラベル/cache分離して再現確認
```
...
理由は、今の成果は **kernel を触らずに host-side chunk shaping だけで改善している**点が強みだからです。kernel / DFS / special kernel へ踏み込むと、correctness リスクと検証コストが一気に上がります。

## 採用・保留・reject の基準

今後は、各候補を次の基準で分けるのがよいです。
...
採用候補:
  ...
  locked PASS
  total=314666222712
  source_records == base_reordered_records == shaped_records == 2025282
  generic-only 維持
  直近 best locked より 2〜3 秒以上速い

保留:
  correctness PASS
  locked PASS
  速度差が ±1 秒程度
  → 実行揺れとして validated branch に保存

reject:
  total 不一致
  partial_total locked 不一致
  record count 不一致
  split_mode / specialized が意図せず変化
  full が 5 秒以上悪化
```

特に `partial_total` 不一致は、130 locked 時点でも「chunkshape / 入力 cache / binary の取り違えを疑う」扱いにしていました。 この運用は 136 以降も継続でよいです。

## ファイル命名・ログ命名

132 以降で決めた新規則を、そのまま固定でよいです。

```text
source:
  137Py_scorestripe_chunkshape_probe.py
...
validation logdir:
  137Py_logs_N21_validation_${TS}

locked logdir:
  137Py_logs_N21_locked_validation_${TS}

memo:
  143_137_validation_review_memo.txt
  144_137_locked_validation_review_memo.txt

tar:
  137Py_validated_experiment_20260616.tar
  137Py_locked_validation_20260616.tar
```

フォルダ名は必ず candidate 番号を先頭にして、

```text
137Py_logs_N21_validation_YYYYMMDD_HHMMSS
137Py_logs_N21_locked_validation_YYYYMMDD_HHMMSS
```
...
毎回 full を走らせると時間が重いので、実験段階では以下の流れがよいです。

```text
新しいアルゴリズム変更あり:
  1. RUN_FULL=0 で 01〜05 quick check
  2. 問題なければ full ありで 1-6
  3. PASS したら locked script 作成
  4. locked full 実行
...
ただし、**採用判断には full が必須**です。`RUN_FULL=0` はあくまで早期 reject 用です。
...
```text
避ける:
  kernel_dfs_iter_gpu の変更
  special kernel 追加
  split_mode の変更
  DFS 分岐 logic の変更
  score key weight の大幅変更
  bucket 数 8 → 16 の変更
  N22 full の頻繁な実行
```

N22 は最終確認としては重要ですが、1 回が重いので、137〜140 の中で本当に有望なものが出てからで十分です。
...
```text
今:
  136 initial / locked 実行中

136 ログ受領後:
  136 PASS 判定
  134/135/136 の v4_lanephase32 再現性を整理
  136 review memo / tar 作成

次:
  137Py = scorestripe_v5_lanephase32_phasejump
  32 lane は維持
  phase jump だけ最小差分変更
...
節目:
  137〜140 の best を 141 carry-forward として固定
  その後、必要なら N22 spot check
```

現時点では、**130 を安全 baseline として残しつつ、134/135/136 の v4_lanephase32 系を高速 experimental baseline として育てる**のが一番よい流れです。
...
VERSION_TAG:str="136 scorestripe v4 lanephase32 carry-forward - mode30/mode31 generic kernel, host-side 32-lane score-bucket shaping"
...
####################################################################################################
# 136 reorder chunkshape harness
#
...
####################################################################################################
# 136 score-aware chunk-shape layer
# 136 is a validated carry-forward of the 135 scorestripe_v4_lanephase32 result with isolated cache/log labels.
#
# Safety boundary:
...
#
# scorestripe_v4_lanephase32 keeps the v2 percentile score bucket calculation, then changes only the
# bucket-internal extraction order:
#   v2: bucket[b] FIFO in the proven 128 cross-stripe order
#   v4: bucket[b][lane0..lane31], with output chunk / bucket dependent lane start rotation
####################################################################################################

CHUNKSHAPE136_REORDER_VERSION:str="scorestripe_v4_lanephase32"
CHUNKSHAPE136_DEFAULT_REASON:str="validated carry-forward of 135 scorestripe_v4_lanephase32 with isolated chunkshape136 cache; generic kernel unchanged"


def chunkshape136_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape136_{CHUNKSHAPE136_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}.bin"


CHUNKSHAPE136_SCORE_KEY_MAX:int=32767
CHUNKSHAPE136_LANE_COUNT:int=32
CHUNKSHAPE136_LANE_MASK:int=31


def chunkshape136_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  136 score key for host-side shaping only.

  The raw score and percentile thresholds are carried forward from 130/131/132 scorestripe_v2.
  This keeps the heavy/light bucket distribution comparable with the validated baseline.  The new
  v4 behavior uses the low 5 bits of the deterministic tie component as a bucket-internal lane.
  ...
  if key<0:
    key=0
  if key>CHUNKSHAPE136_SCORE_KEY_MAX:
    key=CHUNKSHAPE136_SCORE_KEY_MAX
  return key


def chunkshape136_build_thresholds_from_hist(hist:List[int],total_records:int)->List[int]:
  """Build 8 percentile thresholds from the score-key histogram."""
  thresholds:List[int]=[0]*8
  if total_records<=0:
    thresholds[7]=CHUNKSHAPE136_SCORE_KEY_MAX
    return thresholds

  b:int=0
  cum:int=0
  k:int=0
  while k<=CHUNKSHAPE136_SCORE_KEY_MAX:
    cum+=hist[k]
    while b<7:
  ...
  while b<7:
    thresholds[b]=CHUNKSHAPE136_SCORE_KEY_MAX
    b+=1
  thresholds[7]=CHUNKSHAPE136_SCORE_KEY_MAX
  return thresholds


def chunkshape136_bucket_from_key(key:int,thresholds:List[int])->int:
...
def chunkshape136_lane_from_key(key:int)->int:
  """Map a score key to a deterministic lane 0..31 inside its score bucket."""
  lane:int=key&CHUNKSHAPE136_LANE_MASK
  if lane<0:
    lane=0
  if lane>=CHUNKSHAPE136_LANE_COUNT:
    lane=lane&CHUNKSHAPE136_LANE_MASK
  return lane


def chunkshape136_make_quotas(rem_counts:List[int],total_remaining:int,m_target:int)->List[int]:
...
def build_chunkshape136_reordered_bin(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  """
  136 host-side score-aware lanephase chunk shaping.

  Carry-forward from 135: no DFS, kernel, score-key, quota, or lanephase extraction change.
  ...
  Output:
    ...
    to a percentile score bucket using the validated v2 score key.  It then keeps the 128
    cross-stripe traversal order, but splits each bucket into 32 deterministic lanes and rotates
    the lane start phase per output chunk / bucket.
  ...
  out_fname:str=chunkshape136_reorder_output_fname(N,preset_queens,BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_chunkshape136_{CHUNKSHAPE136_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)}_sim.tsv"

  if gpu_log_level>=1:
    print(f"[chunkshape136-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE136_DEFAULT_REASON}")

  with open(progress_fname,"w") as pf:
    lane_header:str=""
    lane_i:int=0
    while lane_i<CHUNKSHAPE136_LANE_COUNT:
  ...
  if len(data)<expected_bytes:
    print(f"[chunkshape136-warning] source byte count shorter than expected: bytes={len(data)} expected={expected_bytes}")
  ...
  key_hist:List[int]=[0]*(CHUNKSHAPE136_SCORE_KEY_MAX+1)
  off:int=0
  classify_chunk:int=0
  key_min:int=CHUNKSHAPE136_SCORE_KEY_MAX
  key_max:int=0
  while off<total_records:
    ...
    i=0
    while i<m:
      key:int=chunkshape136_score_key_from_soa(soa,i,off+i)
      if key<0:
        key=0
      if key>CHUNKSHAPE136_SCORE_KEY_MAX:
        key=CHUNKSHAPE136_SCORE_KEY_MAX
    ...
    if gpu_log_level>=2:
      print(f"[chunkshape136-classify-chunk] N={N} chunk={classify_chunk} off={off} m={m}")
    off+=m
    classify_chunk+=1

  score_thresholds:List[int]=chunkshape136_build_thresholds_from_hist(key_hist,total_records)
  bucket_counts:List[int]=[0]*8
  lane_counts:List[int]=[0]*CHUNKSHAPE136_LANE_COUNT
  idx_count:int=0
  while idx_count<total_records:
    b0:int=chunkshape136_bucket_from_key(score_key_by_idx[idx_count],score_thresholds)
    lane0:int=chunkshape136_lane_from_key(score_key_by_idx[idx_count])
    bucket_counts[b0]+=1
    lane_counts[lane0]+=1
    idx_count+=1

  if gpu_log_level>=1:
    print(f"[chunkshape136-score-key] N={N} records={total_records} key_min={key_min} key_max={key_max} t0={score_thresholds[0]} t1={score_thresholds[1]} t2={score_thresholds[2]} t3={score_thresholds[3]} t4={score_thresholds[4]} t5={score_thresholds[5]} t6={score_thresholds[6]}")
    print(f"[chunkshape136-score-buckets] N={N} records={total_records} b0={bucket_counts[0]} b1={bucket_counts[1]} b2={bucket_counts[2]} b3={bucket_counts[3]} b4={bucket_counts[4]} b5={bucket_counts[5]} b6={bucket_counts[6]} b7={bucket_counts[7]}")
    lane_counts_text:str=""
    lane_i:int=0
    while lane_i<CHUNKSHAPE136_LANE_COUNT:
      lane_counts_text+=f" l{lane_i}={lane_counts[lane_i]}"
      lane_i+=1
    print(f"[chunkshape136-lanes] N={N} records={total_records}{lane_counts_text}")
  ...
  flat:int=0
  while flat<(8*CHUNKSHAPE136_LANE_COUNT):
    one:List[int]=[]
      ...
      while src_ch<n_chunks:
        ...
        if within<STEPS:
          ...
          if idx<total_records:
            bucket:int=chunkshape136_bucket_from_key(score_key_by_idx[idx],score_thresholds)
            lane_val:int=chunkshape136_lane_from_key(score_key_by_idx[idx])
            bucket_lane_indices[bucket*CHUNKSHAPE136_LANE_COUNT+lane_val].append(idx)
            staged_total+=1
        src_ch+=1
      base+=1
    out_ch+=1
  ...
  if staged_total!=total_records:
    print(f"[chunkshape136-warning] staged count mismatch: staged={staged_total} source={total_records}")

  # ------------------------------------------------------------------
  # Stage 3: write output chunks with proportional score-bucket quotas.
  #          Inside each bucket, rotate the 32-lane start phase per chunk and bucket.
  # ------------------------------------------------------------------
  bucket_lane_pos:List[int]=[0]*(8*CHUNKSHAPE136_LANE_COUNT)
  bucket_lane_rem:List[int]=[0]*(8*CHUNKSHAPE136_LANE_COUNT)
  bucket_rem:List[int]=[0]*8
  b:int=0
  while b<8:
    lane:int=0
    while lane<CHUNKSHAPE136_LANE_COUNT:
      flat=b*CHUNKSHAPE136_LANE_COUNT+lane
  ...
  with open(out_fname,"ab") as out:
    while out_ch<n_chunks:
      ...
      quotas:List[int]=chunkshape136_make_quotas(bucket_rem,total_remaining,m_target)
      written_by_bucket:List[int]=[0]*8
      written_by_lane:List[int]=[0]*CHUNKSHAPE136_LANE_COUNT
      written_chunk:int=0
      ...
      while written_chunk<m_target:
        ...
        while oi<8:
          ...
          if quotas[b]>0:
            start_lane:int=(out_ch*7 + b*5 + written_by_bucket[b]*3) & CHUNKSHAPE136_LANE_MASK
            found:int=-1
            scan:int=0
            while scan<CHUNKSHAPE136_LANE_COUNT:
              lane=(start_lane+scan) & CHUNKSHAPE136_LANE_MASK
              flat=b*CHUNKSHAPE136_LANE_COUNT+lane
              ...
              scan+=1
            if found>=0:
              flat=b*CHUNKSHAPE136_LANE_COUNT+found
      ...
      if gpu_log_level>=2:
        ...
        while lane_i<CHUNKSHAPE136_LANE_COUNT:
          ...
          lane_i+=1
        print(f"[chunkshape136-build-chunk] N={N} chunk={out_ch} written={written_chunk} total_written={written_total} b0={written_by_bucket[0]} b1={written_by_bucket[1]} b2={written_by_bucket[2]} b3={written_by_bucket[3]} b4={written_by_bucket[4]} b5={written_by_bucket[5]} b6={written_by_bucket[6]} b7={written_by_bucket[7]}{lane_debug}")
      out_ch+=1
  ...
  if written_total!=total_records:
    print(f"[chunkshape136-warning] record count mismatch: written={written_total} source={total_records}")
  if reordered_records!=written_total:
    print(f"[chunkshape136-warning] output count mismatch: file_records={reordered_records} written={written_total}")
  if gpu_log_level>=1:
    print(f"[chunkshape136-build-summary] N={N} records={reordered_records} chunks={n_chunks} output={out_fname} progress={progress_fname} valid={1 if validate_bin_file(out_fname) else 0}")
...
def exec_solutions_gpu_chunk_split136(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  if gpu_log_level>=1:
    print(f"[split136-buckets] N={N} m={m} generic={m} d2base14={d2base14_m} d0={d0_m} rest=0 split_mode={split_mode} specialized=0")
...
def exec_solutions_gpu_bin_stream_split136(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split136_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split136_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split136_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split136_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split136-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split136-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split136-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split136-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split136(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split136-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split136-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split136 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split136 full run / optional multi-GPU worker
      # Examples:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split136-generic-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split136-generic worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split136_generic_harness_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split136_generic_harness_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  if gpu_log_level>=1 and (bench_mode==30 or bench_mode==31):
    print(f"chunkshape136_params: version={CHUNKSHAPE136_REORDER_VERSION} reason={CHUNKSHAPE136_DEFAULT_REASON}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split136-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split136-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      shaped_fname:str=chunkshape136_reorder_output_fname(N,preset_queens,gpu_block,gpu_max_blocks)
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape136-probe-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape136-probe-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape136_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split136_probe_{CHUNKSHAPE136_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split136(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split136-probe-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE136_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split136-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split136-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split136-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape136-full-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape136-full-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape136_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split136_full_{CHUNKSHAPE136_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split136(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split136-full-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE136_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
