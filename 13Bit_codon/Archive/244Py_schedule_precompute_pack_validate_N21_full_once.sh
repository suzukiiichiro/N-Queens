...
SRC=${SRC:-./244Py_schedule_precompute_pack_probe.py}
CAND=${CAND:-./244Py_schedule_precompute_pack_probe}
...
echo "[start] 244 schedule-precompute-pack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 244 schedule-precompute-pack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '244 schedule-precompute-pack probe' "$SRC"; then
  printf 'source_version_tag\t244 schedule-precompute-pack probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t244 schedule-precompute-pack probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if not m14:
    checks.append(('source_schedule_precompute_pack', 'MAXD14 packed precompute active', 'MAXD14 kernel not found', False))
else:
    body = m14.group(0)
    ok = (
        'sched_lo_arr:Ptr[u32]' in body and
        'sched_hi_term_arr:Ptr[u32]' in body and
        'sched_mask_arr:Ptr[u32]' in body and
        'schedule_lo:u32=sched_lo_arr[i]' in body and
        'hi_term:u32=sched_hi_term_arr[i]' in body and
        'mask_pack:u32=sched_mask_arr[i]' in body and
        'root_action:u32=(hi_term>>u32(30))&u32(3)' in body and
        ...
        'future_check_mask_arr' not in body
    )
    actual = 'packed precompute active' if ok else 'old interpreter or unpacked arrays present'
    checks.append(('source_schedule_precompute_pack', 'MAXD14 packed precompute active', actual, ok))
...
host_ok = (
    ...
    'gpu.raw(soa.sched_hi_term_arr)' in s and
    'gpu.raw(soa.sched_mask_arr)' in s and
    'self.sched_hi_arr:List[u32]' not in s and
    'self.root_action_arr:List[u32]' not in s
)
checks.append(('source_host_precompute_pack_arrays', '3 packed arrays present', 'present' if host_ok else 'missing or unpacked arrays present', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 244 schedule-precompute-pack mode31 split145"
...
echo "[validation-ok] 244 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
