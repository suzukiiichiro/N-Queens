...
# =============================================================================
# 314 N=21 full validation harness (power-cap-diagnosis)
# Parent: 313 clock-cap-diagnosis. `nvidia-smi -q -d CLOCK` confirmed the
#   clock-cap hypothesis numerically:
#     Applications Clocks : Graphics=1710 MHz  (== Default Applications
#                            Clocks 1710 MHz -- NOT an -ac override)
#     Max Clocks           : SM=1710 MHz
#     Clocks (actual)      : SM=1320 MHz
#   1710/1320 = 1.295, matching the observed +29.4% slowdown in 311-313
#   (454s vs 304's 351.070s) to within 0.1 percentage point.
# However: `sudo nvidia-smi -rgc` printed "All done." (success) but the
#   follow-up `nvidia-smi -q -d CLOCK` still showed Clocks.SM=1320 MHz,
#   unchanged. -rgc resets a guest-level `-lgc` clock lock; its having no
#   effect here rules out a simple guest-resettable lock. Two hypotheses
#   remain:
#     (a) a power limit below the card's default is capping achievable
#         clocks -- 312's telemetry showed only ~90-95W draw at 100%
#         utilization, well below A10G-class ~150W TDP, which is consistent
#         with a power cap;
#     (b) a host/hypervisor-level clock policy (common on virtualized/shared
#         GPU instances) that guest-level nvidia-smi cannot override at all.
# This revision: ZERO source changes from 313 (variant=2, K=48 unchanged).
#   The validate script extends the pre-run snapshot with
#   power.limit / power.default_limit / power.min_limit / power.max_limit,
#   and adds a gpu_power_cap_check that warns if power.limit is below
#   power.default_limit. If a gap is found, the documented (NOT
#   auto-executed) next manual step is:
#     sudo nvidia-smi -pl <power.default_limit value>
#   Re-run this validation afterward to see whether Clocks.SM recovers.
# Pragmatic stance: this script does not block indefinitely on unresolved
#   infrastructure. If the power-limit check also comes back clean (limit
#   already at default) or raising it does not change anything, ~454s under
#   the current 1320MHz clock state should be treated as this session's
#   interim practical baseline, and Stall Branch Resolving ncu
#   (--launch-count 1 --set SourceCounters) work should resume using this
#   environment -- the Stall Wait / Stall Branch Resolving percentages ncu
#   reports reflect architectural properties (dependency-chain length,
#   branch execution frequency) that remain informative even under a
#   different absolute clock, even though they are not strictly comparable
#   in absolute terms to 304's profile recorded at 1710MHz.
...
#           to again land near ~454s unless the power-limit adjustment (if
#           tried) changes the achievable clock.
...
SRC=${SRC:-./314Py_power_cap_diagnosis.py}
CAND=${CAND:-./314Py_power_cap_diagnosis}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/314Py_power_cap_diagnosis_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/314Py_power_cap_diagnosis_logs_N21_full_once_${TS}"
...
echo "[start] 314 power-cap-diagnosis validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 314 power-cap-diagnosis SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '314 power-cap-diagnosis' "$SRC"; then
  printf 'source_version_tag	314 power-cap-diagnosis	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	314 power-cap-diagnosis	missing	FAIL
...
TELEMETRY_PID=""
NVSMI_FIELDS="timestamp,temperature.gpu,clocks.current.sm,clocks.current.memory,clocks.max.sm,clocks.max.memory,clocks.applications.sm,clocks.applications.memory,power.draw,power.limit,power.default_limit,power.min_limit,power.max_limit,utilization.gpu,clocks_event_reasons.active"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 314 power-cap-diagnosis mode31 split145"
...
echo "[validation-ok] 314 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); see gpu_clock_cap_check and gpu_power_cap_check in summary.tsv, plus gpu_pre_run_snapshot.csv/gpu_telemetry.csv, for the clock/power-cap diagnosis"
