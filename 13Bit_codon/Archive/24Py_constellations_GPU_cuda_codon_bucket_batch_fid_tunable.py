...
def calc_gpu_bucket_id(
  ...
  # mode=1: functionid単位。funcptn=0 の巨大bucketを分解して分岐ダイバージェンスを下げる。
...
def exec_solutions(
  ...
  if use_gpu:
    ...
    GPU_BUCKETS:int=24 if GPU_BUCKET_MODE==0 else 56
      ...
      for bucket_id in range(GPU_BUCKETS):
        ...
        if GPU_LOG_BUCKETS:
          if GPU_BUCKET_MODE==0:
            ...
            fid:int=bucket_id>>1
            tail:int=bucket_id&1
            _,funcptn,avail_flag=func_meta[fid]
...
def main()->None:
  ...
  if argc == 1:
    ...
    # bucket_mode: 0=旧(funcptn/avail/tail), 1=FID/tail（既定）
