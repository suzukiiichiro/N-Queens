...
# =============================================================================
# 315 N=21 full validation harness (telemetry-fieldname-fix)
# Parent: 314 power-cap-diagnosis. The N=21 full run itself was fine in both
#   313 and 314 (313=454.419s, 314=454.424s, both within 0.002% of 312's
#   454.417s; correctness=314666222712 confirmed both times), but the GPU
#   telemetry/clock-cap/power-cap diagnostics added in 313/314 never actually
#   captured real data: the nvidia-smi --query-gpu field list included
#   `clocks.applications.sm`, which is NOT a valid nvidia-smi field name
#   (nvidia-smi rejects the entire query if any single field is invalid).
#   The actual content of gpu_pre_run_snapshot.csv / gpu_telemetry.csv in
#   both runs was just:
#     Field "clocks.applications.sm" is not a valid field to query.
#   Compounding this, the gpu_telemetry_captured check only checked that the
#   file had >=1 line, so it falsely reported OK on this error message.
# Fix (this revision, validate script only, zero source/kernel changes):
#   1. clocks.applications.sm -> clocks.applications.graphics (the correct
#      field name; matches "Applications Clocks: Graphics / Memory" as shown
#      by the earlier manual `nvidia-smi -q -d CLOCK` output, which has no
#      separate "SM" entry under Applications Clocks). This field's position
#      in NVSMI_FIELDS is unchanged, so the awk field indices used elsewhere
#      (CUR_SM=$3, MAX_SM=$5, POWER_LIMIT=$10, POWER_DEFAULT=$11) are still
#      correct and did not need updating.
#   2. gpu_telemetry_captured now validates that the first line of the CSV
#      actually starts with "timestamp" (the expected header), rather than
#      just counting lines, so a query error is caught as a hard FAIL
#      instead of silently passing.
# With this fix, gpu_clock_cap_check and gpu_power_cap_check (added in
#   313/314) should finally run against real data on the next full run.
# Also requested this round (manual, outside this script): run
#   `nvidia-smi -q -d POWER` directly for a fuller view of power.limit,
#   power.default_limit, and any constraints, since it is more readable than
#   the single-line CSV query and does not depend on this script's field list
#   being correct.
...
#           to again land near ~454s under the current clock/power state;
#           the fixed telemetry output is the primary new information from
#           this run, not the timing itself.
...
SRC=${SRC:-./315Py_telemetry_fieldname_fix.py}
CAND=${CAND:-./315Py_telemetry_fieldname_fix}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/315Py_telemetry_fieldname_fix_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/315Py_telemetry_fieldname_fix_logs_N21_full_once_${TS}"
...
echo "[start] 315 telemetry-fieldname-fix validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 315 telemetry-fieldname-fix SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '315 telemetry-fieldname-fix' "$SRC"; then
  printf 'source_version_tag	315 telemetry-fieldname-fix	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	315 telemetry-fieldname-fix	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 315 telemetry-fieldname-fix mode31 split145"
...
echo "[validation-ok] 315 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); see gpu_clock_cap_check and gpu_power_cap_check in summary.tsv, plus gpu_pre_run_snapshot.csv/gpu_telemetry.csv, for the (now-fixed) clock/power-cap diagnosis"
