...
# =============================================================================
# 278 N=21 full validation harness
# Parent: 276 restore274-after-bucketdiag-reject / 271 final-fastest-complete.
# Experiment: reject 277 depthu-childsave for speed and return to 276 baseline;
#             in MAXD14 only, create ZERO:u32=u32(0) and reuse it for simple
#             u32 zero initializations/assignments. Do not change u32(0)-x
#             lowbit extraction, comparisons, masks, root-preroll, dispatch,
#             cache generation, worker split, or one launch per chunk.
...
SRC=${SRC:-./278Py_zero_const_assign_probe.py}
CAND=${CAND:-./278Py_zero_const_assign_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
# 278: visible startup + force release rebuild by default so a previous non-release
# `codon build` executable is not reused and cannot trigger CUDA_ERROR_INVALID_PTX.
# 278: print start/status early and prevent static pycheck from failing silently under set -e.
FORCE_REBUILD=${FORCE_REBUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/278Py_zero_const_assign_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/278Py_zero_const_assign_logs_N21_full_once_${TS}"
...
echo "[start] 278 zero-const-assign validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 278 zero-const-assign SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '278 zero-const-assign' "$SRC"; then
  printf 'source_version_tag\t278 zero-const-assign\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t278 zero-const-assign\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split278' in s and 'split277' not in s and 'split276' not in s and 'split275' not in s and 'split274' not in s and 'split273' not in s and 'rootaction0-direct' not in s and 'kernel_dfs_iter_gpu_maxd14_root0' not in s and 'split270_' not in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split278 active; 277/275/273/270/240 rejected', 'split278', True))
else:
    checks.append(('source_split_tag', 'split278 active; 277/275/273/270/240 rejected', 'old/depthu/bucket/root0 split runtime present or split278 missing', False))
...
# 278: verify MAXD14 local ZERO reuse is active and 277 depth_u diff is absent
...
if not m14:
    checks.append(('source_zero_const_assign', 'present in MAXD14', 'MAXD14 kernel not found', False))
else:
    body = m14.group(0)
    ok = (
        'ZERO:u32=u32(0)' in body and
        'schedule_lo:u32=ZERO' in body and
        'frame_action:u32=ZERO' in body and
        'pr_nld:u32=ZERO' in body and
        'nibble_op:u32=ZERO' in body and
        'nld:u32=ZERO' in body and
        'cur_avail&(u32(0)-cur_avail)' in body and
        'root_second:u32=root_rest&(u32(0)-root_rest)' in body and
        'depth_u:u32=u32(cur_depth)' not in body and
        'child_jmark:u32=(child_jmark_mask>>u32(cur_depth))&u32(1)' in body and
        'avail[save_sp]=cur_avail|(u32(cur_depth)<<u32(27))' in body
    )
    checks.append(('source_zero_const_assign', 'MAXD14 ZERO reuse; depthu absent', 'present' if ok else 'missing/unsafe pattern', ok))
fail = 0
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 276 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 278 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 278 zero-const-assign mode31 split145"
...
echo "[validation-ok] 278 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
