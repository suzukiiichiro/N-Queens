...
# =============================================================================
# NQ_UPDATE_MEMO
# 207: 更新メモ: 206Pyは正当性OK。207Pyは204/206のroot_after_second scalar形を維持し、MAXD14 kernelのみroot-preroll内のpr_descend初期化をpr_nf由来にして、pr_nf==0分岐を削る微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 207 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 207 returns to
...
SRC=${SRC:-./207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 207 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 207Py source
#   has a harmless header/string/layout difference. This 207 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second scalar if + pr_descend initialized from pr_nf	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
checks = {
    ...
    # 207 keeps 204/197 save payload; 203 depth0-save is not adopted.
    ...
    # 207 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 207 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 207 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if python3 - "$SRC" <<'PYDESCEND'
import sys
s=open(sys.argv[1], encoding='utf-8').read()
compact=''.join(s.split())
root_pos=compact.find('207rootprdescendnf:')
if root_pos < 0:
    root_pos=compact.find('pr_nf:u32=bm&~(pr_nld|pr_nrd|pr_ncol)')
gen_pos=compact.find('whileTrue:', root_pos)
root_block=compact[root_pos:gen_pos if gen_pos >= 0 else len(compact)]
checks={
  'pr_descend_from_nf_present': 'pr_descend:u32=pr_nf' in root_block,
  'old_pr_descend_one_absent': 'pr_descend:u32=u32(1)' not in root_block,
...
if all(checks.values()):
    sys.exit(0)
print('source_rootprdescendnf_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYDESCEND
then
  printf 'source_rootprdescendnf	pr_descend initialized from pr_nf	pr_descend initialized from pr_nf	OK
' >> "$SUMMARY"
else
  actual_descend=$(python3 - "$SRC" <<'PYDESCEND2'
import sys
s=open(sys.argv[1], encoding='utf-8').read()
print('pr_descend=pr_nf' if 'pr_descend:u32=pr_nf' in ''.join(s.split()) else 'missing')
PYDESCEND2
)
  printf 'source_rootprdescendnf	pr_descend initialized from pr_nf	%s	FAIL
' "$actual_descend" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split207'}
bad=sorted(tags-allowed)
if 'split207' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split207 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split207 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 207 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 207 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootprdescendnf static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 207 roottwo-rootprdescendnf futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 207 roottwo-rootprdescendnf futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
