...
# =============================================================================
# NQ_UPDATE_MEMO
# 281: kernel-block-count diagnostic fix。281診断修正版の書き戻し不備を補修し、
#      279/276本線を親に、速度候補ではなく
#      MAXD14 generic DFS loopの論理ブロック別通過回数を診断する。
#      計算結果・task order・1 chunk 1 launch・rootrestlate・futuremask・no-sibling、
#      split145/chunkshape148、broadmarktail、cache生成、worker splitは維持する。
#      kernel内で loop/zero-avail/restore/block/future/terminal/child/save/descend
#      のper-thread countを別配列へ書き、chunkごとに合算してprogressへ出す。
#      277 depth_u、278 zero-const、275 bucket診断、273 root0 direct、
#      270 schedule precompute、240 fid14/rest別launch は入れない。
...
今回は **280の診断書き戻し不備だけを補修**しています。探索・加算ロジック、task order、1 chunk 1 launch、rootrestlate、futuremask/no-sibling、split145/chunkshape148、broadmarktail、cache生成、worker split は280/279/276相当のままです。

修正点は以下です。

```text id="7ct35c"
1. generic DFS loop終了後、results[i]=total*w_arr[i] の直前で diag_*_arr[i] へ必ず書き戻す
2. root_action==2 / root_action==3 / root_action==1後にroot_a==0 の早期returnでも診断配列へゼロ値を書いてからreturn
3. 検証シェルの [kernel-blockdiag] 抽出regexを修正
4. blockdiag_positive / progress_diag_positive が0のまま通らないように検証
```
...
* [281Py_kernel_block_count_diag_fix_probe.py](sandbox:/mnt/data/281Py_kernel_block_count_diag_fix_probe.py)
* [281Py_kernel_block_count_diag_fix_validate_N21_full_once.sh](sandbox:/mnt/data/281Py_kernel_block_count_diag_fix_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [281Py_kernel_block_count_diag_fix_sha256.txt](sandbox:/mnt/data/281Py_kernel_block_count_diag_fix_sha256.txt)
* [281Py_kernel_block_count_diag_fix_bundle_20260709.tar](sandbox:/mnt/data/281Py_kernel_block_count_diag_fix_bundle_20260709.tar)

実行は以下でお願いします。

```bash id="hss62j"
rm -f ./281Py_kernel_block_count_diag_fix_probe

STATIC_ONLY=1 bash 281Py_kernel_block_count_diag_fix_validate_N21_full_once.sh

bash 281Py_kernel_block_count_diag_fix_validate_N21_full_once.sh
```

今回の確認ポイントは、正当性に加えて以下がOKになることです。

```text id="lkmf2z"
blockdiag_rows             131
blockdiag_positive         OK
progress_diag_columns      OK
progress_diag_positive     OK
```

`[kernel-blockdiag]` の `loop_iters` と `descend` が0でなければ、281でようやく診断値として使えます。
...
VERSION_TAG:str="281 kernel-block-count-diagnostic-fix: parent 279/276 baseline; fixes 280 diagnostic writeback so MAXD14 generic DFS block counters are written before normal result store and early returns; calculation, task order, one-launch-per-chunk split145, rootrestlate, futuremask, no-sibling, cache generation, MAXD fallbacks, worker split preserved; 280 broken all-zero diagnostic, 278 zero const, 277 depth_u, 275 bucket diag, 273 root0 direct, 270 precompute, 240 fid14 launch split rejected"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 280 diagnostic arrays: per-thread MAXD14 generic loop block counts.
    # They are written only by kernel_dfs_iter_gpu_maxd14 and reduced on host.
    self.diag_loop_iters_arr:List[u64]=[u64(0)]*m
    self.diag_zero_avail_arr:List[u64]=[u64(0)]*m
    self.diag_restore_arr:List[u64]=[u64(0)]*m
    self.diag_normal_block_arr:List[u64]=[u64(0)]*m
    self.diag_special_block_arr:List[u64]=[u64(0)]*m
    self.diag_nf_zero_arr:List[u64]=[u64(0)]*m
    self.diag_future_check_arr:List[u64]=[u64(0)]*m
    self.diag_future_kill_arr:List[u64]=[u64(0)]*m
    self.diag_terminal_arr:List[u64]=[u64(0)]*m
    self.diag_child_jmark_arr:List[u64]=[u64(0)]*m
    self.diag_save_push_arr:List[u64]=[u64(0)]*m
    self.diag_descend_arr:List[u64]=[u64(0)]*m
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    meta_next:Ptr[u8],
    results:Ptr[u64],
    diag_loop_iters_arr:Ptr[u64],diag_zero_avail_arr:Ptr[u64],diag_restore_arr:Ptr[u64],
    diag_normal_block_arr:Ptr[u64],diag_special_block_arr:Ptr[u64],diag_nf_zero_arr:Ptr[u64],
    diag_future_check_arr:Ptr[u64],diag_future_kill_arr:Ptr[u64],diag_terminal_arr:Ptr[u64],
    diag_child_jmark_arr:Ptr[u64],diag_save_push_arr:Ptr[u64],diag_descend_arr:Ptr[u64],
    ...
    total:u64=u64(0)
    diag_loop_iters:u64=u64(0)
    diag_zero_avail:u64=u64(0)
    diag_restore:u64=u64(0)
    diag_normal_block:u64=u64(0)
    diag_special_block:u64=u64(0)
    diag_nf_zero:u64=u64(0)
    diag_future_check:u64=u64(0)
    diag_future_kill:u64=u64(0)
    diag_terminal:u64=u64(0)
    diag_child_jmark:u64=u64(0)
    diag_save_push:u64=u64(0)
    diag_descend:u64=u64(0)
    ...
    if root_a==u32(0):
      diag_loop_iters_arr[i]=diag_loop_iters
      diag_zero_avail_arr[i]=diag_zero_avail
      diag_restore_arr[i]=diag_restore
      diag_normal_block_arr[i]=diag_normal_block
      diag_special_block_arr[i]=diag_special_block
      diag_nf_zero_arr[i]=diag_nf_zero
      diag_future_check_arr[i]=diag_future_check
      diag_future_kill_arr[i]=diag_future_kill
      diag_terminal_arr[i]=diag_terminal
      diag_child_jmark_arr[i]=diag_child_jmark
      diag_save_push_arr[i]=diag_save_push
      diag_descend_arr[i]=diag_descend
      results[i]=u64(0)
      return
    ...
    while True:
      diag_loop_iters+=u64(1)
      if cur_avail==u32(0):
        diag_zero_avail+=u64(1)
        if save_sp==0:
          break
        diag_restore+=u64(1)
        save_sp-=1
      ...
      if future_check_mask!=u32(0):
        if (nibble_op&u32(8))!=u32(0):
          diag_future_check+=u64(1)
          if (bm&~((nld<<u32(1))|(nrd>>u32(1))|ncol))==u32(0):
            diag_future_kill+=u64(1)
            continue
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.diag_loop_iters_arr),gpu.raw(soa.diag_zero_avail_arr),gpu.raw(soa.diag_restore_arr),
      gpu.raw(soa.diag_normal_block_arr),gpu.raw(soa.diag_special_block_arr),gpu.raw(soa.diag_nf_zero_arr),
      gpu.raw(soa.diag_future_check_arr),gpu.raw(soa.diag_future_kill_arr),gpu.raw(soa.diag_terminal_arr),
      gpu.raw(soa.diag_child_jmark_arr),gpu.raw(soa.diag_save_push_arr),gpu.raw(soa.diag_descend_arr),
...
def split145_blockdiag_progress_header()->str:
  h:str=stream_funcid_reorder_progress_header().strip()
  h+="\tloop_iters\tzero_avail_count\trestore_count\tnormal_block_count\tspecial_block_count"
  h+="\tnf_zero_count\tfuture_check_count\tfuture_prune_kill_count"
  h+="\tterminal_hit_count\tchild_jmark_count\tsave_push_count\tdescend_count"
  h+="\n"
  return h

def append_split145_blockdiag_progress(progress_fname:str,N:int,preset_queens:int,chunk_index:int,off:int,m:int,BLOCK:int,MAX_BLOCKS:int,STEPS:int,gpu_sort_mode:int,elapsed_text:str,elapsed_ms:int,chunk_total:int,gpu_total:int,total_records:int,stats:List[int],diag:List[int])->None:
  done_records:int=off+m
  remaining_records:int=total_records-done_records
  if remaining_records<0:
    remaining_records=0
...
def exec_solutions_gpu_chunk_split145(
  ...
  diag_loop_iters:int=0
  diag_zero_avail:int=0
  diag_restore:int=0
  diag_normal_block:int=0
  diag_special_block:int=0
  diag_nf_zero:int=0
  diag_future_check:int=0
  diag_future_kill:int=0
  diag_terminal:int=0
  diag_child_jmark:int=0
  diag_save_push:int=0
  diag_descend:int=0
  i=0
  while i<m:
    diag_loop_iters+=int(soa.diag_loop_iters_arr[i])
    diag_zero_avail+=int(soa.diag_zero_avail_arr[i])
    diag_restore+=int(soa.diag_restore_arr[i])
    diag_normal_block+=int(soa.diag_normal_block_arr[i])
    diag_special_block+=int(soa.diag_special_block_arr[i])
    diag_nf_zero+=int(soa.diag_nf_zero_arr[i])
    diag_future_check+=int(soa.diag_future_check_arr[i])
    diag_future_kill+=int(soa.diag_future_kill_arr[i])
    diag_terminal+=int(soa.diag_terminal_arr[i])
    diag_child_jmark+=int(soa.diag_child_jmark_arr[i])
    diag_save_push+=int(soa.diag_save_push_arr[i])
    diag_descend+=int(soa.diag_descend_arr[i])
    i+=1

  if gpu_log_level>=1:
    print(f"[kernel-blockdiag] N={N} m={m} loop_iters={diag_loop_iters} zero_avail={diag_zero_avail} restore={diag_restore} normal_block={diag_normal_block} special_block={diag_special_block} nf_zero={diag_nf_zero} future_check={diag_future_check} future_kill={diag_future_kill} terminal={diag_terminal} child_jmark={diag_child_jmark} save_push={diag_save_push} descend={diag_descend}")
  ...
  stages:List[int]=[stage_soa_ms,stage_stats_ms,stage_split_ms,stage_kernel_reduce_ms,stage_compute_ms,stage_no_read_ms,0,d0_m,d2base14_m,m,diag_loop_iters,diag_zero_avail,diag_restore,diag_normal_block,diag_special_block,diag_nf_zero,diag_future_check,diag_future_kill,diag_terminal,diag_child_jmark,diag_save_push,diag_descend]
...
def exec_solutions_gpu_bin_stream_split145(
  ...
  with open(progress_fname,"w") as pf:
    pf.write(split145_blockdiag_progress_header())
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      diag_inner:List[int]=[0]*12
      di:int=0
      while di<12:
        diag_inner[di]=stages_inner[10+di]
        di+=1
      append_split145_blockdiag_progress(progress_fname,N,preset_queens,chunk_index,off,m,BLOCK,MAX_BLOCKS,STEPS,gpu_sort_mode,elapsed_outer_text,elapsed_outer_ms,chunk_total,gpu_total,total_records,stats,diag_inner)
      if gpu_log_level>=1:
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split281_blockdiag_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split281_blockdiag_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split281-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split281-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split281_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
