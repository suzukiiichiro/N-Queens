...
# =============================================================================
# NQ_UPDATE_MEMO
# 285: restore283-after-ncol-early-reject。284 normalfirst+ncol-early は
#      正当性OKだが283を上回らなかったため採用せず、283 generic
#      normal-first へ戻す安全復帰版。MAXD14 generic DFS loopの
#      normal path先行は維持し、284の ncol early、281/280 blockdiag、
#      278 zero const、277 depth_u、273 root0 direct、270 precompute、
#      240 fid14 split は入れない。
...
VERSION_TAG:str="285 restore283-after-ncol-early-reject: 284 ncol-early rejected; restored 283 generic normal-first; root-preroll branch order and all host shaping/cache/dispatch features preserved; rejected blockdiag/depthu/zero-const/root0-direct/precompute/fid14 split remain absent"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split285_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split285_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split285-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split285-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split285_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
