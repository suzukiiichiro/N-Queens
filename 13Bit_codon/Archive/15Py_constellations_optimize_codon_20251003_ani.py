...
class NQueens15:
  ...
  def dfs(self,ld:int,rd:int,col:int,row:int,free:int,jmark:int,endmark:int,mark1:int,mark2:int,board_mask:int,N:int,funcname:str)->int:  
   if funcname=="SQBkBlBjrB":
    ...
    while row==mark1 and avail:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd|blockK,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlBjrB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBkBlBjrB")
      # if next_free:
      #   row_next:int=row+1
    ...
    while row==mark2 and avail:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|1,next_rd,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjrB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlBjrB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    if row==jmark:
      ...
      while avail:
        ...
        # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
          total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQB")
      return total
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      ...
      # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjrB")
    return total 
   elif funcname=="SQB": 
    avail:int=free
    total:int=0
    ...
    while row==mark1 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|1,next_rd|blockK,next_col,row+3,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjrB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBklBjrB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark1 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|1,next_rd,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBkBjrB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlBkBjrB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark2 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd|blockK,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjrB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBkBjrB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark1 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|2,next_rd|blockK,next_col,row+3,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjrB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlkBjrB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    if row==N1-jmark:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBkBlBjrB")
      return total
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      ...
      if _should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra):
      # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjlBkBlBjrB")
    return total  
    ...
    if row==N1-jmark:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBklBjrB")
      return total
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      ...
      if _should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra):
      # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjlBklBjrB")
    return total 
    ...
    if row==N1-jmark:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlBkBjrB")
      return total
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      ...
      if _should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra):
      # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjlBlBkBjrB")
    return total  
    ...
    if row==N1-jmark:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlkBjrB")
      return total
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      ...
      if _should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra):
      # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjlBlkBjrB")
    return total 
    ...
    while row==mark1 and avail:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd|blockK,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BlB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BkBlB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark2 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|1,next_rd,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BlB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while avail:
      ...
      if _should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra):
      # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2B")
    return total
    ...
    while row==mark1 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|1,next_rd|blockK,next_col,row+3,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BklB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark1 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|1,next_rd,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BkB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BlBkB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark2 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd|blockK,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BkB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark1 and avail:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|2,next_rd|blockK,next_col,row+3,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BlkB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark1 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd|blockK,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BlB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      # extra = _extra_block_for_row(row_next, mark1, mark2, jmark, N)
      # if _should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BkBlB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark2 and avail:
      ...
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BlB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while avail:
      ...
      if _should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra):
      # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1B")
    return total
    ...
    while row==mark1 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|1,next_rd|blockK,next_col,row+3,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BklB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark1 and avail:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|1,next_rd,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BkB")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BlBkB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark1 and avail:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld|2,next_rd|blockK,next_col,row+3,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BlkB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while row==mark2 and avail:
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd|blockK,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BkB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
    ...
    while avail:
      ...
      if _should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra):
      # if self._has_future_space_step(next_ld, next_rd, next_col, row_next, endmark, board_mask, extra):
        total+=self.dfs(next_ld,next_rd,next_col,row_next,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd0B")
    return total     
    ...
    while row==mark1 and avail:
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd|blockK,next_col,row+2,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd0B")
    while avail:
      bit:int=avail&-avail
      avail&=avail-1
      next_ld:int=(ld|bit)<<1
      ...
      next_free:int=board_mask&~blocked
      if next_free:
        total+=self.dfs(next_ld,next_rd,next_col,row+1,next_free,jmark,endmark,mark1,mark2,board_mask,N,"SQd0BkB")
      # if next_free:
      #   row_next:int=row+1
      #   extra=0 # 次の行が特殊行なら、その行で実際にORされる追加ブロックを足す
      #   if row_next==mark1:
  ...
  def exec_solutions(self,constellations:List[Dict[str,int]],N:int)->None:
    ...
    for constellation in constellations:
      ...
      if j<(N-3):
        ...
        if j>2 * N-34-start:
          if k<l:
            ...
            if start<l:
              if start<k:
                if l!=k+1:
                  cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBkBlBjrB")
                else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBklBjrB")
              else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlBjrB")
            else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjrB")
          else:
            mark1,mark2=l-1,k-1
            if start<k:
              if start<l:
                if k!=l+1:
                  cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlBkBjrB")
                else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBlkBjrB")
              else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBkBjrB")
            else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjrB")
        else:
          if k<l:
            mark1,mark2=k-1,l-1
            if l!=k+1:
              cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjlBkBlBjrB")
            else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjlBklBjrB")
          else:
            mark1,mark2=l-1,k-1
            if k != l+1:
              cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjlBlBkBjrB")
            else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQBjlBlkBjrB")
      elif j==(N-3):
        endmark=N2
        if k<l:
          mark1,mark2=k-1,l-1
          if start<l:
            if start<k:
              if l != k+1: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BkBlB")
              else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BklB")
            else:
              mark2=l-1
              cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BlB")
          else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2B")
        else:
          mark1,mark2=l-1,k-1
          endmark=N2
          if start<k:
            if start<l:
              if k != l+1:
                cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BlBkB")
              else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BlkB")
            else:
              mark2=k-1
              cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2BkB")
          else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd2B")
          ...
          if start<l:  # 少なくともlがまだ来ていない場合
            if start<k:  # もしkもまだ来ていないなら
              ...
              if l != k+1:  # kとlが隣り合っている場合
                mark2=l-1
                cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BkBlB")
              else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BklB")
            else:  # lがまだ来ていないなら
              mark2=l-1
              cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BlB")
          # すでにkとlが来ている場合
          else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1B")
        else:  # l<k
            ...
            if start<l:  # lがまだ来ていない場合
              if k<N2:  # kが末尾にない場合
                ...
                if k != l+1:  # lとkの間に空行がある場合
                  mark2=k-1
                  cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BlBkB")
                # lとkの間に空行がない場合
                else: cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BlkB")
              else:  # kが末尾の場合
                if l != (N-3):  # lがkの直前でない場合
                  ...
                  endmark=N-4
                  cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1B")
            else:  # もしkがまだ来ていないなら
              if k != N2:  # kが末尾にない場合
                mark2,endmark=k-1,N2
                cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1BkB")
              else:  # kが末尾の場合
                endmark=N-3
                cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd1B")
          else: # kとlはスタートの前
        ...
        endmark=N2
        if start>k:
          cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd0B")
        else: # クイーンをコーナーに置いて星座を組み立てる方法と、ジャスミンを適用する方法
          mark1=k-1
          cnt=self.dfs(ld,rd,col,start,free,jmark,endmark,mark1,mark2,board_mask,N,"SQd0BkB")
