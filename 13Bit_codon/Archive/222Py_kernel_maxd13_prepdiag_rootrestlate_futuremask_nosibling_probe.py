...
# =============================================================================
# NQ_UPDATE_MEMO
# 222: 更新メモ: 221Py rootrestlate_inlinexor は正当性OKだが217最速を上回らなかったため不採用。222Pyは217Py rootrestlate新最速基準相当へ戻し、MAXD14 kernelの探索ロジックは変えずにMAXD13化可否を判断するための save_sp / cur_depth / terminal_depth 診断を追加する。
...
222Py note:
  221Py was correct but slower than 217. 222 returns to the 217 rootrestlate
  baseline shape and does not attempt a new speed path. The purpose is to
  collect MAXD13 preparation diagnostics from the validated MAXD14 kernel: the
  maximum saved ancestor stack pointer, maximum logical cur_depth, terminal
  parent depth, and root availability popcount. The DFS arithmetic and total
  counting semantics remain unchanged.
...
221Pyログは正当性OKでしたが、`0:07:08.033` で217Py `0:07:07.709` より `0.324秒` 遅いため不採用です。これで218〜221の217近傍再結合確認はいったん完了とし、本日は **MAXD13準備診断** に進むのがよいです。

222Pyは高速化候補ではなく、217Py相当のrootrestlateへ戻したうえで、MAXD14 kernelに以下の診断を追加しました。
...
`max_save_sp<=12` が全chunkで確認できれば、次にMAXD13専用kernelを限定dispatchで試す根拠になります。`max_save_sp==13` が出る場合は、MAXD14 fallbackを残したまま別の縮小案へ進む判断になります。

こちらでは `STATIC_ONLY=1` の静的確認まで行い、OKでした。Codon/CUDA実機でのbuild/full実行はcudacodon側でお願いします。

```bash
cd /home/suzuki/Github/N-Queens/13Bit_codon/Archive

STATIC_ONLY=1 bash 222Py_kernel_maxd13_prepdiag_rootrestlate_futuremask_nosibling_validate_full_once.sh

bash 222Py_kernel_maxd13_prepdiag_rootrestlate_futuremask_nosibling_validate_full_once.sh
```

作成物です。

* [222Py_kernel_maxd13_prepdiag_rootrestlate_futuremask_nosibling_probe.py](sandbox:/mnt/data/222Py_kernel_maxd13_prepdiag_rootrestlate_futuremask_nosibling_probe.py)
* [222Py_kernel_maxd13_prepdiag_rootrestlate_futuremask_nosibling_validate_full_once.sh](sandbox:/mnt/data/222Py_kernel_maxd13_prepdiag_rootrestlate_futuremask_nosibling_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/222Py_kernel_maxd13_prepdiag_rootrestlate_futuremask_nosibling_bundle_20260703.tar)

"""

# 222 maxd13-prepdiag memo over validated 211 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
#   204 was correct and 0.407 seconds faster than 197, so this branch adopts
#   204 as the new numerical baseline.  It keeps the validated root one/two-
#   candidate preroll, 204 root_rest clear-lowbit expression, 188 future_check_mask
#   guard, and 184 no-sibling tailcall build.
...
#   only restorable ancestor frames.  211 keeps the narrow root fast-start only when
#   post-root-action root availability has one or two bits, matching the validated
#   204/197/196/194 behavior.  Three-candidate roots explicitly stay on the generic
#   loop.  The root<=2 predicate is still the second-lowbit test and 204
#   clear-lowbit root_rest expression, and restores the 204 root_after_second
#   temporary scalar predicate after 205 failed to beat 204.
#   The no-sibling rule and depth packing in avail[] bits 27..31 are unchanged, so MAXD14 remains four
...
# 210 keeps the validated 204/197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
#   MAXD14 bytes/thread = 4 ancestor stacks * 4 bytes * 13 = 208.
#   Larger bytes/thread = 4 state stacks * 4 bytes * MAXD + 4 * schedule words.
...
VERSION_TAG:str="222 maxd13-prepdiag-rootrestlate: 217 rootrestlate fastest baseline plus MAXD13 preparation diagnostics - README-managed history"
...
class TaskSoA:
  ...
  def __init__(self,m:int)->None:
    # 154: GPU bitboard inputs use their native 32-bit representation end-to-end.
    # 155: ctrl0_arr packs bits0..4=function-id and bits5..9=initial row.
    # 157: markctrl_arr packs jmark/end/mark1/mark2 into four 5-bit u32 fields.
    # N<=27 keeps bitboards, ctrl0, and every valid row/mark value within u32.
...
def schedule_depth_for_task(ctrl0:u32,markctrl:u32,meta_next:List[u8])->int:
  ...
  while True:
    ...
    # P5 same-row transition is folded into the current frame.
    if ((IS_P5_MASK_I>>fu)&1)!=0 and rowv==mark1:
      fu=int(meta_next[fu])

    # 164 consumes a base frame before push, so it is not counted.
    ...
    depth+=1
    # Supported data reaches a base before either condition.  Returning 22
    # forces the explicit unsupported-depth path before any GPU launch.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    meta_next:Ptr[u8],
    results:Ptr[u64],
    diag_save_sp:Ptr[u32],diag_cur_depth:Ptr[u32],diag_terminal_depth:Ptr[u32],diag_root_popcount:Ptr[u32],
    m:int,board_mask:u32,
    n3:u32,n4:u32,
)->None:
    """217 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate root-rest-late preroll."""
    ...
    # Valid mark functions map to five block operations.  The three masks below
    # encode the low/middle/high bit of that operation code directly by fid.
    ...
    #   0 normal step1
    #   1 step2, add0, blockL0, K=n3
    #   2 step2, add0, blockL1, K=0
    #   3 step3, add0, blockL1, K=n4
    #   4 step3, add0, blockL2, K=n3
    #   5 step2, add1, blockL0, K=0
    # These masks decode code -> metadata without an if/elif chain.
    ...
    # MAXD14 PTX-safe scalar nibble schedule layout:
    #   schedule_lo bits  0..31: depths 0..7,  8 x 4-bit nibble op
    #   schedule_hi bits  0..23: depths 8..13, 6 x 4-bit nibble op
    #   nibble bits0..2: direct block code, 0=normal step1, 1..5=block op
    #   nibble bit3    : future check for normal step1 frames
    #   child_jmark_mask bits 0..13: child action is jmark for parent depth
    #   terminal_parent_depth: 0..13
    #   terminal_is_base14: 0=base, 1=base14
    #   root_action: 0 ordinary, 1 jmark, 2 base, 3 base14
    # The local packed_schedule u32[4] array is removed for MAXD14, and all
    # shifts remain 32-bit to avoid Codon/CUDA invalid-PTX generation from
    # high-bit u64 variable shifts.  Compared with 172's 3-bit nibble op, the
    # 4-bit nibble uses power-of-two shifts and decodes block/future directly.
    ...
    if root_a==u32(0):
      diag_save_sp[i]=u32(0)
      diag_cur_depth[i]=u32(0)
      diag_terminal_depth[i]=u32(0)
      diag_root_popcount[i]=u32(0)
      results[i]=u64(0)
      return

    # Generate the deterministic nibble schedule once.  The current-frame
    # operation keeps the original byte-opcode meaning in a 4-bit scalar field:
    # direct block_code in bits0..2 and future_check in bit3.  This costs no
    # local memory and avoids the 172 compact-op remapping/subtract in the hot loop.
    ...
    # 197 keeps the validated 196 current-frame register cache, 13-slot
    # ancestor storage, no-sibling tail-call descent, future-prune guard, and
    # root one/two-candidate preroll. It keeps 195's three-candidate expansion removed: when the root
    ...
    # loop. For multi-candidate prerolled roots, the remaining root siblings are
    # saved exactly as the generic no-sibling loop would save them when the first
    # child descends; for invalid/terminal first candidates, the remaining root
    # candidates are left as the active root frame. Roots with three or more
    # candidates preserve the 194 generic loop.
    #
    ...
    max_cur_depth:int=0
    root_pc:u32=u32(0)
    root_tmp:u32=cur_avail
    while root_tmp!=u32(0):
      root_pc+=u32(1)
      root_tmp=root_tmp&(root_tmp-u32(1))

    # 222 maxd13-prepdiag-rootrestlate:
    # If the root frame has one or two candidates, process the first root
    # candidate before the generic DFS loop, matching the validated 217/204/197/196/194 scope.
    # 195's three-candidate case remains intentionally removed because it was
    # correct but slower.  222 keeps 217's delayed cur_avail=root_rest assignment
    # and late root_first extraction, and restores the root_after_second scalar
    # predicate after the 221 inline-XOR recombination failed to beat 217.
    # Diagnostic arrays record max save_sp / cur_depth / terminal_depth only.
    ...
    while True:
      ...
      if cur_depth>max_cur_depth:
        max_cur_depth=cur_depth
    diag_save_sp[i]=u32(max_save_sp)
    diag_cur_depth[i]=u32(max_cur_depth)
    diag_terminal_depth[i]=u32(terminal_depth)
    diag_root_popcount[i]=root_pc
    results[i]=total*w_arr[i]
...
def kernel_dfs_iter_gpu_maxd16(
    ...
    """172 inherited byte-opcode schedule fallback, static MAXD=16."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
    ...
    # One byte per depth (upper two bits unused):
    #   bits0..2 block operation code
    #   bit3     future check
    #   bits4..5 child entry action: 0 ordinary, 1 jmark, 2 base, 3 base14
    # Four bytes are packed into one u32.  The schedule is written once before
    # DFS and never written on push.
    ...
    # Generate and pack the deterministic depth schedule.  pack_word remains a
    # scalar while four bytes are assembled; each local array word is stored
    # exactly once after the child action for its final byte becomes known.
    schedule_raw:u32=ctrl0_arr[i]
    schedule_depth:int=0
    pack_word:u32=u32(0)
    ...
    while True:
      ...
      if schedule_depth==0:
        ...
        # Crossing to a new u32 means the preceding four opcodes are complete.
      ...
      if frame_action>=u32(2):
        # For a partial final word, the terminal action just completed its last
        # parent byte.  A depth divisible by four was flushed above already.
    ...
    # Root has no parent byte, so apply its entry action once here.
    if root_action==u32(2):
      results[i]=w_arr[i]
      return
    if root_action==u32(3):
...
def kernel_dfs_iter_gpu_maxd18(
    ...
    """172 inherited byte-opcode schedule fallback, static MAXD=18."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd20(
    ...
    """172 inherited byte-opcode schedule fallback, static MAXD=20."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd21(
    ...
    """172 inherited byte-opcode schedule fallback, static MAXD=21."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  meta_next:List[u8],
  results:List[u64],
  diag_save_sp:List[u32],diag_cur_depth:List[u32],diag_terminal_depth:List[u32],diag_root_popcount:List[u32],
  m:int,
  board_mask_gpu:u32,
  n3_gpu:u32,
  n4_gpu:u32,
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(diag_save_sp),gpu.raw(diag_cur_depth),gpu.raw(diag_terminal_depth),gpu.raw(diag_root_popcount),
...
# paths, changing only the MAXD14 local arrays from 14 saved frames to 13
# saved ancestors.  The 170 scalar-u32pair schedule representation and all host
# dispatch logic are retained.
####################################################################################################
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        # ----------------------------------------
        # 154: materialize the GPU bitboards as u32 once on the host.
        # 155: pre-pack the initial function-id/row control word once.
        # 157: pre-pack four signed host mark/end values into one 20-bit u32.
        #      -1 is encoded as 31; supported rows are <=30, so it stays a sentinel.
        soa.ld_arr[t]=u32(ld)
        soa.rd_arr[t]=u32(rd)
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    diag_save_sp:List[u32]=[u32(0)]*STEPS
    diag_cur_depth:List[u32]=[u32(0)]*STEPS
    diag_terminal_depth:List[u32]=[u32(0)]*STEPS
    diag_root_popcount:List[u32]=[u32(0)]*STEPS
    ...
    # 154/155/156/157/158/159/160/161/162/164/167/168/169/170/171/172: convert GPU scalar inputs once per host invocation, not once per GPU thread.
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        if not launch_kernel_dfs_iter_gpu_static_maxd(selected_maxd,sort_soa,sort_w_arr,meta_next,results,diag_save_sp,diag_cur_depth,diag_terminal_depth,diag_root_popcount,m,board_mask_gpu,n3_gpu,n4_gpu,GRID,BLOCK):
        ...
        if gpu_log_level>=2:
          print(f"[maxd-dispatch] N={N} scope=exec chunk={chunk_index} m={m} required_maxd={required_maxd} selected_MAXD={selected_maxd} schedule_words={packed_schedule_words_for_maxd(selected_maxd)} stack_bytes_per_thread={packed_stack_bytes_per_thread(selected_maxd)} capacity_check=OK")
        if not launch_kernel_dfs_iter_gpu_static_maxd(selected_maxd,soa,w_arr,meta_next,results,diag_save_sp,diag_cur_depth,diag_terminal_depth,diag_root_popcount,m,board_mask_gpu,n3_gpu,n4_gpu,GRID,BLOCK):
          print(f"[maxd-error] unsupported required_maxd={required_maxd}; supported maximum is 21")
          return
...
####################################################################################################
# 164 child-frame push initialization over validated 162 and the 148 score-aware chunk-shape layer
# The complete validated 148 scorestripe_v9_lanephase32_octetfirstpairlock29 permutation is
# retained byte-for-byte in algorithmic terms.  Cache names stay at 148 for direct reuse.
#
# Safety boundary:
...
#   - host shaping is byte-equivalent to validated 162; 164 changes only GPU frame initialization placement
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="222 maxd13-prepdiag reuses validated 217 scorestripe_v9 task order/cache; MAXD14 rootrestlate scalar predicate plus save_sp diagnostics"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  164 reuses the validated 162/161/160/159/158/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 164 experiment changes only GPU frame initialization placement.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  164 host-side score-aware lanephase chunk shaping is reused unchanged from validated 162/161/160/159/158/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 164 experiment changes only GPU frame initialization; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/167/168/169/170/171/172: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def exec_solutions_gpu_chunk_split145(
  ...
  if m>0:
    ...
    if not launch_kernel_dfs_iter_gpu_static_maxd(selected_maxd,soa,w_arr,meta_next,results,diag_save_sp,diag_cur_depth,diag_terminal_depth,diag_root_popcount,m,board_mask_gpu,n3_gpu,n4_gpu,GRID,BLOCK):
      ...
      return 0,stats,error_stages,"0:00:00.000",0
    i=0
    diag_max_save_sp:int=0
    diag_save_sp13_count:int=0
    diag_max_cur_depth:int=0
    diag_max_terminal_depth:int=0
    diag_root_pc_max:int=0
    while i<m:
      chunk_total+=int(results[i])
      dsp:int=int(diag_save_sp[i])
      dcd:int=int(diag_cur_depth[i])
      dtd:int=int(diag_terminal_depth[i])
      drp:int=int(diag_root_popcount[i])
      if dsp>diag_max_save_sp:
        diag_max_save_sp=dsp
      if dsp>=13:
        diag_save_sp13_count+=1
      if dcd>diag_max_cur_depth:
        diag_max_cur_depth=dcd
      if dtd>diag_max_terminal_depth:
        diag_max_terminal_depth=dtd
      if drp>diag_root_pc_max:
        diag_root_pc_max=drp
      i+=1
    if gpu_log_level>=1:
      print(f"[maxd13-diag] N={N} scope=split145 m={m} selected_MAXD={selected_maxd} max_save_sp={diag_max_save_sp} save_sp13_count={diag_save_sp13_count} max_cur_depth={diag_max_cur_depth} max_terminal_depth={diag_max_terminal_depth} root_pc_max={diag_root_pc_max}")
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split145 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split145 full run / optional multi-GPU worker
      # Examples:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split222_maxd13_prepdiag_rootrestlate_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split222_maxd13_prepdiag_rootrestlate_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split222-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split222-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split222_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split222-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split222-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split222_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
