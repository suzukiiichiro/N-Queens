...
[91Py_constellations_GPU_cuda_codon_dynamic_p8_stream_measure.py](sandbox:/mnt/data/91Py_constellations_GPU_cuda_codon_dynamic_p8_stream_measure.py)

今回の91は、**84を本線のまま維持**して、探索ロジック・kernel・DFS部分には触っていません。84は `VERSION_TAG` が stream bin GPU runner 系で、N21でも `314666222712 / 0:25:38.459 / ok` の基準ログが出ているため、ここをベースにしています。

変更点はこの3点だけです。

```text
1. VERSION_TAG を 91 measure に変更

2. stream内側の exec_solutions() 呼び出しを
   gpu_log_level=0 に固定
   → chunkごとの [gpu-config] / [gpu-summary] を抑制

3. progress TSV を強化
   progress_N{N}_{preset}_stream_measure.tsv
```
...
```bash
codon build -release 91Py_constellations_GPU_cuda_codon_dynamic_p8_stream_measure.py

./91Py_constellations_GPU_cuda_codon_dynamic_p8_stream_measure \
...
84ではstream経路が `.bin` を使い、N>=21のGPU通常実行では全件 `List[Dict]` ではなく `exec_solutions_gpu_bin_stream()` 側で処理する構成になっています。今回の91もこの流れはそのまま維持しています。

こちらの環境ではCodon実ビルドはできませんでしたが、Pythonの `tokenize` による字句レベル確認は通しています。
...
VERSION_TAG:str="91 stream measure GPU runner from 84 dynamic preset P8"
CROSS_STRIPE_SAFE_DEFAULT:bool=False

# 91 MEASURE:
#   Based on 84 stream.  Kernel / DFS logic is intentionally unchanged.
#   Changes are limited to stream measurement:
#     - suppress inner exec_solutions() [gpu-config]/[gpu-summary] logs per chunk
#     - write an isolated progress_N*_stream_measure.tsv
#     - record elapsed/elapsed_ms and record counters per stream chunk
...
def append_stream_progress(progress_fname:str,N:int,preset_queens:int,chunk_index:int,off:int,m:int,BLOCK:int,MAX_BLOCKS:int,STEPS:int,gpu_sort_mode:int,elapsed_text:str,elapsed_ms:int,chunk_total:int,gpu_total:int,total_records:int)->None:
...
def exec_solutions_gpu_bin_stream(
  ...
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_measure.tsv"
  with open(progress_fname,"w") as pf:
    pf.write("N\tpreset\tchunk\toff\tm\tblock\tmax_blocks\tsteps\tsort_mode\telapsed\telapsed_ms\tchunk_total\tgpu_total\tdone_records\ttotal_records\tremaining_records\n")
  ...
  if gpu_log_level>=1:
    print(f"[stream-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if chunk_only else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} progress={progress_fname} inner_log_level=0 measure=1")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      # 91 MEASURE: keep outer stream logs, but suppress inner per-chunk [gpu-config]/[gpu-summary].
      ...
      append_stream_progress(progress_fname,N,preset_queens,chunk_index,off,m,BLOCK,MAX_BLOCKS,STEPS,gpu_sort_mode,elapsed_text,elapsed_ms,chunk_total,gpu_total,total_records)
      if gpu_log_level>=1:
  ...
  if gpu_log_level>=1:
    print(f"[stream-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} progress={progress_fname} measure=1")
