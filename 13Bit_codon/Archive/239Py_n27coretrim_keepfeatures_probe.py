...
# =============================================================================
# NQ_UPDATE_MEMO
# 239: n27coretrim-keepfeatures cleanup-only。238Py n27diagtrim-keepfeatures r2を親に、
#      N27へ向けた本線品質を落とさず、CPU側の未使用再帰dfs fallbackと
#      到達不能だった切替分岐を削る小整理版。
...
#      MAXD fallback、CPU dfs_iter pathを維持。
...
"""
239 cleanup-only note: 238のN27向け診断削除keepfeaturesを親に、実行本線に不要なCPU再帰DFS fallbackだけを削った版。CUDA kernel本文とGPU本線機能は変更しない。
"""
...
VERSION_TAG:str="239 n27coretrim-keepfeatures cleanup-only: 238 generic N5..27; bare -g split145 mode31; removed unused recursive CPU dfs fallback and unreachable CPU branch; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved; CUDA kernels unchanged"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 239: bare -g stays split145 mode31
...
# 239: keep runtime globals required by broadmarktail/chunkshape148/split145
...
# 239: recursive CPU dfs fallback removed. CPU path uses dfs_iter only.
...
# 239: these helpers are used by the kept broadmarktail/split145
...
CHUNKSHAPE148_DEFAULT_REASON:str="239 n27coretrim keeps validated 238/231/217 scorestripe_v9 task order/cache; CUDA kernels unchanged"
...
def main()->None:
  ...
  if argc == 1:
    ...
    print("[239-default] no arguments: CPU N22 default; use -g for A10G range mode")
  elif argc >= 2:
    arg=sys.argv[1]
    if arg == "-c":
      use_gpu=False
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 239 coretrim; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split239_keepfeatures_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split239_keepfeatures_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split239-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split239-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split239_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
