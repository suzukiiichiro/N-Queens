...
# =============================================================================
# 271 N=21 full validation harness
# Parent: 241 r3 restore239-after-fid14split-reject, the fastest supplied
#         safe baseline after 270 rootpre2flag was rejected for speed.
# Experiment: final-fastest-complete packaging only. CUDA kernels and GPU
#             execution path are intentionally unchanged from 241 r3.
...
SRC=${SRC:-./271Py_final_fastest_complete_probe.py}
CAND=${CAND:-./271Py_final_fastest_complete_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/271Py_final_fastest_complete_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/271Py_final_fastest_complete_logs_N21_full_once_${TS}"
...
echo "[start] 271 final-fastest-complete validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 271 final-fastest-complete SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '271 final-fastest-complete' "$SRC"; then
  printf 'source_version_tag\t271 final-fastest-complete\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t271 final-fastest-complete\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split271' in s and 'split270_' not in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split271 active; 270/240 rejected', 'split271', True))
else:
    checks.append(('source_split_tag', 'split271 active; 270/240 rejected', 'old split runtime present or split271 missing', False))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 271 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 271 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 271 final-fastest-complete mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "271parent241:$BASELINE_271_PARENT_SECONDS" "270rejected:$BASELINE_270_REJECT_SECONDS" "267:$BASELINE_267_SECONDS" "239:$BASELINE_239_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 271 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
