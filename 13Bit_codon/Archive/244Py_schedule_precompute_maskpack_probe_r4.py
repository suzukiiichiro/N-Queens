...
ありがとうございます。では方針は **244 r4 = JIT切り分け版** で進めます。
...
244 r4:
  243 JIT成立済み形を親にする
  child_jmark_mask_arr + future_check_mask_arr だけを sched_mask_arr にpack
  terminal_depth / terminal_base14 / root_action は243同様に別配列のまま
```

実行コマンドはこれです。

```bash
rm -f ./244Py_schedule_precompute_maskpack_probe
rm -f ./244Py_schedule_precompute_maskpack_probe_r4

STATIC_ONLY=1 bash 244Py_schedule_precompute_maskpack_validate_N21_full_once_r4.sh

bash 244Py_schedule_precompute_maskpack_validate_N21_full_once_r4.sh
```
...
```text
r4がJIT成立:
  child/future mask pack はOK
  r1〜r3のJIT不成立源は terminal/root_action側pack にほぼ絞れる
  次は terminal_base14 + root_action だけpack、または terminal_depthだけpack を1本ずつ試す

r4もJIT不成立:
  sched_mask pack/decode自体がCodon/PTXと相性悪い
  243の7配列precomputeへ戻し、pack以外のload削減方法を検討
```

速度はまず二次判定です。
最初の確認ポイントは **JIT成立 → 正当性OK → 243より戻るか** の順で見ます。


はい。244 r1〜r3で共通して `CUDA_ERROR_INVALID_PTX` なので、ここは **一気にpackを進めるのではなく、pack対象を1か所ずつ切り分ける** のが正しいです。

r1〜r3で怪しいのは、`terminal_depth / terminal_base14 / root_action` を同一 `sched_ctrl` にpackしてdecodeした部分です。そこで **244 r4** は、243でJIT成立済みの形に近く戻し、まず **child/future maskだけ** をpackします。

```text
243 JIT成立済み:
  child_jmark_mask_arr
  future_check_mask_arr
  terminal_depth_arr
  terminal_base14_arr
  root_action_arr

244 r4:
  sched_mask_arr = child_jmark_mask | (future_check_mask << 14)
  terminal_depth_arr
  terminal_base14_arr
  root_action_arr
```

つまり、r4でJITが成立すれば、r1〜r3の不成立箇所は `sched_ctrl` 側にかなり絞れます。
r4でもJIT不成立なら、mask pack自体、またはpack decode形がCodon/PTXと相性が悪いと判断できます。

こちらで `STATIC_ONLY=1` は通っています。
...
source_schedule_precompute_maskpack    OK
source_host_precompute_maskpack_arrays OK
...
* [244Py_schedule_precompute_maskpack_probe.py](sandbox:/mnt/data/244Py_schedule_precompute_maskpack_probe.py)
* [244Py_schedule_precompute_maskpack_probe_r4.py](sandbox:/mnt/data/244Py_schedule_precompute_maskpack_probe_r4.py)
* [244Py_schedule_precompute_maskpack_validate_N21_full_once.sh](sandbox:/mnt/data/244Py_schedule_precompute_maskpack_validate_N21_full_once.sh)
* [244Py_schedule_precompute_maskpack_validate_N21_full_once_r4.sh](sandbox:/mnt/data/244Py_schedule_precompute_maskpack_validate_N21_full_once_r4.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [244Py_schedule_precompute_maskpack_sha256.txt](sandbox:/mnt/data/244Py_schedule_precompute_maskpack_sha256.txt)
* [244Py_schedule_precompute_maskpack_bundle_20260707_r4.tar](sandbox:/mnt/data/244Py_schedule_precompute_maskpack_bundle_20260707_r4.tar)

r4の確認ポイントは、まず速度ではなく **JIT成立するか** です。成立したら、次は `terminal_base14 + root_action` だけ、あるいは `terminal_depth` だけ、というように1項目ずつpackしていくのがよいです。
...
VERSION_TAG:str="244 r4 schedule-precompute-maskpack probe: parent 241 restore239; single launch kept; MAXD14 schedule interpreter moved to host precomputed arrays; 240 fid14 split and 242 condition micro-diff rejected; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
# 244 r4: keep runtime globals required by broadmarktail/chunkshape148/split145
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 244 r4: host-precomputed schedule fields for MAXD14. 243's
    # child_jmark_mask/future_check_mask arrays are packed into sched_mask_arr
    # only; terminal/root_action remain separate to isolate the previous PTX JIT
    # failure to mask-pack vs ctrl-pack.
    ...
    self.terminal_base14_arr:List[u32]=[u32(0)]*m
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 244 r4: host-side clone of the former MAXD14 kernel schedule builder.
  # This keeps 243's schedule precompute but packs only child/future masks.
  # terminal/root_action are intentionally not packed in r4 to isolate PTX JIT risk.
  ...
  return schedule_lo,schedule_hi,sched_mask,terminal_parent_depth,terminal_is_base14,root_action
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    terminal_depth_arr:Ptr[u32],terminal_base14_arr:Ptr[u32],root_action_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 244 r4: schedule fields are precomputed on the host in build_soa_for_range().
    # r4 packs only child_jmark_mask/future_check_mask into one low-bit u32.
    # terminal_depth/base14/root_action remain separate, matching the 244 r4 decode shape.
    ...
    terminal_base14:u32=terminal_base14_arr[i]
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.terminal_depth_arr),gpu.raw(soa.terminal_base14_arr),gpu.raw(soa.root_action_arr),
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        terminal_base14:u32=u32(0)
        root_action:u32=u32(0)
        sched_lo,sched_hi,sched_mask,terminal_depth_u,terminal_base14,root_action=precompute_maxd14_schedule_fields(soa.ctrl0_arr[t],soa.markctrl_arr[t],meta_next_sched)
        ...
        soa.terminal_base14_arr[t]=terminal_base14
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        for p in range(m):
          ...
          sort_soa.terminal_base14_arr[p]=soa.terminal_base14_arr[q]
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split244_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split244_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split244-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split244-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split244_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
