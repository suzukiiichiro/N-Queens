...
# =============================================================================
# 269 N=21 full validation harness
# Parent: 267 root0countcache precompute-best / 268 root0future0 rejected.
# Experiment: root_action==0 and child_jmark_mask==0 chunks use a MAXD14
#             root0child0 kernel without root_action load/branch or child_jmark
#             branch. root0-with-child chunks use the 267 root0 kernel;
#             root_action-nonzero chunks use the 265-compatible fallback.
...
SRC=${SRC:-./269Py_schedule_precompute_root0child0_probe.py}
CAND=${CAND:-./269Py_schedule_precompute_root0child0_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/269Py_schedule_precompute_root0child0_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/269Py_schedule_precompute_root0child0_logs_N21_full_once_${TS}"
...
echo "[start] 269 schedule-precompute-root0child0 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 269 schedule-precompute-root0child0 ctrl0-markctrl-free-repack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '269 schedule-precompute-root0child0' "$SRC"; then
  printf 'source_version_tag\t269 schedule-precompute-root0child0\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t269 schedule-precompute-root0child0\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
required_defs = [
    'kernel_dfs_iter_gpu_maxd14','kernel_dfs_iter_gpu_maxd14_root0','kernel_dfs_iter_gpu_maxd14_root0_child0','kernel_dfs_iter_gpu_maxd16','kernel_dfs_iter_gpu_maxd18','kernel_dfs_iter_gpu_maxd20','kernel_dfs_iter_gpu_maxd21',
...
old_splits = ['split240_','split241_','split242_','split243_','split244_','split255_','split256_','split257_','split258_','split259_','split260_','split261_','split262_','split263_','split264_','split265_']
runtime_split_ok = ('split269_' in s and not any(x in s for x in old_splits))
if runtime_split_ok:
    checks.append(('source_split_tag', 'split269 active; old split runtime tags absent', 'split269', True))
else:
    checks.append(('source_split_tag', 'split269 active; old split runtime tags absent', 'old split runtime tag present or split269 missing', False))
...
if not m14:
    ...
    actual = 'root0countcache MAXD14 precompute active' if ok else 'root0countcache missing or old schedule interpreter present or free_arr mask reuse missing or unsafe pack present'
    checks.append(('source_schedule_precompute_root0child0', 'MAXD14 precompute root0child0 active', actual, ok))
...
checks.append(('source_host_precompute_root0child0_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 269 schedule-precompute-root0child0 mode31 split145"
...
echo "[validation-ok] 267 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
