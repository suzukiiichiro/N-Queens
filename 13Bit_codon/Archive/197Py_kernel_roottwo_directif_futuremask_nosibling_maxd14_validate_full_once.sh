...
# =============================================================================
# NQ_UPDATE_MEMO
# 197: 更新メモ: 196は正当性OKかつ194比0.049秒高速で数値上の新最速基準。196 root<=2 second-lowbit prerollを親に、MAXD14 kernelのみroot_preroll flagを削ってroot_after_second==0 direct-ifへ変更する微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 197 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 197 starts from
# validated 196 because 196 was correct and 0.049s faster than the 194 baseline.
#
...
# bytes/thread. The experiment is deliberately narrow: if post-root-action
# root_avail contains one or two bits, process the first depth-0 candidate once
# before the generic DFS loop. 3-bit and wider roots remain generic. The
# root<=2 predicate remains the 196 second-lowbit test, but the intermediate
# root_preroll flag is removed and the preroll body is guarded directly by
# root_after_second==0. Host task order, dispatch, cache, progress verification,
# and README-managed history remain unchanged.
SRC=${SRC:-./197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 197 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/197Py_kernel_roottwo_directif_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 197Py source
#   has a harmless header/string/layout difference. This 197 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 196 root one/two-candidate second-lowbit futuremask/no-sibling MAXD14 dispatch; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if preroll predicate	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'root_two_direct_if' "$SRC" && grep -q 'root_second:u32=root_rest&(u32(0)-root_rest)' "$SRC" && grep -q 'root_after_second:u32=root_rest\^root_second' "$SRC" && grep -q 'if root_after_second==u32(0):' "$SRC" && ! grep -q 'root_preroll:u32' "$SRC" && ! grep -q 'if root_preroll' "$SRC"; then
  printf 'source_root_two_direct_if	present_no_root_preroll_flag	present_no_root_preroll_flag	OK
' >> "$SUMMARY"
else
  printf 'source_root_two_direct_if	present_no_root_preroll_flag	missing_or_flag_retained	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split197' "$SRC" && ! grep -q 'split196' "$SRC" && ! grep -q 'split195' "$SRC" && ! grep -q 'split194' "$SRC" && ! grep -q 'split193' "$SRC" && ! grep -q 'split192' "$SRC" && ! grep -q 'split191' "$SRC" && ! grep -q 'split190' "$SRC" && ! grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split197 only	split197 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split197 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 197 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source roottwo-directif static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 196 roottwo-rebase futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 196 keeps the validated 194 host/chunk structure, then derives chunk-tail diagnostics from
...
# 196: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 197 roottwo-directif futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
