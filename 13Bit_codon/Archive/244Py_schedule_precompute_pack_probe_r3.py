...
# =============================================================================
# NQ_UPDATE_MEMO
# 244 r3: schedule-precompute-lowpack-intdecode probe。241 restore239本線を親に、240別launchと
...
"""
244 r3 schedule-precompute-lowpack-intdecode note: 241 restore239本線を親に、240のfid14/rest別launchと242のfuturemask-depthbit微差を採用せず、単一launchのままMAXD14 schedule precompute情報を3 u32へpackする構造probe。

その通りです。ここは撤回ではなく、**JIT不成立箇所を絞って修正し、packを成立させる方向**で進めるべきです。

244 r2 の怪しい箇所は、packそのものよりも **terminal depth を `u32` のままdecodeして、hot loopで `cur_depth` と比較している部分**だと見ています。
...
r3ではここを、既存の241/243系と同じ比較形へ戻しました。

```python
terminal_depth:int = int(ctrl_pack & u32(15))
...
```text
sched_lo_arr    : schedule_lo
sched_hi_arr    : schedule_hi
sched_mask_arr  : child_jmark_mask / future_check_mask
sched_ctrl_arr  : terminal_depth / terminal_base14 / root_action
```
...
source_schedule_precompute_intdecode   OK
source_host_precompute_intdecode_arrays OK
release_build_policy                   OK
```

古い244/r2を掴まないよう、r3名で実行してください。

```bash
rm -f ./244Py_schedule_precompute_pack_probe
rm -f ./244Py_schedule_precompute_pack_probe_r3

STATIC_ONLY=1 bash 244Py_schedule_precompute_pack_validate_N21_full_once_r3.sh

bash 244Py_schedule_precompute_pack_validate_N21_full_once_r3.sh
```

ファイルはこちらです。

* [244Py_schedule_precompute_pack_probe.py](sandbox:/mnt/data/244Py_schedule_precompute_pack_probe.py)
* [244Py_schedule_precompute_pack_probe_r3.py](sandbox:/mnt/data/244Py_schedule_precompute_pack_probe_r3.py)
...
* [244Py_schedule_precompute_pack_validate_N21_full_once_r3.sh](sandbox:/mnt/data/244Py_schedule_precompute_pack_validate_N21_full_once_r3.sh)
...
* [244Py_schedule_precompute_pack_bundle_20260707_r3.tar](sandbox:/mnt/data/244Py_schedule_precompute_pack_bundle_20260707_r3.tar)

今回のポイントは、**pack方針は維持し、PTXが嫌がりそうなdecode/compare形だけ戻す**ことです。r3でJITが成立すれば、次はさらに `sched_mask_arr` などを既存配列へ再利用して、追加load本数を減らす段階へ進めます。
...
VERSION_TAG:str="244 r3 schedule-precompute-lowpack-intdecode probe: parent 241 restore239; single launch kept; MAXD14 host precompute fields packed into 4 u32 arrays; 240 fid14 split and 242 condition micro-diff rejected; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 244 r3: packed schedule fields are precomputed on the host in build_soa_for_range().
    # 243 loaded 7 separate arrays. 244 r3 uses 4 low-bit fields to avoid the
    # high-bit pack form that triggered CUDA_ERROR_INVALID_PTX on cudacodon; r3 decodes terminal depth as int to avoid u32/int compare PTX risk.
...
CHUNKSHAPE148_DEFAULT_REASON:str="244 r3 schedule-precompute-lowpack-intdecode keeps validated 241/239/238 scorestripe_v9 task order/cache; single-launch MAXD14 packed schedule precompute probe"
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 244 r3 schedule-precompute-lowpack-intdecode; using 0")
        bench_mode=0
