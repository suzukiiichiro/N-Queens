...
299 = **353.896s vs 296 = 353.671s、差 −0.225s (−0.064%) — 誤差級、flat**。K=64 は効果なし。292 と同様 K=32 が最適点として確定です。

296 の `K_PER_THREAD_MAXD14=32` に戻して確定。次の方向へ。## 299 結果まとめと 300 の考え方

**299 不採用**: 353.896s vs 296 = 353.671s → **flat**。K=32 が 296 カーネルでも最適点として再確認。
...
## 300 の着眼点: schedule ブランチ除去

ホット DFS ループの `nibble_op` デコードは現在こうなっています：
...
これを `schedule:u64` 1変数に統合すると：

```python
# 300:
nibble_op:u32=u32((schedule>>u64(cur_depth*4))&u64(15))
```

- ブランチ 1本削除
- u32 レジスタ 1本削減（schedule_hi が不要に）
- depth>=8 のニブルはビルド時に `+32` オフセットで格納

`save_sp`・`next_depth`・`stack_ptr` は 296 から完全保持。

```bash
STATIC_ONLY=1 bash 300Py_schedule_u64_validate_N21_full_once.sh
bash 300Py_schedule_u64_validate_N21_full_once.sh
```
"""
...
VERSION_TAG:str="300 schedule-u64 probe: parent 299 K64-on-296 (rejected flat, K sweet spot confirmed K=32); reverted to 296 (K=32, 353.671s) as base; kernel_dfs_iter_gpu_maxd14 merges schedule_lo:u32 + schedule_hi:u32 into single schedule:u64, removing the cur_depth<8 branch in the hot DFS loop nibble_op decode and reducing live u32 register count by 1 (schedule_hi freed); build encodes depth>=8 nibbles at bit offset (schedule_depth-8)*4+32; hot decode: nibble_op=u32((schedule>>cur_depth*4)&15); preroll decode: pr_nibble_op=u32(schedule&15); stack_ptr/save_sp/next_depth and all other 296 structures unchanged; K_PER_THREAD_MAXD14=32; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while idx<m:
      ...
      while True:
        ...
        if schedule_depth<8:
          schedule|=u64(frame_nibble)<<u64(schedule_depth*4)
        else:
          schedule|=u64(frame_nibble)<<u64((schedule_depth-8)*4+32)
      ...
      if root_after_second==u32(0):
        ...
        pr_nibble_op:u32=u32(schedule&u64(15))
