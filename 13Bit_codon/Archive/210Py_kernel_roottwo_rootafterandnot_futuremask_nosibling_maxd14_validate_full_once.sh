...
# =============================================================================
# NQ_UPDATE_MEMO
# 210: 更新メモ: 209Py roottailclear は正当性OKながら204Py/206Pyを上回らなかったため不採用。210Pyは206/204相当へ戻し、MAXD14 kernelのみroot_after_secondを root_rest ^ root_second ではなく root_rest & ~root_second で作る and-not predicate 微差実験を検証する。
...
# progress TSV and verifies the complete total. Candidate 210 returns to
...
# 204 clear-lowbit root_rest plus root_after_second=root_rest&~root_second scalar predicate.
...
#   - rootafterandnot source check is whitespace/comment robust.
SRC=${SRC:-./210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 210 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 210Py source
#   has a harmless header/string/layout difference. This 210 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second and-not scalar if	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
checks = {
    ...
    'root_after_andnot_present': 'root_after_second:u32=root_rest&(~root_second)' in compact,
    'root_andnot_if_present': 'ifroot_after_second==u32(0):' in compact,
    'root_xor_after_scalar_absent': 'root_after_second:u32=root_rest^root_second' not in root_block,
    'root_inline_if_absent': 'if(root_rest^root_second)==u32(0):' not in root_block,
    'root_preroll_flag_absent': 'root_preroll:u32' not in s,
    # 210 keeps 204/197 save payload; 203 depth0-save is not adopted.
    ...
    # 210 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 210 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 210 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_rootafterandnot_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootafterandnot\tpresent_root_after_andnot_predicate\tpresent_root_after_andnot_predicate\tOK
' >> "$SUMMARY"
else
  printf 'source_rootafterandnot\tpresent_root_after_andnot_predicate\tmissing_or_old_xor_predicate\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split210'}
bad=sorted(tags-allowed)
if 'split210' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split210 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split210 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 210 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 210 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootafterandnot static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 210 roottwo-rootafterandnot futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 210 roottwo-rootafterandnot futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
