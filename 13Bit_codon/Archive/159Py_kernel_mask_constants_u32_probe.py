...
158Pyは問題ありません。全131チャンク、部分和01～05、最終合計`314666222712`が一致し、実行時間は`0:15:54.673`です。

### 159Py

[159Py_kernel_mask_constants_u32_probe.py](sandbox:/mnt/data/159Py_kernel_mask_constants_u32_probe.py)

分類マスク10個と`INIT_MASK`をカーネル内で直接`u32`として宣言し、使用箇所の`u32(MASK_NAME)`を除去しました。`MAXD:Static[int]`と`__array__[u32]`は維持しています。

### 実行シェル

[159Py_kernel_mask_constants_u32_validate_full_once.sh](sandbox:/mnt/data/159Py_kernel_mask_constants_u32_validate_full_once.sh)

```bash
chmod +x 159Py_kernel_mask_constants_u32_validate_full_once.sh
./159Py_kernel_mask_constants_u32_validate_full_once.sh
```

全131チャンクを一度だけ実行し、部分和01～05、全件合計、欠落・重複・エラーを検証します。158Pyの`954.673秒`との速度比較も表示します。

シェル構文、マスク値392ケース、既存の各パック等価性、模擬131チャンクによる検証処理は通過しています。この環境ではCodon/CUDA実機コンパイルは未実施です。

検証完了しました。**159Pyに問題はありません。**
...
* 処理件数：2,025,282件
* 部分和01〜05：すべて一致
* 全チャンク合計：`314666222712`
* 最終GPU累計：`314666222712`
* 最終出力：`ok`
* warning／error／mismatch：0件
* 158Pyとの全チャンク結果・入力統計も完全一致

実行時間は **`0:15:54.667`** でした。158Pyの`0:15:54.673`より0.006秒高速ですが、これは測定誤差範囲です。したがって、マスクをあらかじめ`u32`にした変更は、**正解性を維持し、性能は158Pyと同等**と判断できます。

マスク型、markctrl、row/step、ctrlパックの等価性検査もすべて通過しています。**159Pyを次の基準版として採用可能です。** 160Pyは今回は作成していません。


"""

# 159 GPU kernel-mask-constant-u32 experiment:
#   Retain the fully validated 158 function-id/classification-u32 arithmetic,
#   157 markctrl-u32 prepack, 156 initialized-ctrl-u32 packing, 155 ctrl0-u32
#   prepack, 154 GPU-input-u32 representation, and all earlier DFS/order changes.
#   The sole kernel experiment declares the ten classification masks and
#   INIT_MASK directly as local u32 values, then removes u32(MASK) wrappers at
#   every use site.  MAXD remains Static[int] because it is the compile-time
#   __array__ length.  Host data layout, task order, cache format, DFS
#   transitions, and solution arithmetic remain unchanged from validated 158.
...
VERSION_TAG:str="159 kernel mask constants native-u32 over 158 function-id masks-u32 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    #       32bit mask として焼き込む。159では各maskを入口からu32に固定し、
    #       hot pathの u32(MASK) ラッパを除去する単独実験とする。
    ...
    while sp>=0:
      ...
      if (cv&INIT_MASK)==u32(0):
        # 158/159: function-id and every classification-mask operand stay in u32.
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154/155/156/157/158/159: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155/156/157/158/159: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154/155/156/157/158/159: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 159 native-u32 kernel mask constants over validated 158 function-id-u32 and the 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 158; 159 changes only kernel mask constant declarations/use sites
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 158 scorestripe_v9 task order/cache unchanged; 159 declares kernel masks directly as u32"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  159 reuses the validated 158/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 159 experiment changes only kernel mask constant declarations and removes u32(MASK) use-site wrappers.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  159 host-side score-aware lanephase chunk shaping is reused unchanged from validated 158/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 159 experiment changes only kernel mask constant declarations/use sites; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split159_kernel_masks_u32_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split159_kernel_masks_u32_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split159-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split159-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split159_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split159-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split159-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split159_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
