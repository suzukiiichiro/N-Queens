...
169Pyの `.py` と `.sh` を作成しました。

168Pyの packed byte-opcode schedule 構成を親にして、メモの方針どおり **MAXD14静的カーネル追加** を入れています。
検証シェルも168Py版を親に、N=21 full 1回で `required_maxd=14`、`selected_MAXD=14`、`schedule_words=4`、`stack_bytes_per_thread=240` を期待する形へ更新しました。

ファイルはこちらです。

* [169Py_kernel_packed_opcode_schedule_maxd14_probe.py](sandbox:/mnt/data/169Py_kernel_packed_opcode_schedule_maxd14_probe.py)
* [169Py_kernel_packed_opcode_schedule_maxd14_validate_full_once.sh](sandbox:/mnt/data/169Py_kernel_packed_opcode_schedule_maxd14_validate_full_once.sh)

入れた主な変更は以下です。

```text
MAXD14:Static[int]=14
SCHED_WORDS14:Static[int]=4

dispatch:
  required_maxd <= 14 -> MAXD14
  required_maxd <= 16 -> MAXD16
  required_maxd <= 18 -> MAXD18
  required_maxd <= 20 -> MAXD20
  required_maxd <= 21 -> MAXD21

MAXD14 stack:
  4 state stacks * 4 bytes * 14 + 4 schedule words * 4 bytes
  = 224 + 16
  = 240 bytes/thread
```
...
kernel_normalized_equivalence     OK
frame_opcode_equivalence          506604 cases OK
dfs_164_vs_169_equivalence        12000 cases OK
dispatch_threshold_cases          OK
stack_bytes_MAXD14                240 OK
```

Codon/CUDAでの full GPU 実行はこの環境では未実施です。GPU環境では通常どおり、

```bash
chmod +x 169Py_kernel_packed_opcode_schedule_maxd14_validate_full_once.sh
./169Py_kernel_packed_opcode_schedule_maxd14_validate_full_once.sh
```

で、N=21 full run の合計 `314666222712` と、全131チャンクの `selected_MAXD=14` を確認する流れです。

"""

# 169 GPU packed-byte-opcode schedule + MAXD14 static specialization experiment:
#   Branch from the validated fastest 168 packed-opcode schedule baseline.
#   The host still computes the exact queen-independent stored-frame depth and
#   now dispatches MAXD 14/16/18/20/21.  Inside each kernel, the full u32 ctrl
#   stack remains replaced by a deterministic schedule of byte opcodes, packed
#   four opcodes per u32 word.  One opcode contains a 3-bit block operation, one
#   future bit, and the 2-bit child-entry action.  The schedule is generated once
#   before DFS and is read-only during search.  For MAXD14 this changes local
#   storage to four u32[14] state arrays plus u32[4] packed schedule words
#   (240 bytes/thread).  Host layout, task order, cache, DFS order, bitboard
#   arithmetic, and solution arithmetic are unchanged.
...
# 169 static stack specializations.  MAXD remains the number of stored state
...
VERSION_TAG:str="169 MAXD14 static kernel over 168 packed byte-opcode schedule - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 169 inherits the exact 168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """169 packed byte-opcode schedule kernel, static MAXD=14."""
    ...
    packed_schedule=__array__[u32](SCHED_WORDS14)
    bm:u32=board_mask
...
def kernel_dfs_iter_gpu_maxd16(
    ...
    """169 packed byte-opcode schedule kernel, static MAXD=16."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd18(
    ...
    """169 packed byte-opcode schedule kernel, static MAXD=18."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd20(
    ...
    """169 packed byte-opcode schedule kernel, static MAXD=20."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd21(
    ...
    """169 packed byte-opcode schedule kernel, static MAXD=21."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
####################################################################################################
# 169 packed-opcode schedule + MAXD14 static dispatch harness
#
# This build defines five capacity-specialized packed-schedule @gpu.kernel variants.
# It keeps the proven 168 packed-opcode kernel body and existing split/chunk-shape paths,
# adding only the smaller MAXD14 specialization and the 14/16/18/20/21 dispatch tier.
####################################################################################################
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164/167/168/169: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 168 scorestripe_v9 task order/cache and packed opcodes; 169 adds MAXD14 static dispatch"
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/167/168/169: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 169 branches from validated 168.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 169 MAXD14 packed-opcode GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split169_packed_opcode_schedule_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split169_packed_opcode_schedule_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split169-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split169-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split169_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split169-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split169-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split169_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
