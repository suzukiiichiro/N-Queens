...
""" CPU浅展開後にGPUへ渡す owner 付きタスク配列 """
class ExpandedTasks:
  """ コンストラクタ """
  def __init__(self):
    self.ld_arr:List[int]=[]
    self.rd_arr:List[int]=[]
    self.col_arr:List[int]=[]
    self.row_arr:List[int]=[]
    self.free_arr:List[int]=[]
    self.jmark_arr:List[int]=[]
    self.end_arr:List[int]=[]
    self.mark1_arr:List[int]=[]
    self.mark2_arr:List[int]=[]
    self.funcid_arr:List[int]=[]
    self.w_arr:List[u64]=[]
    self.owner_arr:List[int]=[]

"""bucket index 配列を作る。
mode=0: bucket=(funcptn<<2)|(avail_flag<<1)|tail
mode=1: bucket=(functionid<<1)|tail  ※ funcptn だけで混ぜず、FID単位に分ける
mode=2: hybrid
  - funcptn in {0,2} は FID単位で分ける
  - funcptn in {1,3,5} は coarse bucket のまま残す
  ねらい:
    21(split=8) と 24(split=8) のログ比較では、funcptn=0/2 は細分化で改善し、
    funcptn=1/3/5 は逆に悪化していたため、「全部FID化」はやめて重い所だけ分ける。
"""
def make_bucket_indices(bucket_count:int)->List[List[int]]:
...
"""bucket_mode ごとの bucket 数"""
def gpu_bucket_count(bucket_mode:int)->int:
  if bucket_mode==0:
    return 24
  if bucket_mode==1:
    return 56
  # mode=2:
  #   0..55  : FID/tail（funcptn 0 と 2 だけ使用）
  #   56..59 : coarse funcptn=1
  #   60..63 : coarse funcptn=3
  #   64..67 : coarse funcptn=5
  return 68

"""bucket id を人間可読に戻す（ログ表示用）"""
def describe_gpu_bucket(
  meta:List[Tuple[int,int,int]],
  bucket_id:int,bucket_mode:int
)->Tuple[int,int,int,int,int]:
  if bucket_mode==0:
    funcptn:int=bucket_id>>2
    avail_flag:int=(bucket_id>>1)&1
    tail:int=bucket_id&1
    return -1,funcptn,avail_flag,tail,0
  if bucket_mode==1:
    fid:int=bucket_id>>1
    tail:int=bucket_id&1
    _,funcptn,avail_flag=meta[fid]
    return fid,funcptn,avail_flag,tail,1
  # mode=2:
  if bucket_id<56:
    fid:int=bucket_id>>1
    tail:int=bucket_id&1
  ...
  # coarse側
  if bucket_id<60:
    rel:int=bucket_id-56
    return -1,1,(rel>>1)&1,rel&1,0
  if bucket_id<64:
    rel:int=bucket_id-60
    return -1,3,(rel>>1)&1,rel&1,0
  rel:int=bucket_id-64
  return -1,5,(rel>>1)&1,rel&1,0

"""GPU投入 bucket を計算する"""
def calc_gpu_bucket_id(
  meta:List[Tuple[int,int,int]],
  functionid:int,row:int,endmark:int,
  bucket_mode:int
)->int:
  _,funcptn,avail_flag=meta[functionid]
  tail:int=1 if row>=endmark-1 else 0
  if bucket_mode==0:
    return (funcptn<<2)|(avail_flag<<1)|tail
  if bucket_mode==1:
    # mode=1: functionid単位。funcptn だけで混ぜず、FID単位に分ける。
    return (functionid<<1)|tail
  # mode=2: selective / hybrid
  # funcptn=0,2 は FID分割、funcptn=1,3,5 は coarse に戻す。
  if funcptn==0 or funcptn==2:
    return (functionid<<1)|tail
  if funcptn==1:
    return 56|((avail_flag&1)<<1)|tail
  if funcptn==3:
    return 60|((avail_flag&1)<<1)|tail
  return 64|((avail_flag&1)<<1)|tail

"""展開済みタスクを ExpandedTasks と bucket index に追加"""
def append_expanded_task(
  tasks:ExpandedTasks,
  bucket_indices:List[List[int]],
  meta:List[Tuple[int,int,int]],
  functionid:int,ld:int,rd:int,col:int,row:int,free:int,
  jmark:int,endmark:int,mark1:int,mark2:int,
  weight:u64,owner:int,bucket_mode:int
)->None:
  if free==0:
    return
  bucket_id:int=calc_gpu_bucket_id(meta,functionid,row,endmark,bucket_mode)
  idx:int=len(tasks.ld_arr)
  tasks.ld_arr.append(ld)
  tasks.rd_arr.append(rd)
  tasks.col_arr.append(col)
  tasks.row_arr.append(row)
  tasks.free_arr.append(free)
  tasks.jmark_arr.append(jmark)
  tasks.end_arr.append(endmark)
  tasks.mark1_arr.append(mark1)
  tasks.mark2_arr.append(mark2)
  tasks.funcid_arr.append(functionid)
  tasks.w_arr.append(weight)
  tasks.owner_arr.append(owner)
  bucket_indices[bucket_id].append(idx)

"""split_row < 0 のときは mark/jmark の直前を優先して自動決定"""
def choose_gpu_split_row(split_row:int,start_row:int,jmark:int,endmark:int,mark1:int,mark2:int)->int:
  target:int=split_row
  if target<0:
    target=endmark-2
    if mark1>start_row+1 and mark1<endmark:
      target=mark1-1
    elif mark2>start_row+1 and mark2<endmark:
      target=mark2-1
    elif jmark>start_row+1 and jmark<endmark:
      target=jmark-1
    else:
      target=start_row+2
  if target<start_row:
...
"""CPUで浅くDFS展開し、GPUに渡す細粒度タスクを作る"""
def expand_iter_to_row(
  meta:List[Tuple[int,int,int]],
  blockK:List[int],blockL:List[int],
  board_mask:int,
  src:TaskSoA,src_w:List[u64],
  owner_base:int,m:int,
  split_row:int,
  tasks:ExpandedTasks,
  bucket_indices:List[List[int]],
  bucket_mode:int
  ...
  for i in range(m):
    owner:int=owner_base+i
    functionid:int=src.funcid_arr[i]
    ld0:int=src.ld_arr[i]
    rd0:int=src.rd_arr[i]
    col0:int=src.col_arr[i]
    row0:int=src.row_arr[i]
    free0:int=src.free_arr[i]&board_mask
    jmark:int=src.jmark_arr[i]
    endmark:int=src.end_arr[i]
    mark1:int=src.mark1_arr[i]
    mark2:int=src.mark2_arr[i]
    weight:u64=src_w[i]

    if free0==0:
      continue

    target_row:int=choose_gpu_split_row(split_row,row0,jmark,endmark,mark1,mark2)

    # スタック要素: (functionid, ld, rd, col, row, free)
    stack:List[Tuple[int,int,int,int,int,int]]=[(functionid,ld0,rd0,col0,row0,free0)]
    ...
    while stack:
      ...
      # split 到達後は、以降をGPU側 dfs_iter_gpu に任せる
      if row>=target_row:
        append_expanded_task(tasks,bucket_indices,meta,
          functionid,ld,rd,col,row,free,
          jmark,endmark,mark1,mark2,weight,owner,bucket_mode)
        continue

      next_funcid,funcptn,avail_flag=meta[functionid]
      avail:int=free

      # 基底に達している場合もGPU側に渡して同一カーネルで数えさせる
      if funcptn==5 and row==endmark:
        append_expanded_task(tasks,bucket_indices,meta,
          functionid,ld,rd,col,row,avail,
          jmark,endmark,mark1,mark2,weight,owner,bucket_mode)
        continue
      ...
      if funcptn in (0,1,2):
        ...
        if at_mark and avail:
          ...
          row_step=row+step
          bK=blockK[functionid]
          bL=blockL[functionid]
          use_blocks=True
          use_future=False
...
"""2-pass並列展開用: bucket連続配置済みの展開済みタスク"""
class ExpandedFlatTasks:
  """ コンストラクタ """
  def __init__(self,m:int):
    self.ld_arr:List[int]=[0]*m
    self.rd_arr:List[int]=[0]*m
    ...
    self.mark2_arr:List[int]=[0]*m
    self.funcid_arr:List[int]=[0]*m
    self.w_arr:List[u64]=[u64(0)]*m
    self.owner_arr:List[int]=[0]*m

"""1 source constellation から split_row まで展開したときの bucket別 task 数を数える"""
def count_expand_one_to_row(
  meta:List[Tuple[int,int,int]],
  blockK:List[int],blockL:List[int],
  board_mask:int,
  src:TaskSoA,
  ...
  functionid:int=src.funcid_arr[src_index]
  ld0:int=src.ld_arr[src_index]
  rd0:int=src.rd_arr[src_index]
  col0:int=src.col_arr[src_index]
  row0:int=src.row_arr[src_index]
  free0:int=src.free_arr[src_index]&board_mask
  jmark:int=src.jmark_arr[src_index]
  endmark:int=src.end_arr[src_index]
  mark1:int=src.mark1_arr[src_index]
  mark2:int=src.mark2_arr[src_index]
  ...
  while stack:
    ...
    if row>=target_row:
      b:int=calc_gpu_bucket_id(meta,functionid,row,endmark,bucket_mode)
      counts_flat[counts_base+b]+=1
      continue
...
"""2-pass並列展開用: 1 source constellation から生成される task を prefix 位置へ直接書く"""
def fill_expand_one_to_row(
  meta:List[Tuple[int,int,int]],
  blockK:List[int],blockL:List[int],
  board_mask:int,
  src:TaskSoA,src_w:List[u64],
  src_index:int,owner:int,
  split_row:int,
  bucket_mode:int,bucket_count:int,
  prefix_flat:List[int],
  prefix_base:int,
  out:ExpandedFlatTasks
)->None:
  functionid:int=src.funcid_arr[src_index]
  ld0:int=src.ld_arr[src_index]
  rd0:int=src.rd_arr[src_index]
  ...
  mark1:int=src.mark1_arr[src_index]
  mark2:int=src.mark2_arr[src_index]
  weight:u64=src_w[src_index]

  if free0==0:
    return

  local_counts:List[int]=[0]*bucket_count
  target_row:int=choose_gpu_split_row(split_row,row0,jmark,endmark,mark1,mark2)
  stack:List[Tuple[int,int,int,int,int,int]]=[(functionid,ld0,rd0,col0,row0,free0)]
  ...
  while stack:
    ...
    if row>=target_row:
      b:int=calc_gpu_bucket_id(meta,functionid,row,endmark,bucket_mode)
      pos:int=prefix_flat[prefix_base+b]+local_counts[b]
      local_counts[b]+=1
      out.ld_arr[pos]=ld
      out.rd_arr[pos]=rd
      out.col_arr[pos]=col
      out.row_arr[pos]=row
      out.free_arr[pos]=free
      out.jmark_arr[pos]=jmark
      out.end_arr[pos]=endmark
      out.mark1_arr[pos]=mark1
      out.mark2_arr[pos]=mark2
      out.funcid_arr[pos]=functionid
      out.w_arr[pos]=weight
      out.owner_arr[pos]=owner
      continue
...
def exec_solutions(
    N:int,constellations:List[Dict[str,int]],use_gpu:bool,
    gpu_split_row:int=-1,
    gpu_bucket_mode:int=1,
    gpu_owner_blocks:int=32,
  ...
  # CPU浅展開でも mark 段の blockK/blockL が必要になるため、CPU/GPU 共通で作る
  ...
  MAX_BLOCKS:int=gpu_owner_blocks 
  ...
  ##########
  if use_gpu:
    # 入力 constellation は従来どおり STEPS 件ずつ SoA 化。
    # その後 CPU で split_row まで浅く展開し、owner 付き細粒度タスクを bucket ごとにGPU投入する。
    # 入力 constellation は OWNER_STEPS(=STEPS) 件ずつ SoA 化する。
    # CPU浅展開後の expanded task は桁違いに増えるため、GPU投入batchは別サイズにする。
    # ここを STEPS=8192 のまま使い回すと、数千万taskで数千回のkernel起動になり、launch overheadが支配的になる。
    soa:TaskSoA=TaskSoA(STEPS)
    results_all:List[u64]=[u64(0)]*m_all
    w_arr:List[u64]=[u64(0)]*STEPS

    GPU_TASK_BLOCKS:int=gpu_task_blocks
    GPU_TASK_STEPS:int=BLOCK*GPU_TASK_BLOCKS
    gpu_soa:TaskSoA=TaskSoA(GPU_TASK_STEPS)
    results:List[u64]=[u64(0)]*GPU_TASK_STEPS
    w_task:List[u64]=[u64(0)]*GPU_TASK_STEPS
    owner_batch:List[int]=[0]*GPU_TASK_STEPS
    ...
    # -1 は自動: mark1/mark2/jmark の直前を優先して切る
    # コマンド例: ./21Py_constellations_GPU_cuda_codon_bucket_diag -g 18 18 10
    #   上記では N=18 だけ、split_row=10 で実行
    GPU_SPLIT_ROW:int=gpu_split_row
    # log_level:
    #   0 = 最小ログ: config と summary のみ
    #   1 = chunk単位の expand + summary（bucket詳細は出さない）
    #   2 = 25相当: chunk単位の bucket 詳細も出す
    GPU_LOG_LEVEL:int=gpu_log_level
    GPU_LOG_BUCKETS:bool=(GPU_LOG_LEVEL>=2)
    GPU_LOG_EXPAND:bool=(GPU_LOG_LEVEL>=1)
    GPU_BUCKET_MODE:int=gpu_bucket_mode
    GPU_BUCKETS:int=gpu_bucket_count(GPU_BUCKET_MODE)
    print(f"[gpu-config] N={N} bucketized=1 par_expand=1 split_row={GPU_SPLIT_ROW} bucket_mode={GPU_BUCKET_MODE} block={BLOCK} owner_blocks={MAX_BLOCKS} owner_steps={STEPS} task_blocks={GPU_TASK_BLOCKS} task_steps={GPU_TASK_STEPS} log_level={GPU_LOG_LEVEL}")
    bucket_total_counts:List[int]=[0]*GPU_BUCKETS
    ...
    while off < m_all:
      ...
      # 元 constellation → 開始状態 SoA
      build_soa_for_range(N,constellations, off, m,soa,w_arr)

      # CPU浅展開 → 2-pass並列化。
      # pass1: sourceごとのbucket別task数を @par で数える
      # pass2: prefix位置へ @par で直接書き込み、bucketごとのcompact用indexをなくす
      expand_start=datetime.now()
      count_start=datetime.now()
      counts_flat:List[int]=[0]*(m*GPU_BUCKETS)
      @par
      for ci in range(m):
        count_expand_one_to_row(
          func_meta,
      ...
      count_dt=datetime.now()-count_start

      prefix_start=datetime.now()
      prefix_flat:List[int]=[0]*(m*GPU_BUCKETS)
      bucket_starts:List[int]=[0]*GPU_BUCKETS
      bucket_counts:List[int]=[0]*GPU_BUCKETS
      total_tasks:int=0
      for bucket_id in range(GPU_BUCKETS):
        bucket_starts[bucket_id]=total_tasks
        run:int=total_tasks
        for ci in range(m):
          pbase:int=ci*GPU_BUCKETS+bucket_id
          prefix_flat[pbase]=run
          run+=counts_flat[pbase]
        bucket_counts[bucket_id]=run-total_tasks
        total_tasks=run
      prefix_dt=datetime.now()-prefix_start

      fill_start=datetime.now()
      expanded:ExpandedFlatTasks=ExpandedFlatTasks(total_tasks)
      @par
      for ci in range(m):
        fill_expand_one_to_row(
          func_meta,
          blockK_by_funcid,blockL_by_funcid,
          board_mask,
          soa,w_arr,
          ci,off+ci,
          GPU_SPLIT_ROW,
          GPU_BUCKET_MODE,GPU_BUCKETS,
          prefix_flat,
          ci*GPU_BUCKETS,
          expanded
        )
      fill_dt=datetime.now()-fill_start
      expand_dt=datetime.now()-expand_start
      task_count:int=total_tasks
      expand_total+=task_count
      if GPU_LOG_EXPAND:
        print(f"[gpu-expand] N={N} chunk={input_chunks} owner={off}-{off+m-1} base={m} tasks={task_count} split_row={GPU_SPLIT_ROW} time={str(expand_dt)[:-3]} count={str(count_dt)[:-3]} prefix={str(prefix_dt)[:-3]} fill={str(fill_dt)[:-3]}")

      # bucket は expanded 内で連続配置済み。atomic は使わず、戻ってから owner に逐次加算。
      for bucket_id in range(GPU_BUCKETS):
        bucket_m:int=bucket_counts[bucket_id]
        if bucket_m==0:
          continue
        bucket_total_counts[bucket_id]+=bucket_m
        bucket_start=datetime.now()
        bucket_base:int=bucket_starts[bucket_id]
        pos:int=0
        while pos<bucket_m:
          cm:int=min(GPU_TASK_STEPS,bucket_m-pos)
          for ii in range(cm):
            t:int=bucket_base+pos+ii
            gpu_soa.ld_arr[ii]=expanded.ld_arr[t]
            gpu_soa.rd_arr[ii]=expanded.rd_arr[t]
            gpu_soa.col_arr[ii]=expanded.col_arr[t]
            gpu_soa.row_arr[ii]=expanded.row_arr[t]
            gpu_soa.free_arr[ii]=expanded.free_arr[t]
            gpu_soa.jmark_arr[ii]=expanded.jmark_arr[t]
            gpu_soa.end_arr[ii]=expanded.end_arr[t]
            gpu_soa.mark1_arr[ii]=expanded.mark1_arr[t]
            gpu_soa.mark2_arr[ii]=expanded.mark2_arr[t]
            gpu_soa.funcid_arr[ii]=expanded.funcid_arr[t]
            w_task[ii]=expanded.w_arr[t]
            owner_batch[ii]=expanded.owner_arr[t]

          GRID = (cm + BLOCK - 1) // BLOCK
          kernel_dfs_iter_gpu(
            gpu.raw(gpu_soa.ld_arr), gpu.raw(gpu_soa.rd_arr), gpu.raw(gpu_soa.col_arr),
            gpu.raw(gpu_soa.row_arr), gpu.raw(gpu_soa.free_arr),
            gpu.raw(gpu_soa.jmark_arr), gpu.raw(gpu_soa.end_arr),
            gpu.raw(gpu_soa.mark1_arr), gpu.raw(gpu_soa.mark2_arr),
            gpu.raw(gpu_soa.funcid_arr), gpu.raw(w_task),
            gpu.raw(meta_next),
            gpu.raw(results),
            cm, board_mask,
            n3, n4,
          ...
          # 同一 owner に複数 task が戻るため、atomic ではなくCPU側で安全に加算する
          for ii in range(cm):
            results_all[owner_batch[ii]]+=results[ii]
          pos+=cm

        bucket_dt=datetime.now()-bucket_start
        if GPU_LOG_BUCKETS:
          fid,funcptn,avail_flag,tail,has_fid=describe_gpu_bucket(func_meta,bucket_id,GPU_BUCKET_MODE)
          if has_fid==1:
            print(f"[gpu-bucket] N={N} bucket={bucket_id:02d} fid={fid:02d} funcptn={funcptn} avail={avail_flag} tail={tail} tasks={bucket_m} time={str(bucket_dt)[:-3]}")
          else:
            print(f"[gpu-bucket] N={N} bucket={bucket_id:02d} funcptn={funcptn} avail={avail_flag} tail={tail} tasks={bucket_m} time={str(bucket_dt)[:-3]}")

      off += m

    print(f"[gpu-summary] N={N} base_constellations={m_all} expanded_tasks={expand_total} chunks={input_chunks}")
    for bucket_id in range(GPU_BUCKETS):
      if bucket_total_counts[bucket_id]>0:
        fid,funcptn,avail_flag,tail,has_fid=describe_gpu_bucket(func_meta,bucket_id,GPU_BUCKET_MODE)
        if has_fid==1:
          print(f"[gpu-summary-bucket] bucket={bucket_id:02d} fid={fid:02d} funcptn={funcptn} avail={avail_flag} tail={tail} total_tasks={bucket_total_counts[bucket_id]}")
        else:
          print(f"[gpu-summary-bucket] bucket={bucket_id:02d} funcptn={funcptn} avail={avail_flag} tail={tail} total_tasks={bucket_total_counts[bucket_id]}")
  ##########
  # CPU 
  ##########
  else:
...
def main()->None:
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [N | N_start N_end] [split_row] [bucket_mode] [owner_blocks] [task_blocks] [log_level]")
      return

    # N指定: ./prog -g 18       -> N=18のみ
    #        ./prog -g 18 20    -> N=18..20
    # split: ./prog -g 18 18 10 -> N=18のみ、split_row=10
    # bucket_mode: 0=旧(funcptn/avail/tail), 1=FID/tail, 2=hybrid(0/2のみFID、1/3/5はcoarse)
    # owner_blocks: 入力 constellation chunk のブロック数。深い split で強制終了する場合は 32->16->8->4 と下げる。
    # task_blocks : expanded task の GPU 投入ブロック数。既定1024。メモリが苦しければ512。
    # log_level   : 0=config+summaryのみ、1=expand+summary、2=bucket詳細も出す。
    if argc >= 3:
      nmin = int(sys.argv[2])
      nmax = nmin + 1
    if argc >= 4:
    ...
    if argc >= 5:
      gpu_split_row = int(sys.argv[4])
    if argc >= 6:
      gpu_bucket_mode = int(sys.argv[5])
    if argc >= 7:
      gpu_owner_blocks = int(sys.argv[6])
    if argc >= 8:
      gpu_task_blocks = int(sys.argv[7])
    if argc >= 9:
      gpu_log_level = int(sys.argv[8])
  else:
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        exec_solutions(N,constellations,use_gpu,gpu_split_row,gpu_bucket_mode,gpu_owner_blocks,gpu_task_blocks,gpu_log_level)
