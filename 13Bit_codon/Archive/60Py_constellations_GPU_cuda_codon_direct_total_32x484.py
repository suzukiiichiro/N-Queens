...
GPU
$ codon build -release 60Py_constellations_GPU_cuda_codon_direct_total_32x484.py
$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -g 5 20 32 484 0 -1

N=20 単体ベンチ
$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -g 20 20 32 484 0 -1

ログ確認
$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -g 20 20 32 484 2 -1
...
suzuki@cudacodon$ ./60Py_constellations_GPU_cuda_codon_direct_total_32x484 -g 5 20 32 484 0 -1
GPU mode selected
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk56={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    ...
    while off < m_all:
      ...
      use_sorted:bool=(gpu_sort_mode==1 or gpu_sort_mode==2 or gpu_sort_mode==3 or gpu_sort_mode==4)
      if gpu_log_level>=2:
        ts0=datetime.now()
      if use_sorted:
        nb:int=28
      ...
      if gpu_log_level>=2:
        t3=datetime.now()
        print(f"[gpu-chunk] N={N} chunk={chunks} off={off} m={m} grid={GRID} sort={gpu_sort_mode} build={str(t1-t0)[:-3]} sort_time={str(ts1-ts0)[:-3]} kernel={str(t2-ts1)[:-3]} sum={str(t3-t2)[:-3]}")
      off += m

    if m_all>0:
      constellations[0]["solutions"] = gpu_total
    if gpu_log_level>=1:
      print(f"[gpu-summary] N={N} constellations={m_all} chunks={chunks} direct_total=1")
    return
  ##########
  # CPU 
  ##########
