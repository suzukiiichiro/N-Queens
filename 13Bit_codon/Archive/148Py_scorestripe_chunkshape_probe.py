...
作成しました。`nld`／`nrd`を完全に`u32`化したコンパイル確認版です。

[148Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/148Py_scorestripe_chunkshape_probe.py)
[148Py1-6.sh](sandbox:/mnt/data/148Py1-6.sh)
[148Py1-6_locked.sh](sandbox:/mnt/data/148Py1-6_locked.sh)
[148Py一式 tar.gz](sandbox:/mnt/data/148Py_u32_nld_nrd_bundle.tar.gz)

`u32`左辺の全シフトについて、右辺も`stepu:u32`または`u32(1)`に統一しました。Codonの固定幅`UInt[N]`シフト演算は右辺にも同じ`UInt[N]`型を取ります。([GitHub][1])

```python
nld:u32 = u32(0)
nrd:u32 = u32(0)

stepu:u32 = u32((cvi >> 10) & 3)
addvu:u32 = u32((cvi >> 12) & 1)
bLiu:u32 = u32((cvi >> 15) & 3)
...
`bit_i`、`int(ld[sp])`、`int(rd[sp])`、`u32(nld)`、`u32(nrd)`は対象hotpathから除去しています。

```bash
codon build -release 148Py_scorestripe_chunkshape_probe.py
./148Py1-6.sh
./148Py1-6_locked.sh
```
...
```text
bb9b92cb9d1ce0229d85f7fff65359ea4e7971a663116158eb46497deb9a6ce0
```

シェル構文と対象シフト式の型閉包は確認済みです。Codon実機コンパイルは未実施です。

[1]: https://github.com/exaloop/codon/blob/develop/stdlib/internal/types/int.codon "codon/stdlib/internal/types/int.codon at develop · exaloop/codon · GitHub"
...
VERSION_TAG:str="148 nld/nrd full-u32 hotpath over 147 mark-select - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    # 148: fixed-width operands used by the u32 diagonal hot path.
    n3u:u32=u32(n3)
    n4u:u32=u32(n4)
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148
...
####################################################################################################
# 148 nld/nrd full-u32 inherited score-aware chunk-shape layer
# The complete validated 141 scorestripe_v9_lanephase32_octetfirstpairlock29 permutation is
# retained byte-for-byte in algorithmic terms; only cache/log labels are isolated for 148.
#
# Safety boundary:
...
#   - host shaping is byte-equivalent to validated 147; 148 changes only nld/nrd arithmetic inside kernel_dfs_iter_gpu
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="inherit validated 147 scorestripe_v9 task order unchanged; kernel experiment keeps nld/nrd in u32 end-to-end"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  148 inherited score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 148 experiment keeps the 147 mark selector and changes only nld/nrd arithmetic inside the GPU kernel.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  148 host-side score-aware lanephase chunk shaping inherited unchanged from validated 147.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 148 experiment changes only nld/nrd arithmetic; host shaping changes only cache/log/version labels.
...
def main()->None:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split148-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split148-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split148_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split148-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split148-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split148_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
