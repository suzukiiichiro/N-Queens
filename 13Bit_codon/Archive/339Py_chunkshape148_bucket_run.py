...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 339 (2026-07-27)
...
1. [結論・確定・採用] 非コアレッシングメモリアクセス(w_arrのSoA分割)
   ...
   w_hi_arr読み込み3箇所全てのL2 Excessive=0を確認し、正式にADOPT済み。
...
7. [新規発見・338・未着手・低リスク] w_hi_arrは恒等的にゼロ
   338の仕様化作業中に確認した事実: `symmetry()`(2519行)の戻り値は
   `u64(2)`/`u64(4)`/`u64(8)`の3値のみであり、w_arrの値域は{2,4,8}に
   限られる。したがって328で分割した`w_hi_arr`は**全要素が恒等的に0**
   であり、maxd14カーネルが読む3箇所(576/581/750行)のロードは常に0を
   返す純粋な無駄である。さらにw=1<<e(e∈{1,2,3})なので、`markctrl`の
   未使用ビット20-21にeを詰めれば`w_lo_arr`も不要になり、エピローグの
   64bit乗算はシフトに退化する(markctrlはbit0-19のみ使用: jmark(0-4)/
   endmark(5-9)/mark1(10-14)/mark2(15-19))。プロローグ/エピローグ
   のみの変更でホットループには一切触れないため、5戦5敗のカーネル分解
   とは別軸である。期待効果は小(内側DFSループ支配のため<0.5%と見込む)
   だが、リスクとコストも小。339以降の単一変数実験候補として記録する。

8. [実装・339で対策を実装] warp内コスト不均質(tail effectの直接原因)
   329で確定した「Divergent Branches=0、かつAvg Threads Executed 2〜3/32」
   は、分岐発散ではなくwarp内32レーンのDFS試行回数(trip count)の
   ばらつきを意味する。338で仮説として立て、339でソースを精読して
   **実装上の根拠と対策点を特定した**。

   (a) インデクシング: grid-strideにより、同一warpの32レーンが同一
       イテレーションで処理するのは配列上で連続する32レコード
       (idx = n*stride + 32w + j、j=0..31)である。

   (b) 現状の並べ替えが何をしているか: `build_chunkshape148_reordered_bin`
       は`interleave_order=[7,0,6,1,5,2,4,3]`(スコア最高→最低→2番目に
       高い→…)に従い、**1レコードずつバケットを切り替えて**出力して
       いる。したがって連続する32レコード(=1 warp-iteration)は8つの
       スコアバケット全部を4回ずつ含む、**意図的に最も不均質な**構成に
       なっている。276以来この形である。

   (c) しかしこれはwarp「間」の均衡のための設計でもある: 全warpが同じ
       バケット構成を受け取るため、特定のwarpだけが遅くなることがない。
       単純に全体をコスト順ソートすると今度はwarp間が崩れる。

   (d) **両立できる**: `build_chunkshape148_reordered_bin`の作業単位
       `STEPS=BLOCK*MAX_BLOCKS=15488`は**grid-stride 1イテレーション
       そのもの**であり、バケット順は`order_pos=(oi+out_ch)%8`で
       イテレーションごとに1つ回転する。よって「1バケットから32件
       連続で出す」ようにすると、warp w はイテレーションout_chで
       `interleave_order[(w+out_ch)%8]`を受け取り、N=21の131
       イテレーション全体では全8バケットをほぼ一様に経験する。
       **warp内は均質化され、warp間の均衡は保たれる。**

   339はこれを`CHUNKSHAPE148_BUCKET_RUN`という単一のノブとして実装した
   (1=276以来の現行動作、32=warp幅)。カーネル・ディスパッチャは
   328-338と一切変更なし、ホスト側の並べ替えのみ。並べ替えは順列で
   あるため総和は不変であり、正当性オラクル`314666222712`は構造的に
   保たれる。RUN=1は出力ファイル名も含めて現行とバイト一致し、
   333-338のベースラインは再ビルド不要で再現できる(RUN=1のとき
   のみファイル名にサフィックスを付けない)。
   期待効果は不明。ホスト側並べ替えは本プロジェクト最良の戦績
   (K-batching -4.4%、w3_j7 -1.3%)を持つ軸だが、(c)の綱引きが
   どちらに転ぶかは実測でしか分からない。悪化した場合は「warp間
   均衡が支配的」という重要な知見が得られ、その場合は338で保留した
   Σmax32/Σmean32の直接測定(診断カーネル)に進む。

このセクション自体は通常カーネルロジックとは独立しています。
339は**カーネル・ディスパッチャを一切変更しません**(5つの
@gpu.kernel、launch_kernel_dfs_iter_gpu_static_maxd、K=48、SoA
w_lo_arr/w_hi_arr、funcid_reorder w3_j7 はすべて328-338と同一)。
339のコード変更はホスト側の並べ替え
`build_chunkshape148_reordered_bin` に限定され、その中身も
「1バケットから連続何件出すか」というノブ`CHUNKSHAPE148_BUCKET_RUN`
の追加のみです。**ソース既定値は1(=276以来の現行動作)であり、
引数を渡さなければ338とバイト単位で同じ順序・同じキャッシュ
ファイル名を再現します。** 検証ハーネスが明示的に32を渡します。
================================================================================
...
**338の実行結果(確認)**
- 静的チェック56項目 FAILゼロ、正当性314666222712一致、elapsed 450.329s(337の450.056s比-0.061%、ノイズ内)

**339の内容(ホスト側並べ替えのみ、カーネル非接触)**
338で立てた項目8の仮説を追う過程で、`build_chunkshape148_reordered_bin`を精読して決定的な事実が分かりました:

- この関数の作業単位`STEPS=BLOCK*MAX_BLOCKS=15488`は、**grid-stride 1イテレーションそのもの**です(`exec_solutions_gpu_chunk_split145`側の`STEPS=THREADS*K=743424`とは別物)
- 出力は`interleave_order=[7,0,6,1,5,2,4,3]`(スコア最高→最低→2番目に高い→…)を**1レコードずつ**回しています。つまり連続32レコード=1 warp-iterationは、8バケット全部を4回ずつ含む**意図的に最も不均質な**構成です
- ただしこれはwarp間の均衡設計でもあります。全warpが同じ構成を受け取るので、特定warpだけが遅くなりません

**両立できます。** バケット順は`(oi+out_ch)%8`でイテレーションごとに1つ回転するので、「1バケットから32件連続で出す」ようにすると warp w はイテレーションout_chで`interleave_order[(w+out_ch)%8]`を受け取り、131イテレーション全体では全8バケットをほぼ一様に経験します。warp内は均質化、warp間は維持。

これを`CHUNKSHAPE148_BUCKET_RUN`という単一ノブとして実装しました(1=現行、32=warp幅)。ソース既定は1のままで、引数を渡さなければ338とバイト一致・キャッシュファイル名も同一です。

**期待効果は正直なところ不明です。** 悪化した場合は「warp間均衡が支配的」という重要な知見になり、338で保留したΣmax32/Σmean32の直接測定に進みます。

338のログを確認します。338、確認しました。FAILゼロ・正当性一致・elapsed **450.329s**(337比 −0.061%、ノイズ内)で完全再現です。339の設計に入る前に、ソースを数点確認します。338の結果を踏まえ、339の設計材料としてリオーダ実装を精読しました。**重要な発見があります。**`build_chunkshape148_reordered_bin` の `STEPS = BLOCK*MAX_BLOCKS = 15488` は、**grid-stride 1イテレーション分そのもの**でした。339を作ります。Ç¥å��Ñ��ÑÅ��Ñ��Ñ��ÑÅ��Ñ��Ç¥åÆ��Ç¥å��RUN=1Ç¥åÆ��Ç¥å��Ç¥åâ��Ñ��Ç¥åÆ��Ç¥åÆ��Ç¥å��Ç¥åÆ��Ç¥å��Ç¥åâ��Ç¥å��Ç¥å��Ç¥åÆ��Ç¥å��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ç¥åâ��Ç¥å��Ç¥åÆ��Ç¥åâ��Ç¥åÆ��Ñ��ÑÅ��Ñ��Ñ��ÑÅ��Ñ��Ñ��Ç¥å��Ç¥åÆ��Ñ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��Ç¥åÆ��Ç¥å��Ñ��**338の結果**: 静的チェック56項目 FAILゼロ、正当性 `314666222712` 一致、elapsed **450.329s**(337比 −0.061%、ノイズ内)。設計のみのリビジョンとして期待どおりです。

---

## 339 は方針を変えました

338の仕様書を書いたあと、項目8を追うために `build_chunkshape148_reordered_bin` を精読したところ、**予定していた「まず測定」より先に手を出すべき対象が見つかりました**。

### 判明した3つの事実

1. この関数の作業単位 `STEPS = BLOCK*MAX_BLOCKS = 15488` は、**grid-stride 1イテレーションそのもの**です。`exec_solutions_gpu_chunk_split145` 側の `STEPS = THREADS*K = 743424` とは別物で、ここを取り違えると設計を誤ります(私も一度誤りかけました)

2. 出力ループは `interleave_order=[7,0,6,1,5,2,4,3]`(スコア最高→最低→2番目に高い→…)を **1レコードずつ**回しています。つまり連続する32レコード = 1 warp-iteration は8バケット全部を4回ずつ含む、**意図的に最も不均質な構成**です。276以来この形

3. ただしこれは warp「間」の均衡設計でもあります。全warpが同じ構成を受け取るので、特定warpだけが遅くならない。**単純な全体ソートは、この性質を壊して別の不均衡と交換するだけ**です

### 両立できます

バケット順は `order_pos=(oi+out_ch)%8` でイテレーションごとに1つ回転します。したがって「1バケットから32件連続で出す」ようにすると:

```
warp w がイテレーション out_ch で受け取るバケット = interleave_order[(w+out_ch)%8]
→ N=21 の131イテレーション全体で、全warpが全8バケットをほぼ一様に経験
```

**warp内は均質化、warp間の均衡は維持。** これを `CHUNKSHAPE148_BUCKET_RUN` という単一ノブとして実装しました(1=現行、32=warp幅)。

### 安全性

- **カーネル・ディスパッチャは328-338とバイト一致**(5つの `@gpu.kernel`、ディスパッチャ、K=48、SoA、w3_j7 すべて)。変更はホスト側の並べ替えのみ
- **正当性は構造的に保証**されます。並べ替えは順列なので総和は順序に依存しません。取りこぼし・重複があれば `dispatch_task_sum`(2025282)と `full_chunk_sum` が検出します
- **ソース既定値は1**。引数を渡さなければ338とバイト一致し、`chunkshape148_bucket_run_tag()` がRUN=1のときだけ空文字を返すので**既存キャッシュのファイル名も変わりません**
- RUN=1の等価性は、元ループと新ループを同一入力で回して**出力インデックス列の完全一致を6ケースで確認済み**です(RUN=2/8/32が同一集合の順列であることも確認)

---

## 実行手順(重要な注意あり)

RUN=32には対応するキャッシュがないため、**1回目の実行はshaped binの再構築を含み、N=21のelapsed行にその時間が乗ります**。332/333のw3_j7測定と同じで、**2回目(キャッシュ済み)が比較対象**です。ハーネスが `chunkshape148_cache_state=build|reuse` を summary に記録するので、比較可能かどうかは summary 自身が示します。

```
STATIC_ONLY=1 bash 339Py_chunkshape148_bucket_run_validate_N21_full_once.sh
bash 339Py_chunkshape148_bucket_run_validate_N21_full_once.sh   # 1回目: キャッシュ構築+正当性
bash 339Py_chunkshape148_bucket_run_validate_N21_full_once.sh   # 2回目: これが比較対象
CHUNKSHAPE148_BUCKET_RUN=1 bash 339Py_...sh                     # 任意: 同一バイナリでの338対照点
```

静的チェックは手元で両モード通してあります(38 OK / 0 FAIL、新規4件を含む)。`bash -n` も通過済みです。

**期待値は置きません。** (c)と(d)の綱引きがどちらに転ぶか事前に決めつけると解釈が歪むためです。悪化した場合も「warp間均衡が支配的」という決定的な知見になり、その場合は338で保留した Σmax32/Σmean32 の診断カーネルに進みます。改善すれば340で RUN∈{2,4,8,16,64} のスイープです。
"""
...
VERSION_TAG:str="339 chunkshape148-bucket-run: NO kernel or dispatcher change -- all five @gpu.kernel bodies, launch_kernel_dfs_iter_gpu_static_maxd, K_PER_THREAD_MAXD14=48, the SoA w_lo_arr/w_hi_arr split and the w3_j7 funcid_reorder defaults are byte-for-byte identical to 338/337/336/335/334/333/328-332. 338's own full N=21 validate is CONFIRMED: correctness 314666222712, elapsed 450.329s (vs 337's 450.056s, -0.061%, within noise), zero static-check failures. 339 is the first revision to act on Open Objectives #8 (intra-warp cost variance = the tail effect measured in 329: Divergent Branches=0 with Avg Threads Executed 2-3/32). Source reading established three facts: (a) grid-stride indexing means a warp's 32 lanes process 32 CONSECUTIVE records per iteration (idx = n*stride + 32w + j); (b) build_chunkshape148_reordered_bin has since rev276 emitted ONE record per score bucket while cycling interleave_order=[7,0,6,1,5,2,4,3] (highest score, lowest, second-highest, ...), so every aligned 32-record group deliberately spans all 8 buckets four times over -- i.e. the current order maximises intra-warp cost heterogeneity; (c) that same interleave is what keeps WARPS balanced against each other, so a naive global cost sort would trade one imbalance for another. The resolution is that both can hold at once: build_chunkshape148_reordered_bin works in units of STEPS=BLOCK*MAX_BLOCKS=15488, which is exactly ONE grid-stride iteration, and it already rotates the bucket order by out_ch each iteration (order_pos=(oi+out_ch)%8). Emitting 32 consecutive records from a single bucket therefore gives warp w the bucket interleave_order[(w+out_ch)%8] at iteration out_ch, so each warp still sees all 8 buckets near-uniformly across the 131 iterations of an N=21 run while each individual warp-iteration becomes cost-homogeneous. 339 implements this as a single knob, CHUNKSHAPE148_BUCKET_RUN: the number of records emitted consecutively from one bucket before rotating. The SOURCE DEFAULT REMAINS 1, which is byte-for-byte the 276-338 emit order and, crucially, also keeps the existing cached shaped-bin FILENAME unchanged (chunkshape148_bucket_run_tag() returns an empty string only for 1), so the 333-338 baseline stays reproducible with no rebuild. The validate harness passes 32 explicitly. Because reordering is a permutation, the total is invariant by construction and the correctness oracle 314666222712 is structurally preserved; a run with a new BUCKET_RUN value rebuilds the shaped bin, so its reported elapsed includes cache construction and the harness records chunkshape148_cache_state=build/reuse to flag whether the timing row is comparable. Expected effect is genuinely unknown: host-side reordering is this project's best-performing axis (K-batching -4.4%, w3_j7 -1.3%), but the intra-warp vs inter-warp tug-of-war in (c) can go either way. A regression would itself be the informative result -- it would establish that inter-warp balance dominates, at which point the deferred Sum(max over 32)/Sum(mean over 32) diagnostic kernel from 338 becomes the next step."
...
A10G_FINAL_DEFAULT_CHUNKSHAPE148_BUCKET_RUN:int=1  # 339: bare -g keeps the 276-338 order
...
def chunkshape148_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape148_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}{chunkshape148_bucket_run_tag()}.bin"
...
CHUNKSHAPE148_BUCKET_RUN:int=1
CHUNKSHAPE148_BUCKET_RUN_MAX:int=256
CHUNKSHAPE148_BUCKET_RUN_REASON:str="339 introduces the bucket run-length knob for Open Objectives #8 (intra-warp cost homogeneity). Source default stays 1 = the 276 scorestripe_v9 behaviour so that omitting the argument reproduces 333-338 exactly; the validate harness passes 32 explicitly."
...
def build_chunkshape148_reordered_bin(
  ...
  if gpu_log_level>=1:
    print(f"[chunkshape148-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} bucket_run={chunkshape148_bucket_run_value()} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE148_DEFAULT_REASON}")
