...
# =============================================================================
# NQ_UPDATE_MEMO
# 208: 更新メモ: 206Pyは正当性OK。208Pyは204/206のroot_after_second scalar形を維持し、MAXD14 kernelのみroot-preroll内のpr_descend初期化をpr_nf由来にして、pr_nf==0分岐を削る微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 208 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 208 returns to
...
# 204 clear-lowbit root_rest plus explicit root_after_second scalar predicate.
...
#   - restore-rootafter-scalar source check is whitespace/comment robust.
SRC=${SRC:-./208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 208 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/208Py_kernel_roottwo_prterminalfirst_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 208Py source
#   has a harmless header/string/layout difference. This 208 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second scalar if + root-preroll terminal-depth-zero before future guard	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
marker='206restore_rootafter_scalar:'
root_pos=compact.find('root_first:u32=cur_avail&(u32(0)-cur_avail)')
# Do not anchor on prose/comment markers; source headers may mention rejected forms.
...
checks = {
    ...
    'root_after_scalar_present': 'root_after_second:u32=root_rest^root_second' in compact,
    'root_scalar_if_present': 'ifroot_after_second==u32(0):' in compact,
    'root_inline_if_absent': 'if(root_rest^root_second)==u32(0):' not in root_block,
    'root_preroll_flag_absent': 'root_preroll:u32' not in s,
    # 208 keeps 204/197 save payload; 203 depth0-save is not adopted.
    ...
    # 208 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 208 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 208 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_restore_rootafter_scalar_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_restore_rootafter_scalar\tpresent_root_after_scalar_predicate\tpresent_root_after_scalar_predicate\tOK
' >> "$SUMMARY"
else
  printf 'source_restore_rootafter_scalar\tpresent_root_after_scalar_predicate\tmissing_or_inline_predicate\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if python3 - "$SRC" <<'PYTERM'
import sys
s=open(sys.argv[1], encoding='utf-8').read()
compact=''.join(s.split())
root_pos=compact.find('root_first:u32=cur_avail&(u32(0)-cur_avail)')
if root_pos < 0:
    root_pos=compact.find('pr_nf:u32=bm&~(pr_nld|pr_nrd|pr_ncol)')
gen_pos=compact.find('whileTrue:', root_pos)
root_block=compact[root_pos:gen_pos if gen_pos >= 0 else len(compact)]
term_pos=root_block.find('ifterminal_depth==0:')
fut_pos=root_block.find('iffuture_check_mask!=u32(0):')
checks={
  'terminal_before_future_present': term_pos >= 0 and fut_pos >= 0 and term_pos < fut_pos,
  'root_after_scalar_retained': 'root_after_second:u32=root_rest^root_second' in root_block and 'ifroot_after_second==u32(0):' in root_block,
  'old_pr_descend_one_retained': 'pr_descend:u32=u32(1)' in root_block,
  'descend_from_nf_absent': 'pr_descend:u32=pr_nf' not in root_block,
  'generic_future_guard_retained': compact.find('whileTrue:', root_pos) >= 0 and 'iffuture_check_mask!=u32(0):' in compact[gen_pos:],
}
if all(checks.values()):
    sys.exit(0)
print('source_prterminalfirst_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYTERM
then
  printf 'source_prterminalfirst	root-preroll terminal_depth0 before future guard	root-preroll terminal_depth0 before future guard	OK
' >> "$SUMMARY"
else
  actual_term=$(python3 - "$SRC" <<'PYTERM2'
import sys
s=open(sys.argv[1], encoding='utf-8').read()
compact=''.join(s.split())
root_pos=compact.find('root_first:u32=cur_avail&(u32(0)-cur_avail)')
...
term_pos=root_block.find('ifterminal_depth==0:')
fut_pos=root_block.find('iffuture_check_mask!=u32(0):')
print('terminal_before_future' if term_pos >= 0 and fut_pos >= 0 and term_pos < fut_pos else 'missing_or_wrong_order')
PYTERM2
)
  printf 'source_prterminalfirst	root-preroll terminal_depth0 before future guard	%s	FAIL
' "$actual_term" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split208'}
bad=sorted(tags-allowed)
if 'split208' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split208 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split208 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 208 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 208 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-prterminalfirst static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 208 roottwo-prterminalfirst futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 208 roottwo-prterminalfirst futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
