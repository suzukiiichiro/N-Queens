...
ログ確認しました。**119 d2base14-only は成功**です。

chunk 40 と 120 で `d2base14=15 / 104` と専用 kernel が実際に起動しており、`partial_total=7287470230` で 117/116 の基準値と一致しています。さらに 3 chunk 合計時間は 117 の `1:29.430` から 119 の `1:18.796` へ短縮しています。

次段階として **120 d2 fid13 + base14 kernel probe** を作成しました。

[120Py_d2fid13_14_kernel_probe.py](sandbox:/mnt/data/120Py_d2fid13_14_kernel_probe.py)

120 で追加した新規 PTX kernel はこれです。

```text
kernel_dfs_d2fid13_gpu
  funcid 13 : SQd2BlB
    通常 +1 は fid13 継続
    row == mark2 で step=2, add1=1
    その後 fid14 / SQd2B へ接続
```

119 で通った `funcid 14 / SQd2B` 専用 kernel はそのまま残しています。

```text
専用 kernel:
  funcid 13 : kernel_dfs_d2fid13_gpu
  funcid 14 : kernel_dfs_d2base14_gpu

汎用 kernel:
  funcid 13,14 以外すべて
```

今回の実装は、13Py の `SQd2BlB` が `row==mark2` で `((ld|bit)<<2)|1` に進み、その後 `SQd2B` に接続する構造に合わせています。 115 側でも funcid 13 は `SQd2BlB`、funcid 14 は `SQd2B` として定義されています。

実行はまず同じ chunk list でお願いします。

```bash
codon build -release 120Py_d2fid13_14_kernel_probe.py
```

```bash
./120Py_d2fid13_14_kernel_probe \
...
```text
[split120-buckets] N=21 m=15488 d2fid13=... d2base14=... rest=...
```
...
CUDA_ERROR_INVALID_PTX が出る
  → 新規追加した fid13 kernel が原因。
     fid13 をさらに縮小します。

PTX は通るが partial_total が 7287470230 と違う
  → fid13 → fid14 接続ロジック差分。
     row==mark2 / step=2 / add1=1 の周辺を修正します。

PTX が通り、partial_total が一致
  → fid13 も採用候補。
     次は fid17 / SQd2BkB を同じ方式で追加します。
```

まだ N21 full は不要です。まずは `d2fid13>0` の chunk で PTX が通るかだけ確認してください。
...
VERSION_TAG:str="120 d2fid13+base14 kernel probe - rest uses proven 115 GPU kernel"
...
####################################################################################################
# 120 d2fid13+base14 kernel probe
#
...
#   - add exactly one new specialized kernel at a time in a 120 probe
...
# every bucket still used the generic 115 kernel.  This 120 build adds exactly one new PTX target on top of the validated fid14 base kernel:
...
def kernel_dfs_d2base14_gpu(
    ...
    """120: SQd2B base fid14 だけを処理する最小 PTX kernel。"""
    ld=__array__[int](MAXD)
    rd=__array__[int](MAXD)
    col=__array__[u32](MAXD)
    avail=__array__[u32](MAXD)
...
def kernel_dfs_d2fid13_gpu(
    ...
    """
    120: SQd2BlB / fid13 だけを処理する小型 PTX kernel。
...
####################################################################################################
# 120 d2fid13+base14 split-kernel chunk runner
####################################################################################################

"""120: SoA 1件を別 SoA へコピーする小ヘルパ。"""
...
"""120: fid13 + d2 base fid14 だけ専用 kernel、その他は 115 汎用 kernel にまとめる。"""
def exec_solutions_gpu_chunk_split120(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  """
  120 d2 fid13+base14 split:
    - build one SoA for the input chunk
    - split funcid 13 into a new SQd2BlB(fid13)->SQd2B(fid14) bucket
    - split funcid 14 into the already validated SQd2B base bucket
    - run all other tasks as one rest bucket through the proven 115 generic kernel

  The new PTX risk in this file is intentionally limited to kernel_dfs_d2fid13_gpu.
  Use mode 30 probe chunks first; N21 full is still unnecessary at this stage.
  """
  board_mask:int=(1<<N)-1
  ...
  fid13_soa:TaskSoA=TaskSoA(STEPS)
  d2base_soa:TaskSoA=TaskSoA(STEPS)
  rest_soa:TaskSoA=TaskSoA(STEPS)
  fid13_w:List[u64]=[u64(0)]*STEPS
  d2base_w:List[u64]=[u64(0)]*STEPS
  ...
  while i<m:
    fid:int=soa.funcid_arr[i]
    if use_d2_specialized and fid==13:
      copy_soa_record_116(soa,w_arr,i,fid13_soa,fid13_w,fid13_m)
      fid13_m+=1
    elif use_d2_specialized and fid==14:
      copy_soa_record_116(soa,w_arr,i,d2base_soa,d2base_w,d2base_m)
      d2base_m+=1
    else:
  ...
  if gpu_log_level>=1:
    print(f"[split120-buckets] N={N} m={m} d2fid13={fid13_m} d2base14={d2base_m} rest={rest_m} split_mode={split_mode} d2_specialized={1 if use_d2_specialized else 0}")

  meta_next:List[u8]=[u8(1),u8(2),u8(3),u8(3),u8(2),u8(6),u8(2),u8(2),u8(0),u8(4),u8(5),u8(7),u8(13),u8(14),u8(14),u8(14),u8(17),u8(14),u8(14),u8(20),u8(21),u8(21),u8(21),u8(25),u8(21),u8(21),u8(26),u8(26)]

  # New specialized bucket first: PTX/JIT failures should appear before generic rest work.
  if fid13_m>0:
    GRID=(fid13_m+BLOCK-1)//BLOCK
    kernel_dfs_d2fid13_gpu(gpu.raw(fid13_soa.ld_arr),gpu.raw(fid13_soa.rd_arr),gpu.raw(fid13_soa.col_arr),gpu.raw(fid13_soa.row_arr),gpu.raw(fid13_soa.free_arr),gpu.raw(fid13_soa.jmark_arr),gpu.raw(fid13_soa.end_arr),gpu.raw(fid13_soa.mark1_arr),gpu.raw(fid13_soa.mark2_arr),gpu.raw(fid13_soa.funcid_arr),gpu.raw(fid13_w),gpu.raw(results),fid13_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<fid13_m:
      chunk_total+=int(results[i])
      i+=1

  # Already validated in 119.
  if d2base_m>0:
    GRID=(d2base_m+BLOCK-1)//BLOCK
    kernel_dfs_d2base14_gpu(gpu.raw(d2base_soa.ld_arr),gpu.raw(d2base_soa.rd_arr),gpu.raw(d2base_soa.col_arr),gpu.raw(d2base_soa.row_arr),gpu.raw(d2base_soa.free_arr),gpu.raw(d2base_soa.jmark_arr),gpu.raw(d2base_soa.end_arr),gpu.raw(d2base_soa.mark1_arr),gpu.raw(d2base_soa.mark2_arr),gpu.raw(d2base_soa.funcid_arr),gpu.raw(d2base_w),gpu.raw(results),d2base_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<d2base_m:
      chunk_total+=int(results[i])
      i+=1

  # Rest bucket: one generic launch.  This avoids the 116 PTX-safe multi-bucket overhead.
  if rest_m>0:
  ...
  elapsed_ms:int=stage_no_read_ms
  stages:List[int]=[stage_soa_ms,stage_stats_ms,stage_split_ms,stage_kernel_reduce_ms,stage_compute_ms,stage_no_read_ms,fid13_m,d2base_m,rest_m]
  return chunk_total,stats,stages,elapsed_text,elapsed_ms
def exec_solutions_gpu_bin_stream_split120(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split120_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split120_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split120_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split120_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split120-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split120-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split120-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split120-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split120(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split120-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split120-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split120 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split120 full run / optional multi-GPU worker
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 30 8 7 0 1222 5 "" 2  # 120 d2fid13+base14 probe chunks 1222..1226
      #   -g 22 22 32 484 1 0 7 31 8 7 0 0 1 2  # 120 d2fid13+base14 full GPU single worker
      if argc >= 11:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split120-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split120 worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split120_d2fid13_14_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split120_d2fid13_14_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split120-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split120-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split120_probe_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split120(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split120-probe-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split120-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split120-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split120-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split120_full_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split120(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split120-full-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
