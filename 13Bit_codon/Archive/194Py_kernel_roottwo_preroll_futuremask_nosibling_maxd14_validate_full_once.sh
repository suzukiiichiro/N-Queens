...
# =============================================================================
# NQ_UPDATE_MEMO
# 194: 更新メモ: 193は正当性OKだが192比で7.752秒遅く不採用。192のfuture_check_mask guard/no-sibling/root-single prerollを親に、MAXD14 kernelのみ root_avail 1bit/2bit threadでdepth0先頭候補をDFS loop前に1段prerollする単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 194 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 194 starts from
# validated 192 because 192 is correct and numerically 0.019s faster than 188,
# while 189 forced-chain, 190 blockmask guard, and 191 place-xor remain not adopted. 194 changes
# only the MAXD14 root one/two-candidate preroll.
#
# Kernel arithmetic keeps 192/188 bitboard/schedule semantics: register-held current
...
# before the generic DFS loop. In the two-candidate case, the remaining root
# sibling is saved exactly as the generic loop would save it when the first child
# descends; roots with three or more candidates use the exact 192 loop. Host task
# order, dispatch, cache, progress verification, and README-managed history remain unchanged.
SRC=${SRC:-./194Py_kernel_roottwo_preroll_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./194Py_kernel_roottwo_preroll_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/194Py_kernel_roottwo_preroll_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 194 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/194Py_kernel_roottwo_preroll_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 194Py source
#   has a harmless header/string/layout difference. This 194 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 192 root-single futuremask/no-sibling MAXD14 dispatch; 193 terminal-prefuture, 189 forced-chain, 190 blockmask, and 191 place-xor not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate preroll	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'root_two_preroll' "$SRC" && grep -q 'root_preroll:u32=u32(0)' "$SRC" && grep -q 'elif (root_rest&(root_rest-u32(1)))==u32(0):' "$SRC" && ! grep -q 'if (cur_avail&(cur_avail-u32(1)))==u32(0):' "$SRC"; then
  printf 'source_root_two_preroll	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_root_two_preroll	present	missing_or_old_singleton_only	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split194' "$SRC" && ! grep -q 'split193' "$SRC" && ! grep -q 'split192' "$SRC" && ! grep -q 'split191' "$SRC" && ! grep -q 'split190' "$SRC" && ! grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split194 only	split194 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split194 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 194 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source roottwo-preroll static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 194 roottwo-preroll futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 194 keeps the validated 192 host/chunk structure, then derives chunk-tail diagnostics from
...
# 194: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 192 roottwo-preroll futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
