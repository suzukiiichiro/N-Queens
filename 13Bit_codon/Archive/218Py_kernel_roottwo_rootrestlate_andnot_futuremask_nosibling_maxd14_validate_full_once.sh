...
# =============================================================================
# NQ_UPDATE_MEMO
# 218: 更新メモ: 217Py rootrestlate は正当性OKかつ204Pyを上回ったため新最速基準として採用。218Pyは217Pyを親に、MAXD14 kernelのみ root_after_second scalar を XOR から and-not へ変更する再結合実験を検証する。
# Full update history: see README.md
# =============================================================================

# 218 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 218 keeps the
# near-fastest 211 root_first-late form because 216 was correct but did not
# beat 204/211. 218 is the first 217-neighborhood recombination before moving to
# MAXD13/save_sp diagnostics if it does not beat 204.
#
# Kernel arithmetic keeps 204/197/196/194/192/188 bitboard/schedule semantics:
...
# and wider roots remain generic. The root<=2 predicate uses the validated
# 204 clear-lowbit root_rest plus root_after_second=root_rest^root_second scalar
# predicate, with root_first computed only inside the preroll branch as in 211.
# 218 keeps 217 rootrestlate and changes root_after_second to and-not while preserving first-child nf/future/terminal/
# jmark checks; the active/saved root remainder remains identical. Host task
...
#   - rootrestlate_andnot source check is whitespace/comment robust.
SRC=${SRC:-./218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 218 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/218Py_kernel_roottwo_rootrestlate_andnot_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 218Py source
#   has a harmless header/string/layout difference. This 218 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 217 rootrestlate futuremask/no-sibling MAXD14 dispatch; 217 is new fastest over 204; 218 tests 217 plus root_after_second and-not predicate; 210 rootafterandnot, 216 rootprblockdirect, and 211 rootfirstlate were correct but slower than 217; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second and-not scalar if + late root_first + delayed cur_avail=root_rest	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
marker='# 218 rootrestlate_andnot:'
...
checks = {
    ...
    'root_after_xor_absent': 'root_after_second:u32=root_rest^root_second' not in root_compact,
    'root_after_andnot_present': 'root_after_second:u32=root_rest&(~root_second)' in root_compact,
    ...
    'root_inline_if_absent': 'if(root_rest^root_second)==u32(0):' not in root_compact and 'if(root_rest&(~root_second))==u32(0):' not in root_compact,
...
if all(checks.values()):
    sys.exit(0)
print('source_rootrestlate_andnot_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootrestlate_andnot\tpresent_rootrestlate_andnot_predicate\tpresent_rootrestlate_andnot_predicate\tOK
' >> "$SUMMARY"
else
  printf 'source_rootrestlate_andnot\tpresent_rootrestlate_andnot_predicate\tmissing_or_old_rootrestlate_andnot_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split218'}
bad=sorted(tags-allowed)
if 'split218' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split218 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split218 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 218 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 218 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootrestlate_andnot static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 218 roottwo-rootrestlate_andnot futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 218 roottwo-rootrestlate_andnot futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
