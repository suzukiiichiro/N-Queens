...
[130Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/130Py_scorestripe_chunkshape_probe.py)
[130Py1-6.sh](sandbox:/mnt/data/130Py1-6.sh)
...
130Py:
  ...
  chunkshape130_scorestripe_v2 を追加
...
`130Py1-6.sh` は初回検証用なので、129 と同じく mode30 の `partial_total` は固定値にしていません。まず `partial_total=` の出力確認、worker 0/4 の `expected_total=314666222712` 確認、full の `314666222712.*ok` 確認にしています。

実行は以下です。

```bash
codon build -release 130Py_scorestripe_chunkshape_probe.py
./130Py1-6.sh
```
...
```bash
RUN_FULL=0 ./130Py1-6.sh
```

確認していただきたいログ行は、特にこの 2 つです。

```text
[chunkshape130-score-key] ...
[chunkshape130-score-buckets] ... b0=... b1=... ... b7=...
```

ここで 129 のように `b7=2025282` 全寄せになっていなければ、130 の bucket 退化修正はまず効いています。こちらでは `130Py1-6.sh` の bash 構文確認と、130 側の主要シンボル整合確認まで行っています。


2026-06-15 08:48:56 作成の cand130 ログを確認しました。結論は、**130Py は correctness PASS、generic-only 制約 PASS、かつ 128/129 より明確に速いので、130 を current candidate baseline に昇格**でよいです。
...
generic-only 条件も通っています。ログ上は `version : 130 scorestripe chunkshape probe - mode30/mode31 generic kernel, host-side percentile score-aware shaping`、`chunkshape130_params: version=scorestripe_v2 ... generic kernel unchanged` で、実行 bucket も `generic=15488 ... split_mode=0 specialized=0` です。
...
01〜04 の selected chunk はそれぞれ `split130-probe` として正常終了しています。 worker 0/4 も `partial_total=79323742766`、`expected_total=314666222712`、`0:05:23.119` で正常です。
...
[130Py1-6_locked.sh](sandbox:/mnt/data/130Py1-6_locked.sh)
[137_130_full_result_memo.txt](sandbox:/mnt/data/137_130_full_result_memo.txt)

はい、**実行したほうが良い**です。

理由は、今回の `130Py1-6.sh` 初回検証ではすでに **correctness PASS / generic-only PASS / 128 より約30秒改善**まで確認できていますが、`130Py1-6_locked.sh` はその次段階の **再現性確認**だからです。130 full は `total=314666222712`、`0:21:10.100 ok` で完走しており、`source_records=base_reordered_records=shaped_records=2025282` も保っています。

ただし目的は「新しい correctness 探索」ではなく、以下の固定値 regression です。

```text
130Py1-6_locked.sh の目的:
...
したがって、**はい、130Py1-6_locked.sh は実行してください。** 結果ログが出たら、前回 130 初回ログとの差分まで見て、130 baseline 固定でよいか最終確認します。
...
generic-only 条件も問題ありません。130 full のログ冒頭は `host-side percentile score-aware shaping` で、`chunkshape130_params: version=scorestripe_v2 ... generic kernel unchanged`、実行 bucket も `split_mode=0 specialized=0` です。
...
VERSION_TAG:str="130 scorestripe chunkshape probe - mode30/mode31 generic kernel, host-side percentile score-aware shaping"
...
####################################################################################################
# 130 reorder chunkshape harness
#
...
####################################################################################################
# 130 score-aware chunk-shape layer
#
...
CHUNKSHAPE130_REORDER_VERSION:str="scorestripe_v2"
CHUNKSHAPE130_DEFAULT_REASON:str="host-side percentile score-aware quota stripe over broadmarktail v4; generic kernel unchanged"

def chunkshape130_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape130_{CHUNKSHAPE130_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}.bin"

CHUNKSHAPE130_SCORE_KEY_MAX:int=32767


def chunkshape130_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  ...
  if key<0:
    key=0
  if key>CHUNKSHAPE130_SCORE_KEY_MAX:
    key=CHUNKSHAPE130_SCORE_KEY_MAX
  return key


def chunkshape130_build_thresholds_from_hist(hist:List[int],total_records:int)->List[int]:
  """Build 8 percentile thresholds from the score-key histogram."""
  thresholds:List[int]=[0]*8
  if total_records<=0:
    thresholds[7]=CHUNKSHAPE130_SCORE_KEY_MAX
    return thresholds

  b:int=0
  cum:int=0
  k:int=0
  while k<=CHUNKSHAPE130_SCORE_KEY_MAX:
    cum+=hist[k]
    while b<7:
  ...
  while b<7:
    thresholds[b]=CHUNKSHAPE130_SCORE_KEY_MAX
    b+=1
  thresholds[7]=CHUNKSHAPE130_SCORE_KEY_MAX
  return thresholds


def chunkshape130_bucket_from_key(key:int,thresholds:List[int])->int:
...
def chunkshape130_make_quotas(rem_counts:List[int],total_remaining:int,m_target:int)->List[int]:
...
def build_chunkshape130_reordered_bin(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  out_fname:str=chunkshape130_reorder_output_fname(N,preset_queens,BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_chunkshape130_{CHUNKSHAPE130_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)}_sim.tsv"

  if gpu_log_level>=1:
    print(f"[chunkshape130-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE130_DEFAULT_REASON}")
  ...
  if len(data)<expected_bytes:
    print(f"[chunkshape130-warning] source byte count shorter than expected: bytes={len(data)} expected={expected_bytes}")
  ...
  key_hist:List[int]=[0]*(CHUNKSHAPE130_SCORE_KEY_MAX+1)
  off:int=0
  classify_chunk:int=0
  key_min:int=CHUNKSHAPE130_SCORE_KEY_MAX
  key_max:int=0
  while off<total_records:
    ...
    i=0
    while i<m:
      key:int=chunkshape130_score_key_from_soa(soa,i,off+i)
      if key<0:
        key=0
      if key>CHUNKSHAPE130_SCORE_KEY_MAX:
        key=CHUNKSHAPE130_SCORE_KEY_MAX
    ...
    if gpu_log_level>=2:
      print(f"[chunkshape130-classify-chunk] N={N} chunk={classify_chunk} off={off} m={m}")
    off+=m
    classify_chunk+=1

  score_thresholds:List[int]=chunkshape130_build_thresholds_from_hist(key_hist,total_records)
  bucket_counts:List[int]=[0]*8
  idx_count:int=0
  while idx_count<total_records:
    b0:int=chunkshape130_bucket_from_key(score_key_by_idx[idx_count],score_thresholds)
    bucket_counts[b0]+=1
    idx_count+=1

  if gpu_log_level>=1:
    print(f"[chunkshape130-score-key] N={N} records={total_records} key_min={key_min} key_max={key_max} t0={score_thresholds[0]} t1={score_thresholds[1]} t2={score_thresholds[2]} t3={score_thresholds[3]} t4={score_thresholds[4]} t5={score_thresholds[5]} t6={score_thresholds[6]}")
    print(f"[chunkshape130-score-buckets] N={N} records={total_records} b0={bucket_counts[0]} b1={bucket_counts[1]} b2={bucket_counts[2]} b3={bucket_counts[3]} b4={bucket_counts[4]} b5={bucket_counts[5]} b6={bucket_counts[6]} b7={bucket_counts[7]}")
      ...
      while src_ch<n_chunks:
        ...
        if within<STEPS:
          ...
          if idx<total_records:
            b=chunkshape130_bucket_from_key(score_key_by_idx[idx],score_thresholds)
  ...
  if staged_total!=total_records:
    print(f"[chunkshape130-warning] staged count mismatch: staged={staged_total} source={total_records}")
  ...
  with open(out_fname,"ab") as out:
    while out_ch<n_chunks:
      ...
      quotas:List[int]=chunkshape130_make_quotas(bucket_rem,total_remaining,m_target)
      ...
      if gpu_log_level>=2:
        print(f"[chunkshape130-build-chunk] N={N} chunk={out_ch} written={written_chunk} total_written={written_total} b0={written_by_bucket[0]} b1={written_by_bucket[1]} b2={written_by_bucket[2]} b3={written_by_bucket[3]} b4={written_by_bucket[4]} b5={written_by_bucket[5]} b6={written_by_bucket[6]} b7={written_by_bucket[7]}")
      out_ch+=1
  ...
  if written_total!=total_records:
    print(f"[chunkshape130-warning] record count mismatch: written={written_total} source={total_records}")
  if reordered_records!=written_total:
    print(f"[chunkshape130-warning] output count mismatch: file_records={reordered_records} written={written_total}")
  if gpu_log_level>=1:
    print(f"[chunkshape130-build-summary] N={N} records={reordered_records} chunks={n_chunks} output={out_fname} progress={progress_fname} valid={1 if validate_bin_file(out_fname) else 0}")
...
def exec_solutions_gpu_chunk_split130(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  if gpu_log_level>=1:
    print(f"[split130-buckets] N={N} m={m} generic={m} d2base14={d2base14_m} d0={d0_m} rest=0 split_mode={split_mode} specialized=0")
...
def exec_solutions_gpu_bin_stream_split130(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split130_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split130_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split130_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split130_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split130-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split130-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split130-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split130-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split130(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split130-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split130-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split130 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split130 full run / optional multi-GPU worker
      # Examples:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split130-generic-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split130-generic worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split130_generic_harness_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split130_generic_harness_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  if gpu_log_level>=1 and (bench_mode==30 or bench_mode==31):
    print(f"chunkshape130_params: version={CHUNKSHAPE130_REORDER_VERSION} reason={CHUNKSHAPE130_DEFAULT_REASON}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split130-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split130-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      shaped_fname:str=chunkshape130_reorder_output_fname(N,preset_queens,gpu_block,gpu_max_blocks)
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape130-probe-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape130-probe-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape130_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split130_probe_{CHUNKSHAPE130_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split130(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split130-probe-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE130_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split130-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split130-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split130-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape130-full-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape130-full-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape130_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split130_full_{CHUNKSHAPE130_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split130(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split130-full-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE130_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
