...
# =============================================================================
# NQ_UPDATE_MEMO
# 229: 更新メモ: 228Py root-preroll ncol early は正当性OKだが217/225を上回らなかったため不採用。
#      229Pyは225Py generic ncol-earlyを親に、MAXD14 root-preroll内だけ
#      pr_block_code==0 の normal step1 path を先に置く normal-first 微差実験を検証する。
# =============================================================================

SRC=${SRC:-./229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 229 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
root_pos=s.find('# 229 rootpr-normalfirst:')
if root_pos < 0:
    root_pos=s.find('pr_bit:u32=root_first')
...
root_if_pos=root_compact.find('ifpr_block_code==u32(0):')
root_else_pos=root_compact.find('else:', root_if_pos)
root_step_pos=root_compact.find('pr_stepu:u32=', root_if_pos)
root_norm_pos=root_compact.find('pr_nld=(cur_ld|pr_bit)<<u32(1)pr_nrd=(cur_rd|pr_bit)>>u32(1)', root_if_pos)
...
checks={
  ...
  'rootpr_normalfirst_present': root_if_pos >= 0,
  'rootpr_normal_body_before_else': 0 <= root_if_pos < root_norm_pos < root_else_pos,
  'rootpr_block_path_after_else': root_else_pos >= 0 and root_step_pos > root_else_pos,
  'rootpr_old_blockfirst_absent': 'ifpr_block_code!=u32(0):' not in root_compact,
  'rootpr_ncol_after_branch_kept': pr_ncol_pos > root_step_pos and pr_nf_pos > pr_ncol_pos,
  ...
  'placed_ld_absent': 'placed_ld:u32=' not in generic_compact and 'placed_rd:u32=' not in generic_compact,
}
if all(checks.values()):
    sys.exit(0)
print('source_rootpr_normalfirst_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootpr_normalfirst	rootrestlate_plus_rootpr_normalfirst	present	OK\n' >> "$SUMMARY"
else
  printf 'source_rootpr_normalfirst	rootrestlate_plus_rootpr_normalfirst	missing_or_old_shape	FAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split229'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split229' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit229 active preferred\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit229 active preferred\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if (( static_failures != 0 )); then
  echo "================================================================"; echo "[static-summary]"; cat "$SUMMARY"; echo "[logdir] $LOGDIR"
  echo "[error] 229 source static checks failed" >&2
  exit 66
fi
