...
152Pyは問題ありません。全131チャンク、部分和01〜05、最終合計`314666222712`が一致し、警告・エラー・欠落・重複はありません。

実行時間は`0:16:10.212`で、151Py比`6.642秒（0.680%）`短縮し、131チャンクすべてが高速化しています。

### 153Py

[153Py_future_mark_complement_probe.py](sandbox:/mnt/data/153Py_future_mark_complement_probe.py)

`funcid=0..27`では、future既定値を持つ型が`IS_MARK_MASK`の完全な補集合になるため、次のように変更しました。
...
別途行っていた`META_AVAIL_MASK`のシフトと`aflag`一時変数を削除する単独実験です。探索順序、遷移、枝刈り条件、キャッシュ形式は変更していません。

### 実行シェル

[153Py_validate_full_once.sh](sandbox:/mnt/data/153Py_validate_full_once.sh)

```bash
./153Py_validate_full_once.sh
```

全131チャンクを1回だけ実行し、同じprogress TSVから01〜05と全件結果を検証します。152Pyとの時間比較も自動表示します。

フレーム初期化の旧式・新式について653,184ケースの等価性を確認し、シェルは`bash -n`と模擬全件検証を通過しています。この環境ではCodon/CUDA実機コンパイルは未実施です。


"""

# 153 GPU kernel experiment:
#   Retain the fully validated 152 lazy next-fid lookup, 151 frame-state reload
#   elision, 150 branchless blockK selector, 149 ctrl-u32 decode, 148 task order,
#   MAXD=21, five u32 DFS stacks, and nld/nrd full-u32 arithmetic.  The sole
#   kernel change reuses the already-needed IS_MARK_MASK classification to set
#   use_future=1-ism.  Over valid funcids 0..27, META_AVAIL_MASK is exactly the
#   complement of IS_MARK_MASK, so one per-frame mask shift and the aflag local
#   are removed without changing any transition or pruning condition.
...
VERSION_TAG:str="153 future-from-mark complement over 152 lazy next-fid - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    # 153: fixed-width operands retained for the u32 diagonal, ctrl-decode, and branchless blockK hot path.
    n3u:u32=u32(n3)
    n4u:u32=u32(n4)
    ...
    while sp>=0:
      ...
      if (cv&u32(INIT_MASK))==u32(0):
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148/149/150/151/152/153
      ...
      # 1bit 展開
      #----------------
      # 151/152/153: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
        ...
        bKu:u32=(n3u&(u32(0)-(ktu&u32(1))))|(n4u&(u32(0)-(ktu>>u32(1))))
...
####################################################################################################
# 153 future-from-mark complement over the validated 152/151/150/149/148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 152/151/150/149/148; 153 changes only future-default decoding inside kernel_dfs_iter_gpu
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 152/151/150/149/148 scorestripe_v9 task order/cache unchanged; 153 derives default future flag from the mark-mask complement"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  153 reuses the validated 152/151/150/149/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 153 experiment keeps the 152 lazy-next path and changes only future-default decoding inside the GPU kernel.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  153 host-side score-aware lanephase chunk shaping reused unchanged from validated 152/151/150/149/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 153 experiment changes only future-default decoding; the 148 host-shaping cache is reused unchanged.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split153_future_mark_complement_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split153_future_mark_complement_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split153-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split153-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split153_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split153-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split153-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split153_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
