...
# =============================================================================
# NQ_UPDATE_MEMO
# 259: schedule-precompute-rootactionpack probe。256 base14pack JIT成立後の切り分け版。
#      241 restore239本線を親に、単一launchを維持し、child/future maskpackは保持する。
#      255 actionpack は正しい mode31 引数でも CUDA_ERROR_INVALID_PTX となったため、root_action単独decodeの可否を切り分ける。
#      257では root_action だけを単独 low-bit 配列 sched_root_action_arr へ移し、
#      terminal_depth と terminal_base14 は別配列のまま保持する。
#      240 fid14/rest別launch、242 futuremask-depthbit微差、255 actionpack は入れない。
...
"""
259 schedule-precompute note: 256 base14pack JIT成立結果を受け、255 actionpackのPTX/JIT不成立をさらに切り分けるため、root_actionだけを(root_action<<1)のsched_root_action_arrからdecodeし、terminal_depth/terminal_base14は別配列のまま保持するprobe。

作成しました。259は、ユーザー案どおり **root_action の parenthesized-mask decode** を単独で試す版です。

257では以下が `CUDA_ERROR_INVALID_PTX` でした。

```python
root_action:u32=sched_root_action_arr[i]&u32(3)
```

259では、ここだけを明示括弧形へ変更しています。

```python
root_action:u32 = (sched_root_action_arr[i]) & u32(3)
```

狙いは、Codon GPU IR / PTX生成で、括弧付きindex expressionがJIT挙動を変えるかどうかの切り分けです。
`terminal_depth` と `terminal_base14` は別配列のまま、`child/future maskpack` は244 r4系を維持しています。
...
source_schedule_precompute_rootaction_parenmask OK
source_host_precompute_rootaction_parenmask_arrays OK
release_build_policy                       OK
```

ファイルはこちらです。

* [259Py_schedule_precompute_rootaction_parenmask_probe.py](sandbox:/mnt/data/259Py_schedule_precompute_rootaction_parenmask_probe.py)
* [259Py_schedule_precompute_rootaction_parenmask_validate_N21_full_once.sh](sandbox:/mnt/data/259Py_schedule_precompute_rootaction_parenmask_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [259Py_schedule_precompute_rootaction_parenmask_sha256.txt](sandbox:/mnt/data/259Py_schedule_precompute_rootaction_parenmask_sha256.txt)
* [259Py_schedule_precompute_rootaction_parenmask_bundle_20260707.tar](sandbox:/mnt/data/259Py_schedule_precompute_rootaction_parenmask_bundle_20260707.tar)

sha256です。

```text
1dfb2f8ebaf207b284295360e32a579bd03e921f8a98a95ce3c59657d16cce98  259Py_schedule_precompute_rootaction_parenmask_probe.py
954a066f5080f8ae6ab92b4fec4b93ea755b20c38057339912cb2c92cccf65ba  259Py_schedule_precompute_rootaction_parenmask_validate_N21_full_once.sh
f801c6b6a4e9f1b22e8fcaa31af76af55b4faafae3c8a5bc049f9bc28f140ff9  README.md
```

実行は以下でお願いします。

```bash
rm -f ./259Py_schedule_precompute_rootaction_parenmask_probe

STATIC_ONLY=1 bash 259Py_schedule_precompute_rootaction_parenmask_validate_N21_full_once.sh

bash 259Py_schedule_precompute_rootaction_parenmask_validate_N21_full_once.sh
```

259がJIT成立すれば、257の不成立は「root_action配列 + mask」そのものではなく、式の生成形に依存している可能性があります。逆に259もJIT不成立なら、次は括弧ではなく **一時変数分離** が候補です。
...
VERSION_TAG:str="259 schedule-precompute-rootaction-parenmask probe: parent 256 base14pack JIT-ok / 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; child/future maskpack retained; 256 proved terminal_base14-only low-bit decode is JIT-safe; only root_action is decoded with parenthesized sched_root_action_arr[i] mask while terminal_depth/terminal_base14 stay separate; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 259: bare -g stays split145 mode31; root_action parenthesized mask probe
...
# 259: keep runtime globals required by broadmarktail/chunkshape148/split145
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 259: 256 rootactionpack JIT-ok shape plus root_action parenthesized-mask decode
    # from sched_root_action_arr. terminal_depth/terminal_base14 remain
    # separate to isolate the 255 actionpack PTX/JIT failure source.
    ...
    self.sched_base14_arr:List[u32]=[u32(0)]*m
    self.sched_root_action_arr:List[u32]=[u32(0)]*m
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 259: host-side clone of the former MAXD14 kernel schedule builder.
  # Keeps child/future maskpack and 256's terminal_base14 separate decode,
  # then changes only root_action shifted storage to sched_root_action_arr.
  ...
  sched_root_action:u32=root_action&u32(3)
  return schedule_lo,schedule_hi,sched_mask,terminal_parent_depth,sched_base14,sched_root_action
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    terminal_depth_arr:Ptr[u32],sched_base14_arr:Ptr[u32],sched_root_action_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 259: schedule fields are precomputed on the host in build_soa_for_range().
    # child/future masks are packed; terminal_base14 remains separate; root_action
    # is decoded from its own low-bit array.
    ...
    terminal_base14:u32=sched_base14_arr[i]&u32(1)
    root_action:u32 = (sched_root_action_arr[i]) & u32(3)
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.terminal_depth_arr),gpu.raw(soa.sched_base14_arr),gpu.raw(soa.sched_root_action_arr),
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        sched_root_action:u32=u32(0)
        sched_lo,sched_hi,sched_mask,terminal_depth_u,sched_base14,sched_root_action=precompute_maxd14_schedule_fields(soa.ctrl0_arr[t],soa.markctrl_arr[t],meta_next_sched)
        ...
        soa.sched_base14_arr[t]=sched_base14
        soa.sched_root_action_arr[t]=sched_root_action
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        for p in range(m):
          ...
          sort_soa.sched_base14_arr[p]=soa.sched_base14_arr[q]
          sort_soa.sched_root_action_arr[p]=soa.sched_root_action_arr[q]
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split259_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split259_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      progress_suffix:str=f"split259_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
