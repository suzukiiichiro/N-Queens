...
166Pyは正解性に問題ありませんが、164Pyより37.450秒（5.911%）低速でした。167Pyは最速の164Pyから派生しています。

* [167Py_kernel_static_maxd_dispatch_probe.py](sandbox:/mnt/data/167Py_kernel_static_maxd_dispatch_probe.py)
* [167Py_kernel_static_maxd_dispatch_validate_full_once.sh](sandbox:/mnt/data/167Py_kernel_static_maxd_dispatch_validate_full_once.sh)

MAXD=16／18／20／21の静的カーネルを用意し、各GPU投入前に必要深度を算出して最小容量を選択します。N=21・dynamic preset=6ではMAXD=16となるモデル検証まで通過済みです。Codon/CUDA実機実行は未実施です。


"""

# 167 GPU static-MAXD dispatch experiment:
#   Branch from the fastest validated 164 child-frame-push-initialization
#   baseline.  The slower 165 schedule-precompute and 166 terminal-parent-batch
#   experiments are intentionally not inherited.  The GPU DFS body is copied
#   without semantic changes into four kernels whose five local u32 arrays have
#   compile-time lengths 16, 18, 20, and 21.  Before each kernel launch, host
#   code mirrors the queen-independent control schedule for every task in that
#   launch, obtains the exact maximum number of stored nonterminal frames, and
#   dispatches the smallest sufficient static kernel.  A requirement above 21
#   is rejected before launch.  Host data layout, task order/cache, DFS order,
#   bitboard arithmetic, and solution arithmetic remain inherited from 164.
...
# 167 static stack specializations.  Each value is a compile-time __array__
# length; runtime dispatch selects the smallest capacity that covers every task
# in the current launch.  Five u32 stacks use 20*MAXD bytes per thread.
MAXD16:Static[int]=16
MAXD18:Static[int]=18
MAXD20:Static[int]=20
MAXD21:Static[int]=21

VERSION_TAG:str="167 static MAXD dispatch over 164 child frame push initialization - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd16(
    ...
    引数（抜粋）:
      ...
      ctrl0_arr:
        ...
        164ではroot初期化専用で、raw値をctrl stackへ保存しない。
      markctrl_arr:
    ...
    前提/不変条件:
      ...
      - 現行 dynamic preset の N<=27 では最大 sp=20。MAXD16=21 は index 0..20 を確保する。
      - ctrl stack上の全frameは常に初期化済みmetadataであり、bit19は未使用。
      - 164は162を親版とし、root/child frame初期化位置だけを変更する。
    ホットパス（ソース引用）:
      bit = a & -a
      avail[sp] = a ^ bit
    """
    # GPUではlist/tuple参照を避け、function分類を32bit maskとして焼き込む。
    # 164ではrootをloop前、childをpush前に初期化し、loop内の初期化判定を廃止する。
    ...
    # 145: MAXD16=21、154以降: u32入力/stack、155: ctrl0 prepack、157: markctrl prepack。
    # 158/159: function分類とmask演算をu32化、160: root-pop終了、161: 到達不能guard除去。
    # 162: mark成立箇所でblockL/blockK metadataを直接生成。
    # 164: rootとchildをstack保存前に初期化し、stack上のctrlを常に次のlayoutへ統一する。
    ...
    #   bit  19     : unused (initialization flag removed)
    # low 10 bits remain the raw identity of the child frame, so child initialization starts from cv&1023.
...
def kernel_dfs_iter_gpu_maxd18(
    ...
    前提/不変条件:
      - ld/col/free は board_mask 内に収まる。
      - rd は future attack 用に board_mask 外の高位ビットを持つ場合があるが、bit30 以下の非負値。
      - 現行 dynamic preset の N<=27 では最大 sp=20。MAXD18=21 は index 0..20 を確保する。
      - ctrl stack上の全frameは常に初期化済みmetadataであり、bit19は未使用。
      - 164は162を親版とし、root/child frame初期化位置だけを変更する。
    ホットパス（ソース引用）:
      bit = a & -a
    ...
    MASK_L_1:u32=u32(12689458)
    MASK_L_2:u32=u32(17039488)
    # 145: MAXD18=21、154以降: u32入力/stack、155: ctrl0 prepack、157: markctrl prepack。
    # 158/159: function分類とmask演算をu32化、160: root-pop終了、161: 到達不能guard除去。
    # 162: mark成立箇所でblockL/blockK metadataを直接生成。
    # 164: rootとchildをstack保存前に初期化し、stack上のctrlを常に次のlayoutへ統一する。
    #   bits 0..4   : child/next fid
...
def kernel_dfs_iter_gpu_maxd20(
    ...
    前提/不変条件:
      - ld/col/free は board_mask 内に収まる。
      - rd は future attack 用に board_mask 外の高位ビットを持つ場合があるが、bit30 以下の非負値。
      - 現行 dynamic preset の N<=27 では最大 sp=20。MAXD20=21 は index 0..20 を確保する。
      - ctrl stack上の全frameは常に初期化済みmetadataであり、bit19は未使用。
      - 164は162を親版とし、root/child frame初期化位置だけを変更する。
    ホットパス（ソース引用）:
      bit = a & -a
    ...
    MASK_L_1:u32=u32(12689458)
    MASK_L_2:u32=u32(17039488)
    # 145: MAXD20=21、154以降: u32入力/stack、155: ctrl0 prepack、157: markctrl prepack。
    # 158/159: function分類とmask演算をu32化、160: root-pop終了、161: 到達不能guard除去。
    # 162: mark成立箇所でblockL/blockK metadataを直接生成。
    # 164: rootとchildをstack保存前に初期化し、stack上のctrlを常に次のlayoutへ統一する。
    #   bits 0..4   : child/next fid
...
def kernel_dfs_iter_gpu_maxd21(
    ...
    前提/不変条件:
      - ld/col/free は board_mask 内に収まる。
      - rd は future attack 用に board_mask 外の高位ビットを持つ場合があるが、bit30 以下の非負値。
      - 現行 dynamic preset の N<=27 では最大 sp=20。MAXD21=21 は index 0..20 を確保する。
      - ctrl stack上の全frameは常に初期化済みmetadataであり、bit19は未使用。
      - 164は162を親版とし、root/child frame初期化位置だけを変更する。
    ホットパス（ソース引用）:
      bit = a & -a
    ...
    MASK_L_1:u32=u32(12689458)
    MASK_L_2:u32=u32(17039488)
    # 145: MAXD21=21、154以降: u32入力/stack、155: ctrl0 prepack、157: markctrl prepack。
    # 158/159: function分類とmask演算をu32化、160: root-pop終了、161: 到達不能guard除去。
    # 162: mark成立箇所でblockL/blockK metadataを直接生成。
    # 164: rootとchildをstack保存前に初期化し、stack上のctrlを常に次のlayoutへ統一する。
    #   bits 0..4   : child/next fid
...
####################################################################################################
# 167 static-MAXD dispatch harness
#
# This build defines four capacity-specialized @gpu.kernel variants with identical DFS logic.
# It keeps the proven 164 DFS body and the existing split/candidate/chunk-shape host paths.
# The only runtime addition is one deterministic depth scan and one host dispatch per launch.
####################################################################################################
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164/167: convert GPU scalar inputs once per host invocation, not once per GPU thread.
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        if gpu_log_level>=2:
          print(f"[maxd-dispatch] N={N} scope=exec chunk={chunk_index} m={m} required_maxd={required_maxd} selected_MAXD={selected_maxd} stack_bytes_per_thread={selected_maxd*20} capacity_check=OK")
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 164 scorestripe_v9 task order/cache unchanged; 167 selects the smallest safe static MAXD"
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/167: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def exec_solutions_gpu_chunk_split145(
  ...
  if m>0:
    ...
    selected_maxd:int=select_static_maxd(required_maxd)
    if gpu_log_level>=1:
      print(f"[maxd-dispatch] N={N} scope=split145 m={m} required_maxd={required_maxd} selected_MAXD={selected_maxd} stack_bytes_per_thread={selected_maxd*20} capacity_check=OK")
    if not launch_kernel_dfs_iter_gpu_static_maxd(selected_maxd,soa,w_arr,meta_next,results,m,board_mask_gpu,n3_gpu,n4_gpu,GRID,BLOCK):
      print(f"[maxd-error] unsupported required_maxd={required_maxd}; supported maximum is 21")
...
def main()->None:
  ...
  # 167 branches from validated 164.  The host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 167 static-MAXD-dispatch GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split167_static_maxd_dispatch_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split167_static_maxd_dispatch_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split167-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split167-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split167_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split167-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split167-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split167_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
