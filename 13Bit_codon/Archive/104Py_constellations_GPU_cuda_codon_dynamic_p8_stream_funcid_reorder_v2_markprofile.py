...
104Py のソースコードを作成しました。
103Py の結果で **depth/free_popcount では tail 分離が弱い**ため、次は **mark pattern profile** を見る、という昨日の方針に沿った版です。
土台は添付いただいた 103Py の `bench_mode=23` 実装です。

こちらです。

[104Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markprofile.py](sandbox:/mnt/data/104Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markprofile.py)

差分も付けました。

[103_to_104_funcidmark.patch](sandbox:/mnt/data/103_to_104_funcidmark.patch)

追加内容は以下です。

```text
bench_mode=24
  funcid-mark pattern profile
...
row_to_mark1_min / row_to_mark1_max
row_to_mark2_min / row_to_mark2_max
jmark_min / jmark_max
...
codon build -release \
  104Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markprofile.py
```

smoke test はまずこれがよいです。

```bash
./104Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markprofile \
  -g 22 22 32 484 1 0 7 24 8 7 0 1644 1 "-" "0,4,7" "all;gap1;gap2;gap3p;near_m1_0_2;near_m2_0_2"
```
...
```bash
./104Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markprofile \
  -g 22 22 32 484 1 0 7 24
```

出力 TSV は以下になります。

```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_funcidmark.tsv
```

この環境には `codon` が入っていないため、こちらでは build 実行までは確認していません。ソース生成と差分作成までは完了しています。

104Py の TSV を確認しました。**104Py / bench_mode=24 は正常に動作しています。**
...
```text
103Py:
  depth/free_popcount では分離できなかった

104Py:
  mark pattern ではかなり分離できた
```

## all bucket の再確認

`all` だけを見ると、103Py と同じく record あたりの重さはこの順です。

| fid | records | kernel sec | ms/record | 判定              |
| --: | ------: | ---------: | --------: | --------------- |
|   1 |     473 |     11.190 |    23.658 | very heavy tail |
|  15 |     426 |      8.286 |    19.451 | very heavy tail |
|   0 |    2148 |     27.177 |    12.652 | heavy tail      |
|  22 |    1130 |     13.961 |    12.355 | heavy tail      |
|   4 |    4084 |     42.699 |    10.455 | heavy           |
|  19 |     987 |     10.146 |    10.280 | heavy           |
|   7 |    3837 |     36.853 |     9.605 | heavy           |
|  24 |    1527 |     14.465 |     9.473 | heavy           |
|   5 |   24529 |     47.295 |     1.928 | bulk-heavy      |

`fid=5` は今回も、総 kernel 時間は大きいですが、`all` では record あたりは軽いです。
ただし 104Py で分かった重要点は、**fid=5 の中にも tail subgroup がある**ことです。

## 一番重要な発見

mark bucket では、特定の少数 subgroup がかなり重いです。

| fid | bucket      | records | kernel sec | ms/record | 意味      |
| --: | ----------- | ------: | ---------: | --------: | ------- |
|   0 | gap3p       |      11 |      3.068 |   278.909 | 少数だが極端  |
|  15 | near_m2_3p  |      13 |      3.085 |   237.308 | 少数だが極端  |
|  15 | near_m1_3p  |      13 |      3.084 |   237.231 | 同上      |
|  24 | near_m2_0_2 |      39 |      4.480 |   114.872 | 強い tail |
|  19 | gap3p       |      43 |      2.554 |    59.395 | 強い tail |
|   1 | gap3p       |     143 |      6.022 |    42.112 | 強い tail |
|  19 | near_m1_3p  |      91 |      3.677 |    40.407 | 強い tail |
|   0 | near_m2_3p  |     425 |     14.040 |    33.035 | かなり重要   |
|  24 | near_m1_3p  |      90 |      2.770 |    30.778 | 強い tail |
|  22 | near_m1_3p  |     138 |      3.829 |    27.746 | 強い tail |

これは 103Py の `depth=16 / free_pc>=4` では見えなかった差です。
...
```text
all          records=2148  kernel=27.177 sec  12.652 ms/record
gap2         records=2137  kernel=26.562 sec  12.430 ms/record
gap3p        records=11    kernel=3.068 sec   278.909 ms/record
near_m2_3p   records=425   kernel=14.040 sec  33.035 ms/record
near_m2_0_2  records=1723  kernel=13.137 sec  7.624 ms/record
```

`fid=0` は、**mark2 が遠い `near_m2_3p` が重い**です。
さらに `gap3p` は 11 record だけですが極端に重いです。

### fid=1

```text
all     records=473  kernel=11.190 sec  23.658 ms/record
gap2    records=330  kernel=5.165 sec   15.652 ms/record
gap3p   records=143  kernel=6.022 sec   42.112 ms/record
```

`fid=1` は **gap3p が主犯寄り**です。
`gap2` も軽くはないですが、`gap3p` の方が明確に重いです。

### fid=4

```text
all          records=4084  kernel=42.699 sec  10.455 ms/record
near_m2_3p   records=1789  kernel=33.350 sec  18.642 ms/record
near_m1_3p   records=1165  kernel=19.516 sec  16.752 ms/record
near_m2_0_2  records=2295  kernel=28.655 sec  12.486 ms/record
```

`fid=4` は **mark2 が遠い側 `near_m2_3p` が重い**です。
`gap1` は全件なので分離には効いていません。

### fid=5

```text
all          records=24529  kernel=47.295 sec  1.928 ms/record
gap2         records=4069   kernel=36.928 sec  9.075 ms/record
gap3p        records=20460  kernel=44.863 sec  2.193 ms/record
near_m1_3p   records=5475   kernel=38.853 sec  7.096 ms/record
```

これは重要です。
`fid=5` は全体では bulk-heavy ですが、内部を見ると **gap2 と near_m1_3p が tail 化**しています。

つまり今後は `fid=5` を単に bulk として扱うのではなく、
...
```text
all          records=3837  kernel=36.853 sec  9.605 ms/record
near_m2_0_2  records=676   kernel=15.347 sec  22.703 ms/record
near_m1_3p   records=1767  kernel=25.358 sec  14.351 ms/record
near_m2_3p   records=3161  kernel=34.914 sec  11.045 ms/record
```

`fid=7` は少し特殊で、**mark2 が近い `near_m2_0_2` が重い**です。
`fid=0/4` とは逆方向です。

### fid=15

```text
all          records=426  kernel=8.286 sec  19.451 ms/record
near_m1_0_2  records=413  kernel=5.202 sec  12.596 ms/record
near_m1_3p   records=13   kernel=3.084 sec  237.231 ms/record
near_m2_3p   records=13   kernel=3.085 sec  237.308 ms/record
```

`fid=15` は **13 record の遠い mark subgroup が極端に重い**です。
これはかなり有力な tail 原因です。

### fid=19

```text
all          records=987  kernel=10.146 sec  10.280 ms/record
gap2         records=944  kernel=10.245 sec  10.853 ms/record
gap3p        records=43   kernel=2.554 sec   59.395 ms/record
near_m1_3p   records=91   kernel=3.677 sec   40.407 ms/record
near_m2_3p   records=134  kernel=3.578 sec   26.701 ms/record
```

`fid=19` は **gap3p / near_m1_3p / near_m2_3p** が重いです。

### fid=22

```text
all          records=1130  kernel=13.961 sec  12.355 ms/record
near_m1_0_2  records=992   kernel=10.133 sec  10.215 ms/record
near_m1_3p   records=138   kernel=3.829 sec   27.746 ms/record
near_m2_3p   records=138   kernel=3.827 sec   27.732 ms/record
```

`fid=22` も **遠い mark subgroup が重い**です。

### fid=24

```text
all          records=1527  kernel=14.465 sec  9.473 ms/record
near_m2_0_2  records=39    kernel=4.480 sec   114.872 ms/record
near_m1_3p   records=90    kernel=2.770 sec   30.778 ms/record
near_m2_3p   records=1488  kernel=10.412 sec  6.997 ms/record
```

`fid=24` は **mark2 が近い `near_m2_0_2` が極端に重い**です。
ここも `fid=0/4/22` とは逆方向です。

## 104Py の判定

```text
OK  mark profile は有効
OK  depth/free_popcount より説明力がある
OK  fid=0/1/15/19/22/24 の tail subgroup を発見
OK  fid=5 の中にも tail subgroup を発見
NG  gap bucket だけでは不十分
NG  near_m1 / near_m2 の単独 bucket だけでもまだ粗い
```

## 次に見るべきもの

105Py では、粗い bucket ではなく **exact mark-distance profile** がよいです。
...
104Py:
  診断成功。
  tail の本命は depth/free_popcount ではなく mark geometry。

次:
  105Py で exact mark-distance profile。
  特に gap, mark1-row, mark2-row の組み合わせを見る。
```
...
VERSION_TAG:str="104 funcid-mark pattern profile auto w8_j7 stream funcid risk balanced reorder v2 GPU runner from 103"
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if bench_mode<0 or (bench_mode>11 and bench_mode!=13 and bench_mode!=14 and bench_mode!=15 and bench_mode!=16 and bench_mode!=17 and bench_mode!=18 and bench_mode!=19 and bench_mode!=20 and bench_mode!=21 and bench_mode!=22 and bench_mode!=23 and bench_mode!=24):
    ...
    if bench_mode>=8:
      ...
      preset_queens_arg=5
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24:
        if argc >= 14:
  ...
  if bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and not (bench_mode==8 or bench_mode==9 or bench_mode==10 or bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23):
      ijkl_list,subconst_cache,stream_records,preset_queens,stream_fname=ensure_constellations_bin_stream(N,ijkl_list,subconst_cache,preset_queens,gpu_log_level)
