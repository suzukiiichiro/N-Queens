...
Python/codon Ｎクイーン コンステレーション版 CUDA 高速ソルバ（ctrl2 fidless tuned）
...
def kernel_dfs_iter_gpu(
    ...
    # u32 ビットボード系。
    # 32版: fid[MAXD] も ctrl[int] に吸収する。
    # ctrl < 0  : 未初期化フレーム。現在fidは -ctrl-1 として持つ。
    # ctrl >=0 : 初期化済みフレーム。
    ...
    while sp>=0:
      ...
      if ctrl[sp]<0:
        ...
        fcv=0
        if use_future==1 and (row[sp]+stepv)<endm:
          fcv=1
        bLv=0
        ktype=0
        if use_blocks==1:
          ...
          if ((MASK_K_N4>>f)&1)==1:
            ktype=2
        ctrl[sp]=nextfv | (stepv<<5) | (addv<<7) | (use_blocks<<8) | (fcv<<9) | (bLv<<10) | (ktype<<12)
      #----------------
      # 1bit 展開
      #----------------
      a=avail[sp]
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=256,gpu_max_blocks:int=32,gpu_log_level:int=0)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
