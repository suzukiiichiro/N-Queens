...
# =============================================================================
# 309 N=21 full validation harness (variant4-phase-rotate probe)
# Parent: 308 K52-final-sweep (K_PER_THREAD_MAXD14=52, 351.675s, flat vs 304
#   K=48=351.070s). K sweep is now closed: K=44..56 confirmed flat zone,
#   304 (K=48, 351.070s) remains the confirmed BEST and is left untouched here
#   (K_PER_THREAD_MAXD14 stays 48 in this script/source).
# Experiment: BROAD_MARKDIST_TAIL_VARIANT default 2 (rotate_only: boost=1,
#   simple tail phase, fixed-since-115 rotating F17/GH/R interleave) -> 4
#   (phase_rotate: boost=1, cell/risk-aware tail phase, rotating interleave).
#   The variant=4 branch logic (window_boost/phase_salt/use_phase_mix/
#   use_rotating_interleave) already exists in the source and has simply never
#   been selected since 115 adopted variant=2 as the A10G single-GPU default;
#   this is a pure CLI-argument switch (worker_id worker_count variant), zero
#   kernel code changes. A10G_FINAL_DEFAULT_BROADMARK_VARIANT is updated in
#   lockstep (2->4) so bare `-g` no-argument launches match.
# Motivation: 304 ncu re-profile (lightweight sections, chunk0,
#   --launch-count 1) showed Achieved Occupancy 11.04% (vs 33.33% theoretical)
#   with ncu itself attributing this to warp-to-warp imbalance (Est. Speedup
#   66.88%), Avg. Active Threads Per Warp 6.34, Stall Wait 44.1% (still top,
#   unchanged by K-batching), and Stall Branch Resolving 19.6% (newly visible
#   #2 factor, source not yet isolated). Root cause remains DFS-subtree-size
#   variance driving SIMT lane imbalance; 309 approaches this from the
#   task-reorder side (tail-phase assignment) rather than the kernel side.
# Safety: kernel_dfs_iter_gpu_maxd14/16/18/20/21 bodies are byte-identical to
#   304/308; only two top-level int constants change. No hot-loop, stack, or
#   K-batching logic is touched.
...
#           from 304/308), BROADMARK_VARIANT=4 (phase_rotate).
...
SRC=${SRC:-./309Py_variant4_phase_rotate.py}
CAND=${CAND:-./309Py_variant4_phase_rotate}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/309Py_variant4_phase_rotate_N21_full.lock}
...
BROADMARK_VARIANT=${BROADMARK_VARIANT:-4}
EXPECTED_BROADMARK_VARIANT=4
EXPECTED_BROADMARK_VARIANT_TAG="phase_rotate"
...
LOGDIR="${LOG_ROOT%/}/309Py_variant4_phase_rotate_logs_N21_full_once_${TS}"
...
echo "[start] 309 variant4-phase-rotate validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 309 variant4-phase-rotate SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '309 variant4-phase-rotate' "$SRC"; then
  printf 'source_version_tag	309 variant4-phase-rotate	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	309 variant4-phase-rotate	missing	FAIL
...
if grep -q '^A10G_FINAL_DEFAULT_BROADMARK_VARIANT:int=4' "$SRC"; then
  printf 'source_a10g_default_variant4	4	4	OK\n' >> "$SUMMARY"
else
  printf 'source_a10g_default_variant4	4	missing/mismatch	FAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 309 variant4-phase-rotate mode31 split145"
...
echo "[validation-ok] 309 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=4 (phase_rotate)"
