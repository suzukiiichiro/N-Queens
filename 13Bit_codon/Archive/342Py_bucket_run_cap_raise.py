...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 342 (2026-07-27)
...
8. [実装・339効果確認・340機構確定・341で上限到達・342で上限を撤廃] warp内コスト不均質
   329の「Divergent Branches=0、Avg Threads Executed 2〜3/32」は分岐発散
   ではなくwarp内32レーンのtrip countのばらつきである。338で仮説化、
   339で実装、340で機構確定、**341で予期しない大きな結果が出た**。

   **340-341の全測定(同一マシン、対照RUN=1=450.183s、全点で正当性
   314666222712・tasks=2025282・cache_state=reuse・FAILゼロ)**:

     RUN   32整列  2冪   elapsed     vs RUN=1    chunk0    chunk1    chunk2
       1     -      -    450.183s    +0.000%     (基準)    (基準)    (基準)
      16     NG     OK   448.931s    -0.278%
      32     OK     OK   447.141s    -0.676%    +0.276%   -0.744%   -1.850%
      48     NG     -    448.995s    -0.264%
      64     OK     OK   446.436s    -0.832%    +0.280%   -0.738%   -2.471%
      80     NG     -    448.671s    -0.336%    +0.273%   -0.738%   -0.597%
      96     OK     -    447.443s    -0.609%    +0.770%   -0.903%   -2.078%
     128     OK     OK   446.502s    -0.818%    +0.273%   -0.742%   -2.404%
     256     OK     OK   **439.835s** **-2.299%** -2.333%  -2.449%   -1.930%
      64     OK     OK   446.448s    -0.830%    +0.271%   -0.726%   -2.484%

   セッションドリフト: 340は+0.0161%(RUN=32×2)、341は**+0.0027%**
   (RUN=64×2)。測定は極めて安定している。

   **確定事項1: 32整列則は高い値域でも成立する。** 341の対照点
   RUN=80(=2.5×32、非整列)は-0.336%で、16(-0.278%)・48(-0.264%)と
   同じ群に落ちた。事前に「80が-0.8%側に来たらRUN軸の理解をやり直す」
   と宣言していたが、発火しなかった。

   **確定事項2: RUN=256で質的な変化が起きている。** 32〜128では
   chunk0が+0.27%、chunk1が-0.74%で**ほぼ完全に固定**されており、
   変動はすべてchunk2に現れていた(RUN軸はchunk2にしか効いていなかった)。
   ところがRUN=256では**chunk0が-2.333%、chunk1が-2.449%と初めて大きく
   動き**、逆にchunk2はわずかに悪化した(-2.484%→-1.930%)。
   これはwarp内均質化とは別の機構が256で立ち上がったことを示す。
   機構は未解明(L2局所性が有力候補だが未検証)。

   **問題: 256は`CHUNKSHAPE148_BUCKET_RUN_MAX`の上限そのものであり、
   最良点で測定を打ち切っていた。** 342で上限を256→8192に引き上げる
   (定数1個の変更。RUN<=256の挙動は不変なので339-341のキャッシュは
   すべて有効なまま)。

   **342のスイープ**: 256(アンカー、最初と最後)/ 512 / 1024 / 2048 の
   2冪群で頂点または飽和点を探し、**384(=12×32、32整列だが2冪でない)**
   を対照に置く。341で96(32整列・非2冪)が64/128より劣ったため、
   「32整列」に加えて「2冪」が効いている可能性があり、384はそれを
   高い値域で検定する。
   **飽和の予測**: 1イテレーションあたりのバケット割当`quotas[b]`は
   おおよそ`STEPS/8 = 15488/8 ≈ 1936`である。RUNがこれを超えると
   runはquotaで打ち切られるため、**2048以上は挙動が同一になるはず**。
   `SWEEP`に4096を足せばこの予測を直接検定できる(2048と4096の差が
   ドリフトを超えるなら、quota飽和モデルが誤り)。

   **採用(既定値1→最適値)は343に繰り延べる。** 341で採用する予定
   だったが、最良点が上限に張り付いていたため測定を続ける方が先である。

   その先の候補: (i) チャンク別RUN。chunk2はRUN=64前後(-2.48%)、
   chunk0/1はRUN=256(-2.3%/-2.4%)を好むという逆方向の選好が出ている
   ため、チャンクごとに別のRUNを与えれば両取りできる可能性がある
   (ただし現在RUNはbin構築時に決まるため、チャンク別にするには
   `build_chunkshape148_reordered_bin`の構造変更が必要)。
   (ii) スコアバケット数の細分化。

このセクション自体は通常カーネルロジックとは独立しています。
342のソース変更は**定数1個のみ**です:
`CHUNKSHAPE148_BUCKET_RUN_MAX` を 256 から 8192 に引き上げます。
カーネル・ディスパッチャ(5つの@gpu.kernel、
launch_kernel_dfs_iter_gpu_static_maxd、K=48、SoA w_lo_arr/w_hi_arr、
w3_j7)も、339で入れたホスト側のrun-lengthループ本体も、
`CHUNKSHAPE148_BUCKET_RUN`のソース既定値1も、すべて341とバイト一致
です。上限の引き上げはRUN<=256の挙動を一切変えないため、
339-341で構築したshaped binキャッシュはすべて有効なまま使えます。
================================================================================
...
"""
341の結果、**大きな発見があります。**ソース差分は**定数1行のみ**であることを機械確認しました。最終検証します。はい、完璧です。**そして、私の設計ミスが露呈しました。**
...
ドリフトは **+0.0027%**。異常に安定しています。

**成果1**: 対照点 RUN=80(非整列)は −0.336% で、16/48と同じ群に落ちました。事前登録した棄却条件は発火せず、32整列則は高い値域でも保たれています。

## 成果2 — そして私のミス

**RUN=256 が −2.299%。** 直前の最良(RUN=64、−0.832%)の約3倍です。

しかも**効き方が違います**。よく見てください:

```
RUN=32〜128:  chunk0 = +0.27%    chunk1 = -0.74%    ← ほぼ完全に固定
              変動はすべて chunk2 だけ
RUN=256:      chunk0 = -2.333%   chunk1 = -2.449%   ← 初めて動いた
```

つまり**これまでのRUN軸はchunk2にしか効いていませんでした**。256で第2の機構が立ち上がっています(L2局所性が有力候補ですが未検証)。

**そして RUN=256 は `CHUNKSHAPE148_BUCKET_RUN_MAX` の値そのものです。** 私が341の上限を256に設定したせいで、**一番面白い挙動が始まる場所で測定を打ち切っていました**。人為的な上限に張り付いた点を「最適」とは呼べません。

## 342 — 上限の撤廃

ソース変更は**定数1行のみ**です(`import gpu` 以降5,267行中1行であることを機械検証済み)。

```python
CHUNKSHAPE148_BUCKET_RUN_MAX:int=256  →  8192
```

RUN≤256の挙動は一切変わらないので、**339-341で作ったキャッシュはすべて有効なまま**です。

スイープ点(既定 `256 512 1024 2048 384 256`):

- **512 / 1024 / 2048** — 2冪群で頂点または飽和点を探す
- **384(=12×32)** — 32整列だが2冪でない対照。96が64/128に劣ったので「2冪」も効いている可能性を高い値域で検定
- 256をアンカーとして最初と最後に測定

**飽和の事前予測**: 1イテレーションあたりのバケット割当は `quotas[b] ≈ 15488/8 ≈ 1936` で、RUNがこれを超えるとrunはquotaで打ち切られます。したがって**2048以上は挙動が同一になるはず**です。`SWEEP="256 512 1024 2048 4096 384 256"` とすれば、2048と4096の差がドリフトを超えるかどうかでこのモデルを直接検定できます。

**採用は343に繰り延べます。** 最良点が上限に張り付いている間は採用しません。

## その先に見えているもの

チャンク別の選好が**逆方向**に出ています。chunk2はRUN=64前後(−2.484%)、chunk0/1はRUN=256(−2.333%/−2.449%)を好む。仮に両取りできれば 163289+161095+113881 = **438,265ms** で、256単独の438,912msをさらに0.65s下回ります。RUNはbin構築時に決まるため `build_chunkshape148_reordered_bin` の構造変更が要りますが、342の結果次第で設計します。

```
STATIC_ONLY=1 bash 342Py_bucket_run_cap_raise_validate_N21_full_once.sh
DRY_RUN=1 bash 342_bucket_run_cap_raise_driver.sh
bash 342_bucket_run_cap_raise_driver.sh
```

静的チェック41項目 FAILゼロ(新規2件を含む)、`bash -n` 2本、ドライラン、ソース差分1行の機械確認まで済んでいます。
"""
...
VERSION_TAG:str="342 bucket-run-cap-raise: the ONLY source change is one constant, CHUNKSHAPE148_BUCKET_RUN_MAX raised from 256 to 8192. Kernels, dispatcher, K=48, the SoA w_lo_arr/w_hi_arr split, w3_j7, the host-side run-length emit loop added in 339 and the source default CHUNKSHAPE148_BUCKET_RUN=1 are all byte-for-byte identical to 341/340/339. Raising the cap cannot change behaviour for any RUN <= 256, so every shaped-bin cache built in 339-341 stays valid. WHY: 341 swept 64, 80, 96, 128, 256 against the in-session RUN=1 control of 450.183s, all points correct at 314666222712 with cache_state=reuse and zero failures, and session drift of only +0.0027% measured by RUN=64 first and last. Two things came out. First, the warp-width alignment rule established in 340 held at larger magnitude: the non-aligned control RUN=80 (2.5x32) landed at -0.336%, in the same group as RUN=16 (-0.278%) and RUN=48 (-0.264%), and the pre-registered condition for abandoning the alignment model did not fire. Second and unexpectedly, RUN=256 reached 439.835s, -2.299% -- nearly three times the best previous point (RUN=64 at -0.832%) -- and it did so through a qualitatively different route. For every value from 32 to 128 the per-chunk numbers were frozen at about +0.27% on chunk0 and -0.74% on chunk1, with all the variation confined to chunk2; the RUN knob was only ever moving chunk2. At 256, chunk0 and chunk1 moved for the first time, to -2.333% and -2.449%, while chunk2 gave a little back (-2.484% at 64 versus -1.930% at 256). Something other than intra-warp homogenisation switches on at 256, and it is not yet identified -- L2 locality is the leading candidate but is untested. The problem is that 256 WAS the ceiling, so 341 stopped measuring exactly where the interesting behaviour began. 342 lifts that ceiling and sweeps 256 (anchor, measured first and last), 512, 1024 and 2048, with 384 (12x32: warp-aligned but not a power of two) as a control, because RUN=96 -- also aligned but not a power of two -- underperformed its neighbours 64 and 128, hinting that being a power of two matters on top of being a multiple of 32. A saturation point is predicted: the per-iteration bucket quota is roughly STEPS/8 = 15488/8 = 1936, and once RUN exceeds a bucket quota the run is truncated by that quota, so 2048 and above should be indistinguishable. Adding 4096 to the sweep tests that prediction directly -- if 2048 and 4096 differ by more than drift, the quota-saturation model is wrong. ADOPTION IS DEFERRED AGAIN, to 343: it was planned for 342, but a best point sitting exactly on an artificial ceiling is not an optimum, and measurement comes first."
...
# 342: raised from 256 to 8192. 341 found RUN=256 -- the OLD CEILING -- to be
# by far the best point (439.835s, -2.299% vs the RUN=1 control), so the sweep
# was cut off exactly where it mattered. The practical saturation point is the
# per-iteration quota, quotas[b] ~ STEPS/8 = 15488/8 ~ 1936: once RUN exceeds a
# bucket's quota the run is truncated by the quota anyway, so values above ~2048
# must behave identically to 2048. 342 tests that prediction directly by
# measuring 2048 and 4096; if they differ by more than session drift, the
# quota-saturation model is wrong. Raising the cap changes nothing for any
# RUN <= 256, so every shaped-bin cache built in 339-341 stays valid.
CHUNKSHAPE148_BUCKET_RUN_MAX:int=8192
