...
# 170 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 170 inherits the
# validated 169 MAXD14 static dispatch.  Its single kernel experiment changes
# only the MAXD14 schedule representation: seven current-frame operation classes
# are encoded in 3 bits and kept in PTX-safe scalar u32 fields.  The local u32[4]
# schedule array is removed for MAXD14; high-bit u64 shifts are intentionally avoided.
#
# For N=21 all launches must remain required_maxd=14, select MAXD14, report
# schedule_words=0 and 224 local stack bytes/thread.

SRC=${SRC:-./170Py_kernel_scalar_u32pair_schedule_maxd14_probe.py}
CAND=${CAND:-./170Py_kernel_scalar_u32pair_schedule_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/170Py_kernel_scalar_u32pair_schedule_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 170 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/170Py_kernel_scalar_u32pair_schedule_maxd14_logs_N21_full_once_${TS}"
...
assert '170 scalar-u32pair schedule GPU candidate supports N=5..27 only' in s
...
# MAXD14 must have only the four state stacks and PTX-safe scalar u32 schedule fields.
...
for forbidden in ('ctrl=__array__','ctrl[','root_cv','child_cv','INIT_MASK','u32(524288)','if sp>=MAXD','opcode_word:u32=packed_schedule','child_action:u32=(opcode>>','schedule_u64','schedule_bits:u64','child_jmark_mask:u64','<<u64(','>>u64('):
...
def dfs_scalar(N,raw,ld0,rd0,col0,a0,j,e,m1,m2,capacity=14,limit=120000):
    bm=(1<<N)-1;n3=1<<max(0,N-3);n4=1<<max(0,N-4)
    a0&=bm
    if a0==0:return 0,0
...
while dfs_cases<12000 and attempts<220000:
    ...
    try:
        ...
        got,obs2=dfs_scalar(N,raw,ld,rd,col,a,j,e,m1,m2,14)
    except RuntimeError:
        continue
...
print('MAXD14_SCALAR_U32PAIR_SCHEDULE=1')
...
PYMODEL
then
  printf 'source_scalar_u32pair_schedule_shape\trequested layout\tmodel/static failure\tFAIL\n' >> "$SUMMARY"
...
printf 'source_parent_baseline\tvalidated 169 MAXD14 static dispatch\tverified\tOK\n' >> "$SUMMARY"
printf 'source_scalar_u32pair_schedule_shape\tMAXD14 scalar u32-pair + fallback byte kernels\tverified\tOK\n' >> "$SUMMARY"
...
printf 'maxd14_scalar_u32pair_schedule\t1\t%s\tOK\n' "$MAXD14_SCALAR_U32PAIR_SCHEDULE" >> "$SUMMARY"
...
printf 'dfs_164_vs_170_scalar_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
printf 'dfs_scalar_random_max_depth_tested\t>=7\t%s\tOK\n' "$DFS_MAX_DEPTH_TESTED" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 170 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=224 bytes/thread"
  echo "================================================================"
} | tee "$RUN_LOG"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "169:$BASELINE_169_SECONDS" "168:$BASELINE_168_SECONDS" "167:$BASELINE_167_SECONDS" "166:$BASELINE_166_SECONDS" "165:$BASELINE_165_SECONDS" "164:$BASELINE_164_SECONDS" "162:$BASELINE_162_SECONDS"; do
...
echo "[validation-ok] 170 scalar-u32pair PTX-safe MAXD14 schedule reproduced cases 01-06 with required=14, MAXD14, 224 bytes/thread"
