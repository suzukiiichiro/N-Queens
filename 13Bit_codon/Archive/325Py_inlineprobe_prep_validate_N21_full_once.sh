...
# =============================================================================
# 325 N=21 full validation harness (inlineprobe-prep)
# Parent: 324 devicefunc-specialize-design (documentation-only, no
#   source changes, no build/run). None of 320/321/322/323/324 have
#   their own confirmed N=21 timing; 319 (455.116s, 314666222712)
#   remains the most recent confirmed baseline.
#
# 324 designed (did not implement) an in-kernel device-function
#   specialization approach for kernel_dfs_iter_gpu_maxd14, distinct
#   from the 4 rejected separate-@gpu.kernel attempts (240/266-269/273).
#   Mandatory pre-check #1 before writing any real code: verify whether
#   Codon actually inlines a plain function called from inside a
#   @gpu.kernel, or emits a real CALL/RET boundary.
#
# This revision adds a STANDALONE, throwaway diagnostic probe to answer
#   that question in isolation, with ZERO changes to the main solver:
#     325_gpu_inline_probe.py         -- minimal @gpu.kernel that
#       branches per-thread and calls one of two distinct plain
#       functions (variant_a/variant_b), each containing a small loop
#       that resists constant-folding, so inlining vs a real call
#       boundary is unambiguous in the disassembly either way.
#     325_gpu_inline_probe_check.sh   -- builds it and runs
#       `cuobjdump --dump-sass` (no GPU execution needed) to check for
#       CALL/RET and separate variant_a/variant_b symbols, falling back
...
#   they test an unrelated toy kernel, not the solver. Run them with:
#     bash 325_gpu_inline_probe_check.sh
#   Interpreting the result: CALL/RET + separate symbols means Codon
#   does NOT inline (specialization design needs a different approach,
#   e.g. Codon generics or accepting/measuring real call overhead); no
#   CALL/RET with both variant bodies inlined into probe_kernel means
#   Codon DOES inline (safe to prototype the real future_check_mask==0
#   split next, per 324's plan).
#
# This revision: ZERO changes to kernel logic in the main solver (same
#   as 311-324: variant=2, K=48). The probe files above are entirely
...
SRC=${SRC:-./325Py_inlineprobe_prep.py}
CAND=${CAND:-./325Py_inlineprobe_prep}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/325Py_inlineprobe_prep_N21_full.lock}
...
# NOTE: none of 320/321/322/323/324's own N=21 full runs have been
...
# design writeup -- none requested a build/run of the main solver). No
# fabricated BASELINE_320_*/321_*/322_*/323_*/324_* values are added
# here; 319 remains the most recent confirmed timing baseline until this
# 325 run (or a future one) produces a newer one.
...
LOGDIR="${LOG_ROOT%/}/325Py_inlineprobe_prep_logs_N21_full_once_${TS}"
...
echo "[start] 325 inlineprobe-prep validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 325 inlineprobe-prep SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '325 inlineprobe-prep' "$SRC"; then
  printf 'source_version_tag	325 inlineprobe-prep	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	325 inlineprobe-prep	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 325 inlineprobe-prep mode31 split145"
...
echo "[validation-ok] 325 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); added a standalone 325_gpu_inline_probe.py + 325_gpu_inline_probe_check.sh (unrelated to this harness) to test Codon's device-function inlining behavior in @gpu.kernel, per 324's mandatory pre-check #1; NO change to the main solver in this revision"
