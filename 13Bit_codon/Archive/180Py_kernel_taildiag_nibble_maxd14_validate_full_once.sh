...
# 180 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 180 deliberately
# reverts to the validated fastest 175 nibble MAXD14 kernel after the 178/179
...
# and 208 local stack bytes/thread. The new experiment is on the harness side:
# richer chunk-tail diagnostics are derived from the existing progress TSV so
# the next optimization can target task placement rather than unsafe hot-loop
# rewrites.
SRC=${SRC:-./180Py_kernel_taildiag_nibble_maxd14_probe.py}
CAND=${CAND:-./180Py_kernel_taildiag_nibble_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/180Py_kernel_taildiag_nibble_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 180 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/180Py_kernel_taildiag_nibble_maxd14_logs_N21_full_once_${TS}"
...
assert '180 nibble-schedule GPU candidate supports N=5..27 only' in s
...
# schedule fields, the 175 register-held active frame, and the 180 nibble schedule hot path.
...
def nibble_from_byte(op):
    # 180 keeps the byte-opcode low nibble directly:
...
printf 'source_parent_baseline\tvalidated fastest 175 nibble MAXD14 dispatch\tverified\tOK\n' >> "$SUMMARY"
printf 'source_taildiag_nibble_shape\tvalidated 175 MAXD14 nibble kernel + extended tail diagnostics\tverified\tOK\n' >> "$SUMMARY"
...
printf 'dfs_164_vs_180_nibble_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 180 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, validated 175 nibble kernel + 180 tail diagnostics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 180 keeps the validated 175 kernel structure, then derives chunk-tail diagnostics from
...
# 180: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 180 taildiag nibble baseline reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
