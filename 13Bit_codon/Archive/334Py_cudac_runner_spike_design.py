...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 334 (2026-07-23)
...
6. [設計中・334で着手] CUDA Cランナー(案C-2)スパイク設計
   ...
   最小スパイク設計を334で開始する。方針: コンステレーション生成・
   ...
   唯一の直接対策)とatomic演算(C-1の再検討)。334自体はメインソルバの
   カーネル・ディスパッチャに一切触れず、実装前の必須事前確認(移植版
   324の手法を踏襲)のみを行う: (1) cudacodon実機でのnvcc/CUDA
   toolchainの存在・バージョン確認(`nvcc --version`)、(2) 既存bin
   ファイル形式(schedule/w_lo_arr/w_hi_arr/broadmarktailメタデータの
   バイトレイアウト)をC側readerが読める形で仕様化できるかの調査、
   (3) 正当性確認の最小スコープをN=18(rev84由来の提案どおり)に絞り、
   N=21フルは移植版が安定してから、とする段階分け。このリビジョンでは
   nvcc等の実機確認は未実施(次セッションでvalidateスクリプトのnvcc
   プローブ実行結果をログtarで確認する)。

このセクション自体は通常カーネルロジックとは独立しています。
334はカーネルロジックの変更を含みません(カーネル・ディスパッチャは
328-333と同一、funcid_reorderもw3_j7のまま)。ソースの変更は、
このドキュメント/VERSION_TAGとOpen Objectives項目5・6のみです。
================================================================================
...
**334の内容(CUDA Cランナー案C-2スパイク設計・着手)**
- 333のw3_j7採用結果(正当性314666222712、450.667s、旧w8_j7比+1.18%改善)を確認し、発掘案C-3を完全クローズしました
- カーネル・ディスパッチャは328-333と一切変更なし。ソース変更はOpen Objectives項目5(結果反映)・項目6(新設)とVERSION_TAGのみです
- 課題3(tail effect)への残存対策であるCUDA Cランナー(案C-2、rev84提案)のスパイク設計に着手し、事前確認事項(nvcc/CUDA toolchain確認、bin形式のC側reader仕様化、N=18限定の最小正当性スコープ)を明文化しました
- 検証スクリプトに、実機での`nvcc --version`/`cuobjdump --version`有無を記録するだけの**非ゲーティング**(FAILにしない、INFO記録のみ)プローブを追加しました。既存のN=21フルvalidateには一切影響しません
- 次の一手は、このプローブの実行結果(ログtar)を確認してから、335でC-2スパイクの実装可否を判断することです
...
Now let's update the VERSION_TAG:Now let's add the new 333 baseline and the nvcc/cuobjdump non-gating probe. First, add the baseline variable and update comparison list:Now let's check the remaining "333:" comment at line 434 (w3_j7 check comment, fine to leave as historical reference) and verify LOGDIR line updated:Good. Now let's find where to add the non-gating nvcc/cuobjdump probe — near the other INFO-style checks (like gpu_clock_cap_check) in the run section, after GPU execution.Now let's do a bash syntax check on the script:Good, no stale references. Now let's check the Python file for stale `333Py_w3j7_adopt` references outside what we already updated (e.g. version tags used in log print statements referencing filenames):Good. Now let's finalize and copy outputs, then update README.md with a new rev 334 entry.Now let's create the updated README.md with a new rev 334 entry appended:Now let's append the chronological log entry for rev 334 at the end of README.md:Good. Now copy README to outputs and present the files.おはようございます、鈴木さん。333の成果、確認しました — w3_j7採用で正当性314666222712・elapsed 450.667s、旧w8_j7比+1.18%改善。発掘案C-3、1日4リビジョンでの完結、お見事でした。

334は以下の設計にしました。

**334の内容(CUDA Cランナー案C-2 スパイク設計・着手)**
- カーネル・ディスパッチャ・ホスト側パラメータは328-333と一切変更なし(w3_j7のまま)。ソース変更はOpen Objectives項目5(333結果の反映)・項目6(新設)とVERSION_TAGのみ
- 課題3(tail effect)への唯一の残存対策である案C-2(rev84提案のCUDA Cランナー)のスパイク設計に着手:
  - 移植範囲は`kernel_dfs_iter_gpu_maxd14`一個のみ。コンステレーション生成/並べ替え/binキャッシュはCodonのまま
  - 正当性確認はN=18限定(rev84原提案どおり)
  - 解禁される手段: warp intrinsics(`__shfl_sync`等)とatomic演算
- 検証ハーネスに**非ゲーティングな**環境プローブを追加(`nvcc --version`/`cuobjdump --version`をsummary.tsvにINFO記録のみ、GPU不使用、STATIC_ONLY=1でも実行、失敗にはしない)

実装はまだ行っていません。まず`nvcc`/`cuobjdump`が実機にあるかを確認するための下調べリビジョンです。

次の一手:
```
STATIC_ONLY=1 bash 334Py_cudac_runner_spike_design_validate_N21_full_once.sh
```
問題なければフルN=21実行。ログtar(nvcc/cuobjdumpプローブ結果込み)をいただければ、335でC-2スパイクの実装可否を判断します。
"""
...
VERSION_TAG:str="334 cudac-runner-spike-design: NO kernel or dispatcher change -- byte-for-byte identical to 333/328-332 (w3_j7 remains the funcid_reorder default). 333's own full N=21 validate is now CONFIRMED: correctness 314666222712, elapsed 450.667s (vs 332's direct 450.113s measurement, -0.123%, within noise), kernel_reduce_ms sum 448920ms (within +-0.03% of 331/332's 448789/448791), a +1.18% improvement over the old w8_j7 baseline (456.036/456.187s). Buried-idea C-3 (funcid_reorder w/j retune) is fully closed as of 333. 334 begins design work on buried-idea C-2 (the rev84 CUDA C runner proposal), which is the sole remaining route to Open Objective #3 (tail effect / occupancy collapse: stall_branch_resolving 65.5% concentration on two BRA/BSYNC sites, Avg Threads Executed 2-3/32, confirmed 329) now that plan C-1 (persistent kernel + atomic work queue) is formally closed (330: Codon @gpu.kernel has no device atomics). Design scope for the eventual spike (not implemented in 334): port ONLY kernel_dfs_iter_gpu_maxd14 to a .cu kernel+launcher, keep constellation generation / broadmarktail+chunkshape148+funcid_reorder(w3_j7) reordering / bin caching entirely in Codon, target the same bin file format so the CUDA C side is a drop-in reader, and validate correctness at N=18 only (per the original rev84 proposal) before ever attempting N=21. This unlocks warp intrinsics (__shfl_sync/__ballot_sync/__activemask, the only direct fix for the 2-3/32 tail-effect lanes) and device atomics (revisiting plan C-1), independent of Codon's toolchain limits. 334's only concrete action is a non-gating environment probe appended to the validate harness (nvcc --version / cuobjdump --version presence and version, recorded as INFO, never fails the run) to establish whether cudacodon's CUDA toolchain is usable for a standalone .cu build before any spike code is written. Everything else (kernel, dispatcher, host reorder parameters, K=48, SoA w_lo_arr/w_hi_arr) is unchanged from 333."
