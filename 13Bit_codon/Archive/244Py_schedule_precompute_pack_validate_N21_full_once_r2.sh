...
# =============================================================================
# 244 r2 N=21 full validation harness
...
#             r1 3-array high-bit pack triggered CUDA_ERROR_INVALID_PTX.
...
SRC=${SRC:-./244Py_schedule_precompute_pack_probe_r2.py}
CAND=${CAND:-./244Py_schedule_precompute_pack_probe_r2}
...
echo "[start] 244 r2 schedule-precompute-pack4-lowbitsafe validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 244 r2 schedule-precompute-pack4-lowbitsafe SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '244 r2 schedule-precompute-lowpack-lowbitsafe' "$SRC"; then
  printf 'source_version_tag\t244 r2 schedule-precompute-lowpack-lowbitsafe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t244 r2 schedule-precompute-lowpack-lowbitsafe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if not m14:
    ...
    ok = (
        ...
        'terminal_depth_u:u32=ctrl_pack&u32(15)' in body and
    ...
    actual = 'lowpack precompute active' if ok else 'old interpreter/highpack/unpacked arrays present'
    checks.append(('source_schedule_precompute_lowpack', 'MAXD14 lowpack precompute active', actual, ok))
...
checks.append(('source_host_precompute_lowpack_arrays', '4 lowpack arrays present', 'present' if host_ok else 'missing/highpack/unpacked arrays present', host_ok))
fail = 0
...
echo "[static-ok] 244 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 244 r2 schedule-precompute-pack4-lowbitsafe mode31 split145"
...
echo "[validation-ok] 244 r2 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
