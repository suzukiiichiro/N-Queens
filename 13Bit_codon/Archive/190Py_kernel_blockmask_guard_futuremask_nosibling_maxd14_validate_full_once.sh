...
# =============================================================================
# NQ_UPDATE_MEMO
# 190: 更新メモ: 189は正当性OKだが188比で大幅退行したため不採用。188を親に戻し、MAXD14 kernelのみ block_check_mask==0 のscheduleでblock分岐/decodeを省く軽量guardを単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 190 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 190 starts from
# validated 188 because 188 is the current fastest N=21 baseline and 189
# forced-chain was correct but much slower. 190 changes only the MAXD14
# block_check_mask zero guard.
#
...
# bytes/thread. The experiment is deliberately focused: when a deterministic
# schedule has no block-operation depth, skip the per-candidate block_code
# branch/decode and use the normal one-row diagonal update directly. Host task
# order, dispatch, cache, progress verification, and README-managed history remain unchanged.
SRC=${SRC:-./190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 190 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/190Py_kernel_blockmask_guard_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 190Py source
#   has a harmless header/string/layout difference. This 190 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 188 futuremask/no-sibling MAXD14 dispatch; 189 forced-chain not adopted; 185/186/187 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + block_check_mask zero guard	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'block_check_mask:u32=u32(0)' "$SRC" && grep -q 'if block_check_mask==u32(0):' "$SRC" && grep -q 'schedule_block_code!=u32(0)' "$SRC"; then
  printf 'source_block_check_mask_guard	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_block_check_mask_guard	present	missing	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split190' "$SRC" && ! grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split190 only	split190 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split190 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 190 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source blockmask-guard static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 190 blockmask-guard futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 190 keeps the validated 188 host/chunk structure, then derives chunk-tail diagnostics from
...
# 190: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 190 blockmask-guard futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
