...
"""
260 schedule-precompute note: 259 tempmaskもCUDA_ERROR_INVALID_PTXだったため、root_action decodeを配列loadとmaskの二段式へ分離する。terminal_depth/terminal_base14は別配列のまま保持し、child/future maskpackは維持するprobe。


259は **括弧付きでも `CUDA_ERROR_INVALID_PTX`** なので、`root_action` の index-and-mask 形はJIT危険と見てよいです。

次段として **260Py rootaction tempmask** を作成しました。
259の式をさらに分解し、kernel内で以下の二段式にしています。

```python
root_action_raw:u32 = sched_root_action_arr[i]
root_action:u32 = root_action_raw & u32(3)
```

目的は、`sched_root_action_arr[i] & u32(3)` という単一式がPTX/JITに悪いのか、`root_action` 配列decode自体が悪いのかを切り分けることです。
...
source_schedule_precompute_rootaction_tempmask OK
source_host_precompute_rootaction_tempmask_arrays OK
release_build_policy                       OK
```

ファイルはこちらです。

* [260Py_schedule_precompute_rootaction_tempmask_probe.py](sandbox:/mnt/data/260Py_schedule_precompute_rootaction_tempmask_probe.py)
* [260Py_schedule_precompute_rootaction_tempmask_validate_N21_full_once.sh](sandbox:/mnt/data/260Py_schedule_precompute_rootaction_tempmask_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [260Py_schedule_precompute_rootaction_tempmask_sha256.txt](sandbox:/mnt/data/260Py_schedule_precompute_rootaction_tempmask_sha256.txt)
* [260Py_schedule_precompute_rootaction_tempmask_bundle_20260707.tar](sandbox:/mnt/data/260Py_schedule_precompute_rootaction_tempmask_bundle_20260707.tar)

sha256です。

```text
b14ee3208c7bf1cfa81e95a7bb4a03e81957259f8c1b9b2c9325ae8336a54e07  260Py_schedule_precompute_rootaction_tempmask_probe.py
17782adf25d9ce1741eb76f86bb62d23149c982fc9b985e3730eebc942a809d9  260Py_schedule_precompute_rootaction_tempmask_validate_N21_full_once.sh
e8ac2ecffa423c68c1f56381145c2e3dab7c1fff5dffc0445b9a858da6f6f443  README.md
```

実行は以下でお願いします。

```bash
rm -f ./260Py_schedule_precompute_rootaction_tempmask_probe

STATIC_ONLY=1 bash 260Py_schedule_precompute_rootaction_tempmask_validate_N21_full_once.sh

bash 260Py_schedule_precompute_rootaction_tempmask_validate_N21_full_once.sh
```

260もJIT不成立なら、`root_action` のmask decodeは諦めて、**root_actionは別配列素通しのまま**に戻すのが安全です。
...
VERSION_TAG:str="260 schedule-precompute-rootaction-tempmask probe: parent 256 base14pack JIT-ok / 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; child/future maskpack retained; 257/259 root_action direct/paren mask caused CUDA_ERROR_INVALID_PTX; only root_action decode is split into raw load then mask while terminal_depth/terminal_base14 stay separate; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 260: bare -g stays split145 mode31; root_action tempmask probe
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 260: root_action decode is split into raw load then mask to test
    # whether Codon/PTX rejects only the direct index-and-mask expression. terminal_depth/terminal_base14 remain
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split260_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split260_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      progress_suffix:str=f"split260_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
