...
[129Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/129Py_scorestripe_chunkshape_probe.py)
[129Py1-6.sh](sandbox:/mnt/data/129Py1-6.sh)

129 の中身は、128 の **generic-only / mode30・mode31 harness** を継承し、GPU kernel は触らず、host-side の `chunkshape129_scorestripe_v1` だけを追加しています。

`129Py1-6.sh` は初回検証用です。129 では chunk の構成が変わるため、mode30 の partial はまだ固定値にせず、まず `partial_total=` の出力確認にしています。full correctness は従来どおり `314666222712.*ok` を確認します。

実行は以下の形です。

```bash
codon build -release 129Py_scorestripe_chunkshape_probe.py
./129Py1-6.sh
```
...
```bash
RUN_FULL=0 ./129Py1-6.sh
```

こちらでは `129Py1-6.sh` の bash 構文確認と、129 側の主要シンボル確認まで済ませています。
...
VERSION_TAG:str="129 scorestripe chunkshape probe - mode30/mode31 generic kernel, host-side score-aware shaping"
...
####################################################################################################
# 129 reorder chunkshape harness
#
...
# host-side harness from 127 for mode30 selected-chunk probes and mode31 worker/full runs, plus 129 chunk-shape bin.
#
# Purpose:
...
def gen_constellations(
  ...
  # which caused misleading logs such as preset_queens=13 or preset_queens=129.
  return ijkl_list, subconst_cache, constellations, preset_queens
...
####################################################################################################
# 129 score-aware chunk-shape layer
#
...
# N21 while preserving correctness. 129 keeps that GPU/kernel path intact and changes only the
# second-stage permutation after the 114/115 broadmarktail reorder bin has been built.
#
# scorestripe_v1 is a host-side permutation over 16-byte constellation records:
...
CHUNKSHAPE129_REORDER_VERSION:str="scorestripe_v1"
CHUNKSHAPE129_DEFAULT_REASON:str="host-side score-aware quota stripe over broadmarktail v4; generic kernel unchanged"

def chunkshape129_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape129_{CHUNKSHAPE129_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}.bin"

def chunkshape129_score_bucket_from_soa(soa:TaskSoA,idx:int)->int:
  """
  129 score bucket for host-side shaping only.

  The bucket is deliberately coarse.  It uses SoA fields already produced by
  build_soa_for_range(), but it does not change DFS state, symmetry weights,
  kernel inputs, or solution arithmetic.  The only consumer is the on-disk
  record permutation built below.
  """
  fid:int=soa.funcid_arr[idx]
  ...
  # Risk hints from the previous broadmark / mark-distance investigations.
  # They are used only to distribute records across output chunks.
  if fid==26 or fid==27:
    score+=18
...
def chunkshape129_make_quotas(rem_counts:List[int],total_remaining:int,m_target:int)->List[int]:
...
def build_chunkshape129_reordered_bin(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  """
  129 host-side score-aware chunk shaping.
  ...
  out_fname:str=chunkshape129_reorder_output_fname(N,preset_queens,BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_chunkshape129_{CHUNKSHAPE129_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)}_sim.tsv"

  if gpu_log_level>=1:
    print(f"[chunkshape129-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE129_DEFAULT_REASON}")
  ...
  if len(data)<expected_bytes:
    print(f"[chunkshape129-warning] source byte count shorter than expected: bytes={len(data)} expected={expected_bytes}")
  ...
  # ------------------------------------------------------------------
  # Stage 1: classify every source record into a coarse score bucket.
  # ------------------------------------------------------------------
  score_by_idx:List[int]=[0]*total_records
  ...
  while off<total_records:
    ...
    i=0
    while i<m:
      b:int=chunkshape129_score_bucket_from_soa(soa,i)
      if b<0:
        b=0
      if b>7:
        b=7
      score_by_idx[off+i]=b
      bucket_counts[b]+=1
      i+=1

    if gpu_log_level>=2:
      print(f"[chunkshape129-classify-chunk] N={N} chunk={classify_chunk} off={off} m={m}")
    off+=m
    classify_chunk+=1

  if gpu_log_level>=1:
    print(f"[chunkshape129-score-buckets] N={N} records={total_records} b0={bucket_counts[0]} b1={bucket_counts[1]} b2={bucket_counts[2]} b3={bucket_counts[3]} b4={bucket_counts[4]} b5={bucket_counts[5]} b6={bucket_counts[6]} b7={bucket_counts[7]}")
  ...
  if staged_total!=total_records:
    print(f"[chunkshape129-warning] staged count mismatch: staged={staged_total} source={total_records}")
  ...
  with open(out_fname,"ab") as out:
    while out_ch<n_chunks:
      ...
      quotas:List[int]=chunkshape129_make_quotas(bucket_rem,total_remaining,m_target)
      ...
      if gpu_log_level>=2:
        print(f"[chunkshape129-build-chunk] N={N} chunk={out_ch} written={written_chunk} total_written={written_total} b0={written_by_bucket[0]} b1={written_by_bucket[1]} b2={written_by_bucket[2]} b3={written_by_bucket[3]} b4={written_by_bucket[4]} b5={written_by_bucket[5]} b6={written_by_bucket[6]} b7={written_by_bucket[7]}")
      out_ch+=1
  ...
  if written_total!=total_records:
    print(f"[chunkshape129-warning] record count mismatch: written={written_total} source={total_records}")
  if reordered_records!=written_total:
    print(f"[chunkshape129-warning] output count mismatch: file_records={reordered_records} written={written_total}")
  if gpu_log_level>=1:
    print(f"[chunkshape129-build-summary] N={N} records={reordered_records} chunks={n_chunks} output={out_fname} progress={progress_fname} valid={1 if validate_bin_file(out_fname) else 0}")
...
"""129: reorder/chunkshape generic-only split harness。special kernel は使わず、全 task を 115 汎用 kernel に流す。"""
def exec_solutions_gpu_chunk_split129(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  if gpu_log_level>=1:
    print(f"[split129-buckets] N={N} m={m} generic={m} d2base14={d2base14_m} d0={d0_m} rest=0 split_mode={split_mode} specialized=0")
...
def exec_solutions_gpu_bin_stream_split129(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split129_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split129_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split129_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split129_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split129-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split129-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split129-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split129-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split129(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split129-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split129-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split129 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split129 full run / optional multi-GPU worker
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 30 8 7 0 1222 5 "" 2  # 129 shaped generic-only probe chunks 1222..1226
      #   -g 22 22 32 484 1 0 7 31 8 7 0 0 1 2  # 129 shaped generic-only full GPU single worker
      if argc >= 11:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split129-generic-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split129-generic worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split129_generic_harness_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split129_generic_harness_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  if gpu_log_level>=1 and (bench_mode==30 or bench_mode==31):
    print(f"chunkshape129_params: version={CHUNKSHAPE129_REORDER_VERSION} reason={CHUNKSHAPE129_DEFAULT_REASON}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split129-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split129-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      shaped_fname:str=chunkshape129_reorder_output_fname(N,preset_queens,gpu_block,gpu_max_blocks)
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape129-probe-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape129-probe-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape129_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split129_probe_{CHUNKSHAPE129_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split129(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split129-probe-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE129_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split129-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split129-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split129-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape129-full-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape129-full-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape129_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split129_full_{CHUNKSHAPE129_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split129(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split129-full-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE129_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
