...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  # 54 CHUNKAUTO EXP:
  # gpu_max_blocks <= 0 のときは、全 constellation を 1 kernel launch にまとめる。
  # 49 の kernel ロジックは変更せず、chunk tail / launch 境界の影響だけを見る実験。
  # 例: N=20, BLOCK=32, m_all=139232 -> MAX_BLOCKS=4351, STEPS=139232
  if gpu_max_blocks<=0:
    MAX_BLOCKS=(m_all+BLOCK-1)//BLOCK
    if MAX_BLOCKS<1:
      MAX_BLOCKS=1
  else:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk54={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  # 54 CHUNKAUTO EXP: preset_queens は 5 固定。46で 6 を試したところ N=20 が ng になったため、
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 54 CHUNKAUTO EXP; using 5")
      preset_queens_arg=5
    if argc > 9:
