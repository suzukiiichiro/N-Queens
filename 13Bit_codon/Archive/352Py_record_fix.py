...
**352の設計です。** ソース変更は記録のみ、実行コードは351と1バイトも
変わりません。

## なぜ352が必要か

**351で私が書いた記述に誤りがありました。** VERSION_TAG・
`WHI_ELIM_REASON`・README・docstringの4箇所に、こう書いてあります。

> u64乗算はそのまま残ります。上位オペランドが常にゼロであることを
> コンパイラは知りようがないので、351では乗算は縮みません。

**これは誤りです。** `u64(w_lo_arr[i])` は u32 ロードのゼロ拡張なので、
上位32ビットがゼロであることは中間表現の段階で証明済みです。350では
上位がロード値だったので不透明でしたが、351では違います。
...
64x64 が 64x32 に退化し、さらに累算が `IMAD.WIDE` の加数へ融合されて
います。**351は、私が「352の仕事」として切り分けたものの一部を、
すでに取っていました。** 誤った記述を残したまま次へ進むと、352の
期待値を過大評価したまま設計することになります。

## 何を確定させるか

**351の -0.20% は生成コードでは説明できません。** これが352の中心的な
結論です。

| 候補 | 判定 | 根拠 |
|---|---|---|
| 除去した作業そのもの | **排除** | 高々数百サイクル/件 対 実測 8.4x10^6 サイクル/件。4桁 |
| スピルの消滅 | **排除** | STL/LDL 6=6、Stack Size 1024=1024 |
| 占有率 | **排除** | Block Limit Registers 40、Occupancy 33.33% がいずれも不変 |
| 制御フローの変化 | **排除** | BRA/BSSY/BSYNC/BREAK の差分ゼロ |
| ホットループの命令構成 | **排除** | 構造一致率 93.4%、変更は全てエピローグ3箇所とレジスタ割当のゆらぎ |

残る候補は**命令キャッシュ整列**と**レジスタバンク競合**です。カーネルは
128バイト短くなり、全域でレジスタ番号が付け替わりました(命令テキストの
完全一致率は21.6%)。

同一入力・同一グリッドのN=18 launchで、**`no_instruction`(命令フェッチ
ストール)が +6.07%、`wait` が -0.65%** 動いています。実行命令数が
2,251,673,225 対 2,251,672,257 で一致するので、この2つは互いに比較
できます。**前者が実在の摂動であることの直接的証拠**ですが、
**これは候補であって検証していません。** 347で「機構が判明した」と
書いて348で反証された件を繰り返しません。

## エピローグ軸を閉じます

乗算は既に2命令です。eを `markctrl` に詰めても消えるのは残り2命令程度、
解析的には約 **0.00002%**。**測れば何か出るでしょうが、それはくじの目で
あって、e詰め込みの効果ではありません。**

これを一般化して**項目12「カーネル微修正はくじ引きである」**を新設
しました。運用上の帰結は「測れるが帰属できない」——採否は実測で決めて
よいが、**機構の説明を付けてはならず、次の変更後も効果が残ると期待して
はならない**、です。

## 静的チェック

**`source_code_identical_to_351` を新設しました。** docstringと
VERSION_TAG/WHI_ELIM_REASONを除いた実行コードが、351のソースと
**バイト単位で完全一致**することを検査します。351のソースが同じ
ディレクトリに無い場合はSKIPではなくFAILにしてあります。

351由来の5つの検査(`source_whi_elim_signatures` /
`..._kernel_bodies` / `..._dispatcher` / `source_whi_zero_guard` /
`runtime_whi_elim`)は**そのまま残します**。実行コードが変わっていない
のだから、当然通るべきです。**構造を変えていないのにチェックを触るのは
筋が悪い**ので、ここは一行も動かしていません。

## 実行について

**再測定は確認であって発見ではありません。** バイト一致が静的に証明
されているので、走らせなくても記録としては閉じられます。

ただし1本回しておく価値はあります。**352のバイナリが今後の基準線に
なる**ので、その実測値が1つあるほうが、次のリビジョンでアンカーを
取り直すときに比較先が増えます。13分です。お任せします。

```
STATIC_ONLY=1 bash 352Py_record_fix_validate_N21_full_once.sh
bash 352Py_record_fix_validate_N21_full_once.sh          # 任意
```

キャッシュは `..._isort9.bin` を再利用します(`cache_state=reuse`)。

## 次は項目11(wait / branch_resolving)です

N=18で `wait` 45.6%、`branch_resolving` 20.1%、合わせて **66%**。
これは発散DFSそのものの性質で、エピローグとは無関係です。
**ただしここも「機械についての問い」であって、性能が動く保証は
ありません。** 353の設計時に、まずN=21で正しくncuを取り直す
ところから始めます。
...
VERSION_TAG:str="352 record-fix: RECORD CORRECTION REVISION. THE EXECUTABLE CODE IS BYTE FOR BYTE IDENTICAL TO 351; only VERSION_TAG, WHI_ELIM_REASON and the two docstrings change, and the shell script proves that identity statically with source_code_identical_to_351. THE CORRECTION: 351 stated in four places that the u64 multiply was deliberately left alone because the compiler cannot know the high operand is always zero. THAT WAS WRONG. u64(w_lo_arr[i]) is a zero extension of a u32 load, so the high half is provably zero in the IR, and the SASS comparison run for 352 shows the multiply went from three IMADs to two: 350 issued IMAD.WIDE.U32 for lo times lo plus IMAD for hi_w times lo_t plus IMAD for lo_w times hi_t, while 351 issues only the first and the last, with the accumulation folded into the IMAD.WIDE addend. 64 by 64 degenerated into 64 by 32. 351 had already taken part of what was carved out as the job of 352. THE MEASUREMENT STANDS: chunk0 improved 0.2087 percent against the in-session 350 anchor, 0.1998 percent against the 0728 350, and 0.1998 percent under ncu attachment, three independent controls agreeing within 0.01 points; elapsed moved between 0.17 and 0.29 percent. THE MECHANISM IS NOT THE GENERATED CODE. Structural match ignoring register numbers is 650 of 696 or 93.4 percent, the instruction count fell from 696 to 688 with 6 of those 8 at the three epilogue sites and 2 from register allocation churn, LDG.E fell from 13 to 10 which is exactly the three removed loads with every other memory operation preserved one to one, STL and LDL are 6 in both so nothing spills differently, stack size is 1024 in both, control flow shows zero difference, and registers per thread fell from 47 to 45 while block limit registers stayed at 40 and theoretical occupancy stayed at 33.33 percent so registers were never binding. The work removed is worth at most a few hundred cycles per constellation against a measured saving of 8.4 million cycles per constellation, four orders of magnitude short. On an identical N=18 launch, where both binaries execute 2251673225 and 2251672257 instructions and are therefore directly comparable, no_instruction stalls rose 6.07 percent and wait stalls fell 0.65 percent while every other reason moved less than 0.7 percent and the total sample count moved 0.014 percent. The kernel is 128 bytes shorter and every register number was reassigned, so instruction cache alignment and register bank conflict patterns both changed; the no_instruction movement is direct evidence that the former is a real perturbation. THAT IS A CANDIDATE, NOT A CONCLUSION, and it is recorded as such because 347 claimed a mechanism and 348 refuted it. CONSEQUENCE: the epilogue axis, Open Objectives item 7, is CLOSED. Packing the exponent into markctrl bits 20 and 21 would remove about two more instructions per constellation, analytically worth 0.00002 percent, so whatever it measured would be a lottery draw and not the effect of the change. New item 12 records the general lesson: kernel micro edits on this workload move wall clock by roughly 0.2 percent for reasons unrelated to the edit, results may be adopted on measurement but must not be given a mechanism and must not be expected to survive the next edit. Next axis is item 11, wait at 45.6 percent and branch resolving at 20.1 percent, together 66 percent, which is a property of the divergent DFS itself."
