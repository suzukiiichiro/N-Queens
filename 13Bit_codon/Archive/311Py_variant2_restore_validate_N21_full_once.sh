...
# =============================================================================
# 311 N=21 full validation harness (variant2-restore)
# Parent: 310 variant1-phase-only — REJECTED. 476.932s vs 304 K=48=351.070s,
#   delta -125.862s (-35.851%). Only +0.876% different from 309's
#   481.149s/-37.052%, i.e. within noise of each other despite 309 having
#   rotate_interleave=1 and 310 having rotate_interleave=0.
# Conclusion (BROADMARK_VARIANT direction CLOSED): phase_mix=1 (cell/risk-
#   aware tail phase) is the sole root cause of the ~35-37% regression seen
#   in both 309 and 310, independent of rotate_interleave. variant=2
#   (rotate_only: phase_mix=0, rotate_interleave=1) remains the only
#   validated-safe non-baseline setting and is restored here. variant=3
#   (wide_only, no phase_mix) and variant=5 (wide_phase_rotate, has
#   phase_mix) are not pursued further given this result.
# Change: BROAD_MARKDIST_TAIL_VARIANT 1->2, A10G_FINAL_DEFAULT_BROADMARK_VARIANT
#   1->2 -- restores the exact 304/308 confirmed-safe configuration. K stays
#   48, kernel_dfs_iter_gpu_maxd14/16/18/20/21 bodies remain byte-identical to
#   304/308/309/310. Zero kernel code changes.
# Next priority (per handoff plan, unchanged by this restore): Stall Branch
#   Resolving isolation. 304's lightweight ncu profile (chunk0,
#   --launch-count 1) showed Stall Branch Resolving at 0.93 inst (~19.6%),
#   newly visible as the #2 factor after Stall Wait (44.1%), with the specific
#   branch not yet identified. Per the handoff memo's ncu lessons (a full
#   multi-section ncu run on a K-batched chunk multi-pass-replays and
#   effectively hangs), the recommended next command -- to be run manually,
#   NOT invoked by this script -- is a single-launch, SourceCounters-only
#   profile against this build:
#     ncu --launch-count 1 --set SourceCounters -o 311_sourcecounters_chunk0 \
#         ./311Py_variant2_restore -g 21 21 32 484 1 0 7 31 8 7 0 0 1 2
#   Once the offending branch is identified, decide whether a kernel change is
#   warranted; Stall Wait / dual-lane remains explicitly high-risk (293
#   footnote, 189 regression precedent) and should not be attempted until the
#   SourceCounters result is in hand.
...
#           from 304/308), BROADMARK_VARIANT=2 (rotate_only, restored).
#           Timing should reproduce 304/308's ~351s range, not 309/310's
#           ~477-481s regression.
...
SRC=${SRC:-./311Py_variant2_restore.py}
CAND=${CAND:-./311Py_variant2_restore}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/311Py_variant2_restore_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/311Py_variant2_restore_logs_N21_full_once_${TS}"
...
echo "[start] 311 variant2-restore validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 311 variant2-restore SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '311 variant2-restore' "$SRC"; then
  printf 'source_version_tag	311 variant2-restore	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	311 variant2-restore	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 311 variant2-restore mode31 split145"
...
echo "[validation-ok] 311 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, restored)"
