...
def main()->None:
  ...
  gpu_log_level:int=0
  gpu_sort_mode:int=-1
  bench_mode:int=0  # 0:normal, 1:N=20 same-process warmup then measured repeat
  # 68 WARMUP REPEAT: preset_queens は 5 固定。preset=6 は 48 で正解には直せたが速度面で不採用。
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 68 WARMUP REPEAT; using 5")
      preset_queens_arg=5
    if argc >= 10:
      bench_mode=int(sys.argv[9])
      if bench_mode!=0 and bench_mode!=1:
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        # 68 WARMUP REPEAT:
        #   N=20 単体実行が通し実行より遅い現象を切り分けるため、
        #   bench_mode=1 では同一プロセス内で N=20 を一度だけ warmup 実行し、
        #   直後の二回目だけを通常行として測定する。
        #   kernel/探索ロジックは変更しない。
