...
# 217 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 217 keeps the
# near-fastest 211 root_first-late form because 216 was correct but did not
# beat 204/211. 217 is the final root-preroll microdiff before moving to
...
# 217 delays cur_avail=root_rest until after the first-child nf/future/terminal/
...
#   - rootrestlate source check is whitespace/comment robust.
SRC=${SRC:-./217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 217 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 217Py source
#   has a harmless header/string/layout difference. This 217 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second XOR scalar if + late root_first + delayed cur_avail root_rest + delayed cur_avail=root_rest	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if all(checks.values()):
    sys.exit(0)
print('source_rootrestlate_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootrestlate\tpresent_root_rest_late_preroll\tpresent_root_rest_late_preroll\tOK
' >> "$SUMMARY"
else
  printf 'source_rootrestlate\tpresent_root_rest_late_preroll\tmissing_or_old_rootrestlate_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split217'}
bad=sorted(tags-allowed)
if 'split217' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split217 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split217 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 217 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 217 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootrestlate static checks failed" >&2
  exit 66
fi
