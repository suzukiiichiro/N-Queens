...
20:       39029188884              0         0:31:09.865    ok
...
def kernel_dfs_iter_gpu(
    ...
    # [CHG] 0/1テーブルはビットマスク化（タプル/配列参照を消す）
    ...
    while sp >= 0:
      ...
      if inited[sp] == u8(0):
        ...
        # [CHG] nfid: if/elif 直書き（この方が速いことが多い）
        if f == 0:      nfid = 1
        elif f == 1:    nfid = 2
        elif f == 2:    nfid = 3
        elif f == 3:    nfid = 3
        ...
        # if t_ism[f] == 1:
          ...
          if at_mark and a:
            ...
            stp = 3 if ((STP3_MASK >> f) & 1) else 2
            step[sp]  = stp
            # add1[sp]  = mark_add1[f]
            # add1[sp]  = MARK_ADD1[f]
            # add1[sp]  = t_add1[f]
            add1v = 1 if f == 20 else 0
            add1[sp]  = add1v
            rowst[sp] = row[sp] + step[sp]
            # ---- blockK/blockL 設定 ---- シェアードメモリ
            # bK[sp]    = blockK_tbl[f]
            # bK[sp]    = sK[f]
            # bK[sp]    = t_bK[f]
            # bL[sp]    = blockL_tbl[f]
            ...
            # bL[sp]    = t_bL[f]
            # ============================
            # [CHG] blockK_tbl[f] を廃止（mask → n3/n4 合成）
            # ============================
            # bk = 0
            # if ((MASK_K_N3 >> f) & 1) == 1:
            #   bk = n3
            ...
            bK[sp] = bk
            # ============================
            # [CHG] blockL_tbl[f] を廃止（0/1/2 をマスクで合成）
            # bL[sp] = blockL_tbl[f]   # blockL は一旦そのまま
            # ============================
