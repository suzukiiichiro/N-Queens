...
class NQueens21:
  ...
  def exec_solutions(self, constellations, N):
    ...
    def dfs(ld, rd, col, row, need, k, l, LD, RD, idx):
        if need == 0:
            return 1
        if row >= N:
            return 0
        ...
        if row == k or row == l:
            return dfs((ld << 1) & MASK, rd >> 1, col, row + 1, need, k, l, LD, RD, idx)

        # この行で禁止される列（LD/RD は行ごとに動的）
        forbid = (ld | rd | col | (LD >> (N - 1 - row)) | ((RD << (N - 1 - row)) & MASK)) & MASK
        avail = (~forbid) & MASK
        ...
        while avail:
            bit = avail & -avail
            avail ^= bit
            total += dfs(((ld | bit) << 1) & MASK, (rd | bit) >> 1, (col | bit) & MASK,
                         row + 1, need - 1, k, l, LD, RD, idx)
        return total

    for idx, c in enumerate(constellations):
        s = int(c.get("startijkl", 0))
        start = s >> 20
        ijkl  = s & ((1 << 20) - 1)
        j = self.getj(ijkl)
        k = self.getk(ijkl)
        ...
        ld  = int(c.get("ld", 0))  & MASK
        rd  = int(c.get("rd", 0))  & MASK
        col = int(c.get("col", 0)) & MASK

        placed = (lambda x: (x & -x) != 0 or 0)(0)  # ダミー抑止
        placed = 0
        tmp = col
        while tmp:
            tmp &= tmp - 1
        ...
        unique_cnt = dfs(ld, rd, col, start, need, k, l, LD, RD, idx)
