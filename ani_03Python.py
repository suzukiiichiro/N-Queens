...
class NQueens19():
  ...
  def symmetryops(self,size,aboard,topbit,endbit,sidemask,lastmask,bound1,bound2):
    count2 = count4 = count8 = 0  # スレッド専用のカウント変数
    if aboard[bound2]==1:
      own,ptn=1,2
  ...
  def backTrack2(self,size,row,left,down,right,aboard,topbit,endbit,sidemask,lastmask,bound1,bound2):
    ...
    if row==size-1:
      if bitmap and (bitmap&lastmask)==0:
        aboard[row]=bitmap
        c2, c4, c8 = self.symmetryops(size, aboard, topbit, endbit, sidemask, lastmask, bound1, bound2)
        count2 += c2
        count4 += c4
        count8 += c8
      return count2, count4, count8
  ...
  def solve(self,size):
    ...
    #
      ...
      results = list(executor.map(self.nqueen_multi, value))
