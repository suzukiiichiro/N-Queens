...
void NQueen(int size){
  ...
  int row=0;
    ...
    printf("pd:%d:row:%d\n",p->bitmap,row);
        ...
        int n=(!(size & 1)||d->bitmap);
        printf("si:%d:n:%d:pb:%d:db:%d\n",!(size&1),n,p->bitmap,d->bitmap);
			  TOTAL+=1 + n;
      }
		}
		else if (--p<d) return;
...
//再帰
void NQueenR(int size,int row,int left, int down, int right,int mask) {
	int bit,bitmap,dbitmap;
	int sizeE=size-1;
	for(dbitmap=bitmap=~((left<<=1)|down|(right>>=1))&mask;bitmap;bitmap&=~bit){
