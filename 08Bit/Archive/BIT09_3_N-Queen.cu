/**
 ・BackTrack1,2の内部でやっている枝刈りを前に持ってこれるか？
 ・前でsymmetryOpsをやることによってBackTrack1,2の枝刈りは不要になっていないか？
 ・prepareメソッドの配列は有限。
...
9-3．n27対称解除(前)+n27枝刈り+backTrack1+backTrack2+ビット(n27)
   上下左右２行にクイーンを配置したのち（ビット(n27)）対称解除で解を求めます。
   n27の枝刈り,backTrack1,backTrack2を追加
