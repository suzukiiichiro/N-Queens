...
void NQueenR(int size)
{
  ...
  //for(int w=0;w<size*size;w++){
    ...
    Board nB=B;
    //int lsize=(size-2)*(size-1)-w;//8
    for(int n=0;n<idx;n++){//8
      //左２列に置く
      B=nB;
      //1行目にクイーンが来るときはスキップする
      if(pres_a[n]==0){continue;}//8 
      if(pres_b[n]==0){continue;} //8
...
int main(int argc,char** argv)
{
  ...
  /** 出力と実行 */
  if(cpu){
    printf("\n\n８．CPU 非再帰 バックトラック＋ビットマップ＋対称解除法\n");
  }else if(cpur){
    printf("\n\n８．CPUR 動的分割統治法\n");
  }else if(gpu){
    printf("\n\n８．GPU 非再帰 バックトラック＋ビットマップ＋対称解除法\n");
  }else if(sgpu){
