 ...
 *
 * rev397 -- FIRST kernel change since 395a r2. The DFS stack frame is
 * repacked from 16 bytes to 12; nothing else in the algorithm moves.
 *
 * WHAT 396 ESTABLISHED
 *   ptxas: 208 bytes stack frame, 0 spill stores, 0 spill loads, 37
 *   registers. So the frame is not spilled registers -- it is the declared
 *   array uint64_t stack[MAXD14_ANCESTOR*2], and it lives in local memory
 *   because it is indexed dynamically.
 *   Occupancy on the real N=21 run: theoretical 33.33% (BLOCK=32 gives one
 *   warp per block against a 16-block-per-SM limit; registers would allow
 *   48 blocks, so registers are NOT the cap), achieved 10.32%, 4.95 warps
 *   per SM. Scheduler Statistics: No Eligible 61.63%, 2.41 active warps per
 *   scheduler out of 12, 0.38 issued warps per scheduler.
 *   Speed of Light, on a one-round input (NOT representative -- see below):
 *   L1/TEX 52.96%, L2 46.73%, DRAM 16.09%, Compute (SM) 19.41%. The memory
 *   pipe runs 2.7x the compute pipe.
 *
 * WHAT 397 TESTS -- one hypothesis, cleanly
 *   Two explanations survive for that L1 pressure:
 *     (a) VOLUME. Local-memory bytes moved per push and pop.
 *     (b) DIVERGENCE. Threads sit at different stack depths, so the
 *         addresses in a warp scatter and each request splits into many
 *         sectors regardless of how few bytes are live.
 *   397 attacks (a) alone. It cuts bytes per frame by 25% while keeping the
 *   number of local accesses at exactly two per push and two per pop, so
 *   request count and divergence are unchanged. If the run gets faster in
 *   proportion, volume is what costs. If nothing moves, (a) is refuted and
 *   the next revision goes after (b).
 *
 * WHAT IS NOT CLAIMED
 *   The Speed of Light numbers above come from the one-round proxy that 396
 *   itself rejected as unrepresentative (a single round measures the maximum
 *   record cost across threads, not the mix; see the 396-r2 notes). They are
 *   the reason to suspect local memory, not proof of it. 397 is therefore an
 *   experiment whose value is the same either way it lands.
 *   The 21-bit packing also caps N at 21, so this exact form does not carry
 *   forward to maxd16 for N=22/23 even if it wins.
 *
...
/* 397: field width of the repacked DFS stack. Three fields of 21 bits fill
 * 63 of the 64 bits of stack_a, so N must not exceed 21. Bits at or above
 * 2^21 of ld and rd are dropped on push: every use site masks with the board
 * mask, so for N <= 21 those bits are dead. That reasoning is checked, not
 * trusted -- the harness compares the per-record output of 396-r2 and 397
 * byte for byte over all 2,025,282 records before any timing is read. */
#define MAXD14_PACK_BITS 21
#define MAXD14_PACK_MASK ((uint32_t)((1u << MAXD14_PACK_BITS) - 1u))
...
static uint64_t process_one_task(
    ...
    /* 397: the DFS stack, repacked. 396 measured 208 bytes of stack frame
     * (uint64_t stack[26], two words per frame) with zero spills and 37
     * registers -- so the frame is not register pressure, it is this array.
     * Each frame carried ld, rd, col, avail in 128 bits while holding only
     * 84 bits of information for N<=21 (21 bits each, depth already riding
     * in avail's spare bits). 397 packs ld|rd|col into one uint64_t at 21
     * bits each and leaves avail|depth in a uint32_t: 12 bytes per frame
     * instead of 16, and still exactly TWO local-memory accesses per push
     * and per pop. Holding the access COUNT fixed is the whole point -- it
     * makes this a clean test of whether local-memory BYTE VOLUME is what
     * costs, separate from request count and divergence.
     *   208 -> 156 bytes, sectors per push/pop 16 -> 12.
     * LIMIT: three 21-bit fields fill 63 of 64 bits, so this packing is
     * valid only for N <= 21. main() refuses larger N. maxd16 for N=22/23
     * will need a different layout; this revision is an experiment, not a
     * form that carries forward unchanged. */
    uint64_t stack_a[MAXD14_ANCESTOR];   /* ld | rd<<21 | col<<42 */
    uint32_t stack_b[MAXD14_ANCESTOR];   /* avail | depth<<27, unchanged */
...
#endif
                stack_a[stack_ptr] = ((uint64_t)(cur_ld  & MAXD14_PACK_MASK))
                                   | ((uint64_t)(cur_rd  & MAXD14_PACK_MASK) << 21)
                                   | ((uint64_t)(cur_col & MAXD14_PACK_MASK) << 42);
                stack_b[stack_ptr] = cur_avail | ((uint32_t)cur_depth << 27);
                stack_ptr += 1;
                save_sp   += 1u;
            }
            cur_ld = pr_nld;
    ...
    for (;;) {
        ...
        if (cur_avail == 0u) {
            ...
            uint32_t saved_avail = stack_b[stack_ptr];
            cur_ld  = (uint32_t)( packed_a        & MAXD14_PACK_MASK);
            cur_rd  = (uint32_t)((packed_a >> 21) & MAXD14_PACK_MASK);
            cur_col = (uint32_t)((packed_a >> 42) & MAXD14_PACK_MASK);
...
int main(int argc, char **argv) {
    ...
    if (N > (int64_t)MAXD14_PACK_BITS) {
        fprintf(stderr, "[ERROR] 397: the repacked DFS stack holds three %d-bit fields in one "
                        "uint64_t, so N must be <= %d (got %lld). Use 396_r2_kernel_maxd14 for "
                        "larger N, or redesign the packing.\n",
                (int)MAXD14_PACK_BITS, (int)MAXD14_PACK_BITS, (long long)N);
        return 1;
    }
