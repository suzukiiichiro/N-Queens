...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 398-r2 (2026-09-09)
...
"""
W1は成立しています。アンカー `133,197.422`、基準133,185から **+0.009%**、`extra_ctx=2`、`free_mb=21762`、MATCH。**ディスパッチャなしで本番状態を再現できることが確定しました。**

N=20 と N=19 も正しい値で完走しています（39,029,188,884 / 4,968,057,848、ともに `ok`）。**しかしCRunnerログが見つからず、そこで停止しました。** 理由をソースで確認します。`-g 20 20` がCRunnerを通らない理由が分かりました。ディスパッチ条件にNの下限があります。

```python
if use_gpu and N>=21 and bench_mode==37:
```

**CRunner経路は `N >= 21` でゲートされています。** N=20とN=19はCodon側のGPU経路を通ったので、CRunnerログもスケジュール済み入力も作られませんでした（N=20が42.977秒かかっているのも別経路だからです）。私の設計漏れです。

回避策を決めるために、入力生成の実体を確認します。原因が特定できました。入力生成の4段のうち、**前半2段がCodonバイナリの内側**にあります。

```
1. ensure_constellations_bin_stream(N)      ← Codon内
2. dump_soa_reference_c_port(N, …)          ← Codon内
3. python3 363_filter_maxd14_only.py …      ← 外部
4. python3 394f_permute_soa7.py sched …     ← 外部
```

外部スクリプトだけではN=20の入力を作れません。**ディスパッチの下限を下げるのが唯一まっすぐな道**です。398-r2として、その1行を変えます。398-r2、四点セットです。静的ゲートは **OK=19 FAIL=0**。

## 398で得られたもの

**W1は成立しました。** アンカー `133,197.422`、基準から **+0.009%**。397で見つけたV4——`NQ_EXTRA_CTX=2` の直接実行で本番状態を再現できる——が実運用で確認できました。以後の測定はディスパッチャ1回ぶん短くなります。

N=20 と N=19 も正しい値で完走しています。止まったのは**その次**でした。

## 変更は実行行1つ

```python
CRUNNER_MIN_N:int=19
if use_gpu and N>=CRUNNER_MIN_N and bench_mode==37:   # ← ここだけ
```

**mode-37のゲートだけ**を下げます。他の9つの `N>=21` ゲート（mode 30/31、32、33、34、35、36、38、39、末尾のガード）は触りません。**ハーネスがその9つが残っていることを静的に数えます。**

安全性は自己検査です。N=19/20 の `required_maxd` は14以下なので同じ表の行に解決し、CRunnerは**Codon経路が既に `ok` を出したのと同じオラクル表**と突き合わせます。`required_maxd>14` なら既存の `maxd-unsupported` 経路に乗ります。

Py差分は **removed=4 added=5**、`.cu` は398のリネームでカーネル領域は `ebd7f523…` のままです。

## W6の重みが上がりました

準備中に気づいた点です。**N=21 の `k_per_thread` は 79.1 ですが、N=20 はレコード数が約1/8なので 10前後に落ちます。** スレッド間の負荷平均化はN=21より弱い。

つまり「N=20が代表である」ことは**仮定できません**。幸いN=21では Occupancy と Scheduler Statistics を396で実測済みなので、W6でそれと直接突き合わせます。

**達成オカパンシー 10.32%±3ポイント、No Eligible 61.63%±5ポイント。外れたらN=20は代理として使えず、N=21を夜間に回す判断になります。** 形式的な確認ではなく、結論が使えるかどうかを決める検証です。

## 失敗メッセージも直しました

CRunnerログが出なかった場合、`dispatch.log` の `[crunner-unsupported]` を見るよう具体的に案内します。今回の私の見落としが、次は3秒で判別できるようにしました。
...
N=22は既に**ゲートの上限には掛かっていません**（`N>=CRUNNER_MIN_N` に上限はなく、`required_maxd=14` なら同じ表の行に解決します）。ただ、実際に通るかは別問題なので確認します。`expected[22] = 2,691,008,701,644` は表にあり、ゲートに上限もありません。**N=22 は原理的には既に通ります。** 実際に走らせる価値はあります。ただ、profilingの本体と一緒にするには重すぎます。

## N=22の実際のコスト

解の個数比から `2,691,008,701,644 / 314,666,222,712 = 8.55`。**カーネルだけで約19分**（133.2秒 × 8.55 ≈ 1,139秒）。加えてレコード数が12〜17M規模になるので、361 dump → 363 filter → 394f sched の入力生成がPython側で相当かかります。rev372の記述にある「12.2M〜13.0Mレコード規模の失敗ブラケット」もこの帯です（g5.2xlarge移行後に366で3/3確認済み、とはいえ）。

**35分のプロファイリングと同居させると、権限やタイムアウトの問題が出たときに全部巻き添えになります。** オプトインの独立セルにします。

## そしてN=22には、予測できる副作用があります

395cで確定した「速い窓」は **幅128 MiB未満の切り欠き**で、`free_mb = 21762` に居ることで413 msを得ています。

N=22ではCRunner自身のデバイス確保が 57 MB から **約460 MB** へ増えます（16M記録 × 28 B）。`free_mb` は 21762 から **21300前後**へ落ちる。**これは窓の外です。**

> **r6で採用した413 msのチューニングは、N=21固有の値であってN=22には引き継がれません。** `NQ_EXTRA_CTX=1` が最適である保証もなくなります。

これは395cの知見から導ける、検証可能な予測です。N=22セルで `free_mb` を記録すれば、その場で確かめられます。

ハーネスにオプトインで足します。`.sh` と README を差し替えてください。`.py` と `.cu` は先ほどのままで変更ありません。静的ゲートは **OK=19 FAIL=0** のままです。

## 追加した内容

`WITH_N22=1` でオプトインの独立セルが走ります。**プロファイリング本体がすべて終わった後、最後に**実行されるので、N=22で何かあっても35分の測定を巻き添えにしません。

セルが記録するもの。

- レコード数、`kernel_ms`、`free_mb`、`extra_ctx`、壁時計
- オラクル照合（`2,691,008,701,644`）
- **N=21比のスケーリング係数**（解の個数比8.55倍に対して実際どうか）
- **W7判定**: `free_mb < 21700` なら「予測どおり窓の外」と明示

失敗時のメッセージも具体的にしました。CRunnerログが出なければ `dispatch.log` の `[crunner-unsupported]` と、入力生成の失敗（rev369-372が大規模確保の失敗を見た12〜17Mレコード帯）の両方を見るよう案内します。

## 一点、期待値を下げておきたいこと

N=22が動くこと自体は確認できますが、**395cで得た413 msはN=22には持ち越せない見込みです。**

窓の幅が128 MiB未満で、N=22ではCRunner自身の確保が57 MB → 約460 MBへ増えるためです。`NQ_EXTRA_CTX=1` が最適である保証もなくなります。

N=22用の最適点探索は1点あたり19分かかるので、本リビジョンの範囲外にしました。**N=22に本腰を入れる段階で、別リビジョンとして組むのが筋**だと思います。そのときは今日確立した測定基盤——占有ゲート、`NQ_EXTRA_CTX=2` の直接実行アンカー、0.01%のノイズフロア——がそのまま使えます。

```
bash 398_r2_validate.sh              # 本体のみ、約35分
WITH_N22=1 bash 398_r2_validate.sh   # + N=22セル（入力生成込みで30〜50分）
```

まずは本体だけ走らせて、W3〜W6の結果を見てからN=22に進むほうが安全だと思いますが、時間が許すなら一度に流していただいても構いません。


W2は両方成立です。予測帯（N=20: 14,000〜20,000 / N=19: 1,500〜3,000）にどちらも収まりました。

| | records | kernel_ms | k_per_thread |
|---|---:|---:|---:|
| N=21 | 2,025,282 | 133,197 | 79.1 |
| **N=20** | 1,369,061 | 16,413.852 | **53.5** |
| N=19 | 897,071 | 2,121.613 | 35.0 |

`CRUNNER_MIN_N` の変更は効いています。両方ともCRunner経路を通り、スケジュール済み入力も作られました。

**そして私の懸念が1つ外れて、良い方向でした。** 私はN=20のレコード数がN=21の約1/8（k≈10）に落ちると見積もっていましたが、実際は **0.676倍で k=53.5** です。レコード数は解の個数ではなく constellation 数に比例するので、私の見積もりが誤りでした。**79.1に対して53.5なら、N=20はかなり良い代理です。** W6の見通しは当初より明るくなりました。

## 待って大丈夫です

プロファイル段階は予算で頭打ちになっています。

- セクションごと `timeout 600秒`
- 全体予算 `NCU_BUDGET_S=1800秒`（N=20とN=19で**共有**）

予算を超えた時点で新しいパスは起動せず `budget_exhausted` を出してスキップします。したがって**最悪でもプロファイル全体で約40分**（予算1800秒＋境界で始まった1パスの600秒）。そこで必ず抜けて順位表の生成に進みます。**無限には走りません。**

N=19が遅く見えるのは、N=20の4パスが予算内で終わっていることの裏返しでもあります（N=20が食い尽くしていれば、N=19は即スキップされて速く見えるはずです）。

## 終わったら見るところ

`6_ranked.txt` にすべて出ます。優先順位はこうです。

1. **W3** — N=20の支配的ストール理由。**この案件で一度も出ていない答え**です
2. **W4** — ローカルメモリの sectors/request。私は ≥2（発散支配）に賭けています
3. **W6** — N=20の達成オカパンシーとNo Eligibleが、N=21の 10.32% / 61.63% に対してどうか。k=53.5なら通る見込みが上がりました
4. **W5** — N=19とN=20で上位3件の順序が一致するか

タールボールをお待ちしています。

W4が決着しました。**しかもN=19とN=20で一致しています。** そしてW3は取れているのに私のパーサが取り逃しています。生データを読みます。398-r2、**大きな収穫がありました。** ただし最初に、私の事前登録の1つが**校正を誤っていた**ことを訂正させてください。

## 訂正 — W4の閾値設定が誤っていました

`sectors/request = 4.69` を「≥2なら発散支配」と登録しましたが、**この閾値は無意味でした**。スタックは `uint64_t` で、32レーンが同じ添字を触る完全合体アクセスでも 32×8 = 256 B = **8セクタ**です。1ではありません。

したがって 4.69 という数字だけでは発散を証明できません。**発散の証拠は別のところにありました。**

## 決定的な数字

```
Avg. Active Threads Per Warp = 10.53 / 32   (N=19では 10.25)
```

**ワープ32レーンのうち、平均10.5レーンしか生きていません。33%です。** これは曖昧さのない発散の直接測定で、N=19とN=20で一致しています。
...
| 稼働レーン内のセクタ効率 | 84.2 B有効 / 150.1 B転送 = **56.1%** |
| **ローカルメモリの実効効率** | **18.5%** |

1リクエストあたり150バイトを動かして、有効なのは84バイト。しかもそのリクエスト自体が3回に2回は空きレーンを含む。

## そして(a)と(b)は二者択一ではありませんでした

N=20の SpeedOfLight です。**今度は棄却した1ラウンド代理ではなく、実物のスケジュールを持つ負荷での値**です。

```
L1/TEX Cache Throughput  70.64%   ← 全パイプ中の最大
Memory Throughput        62.25%
Compute (SM) Throughput  31.55%
DRAM Throughput          25.22%
```

**L1が70.6%で律速、演算の2.24倍。** ローカルメモリのセクタ転送は 33.7 TB / 16.4秒 ≈ **2.05 TB/s**。1レコードあたり16.4万回のローカルアクセス。

つまり**(a)量も(b)発散も、両方が同じ壁の別の顔**でした。私が二者択一に立てたのが誤りです。フレームを25%縮めればセクタ数もほぼ比例して減る——**397の着想は正しく、実装だけが無効だった**ということになります。

## Scheduler — N=21と一致

```
                        N=20      N=21(396実測)
No Eligible            60.53%    61.63%
Active Warps/Scheduler   2.43      2.41
Issued Warp/Scheduler    0.39      0.38
```

**スケジューラの絵はN非依存です。** Little's law も整合します（2.43 ÷ 6.14 warp-cycles/issue = 0.396 ≈ 実測0.39）。

## W6 — 半分成立、半分失敗

| | N=20 | N=21 | 判定 |
|---|---:|---:|---|
| No Eligible | 60.53% | 61.63% | ✓ ±5pt内 |
| Achieved Occupancy | **19.33%** | 10.32% | ✗ +9.01pt |

オカパンシーだけが外れました。理由は明確で、**テール長の違い**です。N=21は `k_per_thread=79`、N=20は53.5。長く走るほど早く終わったワープが増え、時間平均のオカパンシーが下がります。

**読み方**: 1アクセスあたり・1ワープあたりの比率（セクタ効率、レーン稼働率、スケジューラ圧）はN=19とN=20で一致しており、**N=21へ持ち越して差し支えない**。一方でオカパンシーとテールの数字は**持ち越せない**。

## W3はまだ取れていません

`WarpStateStats` セクションは完走しましたが、ncuが出したのは要約4項目だけで、**ストール理由の内訳が含まれていません**。`--metrics smsp__average_warps_issue_stalled_*` を明示的に要求する必要があります。ハーネス側の不備です。N=20で1パス追加すれば5分ほどで取れます。

## 399で狙うべきもの

**レーン稼働率33%が、今日見つかった中で圧倒的に大きい余地です。** これまで追ってきた0.3%とは桁が違います。

そして狙い方に、私は有望な線があると思っています。**394fはスケジュール全体でスレッドごとの累積負荷を揃えていますが、「同じワープの32レーンが似た形の仕事をする」ようには並べていません。** スレッド t はレコード t, t+25600, … を処理するので、ワープ0のレーン0〜31はレコード0〜31を担当します。**どのレコードが同じ32個組に入るかは、394fの並べ方が決めています。**

つまり **レーン稼働率はホスト側のスケジューリングで動かせる可能性がある**——カーネルに触らずに。394fのツールは既にあります。

399の提案は3点です。

1. **ストール理由を明示メトリクスで取る**（5分、W3の宿題を片付ける）
2. **32個組内の同質性を指標化する**（探索深さ・分岐形状の分散を394fの出力から計算。GPU不要）
3. **その指標を改善する並べ替えを1つ試し、レーン稼働率とkernel_msの両方で評価する**

2はGPU時間ゼロで、3の見込みが立つかどうかを先に判定できます。**今度は測ってから作ります。**

399の四点セットを書き起こしましょうか。それとも先に、今日の結果をREADMEとOpen Objectivesへ反映させますか。
...
VERSION_TAG:str="398-r2 PY-REVTAG: 398Py plus ONE dispatch change -- the mode-37 CRunner path was gated on N>=21, so -g 20 20 and -g 19 19 never built a scheduled input or ran the CRunner, and 398 had nothing to profile. r2 introduces CRUNNER_MIN_N and sets it to 19. The table names ./398_r2_kernel_maxd14 and keeps the env_prefix NQ_EXTRA_CTX=1 from 395c-r6; the .cu is a rename with the kernel region still at the 395a r2 sha256 ebd7f523. Correctness at the new N values is self-checking: the CRunner compares against the same oracle table the Codon path used, which already reported ok for both."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./398_r2_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
