...
VERSION_TAG:str="366 maxd_check: ADDITIVE REVISION. Executable code region is byte-identical to 365 outside five clearly delimited deltas (two pure insertions, two 2-word CLI-gate modifications matching the 361/365 pattern, one pure insertion for the dispatch branch), verified by a static check that strips/reverses those spans before hashing against 365s reference hash for the identical region. The deltas are: (1) ===366-INSERT-BEGIN/END=== -- new function check_required_maxd_for_N(), a read-only diagnostic composition of existing unmodified functions (ensure_constellations_bin_stream, build_soa_for_range, max_schedule_depth_of_tasks, select_static_maxd) that reports required_maxd/selected_maxd for a given N without uploading any SoA array to a GPU or launching any kernel; (2)-(3) ===366-CLIGATE-COMMENT-BEGIN/END=== and ===366-PRESETGATE-COMMENT-BEGIN/END=== -- comments plus a 2-word addition (or bench_mode==34) to the same two CLI gate lines 361 and 365 modified, applied preemptively this time, same as 365 did for bench_mode==33; (4) ===366-DISPATCH-INSERT-BEGIN/END=== -- new bench_mode==34 dispatch branch, appended after the existing bench_mode==33 block. Context: Suzuki compared this projects N=21 result against a 2023-11-22 benchmark from a different implementation (symmetry-elimination GPU bitboard) that computes both Total and Unique; the Total value (314666222712) matched this projects oracle exactly, but since that other implementation also computes Unique (which this projects kernel does not, using symmetrys weight multiplier instead), a direct timing comparison was judged unfair and Unique was agreed out of scope. Suzuki then asked whether N=22-26 could be attempted, correctly anticipating that required_maxd likely exceeds 14 there; kernel_dfs_iter_gpu_maxd14 (361-365) only covers required_maxd<=14, and porting kernel_dfs_iter_gpu_maxd16/18/20/21 to C would be substantial additional work. 366 answers the cheaper prior question first: what is the actual required_maxd for N=22 onward, before committing to any further kernel-porting effort. This revision has no GPU available in-session; Suzukis next action is running this for N=22 first (single-variable discipline), with N=23-26 to follow based on that result."
