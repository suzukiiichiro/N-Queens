 ...
 * different backend).
 *
 * 374 RENAME NOTE: this file is a pure rename of 364_kernel_maxd14.cu to
 * 374Py_kernel_maxd14.cu, done purely so every deliverable in the 374
 * consolidation package shares one revision number and is easy to find
 * together. Every line of actual C/CUDA code below (process_one_task(),
 * the __global__ kernel, the CPU test main(), and the 364 GPU host
 * runner main()) is BYTE-IDENTICAL to 364_kernel_maxd14.cu -- only
 * filename self-references in these header comments changed.
 *
 * Build (device, on cudacodon):
 *   /usr/local/cuda/bin/nvcc -O3 -arch=sm_86 -o 374Py_kernel_maxd14 374Py_kernel_maxd14.cu
 * Run (device):
 *   ./374Py_kernel_maxd14 <N> <in_soa7_bin> <out_results_bin> [expected_total]
 *   e.g. ./374Py_kernel_maxd14 21 constellations_N21_6.bin.soa_ref_361.bin.maxd14only_363.bin \
 *          /tmp/gpu_results.bin 314666222712
 * Build (host-only CPU test, no CUDA toolkit needed, this sandbox):
 *   gcc -O2 -Wall -Wextra -o 374Py_kernel_maxd14_cputest 374Py_kernel_maxd14.cu -x c -lm
 * Build (host-only CPU test, OpenMP-parallelized outer loop):
 *   gcc -O2 -fopenmp -Wall -Wextra -o 374Py_kernel_maxd14_cputest_omp 374Py_kernel_maxd14.cu -x c -lm
 *
