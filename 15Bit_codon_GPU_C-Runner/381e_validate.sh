...
# 381a_validate.sh
#
# rev381e — First real GPU test of the seed_kernel + hybrid_kernel
# two-stage design (380 spec). K_THRESHOLD fixed at 13 (see file
# header of 381e_hybrid_kernel_localtotal.cu): this run's only job is
...
set -u
CUSRC="${CUSRC:-381e_hybrid_kernel_localtotal.cu}"
BIN="${BIN:-381e_hybrid_kernel_localtotal}"
...
echo "381e PASSED: run_hybrid_episode now accumulates locally and does ONE"
echo "atomicAdd per episode (not per terminal hit) -- directly testing"
echo "Suzuki-san's hypothesis after ncu's per-instruction data proved too"
echo "ambiguous to trust for this stateful kernel. Compare kernel_ms above"
echo "to 374 (~201,232ms), 381d (265,403ms, the previous best), 381c"
echo "(569,157ms), 381b (613,041ms), 381a (285,841ms)."
exit 0
