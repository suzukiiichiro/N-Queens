/*
 * 377c_frame_kernel.cu
 *
 * rev377c — Same frame-queue DFS engine as 377b (process_one_task_
 * reference / TaskSchedule / Frame / FQSlot / fq_push / fq_pop /
 * seed_task / step_one_frame: ALL byte-identical to 377b, copied
 * verbatim, not re-derived). The ONLY change in this file is in
 * main(): synthetic random data generation is replaced with reading
 * REAL data from a SoA7 binary file (7 x u32 LE per record: ld, rd,
 * col, ctrl0, free, markctrl, w_lo -- the same layout
 * 363_filter_maxd14_only.py produces and 363_kernel_reference_sim.py/
 * 374Py_kernel_maxd14.cu's CPU test harness consume). If no file is
 * given, falls back to 377b's synthetic generator (self-test mode,
 * useful when no real data is at hand).
 *
 * Since real N=21 data already satisfies the maxd<=14 precondition
 * (363's finding: 0 records excluded when filtered), the n_skipped
 * counter here should read 0 (or very close to it) when run against
 * real data -- if it doesn't, that itself is a meaningful finding,
 * not just noise to route around.
 *
 * Usage:
 *   ./377c_frame_kernel_test <n_threads> <soa7_file> [N=21] [max_records]
 *   ./377c_frame_kernel_test <n_threads> --synthetic [n_tasks]   (377b-equivalent self-test)
 */
...
int main(int argc, char** argv) {
    ...
    if (strcmp(argv[2], "--synthetic") == 0) {
        ...
        printf("[377c] mode=synthetic n_tasks=%ld N=%d\n", n_tasks, N);
    } else {
        ...
        if (!fin) {
            fprintf(stderr, "[377c] FAIL: could not open %s\n", path);
            return 1;
        }
        ...
        if (fsize % 28 != 0) {
            fprintf(stderr, "[377c] FAIL: %s size %ld is not a multiple of 28 "
        ...
        for (long i = 0; i < n_tasks; i++) {
            ...
            if (fread(rec, sizeof(uint32_t), 7, fin) != 7) {
                fprintf(stderr, "[377c] FAIL: short read at record %ld in %s\n", i, path);
                fclose(fin);
                return 1;
            }
        ...
        fclose(fin);
        printf("[377c] mode=real_data file=%s N=%d records_in_file=%ld records_used=%ld\n",
...
#endif
    fprintf(stderr, "[377c] starting reference computation: n_tasks=%ld ref_threads=%d ...\n", n_tasks, ref_threads);
    fflush(stderr);
#ifdef _OPENMP
    ...
    fprintf(stderr, "[377c] reference computation done: n_skipped=%ld/%ld\n", n_skipped, n_tasks);
    fflush(stderr);
    ...
    if (!qbuf) {
        fprintf(stderr, "[377c] FAIL: malloc failed for queue buffer (capacity=%u, %.1f MB requested)\n",
    ...
    fprintf(stderr, "[377c] starting seed_task for %ld tasks (capacity=%u slots, %.1f GB)...\n",
    ...
    if (push_overflow) {
        fprintf(stderr, "[377c] FAIL: queue overflow during seeding (capacity=%u too small)\n", capacity);
        return 1;
    }
    fprintf(stderr, "[377c] seeding done: %llu frames pushed so far. Starting %d worker(s)...\n",
            (unsigned long long)q.enqueue_pos, n_threads);
    fflush(stderr);
        ...
        for (;;) {
            ...
            if (fq_pop(&q, &f)) {
                ...
                if (pc > 0 && pc % 2000000 == 0) {
                    fprintf(stderr, "[377c] progress: %ld frames processed so far (enqueue_pos=%llu, "
    ...
    fprintf(stderr, "[377c] worker loop done: %llu total frames processed.\n",
            (unsigned long long)fq_atomic_load_u64(&q.dequeue_pos));
    fflush(stderr);

    if (push_overflow) {
        fprintf(stderr, "[377c] DIAG: final enqueue_pos=%llu dequeue_pos=%llu capacity=%u\n",
                (unsigned long long)q.enqueue_pos, (unsigned long long)q.dequeue_pos, capacity);
        fprintf(stderr, "[377c] FAIL: queue overflow during frame processing (capacity=%u too small for n_tasks=%ld)\n",
                capacity, n_tasks);
        return 1;
    }
    ...
    for (long i = 0; i < n_tasks; i++) {
        ...
        if (ref_results[i] != new_results[i]) {
            mismatches++;
            if (mismatches <= 10) {
                fprintf(stderr, "[377c] MISMATCH task_id=%ld ref=%llu new=%llu "
    ...
    printf("[377c] n_tasks=%ld n_skipped=%ld n_threads=%d ref_sum=%llu new_sum=%llu mismatches=%ld "
           "total_frames=%llu avg_frames_per_task=%.1f capacity=%u\n",
    ...
    if (n_skipped > 0) {
        fprintf(stderr, "[377c] WARN: %ld/%ld tasks skipped (schedule precompute cap hit). "
    ...
    if (mismatches != 0) {
        printf("===== 377c: FAIL (%ld/%ld task mismatches) =====\n", mismatches, n_tasks - n_skipped);
        return 1;
    }
    printf("===== 377c: PASS (all %ld valid tasks byte-identical to process_one_task_reference, %ld skipped) =====\n",
