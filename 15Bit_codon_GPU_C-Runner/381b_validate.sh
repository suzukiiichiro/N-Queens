...
# 381a_validate.sh
#
# rev381b — First real GPU test of the seed_kernel + hybrid_kernel
# two-stage design (380 spec). K_THRESHOLD fixed at 13 (see file
# header of 381b_hybrid_kernel_unified.cu): this run's only job is
...
set -u
CUSRC="${CUSRC:-381b_hybrid_kernel_unified.cu}"
BIN="${BIN:-381b_hybrid_kernel_unified}"
...
echo "381b PASSED: single unified kernel_dfs_hybrid_maxd14 (K_THRESHOLD=13,"
echo "static grid-stride task acquisition, shared queue touched only on"
echo "genuine overflow) reproduces 374's own oracle on real N=21 data."
echo "Compare kernel_ms above to 374's own kernel_ms (~201,232ms) AND to"
echo "381a's kernel_ms (285,841ms + seed_ms 13,259ms) -- this run should land"
echo "much closer to 374 than 381a did, confirming the seed_kernel was the"
echo "source of 381a's regression, not the hybrid design itself."
echo "381c+: sweep K_THRESHOLD downward on real GPU hardware for actual speed data."
exit 0
