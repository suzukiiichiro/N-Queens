...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 363 (2026-08-27)
...
6. [進行中・363で.cu本体を実装、実機nvccビルドは未実施] CUDA Cランナー
   (案C-2)スパイク設計
   課題3(tail effect)への**唯一の残存対策経路**。方針は334以来不変。
   334-337で土台確立、338→360で移植仕様確定、361でSoA導出のCポート
   が実機確定(2,025,282件バイト単位一致)、362でカーネル本体の移植
   設計(`362_kernel_port_spec.md`)を確定。

   **363で`kernel_dfs_iter_gpu_maxd14`本体を`363_kernel_maxd14.cu`
   として実装した。** `__host__ __device__`(nvcc未使用時は空展開)で
   per-task本体を`process_one_task()`として分離し、実カーネル
   (`__global__`)とCPU専用テストハーネス(`main()`、`#ifndef
   __CUDACC__`)の両方から同一ロジックを呼ぶ単一ソース構成とした——
   GPUに送られるコードとテストされるコードが同一であることを構造的に
   保証する。

   **セッション内で二段階の交差検証を実施(いずれもgcc、GPU不要)**:
   (1) 360Pyから実カーネルソースを一字一句抽出しCPython上で再実行する
   シミュレータを構築、合成データ(N=21制約を満たす20,003件+
   200,003件、root_action 1/2/3・root高速パス1/2候補の境界ケースを
   意図的に注入)でCポートと突き合わせ、**全件バイト単位完全一致**
   (この過程で発見したバグ: `process_one_task`が引数で受け取った
   `meta_next`ポインタを使わずグローバル定数を参照していた実装ミス
   を修正済み)。(2) 361の実機ダンプ(N=21実データ2,025,282件、
   `constellations_N21_6.bin.soa_ref_361.bin`)から必要7フィールド
   (ld/rd/col/ctrl0/free/markctrl/w_lo)を新規変換ツール
   `363_extract_kernel_input_from_361_dump.py`で抽出し、CPU専用ビルド
   の`363_kernel_maxd14.cu`に通すことで、**新規Codonコード変更ゼロで
   実データによる正当性検証(total_sum==314666222712判定)が可能**
   という設計にした——GPU実行を待たずにCPU上で本物の正解値との照合が
   できる。

   358・359のリスクノート(pushガード`if cur_avail!=u32(0):`の制御
   フロー形状はCodonバックエンドで極めて敏感)は363でも厳守し、完全に
   1:1の直訳のまま実装、形状変更は一切行っていない。

   **363自体はまだ実機での結果待ち**(CPU専用ビルドでのtotal_sum
   確認、その後nvcc実機ビルド+GPU実行+タイミング比較)。実機nvcc
   ビルド・GPU実行・±3%タイミング判定は364以降。
...
363を用意しました(`kernel_dfs_iter_gpu_maxd14`本体のCUDA C実装、
362 specに基づく初回実装)。363Py自体はコード変更ゼロです(362と
バイト単位で完全一致、362と同じ規律)。

**成果物は3点**:
1. `363_kernel_maxd14.cu`: カーネル本体。per-task本体を`__host__
   __device__`関数`process_one_task()`として分離し、実カーネル
   (`__global__`、nvccビルド時のみ)とCPU専用テストハーネス
   (`main()`、`#ifndef __CUDACC__`、gccでビルド可)の両方から同一
   ロジックを呼ぶ構成。GPUに送るコードとテストするコードが同一
   ソースであることを構造的に保証しています。
2. `363_extract_kernel_input_from_361_dump.py`: 361の実機ダンプ
   (N=21実データ全件)から、カーネルが実際に使う7フィールドのみを
   抽出する変換ツール。新規Codonコード不要で実データ検証を可能にする
   ためのものです。

**このセッション内で完了した検証(GPU不要、いずれもgcc)**:
- 360Pyから実カーネルソースを一字一句抽出したPythonシミュレータと、
  合成データ(N=21制約準拠、境界ケース含む20,003件+200,003件)で
  Cポートを突き合わせ、**全件バイト単位完全一致**を確認しました。
  この過程で`process_one_task`が`meta_next`引数を使わずグローバル
  定数を参照していた実装ミスを発見・修正しています(gccの
  `-Wunused-parameter`警告で検出)。

**まだ実施していない検証(次のステップ、お手数をおかけします)**:
実データでの照合です。手元に361の実機ダンプファイル
(`constellations_N21_6.bin.soa_ref_361.bin`、2,025,282件)が既に
あるはずなので、GPUもnvccも使わず、CPU専用ビルドだけで本物の正解値
`314666222712`との照合ができます。
...
```
STATIC_ONLY=1 bash 363Py_kernel_maxd14_validate_N21_full_once.sh
bash 363Py_kernel_maxd14_validate_N21_full_once.sh
```

`.sh`はコード無変更の静的チェックに加え、361の実機ダンプファイルが
カレントディレクトリにあればそれを使ってCPU専用ビルドでの正当性照合
(total_sum==314666222712)まで自動で行います。見つからない場合は
その旨を表示してスキップします(361のダンプファイルを同じ
ディレクトリに置いてから再実行してください)。

一致すればカーネルロジックの移植が実データで確定し、364でnvcc実機
ビルド+GPU実行+±3%タイミング判定に進めます。
"""
...
VERSION_TAG:str="363 kernel_maxd14_cu: DESIGN/IMPLEMENTATION REVISION, NO CODON CODE CHANGE. Executable code region (import gpu onward) is byte-identical to 362, which was itself byte-identical to 361 (r2): verified by a static check hashing the docstring-stripped/VERSION_TAG-excluded code region against 362s own reference hash for the identical region. 363s actual deliverables are two new standalone files outside this one: (1) 363_kernel_maxd14.cu, a CUDA C port of kernel_dfs_iter_gpu_maxd14 per 362_kernel_port_spec.md, structured with the per-task body factored into a HOSTDEV (__host__ __device__ under nvcc, plain under gcc) function process_one_task() so the exact same source compiles into both the real __global__ kernel (nvcc) and a CPU-only test harness (gcc, no CUDA toolkit required) driven by a #ifndef __CUDACC__ main(); (2) 363_extract_kernel_input_from_361_dump.py, a small utility that extracts the 7 fields the kernel actually consumes (ld, rd, col, ctrl0, free, markctrl, w_lo) from 361s existing 10-field SoA reference dump, so the C kernel can be cross-validated against real N=21 production data (2,025,282 records, correctness oracle 314666222712) using only a CPU build -- no GPU, no nvcc, no new bench_mode, no Codon code change at all. Two rounds of cross-validation were completed this session before real-hardware handoff: first, a literal line-by-line re-execution of the actual Codon kernel source under CPython (u32/u64 stubbed as bitmasks) against synthetic-but-shift-valid N=21 data (20,003 then 200,003 records, including deliberately injected root_action 1/2/3 and root-fast-path 1-or-2-candidate edge cases) produced byte-for-byte identical per-task results against the C port at both scales; this process caught and fixed a real bug where process_one_task ignored its meta_next parameter and read a global constant instead (surfaced by gccs -Wunused-parameter warning). The push-guard risk note from 358/359 (362 spec section 2) was honored: the `if (cur_avail != 0)` branch immediately before every stack push was ported as a completely literal 1:1 translation with zero shape changes. Real-hardware validation (running 363_extract_kernel_input_from_361_dump.py plus the CPU-only build against 361s actual N=21 dump to confirm total_sum==314666222712, followed by an nvcc device build, a real GPU launch, and the plus-or-minus 3 percent timing comparison against the 356 anchor of 393.404s) is not yet done and is this revisions main open item; a real nvcc/GPU build and launch is deferred to 364 pending this CPU-side real-data confirmation."
