...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 401 (2026-09-09)
...
"""
401の四点セットです。静的ゲートは **OK=12 FAIL=0**。**カーネルもホストも一切変更なし**（400-r3のリネーム、コード領域全体 `f1a03a7f…` 一致）。掃引だけで答えが出ます。

## 崖の手前を、初めて刻みます

400-r2/r3が測ったstrideは**崖の遥か先ばかり**でした。65 KB の次が 208 KB です。**あいだを見ていません。**

| MB | warps/SM | 作業集合 | | MB | warps/SM | 作業集合 |
|---:|---:|---:|---|---:|---:|---:|
| 800 | 10 | 65.0 KB | | 1280 | 16 | 104.0 KB |
| 880 | 11 | 71.5 KB | | 1440 | 18 | 117.0 KB |
| 960 | 12 | 78.0 KB | | 1600 | 20 | 130.0 KB |
| 1040 | 13 | 84.5 KB | | 1920 | 24 | 156.0 KB |
| 1120 | 14 | 91.0 KB | | 2560 | 32 | 208.0 KB |
| 1200 | 15 | 97.5 KB | | | | |

`MAX_BLOCKS` は80の倍数だけにしました。400-r2で「200ブロック÷80SM」の割り切れなさが6.5%の罰則を生んだので。

## L5が本リビジョンの全てです

> **作業集合がまだ収まっている範囲で、10 warps/SM を超える構成の中に、ベースラインより速いものが1つでもあるか。**

- **ある** → フレーム半減はその範囲を広げる。**402で詰め替えを実装する価値が確定**
- **無い** → **このアルゴリズムではオカパンシーはそもそも効かない。** どれだけ上手く詰め替えても無駄です

後者だった場合、`wait` 48% は「ワープを増やす」ではなく「**内側ループの依存鎖を短くする**」方向で攻めることになります。ハーネスがその旨まで出力します。

**12分、コード変更ゼロで、どちらかが判ります。**

## 104バイト案の値札も自動で出ます

ハーネスは測定した「99%ヒットを保つ最大作業集合」から、**104バイトフレームなら何 warps/SM 載るか**を計算して出力します。

```
BUDGET: XX.X KB is the largest footprint measured at >=99% hit.
With a 104-byte frame that budget holds YYY threads/SM = Z.Z warps/SM,
against 10 today.
```

着手はその数字を見てからです。そして着手したら、**397を止めたのと同じ全2,025,282レコードのバイト一致ゲート**を先に通します。
...
出ました。そして**私のハーネスに2つ誤りがあります。** まずそれを直してから読みます。

## 誤り1 — 作業集合の計算が MB>1280 で間違っています

**sm_86 はSMあたり16ブロックが上限**です。`BLOCK=32` なら1ブロック=1ワープなので、**MB=1280（16ブロック/SM）を超えると常駐は増えません。** 余ったブロックは待つだけです。

したがって表の右半分は実際にはこうです。

| MB | blocks/SM | **常駐 warps/SM** | **作業集合** | mean ms |
|---:|---:|---:|---:|---:|
| 1280 | 16 | 16 | 104.0 KB | 3,304.5 |
| 1440 | 18→**16** | **16** | **104.0 KB** | 3,408.9 |
| 1600 | 20→**16** | **16** | **104.0 KB** | 3,481.6 |
| 1920 | 24→**16** | **16** | **104.0 KB** | 3,322.7 |
| 2560 | 32→**16** | **16** | **104.0 KB** | 3,049.1 |

ncuが裏付けています。MB=2560 の Achieved Occupancy は 30.25%、warps/sched 3.75 = **15 warps/SM**。32ではありません。

**したがって `L2 FAILED`（非単調）は私のモデルの誤りで、物理の話ではありません。** 作業集合が実際に変化する範囲（MB 800→1280）では、2,154.7 → 2,301.0 → 2,279.5 → 2,433.5 → 2,854.0 → 3,032.6 → 3,304.5 と**ほぼ単調**です。

400-r2で「stride 81,920 は10,729 ms」だったのが今回3,049 msなのも同じ理由です。あれは `64:1280`（16ブロック×2ワープ＝**32 warps/SM、208 KB**）で、今回の `32:2560` は16 warps/SM・104 KBでした。**別の点です。**

## 誤り2 — L5の判定に交絡があります

**`warps/SM` と `k_per_thread` は分離できません。** どちらも stride で決まります。

| MB | warps/SM | **k_per_thread (N=19)** | mean ms |
|---:|---:|---:|---:|
| 800 | 10 | **35.0** | 2,154.7 |
| 880 | 11 | 31.9 | 2,301.0（+6.79%） |
| 1280 | 16 | 21.9 | 3,304.5（+53.4%） |

ワープを増やすと、**同時にスレッドあたりレコード数が減ってテールが悪化します。** MB=880 は L1ヒット率がまだ99%台のはずなのに +6.79%。**これはL1ではなくkの効果**である可能性が高い。

**そしてN=19はこの交絡が最も強く出る規模です。** 400-r2のノートで自分で警告したことが、また効いています。
...
```
65.0 KB → L1ヒット 99.51%
78.0 KB → L1ヒット 98.56%   時間 +5.79%
```

**現行の65 KBが既に崖の縁です。** 実効L1は70 KB前後。私のL3予測（99%割れは80〜120 KB）も外れました。

そして予算計算はこう出ています。

```
71.5 KB が99%を保つ最大。104バイトフレームなら 704 threads/SM = 22.0 warps/SM
```

## 401-r2の提案 — 交絡を外す

**N=21なら、16 warps/SM でも k=49.4** です。N=19のベースライン（k=35.0）より**多い**。

| | warps/SM | k_per_thread |
|---|---:|---:|
| N=19 MB=800 | 10 | 35.0 |
| N=21 MB=800 | 10 | **79.1** |
| **N=21 MB=1280** | **16** | **49.4** |

**N=21で MB=1280 が MB=800 より遅ければ、「kが十分あってもワープ増は損」が確定します。** 逆に速ければ、L5の棄却はN=19の交絡による誤判定だったことになり、**104バイト案が生き返ります。**

MB=800 / 960 / 1120 / 1280 の4点 × 2反復（ノイズフロア0.01%なので2回で十分）。**N=21で約18分**です。

併せてハーネスの `blocks/SM ≤ 16` 上限と `k_per_thread` 列を直します。

これで今夜の決着がつきます。書き起こしますか。
...
VERSION_TAG:str="401 PY-REVTAG: pure rename of 400-r3Py; the table names ./401_kernel_maxd14 and keeps env_prefix NQ_EXTRA_CTX=1, CRUNNER_MIN_N=19, NQ_BLOCK and NQ_CARVEOUT. No functional change. 400-r3 showed the carveout knob is inert because the driver already maximises L1, and located the wall precisely: L1 hit rate falls from 99.48 percent at a 66.6 KB working set to 85.96 percent at 205 KB, L2 traffic rises 25.7 times, and the dominant stall inverts from wait to long_scoreboard. 401 maps that cliff finely, with no code change, to answer whether more warps help at all while the working set still fits."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./401_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
