...
#
# USAGE
#   STATIC_ONLY=1 bash 400_r3_validate.sh
#                 bash 400_r3_validate.sh         # ~15 min (sweep N=19, confirm N=20)
#   NPROF=20 NCONF=21 NBIG=22 bash 400_r3_validate.sh   # production scale, ~45 min
#   WITH_NBIG=1   bash 400_r3_validate.sh         # + the big-N confirmation
#   NCU_PREFIX=sudo bash 400_r3_validate.sh
#
...
REV="400_r3"
PY_SRC="${PY_SRC:-400_r3Py_kernel_maxd14_final.py}"
PY_BIN="${PY_BIN:-400_r3Py_kernel_maxd14_final}"
PREV_PY="${PREV_PY:-400_r2Py_kernel_maxd14_final.py}"
HELPER_SRC="${HELPER_SRC:-rev386_validation_helpers.py}"
CU_SRC="${CU_SRC:-400_r3_kernel_maxd14.cu}"
CU_BIN="${CU_BIN:-400_r3_kernel_maxd14}"
PREV_CU="${PREV_CU:-400_r2_kernel_maxd14.cu}"
CRLOG_DIR="${CRLOG_DIR:-400_r3_crunner_logs}"
CODON="${CODON:-codon}"
...
#   NPROF=20 NCONF=21 NBIG=22 bash 400_r3_validate.sh
NPROF="${NPROF:-19}"
NCONF="${NCONF:-20}"
NBIG="${NBIG:-21}"
oracle_of() {  # $1 = N
...
# 400-r3: fewer shapes, because r2 showed block shape is nearly neutral --
# what matters is stride. One representative config per stride, swept with
# and without the carveout override.
SWEEP="${SWEEP:-32:800 64:640 64:1280 128:960}"
CARVEOUTS="${CARVEOUTS:-default 0}"
...
grep -qE '^REV_TAG:str="400_r3"' "$CODE" && pass "source_rev_tag_is_400_r3" || fail "source_rev_tag_is_400_r3" "wrong REV_TAG"
grep -q 'CRunnerEntry(14,"./400_r3_kernel_maxd14","NQ_EXTRA_CTX=1 "' "$CODE" && pass "source_table_points_at_400_r3_and_keeps_the_treatment" || fail "source_table_points_at_400_r3_and_keeps_the_treatment" "table entry wrong"
...
if [[ -f "$PREV_PY" ]]; then
  ...
  [[ "$PR_" == "3" && "$PA_" == "3" ]] && pass "py_diff_fingerprint_vs_400_r2Py (removed=3 added=3 EXECUTABLE lines)" \
    || { fail "py_diff_fingerprint_vs_400_r2Py" "removed=$PR_ added=$PA_, expected 3/3"; diff "/tmp/${REV}_prev_code.py" "$CODE" | head -20 | sed 's/^/      /'; }
else info "py_diff_fingerprint_vs_400_r2Py" "skipped"; fi
...
if [[ -f "$PREV_CU" ]]; then
  ...
  [[ "$CR_" == "0" && "$CA_" == "46" ]] && pass "cu_diff_fingerprint_vs_400_r2 (removed=0 added=46: the NQ_CARVEOUT knob and the attribute report)" \
    || { fail "cu_diff_fingerprint_vs_400_r2" "removed=$CR_ added=$CA_, expected 0/46"; diff "/tmp/${REV}_prev_code.cu" "/tmp/${REV}_cur_code.cu" | head -30 | sed 's/^/      /' | cut -c1-140; }
else info "cu_diff_fingerprint_vs_400_r2" "skipped"; fi
grep -q 'cudaFuncAttributePreferredSharedMemoryCarveout' "$CU_SRC" && pass "cu_carveout_knob_present" || fail "cu_carveout_knob_present" "NQ_CARVEOUT is not wired to the attribute"
grep -q 'cudaFuncGetAttributes(&fa, kernel_dfs_iter_gpu_maxd14)' "$CU_SRC" && pass "cu_attribute_report_present" || fail "cu_attribute_report_present" "no cudaFuncGetAttributes report"
grep -q 'int co_req = -1;' "$CU_SRC" && pass "cu_carveout_inert_when_unset (the attribute is only set when NQ_CARVEOUT is given)" || fail "cu_carveout_inert_when_unset" "the guard changed"
...
# 2. Sweep
# ---------------------------------------------------------------------
printf 'N\tcarveout\tblock\tmax_blocks\tstride\twarps_per_sm\trep\tkernel_ms\teff_pref\tlocal_bytes\ttotal_sum\tmatch\n' > "$TSV"   # 400-r3: r2's TSV had no N or carveout column, so sweep and confirm rows merged and printed a 154% spread
declare -A SMEAN
run_cfg() {  # block max_blocks N input oracle reps tag [carveout]
  local bk="$1" mb="$2" n="$3" input="$4" orc="$5" reps="$6" tag="$7" co="${8:-default}"
  local stride=$((bk*mb)) bps=$((mb/80)) wps=$(( (mb/80) * (bk/32) ))
  [[ "$wps" -gt 48 ]] && wps=48
  local sum=0 k
  for r in $(seq 1 "$reps"); do
    local lg="$LOGDIR/3_${tag}_co${co}_b${bk}_m${mb}_r${r}.log"
    if [[ "$co" == "default" ]]; then
      env -u NQ_PAD_MB -u NQ_CARVEOUT NQ_BLOCK="$bk" NQ_MAX_BLOCKS="$mb" NQ_EXTRA_CTX="$EXTRA_CTX" \
        "./$CU_BIN" "$n" "$input" "/tmp/${REV}_out.bin" "$orc" > "$lg" 2>&1 || true
    else
      env -u NQ_PAD_MB NQ_CARVEOUT="$co" NQ_BLOCK="$bk" NQ_MAX_BLOCKS="$mb" NQ_EXTRA_CTX="$EXTRA_CTX" \
        "./$CU_BIN" "$n" "$input" "/tmp/${REV}_out.bin" "$orc" > "$lg" 2>&1 || true
    fi
    ...
    local ep lb
    ep="$(grep -o 'effective_pref=[-0-9]*' "$lg" | head -1 | cut -d= -f2)"
    lb="$(grep -o 'local_bytes_per_thread=[0-9]*' "$lg" | head -1 | cut -d= -f2)"
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$n" "$co" "$bk" "$mb" "$stride" "$wps" "$r" "${kms:-?}" "${ep:-?}" "${lb:-?}" "${tot:-?}" "$m" >> "$TSV"
    if [[ "$m" != "1" || "${tot:-}" != "$orc" ]]; then
  ...
  SMEAN["${tag}_${co}_${bk}_${mb}"]="$(awk -v s="$sum" -v r="$reps" 'BEGIN{printf "%.3f",s/r}')"
  info "cfg[$tag co=$co]" "BLOCK=$bk MAX_BLOCKS=$mb stride=$stride warps/SM~$wps  mean=${SMEAN["${tag}_${co}_${bk}_${mb}"]} ms"
  return 0
}
...
for co in $CARVEOUTS; do
  for pair in $SWEEP; do
    bk="${pair%%:*}"; mb="${pair##*:}"
    run_cfg "$bk" "$mb" "$NPROF" "$IN20" "$ORC_PROF" "$REPS" nP "$co" || { echo "OK=$PASS FAIL=$FAIL"; exit 1; }
  done
done
EFF_DEFAULT="$(grep -o 'effective_pref=[-0-9]*' "$LOGDIR/3_nP_codefault_b${BASE_BLOCK}_m${BASE_MB}_r1.log" | head -1 | cut -d= -f2)"
LOCB="$(grep -o 'local_bytes_per_thread=[0-9]*' "$LOGDIR/3_nP_codefault_b${BASE_BLOCK}_m${BASE_MB}_r1.log" | head -1 | cut -d= -f2)"
SHB="$(grep -o 'static_shared_bytes=[0-9]*' "$LOGDIR/3_nP_codefault_b${BASE_BLOCK}_m${BASE_MB}_r1.log" | head -1 | cut -d= -f2)"
info "carveout_default" "effective_pref=${EFF_DEFAULT:-?}  local_bytes_per_thread=${LOCB:-?}  static_shared_bytes=${SHB:-?}"
[[ "${LOCB:-0}" == "208" && "${SHB:-1}" == "0" ]] && pass "K2_local_and_shared_as_expected (208 B local per thread, 0 B static shared)" \
  || fail "K2_local_and_shared_as_expected" "local=${LOCB:-?} shared=${SHB:-?}, expected 208 / 0 -- every capacity number in this revision is computed from these two"
if [[ -n "${EFF_DEFAULT:-}" ]]; then
  [[ "$EFF_DEFAULT" != "0" ]] && pass "K3_default_carveout_is_not_L1_maximal (effective_pref=$EFF_DEFAULT)" \
    || info "K3_default_is_already_L1_maximal" "effective_pref=0 -- the driver already gives L1 the maximum, so K4 is expected to be a null result and capacity can only be attacked by shrinking the 208-byte frame"
fi
BASE20="${SMEAN[nP_default_${BASE_BLOCK}_${BASE_MB}]:-}"
[[ -n "$BASE20" ]] && pass "baseline_measured[N=$NPROF] (${BASE_BLOCK}x${BASE_MB} = $BASE20 ms)" || { fail "baseline_measured" "no baseline"; exit 1; }

banner "K4/K6 evaluation at N=$NPROF -- carveout against default, stride by stride"
BESTK=""; BESTV=""; BESTCO="default"
for pair in $SWEEP; do
  bk="${pair%%:*}"; mb="${pair##*:}"; stride=$((bk*mb))
  vd="${SMEAN[nP_default_${bk}_${mb}]:-}"; vc="${SMEAN[nP_0_${bk}_${mb}]:-}"
  [[ -z "$vd" ]] && continue
  rel="$(awk -v a="$BASE20" -v b="$vd" 'BEGIN{printf "%+.2f",(b-a)/a*100}')"
  if [[ -n "$vc" ]]; then
    d="$(awk -v a="$vd" -v b="$vc" 'BEGIN{printf "%.3f",(b-a)/a*100}')"
    info "stride[$stride]" "default $vd ms (${rel}% vs base) | carveout=0 $vc ms | carveout effect ${d}%"
    if [[ "$stride" == "25600" ]]; then
      ad="$(awk -v d="$d" 'BEGIN{printf "%.3f",(d<0?-d:d)}')"
      awk -v a="$ad" 'BEGIN{exit !(a<=1)}' && pass "K6_carveout_neutral_at_production_stride (${d}%) -- 66.6 KB fits either way, as the capacity model says" \
        || fail "K6_carveout_neutral_at_production_stride" "${d}% at stride 25,600 -- the capacity model is wrong: 66.6 KB should fit in L1 whatever the carveout"
    fi
    if [[ "$stride" == "81920" ]]; then
      awk -v d="$d" 'BEGIN{exit !(d <= -10)}' && pass "K4_CONFIRMED_carveout_helps_at_high_occupancy (${d}% at stride 81,920)" \
        || { ad="$(awk -v d="$d" 'BEGIN{printf "%.3f",(d<0?-d:d)}')"
             awk -v a="$ad" 'BEGIN{exit !(a<=2)}' && fail "K4_REFUTED" "carveout=0 changes stride 81,920 by only ${d}% -- L1 is already as large as it gets, so the local-memory working set can only be cut by shrinking the 208-byte frame (397's idea, done correctly)" \
               || info "K4_partial" "${d}%"; }
    fi
  else
    info "stride[$stride]" "default $vd ms (${rel}% vs base)"
  fi
  for c in default 0; do
    v="${SMEAN[nP_${c}_${bk}_${mb}]:-}"; [[ -z "$v" ]] && continue
    if [[ -z "$BESTV" ]] || awk -v a="$v" -v b="$BESTV" 'BEGIN{exit !(a<b)}'; then BESTK="$bk:$mb"; BESTV="$v"; BESTCO="$c"; fi
  done
done
GAIN="$(awk -v a="$BASE20" -v b="$BESTV" 'BEGIN{printf "%.3f",(a-b)/a*100}')"
info "best_config" "$BESTK carveout=$BESTCO = $BESTV ms, ${GAIN}% vs the default-carveout baseline"
...
ncu_cfg() {  # tag block max_blocks metrics [carveout]
  local tag="$1" bk="$2" mb="$3" met="$4" co="${5:-}"
  local coenv=""; [[ -n "$co" ]] && coenv="NQ_CARVEOUT=$co"
  timeout "$SECTION_TIMEOUT_S" $NCU_PREFIX env NQ_EXTRA_CTX=1 $coenv NQ_BLOCK="$bk" NQ_MAX_BLOCKS="$mb" "$NCU" \
...
if [[ -n "${NCU_PREFIX+x}" ]]; then
  # K5: the L1 hit rate for local traffic is the direct test of the capacity
  # story, so it is measured at the production stride and at the stride where
  # the slowdown is unmistakable, with and without the carveout override.
  L1_METRICS="l1tex__t_sector_hit_rate.pct,l1tex__t_sectors_pipe_lsu_mem_local_op_ld.sum,l1tex__t_requests_pipe_lsu_mem_local_op_ld.sum,lts__t_sectors_srcunit_tex_op_read.sum"
  ncu_cfg "s25600_default" 32  800 "$STALL_METRICS,$OCC_METRICS,$L1_METRICS"    || true
  ncu_cfg "s25600_co0"     32  800 "$STALL_METRICS,$OCC_METRICS,$L1_METRICS" 0  || true
  ncu_cfg "s81920_default" 64 1280 "$STALL_METRICS,$OCC_METRICS,$L1_METRICS"    || true
  ncu_cfg "s81920_co0"     64 1280 "$STALL_METRICS,$OCC_METRICS,$L1_METRICS" 0  || true
fi
...
if [[ -n "${NIN[$NCONF]:-}" && -n "$BESTK" && -n "$ORC_CONF" ]]; then
  run_cfg "$BASE_BLOCK" "$BASE_MB" "$NCONF" "${NIN[$NCONF]}" "$ORC_CONF" "$REPS21" nC default || { echo "OK=$PASS FAIL=$FAIL"; exit 1; }
  if [[ "$BESTK" != "${BASE_BLOCK}:${BASE_MB}" || "$BESTCO" != "default" ]]; then
    run_cfg "${BESTK%%:*}" "${BESTK##*:}" "$NCONF" "${NIN[$NCONF]}" "$ORC_CONF" "$REPS21" nC "$BESTCO" || { echo "OK=$PASS FAIL=$FAIL"; exit 1; }
    BC="${SMEAN[nC_default_${BASE_BLOCK}_${BASE_MB}]}"; WC="${SMEAN[nC_${BESTCO}_${BESTK%%:*}_${BESTK##*:}]}"
...
if [[ "$WITH_NBIG" == "1" && -n "$BESTK" ]]; then
  ...
  if [[ -f "$g22" ]]; then
    ...
    if [[ -f "${IN22:-/nonexistent}" ]]; then
      run_cfg "$BASE_BLOCK" "$BASE_MB" "$NBIG" "$IN22" "$ORC_BIG" 1 nB default || true
      run_cfg "${BESTK%%:*}" "${BESTK##*:}" "$NBIG" "$IN22" "$ORC_BIG" 1 nB "$BESTCO" || true
      B22="${SMEAN[nB_default_${BASE_BLOCK}_${BASE_MB}]:-}"; W22="${SMEAN[nB_${BESTCO}_${BESTK%%:*}_${BESTK##*:}]:-}"
      if [[ -n "$B22" && -n "$W22" ]]; then
...
by={}
for r in rows:
    by.setdefault((r['N'],r['carveout'],r['block'],r['max_blocks'],r['stride'],r['warps_per_sm'],r['eff_pref'],r['local_bytes']),[]).append(float(r['kernel_ms']))
print(f"{'N':>3} {'carve':>7} {'BLOCK':>6} {'MB':>6} {'stride':>8} {'w/SM':>5} {'eff':>4} {'locB':>5} {'mean ms':>12} {'spread':>8}")
for k,v in sorted(by.items(), key=lambda x:(x[0][0], st.fmean(x[1]))):
    m=st.fmean(v); sp=(max(v)-min(v))/m*100 if len(v)>1 else 0.0
    print(f"{k[0]:>3} {k[1]:>7} {k[2]:>6} {k[3]:>6} {k[4]:>8} {k[5]:>5} {k[6]:>4} {k[7]:>5} {m:12.3f} {sp:7.3f}%")
def load(p):
    m={}; hdr=None
...
for p in sorted(glob.glob(os.path.join(logdir,'8_ncu_*.csv'))):
    cfgs[re.search(r'8_ncu_(.+)\.csv',p).group(1)]=load(p)
if cfgs:
    print("\n=== K5: L1 behaviour and stalls ===")
    for tag,m in cfgs.items():
        hit=m.get('l1tex__t_sector_hit_rate.pct')
        ls=m.get('l1tex__t_sectors_pipe_lsu_mem_local_op_ld.sum'); lr=m.get('l1tex__t_requests_pipe_lsu_mem_local_op_ld.sum')
        l2=m.get('lts__t_sectors_srcunit_tex_op_read.sum')
        occ=m.get('Achieved Occupancy'); aw=m.get('Active Warps Per Scheduler')
        spr=(ls/lr) if (ls and lr) else None
        print(f"  [{tag:16s}] L1 hit {hit if hit is not None else '?'}%  local sectors/req {spr if spr is None else round(spr,3)}  L2 read sectors {l2 if l2 is None else int(l2)}  occ {occ}%  warps/sched {aw}")
        st_=[(k,v) for k,v in m.items() if 'issue_stalled' in k and 'selected' not in k]
        tot=sum(v for _,v in st_)
        if tot:
            top=sorted(st_,key=lambda x:-x[1])[:3]
            print("      " + "  ".join(f"{re.sub(r'^smsp__average_warps_issue_stalled_','',re.sub(r'_per_issue_active.ratio$','',k))} {v/tot*100:.1f}%" for k,v in top))
    def hit(t): return cfgs.get(t,{}).get('l1tex__t_sector_hit_rate.pct')
    a,b=hit('s25600_default'),hit('s81920_default')
    if a is not None and b is not None:
        print(f"\n  K5 stride effect: L1 hit {a:.2f}% at stride 25,600 -> {b:.2f}% at 81,920")
        print("  K5 HELD: the working set is falling out of L1 as threads per SM rise -- the capacity model stands."
              if b < a - 5 else
              "  K5 FAILED: L1 hit rate did not fall with stride, so the 6-7x slowdown is NOT a capacity effect and needs another explanation.")
    c,d=hit('s81920_default'),hit('s81920_co0')
    if c is not None and d is not None:
        print(f"  carveout effect at stride 81,920: L1 hit {c:.2f}% -> {d:.2f}%")
EOF
