...
"""
Now the ARGPARSE insert for `record_limit_arg`, right after the existing chunk-mode arg block:Now the dispatch branch, after 368's dispatch block:Now update the VERSION_TAG:Regex extraction works correctly. Let me copy the deliverables and write the README append.369として3ファイル揃えました。

## やったこと

- **`369Py_mem_probe.py`**: 368への純追加(6スパン: `VARINIT`/`CLIGATE-COMMENT`/`PRESETGATE-COMMENT`/`ARGPARSE-INSERT`/`INSERT`/`DISPATCH-INSERT`)。366・368の既存関数は無変更のまま残しています。新規`read_vmhwm_kb()`(`/proc/self/status`の`VmHWM`=ピークRSSをKBで取得)と`probe_partial_load_memory()`(count→read→build_soaの3チェックポイントでVmHWMを記録)を追加。読み込み件数を`record_limit`としてCLI引数化(`bench_mode==30`が`debug_chunk_start`に使うのと同じ`argv[13]`スロットを再利用、モード排他なので曖昧さなし)。新規`bench_mode=36`、完了時は必ず`[mem-probe-done] ... delta_read_kb=... delta_soa_kb=... delta_total_kb=...`を出力します。

- **`369Py_mem_probe_validate_N22_sweep.sh`**: 静的チェック部は366/368と同型(照合先は**368自身のフルハッシュ**: `362bb7fad3c026a625e7bb276c9b2e9b2de6ed0af29dda1934980890c0b85454`, 5997行、このセッションで新規算出)。366・368双方の元関数が無変更で残っているかも追加チェック。**実行部はスウィープ形式**にしました: `record_limit`を`1,000,000 / 5,000,000 / 10,000,000 / 15,000,000 / 20,000,000 / 25,000,000 / 28,719,035`(=N=22の実件数)の順に1バイナリ呼び出しずつ試し、**完走しなかった段で自動的に止まり**、直前の成功値との間に閾値を挟み込みます。成功した段は`delta_read_kb`/`delta_soa_kb`/`delta_total_kb`を集計し、最後に結果テーブルと(閾値割れの場合は)簡易な線形外挿を出力します。

- **`STATIC_ONLY=1`でこのセッション内実行済み: `OK=27 FAIL=0 INFO=0 WARN=1`**(想定内のsudo警告のみ)。テーブル抽出用の正規表現も合成ログ行で動作確認済みです。ただしcodon/GPU環境が無いため実ビルド・実行はまだです。
...
`MAX_MEM_PERCENT`は今回はデフォルト(70)のまま推奨です(天井を上げるより、どこで閾値にぶつかるかを見たいので)。

もし全段(28,719,035件フル)が完走した場合は、bin読み込み自体は原因ではなく、366/368がクラッシュしたのはこの後の段階(カーネル起動・GPU転送周り)だという結論になります。その場合はREADME(`369_README_append.md`)にも書いた通り、次のRevは読み込みより先の段階を計測する方向に舵を切る想定です。結果が出たら教えてください。
"""
