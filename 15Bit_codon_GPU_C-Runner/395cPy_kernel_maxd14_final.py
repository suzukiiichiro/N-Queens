...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 395c (2026-09-08)
...
"""
解析しました。予測は逆方向に外れ、代わりに4.2%の正体が確定しました。

## 395b 結果(全セルオラクル一致、Gは`-g 21 21`で漏れなし)

| セル | 内容 | `kernel_ms` |
|---|---|---:|
| I0 | 2つのschedファイルを`cmp` | **バイト一致** |
| I1 / I2 | 395aバイナリ、直接、800(本番sched / 394f L3) | 139,166 / 139,181 |
| **K1** | **395bバイナリ**、直接、800 | **144,032(+3.5%)** |
| G | 395bバイナリ、`-g 21 21` | 138,650(K1比−3.7%) |
| A | 395b、484 | 201,705(395a比+5.2%) |

## 4.2%の正体は「起動経路」

ファイルは同一で、直接実行なら両方139.2秒。**同じバイナリ・同じファイル・同じstrideを、Codonディスパッチャ経由で起動すると約4%速い**(395a −4.0%、395b −3.7%、2回再現)。機構は未解明です。本番`-g`は既にこの4%を得ていますが、**直接実行による全測定は本番より約4%悲観的**だったことになります。

## レジスタ常駐化は退行 — カーネル内の局所的制御フロー編集はクローズ

push/popの周りに`if (save_sp != 0)`を2つ加えた変更は制御フロー形状の変更で、240/266-269/273/326/358/359に続く**7戦7敗**です。395a r2(命令数削減)は中立、395b(分岐追加)は退行。IPC 0.28のストール律速領域では、ALU削減は効かず、分岐追加は再収束コストで負ける。予定していた395c案(pop→decode直結)は同じ失敗クラスなので**行いません**。実際に利得を出してきたのは非カーネル側(占有率−19%、順序−5%、経路−4%)で、残るカーネル側の手は構造的なもの(フレーム圧縮による占有率天井)だけです。

## 395c — 差し戻し+起動経路の約4%を測る

- `395c_kernel_maxd14.cu`: **カーネル領域を395a r2とsha256一致に復帰**(ホスト既定800は維持、差分3/6のみ)。395b入力で3項目FAILすることを確認済み
- `395c_ctx_holder.cu`: CUDAコンテキスト(+任意サイズの確保)をN秒保持するだけのプログラム

| セル | 内容 | 判定 |
|---|---|---|
| D0 | 直接、GPU上に他プロセスなし | ≈139.2 |
| **D1** | 直接、**別プロセスが素のコンテキストを保持** | ≤D0−3%なら**H_ctx**(コンテキストの存在) |
| **D2** | 直接、コンテキスト+**2 GB**をtouch済みで保持 | D1と1.5%以上違えば**H_place**(メモリ配置) |
| G | `-g 21 21` | ≈133.6(差し戻しの確認) |

各セルの実行中に5秒ごとのSMクロック・電力サンプラを走らせ、**H_clock**(直接実行中だけクロックが低い可能性)も実測で排除します。H_ctxが反証(D1≈D0)なら、395c-r2でディスパッチャのコマンド行(環境変数・cwd・stdioの向き先)を二分探索します。

この4%は、機構が分かれば直接実行でも再現でき(測定の悲観バイアスが消える)、配置由来ならさらに取れる可能性があります——968の崖の縁での±1.6%(394c)も同根かもしれません。

```bash
STATIC_ONLY=1 bash 395c_validate.sh      # OK=24
...
VERSION_TAG:str="395c PY-REVTAG: pure rename of 395bPy plus the dispatch table referencing ./395c_kernel_maxd14 (395a r2 kernel restored; 395b reverted). The 395b policy changes stay: A10G_FINAL_DEFAULT_* applied for argc<=4, MAX_BLOCKS 800, sched input order, redirected external tools. 395c measures the dispatcher-vs-direct ~4% (same binary, same file) with a context-holder experiment and an in-run clock sampler; the kernel is not the variable."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./395c_kernel_maxd14","","[gpu-run-done]","[gpu-run-correctness]"),
  ]
