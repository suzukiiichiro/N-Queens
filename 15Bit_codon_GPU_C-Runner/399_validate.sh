...
REV="399"
PY_SRC="${PY_SRC:-399Py_kernel_maxd14_final.py}"
PY_BIN="${PY_BIN:-399Py_kernel_maxd14_final}"
PREV_PY="${PREV_PY:-398_r2Py_kernel_maxd14_final.py}"
HELPER_SRC="${HELPER_SRC:-rev386_validation_helpers.py}"
CU_SRC="${CU_SRC:-399_kernel_maxd14.cu}"
CU_BIN="${CU_BIN:-399_kernel_maxd14}"
PREV_CU="${PREV_CU:-398_r2_kernel_maxd14.cu}"
CRLOG_DIR="${CRLOG_DIR:-399_crunner_logs}"
CODON="${CODON:-codon}"
...
grep -qE '^REV_TAG:str="399"' "$CODE" && pass "source_rev_tag_is_399" || fail "source_rev_tag_is_399" "wrong REV_TAG"
grep -q 'CRunnerEntry(14,"./399_kernel_maxd14","NQ_EXTRA_CTX=1 "' "$CODE" && pass "source_table_points_at_399_and_keeps_the_treatment" || fail "source_table_points_at_399_and_keeps_the_treatment" "table entry wrong"
...
if [[ -f "$PREV_PY" ]]; then
  ...
  [[ "$PR_" == "3" && "$PA_" == "3" ]] && pass "py_diff_fingerprint_vs_398_r2Py (removed=3 added=3 EXECUTABLE lines)" \
    || { fail "py_diff_fingerprint_vs_398_r2Py" "removed=$PR_ added=$PA_, expected 3/3"; diff "/tmp/${REV}_prev_code.py" "$CODE" | head -20 | sed 's/^/      /'; }
else info "py_diff_fingerprint_vs_398_r2Py" "skipped"; fi
...
if [[ -f "$PREV_CU" ]]; then
  ...
  [[ "$CA" == "$CB" ]] && pass "cu_whole_code_region_identical_to_398_r2 (${CB:0:16}...)" || fail "cu_whole_code_region_identical_to_398_r2" "399 must be a rename"
else info "cu_whole_code_region_identical_to_398_r2" "skipped"; fi
...
command -v "$NCU" >/dev/null 2>&1 && pass "ncu_present" || { fail "ncu_present" "set NCU=..."; exit 1; }
...
# ---------------------------------------------------------------------
banner "Phase D: X5 -- does lane utilisation move with the time?"
for mode in asis "${BEST:-asis}"; do
  ob="/tmp/${REV}_order_${mode}.bin"
  [[ -f "$ob" ]] || continue
  [[ -f "$LOGDIR/3_ncu_lane_${mode}.csv" ]] && continue
