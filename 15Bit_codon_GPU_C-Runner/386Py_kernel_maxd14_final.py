...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 386 (2026-09-07)
...
VERSION_TAG:str="386 VALIDATION-SPLIT: relocated 12 pure verification/diagnostic helper functions (validate_chunk_range, validate_reordered_count, validate_reordered_indices, file_exists, validate_bin_file, count_constellations_bin_records, read_stream_done_count, write_stream_done_count, read_vmhwm_kb, crunner_parse_result, crunner_input_fname, crunner_input_valid) out of this file into rev386_validation_helpers.py, using the exact local multi-file import mechanism 384's probe confirmed works on real hardware. Selection was conservative: only functions with zero dependency on kernel-adjacent structures (TaskSoA, build_soa_for_range, the meta_next schedule table) moved -- check_required_maxd_for_N, probe_partial_load_memory and similar diagnostics stay here since moving them would require moving core data structures too or a circular import. No function body was changed during the move, only relocated and call sites re-prefixed with rev386_validation_helpers. Legacy bench_mode/parameter cleanup was explicitly NOT done in this revision -- flagged as a separate next step requiring confirmation on which branches are still in active use before anything is removed."
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31 or bench_mode==32 or bench_mode==33 or bench_mode==34 or bench_mode==35 or bench_mode==36 or bench_mode==37):
    ...
    if bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31 or bench_mode==32 or bench_mode==33 or bench_mode==34 or bench_mode==35 or bench_mode==36 or bench_mode==37:
