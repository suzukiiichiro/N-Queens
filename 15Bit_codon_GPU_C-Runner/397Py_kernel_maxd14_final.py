...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 397 (2026-09-09)
...
"""
了解しました。(a)容量説を先に検証します。

その前に、この実験の設計で1点だけ詰めさせてください。**単純に3ワードへ詰め替えると、アクセス回数が2回から3回に増えます。** それでは (a) と (b) が交絡します。バイト数は減るがリクエスト数は増えるので、遅くなっても原因が分かりません。

**アクセス回数を2回のまま、バイト数だけ25%減らす**構成にします。

```
word A (u64): ld(21) | rd(21)<<21 | col(21)<<42     ← 63ビット
word B (u32): avail | depth<<27                     ← 現行のまま
```

16 B → 12 B、アクセスは2回のまま。セクタ数は push/pop それぞれ 16 → 12 に減ります。**これなら「ローカルメモリのバイト量が効くか」だけを単独で測れます。** 時間が比例して改善すれば (a)、変わらなければ (b) です。

ソースを確認します。397の四点セットです。静的ゲートは **OK=19 FAIL=0**。

## カーネルが変わります

**395a r2以来、初めてです。** sha256 は `ebd7f523…` → `df7c46c6a10be0d3…`。ハーネスは「変わったこと」と「変わった箇所が意図した行だけであること」の両方をゲートします（`396_r2` からのコード領域差分 **removed=18 added=58**：宣言、オーバーフロー判定2箇所、push 2箇所、pop 1箇所、マスク定数、Nガード2箇所）。
...
16 B → 12 B、**ローカルアクセスは push 2回 / pop 2回のまま**。セクタ数は 16 → 12。3ワードに分ければバイトは減ってもアクセスが3回になり、遅くなったとき原因が分かりません。そこを避けてあります。

## 通常より厳しいゲート

この詰め方は **ld と rd の 2^21 以上のビットを落とします**。「全使用箇所が board_mask で覆っているので N≤21 では死にビット」というのが根拠ですが、**これは推論です。この案件は推論を信用しません。**

**タイミングを1つも読む前に**、両バイナリでフル入力（2,025,282レコード）を走らせ、per-record出力を**バイト単位で比較**します。一致しなければ停止し、数値は一切報告しません。

さらにV1として、ptxasでフレームが実際に縮んだかを**GPU時間を使う前に**確認します。208 → 140〜168 B に入らなければそこで止まります。

## 事前登録 — V3で私は「棄却」に賭けます

**(a)が正しければ `Gn ≤ Gb − 1.0%`。私の表明する予測は `|Gn − Gb| ≤ 0.3%`、つまり(a)は棄却される、です。**

根拠は 320スレッド/SM × 208 B = 66.6 KB で、L1の128 KBに収まっていること。容量が溢れているとは考えにくい。ただし**確信は強くありません**。だから走らせる価値があります——どちらに転んでも398の行き先が決まります。

- **(a)確認** → 398はさらに量を削る。フレームが小さくなった今なら BLOCK サイズの再検討も意味を持ちます（現在 BLOCK=32 が理論オカパンシーを33%に縛っています）
- **(a)棄却** → 398は(b)へ。**代表的なN=20負荷で `l1tex` のリクエスト対セクタ比**を測ってから手を動かします

## 記録した限界

21ビット×3で64ビットの63を使い切るので、**この詰め方は N≤21 専用**です。両方の `main()` にガードを入れ、ハーネスがN=22で実際に拒否されることを実行時に確認します。

**N=22/23 の maxd16 には別のレイアウトが要ります。397は実験であって、勝ってもそのまま将来へ持ち越せる形ではありません。**

```
STATIC_ONLY=1 bash 397_validate.sh   # OK=19
              bash 397_validate.sh   # V1(数秒) → E0 E1(等価性) → Gb1 Gn1 Gb2 Gn2、約20分
```

`396_r2_kernel_maxd14.cu` と `396_r2Py_kernel_maxd14_final.py` が残っていれば、バイナリは無くてもハーネスがビルドします。

記憶に頼らず、ソースで確認します。ご指摘ありがとうございます。**ソースを確認したところ、私が間違っていました。** READMEに明記されています（rev372の記述）。

> **実務上の疑問（N=22のカーネル対応可否）は独立に解決済み**: N=22も `required_maxd=14` でN=21と同一（366で3/3安定確認）、maxd16/18/20/21への追加移植は不要。

`select_static_maxd()` は `required_maxd <= 14` なら 14 を返します。**N=22 は maxd14 で収まります。** 366で3回とも安定確認済みです。

つまり「maxd14では収まらなかった」というご記憶も、「N=22/23はmaxd16が要る」という私の発言も、どちらも誤りでした。maxd16/18/20/21のカーネルは実装されていますが、N=22のために必要なのではありません。

なお **N=23 の `required_maxd` は、READMEに記載がありません。** N=22までしか確認されていないので、N=23が14で収まるかは未検証です。

## これは397にとって重大です

私は「21ビット詰めのN≤21制限は、将来のmaxd16作業の話だから遠い」と書きました。**逆でした。**

> **N=22 は maxd14 を使う。したがって397の詰め方は、次の目標そのものを塞ぎます。**

N=22 では board_mask が22ビット。22×3 = 66ビットで、64ビットに入りません。397のバイナリは N=22 を実行時に拒否します。

## 修正の選択肢

**(1) 397は実験のまま、N≤21で走らせる。** 目的は(a)と(b)の判別であり、測定対象はN=21です。(a)が勝ったら、398で N≤22 対応のレイアウトを実装する。

12バイト＝96ビットに 4フィールド×Nビット + depth を収めるので、`4N + 5 ≤ 96` → **N ≤ 22 まではワード跨ぎの分割で実装可能**です。たとえば

```
word A (u64): ld(22) | rd(22)<<22 | col_low(20)<<44        = 64ビット丁度
word B (u32): col_high(2) | avail(22)<<2 | depth(5)<<24    = 29ビット
```

アクセス回数は2回のまま、12バイトのまま。演算が数命令増えますが、Compute利用率19.41%なので誤差でしょう。**N=23 は 4×23+5 = 97 > 96 で入りません。**

**(2) 最初からN≤22対応で実装する。** 判別実験としての価値は同じで、勝った場合にそのまま使えます。代わりにビット演算が複雑になり、等価性の検証リスクがやや上がります。

**私は(1)を推します。** いま知りたいのは「ローカルメモリのバイト量が効くか」という一点で、そのためにコードを複雑にする理由がありません。(a)が棄却されれば、22ビット対応版を書く必要そのものが消えます。

ただし**採用の可否が変わります**。397は勝っても**そのままでは本番に入れられません**——N=22を塞ぐので。398でレイアウトを組み直してから採用、という順序になります。

## 文書を直します

`.cu` ヘッダ、Pyノート、READMEの3箇所に「N=22/23はmaxd16」という誤った記述が入っています。将来これを読んだ人（私を含む）を誤らせるので、直したものを出し直させてください。

その際、(1)と(2)のどちらで進めるかだけ、ご判断をいただけますか。**(1)なら文言修正だけ、(2)ならカーネルも書き直し**になります。

**V2ハードゲートが発火しました。** 設計どおりの止まり方です。

```
E0  396_r2  total_sum=314666222712  MATCH
E1  397     total_sum=427786151034  ← 不一致
```

**私の「2^21以上のビットは死んでいる」という推論が誤りでした。** 解が多く出ている（427.8億 対 314.7億）ので、マスクによって制約が落ちています。**タイミングは1つも読んでいません。** 397は破棄です。

## 得られたもの

**V1は成立**しました。208 → **160バイト**（−23%）、spill 0/0、レジスタ 37 → 38。詰め替え自体は効いています（156を予測、アラインメントで160）。

**V4も成立**しています。ハーネスはE1の失敗で先に抜けたので判定は出ていませんが、`E0 = 133,155.344`（`NQ_EXTRA_CTX=2`、`free_mb` 21762）は基準 133,185 から **−0.022%**。**ディスパッチャなしで本番状態を再現できます。**以後の測定が1回2.5分短くなります。

Nガードも実行時に正しく効いています。

どのフィールドが悪いのか、推測せずソースで特定します。## どのフィールドが原因か

全出現箇所を洗いました。

```
cur_ld :  << のみ    （611, 614, 733, 747 — すべて左シフト）
cur_rd :  >> のみ    （612, 615, 734, 748 — すべて右シフト）
```

**cur_rd は右シフトしかしません。** つまり 2^21 以上のビットは、階層を降りるにつれて**盤面範囲へ降りてきます**。マスクで落とせば、後で効くはずの制約が消える。解が増えたことと符合します。

一方 cur_ld は左シフトのみで、上へ逃げたビットは全使用箇所の `bm & ~(...)` で覆われます。**ld のマスクは安全、rd のマスクが致命的**、というのが機構です。
...
python3 -c "
import struct,sys
f=open('constellations_N21_6.bin.soa_ref_361.bin.maxd14only_363.bin.sched394f.bin','rb')
mx=[0]*7
while True:
    b=f.read(28)
    if len(b)<28: break
    v=struct.unpack('<7I',b)
    for i in range(7): mx[i]=max(mx[i],v[i])
print('field max:', [hex(x) for x in mx])
print('bits     :', [x.bit_length() for x in mx])
"
```

2番目（rd）のビット幅が21を超えていれば確定です。

## 12バイト詰めは、そもそも成立しないかもしれません

正しい幅を積み上げるとこうなります（N=21）。

| フィールド | 必要幅 | 根拠 |
|---|---:|---|
| ld | 20 bit | 左シフトのみ。bit 20 は1回シフトで21へ抜け `bm` 外 |
| **rd** | **32 bit** | **右シフトで上位が降りてくる。切り詰め不可** |
| col | 21 bit | `bm & ~(…\|ncol)` で直接使用 |
| avail | 21 bit | 復元時に `& bm` |
| depth | 4 bit | 最大13 |
| **合計** | **98 bit** | |

12バイト = **96ビット**。**2ビット足りません。**

深さに応じて rd の幅を変える（スロット i では rd は 32−i ビット以下）といった小細工は可能ですが、複雑さと検証コストに見合いません。

**つまり「フレームを縮めて仮説(a)を検証する」という道自体が、この構造では塞がっている可能性が高いです。**

## 順序を間違えました

私は測定より先に手を動かしました。この案件でずっと避けてきたことです。

**本来やるべきだったのは(b)の直接測定です。** `l1tex` のリクエスト対セクタ比を見れば、

- **セクタ/リクエスト ≈ 1**（よく合体している）→ 発散は問題でなく、量が効いている。そこで初めて詰め方を工夫する価値が出る
- **セクタ/リクエストが大きい** → (b)確定。(a)は無関係になり、12バイト詰めに悩む必要そのものが消える

しかもこの測定は、**まだ一度も答えの出ていないU5（支配的ストール理由）も同時に片付けます**。

## 398の提案

**396-r3としてお預けにしていた「N=20で測る」を、いま実行します。**

- N=20 は約16.5秒。ncuは1.5秒で通り191秒で落ちるという実測があるので、載る見込みが立っています
- セクション優先順: **WarpStateStats**（U5）→ **MemoryWorkloadAnalysis**（(a)/(b)判別）→ SpeedOfLight → Occupancy / SchedulerStats
- 代表性は「N=19とN=20で順位表が一致すること」で担保。加えて Occupancy と SchedulerStats は**N=21の実測値を持っている**ので、3点でN依存性を直接検証できます
- V4が成立したので、アンカーは `NQ_EXTRA_CTX=2` の直接実行で取れます（1回2.5分の節約）

397は破棄しますが、拾えるものが3つあります。**フレームは実際に208→160まで縮む**こと、**ld と rd の非対称性**（左シフト/右シフトの構造的な違い）、そして **V4の確立**です。

## 併せて、文書の誤りを直します

`.cu` ヘッダ、Pyノート、READMEに「N=22/23はmaxd16」と書いてしまっています。**N=22は `required_maxd=14` で maxd14 に収まる**（366で3/3確認済み）が正しい。N=23は未検証、が正確なところです。398の四点セットに正しい記述で入れます。
...
# =====================================================================
# 397 リビジョンノート  (2026-09-09)
# ---------------------------------------------------------------------
# 【395a r2以来、初のカーネル変更】
# DFSスタックのフレームを16バイトから12バイトに詰め替える。アルゴリズム
# は動かさない。カーネル領域のsha256は当然変わる (ebd7f523... ではなく
# なる)。ハーネスは「変わったこと」と「変わった箇所がスタックの行だけで
# あること」の両方をゲートする。
#
# 【396が確定させたこと】
#   ptxas: 208 bytes stack frame, spill 0/0, 37 registers
#     → フレームはレジスタ溢れではなく、宣言された配列
#       uint64_t stack[MAXD14_ANCESTOR*2] そのもの。動的添字なので
#       ローカルメモリに置かれる。
#   Occupancy (実N=21): 理論 33.33% / 達成 10.32% / 4.95 warps per SM
#     BLOCK=32 は 1ブロック=1ワープで、sm_86 のブロック数上限16と
#     掛かって理論値が33%で頭打ち。レジスタは48ブロックぶん余っており、
#     制約になっていない。
#   Scheduler: No Eligible 61.63% / Active Warps Per Scheduler 2.41 (上限12)
#   SpeedOfLight (1ラウンド代理・非代表): L1/TEX 52.96%, L2 46.73%,
#     DRAM 16.09%, Compute 19.41% → メモリパイプが演算の2.7倍
#
# 【397が検証する仮説はひとつだけ】
#   L1圧の説明候補は2つ残っている:
...
#   397は(a)だけを叩く。フレームのバイト数を25%減らし、**ローカルアクセス
#   回数は push 2回 / pop 2回 のまま変えない**。リクエスト数と発散は不変
#   なので、時間が比例して改善すれば(a)、動かなければ(a)は棄却され、
#   次のリビジョンが(b)に向かう。どちらに転んでも情報が出る設計である。
#
# 【詰め方】
#   word A (uint64_t): ld(21) | rd(21)<<21 | col(21)<<42     = 63ビット
#   word B (uint32_t): avail | depth<<27                      = 現行のまま
#   16 B → 12 B。push/pop のセクタ数は 16 → 12。
#   ld / rd の 2^21 以上のビットは push 時に落ちる。全使用箇所が
#   board_mask で覆っているため N<=21 では死にビットである、というのが
#   根拠だが、これは信用せず**全2,025,282レコードのバイト一致**で検証する。
#
# 【限界 — 意図的に記録する】
#   21ビット×3で64ビットの63を使い切るので、この詰め方は N<=21 専用。
#   main() は N>21 を拒否する。N=22/23 の maxd16 には別のレイアウトが要る。
#   **397は実験であって、勝っても そのまま将来へ持ち越せる形ではない。**
#
# 【事前登録予測 — 実測前に固定】
#   V1 ptxas のスタックフレームが 208 → 140〜168 バイト、spill 0/0、
#      レジスタ 42本以下。外れたら詰め替えが効いていないので即停止。
#   V2 396-r2 と 397 の per-record 出力が、フル入力で**バイト一致**し、
#      両者とも total_sum=314666222712 MATCH。**これが落ちたら
#      「2^21以上は死にビット」という推論が誤りで、時間は一切読まない。**
#   V3 (a)が正しければ Gn <= Gb - 1.0%。
#      **私の予測は |Gn - Gb| <= 0.3%、すなわち(a)は棄却される。**
#      根拠: 320スレッド/SM × 208 B = 66.6 KB で L1 128 KB に収まって
#      おり、容量が溢れているとは考えにくい。ただし確信は強くない。
#   V4 直接実行 + NQ_EXTRA_CTX=2 (=3コンテキスト, free_mb 21763) が
#      133,18x の ±0.15% に入る。ディスパッチャなしで本番状態を再現できる
#      ことの確認であり、以後の測定を2.5分短くする。
# =====================================================================
...
VERSION_TAG:str="397 PY-REVTAG: pure rename of 396-r2Py; the dispatch table names ./397_kernel_maxd14 and keeps the env_prefix NQ_EXTRA_CTX=1 adopted in 395c-r6. The .cu is NOT a rename this time: 397 is the first kernel change since 395a r2. It repacks the DFS stack frame from 16 bytes to 12 while holding the number of local-memory accesses at two per push and two per pop, so that local-memory byte VOLUME is tested separately from request count and divergence. Equivalence is gated by a byte-for-byte comparison of the per-record output of 396-r2 and 397 over all 2,025,282 records, run before any timing is read. The packing holds three 21-bit fields in one uint64_t, so it caps N at 21 and main refuses larger N."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./397_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
