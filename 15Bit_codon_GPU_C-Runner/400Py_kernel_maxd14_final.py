...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 400 (2026-09-09)
...
"""
400の四点セットです。静的ゲートは **OK=16 FAIL=0**。

## 変更は `NQ_BLOCK` 1つだけ

`BLOCK` はカーネルには `blockDim.x` と起動構成としてしか届きません。したがって**カーネル領域の sha256 は `ebd7f523…` のまま**です。`.cu` のコード領域差分は removed=1 added=30、ptxas も37レジスタ・208Bフレームのままであることをゲートします。

**`MAX_BLOCKS` は394b/394gで掃引済みですが、`BLOCK` は一度も動かされていません。** そして天井を押さえていたのはそちらでした。

## 掃引の格子

| BLOCK | MAX_BLOCKS | stride | warps/SM |
|---:|---:|---:|---:|
| 32 | 800 | 25,600 | 10 ← 現行 |
| 64 / 128 | 400 / 200 | 25,600 | 10 / 8（**梱包対照**） |
| 32 | 1,280 | 40,960 | 16（BLOCK=32の天井） |
| 64 | 1,280 | 81,920 | 32 |
| **128** | **960** | 122,880 | **48（100%）** |
| 256 | 480 | 122,880 | 48 |

上段の3つは stride を25,600に固定した対照です。warps/SM が同じなので、**「形だけ変えた影響」を本体から分離できます**。

N=20で1点16秒・3反復。399-r2で実測したノイズフロアは0.003〜0.05%なので、**極めて安く、十分に鋭い測定**です。測定は `NQ_EXTRA_CTX=2`——397のV4で確立した、ディスパッチャなしで本番の3コンテキスト状態を再現する方法を使います。

## 事前登録

**Z3が本命です。32 warps/SM 以上の構成のうち少なくとも1つがベースラインより5%以上速い、と表明します。**

**1%を超えて勝つ構成が皆無なら、オカパンシーは梃子ではなく、`wait` 45%という読み自体をやり直します。** その判定もハーネスが出します。

**Z4を必ず通します。** 時間が縮んだだけでは採用しません。**Achieved Occupancy が実際に上がり、`wait` のストール占有率が実際に下がった**ことをncuで確認します。結果と機構が一致しなければ、何か別のものを測っています。

## N=22について

**起動構成はNに依らない軸なので、効くならN=22にも効くはずです。** 395c-r6の413 ms は `free_mb` 21762 の狭い切り欠きに依存していたのでN=22に持ち越せませんが、**こちらは持ち越せる見込みがあります**。

`WITH_N22=1` で確認セルが走ります。ただし1構成あたり約19分なので各1反復、参考値扱いです。
...
全走行がオラクルゲート下です。BLOCKやMAX_BLOCKSを変えても同じレコードをスレッドへ再分割するだけなので、答えは動いてはなりません。
...
VERSION_TAG:str="400 PY-REVTAG: pure rename of 399-r2Py; the table names ./400_kernel_maxd14 and keeps env_prefix NQ_EXTRA_CTX=1 and CRUNNER_MIN_N=19. The .cu adds ONE host-side knob, NQ_BLOCK, leaving the kernel region at the 395a r2 sha256 ebd7f523. Rationale: 399-r2 finally obtained the stall breakdown and wait (fixed-latency dependency) is 45 percent of stall cycles against 14 percent for memory, with 2.43 of 12 warp slots per scheduler filled. BLOCK=32 makes a block one warp, so the sm_86 limit of 16 blocks per SM caps theoretical occupancy at 33.3 percent whatever the grid size, and the production grid reaches only 10 of those 16. MAX_BLOCKS has been swept before; BLOCK never has."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./400_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
