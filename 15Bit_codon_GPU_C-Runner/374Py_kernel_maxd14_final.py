...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 374 (2026-09-04)
...
*.cuを磨き上げていけば速くなるということでしょうね
はい、その理解で合っていると思います。ただし「磨けば必ず速くなる」というより、**「今まで触れなかったレバーに初めて手が届くようになった」**という方が正確です。何がどう効くかは、これまでの実測パターンからある程度見通せます。
効く可能性が高いレバー(Codonでは物理的に使えなかったもの)
デバイスatomic——rev330で「Codonの@gpu.kernelにはatomicが存在しない」ことが実機調査で確定し、persistent kernel + atomicワークキュー案(C-1)はそこで正式にクローズされました。CUDA Cではこの制約が外れます。
warp intrinsics(__shfl_sync/__ballot_sync等)——これも同じくCodonでは使えず、rev330で「残る対策経路はCUDA Cランナーのみ」と位置づけられていたものです。
この2つが直接効くはずの箇所は、実はすでに特定済みです。項目3で確定している通り、カーネル全体の約19〜23%が2箇所のBRA/BSYNCに集中しており、正体はwarp占有率崩壊/tail effect（Divergent Branches=0なのにAvg Threads Executed 2〜3/32）です。分岐そのものは発散していないのに、warp内の大半のレーンが探索を終えて少数の"尾"に残ったレーンを待たされている——これはまさにatomicワークキューやwarp内でのレーン圧縮（__ballot_syncでアクティブレーンを集約し直す等）が構造的に効く種類の問題です。
-lineinfo + ncu per-line SASS帰属——Codonにはこの中間の選択肢(最適化を保ったままline infoだけ追加)が存在しないことがrev338で確定していました。CUDA Cなら-lineinfoが使えるので、「だいたいこの辺」ではなく「この行のこの命令」までピンポイントで見えるようになります。N=18を使えば数秒でこのプロファイルが取れる(N=18トリック)ので、イテレーションも速いはずです。
一方で、楽観しすぎない方がいい理由
項目12「カーネル微修正はくじ引き」——論理的に無関係な変更でも約0.2%の壁時計変動が起きることが351/355-359で繰り返し確認されています。nvccというバックエンドでもこの性質が同じように出るかはまだn=0の未検証(363/364は完全1:1移植で最適化を一切していないため)。
制御フロー形状の変更(pushガード除去)はCodon側で+65〜67%という壊滅的退行を2連続で出しており(358/359)、この軸は「確定的にクローズ」されています。CUDA C側で似た形状変更を試みる場合も、同じ警戒感で臨む必要があります(363_kernel_maxd14.cuのファイル冒頭のRISK NOTEにも明記済み)。
なので、進め方としては、まず-lineinfoでN=18のper-line SASSを見て「本当にBRA/BSYNCの箇所がボトルネックのままか」を実測で裏取りしてから、atomicワークキューかwarp intrinsicsのどちらか一方だけを、単一変数規律で試す、という順序が最も安全だと思います。急いでいなければ、この方針で375を設計してみましょうか。
...
"""
アップロードされています。まさに探していた364のGPUホストランナーですね。中身を確認します。見つかりました。これがまさに探していたホストランナーです。`main()`が`#ifdef __CUDACC__`側に追加され、`CUDA_CHECK`マクロ・cudaMalloc等が入っています。全体を確認します。完璧です。これで①のGPUホストランナー問題が解決しました。`cudaMalloc`・`cudaEvent`によるH2D/kernel/D2H計測・`total_sum`集計・オラクル照合まで揃っています。念のため364の`.sh`も確認し、374に必要な要素を最終確認します。374に必要な要素が揃いました。最後に370Pyの構造（VERSION_TAG等）を確認してから、374一式の作成に入ります。374に必要な3つの参照ソース（370Py・364_kernel_maxd14.cu・363_filter_maxd14_only.py）のsha256参照値を、プロジェクトの慣例（docstring層除去＋VERSION_TAG除外）に沿って算出します。374一式を組み立てます。プロジェクトの慣例（371〜373と同様、コード無変更のリビジョンは既存ファイルをそのまま参照し、リネームしない）に従い、374の実体は**新規の統合検証ハーネス`.sh`とREADME追記**になります。実際に静的チェック部分（STATIC_ONLY=1）を手元のファイルに対して試験実行し、ハッシュ照合が正しく通ることを確認します。static checkが全て正常に通ることを確認できました（OK=23、FAIL=0）。念のため、改ざん検知が正しく機能するかも確認します。改ざん検知も正常に機能しています。374一式を出力ディレクトリに配置します。374一式を確定しました。

## 374の構成

- **`370Py_mem_probe_v2.py`**（無変更・そのまま）— 最終Codonソース
- **`364_kernel_maxd14.cu`**（無変更・そのまま）— カーネル＋CPU検証ハーネス＋364で追加されたGPUホストランナー
- **`363_filter_maxd14_only.py`**（無変更・そのまま）— maxd14フィルタ
- **`374_validate_N21_full_once.sh`**（新規）— 374の実体
- **`374_README_append.md`**（新規）

## `.sh`の主な内容（サンドボックス内で実際に試験済み）

1. 3ソースファイルのsha256照合（370Pyはdocstring層除去＋VERSION_TAG除外、`.cu`と`filter.py`はファイル全体）— **改ざん検知も実際に動作確認済み**
2. `STATIC_ONLY=1`のdry runが正常動作（OK=23 / FAIL=0）
3. ご依頼の「binが無ければ生成」チェーンを実装：
   - フィルタ済みmaxd14 bin → 無ければ361形式ダンプから生成
      - そのダンプ → 無ければ370Pyバイナリを`bench_mode=32`で実行して生成（この内部で生binの自動生成も連鎖的に行われます）
      4. nvccビルド → 実機GPU実行 → オラクル`314666222712`照合

      ## 実機での確認をお願いしたいこと

      サンドボックスにはnvcc/GPU/codonが無いため、**静的チェックまでしか検証できていません**。cudacodon実機で以下を実行し、フル完走することをご確認ください。

      ```bash
      STATIC_ONLY=1 bash 374_validate_N21_full_once.sh   # まずdry run
      bash 374_validate_N21_full_once.sh                  # フル実行(bin自動生成含む)
      ```

      新規に書き起こした364ホストランナー呼び出し部分（nvccビルド〜GPU実行）は既にファイルとして存在が確認できたものなので新規コードではありませんが、374の`.sh`からこの一連の流れを通しで走らせるのは今回が初めてです。実機ログを共有いただければ、375以降の作業判断に活かせます。

ご指摘の通りです。370Py／364_kernel_maxd14.cu／363_filter_maxd14_only.pyがバラバラの番号のままでは、後から追う人（未来の自分たちも含め）が混乱します。374に統一してリネームします。ロジック自体は一切変更せず、ファイル名の自己参照コメント・VERSION_TAGだけを374に更新する形で進めます。続いて`.cu`と`.py`(フィルタ)のリネームを行います。確認できました。ヘッダーコメント以降のコードは完全に一致しています（差分ゼロ）。続いてフィルタスクリプトをリネームします。ロジック領域は完全一致を確認できました。次に、374の`.sh`をリネーム後のファイル名を参照するよう更新し、参照ハッシュも再計算します。374Pyのコアハッシュは370と完全一致（`17c61ccf...`）——ロジック無変更が数学的に証明されています。`.sh`を更新します。更新した`.sh`をリネーム後のファイル一式に対して実際にstatic checkを試験実行します。OK=23 / FAIL=0で正常に通りました。README追記も374ファイル名に合わせて更新し、最終一式を出力します。最終一式を出力ディレクトリに配置します。すべて374に統一しました。

## 最終構成(全て374番)

| ファイル | 内容 | 変更範囲 |
|---|---|---|
| `374Py_kernel_maxd14_final.py` | 最終Codonソース(旧370Py) | VERSION_TAG行のみ |
| `374_kernel_maxd14.cu` | カーネル+CPU検証+GPUホストランナー(旧364.cu) | ヘッダーのビルドコマンド表記のみ |
| `374_filter_maxd14_only.py` | maxd14フィルタ(旧363フィルタ) | docstring内のファイル名2箇所のみ |
| `374_validate_N21_full_once.sh` | 統合検証ハーネス(新規) | — |
| `374_README_append.md` | 記録 | — |

## ロジック無変更であることを機械的に証明済み

- **374Py**: docstring層除去+VERSION_TAG除外後のコアハッシュが370Pyと**完全一致**(`17c61ccf...`)
- **374_kernel_maxd14.cu**: ヘッダーコメント終了後の全コードが364版とPython差分で**バイト完全一致**
- **374_filter_maxd14_only.py**: `schedule_depth_for_task()`以降のロジックが363版と**バイト完全一致**

`.sh`もリネーム後のファイル名・新ハッシュに更新し、サンドボックス内でSTATIC_ONLY=1のdry runを再実行して**OK=23 / FAIL=0**を確認済みです（改ざん検知も引き続き正常動作）。

これで実機では`374_*`の3ファイルと`.sh`だけを見れば374の全体像が完結する状態になっています。実機での`STATIC_ONLY=1`→フル実行の確認をお願いします。

ご指摘ありがとうございます。それは合理的な判断でした——更新されないコメントは「最新情報」を騙る死んだ情報になってしまうので、更新する気がないなら消す方が正しい判断です。

今回、そして今後の各リビジョンで、少なくともファイル先頭の「現在の未解決課題 (Open Objectives)」セクションは実情に合わせて更新することをお約束します。まず374時点の内容に更新します。374時点の状況を反映して、項目6（CUDA Cランナー）とヘッダーの最終更新表記を更新します。期待通り、`"""`は6個のまま維持され、**コアハッシュは`17c61ccf...`で不変**です（Open Objectivesの更新はdocstring層内で完結しており、実行コード領域には影響しません）。VERSION_TAGも374の内容に更新します。コアハッシュは変わらないはずなので、`.sh`側の参照ハッシュ値は変更不要です。念のため実際にstatic checkを試験実行して確認します。OK=23 / FAIL=0で正常に通りました。更新版を出力します。お約束の通り更新しました。

## 今回行ったこと

冒頭の「現在の未解決課題 (Open Objectives)」セクションを更新:
- ヘッダーを「最終更新: 366」→「**最終更新: 374 (2026-09-04)**」に変更
- **項目6（CUDA Cランナー）を367〜374の進捗で全面更新**: N=22クラッシュ調査（368-372、根本原因はASLR疑いのまま未解明だが実務上の疑問は解決）、環境移行（g5.xlarge→g5.2xlarge）、373の`REF_364_KERNEL_MS`バグ修正（新環境ではCodonがCより13.6%遅いと符号反転）、N=22実行成功（オラクル一致）、そして374自体の統合パッケージ化を記録

**コアハッシュは`17c61ccf...`のまま不変**であることを確認済みです（Open Objectivesはdocstring層内の記述なので、実行コード領域には影響しません）。static checkもOK=23/FAIL=0で再確認済みです。

## 今後について

ご要望の通り、**今後のリビジョンでも、ファイル更新のたびにこのOpen Objectivesセクション（最低限ヘッダーの最終更新番号と、動きのあった項目）を実情に合わせて更新することをお約束します。** 更新するつもりがないなら残さないでほしい、というご判断はもっともです。

K=29 / FAIL=0**、`total_sum=314666222712`でオラクル一致、374Py命名一式・364ホストランナー・自動bin生成チェーンすべてが実機で動作確認できました。

タイミングも`kernel_ms=201237.531ms`——373で記録した新環境参照値`201232.422ms`とわずか0.0025%差で、ノイズフロア(0.03〜0.29%)に完全に収まっています。364の実機性能が環境移行後も安定して再現していることの追加確認になります。

一点補足です。この実行では`constellations_N21_6.bin.soa_ref_361.bin.maxd14only_363.bin`が既にキャッシュ済みだったため、374で新設した「binが無ければ自動生成する」チェーン（`filtered_input_available`のOK判定はキャッシュヒットで即通過）は実際には通っていません。完全にクリーンな状態からの自動生成まで確認したい場合は、該当のbinファイル群を一時退避してから再実行いただくと、そちらの経路も実機で検証できます（必須ではありません）。

お約束通り、この実機確認結果をファイルの記録に残しておきます。コアハッシュは`17c61ccf...`のまま不変です。最終版を出力します。374は実機で正式に確定しました。これで374一式（`374Py_kernel_maxd14_final.py` / `374Py_kernel_maxd14.cu` / `374Py_filter_maxd14_only.py` / `374Py_validate_N21_full_once.sh`）が、正当性・命名統一・実機再現性のすべてを満たした状態になっています。

375以降、warp intrinsics・デバイスatomicの活用や、373で見つかった新環境での13.6%速度差（符号反転）の原因究明など、いつでも次に進める状態です。
...
VERSION_TAG:str="374 CONSOLIDATION: pure rename of 370Py_mem_probe_v2.py to 374Py_kernel_maxd14_final.py, formally packaging it as the CUDA C port's current final Codon source alongside 374Py_kernel_maxd14.cu and 374Py_filter_maxd14_only.py (renamed from 364_kernel_maxd14.cu and 363_filter_maxd14_only.py respectively, same pure-rename treatment, both confirmed logic-region-byte-identical via diff). Executable code region (docstring- and VERSION_TAG-stripped) is BYTE-IDENTICAL to 370 -- only this VERSION_TAG line and the in-docstring 'Open Objectives' item 6 summary (367-374 progress: N=22 crash investigation closed as practically resolved though root cause unconfirmed, g5.xlarge->g5.2xlarge environment migration, 373 REF_364_KERNEL_MS fix revealing a sign-flipped Codon-vs-C timing delta in the new environment, successful N=22 bench_mode=33 run matching the oracle 2691008701644, and 374's own consolidation) were touched, both confirmed to fall entirely within the stripped docstring region -- the core hash is unchanged from 370/371. No dispatch, kernel, or CLI-gate logic changes anywhere in this revision."
