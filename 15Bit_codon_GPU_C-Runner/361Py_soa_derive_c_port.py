...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 361 (2026-08-25)
...
6. [進行中・361で最初の実装(SoA導出関数のCポート)] CUDA Cランナー
   (案C-2)スパイク設計
   ...
   **334-337で土台が全て揃った(確定)**: nvcc=`/usr/local/cuda/bin/nvcc`
   ...
   450.056sで再現済み。
   **338(設計)→360(設計、338への差分パッチ)で仕様が確定した**:
   SoA配列は7本(ld_arr/rd_arr/col_arr/ctrl0_arr/free_arr/markctrl_arr/
   w_lo_arr)、`save_sp`はC側`uint32_t`・`stack_ptr`は`int`のまま、
   ±3%判定基準の分母は356の393.404秒(chunk0/1/2=144,590/144,473/
   103,271ms)。詳細は`338_maxd14_port_spec.md`と
   `360_maxd14_port_spec_update.md`を参照。

   **361は仕様確定後、最初の実装リビジョン**。単一変数規律に従い、
   スコープを`build_soa_for_range`+`symmetry`(bin→SoA導出、28分岐の
   funcid決定木)のCポートのみに限定し、カーネル本体
   (`kernel_dfs_iter_gpu_maxd14`)には一切着手しない。実装方針:
   (1)新規ホスト専用Cファイル`361_soa_derive.c`(CUDAコードなし、GPU
   起動なし)に、Codon側`build_soa_for_range`/`symmetry`/`symmetry90`/
   `geti`/`getj`/`getk`/`getl`を1対1で移植、(2)実ファイルの生bin
   (337が検証済みの`ensure_constellations_bin_stream`が解決する
   stream bin、並べ替えパイプライン適用前)を両側(C/Codon)で同一順序に
   読み、同一のper-record 10フィールド(ld/rd/col/row/ctrl0/free/
   markctrl/funcid/ijkl/w_lo、各u32 LE)をフラットバイナリへダンプ、
   (3)N=21フル(2,025,282レコード全件)でバイト単位diffし、0差分を
   要求(速度目的の移植ではなく振る舞い等価性の移植のため、±3%のような
   許容幅ではなく完全一致を判定基準とする)。Codon側の追加は
   `dump_soa_reference_c_port`という新規関数1個と新規`bench_mode==32`
   分岐1個のみで、いずれも既存コードへの追記であり、カーネル・
   ディスパッチャ・並べ替えパイプライン・既存bench_modeは一切変更
   しない(361Pyの`.sh`が2箇所の追記範囲を明示的なマーカーで区切って
   除外した残り全域を356/360とのハッシュ照合で検証する)。
...
361を用意しました(CUDA Cランナー移植の最初の実装リビジョン)。

**r2(実機での不具合修正版)**: 最初に提示した版は、CLI引数パース内に
既存の`bench_mode`ホワイトリストゲート(276 restore274/coretrimで
導入、`{0,11,28,29,30,31}`のみ許可)があることを見落としていた。この
ゲートが新設の`bench_mode==32`を実行前に黙って0へリセットしてしまい、
新設のディスパッチ分岐に到達しなかった。実機での初回実行は正当性
314666222712を確認した通常のN=21フルラン(約11分22秒)を走らせてしまい、
soa-ref-dumpは一切出力されなかった(GPU時間を無駄にしてしまい申し訳
ありません)。r2ではこのホワイトリストと、隣接する`preset_queens`
ゲート条件の2箇所に`or bench_mode==32`を追加し(いずれも2語のみの
最小修正、明示マーカーで区切って文書化)、`.sh`の静的チェックもこの
2箇所の修正を検出・検証するよう更新した。ローカルでのシミュレーションで
`bench_mode=32`が最後まで生き残り新設分岐に到達することを確認済み。

**前回(360)までの状態**
- 356/357/360で確定・再現済みのアンカー: 393.404秒
  (chunk0/1/2=144,590/144,473/103,271ms)
- 358・359でpushガード軸(制御フロー形状変更)が確定的にクローズ、
  残る対策経路は項目6(CUDA Cランナー)のみ
- 338(設計)→360(設計、338への差分パッチ)で移植仕様が確定
  (SoA配列7本、save_sp=uint32_t、stack_ptr=int、判定分母=393.404秒)

**361の内容(実装、単一変数規律によりスコープを限定)**
- 新規ファイル`361_soa_derive.c`: `build_soa_for_range`+`symmetry`
  (bin→SoA導出、28分岐のfuncid決定木)のみをCへ1対1移植。カーネル
  本体・K-batching・スタックには一切着手しない
- 361Pyへの追記(既存コードは無変更、2箇所の追記のみ):
  1. `dump_soa_reference_c_port()`: 実ファイルの生bin(並べ替え前、
     337が検証済みの`ensure_constellations_bin_stream`が解決する
     stream bin)を全件読み、既存の`build_soa_for_range`を無改造で
     呼び出し、10フィールド/レコードのフラットバイナリ参照ダンプを
     出力(`build_soa_for_range`定義直後に挿入)
  2. `bench_mode==32`: 上記関数を呼ぶだけの新規ディスパッチ分岐
     (mode30/31ブロックの直後に挿入)
- `.sh`は2箇所の追記範囲を明示マーカー(`===361-INSERT-BEGIN/END===`,
  `===361-DISPATCH-INSERT-BEGIN/END===`)で区切って除外した残り全域を
  356/360の参照ハッシュ(`793eef69...`)と照合し、既存コードが一切
  変更されていないことを検証する
...
し、362以降でカーネル本体(`kernel_dfs_iter_gpu_maxd14`)のポートへ
進める。不一致が出た場合は最初の不一致レコードのt/funcidを特定し、
28分岐決定木のどの枝で分岐したかを両側の出力を突き合わせて切り分ける。

"""

"""
# 360_maxd14_port_spec_update.md

## 位置づけ

本文書は`338_maxd14_port_spec.md`（338で確定したCUDA Cランナー移植仕様）
に対する**差分パッチ**である。338は337時点のソースを前提に仕様化した
が、338以降にカーネルへ2件の変更（351: w_hi_arr削除、356: save_sp
narrowing）が入ったため、その2点を反映する。338 spec本体（bin→SoA
導出の28葉決定木、build_soa_for_range/symmetry()のC言語仕様、シフト
量非負のassert 3件、N=18限定の突き合わせスコープの4段階チェック）は
変更なくそのまま有効。

## 差分1: `w_hi_arr`の完全削除（351由来）

### 変更前（338 spec、337ソース基準）

SoA配列は8本: `ld_arr / rd_arr / col_arr / ctrl0_arr / free_arr /
markctrl_arr / w_lo_arr / w_hi_arr`

カーネルは`w_hi_arr`を3箇所でロードする（338 specが特定した旧576/
581/750行）。エピローグは`w_lo_arr`と`w_hi_arr`から64bit値`w`を
`w = u64(w_lo) | (u64(w_hi) << 32)`のように合成し、`total * w`の
64bit乗算に使う。

### 変更後（356ソース基準、360で反映）

SoA配列は**7本**: `ld_arr / rd_arr / col_arr / ctrl0_arr / free_arr /
markctrl_arr / w_lo_arr`

`w_hi_arr`および対応するロード3箇所は完全に消滅。エピローグは
`w_lo_arr`のみを読み、`total * u64(w_lo_arr[idx])`という単純な
64bit拡張＋乗算になる（356の`WHI_ELIM_REASON`定数に記録されている
通り、351時点では「コンパイラがhigh operandがゼロだと知りようが
ないので乗算は変わらない」と考えられていたが、352の訂正で「ゼロ
拡張は証明可能なので乗算はIMAD3命令からwidening MADへ縮退した」
ことがSASS比較で確定している）。

### Cポート仕様への反映

- `build_soa_for_range`のC言語仕様から`w_hi_arr`の生成・書き込みを
  削除する（338 specの手続きから該当ステップを除去）
- カーネル引数リストから`Ptr[u32] w_hi_arr`相当のポインタパラメータ
  を削除する（5カーネル分すべて）
- ホスト側ガード: `w_lo_arr`の各要素は`{2,4,8}`の3値のいずれかで
  あるという不変条件を、Cポート側でも起動前アサートとして踏襲する
  ことを推奨する（356の`WHI_ELIM_REASON`が記述する「仮定せず検査
  する」という設計哲学を移植先でも維持する）

## 差分2: `save_sp`のビット幅narrowing（356由来）

### 変更前（338 spec、337ソース基準）

`save_sp`はCodon側で`int`（符号付き）として宣言。338 specはC側の
対応する型を明記していなかった（337時点でCodon側自体が`int`だった
ため、暗黙にC側`int`/`int32_t`相当を想定していたと考えられる）。
...
356採用時の実測（352アンカー比、chunk別 -0.83% / -0.93% / -2.00%、
351/352で確立したノイズ床0.03〜0.29%を明確に超える）により、この
narrowingは確定的に採用されている。

**`stack_ptr`は356でも意図的に`int`のまま据え置かれている**（356の
README曰く「配列添字であり、Codonがu32添字を素直に受け付けるか未確認
のため、リスクを避けた」）。

### Cポート仕様への反映

- `save_sp`はC側で**`uint32_t`**として宣言する
- `save_sp==0`の比較・`-=1`/`+=1`の増減は、すべて符号なし演算として
  行う（Codon側の`u32(0)`/`u32(1)`リテラルはCの`0u`/`1u`に対応する
  と読み替える）
- `stack_ptr`は**`int32_t`のまま**とし、356の判断（配列添字の幅を
  安易に変えない）をCポートでも踏襲する。Cではu32添字自体は言語上
  問題にならないが、既存の判断を無根拠に覆さないという単一変数
  原則を優先する
- K-batchingレイアウト（K=48、`STEPS=THREADS*K=15488*48=743424`、
  スタック`u64[13*2]`=208バイト/スレッド、`stack_ptr`/`save_sp`/
  `cur_depth`/`next_depth`という4変数構成）自体は338 specのまま
  変更なし。`save_sp`の型だけが変わる

## ±3%判定基準の分母更新

338で固定した「移植は速度向上を目的としない。目的はwarp intrinsics・
デバイスatomic・`-lineinfo`のper-line SASS帰属の解禁。±3%以内なら
成功」という判定基準の**枠組みは変更しない**。ただし分母となる
Codon側の基準タイミングを更新する。

| | 338時点（337基準） | 360時点（356基準） |
|---|---:|---:|
| N=21フル elapsed | 450.056s | **393.404s** |
| chunk0 | (337実測値、README要参照) | **144,590ms** |
| chunk1 | 〃 | **144,473ms** |
| chunk2 | 〃 | **103,271ms** |

Cポートの成否は、この356基準（393.404s、chunk0/1/2）に対する
±3%以内で判定する。

## リスクノート（358/359から継承、移植そのものへの制約ではない）

358・359で「pushガード（`if cur_avail!=u32(0):`）への制御フロー形状
変更は、完全除去・部分除去いずれの形でも実測上むしろ悪化する」ことが
確定した（358: +3.47〜5.51%、359: +65.0〜66.8%）。post-mortem ncu
解析では、コンパイラの命令スケジューリング崩壊（359では総サンプル数
が壁時計悪化率とほぼ一致して1.64倍化、`long_sb`ストールの94.40%が
単一のLOP3命令に集中）が示唆されている。

**この知見はCodonコンパイラのバックエンド固有の挙動である可能性が
高く、nvccという別のバックエンドに移植した時点で必ずしも再現しない**
（機構は断定しない、というのが359 READMEの立場でもある）。したがって
この知見は、

1. **初回のCポート（356→.cu、振る舞い等価の移植）そのものへの制約
   ではない** — 移植はソースを書き換えるのではなく、既存の制御フロー
   構造をそのままC言語へ写すため
2. **移植完了後、C側で独自の最適化（pushガードの形状変更を含む）を
   試す段になったら思い出すべきリスクノート** — nvccでも同種の
   問題が起きるかどうかは未検証であり、n=2（Codon上での2敗）の
   観測をnvccに外挿する根拠はまだない
...
上記の設計内容を確定させた後、356Py実物（`356Py_savesp_narrow.py`）
を受領した。これにより以下が全て解決済みとなった。

1. **再構成の正しさが実証された**: 前段階で359Pyから復元した356相当
   コードと、実物356Pyのコード領域（docstring剥離・VERSION_TAG除外
   後）を直接diffした結果、**完全一致（差分0行）**を確認した。
2. **360Pyは実物356Pyを直接土台として組み直した**: 再構成コードでは
   なく、実物のコード領域をそのまま使用している。
3. **`source_code_identical_to_356`静的チェックの参照ハッシュを実証
   済みの値に更新した**: `793eef693d8f5af43ca6f131fcdd37d000efb785be5c295e54188885f5625fd4`
   （5601行、docstring剥離・VERSION_TAG除外後）。この値は実物356Pyから
   このセッションで直接算出したものであり、推測でも自己参照でもない。
4. **Open Objectives項目1-12は356Py実物の原文をそのまま360Pyに転記
   した**（項目6のみ360の内容で更新）。項目10・11も含め、捏造なしで
   完全に反映済み。

357自身が保持していた過去の参照ハッシュ（`6db7de43...`、5607行）とは
依然として一致しないが、これは357のdocstring剥離スクリプトの実装が
本ハーネスと異なる（行数の数え方等の違いと推定）ためであり、コード
内容の相違ではないことを実物356Pyとの直接diffで確認済みである
（`.sh`内`note_357_historical_hash_mismatch`としてINFO記録）。

## 未反映のまま残る事項（次回以降）

特になし。356Py実物の受領により、当初想定していた不確実性はすべて
解消された。361で`.cu`スケルトンの作成に進める状態にある。


## 361での実装状況(追記)

上記の仕様(SoA配列7本、save_sp=uint32_t、stack_ptr=int、判定分母=
393.404秒)に基づき、361で`build_soa_for_range`+`symmetry`(bin→SoA
導出部分)のみを`361_soa_derive.c`としてCへ1対1移植した。カーネル本体
(K-batching/スタック/DFS)は362以降に持ち越し。361の検証結果は
README.mdの「361 結果」セクションを参照。
"""
...
VERSION_TAG:str="361 soa_derive_c_port: ADDITIVE REVISION, r2 (post-run bugfix). Executable code region is byte-identical to 356/357/360 outside four clearly delimited deltas, verified by a static check that strips/reverses those four spans before hashing against the 360 reference hash 793eef693d8f5af43ca6f131fcdd37d000efb785be5c295e54188885f5625fd4: (1) ===361-INSERT-BEGIN/END=== -- new function dump_soa_reference_c_port(), placed immediately after build_soa_for_range()s existing return statement, which reads every record directly from the raw stream bin (the same file/format 337 already validated end-to-end), runs the UNMODIFIED build_soa_for_range()+symmetry() over all of them in file order, and writes a flat 10-field-per-record u32 LE binary reference dump plus a running checksum; (2) ===361-DISPATCH-INSERT-BEGIN/END=== -- new bench_mode==32 dispatch branch, appended after the existing mode30/31 block and before the generic N>=21 fallback, that resolves the stream bin path and calls the new function; (3) ===361-CLIGATE-COMMENT-BEGIN/END=== plus a 2-word addition ('or bench_mode==32') to the CLI-level whitelist condition immediately below it; (4) ===361-PRESETGATE-COMMENT-BEGIN/END=== plus the same 2-word addition to the adjacent preset_queens gating condition. Deltas 3 and 4 are a same-session bugfix: the first attempt at this revision omitted them, so the pre-existing whitelist gate (added in rev276, restore274/coretrim) silently reset bench_mode=32 back to 0 before it ever reached delta (2)s dispatch branch, causing Suzukis first real run to fall through to the plain ~11-minute default N=21 execution instead of the new soa-ref-dump path (correctness 314666222712 was still confirmed on that run, but it produced no soa-ref-dump output and consumed real GPU time for nothing). Deltas 3/4 are two true 2-word modifications to existing 356/360 lines, not pure insertions -- the static check accounts for this by stripping the comment spans and then reversing the exact known 2-word addition on each of the two modified lines before the final hash comparison, rather than pretending the whole span is discardable. None of the four deltas touch the kernel, the dispatcher, the reorder pipeline (broadmarktail/chunkshape148/funcid_reorder), or any existing bench_mode branch. Purpose: cross-validate the new 361_soa_derive.c host-side C port of build_soa_for_range()+symmetry() (the bin-to-SoA derivation function specified in 338_maxd14_port_spec.md and its 360 delta patch, 28-branch funcid decision tree) against this Codon reference dump, byte-for-byte, on the real N=21 bin file (2025282 records) -- the first concrete implementation step of Open Objectives item 6 (CUDA C runner), scoped to the derivation function alone per single-variable discipline; the GPU kernel itself (kernel_dfs_iter_gpu_maxd14) is out of scope for 361 and remains a later revision."
