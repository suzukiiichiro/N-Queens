 ...
 *   /usr/local/cuda/bin/nvcc -O3 -arch=sm_86 -o 395b_kernel_maxd14 395b_kernel_maxd14.cu
 * Run (device):
 *   ./395b_kernel_maxd14 <N> <in_soa7_bin> <out_results_bin> [expected_total]
 *   e.g. ./395b_kernel_maxd14 21 constellations_N21_6.bin.soa_ref_361.bin.maxd14only_363.bin \
 *          /tmp/gpu_results.bin 314666222712
 *   395a: NQ_MAX_BLOCKS=800 ./395b_kernel_maxd14 21 <in> <out> 314666222712
 * Build (host-only CPU test, no CUDA toolkit needed, this sandbox):
 *   gcc -O2 -Wall -Wextra -x c -o 395b_kernel_maxd14_cputest 395b_kernel_maxd14.cu -lm
 * Build (host-only CPU test, OpenMP-parallelized outer loop):
 *   gcc -O2 -fopenmp -Wall -Wextra -x c -o 395b_kernel_maxd14_cputest_omp 395b_kernel_maxd14.cu -lm
 ...
 * r2 RESULT (2026-09-08): oracle MATCH; SASS still 472 instructions; the
 * cur_depth<8 predication is gone (0 x ISETP R7,0x8) but nvcc STILL
 * rematerialises the extraction (2 x SHF.R.U64, at 289650 and 289950);
 * kernel_ms 139,179/139,158 vs the 394g control 139,437/139,443 = -0.2%.
 * Safe, reproducible, but essentially neutral: in this stall-bound
 * regime (IPC 0.28) removing ALU instructions does not shorten the
 * critical path -- the samples simply move to neighbouring stalls. The
 * sample-share of the duplicate (4.09%) was an upper bound on ATTENTION,
 * not on time. Kept (no downside).
 *
 * 395b -- REGISTER-RESIDENT TOP OF STACK (process_one_task() only, three
 * sites: root fast-path push, pop, main push). The explicit stack's top
 * frame lives in registers top0/top1; a push spills the old top to local
 * memory (one STL.128, as before) and installs the new frame in top; a
 * pop reads the top from registers and refills top from memory, whose
 * LDL.128 is not consumed until the NEXT pop or push -- a whole node
 * expansion later. This targets LATENCY on the critical path (394a: the
 * pop's LDL consumer carried 100.0% of long_scoreboard = 6.8% of samples,
 * with only 2 independent instructions between load and use), not
 * instruction count. save_sp counts live frames including the register
 * top; stack_ptr = 2*(save_sp-1); frame layout, the 26-slot array, the
 * push guard's form (RISK NOTE) and every branch shape are unchanged.
 * The CPU test harness compiles this same function; 395b_validate.sh
 * gates per-record equality against 395a on real records before the GPU.
 * Also (host side, policy from 395b on): the binary's own NQ_MAX_BLOCKS
 * default is 800, the confirmed value, so a plain direct invocation runs
 * the production launch config too.
...
static uint64_t process_one_task(
    ...
    uint32_t save_sp  = 0;      /* live frames, INCLUDING the register-resident top */
    int      stack_ptr = 0;     /* = 2*(save_sp-1) when save_sp>=1: frames in local memory = save_sp-1 */
    /* 395b: the top of the explicit stack lives in registers (top0/top1).
     * 394a: the pop's LDL was consumed 2 instructions later and carried
     * 100.0% of long_scoreboard (6.8% of all samples); the loop head then
     * tested the just-popped avail at once. With the top frame in
     * registers a pop is register moves, and the LDL that refills top
     * (the frame BELOW) is not consumed until the NEXT pop -- a whole node
     * expansion later -- so its latency is off the critical path. A push
     * spills the old top (STL.128, as before) and installs the new frame
     * in top. Frame layout and the 26-slot array are unchanged; at most
     * save_sp-1 <= 12 frames ever reach memory. */
    uint64_t top0 = 0, top1 = 0;
    int      cur_depth = 0;
...
#ifndef __CUDACC__
                if (save_sp != 0u && stack_ptr + 1 >= MAXD14_ANCESTOR * 2) {
...
#endif
                /* 395b: at the root the stack is empty (save_sp==0), so the
                 * frame goes straight into the register top; nothing spills. */
                top0 = (uint64_t)cur_ld | ((uint64_t)cur_rd << 32);
                top1 = (uint64_t)cur_col
    ...
    for (;;) {
        ...
        if (cur_avail == 0u) {
            ...
            /* 395b: pop = take the register top; then refill top from
             * memory (if any frame is left there). The refill's load is
             * next read at the following pop or push, never here. */
            uint64_t packed_ldrd  = top0;
            uint64_t packed_colav = top1;
            save_sp -= 1u;
            if (save_sp != 0u) {
                stack_ptr -= 2;
                top0 = stack[stack_ptr];
                top1 = stack[stack_ptr+1];
            }
...
#endif
            /* 395b: spill the current register top (if any) to memory,
             * then the new frame becomes the register top. */
            if (save_sp != 0u) {
                stack[stack_ptr]   = top0;
                stack[stack_ptr+1] = top1;
                stack_ptr += 2;
            }
            top0 = (uint64_t)cur_ld | ((uint64_t)cur_rd << 32);
            top1 = (uint64_t)cur_col
