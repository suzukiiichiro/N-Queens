 ...
 *   /usr/local/cuda/bin/nvcc -O3 -arch=sm_86 -o 394g_kernel_maxd14 394g_kernel_maxd14.cu
 * Run (device):
 *   ./394g_kernel_maxd14 <N> <in_soa7_bin> <out_results_bin> [expected_total]
 *   e.g. ./394g_kernel_maxd14 21 constellations_N21_6.bin.soa_ref_361.bin.maxd14only_363.bin \
 *          /tmp/gpu_results.bin 314666222712
 *   394g: NQ_MAX_BLOCKS=864 ./394g_kernel_maxd14 21 <in> <out> 314666222712
 * Build (host-only CPU test, no CUDA toolkit needed, this sandbox):
 *   gcc -O2 -Wall -Wextra -x c -o 394g_kernel_maxd14_cputest 394g_kernel_maxd14.cu -lm
 * Build (host-only CPU test, OpenMP-parallelized outer loop):
 *   gcc -O2 -fopenmp -Wall -Wextra -x c -o 394g_kernel_maxd14_cputest_omp 394g_kernel_maxd14.cu -lm
