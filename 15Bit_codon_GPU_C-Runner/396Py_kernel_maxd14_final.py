...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 396 (2026-09-09)
...
"""
了解しました。順番もそのままで進めます。396として、**ncuでのストール内訳測定**を組みます。

設計上、先に潰しておくべき点が2つあります。

**プロファイルの代理入力が本番を代表しているか。** N=21のフル実行は2:13で、ncuのリプレイを掛けたら手に負えません。レコード数を削った代理を使いますが、`k_per_thread` が変わると命令ミックスも分岐挙動も変わります。**代理を先に較正します** — 本番のレコードあたりコスト（133,192.071 ms ÷ 2,025,282 = 65.765 µs/record）に1%以内で一致する最小のkを実測で選びます。

**測定のコストを測ってから測定する。** セクションを7つ並べると何回リプレイされるか読めません。1セクションでのパイロットを先に走らせ、実測から総所要を外挿して、15分を超えるなら自動でkを落とします。

四点セットです。396の四点セットです。静的ゲートは **OK=17 FAIL=0**。

## 同一性

`396_kernel_maxd14.cu` はr6のリネームで、**最初の `#include` 以降はバイト一致**（コード領域全体 `c5dbde3f1002229b…`）。カーネル領域は395a r2以来不変（`ebd7f523deb59134…`）。Pyはr6Pyから**実行行3行**の差分で、r6で採用した `env_prefix="NQ_EXTRA_CTX=1 "` はそのまま維持しています。

ビルドに `-lineinfo` を足しました。ncuがストールをソース行に帰属できるようにするためで、`-O3` の最適化そのものには影響しません。

## 実行の流れ

**T2 アンカー**（2.5分）→ `-g 21 21` が 133,192 の ±0.1% かつ `extra_ctx=1`。外れたら**そこで停止**します。セッションが本番状態にないなら、その後のプロファイルは別の機械を記述しているだけなので。

**T1 代理較正**（約1分）→ ncuなしで k=1,2,4,8,16 を掃引し、µs/record が本番値 **65.765** と1%以内で一致する最小のkを選びます。どれも合格しなければ**プロファイルを取らずに停止**します。代表していない代理は、自信に満ちた誤った順位表を生むので。

**ncu権限プローブ**（3秒）→ `ERR_NVGPUCTRPERM` を検出したら、sudo実行か恒久設定を案内して停止します。19分費やす前に判定します。

**コストパイロット** → 1セクションの実測から7セクションの総所要を外挿。予算900秒を超えるなら自動で代理を1段小さくし、**「較正帯から外れた可能性がある」旨を明示して**続行します。ぶら下がりません。

**プロファイル本体** → 7セクション。実行中のcompute-appsを5秒ごとに採取して、**ncu自身のGPU占有をログに残します**。

## 事前登録

**T3** メモリスループットはピークの5%未満。57 MBを133秒で1回読むだけなので実効0.4 MB/s、帯域律速ではないことをカウンタで確認します。

**T4（本命）** 支配的なwarpストール理由は分岐解決系で、ストールサイクルの25%以上。394a時点の知見の追認です。

**反証された場合が最大の情報量です。** 律速要因が移動していれば、397は最適化ファミリごと変える必要があります。ハーネスは順位表の直後にその判定を出力し、反証時には「全順位表を読んでから選べ」と明示します。

## 一点だけ、留意事項

ncuはCUDAコンテキストとデバイスメモリを持ちます。395cで、コンテキスト数と占有がこのカーネルを0.3〜10%動かすことが分かっている以上、**プロファイル中の壁時計は本番のタイミングと比較できません**。396が読むのはカーネル単位のカウンタと比率だけです。これは.cuのヘッダとPyのノート、READMEの3箇所に書いてあります。
...
ncuの権限が管理者限定になっている環境でしたら、先に `sudo` で走らせるか、恒久設定を入れてからのほうが早いかもしれません。
"""
...
VERSION_TAG:str="396 PY-REVTAG: pure rename of 395c-r6Py; the dispatch table now names ./396_kernel_maxd14 and keeps the env_prefix NQ_EXTRA_CTX=1 adopted in 395c-r6. Code region of the .cu is byte-identical to r6 and r4; kernel region byte-identical to 395a r2. 396 changes NOTHING functional: it is a measurement revision that profiles the current kernel with ncu and produces a ranked warp-stall breakdown, after calibrating a truncated-record proxy against the full-run microseconds-per-record and after measuring the cost of the profiling itself. The dispatcher is present so that every 396 session can anchor on the adopted production number 133,192.071 ms before any profiling is believed."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./396_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
