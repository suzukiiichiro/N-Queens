...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 400-r3 (2026-09-09)
...
"""
25:00でしたら十分いけますね。**400-r3、行きましょう。**

そして書きながら気づいたことがあります。数字が示唆する既定値を計算すると、この実験の見込みが上がります。

```
L1が128 KBなら:  320 threads × 208 B = 66.6 KB → 収まる。だが実測 +0%
                 512 threads × 106.5 KB → 収まる。だが実測 +82%
L1が 64 KBなら:  66.6 KB → わずかに超過
                 106.5 KB → 1.66倍超過 → +82%
                 213.0 KB → 3.3倍超過  → +405%
```

**後者の方が実測に合います。既定のカーブアウトが共有メモリ側にかなり割いている可能性が高い。** だとすれば `carveout=0` でL1が倍近くになり、strideスケーリングが実際に変わります。

しかも `cudaFuncGetAttributes` を1回呼べば、**既定値も1スレッドあたりローカルメモリ量も、走らせる前に分かります。**400-r3、四点セットです。静的ゲートは **OK=19 FAIL=0**。

## 走らせる前に答えが出るかもしれません

`cudaFuncGetAttributes` を毎回呼ぶようにしたので、各実行の冒頭にこれが出ます。

```
[gpu-carveout] requested=-1 effective_pref=?? local_bytes_per_thread=208
               static_shared_bytes=0 registers=37
```

**`effective_pref` が既定値です。** ここが 0 でなければ共有側に割かれており、`NQ_CARVEOUT=0` に意味があります。0 なら既にL1最大で、この軸は空振りです。**タイミングを1つも読む前に分かります。**

## 事前登録

**K3で「既定は0ではない」に賭けます。** 根拠は400-r2の曲線で、L1が128 KBなら106.5 KBは収まるはずなのに既に+82%だからです。L1が64 KB程度と仮定すると曲線に合います。

**K4で「stride 81,920 でカーブアウト0が10%以上速い」に賭けます。** 2%以内なら、**容量はフレーム縮小でしか叩けない**ことになります——397の着想を、正しいやり方でやり直す根拠が立ちます。

**K5が容量説の直接検定です。** ローカルメモリのL1ヒット率が stride とともに落ちるか。落ちなければ6〜7倍は容量効果ではなく、別の機構を探すことになります。

**K6は対照です。** stride 25,600（現行本番）ではカーブアウトが1%以内で効かないはず。66.6 KB は既定L1にも収まっているので。**ここが動いたら容量モデルが間違っています。**

## ハーネスの修正

r2のTSVに**N列とcarveout列が無く**、N=19とN=20がマージされて `spread 154%` を出した件を直しました。ログファイル名にもカーブアウトを含めています。

掃引はstrideごとに代表1構成へ絞りました（r2で「ブロック形状はほぼ中立」と判明したので）。4 stride × 2カーブアウト × 3反復で約4分、ncuのL1診断が約5分、**通しで15分**です。
...
`[gpu-carveout]` の行だけでも先に見えたら教えてください。**それだけでK3が決まり、この夜の方向が決まります。**


その1点だけ残っていました。**過去4リビジョンのノートの訂正**です。

ただし、**既に走らせて指紋を検証済みのPyソースを遡って書き換えるのは避けます。** この案件の規律は監査可能な記録にあるので、誤りを消すのではなく**訂正記事として上書きする**のが筋だと考えます。

READMEへの訂正追記を1本作ります。3項目とも揃いました。400-r3の四点は先ほどのままです（再掲しました）。

## 訂正記事について

**過去のノートは書き換えません。** 誤った主張を明示して、この記事で上書きします。指紋を検証済みのソースを遡って編集すると、この案件の規律そのものが失われるので。

記事は「何が誤りだったか」より「**何が生き残り、何が言い直しになるか**」に紙幅を割いています。

**生き残るもの** — 部分集合が代理にならないこと、並び順が35.2%の価値を持つこと、399のフェーズB。いずれも実測で、誤った説明に依存していません。むしろフェーズB（32個組内で `w_lo` の分散が全体の8.2%）は、**訂正後の理解を裏付ける測定**でした。

**言い直すもの** — 3件です。特に「なぜ部分集合が代理にならないか」は、旧説明が誤りでも結論は変わらず、新しい説明の方が根拠が強い。単独ラウンドが最大値を測っていること（平均化が42.5%を稼いでいる）と、`(markctrl, ctrl0)` ソートの階級構造。398-r2で測ったラウンド別コストの非単調性が、後者と整合します。

## 一箇所だけ、実害の一歩手前まで行っていました

**400-r2の解析で、私は誤った説明のまま次の実験を設計しかけていました。**

> 「394fをstride 122,880用に再生成すれば公平に再試行できる」

394fにstrideが無い以上、その再生成は存在しません。**ソースを見せていただいて初めて止まりました。** 推論を検証せずに次の設計へ持ち込むと何が起きるかの実例として、記事に残しました。

## 恒久ルールを1つ提案しています

この案件は「規律は人の記憶ではなくゲートに置く」でやってきましたが、**外部ツールの振る舞いについての主張には、ゲートがありませんでした。**

> リビジョンノートが外部スクリプト（`394f_permute_soa7.py`、`363_filter_maxd14_only.py`）の振る舞いに言及する場合、**そのスクリプトの該当行を引用すること**を要件とする。引用できない主張は書かない。

私が4リビジョンにわたって同じ誤りを書き続けられたのは、引用を求められなかったからです。

あとは400-r3を走らせるだけです。**`[gpu-carveout]` の1行で、この夜の方向が決まります。**

そのご期待に、ひとつだけ正直に申し上げておきたいことがあります。

**N=20の2.4倍は、カーネルが速くなったのではありません。** `CRUNNER_MIN_N=19` によって、N=20が **Codon側のGPU経路からCRunner経路へ切り替わった**結果です（42.8秒 → 17.9秒）。

**N=21はもともとCRunner経路です。** ですからあの2.4倍は、N=21以降では原理的に再現しません。あれは「まだ通っていなかった経路に載せた」利得で、N≤20にだけ効きます。

とはいえ**本物の利得**ではあります。N=19/20は今後の測定基盤そのものなので、正式化する価値があります（交互A/Bと事前登録を経て、小さなリビジョンで）。

## では、N=21以降の望みはどこにあるか

今日測った中で、**桁の違う余地があるのはひとつだけ**です。

```
wait 45%（固定レイテンシ依存待ち）
Active Warps Per Scheduler 2.43 / 12
```

**スケジューラの椅子が12あって、2.43しか埋まっていません。** 埋められれば、`wait` は原理的に叩けます。0.3%を追ってきた軸とは規模が違います。

そして今夜わかったのは、**その椅子を埋めようとすると6〜7倍遅くなる**ということでした。壁があります。400-r3が答えるのは「**その壁は取り除けるものか**」の一点です。

- `[gpu-carveout]` の既定値が 0 でなく、`NQ_CARVEOUT=0` が効く → 壁の一部はドライバの設定で、1行で崩れる
- 効かない → 壁は208バイトのフレームそのもの。**397を正しいやり方でやり直す根拠**が立つ（あのときは `rd` のマスクだけが無効で、着想は間違っていなかった）

どちらでも前進です。**空振りが確定するのは、L1ヒット率がstrideとともに落ちなかった場合だけ**で、そのときは容量説そのものを捨てて別の機構を探すことになります。

## 見通しについて、控えめに

椅子が2.43から仮に6〜8まで埋まったとして、`wait` が半減すればストール総量の2割強が消えます。時間がそのまま2割縮むとは限りませんが、**この案件で初めて「%ではなく割」の話ができる軸**です。

2:13.19 から 2:02 まで −8.4%。**届く可能性があるのは、いまのところこの軸だけです。**

走らせてみてください。`[gpu-carveout]` の行が出たら、それだけでも先に見せていただければ。

**壁の正体が完全に分かりました。そして予想外の場所に出口があります。**
...
```
既定           effective_pref=-1   （ドライバ任せ）
NQ_CARVEOUT=0  effective_pref=0
stride 81,920: 10,729.669 → 10,729.866 ms   （+0.002%）
L1ヒット率   :     85.96% →     85.92%
```

私のK3・K4の予測は外れです。**ドライバは既にL1へ最大限を割り当てています。**

しかしK5が、探していた答えそのものを出しました。

| stride | threads/SM | 作業集合 | **L1ヒット率** | **L2読み出しセクタ** | occ | warps/sched |
|---:|---:|---:|---:|---:|---:|---:|
| 25,600 | 320 | 66.6 KB | **99.48%** | 477.7 M | 18.88% | 2.44 |
| 81,920 | 987 | 205 KB | **85.96%** | **12,298 M** | 63.73% | **7.71** |

**L2トラフィックが25.7倍**。ミス率は 0.52% → 14.04% で **27倍**。完全に一致します。
...
**オカパンシーの梃子は機構としては効いています。** warps/sched は 2.44 → **7.71**、椅子が3.2倍埋まりました。狙いどおりです。

**しかし椅子を埋めた瞬間、DFSスタックがL1から溢れ、メモリ待ちが94.5%を占める別のカーネルになりました。**

> **208バイトのフレームが、オカパンシー軸そのものの門番です。**

## 出口はどこか

計算すると厳しい。現行 320スレッド × 208 B = 66.6 KB でヒット率99.48%。椅子を4本埋める（32 warps/SM = 1024スレッド）には、**1スレッドあたり約65バイト**でないと同じ足跡に収まりません。208 → 65 は3.2分の1です。

そして397で確定した真の必要幅は **ld 20 + rd 32 + col 21 + avail 21 + depth 4 = 98ビット**。12バイト（96ビット）にすら2ビット足りない。**今の持ち方では1バイトも縮みません。**

**ただし、持ち方を変えれば話が違います。**

397でrdが逆算できないと結論したのは、`>> step` で下位 `step` ビットが失われるからでした。しかし **`step` は1〜3、つまり失われるのは最大3ビットです。それを保存すれば復元できます。**

```
現在保存      : ld(20) rd(32) col(21) avail(21) depth(4)     = 98ビット
bit方式で保存 : bit(21) rd_lost(3) avail(21) depth(4)        = 49ビット
```

`bit` があれば `col = col_child & ~bit`、`ld = (ld_child >> step) & ~bit`、
`rd = ((rd_child & ~bK) << step | rd_lost) & ~bit` で全部復元できます。
`step` と `bK` はその階層の `block_code` から導けるので保存不要です。

**49ビット = 1ワード。フレーム 208 → 104 バイト、ちょうど半分です。**

104バイトなら 640スレッド/SM（20 warps/SM）が 66.6 KB に収まります。**椅子は2.44から5前後へ。**

## 401の提案 — ただし、また先走らないために

397の失敗は「診断が終わる前に手を動かした」ことでした。同じ轍を踏まないよう、**逆算の正しさを確かめる前に、そもそもその投資が見合うかを先に測ります。**

**(1) 崖の位置を細かく測る**（コード変更なし、N=19で10分）
stride を 25,600 / 32,000 / 38,400 / 44,800 / 51,200 / 64,000 と刻み、時間とL1ヒット率を対応づける。**「作業集合が何KBまでなら99%を保つか」**が出れば、104バイトで何スレッド載るかが計算で決まります。

**(2) その曲線が「104バイトなら勝てる」と言った場合にのみ、逆算方式を実装する**
実装したら、全2,025,282レコードのバイト一致ゲート——397を止めたのと同じゲート——を先に通します。

今日いちばん重要な数字を最後にもう一度。**stride 25,600 では L1ヒット率99.48%、long_scoreboard 12.9%。本番点でメモリは全く問題になっていません。** 問題は椅子が2.44しか埋まっていないことだけで、**その門番が208バイトだと特定できた**——これが今夜の到達点です。

401の四点セットを書きますか。それとも今日はここで区切りましょうか。
...
VERSION_TAG:str="400-r3 PY-REVTAG: pure rename of 400-r2Py; the table names ./400_r3_kernel_maxd14 and keeps env_prefix NQ_EXTRA_CTX=1, CRUNNER_MIN_N=19 and NQ_BLOCK. The .cu adds NQ_CARVEOUT and a cudaFuncGetAttributes report, both host-side, so the kernel region stays at the 395a r2 sha256 ebd7f523. Rationale: 400-r2 showed that raising the thread count makes the kernel 6 to 7 times slower and that block shape is nearly irrelevant, so the binding resource is the local-memory working set per SM. This kernel uses no shared memory and no barriers, so any carveout given to shared is L1 the DFS stack cannot use. r3 measures the default carveout and tries setting it to zero."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./400_r3_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
