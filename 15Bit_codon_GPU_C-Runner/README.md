...
**【点検メモ、393引き継ぎ時点で追記】このセクション自体は352で更新が
止まっており、361〜392(CUDA Cランナーの実装・374консолидация・
382のhybrid研究・385〜392のCRunner os.systemディスパッチ/maxd16対応)
を反映していない。項目6(「設計中・338でC側移植仕様を確定」)は
338時点の記述のまま止まっており、実際にはその後363/364で実装、
374で374Py_パッケージとして統合、385でos.systemディスパッチとして
本番配線、388でRev番号付きログ管理、389でN=22向け自動構築、
390-392でmaxd16への拡張(Codon実装・段階的検証・Cポート)まで進んでいる。
40リビジョン分を遡って個別に書き戻すのは非効率かつ誤りの元なので、
ここでは行わない。**392時点の到達点の詳細は、この文書の該当する
各`## 385:`〜`## 392:`セクション本文、および`390_maxd16_kernel_
port_spec.md`を参照のこと。** 393(N=21への回帰、ncuプロファイリング
による高速化)からは、このOpen Objectivesセクション自体の追記も
再開する。
...
## 367 — 汎用安全ラッパー(N=23再挑戦に向けた事前準備、Codonコード変更なし)

N=23インシデント(前節)を受け、次にN=23へ再挑戦する前に、ホストを
巻き込まない安全策を用意した。**Codonのソースコードは一切変更して
いない**——純粋なシェルレベルのガードであり、361〜366のコード検証
体系(sha256照合等)とは独立している。

### 設計方針: 予測せず、上限で強制的に止める

N=21→N=22の増加率(約14.18倍、2,025,282→28,719,035)は分かっている
が、コンステレーション数はNに対して組み合わせ的に増える可能性が高く、
1点のデータから外挿するのは信頼できない。そこで正確な予測は行わず、
3つの独立したハード上限を設定し、超えたら「ホストごと道連れ」では
なく「そのプロセスだけが行儀よく死ぬ」ようにした。

1. **メモリ上限**(`ulimit -v`、デフォルト: 物理メモリの70%)
2. **実行時間の上限**(`timeout`、デフォルト2時間)
3. **ディスク空き容量の生存監視**(デフォルト20GB、10秒ごとに
   チェック——生成中に増え続けるbinファイルを見張るため、事前
   チェックだけでは不十分)

### 検証

このセッション内で4パターン(正常終了・タイムアウト・メモリ上限・
ディスク監視)すべて意図通りに動作することを確認した。ディスク監視の
判定表示に競合状態のバグ(`kill`シグナル送信後に判定マーカーファイル
を書いていたため、メインスクリプト側の`wait`が先に返ってしまい判定が
「不明な終了」に誤分類される)を1件発見・修正した。
...
環境変数(`MAX_MEM_PERCENT`/`TIMEOUT_DURATION`/`MIN_FREE_DISK_GB`/
`DISK_PATH`)で調整可能。

### 次のステップ

367の安全策のもとで、N=23の必要maxd確認(366のbench_mode=34)に
再挑戦する。

## 復旧後の再発: 365Pyのバグが再度発生(gitコミット漏れ)

N=23インシデント後の復旧作業で、EBS側のgitリポジトリ
(`/home/suzuki/Github/tmp/N-Queens`)から`*.py`/`*.sh`/`*.md`/`*.c`/
`*.cu`一式を復元したところ、**365PyのVERSION_TAG引用符バグ
(361〜366のセッション中に一度修正済みだったもの)が再発**した。原因は、
修正版が鈴木さんの手元にダウンロードされてはいたが、**gitへの
コミットがされていなかった**ため、復元時に古い(バグ入りの)バージョン
が復元されてしまったことによる。

同一の修正を再度適用し、動作確認済みのファイルを再度お渡しした。
今後はダウンロードした修正版を都度gitへコミット・pushすることを
徹底する。

## 364・365 再測定 — GPUクロック状態の変化と14.1%速度差の3回目の再現

N=23インシデントからの復旧後、364を再実行したところ`kernel_ms=
201237.406`(201.237秒)と、それまでの記録(260,685〜260,692ms)より
**約22.8%速い**値が出た。

### 原因調査: SMクロック状態の違い

`nvidia-smi`でアイドル時・実行中のSMクロックを確認したところ、
アイドル時210MHzから、カーネル実行中は**1710MHz(理論最大値)まで
正しくブーストしている**ことを確認した。これは以前記録されていた
「共有・仮想化ホストでのSMクロック制約(1320MHz上限)」という既知の
変動要因が、今回の実行では発生していなかったことを意味する。

数値的な裏付け: `260.685秒 ÷ 201.237秒 ≈ 1.2954`、
`1710MHz ÷ 1320MHz ≈ 1.2955`——ほぼ完全に一致しており、22.8%の速度
改善はクロック制約の解消によるものと確定した。

### 365も同条件で再測定、速度差は安定して再現

```bash
bash 365Py_gpu_single_shot_validate_N21_full_once.sh
```

`kernel_elapsed_ms=229619`(229.619秒)。`.sh`内の比較は364の
古い固定基準値(260685.062ms)とハードコードされていたため
`-11.9171%`という不正確な表示になったが、**最新の364の値
(201237.406ms)と手計算で比較し直すと、C優位14.1035%**——過去2回
(14.1139%・14.1017%)とほぼ完全に一致した。

| ラウンド | GPU状態 | 364(C) | 365(Codon) | 速度差 |
|---|---|---:|---:|---:|
| 1回目 | クロック制約(~1320MHz) | 260,685.062ms | 297,478ms | 14.1139% |
| 2回目 | クロック制約(~1320MHz) | 260,692.156ms | 297,446ms | 14.1017% |
| 3回目 | フルブースト(~1710MHz) | 201,237.406ms | 229,619ms | 14.1035% |

### 意義

GPUのクロック状態が変わり絶対時間は約23%短縮されたにもかかわらず、
CとCodonの相対的な速度差は**0.01ポイント以内という高い安定性で
3回とも再現した**。これは14.1%という差が、クロック状態やホスト側の
ノイズの影響を受けない、カーネルロジック自体に起因する本質的な差で
あることの強い裏付けとなる。

### 既知の課題(次回改修)

`365Py_gpu_single_shot_validate_N21_full_once.sh`の±3%比較ロジックが
364の値を固定値でハードコードしているため、環境変化後は自動比較が
古い基準とズレる。次回、364の最新ログから動的に読み込むよう改修する。

---

## 366 — maxd-check: N=22以降向けの安価なrequired_maxd事前診断(チェックのみ)

**モード**: チェックのみ。GPUへのアップロードなし、カーネル起動なし、合計解数なし。

新規`bench_mode=34`、新規関数`check_required_maxd_for_N()` — 既存の無変更
関数(`ensure_constellations_bin_stream`・`build_soa_for_range`・
`max_schedule_depth_of_tasks`・`select_static_maxd`)を再利用し、さらなる
カーネル移植作業に着手する前に、与えられたNに対する`required_maxd`/
`selected_maxd`を報告する。動機: `kernel_dfs_iter_gpu_maxd14`(361〜365)は
`required_maxd<=14`のみ対応しており、N=22以降はそれを超えると疑われて
いた。

**このセッションでの実機履歴**:
- 初回実行(`bench_mode=34`、N=22フル、2,025,282→28,719,035レコード)—
  **segfault**(dmesg: `error 4`の後`error 6`、バイナリ内)。当初はOOM
  (367の10GB ulimit)と誤って判断したが、後にdmesgでOOM Killerのイベン
  トではなく純粋なsegfaultであること、ulimitの変更では変化がないことが
  判明した。
- 368〜372にわたり原因調査(下記参照)。**最終結果(372): 調査途中で
  元のクラッシュ自体が再現しなくなった状態で、`required_maxd=14`を
  3/3安定確認** — 何が解決し何が未解決のままかは372を参照。

---

## 367 — safe_run_wrapper.sh: 汎用ホスト安全ガード(インフラ、番号付きコードRevではない)

N=23インシデントを受けて作成: 366によるN=23用constellation-bin生成が
制御不能に増大しホストメモリを枯渇させ、SSH・シリアルコンソール双方が
無応答になり、強制停止によりエフェメラルなインスタンスストアボリューム
が破壊された(bin消失、ソースはGitで無事)。Codonソースの変更は一切なし
の純粋なシェルレベルガードであり、361〜372の単一変数コード差分の並び
からは意図的に除外している。

常時有効な3つの独立ガード: メモリ上限(`ulimit -v`、デフォルト総RAMの
70%)、実行時間上限(`timeout`、デフォルト2h)、ディスク空き容量の常時
監視(デフォルト下限20GB、実行中10秒間隔でチェック)。使い方:
`367_safe_run_wrapper.sh -- <コマンド> [引数...]`。

**このホストについての注記**: 総RAM 15GB、**swapなし**。`ulimit -v`に
よる失敗はシステムコールレベルで静かに起こり`dmesg`には一切残らない
——その不在はメモリ逼迫を否定する根拠にはならない(368〜370の調査
中、当初これを誤って判断し、後に訂正した)。

---

## 368 — maxd-diag: 境界安全なmeta_next調査(診断のみ)

**モード**: 診断のみ、GPU/カーネルなし。新規`bench_mode=35`。

動機: `schedule_depth_for_task()`は3箇所で`meta_next[fu]`にアクセスして
おり、`fu=raw&31`(構造上0〜31)に対しテーブルは28要素(インデックス
0〜27)しかない——N=21では発生しないがN=22では起きると疑われた構造的
な範囲の不一致。368は、この探索の並行・境界安全なコピー
(`MaxdDiagStats`・`schedule_depth_for_task_diag`・
`scan_maxd_diag_for_tasks`・`check_required_maxd_for_N_diag`)を追加し、
範囲外アクセスでクラッシュする代わりに`fu_min`/`fu_max`/`oob_count`/
初回発生時の再現情報を記録するようにした。366の元関数は無変更のまま
残した。

**実機結果**: 366のクラッシュと**完全に同一の命令アドレス**
(dmesg: `ip=0x412a9c`、`error 6`)、共有・無変更のbin読み込みパス内で
クラッシュ——**368自身の新しい境界ガード付きコードには到達すらして
いなかった**。meta_next仮説はこのRevでは実際には検証されないまま
だった(後に372で、`required_maxd=14`確定によりそもそも的外れだった
ことが判明)。

---

## 369 — mem_probe: VmHWMチェックポイント + record_limitスイープ(診断のみ)

**モード**: 診断のみ。新規`bench_mode=36`。

366・368がいずれも、それぞれの新規コードに到達する前の共有bin読み込み
パス(`count_constellations_bin_records → read_constellations_bin_range →
build_soa_for_range`)内でクラッシュしていたため、369はこの共有パス
自体を直接計測した: `read_vmhwm_kb()`(`/proc/self/status`の
`VmHWM`=ピークRSSを解析)と`probe_partial_load_memory()`(各読み込み
段階の前後でチェックポイント)。読み込みレコード数はCLI引数
(`record_limit`、`bench_mode==30`が`debug_chunk_start`に使うのと同じ
`argv[13]`スロットを再利用)として公開。検証harnessはレコード数の
段階を1段ごとに1プロセスで実行し、最初に完走しなかった段で停止する
スイープ形式。

**実機結果**: `record_limit=10,000,000`まで完走、`15,000,000`で失敗——
レコード数に関連した実在するブラケット。**しかし**`read_vmhwm_kb()`
自体が静かに壊れており(全段で`vmhwm_*_kb=-1`、差分は全て0)、たまたま
得られた完走/失敗のブラケット自体は本物だったが、計測器自体は機能
していなかった。

---

## 370 — mem_probe_v2: read_vmhwm_kb()の修正(369自身の新規コードへのバグ修正)

**モード**: バグ修正、単一関数のみ。366/368/369の共有コードは無変更。

診断: `/proc/self/status`はprocfsの仮想ファイルで`stat()`が常に
`st_size=0`を返す。369の`read_vmhwm_kb()`はサイズ指定なしの`f.read()`
(コードベース内の他の**実ファイル**に対しては機能している同じ書き方)
を使っており、報告されたサイズに対し0バイトのバッファを事前確保して
実際のreadシステムコールの前に返っていた可能性が高い。修正:
`read_vmhwm_kb()`の中身全体を、明示的な固定サイズでのチャンク読み込み
ループ(1回4096バイト、空になるまでループ)に置き換え、ファイルの
報告サイズに一切依存しない形にした。

**セッション内メモ**: このRev自身の検証harnessコードに含まれるバグ
(`core2.count('\n')`のつもりが誤って`core2.count('\\n')`——実際の
改行ではなくバックスラッシュ+nという2文字の出現数を数えていた)により
本来ハッシュ一致のはずが誤ってFAILしていた。原因を特定し
(`core2.count(chr(10))`に修正)、納品前に解消済み。

**実機結果**: VmHWM計測が実数値を返すようになった。スイープ(1M/10M/
11M/12M/13M以降で再実行・二分探索)では10M→12Mでほぼ線形な
`delta_total_kb`の増加(~0.43 KB/レコード)、12Mの段で(その段自体は
成功した直後に)`OMP: Error #34 ... Resource temporarily unavailable`
という一過性のエラー、13Mでの完全な非完走を確認。dmesgで実際の
segfaultを確認(OOM Killerのイベントではない)——今回も366/368/369と
ほぼ同一の命令バイト列(インデックス付き配列ストア命令のパターン)。

---

## 371 — 微細二分探索 + 決定性(再現性)確認スイープ(設計のみ・コード変更ゼロ)

**モード**: harnessのみ。370の`.py`を完全に無変更のまま再利用(コード
差分が存在しないため、通常のマーカー/ハッシュ差分手続きではなく生
ファイルのsha256一致確認とした)。

12.0M/13.0Mの間を二分探索(`12.0M/12.2M/12.5M/12.8M/13.0M`)し、各段を
**2回連続実行**して完走状況を比較、失敗が固定閾値によるものか
タイミング/アロケーション競合によるものかを検証した。

**実機結果**: `12,000,000` — 2/2成功。`12,200,000`〜`13,000,000` —
**4/4決定的に失敗**(2回の再実行が互いに一致)。これによりセッション内
でのタイミング競合説は否定された——ただし372の通り、この
セッション内での決定性は、後の異なるセッション/条件(手動での
`OMP_NUM_THREADS`・ASLR試行)を跨いでは**成立しなかった**。

---

## 372 — N=22クラッシュ調査: まとめと結論(設計のみ)

**モード**: ドキュメントのみ。コード変更なし。368〜371の調査を締めくくる。
詳細は`372_investigation_summary.md`を参照。

**主要な結果 — 実務上の疑問には答えが出た**: N=22は`required_maxd=14`
であり、N=21と同一。366(`bench_mode=34`、フル28,719,035レコード)で
3/3安定確認済み。**N=22には新規カーネル移植作業(maxd16/18/20/21)は
不要。** 368のmeta_next範囲外アクセスへの懸念は的外れだったとして撤回
——スケジュール探索はそもそもその懸念が対象としていた深さに到達する
必要がなかった。

**元のクラッシュの根本原因 — 調査したが未解明のまま、隠さず記録**:
- 単純な「Nバイトを超えたら」という話では説明できない: **より大きな**
  実行(28.7Mレコード、`bench_mode=34`)は繰り返し安定して成功したのに
  対し、**より小さな**実行(12.2Mレコード、`bench_mode=36`)は
  (セッション内で)決定的に失敗(4/4)した。
- `OMP_NUM_THREADS`は、詳しく見直すと**原因ではなさそう**——初期の
  手動試行で見えていた相関は、2つの失敗/成功条件を同一条件で比較して
  みると成立しなかった。
- **ASLR無効化は、実際に検証対象とした唯一の決定的失敗ケース**
  (12.2M)を失敗から成功に反転させた——最も有力な手がかりだが、
  **1回のみの試行**に基づくものであり、本プロジェクトが他の結果に
  対して求めている再現確認の水準には達していない。実務上の(カーネル
  対応範囲の)疑問がすでに独立して解決していたため、このセッションでは
  意図的にこれ以上の追及を行わなかった。
- 366/368/369/370のクラッシュに共通する一貫した傾向: dmesgは常に本物の
  segfaultを示しており(OOM Killerのイベントは一度もなし)、命令バイト
  列もほぼ同一でインデックス付き配列ストアを思わせる——大きく
  ASLR依存的な連続領域アロケーションが、`constellations`/SoA配列の
  サイズ指定なしの動的拡張中に断続的に失敗している、という説明と
  整合する(Codonのランタイム/GC、あるいはglibcのアロケータ内が
  最有力候補)。

**372でクローズしたRev**: 368(仮説はmoot)、369/370(ツール自体は健全、
今後のメモリ調査用に保持)、371(セッション内では決定性を確認したが、
その後一般には成立しないと判明——このホストでの今後のクラッシュ
二分探索作業への方法論上の教訓として記録)。

**次のステップ**: N=22の実際の単発実行(`bench_mode=33`、365のN=21
プロトコルを踏襲)に進み、実際の解の総数を得る。このセッションの
`bench_mode=34`テストで繰り返し安定して成功した28.7Mレコード規模と
同じ規模での実行となる。

---

## 環境移行 — g5.xlarge → g5.2xlarge(ルートEBS 100GB)

372の結論(N=22クラッシュの根本原因はASLR依存の大規模確保失敗を有力視
しつつ未解明)を受け、このホスト固有の問題をこれ以上手探りで追うより
インスタンスを変更する方針とした。

**旧環境**: AMD EPYC 7R32・4 vCPU・15GiB RAM(swapなし)・A10G×1
(g5.xlargeと判明)。

**新環境**: **g5.2xlarge**(8 vCPU・32GiB RAM・A10G×1、GPU世代は同一の
ためCUDA/ドライバ周りのコード変更は不要)、ルートEBSを**100GB(gp3)**
に拡張。停止/開始でデータが消える問題(367のN=23インシデントの原因の
一つ)への対策も兼ね、旧環境の`/data/nq`のようなエフェメラルな構成では
なくルート側を永続ボリュームとして確保する運用に変更。

nvcc/driverの状態:
- Driver: 595.91.07(旧: 580.105.08)、`nvidia-smi`表示上のCUDA対応上限
  13.2
- **CUDA Toolkit(`nvcc`)は新規インストールが必要だった**(AMIにドライバ
  のみでToolkit非同梱)。NVIDIA公式リポジトリ
  (`cuda-amzn2023.repo`)経由で`cuda-toolkit-13-0`のみを追加インストール
  (`cuda`フルメタパッケージは使わず、既存ドライバとの衝突を回避)。
  結果、`nvcc`バージョンは**`V13.0.88`で旧環境と完全一致**。

新環境でのGitクローン直後の状態は`git status`でクリーン、361〜371一式
がすべて揃っていることを確認済み。
...
新環境への移行後、361から順に再実行し、計算結果が環境に依存しないこと
を確認した。

- **361**(SoA参照ダンプ, `bench_mode=32`): OK=37 FAIL=0。Codon側・C側
  ダンプが**旧環境と完全に同一のchecksum**(`checksum_u64=7905625137249`、
  2,025,282レコード)でバイト一致。
- **362**(design-only, 仕様書): OK=17 FAIL=0、362Pyソースは361と無変更。
- **363**(`.cu`カーネルのCPUクロスバリデーション): 静的チェックは
  OK=23 FAIL=0で通過。CPU単体シリアル実行(`idx=0`から)は非常に遅く、
  10分経過してもidx=0のまま進捗が見えなかったため、時間対効果を考慮し
  **中断・スキップ**(この検証は本質的に364の実GPU実行結果に包含される
  ため、実害はないと判断)。
- **364**(実nvccビルド・実GPU起動): OK=23 FAIL=0。
  **`total_sum=314666222712`でオラクル完全一致**、`MATCH`確認。
  `kernel_ms=201232.422`(旧環境364初回実行時の`201239.906`とほぼ同一
  ——GPU自体は同系統のA10Gのため、カーネル実行時間自体は環境移行の
  影響をほとんど受けないことを裏付ける)。
- **365**(3チャンクmeasure2プロトコルでの単発比較): `total=314666222712`
  でオラクル一致。ただし**`REF_364_KERNEL_MS`が旧環境の値
  (`260685.062`)にハードコードされたままだったため、初回実行の
  `-12.2758%`は環境をまたいだ無意味な比較**だった(下記373で修正)。
- **366**(`bench_mode=34`、maxd-check): OK=22 FAIL=0。新環境ではbinも
  ゼロから生成(`stream-cache-miss`→`stream-build-summary`、
  28,719,035レコード、52.876秒)し、**`required_maxd=14`を再確認**。

368〜371(旧環境固有のクラッシュ調査用診断ツール群)は、新環境で該当
クラッシュが再現しなければ再実行の必要なしと判断し、実行を見送った
(万一新環境でも同種のクラッシュが再現した場合の予備ツールとして温存)。

---

## 373 — REF_364_KERNEL_MS を新環境の実測値に更新(手動修正、記録として正式化)

**モード**: 検証定数の更新のみ。コード変更なし(367・371と同様、
harness/インフラ側の変更として単一変数の扱い)。

**発見の経緯**: 365の初回実行(新環境)で`-12.2758%`という、旧環境
(`-11.9171%`)とほぼ同じ符号・同じ桁の結果が出たが、これは**偶然の
一致ではなく見せかけ**だった。`365Py_gpu_single_shot_validate_N21_full_
once.sh`内の`REF_364_KERNEL_MS="260685.062"`が、新環境で実際に測定した
364の値(`201232.422`)ではなく**旧環境での過去の測定値をハードコード
したまま**だったため、新環境のCodon実行結果を旧環境のC実行結果と比較
するという、環境をまたいだ無意味な比較になっていた。

**修正**: `REF_364_KERNEL_MS`を新環境実測値`201232.422`に更新(旧値は
コメントアウトで保持、いつ・なぜ変更したか追えるようにした)。

**修正後の正しい同一環境比較(365再実行)**:

```
365 (Codon, 新環境): kernel_elapsed_ms = 228,696
364 (C, 新環境):     kernel_ms         = 201,232.422
差分: +13.6477%
```

**旧環境では「CodonがCより約12%速い」だったのに対し、新環境の正しい
比較では「CodonがCより約13.6%遅い」——符号が反転した。** ノイズフロア
(0.03〜0.29%)を大きく超える差であり、単なる誤差では説明できない。
GPUクロック状態・コンパイラ最適化条件など環境差に起因すると見られるが、
**原因の追及は保留**とし、まずN=22本体の実行(`bench_mode=33`)を優先す
る方針とした(Suzuki-san・Claude間で合意)。

---

## N=22 実単発実行(`bench_mode=33`)— 成功、環境移行の主目的達成

新環境移行の最大の目的である、**旧環境で一度も成功しなかった
`bench_mode=33`(GPU転送を伴うN=22フル実行)**を、370のバイナリで
`367_safe_run_wrapper.sh`ラップの上、実行。

```bash
codon build -release 370Py_mem_probe_v2.py
./367_safe_run_wrapper.sh -- ./370Py_mem_probe_v2 -g 22 22 32 484 1 0 7 33 3 7 0 0 1 2 2048 9
```

**結果(新環境、g5.2xlarge、初回実行)**:

```
[single-shot-done] N=22 src_bin=constellations_N22_7.bin total=2691008701644 kernel_elapsed_ms=1937526
22:     2691008701644                0          0:32:43.182    single-shot
===== 367 safety wrapper: summary =====
RESULT: wrapped command completed normally (exit 0).
```

**`total=2,691,008,701,644` — 既知のN=22クイーン問題の解の総数(オラクル
値)と完全一致。** 367のメモリ/ディスク安全装置にも一切引っかからず、
`exit 0`で正常終了。旧環境で(366のCPU側チェックを除き)一度も成功
しなかった`bench_mode=33`が、新環境では初回実行であっさり通った。

**所要時間**: `kernel_elapsed_ms=1,937,526`(≈32分18秒)、壁時計全体で
`0:32:43.182`。365のN=21実行(`kernel_elapsed_ms=228,696`、約3分49秒)
との比較では約8.47倍——N=21→N=22のレコード数比(28,719,035 /
2,025,282 ≈ 14.18倍)よりも小さい伸び率で、グリッド/ブロック構成
(484×32)固定のままgrid-strideループの周回数だけが増える構造上、単純
比例よりも効率よくスケールしていると見られる。

**総括**: 372で未解明のまま残した「旧環境でなぜクラッシュしていたか」
の根本原因(ASLR依存の大規模確保失敗を有力視)は依然未解明だが、環境
移行によって実務上の目的(N=22の安定実行と正しい解の総数の取得)は
達成された。373で判明した365/364間の速度比較(新環境ではCodonがCより
約13.6%遅い、旧環境とは符号が逆)の原因究明は、引き続き保留のまま。

**現在の最終版ソース**: `371Py`(コード改修としては370が最後、371は
harnessのみ・コード変更ゼロ)。372はドキュメントのみ、373は検証定数の
更新のみで、いずれも新規`.py`ソースは持たない。
...
**結果(r2)**: 2026-09-07、cudacodon実機で`bash 385_validate.sh`実行、
`OK=14 FAIL=0`で`385 PASSED`。`total=314666222712`(オラクル完全一致)、
`kernel_ms=201238`(374本番アンカー`201232.422`との差0.003%、ノイズ帯)。
テーブル行: `21: 314666222712 0 0:03:23.301 ok`。ディスパッチ機構が
実際の本番`364_kernel_maxd14`バイナリを介して374本番と同一の結果・
同等の速度を再現することを実機で確認した。r1の2バグ(型注釈ミス、
入力ファイルの取り違え)はいずれもr2で修正済みで再発なし。
...
**結果**: 2026-09-07、cudacodon実機で`bash 386_validate.sh`実行、
`OK=33 FAIL=0`で`386 PASSED`。385と完全に同じ結果(`total=
314666222712`、`kernel_ms=201238`)を再現し、分離が挙動に影響しない
ことを実機で確認した。
...
**結果**: 2026-09-07、cudacodon実機で`./387Py_kernel_maxd14_final -g`
(bare、他の引数なし)を実行。**完全に想定通りの挙動**。N=5〜20は一瞬で
完走(N=5のみ`ok`サフィックスなし、当初ご提示いただいたサンプル出力と
完全一致)。N=21: `total=314666222712`(オラクル一致)、`0:03:23.301`。
N=22: `[crunner-input-missing]`でクリーンに停止(想定通り、当時は
まだN=22のCRunner入力ファイルが未構築だったため)。クラッシュや無限
ループなし。`387_validate.sh`自体は`OK=14 FAIL=1`だったが、FAILは
harness側のバグ(`{N:2d}`が1桁のNの前に半角スペースを入れる仕様に
対し、`grep -qE "^5:"`が行頭一致で拾えなかっただけ)——実際のログには
N=5の行が正しく出力されており、コード側の問題ではない
(`^[[:space:]]*5:`に修正済み)。
...
**結果**: 2026-09-07、cudacodon実機で`bash 388_validate.sh`実行、
`OK=10 FAIL=0`で`388 PASSED`。`-d`を先頭に置いても末尾に置いても
N=6の`total=4`という結果自体は変わらず、`-d`ありの時のみ
`[debug-mode] enabled via -d: gpu_log_level=1 ...`のバナー行が出て、
`-d`なしでは出ないことを確認した(`leading_-d_banner_present`・
`trailing_-d_banner_present`・`no_d_means_no_banner`すべてOK)。
...
**結果(r3)**: r3単体では実機実行していない——直後に鈴木さんから
`.cu`ファイル自体のリネーム(r4)のご依頼をいただき、続けて着手した
ため、r3単独の実機確認は行わなかった。r3の変更(ログディレクトリ名
への`REV_TAG`導入)自体は、r4の実機テスト(下記)で
`388_crunner_logs`ディレクトリが正しく使われていることとして
暗黙に再確認されている。
...
**結果**: 390単独では実機実行していない——391(段階的GPU-CPU突き
合わせ)に統合され、391の実機テストでまとめて確認された。詳細は
391のセクション参照。
...
**結果**: r1は実機で実行したところ、CPU側ループ(単一スレッドの
`while`ループ)がボトルネックとなり20分以上出力ゼロのまま停止して
いるように見えた。`nvidia-smi`でGPU使用率0%(アイドル水準)を確認、
鈴木さんがCtrl+Cで安全に中断(実害なし)。有効な結果データは無し
——根本原因と修正内容はr2訂正セクション参照。
...
**結果**: 2026-09-07、cudacodon実機で`bash 392_validate.sh`実行、
`OK=12 FAIL=0`で`392 PASSED`(`real 0m1.340s`)。`nvcc`(device)・
`gcc`(CPU専用テストハーネス)とも実機でビルド成功。想定通り
ビルドのみで即終了、GPU/CPU実行時間は消費していない。

**追記(`.py`のリネーム)**: 直後に鈴木さんから「`.py`も392で」との
ご依頼をいただき、`391Py_kernel_maxd14_final.py`を`392Py_kernel_
maxd14_final.py`へ純粋にリネーム(実行コードは`import gpu`以降
byte-identicalとスクリプトで確認済み)。作業中に`REV_TAG`が
**388から一度も更新されていなかった**ことを発見(389・391の間で
見落としていた)、`REV_TAG="392"`へ修正。`392_validate.sh`にも
`392Py`のcodonビルド確認を追加し、2026-09-07に再実機実行、
`OK=17 FAIL=0`で`392 PASSED`(`nvcc`・`gcc`・`codon`3ビルドとも成功、
`rev_tag_is_392`確認済み)。
...
## 395b 結果(確定)・レジスタ常駐化は**+3.5%の退行**、4.2%の正体は「起動経路」

**実行完了。** 全セルオラクル一致、G(`-g 21 21`、`-d`なし、sched再生成込み)で
漏れなし、バイナリ既定800の確認OK、CPU等価ゲート通過。

| セル | 内容 | `kernel_ms` |
|---|---|---:|
| I0 | `cmp 394f_input_L3_sched.bin` vs 本番`.sched394f.bin` | **バイト一致** |
| I1 | 395aバイナリ、直接、本番sched、800 | 139,166 |
| I2 | 395aバイナリ、直接、394f L3、800 | 139,181(I1比+0.01%) |
| **K1** | **395bバイナリ**、直接、本番sched、800 | **144,032(I1比 +3.5%)** |
| G | 395bバイナリ、`-g 21 21`(ディスパッチャ) | 138,650(K1比 −3.7%) |
| A | 395bバイナリ、484 | 201,705(395aのA 191,780比 +5.2%) |

### 事前登録予測の判定

| # | 予測 | 実測 | 判定 |
|---|---|---|---|
| 1 | I0はバイト一致、一致ならI1≈I2 | 一致、I1≈I2(+0.01%) | 的中 |
| 2 | **K1 vs K0: −2〜−5%** | **+3.5%** | **反証(逆方向)** |
| 4 | GはK1±1% | −3.7% | 外れ(=経路効果が再現) |

### 4.2%の正体: ファイルではなく起動経路

2つのschedファイルは同一で、直接実行なら両方139.2秒。**同じバイナリを
同じファイル・同じstrideで、Codonディスパッチャ経由で起動すると約4%
速い**(395a: 133,580 vs 139,179、395b: 138,650 vs 144,032)。機構は
未解明——395cで、他プロセスがCUDAコンテキストを保持しているだけで
速くなるのか(コンテキスト/メモリ配置)、実行中のSMクロックが違うのか
(サンプラで実測)を切り分ける。本番経路(`-g`)は既にこの4%を享受して
いるが、**直接実行による全測定は本番より約4%悲観的**だったことになる。

### カーネル側の結論: 局所的な制御フロー編集はクローズ(7戦7敗)

395bはpush/popの周りに`if (save_sp != 0)`を2つ加えた——制御フロー形状の
変更であり、240 / 266-269 / 273 / 326 / 358 / 359 に続く**7戦7敗**。
395a r2(命令数削減)は中立、395b(レイテンシ短縮のつもりの分岐追加)は
退行。IPC 0.28のストール律速領域では、ALU削減は効かず、分岐追加は
再収束コストで確実に負ける。**このカーネルの内部を局所的にいじる軸は
ここでクローズする。** 残るカーネル側の手は構造的なもの(フレーム圧縮に
よる占有率天井の引き上げ)のみで、非カーネル側(起動経路、順序)が実際に
利得を出してきた。395c(pop→decode直結)は同じ失敗クラスなので**行わない**。

**395bのカーネル変更は差し戻す(395cで395a r2へ復帰)。**
