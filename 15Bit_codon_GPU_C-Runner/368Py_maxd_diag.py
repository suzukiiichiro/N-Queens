...
VERSION_TAG:str="368 maxd_diag: DIAGNOSTIC-ONLY, ADDITIVE REVISION. Executable code region is byte-identical to 366 outside five clearly delimited deltas (one pure insertion of a diagnostic class+three functions, two 2-word CLI-gate modifications matching the 361/365/366 pattern, one pure insertion for the dispatch branch), verified by a static check that strips/reverses those spans before hashing against 366s reference hash for the identical region. The deltas are: (1) ===368-INSERT-BEGIN/END=== -- new class MaxdDiagStats and three new functions (schedule_depth_for_task_diag, scan_maxd_diag_for_tasks, check_required_maxd_for_N_diag) that are a parallel, bounds-safe COPY of 366s schedule_depth_for_task/check_required_maxd_for_N, NOT a modification of them (both originals remain byte-identical and reachable via bench_mode==34); (2)-(3) ===368-CLIGATE-COMMENT-BEGIN/END=== and ===368-PRESETGATE-COMMENT-BEGIN/END=== -- comments plus a 2-word addition (or bench_mode==35) to the same two CLI gate lines 361/365/366 modified, applied preemptively from the start; (4) ===368-DISPATCH-INSERT-BEGIN/END=== -- new bench_mode==35 dispatch branch, appended after the existing bench_mode==34 block. Context: a real-hardware run of 366/bench_mode=34 for N=22 (records=28,719,035, preset_queens=7) produced no [maxd-check-done] line; dmesg (via sudo) showed two segfault at 0 events for the 366Py_maxd_chec binary (error 4 then error 6), with no OOM-killer entry and no allocation-failure message -- inconsistent with the initial working hypothesis that the 367 memory ceiling (10GB, 70 percent of a 15GB host) was the cause. Inspection of schedule_depth_for_task() found meta_next indexed by fu=raw&31 (0..31 by construction, 5-bit mask) at three call sites, against a meta_next table with exactly 28 elements (indices 0..27) -- a structural range mismatch that N=21 apparently never triggers but N=22 may. 368 adds a bounds-safe survey (fu_min/fu_max/oob_count/first-occurrence repro) to confirm or refute this on real N=22 data without crashing, so a correctness fix (extending or reworking meta_next, not in scope here) can be designed with an actual repro case rather than a guess. Suzukis next action is running bench_mode=35 for N=22 (single-variable discipline vs 366, diagnostic-only)."
