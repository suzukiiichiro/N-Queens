...
# 398_validate.sh
#
# rev398-r2 -- same measurement as 398, with the one bound that blocked it.
#
# WHAT STOPPED 398
#   The mode-37 CRunner dispatch is gated on "N >= 21", so -g 20 20 and
#   -g 19 19 took the Codon GPU path: correct answers (39,029,188,884 and
#   4,968,057,848, both ok) but no CRunner log, no scheduled input, nothing
#   to profile. The first two stages that build that input live inside the
#   Codon binary, so no external script can produce it either.
#   r2 introduces CRUNNER_MIN_N = 19 and uses it for mode 37 ONLY. The other
#   nine N>=21 gates are untouched, and the harness gates that too.
#   398's anchor did hold: 133,197.422, +0.009% of 133,185.
#
# WHERE THIS STANDS
...
REV="398_r2"
PY_SRC="${PY_SRC:-398_r2Py_kernel_maxd14_final.py}"
PY_BIN="${PY_BIN:-398_r2Py_kernel_maxd14_final}"
PREV_PY="${PREV_PY:-398Py_kernel_maxd14_final.py}"
HELPER_SRC="${HELPER_SRC:-rev386_validation_helpers.py}"
CU_SRC="${CU_SRC:-398_r2_kernel_maxd14.cu}"
CU_BIN="${CU_BIN:-398_r2_kernel_maxd14}"
PREV_CU="${PREV_CU:-398_kernel_maxd14.cu}"
CRLOG_DIR="${CRLOG_DIR:-398_r2_crunner_logs}"
CODON="${CODON:-codon}"
...
{ grep -q "^# ${REV//_/-} " "$PY_SRC" && [[ "$NOTE_LINES" -ge 20 ]]; } && pass "py_revision_notes_present ($NOTE_LINES comment lines)" \
  || fail "py_revision_notes_present" "no '# ${REV//_/-} ...' note block in $PY_SRC"
...
grep -qE '^REV_TAG:str="398_r2"' "$CODE" && pass "source_rev_tag_is_398_r2" || fail "source_rev_tag_is_398_r2" "wrong REV_TAG"
grep -q 'CRunnerEntry(14,"./398_r2_kernel_maxd14","NQ_EXTRA_CTX=1 "' "$CODE" && pass "source_table_points_at_398_r2_and_keeps_the_treatment" || fail "source_table_points_at_398_r2_and_keeps_the_treatment" "table entry wrong"
# the one behavioural change in r2, and the proof that it is only one
grep -qE '^CRUNNER_MIN_N:int=19' "$CODE" && pass "source_crunner_min_n_is_19" || fail "source_crunner_min_n_is_19" "CRUNNER_MIN_N missing or not 19 -- -g 20 20 would take the Codon path again and there would be nothing to profile"
grep -q 'if use_gpu and N>=CRUNNER_MIN_N and bench_mode==37:' "$CODE" && pass "source_mode37_uses_the_constant" || fail "source_mode37_uses_the_constant" "the mode-37 gate still has a literal bound"
OTHER_GATES=$(grep -c 'if use_gpu and N>=21' "$CODE")
[[ "$OTHER_GATES" == "9" ]] && pass "source_other_modes_untouched (nine N>=21 gates remain: modes 30/31, 32, 33, 34, 35, 36, 38, 39 and the closing guard)" \
  || fail "source_other_modes_untouched" "$OTHER_GATES gates with the literal 21 remain, expected 9 -- r2 must lower mode 37 only"
...
if [[ -f "$PREV_PY" ]]; then
  ...
  [[ "$PR_" == "4" && "$PA_" == "5" ]] && pass "py_diff_fingerprint_vs_398Py (removed=4 added=5: VERSION_TAG, REV_TAG, table, the mode-37 gate, plus the CRUNNER_MIN_N constant)" \
    || { fail "py_diff_fingerprint_vs_398Py" "removed=$PR_ added=$PA_, expected 4/5"; diff "/tmp/${REV}_prev_code.py" "$CODE" | head -30 | sed 's/^/      /'; }
else info "py_diff_fingerprint_vs_398Py" "skipped"; fi
...
if [[ -f "$PREV_CU" ]]; then
  ...
  [[ "$CA" == "$CB" ]] && pass "cu_whole_code_region_identical_to_398 (${CB:0:16}...)" || fail "cu_whole_code_region_identical_to_398" "r2 must be a rename"
else info "cu_whole_code_region_identical_to_398" "skipped"; fi
...
for n20 in $PROFILE_NS; do
  ...
  if [[ ! -f "$gcr" ]]; then
    fail "crunner_path_taken[N=$n20]" "no $gcr. Either CRUNNER_MIN_N did not take effect (check the console for a plain solver row instead of a CRunner run) or required_maxd for N=$n20 exceeds 14 -- look for [crunner-unsupported] in $CRLOG_DIR/dispatch.log"
    continue
  fi
