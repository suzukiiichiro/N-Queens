...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 398 (2026-09-09)
...
"""
398、四点セットを組みます。397は破棄し、カーネルは396-r2（＝395a r2と同一）に戻します。398の四点セットです。静的ゲートは **OK=16 FAIL=0**。

## カーネルは395a r2に戻ります

```
コード領域全体 sha256 : c5dbde3f1002229b…  （396-r2と一致）
カーネル領域   sha256 : ebd7f523deb59134…  （395a r2に復帰）
```

397の詰め替えが残っていないことも静的にゲートしています（`MAXD14_PACK_MASK` が0件）。

## 構成

**ncu権限プローブを高価な処理の前に置きました。** 3秒で判定します。sudo自動フォールバックと `reclaim` も入れてあります。

入力ファイル名は**推測しません**。`-g 20 20` を走らせ、CRunnerログの `src=` から実際のパスを読み取ります。N=20が maxd14 経路を通らなかった場合もそこで検出されます。

プロファイルは N=20 → N=19 の順、各4パスで、**`WarpStateStats` を必ず先頭**に置いています。U5に直結するので、予算が尽きるとしても最初に確保します。

アンカーは397で確立した `NQ_EXTRA_CTX=2` の直接実行です。ディスパッチャ1回ぶん、2.5分節約できます。

## 事前登録 — W4で私は「(b)発散」に賭けます

**表明する予測は sectors/request ≥ 2、つまり発散が支配的、です。**

根拠は、ローカルメモリがスレッド間インタリーブ配置であること。ワープ内の全レーンが同じワード添字を触れば完全合体しますが、**DFSの深さがレーンごとに違えば添字が散ってセクタが割れます**。深さの発散はこのアルゴリズムに内在します。

**≤ 1.2 が出れば(b)は棄却**され、量が効いていることになります。**そのときに限り**、98ビットを96に収める工夫に価値が出ます。

**W6が今回いちばん効く設計だと思っています。** N=21の Occupancy と Scheduler Statistics は396で実測済みなので、N=20の値を直接突き合わせられます。代表性を仮定ではなく検証にできる。達成オカパンシー ±3ポイント、No Eligible ±5ポイント以内なら、N=20をN=21の代理として使ってよい。
...
破棄しますが、3つ残ります。**フレームは実際に208→160まで縮む**こと、**ldとrdの構造的非対称性**、そして**V4の確立**です。負けた実験としては、悪くない収穫だと思います。
...
VERSION_TAG:str="398 PY-REVTAG: pure rename of 396-r2Py; the dispatch table names ./398_kernel_maxd14 and keeps the env_prefix NQ_EXTRA_CTX=1 adopted in 395c-r6. The .cu is a rename of 396-r2, so the kernel region is back to the 395a r2 sha256 ebd7f523 and 397 is discarded. 398 changes nothing functional: it profiles N=20 and N=19, each with its own 394f schedule and therefore balanced by construction, to answer the warp-stall question that has never completed on N=21 and to decide whether local-memory cost is byte volume or address divergence."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./398_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
