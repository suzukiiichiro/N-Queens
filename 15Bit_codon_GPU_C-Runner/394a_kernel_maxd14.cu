/* ===================================================================
 * 394a_kernel_maxd14.cu
 *
 * Derived mechanically from 389_kernel_maxd14.cu by 394a_run_csrc_ncu.sh:
 * this 15-line comment block is PREPENDED and nothing else is
 * changed. Dropping the first 15 lines reproduces 389_kernel_maxd14.cu
 * byte for byte (checked below, gating).
 *
 * Purpose: a -lineinfo diagnostic build for ncu SourceCounters.
 * This is NOT a production binary -- crunner_dispatch_table() in
 * 394aPy deliberately still points at ./389_kernel_maxd14, the same
 * separation 320 kept between its debug build and its timing build.
 *
 * LINE NUMBER OFFSET vs 389_kernel_maxd14.cu: +15
 * =================================================================== */
/*
 ...
 *   /usr/local/cuda/bin/nvcc -O3 -arch=sm_86 -o 389_kernel_maxd14 389_kernel_maxd14.cu
 * Run (device):
 *   ./389_kernel_maxd14 <N> <in_soa7_bin> <out_results_bin> [expected_total]
 *   e.g. ./389_kernel_maxd14 21 constellations_N21_6.bin.soa_ref_361.bin.maxd14only_363.bin \
 *          /tmp/gpu_results.bin 314666222712
 * Build (host-only CPU test, no CUDA toolkit needed, this sandbox):
 *   gcc -O2 -Wall -Wextra -o 389_kernel_maxd14_cputest 389_kernel_maxd14.cu -x c -lm
 * Build (host-only CPU test, OpenMP-parallelized outer loop):
 *   gcc -O2 -fopenmp -Wall -Wextra -o 389_kernel_maxd14_cputest_omp 389_kernel_maxd14.cu -x c -lm
 *
