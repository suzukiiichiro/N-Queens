/*
 * 381d_hybrid_kernel_nospin.cu
 *
 * rev381d — CORRECTS the REAL root cause found via -lineinfo/ncu
 * measurement (matching 375's "prove, don't assume" discipline):
 * 381c's kernel_ms=569,157 (still ~2.8x worse than 374's 201,232)
 * was NOT primarily atomic contention (381c's active_static_workers
 * fix over 381b only bought ~7%) -- ncu SourceCounters showed
 * specific instructions (ATOMG.E.ADD / SHFL.IDX / VOTEU.ANY, exactly
 * the termination-check spin loop) with "Instructions Executed" =
 * 5,913,220,538 (5.9 BILLION). Threads that finish their static task
 * supply early were busy-spinning with NO backoff, re-checking two
 * atomics every cycle, for as long as the SLOWEST thread in the
 * kernel needed -- exactly the tail-effect imbalance 375 first
 * identified, now manifesting as a literal multi-billion-iteration
 * spin loop instead of a warp-level BSYNC stall.
 *
 * FIX (scoped to this revision's K_THRESHOLD=13 case specifically):
 * at K_THRESHOLD>=13 (K_slots>=MAXD14_ANCESTOR*2), local-stack
 * overflow is PROVABLY impossible (the local capacity already equals
 * 374's own stack bound) -- so there is never anything for a thread
 * to "help drain" from the shared queue in the first place. The
 * entire queue-draining fallback phase (and its spin-wait) is now
 * skipped in that case: a thread simply returns once its static task
 * supply (idx>=m) is exhausted, exactly like 374's own kernel does.
 * This is NOT yet a general fix for K<13 (where overflow, and
 * therefore a genuine need to wait for stragglers, is possible) --
 * revisiting the termination protocol for the K<13 sweep (381e+) is
 * flagged as separate future work, not resolved here.
 *
 * Everything else (process_task_hybrid, run_hybrid_episode, the
 * Vyukov queue, the active_static_workers counter from 381c) is
 * UNCHANGED -- only kernel_dfs_hybrid_maxd14's fallback-phase branch
 * gained a K_slots-based skip.
 */
