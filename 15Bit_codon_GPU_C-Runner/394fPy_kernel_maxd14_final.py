...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 394f (2026-09-08)
...
"""
解析しました。**主仮説M2はまた外れ、M1(warp内)で確定です。ただし今回は機構までソースで追えました。**

## 394e 結果

| セル | 入力 | MB | `kernel_ms` | vs R484 |
|---|---|---:|---:|---:|
| R484 / R485 | 生 | 484 / 485 | 201,234 / 200,061 | — / −0.58% |
| B484 | base | 484 | 296,899 | +47.5%(E1を0.001%で再現) |
| **B485** | base | **485** | **282,374** | **+40.3%** |
| **B483** | base | **483** | **290,851** | **+44.5%** |
| S485 | shaped | 485 | 260,624 | +29.5%(S484比−0.17%) |

同じ32件のwarpグループのままper-thread列だけを変えても、罰は**+40〜45%残りました**。stride移動で回復したのは3〜7ポイント——これがM2(尻尾)の全寄与です。6セルすべてオラクル一致。

## 機構をソースで確認しました

`interleave_broad_markdist_tail_subparts()`(broadmarktail base)は、各チャンクをF17(39,755件)/GH(15,867件)/R(残り)の3パートから**レコード単位でラウンドロビン**に組み立てています(phase 0〜3で `R, F17, R, GH, R` の順など)。**重いレコードをすべてのwarpに均等に散らす**設計——rev 111-113期のwarp間均衡の発想です。

一方、C版カーネルは外側grid-strideループの末尾(`BSYNC B4`、394aのSASS `289e20`)で**毎周回、32レーン全員を再収束**させます。warpの1周回は最遅レーンで決まるので、「重い6レーン+軽い26レーン」のwarpは6レーンに引きずられる。生成器の自然順は隣接する(i,j,k,l)——ほぼ同型の部分木——を同じwarpに置くため max≈mean。**これが生順序の隠れた利点で、並べ替えはそれを設計的に壊していました。** 394cのD(968、−1.0%)が無傷だったのは、cap超過で"stable packing"経路を外れグループ全体がスコア順に近くなった(=コスト同質なwarp)ためと読めます(未確認)。

結論: broadmarktail→chunkshape148の並べ替え軸は本番C経路について**不採用**。ただし**warp内同質性が30〜47%を動かすレバー**だと分かった以上、軸自体は閉じません。生を超える並びがあるかを直接測ります。

## 394f — 順序プローブ(コード変更ゼロ、sudo不要)

`394f_permute_soa7.py`で生の入力の順列を作り(順序非依存チェックサムで順列性を保証)、C版を直接叩きます。

| セル | 内容 | |
|---|---|---|
| R | 生 | アンカー |
| **X** | **ランダムシャッフル** | **対照**。隣接性が要因なら+30〜50% |
| L1 | `(col, ld, rd)`安定ソート | ルート状態の類似 |
| L2 | `(−popcount(free), col, ld, rd)` | コスト代理を先に |
| L3 | `(markctrl, ctrl0)`安定ソート | **インターリーブなしのfuncidグルーピング** |

5セル約20分。`results[tid]`を回収してmax/mean・CVをスレッド間均衡の代理として記録します。

事前登録: **X ≥ +30%**(±5%以内なら隣接性は要因でなく、インターリーブ固有の問題)、**L1 ≤ R−3%**(主予測。L1 ≥ Rなら生は構造ソートで超えられない)、L3 ≤ R(悪いのはインターリーブだけ、という上の読みの直接検証)。

```bash
STATIC_ONLY=1 bash 394f_run_order_probe.sh
              bash 394f_run_order_probe.sh      # R X L1 L2 L3
```

X大・L1小なら「似たもの同士のwarp」を395系の正式な軸にし、chunkshape148のスコアキーそのものでのグローバルソートを394b r3の最適MAX_BLOCKSと組み合わせます。X小なら軸をクローズして395a(`nibble_op`二重計算除去)へ。
"""
...
VERSION_TAG:str="394f PY-REVTAG: pure rename of 394ePy to 394fPy plus the dispatch table referencing ./394f_kernel_maxd14 (header-only rename of the .cu). ZERO functional change otherwise. 394f is a C-side measurement revision: 394f_permute_soa7.py builds permutations of the RAW filtered input (random shuffle as the control; stable sorts by root state, by a free-count cost proxy, and by schedule) and 394f_run_order_probe.sh drives the CRunner binary on them directly at 484x32. This .py is not executed by that measurement."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./394f_kernel_maxd14","","[gpu-run-done]","[gpu-run-correctness]"),
  ]
