...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 400-r2 (2026-09-09)
...
"""
21:50ですね。**掃引をN=19、確認をN=20、オプトインをN=21に下げます。** N=19は1実行2.1秒なので、掃引30本が2分弱で終わります。400-r2、四点セットです。静的ゲートは **OK=16 FAIL=0**。
...
ビルドと入力生成、ncuを含めて**通しで約15分**です。測る中身は1つも変えていません（`.cu` は400のリネーム、コード領域全体 `6821abd1…` 一致）。
...
## 小規模化の偏りを、ハーネスに織り込みました

`k_per_thread` はNとともに落ちます。stride 163,840 で N=19 は **5.5**、N=21 は 12.4。**`k` が小さいほど394fの均衡が効かないので、デバッグ規模は掃引が探している大stride構成に対して不利に働きます。**

そこで **Z3の最終判定をN=20の確認セルへ移しました**。N=19で1%を超えなくても `Z3_REFUTED` は出さず、`Z3_not_reached_at_debug_scale` と理由を出して先へ進みます。**棄却するならN=20の結果で棄却します。**

## その他の変更

- オラクルをN=18〜23の表から自動引き（ハードコード廃止）
- 確認Nの入力も**ディスパッチャで生成し `src=` から読む**（`IN21` のハードコード廃止）。N=20の入力が無い環境でも動きます
...
これなら日付が変わる前に結果が見られるはずです。掃引が通れば、そのまま `NPROF=20 NCONF=21` で本番規模に上げる形が良いと思います。


ハーネスの集計にバグがあります。**TSVにN列を入れ忘れた**ので、N=19の掃引とN=20の確認が同じ行にマージされ、`spread 154%` という異常値が出ています。生ログから読み直します。ハーネスの集計バグ（TSVにN列が無く、N=19とN=20が混ざった）は私の不備です。生ログから読み直したところ、**結果は極めて明快でした。**

## Z3 — 棄却。しかも劇的に

N=19掃引、全走行オラクル一致。

| BLOCK | MB | stride | w/SM | k/thr | mean ms | vs base |
|---:|---:|---:|---:|---:|---:|---:|
| 64 | 400 | 25,600 | 10 | 35.0 | 2,097.4 | **−1.07%** |
| 32 | 800 | 25,600 | 10 | 35.0 | 2,120.2 | — |
| 128 | 200 | 25,600 | 8 | 35.0 | 2,258.9 | +6.54% |
| 64 | 640 | 40,960 | 16 | 21.9 | 3,849.1 | **+81.6%** |
| 32 | 1280 | 40,960 | 16 | 21.9 | 3,906.7 | +84.3% |
| 64 | 1280 | 81,920 | 32 | 11.0 | 10,699.4 | **+404.7%** |
| 128 | 640 | 81,920 | 32 | 11.0 | 10,721.9 | +405.7% |
| 128 | 1280 | 163,840 | 48 | 5.5 | 15,416.0 | +627.1% |
| 128 | 960 | 122,880 | 48 | 7.3 | 16,241.9 | **+666.1%** |

**オカパンシーを上げると6〜7倍遅くなります。**

そして決定的なのは、**同じstrideならBLOCKが違ってもほぼ同値**であることです（64:640 と 32:1280 で 3,849 対 3,907、64:1280 と 128:640 で 10,699 対 10,722）。

> **効いているのはBLOCKでもワープ数でもなく、strideです。**

**Z2は成立**しました。stride 25,600固定なら BLOCK=64 は −1.07%、BLOCK=128 は +6.54%（200ブロック ÷ 80SM = 2.5 で、SM間の割り切れなさが効いています）。**梱包はほぼ中立**です。

## なぜ7倍になるのか

総仕事量は同じで、スレッドを増やせば並列度は上がるのですから、**遅くなること自体があり得ません**——負荷が均衡していれば。

`k_per_thread` が 35 → 5.5 に落ちると、スレッドあたりレコードが少なすぎて平均化が効かず、最も遅いスレッドが全体を律速します。しかもこれは単なる平均化不足では説明がつかない大きさです。

**394fのスケジュールには stride 25,600 が焼き込まれています。** 394fはレコードを「位置 i, i+25600, i+2·25600 … が均衡する」ように並べています。stride を 163,840 にすると、各スレッドが拾うのは**その並びの中の特定の位相だけ**——エイリアシングが起きて、系統的に高い（あるいは低い）ものばかりを拾うスレッドが生じます。

## Z4について

base（32:800）と best（64:400）でストール内訳が完全に同一だったのは正常です。**どちらも stride 25,600 で同じ25,600スレッド**、梱包が違うだけなので。時間差も1%なので、説明すべき変化がそもそもありません。ハーネスの `Z4 FAILED` は判定条件の設計ミスで、実質は情報なしです。

## 次の一手 — 1コマンドで決まります

**オカパンシーの軸は「効かない」のではなく、「スケジューラが追随しないので試せていない」だけです。**

394fのスケジュールを新しいstride用に再生成できれば、公平な再試行ができます。できなければ、この軸はツール側で塞がれていることになります。

お手数ですが、これを見せていただけますか。

```bash
grep -n "25600\|stride\|STRIDE" 394f_permute_soa7.py | head -30
head -40 394f_permute_soa7.py
```

- **strideが引数か定数として存在する** → 122,880用のスケジュールを作って掃引をやり直します。**オカパンシー100%を初めて公平に測れます**
- **存在しない（暗黙に25,600前提）** → 394fの改修が必要で、それ自体が別リビジョンになります

今夜はここまでにして、明日この確認から始めるのが良いと思います。**今日だけで395c〜400の11リビジョンを回して、律速要因（wait 45%）の特定と、並び順・オカパンシー両軸の見極めまで到達しています。** 十分な進捗だと思います。


これは重要です。**394fは私が思っていたものと違いました。**
...
**strideという概念がありません。** 単に `(markctrl, ctrl0)` で安定ソートしているだけです。

## 私の誤りを訂正します

396以降のリビジョンノートに、私は繰り返しこう書きました。

> 「394fは79ラウンド全体でスレッドごとの累積負荷を揃えている」

**これは検証しないまま述べた推論で、誤りです。** 394fは負荷を均衡させていません。**似たスケジュールワードのレコードを隣接させているだけ**です。

396・396-r2・398・399のノートに同じ誤りが入っています。訂正が要ります。

正しい理解はこうです。ワープを共有する32レコードはファイル上の連続32個で、それらは `(markctrl, ctrl0)` が同一かごく近い。**つまり同じ制御フローを辿るのでワープ内が揃う。** 399で測った「shuffleは35%遅い、レーン稼働率10.53→6.78」は、まさにこれです。**394fの価値はワープ内同質性であって、負荷均衡ではありませんでした。**

## するとstrideで7倍になる理由は別にあります

ワープ内同質性はstrideに依りません（どのstrideでも連続32個）。変わるのは**SMあたりの常駐スレッド数**です。

```
スタックフレーム 208 B/thread、L1は128 KB/SM

10 warps/SM :  320 threads × 208 B =  66.6 KB   → 2,120 ms
16 warps/SM :  512 threads × 208 B = 106.5 KB   → 3,849 ms  (+82%)
32 warps/SM : 1024 threads × 208 B = 213.0 KB   → 10,699 ms (+405%)
48 warps/SM : 1536 threads × 208 B = 319.5 KB   → 16,242 ms (+666%)
```

**ローカルメモリの作業集合がL1に入らなくなっています。** 1レコードあたり16.4万回のローカルアクセスがL2/DRAMへ溢れれば、この桁の悪化は説明がつきます。

そしてこれが正しければ、**397の着想は結局正しかった**ことになります。フレームを縮める意味は「転送量を減らす」ことではなく、**「L1に収まるワープ数を増やす」**ことでした。208 B → 104 B なら、同じL1で倍のワープが載ります。

## 明日の最初の一手 — 1行で試せます

このカーネルは `__shared__` も `__syncthreads` も使いません（ptxas `used 0 barriers`）。**sm_86 の 128 KB はL1と共有メモリで分割され、既定では共有側にかなり割り当てられています。** ローカルメモリが使えるL1を最大化する1行があります。

```c
cudaFuncSetAttribute(kernel_dfs_iter_gpu_maxd14,
                     cudaFuncAttributePreferredSharedMemoryCarveout, 0);
```

396のときに提案して、そのままになっていたものです。**ホスト側1行、カーネル領域のsha256は不変。**

401の設計はこうなります。

1. **394fに関する記述の訂正**（4リビジョン分のノート）
2. **カーブアウトの有無 × stride 5点**を N=19 で掃引（約5分）
3. 効けば、**397のフレーム縮小を正しいやり方で再挑戦する根拠**が立つ

カーブアウトがstrideスケーリングを改善すれば、L1容量説が確定し、**「フレームを縮める → ワープを増やせる → wait 45%を叩ける」**という一本の筋道が通ります。効かなければL1容量説は棄却で、strideの7倍は別の機構です。

22時を回りました。**今日は395c〜400の11リビジョンで、律速要因の特定（wait 45%）、並び順の軸の見極め、オカパンシーの壁の正体まで到達しています。** ここで区切って、401は明日の最初にしましょうか。四点セットは今書いておくこともできます。
...
VERSION_TAG:str="400-r2 PY-REVTAG: pure rename of 400Py; the table names ./400_r2_kernel_maxd14 and keeps env_prefix NQ_EXTRA_CTX=1, CRUNNER_MIN_N=19 and the NQ_BLOCK knob. No functional change from 400. r2 exists only to rescale the harness: sweep at N=19 instead of N=20, confirm at N=20 instead of N=21, optional check at N=21 instead of N=22, which turns a 45 minute run into about 15. NPROF, NCONF and NBIG restore production scale in one line."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./400_r2_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
