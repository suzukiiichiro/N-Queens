...
# 381a_validate.sh
#
# rev381a — First real GPU test of the seed_kernel + hybrid_kernel
# two-stage design (380 spec). K_THRESHOLD fixed at 13 (see file
# header of 381a_seed_hybrid_kernel.cu): this run's only job is
...
set -u
CUSRC="${CUSRC:-381a_seed_hybrid_kernel.cu}"
BIN="${BIN:-381a_seed_hybrid_kernel}"
...
for sym in seed_kernel_maxd14 kernel_dfs_hybrid_maxd14 seed_task run_hybrid_episode fq_push fq_pop FQSlot LocalStack; do
...
echo "381a PASSED: seed_kernel + kernel_dfs_hybrid_maxd14 (K_THRESHOLD=13)"
echo "reproduce 374's own oracle on real N=21 data. Compare kernel_ms (in the"
echo "run log above, note this now includes seed_ms as a separate phase) to"
echo "374's own kernel_ms (~201,232ms in the current environment) -- a small"
echo "regression from the extra kernel launch is expected and OK at this stage."
echo "381b+: sweep K_THRESHOLD downward on real GPU hardware for actual speed data."
exit 0
