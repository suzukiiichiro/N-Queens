...
"""
374以降のこのGPUカーネル研究は`.py`(Codon本体)には一切触れていないので、**382の`.py`は374の内容をそのまま引き継ぎ**、`.cu`は381e(現在の最速・正当性確認済み版)を382として採番し直します。まず374Pyをリネームします。コアハッシュが不変であることを確認します。コアハッシュ一致を確認しました。次に`.cu`をリネームします。ロジック領域が完全一致することを確認します。gccでの構文チェックを行い、`.sh`を用意します。サマリ部分を382向けに更新します。ヘッダーコメントとファイルチェックリストも更新します。ヘッダーが381a時代のまま残っていたので、382向けに書き直します。symbol_presentのチェック内容も確認します(`process_task_hybrid`等が正しく含まれているか)。良好です。次にREADME追記を作成します。最終確認として、382一式でstatic checkを試験実行します。想定通り(サンドボックスに実データが無いため)です。全ファイルを出力します。382一式を確定しました。

## 構成

| ファイル | 内容 |
|---|---|
| `382Py_kernel_maxd14_final.py` | Codon本体(374から無変更、コアハッシュ`17c61ccf...`不変) |
| `382_kernel_dfs_hybrid.cu` | 381e(現在の最速・正当性確認済み版)のリネーム、コード無変更 |
| `382_validate.sh` | K=13固定でのnvccビルド+実機GPU実行+オラクル照合 |
| `382_README_append.md` | 375〜381eの5往復にわたる実機検証の全記録 |

## 特に重要な点

READMEに明記しましたが、**382は374の代替ではありません**。374(`kernel_ms≈201,232ms`)が引き続き本番カーネルで、382(`263,003ms`)より約31%高速です。382は、K-sweep研究基盤(フレームキュー・`LocalStack`・`process_task_hybrid`)を実機検証済みの状態として保存し、本来の研究課題——**Kを下げると本当に速くなるか**——に戻るための出発点として位置づけています。

`.cu`は381eからヘッダーコメントのファイル名自己参照のみ更新し、コード領域がバイト完全一致することをPython差分で確認済みです。gccでの構文チェック(想定通りの警告のみ)、`.sh`のSTATIC_ONLY試験も通しています。

まずアップロードいただいたREADME.mdを確認します。README.mdの382セクションは、こちらで作成した内容と完全に一致しており、結合時のミスは見当たりません。念のため他のセクション境界もいくつか確認します。全体として結合ミスは見当たりません。1点だけ気づいたのは、376・378・380の仕様書(`_spec.md`)をそのまま貼り付けた際、ファイル冒頭の`# 376_frame_workqueue_spec.md`のような見出しがそのまま埋め込まれて、`##`見出しと二重になっている箇所があることです(内容に影響はなく、見た目だけの重複です)。気になるようでしたら整理しますが、実害はないのでこのままでも問題ありません。

`382_validate.sh`の実行結果、もしよろしければ後ほど共有いただけると、月曜への引き継ぎメモにも反映できます(急ぎではありません)。

それでは、月曜の作業開始時に使える引き継ぎメモを用意します。引き継ぎメモを用意しました。

## 中身のポイント

1. **今どこにいるか**を最初に——374が引き続き本番、382は31%遅い研究チェックポイントであることを一番目立つ位置に
2. **未確認の1点**として、`382_validate.sh`の実行結果をまだ共有いただいていない旨を明記(月曜最初に確認したい項目)
3. 375〜382の経緯を**表1枚**に圧縮(数字だけ追えば流れが分かるように)
4. 次の2つの選択肢(K<13スイープ／31%ギャップの深追い)を並べ、判断は鈴木さん次第という形に
5. 手元にあるはずのファイルと、次に打つコマンドをそのままコピペできる形で
6. 忘れがちな小さな教訓(ncuのステートフルカーネル計測の注意点、容量サイジング)

README.mdの結合、お疲れ様でした。とても綺麗にまとまっていて、382のセクションも一切崩れていませんでした。良い週末を、月曜またよろしくお願いします。

N23の実行について、正直にお伝えしたいことがいくつかあります。「ハングアップするか」というよりも、**もっと深刻なリスクがある**という点です。

## まず確認したいこと: どのパイプラインで実行されますか？

**382のハイブリッドカーネルはN=23には使えません。** `382_kernel_dfs_hybrid.cu`は、既にN=21用に生成・maxd14フィルタ済みのSoA7ファイルを読み込むだけの構成で、**制約(constellation)生成機能を持っていません**。N=23を実行するには、374以前の本番パイプライン(370Pyの`-g 23 23 ...`、制約生成からGPU実行まで一気通貫)を使う必要があります。

## 過去の実例: N=23は一度も完走していません

READMEの記録にある通り、**N=23の制約生成は過去に一度、ホスト全体のOOM(メモリ枯渇)を起こし、エフェメラルなインスタンスストアのボリュームを破壊した**という事故があります(367安全ラッパーが生まれた直接の原因)。「ハングアップ」ではなく「マシンごと応答しなくなり、強制停止が必要だった」という重さの事故です。

N=21→N=22のレコード数の伸び率(約14.18倍)をN=22→N=23に単純に当てはめると、N=23は数億件規模になり得ます。ただしREADME自体が「制約数はNに対して組み合わせ的に増えると見るべきで、単一の伸び率を外挿するのは危険」と明記しており、**単純な倍率以上に膨らむ可能性**があります。

## 提案したいこと

金土日と無人で走らせ続けるのは、正直あまりお勧めできません。代わりに:

1. **まずbench_mode=34(制約生成もカーネル起動もしない、必要maxdの確認だけ)をN=23で試す**——安価で、それだけでも有用な情報が得られます
2. 本格的な生成+実行に進む場合は、**必ず367の安全ラッパーを使う**(メモリ/ディスク/タイムアウトの上限を設定)
3週末通しで無人運転する場合、367のラッパーが働いても「制御された失敗」で止まるだけで、月曜に原因を見て仕切り直しになる可能性はそれなりに高いとお考えください

N=24まで到達するのは、N=23が一度も成功したことがない現状では、かなり楽観的な見通しだと思います。まずbench_mode=34から始めてみますか？

良い確認です。実際に382のファイルを見て確かめます。はい、含まれています。確認できました。

`382_kernel_dfs_hybrid.cu`の550行目と573行目に、381dで実装した`no_fallback_needed`(`K_slots >= MAXD14_ANCESTOR * 2`のとき後始末フェーズを丸ごとスキップする)ロジックがそのまま存在しています。

## なぜ反映されているか

382は381eをそのままリネームしたものですが、381e自体が**381dをベースに**(`cp 381d_hybrid_kernel_nospin.cu 381e_hybrid_kernel_localtotal.cu`として)作られ、そこに`atomic_add_result`の呼び出し頻度を減らす変更だけを追加したものでした。つまり継承関係は:

```
381c(active_static_workers導入)
  → 381d(no_fallback_needed追加、スピン待機スキップ) ← 本命の修正
      → 381e(atomic_add_resultの頻度削減を追加)
            → 382(381eをリネーム、ロジック無変更)
            ```

            なので382には381c・381dの修正も含めて全て積み重なった状態で入っています。実機で`kernel_ms≈263,000ms`(374比31%)が再現できているのも、この`no_fallback_needed`が正しく効いているからこそです。ご心配の通りの確認、良い着眼点でした。
"""
...
VERSION_TAG:str="382 CONSOLIDATION: pure rename of 374Py_kernel_maxd14_final.py to 382Py_kernel_maxd14_final.py, carried forward completely UNCHANGED -- the 375-381e CUDA C hybrid-kernel research track (frame-queue work-stealing design, K_THRESHOLD sweep infrastructure) never touched this Codon source at all. Executable code region (docstring- and VERSION_TAG-stripped) is BYTE-IDENTICAL to 370/371/374 -- only this VERSION_TAG line and the in-docstring Open Objectives item 6 summary were touched, both confirmed to fall entirely within the stripped docstring region. No dispatch, kernel, or CLI-gate logic changes anywhere in this revision."
