...
#
# USAGE
#   STATIC_ONLY=1 bash 400_validate.sh
#                 bash 400_validate.sh            # ~45 min
#   WITH_N22=1    bash 400_validate.sh            # + N=22 confirmation
...
REV="400"
PY_SRC="${PY_SRC:-400Py_kernel_maxd14_final.py}"
PY_BIN="${PY_BIN:-400Py_kernel_maxd14_final}"
PREV_PY="${PREV_PY:-399_r2Py_kernel_maxd14_final.py}"
HELPER_SRC="${HELPER_SRC:-rev386_validation_helpers.py}"
CU_SRC="${CU_SRC:-400_kernel_maxd14.cu}"
CU_BIN="${CU_BIN:-400_kernel_maxd14}"
PREV_CU="${PREV_CU:-399_r2_kernel_maxd14.cu}"
CRLOG_DIR="${CRLOG_DIR:-400_crunner_logs}"
CODON="${CODON:-codon}"
...
N21_ORACLE="${N21_ORACLE:-314666222712}"
N22_ORACLE="${N22_ORACLE:-2691008701644}"
IN21="${IN21:-constellations_N21_6.bin.soa_ref_361.bin.maxd14only_363.bin.sched394f.bin}"
...
grep -qE '^REV_TAG:str="400"' "$CODE" && pass "source_rev_tag_is_400" || fail "source_rev_tag_is_400" "wrong REV_TAG"
grep -q 'CRunnerEntry(14,"./400_kernel_maxd14","NQ_EXTRA_CTX=1 "' "$CODE" && pass "source_table_points_at_400_and_keeps_the_treatment" || fail "source_table_points_at_400_and_keeps_the_treatment" "table entry wrong"
...
if [[ -f "$PREV_PY" ]]; then
  ...
  [[ "$PR_" == "3" && "$PA_" == "3" ]] && pass "py_diff_fingerprint_vs_399_r2Py (removed=3 added=3 EXECUTABLE lines)" \
    || { fail "py_diff_fingerprint_vs_399_r2Py" "removed=$PR_ added=$PA_, expected 3/3"; diff "/tmp/${REV}_prev_code.py" "$CODE" | head -20 | sed 's/^/      /'; }
else info "py_diff_fingerprint_vs_399_r2Py" "skipped"; fi
...
if [[ -f "$PREV_CU" ]]; then
  ...
  [[ "$CR_" == "1" && "$CA_" == "30" ]] && pass "cu_diff_fingerprint_vs_399_r2 (removed=1 added=30: the NQ_BLOCK knob and its note)" \
    || { fail "cu_diff_fingerprint_vs_399_r2" "removed=$CR_ added=$CA_, expected 1/30"; diff "/tmp/${REV}_prev_code.cu" "/tmp/${REV}_cur_code.cu" | head -40 | sed 's/^/      /' | cut -c1-140; }
else info "cu_diff_fingerprint_vs_399_r2" "skipped"; fi
...
banner "Generating and timing N=$NPROF via ./$PY_BIN -g $NPROF $NPROF"
gcr="$CRLOG_DIR/crunner_${CU_BIN}_N${NPROF}.log"; rm -f "$gcr"
env -u NQ_MAX_BLOCKS -u NQ_PAD_MB -u NQ_EXTRA_CTX -u NQ_BLOCK "./$PY_BIN" -g "$NPROF" "$NPROF" 2>&1 | tee "$LOGDIR/1_gen_N${NPROF}_console.log"
cp "$gcr" "$LOGDIR/1_gen_N${NPROF}_crunner.log" 2>/dev/null || true
[[ -f "$gcr" ]] || { fail "crunner_path_taken[N=$NPROF]" "no $gcr"; exit 1; }
IN20="$(grep -o 'src=[^ ]*' "$gcr" | head -1 | cut -d= -f2)"
...
[[ -f "${IN20:-/nonexistent}" ]] && pass "input_located[N=$NPROF] ($IN20)" || { fail "input_located[N=$NPROF]" "src='${IN20:-<none>}'"; exit 1; }
...
for pair in $SWEEP; do
  bk="${pair%%:*}"; mb="${pair##*:}"
  run_cfg "$bk" "$mb" "$NPROF" "$IN20" "$N20_ORACLE" "$REPS" n20 || { echo "OK=$PASS FAIL=$FAIL"; exit 1; }
done
BASE20="${SMEAN[n20_${BASE_BLOCK}_${BASE_MB}]:-}"
...
BESTK=""; BESTV=""
for pair in $SWEEP; do
  bk="${pair%%:*}"; mb="${pair##*:}"; v="${SMEAN[n20_${bk}_${mb}]:-}"
  [[ -z "$v" ]] && continue
...
# ---------------------------------------------------------------------
banner "Z5: confirm at N=21"
if [[ -f "$IN21" && -n "$BESTK" ]]; then
  run_cfg "$BASE_BLOCK" "$BASE_MB" 21 "$IN21" "$N21_ORACLE" "$REPS21" n21 || { echo "OK=$PASS FAIL=$FAIL"; exit 1; }
  if [[ "$BESTK" != "${BASE_BLOCK}:${BASE_MB}" ]]; then
    run_cfg "${BESTK%%:*}" "${BESTK##*:}" 21 "$IN21" "$N21_ORACLE" "$REPS21" n21 || { echo "OK=$PASS FAIL=$FAIL"; exit 1; }
    B21="${SMEAN[n21_${BASE_BLOCK}_${BASE_MB}]}"; W21="${SMEAN[n21_${BESTK%%:*}_${BESTK##*:}]}"
    G21="$(awk -v a="$B21" -v b="$W21" 'BEGIN{printf "%.3f",(a-b)/a*100}')"
    info "N21" "baseline $B21 ms -> best $W21 ms  = ${G21}% (N=20 gave ${GAIN}%)"
    awk -v g="$G21" -v g20="$GAIN" 'BEGIN{exit !(g >= g20/2)}' && pass "Z5_carries_to_N21 (${G21}% >= half of ${GAIN}%)" \
      || fail "Z5_carries_to_N21" "${G21}% is less than half the N=20 gain -- the configuration is N-specific and must be tuned per N"
  fi
else info "Z5" "skipped ($IN21 absent or no winner)"; fi

# ---------------------------------------------------------------------
# 5. Z6 -- optional N=22
# ---------------------------------------------------------------------
if [[ "$WITH_N22" == "1" && -n "$BESTK" ]]; then
  banner "Z6: N=22 (long -- input build plus ~19 min of kernel per run)"
  g22="$CRLOG_DIR/crunner_${CU_BIN}_N22.log"; rm -f "$g22"
  env -u NQ_MAX_BLOCKS -u NQ_PAD_MB -u NQ_EXTRA_CTX -u NQ_BLOCK "./$PY_BIN" -g 22 22 2>&1 | tee "$LOGDIR/9_N22_console.log"
  cp "$g22" "$LOGDIR/9_N22_crunner.log" 2>/dev/null || true
  if [[ -f "$g22" ]]; then
    IN22="$(grep -o 'src=[^ ]*' "$g22" | head -1 | cut -d= -f2)"
    info "N22" "input=$IN22 records=$(grep -o 'records=[0-9]*' "$g22" | head -1 | cut -d= -f2) free_mb=$(grep -o 'free_mb=[0-9]*' "$g22" | head -1 | cut -d= -f2)"
    if [[ -f "${IN22:-/nonexistent}" ]]; then
      run_cfg "$BASE_BLOCK" "$BASE_MB" 22 "$IN22" "$N22_ORACLE" 1 n22 || true
      run_cfg "${BESTK%%:*}" "${BESTK##*:}" 22 "$IN22" "$N22_ORACLE" 1 n22 || true
      B22="${SMEAN[n22_${BASE_BLOCK}_${BASE_MB}]:-}"; W22="${SMEAN[n22_${BESTK%%:*}_${BESTK##*:}]:-}"
      if [[ -n "$B22" && -n "$W22" ]]; then
        G22="$(awk -v a="$B22" -v b="$W22" 'BEGIN{printf "%.3f",(a-b)/a*100}')"
        info "N22_gain" "${G22}% (single rep each; treat as indicative)"
        awk -v g="$G22" 'BEGIN{exit !(g>0)}' && pass "Z6_carries_to_N22 (${G22}%)" || fail "Z6_carries_to_N22" "${G22}% -- the configuration does not help at N=22"
      fi
    fi
  else fail "crunner_path_taken[N=22]" "no $g22 -- see $CRLOG_DIR/dispatch.log"; fi
fi
