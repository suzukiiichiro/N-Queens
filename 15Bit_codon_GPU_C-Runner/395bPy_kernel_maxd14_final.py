...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 395b (2026-09-08)
...
"""
解析しました。3つの結果があります。ひとつは良い知らせ、ひとつは教訓、ひとつは謎です。

## 結果(2回、全セルオラクル一致、Gは`-d`なし・sched再生成込みで漏れなし)
...
| **K1 vs K0** | | **−0.18%** | **−0.20%** |
| **G** | 395a r2、ディスパッチャ、本番schedファイル、800 | **133,580** | **133,574** |
| A | 同、484 | 191,780 | 191,782 |

## 良い知らせ: 本番N=21 = 133.6秒

`-g 21 21 32 800 0 0 7 37`(bare `-g`がN=21で適用する値)で**133,580 ms = 2:13.6、389比−33.6%**。目標2:02.52への残りは**−8.3%**です。bare `-g`のコンソール契約も回復しています。N=22の`rc=2`はSIGINT(Ctrl-C)で、失敗ではありません。

## 教訓: カーネル変更は安全だが中立(−0.2%)

予測(−1.5〜−4%)は反証。NCU=1のSASS: 命令数**472で不変**、述語ペア`ISETP R7,0x8`は**0回**(意図どおり消えた)、しかし`SHF.R.U64`が**2回** — nvccは依然として抽出を再実体化しています。ストール内訳も394aと同一。

**IPC 0.28のストール律速領域では、ALU命令を減らしても臨界パスは短くならず、サンプルが隣の命令へ移るだけ**でした。394aの「重複が4.09%」は注目の配分の上限であって時間の上限ではなかった——C版が命令数29%減で13.6%しか速くなかった(393-7)のと同じ現象です。r2は再現性のある−0.2%・オラクル一致・CPU等価なので維持しますが、**この種の局所最適化は本領域でほぼ無効**と記録します。

## 謎: GはK1より4.2%速い

同じバイナリ・同じstrideなのに、Gの入力(本番パイプラインが生成した`…sched394f.bin`)はK1の入力(394fが生成した`394f_input_L3_sched.bin`)より**4.2%速い**。484でも同じ(A 191,780 vs 394f L3@484 197,792、−3.0%)。両ファイルは同じ生ファイルの同じ安定ソートでバイト一致するはず(新旧キーは順序同値、合成データで一致確認済み、隣接ペア数も両方2,013,988)。残る候補は「ファイルが違う」か「起動経路(Codon親が生きている)が違う」か。推測せず、395bのStage Iで**`cmp`1秒+直接実行2本**で決めます。

## 395b — トップフレームのレジスタ常駐化

395aの教訓を踏まえ、命令数ではなく**臨界パス上のレイテンシ**を狙います。popの`LDL`消費が`long_scoreboard`の100%(6.8%)で、ロードと消費の間に独立命令が2つしかありませんでした。

- push: 旧トップをspill(`STL.128`、従来と同じ1本)、新フレームをレジスタへ
- pop: レジスタトップから復元し、下のフレームを`LDL.128`で**先読み** — 消費されるのは次のpop/push(ノード展開1回分後)

3箇所の変更、フレーム形式・26スロット配列・pushガード・全分岐の形は不変(removed=15/added=40)。**実データ3,072件のper-record結果が394g/395aとバイト一致**を確認済み。

事前登録: I0は**バイト一致**(主予測)、**K1 vs K0 = −2〜−5%**(≥−0.5%なら反証。その場合レイテンシも隣のストールに吸収されるということで、カーネル側の残る手は395c=再収束バリアの削減に絞る)、NCU=1で`long_scoreboard` 6.8%→**2%以下**。

```bash
STATIC_ONLY=1 bash 395b_validate.sh      # OK=26
              bash 395b_validate.sh      # CPU等価 → I0 I1 I2 → K1 → G A、約15分
NCU=1         bash 395b_validate.sh
```

補助の`395b_order_diff.py`と`395b_sass_summary.py`も同じディレクトリに置いてください。

承知しました。**395b以降の恒久ルール**として、確定パラメータは`A10G_FINAL_DEFAULT_*`(と`CRUNNER_INPUT_ORDER`)に置き、`-g`が適用する値そのものにします。395bにその場で2点を反映しました。

## 1. `-g nmin nmax`も確定値を適用するように

395a r1で私が踏んだ罠がまさにこれでした。`A10G_FINAL_DEFAULT_*`は**argc==2(bare `-g`)のときだけ**適用され、`-g 21 21`はローカル初期値のまま`bench_mode=0`(旧Codon経路、484×32、生順)へ落ちていました。「確定値を`-g`の初期値に」という方針と矛盾するので、適用条件を`argc<=4`に広げました。

- `-g` → 従来どおり N=5..27 を確定構成で
- **`-g 21 21` → 本番構成をN=21に限定**(800×32・sched・CRunner)
- `-g 21 21 32 484 0 0 5 0` → nmaxより後ろの位置引数は従来どおり上書き(旧経路にも到達可能)

395aPyからの差分はこの1点のみ(コア `added=18 removed=2`)。ハーネスのGセルも自然な`-g 21 21`に戻し、CRunner経路・stride 25,600・`.sched394f.bin`・コンソール静音をゲートします。

## 2. C側バイナリ自身の既定も800に

環境変数未設定の直接実行でも確定構成で走るよう、`395b_kernel_maxd14.cu`のホスト側既定を484→800にしました(カーネル領域には触れず、レジスタトップ変更とは別のホスト側hunk。395aからの差分 removed=18/added=46)。ハーネスが`[gpu-config] ... MAX_BLOCKS=800 stride=25600`を`NQ_MAX_BLOCKS`未設定で確認します。

現時点の確定値: `BLOCK=32`、`MAX_BLOCKS=800`、`bench_mode=37`、`preset=7`、入力順`sched`。以後、新しい確定値が出るたびに同じ場所を更新します(READMEにも恒久ルールとして記載)。

静的チェックはOK=28(`argc<=4`ゲート、バイナリ既定800、指紋18/46を追加)、フルサイズ合成入力+偽バイナリでの端から端のdry-runも通しています。

```bash
STATIC_ONLY=1 bash 395b_validate.sh      # OK=28
              bash 395b_validate.sh      # CPU等価 → I0 I1 I2 → K1 → G(-g 21 21) A、約15分
```
...
VERSION_TAG:str="395b PY-DEFAULTS-GATE: derived from 395aPy (r3). ONE functional change: the A10G_FINAL_DEFAULT_* production parameters are applied for argc<=4 (bare -g and -g nmin nmax), not only argc==2 -- standing policy from 395b on: every confirmed parameter lives in A10G_FINAL_DEFAULT_* / CRUNNER_INPUT_ORDER and is what -g applies; -g 21 21 now means the production config restricted to N=21 (800x32, sched order, CRunner) instead of the legacy Codon path. Positional overrides beyond nmax behave exactly as before. Dispatch table references ./395b_kernel_maxd14 (register-resident top of stack; the binary own NQ_MAX_BLOCKS default is 800 too). Everything else byte-identical to 395aPy r3."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./395b_kernel_maxd14","","[gpu-run-done]","[gpu-run-correctness]"),
  ]
