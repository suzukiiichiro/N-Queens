...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 364 (2026-08-28)
...
6. [進行中・364でGPUランナー追加、実機nvccビルド結果待ち] CUDA C
   ランナー(案C-2)スパイク設計
   課題3(tail effect)への唯一の残存対策経路。334-337で土台確立、
   338→360で移植仕様確定、361でSoA導出のCポートが実機確定、362で
   カーネル本体の移植設計確定。

   **363で`kernel_dfs_iter_gpu_maxd14`本体を実装、実機でのCPU側検証**
   (r1→r2→r3の3段階で問題を発見・修正): (1) `process_one_task`が
   `meta_next`引数を使わずグローバル定数を参照していた実装ミスを修正、
   (2) 361の全件ダンプ(振り分け前)をそのままmaxd14専用カーネルに
   投入していたテストスコープの誤りを`363_filter_maxd14_only.py`
   (`schedule_depth_for_task`の1対1移植)で修正——ただし実データは
   N=21(preset_queens=6)で全件depth=14と判明(絞り込みでの除外は
   0件、これはN=21の全チャンクがselected_maxd=14に収まるという既存の
   知見と整合)、(3) フィルタ後も`total_sum==314666222712`との比較が
   原理的に不適切という判断は誤りだったと判明(全件がmaxd14に属する
   ため、正解値と直接比較可能)。真因はCPU1スレッドでの逐次実行が
   GPU15,488並列に対して本質的に桁違いに遅いだけ(バグではない)。
   実データ先頭20件でC/Python(独立実装`363_kernel_reference_sim.py`)
   がバイト単位完全一致を確認済み。全量のCPU側確認はバックグラウンド
   実行中(完走を待たず364へ進む判断)。

   **364で実機nvccビルド用のGPUランナーを追加した**
   (`364_kernel_maxd14.cu`、363からの追記のみ、`process_one_task`と
   `__global__`カーネル本体は無変更)。ホスト側`main()`
   (`#ifdef __CUDACC__`)がSoA7入力ファイル+`meta_next`をデバイスへ
   アップロードし、本番と同じ32×484のgrid/block構成で起動、結果を
   ダウンロードして合計・cudaEventによる各フェーズ(H2D/kernel/D2H)
   タイミングを報告する。364はまず正当性確認(実データ全件で
   `total_sum==314666222712`)を優先し、356の3チャンクmeasure2
   プロトコルに揃えた厳密な±3%比較は正当性確定後の次リビジョンに
   持ち越す。364自体はこのセッション内でnvcc/GPU環境が無いため未
   実機検証、Suzukiさんの実行結果待ち。
...
364を用意しました(GPUランナー追加、実機nvccビルド+GPU実行の準備)。
364Py自体はコード変更ゼロです(363とバイト単位で完全一致)。

**364の内容**: `364_kernel_maxd14.cu`(363からの追記のみ、カーネル
本体・`process_one_task`は無変更)にホスト側GPUランナー
(`main()`、`#ifdef __CUDACC__`)を追加しました。SoA7形式の入力
ファイル(361の実機ダンプを`363_filter_maxd14_only.py`で絞り込んだ
もの、実際にはN=21では全件maxd14に属するため絞り込みで除外0件)を
読み、`meta_next`と共にデバイスへアップロード、本番と同じ32×484の
grid/block構成でカーネルを起動、結果をダウンロードして合計と
H2D/kernel/D2Hそれぞれの所要時間を報告します。

まず正当性(実データ全件で`total_sum==314666222712`と一致するか)を
優先し、356の3チャンクmeasure2プロトコルに揃えた厳密な±3%比較は
正当性確定後に回します。

---

## 実行手順(実機、Suzukiさんの環境で)

```
STATIC_ONLY=1 bash 364Py_kernel_maxd14_gpu_validate_N21_full_once.sh
bash 364Py_kernel_maxd14_gpu_validate_N21_full_once.sh
```

`.sh`はコード無変更の静的チェックに加え、nvccでのビルドと実機GPU
実行、正当性照合まで自動で行います。361のダンプファイルと、363で
作成した絞り込み済みSoA7ファイル(`*.maxd14only_363.bin`、既にお手元
にあるはずです)を使います。

一致すれば364は正当性確認として完了、365で356のタイミング
プロトコルに揃えた±3%比較へ進めます。
"""
...
VERSION_TAG:str="364 kernel_maxd14_gpu_runner: DESIGN/IMPLEMENTATION REVISION, NO CODON CODE CHANGE. Executable code region (import gpu onward) is byte-identical to 363, which was itself byte-identical to 362/361(r2): verified by a static check hashing the docstring-stripped/VERSION_TAG-excluded code region against 363s own reference hash for the identical region. 364s actual deliverable is a single new addition to 364_kernel_maxd14.cu (a copy of 363s file): a host-side GPU runner (main(), guarded by #ifdef __CUDACC__ so it only compiles under nvcc, never under the CPU-test gcc build) that uploads a SoA7 input file (the same 7-field format 363s CPU-test harness and 363_filter_maxd14_only.py use) plus the META_NEXT table to device memory, launches kernel_dfs_iter_gpu_maxd14 with the unchanged production 32x484 grid/block configuration (stride=15488), downloads the per-thread results, sums them on the host, times each phase (H2D copy, kernel execution, D2H copy) via cudaEvent, and optionally compares the total against an expected value passed as a 4th CLI argument. Nothing outside this new main() differs from 363(r2) -- process_one_task() and the __global__ kernel itself are byte-identical, confirmed both by diffing the file directly and by rebuilding the CPU-test path (gcc, __CUDACC__ undefined) and re-running the existing 220,000+-record synthetic byte-identical cross-check, which still passes unchanged. 364 also carries forward a significant correction from 363s investigation: the filtered N=21 real data was found to have ZERO records excluded by the maxd<=14 filter (every real N=21 record is exactly depth=14, matching the existing understanding that all of N=21s chunks fit selected_maxd=14) -- so the maxd14-only subset IS the full real dataset, and total_sum==314666222712 (the real correctness oracle) is in fact the correct and meaningful target for 364s real-hardware GPU run, unlike what was initially assumed during 363s mid-session course correction. This revision prioritizes establishing real-hardware correctness (does the actual nvcc-built, GPU-executed kernel reproduce 314666222712 on real N=21 data) over timing: 364 is a single-shot, non-chunked run, and wiring it into the exact 3-chunk measure2 protocol for a rigorous plus-or-minus 3 percent comparison against the 356 anchor (393.404s) is deferred to a later revision once correctness here is confirmed on real hardware. This session has no CUDA toolkit or GPU available, so 364_kernel_maxd14.cu is unverified on real hardware as of this writing and is Suzukis next action item."
