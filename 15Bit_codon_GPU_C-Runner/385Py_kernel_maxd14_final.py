...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 385 (2026-09-07)
...
6. [実務上の目的は達成・374で統合パッケージ化・速度差の原因究明は保留]
   ...
   **【r1の2つのバグと修正、r2】**
   ...
   になる設計だったため。r2で`crunner_input_fname()`/`crunner_input_
...
2026年  9月  7日 月曜日 13:27:17 JST
$ ./385Py_kernel_maxd14_final -g 21 21 32 484 1 0 7 37
21:      314666222712                0          0:03:23.267    ok
...
"""
Now let's make the edits: header narrative, VERSION_TAG, imports, new dispatch code, and the main() integration.次にVERSION_TAGとimportを更新します。次に、ディスパッチ本体(`CRunnerEntry`クラス・対応表・パーサ・実行関数)を369ブロックの直後に挿入します。次にCLIゲート(bench_modeホワイトリスト)とpresetゲートに`37`を追加します。最後に、Nループ内へ`bench_mode=37`のディスパッチ分岐を追加します。Good — the new block is valid Python syntax on its own. Now let's check the `main()`-side dispatch insert and the surrounding diffs for indentation consistency.All the required gate insertions are in place, and the VERSION_TAG has a clean 2-quote balance (matches the known quoting-bug guard). Now let's produce the deliverables: the renamed source file, a validation harness for rev385, and a README append.Now let's write the validation harness for 385, mirroring the existing project conventions (STATIC_ONLY dry run, then real build+run against N=21).Now the README append, following their established format (pre-registered predictions before results).385の実装ができました。3ファイルです。

**やったこと**:
- `364_kernel_maxd14.cu`の実ソース(729〜738行)から`[gpu-run-done]` / `[gpu-run-correctness]`マーカーとCLI引数順を直接確認し、`CRunnerEntry`/`crunner_dispatch_table()`/`crunner_run()`として配線
- 新設`bench_mode=37`のみ追加(既存の`bench_mode=0〜36`のロジック・364の`.cu`は一切変更なし、CLIGATE/PRESETGATEも369までの慣習どおり追記)
- 対応表未登録のmaxdに当たったら`[crunner-unsupported]`で明示して止める(誤答を出さない)

**確認したこと**(実機がないので範囲は限定的ですが):
- 新規追加ブロックだけを切り出してPython ASTでの構文チェックはOK
- `VERSION_TAG`のダブルクォート個数(既知のクォート起因ビルド障害の再発防止チェック)は2個で正常
- ファイル全体をASTでパースすると`@par`(Codon固有構文)でエラーになりますが、これは**382オリジナルにも同じ箇所で起きる既知の制約**であることを確認済みです(私の変更由来ではありません)

実行コマンド:
```bash
bash 385_validate.sh
```

デフォルトでN=21、`bench_mode=37`を実行し、`[crunner-dispatch-summary]`の`total`がオラクル(314666222712)と一致するか、`kernel_ms`が374本番アンカー(≈201,232ms)に近いかを見ます。`364_kernel_maxd14`バイナリは385側では再ビルドしないので、既にビルド済みのものが必要です(なければ`374_validate.sh`/`382_validate.sh`側で先にビルドしてください)。

結果が出たら教えてください。

PASSしました。`total=314666222712`でオラクル一致、`kernel_ms=201238`は374本番アンカー(201232.422)とほぼ完全に一致(誤差0.003%、ノイズ帯)。ディスパッチ機構が本物の364バイナリを介して正しく動くことを確認できました。READMEに結果を追記します。385、確定です。maxd→CRunnerバイナリのos.systemディスパッチ機構が、実際の本番`364_kernel_maxd14`を介して374本番と同一の結果(オラクル完全一致)・同等の速度(誤差0.003%)を再現することを実機で確認できました。

これで、今週のCRunner maxd拡張(N=23以降向けのmaxd16/18/20/21バイナリ)ができるたびに、`crunner_dispatch_table()`へエントリを1行足すだけで対応範囲が伸びていく土台ができたことになります。

次、いかがしましょうか。当初ご提案した順番だと386(検証関数群の別ファイル分離)ですが、今週の本題は「N23以降対応のmaxd拡張CRunner実装」とのことでしたので、386を先にやるか、386は後回しにしてCRunner本体(maxd16など)の設計に入るか、お任せします。
...
VERSION_TAG:str="385 r2 CRUNNER-DISPATCH: added maxd-gated os.system dispatch to external CRunner binaries (CRunnerEntry class, crunner_dispatch_table, crunner_select_entry_index, crunner_parse_result, crunner_run -- new bench_mode=37 branch in main()). Registers the UNCHANGED 364_kernel_maxd14.cu as the maxd<=14 entry; its CLI arg order and gpu-run-done / gpu-run-correctness stdout markers were read directly from that file, not guessed. r1 had two bugs, both fixed here, neither reached an oracle comparison: (a) a stray timedelta value mistyped as datetime, caught by codon build itself before any run; (b) crunner_run() was called with stream_fname (raw, unfiltered) instead of the required maxd14-filtered SoA reference dump, caught on real hardware after 40+ minutes of the CRunner binary running on the wrong input -- fixed via new crunner_input_fname/crunner_input_valid, which fail fast with a clear message instead of guessing or auto-building the missing filter stage. No existing kernel, CLI-gate, or dispatch logic from 382 was modified -- purely additive. Prerequisite: 384's standalone probe confirmed on real hardware that local Codon multi-file import and the os.system-plus-logfile-round-trip mechanism both work; 383 was never made, 384 consumed that slot as a feasibility spike."
...
def dump_soa_reference_c_port(N:int,fname:str,out_fname:str,gpu_log_level:int=0)->Tuple[int,int]:
  total:int=count_constellations_bin_records(fname)
...
def crunner_run(entry:CRunnerEntry,N:int,in_bin:str,out_bin:str,expected_total:int,log_dir:str,gpu_log_level:int=0)->Tuple[bool,int,float]:
  ...
  found_done,total_sum,kernel_ms_x1000,match_status=crunner_parse_result(log_path,entry.done_prefix,entry.correctness_prefix)
...
def main()->None:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==37:
      ...
      filtered_fname:str=crunner_input_fname(stream_fname)
      if not crunner_input_valid(filtered_fname):
