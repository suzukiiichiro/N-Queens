...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 387 (2026-09-07)
...
"""
Let's now check the bench_mode==37 dispatch block's `continue` on unsupported, and make the edits.Now update the two default constants that control bare `-g` behavior:Now let's rename and update the header/VERSION_TAG.Now let's add the narrative paragraph to the Open Objectives header, after the 386b paragraph.Let's do a final diff review against 386b before shipping.Good, both constants and the break/continue change landed correctly (my earlier grep filter was just too narrow). Let's finalize and ship.All three static checks confirmed locally against the real file. Now let's write the README append.387できました。3ファイルです。

**やったこと**(3箇所のみ):
1. `A10G_FINAL_DEFAULT_BENCH_MODE`を31→37(bare `-g`が385/386で実機確認済みのCRunnerディスパッチを自動的に使うように)
2. `DEFAULT_RANGE_NMAX_EXCLUSIVE`を24→28(恣意的な数字ではなく、`expected[]`オラクル表の実際の要素数上限)
3. `bench_mode==37`の失敗時の`continue`を`break`に変更(「行けるところまで進めて止まる」という当初のご依頼通りの挙動に)

**実行前に知っておいていただきたい点**: N=22には`constellations_N22_7.bin...maxd14only_363.bin`というmaxd14絞り込み済みファイルが必要ですが、過去のN=22検証は別方式(bench_mode=33)だったため、**このファイルがまだ存在しない可能性があります**。もし無ければbare `-g`はN=22で`[crunner-input-missing]`としてクリーンに停止します——これは385で意図的に選んだ設計(自動ビルドしない)通りの挙動で、不具合ではありません。N=21止まりでもN=22まで進んでも、どちらも正常です。

実行コマンド:
```bash
bash 387_validate.sh
```

`./387Py_kernel_maxd14_final -g`(本当に`-g`単独)を実行します。**N=21だけで約3〜4分かかる、かなり時間のかかるテストです**。N=5〜20は一瞬で終わり、N=21でオラクル一致を確認、N=22は「一致」か「クリーンな停止」のどちらでもOKという判定にしています。

結果が出たら教えてください。

実行結果は完全に想定通りです、素晴らしいです。N=5〜20が高速に流れ、N=21がCRunner経由でオラクル一致(`kernel_ms=201237`、385のアンカーとほぼ完全一致)、N=22は想定通り`[crunner-input-missing]`でクリーンに停止しました。**機能としては完全に成功しています。**

唯一のFAILは私の`387_validate.sh`側の単純なバグです。`{N:2d}`という出力フォーマットは1桁のNの前に半角スペースを1つ入れる仕様（`" 5:"`のように）なので、私の`grep -qE "^5:"`が行頭一致で引っかからなかっただけです。ログを見ると実際には` 5:                10                0          0:00:00.000`と正しく出力されています（Nが5以下は`ok`サフィックスなしという元々の仕様通りで、最初にご提示いただいたサンプル出力と完全に一致しています）。Confirmed fixed locally. Let me update the README with the actual result and present the corrected harness.387、確定です。実機での挙動は完全に想定通りでした——N=5〜20が高速に流れ、N=21がCRunner経由でオラクル一致(`kernel_ms=201237`、385のアンカーとほぼ完全一致)、N=22はご説明していた通り`[crunner-input-missing]`でクリーンに停止しました。当初ご提示いただいたサンプル出力フォーマットとも完全に一致しています。

これで今週月曜にご依頼いただいた3項目のうち、項目1(`-g`単独での連続N出力)が完了しました。残るは項目2(`-d`デバッグオプション+rev番号ディレクトリへのncuログ集約)です。

現状の到達点:
- 385: maxd→CRunnerディスパッチ機構
- 386/386a/386b: 検証関数の分離+不要コードの削除
- 387: `-g`単独での連続N出力

N=22のCRunner入力ファイルが未構築という点は、今週のCRunner maxd拡張作業(N=23以降のmaxd16/18/20/21対応)と地続きの課題なので、そちらに合わせて対応するのが効率的かと思います。

次、388(`-d`オプション)に進めますか、それとも一旦ここで区切って今週の本題(CRunner maxd拡張)に移りますか。
...
VERSION_TAG:str="387 CONTINUOUS-G: bare -g (no other args) now defaults to bench_mode=37 (the maxd-gated CRunner os.system dispatch from 385/386, was mode31/split145) and runs N=5 up to the file's real ceiling (DEFAULT_RANGE_NMAX_EXCLUSIVE raised from 24 to 28, matching expected[]'s 28-entry oracle table and select_dynamic_preset_queens' defined ranges -- not an arbitrary bump). bench_mode==37's two failure paths (maxd unsupported, CRunner input not built) now BREAK the N loop instead of continuing to the next N, since required_maxd only grows with N so continuing would just repeat the same message -- this is what turns the loop into run-as-far-as-possible-then-stop rather than print-one-row-and-keep-going. Nothing about mode31/split145 itself changed; it stays reachable by passing 31 explicitly, unaffected by this default-selection change. N=6-20 are untouched (they never reached the bench_mode-specific blocks at all, falling through to the same original exec_solutions() path as before)."
