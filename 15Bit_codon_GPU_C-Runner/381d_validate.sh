...
# 381a_validate.sh
#
# rev381d — First real GPU test of the seed_kernel + hybrid_kernel
# two-stage design (380 spec). K_THRESHOLD fixed at 13 (see file
# header of 381d_hybrid_kernel_nospin.cu): this run's only job is
...
BIN="${BIN:-381d_hybrid_kernel_nospin}"
...
echo "381d PASSED: kernel_dfs_hybrid_maxd14 (K_THRESHOLD=13) with the"
echo "fallback/spin-wait phase skipped entirely (provably unreachable at"
echo "this K) reproduces 374's own oracle on real N=21 data. ncu measured"
echo "381c's termination-check spin loop executing 5.9 BILLION times --"
echo "this was the real dominant cost, not atomic contention count. Compare"
echo "kernel_ms above to 374 (~201,232ms), 381a (285,841ms), 381b (613,041ms),"
echo "381c (569,157ms). This should finally land close to 374."
echo "381e+: once K=13 truly matches 374, design a real (non-spinning)"
echo "termination protocol before sweeping K downward for real speed gains."
exit 0
