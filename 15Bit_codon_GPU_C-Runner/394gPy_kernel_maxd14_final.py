...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 394g (2026-09-08)
...
16:          14772512                0          0:00:00.066    ok
17:          95815104                0          0:00:00.210    ok
18:         666090624                0          0:00:01.619    ok
19:        4968057848                0          0:00:08.479    ok
20:       39029188884                0          0:01:05.189    ok
21:      314666222712                0          0:03:23.297    ok
22:     2691008701644                0          0:28:44.275    ok
...
"""
解析しました。**対照は的中、主予測は外れ、そして生を初めて超える並びが出ました。**

## 394f 結果

| セル | 内容 | `kernel_ms` | vs R | 生隣接ペア保持 |
|---|---|---:|---:|---:|
| R | 生 | 201,238 | — | 100% |
| **X** | ランダムシャッフル | **290,157** | **+44.2%** | 0% |
| L1 | `(col, ld, rd)`ソート | 243,103 | +20.8% | 0% |
| L2 | `(−popcount(free), col, ld, rd)` | 243,830 | +21.2% | 0% |
| **L3** | `(markctrl, ctrl0)`安定ソート | **197,792** | **−1.71%** | **99.4%** |
...
**隣接性が要因です。** ランダム順(+44.2%)はbroadmarktail base(+47.5%)と同等——baseの罰は「隣接性の破壊」でほぼ全額説明でき、それ以上の固有の悪さはありません。

**ただし「隣接」の中身はルート状態の数値的近さではない。** `(col,ld,rd)`ソートは+20.8%、空き数優先でも+21.2%。生成器が同じwarpに置いているのは(i,j,k)を共有する**兄弟コンステレーション**——ほぼ同型の部分木——で、ビットマスクの辞書順はそれを散らします。生の価値の約半分はマスク類似で回収でき、残り半分は兄弟性そのものにある。

**スケジュール同質性は独立した小さな利得。** L3は隣接を99.4%保持したまま同じスケジュールクラスを固めて−1.71%。broadmarktailの狙い(funcidグルーピング)自体は正しく、**インターリーブが台無しにしていた**という394eの読みが確定しました。

**warp間均衡が無関係である決定的証拠**: per-warp解数のCVはランダム順が**最も均衡**(1.26%、生5.35%)でありながら44%遅い。外側`BSYNC B4`にとって、warp内32レーンがロックステップで進めるかどうかがすべてです。

## 採用方針

並べ替え軸(broadmarktail→chunkshape148)は本番C経路について**クローズ**。**L3を採用候補**にします(−1.71%、コード変更ゼロ、順列ツール1段をCRunner入力パイプラインの4段目に足すだけ)。順序軸の残る上振れは「兄弟性を保ったままコスト均質化」ですが、兄弟グループは32未満で生成器の順序が既にほぼそれを実現しています。**この軸はL3で一段落**とし、重心をカーネル側(395a〜c)へ移します。

## 394g — 968以下の`MAX_BLOCKS`細分 × {生, L3}

484〜968は未測定で、968の直上から急坂だったので最適点は968以下にある可能性が高い。5点(704/800/**864**/**928**/968)で生→L3を対にして連続実行(熱ドリフトが両方に等しく乗る)、11実行約40分、sudo不要。

事前登録: 484生±1%・968生±2%再現、**MB\*は[864, 968]**(704/800が最良なら反証)、**L3 vs 生は全点で−1.7±1%かつ同符号**(符号が点で変われば占有率との交互作用)、最良L3点は**≤158,000 ms**(N=21 ≈ 2:38)。

```bash
STATIC_ONLY=1 bash 394g_sweep_fine_and_L3.sh
              bash 394g_sweep_fine_and_L3.sh      # 11実行、約40分
```

今回もフルサイズ合成入力+偽バイナリで末尾まで通すdry-runを行い、`local`の同一行参照がないことも確認済みです。394gの後は`env_prefix`(1トークン)+パイプライン4段目で本番採用し、395aへ進みます。
"""
...
VERSION_TAG:str="394g PY-REVTAG: pure rename of 394fPy to 394gPy plus the dispatch table referencing ./394g_kernel_maxd14 (header-only rename of the .cu). ZERO functional change otherwise. 394g is a C-side measurement revision: 394g_sweep_fine_and_L3.sh drives the CRunner binary directly on the raw input and on the 394f L3 (schedule-sorted, stable) input at MAX_BLOCKS 484/704/800/864/928/968, to bracket the occupancy optimum below 968 and to confirm that the L3 -1.7% composes with it. This .py is not executed by that measurement."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./394g_kernel_maxd14","","[gpu-run-done]","[gpu-run-correctness]"),
  ]
