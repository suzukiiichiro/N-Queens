...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 394d (2026-09-08)
...
"""
解析しました。**予測は逆符号・5倍で外れ、この計画全体で最大の効果が出ました。**

## 394c 結果

| | 生入力(37) | 並べ替え(39) |
|---|---:|---:|
| **484** | A **201,236**(389比−0.001%) | B **261,060(+29.7%)** |
| **968** | C **160,528** | D **158,955(−1.0%)** |

4セルすべてオラクル一致、stride結合ゲート全通過。A vs D = **−21.0%**(現時点の最速セル、N=21≈2:39)。

| # | 予測 | 実測 | 判定 |
|---|---|---|---|
| 2 | B vs A: −3〜−6% | **+29.7%** | ❌ 逆符号・5倍 |
| 3 | D vs C: Bと同符号 | −1.0% | ❌ |
| 1 | A/C ±1% | A ✅ / **C −1.6%** | 半分 |

**カーネルを1バイトも変えずに、データの順序だけでC版が30%動きました。** 順序は本番C経路の第一級レバーです——今のところ悪い方向にですが、逆方向に同程度の余地がある可能性を捨てずに済むよう、この軸は閉じません。
...
stride 30,976ではグループ(48×30,976)が`SORT_SEQ_BITS=20`のcapを超え、**isort9が無効化**されています。つまり:

- B(484): broadmarktail + scorestripe + bucket_run + **isort9あり** → +29.7%
- D(968): 同じ構成で **isortなし** → −1.0%

strideとiter_sortが交絡しており、+29.7%がどの段のせいかは要因計画だけでは決まりません。ただ、Dで消えていた段がまさにper-thread列の並べ替え(345-350、K=48のlaunch境界の暗黙同期を前提に設計)で、C版の単一launch 131周回にはその前提がない。**H_isort**を主仮説とします。

**Cの−1.6%**(394bの163,060〜163,197比)も記録しました。484側は3セッション±0.003%なのに968側はセッション間±1.6%——L1の崖の縁で、ブロック配置(80×12+8)に敏感。**本番採用点は縁より少し下(r3: 864〜928)を選ぶ根拠**になります。

## 394d — 段階分解ラダー

mode 39に`argv[10..15]`(window_mult / phase_jump / cross_stripe / **bucket_run / iter_sort / input_stage**)を追加しました。mode 30/31が書くのと同じローカルへ書くので、キャッシュ名(`_run2048`/`_isort9`)とビルダーは同一挙動。argc=10の呼び出しは394cと無変更。

| セル | 内容 | 加わる段 |
|---|---|---|
| A | 生 | アンカー |
| E1 | `input_stage=1` broadmarktail baseのみ | funcid w3_j7+rotate |
| E2 | run=1 isort=0(276-338順) | + scorestripe |
| E3 | run=2048 isort=0(344順) | + bucket_run |
| E4 | run=2048 isort=1(346順) | + iter_sort=1 |
| E5 | run=2048 isort=9(350順=B) | + isort9 |

各段が構成要素を**1つずつ**加え、最初に+20%跳ねる段が原因を名指しします。オラクル・stride・**ノブが本当に解釈されたか**(`crunner_reordered_params:`行との照合)をゲート。約30分、sudo不要。`NCU=1`で生とE5のchunk0スライス(48周回=並べ替えの周期)にSchedulerStats+WarpStateStatsを取り、warp内(レーン利用率低下)かwarp間(常駐低下)かを分けられます。

事前登録: **H_isort** — E1・E2・E3はA±3%、跳ねはE4/E5。反証条件: E3が既に+20%なら原因はscorestripe/bucket_run、E1が既に+20%ならbroadmarktail。

```bash
STATIC_ONLY=1 bash 394d_validate.sh     # OK=15、負のテスト済み
              bash 394d_validate.sh     # A E1 E2 E3 E4 E5
```

4ファイル(`.py` / `.cu` / `.sh` / `.md`)と、394c結果を追記したREADMEをお渡しします。
"""
...
VERSION_TAG:str="394d PY-REORDER-LADDER: derived from 394cPy. bench_mode=39 gains its own compact positional knobs argv[10..15] = window_mult, phase_jump, cross_stripe_safe, bucket_run, iter_sort, input_stage (1 = broadmarktail base bin only, 2 = chunkshape148-shaped = 394c behaviour, default). They write the SAME locals the mode 30/31 parser writes, which are copied to the module globals unconditionally, so cache filenames and builders behave exactly as for 30/31. Purpose: decompose 394c cell B (+29.7% at stride 15,488 with the fully shaped input) rung by rung: base only / bucket_run=1 iter_sort=0 / 2048,0 / 2048,1 / 2048,9. A mode-39-specific params line is printed at startup. bare -g 21 21 32 484 0 0 7 39 (argc=10) is unchanged from 394c. crunner_dispatch_table() references ./394d_kernel_maxd14 (header-only rename). Every other path is byte-identical to 394cPy."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./394d_kernel_maxd14","","[gpu-run-done]","[gpu-run-correctness]"),
  ]
