/*
 * 381b_hybrid_kernel_unified.cu
 *
 * rev381b — CORRECTS a real design flaw found in 381a's real-GPU
 * results: 381a's separate seed_kernel_maxd14 routed EVERY one of
 * M=2,025,282 tasks' INITIAL acquisition through the shared atomic
 * queue (push during seeding, pop during the worker phase) -- even
 * at K_THRESHOLD=13 where local-stack overflow should never happen.
 * That is not what 378's design intended ("share only the overflow,
 * not everything") and it shows: 381a measured kernel_ms=285,841 vs
 * 374's 201,232 (+42%), plus a new seed_ms=13,259 phase, for a total
 * +48.6% regression even at the "should be nearly free" K=13 setting.
 *
 * FIX: seed_kernel_maxd14 is removed entirely. A single kernel
 * (kernel_dfs_hybrid_maxd14) now claims tasks via the SAME free,
 * atomic-free grid-stride index (idx += stride) 374 itself uses.
 * Root-schedule precompute + the root fast path (process_task_hybrid,
 * new in this file, folding seed_task's arithmetic together with a
 * direct call into run_hybrid_episode) push their "sibling" state
 * onto the thread's OWN LocalStack first, exactly as 374's
 * process_one_task does -- the shared queue is touched ONLY when (a)
 * a local push would exceed K_slots (genuine overflow, the whole
 * point of 378's design), or (b) a thread has exhausted its static
 * task supply (idx>=m) and starts helping drain any overflow OTHER
 * threads published. At K_THRESHOLD=13 (K_slots=26=MAXD14_ANCESTOR*2,
 * 374's own stack bound), (a) should never trigger at all -- this
 * run is expected to come much closer to 374's own kernel_ms than
 * 381a did, though a small residual cost remains (the active_workers
 * atomicAdd/Sub bracketing each task, needed for correctness at lower
 * K, is a real but much smaller overhead than a full queue push+pop).
 *
 * run_hybrid_episode()/the
 * Vyukov queue (fq_push/fq_pop/FQSlot/FrameQueue) are otherwise
 * UNCHANGED from 381a (which itself copied them verbatim from
 * 377a/379a) -- process_task_hybrid() is the only new function,
 * combining seed_task's precompute+fast-path arithmetic (copied,
 * not altered) with local-stack-first sibling handling.
 *
 * IMPORTANT: like 381a originally, this file has NOT been compiled
 * with nvcc by the author (no CUDA toolkit in the authoring sandbox).
 * 381a's real build DID succeed after two rounds of real-hardware
 * fixes (HOSTDEV->__device__-only, meta_next as a pointer parameter
 * instead of direct global-array access) -- both of those fixes are
 * already applied here since this file is a direct descendant of the
 * now-building 381a. New risk in 381b is confined to the new
 * process_task_hybrid() function and the restructured kernel body.
 */
...
int main(int argc, char **argv) {
    ...
    kernel_dfs_hybrid_maxd14<<<grid, block>>>(
        ...
        d_q, K_slots, d_active_workers, d_push_overflow
    );
