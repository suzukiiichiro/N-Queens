...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 399-r2 (2026-09-09)
...
"""
399-r2、四点セットです。静的ゲートは **OK=13 FAIL=0**。

## 直したこと

**ncu権限プローブを復元しました。** 399で `command -v ncu` だけに縮めてしまい、全プロファイルパスが `ERR_NVGPUCTRPERM` で落ちた箇所です。398-r2で動いた形——非特権プローブ → 失敗ならsudo自動フォールバック → `reclaim` で所有者を戻す——をそのまま戻し、**なぜ入っているのかをコメントに残しました**。同じ削り方を繰り返さないためです。

プローブ用の入力は、ディスク上の N=20 / N=21 のスケジュール済みファイルから自動で拾います。無ければ**3秒で停止**します。

## 加えたこと

**フェーズD を3点較正に拡張。** `asis` / `shuffle` / `desc` でレーン稼働率を測ります。asisとshuffleは**実測35.2%の時間差の両端**なので、この2点で軸に傾きが付きます。descは中間で単調性の確認です。

ハーネスは較正結果をこう出力します。

```
Y2: predicted shuffle 7.79/32 ... measured X.XX
slope: 35.21% of time per (10.53 - X.XX) lanes = Y.YY% per live lane
extrapolation: 32/32 lanes would be Z.Z% faster than the 394f order
```

外挿は線形なので鵜呑みにはできませんが、**「レーン稼働率32.9%」という診断に初めて桁の目安が付きます。**

**フェーズE を新設。** `NQ_MAX_BLOCKS=42784` で `stride ≥ records` にすると1スレッド1レコードになり、**出力ファイルがレコードごとの解数**になります。推測ではない実測量です。

これでソートすると、ランク r はスレッド `r mod stride` に載るので、**各スレッドが全ランク帯から1つずつ受け取り（負荷均衡）、各ワープが隣接32ランクを持つ（ワープ内同質性）** が同時に成り立ちます。昇順・降順の両方を3反復ずつ測ります。

## 事前登録

**Y2が本命です。** shuffleのレーン稼働率が **7.0〜8.6 / 32** に入るか。点予測は7.79。**外れれば時間とレーン稼働率は単純比例ではなく、この軸の値札付けはやり直し**になります。

**Y4では、また自分の実験が勝たない方に賭けます**（394fの±1%以内）。ただし1%以上速ければ、**この軸から得た最初の実利**です。

**Y1は3度目の正直**です。首位が `long_scoreboard` なら、394a時点の「分岐解決が支配的」は更新されます。
...
置換不変性は全順序でハードゲートのままです。

**U5がついに答えられました。そして私の予測も、394a時点の知見も、どちらも首位ではありませんでした。**

## X1 — ストール理由（N=20、`selected` を除いた実ストール）
...
| not_selected | 4.27% | 0.22 |
| math_pipe_throttle | 2.14% | 0.11 |

**N=19の順位表は完全に同一**です（wait 38.25/37.72、branch 22.68/22.44、long_scoreboard 10.26/11.71）。**代表性はこれで担保されました。**

私は `long_scoreboard`（メモリ待ち）が25%以上と表明しました。**実際は13.98%。明確に外れです。** 394a時代の「分岐解決が支配的」も首位ではなく2位でした。

首位の **`wait` は固定レイテンシ依存待ち**——直前の演算結果を待っている状態です。

## これは私の「L1律速」という読みを覆します

L1/TEX 70.64% という数字から、私はメモリ律速だと述べました。**誤りでした。**

パイプが忙しいことと、ワープがそこで止まっていることは別です。**メモリ起因のストールは14%にすぎず、45%は演算依存の待ち。** そしてスケジューラあたり **2.43／12ワープ**しか居ないので、短い依存レイテンシすら隠せていない。

> **カーネルはメモリ律速ではなく、ワープ不足による依存レイテンシ律速です。**

## Y2 — 棄却。そして値札は付きませんでした

| 順序 | lanes/32 | kernel_ms |
|---|---:|---:|
| asis | 10.53 | 16,482.6 |
| desc | 10.18 | 16,549.3 |
| shuffle | 6.78 | 22,284.4 |

予測7.79に対し実測6.78。反比例ではありません。さらに深刻なのは**傾きが区間で全く違う**ことです。

```
asis→shuffle : lanes ×1.553、time ×1.352  → 指数 0.685
asis→desc    : lanes ×1.034、time ×1.004  → 指数 0.119
```

**現在の動作点の近傍では、レーン稼働率は弱い梃子です。** descは0.35レーン失って0.40%しか遅くなっていません。ハーネスが出した「32レーンなら201.6%速い」という外挿は、注記どおり無意味です。

そしてshuffleの35.2%は**レーン稼働率だけでは説明できません**。おそらく (i) ワープ内発散と (ii) 394fが揃えているスレッド単位の累積負荷均衡（テール長）の**両方**が混ざっています。分離できていません。

## Y4 — 成立。実測コストソートも394fに負けました
...
**レコードごとの実測解数でソートしても、394fより悪い。** 394fの「兄弟保存」順は、単なる仕事量整列より強い性質を持っています。兄弟は祖先状態を共有するので、同じワープのレーンが似た経路を辿る——仕事量ソートはそれを壊します。

**並び順の軸は、394fが既にほぼ掘り尽くしていると考えるのが妥当です。**

## 400の提案 — オカパンシー

`wait` 45% ＋ ワープ2.43/12。これに対する定石は**常駐ワープを増やすこと**です。そして道が開いています。

```
sm_86: 48 warps/SM, 16 blocks/SM, 37 regs/thread
  BLOCK= 32: ブロック上限16ワープ → 33.3%   ← 現状
  BLOCK= 64: 32ワープ            → 66.7%
  BLOCK=128: 48ワープ            → 100%    （レジスタ上限52ワープで余裕）
```

**`BLOCK = 32` は `main()` 内のホスト側定数**です（`const int BLOCK = 32;`）。カーネルは `blockDim.x` を読むだけなので、**環境変数化してもカーネル領域のsha256は変わりません**。

さらに、現状は 800ブロック ÷ 80 SM = **10ブロック/SM** で、BLOCK=32のブロック上限16にすら届いていません。`MAX_BLOCKS=1280` で16に達します（stride 40,960）。

400は `NQ_BLOCK` を新設し、`(BLOCK, MAX_BLOCKS)` を掃引します。N=20なら1点16秒、ノイズフロア0.02%。**極めて安い測定で、45%のストール要因に正面から当たります。**

ただし注意が2つ。strideが変わると394fのスケジュールとの整合を確認する必要があり、レーン稼働率も同時に測って機構を追跡すべきです。

400の四点セットを書き起こしましょうか。
...
VERSION_TAG:str="399-r2 PY-REVTAG: pure rename of 399Py; the table names ./399_r2_kernel_maxd14 and keeps env_prefix NQ_EXTRA_CTX=1 and CRUNNER_MIN_N=19. No functional change. r2 repays what 399 lost to a harness defect (the ncu permission probe had been abbreviated away, so every profiling pass died on ERR_NVGPUCTRPERM), calibrates lane utilisation against the 35.2 percent ordering swing 399 did measure, and replaces the guessed popcount cost proxy with a measured per-record solution count obtained by running with stride >= records."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./399_r2_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
