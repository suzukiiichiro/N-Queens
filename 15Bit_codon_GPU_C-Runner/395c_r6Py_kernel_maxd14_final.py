...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 395c-r6 (2026-09-09)
...
"""
四点セットです。おっしゃるとおり、以後この4つを揃えます。静的ゲートは **OK=27 FAIL=0** を確認しました。

## `395c_r6_kernel_maxd14.cu`

r4のリネームです。**最初の `#include` 以降はバイト一致**で、差分はヘッダのコメントだけ。

```
コード領域全体 sha256 : c5dbde3f1002229b…  （r4と一致）
カーネル領域   sha256 : ebd7f523deb59134…  （395a r2以来不変）
```

今回はカーネル領域だけでなく**コード領域全体のsha256**でゲートしています。「リネームである」ことをそれ以上強く言う方法がないので。gccのCビルドも通過します。

## `395c_r6Py_kernel_maxd14_final.py`

r5Pyからの差分は、コメントを除いた**実行行で3行**。

```
VERSION_TAG / REV_TAG / CRunnerEntry
CRunnerEntry(14,"./395c_r6_kernel_maxd14","NQ_EXTRA_CTX=1 ",...)
```

リビジョンノートは `#` コメントで、r5のブロックの上に積んであります（新しい順）。r6ノートも私が書き入れ済みです。ゲートが486行のコメントを数えて通っています。

**処遇をバイナリ既定ではなく `env_prefix` に置いた**のは記録のためです。`[crunner-config] env_prefix=` と `dispatch.log` に必ず残るので、どの走行がどの状態だったか後から追えます。バイナリ既定を1にすると追えません。`NQ_EXTRA_CTX` は未設定時0のままなので、**直接実行は従来どおりで対照として使い続けられます**。

## セル — 交互配置

`Gb1 → Gn1 → Gb2 → Gn2`、約11分。ベースラインには**r5ディスパッチャの現物**を使います。G0を出したのがそれなので、新しい対照を作らずに済みます。交互配置でセッション内ドリフトが相殺されます。

## 事前登録と採用ルール

**S1** ベースラインの複製が0.05%以内、かつ両者が133,585の±0.1%。**S2** 処遇側の複製が0.05%以内。**S3** `mean(Gn) ≤ mean(Gb) − 0.25%`。**S4** `dispatch.log` に `env_prefix=NQ_EXTRA_CTX=1`、CRunnerログに `extra_ctx=1`。

S4には明示的な反証条件を入れてあります。**prefixは出ているのに `extra_ctx=0` がバイナリに届いている場合、テーブルの `env_prefix` がコマンド行に反映されておらず、このリビジョンは無効**です。ハーネスがその文言で落とします。

```
採用: S1〜S4 すべて成立 かつ 実測利得 ≥ 0.25%
不成立: テーブルをr5の形へ差し戻す
```

セッション内の交互A/Bを生き延びない利得は利得ではない、という基準です。

## 実行

```
STATIC_ONLY=1 bash 395c_r6_validate.sh   # OK=27
              bash 395c_r6_validate.sh   # ビルド → 4セル、約13分
SMALL_N=1     bash 395c_r6_validate.sh   # 採用前に一度、-g 5 15 の正当性確認
```

`SMALL_N` を既定で無効にしたのは、小さいNの入力が未キャッシュだと所要時間が読めず、本ハーネスを11分に保てなくなるためです。**本番既定を変える以上、採用前に一度は他のNでの正当性を見ておくべき**なので、1回だけ回してください。

事前条件は `395c_r5Py_kernel_maxd14_final` と `395c_r4_kernel_maxd14` が残っていることだけです。ハーネスが冒頭で確認します。

R6、**S1〜S4すべて成立。採用ルールを満たしました。**

## 結果

| セル | ディスパッチャ | extra_ctx | free_mb | kernel_ms |
|---|---|---:|---:|---:|
| Gb1 | r5（処遇なし） | 0 | 22,018 | 133,611.203 |
| Gn1 | **r6（env_prefix）** | 1 | 21,762 | **133,198.297** |
| Gb2 | r5 | 0 | 22,018 | 133,572.516 |
| Gn2 | **r6** | 1 | 21,762 | **133,185.844** |

- **S1** ✓ ベースライン複製 0.029%（≤0.05）、133,585との乖離 0.020% / 0.009%（≤0.1）
- **S2** ✓ 処遇側複製 **0.0093%**（≤0.05）
- **S3** ✓ 平均 133,591.860 → 133,192.071、**利得 0.299%（400 ms）**（≥0.25）
- **S4** ✓ 実際のコマンド行まで確認できました

```
[crunner-config] ... env_prefix=NQ_MAX_BLOCKS=800 NQ_EXTRA_CTX=1
cmd=NQ_MAX_BLOCKS=800 NQ_EXTRA_CTX=1 ./395c_r6_kernel_maxd14 21 ...
```

4セルすべてで `[gpu-ctx]` が期待どおり（Gb=0、Gn=1）。全セル MATCH、1710 MHz固定。電力もパターンどおりで、速い側が高い（249.6/249.2W 対 244.7/245.7W）。

**採用です。** 本番は **133,192 ms = 2:13.19**。
...
```
r5 G0        133,585.359  →  r6 Gb平均 133,591.860   +0.0049%
r5 Gx/Gxb平均 133,189.094  →  r6 Gn平均 133,192.071   +0.0022%
```

**別セッション、別ビルド、別バイナリ名で、0.005%以内。** 条件さえ固定すれば、この環境は事実上決定論的です。394cの±1.6%が占有状態の未管理によるものだったことが、これで完全に裏付けられました。

## SMALL_N について

走らせていただいたようで、N=5〜15すべて正しい値が出ています。1点だけ、**N=5 の行にだけ `ok` マーカーが付いていません**（値10は `expected[5]=10` と一致しているので、正当性は問題ありません）。

r6が持ち込んだものではないと思いますが、確証はありません。r5ディスパッチャで `-g 5 15` を一度走らせて同じ表示になるか、10秒ほどで確認できます。同じならr6とは無関係の既存表示です。

## 395cシリーズの締めくくり

7リビジョンで得たものを整理します。

**実利** — 400 ms、−0.299%。133,592 → 133,192。

**測定基盤** — ノイズフロアが 0.29% から **0.009〜0.029%** になりました。これが本当の収穫だと思います。「誤差に埋もれて棄却できなかった」過去の小改善が、いま測り直せば読めます。**0.3%のものさしで navigate していた過去のリビジョンで、見落とした改善がある可能性は十分あります。**

**恒久ルール** — 測定前のGPU占有ゲート。他プロセスのコンテキスト有無で4%、居残りメモリで最大6%動きます。

**設計制約** — CRunnerのデバイスメモリ使用量は性能に非線形かつ鋭敏に効きます。谷の幅は128 MiB未満。**N=22/23のmaxd16はこれに真正面から当たります。**

## 次にどこへ向かうか

残り **−8.4%**（133,192 → 122,000）。今日の0.30%は27分の1でした。次は headroom のある軸に移るべきだと思います。候補を3つ、私の推す順で。

**(1) ncuでストール内訳を測り直す。** 過去のプロファイルは 0.3〜1.6% のものさしで取られたものです。いまなら `stall_branch_resolving` 以下の内訳を、実際に効く粒度で読めます。カーネルの支配要因を現在の構成で確定させてから次の一手を選ぶのが、遠回りに見えて最短だと考えます。

**(2) 過去に棄却した小改善の再評価。** ノイズフロアが二桁下がった以上、「効果なし」と判定したもののうち、実は0.1〜0.3%あったものが混じっている可能性があります。棄却リストがあれば、上位いくつかを再測定する価値があります。

**(3) maxd16のメモリコスト先行測定。** N=22/23へ進む前提条件です。急ぎではありませんが、着手前に必ず要ります。

どれから始めましょうか。あるいは今日はここまでとして、READMEへの追記とOpen Objectivesの更新を先に固めますか。
...
VERSION_TAG:str="395c-r6 PY-REVTAG: pure rename of 395c-r5Py plus ONE behavioural change -- the dispatch table entry now carries env_prefix NQ_EXTRA_CTX=1 and names ./395c_r6_kernel_maxd14 (code region byte-identical to r4; kernel region byte-identical to 395a r2). This is the adoption of the 413 ms (-0.309%) that 395c-r5 measured on the -g path with all six pre-registered predictions holding. The treatment lives in the table, not in the binary default, so every run records its state via [crunner-config] env_prefix= and dispatch.log; NQ_EXTRA_CTX stays 0 when unset, leaving direct runs unchanged as a control. No other policy change: argc<=4 defaults, MAX_BLOCKS 800, sched input order, redirected external tools all stand."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./395c_r6_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
