...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 386b (2026-09-07)
...
VERSION_TAG:str="386b LEGACY-CLEANUP: removed the broadmark_tail_variant CLI-override capability from bench_mode==30 (was argv[16]) and bench_mode==31 (was argv[15]) -- confirmed unused by Suzuki, no plan to try variant values other than the already-adopted 2. The variable itself, its module-level default (BROAD_MARKDIST_TAIL_VARIANT=2 at declaration) and the bare -g default (A10G_FINAL_DEFAULT_BROADMARK_VARIANT=2) are all UNCHANGED -- only the CLI argv-read lines that could override it are gone. Deliberately did NOT renumber any subsequent positional argv slots (chunkshape148_bucket_run/chunkshape148_iter_sort keep reading argv[16]/argv[17] exactly as before for bench_mode==31; nothing followed the removed slot for bench_mode==30) -- the now-unused argv position is simply left unconsumed rather than repurposed, so any existing saved command line that still passes a value there keeps working identically except that value is now ignored instead of setting the variant. Continues 386a's staged legacy-cleanup approach (386a/386b micro-revisions, same real-hardware-round-trip discipline as 381a-381e)."
