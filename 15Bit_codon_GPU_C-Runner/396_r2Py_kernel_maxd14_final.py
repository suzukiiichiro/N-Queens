...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 396-r2 (2026-09-09)
...
"""
T1が設計どおりに落ちました。**ゲートが仕事をしています** — 代表していない代理でプロファイルを取れば、自信に満ちた誤った順位表が出ていました。

原因は数字から読めます。プレフィックス総和を差分すると、ラウンド（25,600レコード＝全スレッド1周）あたりのコストが出ます。
...
**ファイルの後半が圧倒的に重い。** 394fのスケジュールはレコードを並べ替えているので、先頭を切り出すと安い部分しか取れません。kに対して非単調だったのもこれで説明がつきます。切り詰めは、この入力に対して妥当な縮小ではありませんでした。

396-r2 を組みます。396-r2、四点セットです。静的ゲートは **OK=16 FAIL=0**。

## 代理の作り直し

**1ラウンド = 25,600レコード = stride 25,600 でちょうど全スレッド1周ぶん**です。ラウンド単位で抜けば、レコードとスレッドの対応が完全に保たれ、**warp内の分岐パターンもそのまま維持されます**。変わるのはループ回数だけ。

先頭k個ではなく、全79ラウンドに等間隔で散らしたk個を取ります（k=8なら 0, 11, 22, 33, 45, 56, 67, 78）。

## 順位表を人質にしない

層化でも較正に落ちた場合、**実物のフル入力を1セクションずつプロファイルします**。順序は `WarpStateStats` を先頭に（U5が本題なので）。セクションごとにタイムアウト900秒、全体予算2400秒。予算内に終わったものだけ報告します。

396の成果物はストール順位表であって、代理の設計問題でそれが得られないのは本末転倒なので。`FORCE_FULL=1` で最初からこちらを選べます。

## 事前登録

**U1** 層化 k ∈ {4, 8, 16} のいずれかで ±1% に入る。ラウンドコスト模型からの予測値は **k=4: 66.33（+0.85%）、k=8: 65.21（−0.84%）、k=16: 65.47（−0.45%）、k=2 は −3.64% で外れる**。

模型は rounds 16–79 を一様と仮定した粗いものです。実測がこれから外れること自体は模型の粗さを示すだけで、方法の否定にはなりません。だからこそフェーズ2で分布を直接測ります。

**U2** ラウンド単体のコストは位置に対して単調でなく、round ≥ 16 が一貫して高い。round 0,10,…,70 の8点で確認（各約1.5秒、合計30秒）。**これまで推定でしか言えなかったコスト分布を、初めて直接見ることになります。**

**U5（本題）** 支配的ストール理由は分岐解決系で25%以上。反証されれば律速要因は移動しており、397は最適化ファミリを変えます。

## 所要

アンカー2.5分 + ラウンド測定30秒 + 層化較正2分 + ncu。較正が通れば数分、フォールバックなら最大40分です。
...
一点、記録として。ラウンドコストが位置によって 1,160〜1,774 ms（**1.53倍**）違うのは394fスケジュールの性質として残す価値があります。ただし**全スレッドが全ラウンドを1つずつ処理するので、この偏りが負荷不均衡を生むことはありません**。念のため。

`sudo` の件、承知しました。ただし**ハーネス全体をsudoで走らせるのは避けたほうがいい**です。ログディレクトリ、バイナリ、`396_r2_crunner_logs/`、タールボールがすべてroot所有になり、次回の非sudo実行が壊れます。

**ncuの呼び出しだけをsudoにする**よう直します。併せて、私の設計ミスも1つ直します。

## 順序が間違っていました

権限プローブが**アンカーと較正の後**にありました。3秒で分かることのために、そちらは5分待たされています。ビルド直後に移します。`.sh` だけ差し替えてください。`.cu`、`.py`、READMEは変更ありません。手元で **OK=16 FAIL=0**。

## 変更点

**1. 権限プローブをビルド直後に移しました。** 元の版はアンカー（2.5分）と較正の後に置いていて、3秒で分かることのために5分待たせていました。私の順序ミスです。フェーズ順は次のとおりになりました。

```
静的ゲート → ビルド → ncu権限プローブ(3秒) → アンカー → ラウンド測定 → 較正 → プロファイル
```

**2. sudoはncuの呼び出しだけに掛けます。** 権限エラーを検出したら、自動でsudo付きプローブを再試行します。通れば `NCU_PREFIX="sudo"` を設定し、以降のncu実行だけを昇格します。

**ハーネス全体をsudoで走らせないでください。** ログディレクトリ、バイナリ、`396_r2_crunner_logs/`、タールボールが全部root所有になり、次回の非sudo実行が壊れます。アンカーも代理実行もファイル書き込みも、すべて通常ユーザーのままです。

**3. `reclaim` を追加しました。** ncuがroot権限で作ったファイル（`.ncu-rep` と `/tmp` の出力）を、各プロファイル実行の直後に元の所有者へ戻します。
...
そのままで大丈夫です。権限エラーを検出したら自動でsudoに切り替わり、パスワードを聞かれる場合があります。明示的に指定したければこちらでも。

```
NCU_PREFIX=sudo bash 396_r2_validate.sh
```

sudoでも通らない場合は、恒久設定を案内して停止します。
...
権限が通れば、アンカー2.5分 → ラウンド測定30秒 → 較正2分と進みます。**U2のラウンドコスト分布が、今日はじめて直接見える数字**になるはずです。


はい、**放置しても安全です。** 予算で頭打ちになっています。

残りの動きは決まっています。WarpStateStatsで900秒使ったので、SpeedOfLight（最大900秒）→ Occupancy（最大900秒）で予算2400秒に達して停止します。**最悪であと約30分**、そこで必ず終わって順位表の生成に進みます。無限には走りません。

ただし正直に言うと、**待つ価値は薄い**です。U5を答えるのはWarpStateStatsで、それが既に落ちています。残り2つが通ってもU4（メモリ律速でないこと）と利用率が分かるだけです。止めても構いません。Ctrl-Cする場合は、rootが作ったファイルが残るので後始末を。

```bash
sudo chown -R $(id -u):$(id -g) 396_r2_validate_*
```

## 層化も外れました。しかも外れ方が本質的です

| k | 切り詰め ms/round | 層化 ms/round |
|---:|---:|---:|
| 2 | 1,315.5（−21.9%） | 1,177.7（**−30.0%**） |
| 4 | 1,251.8（−25.6%） | 2,298.0（**+36.5%**） |
| 8 | 1,414.7（−16.0%） | 1,890.7（+12.3%） |
| 16 | 1,328.9（−21.1%） | 1,914.7（+13.7%） |
| フル | — | 1,683.6 |

切り詰めは一貫して安く、層化は一貫して高い。どちらも収束しません。

決定的なのは**加法性が壊れている**ことです。

```
round 0 単独            : 1,471.2 ms
rounds {0,78}           : 2,355.4 ms  → round 78 は   884 ms 相当
rounds {0,26,52,78}     : 9,191.8 ms  → 追加2本で 3,418 ms/本相当
```

同じ「1ラウンド」が、組み合わせ次第で884 msにも3,418 msにもなります。ラウンド個々のコストの問題ではありません。

**理由はスケジュールの性質です。** カーネル時間は最も遅いスレッドで決まります。394fは79ラウンド**全体**でスレッドごとの累積負荷が揃うように並べ替えている。部分集合を取ると、その均衡が壊れ、max がmeanから乖離します。切り詰めはたまたま楽な組み合わせに、層化はたまたま偏った組み合わせに当たった、というだけです。

> **均衡は集合全体の性質であって、部分集合に受け継がれません。スケジュール済み入力の部分集合を取ること自体が、この案件では原理的に誤った縮小です。**

これは私の設計ミスで、r2の層化も筋が悪かったです。ただ、この否定的知見自体は記録に値します。今後だれかが「入力を削って測る」と言い出したときに、二度と同じ穴に落ちないためです。

## 396-r3 の方針 — レコードではなくNを削る

部分集合が使えないなら、**それ自体が均衡している別の問題**を測ればよい。N=20とN=19は、同じカーネル・同じmaxd14経路を通り、**それぞれ自前の394fスケジュールを持つので構成上バランスしています**。

解の個数比から、N=20 ≈ 16.5秒、N=19 ≈ 2.1秒。ncuに載ります。

代表性の担保はこう置きます。**N=19とN=20でストール順位表が一致すること**を要求する。8倍の問題サイズ変化にわたって順位が動かないなら、N=21でも同じと考えてよい。一致しなければ順位はN依存で、N=21そのものを夜間に回すしかない、と結論できます。どちらに転んでも答えが出ます。

入力生成は既存パイプラインに任せられます。`-g 20 20` と `-g 19 19` を走らせれば、そのNのスケジュール済み入力が自動で作られ、ベースラインの `kernel_ms` も同時に取れます。外部スケジューラのCLIを知る必要がありません。

## お願い

いまの実行が終わったら（あるいは止めたら）、**タールボールを送ってください**。特に `round_profile.tsv` — フェーズ2の8点測定を私はまだ見ていません。ラウンドコストの実際の分布が分かれば、上の「均衡が集合全体の性質」という説明が裏付けられるか、あるいは別の絵が出るかが確かめられます。

それを見てからr3の四点セットを書き起こします。


数字が出ました。そして**私の予測U4は外れています。** しかも外れ方が、この案件の見立てを根本から変えるものです。

## 何が動いて何が落ちたか

| セクション | 結果 |
|---|---|
| WarpStateStats | **失敗**（`Failed to profile`、タイムアウトではなくエラー） |
| SpeedOfLight | **失敗**（同上） |
| Occupancy | 成功 |
| Scheduler Statistics | 成功 |

原因は**カーネル長**です。決定的な証拠があります。権限プローブは同じ SpeedOfLight を**1ラウンド入力（1.5秒）で成功させています**。フル入力ではncu下で191秒かかり、そこで落ちる。1.5秒で通り、191秒で落ちる。r3で N を下げる方針は、これで裏付けが取れました。

なお `6_ranked.txt` が空だったのは**私の抽出コードのバグ**です。ncuのCSVはデータ行の末尾フィールドが空で、ヘッダより列数が少ない。それを `len(r)<len(hdr)` で捨てていました。手元で直して読み出しています。
...
単独実行の平均は **114.3 µs/record**、フル実行は **65.8**。**1.74倍**です。79ラウンドを個別に走らせれば 231,500 ms、まとめて走らせれば 133,192 ms。

> **394fのスケジュールによる平均化が42.5%を稼いでいる。** これはこれまで直接測られたことのない数字です。

そして部分集合が使えない理由も確定しました。単独ラウンド＝「1レコードのコストのスレッド間最大値」、フル＝「79レコード合計の最大値」。後者は平均化でmeanに寄ります。**部分集合を測ると、命令ミックスではなく負荷不均衡を測ってしまう。**

## Occupancy — ここに構造的な天井があります

```
Block Limit SM         16 blocks   ← ハードウェア上限(sm_86)
Block Limit Registers  48 blocks   ← レジスタは制約になっていない
Theoretical Occupancy  33.33%
Achieved Occupancy     10.32%   (4.95 warps/SM)
```

`BLOCK = 32` は**1ブロック＝1ワープ**です。sm_86のブロック数上限16と掛け合わさって、理論オカパンシーが33.3%で頭打ちになっています。

| BLOCK | warps/SM | 理論オカパンシー |
|---:|---:|---:|
| **32（現行）** | 16 | **33.3%** |
| 64 | 32 | 66.7% |
| 128 | 48 | **100%** |

**レジスタは48ブロックぶん余っています。** つまり天井を作っているのはブロックサイズだけで、レジスタ圧ではありません。さらに達成値は理論値の1/3の10.32% — 半分のワープが早々に退出している（テールです）。

## Scheduler — 6割の時間、発行できる warp がありません

```
No Eligible                 61.63%
Active Warps Per Scheduler   2.41  (上限12)
Eligible Warps Per Scheduler 0.47
Issued Warp Per Scheduler    0.38
```

スケジューラあたり2.41ワープしか居らず、その6割の時間は誰も発行可能でない。

## U4 は外れました。そして私の推論の誤りです

権限プローブ（1ラウンド）のSpeedOfLightです。

```
Memory Throughput      46.73%
L1/TEX Cache Throughput 52.96%
L2 Cache Throughput    46.73%
DRAM Throughput        16.09%
Compute (SM) Throughput 19.41%
```

私は「入力57 MBを133秒で1回読むだけだから実効0.4 MB/s、帯域律速ではない」と何度も申し上げました。**グローバルメモリについてはその通りでしたが、そこで話を止めたのが誤りでした。**

L1/TEXが52.96%、コンピュートが19.41%。**メモリパイプの利用率が演算の2.7倍**です。DRAMは16%しか動いていないのにL1/L2が半分回っているということは、トラフィックの出所はDRAMではありません。

**ほぼ確実にローカルメモリ — レジスタスピルです。** maxd14のDFSは深さ14ぶんのスタック（ld / rd / col / …）を持ちます。1スレッドあたり4配列×14 = 56ワード以上。ところがレジスタは、48ブロック×32スレッド = 1536スレッド/SM から逆算して**1スレッドあたり最大42本**。**入りません。** スタックはローカルメモリに落ちているはずです。

これで全部が一本につながります。ローカルメモリのレイテンシを隠すべきワープが、BLOCK=32の天井とテールで2.41本しか居ない。だから6割の時間、発行できるものがない。

## いますぐできる、GPU時間ゼロの決定的な確認

```bash
nvcc -O3 -arch=sm_86 -Xptxas -v -c 396_r2_kernel_maxd14.cu -o /dev/null 2>&1 | grep -A2 kernel_dfs
```

`Used N registers, M bytes stack frame, S bytes spill stores, L bytes spill loads` が出ます。**スタックフレームが数十〜数百バイトあれば、ローカルメモリ仮説は確定**です。コンパイルするだけなので数秒、GPUも要りません。

これを先に見てから396-r3を設計させてください。**結果次第で、r3で測るべきものが変わります。**

- スピルが大きい → 主眼は `MemoryWorkloadAnalysis` の `mem_local` 系カウンタ。397の候補は「スタックのパッキング」「shared memoryへの移設」「BLOCK拡大」の三択になります。なお395bのレジスタトップ化（+3.5%で差し戻し）は、**オカパンシーをさらに下げたせいで負けた**可能性があり、BLOCK拡大と組み合わせれば結論が変わるかもしれません。
- スピルが無い → ローカルメモリ説は棄却。L1トラフィックの出所を別途特定する必要があり、r3の設計をやり直します。

この1コマンドの出力をいただけますか。

**確定しました。** そして予想と少し違う、より扱いやすい形です。
...
**スピルはゼロ。しかしスタックフレームが208バイトある。** これは「レジスタが足りずに追い出された」のではなく、**動的添字でアクセスされる配列が最初からローカルメモリに置かれている**という意味です。208バイト = 52ワード。深さ14のDFSスタックが4本ぶん、ちょうどです。

L1/TEX 52.96%、Compute 19.41%、DRAM 16.09% — トラフィックの出所はDRAMではなくローカルメモリ。**仮説は裏付けられました。**

ただし対処法は「スピル対策」ではありません。レジスタは37本で、48ワープ/SM時の上限42本にまだ5本余っています。**レジスタ圧の問題ではなく、配列そのものの問題です。**

## そしてここに、見落とされていた制約があります

ローカルメモリはL1に載ります。SMあたりの作業集合を計算します。

| 構成 | threads/SM | ローカル作業集合 | L1(128 KB) |
|---|---:|---:|---|
| **現行 800ブロック** | 320 | **66.6 KB** | 収まる |
| BLOCK=64・16ブロック/SM | 1,024 | 213 KB | **1.7倍超過** |
| BLOCK=128・48ワープ/SM | 1,536 | 319 KB | **2.5倍超過** |

**オカパンシーを上げると、ローカル作業集合がL1から溢れます。**

これは前回私が「BLOCK=32が33%の天井を作っている、128にすれば100%」と申し上げたのを**否定します**。ハードウェアの天井は確かに33%ですが、その手前に**L1容量という実効的な天井**があり、現行構成はそのすぐ内側に収まっています。800ブロックが経験的に最適だった理由が、これで説明できるかもしれません。

そして395bのレジスタトップ化が+3.5%で負けた件も、この枠組みなら再解釈できます。

**順序が逆でした。オカパンシーを上げる前に、スタックフレームを縮める必要があります。** 両者は結合していて、フレームサイズが「L1が溢れる前に何スレッド載せられるか」を決めます。

| フレーム | 100 KBに収まる threads/SM | 到達可能オカパンシー |
|---:|---:|---:|
| 208 B（現行） | 480 | 31% |
| 104 B | 960 | 62% |
| 52 B | 1,920 | 100%（48ワープで頭打ち） |

## 提案 — 397は3段構えで、最初の1手は1行です

**(A) L1カーブアウトの最大化。** このカーネルは `__shared__` も `__syncthreads` も使っていません（barriers=0）。sm_86の128 KBはL1と共有メモリで分割されるので、共有側に割り当てが残っていればローカルメモリが使えるL1が目減りします。
...
1行、アルゴリズム変更ゼロ、ノイズフロア0.01%で測れます。効かなければ即棄却できます。

**(B) L1容量仮説の直接検証。** MAX_BLOCKSを掃引し、`threads/SM × 208 B` がL1容量を跨ぐ点で性能が折れるかを見る。折れれば模型が確定し、**フレームを縮める価値が定量化されます**。ただしMAX_BLOCKSを変えるとstrideが変わり、スケジュール再生成が要る＝均衡の質も同時に動くので、交絡があります。設計に少し工夫が要ります。

**(C) スタックフレームの縮小。** 本命ですが実作業です。208バイトの中身を特定し、パッキングか再計算に置き換える。

## その前に一つ確認させてください

208バイトの内訳を見たいのですが、**現在のカーネルで、深さ方向にどんな配列を持っているか**を教えていただけますか。`process_one_task` の中の配列宣言（`uint32_t ld_stack[15]` のような行）です。手元の395c版ソースから読み取ることもできますが、**そちらの現物と食い違っていないか確認したい**ので。

それが分かれば、どれが必須でどれが再計算可能か、パッキングでどこまで縮むかを具体的に見積もれます。**52ワードのうち何ワード落とせるかが、そのままオカパンシーの上限に効きます。**

397の四点セットは、その見積もりを踏まえて書き起こします。


はい、そちらです。私が生成したものと**sha256が完全一致**（`fa5e5254…`）しています。食い違いはありません。中身を見ます。中身が分かりました。**208バイトはまるごとDFSスタックです。**

```c
#define MAXD14_ANCESTOR 13
uint64_t stack[MAXD14_ANCESTOR * 2];     // 26 × 8 = 208 バイト
```

1フレーム = 2ワード（16バイト）で、現在の詰め方はこうです。

```c
stack[sp]   = cur_ld | (cur_rd << 32);
stack[sp+1] = cur_col | ((cur_avail | (cur_depth << 27)) << 32);
```

`cur_depth` は既に `cur_avail` の空きビットに同居していて、そこは無駄がありません。問題は **ld / rd / col / avail の4本を、それぞれ32ビット枠で保存している**点です。N=21なら有効幅は各21ビット、合計84ビット。**128ビット使って84ビットを運んでいます。**

ただ、単純な詰め直し（84ビット → 3ワード96ビット）では 208 → 156バイト、25%減にしかなりません。オカパンシー階級を変えるには足りない。

そこで、この構造にはもっと大きな冗長性があるはずです。確認させてください。スタックの中身が読めました。そして**私が前回口にした「ld/rd/colは復元できる」は、そのままでは成立しません**。訂正します。

## 子状態の作り方と、逆算可能性

```c
nld  = ((ld | bit) << step) | addv | bLi;   // step は 1〜3
nrd  = ((rd | bit) >> step) | bK;
ncol = col | bit;
```

- **col は完全に逆算可能。** `col = ncol & ~bit`。
- **ld も逆算可能。** `(nld >> step) & ~bit`。左シフトで空いた下位に OR された `addv | bLi`（および jmark の `|= 1u`）は、右シフトで捨てられます。
- **rd は逆算できません。** `>> step` で下位 step ビットが破壊されます。`bK` を消しても元には戻りません。

したがって「bit だけ持てば半減」という前回の見通しは誤りでした。

## 確実に取れるもの

現在1フレームは **128ビットで84ビットの情報**を運んでいます（ld/rd/col/avail 各21ビット、depth は avail の空きに同居済み）。

| 案 | フレーム | スタック | 確実性 |
|---|---:|---:|---|
| 現行 | 16 B | 208 B | — |
| **単純な再パック**（84ビット→3ワード） | 12 B | **156 B**（−25%） | 代数不要、純粋な詰め替え |
| bit方式（rdの扱いを解決できれば） | 8 B | 104 B（−50%） | **未解決** |

156バイトなら L1 100 KB に載るスレッド数は 480 → 656、オカパンシー上限は 31% → 43%。効きますが、階級は変えません。

## ただ、ここで手を止めるべきだと思います

いま私が根拠にしているメモリ側の数字（L1/TEX 52.96%、Compute 19.41%、DRAM 16.09%）は、**すべて1ラウンド代理から出たもの**です。その代理は、我々自身が「代表していない」と結論して棄却したものです。

しかも競合する説明が2つ残っていて、区別できていません。

**(a) 容量説** — 320スレッド/SM × 208 B = 66.6 KB。L1の128 KBには収まるので、この説だと現行構成では溢れていないことになります。ならば L1圧の主因は別にあります。

**(b) コアレス説** — スタック深さがスレッドごとに違うため、`stack[stack_ptr]` のアドレスがワープ内で発散し、1リクエストが多数のセクタに割れる。作業集合が小さくてもL1トラフィックは膨らみます。DRAM 16%（数十GB/s相当）が出ているのは、こちらのほうが説明しやすい。

**(a)なら詰め替えが効きます。(b)なら詰め替えの効果は限定的で、効くのはアクセスパターンの是正です。** 打ち手が変わります。

そして何より、**U5（支配的ストール理由）の答えをまだ持っていません。** 元々の問いです。WarpStateStatsは一度も成功していません。

208バイトの正体が分かったのは前進ですが、**代表性のないプロファイルを根拠にカーネルへ手を入れるのは、この案件でずっと避けてきたことそのもの**です。395a r2以来 sha256 が動いていないカーネルに触るなら、根拠は代表的な測定でなければいけません。

## 396-r3 を先に

方針は変わりません。**N=20 / N=19 で測る。** 1.5秒で通り191秒で落ちるという実測があるので、N=20（約16秒）は通る見込みです。

セクションの優先順位だけ組み替えます。

1. **WarpStateStats** — U5。まだ一度も答えが出ていない
2. **MemoryWorkloadAnalysis** — (a)と(b)の判別。`mem_local` のリクエスト数とセクタ数の比、L1ヒット率
3. SpeedOfLight — U4の再取得（今度は代表的な負荷で）
4. Occupancy / SchedulerStats — N依存性の確認（N=21での値と比較できる）

代表性は「N=19とN=20で順位表が一致すること」で担保します。加えて今回、**N=21で Occupancy と SchedulerStats は取れている**ので、その2つについては N=19/20/21 の3点比較ができます。N依存性の直接検証になります。

r3の四点セットを書き起こしましょうか。それとも、単純な再パック（208→156、代数不要）を先にCPU等価ゲート付きで試して、L1容量説の当否を性能で直接見る、という順序をお好みでしょうか。後者は測定を待たずに動けますが、**(b)が正解だった場合は空振りします**。
...
VERSION_TAG:str="396-r2 PY-REVTAG: pure rename of 396Py; the dispatch table names ./396_r2_kernel_maxd14 and keeps the env_prefix NQ_EXTRA_CTX=1 adopted in 395c-r6. No functional change; the .cu code region is byte-identical to 396 and r6, the kernel region to 395a r2. r2 exists because 396 T1 rejected prefix truncation as a proxy: the 394f schedule puts the cheap records first, so a prefix runs 13-26 percent cheap per record. r2 samples whole rounds spread across the file instead, and falls back to profiling the real full input one section at a time if that still fails."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./396_r2_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
