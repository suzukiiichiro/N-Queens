...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 365 (2026-08-29)
...
6. [進行中・365で単発構成のCodon側基準測定を追加、実機結果待ち]
   CUDA Cランナー(案C-2)スパイク設計
   334-337で土台確立、338→360で移植仕様確定、361でSoA導出のCポート
   実機確定、362でカーネル設計確定、363でカーネル実装(CPU側検証)、
   **364で実機nvccビルド+GPU実行が完了し、正当性確定**
   (N=21実データ全件2,025,282件でtotal_sum=314666222712、単発
   カーネル起動でkernel_ms=260685.062・total_ms=260690.297)。

   364の測定は全件を1回のカーネル起動で処理する単発構成であり、
   356アンカー(393.404秒、3チャンクmeasure2プロトコル)とは測定条件
   が異なる。364の結果が356比33.7%速い数値だったため、Suzukiさんの
   判断により**364の単発構成そのものを今後の正式な比較基準として
   採用**することとした(356の3チャンク構成を厳密に再現する方向では
   なく)。

   **365でCodon側にも同じ単発プロトコルの基準測定を追加した**
   (`exec_solutions_gpu_single_shot`、新規bench_mode=33)。
   `launch_kernel_dfs_iter_gpu_static_maxd`(既存のカーネル起動
   プリミティブ、変更なし)を、STEPS(15488)でチャンク分割する
   `exec_solutions`とは異なり、全2,025,282件のSoAを一括構築した上で
   **1回だけ**呼び出す構成とし、364のCランナーと同一のアーキテクチャ
   (grid=484,block=32,stride=15488、カーネル内部のgrid-strideループが
   全件を処理)で測定する。361のr1教訓(CLIホワイトリストゲートに
   新規bench_modeを追加し忘れる不具合)を踏まえ、今回は最初から
   `bench_mode==33`を2箇所のゲートへ追加済み。

   365自体はこのセッション内でGPU環境が無いため未実機検証、
   Suzukiさんの実行結果(364の260.690秒との±3%比較)待ち。
...
365を用意しました(単発構成でのCodon側基準測定、364との公平な比較
のため)。

**364までの状態**: 実機nvccビルド+GPU実行が完了し、N=21実データ全件
でtotal_sum=314666222712——正当性確定。単発カーネル起動での測定は
kernel_ms=260685.062(260.685秒)。356アンカー(393.404秒、3チャンク
measure2プロトコル)より33.7%速い数値だったが、測定条件(単発 vs
3チャンク)が異なるため、そのまま比較基準にはできなかった。

**365の内容**: Suzukiさんの決定(364の単発構成を新しい基準として
採用)を受け、Codon側にも**同じ単発プロトコル**での基準測定を追加
しました。新規関数`exec_solutions_gpu_single_shot`+新規
`bench_mode=33`。既存のカーネル起動プリミティブ
`launch_kernel_dfs_iter_gpu_static_maxd`は無変更のまま、呼び出し方
だけを364のCランナーと同一のアーキテクチャ(全件一括SoA構築→
grid=484,block=32での単発起動)に揃えています。361のr1で踏んだCLI
ホワイトリストゲートの教訓を活かし、今回は最初から`bench_mode==33`
を2箇所のゲートに追加済みです。
...
```
STATIC_ONLY=1 bash 365Py_gpu_single_shot_validate_N21_full_once.sh
bash 365Py_gpu_single_shot_validate_N21_full_once.sh
```

`.sh`は静的チェックに加え、実機でbench_mode=33を実行し、
`total=314666222712`との正当性照合、および364の`kernel_ms`
(260685.062ms)との±3%比較を自動で行います。
"""
...
VERSION_TAG:str="365 gpu_single_shot: ADDITIVE REVISION. Executable code region is byte-identical to 364/363(r2) outside five clearly delimited deltas (two pure insertions, two 2-word CLI-gate modifications matching 361-style, one pure insertion for the dispatch branch), verified by a static check that strips/reverses those spans before hashing against 364s reference hash for the identical region. The deltas are: (1) ===365-INSERT-BEGIN/END=== -- new function exec_solutions_gpu_single_shot(), inserted immediately before select_dynamic_preset_queens(), which builds SoA for the FULL N=21 record set (not chunked at STEPS=15488 the way exec_solutions() internally chunks) and calls the existing launch_kernel_dfs_iter_gpu_static_maxd() primitive (unchanged) exactly ONCE, mirroring 364s CUDA C host runners single-shot architecture (grid=484, block=32, stride=15488, the kernels own grid-stride loop covers all 2025282 records in one launch); (2)-(3) ===365-CLIGATE-COMMENT-BEGIN/END=== and ===365-PRESETGATE-COMMENT-BEGIN/END=== -- comments plus a 2-word addition ('or bench_mode==33') to the same two CLI gate lines 361 modified for bench_mode==32, applied preemptively this time (361s r1 hung for an hour because this exact gate silently reset an unlisted bench_mode back to 0 before its dispatch branch was ever reached; 365 does not repeat that mistake); (4) ===365-DISPATCH-INSERT-BEGIN/END=== -- new bench_mode==33 dispatch branch, appended after the existing bench_mode==32 block. Purpose: 364s single-shot GPU run (kernel_ms=260685.062ms on real N=21 data, total_sum=314666222712 confirmed) was measured under a DIFFERENT protocol than the 356 anchor (393.404s, 3-chunk measure2) -- 33.7 percent faster, but not yet a valid plus-or-minus 3 percent comparison since the measurement conditions differ. Per Suzukis decision, rather than replicating 356s exact 3-chunk protocol, 364s simpler single-shot configuration is adopted as the new standard going forward; 365 gives Codon a fair baseline measurement under that SAME single-shot protocol (one kernel launch covering all 2025282 records, timed the same way, no artificial chunking) so the eventual comparison against 364s CUDA C result is apples-to-apples. This revision has no GPU available in-session and is Suzukis next action item; the eventual plus-or-minus 3 percent check is against 364s kernel_ms=260685.062ms, not the old 356 anchor of 393.404s."
