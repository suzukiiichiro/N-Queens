 ...
 *   /usr/local/cuda/bin/nvcc -O3 -arch=sm_86 -o 388_kernel_maxd14 388_kernel_maxd14.cu
 * Run (device):
 *   ./388_kernel_maxd14 <N> <in_soa7_bin> <out_results_bin> [expected_total]
 *   e.g. ./388_kernel_maxd14 21 constellations_N21_6.bin.soa_ref_361.bin.maxd14only_363.bin \
 *          /tmp/gpu_results.bin 314666222712
 * Build (host-only CPU test, no CUDA toolkit needed, this sandbox):
 *   gcc -O2 -Wall -Wextra -o 388_kernel_maxd14_cputest 388_kernel_maxd14.cu -x c -lm
 * Build (host-only CPU test, OpenMP-parallelized outer loop):
 *   gcc -O2 -fopenmp -Wall -Wextra -o 388_kernel_maxd14_cputest_omp 388_kernel_maxd14.cu -x c -lm
 *
 ...
 * byte-identical to 364_kernel_maxd14.cu by direct diff. No new nvcc
 * build has been run against this exact filename yet; that is the
 * next real-hardware step before crunner_dispatch_table() is updated
 * to point at it.
 */
