/*
 * 381e_hybrid_kernel_localtotal.cu
 *
 * rev381e — Tests Suzuki-san's hypothesis directly (ncu's per-
 * instruction data for this persistent/stateful kernel was too
 * ambiguous to trust -- every ATOMG.ADD instruction's row showed
 * uniformly zero across ALL columns including "Instructions
 * Executed", which is consistent with genuine zero executions in the
 * profiled slice but ALSO consistent with ncu's replay mechanism not
 * resetting device state between the multiple passes SourceCounters
 * needs for THIS kind of stateful kernel -- a risk flagged before
 * profiling even started). Rather than trust ambiguous profiling
 * data, this revision directly implements and measures the fix:
 * run_hybrid_episode() now accumulates into a local uint64_t
 * (register-resident, like 374's own `total`) instead of calling
 * atomic_add_result() on every terminal-depth hit, doing exactly ONE
 * atomicAdd at each of its two exit points. This is correct at ANY
 * K_THRESHOLD, not just 13: a thread handling an overflow Frame from
 * the queue still gets its own local accumulator and its own single
 * atomicAdd, independent of any other thread/episode working on the
 * same task_id.
 *
 * Everything else (process_task_hybrid's own structure, the Vyukov
 * queue, active_static_workers, the K=13 no-fallback-phase skip from
 * 381d) is UNCHANGED.
 */
