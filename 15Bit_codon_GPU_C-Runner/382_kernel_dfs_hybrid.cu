/*
 * 382_kernel_dfs_hybrid.cu
 *
 * rev382 — CONSOLIDATION of the 375-381e CUDA C hybrid-kernel
 * research track (no logic changes from 381e; pure rename, same
 * treatment already applied several times this project: only
 * filename self-references in comments changed).
 *
 * WHAT THIS FILE IS: the current fastest VERIFIED-CORRECT build of
 * the "local-stack-primary DFS + threshold-K overflow to a shared
 * Vyukov MPMC queue" design (378 spec), running at K_THRESHOLD=13
 * (local capacity == MAXD14_ANCESTOR*2, provably zero shared-queue
 * traffic at this K). Real-hardware confirmed:
 *   total_sum=314666222712 (MATCH), kernel_ms=263,003.750
 *
 * WHAT THIS FILE IS NOT: a replacement for 374Py_kernel_maxd14.cu /
 * 382Py_kernel_maxd14_final.py, which remain the actual PRODUCTION
 * kernel (kernel_ms~=201,232, still ~31% FASTER than this file at
 * K=13). This file exists to preserve the K-sweep research
 * infrastructure (frame queue, LocalStack, process_task_hybrid,
 * run_hybrid_episode) at its best-known-working state, so future
 * work sweeping K_THRESHOLD downward (the ORIGINAL research question
 * this whole track exists to answer -- does sharing overflow via the
 * queue actually improve on 374's static grid-stride once K<13?) can
 * resume from a verified, real-hardware-tested starting point instead
 * of re-deriving it. See 382Py_README_append.md for the full 375-
 * 381e journey (5 real-hardware round trips: 613,041ms -> 569,157ms
 * -> 265,403ms -> 263,003ms, with two disproven hypotheses along the
 * way -- atomic contention count and per-terminal-hit atomicAdd
 * frequency -- and one confirmed dominant cause, a multi-billion-
 * iteration termination-check spin loop).
 *
 * The 32-484 grid/block launch shape, the FrameQueue/TaskSchedule
 * device allocations, process_task_hybrid()/run_hybrid_episode()'s
 * arithmetic (byte-identical to 374's process_one_task() throughout),
 * and the K_slots>=MAXD14_ANCESTOR*2 fallback-phase skip are all
 * UNCHANGED from 381e.
 */
    ...
    uint64_t initial_total)
{
    ...
    /* BUG-HYPOTHESIS TEST (381e): 374's process_one_task accumulates
     * into a plain register-resident `total` for the task's ENTIRE
     * traversal, writing results[] exactly once. 381b/c/d instead
     * called atomic_add_result() on EVERY terminal-depth hit -- at
     * K=13, each such hit is still processed by the SAME thread that
     * owns this episode (no queue involvement), so there is no
     * correctness reason to make each one a separate atomic. Now
     * accumulated locally, with exactly ONE atomicAdd at each of this
     * function's two exit points (still correct at ANY K: a thread
     * that picks up an overflow Frame from the queue at lower K also
     * gets its own local_total here and does its own single atomicAdd
     * for whatever IT contributes, independent of other threads/
     * episodes working on the same task_id). */
    uint64_t local_total = initial_total;

    for (;;) {
        if (cur_avail == 0u) {
            if (local->save_sp == 0u) {
                atomic_add_result(results, task_id, local_total);
                return;
            }
            local->save_sp -= 1u;
            local->stack_ptr -= 2;
    ...
    if (root_after_second == 0u) {
        ...
        if (pr_descend != 0u) {
            ...
            run_hybrid_episode(task_id, pr_nld, pr_nrd, pr_ncol, pr_nf, 1,
                                ts_out, bm, n3, n4, local, K_slots, q, results, push_overflow, 0);
        }
        return;
    }
...
__global__ void kernel_dfs_hybrid_maxd14(
    ...
    /* FIX (381d): at K_slots>=MAXD14_ANCESTOR*2 (K_THRESHOLD>=13),
     * local-stack overflow is PROVABLY impossible -- nothing can ever
     * reach the shared queue, so there is nothing to "help drain" and
     * no reason for any thread to wait for stragglers. Skipping the
     * whole fallback+spin-wait phase in that case eliminates the
     * multi-billion-iteration busy-spin ncu measured in 381c (see
     * file header). This flag is NOT a correctness shortcut for
     * K<13 -- the full fallback path below is unchanged and still
     * runs whenever real overflow is possible. */
    const int no_fallback_needed = (K_slots >= MAXD14_ANCESTOR * 2);
    ...
    for (;;) {
        ...
        if (no_fallback_needed) {
            break; /* K>=13: provably nothing to drain, just finish -- matches 374 exactly */
        }
        if (!announced_done) {
            announced_done = 1;
        ...
        if (fq_pop(q, &f)) {
            ...
            run_hybrid_episode(f.task_id, f.cur_ld, f.cur_rd, f.cur_col, f.cur_avail, (int)f.cur_depth,
                                &schedules[f.task_id], board_mask, n3, n4, &local, K_slots, q, results, &of, 0);
            if (of) atomicExch(push_overflow_flag, 1);
            atomicAdd(active_workers, -1);
        } else {
