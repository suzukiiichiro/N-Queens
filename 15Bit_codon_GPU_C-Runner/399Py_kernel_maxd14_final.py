...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 399 (2026-09-09)
...
"""
399の四点セットです。カーネルは触りません（398-r2のリネーム）。

**フェーズCの設計で1つ、有利な性質を使います。** レコードは互いに独立した課題で、`total_sum` はその総和です。したがって**並べ替えは答えを変えません**。順序実験は構成上安全で、それでもオラクルゲートは全走行に掛けます。

そして**ワープを共有する32レコードは、ファイル上の連続32個**です（`tid` が32連続、各スレッドは `tid + k·25600` を担当）。これで同質性の指標がそのまま計算できます。399の四点セットです。静的ゲートは **OK=13 FAIL=0**、置換ツールも手元で自己検査済みです（4種すべてでバイト多重集合が一致）。

## 4フェーズ

**A（10分）— 宿題** `smsp__average_warps_issue_stalled_*` を18種すべて明示列挙します。`WarpStateStats` セクションが返すのは要約4項目だけで内訳が入っていなかった、というのが398の不備でした。N=20とN=19の両方で取ります。

**B（1分・GPU不要）— 394fは何を並べているのか** ワープを共有する32レコードは**ファイル上の連続32個**なので、連続32ブロックの内部分散と全体分散の比を取るだけで判ります。`popcount(free)`、`popcount(col)`、`w_lo` の3つのproxyで。

**C（5分）— 順序実験** `asis`（394f）/ `shuffle` / `asc` / `desc` を各3反復。N=20は1回16秒なので安く回せます。

**D（8分）— 機構の確認** 勝った順序と394f順で `Avg Active Threads Per Warp` を実測し、時間の動きと稼働率の動きが結びつくかを見ます。

## 事前登録で、私は2つ賭けています

**X1で、私は394a時点の知見を覆す側に賭けます。** 首位は `long_scoreboard`（メモリ待ち）で25%以上、と表明します。根拠はL1が70.64%で最大パイプ、発行あたり6.14 warp-cycle、1レコード16.4万回のローカルアクセス。**分岐解決が首位なら、レーン稼働率の解釈もやり直しです。**

**X4では自分の実験が勝たない方に賭けます。** proxyソートは394fを1%超えて上回らない、と。394fはまさにこの問題のために設計されています。**このフェーズの価値はX3の対照とフェーズDの実測にあり、勝つことではありません。**

**そのX3が今回いちばん効きます。** 無作為シャッフルと394f順の差が0.5%以内なら、**並び順は無関係**で、フェーズCの軸そのものが死にます。否定的結果ですが、それが分かれば無駄な労力を大きく節約できます。逆に2%以上遅ければ、並び順は生きた軸です。

## 安全性

全順序で `total_sum = 39,029,188,884` を要求します。**置換が答えを変えたら、レコード間に隠れた依存があるということで、並べ替え実験以前に本案件の前提が崩れます。** ハーネスはそこで停止し、時間を報告しません。
...
カーネルは触っていません。コード領域全体 `c5dbde3f…`、カーネル領域は395a r2の `ebd7f523…` のままです。

数字の出どころを整理すると、**N=20の方が本命です。**

## この比較が何を測っているか

どちらも壁時計（入力読み込みとCodon側のオーバーヘッド込み）で、`kernel_ms` ではありません。

| | 395bPy | 現行 | 差 |
|---|---:|---:|---:|
| N=20 | 42.844 s | **17.877 s** | **−58.3%（2.40倍）** |
| N=21 | 141.052 s | 135.103 s | −4.22% |

## N=20の2.4倍は、398-r2の1行の副産物です

395bPyでは `N>=21` のゲートにより **N=20 はCodon側のGPU経路**を通っていました。`CRUNNER_MIN_N=19` にしたことで CRunner 経路に乗り、`kernel_ms = 16,413.852`（壁17.877秒はこれ＋約1.5秒）になりました。

**Codon経路の42.8秒に対してCRunnerで16.4秒。** これは測定を可能にするための変更でしたが、**それ自体が本物の改善**です。N=19も同様で、5.944秒 → `kernel_ms = 2,121.613` に落ちています。

## N=21の4.22%は、今日の成果ではありません

**395bPyが指しているのは395bカーネル**——レジスタトップ化で **+3.5%の退行**を起こし、395cで差し戻したものです。

```
133.192 s × 1.035 = 137.85 s  ← 395bカーネルの推定
+ Codon側オーバーヘッド 3.2 s  = 141.05 s   （実測141.052）
133.192 s + 1.9 s              = 135.10 s   （実測135.103）
```

つまりこの4.22%は**395cで既に回収済みの退行分**であって、今日追加したものではありません。

**ご指摘のとおりです。N=21に対して今日確定した改善は、395c-r6の413 ms（−0.31%）だけです。** それ以降の396〜399は、すべて診断です。

## ただ、診断は空振りではありません

今日判明したもののうち、N=21に効く可能性があるのはこれです。

```
レーン稼働率   10.53 / 32 = 32.9%
セクタ効率                  56.1%
ローカルメモリ実効効率      18.5%
L1/TEX 70.64% 対 Compute 31.55%
```

**32.9%は、この案件がこれまでに測った中で最大の単一非効率です。** 0.3%を追ってきた軸とは桁が違います。399はここに手を掛けるための最初の一歩で、X3（無作為シャッフル対照）が通れば軸が生きていることが確定し、通らなければ**その軸を捨てて別の攻め方に移る判断**が3分でつきます。

## 提案 — `CRUNNER_MIN_N` は正式に採用すべきです

N=20で2.4倍、N=19でも改善しています。**測定用の便宜として入れたものが、実は本番の改善でした。** ただし現状は事前登録も交互A/Bも経ていません。

399を走らせた後、小さなリビジョンで正式化することをお勧めします。

- `-g 20 20` と `-g 19 19` を、`CRUNNER_MIN_N=19`（新）と `=21`（旧）で交互A/B、各2回
- N=18以下への波及がないことの確認（`-g 5 18` の正当性）
- 事前登録: N=20 で新 ≤ 旧 −50%、N=21 は不変（±0.15%）

**N=21には効きませんが、N=19/20は今後の測定基盤そのものです。** 398-r2で代理として使うと決めた以上、その経路が速いことには実務上の価値があります。

まず399を走らせてください。X1（ストール理由）とX3（順序の対照）が、次にどこへ向かうかを決めます。

**X3が圧倒的に成立しました。** 並び順は生きた軸です。そして**私のハーネスにバグがあり、フェーズAとDが丸ごと失われました。**
...
| **shuffle** | **22,285.509** | 0.010% | **+35.205%** |

全12走行でオラクル一致。**置換不変性は成立**しています。

**無作為シャッフルは35.2%遅い。** 事前登録の閾値は2%でしたから、桁違いに超えました。**394fのスケジュールは、無作為に対して26.0%を稼いでいます。**

X4も表明どおり成立しました。私のproxyソートはどちらも394fに勝てていません（desc +0.40%、asc +3.47%）。

反復幅が0.01〜0.04%というのも収穫です。**N=20でもノイズフロアは十分低い。** 16秒で回せる測定台が手に入りました。

## フェーズB — 394fが何を揃えているか

| proxy | 32個組内分散 ÷ 全体分散 |
|---|---:|
| `popcount(free)` | 0.638（部分的） |
| `popcount(col)` | 全体分散ゼロ（**全レコードで同一**、proxyとして無効） |
| **`w_lo`** | **0.082（既に強く整列）** |

**394fは `w_lo`（スケジュールワード）でワープ内を強く揃えています。** X2は `w_lo` について棄却されました。`desc` が+0.40%まで迫れたのは、`popcount(free)` 降順が394fの整列を粗く近似しているからでしょう。

`popcount(col)` の分散がゼロなのは道理で、同じ深さのレコードは配置済みクイーン数が同じです。**proxyとして意味がありません。**
...
**399のハーネスから、ncu権限プローブとsudo自動フォールバックを落としてしまいました。** 398-r2には入れてあったものを、399を書くときに `command -v ncu` だけに縮めています。結果、全ncuパスが非特権で走って失敗しました。

X1（ストール理由）とX5（レーン稼働率）は取れていません。**U5はまだ未回答のままです。**

## それでも、失われた測定の価値が上がりました

シャッフルとの35.2%差は、**レーン稼働率の較正点**として使えます。

394f順で `Avg Active Threads Per Warp = 10.53/32`。もし時間がレーン稼働率に反比例するなら、

```
shuffle の予測値 = 10.53 × (16,482.7 / 22,285.5) = 7.79 / 32
```

**この予測が当たれば、機構が確定するだけでなく、傾きが得られます。** 「レーン稼働率を1レーン上げると何%速くなるか」が定量化でき、399で開けた軸に**値札が付きます**。

## 399-r2の提案

1. **権限プローブを戻す**（398-r2のブロックをそのまま）
2. **フェーズA**: ストール理由を明示メトリクスで（X1の宿題、3回目）
3. **フェーズD拡張**: `asis` / `shuffle` / `desc` の**3点**でレーン稼働率を実測 → 較正
4. **フェーズE（新規）**: レコードごとの実測コストproxyを作る

4について。`NQ_MAX_BLOCKS` を大きく取って **1スレッド1レコード**にすると、出力ファイルは**レコードごとの解数**になります（現在はスレッドごとの合計）。これは実測の仕事量proxyです。それでソートして時間を測れば、**「理想に近い並び順」がどこまで行けるかの上限**が見えます。1回の追加実行と数分で済みます。

事前登録は、shuffleのレーン稼働率 **7.0〜8.6/32**、外れれば時間とレーン稼働率は単純比例ではないと結論します。

399-r2の四点セットを書き起こしましょうか。
"""
...
VERSION_TAG:str="399 PY-REVTAG: pure rename of 398-r2Py; the table names ./399_kernel_maxd14, keeps the env_prefix NQ_EXTRA_CTX=1 from 395c-r6 and keeps CRUNNER_MIN_N=19 from 398-r2. No functional change. 399 finishes 398s homework (the warp stall breakdown, which needs explicit smsp metrics rather than the WarpStateStats section) and opens the lane-utilisation axis: 398-r2 measured 10.53 active threads per warp out of 32, the largest single inefficiency found so far, and which 32 records share a warp is decided purely by the host-side record ordering."
...
def crunner_dispatch_table()->List[CRunnerEntry]:
  ...
  return [
    ...
    CRunnerEntry(14,"./399_kernel_maxd14","NQ_EXTRA_CTX=1 ","[gpu-run-done]","[gpu-run-correctness]"),
  ]
