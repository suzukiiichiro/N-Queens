...
function symmetryOps()
{
  ...
  #     local -i bit=1;
  #     local -i you=size-1;
  #     while (( board[you]!=ptn && board[own]>=bit ));do
  #       ((bit<<=1,you--));
  #     done
  #     if ((board[own]>bit));then
  #       return ;
  #     fi
  #     if ((board[own]<bit));then
  #       break ;
  ...
  # fi
    ...
    for ((i=0;i<size;i++));do
      for ((j=0;j<size;j++));do
       if (( bitBoard[i]&1<<j ));then
          #sEcho="${sEcho}$((j)) ";
          solution[$i]="$j"; # colの値が代入
       fi 
      done
    done
  ...
  local -a r90;
  for ((i=0;i<size;i++));do
    r90[$((solution[i]))]=$(( size-1-i ));  
   r180[$((n-1-i))]=$(( n-1-solution[i] ));  
   r270[$((n-1-solution[i]))]=$(( i ));  
  done 
  flag=0;
  for ((i=0;i<size;i++));do
    if (( solution[i]==r90[i]==r180[i]==r270[i]));then
      flag=1;
    else
      flag=0;
    fi
  ...
  ';
  #180度回転
  if (( board[size-1]==ENDBIT ));then
    local -i you=size-1-1;
    local -i own=1;
    while ((own<=size-1 ));do
      local -i bit=1;
      local -i ptn="$TOPBIT";
      while (( ptn!=board[you] && board[own]>=bit ));do
          ((bit<<=1,ptn>>=1)) ;
      done
      if ((board[own]>bit));then
        return ;
