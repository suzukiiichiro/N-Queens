...
void backTrack2(int y,int left,int down,int right,int *si, int *bo,int *bo2,int *ma,int *sm,int *lm,int *to,int *en,int *p,long *c2,long *c4,long *c8);
void backTrack1(int y,int left,int down,int right,int *si,int *bo,int *ma,int *p,long *c8);
void *run(void *args);
void *NQueenThread( void *args);
void NQueen(int si);
int main(void);
void symmetryOps_bm(int *si,int *bo,int *bo2,int *to,int *en,int *p,long *c2,long *c4,long *c8);
...
void backTrack2(int y,int left,int down,int right,
    int *si,int *bo,int *bo2,int *ma,int *sm,int *lm,
    int *to,int *en,int *p,long *c2,long *c4,long *c8){
  int bit=0;
  int bm=*ma&~(left|down|right);  //配置可能フィールド
  if(y==*si){                     //【枝刈り】
    if(bm){                    
      if((bm&*lm)==0){            //【枝刈り】最下段枝刈り
        *(p+y)=bm;
        symmetryOps_bm(si,bo,bo2,to,en,p,c2,c4,c8);
      } 
    }
  }else{
...
void backTrack1(int y,int left,int down,int right,
    int *si,int *bo,int *ma,int *p,long *c8){
  int bit=0; 
  int bm=*ma&~(left|down|right);  //配置可能フィールド
  if(y==*si){
...
void *run(void *args){
  ...
  if(B1>1&&B1<siE) { 
    if(B1<siE){
      ...
      *(p+1)=bit=(1<<B1);
      //２行目から探索
      backTrack1(2,(2|bit)<<1,(1|bit),(bit>>1),sE,bo,ma,p,c8); 
    }
  }
  int EB=(TB>>l->B1);int *en=&(EB);
  int SM=(TB|1);int *sm=&(SM);
  int LM=(TB|1);int *lm=&(LM);
  /* 最上段行のクイーンが角以外にある場合の探索
    ...
    if(l->B1<l->B2) {
      *(p)=bit=(1<<B1);
      backTrack2(1,bit<<1,bit,bit>>1,
        sE,bo,bo2,ma,sm,lm,to,en,p,c2,c4,c8); 
    }
    EB>>=si;
  }
  l->COUNT2=*c2; //構造体に格納
