...
class NQueen{
   ...
   　全ての可能性のある解の候補を体系的に数え上げ、それぞれの解候補が問題の解となるかをチェックする方法
     (※)各行に１個の王妃を配置する組み合わせを再帰的に列挙組み合わせを生成するだけであって8王妃問題を解いているわけではない
      :
      :
    ...
    */ 
    ...
    /**
		...
		*/
		...
		/**
	   NQueen3  バックトラック
	  各縦横列に加え斜め１個の王妃を配置する組み合わせの配置フラグ
    各列、対角線上にクイーンがあるかどうかのフラグを用意し、途中で制約を満たさない事が明らかな場合は、それ以降のパターン生成を行わない。
    各列、対角線上にクイーンがあるかどうかのフラグを用意することで高速化を図る。
    これまでは行方向と列方向に重複しない組み合わせを列挙するものですが、王妃は斜め方向のコマをとることができるので、どの斜めライン上にも王妃をひとつだけしか配置できない制限を加える事により、深さ優先探索で全ての葉を訪問せず木を降りても解がないと判明した時点で木を引き返すということができる。
...
/**
 * 各縦横列に１個の王妃を配置する組み合わせを再帰的に列挙 Ellis HorowitzとSartaj Sahni(1978)の配置フラグ
 * 
 * @author suzukiiichiro
 *
 */
...
/**
 * Wirth's validity check(1986)の配置フラグ 各縦横列に１個の王妃を配置する組み合わせを再帰的に列挙
 * 
 * @author suzukiiichiro
 *
 */
...
/**
 * Wirth's validity check(1986)の配置フラグ 各縦横列に１個の王妃を配置する 組み合わせを再帰的に列挙
 * 
 * @author suzukiiichiro
 *
 */
