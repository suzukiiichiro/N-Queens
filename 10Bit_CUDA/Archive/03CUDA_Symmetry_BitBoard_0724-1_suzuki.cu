...
__global__
void BitBoard_cuda_kernel_b1(int size,int mark,unsigned int* totalDown,unsigned int* totalLeft,unsigned int* totalRight,unsigned int* d_results,unsigned int* d_uniq,long totalCond,int h_row,int B1,struct local* l)
{
...
__global__
void BitBoard_cuda_kernel_b2(int size,int mark,unsigned int* totalDown,unsigned int* totalLeft,unsigned int* totalRight,unsigned int* d_results,unsigned int* d_uniq,long totalCond,unsigned int* t_aBoard,int h_row,int B1,int B2,int SM,int LM,int TB,int EB,struct local* l)
{
  const unsigned int mask=(1<<size)-1;
  unsigned long total=0;
  unsigned int unique=0;
...
// GPU -n Ｑが角にない
long BitBoard_backTrack2G(int size,int mask,int row,int n_left,int n_down,int n_right,int steps,unsigned int* aBoard,struct local* l)
{
      ...
      if((bit&mask)!=0){//置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          if(totalCond==steps){
            ...
            //  >>>(size,size-mark,downCuda,leftCuda,rightCuda,resultsCuda,d_uniq,totalCond,d_aBoard,row,l->BOUND1,l->BOUND2,SIDEMASK,LASTMASK);
            BitBoard_cuda_kernel_b2<<<steps/THREAD_NUM,THREAD_NUM
              >>>(size,size-mark,downCuda,leftCuda,rightCuda,resultsCuda,d_uniq,totalCond,d_aBoard,row,l->BOUND1,l->BOUND2,l->SIDEMASK,l->LASTMASK,l->TOPBIT,l->ENDBIT,l);
  ...
  /***12 TOPBIT,ENDBIT追加*********************/
  //cuda_kernel_b2<<<steps/THREAD_NUM,THREAD_NUM
  //  >>>(size,size-mark,downCuda,leftCuda,rightCuda,resultsCuda,d_uniq,totalCond,d_aBoard,mark,l->BOUND1,l->BOUND2,SIDEMASK,LASTMASK);
  BitBoard_cuda_kernel_b2<<<steps/THREAD_NUM,THREAD_NUM
    >>>(size,size-mark,downCuda,leftCuda,rightCuda,resultsCuda,d_uniq,totalCond,d_aBoard,mark,l->BOUND1,l->BOUND2,l->SIDEMASK,l->LASTMASK,l->TOPBIT,l->ENDBIT,l);
  cudaMemcpy(h_results,resultsCuda,
      sizeof(int)*steps/THREAD_NUM,cudaMemcpyDeviceToHost);
  cudaMemcpy(h_uniq,d_uniq,
      sizeof(int)*steps/THREAD_NUM,cudaMemcpyDeviceToHost);
...
// GPU -n Ｑが角にある
long BitBoard_backTrack1G(int size,int mask,int row,int n_left,int n_down,int n_right,int steps,struct local* l)
{
  //何行目からGPUで行くか。ここの設定は変更可能、設定値を多くするほどGPUで並行して動く
      ...
      if((bit&mask)!=0){//置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          if(totalCond==steps){
            ...
            BitBoard_cuda_kernel_b1<<<steps/THREAD_NUM,THREAD_NUM >>>(size,size-mark,downCuda,leftCuda,rightCuda,resultsCuda,d_uniq,totalCond,row,l->BOUND1,l);
  ...
  BitBoard_cuda_kernel_b1<<<steps/THREAD_NUM,THREAD_NUM >>>(size,size-mark,downCuda,leftCuda,rightCuda,resultsCuda,d_uniq,totalCond,mark,l->BOUND1,l);
  cudaMemcpy(h_results,resultsCuda,
      sizeof(int)*steps/THREAD_NUM,cudaMemcpyDeviceToHost);
  cudaMemcpy(h_uniq,d_uniq,
      sizeof(int)*steps/THREAD_NUM,cudaMemcpyDeviceToHost);
...
void symmetry_build_bitBoard(unsigned int size,int steps)
{
  ...
  */
  for(l.BOUND1=2;l.BOUND1<sizeE;l.BOUND1++){
    aBoard[1]=bit=(1<<l.BOUND1);
  ...
  */
  for(l.BOUND1=1,l.BOUND2=sizeE-1;l.BOUND1<l.BOUND2;l.BOUND1++,l.BOUND2--){
    aBoard[0]=bit=(1<<l.BOUND1);
