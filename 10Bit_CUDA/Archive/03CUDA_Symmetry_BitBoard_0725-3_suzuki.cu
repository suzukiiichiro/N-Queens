...
typedef struct local
{
  ...
  unsigned long COUNT2,COUNT4,COUNT8;
...
void BitBoard_cuda_kernel_b1(const unsigned int size,unsigned int mark,unsigned int* _down,unsigned int* _left,unsigned int* _right,unsigned long _cond,unsigned int _row,struct local* l)
{
...
void BitBoard_cuda_kernel_b2(const unsigned int size,unsigned int mark,unsigned int* _down,unsigned int* _left,unsigned int* _right,unsigned long _cond,unsigned int* board,unsigned int _row,struct local* l)
{
  const unsigned int mask=(1<<size)-1;
  unsigned long total=0;
  unsigned long unique=0;
...
void BitBoard_backTrack2G(const unsigned int size,unsigned int row,unsigned int _left,unsigned int _down,unsigned int _right,struct local* l)
{
  ...
  */
        ...
        if(row==mark){
          ...
          if(totalCond==l->STEPS){
            ...
            if(matched){
              ...
              cudaMemcpy(hostLocal,deviceLocal,sizeof(struct local)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
              for(int col=0;col<l->STEPS/THREAD_NUM;col++){
                // total+=hostTotal[col];
                // unique+=hostUnique[col];
                total+=hostLocal[col].TOTAL;
                unique+=hostLocal[col].UNIQUE;
              }
              matched=false;
            }
            // ホストからデバイスへ転送
            ...
            cudaMemcpy(deviceLocal,hostLocal,sizeof(struct local)*l->STEPS/THREAD_NUM,cudaMemcpyHostToDevice);
            // CUDA起動
            BitBoard_cuda_kernel_b2<<<l->STEPS/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,totalCond,deviceBoard,row,deviceLocal);
  ...
  BitBoard_cuda_kernel_b2<<<l->STEPS/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,totalCond,deviceBoard,mark,deviceLocal);
  // デバイスからホストへ転送
  //cudaMemcpy(hostTotal,deviceTotal,sizeof(int)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
  //cudaMemcpy(hostUnique,deviceUnique,sizeof(int)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
  cudaMemcpy(hostLocal,deviceLocal,sizeof(struct local)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
...
void BitBoard_backTrack1G(const unsigned int size,unsigned int row,unsigned int _left,unsigned int _down,unsigned int _right,struct local* l)
{
  ...
  */
      ...
      if((bit&mask)!=0){//置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          if(totalCond==l->STEPS){
            ...
            BitBoard_cuda_kernel_b1<<<l->STEPS/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,totalCond,row,deviceLocal);
            //STEPS数の数だけマルチスレッドで起動するのだが、実際に計算が行われ
            //るのはtotalCondの数だけでそれ以外は空回しになる
            //GPU内でカウントしているので、GPUから出たらmatched=trueになってる
            matched=true;
  ...
  BitBoard_cuda_kernel_b1<<<l->STEPS/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,totalCond,mark,deviceLocal);
  // デバイスからホストへ転送
  //cudaMemcpy(hostTotal,deviceTotal,sizeof(int)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
  //cudaMemcpy(hostUnique,deviceUnique,sizeof(int)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
  cudaMemcpy(hostLocal,deviceLocal,sizeof(struct local)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
