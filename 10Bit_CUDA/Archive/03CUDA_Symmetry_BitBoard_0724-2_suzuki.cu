...
// GPU -n Ｑが角にない
long BitBoard_backTrack2G(unsigned int size,unsigned int row,unsigned int _left,unsigned int _down,unsigned int _right,struct local* l)
{
      ...
      if((bit&mask)!=0){//置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          if(totalCond==l->steps){
            ...
            BitBoard_cuda_kernel_b2<<<l->steps/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,deviceTotal,deviceUnique,totalCond,deviceBoard,row,l->BOUND1,l->BOUND2,l->SIDEMASK,l->LASTMASK,l->TOPBIT,l->ENDBIT,l);
  ...
  BitBoard_cuda_kernel_b2<<<l->steps/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,deviceTotal,deviceUnique,totalCond,deviceBoard,mark,l->BOUND1,l->BOUND2,l->SIDEMASK,l->LASTMASK,l->TOPBIT,l->ENDBIT,l);
  cudaMemcpy(hostTotal,deviceTotal,sizeof(int)*l->steps/THREAD_NUM,cudaMemcpyDeviceToHost);
  cudaMemcpy(hostUnique,deviceUnique,sizeof(int)*l->steps/THREAD_NUM,cudaMemcpyDeviceToHost);
  for(int col=0;col<l->steps/THREAD_NUM;col++){
    total+=hostTotal[col];
...
// GPU -n Ｑが角にある
long BitBoard_backTrack1G(unsigned int size,unsigned int row,unsigned int _left,unsigned int _down,unsigned int _right,struct local* l)
{
  //何行目からGPUで行くか。ここの設定は変更可能、設定値を多くするほどGPUで並行して動く
      ...
      if((bit&mask)!=0){//置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          if(totalCond==l->steps){
            ...
            cudaMemcpy(deviceLeft,hostLeft,sizeof(int)*totalCond,cudaMemcpyHostToDevice);
            cudaMemcpy(deviceRight,hostRight,sizeof(int)*totalCond,cudaMemcpyHostToDevice);
            BitBoard_cuda_kernel_b1<<<l->steps/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,deviceTotal,deviceUnique,totalCond,row,l->BOUND1,l);
            //steps数の数だけマルチスレッドで起動するのだが、実際に計算が行われ
            //るのはtotalCondの数だけでそれ以外は空回しになる
            //GPU内でカウントしているので、GPUから出たらmatched=trueになってる
            matched=true;
  ...
  BitBoard_cuda_kernel_b1<<<l->steps/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,deviceTotal,deviceUnique,totalCond,mark,l->BOUND1,l);
  cudaMemcpy(hostTotal,deviceTotal,sizeof(int)*l->steps/THREAD_NUM,cudaMemcpyDeviceToHost);
  cudaMemcpy(hostUnique,deviceUnique,sizeof(int)*l->steps/THREAD_NUM,cudaMemcpyDeviceToHost);
  for(int col=0;col<l->steps/THREAD_NUM;col++){
    total+=hostTotal[col];
...
void symmetry_build_bitBoard(unsigned int size,int steps)
{
  ...
  */
  for(l.BOUND1=2;l.BOUND1<size-1;l.BOUND1++){
    ...
    total+=BitBoard_backTrack1G(size,2,(left|bit)<<1,(down|bit),(right|bit)>>1,&l);
  }
  l.TOPBIT=1<<(size-1);
  ...
  */
  for(l.BOUND1=1,l.BOUND2=size-1-1;l.BOUND1<l.BOUND2;l.BOUND1++,l.BOUND2--){
    ...
    total+=BitBoard_backTrack2G(size,1,bit<<1,bit,bit>>1,&l);
