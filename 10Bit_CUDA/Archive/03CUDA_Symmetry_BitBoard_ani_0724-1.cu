...
__global__
void BitBoard_cuda_kernel_b1(unsigned int size,unsigned int mark,unsigned int* _down,unsigned int* _left,unsigned int* _right,unsigned int* _total,unsigned int* _unique,unsigned long _cond,unsigned int _row,unsigned int BOUND1)
{
...
long BitBoard_backTrack2G(unsigned int size,unsigned int row,unsigned int _left,unsigned int _down,unsigned int _right,struct local* l)
{
  ...
  local* hostL;
  cudaMallocHost((void**) &hostL,sizeof(local));
  ...
  local* deviceL=NULL;
  cudaMalloc((void**) &deviceL,sizeof(local));
      ...
      if((bit&mask)!=0){//置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          if(totalCond==l->steps){
            ...
            hostL=l;
            cudaMemcpy(deviceL,hostL,sizeof(local),cudaMemcpyHostToDevice);
            ...
            BitBoard_cuda_kernel_b2<<<l->steps/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceDown,deviceLeft,deviceRight,deviceTotal,deviceUnique,totalCond,deviceBoard,row,l->BOUND1,l->BOUND2,l->SIDEMASK,l->LASTMASK,l->TOPBIT,l->ENDBIT,deviceL);
