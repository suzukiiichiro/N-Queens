...
void buildChain()
{
  ...
  pthread_t pt[STEPS];
  if(THREAD){
    for(unsigned int w=0;w<STEPS;++w){
      initLocal(&l[w]); //初期化
...
// memcpy(&l->B,&l->wB,sizeof(Board));       // B=wB;
  ...
  for(l->n=l->w;l->n<(g.size-2)*(g.size-1)-l->w;++l->n){
    ...
    if(THREAD){ // pthreadのスレッドの生成
      ...
      l[stepcnt].s=l->s;
      l[stepcnt].w=l->w;
      l[stepcnt].dimx=l->dimx;
      l[stepcnt].dimy=l->dimy;
      l[stepcnt].B.row=l->B.row;
      l[stepcnt].B.down=l->B.down;
      l[stepcnt].B.left=l->B.left;
      l[stepcnt].B.right=l->B.right;
      for(unsigned int i=0;i<g.size;++i){ l[stepcnt].B.x[i]=l->B.x[i]; }
      
      int iFbRet;
      iFbRet=pthread_create(&pt[stepcnt],NULL,&thread_run,&l[stepcnt]);
      if(iFbRet>0){
