...
$ nvcc -O3 -arch=sm_61 03CUDA_Symmetry.cu && ./a.out -n
対称解除法 GPUノードレイヤー
...
__host__ 
long GPU_symmetry_R(unsigned int size,struct local* hostLocal)
{
  struct dlocal l; // ＧＰＵ実行時に使う構造体
  l.TOTAL=l.UNIQUE=l.COUNT2=l.COUNT4=l.COUNT8=0;
  unsigned int bit=0;
  l.TOPBIT=1<<(size-1);
  l.ENDBIT=l.LASTMASK=l.SIDEMASK=0;
...
long GPU_symmetry_solve_nodeLayer(unsigned int size,unsigned long left,unsigned long down,unsigned long right,struct dlocal* l)
{
  ...
  if(row==(size-1)){
    if(bitmap){
      if( (bitmap& l->LASTMASK)==0){
        l->board[row]=bitmap;  //Qを配置
        return GPU_symmetryOps(size,l);    //対称解除;
      }
    }
  }else{
    if(row<l->BOUND1){
...
void dim_nodeLayer(unsigned int size,long* nodes,long* solutions,unsigned int numElements,struct local* l)
{
  ...
  if(i<numElements){
    if(l[0].TYPE==0){
      solutions[i]=GPU_symmetry_solve_nodeLayer_corner(size,nodes[3*i],nodes[3*i+1],nodes[3*i+2],&gdl[i]);
    }else{
      solutions[i]=GPU_symmetry_solve_nodeLayer(size,nodes[3*i],nodes[3*i+1],nodes[3*i+2],&gdl[i]);
    }
  }
}
