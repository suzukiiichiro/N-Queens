...
typedef struct{
  ...
  long unique;
  long total;
  int canonical;   // 1: このstartが基底代表
  int factor;      // 1,2,4,8 のいずれか
  int pad;         // アライン用
}Constellation;
/**
  IntHashSet構造体の定義
...
/**
  ...
  */
bool is_canonical_ijkl(uint32_t x, int N){
  uint32_t m = x, t = x;
  // 回転4通り
  for (int r=0;r<3;++r){ t=rot90(t,N); if (t<m) m=t; }
  // ミラー＋回転4通り
  t = symmetry90(x,N); if (t<m) m=t;
  for (int r=0;r<3;++r){ t=rot90(t,N); if (t<m) m=t; }
  return x == m;
}
/**
  */
int symmetry_factor(uint32_t x, int N){
  // 8個の像の重複数から決定（簡易例：厳密版はあなたの symmetry() を使用）
  // ここでは既存の symmetry(ijkl,N) をそのまま呼ぶのを推奨
//  extern int symmetry(uint32_t, int);   // 1,2,4 の返りもあるなら 8/その値 などに合わせる
  return symmetry(x, N);       // 既存実装を採用
}
/**
...
/**
  ...
  */
void finalize_constellations(ConstellationArrayList* A, int currentSize, unsigned counter, uint32_t ijkl, int N)
{
    for (unsigned a = 0; a < counter; ++a) {
        Constellation* dst = &A->data[currentSize - a - 1];

        // 下位20bitに ijkl を (start は既に上位に入っている前提)
        dst->startijkl |= ijkl;

        // Unique 用：基底代表のみ 1
        // jasmin が __host__ でも使えるならこちら、難しければ is_canonical_ijkl(...) を使う
        dst->canonical = (jasmin(ijkl, N) == ijkl) ? 1 : 0;

        // Total 用：対称係数（あなたの symmetry() と同じ定義に合わせる）
        dst->factor = (uint8_t) symmetry(ijkl, N);

        // 念のための初期化安全策（なくてもOK）
        if (dst->factor == 0) dst->factor = 1;
    }
}
/**
    ...
    if(idx<totalSize)
    {
      ...
      if(start==69){
        ...
        */
        ...
        /**
        //if(constellation->canonical) unique=cnt;
        unique=constellation->canonical ? cnt : 0;
        //cnt*=symmetry(ijkl,N);
        //total=cnt*symmetry(ijkl,N);
        //total=cnt*(unsigned)constellation->factor;
        // ★total は常に factor を掛ける（factor==0 の保険付き）
        unsigned int f = (unsigned)constellation->factor;
        if (f == 0) {
          // デバッグ保険：本番では 0 にならないはず
          const unsigned ijkl = (unsigned)constellation->startijkl & ((1u<<20)-1u);
          f = symmetry(ijkl, N); // もしくは 1
        }
        total = cnt * f;
        */
        /**
        // ---- Unique（canonical フォールバック付き）----
        unsigned canon = (unsigned)constellation->canonical;
        if (!canon) {
            const unsigned ijkl = (unsigned)constellation->startijkl & ((1u<<20) - 1u);
            canon = (jasmin(ijkl, N) == ijkl);  // 最小像判定（D8）
        }
        unique = canon ? cnt : 0;
        // ---- Total（あなたの現在の保険付きのままでOK）----
        unsigned int f = (unsigned)constellation->factor;
        if (!f) {
            const unsigned ijkl = (unsigned)constellation->startijkl & ((1u<<20) - 1u);
            f = symmetry(ijkl, N);  // デバッグ保険
        }
        total= cnt * f;
        */
        // ---- Total ----
...
if (!f) {
    const unsigned ijkl = (unsigned)constellation->startijkl & ((1u<<20) - 1u);
    f = symmetry(ijkl, N);  // フォールバック（デバッグ時）
}
total = cnt * f;

// ---- Unique（重み付き法）----
// 8/f は 1,2,4,8 のいずれか（f が 8/4/2/1）
unsigned int w8 = 8u / f;           // 例: f=8→1, f=4→2, f=2→4
unique = cnt * w8;    // “Unique×8” の素片
      }
    }
  ...
  */
  ...
  for(unsigned int s=0;s<ijklList->size;s++){
    ...
    */
    ...
    // ↓ ループ外で一度 pack してから “まとめ付け”
    currentSize = constellations->size;
    const uint32_t ijkl = toijkl(i, j, k, l);
    for (unsigned a = 0; a < counter; ++a) {
        Constellation* dst = &constellations->data[currentSize - a - 1];
        dst->startijkl |= ijkl;
        dst->canonical  = (jasmin(ijkl, N) == ijkl) ? 1 : 0;   // あるいは is_canonical_ijkl(…)
        dst->factor     = (uint8_t) symmetry(ijkl, N);
    }
    finalize_constellations(constellations, currentSize, counter, ijkl, N);
    // jklとsymとstartはすべてのサブコンステレーションで同じである
    for(unsigned int a=0;a<counter;a++){
      Constellation* dst = &constellations->data[currentSize - a - 1];
      // 下位20bitに ijkl を足す（既存動作）
      dst->startijkl |= ijkl;
      // Unique 用：この start 盤が “基底代表” なら 1
      dst->canonical = (jasmin(ijkl, N) == ijkl) ? 1 : 0;
      // Total 用：対称係数（既存の macro でOK：2 / 4 / 8）
      dst->factor = (uint8_t) symmetry(ijkl, N);
    }
  }
}
