...
/**
  ...
  一番右端(右上の角)にクイーンがある場合を考えてみます。他の３つの角にクイーンを置くことはできないので、ユニーク解であるかどうかを判定するには、右上角から左下角を通る斜軸で反転させたパターンとの比較だけになります。突き詰めれば、上から２行目のクイーンの位置が右から何番目にあるかと、右から２列目のクイーンの位置が上から何番目にあるかを比較するだけで判定することができます。この２つの値が同じになることはないからです。
結局、再帰探索中において y<BOUND1の時に右から２列目に枝狩りを入れておけばユニーク解になる。  
                     BOUND1(2)
	  0 1 2 3 4 5 6 7
	0 - - - - - - - Q  
	1 - - - - - Q - -  
