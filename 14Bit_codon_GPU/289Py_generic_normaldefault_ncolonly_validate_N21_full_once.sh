...
# =============================================================================
# 289 N=21 full validation harness
# Parent: 287 adopt286-normaldefault-nldnrd. Experiment: ncol-only early in MAXD14 generic loop; nf remains after branch.
...
SRC=${SRC:-./289Py_generic_normaldefault_ncolonly_probe.py}
CAND=${CAND:-./289Py_generic_normaldefault_ncolonly_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/289Py_generic_normaldefault_ncolonly_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/289Py_generic_normaldefault_ncolonly_logs_N21_full_once_${TS}"
...
echo "[start] 289 generic-normaldefault-ncolonly validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 289 generic-normaldefault-ncolonly SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '289 generic-normaldefault-ncolonly' "$SRC"; then
  printf 'source_version_tag	289 generic-normaldefault-ncolonly	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	289 generic-normaldefault-ncolonly	missing	FAIL
...
old_split_markers = ['split288', 'split287', 'split286', 'split285', 'split284', 'split282', 'split281', 'split280', 'split278', 'split277', 'split275', 'split273', 'split270_', 'split240', 'split239_', 'kernel-blockdiag', 'block_count_diag', 'rootaction0-direct', 'ncol_early']
old_present = [x for x in old_split_markers if x in s]
if 'split289' in s and not old_present:
    checks.append(('source_split_tag', 'split289 active; rejected runtime tags absent', 'split289', True))
else:
    checks.append(('source_split_tag', 'split289 active; rejected runtime tags absent', 'present=' + ','.join(old_present) if old_present else 'split287 missing', False))
...
# Check MAXD14 generic DFS loop normal-default nld/nrd shape. Keep root-preroll branch untouched.
...
if not m14:
    ...
    expected_snippet = '''nld:u32=(cur_ld|bit)<<u32(1)
      ...
      if block_code!=u32(0):
        stepu:u32='''
    bad_snippet = '''nld:u32=u32(0)
      nrd:u32=u32(0)
      if block_code==u32(0):'''
    ok = expected_snippet in tail and bad_snippet not in tail
    checks.append(('source_generic_normaldefault', 'MAXD14 generic normal-default nld/nrd', 'present' if ok else 'missing/old-form', ok))

    ncol_before = '''nld:u32=(cur_ld|bit)<<u32(1)
      nrd:u32=(cur_rd|bit)>>u32(1)
      ncol:u32=cur_col|bit
      if block_code!=u32(0):''' in tail
    nf_default_bad = '''nf:u32=bm&~(nld|nrd|ncol)
      if block_code!=u32(0):''' in tail
    checks.append(('source_ncolonly_early', 'ncol before special branch; nf after branch', 'present' if (ncol_before and not nf_default_bad) else 'missing/old-form-or-nfdefault', ncol_before and not nf_default_bad))
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 289 generic-normaldefault-ncolonly mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "288rejected:$BASELINE_288_REJECT_SECONDS" "287adopt:$BASELINE_287_ADOPT_SECONDS" "286adopt:$BASELINE_286_ADOPT_SECONDS" "285restore:$BASELINE_285_RESTORE_SECONDS" "284rejected:$BASELINE_284_REJECT_SECONDS" "283normalfirst:$BASELINE_283_NORMALFIRST_SECONDS" "276current:427.672" "271fastest:427.705" "239:427.703" "217:427.709"; do
...
echo "[validation-ok] 289 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
