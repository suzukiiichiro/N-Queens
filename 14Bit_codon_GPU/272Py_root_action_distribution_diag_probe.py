...
# =============================================================================
# NQ_UPDATE_MEMO
# 272: root-action-distribution diagnostic.
#      271Py final-fastest-complete を親に、CUDA MAXD14/16/18/20/21 kernel本文、
#      rootrestlate、futuremask、no-sibling、split145/chunkshape148、broadmarktail、
#      cache生成、worker split、1 chunk 1 launch は変更しない。
#      速度候補ではなく、次の専用kernel候補を決めるため、host側で
#      root_action 0/1/2/3、root0 chunk、fallback chunk、対象task数を
#      progress TSV と [root-action-*] ログへ出す軽量診断版。
#      270 rootpre2flag、268/269 root0細分化、240 fid14別launchは入れない。
...
"""
272 root-action-distribution note: 271 final-fastest-completeを親に、CUDA kernel本文とGPU本線機能は変更しない。root_action 0/1/2/3 と root0/fallback chunk/task分布だけをhost側で診断出力する。


272一式を作成しました。今回は **271を親にしたroot_action分布診断版**です。CUDA kernel本文と1 chunk 1 launchは維持し、host側で `root_action 0/1/2/3`、root0/fallback chunk数・task数をprogress TSVとログへ出す形にしています。

`STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [272Py_root_action_distribution_diag_probe.py](sandbox:/mnt/data/272Py_root_action_distribution_diag_probe.py)
* [272Py_root_action_distribution_diag_validate_N21_full_once.sh](sandbox:/mnt/data/272Py_root_action_distribution_diag_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [272Py_root_action_distribution_diag_sha256.txt](sandbox:/mnt/data/272Py_root_action_distribution_diag_sha256.txt)
* [272Py_root_action_distribution_diag_bundle_20260709.tar](sandbox:/mnt/data/272Py_root_action_distribution_diag_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./272Py_root_action_distribution_diag_probe

STATIC_ONLY=1 bash 272Py_root_action_distribution_diag_validate_N21_full_once.sh

bash 272Py_root_action_distribution_diag_validate_N21_full_once.sh
```

確認したい追加出力は主にこの2つです。

```bash
grep -E "root-action-diag|root-action-summary" \
  272Py_root_action_distribution_diag_logs_N21_full_once_*/full_once.log
```

summaryには、通常の正当性検証に加えて以下が出るようにしています。

```text
progress_root_action_columns
progress_root_action_task_sum
progress_root_action_chunk_sum
root_action_summary
root_action_distribution
```

次の273候補は、272の `[root-action-summary]` を見てから決めるのが安全です。
...
VERSION_TAG:str="272 root-action-distribution-diag: parent 271 final-fastest-complete; CUDA kernels and 1 chunk 1 launch remain unchanged; host computes root_action 0/1/2/3 distribution per chunk and writes progress/log diagnostics for root0/fallback candidate planning; generic N5..27; bare -g split145 mode31; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
def root_action_for_task(ctrl0:u32,markctrl:u32,meta_next:List[u8])->int:
  # 272 diagnostic: return only the depth-0 action generated by the
  # existing schedule interpreter. 0=normal, 1=jmark, 2=terminal, 3=base14.
  IS_BASE_MASK_I:int=69222408
  IS_JMARK_MASK_I:int=4
  IS_P5_MASK_I:int=3840
...
def stream_measure2_progress_header()->str:
  ...
  while fid<28:
    ...
    fid+=1
  h+="\troot_action0_count\troot_action1_count\troot_action2_count\troot_action3_count"
  h+="\troot_action_nonzero_count\troot0_kernel_chunk\tfallback_chunk"
  h+="\troot0_task_count\tfallback_task_count"
  h+="\n"
  return h
...
def stream_measure2_stats_suffix(stats:List[int],m:int)->str:
  ...
  while fid<28:
    s+=f"\t{stats[18+fid]}"
    fid+=1
  s+=f"\t{stats[46]}\t{stats[47]}\t{stats[48]}\t{stats[49]}"
  s+=f"\t{stats[50]}\t{stats[51]}\t{stats[52]}"
  s+=f"\t{stats[53]}\t{stats[54]}"
  return s
...
def analyze_stream_chunk_input_stats(N:int,chunk_constellations:List[Dict[str,int]])->List[int]:
  ...
  meta_next_diag:List[u8]=[u8(1),u8(2),u8(3),u8(3),u8(2),u8(6),u8(2),u8(2),u8(0),u8(4),u8(5),u8(7),u8(13),u8(14),u8(14),u8(14),u8(17),u8(14),u8(14),u8(20),u8(21),u8(21),u8(21),u8(25),u8(21),u8(21),u8(26),u8(26)]
  ...
  while i<m:
    ...
    root_action:int=root_action_for_task(soa.ctrl0_arr[i],soa.markctrl_arr[i],meta_next_diag)
    if root_action==0:
      stats[46]+=1
    elif root_action==1:
      stats[47]+=1
  ...
  stats[50]=stats[47]+stats[48]+stats[49]
  stats[51]=1 if stats[50]==0 else 0
  stats[52]=1 if stats[50]!=0 else 0
  stats[53]=stats[46]
  stats[54]=stats[50]
  return stats
...
CHUNKSHAPE148_DEFAULT_REASON:str="272 diagnostic keeps validated 271/241/239/238/231/217 scorestripe_v9 task order/cache and CUDA kernels unchanged; adds host root_action distribution only"
...
def exec_solutions_gpu_bin_stream_split145(
  ...
  gpu_total:int=0
  root_action0_total:int=0
  root_action1_total:int=0
  root_action2_total:int=0
  root_action3_total:int=0
  root_action_nonzero_total:int=0
  root0_kernel_chunks:int=0
  fallback_chunks:int=0
  root0_task_total:int=0
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      root_action0_total+=stats[46]
      root_action1_total+=stats[47]
      root_action2_total+=stats[48]
      root_action3_total+=stats[49]
      root_action_nonzero_total+=stats[50]
      root0_kernel_chunks+=stats[51]
      fallback_chunks+=stats[52]
      root0_task_total+=stats[53]
      fallback_task_total+=stats[54]
      if gpu_log_level>=1:
        print(f"[root-action-diag] N={N} chunk={chunk_index} off={off} m={m} root0_kernel_chunk={stats[51]} fallback_chunk={stats[52]} root_action0={stats[46]} root_action1={stats[47]} root_action2={stats[48]} root_action3={stats[49]} nonzero={stats[50]} root0_tasks={stats[53]} fallback_tasks={stats[54]}")
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
  ...
  if gpu_log_level>=1:
    ...
    print(f"[root-action-summary] N={N} chunks={chunk_index} executed_chunks={executed_chunks} root0_kernel_chunks={root0_kernel_chunks} fallback_chunks={fallback_chunks} root_action0={root_action0_total} root_action1={root_action1_total} root_action2={root_action2_total} root_action3={root_action3_total} nonzero={root_action_nonzero_total} root0_tasks={root0_task_total} fallback_tasks={fallback_task_total}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 272 root-action diagnostic; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split272_root_action_diag_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split272_root_action_diag_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split272-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split272-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split272_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
