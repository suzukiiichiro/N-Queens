...
# =============================================================================
# NQ_UPDATE_MEMO
# 217: 更新メモ: 216Py rootprblockdirect は正当性OKだが204最速を上回らなかったため不採用。217Pyは211Pyのroot_first遅延形を親に、MAXD14 kernelのみroot-preroll内のcur_avail=root_rest代入をfirst-child検証後へ遅延する最後のroot-preroll微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 219 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 219 keeps the
# 217 rootrestlate fastest baseline because 218 rootrestlate_andnot was correct
# but did not beat 217. 219 is the second 217-neighborhood recombination: it
# keeps root_after_second XOR and changes only the root-preroll block-code
# extraction from pr_nibble_op&7 to schedule_lo&7.
#
# Kernel arithmetic keeps 217/204/197/196/194/192/188 bitboard/schedule semantics:
...
# and wider roots remain generic. Host task order, dispatch, cache, progress
# verification, and README-managed history remain unchanged.
#
# Shell policy:
#   - STATIC_ONLY=1 does not take the full-run lock.
#   - source static-check failure prints the summary before exiting.
#   - rootrestlate_prblockdirect source check is whitespace/comment robust.
SRC=${SRC:-./219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
BASELINE_216_SECONDS=${BASELINE_216_SECONDS:-427.867}
BASELINE_215_SECONDS=${BASELINE_215_SECONDS:-428.061}
BASELINE_214_SECONDS=${BASELINE_214_SECONDS:-429.104}
BASELINE_213_SECONDS=${BASELINE_213_SECONDS:-428.120}
BASELINE_212_SECONDS=${BASELINE_212_SECONDS:-427.828}
BASELINE_211_SECONDS=${BASELINE_211_SECONDS:-427.801}
BASELINE_210_SECONDS=${BASELINE_210_SECONDS:-427.874}
BASELINE_209_SECONDS=${BASELINE_209_SECONDS:-428.003}
BASELINE_208_SECONDS=${BASELINE_208_SECONDS:-429.235}
BASELINE_207_SECONDS=${BASELINE_207_SECONDS:-428.800}
BASELINE_206_SECONDS=${BASELINE_206_SECONDS:-427.908}
BASELINE_205_SECONDS=${BASELINE_205_SECONDS:-428.074}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 219 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/219Py_kernel_roottwo_rootrestlate_prblockdirect_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 219Py source
#   has a harmless header/string/layout difference. This 219 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 217 rootrestlate futuremask/no-sibling MAXD14 dispatch; 217 is new fastest over 204; 218 rootrestlate_andnot was correct but slower than 217; 219 tests 217 plus pr_block_code=schedule_lo&7 direct extraction; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second XOR + rootrestlate + pr_block_code schedule_lo direct	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
marker='# 219 rootrestlate:'
root_pos=s.find(marker)
if root_pos < 0:
    root_pos=s.find('root_rest:u32=cur_avail&(cur_avail-u32(1))')
gen_pos=s.find('    while True:', root_pos)
root_block=s[root_pos:gen_pos if gen_pos >= 0 else len(s)]
# Strip comments before shape-position checks so prose such as cur_avail=root_rest
# in the explanatory block cannot satisfy or break active-code checks.
root_code='\n'.join(line for line in root_block.splitlines() if not line.lstrip().startswith('#'))
root_compact=''.join(root_code.split())
...
if_pos=root_compact.find('ifroot_after_second==u32(0):')
rf_pos=root_compact.find('root_first:u32=cur_avail&(u32(0)-cur_avail)')
pre_if=root_compact[:if_pos if if_pos >= 0 else 0]
checks = {
    'root_clearrest_present': 'root_rest:u32=cur_avail&(cur_avail-u32(1))' in root_compact,
    'root_xor_rest_removed': 'root_rest:u32=cur_avail^root_first' not in root_compact,
    'root_after_xor_present': 'root_after_second:u32=root_rest^root_second' in root_compact,
    'root_after_andnot_absent': 'root_after_second:u32=root_rest&(~root_second)' not in root_compact,
    'pr_nibble_op_present': 'pr_nibble_op:u32=schedule_lo&u32(15)' in root_compact,
    'pr_block_direct_present': 'pr_block_code:u32=schedule_lo&u32(7)' in root_compact,
    'pr_block_from_nibble_absent': 'pr_block_code:u32=pr_nibble_op&u32(7)' not in root_compact,
    'root_after_if_present': if_pos >= 0,
    'root_first_late_present': rf_pos > if_pos >= 0,
    'root_first_before_predicate_absent': 'root_first:u32=' not in pre_if,
    'root_rest_assignment_present': 'cur_avail=root_rest' in root_compact,
    'root_rest_assignment_after_jmark': root_compact.find('cur_avail=root_rest') > root_compact.find('pr_child_jmark:u32=child_jmark_mask&u32(1)') >= 0,
    'root_rest_assignment_before_nld_absent': not (0 <= root_compact.find('cur_avail=root_rest') < root_compact.find('pr_nld:u32=u32(0)')),
    'root_inline_if_absent': 'if(root_rest^root_second)==u32(0):' not in root_compact,
    'root_preroll_flag_absent': 'root_preroll:u32' not in s,
    # 211 keeps 204/197 save payload; 203 depth0-save is not adopted.
    'root_depth_pack_present': 'avail[save_sp]=cur_avail|(u32(cur_depth)<<u32(27))' in root_compact,
    'generic_depth_pack_present': 'avail[save_sp]=cur_avail|(u32(cur_depth)<<u32(27))' in generic_compact,
    # 211 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 211 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    'pr_child_jmark_scalar_present': 'pr_child_jmark:u32=child_jmark_mask&u32(1)' in root_compact,
    'direct_child_jmark_bit0_absent': 'if(child_jmark_mask&u32(1))!=u32(0):' not in compact,
    # 211 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
    'pr_future_mask_guard_present': 'ifpr_descend!=u32(0):iffuture_check_mask!=u32(0):if(pr_nibble_op&u32(8))!=u32(0):' in root_compact,
...
if all(checks.values()):
    sys.exit(0)
print('source_rootrestlate_prblockdirect_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootrestlate_prblockdirect\tpresent_rootrestlate_prblockdirect\tpresent_rootrestlate_prblockdirect\tOK
' >> "$SUMMARY"
else
  printf 'source_rootrestlate_prblockdirect\tpresent_rootrestlate_prblockdirect\tmissing_or_old_rootrestlate_prblockdirect_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split219'}
bad=sorted(tags-allowed)
if 'split219' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split219 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split219 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 219 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 219 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootrestlate_prblockdirect static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 210 roottwo-rootrestlate futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 210 roottwo-rootrestlate futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
