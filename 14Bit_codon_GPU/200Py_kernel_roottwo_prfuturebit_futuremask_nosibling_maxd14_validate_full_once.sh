...
# =============================================================================
# NQ_UPDATE_MEMO
# 200: 更新メモ: 199は正当性OKだが197比0.091秒遅いため不採用。197 root<=2 direct-if prerollを親に戻し、MAXD14 kernelのみroot-preroll内future-prune判定から冗長なfuture_check_mask外側guardを外す微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 200 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 200 starts from
# validated 197 because both 198 and 199 were correct but did not beat the
...
# schedule, no-sibling spill elision, future_check_mask zero guard in the
# generic DFS loop, schedule_words=0, and 208 local stack bytes/thread. The
# experiment is deliberately narrow: if post-root-action root_avail contains
# one or two bits, process the first depth-0 candidate once before the generic
# DFS loop. 3-bit and wider roots remain generic. The root<=2 predicate is
# restored to the 197 second-lowbit direct-if form; only the root-preroll
# future-prune check tests pr_nibble_op bit8 directly instead of first checking
# aggregate future_check_mask. Host task order, dispatch, cache, progress
# verification, and README-managed history remain unchanged.
SRC=${SRC:-./200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 200 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 200Py source
#   has a harmless header/string/layout difference. This 200 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 197 root one/two-candidate direct-if futuremask/no-sibling MAXD14 dispatch; 198 tailtest and 199 eqsecond correct but not adopted; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root-preroll future-bit direct test	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
ok=True
ok = ok and ('rootpr_futurebit' in s)
ok = ok and ('root_after_second:u32=root_rest^root_second' in compact)
ok = ok and ('ifroot_after_second==u32(0):' in compact)
ok = ok and ('if(pr_nibble_op&u32(8))!=u32(0):' in compact)
# The old 197 root-preroll nested guard must be gone, while the generic DFS
# future_check_mask guard may and should remain.
ok = ok and ('ifpr_descend!=u32(0):iffuture_check_mask!=u32(0):if(pr_nibble_op&u32(8))!=u32(0):' not in compact)
ok = ok and ('root_rest==root_second' not in s)
ok = ok and ('root_rest_more:u32' not in s)
ok = ok and ('root_preroll:u32' not in s)
sys.exit(0 if ok else 1)
PYCHECK
then
  printf 'source_rootpr_futurebit	present_direct_pr_nibble_bit8	present_direct_pr_nibble_bit8	OK
' >> "$SUMMARY"
else
  printf 'source_rootpr_futurebit	present_direct_pr_nibble_bit8	missing_or_old_preroll_future_guard	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split200'}
bad=sorted(tags-allowed)
if 'split200' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split200 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split200 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 200 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source roottwo-prfuturebit static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 200 roottwo-prfuturebit futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 200 keeps the validated 197 host/chunk structure, then derives chunk-tail diagnostics from
...
# 200: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 200 roottwo-prfuturebit futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
