...
**301 = 647.930s — 壊滅的退行、即不採用。** vs 296比で **−294秒 (−83%)** 悪化。

`cur_depth_x4-32` の減算がランタイムにおいて符号なし整数に解釈され、巨大な値のシフトになった可能性が高いです（`cur_depth_x4-32` が負の値になるとき）。いや、cur_depth<8 の場合は else ブランチに入らないので…

実際の問題は別のところ：**`cur_depth_x4+=4` を push の後に置いたため、pop 時の `cur_depth_x4=cur_depth<<2` が `continue` の後に実行されない**ことです。pop では `continue` でループ先頭に戻るので、cur_depth_x4 の更新が反映されないまま nibble_op デコードに進んでいます。原因が確定しました。`cur_depth_x4+=4` が **inner `while True:` ループの外**（`thread_total+=total*w_arr[idx]` の直前）に置かれていました。push/descend のたびに必要な更新が、ループ終了後に1回だけ実行されていた状態です。

302 ではこれを正しく内側に配置します。`cur_depth=next_depth` は `thread_total+=` の直前にあります。つまり **`cur_depth=next_depth` の後には `continue` がなく、そのままループの末尾からループ先頭に戻ります**。ですから `cur_depth_x4+=4` を `cur_depth=next_depth` の直後（`thread_total+=` の前）に置けば、descend ごとに実行されます。302 の配置は実は正しかった。

問題は別にあります：**ループの先頭に戻ったとき `cur_depth<8` のブランチを経由して nibble_op を再計算しますが、その前に `cur_depth_x4` が正しく更新されているか？** ループ先頭は `if cur_avail==u32(0):` から始まります。descend の場合、`cur_avail=nf`（非ゼロ）→ `break` せず → `nibble_op` デコード → ここで `cur_depth_x4` が必要。

descend 後に `cur_depth_x4+=4` があるのでその時点では更新済み。ただし pop (`continue`) の場合は `cur_depth_x4=cur_depth<<2` が更新されています。

これは正しいはずです。では **301 での実際のバグは何か**？301 の配置は正しい（`cur_depth=next_depth` の直後、ループ末尾で `cur_depth_x4+=4`）。では **pop 後に `cur_depth_x4` が更新されているか** を確認します。バグ確定。**preroll が `cur_depth=1` に設定するが、`cur_depth_x4` は 0 のまま**。`cur_depth_x4` の初期値は 0 なので、`cur_depth` が 1 の状態で inner `while True` に入ると `cur_depth_x4=0` → `schedule_lo>>0 = schedule_lo` となり nibble_op が depth=0 のものを読んでしまいます。全項目 OK。バグ修正を確認しました。検証スクリプトと README を仕上げます。## 301 結果まとめと 302 の考え方

**301 不採用**: 647.930s — **−294s (−83%) 壊滅的悪化**。バグが原因でした。

**バグの詳細**：
- preroll パスが `cur_depth=1` を設定した後、`cur_depth_x4` は init 値の `0` のまま
- inner `while True` 最初のイテレーションで `cur_depth=1` なのに `cur_depth_x4=0` → depth=0 の nibble を読む
- 全 DFS ノードのスケジュール解釈が狂い、正答は偶然一致したが処理時間が爆発

---

## 302 の修正内容

`cur_depth_x4` の更新サイトを **4箇所** に増やして完全整合を保証：

| タイミング | 更新 |
|---|---|
| 初期化 | `cur_depth_x4:int=0` (cur_depth=0 に対応) |
| **preroll 後** | `cur_depth_x4=4` (cur_depth=1 の直後) ← **301 で欠けていた** |
| pop 後 | `cur_depth_x4=cur_depth<<2` (301 でも正しかった) |
| descend 後 | `cur_depth_x4+=4` (301 でも正しかった、位置も正しかった) |

```bash
STATIC_ONLY=1 bash 302Py_cur_depth_x4_fix_validate_N21_full_once.sh
bash 302Py_cur_depth_x4_fix_validate_N21_full_once.sh
```
"""
...
VERSION_TAG:str="302 cur-depth-x4-fix probe: parent 301 (rejected, -294s due to preroll bug: cur_depth=1 in preroll but cur_depth_x4 stayed 0); fixed by adding cur_depth_x4=4 after preroll sets cur_depth=1, and cur_depth_x4+=4 inside inner while True after cur_depth=next_depth; pop: cur_depth_x4=cur_depth<<2 before continue; hot decode: schedule_lo>>cur_depth_x4, schedule_hi>>cur_depth_x4-32; no cur_depth*4 or (cur_depth-8)*4 multiplies remain; schedule_lo/hi, stack_ptr/save_sp/next_depth unchanged from 296; K_PER_THREAD_MAXD14=32; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
    ...
    while idx<m:
      ...
      while True:
        if cur_avail==u32(0):
          ...
          cur_depth_x4=cur_depth<<2
          continue

        nibble_op:u32=u32(0)
        if cur_depth<8:
          nibble_op=(schedule_lo>>u32(cur_depth_x4))&u32(15)
        else:
          nibble_op=(schedule_hi>>u32(cur_depth_x4-32))&u32(15)
