...
# =============================================================================
# 347 N=21 full validation harness (serpentine)
#
# 347 IS A SINGLE-VARIABLE MEASUREMENT REVISION on top of the adopted 346.
# Kernels, dispatcher, K=48, the SoA w_lo/w_hi split, w3_j7,
# CHUNKSHAPE148_BUCKET_RUN=2048 and the adopted CHUNKSHAPE148_ITER_SORT=1 are
# all byte-for-byte identical to 346. The only source change is inside
# chunkshape148_reorder_group: two new modes, 5 and 6, and the mode ceiling
# rises from 4 to 6. THE DEFAULT STAYS 1, so a run with no environment
# override reproduces 346 exactly and reuses its cached shaped bin.
#
# 346 IS CONFIRMED (2026-07-28):
#     default (mode 1)          402.258s
#     in-session mode 0 control 431.812s      -6.844%
#     chunk0 -7.853%   chunk1 -6.412%   chunk2 -6.007%
#     correctness 314666222712, 85 static checks with zero FAIL, both runs
#     reported cache_state=reuse.
#
# CROSS-SESSION REPRODUCIBILITY IS NOW ESTABLISHED. Three independent
# measurements of the same configuration -- 344 at 431.944s, the 345 anchor at
# 431.983s, the 346 control at 431.812s -- span 0.040%, and mode 1 itself moved
# only -0.050% from 402.460s to 402.258s. The -6.8% effect therefore carries a
# margin of more than a hundred times the noise floor.
#
# WHAT 347 TESTS: mode 1 still leaves a SYSTEMATIC residual imbalance. A sorted
# launch group hands warp w the ranks 32*w .. 32*w+31 inside EVERY one of the
# 48 strata, so warp 0 always draws the floor of each stratum and warp 483
# always draws the ceiling. Which warp is heaviest is fixed, and the makespan
# is fixed with it. Reversing the order inside every odd-numbered 15488-record
# stratum -- a serpentine or boustrophedon layout -- makes each warp draw the
# ceiling in half the strata and the floor in the other half, equalising the
# warp totals. The reversal operates on whole strata, so the 32-alignment rule
# established in 340 is preserved exactly, and the reordering is still a pure
# permutation, so membership is unchanged.
#
#     mode 1  (346 default) launch group ascending        anchor, ~402.3s
#     mode 5  mode 1 + serpentine                         <- candidate
#     mode 6  mode 2 (descending) + serpentine
#
# PRE-REGISTERED PREDICTIONS:
#   1. mode 5 beats mode 1 by roughly 1%.
#   2. mode 6 matches mode 5, because once the systematic offset is removed
#      direction should become genuinely symmetric.
#   3. The unexplained chunk2 direction dependence from 345 -- under mode 2,
#      chunk0 at -8.736% and chunk1 at -6.843% beat mode 1 while chunk2 fell
#      to -3.372% -- should SHRINK OR DISAPPEAR under mode 6. If it does, the
#      systematic-offset explanation is supported; if it survives, the
#      mechanism is something else and stays an open question.
#   A synthetic estimate on the real launch shapes puts mode 5 at -1.32% on a
#   full launch and -0.60% on the partial one, with mode 6 at -1.34% and
#   -1.03%. These are estimates from a synthetic cost distribution, not
#   predictions of the exact magnitude.
#
# IF MODE 5 FAILS TO BEAT MODE 1, the residual imbalance is not what limits the
# makespan. In that case close the sort-layout axis and move to Open Objectives
# item 7, the identically zero w_hi_arr, which touches only the prologue and
# epilogue and never the hot loop.
#
# MANDATORY VALIDATION ORDER:
#   1. STATIC_ONLY=1 first.
#   2. CHUNKSHAPE148_ITER_SORT=1 -- the anchor. Cache MUST report reuse and the
#      elapsed time must land near 402.3s. If it does not, stop: nothing
#      downstream is comparable.
#   3. Then mode 5 twice, then mode 6 twice.
#   4. Confirm 314666222712 FIRST on every run, before any timing.
#
# CACHE WARNING: modes 5 and 6 each get their own shaped bin (..._isort5.bin,
# ..._isort6.bin) and therefore BUILD it on first use. A run that reports
# chunkshape148_cache_state=build is NOT timing-comparable. Run each mode TWICE
# and take the second run. Mode 1 reuses the existing 345/346 bin and needs
# only one run.
#
#     STATIC_ONLY=1 bash 347Py_serpentine_probe_validate_N21_full_once.sh
#     CHUNKSHAPE148_ITER_SORT=1 bash 347Py_serpentine_probe_validate_N21_full_once.sh
#     CHUNKSHAPE148_ITER_SORT=5 bash 347Py_serpentine_probe_validate_N21_full_once.sh
#     CHUNKSHAPE148_ITER_SORT=5 bash 347Py_serpentine_probe_validate_N21_full_once.sh
#     CHUNKSHAPE148_ITER_SORT=6 bash 347Py_serpentine_probe_validate_N21_full_once.sh
#     CHUNKSHAPE148_ITER_SORT=6 bash 347Py_serpentine_probe_validate_N21_full_once.sh
#
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./348Py_lighttail_probe.py}
CAND=${CAND:-./348Py_lighttail_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/348Py_lighttail_probe_N21_full.lock}
...
# 8 = mode 1 plus the light-tail rotation, no serpentine. Modes 7 and 8 build
# their own shaped bin on first use; modes 1 and 5 reuse the existing ones.
...
LOGDIR="${LOG_ROOT%/}/348Py_lighttail_probe_logs_N21_full_once_${TS}"
...
echo "[start] 348 lighttail validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 348 lighttail SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '348 lighttail' "$SRC"; then
  printf 'source_version_tag	348 lighttail	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	348 lighttail	missing	FAIL
...
record_check source_chunkshape148_iter_sort_max 8 "${IS_MAX:-missing}" || static_failures=$((static_failures+1))
...
# The serpentine pass must exist, must be gated to modes 5 and 6 only, and must
# operate on whole iter_len strata so the 32-alignment rule from 340 survives.
if grep -q 'if (mode==5 or mode==6 or mode==7) and iter_len>=1:' "$SRC_NODOC" \
   && grep -q 'sseg:int=iter_len' "$SRC_NODOC" \
   && grep -q 'if (sidx&1)==1:' "$SRC_NODOC"; then
  printf 'source_chunkshape148_serpentine_pass\tgated to modes 5/6, reverses whole iter_len strata\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_chunkshape148_serpentine_pass\tgated to modes 5/6, reverses whole iter_len strata\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if grep -q 'if (mode==5 or mode==6 or mode==7) and iter_len>=1:' "$SRC_NODOC"; then
...
SERP_LINE=$(grep -n 'if (mode==5 or mode==6 or mode==7) and iter_len>=1:' "$SRC_NODOC" | head -n1 | cut -d: -f1)
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 348 source static checks failed" >&2
  exit 66
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 348 lighttail (kernel-identical to 347/346/345/344/338/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT; 348 adds modes 7 and 8, the light-tail rotations, completing a 2x2 factorial over serpentine and tail weight together with modes 1 and 5, and leaves the 346 default of 1 untouched. The rotation is a no-op on a full launch, so mode 8 must reproduce mode 1 and mode 7 must reproduce mode 5 on chunk0 and chunk1; only chunk2 can move. Mode 7 is the candidate. To reach the 344 baseline at any time, run with CHUNKSHAPE148_ITER_SORT=0"
