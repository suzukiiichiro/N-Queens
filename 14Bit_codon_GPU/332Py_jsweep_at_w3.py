...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 332 (2026-07-23)
...
5. [W=3確定・332でJ側スイープ中] funcid_reorder w/jのN21再調整
   330のW-スイープでW=4がw8_j7比-0.523%(kernel_reduce_ms)の改善候補と
   なり、331の下側境界スイープ(W∈{2,3,4,5,8}、全点正当性OK)で**W=3が
   新最良(448,789ms、W=8対照点454,673ms比-1.294%)**となった。W=3は
   W=2(450,840ms)とW=4(452,288ms)の両側からブラケットされた内部最適点
   であり、境界問題は解消。指標の再現性は3セッション連続で確認済み
   (W=8対照点: 328/330/331で454,672/454,674/454,673ms、差1〜2ms。
   W=4再現: 330比10ms差)。331でのキャッシュ済みW=4のelapsed 453.623s
   は予測(~453.6s)と一致し、bin生成コスト説明も検証完了。W=3採用後の
   実効elapsedは約450.1s(456.0s比約-1.3%)の見込み。332はJ側スイープ:
   W=3固定でJ∈{3,5,7,11}(J=7はセッション内アンカー)、加えて
   グローバル対照点(8,7)と完全性確認点(1,7)の計6ペアを実行する。
   判定基準は同一(kernel_reduce_ms合計、ノイズ帯0.03〜0.12%超え)。
   J側で(3,7)を上回る点が出なければ、333でw3_j7をソースデフォルトへ
   正式採用(defaults/reason変更+通常のフルvalidate)する。

このセクション自体は通常カーネルロジックとは独立しています。
332はカーネルロジックの変更を含みません(328/329/330/331と同一。
ソースの変更はこのドキュメントとVERSION_TAG/reason文字列のみ)。
================================================================================
...
`old_ok`の検索が-1を返して改行置換になってしまいました。331スクリプトから作り直します。332を用意しました。

**332の設計(J側スイープ)**
- ハーネスを(W,J)ペアリスト方式に一般化(`SWEEP_WJ="3:3 3:5 3:7 3:11 8:7 1:7"`)
- **(3,7)**: W=3のセッション内アンカー(448,789ms前後を再現するはず)
- **(8,7)**: グローバル対照点(3セッション連続で454,672〜674msに収まっている、最強の再現性指標)
- **(1,7)**: ブラケット上は不要ですが、地形が非単調(W=6異常あり)なので、低側の第2極小を約8分で排除できる保険として追加
- 新規binは(3,3)(3,5)(3,11)(1,7)の4ペア分

判定はいつも通りkernel_reduce_ms合計です。J側で(3,7)を上回る点が出なければ、**333でw3_j7をソースデフォルトへ正式採用**(デフォルト定数+reason文字列の変更、通常の単発フルvalidateで確定)します。上回る点が出た場合はその周辺を追加探索してから採用に進みます。

`STATIC_ONLY=1 bash 332Py_jsweep_at_w3_wjsweep_N21.sh` → フルスイープ(約50分)をお願いします。
"""
...
VERSION_TAG:str="332 jsweep-at-w3: kernel logic is BYTE-FOR-BYTE IDENTICAL to 328-331 (adopted SoA baseline); source changes are documentation/VERSION_TAG/reason-string only. 331's low-boundary sweep (W in {2,3,4,5,8}, J=7, all 5 points correctness-OK at 314666222712) found W=3 as the new best: kernel_reduce_ms sum 448789 vs the in-session W=8 control's 454673, i.e. -1.294%, with W=3 BRACKETED as an interior optimum by W=2 (450840) and W=4 (452288), both worse -- the 330 boundary concern is resolved; metric reproducibility is now confirmed across three sessions (W=8 control: 454672/454674/454673 in 328/330/331, spread 1-2ms = 0.0004%; W=4 reproduction within 10ms of 330) and the cached-bin W=4 elapsed of 453.623s matched the ~453.6s prediction, closing the bin-generation-cost explanation; projected adopted-state elapsed at W=3 is ~450.1s (~-1.3% vs the 456.0s baseline); 332 now sweeps the J side: W=3 fixed with J in {3,5,7,11} (J=7 doubles as the in-session anchor for the new W), plus the global control pair (8,7) and the completeness point (1,7) -- W=1 is not required for bracketing (W=3 is already interior) but is cheap and rules out a second low-side local minimum given the terrain's known non-monotonicity (the W=6 anomaly); the harness is generalized from a W-list to a (W,J)-pair list for this purpose; same decision rule (kernel_reduce_ms sum, noise band 0.03-0.12%); if no J-side point beats (3,7), revision 333 formally adopts w3_j7 into the source defaults (funcid_reorder default + reason string) with a standard single-run full validate; unchanged: Codon-atomic question CLOSED, rev84 CUDA C runner remains the mid-term tail-effect route"
...
BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON:str="332 jsweep-at-w3: kernel logic identical to 328-331 (adopted SoA baseline); 331 found W=3 as the bracketed interior optimum (-1.294% kernel_reduce_ms vs W=8, metric reproducible to 0.0004% across three sessions); 332 sweeps J in {3,5,7,11} at W=3 plus control pair (8,7) and completeness point (1,7); runtime-args-only pair sweep, no source or kernel change; if no J-side point beats (3,7), 333 adopts w3_j7 into source defaults"
