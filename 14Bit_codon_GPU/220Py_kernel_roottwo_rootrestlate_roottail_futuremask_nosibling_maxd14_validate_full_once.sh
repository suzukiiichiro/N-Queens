...
# =============================================================================
# NQ_UPDATE_MEMO
# 220: 更新メモ: 219Py rootrestlate_prblockdirect は正当性OKだが217最速を上回らなかったため不採用。220Pyは217Py rootrestlate新最速基準を親に、MAXD14 kernelのみroot<=2判定をroot_tail clear-lowbit判定へ置き換える再結合実験を検証する。
# Full update history: see README.md
# =============================================================================

# 220 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 220 keeps the
# 217 rootrestlate fastest baseline because 218 rootrestlate_andnot and 219
# rootrestlate_prblockdirect were correct but did not beat 217. 220 is the
# third 217-neighborhood recombination: it keeps 217 rootrestlate/root_first late
# and changes only the root<=2 predicate to root_tail = root_rest & (root_rest-1).
#
...
#   - rootrestlate_roottail source check is whitespace/comment robust.
SRC=${SRC:-./220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
LOGDIR="${LOG_ROOT%/}/220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 217 rootrestlate futuremask/no-sibling MAXD14 dispatch; 217 is new fastest over 204; 218 rootrestlate_andnot and 219 rootrestlate_prblockdirect were correct but slower than 217; 220 tests 217 plus root_tail clear-lowbit predicate; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_tail clear-lowbit predicate + rootrestlate	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
marker='# 220 rootrestlate-roottail:'
...
# Strip comments before shape-position checks so prose cannot satisfy or break
# active-code checks.
...
if_pos=root_compact.find('ifroot_tail==u32(0):')
...
checks = {
    ...
    'root_tail_present': 'root_tail:u32=root_rest&(root_rest-u32(1))' in root_compact,
    'root_tail_if_present': if_pos >= 0,
    'root_second_active_absent': 'root_second:u32=' not in root_compact,
    'root_after_scalar_absent': 'root_after_second:u32=' not in root_compact,
    'root_after_if_absent': 'ifroot_after_second==u32(0):' not in root_compact,
    'root_inline_xor_if_absent': 'if(root_rest^root_second)==u32(0):' not in root_compact,
    'root_andnot_absent': 'root_rest&(~root_second)' not in root_compact,
    ...
    'pr_block_from_nibble_present': 'pr_block_code:u32=pr_nibble_op&u32(7)' in root_compact,
    'pr_block_direct_absent': 'pr_block_code:u32=schedule_lo&u32(7)' not in root_compact,
...
if all(checks.values()):
    sys.exit(0)
print('source_rootrestlate_roottail_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootrestlate_roottail\tpresent_rootrestlate_roottail\tpresent_rootrestlate_roottail\tOK
' >> "$SUMMARY"
else
  printf 'source_rootrestlate_roottail\tpresent_rootrestlate_roottail\tmissing_or_old_rootrestlate_roottail_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split220'}
bad=sorted(tags-allowed)
if 'split220' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split220 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split220 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 220 runtime-only harness disabled source/model assertions"
