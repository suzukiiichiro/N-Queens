...
# 155 single-pass validation harness
#
...
# Candidate 155 retains the validated 154 DFS and u32 input layout. Its sole experiment is:
# row/functionid are pre-packed on the host as ctrl0_arr:u32, and the GPU kernel
# receives one packed pointer instead of separate row_arr/funcid_arr pointers.
# A real flock guard prevents two copies of this harness from competing.

SRC=${SRC:-./155Py_ctrl0_u32_prepack_probe.py}
CAND=${CAND:-./155Py_ctrl0_u32_prepack_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/155Py_nqueens_gpu_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 154/153/152/151/150/149 full runs and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 155 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/155Py_logs_N21_full_once_${TS}"
...
if [[ -f "$SRC" ]]; then
  ...
  # 155 sole experiment: pre-pack initial functionid/row as ctrl0:u32.
  ...
  # CPU/diagnostic mirrors remain available, but must not cross the GPU boundary.
  grep -Fq 'self.row_arr:List[int]' "$SRC" || source_failures=$((source_failures+1))
  grep -Fq 'self.funcid_arr:List[int]' "$SRC" || source_failures=$((source_failures+1))
  ...
  # Exhaustive identity check over valid initial 5-bit fields: 32 rows x 28 fids.
  ...
  if (( source_failures != 0 )); then
    printf 'source_ctrl0_shape\trequested layout\t%d check failures\tFAIL\n' "$source_failures" >> "$SUMMARY"
    echo "[error] source does not match the requested 155 ctrl0-u32 prepack experiment" >&2
    exit 65
  fi
  printf 'source_ctrl0_shape\trequested layout\tverified\tOK\n' >> "$SUMMARY"
  printf 'ctrl0_pack_equivalence\t896 cases\t%s cases\tOK\n' "$pack_cases" >> "$SUMMARY"
fi
...
echo "[validation-ok] 155 one full run reproduced cases 01-06"
