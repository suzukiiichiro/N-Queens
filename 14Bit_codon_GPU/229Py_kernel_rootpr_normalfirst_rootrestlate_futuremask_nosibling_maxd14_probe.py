...
# =============================================================================
# NQ_UPDATE_MEMO
# 229: 更新メモ: 228Py root-preroll ncol early は正当性OKだが217/225を上回らなかったため不採用。229Pyは225Py generic ncol-earlyを親に、MAXD14 root-preroll内だけ pr_block_code==0 の normal step1 path を先に置く normal-first 微差実験。generic loop、rootrestlate、futuremask、no-sibling、root one-two preroll は維持する。
...
229Py note:
  228Py was correct but slower, so this branch returns to the 225 generic
  ncol-early shape and changes only the MAXD14 root-preroll block-code branch
  order.  Since the normal step1 path is expected to be common, 229 tests
  `if pr_block_code==0` first in the root-preroll body while leaving the
  generic DFS loop, rootrestlate predicate, futuremask guard, no-sibling
  stack behavior, and host task order unchanged.
...
したがって、228Pyの root-preroll `pr_ncol` early は**不採用**です。

229Pyは225Py相当へ戻し、MAXD14 root-preroll内だけで **normal-first** を試す版にしました。
...
`STATIC_ONLY=1` の静的確認はOKです。

```bash
cd /home/suzuki/Github/N-Queens/13Bit_codon/Archive

STATIC_ONLY=1 bash 229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物です。

* [229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_probe.py)
* [229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/229Py_kernel_rootpr_normalfirst_rootrestlate_futuremask_nosibling_maxd14_bundle_20260703.tar)

"""

# 225 generic-ncol-early/rootpr-normalfirst memo over validated 217 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="225 generic-ncol-early/rootpr-normalfirst: 222 MAXD13 diagnostics withdrawn; validated 217 rootrestlate scalar predicate restored - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 225 generic-ncol-early/rootpr-normalfirst:
    ...
    if root_after_second==u32(0):
      ...
      # 229 rootpr-normalfirst: keep 225 generic ncol-early, but test the
      # root-preroll normal step1 path before the block-specific path.
      # Semantics are unchanged; this is only a branch-shape experiment.
      pr_nld:u32=u32(0)
      pr_nrd:u32=u32(0)
      if pr_block_code==u32(0):
    ...
    while True:
      ...
      # 225 generic-ncol-early/rootpr-normalfirst: ncol is independent of block_code and nld/nrd.
...
CHUNKSHAPE148_DEFAULT_REASON:str="225 generic-ncol-early/rootpr-normalfirst reuses validated 217 scorestripe_v9 task order/cache; MAXD14 scalar root_after_second predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split229_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split229_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split229-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split229-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split229_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split229-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split229-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split229_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
