...
# =============================================================================
# 276 N=21 full validation harness
# Parent: 271 final-fastest-complete.
# Experiment: reject 273 root_action==0 direct MAXD14 kernel for speed and
#             restore the 271 kernel/execution baseline. No schedule precompute,
#             no root0 direct kernel, no fid14/rest launch split.
...
SRC=${SRC:-./276Py_restore274_after_bucketdiag_reject_probe.py}
CAND=${CAND:-./276Py_restore274_after_bucketdiag_reject_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/276Py_restore274_after_bucketdiag_reject_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/276Py_restore274_after_bucketdiag_reject_logs_N21_full_once_${TS}"
...
echo "[start] 276 restore274-after-bucketdiag-reject validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 276 restore274-after-bucketdiag-reject SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '276 restore274-after-bucketdiag-reject' "$SRC"; then
  printf 'source_version_tag\t276 restore274-after-bucketdiag-reject\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t276 restore274-after-bucketdiag-reject\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split276' in s and 'split275' not in s and 'split274' not in s and 'split273' not in s and 'rootaction0-direct' not in s and 'kernel_dfs_iter_gpu_maxd14_root0' not in s and 'split270_' not in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split276 active; 275/273/270/240 rejected', 'split276', True))
else:
    checks.append(('source_split_tag', 'split276 active; 275/273/270/240 rejected', 'old/bucket/root0 split runtime present or split276 missing', False))
...
echo "[static-ok] 276 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 276 restore274-after-bucketdiag-reject mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "276parent274:$BASELINE_276_PARENT_274_SECONDS" "275diag:$BASELINE_275_DIAG_SECONDS" "273rejected:$BASELINE_273_REJECT_SECONDS" "272diag:$BASELINE_272_DIAG_SECONDS" "271fastest:$BASELINE_271_FASTEST_SECONDS" "271parent241:$BASELINE_271_PARENT_SECONDS" "270rejected:$BASELINE_270_REJECT_SECONDS" "267:$BASELINE_267_SECONDS" "239:$BASELINE_239_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 276 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
