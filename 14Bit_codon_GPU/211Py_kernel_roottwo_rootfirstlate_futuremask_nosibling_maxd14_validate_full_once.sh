...
# =============================================================================
# NQ_UPDATE_MEMO
# 211: 更新メモ: 210Py rootafterandnot は正当性OKで206より速いが204最速を上回らなかったため不採用。211Pyは204/206相当のroot_after_second XOR predicateへ戻し、MAXD14 kernelのみroot_first lowbit抽出をroot<=2 predicate成立後へ遅延する微差実験を検証する。
...
# progress TSV and verifies the complete total. Candidate 211 returns to
...
#   - rootfirstlate source check is whitespace/comment robust.
SRC=${SRC:-./211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 211 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 211Py source
#   has a harmless header/string/layout difference. This 211 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second XOR scalar if + late root_first extraction	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if all(checks.values()):
    sys.exit(0)
print('source_rootfirstlate_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootfirstlate\tpresent_late_root_first_predicate\tpresent_late_root_first_predicate\tOK
' >> "$SUMMARY"
else
  printf 'source_rootfirstlate\tpresent_late_root_first_predicate\tmissing_or_old_rootfirst_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split211'}
bad=sorted(tags-allowed)
if 'split211' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split211 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split211 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 211 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 211 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootfirstlate static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 210 roottwo-rootfirstlate futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 210 roottwo-rootfirstlate futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
