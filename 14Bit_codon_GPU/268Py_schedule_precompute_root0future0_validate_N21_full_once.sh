...
# =============================================================================
# 268 N=21 full validation harness
# Parent: 267 root0countcache precompute-best / 241 restore239.
# Experiment: keep 267 root0 MAXD14 variant and also cache future_check_nonzero_count
#             during build_soa_for_range. root_action==0 and future_check_mask==0 chunks
#             use a root0future0 MAXD14 kernel with no root_action or future-prune branches.
...
SRC=${SRC:-./268Py_schedule_precompute_root0future0_probe.py}
CAND=${CAND:-./268Py_schedule_precompute_root0future0_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/268Py_schedule_precompute_root0future0_N21_full.lock}
...
BASELINE_263_SECONDS=${BASELINE_263_SECONDS:-429.417}
BASELINE_264_SECONDS=${BASELINE_264_SECONDS:-429.539}
BASELINE_265_SECONDS=${BASELINE_265_SECONDS:-429.233}
BASELINE_266_SECONDS=${BASELINE_266_SECONDS:-428.113}
...
LOGDIR="${LOG_ROOT%/}/268Py_schedule_precompute_root0future0_logs_N21_full_once_${TS}"
...
echo "[start] 268 schedule-precompute-root0future0 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 268 schedule-precompute-root0future0 ctrl0-markctrl-free-repack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '268 schedule-precompute-root0future0' "$SRC"; then
  printf 'source_version_tag\t268 schedule-precompute-root0future0\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t268 schedule-precompute-root0future0\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
required_defs = [
    'kernel_dfs_iter_gpu_maxd14','kernel_dfs_iter_gpu_maxd14_root0_future0','kernel_dfs_iter_gpu_maxd14_root0','kernel_dfs_iter_gpu_maxd16','kernel_dfs_iter_gpu_maxd18','kernel_dfs_iter_gpu_maxd20','kernel_dfs_iter_gpu_maxd21',
...
old_splits = ['split240_','split241_','split242_','split243_','split244_','split255_','split256_','split257_','split258_','split259_','split260_','split261_','split262_','split263_','split264_','split265_','split266_','split267_']
runtime_split_ok = ('split268_' in s and not any(x in s for x in old_splits))
if runtime_split_ok:
    checks.append(('source_split_tag', 'split268 active; old split runtime tags absent', 'split268', True))
else:
    checks.append(('source_split_tag', 'split268 active; old split runtime tags absent', 'old split runtime tag present or split268 missing', False))
...
if not m14:
    ...
    ok = (
        ...
        'free_arr:Ptr[u32]' in body and
        'markctrl_arr:Ptr[u32]' in body and
        'sched_hi_arr:Ptr[u32]' in body and
        'root_action_arr:Ptr[u32]' in body and
        'sched_mask_arr:Ptr[u32]' not in body and
        'sched_lo_arr:Ptr[u32]' not in body and
        'sched_terminal_arr:Ptr[u32]' not in body and
        'schedule_lo:u32=ctrl0_arr[i]' in body and
        'schedule_hi:u32=sched_hi_arr[i]' in body and
        'sched_mask:u32=free_arr[i]' in body and
        'root_a:u32=bm&~(root_ld|root_rd|root_col)' in body and
        'root_a:u32=free_arr[i]&bm' not in body and
    ...
    actual = 'root0future0 MAXD14 precompute active' if ok else 'root0future0 missing or old schedule interpreter present or free_arr mask reuse missing or unsafe pack present'
    checks.append(('source_schedule_precompute_root0future0', 'MAXD14 precompute root0countcache active', actual, ok))

future0 = re.search(r'^@gpu\.kernel\s*\n^def\s+kernel_dfs_iter_gpu_maxd14_root0_future0\b.*?(?=^@gpu\.kernel\s*\n^def\s+kernel_dfs_iter_gpu_maxd14_root0\b)', s, re.M | re.S)
if not future0:
    checks.append(('source_rootaction_future_zero_kernel', 'root0future0 kernel without root_action/future branch', 'missing', False))
else:
    fbody = future0.group(0)
    ok = ('root_action_arr:Ptr[u32]' not in fbody and 'root_action:u32=' not in fbody and 'root_action==u32' not in fbody and 'future_check_mask:u32=' not in fbody and 'if future_check_mask' not in fbody and 'kernel_dfs_iter_gpu_maxd14_root0_future0' in fbody)
    checks.append(('source_rootaction_future_zero_kernel', 'root0future0 kernel without root_action/future branch', 'present' if ok else 'unsafe/missing pattern', ok))


root0 = re.search(r'^@gpu\.kernel\s*\n^def\s+kernel_dfs_iter_gpu_maxd14_root0\b.*?(?=^@gpu\.kernel\s*\n^def\s+kernel_dfs_iter_gpu_maxd16\b)', s, re.M | re.S)
if not root0:
    checks.append(('source_rootaction_zero_kernel', 'present', 'missing', False))
else:
    rbody = root0.group(0)
    ok = (
        'root_action_arr:Ptr[u32]' not in rbody and
        'root_action:u32=' not in rbody and
        'root_action==u32' not in rbody and
        'schedule_lo:u32=ctrl0_arr[i]' in rbody and
        'sched_mask:u32=free_arr[i]' in rbody and
        'sched_terminal:u32=markctrl_arr[i]' in rbody and
        'root_a:u32=bm&~(root_ld|root_rd|root_col)' in rbody
    )
    checks.append(('source_rootaction_zero_kernel', 'root0 kernel without root_action load/branch', 'present' if ok else 'unsafe/missing pattern', ok))
...
root0_launch_ok = (
    'root_action_nonzero_count:int=soa.root_action_nonzero_count' in launch_body and
    'if soa.root_action_arr[ri]!=u32(0):' not in launch_body and
    'if root_action_nonzero_count==0:' in launch_body and
    'kernel_dfs_iter_gpu_maxd14_root0(' in launch_body and
    'kernel_dfs_iter_gpu_maxd14(' in launch_body
)
checks.append(('source_rootaction_zero_dispatch', 'cached count root0/fallback dispatch', 'present' if root0_launch_ok else 'missing', root0_launch_ok))
...
host_ok = (
    ...
    'self.sched_hi_arr:List[u32]' in s and
    'self.sched_mask_arr:List[u32]' in s and
    'self.sched_terminal_arr:List[u32]' in s and
    'self.root_action_arr:List[u32]' in s and
    'self.root_action_nonzero_count:int=0' in s and
    'soa.root_action_nonzero_count=0' in s and
    'soa.root_action_nonzero_count+=1' in s and
    'sort_soa.root_action_nonzero_count=soa.root_action_nonzero_count' in s and
    'soa.ctrl0_arr[ri]=soa.sched_lo_arr[ri]' in s and
    'soa.markctrl_arr[ri]=soa.sched_terminal_arr[ri]' in s and
    'soa.free_arr[ri]=soa.sched_mask_arr[ri]' in s and
    'gpu.raw(soa.sched_hi_arr)' in s and
    'gpu.raw(soa.root_action_arr)' in s and
    'kernel_dfs_iter_gpu_maxd14_root0(' in s and
    'root_action_nonzero_count:int=soa.root_action_nonzero_count' in s and
    'gpu.raw(soa.sched_mask_arr)' not in s and
...
checks.append(('source_host_precompute_root0countcache_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 268 schedule-precompute-root0future0 mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "241parent:$BASELINE_241_SECONDS" "263:$BASELINE_263_SECONDS" "265:$BASELINE_265_SECONDS" "264rejected:$BASELINE_264_SECONDS" "262:$BASELINE_262_SECONDS" "239:$BASELINE_239_SECONDS" "242rejected:$BASELINE_242_REJECT_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 268 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
