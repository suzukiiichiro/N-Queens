...
ログ確認しました。**117 d0-only は成功扱いでよい**です。

`codon build` 後、117 の chunk 0 probe は `CUDA_ERROR_INVALID_PTX` なしで完走し、`chunk_total=2132383228`、`partial_total=2132383228` になっています。 さらに chunk `0,40,120` のまとめ実行でも、それぞれ `2132383228 / 2863018070 / 2292068932` で、合計 `7287470230` まで到達しています。

116 PTX-safe との比較では、同じ chunk 0 が `53.572s → 25.673s`、chunk 40 が `53.951s → 30.846s`、chunk 120 が `67.602s → 32.033s` です。   かなり良いです。特に chunk 0 と 120 は約 **2.1倍**まで戻っています。

一点だけ訂正です。前回「`log_level=1` で `[split117-buckets]` が出る」と説明しましたが、117 側では内部呼び出しで `gpu_log_level<2` のとき 0 を渡していたため、実際には表示されませんでした。118 ではそこを直して、`log_level=1` で bucket 数が出るようにしています。

次段階として **118 d2-only kernel probe** を作成しました。

[118Py_d2_only_kernel_probe.py](sandbox:/mnt/data/118Py_d2_only_kernel_probe.py)

118 は **d2 だけ**を新規専用 kernel にしています。

```text
専用化:
  funcid 12..18
  SQd2BkBlB / SQd2BlB / SQd2B / SQd2BklB / SQd2BlBkB / SQd2BkB / SQd2BlkB

汎用 kernel のまま:
  d2 以外すべて
```

117 の d0 kernel は 118 には入れていません。理由は、今回の目的が **d2 kernel 単体の PTX/JIT と total 差分の切り分け**だからです。d2 が通ったら、次に `119 d0+d2` として統合するのが安全です。

まず build です。

```bash
codon build -release 118Py_d2_only_kernel_probe.py
```

最初は同じ代表 chunk で確認してください。

```bash
./118Py_d2_only_kernel_probe \
...
```text
[split118-buckets] N=21 m=15488 d2=... rest=... split_mode=2 d2_specialized=1
```

判定基準は以下です。

```text
CUDA_ERROR_INVALID_PTX が出る
  → d2 kernel の PTX 生成問題。d2 kernel をさらに縮小。

PTX は通るが partial_total が 117/116 と違う
  → d2 専用 DFS ロジック差分。funcid 12..18 の mark1/mark2/blockK/blockL を再点検。

PTX が通り、partial_total が一致
  → d2 専用 kernel 採用候補。次は 119 で d0+d2 統合。
```

期待する partial total は、今回と同じ chunk list ならこれです。

```text
chunk 0   = 2132383228
chunk 40  = 2863018070
chunk 120 = 2292068932
合計      = 7287470230
```

こちらの環境では Codon/CUDA 実行ができないため、`codon build` と GPU 実機確認は未実施です。まずは `mode 30` の probe だけで十分です。N21 full はまだ不要です。
...
VERSION_TAG:str="118 d2-only kernel probe - rest uses proven 115 GPU kernel"
...
# the proven 115 kernel_dfs_iter_gpu device code, plus one small d2-only kernel.
#
# Purpose:
...
#   - add exactly one new specialized kernel at a time in a 118 probe
...
# every bucket still used the generic 115 kernel.  This 118 build adds exactly one new PTX target:
# kernel_dfs_d2_gpu for funcid 12..18.  All non-d2 tasks are grouped into one rest bucket and continue
# to use the proven 115 kernel_dfs_iter_gpu.
#
# Purpose:
#   - isolate PTX/JIT risk to one small specialized kernel
#   - validate d2 specialization with N21 probe chunks before any full run
...
def kernel_dfs_d2_gpu(
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        # SQd2B base: row==endmark で、列0以外の avail が残る場合だけ数える。
        ...
        # d2 mark transitions:
        #   12 SQd2BkBlB -> 13, step2, mark1, K=n3, L=0
        #   13 SQd2BlB   -> 14, step2, mark2, K=0,  L=1
        #   15 SQd2BklB  -> 14, step3, mark1, K=n4, L=1
        #   16 SQd2BlBkB -> 17, step2, mark1, K=0,  L=1
        #   17 SQd2BkB   -> 14, step2, mark2, K=n3, L=0
        #   18 SQd2BlkB  -> 14, step3, mark1, K=n3, L=2
        if a!=u32(0):
          if f==12 and rowv==mark1:
            use_blocks=1
            use_future=0
            stepv=2
            nextfv=13
            ktype=1
          elif f==13 and rowv==mark2:
            use_blocks=1
            use_future=0
            stepv=2
            nextfv=14
            bLv=1
          elif f==15 and rowv==mark1:
            use_blocks=1
            use_future=0
            stepv=3
            nextfv=14
            bLv=1
            ktype=2
          elif f==16 and rowv==mark1:
            use_blocks=1
            use_future=0
            stepv=2
            nextfv=17
            bLv=1
          elif f==17 and rowv==mark2:
            use_blocks=1
            use_future=0
            stepv=2
            nextfv=14
            ktype=1
          elif f==18 and rowv==mark1:
            use_blocks=1
            use_future=0
            stepv=3
            nextfv=14
...
####################################################################################################
# 118 d2-only split-kernel chunk runner
####################################################################################################

"""118: SoA 1件を別 SoA へコピーする小ヘルパ。"""
...
"""118: d2 だけ専用 kernel、その他は 115 汎用 kernel にまとめる。"""
def exec_solutions_gpu_chunk_split118(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  118 d2-only split:
    ...
    - split only funcid 12..18 into a d2 bucket
    - run all other tasks as a single rest bucket through the proven 115 generic kernel
    - run d2 through kernel_dfs_d2_gpu

  This keeps the PTX experiment minimal.  It should be used first with mode 30 probe chunks.
  """
  board_mask:int=(1<<N)-1
  ...
  while i<m:
    fid:int=soa.funcid_arr[i]
    if use_d2_specialized and (fid>=12 and fid<=18):
  ...
  if gpu_log_level>=1:
    print(f"[split118-buckets] N={N} m={m} d2={d2_m} rest={rest_m} split_mode={split_mode} d2_specialized={1 if use_d2_specialized else 0}")

  meta_next:List[u8]=[u8(1),u8(2),u8(3),u8(3),u8(2),u8(6),u8(2),u8(2),u8(0),u8(4),u8(5),u8(7),u8(13),u8(14),u8(14),u8(14),u8(17),u8(14),u8(14),u8(20),u8(21),u8(21),u8(21),u8(25),u8(21),u8(21),u8(26),u8(26)]

  # Rest bucket: one generic launch.  This avoids the 116 PTX-safe 5-bucket overhead.
  if rest_m>0:
  ...
  # d2 bucket: the only new specialized kernel in 118.
  if d2_m>0:
  ...
  elapsed_ms:int=stage_no_read_ms
  stages:List[int]=[stage_soa_ms,stage_stats_ms,stage_split_ms,stage_kernel_reduce_ms,stage_compute_ms,stage_no_read_ms,d2_m,rest_m]
  return chunk_total,stats,stages,elapsed_text,elapsed_ms

def exec_solutions_gpu_bin_stream_split118(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split118_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split118_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split118_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split118_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split118-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split118-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split118-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split118-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split118(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,0 if gpu_log_level<2 else gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split118-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split118-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split118 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split118 full run / optional multi-GPU worker
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 30 8 7 0 1222 5 "" 2  # 118 d2-only probe chunks 1222..1226
      #   -g 22 22 32 484 1 0 7 31 8 7 0 0 1 2  # 118 d2-only full GPU single worker
      if argc >= 11:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split118-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split118 worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split118_d2_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split118_d2_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split118-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split118-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split118_probe_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split118(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split118-probe-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split118-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split118-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split118-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split118_full_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split118(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split118-full-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
