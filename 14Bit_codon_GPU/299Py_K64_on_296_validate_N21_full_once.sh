...
# =============================================================================
# 299 N=21 full validation harness (K64-on-296 probe)
...
# Chunk count changes: 296 K=32 -> 5 chunks; 299 K=64 -> 3 chunks.
...
SRC=${SRC:-./299Py_K64_on_296_probe.py}
CAND=${CAND:-./299Py_K64_on_296_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/299Py_K64_on_296_probe_N21_full.lock}
...
EXPECTED_K_PER_THREAD_MAXD14=64
...
LOGDIR="${LOG_ROOT%/}/299Py_K64_on_296_logs_N21_full_once_${TS}"
...
echo "[start] 299 K64-on-296 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 299 K64-on-296 SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '299 K64-on-296 probe' "$SRC"; then
  printf 'source_version_tag	299 K64-on-296 probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	299 K64-on-296 probe	missing	FAIL
...
# 299: K64-on-296 shape checks (296 kernel logic, K=64 sweep).
    ...
    ok_next_depth_present = 'next_depth:int=cur_depth+1' in body14
    ...
    ok_stack_shape = (ok_stride_param and ok_stack_array and ok_save_sp_retained and ok_next_depth_present
    ...
    checks.append(('source_K64_on_296_shape',
                    '296 kernel shape (stack array + save_sp + next_depth + stack_ptr+2/-2) + K=64',
                    'present' if ok_stack_shape else (
                        'missing (stride=%s stack=%s save_sp=%s next_depth=%s ptr_incr=%s ptr_decr=%s gs=%s wb=%s push=%s pop=%s)'
                        % (ok_stride_param, ok_stack_array, ok_save_sp_retained, ok_next_depth_present,
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 299 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 299 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 299 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 299 K64-on-296 mode31 split145"
...
echo "[validation-ok] 299 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=64 (3 dispatch/progress rows)"
