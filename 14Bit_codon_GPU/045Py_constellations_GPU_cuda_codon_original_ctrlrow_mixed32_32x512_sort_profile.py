...
max_blocks = 512
steps      = 16384
log level  = 0(no log)  1(info)  2(debug)  3(chunk profile)
sort_mode  = 0 : sortなし、基準
             1 : chunk内 functionid sort
             2 : chunk内 funcptn sort

$ codon build -release <filename>.py
$ ./<filename> -g 18 18 32 512 0
または
$ ./<filename> -g 18 19  N18とN19の両方を処理 32 512 0 
======
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=512,gpu_log_level:int=0,gpu_sort_mode:int=0)->None:
  """
  機能:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk45=32x512 sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    ...
    while off < m_all:
      ...
      if gpu_log_level>=2:
        t1=datetime.now()
      if gpu_log_level>=3:
        # chunk の中身診断。kernel には触らず、build 済み SoA だけを集計する。
        # ptn/fid/start/free の偏りを見ることで、tail chunk がなぜ重いかを切り分ける。
        fid_counts:List[int]=[0]*28
        ptn_counts:List[int]=[0]*6
        row_counts:List[int]=[0]*32
        free_sum:int=0
        free_max:int=0
        row_min:int=99
        row_max:int=0
        for di in range(m):
          fidv:int=soa.funcid_arr[di]
          ptnv:int=funcptn_by_fid[fidv]
          fid_counts[fidv]+=1
          ptn_counts[ptnv]+=1
          rv:int=soa.row_arr[di]
          if rv>=0 and rv<32:
            row_counts[rv]+=1
          if rv<row_min:
            row_min=rv
          ...
          if fb>free_max:
            free_max=fb
        free_avg_x1000:int=0
        if m>0:
          free_avg_x1000=(free_sum*1000)//m
        print(f"[gpu-profile] N={N} chunk={chunks} off={off} m={m} sort={gpu_sort_mode} row_min={row_min} row_max={row_max} free_avg_x1000={free_avg_x1000} free_max={free_max} ptn0={ptn_counts[0]} ptn1={ptn_counts[1]} ptn2={ptn_counts[2]} ptn3={ptn_counts[3]} ptn4={ptn_counts[4]} ptn5={ptn_counts[5]}")
        print(f"[gpu-fid] N={N} chunk={chunks} f0={fid_counts[0]} f1={fid_counts[1]} f2={fid_counts[2]} f3={fid_counts[3]} f4={fid_counts[4]} f5={fid_counts[5]} f6={fid_counts[6]} f7={fid_counts[7]} f8={fid_counts[8]} f9={fid_counts[9]} f10={fid_counts[10]} f11={fid_counts[11]} f12={fid_counts[12]} f13={fid_counts[13]} f14={fid_counts[14]} f15={fid_counts[15]} f16={fid_counts[16]} f17={fid_counts[17]} f18={fid_counts[18]} f19={fid_counts[19]} f20={fid_counts[20]} f21={fid_counts[21]} f22={fid_counts[22]} f23={fid_counts[23]} f24={fid_counts[24]} f25={fid_counts[25]} f26={fid_counts[26]} f27={fid_counts[27]}")
      if gpu_log_level>=2:
      ...
      if gpu_log_level>=2:
        t3=datetime.now()
        print(f"[gpu-chunk] N={N} chunk={chunks} off={off} m={m} grid={GRID} sort={gpu_sort_mode} build={str(t1-t0)[:-3]} sortprep={str(t_sort1-t_sort0)[:-3]} kernel={str(t2-t_sort1)[:-3]} span={str(t2-t1)[:-3]} copy={str(t3-t2)[:-3]}")
      off += m
