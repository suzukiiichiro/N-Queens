...
# =============================================================================
# NQ_UPDATE_MEMO
# 288: generic-normaldefault-nfdefault。287/286のnormal-default nld/nrdを親に、MAXD14 generic loopでnormal pathのncol/nfも既定値として先に作り、special pathだけnld/nrd/nfを上書きする微差実験版。
#      MAXD14 generic DFS loopだけ normal path の nld/nrd を既定値として
#      先に計算し、block_code!=0 の special path のときだけ上書きする
#      normal path局所実験版。ncol early、placed_ld/placed_rd共通化、
...
どちらも `STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

## 287 採用固定版

286の `0:07:04.033` を新最速基準として固定する版です。CUDA kernelの探索・加算ロジックは286と同一です。

* [287Py_adopt286_normaldefault_nldnrd_probe.py](sandbox:/mnt/data/287Py_adopt286_normaldefault_nldnrd_probe.py)
* [287Py_adopt286_normaldefault_nldnrd_validate_N21_full_once.sh](sandbox:/mnt/data/287Py_adopt286_normaldefault_nldnrd_validate_N21_full_once.sh)
* [287Py_adopt286_normaldefault_nldnrd_sha256.txt](sandbox:/mnt/data/287Py_adopt286_normaldefault_nldnrd_sha256.txt)
* [287Py_adopt286_normaldefault_nldnrd_bundle_20260709.tar](sandbox:/mnt/data/287Py_adopt286_normaldefault_nldnrd_bundle_20260709.tar)

```bash
rm -f ./287Py_adopt286_normaldefault_nldnrd_probe

STATIC_ONLY=1 bash 287Py_adopt286_normaldefault_nldnrd_validate_N21_full_once.sh

bash 287Py_adopt286_normaldefault_nldnrd_validate_N21_full_once.sh
```

## 288 normal nf-default 実験版

287/286を親に、normal path の `ncol` と `nf` も既定値として先に作り、special pathだけ `nld/nrd/nf` を上書きする実験版です。

* [288Py_generic_normaldefault_nfdefault_probe.py](sandbox:/mnt/data/288Py_generic_normaldefault_nfdefault_probe.py)
* [288Py_generic_normaldefault_nfdefault_validate_N21_full_once.sh](sandbox:/mnt/data/288Py_generic_normaldefault_nfdefault_validate_N21_full_once.sh)
* [288Py_generic_normaldefault_nfdefault_sha256.txt](sandbox:/mnt/data/288Py_generic_normaldefault_nfdefault_sha256.txt)
* [288Py_generic_normaldefault_nfdefault_bundle_20260709.tar](sandbox:/mnt/data/288Py_generic_normaldefault_nfdefault_bundle_20260709.tar)

```bash
rm -f ./288Py_generic_normaldefault_nfdefault_probe

STATIC_ONLY=1 bash 288Py_generic_normaldefault_nfdefault_validate_N21_full_once.sh

bash 288Py_generic_normaldefault_nfdefault_validate_N21_full_once.sh
```
...
実行順は、まず287で286相当へ戻ることを確認し、その後に288を回すのが安全です。
...
VERSION_TAG:str="288 generic-normaldefault-nfdefault: parent 285/283 generic normal-first; MAXD14 generic loop computes normal-path nld/nrd as default and only overwrites them for block_code!=0 special path; no ncol early, no placed_ld/placed_rd commonization, root-preroll and host shaping/cache/dispatch preserved"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while True:
      ...
      # 288: normal path is dominant; compute normal nld/nrd/ncol/nf as defaults,
      # then overwrite nld/nrd/nf only for special block_code.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split288_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split288_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split288-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split288-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split288_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
