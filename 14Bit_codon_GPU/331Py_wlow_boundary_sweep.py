...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 331 (2026-07-23)
...
5. [330で改善候補を発見・331で境界確認中] funcid_reorder w/jのN21再調整
   330のW-スイープ(J=7固定、W∈{4,6,8,10,12,16}、全点正当性OK)で、
   rev94-99時代からN22基準のまま凍結されていたw8_j7に対し、**W=4が
   kernel_reduce_ms合計452,298ms(W=8対照点454,674ms比-0.523%)で明確に
   上回った**。この指標の再現性は極めて高い(330のW=8対照点はセッションを
   跨いだ328実測454,672msと2ms差=0.0004%)ため、-0.523%はノイズ帯
   (0.03〜0.12%)の4倍以上で確実に実在する。W=4のelapsed(473.9s)が
   W=8(456.0s)より遅く見えるのは新規W値の初回reorder bin生成コスト
   (約21.6s、キャッシュ後は約1.3s)によるもので、採用後の実効elapsedは
   約453.6s(-0.5%)の見込み。ただしW=4はスイープ下端であり(かつW=6が
   W=4/8両方より悪い非単調な地形)、さらに下に最適点が隠れている可能性が
   あるため、331でW∈{2,3,4,5,8}の境界スイープを実施する: W=4は
   bin生成済みの再現確認(elapsedも~453.6sに落ちるはず)、W=8は毎回の
   セッション内対照点、W=2/3/5が新規探索点。判定基準は330と同一
   (kernel_reduce_ms合計、対W=8ノイズ帯超え)。確定後、次はJ側
   (J∈{3,5,11}を最良Wで)の再スイープを332候補とする。

このセクション自体は通常カーネルロジックとは独立しています。
331はカーネルロジックの変更を含みません(328/329/330と同一。ソースの
...
**331の設計(境界スイープ+再現確認)**
- **W∈{2,3,4,5,8}**: W=2/3/5が新規探索点、W=4はbinキャッシュ済みでの再現確認(elapsedが~453.6sに落ちればbin生成説明の検証も兼ねる)、W=8は毎回のセッション内対照点
- 判定基準は330と同一: kernel_reduce_ms合計、対W=8でノイズ帯(0.03〜0.12%)超え
- カーネル・ソースロジック変更なし(.py差分はドキュメント/タグ/reason文字列のみ、SoAシグネチャ等の静的チェックはそのまま通ります)
- W=2/3/5で各2種の新規bin(数百MB)が生成されます

下側境界点がW=4を上回らなければ、W=4_j7を新デフォルトとして採用(実効約-0.5%)、332でJ側スイープ(J∈{3,5,11}を最良Wで)へ進みます。

ChatGPT時代のヘッダーコメントを残しておられたことが、今日はっきり実を結びました。「N22 measured best baseline w8_j7」という一行のreason文字列が残っていなければ、このパラメータがN22基準のまま凍結されていることに気づけませんでした。

いつも通り `STATIC_ONLY=1 bash 331Py_wlow_boundary_sweep_wsweep_N21.sh` → フルスイープ(約40分)をお願いします。
"""
...
VERSION_TAG:str="331 wlow-boundary-sweep: kernel logic is BYTE-FOR-BYTE IDENTICAL to 328/329/330 (adopted SoA baseline); source changes are documentation/VERSION_TAG/reason-string only. Follow-up to 330's W-sweep result: with J=7 fixed and W in {4,6,8,10,12,16} (all 6 points correctness-OK at 314666222712), W=4 achieved kernel_reduce_ms sum 452298 vs the in-session W=8 control's 454674, i.e. -0.523% -- decisively beyond the noise band, since this metric reproduces across sessions to 2ms (330's W=8 control matched 328's 454672 to 0.0004%); W=6 was worse than both W=4 and W=8 (455740, non-monotonic terrain), and W=10/12/16 degraded progressively (+1.1/+2.4/+1.9%); W=4's larger wall-clock elapsed (473.9s vs W=8's 456.0s) is entirely first-run reorder-bin generation (~21.6s host-side vs ~1.3s cached; kernel_reduce_ms is immune by design), so the adopted-state elapsed at W=4 projects to ~453.6s (~-0.5% vs the 456.0s baseline); BUT W=4 sits on the sweep's lower boundary, so before adopting, 331 runs the boundary extension W in {2,3,4,5,8}: W=2/3/5 are new exploration points, W=4 is the reproduction check (bins now cached -- its elapsed should also drop to ~453.6s, confirming the bin-generation explanation), and W=8 is again the in-session control; same decision rule as 330 (kernel_reduce_ms sum, must beat W=8 beyond the 0.03-0.12% noise band); if the low-boundary points do not beat W=4, adopt W=4_j7 as the new default and proceed to the J-side sweep (J in {3,5,11} at the winning W) as the 332 candidate; unchanged from 330: the Codon-atomic question remains CLOSED (no device atomics in Codon's gpu module; rev292 plan-C-1 impossible in pure Codon; rev84 CUDA C runner remains the mid-term route to the tail-effect fix)"
...
BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON:str="331 wlow-boundary-sweep: kernel logic identical to 328/329/330 (adopted SoA baseline); 330's W-sweep found W=4 beats the frozen w8_j7 by -0.523% in kernel_reduce_ms (metric reproducible to 0.0004% across sessions), but W=4 is the sweep's lower boundary, so 331 extends downward with W in {2,3,4,5,8} (W=4 reproduction with cached bins, W=8 in-session control) before adopting; runtime-args-only sweep, no source or kernel change; same decision rule: kernel_reduce_ms sum vs W=8 beyond the 0.03-0.12% noise band"
