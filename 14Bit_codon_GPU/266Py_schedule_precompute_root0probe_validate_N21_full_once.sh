...
# =============================================================================
# 266 N=21 full validation harness
# Parent: 265 reusemask precompute-best / 241 restore239.
# Experiment: keep single launch per chunk and add a root_action==0 MAXD14 variant.
#             Chunks with all root_action==0 use a root0 kernel without root_action load/branch;
#             nonzero chunks fall back to the 265-compatible kernel.
...
SRC=${SRC:-./266Py_schedule_precompute_root0probe_probe.py}
CAND=${CAND:-./266Py_schedule_precompute_root0probe_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/266Py_schedule_precompute_root0probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/266Py_schedule_precompute_root0probe_logs_N21_full_once_${TS}"
...
echo "[start] 266 schedule-precompute-root0probe validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 266 schedule-precompute-root0probe ctrl0-markctrl-free-repack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '266 schedule-precompute-root0probe' "$SRC"; then
  printf 'source_version_tag\t266 schedule-precompute-root0probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t266 schedule-precompute-root0probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split266_' in s and not any(x in s for x in old_splits))
if runtime_split_ok:
    checks.append(('source_split_tag', 'split266 active; old split runtime tags absent', 'split266', True))
else:
    checks.append(('source_split_tag', 'split266 active; old split runtime tags absent', 'old split runtime tag present or split266 missing', False))
...
if not m14:
    ...
    actual = 'root0probe MAXD14 precompute active' if ok else 'root0 kernel missing or old schedule interpreter present or free_arr mask reuse missing or unsafe pack present'
    checks.append(('source_schedule_precompute_root0probe', 'MAXD14 precompute root0probe active', actual, ok))
...
root0_launch_ok = (
    'root_action_nonzero_count:int=0' in s and
    'if soa.root_action_arr[ri]!=u32(0):' in s and
    'if root_action_nonzero_count==0:' in s and
    'kernel_dfs_iter_gpu_maxd14_root0(' in s and
    'kernel_dfs_iter_gpu_maxd14(' in s
)
checks.append(('source_rootaction_zero_dispatch', 'conditional root0/fallback dispatch', 'present' if root0_launch_ok else 'missing', root0_launch_ok))
...
checks.append(('source_host_precompute_root0probe_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 266 schedule-precompute-root0probe mode31 split145"
...
echo "[validation-ok] 266 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
