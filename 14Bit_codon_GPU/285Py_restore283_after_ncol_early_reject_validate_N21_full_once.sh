...
# =============================================================================
# 285 N=21 full validation harness
# Parent: 283 generic-normalfirst. Experiment: reject 284 ncol-early and restore 283 normal-first baseline.
...
SRC=${SRC:-./285Py_restore283_after_ncol_early_reject_probe.py}
CAND=${CAND:-./285Py_restore283_after_ncol_early_reject_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/285Py_restore283_after_ncol_early_reject_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/285Py_restore283_after_ncol_early_reject_logs_N21_full_once_${TS}"
...
echo "[start] 285 restore283-after-ncol-early-reject validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 285 restore283-after-ncol-early-reject SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '285 restore283-after-ncol-early-reject' "$SRC"; then
  printf 'source_version_tag	285 restore283-after-ncol-early-reject	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	285 restore283-after-ncol-early-reject	missing	FAIL
...
old_split_markers = ['split286', 'split284', 'split282', 'split281', 'split280', 'split278', 'split277', 'split275', 'split273', 'split270_', 'split240', 'split239_', 'kernel-blockdiag', 'block_count_diag', 'rootaction0-direct']
old_present = [x for x in old_split_markers if x in s]
if 'split285' in s and not old_present:
    checks.append(('source_split_tag', 'split285 active; rejected runtime tags absent', 'split285', True))
else:
    checks.append(('source_split_tag', 'split285 active; rejected runtime tags absent', 'present=' + ','.join(old_present) if old_present else 'split285 missing', False))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 285 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 285 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 285 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 285 restore283-after-ncol-early-reject mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "284rejected:$BASELINE_284_REJECT_SECONDS" "283normalfirst:$BASELINE_283_NORMALFIRST_SECONDS" "282restore:427.728" "281diag:574.374" "279restore:427.728" "276current:427.672" "271fastest:427.705" "239:427.703" "217:427.709"; do
...
echo "[validation-ok] 285 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
