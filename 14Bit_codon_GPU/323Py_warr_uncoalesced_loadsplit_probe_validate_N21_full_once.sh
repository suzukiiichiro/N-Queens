...
# =============================================================================
# 323 N=21 full validation harness (warr-uncoalesced-loadsplit-probe)
# Parent: 322 manualreview-reconverge-loopback (documentation-only, no
#   source changes, no build/run). None of 320/321/322 have their own
#   confirmed N=21 timing; 319 (455.116s, 314666222712) remains the most
#   recent confirmed baseline.
#
# NEW ANALYSIS (this revision, still no source changes): re-examined
#   319_ncu_source.txt beyond stall_branch_resolving. Two additional
#   angles:
#   1. stall_wait (the LARGEST single stall category, 43.39% of all PC
#      samples, bigger than stall_branch_resolving's 22.63%) is
#      concentrated in the same address range (0x...d440-0x...e040) as
#      the five continue-condition branches identified in 322's manual
#      review, corroborating that mapping from an independent metric.
#   2. L2 Theoretical Sectors Global Excessive (the ncu OPT advisor's
#      11.79%-speedup uncoalesced-access finding) is 100% explained by
#      exactly 4 instructions: two LDG.E at addr 0x...e120/0x...e130
#      (92,928 excessive sectors each, 96% of the 189,728 total) and two
#      STG.E at addr 0x...e310/0x...e320 (1,936 each). This is a MUCH
#      more precisely localized finding than the branch-resolving one.
#
# HYPOTHESIS for the LDG.E pair (addr e120/e130, offsets +0/+4 of a
#   single base pointer R12): this is `w_arr[idx]` (the only Ptr[u64]
#   array read via a single idx index), specifically the read at line
#   ~861 (`thread_total+=total*w_arr[idx]`), based on: (a) execution
#   count of the two LDG.E rows (~729,860-743,424) matches m (tasks per
#   chunk) almost exactly -- i.e. once per outer-loop iteration, not
#   once per DFS node like the hot inner-loop instructions (which run
#   into the tens/hundreds of billions); (b) SASS address ordering places
#   e120/e130 immediately after the DFS-loop-exit reconvergence point
#   (e0f0/e100/e110), matching line 861's position right after the
#   `while True:` loop in source; (c) the split into two 32-bit LDG.E at
#   offset 0/+4 of one base register is the classic pattern for a u64
#   value loaded as two 32-bit halves instead of one 64-bit transaction.
#   NOT CONFIRMED via any line-level tool output (still unavailable in
#   this toolchain per 318-321) -- this is address/frequency/type-based
#   inference, offered with appropriate uncertainty.
#
# Why this matters: unlike the Stall Branch Resolving loop-back (322,
#   deep inside the divergent DFS traversal, high risk to touch per the
#   rev-189 precedent), this is a single, isolated, once-per-task load
#   of one array -- outside the hot recursive loop -- making it a much
#   LOWER-RISK candidate for future investigation, IF the hypothesis
#   above is confirmed.
#
# This revision: ZERO changes to kernel logic (variant=2, K=48
#   unchanged), and NO fix is proposed yet -- this is documentation of a
#   new lead only. Per the user's request, this revision also adds a
#   persistent "Open Objectives" section near the top of both this
#   source file's docstring and README.md, summarizing current open/
#   closed investigation threads so they don't require re-reading the
#   full chronological history to find current status.
...
SRC=${SRC:-./323Py_warr_uncoalesced_loadsplit_probe.py}
CAND=${CAND:-./323Py_warr_uncoalesced_loadsplit_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/323Py_warr_uncoalesced_loadsplit_probe_N21_full.lock}
...
# NOTE: none of 320's, 321's, or 322's own N=21 full runs have been
...
# attempted; 322 was a documentation-only manual source review with no
# build/run requested). No fabricated BASELINE_320_*/321_*/322_* values
# are added here; 319 remains the most recent confirmed timing baseline
# until this 323 run (or a future one) produces a newer one.
...
LOGDIR="${LOG_ROOT%/}/323Py_warr_uncoalesced_loadsplit_probe_logs_N21_full_once_${TS}"
...
echo "[start] 323 warr-uncoalesced-loadsplit-probe validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 323 warr-uncoalesced-loadsplit-probe SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '323 warr-uncoalesced-loadsplit-probe' "$SRC"; then
  printf 'source_version_tag	323 warr-uncoalesced-loadsplit-probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	323 warr-uncoalesced-loadsplit-probe	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 323 warr-uncoalesced-loadsplit-probe mode31 split145"
...
echo "[validation-ok] 323 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); deeper analysis of 319_ncu_source.txt found L2 Theoretical Sectors Global Excessive (11.79% OPT-advisor lead) is 100% explained by 4 instructions (2 LDG.E + 2 STG.E), hypothesized as the w_arr[idx] read at line ~861 based on execution-count/address/type reasoning (not line-level confirmed); NO fix proposed yet -- documentation of a new, lower-risk lead per the header comment"
