...
175Pyログを確認しました。**問題ありません**。むしろ175Pyはかなり良い結果です。

```text
175Py : 0:07:51.106
172Py : 0:08:11.190
差分  : 175Pyが 20.084秒高速、約4.089%改善
```
...
したがって、**175Pyを現時点の最速基準**として扱ってよいです。

次の176Pyは、175Pyを親にして **current frame の nibble opcode をレジスタ保持**する版にしました。
173Pyのようにmetadata全体をframe entryでdecodeするのではなく、保持するのは `cur_nibble` 1個だけです。

ファイルはこちらです。

* [176Py_kernel_current_nibble_register_maxd14_probe.py](sandbox:/mnt/data/176Py_kernel_current_nibble_register_maxd14_probe.py)
* [176Py_kernel_current_nibble_register_maxd14_validate_full_once.sh](sandbox:/mnt/data/176Py_kernel_current_nibble_register_maxd14_validate_full_once.sh)

176Pyの変更点です。

```text
175Py:
  候補bit処理ごとに current sp から nibble を再抽出

    if sp < 8:
      nibble_op = (schedule_lo >> (sp*4)) & 15
    else:
      nibble_op = (schedule_hi >> ((sp-8)*4)) & 15

176Py:
  current frame が active になった時点で cur_nibble に保持
  ...
  候補bit処理中は、
    nibble_op = cur_nibble
  だけにする
```

狙いは、同一frameに複数候補bitがある場合の schedule shift/extract 削減です。

```text
維持:
  175Py 4-bit nibble schedule
...
```bash
chmod +x 176Py_kernel_current_nibble_register_maxd14_validate_full_once.sh
./176Py_kernel_current_nibble_register_maxd14_validate_full_once.sh
```

この176Pyは、173Pyの大幅悪化原因だった「decode済みmetadataを多数scalar保持」は避けています。
速くなれば176Pyを新基準、遅ければ175Py基準へ戻すのがよいです。


"""

# 176 GPU current-nibble-register experiment over validated 175 nibble MAXD14:
#   Branch from the validated fastest 175 nibble-schedule build.  Host-side depth
...
# 176 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 176 MAXD14 uses scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="176 current-nibble register for MAXD14 over validated 175 nibble schedule - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 176 inherits the exact 175/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """176 MAXD14 current-nibble register plus register-held current frame and 13-slot ancestor stack."""
    ...
    # 176 additionally keeps the current depth's nibble in a scalar so multiple
    # candidate bits from the same frame do not repeatedly shift/extract it.
    ...
    # 176 keeps the validated 175 nibble schedule and caches the current frame nibble.  The active
    ...
    # 176: cache the schedule nibble for the active frame.  It is refreshed
    # only when descending to a child or restoring an ancestor, not for every
    # candidate bit inside the same frame.
    cur_nibble:u32=schedule_lo&u32(15)
    ...
    while True:
      if cur_avail==u32(0):
        ...
        cur_avail=avail[sp]
        if sp<8:
          cur_nibble=(schedule_lo>>u32(sp*4))&u32(15)
        else:
          cur_nibble=(schedule_hi>>u32((sp-8)*4))&u32(15)
        continue

      nibble_op:u32=cur_nibble
...
####################################################################################################
# 176 current-nibble-register MAXD14 + scalar-u32pair dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 175 scorestripe_v9 task order/cache and nibble MAXD14; 176 caches current nibble per active frame"
...
def main()->None:
  ...
  # 176 branches from validated 175.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 176 current-nibble-register GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split176_current_nibble_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split176_current_nibble_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split176-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split176-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split176_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split176-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split176-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split176_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
