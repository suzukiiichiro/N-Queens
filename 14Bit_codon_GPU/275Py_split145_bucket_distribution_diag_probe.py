...
# =============================================================================
# NQ_UPDATE_MEMO
# 275 r2: split145-bucket-distribution diagnostic。初版のbucket counter初期化位置を補修。274Pyで273 root0 direct kernelを
#      撤回し271相当へ戻ったことを受け、次候補選定用に d2base14(fid14) と
#      d0(fid26/27) のchunk/task分布をsummaryへ集約する軽量診断版。
#      CUDA MAXD14/16/18/20/21 kernel本文、rootrestlate、futuremask、no-sibling、
#      split145/chunkshape148、broadmarktail、cache生成、worker split、1 chunk 1 launchは
#      274/271相当で維持する。速度候補ではなく、273以降の分解根拠を取る版。
#      Full update history: see README.md
# 275 split145-bucket-distribution-diag r2 compile fix: bucket counters initialized in split145 stream function.
# =============================================================================
"""
   ,     #_
   ~\_  ####_        N-Queens
...
* [274Py_restore271_after_root0direct_reject_probe.py](sandbox:/mnt/data/274Py_restore271_after_root0direct_reject_probe.py)
* [274Py_restore271_after_root0direct_reject_validate_N21_full_once.sh](sandbox:/mnt/data/274Py_restore271_after_root0direct_reject_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [274Py_restore271_after_root0direct_reject_sha256.txt](sandbox:/mnt/data/274Py_restore271_after_root0direct_reject_sha256.txt)
* [274Py_restore271_after_root0direct_reject_bundle_20260709.tar](sandbox:/mnt/data/274Py_restore271_after_root0direct_reject_bundle_20260709.tar)
...
VERSION_TAG:str="275 r2 split145-bucket-distribution-diag: parent 274 restore271-after-root0direct-reject / 271 final-fastest-complete; no CUDA kernel body changes; no schedule precompute; no root0 direct kernel; no fid14/rest extra launch; collect d2base14(fid14) and d0(fid26/27) chunk/task distribution summary for next split145 optimization planning; generic N5..27; bare -g split145 mode31; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
CHUNKSHAPE148_DEFAULT_REASON:str="274 restores 271 final validated 241/239/238/231/217 scorestripe_v9 task order/cache; CUDA kernels unchanged; 273 root0 direct kernel, 270 rootpre2flag, and 240 fid14 launch split rejected"
...
def exec_solutions_gpu_bin_stream_funcid_reorder(
  ...
  chunk_index:int=0
  executed_chunks:int=0
  # 275: diagnostic-only aggregate for split145 bucket planning.
  # Uses existing per-chunk d0/d2base14 counts from stages_inner;
  # no CUDA kernel body, task order, or launch count is changed.
  bucket_total_tasks:int=0
  bucket_d0_tasks:int=0
  bucket_d2base14_tasks:int=0
  bucket_d0_or_d2_tasks:int=0
  bucket_d0_chunks:int=0
  bucket_d2base14_chunks:int=0
  bucket_d0_or_d2_chunks:int=0
  bucket_both_chunks:int=0
  _read_uint32_le=read_uint32_le
...
def exec_solutions_gpu_bin_stream_split145(
  ...
  chunk_index:int=0
  executed_chunks:int=0
  # 275 r2: diagnostic-only aggregate for split145 bucket planning.
  # Initialize in this outer split145 stream function. The first 275 build
  # had these counters only in another stream function, which Codon correctly
  # rejected as local variables referenced before assignment here.
  bucket_total_tasks:int=0
  bucket_d0_tasks:int=0
  bucket_d2base14_tasks:int=0
  bucket_d0_or_d2_tasks:int=0
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      d0_chunk_tasks:int=0
      d2base14_chunk_tasks:int=0
      if len(stages_inner)>8:
        d0_chunk_tasks=stages_inner[7]
        d2base14_chunk_tasks=stages_inner[8]
      d0_or_d2_chunk_tasks:int=d0_chunk_tasks+d2base14_chunk_tasks
      bucket_total_tasks+=m
      bucket_d0_tasks+=d0_chunk_tasks
      bucket_d2base14_tasks+=d2base14_chunk_tasks
      bucket_d0_or_d2_tasks+=d0_or_d2_chunk_tasks
      if d0_chunk_tasks>0:
        bucket_d0_chunks+=1
      if d2base14_chunk_tasks>0:
        bucket_d2base14_chunks+=1
      if d0_or_d2_chunk_tasks>0:
        bucket_d0_or_d2_chunks+=1
      if d0_chunk_tasks>0 and d2base14_chunk_tasks>0:
        bucket_both_chunks+=1
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      chunk_index+=1
  if gpu_log_level>=1:
    bucket_other_tasks:int=bucket_total_tasks-bucket_d0_or_d2_tasks
    if bucket_other_tasks<0:
      bucket_other_tasks=0
    print(f"[split145-bucket-summary] N={N} chunks={chunk_index} executed_chunks={executed_chunks} total_tasks={bucket_total_tasks} d2base14_chunks={bucket_d2base14_chunks} d2base14_tasks={bucket_d2base14_tasks} d0_chunks={bucket_d0_chunks} d0_tasks={bucket_d0_tasks} d0_or_d2_chunks={bucket_d0_or_d2_chunks} d0_or_d2_tasks={bucket_d0_or_d2_tasks} both_chunks={bucket_both_chunks} other_tasks={bucket_other_tasks} split_mode={split_mode}")
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 274 restore271/coretrim; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split275_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split275_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split275-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split275-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split275_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
