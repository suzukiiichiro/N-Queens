...
- elapsed = **353.671s** (0:05:53.671)
- vs 294 (362.782s) → **+9.111s (+2.511%) 改善**
- vs 292基準 (367.413s) → **累計 +13.742s (+3.740%) 改善**

`save_sp*2` 乗算の排除が効きました。この方向でさらに攻めます。コメント内のみ。ロジックコードには `save_sp` ゼロ — 問題なし。チェックを修正します。## 296 結果まとめと 297 の考え方

**296 採用確定**: 353.671s、vs 294比 **+9.111s (+2.511%)** 改善、vs 292累計 **+13.742s (+3.740%)** 改善。

`save_sp*2` 乗算の排除が大きく効きました。乗算1命令の削除でこれだけ改善するということは、push/pop のパスが非常にホットであることを示しています。
...
| 294 | 4xu32→2xu64 | 362.782s | −1.26% |
| **296** | **sp2乗算排除** | **353.671s** | **−3.74%** |

## 297 の着眼点

296 では `stack_ptr` と `save_sp` を並列管理していましたが、`save_sp` は今や break チェック (`save_sp==0`) と push/pop 時の `±1` カウンタとしか使われていません。`stack_ptr==0` が完全に等価なので、**`save_sp` を丸ごと削除**してレジスタを1本解放します。

```bash
STATIC_ONLY=1 bash 297Py_save_sp_elim_validate_N21_full_once.sh
bash 297Py_save_sp_elim_validate_N21_full_once.sh
```
"""
...
VERSION_TAG:str="297 save-sp-elim probe: parent 296 stack-ptr (adopted, +9.111s/+2.511% vs 294, +13.742s/+3.740% vs 292); kernel_dfs_iter_gpu_maxd14 eliminates the save_sp integer variable entirely: save_sp==0 break check replaced by stack_ptr==0, save_sp+=1/save_sp-=1 bookkeeping removed, save_sp:int=0 init removed; stack_ptr counter from 296 retained as the sole stack management variable; reduces live integer register count by 1 in the hot DFS loop; single stack array (MAXD14_ANCESTOR*2), K_PER_THREAD_MAXD14=32, all launch params unchanged; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 297: save_sp eliminated; stack_ptr==0 used for empty-stack check.
