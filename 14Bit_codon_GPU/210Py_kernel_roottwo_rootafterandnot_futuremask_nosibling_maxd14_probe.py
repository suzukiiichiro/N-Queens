...
# =============================================================================
# NQ_UPDATE_MEMO
# 210: 更新メモ: 209Py roottailclear は正当性OKながら204Py/206Pyを上回らなかったため不採用。210Pyは206/204相当へ戻し、MAXD14 kernelのみroot_after_secondを root_rest ^ root_second ではなく root_rest & ~root_second で作る and-not predicate 微差実験。root_rest clear-lowbit / futuremask / no-sibling / root one-two preroll は維持する。
...
209Pyは197より速いですが、204/206を上回らないため**正当性OKだが不採用**です。

210Pyは209の `root_tail` 判定を撤回し、206/204相当へ戻したうえで、MAXD14 kernel内だけの小差分にしました。
...
`root_second` は `root_rest` のlowbitなので、`root_rest ^ root_second` と `root_rest & ~root_second` は意味としては同じです。codegen差だけを見る微差実験です。

手元確認は以下です。

```text
bash -n 210Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 210Py...sh OK
source_rootafterandnot                OK
source_split_tag split145 split210    OK
```

ファイルはこちらです。

* [210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_probe.py)
* [210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_validate_full_once.sh

bash 210Py_kernel_roottwo_rootafterandnot_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 210 rootafterandnot memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  210 keeps the narrow root fast-start only when
...
VERSION_TAG:str="210 roottwo-rootafterandnot: 204 rootclearrest futuremask/no-sibling MAXD14 baseline with root_after_second and-not scalar predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # correct but slower.  210 keeps 204's clear-lowbit root_rest construction and restores
...
CHUNKSHAPE148_DEFAULT_REASON:str="210 rootafterandnot reuses validated 204 scorestripe_v9 task order/cache; MAXD14 root_after_second uses root_rest & ~root_second and-not predicate"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split210_roottwo_rootafterandnot_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split210_roottwo_rootafterandnot_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split210-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split210-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split210_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split210-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split210-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split210_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
