...
98Py の `mode17` / `mode18` は残し、新しく **`bench_mode=19`** を追加しました。98Py では `mode18` が stage列つき profile を出す構造になっており、直近ログでも `executed_chunks=20`、`partial_total=27298463716`、`sum_kernel_ms=153.025 sec` まで確認できています。

ソースはこちらです。

[99Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_chunksize.py](sandbox:/mnt/data/99Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_chunksize.py)

差分はこちらです。

[98_to_99_chunksize.patch](sandbox:/mnt/data/98_to_99_chunksize.patch)

## 追加したもの

```text
bench_mode=19
  funcid-reorder-v2-chunksize-profile
```

99Py の `mode19` は、98Py の代表20 chunkを起点にして、以下を比較します。

```text
factor=1  m=15488   max_blocks=484
factor=2  m=30976   max_blocks=968
factor=4  m=61952   max_blocks=1936
```

つまり、`gpu_block=32`、`base_max_blocks=484` のまま、
...
として、**1 kernel invocation あたりの投入 record 数を変える**設計です。

kernel / DFS 本体は変更していません。98Py と同じ `exec_solutions_gpu_chunk_profile()` を使い、投入サイズだけを変えています。98Py は SoA構築、stats、sort、kernel、reduce の stage列を追加する実装でした。
...
```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_chunksize.tsv
```

## 追加される列

98Py の profile列に加えて、末尾に以下を追加しました。
...
まず1 chunk / factor=1 だけ確認するなら、これが軽いです。
...
## 代表 chunk だけで factor=1,2,4 を見る場合
...
```bash
PROG=progress_N22_7_stream_funcid_reorder_v2_w8_j7_chunksize.tsv
...
}
{
  f=$(c["chunk_factor"])
  rows[f]++
  sum_elapsed[f]+=$(c["elapsed_ms"])
  sum_kernel[f]+=$(c["stage_kernel_ms"])
  sum_total[f]+=$(c["stage_total_ms"])
  sum_m[f]+=$(c["m"])
  sum_chunk_total[f]+=$(c["chunk_total"])
}
END {
  for (f in rows) {
    printf "factor=%s rows=%d records=%d chunk_total=%d elapsed=%.3f sec kernel=%.3f sec total=%.3f sec kernel_ms_per_record=%.6f\n", \
      f, rows[f], sum_m[f], sum_chunk_total[f], \
      sum_elapsed[f]/1000.0, sum_kernel[f]/1000.0, sum_total[f]/1000.0, \
      sum_kernel[f]/sum_m[f]
  }
}
' "$PROG" | sort -n -t= -k2
```
...
}
{
  print $(c["stage_kernel_ms"]), \
        "factor=" $(c["chunk_factor"]), \
        "base_chunk=" $(c["base_chunk"]), \
        "range=" $(c["range_start_chunk"]) "-" $(c["range_end_chunk"]), \
        "m=" $(c["m"]), \
        "total=" $(c["chunk_total"]), \
        "f5=" $(c["funcid_5_count"]), \
        "f7=" $(c["funcid_7_count"]), \
...
```text
stage_kernel_ms / m
stage_kernel_ms / chunk_total
stage_total_ms / m
chunk_factor=1,2,4 の相対変化
```

もし `factor=2` や `factor=4` で `kernel_ms_per_record` が下がるなら、chunk size 拡大が有望です。
逆に下がらない、または悪化するなら、Step 4 は早めに打ち切って Step 5 の **funcid別重い経路**に移るのがよいです。

今回の99Pyは、実機での `codon build -release` まではこちらでは確認できていません。まず smoke test の `factor=1 / chunk=1222` でビルド・起動確認をお願いします。
