...
# =============================================================================
# NQ_UPDATE_MEMO
# 206: 更新メモ: 205Py rootafterinlineは正当性OKながら204Pyより0.279秒遅いため不採用。206Pyは204Py相当へ戻し、MAXD14 kernelのみroot_after_second一時scalarを復元する安全復帰版を検証する。
# Full update history: see README.md
# =============================================================================

# 206 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 206 returns to
...
SRC=${SRC:-./206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 206 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/206Py_kernel_roottwo_restore_rootafter_scalar_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 206Py source
#   has a harmless header/string/layout difference. This 206 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second scalar if	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
checks = {
    ...
    # 206 keeps 204/197 save payload; 203 depth0-save is not adopted.
    ...
    # 206 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 206 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 206 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
allowed={'split145','split206'}
bad=sorted(tags-allowed)
if 'split206' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split206 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split206 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 206 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 206 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-restore_rootafter_scalar static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 206 roottwo-restore-rootafter-scalar futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 206 roottwo-restore-rootafter-scalar futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
