...
179Pyも `CUDA_ERROR_INVALID_PTX` なので、**178/179のblock-switch系は不採用**にします。

今回の180Pyは、危険なMAXD14 hot loop変更をやめて、**最速基準の175Py kernelへ完全に戻す**方針です。
そのうえで、次の最適化対象を決めるために、`.sh`側へ **chunk tail診断** を追加しました。

ファイルはこちらです。

* [180Py_kernel_taildiag_nibble_maxd14_probe.py](sandbox:/mnt/data/180Py_kernel_taildiag_nibble_maxd14_probe.py)
* [180Py_kernel_taildiag_nibble_maxd14_validate_full_once.sh](sandbox:/mnt/data/180Py_kernel_taildiag_nibble_maxd14_validate_full_once.sh)
...
撤回:
  ...
  178Py host-precomputed schedule
  178/179 block-switch系

180Py:
  GPU kernel arithmeticは175Py相当へ戻す
  追加SoAなし
  追加kernel引数なし
  block-switch分岐なし
  hot loopの危険な書き換えなし
...
tail_top_chunks.tsv
  遅いchunk上位25件
  chunk / elapsed_ms / score_avg / free_popcount_avg / depth_avg / dominant_funcid など

tail_bucket_summary.tsv
  chunk範囲 0-31 / 32-63 / 64-95 / 96-130 ごとの elapsed 集計

summary.tsv 追加項目:
  tail_top1_ms
  tail_top5_share_permille
  tail_top10_share_permille
  tail_suggested_chunks
  tail_top_chunks_file
  tail_bucket_summary_file
```
...
```bash
chmod +x 180Py_kernel_taildiag_nibble_maxd14_validate_full_once.sh
./180Py_kernel_taildiag_nibble_maxd14_validate_full_once.sh
```

180Pyは高速化そのものより、**175Py相当へ安全に戻して、次のtask配置・tail splitの材料を取る版**です。速度は175Py近辺に戻る想定で、178/179のようなPTX危険差分は外しています。


手付かずではありません。**かなり手は入っています。**
ただし、今回の **169Py〜179Py** の流れでは、主戦場をいったん **MAXD14 kernel内部・local memory・schedule表現** に絞っていたので、直近だけを見ると「task配置/tail splitはまだ再開していない」に近いです。

大きく分けると、過去には次の系統がすでにあります。

**1. chunk / stream reorder は実施済みです。**
96系では `funcid risk reorder v2` として、kernel/DFS本体は変えず、stream側の並び替えだけを変更する設計が入っています。risk-balanced bucket、bucket内 phase/stripe sampling、`w{8,16,32} x j{5,7,11}` の sweep などがあり、これは明確に task配置最適化です。

**2. tail split / heavy funcid 診断も実施済みです。**
98〜105系では、stage profile、chunk size profile、funcid target、funcid single、funcid split queue、depth/free-popcount、mark pattern、exact mark-distance まで分解しています。特に102系は `heavy_tail / bulk_heavy / rest` を別kernel launchで比較する診断で、「heavy funcid queueを分離すべきか」を調べる目的でした。

**3. exact tail island を使った reorder も実施済みです。**
105で見えた exact mark-distance tail island を、106で `fid x gap x d1` の risk key として stream reorderへ使い、107では broad funcid quotaを主制約にして、その内側で mark-distance riskを配分しています。

**4. broadmarktail も実施済みで、115系の実用デフォルトに採用されています。**
114系では broad funcid + mark-distance + tail の ablation reorder が入り、`funcid_17` と `cell_G_H` の弱い三次splitを維持しつつ、variant切替を行っています。ソース上では、115 final default が `broadmarktail mode29, w8_j7, variant 2 rotate_only` を採用した理由も明記されています。

**5. chunkshape148 / scorestripe も既にtask配置最適化です。**
168Pyは「scorestripe v9 lanephase32 octetfirstpairlock29」を引き継いでいます。そこでは score key、percentile threshold、bucket quota、32-lane extraction、within-bucket stride、`+11/+1` pair selector、`+29` first-pair lock が既存のvalidated host-shapingとして扱われています。これはまさに host側のtask配置/chunk shapingです。

一方で、**168Py以降の今回の連番では意図的に別軸へ寄せています。**
168Pyの説明には、host layout、task order、cache、DFS order、bitboard arithmetic、solution arithmetic は変えず、kernel内のctrl metadata表現だけを変更した、とあります。つまり168Py以降のまず前半は「task配置は固定して、kernel local memoryを削る」実験でした。

再開メモでも、169〜173は `MAXD14追加 → schedule削減 → current frame register → ancestor stack縮小 → metadata decode` というlocal-memory/kernel内部の計画で、174以降に「thread/warp単位の探索量診断」、175以降に「診断結果に応じてtask再配置または重量task追加split」と整理されています。

なので結論はこうです。

```text
過去全体:
  task配置・tail split は手付かずではない。
  むしろ 96〜148/164/167 あたりで相当やっている。

今回の169〜179:
  ほぼ kernel 内部最適化に集中。
  task配置・tail split の新規改善はまだ本格再開していない。

次にやるべきこと:
  175Pyを最速kernel基準に戻し、
  180Py相当でtail診断を取り直して、
  既存の scorestripe148 / broadmarktail / funcid split 系に
  N=21・MAXD14 kernel後の実測を反映する。
```

つまり「未着手」ではなく、**過去に大きく進めた領域を、175Py後の新しいkernel性能で再評価する段階**だと思います。


"""

# 180 tail-diagnostic rebound over validated 175 nibble-schedule MAXD14:
...
# 180 keeps the validated 175 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 180 MAXD14 uses scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="180 taildiag stable rebound: validated 175 nibble MAXD14 kernel unchanged - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 180 inherits the exact 172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """180 MAXD14 nibble schedule plus register-held current frame and 13-slot ancestor stack."""
    ...
    # 180 keeps the validated 172 current-frame register cache plus ancestor stack shrink.  The active
...
####################################################################################################
# 180 nibble-schedule MAXD14 + scalar-u32pair dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="180 taildiag reuses validated 175 nibble MAXD14 kernel and scorestripe_v9 task order/cache"
...
def main()->None:
  ...
  # 180 branches from validated 172.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 180 nibble-schedule GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split180_nibble_schedule_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split180_nibble_schedule_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split180-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split180-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split180_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split180-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split180-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split180_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
