...
SRC=${SRC:-./255Py_schedule_precompute_actionpack_probe.py}
CAND=${CAND:-./255Py_schedule_precompute_actionpack_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/255Py_schedule_precompute_actionpack_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/255Py_schedule_precompute_actionpack_logs_N21_full_once_${TS}"
...
echo "[start] 255 schedule-precompute-actionpack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 255 schedule-precompute-actionpack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '255 schedule-precompute-actionpack probe' "$SRC"; then
  printf 'source_version_tag\t255 schedule-precompute-actionpack probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t255 schedule-precompute-actionpack probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 255 schedule-precompute-actionpack mode31 split145"
...
echo "[validation-ok] 255 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
