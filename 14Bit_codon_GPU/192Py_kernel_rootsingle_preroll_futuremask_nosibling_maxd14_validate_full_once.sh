...
# =============================================================================
# NQ_UPDATE_MEMO
# 192: 更新メモ: 191は正当性OKだが188比で0.007秒の微小退行となったため不採用。188のfuture_check_mask guard/no-sibling spill elisionを親に戻し、MAXD14 kernelのみ root_avail 単一bitのthreadをDFS loop前に1段prerollする単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 192 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 192 starts from
# validated 188 because 188 remains the current fastest N=21 baseline; 189
# forced-chain, 190 blockmask guard, and 191 place-xor were correct but not adopted. 192 changes
# only the MAXD14 root-singleton preroll.
#
...
# root_avail contains exactly one bit, process depth 0 once before the generic
# DFS loop and then continue at depth 1. Multi-candidate root threads use the
# exact 188 loop. Host task order, dispatch, cache, progress verification, and
# README-managed history remain unchanged.
SRC=${SRC:-./192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 192 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 192Py source
#   has a harmless header/string/layout difference. This 192 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 188 futuremask/no-sibling MAXD14 dispatch; 189 forced-chain, 190 blockmask, and 191 place-xor not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root-singleton preroll	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'split192' "$SRC" && ! grep -q 'split191' "$SRC" && ! grep -q 'split190' "$SRC" && ! grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split192 only	split192 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split192 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 192 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source rootsingle-preroll static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 192 rootsingle-preroll futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 192 rootsingle-preroll futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
