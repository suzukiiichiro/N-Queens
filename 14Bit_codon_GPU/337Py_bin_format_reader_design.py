...
6. [設計中・337でbin形式を仕様確定] CUDA Cランナー(案C-2)スパイク設計
   330で292案C-1(persistent kernel + atomicワークキュー)が「Codonの
   @gpu.kernelにデバイスatomicが存在しない」ため実装不可能と確定した
   ことを受け、課題3(tail effect、stall_branch_resolvingの65.5%集中、
   Avg Threads Executed 2〜3/32のoccupancy collapse)への唯一の残存
   対策経路として、rev84で提案されたまま未着手のCUDA Cランナーの
   最小スパイク設計を334で開始した。方針: コンステレーション生成・
   並べ替え(broadmarktail/chunkshape148/funcid_reorder w3_j7)・bin
   キャッシュはCodonのまま維持し、**kernel_dfs_iter_gpu_maxd14一個
   のみ**を.cuに移植してCUDA Cランナー(ランチャー含む)から同じbin
   形式を読む最小構成とする。
   **334-335の実機プローブ結果(確定)**: nvcc=`/usr/local/cuda/bin/nvcc`
   (CUDA 13.0)、A10G compute_cap=**8.6**(`-arch=sm_86`)、cuobjdumpは
   このツールキットに同梱されていないことを確定(SASS逆アセンブルは
   `sudo ncu --section SourceCounters --page source`経路に一本化)。
   **336のスモークテスト結果(成功)**: `336_cudac_smoke_test.cu`を
   `nvcc -O3 -arch=sm_86`でビルド(build_exit=0)・実行(run_exit=0)し、
   GPU側計算結果とCPU側期待値が一致(`gpu_sum=cpu_sum=22898104320`
   `match=True`)。**これにより、cudacodon実機でCUDA Cツールチェーン
   単体でのビルド・実行round-tripが確認された。** rev84の提案から
   数か月越しの最初の実証。
   **337でbin形式を仕様確定**: ソース精読により、この プロジェクトの
   全`.bin`ファイル(base生成`gen_constellations_stream_to_bin`、
   broadmarktail subcellバケット、broadmarktail/chunkshape148
   並べ替え後出力)が例外なく単一の書き込み関数
   `append_constellations_bin`を経由しており、**レコード形式は
   一貫して「ヘッダーなし・マジックナンバーなしの、16バイト固定長
   レコードのフラット配列」**であることを確定した: `ld`(u32
   リトルエンディアン、4バイト)・`rd`(同)・`col`(同)・
   `startijkl`(同)の順で計16バイト/レコード。並べ替え処理
   (broadmarktail/chunkshape148/funcid_reorder)はレコードの並び順を
   変えるのみで、エンコーディング自体には一切手を加えない
   (`validate_bin_file`/`count_constellations_bin_records`の
   `size%16==0`チェック、`read_constellations_bin_range`の
   `read_uint32_le`4連続読み出しがこれを裏付ける)。この仕様に基づき、
   メインソルバとは独立した最小限のC側binリーダー
   (`337_bin_format_reader.cu`)を追加した。実際の`.bin`ファイルを
   読み込み、サイズが16の倍数であることを検証し、全レコードの
   `ld`/`rd`/`col`/`startijkl`をu64チェックサムに畳み込んで出力する
   (DFSロジック・GPU実行は一切含まない、純粋なホスト側ファイルI/O)。
   次のマイルストーンは、N=18限定でこのリーダーの出力(レコード数・
   チェックサム)をCodon側の対応する値と突き合わせて一致を確認し、
   その後に`kernel_dfs_iter_gpu_maxd14`本体の移植設計に進むこと。

このセクション自体は通常カーネルロジックとは独立しています。
337はカーネルロジックの変更を含みません(カーネル・ディスパッチャは
328-336と同一、funcid_reorderもw3_j7のまま)。ソースの変更は、
このドキュメント/VERSION_TAGと項目6のみです。337で追加された
唯一のコードは、メインソルバとは独立した`337_bin_format_reader.cu`
(および対応するビルド/実行チェックスクリプト)です。
================================================================================
...
**336の実行結果(確認、両方成功)**
- N=21フルvalidate: 静的チェック全OK。正当性314666222712一致。elapsed 450.432s(335の450.218s比-0.048%、ノイズ内)で完全に再現しました
- スモークテスト(`336_cudac_smoke_test_check.sh`): ビルド成功(build_exit=0)・実行成功(run_exit=0)・`gpu_sum=cpu_sum=22898104320`で**match=True**。**`nvcc -arch=sm_86`によるcudacodon実機でのCUDA Cビルド・GPU実行round-tripが確認できました。** rev84の提案から数か月越しの最初の実証です

**337の内容(bin形式の仕様確定+C側リーダー)**
- カーネル・ディスパッチャは328-336と一切変更なし。ソース変更はOpen Objectives項目6(更新)とVERSION_TAGのみです
- ソースを精読し、このプロジェクトの全`.bin`ファイル(base生成・broadmarktail subcellバケット・broadmarktail/chunkshape148並べ替え後出力)が例外なく単一の書き込み関数`append_constellations_bin`を経由していることを確認しました。レコード形式は一貫して**「ヘッダーなし・16バイト固定長レコードのフラット配列」**(`ld`/`rd`/`col`/`startijkl`、各u32リトルエンディアン)です。並べ替え処理はレコードの並び順を変えるだけで、エンコーディングには一切手を加えません
- この仕様に基づき、メインソルバとは独立した最小限のC側binリーダー`337_bin_format_reader.cu`を追加しました。実際の`.bin`ファイルを読み込み、サイズが16の倍数であることを検証し、全レコードをu64チェックサムに畳み込んで出力します(DFSロジック・GPU実行は一切含まない、純粋なホスト側ファイルI/Oです)
- 対応するチェックスクリプト`337_bin_format_reader_check.sh`も同梱しています。実際の`.bin`ファイルパスを引数に取り、レコード数・チェックサムを出力します(GPUは使用しません)
- 次の一手は、このリーダーの出力(レコード数・チェックサム)を、既存のCodon側集計(例えば`count_constellations_bin_records`相当の値)と手動で突き合わせて一致を確認することです。確認できれば、`kernel_dfs_iter_gpu_maxd14`本体のN=18限定移植設計に進みます


Now the trailing chat-log docstring and VERSION_TAG:Now update VERSION_TAG:Good. Now let's rename in validate.sh (336→337):Now let's rewrite the header block and validation-ok line completely for 337:Now the validation-ok line and baseline entries:Now add the 336 baseline variable and comparison entry:Now let's create the standalone bin-format reader `.cu` and its check script:Good. Now let's finalize outputs and update README:Now update README: Open Objectives header + item 6, and append chronological entry:Now append the chronological entry:336のスモークテスト、両方成功でした。ビルド成功・GPU実行成功・`gpu_sum=cpu_sum=22898104320`で一致 — cudacodon実機でCUDA Cツールチェーンのround-tripが初めて実証できました。rev84の提案から数か月越しですね。

337では、これを土台にbin形式を仕様確定しました。

**bin形式の仕様(ソース精読による確定)**
このプロジェクトの全`.bin`ファイル(base生成・broadmarktail 75subcellバケット・並べ替え後出力)は例外なく`append_constellations_bin`一つを経由しており、形式は完全に統一:
- ヘッダーなし・マジックナンバーなし
- 16バイト固定長レコードのフラット配列
- 各レコード: `ld`/`rd`/`col`/`startijkl`(すべてu32リトルエンディアン)
- 並べ替え処理はレコードの並び順だけを変え、エンコーディングには一切手を加えない

**337の内容**
- カーネル・ディスパッチャは328-336と一切変更なし
- `337_bin_format_reader.cu` — GPUカーネル・DFSロジックを一切含まないホスト側リーダー。実際の`.bin`ファイルを読み、サイズ検証+チェックサム算出
- `337_bin_format_reader_check.sh` — ビルドして既存の`constellations_N21_6*.bin`に対して実行(GPU不使用)

次の一手、2つ独立に実行してください:
```
STATIC_ONLY=1 bash 337Py_bin_format_reader_design_validate_N21_full_once.sh   # 既存N=21(変更なし)
bash 337_bin_format_reader_check.sh                                             # binリーダー(GPU不使用)
```
両方成功すれば、338でN=18限定の`kernel_dfs_iter_gpu_maxd14`本体の移植設計に着手します。

Only `summary.tsv` is present in this tar — this looks like a `STATIC_ONLY=1` run only (static checks passed, all OK), not the full N=21 run.337の静的チェック、確認しました — 全34項目+nvcc/cuobjdump/compute_capの4プローブ、FAILゼロで全OKです。ツールチェーン情報(nvcc 13.0、compute_cap 8.6、cuobjdumpなし)も引き続き一貫していますね。

ただ、このtarには`summary.tsv`しか入っておらず、`build.log`・`full_once.log`・`dispatch.env`などが含まれていません。これは`STATIC_ONLY=1`だけを実行して、フルN=21実行(正当性314666222712・elapsedの確認)まではまだ届いていないログに見えます。

お手数ですが、`STATIC_ONLY=1`を外して(または`STATIC_ONLY=0`で)フルN=21実行まで進めていただけますか。あわせて、`337_bin_format_reader_check.sh`(binリーダー本体)の結果もまだ届いていないので、そちらもお願いします。


これは先ほどと全く同じファイル(同じタイムスタンプ・同じ内容、`summary.tsv`のみ)ですね — おそらく再アップロードの取り違えかと思います。

こちらは`STATIC_ONLY=1`の静的チェック結果のみで、全34項目+ツールチェーンプローブ4項目、FAILゼロで確認済み(前回お伝えした内容と同一です)。

フルN=21実行(正当性314666222712とelapsedの確認)の結果と、`337_bin_format_reader_check.sh`の結果がまだ届いていません。お手数ですが、以下2点のログをお願いします:

1. `STATIC_ONLY=0`(またはオプションなし)でのフルN=21実行ログ一式(`build.log`・`full_once.log`・`dispatch.env`・`progress_metrics.env`等が含まれるtar)
2. `bash 337_bin_format_reader_check.sh` の実行結果(`337_bin_format_reader_check_<timestamp>/`フォルダ)

This one has the full run's logs.337のフルN=21実行、確認しました — 全項目OK、正当性314666222712・elapsed 450.056s(336比+0.083%、ノイズ内)で完全再現です。

これで337のN=21ハーネス側は問題ありません。あとは`337_bin_format_reader_check.sh`(binリーダー本体)の結果待ちです。実行いただければ、実際の`.bin`ファイルに対するチェックサムと`records_ok`の結果を確認して、338(N=18限定のmaxd14移植設計)に進めます。

こちらは先ほど確認した337のフルN=21実行と同じログですね(正当性314666222712・elapsed 450.056s、全項目OK)。確認済みです、ありがとうございます。

あとは`337_bin_format_reader_check.sh`(binリーダー本体)の結果だけ残っています。届き次第、338(N=18限定の`kernel_dfs_iter_gpu_maxd14`移植設計)に進みます。
...
```
size_bytes=32404512 (= 2025282 × 16、割り切れる)
total_records_expected=2025282 = records_read=2025282  records_ok=True
checksum_u64=13342728758502
```

`EXPECTED_TASKS=2025282`(このプロジェクトの既知の不変量)とも一致しています。これで337は完全にクローズです。

今日一日の進捗を整理すると:
- **334-335**: nvcc(`/usr/local/cuda/bin/nvcc`、CUDA 13.0)、A10G compute_cap(**8.6**)、cuobjdump不在を実機確定
- **336**: `nvcc -arch=sm_86`でのビルド・GPU実行round-tripを初実証(`match=True`)
- **337**: bin形式(16バイト/レコード、ld/rd/col/startijkl、u32 LE)をソース精読で仕様確定し、実ファイルに対してC側リーダーで検証済み

rev84の提案から数か月越しで、CUDA Cランナーの土台(ツールチェーン・GPU実行・binフォーマット)が揃いました。

338では、実際に`kernel_dfs_iter_gpu_maxd14`をN=18限定で移植する設計に入ります。ただしここからは一段レベルが上がります — bin上のld/rd/col/startijklだけでなく、SoA変換(`symmetry`関数によるw_arr導出、mark1/mark2/funcid/ijklの各配列)とK-batching(K=48、stack_bytes_per_thread=208)のレイアウトをC側で再現する必要があり、まず「コード無し・設計仕様書のみ」のリビジョンにするのが安全だと考えています(324が325の前にコード無し設計だったのと同じパターンです)。

このまま338(設計仕様のみ、まだ.cuコードは書かない)に進めてよろしいですか、それとも今日はここまでにしますか?
...
VERSION_TAG:str="337 bin-format-reader-design: NO kernel or dispatcher change -- byte-for-byte identical to 336/335/334/333/328-332 (w3_j7 remains the funcid_reorder default). 336's own full N=21 validate is CONFIRMED: correctness 314666222712, elapsed 450.432s (vs 335's 450.218s, -0.048%, within noise). 336's standalone CUDA C smoke test (336_cudac_smoke_test.cu, run via 336_cudac_smoke_test_check.sh, independent of this N=21 harness) SUCCEEDED: build_exit=0, run_exit=0, gpu_sum=cpu_sum=22898104320, match=True -- the first confirmed end-to-end nvcc -arch=sm_86 compile/link/execute round-trip on cudacodon, months after rev84 first proposed a CUDA C runner. 337 advances buried-idea C-2 by pinning down the .bin file format via direct source reading rather than assumption: every .bin write in this codebase (base generation in gen_constellations_stream_to_bin, the 75 broadmarktail subcell buckets, and the final broadmarktail/chunkshape148-reordered output) routes through the single function append_constellations_bin, and the record encoding is uniform throughout the entire pipeline: a header-less, magic-number-less flat array of fixed 16-byte records, each four little-endian uint32 fields in order ld, rd, col, startijkl. Reordering stages (broadmarktail/chunkshape148/funcid_reorder) only change record SEQUENCE, never the encoding -- confirmed by validate_bin_file/count_constellations_bin_records's size%16==0 check and read_constellations_bin_range's four consecutive read_uint32_le calls. Based on this now-confirmed spec, 337 adds a second standalone C-2 artifact: 337_bin_format_reader.cu (+ 337_bin_format_reader_check.sh), a minimal host-only (no GPU kernel, no DFS logic) C++ reader that opens an actual .bin file passed as a CLI argument, validates size%16==0, reads every record, and folds all four fields of every record into a uint64 checksum for manual cross-reference against the Codon side. Next milestone once this checksum is cross-checked at N=18: begin the actual kernel_dfs_iter_gpu_maxd14 port design. Main solver kernel/dispatcher/host params (w3_j7, K=48, SoA w_lo_arr/w_hi_arr) remain completely unchanged from 336/335/334/333."
