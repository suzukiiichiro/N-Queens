...
# =============================================================================
# 303 N=21 full validation harness (cur-depth-x4-neutral probe)
...
#           schedule_words=0, stack=208 bytes/thread (unchanged),
#           K_PER_THREAD_MAXD14=32 -> 5 dispatch/progress rows.
# EXPECTED_CHUNKS=5, K=32 (unchanged from 296).
...
SRC=${SRC:-./303Py_cur_depth_x4_neutral_probe.py}
CAND=${CAND:-./303Py_cur_depth_x4_neutral_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/303Py_cur_depth_x4_neutral_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/303Py_cur_depth_x4_neutral_logs_N21_full_once_${TS}"
...
echo "[start] 303 cur-depth-x4-neutral validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 303 cur-depth-x4-neutral SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '303 cur-depth-x4-neutral probe' "$SRC"; then
  printf 'source_version_tag	303 cur-depth-x4-neutral probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	303 cur-depth-x4-neutral probe	missing	FAIL
...
# 303: cur-depth-x4-neutral shape checks (register-neutral: cur_depth replaced by cur_depth_x4).
    ok_stride_param = 'stride:int,' in body14.split(')->None:')[0]
    ok_schedule_lo_hi = 'schedule_lo:u32=u32(0)' in body14 and 'schedule_hi:u32=u32(0)' in body14
    active303 = re.sub(r'#[^\n]*', '', body14)
    ok_no_cur_depth = 'cur_depth' not in re.sub(r'cur_depth_x4', '', active303)
    ok_x4_init = 'cur_depth_x4:int=0' in body14
    ok_preroll_x4 = 'cur_depth_x4=4\n' in body14
    ok_pop_x4 = 'cur_depth_x4=int(saved_avail>>u32(27))<<2' in body14
    ok_x4_incr = body14.count('cur_depth_x4+=4') == 1
    ok_hot_decode = ('if cur_depth_x4<32:' in body14 and
                     'schedule_lo>>u32(cur_depth_x4))' in body14 and
                     'schedule_hi>>u32(cur_depth_x4-32))' in body14)
    ok_terminal_x4 = 'cur_depth_x4==terminal_depth_x4' in body14
    ok_child_jmark = 'child_jmark_mask>>u32(cur_depth_x4>>2))' in body14
    ok_avail_pack = body14.count('u32(cur_depth_x4)<<u32(25)') == 2
    ...
    ok_shape = (ok_stride_param and ok_schedule_lo_hi and ok_no_cur_depth
                 and ok_x4_init and ok_preroll_x4 and ok_pop_x4 and ok_x4_incr
                 and ok_hot_decode and ok_terminal_x4 and ok_child_jmark and ok_avail_pack
                 and ok_save_sp and ok_stack_ptr_incr and ok_gridstride_loop and ok_single_writeback)
    checks.append(('source_cur_depth_x4_neutral_shape',
                    'register-neutral: cur_depth replaced by cur_depth_x4 throughout; hot decode/terminal/jmark/pack all x4',
                    'present' if ok_shape else (
                        'missing (stride=%s sched=%s no_cd=%s x4_init=%s preroll=%s pop=%s incr=%s hot=%s term=%s jmk=%s pack=%s sp=%s ptr=%s gs=%s wb=%s)'
                        % (ok_stride_param, ok_schedule_lo_hi, ok_no_cur_depth, ok_x4_init, ok_preroll_x4,
                           ok_pop_x4, ok_x4_incr, ok_hot_decode, ok_terminal_x4, ok_child_jmark, ok_avail_pack,
                           ok_save_sp, ok_stack_ptr_incr, ok_gridstride_loop, ok_single_writeback)
                    ),
                    ok_shape))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 303 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 303 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 303 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 303 cur-depth-x4-neutral mode31 split145"
...
echo "[validation-ok] 303 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
