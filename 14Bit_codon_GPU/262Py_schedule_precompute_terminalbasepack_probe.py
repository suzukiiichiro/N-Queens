...
# =============================================================================
# NQ_UPDATE_MEMO
# 261: schedule-precompute-terminalbasepack probe。256 terminalbasepack JIT成立と
...
"""
262 schedule-precompute note: 261 terminal_depth 単独decode JIT成立を受け、root_actionは別配列素通しのまま、terminal_depth と terminal_base14 だけを同一 sched_terminal_arr の低位bitsへpackする切り分けprobe。

261の結果を受けて、262を作成しました。
262は **261親 / terminal_depth + terminal_base14 の同一pack切り分け版**です。

261で確認できたことは以下です。

```text
261 terminaldepthpack:
  JIT成立
  正当性OK
...
262では、`root_action` は引き続き別配列素通しに固定し、JIT成立済みの `terminal_depth` と `terminal_base14` だけを同じ `sched_terminal_arr` にpackしています。

```text
sched_terminal_arr:
  bits 0..3  terminal_depth
  bit  4     terminal_base14
```

kernel側のdecodeは以下です。

```python
sched_terminal:u32 = sched_terminal_arr[i]
...
source_schedule_precompute_terminalbasepack OK
source_host_precompute_terminalbasepack_arrays OK
release_build_policy                       OK
```

ファイルはこちらです。

* [262Py_schedule_precompute_terminalbasepack_probe.py](sandbox:/mnt/data/262Py_schedule_precompute_terminalbasepack_probe.py)
* [262Py_schedule_precompute_terminalbasepack_validate_N21_full_once.sh](sandbox:/mnt/data/262Py_schedule_precompute_terminalbasepack_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [262Py_schedule_precompute_terminalbasepack_sha256.txt](sandbox:/mnt/data/262Py_schedule_precompute_terminalbasepack_sha256.txt)
* [262Py_schedule_precompute_terminalbasepack_bundle_20260707.tar](sandbox:/mnt/data/262Py_schedule_precompute_terminalbasepack_bundle_20260707.tar)

sha256です。

```text
7fe7390865202291db29938338a3f67b29b65296e7c70b758d8a680c1eec978e  262Py_schedule_precompute_terminalbasepack_probe.py
7e409929c1a24815aa26e0b196c077c321e9b62281a83a7a93847c15dc467b24  262Py_schedule_precompute_terminalbasepack_validate_N21_full_once.sh
141d13a13e0d6c833b3f6280817115a72a83147072bfaf82eaab30c3a4213770  README.md
```

実行は以下でお願いします。

```bash
rm -f ./262Py_schedule_precompute_terminalbasepack_probe

STATIC_ONLY=1 bash 262Py_schedule_precompute_terminalbasepack_validate_N21_full_once.sh

bash 262Py_schedule_precompute_terminalbasepack_validate_N21_full_once.sh
```

262がJIT成立すれば、`terminal_depth + terminal_base14` のpackは安全で、危険箇所はほぼ `root_action` decode/pack系に限定できます。
...
VERSION_TAG:str="262 schedule-precompute-terminalbasepack probe: parent 261 terminalbasepack JIT-ok / 256 base14pack JIT-ok / 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; child/future maskpack retained; root_action stays separate pass-through after 257/259/260 CUDA_ERROR_INVALID_PTX; terminal_depth and terminal_base14 are packed into sched_terminal_arr low bits; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 262: 261 terminal_depth and 256 base14pack were JIT-ok separately.
    # Pack only terminal_depth + terminal_base14 together; keep root_action
    # as separate pass-through after 257/259/260 root_action JIT failures.
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  ...
  # terminal_depth storage to sched_terminal_arr; root_action remains separate.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    sched_terminal_arr:Ptr[u32],root_action_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 262: schedule fields are precomputed on the host in build_soa_for_range().
    # child/future masks are packed; terminal_depth/base14 share sched_terminal_arr;
    # root_action remains separate pass-through.
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.sched_terminal_arr),gpu.raw(soa.root_action_arr),
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split262_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split262_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      progress_suffix:str=f"split262_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
