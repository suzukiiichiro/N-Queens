...
# =============================================================================
# 343 N=21 full validation harness (bucket-run-cap-raise2)
#
# 343's ONLY source change is one constant: CHUNKSHAPE148_BUCKET_RUN_MAX
# raised from 8192 to 65536 (plus a corrected comment above it). Kernels,
# dispatcher, K=48, SoA w_lo/w_hi, w3_j7, the 339 run-length emit loop and
# the source default CHUNKSHAPE148_BUCKET_RUN=1 are byte-for-byte identical
# to 342/341/340/339. The raise cannot change behaviour for any RUN <= 8192,
# so every shaped-bin cache built in 339-342 remains valid and reusable.
#
# 340+341+342 RESULTS (in-session RUN=1 control = 450.183s; every point
# correct at 314666222712, tasks 2025282, cache_state=reuse, zero failures;
# session drift +0.0161% / +0.0027% / -0.0195%):
#
...
#   256     yes   yes  439.875s    -2.290%     -2.331%   -2.450%   -1.937%
#   384     yes   -    447.077s    -0.690%     +0.423%   -0.745%   -2.079%
#   512     yes   yes  440.235s    -2.210%     -3.256%   -1.163%   -2.121%
#  1024     yes   yes  438.211s    -2.659%     -2.719%   -3.562%   -1.309%
#  2048     yes   yes  431.684s    -4.109%     -3.851%   -4.530%   -3.835%
#
# THREE ESTABLISHED RULES:
#   1. Warp alignment (340). Non-multiples of 32 -- 16, 48, 80 -- all sit in
#      a tight -0.26% to -0.34% band regardless of magnitude.
#   2. Power of two (confirmed by 342). 96 and 384 are multiples of 32 but
#      not powers of two, and both land at the bottom of the aligned group
#      (-0.609%, -0.690%) while their power-of-two neighbours 64, 128 and
#      512 reach -0.832%, -0.818% and -2.210%. 384 was the pre-registered
#      control and regressed exactly as predicted.
#   3. At RUN=2048 all three chunks improved together for the first time
#      (-3.851%, -4.530%, -3.835%). The tug-of-war seen from 256 to 1024,
#      where chunk2 preferred RUN=64 while chunk0/chunk1 preferred RUN=256,
#      is gone -- which retires the chunk-specific-RUN idea.
#   The curve is NOT monotone: 512 is marginally worse than 256.
#
# WHAT 343 CORRECTS -- AN ERROR IN 342's REASONING:
#   342 set the ceiling at 8192 because saturation was expected near
#   quotas[b] ~ STEPS/8 = 15488/8 ~ 1936. But 1936 is only the AVERAGE
#   quota. quotas[] is proportional to each bucket's remaining population
#   and merely SUMS to m_target = STEPS = 15488, so a single bucket's quota
#   can be as large as 15488. A run emits min(RUN, quotas[b], remaining),
#   so the guaranteed saturation point is RUN >= 15488 -- still above the
#   8192 ceiling. This is the same mistake as 341, which capped at 256 and
#   then found 256 to be the best point; it was repeated one scale up.
#   65536 clears 15488 with room, and a new gating check now verifies that
#   the cap is at least BLOCK*MAX_BLOCKS so this cannot recur silently.
#
# THE PRE-REGISTERED PREDICTION 343 TESTS:
#   Every RUN >= 15488 must emit each bucket's whole quota as one unbroken
#   block, so all such values MUST produce BYTE-IDENTICAL shaped bins. The
#   driver compares digests for the swept values, which costs no GPU time.
#   Equal digests confirm the model; differing digests falsify it and the
#   quota reasoning must be re-derived before sweeping further.
#
# THIS SCRIPT is the ordinary single-point validate harness; its default is
# now CHUNKSHAPE148_BUCKET_RUN=2048 (342's best). The 343 experiment is the
# sweep:
#
#     bash 343_bucket_run_cap_raise2_driver.sh
#
# 343 SWEEP POINTS (default: 2048 4096 8192 16384 32768 2048):
#   2048    anchor, measured first AND last so session drift stays visible
#   4096, 8192      powers of two below the saturation point
#   16384, 32768    BOTH at or above 15488 -- this pair is the saturation
#                   test, at the timing level and at the digest level
#   Optional: add 3072 (= 1.5 x 2048, warp-aligned but not a power of two)
#   to re-test rule 2 at this magnitude: SWEEP="2048 4096 8192 16384 32768 3072 2048"
#
# ADOPTION REMAINS DEFERRED until saturation is confirmed. A best point
# pinned to an artificial ceiling is not an optimum -- that is precisely the
# error 341 and 342 each made once.
#
# MANDATORY VALIDATION ORDER:
#   1. STATIC_ONLY=1 first. Three gating checks cover the cap, including one
#      that fails if the cap does not clear BLOCK*MAX_BLOCKS.
#   2. Full N=21 run; confirm 314666222712 FIRST, before any timing.
#   3. Then the sweep via 343_bucket_run_cap_raise2_driver.sh.
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./343Py_bucket_run_cap_raise2.py}
CAND=${CAND:-./343Py_bucket_run_cap_raise2}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/343Py_bucket_run_cap_raise2_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/343Py_bucket_run_cap_raise2_logs_N21_full_once_${TS}"
...
echo "[start] 343 bucket-run-cap-raise2 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 343 bucket-run-cap-raise2 SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '343 bucket-run-cap-raise2' "$SRC"; then
  printf 'source_version_tag	343 bucket-run-cap-raise2	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	343 bucket-run-cap-raise2	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN")
{
  ...
  echo "validation: one N=21 full run through 343 bucket-run-cap-raise2 (kernel-identical to 338/337/336/335/334/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN"
...
echo "[validation-ok] 343 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); the only source change in 343 is CHUNKSHAPE148_BUCKET_RUN_MAX 8192 to 65536, which cannot affect any RUN <= 8192. This run used CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN. 342 measured RUN=2048 at 431.684s (-4.109% vs the RUN=1 control 450.183s), the best point so far. CHECK chunkshape148_cache_state BEFORE COMPARING TIMING: only reuse rows are comparable. The 343 experiment proper is the sweep -- run bash 343_bucket_run_cap_raise2_driver.sh"
