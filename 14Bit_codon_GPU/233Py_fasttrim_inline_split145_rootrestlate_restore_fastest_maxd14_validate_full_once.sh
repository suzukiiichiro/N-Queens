...
# =============================================================================
# NQ_UPDATE_MEMO
# 233 r3 validation: fasttrim-inline版。232 cleanup-onlyを親に、MAXD14 kernelは変更せず、
#                 N=21 split145 fullのhost hot path inline、旧診断関数削減、main entry復旧を検証する。
# Expected: N=21 full once, split145/split233, required=14, MAXD14, schedule_words=0, stack=208.
# =============================================================================

SRC=${SRC:-./233Py_fasttrim_inline_split145_rootrestlate_restore_fastest_maxd14_probe.py}
CAND=${CAND:-./233Py_fasttrim_inline_split145_rootrestlate_restore_fastest_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/233Py_fasttrim_inline_split145_rootrestlate_restore_fastest_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 233 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/233Py_fasttrim_inline_split145_rootrestlate_restore_fastest_maxd14_logs_N21_full_once_${TS}"
...
static_failures=0
if grep -q 'VERSION_TAG:str="233 fasttrim-inline:' "$SRC"; then
  printf 'source_version_tag\t233 fasttrim-inline\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t233 fasttrim-inline\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi

if python3 - "$SRC" <<'PYMAINENTRY'
import sys, tokenize, io
s=open(sys.argv[1],encoding='utf-8').read()
compact=''.join(s.split())
try:
    toks=[t for t in tokenize.generate_tokens(io.StringIO(s).readline)
...
def active_def(name):
    i=0
    while i+1<len(toks):
        if toks[i].type==tokenize.NAME and toks[i].string=='def' and toks[i+1].type==tokenize.NAME and toks[i+1].string==name:
            return True
        i+=1
    return False
checks={
  'main_def_present': active_def('main'),
  'main_entry_present': 'if__name__=="__main__":main()' in compact or "if__name__=='__main__':main()" in compact,
}
if all(checks.values()):
    sys.exit(0)
print('main_entry_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYMAINENTRY
then
...
compact=''.join(s.split())
checks={
  'save_sp_present':'save_sp:int=0' in compact,
  'cur_depth_present':'cur_depth:int=0' in compact,
  'packed_depth_save':'avail[save_sp]=cur_avail|(u32(cur_depth)<<u32(27))' in compact,
  'packed_depth_restore':'cur_depth=int(saved_avail>>u32(27))' in compact,
  'futuremask_guard':'iffuture_check_mask!=u32(0):' in compact,
}
if all(checks.values()):
    sys.exit(0)
print('nosibling_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_nosibling_parent\t188-style save_sp/cur_depth code\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_nosibling_parent\t188-style save_sp/cur_depth code\tmissing_or_changed\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi

if python3 - "$SRC" <<'PYCHECK'
import sys,re
s=open(sys.argv[1],encoding='utf-8').read()
start=s.find('root_rest:u32=cur_avail&(cur_avail-u32(1))')
end=s.find('while True:', start)
root=s[start:end if end>start else len(s)]
root_compact=''.join(root.split())
gen=s[end if end>start else 0:]
gen_compact=''.join(gen.split())
checks={
  'root_rest_clearlowbit': start>=0,
  'root_second_scalar':'root_second:u32=root_rest&(u32(0)-root_rest)' in root_compact,
  'root_after_scalar':'root_after_second:u32=root_rest^root_second' in root_compact,
  'root_after_if':'ifroot_after_second==u32(0):' in root_compact,
  'root_first_late':'root_first:u32=cur_avail&(u32(0)-cur_avail)' in root_compact,
  'pr_bit_alias':'pr_bit:u32=root_first' in root_compact,
  'rootpr_block_first':'ifpr_block_code!=u32(0):' in root_compact and 'else:pr_nld=(cur_ld|pr_bit)<<u32(1)' in root_compact,
  'rootrestlate_cur_avail_after_checks': root_compact.find('cur_avail=root_rest') > root_compact.find('ifpr_descend!=u32(0):pr_child_jmark:u32=child_jmark_mask&u32(1)'),
  'generic_ncol_after_diagonal': gen_compact.find('nld:u32=u32(0)') >= 0 and gen_compact.find('ncol:u32=cur_col|bit') > gen_compact.find('else:nld=(cur_ld|bit)<<u32(1)'),
  'generic_xor_clear_kept':'cur_avail=cur_avail^bit' in gen_compact,
  'no_230_placeor':'pr_placed_ld' not in s and 'pr_placed_rd' not in s,
  'no_split230':'split230' not in s,
  'no_split231':'split231' not in s,
  'no_split232':'split232' not in s,
  'split233':'split233' in s,
}
if all(checks.values()):
    sys.exit(0)
print('rootrestlate_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_217_232_restore_shape\trootrestlate_fasttrim_inline\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_217_232_restore_shape\trootrestlate_fasttrim_inline\tmissing_or_changed\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split233'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split233' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit145 split233 active\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit145 split233 active\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi


if python3 - "$SRC" <<'PYINLINE'
import sys, tokenize, io
s=open(sys.argv[1],encoding='utf-8').read()

# r2: check active code only for removed split worker symbols.
# The first 233 harness searched raw text, so an explanatory comment/docstring
# containing exec_solutions_gpu_chunk_split145(...) could make STATIC_ONLY fail
# even though the active call had been removed.
try:
    toks=[t for t in tokenize.generate_tokens(io.StringIO(s).readline)
          if t.type not in (tokenize.COMMENT, tokenize.NL, tokenize.NEWLINE, tokenize.INDENT, tokenize.DEDENT, tokenize.ENCODING)]
except Exception:
...
def active_name_call(name):
    i=0
    while i<len(toks):
        if toks[i].type==tokenize.NAME and toks[i].string==name:
            prev=toks[i-1].string if i>0 else ''
            j=i+1
            while j<len(toks) and toks[j].type in (tokenize.NL, tokenize.NEWLINE):
                j+=1
            if j<len(toks) and toks[j].string=='(' and prev!='def':
                return True
        i+=1
    return False
...
checks={
  'split_chunk_worker_removed': not active_def('exec_solutions_gpu_chunk_split145'),
  'split_chunk_call_removed': not active_name_call('exec_solutions_gpu_chunk_split145'),
  'build_soa_inline_unpack':'j:int=(ijkl>>10)&31' in s and 'k:int=(ijkl>>5)&31' in s and 'l:int=ijkl&31' in s,
  'build_soa_inline_symmetry':'rot90v:int=' in s and 'w_arr[t]=symmetry(' not in s,
  'per_record_read_inline':'inline read_uint32_le() in the per-record read path' in s,
  'old_exec_solutions_removed': not active_def('exec_solutions') and not active_def('dfs_iter') and not active_def('dfs'),
  'old_profiles_removed': not active_def('exec_solutions_gpu_bin_stream_funcid_reorder_profile') and not active_def('exec_solutions_gpu_bin_stream_funcid_reorder_funcid_markdist_profile'),
}
if all(checks.values()):
    sys.exit(0)
print('inline_trim_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYINLINE
then
  printf 'source_fasttrim_inline\thost hot-path inline + diagnostic trim\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_fasttrim_inline\thost hot-path inline + diagnostic trim\tmissing_or_changed\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if (( static_failures != 0 )); then
  echo "================================================================"; echo "[static-summary]"; cat "$SUMMARY"; echo "[logdir] $LOGDIR"
  echo "[error] 233 source static checks failed" >&2
  exit 66
fi
...
if ! grep -q '^ N:[[:space:]]*Total' "$RUN_LOG"; then
  printf 'runtime_main_output\tN table present\tmissing\tFAIL\n' >> "$SUMMARY"
  echo
  echo "================================================================"
  echo "[summary]"
  cat "$SUMMARY"
  echo "[logdir]   $LOGDIR"
  echo "================================================================"
  echo "[error] candidate exited without the N table; check source_main_entry / main() path" >&2
  exit 65
fi
...
if [[ -z "$PROGRESS" || ! -s "$PROGRESS" ]]; then
  ...
  echo "[logdir]   $LOGDIR"
  echo "================================================================"
  echo "[error] progress TSV was not produced" >&2
  exit 65
fi
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "231:$BASELINE_231_SECONDS" "217:$BASELINE_217_SECONDS" "204:$BASELINE_204_SECONDS" "230:$BASELINE_230_SECONDS"; do
...
echo "[validation-ok] 233 fasttrim-inline reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
