...
# =============================================================================
# 335 N=21 full validation harness (cudac-toolchain-locate)
# Base: 334/333/328-332 lineage (adopted SoA baseline + w3_j7 reorder
#   default; kernel logic and dispatcher byte-for-byte identical
#   throughout). 335 makes NO source change to the main solver at all --
#   kernel, dispatcher, and host reorder parameters (w3_j7, K=48) are all
#   unchanged from 334/333. The only content is an Open Objectives #6
#   update plus, in THIS harness only, two new NON-GATING environment
#   probes that advance 334's toolchain reconnaissance.
#
# 334 RESULT (now confirmed, used as this run's expected-match baseline):
#   correctness 314666222712, elapsed 450.181s (333's 450.667s, +0.108%;
#   332's direct 450.113s measurement, -0.015%; both within noise). All
#   34 static checks OK. nvcc --version confirmed CUDA 13.0 (V13.0.88);
#   cuobjdump --version NOT found via a plain PATH lookup.
#
# 335's NEW ENVIRONMENT PROBES (further design prerequisites for
# buried-idea C-2, the rev84 CUDA C runner proposal -- the sole remaining
# route to Open Objective #3 / tail effect now that plan C-1 is closed
# per 330):
#   (1) Resolve nvcc's full path via `command -v nvcc`, derive its
#       directory, and re-probe for `cuobjdump` directly inside that
#       directory (PATH-independent, since 334's plain PATH lookup came
#       up empty despite 325/327 having used cuobjdump successfully in
#       the past).
#   (2) Query `nvidia-smi --query-gpu=compute_cap` to empirically pin
#       down the A10G's actual Compute Capability (needed for the
#       -arch=sm_XX flag of any future nvcc build; the only sm_XX
#       reference in this codebase's history, sm_61, was for an older,
#       different GPU and was never verified for the A10G).
#   Both probes append INFO-only rows to summary.tsv, never fail the
#   run (unlike static source checks), and do not touch the GPU kernel
#   path at all -- pure reconnaissance for a 336+ decision on writing
#   the first concrete C-2 spike artifact. No spike code is written in 335.
#
# MANDATORY VALIDATION ORDER (unchanged discipline):
#   1. STATIC_ONLY=1 first (no new gating source checks in 335; both new
#      probes also run under STATIC_ONLY=1 since neither touches the GPU).
#   2. Full N=21 run; confirm 314666222712 FIRST, before timing.
#   3. Compare timing against 334 (450.181s) -- expect reproduction
#      within noise, since nothing in the solver changed.
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./335Py_cudac_toolchain_locate.py}
CAND=${CAND:-./335Py_cudac_toolchain_locate}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/335Py_cudac_toolchain_locate_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/335Py_cudac_toolchain_locate_logs_N21_full_once_${TS}"
...
echo "[start] 335 cudac-toolchain-locate validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 335 cudac-toolchain-locate SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '335 cudac-toolchain-locate' "$SRC"; then
  printf 'source_version_tag	335 cudac-toolchain-locate	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	335 cudac-toolchain-locate	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 335 cudac-toolchain-locate (kernel-identical to 334/333/328) mode31 split145"
...
echo "[validation-ok] 335 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); NO solver source change in 335 (kernel/dispatcher/host params identical to 334/333/328-332, w3_j7 remains default); expected elapsed ~450.2s matching 334s own measurement 450.181s -- see cudac_toolchain_probe_* rows in summary.tsv for the nvcc/cuobjdump/compute_cap environment reconnaissance (INFO only, non-gating)"
