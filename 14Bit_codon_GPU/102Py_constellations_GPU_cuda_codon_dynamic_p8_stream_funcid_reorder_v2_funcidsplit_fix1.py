...
作成しました。102Py は、101Py を土台にした **funcid split-queue 診断版**です。
101Py では `bench_mode=21` で funcid 単体 profile を行う構造になっていたので、その構造を引き継ぎ、今回は **`bench_mode=22`** を追加しました。

ソースはこちらです。

[102Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsplit.py](sandbox:/mnt/data/102Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsplit.py)

差分はこちらです。

[101_to_102_funcidsplit.patch](sandbox:/mnt/data/101_to_102_funcidsplit.patch)
...
```text
bench_mode=22
  funcid-split profile
```

目的は、現状の混在 chunk 実行と、funcid group を分離した別 kernel 実行を比較することです。

```text
all
  現状の mixed chunk baseline
...
101Py の結果では、`funcid_4 / funcid_7 / funcid_0` が heavy-tail 本命、`funcid_5` が総量最大の bulk path でした。今回の 102Py は、その結論を受けて、**分けるだけで速くなるか**を確認する診断版です。
...
codon build -release \
  102Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsplit.py
```
...
```bash
./102Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsplit \
  -g 22 22 32 484 1 0 7 22 8 7 0 94 1 "-" "all;heavy_tail;bulk_heavy;rest"
```

## 本実行

デフォルトの重い6 chunkで実行します。

```bash
./102Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsplit \
  -g 22 22 32 484 1 0 7 22
```
...
split_group_list:
  all;heavy_tail;bulk_heavy;rest
```
...
```bash
./102Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsplit \
  -g 22 22 32 484 1 0 7 22 8 7 0 0 1 \
  "94,1587,1625,1644,1772,1854" \
  "all;heavy_tail;bulk_heavy;rest"
```
...
```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_funcidsplit.tsv
```
...
```text
split_total = heavy_tail + bulk_heavy + rest
```
...
split_total >= all
  → kernel起動回数増加・低occupancyの方が悪い
     次はDFS内部の専用最適化へ進む
```

## 集計コマンド

chunkごとに `all` と `split_total` を比較します。

```bash
PROG=progress_N22_7_stream_funcid_reorder_v2_w8_j7_funcidsplit.tsv
...
END {
  printf "chunk\tall_kernel\tsplit_kernel\tdelta_kernel\tratio_kernel\tall_elapsed\tsplit_elapsed\tdelta_elapsed\n"
  for (ch in split_kernel) {
    ak=kernel[ch,"all"]
    ae=elapsed[ch,"all"]
    sk=split_kernel[ch]
    se=split_elapsed[ch]
    ratio = (ak>0 ? sk/ak : 0)
    printf "%s\t%.3f\t%.3f\t%.3f\t%.6f\t%.3f\t%.3f\t%.3f\n", \
      ch, ak/1000.0, sk/1000.0, (sk-ak)/1000.0, ratio, ae/1000.0, se/1000.0, (se-ae)/1000.0
  }
}
' "$PROG" | sort -n
```
...
./102Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsplit \
  -g 22 22 32 484 1 0 7 22 8 7 0 0 1 \
  "94,1587,1625,1644,1772,1854" \
  "all;heavy_tail;bulk_heavy;medium;light"
```
...
で十分です。これで **分離 kernel が有効かどうか**を判定できます。
...
VERSION_TAG:str="102 funcid-split auto w8_j7 stream funcid risk balanced reorder v2 GPU runner from 101"
...
def main()->None:
  ...
  bench_mode:int=0  # 0:normal, 1:N20 warmup repeat, 2:N19 preheat, 3:N18+N19 preheat, 4:N20 repeat3 sweep, 5:N20 repeat2 benchmark, 6:reorder-only debug, 7:chunk-only debug, 8:boundary-classification-only, 9:boundary-solution-summary, 10:boundary-classification-only + signature prune disabled, 11:stream-bin-build-only, 13:stream-input-stats-only, 14:funcid-reorder-v2-sim-only, 15:funcid-reorder-v2-gpu, 16:funcid-reorder-v2-sim-sweep, 17:funcid-reorder-v2-microbench, 18:funcid-reorder-v2-profile, 19:funcid-reorder-v2-chunksize-profile, 20:funcid-reorder-v2-funcid-target-profile, 21:funcid-reorder-v2-funcid-single-profile, 22:funcid-reorder-v2-funcid-split-profile
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if bench_mode<0 or (bench_mode>11 and bench_mode!=13 and bench_mode!=14 and bench_mode!=15 and bench_mode!=16 and bench_mode!=17 and bench_mode!=18 and bench_mode!=19 and bench_mode!=20 and bench_mode!=21 and bench_mode!=22):
    ...
    if bench_mode>=8:
      ...
      preset_queens_arg=5
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22:
        if argc >= 14:
  ...
  if bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and not (bench_mode==8 or bench_mode==9 or bench_mode==10 or bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22):
      ijkl_list,subconst_cache,stream_records,preset_queens,stream_fname=ensure_constellations_bin_stream(N,ijkl_list,subconst_cache,preset_queens,gpu_log_level)
