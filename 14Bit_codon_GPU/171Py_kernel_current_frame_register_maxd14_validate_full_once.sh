...
# 171 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 171 inherits the
# validated 170 scalar-u32pair MAXD14 static dispatch.  Its single kernel
# experiment keeps the 170 scalar schedule but changes the MAXD14 DFS hot loop:
# the active frame is held in scalar registers, and local u32[14] arrays are
# used only for ancestor frames during descent/backtrack.
#
# For N=21 all launches must remain required_maxd=14, select MAXD14, report
# schedule_words=0 and 224 local stack bytes/thread.

SRC=${SRC:-./171Py_kernel_current_frame_register_maxd14_probe.py}
CAND=${CAND:-./171Py_kernel_current_frame_register_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/171Py_kernel_current_frame_register_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 171 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/171Py_kernel_current_frame_register_maxd14_logs_N21_full_once_${TS}"
...
assert '171 current-frame register GPU candidate supports N=5..27 only' in s
...
# MAXD14 must have only the four state stacks, PTX-safe scalar u32 schedule fields,
# and the 171 register-held active frame.
st=s.index('@gpu.kernel\ndef kernel_dfs_iter_gpu_maxd14(')
en=s.index('\n@gpu.kernel\ndef kernel_dfs_iter_gpu_maxd16',st)
b14=s[st:en]
assert b14.count('__array__[u32](MAXD14)')==4,b14.count('__array__[u32](MAXD14)')
...
for forbidden in ('ctrl=__array__','ctrl[','root_cv','child_cv','INIT_MASK','u32(524288)','if sp>=MAXD','opcode_word:u32=packed_schedule','child_action:u32=(opcode>>','schedule_u64','schedule_bits:u64','child_jmark_mask:u64','<<u64(','>>u64(','a:u32=avail[sp]','avail[sp]=a^bit','ld[0]=root_ld','rd[0]=root_rd','col[0]=root_col','avail[0]=root_a'):
...
assert {md:stack_bytes(md) for md in word_counts}=={14:224,16:272,18:308,20:340,21:360}
...
def dfs_register(N,raw,ld0,rd0,col0,a0,j,e,m1,m2,capacity=14,limit=120000):
    bm=(1<<N)-1;n3=1<<max(0,N-3);n4=1<<max(0,N-4)
    a0&=bm
    if a0==0:return 0,0
    sched,depth=build_scalar_schedule(raw,j,e,m1,m2,capacity)
    root=scalar_root_at(sched)
    if root==2:return 1,0
    if root==3:return int((a0&~1)!=0),0
    if root==1:
        a0&=~1
        if a0==0:return 0,0
        ld0|=1
    ...
    # Mirrors 171: arrays contain ancestor frames only; the active frame is scalar.
    ld=[0]*capacity;rd=[0]*capacity;col=[0]*capacity;av=[0]*capacity
    sp=0
    cur_ld=ld0; cur_rd=rd0; cur_col=col0; cur_avail=a0
    total=0;maxframes=1;steps=0
    store_count=0;load_count=0;candidate_count=0
    while True:
        steps+=1
        if steps>limit:raise RuntimeError
        if cur_avail==0:
...
random.seed(171170)
dfs_cases=0;attempts=0;max_observed=0;max_depth_tested=0
while dfs_cases<12000 and attempts<220000:
    ...
    try:
        ...
        got,obs2=dfs_register(N,raw,ld,rd,col,a,j,e,m1,m2,14)
    except RuntimeError:
        continue
        ...
        for k in range(N):
            for l in range(N):
                ...
                if d<=21:
                    ...
                    if d==14:
                        build_scalar_schedule(f|(start<<5),jm,e,m1,m2,14)
...
print('MAXD14_CURRENT_FRAME_REGISTER=1')
...
PYMODEL
then
  printf 'source_current_frame_register_shape\trequested layout\tmodel/static failure\tFAIL\n' >> "$SUMMARY"
...
printf 'source_parent_baseline\tvalidated 170 scalar-u32pair MAXD14 dispatch\tverified\tOK\n' >> "$SUMMARY"
printf 'source_current_frame_register_shape\tMAXD14 scalar schedule + register-held current frame\tverified\tOK\n' >> "$SUMMARY"
...
printf 'maxd14_current_frame_register_schedule\t1\t%s\tOK\n' "$MAXD14_CURRENT_FRAME_REGISTER" >> "$SUMMARY"
...
printf 'dfs_164_vs_171_register_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
printf 'stack_bytes_MAXD14\t224\t%s\tOK\n' "$STACK_BYTES14" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 171 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=224 bytes/thread, current-frame register cache"
  echo "================================================================"
} | tee "$RUN_LOG"
...
record_check "dispatch_non_224_bytes" "0" "$DISPATCH_NON224" || failures=$((failures+1))
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "170:$BASELINE_170_SECONDS" "169:$BASELINE_169_SECONDS" "168:$BASELINE_168_SECONDS" "167:$BASELINE_167_SECONDS" "166:$BASELINE_166_SECONDS" "165:$BASELINE_165_SECONDS" "164:$BASELINE_164_SECONDS" "162:$BASELINE_162_SECONDS"; do
...
echo "[validation-ok] 171 current-frame register cache reproduced cases 01-06 with required=14, MAXD14, 224 bytes/thread"
