...
"""
265 schedule-precompute-reusemask note: 262 terminal_depth+terminal_base14 pack JIT成立/正当性OKを受け、5配列安全形は維持しつつMAXD14 dispatch直前に既存 ctrl0_arr / markctrl_arr を schedule_lo / sched_terminal へ再利用するprobe。root_actionはJIT危険のため別配列素通し固定。


作成しました。265は **263親 / `sched_mask` を `free_arr` に再利用するprobe** です。

264はJIT成立・正当性OKでしたが、263より遅かったため採用せず、263形へ戻しています。265では、`root_action` と `schedule_hi` は触らず、JIT安全だった構成を維持したまま、MAXD14 selected 確定後だけ以下を行います。
...
MAXD14 kernel側では、元の `free_arr` を使わず root free を再計算します。
...
狙いは、263で残っていた `sched_mask_arr` の追加global loadを、既存の `free_arr` carrierへ置き換えることです。
...
source_schedule_precompute_reusemask   OK
source_host_precompute_reusemask_arrays OK
release_build_policy                   OK
```

ファイルはこちらです。

* [265Py_schedule_precompute_reusemask_probe.py](sandbox:/mnt/data/265Py_schedule_precompute_reusemask_probe.py)
* [265Py_schedule_precompute_reusemask_validate_N21_full_once.sh](sandbox:/mnt/data/265Py_schedule_precompute_reusemask_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [265Py_schedule_precompute_reusemask_sha256.txt](sandbox:/mnt/data/265Py_schedule_precompute_reusemask_sha256.txt)
* [265Py_schedule_precompute_reusemask_bundle_20260707.tar](sandbox:/mnt/data/265Py_schedule_precompute_reusemask_bundle_20260707.tar)

sha256です。

```text
051437823e4fc38ddf5a05be58cf96b91e2d7c42bae96d5c59c5dbb95d28160a  265Py_schedule_precompute_reusemask_probe.py
5fe92bc967c60124f6c04537a12d98ec7b09fc09160870d3d6b1e33ee8da2482  265Py_schedule_precompute_reusemask_validate_N21_full_once.sh
d5dd966572559f06bf6f321385eacc500c4ed1e58a36b5b6337a676c7d4f97db  README.md
```

実行は以下でお願いします。

```bash
rm -f ./265Py_schedule_precompute_reusemask_probe

STATIC_ONLY=1 bash 265Py_schedule_precompute_reusemask_validate_N21_full_once.sh

bash 265Py_schedule_precompute_reusemask_validate_N21_full_once.sh
```

採否は、まず **JIT成立 → 正当性OK → 263の `0:07:09.417` から戻るか** で見るのがよいです。
...
VERSION_TAG:str="265 schedule-precompute-reusemask probe: parent 263 reusectrl JIT-ok / 262 terminalbasepack JIT-ok / 241 restore239; single launch kept; MAXD14 host precompute retained; ctrl0_arr is repacked to schedule_lo, markctrl_arr to terminal_depth/base14, and free_arr to child/future sched_mask only after selected_MAXD14; root free is recomputed from ld/rd/col in MAXD14 kernel; sched_hi/root_action remain separate; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 265: keep 262's safe terminal_depth+terminal_base14 pack and 244 r4 maskpack.
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 265: host-side clone of the former MAXD14 kernel schedule builder.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 265: free_arr is repacked to sched_mask for selected MAXD14 launch.
    ...
    # 265: schedule fields are precomputed on host. For MAXD14 launch,
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    # 265: selected_MAXD14 is known here, so original ctrl0/markctrl/free
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split265_reusemask_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split265_reusemask_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split265-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split265-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split265_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
