...
# StateKey=Tuple[int,int,int,int,int,int,int,int,int,int]  # (ld,rd,col,row,queens,k,l,LD,RD,N)
...
class NQueens17:
  ...
  def __init__(self)->None:
    self.use_visited_prune = False  # dynamicKのときFalse
  ...
  def exec_solutions(self,constellations:List[Dict[str,int]],N:int,use_gpu:bool)->None:
    ...
    bm = self._board_mask   # 以降は bm を使うと安全
    N1 = N-1
    N2 = N-2
    ...
    for i,constellation in enumerate(constellations):
      ...
      # free=~(ld|rd|col)
      # free は必ず board_mask で絞る（これが重要）
      free=self._board_mask&~(ld|rd|col)
...
class NQueens17_constellations():
  ...
  """プリセットクイーン数の選択。N と use_gpu に基づいて適切な値を返す。"""
  def choose_preset_queens0(self, N:int, use_gpu: bool) -> int:
    if not use_gpu:
  ...
  def build_constellations_dynamicK(self, NQ, size: int, use_gpu: bool) -> Tuple[int, List[Dict[str,int]]]:
    ...
    try:
      ...
      # 最初
      bestK = K
      bestC: List[Dict[str,int]] = []
      ijkl_list=set(); bestC=[]
      NQ.gen_constellations(ijkl_list, bestC, size, K)
      bestM = len(bestC)
      # 追加で最大2回
      for _ in range(2):
        if bestM == 0 or (not use_gpu) or (m_target > 0 and bestM >= m_target):
          break
        K += 1
        ijkl_list=set()
        constellations: List[Dict[str,int]] = []
        NQ.gen_constellations(ijkl_list, constellations, size, K)
        m = len(constellations)
        if m > bestM:
          bestM = m
          bestK = K
          bestC = constellations
      # print(f"[dynamicK] size={size} choose bestK={bestK} bestM={bestM}")
      return bestK, bestC
    finally:
  ...
  #     # ★ まず1回作って best を確定させる（Noneを使わない）
  #     ijkl_list = set()
  #     bestC: List[Dict[str,int]] = []
  #     # bestC = NQ.load_or_build_constellations_txt(ijkl_list, bestC, size, K) # テキスト版
  #     bestC = NQ.load_or_build_constellations_bin(ijkl_list, bestC, size, K) # バイナリ版
  #     bestM = len(bestC)
  #     bestK = K
  #     # print(f"size={size} preset_queens={K} m={bestM}")
  #     # ★ 追加で最大2回だけ試す（合計3回）
  #     for _ in range(2):
  #       if bestM == 0:
  #         break
  #       if (not use_gpu):
  #         break
  #       if m_target > 0 and bestM >= m_target:
  #         break
  #       K += 1
  #       ijkl_list = set()
  #       constellations: List[Dict[str,int]] = []
  #       # constellations = NQ.load_or_build_constellations_txt(ijkl_list,constellations, size, K) # テキスト版
  #       constellations = NQ.load_or_build_constellations_bin(ijkl_list, constellations, size, K) # バイナリ版
  #       m = len(constellations)
  #       # print(f"size={size} preset_queens={K} m={m}")
  #       if m > bestM:
  #         bestM = m
  #         bestK = K
  #         bestC = constellations
  ...
  #   # # （それ以上試さない）
  #   # K = 5
  #   # bestC = NQ.load_or_build_constellations_bin(set(), [], size, K)
  #   # return K, bestC
