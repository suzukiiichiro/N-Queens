...
# =============================================================================
# NQ_UPDATE_MEMO
# 256: schedule-precompute-base14pack probe。244 r4 maskpack JIT成立後の切り分け版。
#      241 restore239本線を親に、単一launchを維持し、child/future maskpackは保持する。
#      255 actionpack は正しい mode31 引数でも CUDA_ERROR_INVALID_PTX となったため不採用。
#      256では terminal_base14 だけを単独 low-bit 配列 sched_base14_arr へ移し、
#      terminal_depth と root_action は243/244 r4同様に別配列のまま保持する。
...
"""
256 schedule-precompute note: 244 r4 maskpack JIT成立形を親に、255 actionpackのPTX/JIT不成立を切り分けるため、terminal_base14だけをsched_base14_arrからdecodeし、terminal_depth/root_actionは別配列のまま保持するprobe。

255 r2で command vector は正しくなり、それでも `CUDA_ERROR_INVALID_PTX` なので、**actionpack本体がJIT不成立源**と見てよいです。

今回の切り分け結果はこうです。

```text
244 r4 maskpack:
  child_jmark_mask + future_check_mask pack
  JIT成立 / 正当性OK

255 r2 actionpack:
  terminal_base14 + root_action pack 追加
  CUDA_ERROR_INVALID_PTX

結論:
  child/future maskpack は安全
  terminal_base14/root_action 同時pack が危険
```

次は、同時packを分解して **terminal_base14 単独pack** だけを試します。
256は、`root_action` は別配列のまま残し、`terminal_base14` だけを `sched_base14_arr` からdecodeします。

```text
256:
  sched_mask_arr = child_jmark_mask | (future_check_mask << 14)
  terminal_depth_arr separate
  sched_base14_arr  = terminal_base14 only
  root_action_arr   separate
```
...
source_schedule_precompute_base14pack  OK
source_host_precompute_base14pack_arrays OK
release_build_policy                   OK
```

ファイルはこちらです。

* [256Py_schedule_precompute_base14pack_probe.py](sandbox:/mnt/data/256Py_schedule_precompute_base14pack_probe.py)
* [256Py_schedule_precompute_base14pack_validate_N21_full_once.sh](sandbox:/mnt/data/256Py_schedule_precompute_base14pack_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [256Py_schedule_precompute_base14pack_sha256.txt](sandbox:/mnt/data/256Py_schedule_precompute_base14pack_sha256.txt)
* [256Py_schedule_precompute_base14pack_bundle_20260707.tar](sandbox:/mnt/data/256Py_schedule_precompute_base14pack_bundle_20260707.tar)

sha256です。

```text
d2ed4188769c2335caf30683b70cb83b9f969fa0cf29e86a8c5456773462f4b5  256Py_schedule_precompute_base14pack_probe.py
3fcd0b5415ce170b18626b7f97dd37be93d9635b2ce36526a43ab70e6138e620  256Py_schedule_precompute_base14pack_validate_N21_full_once.sh
fe33f2275918fde27770904b9b43abd1e8df0056d2a56f5815c52e7daa0fc944  README.md
```

実行は以下でお願いします。

```bash
rm -f ./256Py_schedule_precompute_base14pack_probe

STATIC_ONLY=1 bash 256Py_schedule_precompute_base14pack_validate_N21_full_once.sh

bash 256Py_schedule_precompute_base14pack_validate_N21_full_once.sh
```

256がJIT成立すれば、次は `root_action` 単独packが原因かを切り分けます。
...
VERSION_TAG:str="256 schedule-precompute-base14pack probe: parent 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; child/future maskpack retained; 255 actionpack rejected after correct mode31 command still hit CUDA_ERROR_INVALID_PTX; only terminal_base14 decoded from sched_base14_arr while terminal_depth/root_action stay separate; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 256: 244 r4 maskpack JIT-ok shape plus terminal_base14-only decode
    # from sched_base14_arr. terminal_depth/root_action remain separate to
    # isolate the 255 actionpack PTX/JIT failure source.
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 256: host-side clone of the former MAXD14 kernel schedule builder.
  # Keeps 244 r4 maskpack and changes only terminal_base14 storage to
  # sched_base14_arr; terminal_depth/root_action remain separate.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    terminal_depth_arr:Ptr[u32],sched_base14_arr:Ptr[u32],root_action_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # low-bit array; terminal_depth/root_action remain separate.
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.terminal_depth_arr),gpu.raw(soa.sched_base14_arr),gpu.raw(soa.root_action_arr),
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split256_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split256_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      progress_suffix:str=f"split256_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
