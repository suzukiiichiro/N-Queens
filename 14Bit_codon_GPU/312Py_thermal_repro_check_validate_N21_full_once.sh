...
# =============================================================================
# 312 N=21 full validation harness (thermal-repro-check)
# Parent: 311 variant2-restore. Correctness OK, dispatch composition matched
#   304 exactly (same bucket counts, same MAXD14 selection, same
#   schedule_words/stack_bytes), yet elapsed=454.422s vs 304 K=48=351.070s,
#   delta -103.352s (-29.439%). Per-chunk breakdown was uniformly ~40% slower
#   (chunk0=167.028s, chunk1=164.792s, chunk2=121.612s vs 304's ~117s/chunk
#   average), and kernel_reduce_ms accounted for >99.9% of each chunk's
#   elapsed_ms -- i.e. the slowdown is inside GPU kernel execution time
#   itself, not host-side I/O or cache rebuilding (both
#   [split291-base-reuse] and [chunkshape148-reuse] confirmed cache hits).
# Session pattern: 309(481.149s) -> 310(476.932s) -> 311(454.422s), three
#   back-to-back GPU-saturating runs totaling ~23.5 minutes, monotonically
#   improving but still far above 304's confirmed 351.070s. This is
#   consistent with GPU thermal throttling / clock drift accumulating across
#   the session rather than a code regression, since 311's source is
#   byte-identical to 304/308's confirmed-safe configuration.
# This revision: ZERO source changes from 311. BROAD_MARKDIST_TAIL_VARIANT
#   stays 2, A10G_FINAL_DEFAULT_BROADMARK_VARIANT stays 2,
#   K_PER_THREAD_MAXD14 stays 48. This is a pure re-run to test
#   reproducibility -- if 312 recovers toward ~351s, the 309-311 slowness was
#   transient session-level thermal drift; if 312 stays near ~450s, suspect a
#   persistent hardware/environment state change instead.
# New in this script: optional GPU telemetry capture via nvidia-smi
#   (temperature/clocks/power/utilization, sampled every
#   ${TELEMETRY_INTERVAL_SECONDS}s) running in the background for the
#   duration of the full run, written to $LOGDIR/gpu_telemetry.csv. A single
#   pre-run nvidia-smi snapshot is also captured. An optional cooldown
#   (COOLDOWN_SECONDS, default 0) can be inserted before the run if the
#   previous session run was long-running; 120s is a reasonable starting
#   point if thermal drift is suspected. Telemetry capture is best-effort:
#   if nvidia-smi is not on PATH, it is skipped with a note in the log, and
#   does not fail the validation.
# Rationale for pausing kernel work: proceeding to the handoff plan's next
#   priority (Stall Branch Resolving isolation via ncu --launch-count 1
#   --set SourceCounters) while GPU clocks may be throttled would produce
#   branch-stall statistics that are not comparable to 304's baseline
#   profile. Resolving this reproducibility question comes first.
...
#           BROADMARK_VARIANT=2 (rotate_only, unchanged from 311).
#           Timing is the open question this run is meant to answer.
# EXPECTED_CHUNKS=3, K=48 (unchanged from 304).
# NOTE: this validation script has not yet been executed on real hardware as of
#   this handoff. Run STATIC_ONLY=1 first, then the full run. Consider
#   inserting a cooldown (COOLDOWN_SECONDS) before the full run if this
#   session has just finished other long GPU jobs.
# =============================================================================

SRC=${SRC:-./312Py_thermal_repro_check.py}
CAND=${CAND:-./312Py_thermal_repro_check}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/312Py_thermal_repro_check_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/312Py_thermal_repro_check_logs_N21_full_once_${TS}"
...
echo "[start] 312 thermal-repro-check validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 312 thermal-repro-check SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '312 thermal-repro-check' "$SRC"; then
  printf 'source_version_tag	312 thermal-repro-check	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	312 thermal-repro-check	missing	FAIL
...
if [[ "$CAPTURE_TELEMETRY" == "1" ]] && command -v nvidia-smi >/dev/null 2>&1; then
  nvidia-smi --query-gpu=timestamp,temperature.gpu,clocks.sm,clocks.mem,power.draw,utilization.gpu,clocks_throttle_reasons.active --format=csv > "$LOGDIR/gpu_pre_run_snapshot.csv" 2>&1 || true
...
if [[ "$CAPTURE_TELEMETRY" == "1" ]] && command -v nvidia-smi >/dev/null 2>&1; then
  nvidia-smi --query-gpu=timestamp,temperature.gpu,clocks.sm,clocks.mem,power.draw,utilization.gpu,clocks_throttle_reasons.active --format=csv -l "$TELEMETRY_INTERVAL_SECONDS" > "$TELEMETRY_LOG" 2>&1 &
  TELEMETRY_PID=$!
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 312 thermal-repro-check mode31 split145"
...
echo "[validation-ok] 312 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); see gpu_telemetry.csv for thermal/clock diagnosis"
