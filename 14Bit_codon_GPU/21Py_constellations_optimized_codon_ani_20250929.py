...
class NQueens21:
  ...
  def dfs_rd_micro(self, N:int, mask:int, ld:int, rd:int, col:int,
                   row:int, need:int, k:int, l:int, LD:int, RD:int,
                   idx:int, near_row:int) -> int:
    """RD の行マスクを毎行 rd_row として作り、直隣接の near 行で 1 回だけ
       “先回り 1 ビット” (rd_row<<1) を加える軽量版 DFS。
       非隣接では呼ばないので既存 dfs の速度を損なわない。
    """
    if need == 0:
      return 1

    # k/l 行は “既に置いてある” のでまとめてスキップ
    while row < N and (row == k or row == l):
      ld, rd, row = (ld << 1) & mask, rd >> 1, row + 1
    if row >= N:
      return 0
    ...
    # その行に適用される RD 行マスクをまず作る
    rd_row = (RD << (N - 1 - row)) & mask
    # 端のゴーストは &mask で自然に消えるので追加条件は不要

    # 直隣接の“先回り 1 ビット”は列ではなく rd_row から導く（行マスク空間で完結）
    if row == near_row:
      # RD の次行の到達点を 1 ビットだけ先回りでブロック
      rd_row |= (rd_row << 1) & mask
    # 探索禁止マスク（行依存の LD/RD をこの行位置に合わせて適用）
    # ここで (ld|rd|col) に加えて (LD >> (N-1-row)) | (RD << (N-1-row)) を足すことで、
    # “下寄り j” のケースを含め、そもそも分岐が生じない（常に衝突）行は avail=0 になり、
    # その場で自然にスキップされる。
    # → つまり「5. j が下寄り時の探索区間の限定（jmark/endmark）」が狙っていた
    #    “無意味行の早期カット”は既にこの forbid で実現済み。
    #    現在の統一 DFS では jmark/endmark を別途持ち込むと分岐・前処理が増え、
    #    実測で ~100ms 程度のオーバーヘッドになりがちなので、5 の追加は不要。
    forbid = mask & (ld | rd | col | (LD >> (N - 1 - row)) | rd_row)
    avail  = mask & ~forbid
    if not avail:
      return 0
    ...
    while avail:
      bit   = -avail & avail
      avail &= avail - 1
      total += self.dfs_rd_micro(
        N, mask,
        (ld | bit) << 1,
        (rd | bit) >> 1,
        col | bit,
        row + 1, need - 1,
        k, l, LD, RD, idx, near_row
      )
    return total
  ...
  def exec_solutions(self,constellations:List[Dict[str,int]],N:int):
    ...
    for idx,c in enumerate(constellations):
        ...
        unique_cnt: int
        # ── 3. RD 微調整：本当に必要なときだけ「1段だけ」適用 ──
        # 条件を強く絞る：
        #  - 直隣接のみ（abs(k-l)==1）
        #  - near 行から開始（start==min(k,l)）
        #  - RD の典型的な“端ゴースト”ケース（右端 l==N-1）
        if (abs(k - l) == 1) and (start == (k if k < l else l)) and (l == N - 1):
            row = start
            # k / l 行は「既に置いてある」のでスキップ（dfs と同等の前処理）
            while row < N and (row == k or row == l):
                ld, rd, row = (ld << 1) & mask, rd >> 1, row + 1
            ...
            if row >= N:
                ...
                if need > remain:
                    ...
                    if not avail:
                        ...
                        # 子はすべて「通常の dfs」に委譲（追加コストはこの 1 段だけ）
                        total = 0
                        a = avail
                        while a:
                            bit = -a & a
            ...
            # 非対象ケースは完全に元の最速経路
            unique_cnt = self.dfs(N, mask, ld, rd, col, start, need, k, l, LD, RD, idx)
        c["unique"]=unique_cnt
