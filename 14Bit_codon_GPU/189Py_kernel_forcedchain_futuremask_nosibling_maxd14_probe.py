...
# =============================================================================
# NQ_UPDATE_MEMO
# 189: 更新メモ: 188は正当性OKかつ新最速。188のfuture_check_mask guard/no-sibling spill elisionを親に、MAXD14 kernelのみ単一候補frameを連続処理するforced-chain fast pathを単独実験として導入。
...
188Pyログを確認しました。**正当性・実行結果ともに問題ありません。**
...
速度は `0:07:10.137` でした。
これは184Pyの `0:07:15.635` より `5.498秒`、`1.262%` 高速です。188Pyは**新最速基準として採用**扱いにしました。

作成物はこちらです。

* [189Py_kernel_forcedchain_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/189Py_kernel_forcedchain_futuremask_nosibling_maxd14_probe.py)
* [189Py_kernel_forcedchain_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/189Py_kernel_forcedchain_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/189Py_kernel_forcedchain_futuremask_nosibling_maxd14_bundle_20260630.tar)

189Pyは188Pyを親にしています。188の `future_check_mask` guard と no-sibling spill elision は維持し、MAXD14 kernelだけに **single-bit forced-chain fast path** を追加しました。
...
189Py:
  cur_avail が単一bitのframeでは、
  連続する単一候補frameを小さなinner loopで進める

multi-candidate frame:
  188 generic pathのまま
```

狙いは、深いforced move chainでouter loop反復と空親復元をさらに削ることです。ただし、loop複製とscalar追加によるregister pressureのリスクがあるため、189Pyは188Py親の単独実験扱いです。

手元では以下を確認済みです。

```text
bash -n 189Py...sh                         OK
STATIC_ONLY=1 189Py...sh                   OK
source_forced_chain_fastpath               OK
source_future_check_mask_guard_inherited   OK
source_nosibling_parent                    OK
source_split_tag split189 only             OK
abstract forced-chain equivalence          50000 cases OK
```

こちらではCodon/CUDA実機build/full実行はしていません。189Pyの採用可否は、添付の `189Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に188比のtimingを見て判断してください。

"""

# 189 forced-chain revision memo over validated 188 futuremask/no-sibling MAXD14:
#   Branch from the validated 188 futuremask/no-sibling build after 188 became the new fastest N=21 baseline.  Host-side depth
#   computation, scorestripe/chunkshape148 task order, cache naming, dispatch range
#   14/16/18/20/21, bitboard arithmetic, and solution arithmetic are unchanged.
#
...
#   parent depth, terminal base14 bit, and root action.  MAXD16/18/20/21 keep the
#   validated byte-opcode local schedule fallback for larger required depths.
#
#   The active DFS frame remains in scalar registers and local arrays still hold
#   only restorable ancestor frames.  189 keeps the 188 future-check guard and adds a conservative single-candidate forced-chain fast path:
#   when the active frame has exactly one candidate bit, execute consecutive no-sibling frames in a small inner loop; the existing no-sibling rule still applies after selecting a bit: if the parent has no remaining sibling candidates,
#   do not spill that parent to local memory.  Descend directly into the child
#   active frame and keep the current saved-stack height.  If the parent does
#   have siblings, save it as before, packing its logical depth into avail[] bits
#   27..31.  N<=27 leaves those high five bits free, so MAXD14 remains
#   four u32[13] arrays = 208 bytes/thread.  The supported GPU range remains N=5..27.
...
# 189 keeps the validated 188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 189 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="189 forced-chain: 188 futuremask/no-sibling MAXD14 baseline plus single-bit forced-chain fast path - README-managed history"
...
# 189 inherits the exact 188/184/183/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """189 MAXD14 nibble schedule plus no-sibling/futuremask and single-bit forced-chain fast path."""
    ...
    # 189 keeps the validated 188 current-frame register cache, 13-slot
    # ancestor storage, and no-sibling tail-call descent. It adds only a light
    # guard around the future-prune check and adds a conservative forced-chain path: schedules with future_check_mask==0
    # skip the per-candidate nibble bit3 branch entirely; single-candidate frames may run through a small inner loop. The nonzero-mask path
    # keeps the exact 184 arithmetic.
    #
    ...
    while True:
      ...
      # 189: forced-chain fast path.  When the active frame has exactly
      # one candidate bit, there is no sibling to preserve.  Run consecutive
      # single-candidate no-sibling frames in-place until the chain terminates
      # or reaches a frame with multiple candidates.  This keeps the 188
      # future_check_mask guard and the 184 save_sp/cur_depth semantics.
      if (cur_avail&(cur_avail-u32(1)))==u32(0):
        while True:
          fast_nibble_op:u32=u32(0)
          if cur_depth<8:
            fast_nibble_op=(schedule_lo>>u32(cur_depth*4))&u32(15)
          else:
            fast_nibble_op=(schedule_hi>>u32((cur_depth-8)*4))&u32(15)
          fast_block_code:u32=fast_nibble_op&u32(7)

          fast_bit:u32=cur_avail
          cur_avail=u32(0)

          fast_nld:u32=u32(0)
          fast_nrd:u32=u32(0)
          if fast_block_code!=u32(0):
            fast_stepu:u32=u32(2)+((OP_STEP3_MASK>>fast_block_code)&u32(1))
            fast_addvu:u32=(OP_ADD1_MASK>>fast_block_code)&u32(1)
            fast_bLiu:u32=(
              ((OP_BL1_MASK>>fast_block_code)&u32(1))
              |(((OP_BL2_MASK>>fast_block_code)&u32(1))<<u32(1))
            )
            fast_ktu:u32=(
              ((OP_KN3_MASK>>fast_block_code)&u32(1))
              |(((OP_KN4_MASK>>fast_block_code)&u32(1))<<u32(1))
            )
            fast_bKu:u32=(n3&(u32(0)-(fast_ktu&u32(1))))|(n4&(u32(0)-(fast_ktu>>u32(1))))
            fast_nld=((cur_ld|fast_bit)<<fast_stepu)|fast_addvu|fast_bLiu
            fast_nrd=((cur_rd|fast_bit)>>fast_stepu)|fast_bKu
          else:
            fast_nld=(cur_ld|fast_bit)<<u32(1)
            fast_nrd=(cur_rd|fast_bit)>>u32(1)
          fast_ncol:u32=cur_col|fast_bit
          fast_nf:u32=bm&~(fast_nld|fast_nrd|fast_ncol)
          if fast_nf==u32(0):
            cur_avail=u32(0)
            break
          if future_check_mask!=u32(0):
            if (fast_nibble_op&u32(8))!=u32(0):
              if (bm&~((fast_nld<<u32(1))|(fast_nrd>>u32(1))|fast_ncol))==u32(0):
                cur_avail=u32(0)
                break
          ...
          if cur_depth==terminal_depth:
            if terminal_base14==u32(0):
              ...
              total+=u64(1) if ((fast_nf&~u32(1))!=u32(0)) else u64(0)
            cur_avail=u32(0)
            break

          fast_child_jmark:u32=(child_jmark_mask>>u32(cur_depth))&u32(1)
          if fast_child_jmark!=u32(0):
            fast_nf&=~u32(1)
            if fast_nf==u32(0):
              cur_avail=u32(0)
              break
      ...
      if nf==u32(0):
        continue
      # 188/189: if no depth in this schedule carries the future-prune bit, avoid
...
CHUNKSHAPE148_DEFAULT_REASON:str="189 forced-chain reuses validated 188 scorestripe_v9 task order/cache and changes only MAXD14 single-candidate descent"
...
def main()->None:
  ...
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 189 forced-chain futuremask GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split189_forcedchain_futuremask_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split189_forcedchain_futuremask_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split189-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split189-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split189_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split189-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split189-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split189_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
