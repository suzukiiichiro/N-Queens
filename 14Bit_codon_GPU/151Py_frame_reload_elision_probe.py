...
150Pyは問題ありません。全131チャンク、部分和01〜05、最終合計`314666222712`が一致しています。実行時間は`0:16:16.988`で、149比`+0.056秒`のため速度効果は実質中立です。

### 151Py

[151Py_frame_reload_elision_probe.py](sandbox:/mnt/data/151Py_frame_reload_elision_probe.py)

ループ先頭で読み込んだ`avail[sp]`と`ctrl[sp]`を展開部まで再利用し、重複する2回目の読込みだけを除去しました。探索順序・計算式・キャッシュ形式は変更していません。

### 実行シェル

[151Py_validate_full_once.sh](sandbox:/mnt/data/151Py_validate_full_once.sh)

```bash
chmod +x 151Py_validate_full_once.sh
./151Py_validate_full_once.sh
```

全131チャンクを1回だけ実行し、そのprogress TSVから旧01〜05と全件結果を検証します。シェル構文および模擬131チャンクによる検証処理は確認済みです。この環境ではCodon/CUDA実機コンパイルは行えていません。



"""

# 151 GPU kernel experiment:
#   Retain the fully validated 150 branchless blockK selector, 149 ctrl-u32
#   decode, 148 task order, MAXD=21, five u32 DFS stacks, and nld/nrd
#   full-u32 arithmetic.  The sole kernel change is frame-state reload
#   elision: preserve the loop-head avail/ctrl locals through initialization
#   and expansion instead of rereading avail[sp] and ctrl[sp].
...
VERSION_TAG:str="151 frame-state reload elision over 150 branchless blockK - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    # 151: fixed-width operands retained for the u32 diagonal, ctrl-decode, and branchless blockK hot path.
    n3u:u32=u32(n3)
    n4u:u32=u32(n4)
    ...
    while sp>=0:
      ...
      if (cv&u32(INIT_MASK))==u32(0):
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148/149/150/151
      ...
      # 1bit 展開
      #----------------
      # 151: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
####################################################################################################
# 151 frame-state reload elision over the validated 150/149/148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 150/149/148; 151 changes only loop-local frame-state reuse inside kernel_dfs_iter_gpu
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 150/149/148 scorestripe_v9 task order/cache unchanged; 151 kernel experiment elides duplicate avail/ctrl frame reloads"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  151 reuses the validated 150/149/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 151 experiment keeps the 150 blockK path and changes only loop-local frame-state reuse inside the GPU kernel.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  151 host-side score-aware lanephase chunk shaping reused unchanged from validated 150/149/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 151 experiment changes only loop-local frame-state reuse; the 148 host-shaping cache is reused unchanged.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split151_frame_reload_elision_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split151_frame_reload_elision_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split151-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split151-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split151_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split151-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split151-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split151_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
