...
結果が明確です。293 は **367.652s vs 292確定値 367.413s**、差 −0.239s (−0.065%) — **誤差級、実質 flat**。dual-lane による Stall Wait 改善効果はゼロと判定します。

次の戦略を考えて294を組みます。293の結果が明確になりました。`colav` という u64 パックのアイデアが会話履歴にあります。これを拾います。アイデアが揃いました。**294 = 4本の u32 スタック配列 → 2本の u64 パック配列**で、local memoryの stride を 4B→8B に変えてアクセスパターンを改善する実験です。完璧。`ldrd[`/`colav[` それぞれ4回（push×2 + pop×1 = 3 ではなく push_preroll・push_second・push_main・pop で4回ずつ）。次に定数・VERSION_TAG・`DUAL_LANE_MAXD14` 定数を整理します。VERSION_TAGの文字列中にのみ残留 — 問題なし。これはコメント的な言及です。次に検証スクリプトを作成します。全項目 OK。README を更新します。## 293 結果まとめと 294 の考え方

**293 結果**: 367.652s vs 292確定値 367.413s → **差 −0.239s (−0.065%)、実質 flat、不採用**。

dual-lane が効かなかった理由として2つ考えられます。(a) Codon `-release` が2つの `while True:` を PTX レベルで直列化した、(b) 両レーンが同一の local memory アドレス空間にアクセスするため依存チェーンが交差し相殺された。どちらの場合でも、**問題は「命令ストリームの多重化」より「1回のスタック操作で何個の load/store が発生するか」**の方にある可能性が高いです。
...
| 変更 | 内容 |
|---|---|
| `ld/rd/col/avail` (4本 × u32) | → `ldrd/colav` (2本 × u64) |
| push あたりのメモリ操作 | 4 store → 2 store |
| pop あたりのメモリ操作 | 4 load → 2 load |
| per-thread footprint | 208 bytes のまま不変 (13エントリ × 16B) |

`ldrd[i] = u64(cur_ld) | (u64(cur_rd) << 64)`、`colav[i] = u64(cur_col) | (u64(cur_avail|(depth<<27)) << 64)` とするだけで、push/pop ごとの依存チェーン長が半減します。

## 今日の手順

```bash
# Step 1: 静的チェック
STATIC_ONLY=1 bash 294Py_colav_ldrd_pack_validate_N21_full_once.sh

# Step 2: 本番実行
bash 294Py_colav_ldrd_pack_validate_N21_full_once.sh
```

確認ポイント: `final total: 314666222712`、`stack_bytes_per_thread=208`(変わらないはず)、`timing_vs_292k32confirmed` の delta。結果が出たらログをお送りください。
"""
...
VERSION_TAG:str="294 colav-ldrd-pack probe: parent 293 dual-lane-maxd14 (rejected, flat vs 292); kernel_dfs_iter_gpu_maxd14 replaces 4x u32 stack arrays (ld/rd/col/avail, 208 bytes/thread) with 2x u64 packed arrays (ldrd=lo32:ld|hi32:rd, colav=lo32:col|hi32:avail|(depth<<27)), reducing push/pop from 4 u32 stores+loads to 2 u64 stores+loads per DFS stack operation, to reduce Stall Wait from local-memory dependency chains; hot-loop DFS logic, schedule decode, root-preroll, and all launch parameters are otherwise unchanged from 292/293; DUAL_LANE_MAXD14 flag removed (293 experiment concluded); kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 294: pack 4x u32 (ld/rd/col/avail) into 2x u64 (ldrd/colav) to reduce
    # local-memory accesses from 4 per push/pop to 2, targeting Stall Wait
    # reduction via fewer load/store dependency chains in the hot DFS loop.
    # ldrd[i] = lo32:ld, hi32:rd; colav[i] = lo32:col, hi32:avail|(depth<<27)
    ldrd=__array__[u64](MAXD14_ANCESTOR)
    colav=__array__[u64](MAXD14_ANCESTOR)
    bm:u32=board_mask
