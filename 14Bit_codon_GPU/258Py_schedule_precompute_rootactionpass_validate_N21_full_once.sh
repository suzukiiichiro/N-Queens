...
# =============================================================================
# 258 N=21 full validation harness
...
#             separate. Decode only root_action from low-bit sched_root_action_arr.
#             This isolates whether kernel-side root_action bitmask caused the PTX/JIT
#             failure seen in 257 root_action decode.
...
SRC=${SRC:-./258Py_schedule_precompute_rootactionpass_probe.py}
CAND=${CAND:-./258Py_schedule_precompute_rootactionpass_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/258Py_schedule_precompute_rootactionpass_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/258Py_schedule_precompute_rootactionpass_logs_N21_full_once_${TS}"
...
echo "[start] 258 schedule-precompute-rootactionpass validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 258 schedule-precompute-rootactionpass root-action-passthrough SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '258 schedule-precompute-rootactionpass probe' "$SRC"; then
  printf 'source_version_tag\t258 schedule-precompute-rootactionpass probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t258 schedule-precompute-rootactionpass probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split258_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s and 'split244_' not in s and 'split255_' not in s and 'split256_' not in s and 'split257_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split258 active; split240/241/242/243/244/255/256/257 runtime tags absent', 'split258', True))
else:
    checks.append(('source_split_tag', 'split258 active; split240/241/242/243/244/255/256/257 runtime tags absent', 'old split runtime tag present or split258 missing', False))
...
if not m14:
    ...
    ok = (
        ...
        'root_action:u32=sched_root_action_arr[i]' in body and 'root_action:u32=sched_root_action_arr[i]&u32(3)' not in body and
        'schedule_raw:u32=ctrl0_arr[i]' not in body and
        'schedule_fu:u32=' not in body and
        'sched_ctrl_arr:Ptr[u32]' not in body
    )
    actual = 'rootactionpass precompute active' if ok else 'old schedule interpreter present or rootactionpass loads missing'
    checks.append(('source_schedule_precompute_rootactionpass', 'MAXD14 precompute rootactionpass active', actual, ok))
...
checks.append(('source_host_precompute_rootactionpass_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
echo "[static-ok] 258 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 258 schedule-precompute-rootactionpass mode31 split145"
...
echo "[validation-ok] 258 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
