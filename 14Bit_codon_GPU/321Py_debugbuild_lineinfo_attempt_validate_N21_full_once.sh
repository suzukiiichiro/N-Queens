...
# =============================================================================
# 321 N=21 full validation harness (debugbuild-lineinfo-attempt)
# Parent: 320 sourceline-debug-build-probe. NOTE: 320's own N=21 full run
#   was not executed before this handoff -- the user went straight to
#   `codon build --help` to check for a debug/line-info flag, per 320's
#   header proposal. So there is no 320-specific timing baseline yet;
#   319 (455.116s, 314666222712) remains the most recent confirmed run.
#
# `codon build --help | grep -iE 'debug|line|-g\b'` result: Codon exposes
#   only a binary choice, not a middle ground like nvcc's `-lineinfo`:
#     --debug    - Turn off compiler optimizations and show backtraces
#     --release  - Turn on compiler optimizations and disable debug info
#   Everything else returned (--debug-entry-values, --debugger-tune,
#   --dwarf64, etc.) are generic pass-through LLVM backend flags with the
#   same property: they assume debug info comes paired with disabled
#   optimization, not layered on top of an optimized build.
#
# RISK flagged for this revision: a `--debug` build disables the very
#   optimizations that shaped the SASS we've been profiling since 292.
#   Loop structure, inlining, and BSYNC placement can all differ. The two
#   hotspot instructions identified in 319 (addr 0x...e100 `BRA
#   0x...d030` = 50.8% of stall_branch_resolving, addr 0x...e010 `BRA
#   0x...e0f0` = 14.9%) may not have a 1:1 counterpart in a `--debug`
#   build's SASS. The user has chosen to proceed anyway, understanding
#   this risk, to see what a line-info-carrying profile actually looks
#   like before deciding whether it is usable.
#
# This revision: ZERO changes to kernel LOGIC (variant=2, K=48
#   unchanged). Proposed next steps (NOT auto-executed by this script):
#   1. Build a SEPARATE diagnostic-only debug binary (never substituted
#      for the -release binary this script validates against):
#        codon build -debug -o 321Py_debugbuild_lineinfo_attempt_dbg \
#          321Py_debugbuild_lineinfo_attempt.py
#   2. Profile chunk 0 only (same args as every prior ncu run in this
#      investigation -- no need for a full N=21 pass, and a --debug
#      binary would be far slower per-chunk than -release):
...
#   3. Inspect 321_ncu_dbg_source.txt for two things, in this order:
#      a. Does the Source column now show real .py file:line references
#         (rather than SASS-only text)? If not, the --debug flag does not
#         solve the underlying problem either, and per-line attribution
#         is unavailable in this toolchain -- fall back to 319's
#         address-level finding as final.
#      b. If it DOES show source lines: does the debug build's control
#         flow still contain a recognizable analog of the 319 hotspot
#         (an unconditional BRA immediately preceding/following a BSYNC,
#         near the DFS loop back-edge)? If the structure looks
#         unrecognizable (heavily different branch count/shape from the
#         -release profile), treat any line numbers found with caution --
#         they describe the debug build's control flow, not necessarily
#         the -release kernel's.
#   Regardless of outcome, this validation script continues to run the
#   standard -release binary for the N=21 correctness/timing check below;
#   the debug binary and its profile are diagnostic-only and never touch
...
SRC=${SRC:-./321Py_debugbuild_lineinfo_attempt.py}
CAND=${CAND:-./321Py_debugbuild_lineinfo_attempt}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/321Py_debugbuild_lineinfo_attempt_N21_full.lock}
...
# NOTE: 320's own N=21 full run has not been executed yet (the user went
# straight to `codon build --help` after receiving 320). No fabricated
# BASELINE_320_* value is added here; 319 remains the most recent
# confirmed timing baseline until 320 (or this 321 run) produces one.
...
LOGDIR="${LOG_ROOT%/}/321Py_debugbuild_lineinfo_attempt_logs_N21_full_once_${TS}"
...
echo "[start] 321 debugbuild-lineinfo-attempt validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 321 debugbuild-lineinfo-attempt SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '321 debugbuild-lineinfo-attempt' "$SRC"; then
  printf 'source_version_tag	321 debugbuild-lineinfo-attempt	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	321 debugbuild-lineinfo-attempt	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 321 debugbuild-lineinfo-attempt mode31 split145"
...
echo "[validation-ok] 321 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); 320's 'codon build --help' check found only a binary --debug(no-opt)/--release(no-debuginfo) choice, no nvcc--lineinfo-style middle ground; this revision attempts a diagnostic-only --debug build + ncu --page source anyway (user-accepted risk that its SASS may not match the -release kernel's structure) per the header comment"
