...
# =============================================================================
# 267 N=21 full validation harness
# Parent: 266 root0probe precompute-best / 241 restore239.
# Experiment: keep 266 root0 MAXD14 variant and cache root_action_nonzero_count
#             during build_soa_for_range, avoiding a second root_action_arr scan in
#             the MAXD14 repack/dispatch loop.
...
SRC=${SRC:-./267Py_schedule_precompute_root0countcache_probe.py}
CAND=${CAND:-./267Py_schedule_precompute_root0countcache_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/267Py_schedule_precompute_root0countcache_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/267Py_schedule_precompute_root0countcache_logs_N21_full_once_${TS}"
...
echo "[start] 267 schedule-precompute-root0countcache validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 267 schedule-precompute-root0countcache ctrl0-markctrl-free-repack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '267 schedule-precompute-root0countcache' "$SRC"; then
  printf 'source_version_tag\t267 schedule-precompute-root0countcache\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t267 schedule-precompute-root0countcache\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split267_' in s and not any(x in s for x in old_splits))
if runtime_split_ok:
    checks.append(('source_split_tag', 'split267 active; old split runtime tags absent', 'split267', True))
else:
    checks.append(('source_split_tag', 'split267 active; old split runtime tags absent', 'old split runtime tag present or split267 missing', False))
...
if not m14:
    ...
    checks.append(('source_schedule_precompute_root0countcache', 'MAXD14 precompute root0countcache active', actual, ok))
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 267 schedule-precompute-root0countcache mode31 split145"
