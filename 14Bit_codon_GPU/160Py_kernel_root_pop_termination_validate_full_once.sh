...
# 160 single-pass validation harness
#
...
#
# Candidate 160 retains the validated 159 kernel-local u32 masks and every
# earlier GPU representation/DFS/order change.  Its sole kernel experiment is:
#   - replace `while sp>=0` with `while True`
#   - terminate only at root pop in candidate exhaustion, base, and jmark paths
# The MAXD overflow guard remains for the later 161 experiment.  Host
# representation, task order/cache, DFS transitions, and solution arithmetic are
# unchanged from validated 159.

SRC=${SRC:-./160Py_kernel_root_pop_termination_probe.py}
CAND=${CAND:-./160Py_kernel_root_pop_termination_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/160Py_kernel_root_pop_termination_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 159..149 full runs and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 160 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/160Py_kernel_root_pop_termination_logs_N21_full_once_${TS}"
...
if [[ -f "$SRC" ]]; then
  ...
  # 160 sole experiment: remove the per-iteration signed stack test and guard
  # all three frame-pop paths at the root.  Keep the 161 overflow guard intact.
  ...
  stack_guard_count=$(grep -Ec '^[[:space:]]*if[[:space:]]+sp>=MAXD:' "$KERNEL_SNIP" || true)
  ...
  [[ "$stack_guard_count" == "1" ]] || source_failures=$((source_failures+1))
  ...
  if (( source_failures != 0 )); then
    printf 'source_root_pop_termination_shape\trequested layout\t%d check failures\tFAIL\n' "$source_failures" >> "$SUMMARY"
    echo "[error] source does not match the requested 160 root-pop termination experiment" >&2
    exit 65
  fi
  printf 'source_inherited_u32_shape\tvalidated 159 layout\tverified\tOK\n' >> "$SUMMARY"
  printf 'source_root_pop_termination_shape\twhile True + 3 guarded pops\tverified\tOK\n' >> "$SUMMARY"
...
echo "[validation-ok] 160 one full run reproduced cases 01-06"
