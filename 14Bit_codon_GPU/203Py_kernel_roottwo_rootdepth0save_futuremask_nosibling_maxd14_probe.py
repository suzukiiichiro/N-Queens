...
# =============================================================================
# NQ_UPDATE_MEMO
# 203: 更新メモ: 202は正当性OKだが197比で遅いため不採用。197 root<=2 direct-if prerollを親に戻し、MAXD14 kernelのみroot-prerollで残りroot siblingを保存する際、cur_depth==0が既知のためavail payloadのdepth packを省く微差実験を導入。
...
"""
202Pyログを確認しました。**正当性・実行結果は問題ありません。**
...
```text
202Py  0:07:08.482
197Py  0:07:08.202  → 202が 0.280秒 / 0.065% 遅い
201Py  0:07:08.383  → 202が 0.099秒 遅い
200Py  0:07:09.520  → 202が 1.038秒 高速
199Py  0:07:08.293  → 202が 0.189秒 遅い
198Py  0:07:08.239  → 202が 0.243秒 遅い
196Py  0:07:08.239  → 202が 0.243秒 遅い
194Py  0:07:08.288  → 202が 0.194秒 遅い
```

そのため、**202Pyは正当性OKだが197Pyを上回らないため不採用**としました。
新最速基準は引き続き **197Py** です。

203Pyは202を親にせず、197Pyへ戻しています。

203Pyの差分はMAXD14 kernel内だけです。
...
203Py:
  root-prerollで残りroot候補を保存するとき、
  cur_depth==0 が既知なので、
  avail[save_sp]=cur_avail
```

generic DFS loop側の保存は従来通りです。
...
```bash
STATIC_ONLY=1 bash 203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_validate_full_once.sh
bash 203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物はこちらです。

* [203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_probe.py)
* [203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_bundle_20260701.tar)

"""

# 203 root-depth0-save memo over validated 197 root-two direct-if futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="203 roottwo-rootdepth0save: 197 root<=2 direct-if futuremask/no-sibling MAXD14 baseline with root-preroll depth0 save payload - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 203 keeps the validated 197 current-frame register cache, 13-slot
    ...
    # 203 root_depth0_save:
    # If the root frame has one or two candidates, process the first root
    # candidate before the generic DFS loop, matching the validated 197 scope.
    # 195's three-candidate case remains intentionally removed because it was
    # correct but slower.  The predicate is still the 197 second-lowbit direct-if
    # form.  203 changes only the root-preroll save payload: when a remaining
    # root sibling is saved, cur_depth is known to be 0, so the depth pack
    # `|(u32(cur_depth)<<u32(27))` is omitted for this one save.  Generic loop
    # saves keep the validated packed-depth payload.
    ...
    if root_after_second==u32(0):
      ...
      if pr_descend!=u32(0):
        if cur_avail!=u32(0):
          ...
          rd[save_sp]=cur_rd
          col[save_sp]=cur_col
          # 203: root-preroll saves a depth-0 parent; high depth bits are known zero.
          avail[save_sp]=cur_avail
          save_sp+=1
        cur_ld=pr_nld
        cur_rd=pr_nrd
...
CHUNKSHAPE148_DEFAULT_REASON:str="203 rootdepth0save reuses validated 197 scorestripe_v9 task order/cache; only MAXD14 root-preroll depth0 save payload changes"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split203_roottwo_rootdepth0save_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split203_roottwo_rootdepth0save_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split203-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split203-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split203_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split203-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split203-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split203_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
