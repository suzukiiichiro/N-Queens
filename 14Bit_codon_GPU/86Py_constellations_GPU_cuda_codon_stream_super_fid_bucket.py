...
了解です。**86を作成しました。**
85の反省点を踏まえて、**STEPSごとのfid bucketではなく、STEPS×super_factor件をまとめてからfid bucket化する superchunk版**にしています。

[86Py_constellations_GPU_cuda_codon_stream_super_fid_bucket.py](sandbox:/mnt/data/86Py_constellations_GPU_cuda_codon_stream_super_fid_bucket.py)
...
```text
86:
  1 superchunk = STEPS × super_factor
  デフォルト super_factor = 64
  つまり 15488 × 64 = 991232 records をまとめて読む
  その superchunk 全体を fid 0..27 に分割
  fidごとに同じ kernel_dfs_iter_gpu を呼ぶ
```

これで、85のような細かすぎるkernel launchを大幅に減らします。
...
```text
FID_SUPER_FACTOR_DEFAULT:int=64
exec_solutions_gpu_bin_stream_fid_super_bucket()
bench_mode=12
```
...
```text
progress_N{N}_{preset}_super_fid_bucket.tsv
fid_profile_N{N}_{preset}_super_fid_bucket.tsv
```

例：

```text
progress_N21_6_super_fid_bucket.tsv
fid_profile_N21_6_super_fid_bucket.tsv
```

## 実行コマンド

まずN21で確認してください。N24実行中に同時実行するとまたGPUを奪い合うので、速度評価は単独実行時だけ有効です。

```bash
codon build -release 86Py_constellations_GPU_cuda_codon_stream_super_fid_bucket.py
```

N21テスト：

```bash
./86Py_constellations_GPU_cuda_codon_stream_super_fid_bucket \
...
```bash
./86Py_constellations_GPU_cuda_codon_stream_super_fid_bucket \
  -g 21 21 32 484 1 0 5 12 0 0 32
```
...
```bash
./86Py_constellations_GPU_cuda_codon_stream_super_fid_bucket \
  -g 21 21 32 484 1 0 5 12 0 0 16
```

引数の意味は末尾だけ見るとこうです。

```text
12 : bench_mode = super fid bucket
0  : cross_stripe_safe unused
0  : superchunk start offset
32 : super_factor
```
...
```text
21:      314666222712                0    ...    super-fid-ok
```

N22なら：

```text
22:     2691008701644                0    ...    super-fid-ok
```

## ログの見方

`log_level=1` ではこのような行が出ます。
...
[super-fid-end] ...
[super-fid-summary] ...
[super-fid-fid-summary] ...
```

`log_level=2` ではfidごとの詳細も出ます。

```text
[super-fid] N=21 superchunk=0 off=0 fid=14 SQd2B count=... grid=... pack=... kernel=... sum=... total=...
```

fid別の集計はファイルでも確認できます。

```bash
tail -20 fid_profile_N21_6_super_fid_bucket.tsv
```
...
END {
  for (f in cnt) print f, cnt[f], total[f]
}
' fid_profile_N21_6_super_fid_bucket.tsv | sort -n
```

## 注意点

86は85よりlaunch回数を大きく減らしましたが、**84より必ず速いとはまだ断言できません**。
ただし85のように「1時間でN21の3分の1」という極端な遅さはかなり改善されるはずです。
...
を比較してください。
86がN21で84に近い、または速い場合にN22へ進むのがよいです。
...
VERSION_TAG:str="86 stream super fid bucket GPU runner from 84 dynamic preset P8"
CROSS_STRIPE_SAFE_DEFAULT:bool=False
FID_SUPER_FACTOR_DEFAULT:int=64  # bench_mode=12: superchunk = STEPS * this factor
...
"""super fid-bucket profile を TSV に追記する。"""
def append_fid_super_profile(profile_fname:str,N:int,preset_queens:int,super_index:int,off:int,fid:int,count:int,elapsed:str,fid_total:int,gpu_total:int)->None:
  with open(profile_fname,"a") as f:
    f.write(f"{N}\t{preset_queens}\t{super_index}\t{off}\t{fid}\t{fid_name(fid)}\t{count}\t{elapsed}\t{fid_total}\t{gpu_total}\n")

"""bin stream + super fid bucket 版。

85 の遅さの原因は、STEPS chunk ごとに fid 0..27 の kernel launch を行ったこと。
86 では STEPS * FID_SUPER_FACTOR_DEFAULT 件をまとめて読み込み、superchunk 単位で
fid bucket 化する。これにより kernel launch 数を大幅に減らしつつ、funcid 混在を減らす。

bench_mode=12 で使用する。
"""
def exec_solutions_gpu_bin_stream_fid_super_bucket(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  SUPER_FACTOR:int=FID_SUPER_FACTOR_DEFAULT
  # debug_chunk_count を 2 以上で渡した場合だけ super factor として使える。
  # 例: ... 12 0 0 32  => super_factor=32
  if debug_chunk_count>1:
    SUPER_FACTOR=debug_chunk_count
  if SUPER_FACTOR<1:
    SUPER_FACTOR=1

  SUPER_STEPS:int=STEPS*SUPER_FACTOR
  if SUPER_STEPS<STEPS:
    SUPER_STEPS=STEPS

  total_records:int=count_constellations_bin_records(fname)
  progress_fname:str=f"progress_N{N}_{preset_queens}_super_fid_bucket.tsv"
  profile_fname:str=f"fid_profile_N{N}_{preset_queens}_super_fid_bucket.tsv"
  with open(progress_fname,"w") as pf:
    pf.write("N\tpreset\tsuperchunk\toff\tm\tsuper_factor\tsuper_total\tgpu_total\n")
  with open(profile_fname,"w") as pf:
    pf.write("N\tpreset\tsuperchunk\toff\tfid\tfid_name\tcount\telapsed\tfid_total\tgpu_total\n")

  if gpu_log_level>=1:
    print(f"[super-fid-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} super_factor={SUPER_FACTOR} super_steps={SUPER_STEPS} sort_mode={gpu_sort_mode} fid_count=28 progress={progress_fname} profile={profile_fname}")
    if gpu_sort_mode!=0:
      print(f"[super-fid-note] N={N} sort_mode={gpu_sort_mode} is ignored in super fid bucket mode; tasks are grouped by funcid")
    if cross_stripe_safe:
      print(f"[super-fid-note] N={N} cross_stripe_safe is not used in super fid bucket mode")
  ...
  # superchunk 全体の SoA。funcid 計算用。
  soa:TaskSoA=TaskSoA(SUPER_STEPS)
  w_arr:List[u64]=[u64(0)]*SUPER_STEPS

  # fid bucket 用。最大でも SUPER_STEPS 件。
  bucket_soa:TaskSoA=TaskSoA(SUPER_STEPS)
  bucket_w_arr:List[u64]=[u64(0)]*SUPER_STEPS
  bucket_results:List[u64]=[u64(0)]*SUPER_STEPS

  counts:List[int]=[0]*28
  pos:List[int]=[0]*28
  cur:List[int]=[0]*28
  order:List[int]=[0]*SUPER_STEPS
  ...
  off:int=0
  super_index:int=0
  executed_superchunks:int=0
  _read_uint32_le=read_uint32_le

  with open(fname,"rb") as f:
    while True:
      super_constellations:List[Dict[str,int]]=[]
      i:int=0
      while i<SUPER_STEPS:
        raw=f.read(16)
        if len(raw)<16:
        ...
        col=_read_uint32_le(raw[8:12])
        startijkl=_read_uint32_le(raw[12:16])
        super_constellations.append({"ld":ld,"rd":rd,"col":col,"startijkl":startijkl,"solutions":0})
        i+=1

      m:int=len(super_constellations)
      if m==0:
        break

      # debug_chunk_start は superchunk start として使う。
      if debug_chunk_start>0 and super_index<debug_chunk_start:
        if gpu_log_level>=2:
          print(f"[super-fid-skip] N={N} superchunk={super_index} off={off} m={m}")
        off+=m
        super_index+=1
        continue

      super_t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[super-fid-start] N={N} superchunk={super_index} off={off} m={m}")

      build_soa_for_range(N,super_constellations,0,m,soa,w_arr)
      ...
      if invalid_fid>0:
        print(f"[super-fid-warning] N={N} superchunk={super_index} invalid_fid={invalid_fid}")
      ...
      for fid in range(28):
        ...
        super_total+=fid_total
        gpu_total+=fid_total
        fid_total_counts[fid]+=bm
        fid_total_solutions[fid]+=fid_total

        elapsed:str=str(fid_t3-fid_t0)[:-3]
        append_fid_super_profile(profile_fname,N,preset_queens,super_index,off,fid,bm,elapsed,fid_total,gpu_total)
        if gpu_log_level>=2:
          print(f"[super-fid] N={N} superchunk={super_index} off={off} fid={fid} {fid_name(fid)} count={bm} grid={GRID} pack={str(fid_t1-fid_t0)[:-3]} kernel={str(fid_t2-fid_t1)[:-3]} sum={str(fid_t3-fid_t2)[:-3]} total={fid_total} gpu_total={gpu_total}")

      executed_superchunks+=1
      super_t1=datetime.now()
      with open(progress_fname,"a") as pf:
        pf.write(f"{N}\t{preset_queens}\t{super_index}\t{off}\t{m}\t{SUPER_FACTOR}\t{super_total}\t{gpu_total}\n")
      if gpu_log_level>=1:
        print(f"[super-fid-end] N={N} superchunk={super_index} off={off} m={m} elapsed={str(super_t1-super_t0)[:-3]} super_total={super_total} gpu_total={gpu_total}")

      off+=m
      super_index+=1

  if gpu_log_level>=1:
    print(f"[super-fid-summary] N={N} records={total_records} superchunks={super_index} executed_superchunks={executed_superchunks} total={gpu_total} progress={progress_fname} profile={profile_fname}")
    for fid in range(28):
      if fid_total_counts[fid]>0:
        print(f"[super-fid-fid-summary] N={N} fid={fid} {fid_name(fid)} count={fid_total_counts[fid]} total={fid_total_solutions[fid]}")
...
def main()->None:
  ...
  if use_gpu:
    ...
    if bench_mode==12:
      print(f"super_fid_bucket: mode={bench_mode} preset={preset_queens_arg} super_factor={FID_SUPER_FACTOR_DEFAULT}")
  ...
  for N in range(nmin,nmax):
    ...
    #   bench_mode=12 は STEPS*super_factor 件をまとめ、funcid 別 bucket で同一 kernel を投入する。
    if bench_mode==11:
    ...
    if use_gpu and N>=21 and not (bench_mode==8 or bench_mode==9 or bench_mode==10):
      ...
      if bench_mode==12:
        total:int=exec_solutions_gpu_bin_stream_fid_super_bucket(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count)
      else:
      ...
      if bench_mode==12:
        status="super-fid-ok" if expected[N]==total else status
