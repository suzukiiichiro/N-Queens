...
# =============================================================================
# 274 N=21 full validation harness
...
SRC=${SRC:-./274Py_restore271_after_root0direct_reject_probe.py}
CAND=${CAND:-./274Py_restore271_after_root0direct_reject_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/274Py_restore271_after_root0direct_reject_N21_full.lock}
...
BASELINE_274_PARENT_271_SECONDS=${BASELINE_274_PARENT_271_SECONDS:-427.705}
...
LOGDIR="${LOG_ROOT%/}/274Py_restore271_after_root0direct_reject_logs_N21_full_once_${TS}"
...
echo "[start] 274 restore271-after-root0direct-reject validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 274 restore271-after-root0direct-reject SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '274 restore271-after-root0direct-reject' "$SRC"; then
  printf 'source_version_tag\t274 restore271-after-root0direct-reject\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t274 restore271-after-root0direct-reject\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split274' in s and 'split273' not in s and 'rootaction0-direct' not in s and 'kernel_dfs_iter_gpu_maxd14_root0' not in s and 'split270_' not in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split274 active; 273/270/240 rejected', 'split274', True))
else:
    checks.append(('source_split_tag', 'split274 active; 273/270/240 rejected', 'old/root0 split runtime present or split274 missing', False))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 274 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 274 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 274 restore271-after-root0direct-reject mode31 split145"
...
echo "[validation-ok] 274 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
