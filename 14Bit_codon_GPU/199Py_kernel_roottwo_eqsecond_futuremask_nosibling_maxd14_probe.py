...
# =============================================================================
# NQ_UPDATE_MEMO
# 199: 更新メモ: 198は正当性OKだが197比0.037秒遅いため不採用。197のroot<=2 direct-if preroll/future_check_mask guard/no-siblingを親に、MAXD14 kernelのみroot<=2判定をafter-second XOR zeroからroot_rest==root_secondの等価判定へ置き換える微差実験を導入。
...
199Py note:
  198Py was correct but did not beat the 197 baseline, so this branch returns
  to the validated 197 MAXD14 root fast-start scope.  Only post-root-action
  root_avail with one or two bits is prerolled, and 3-bit or wider roots stay
  on the generic loop.  The only new micro-experiment is the predicate shape:
  keep the second-lowbit helper, but replace after-second XOR zero with the
  equivalent root_rest == root_second direct comparison, removing one XOR
  scalar from the predicate.
...
"""
198Pyログを確認しました。**正当性・実行結果は問題ありません。**
...
```text
198Py  0:07:08.239
197Py  0:07:08.202  → 198が 0.037秒 / 0.009% 遅い
196Py  0:07:08.239  → 同タイム
194Py  0:07:08.288  → 198が 0.049秒 / 0.011% 高速
195Py  0:07:08.738  → 198が 0.499秒 / 0.116% 高速
188Py  0:07:10.137  → 198が 1.898秒 / 0.441% 高速
```

そのため、**198Pyは正当性OKだが197Pyを上回らないため不採用**としました。
新最速基準は引き続き **197Py** です。

199Pyは198を親にせず、197Pyへ戻しています。root fast-startの対象は197/196/194と同じく **root_avail 1bit/2bitのみ**です。195の3bit root preroll、198のtail-testは採用していません。

199Pyの差分はMAXD14 kernel内だけです。

```text
197Py:
  root_first        = cur_avail の先頭bit
  root_rest         = root_first除去後
  root_second       = root_rest の先頭bit
  root_after_second = root_rest ^ root_second

  if root_after_second == 0:
...
root_avail 2bit:
  root_rest が単一bit
  root_second == root_rest
  → preroll

root_avail 3bit以上:
  root_rest に2bit以上残る
  root_second はそのうち1bitだけ
  → generic loop
```
...
```text
197 second-lowbit predicate
199 root_rest==root_second predicate
equivalence OK: 65535 exhaustive + 200000 random cases
```

こちらではCodon/CUDA実機build/full実行はしていません。cudacodon側では、以下でN=21 fullのcases 01-06、dispatch、final total、warning/error、特に197/198/196比のtimingを見て採用可否を判断してください。

```bash
STATIC_ONLY=1 bash 199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_validate_full_once.sh
bash 199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物はこちらです。

* [199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_probe.py)
* [199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_bundle_20260701.tar)

"""

# 199 root<=2 eq-second memo over validated 197 root-two futuremask/no-sibling MAXD14:
#   198 was correct but 0.037 seconds slower than 197, so this branch does not
#   adopt the root_rest tail-test predicate.  It returns to 197 as the numerical
#   baseline and keeps the validated root one/two-candidate preroll, 188
#   future_check_mask guard, and 184 no-sibling tailcall build.
...
#   only restorable ancestor frames.  199 keeps the narrow root fast-start only when
#   post-root-action root availability has one or two bits, matching the validated
#   197/196/194 behavior.  Three-candidate roots explicitly stay on the generic loop.
#   The root<=2 predicate changes from after-second XOR zero to the equivalent
#   root_rest==root_second comparison.  For a 1-bit root root_rest==root_second==0;
#   for a 2-bit root root_rest is exactly root_second; for 3-bit or wider roots
#   root_rest has more bits than root_second and the generic loop is used.
...
# 199 keeps the validated 197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 199 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="199 roottwo-eqsecond: 197 root<=2 direct-if futuremask/no-sibling MAXD14 baseline with root_rest==root_second predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """199 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate eq-second preroll predicate."""
    ...
    # 199 keeps the validated 197 current-frame register cache, 13-slot
    ...
    # 199 root_two_eqsecond:
    # If the root frame has one or two candidates, process the first root
    # candidate before the generic DFS loop, matching the validated 197/196/194 scope.
    # 195's three-candidate case remains intentionally removed because it was
    # correct but slower.  This keeps the second-lowbit helper but replaces the
    # 197 after-second XOR zero test with the equivalent root_rest==root_second
    # comparison. Therefore 3-bit and wider roots stay on the generic loop.
...
CHUNKSHAPE148_DEFAULT_REASON:str="199 roottwo-eqsecond reuses validated 197/196/194 scorestripe_v9 task order/cache; only MAXD14 root<=2 predicate comparison shape changes"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split199_roottwo_eqsecond_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split199_roottwo_eqsecond_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split199-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split199-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split199_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split199-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split199-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split199_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
