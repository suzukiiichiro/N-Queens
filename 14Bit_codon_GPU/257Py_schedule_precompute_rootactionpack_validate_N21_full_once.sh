...
# =============================================================================
# 257 N=21 full validation harness
...
#             This isolates whether root_action shifted decode is the PTX/JIT
#             failure source seen in 255 actionpack.
...
SRC=${SRC:-./257Py_schedule_precompute_rootactionpack_probe.py}
CAND=${CAND:-./257Py_schedule_precompute_rootactionpack_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/257Py_schedule_precompute_rootactionpack_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/257Py_schedule_precompute_rootactionpack_logs_N21_full_once_${TS}"
...
echo "[start] 257 schedule-precompute-rootactionpack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 257 schedule-precompute-rootactionpack root-action-only SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '257 schedule-precompute-rootactionpack probe' "$SRC"; then
  printf 'source_version_tag\t257 schedule-precompute-rootactionpack probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t257 schedule-precompute-rootactionpack probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split257_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s and 'split244_' not in s and 'split255_' not in s and 'split256_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split257 active; split240/241/242/243/244/255/256 runtime tags absent', 'split257', True))
else:
    checks.append(('source_split_tag', 'split257 active; split240/241/242/243/244/255/256 runtime tags absent', 'old split runtime tag present or split257 missing', False))
...
if not m14:
    ...
    actual = 'rootactionpack precompute active' if ok else 'old schedule interpreter present or rootactionpack loads missing'
    checks.append(('source_schedule_precompute_rootactionpack', 'MAXD14 precompute rootactionpack active', actual, ok))
...
checks.append(('source_host_precompute_rootactionpack_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
echo "[static-ok] 257 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 257 schedule-precompute-rootactionpack mode31 split145"
...
echo "[validation-ok] 257 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
