...
SRC=${SRC:-./347Py_serpentine_probe.py}
CAND=${CAND:-./347Py_serpentine_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/347Py_serpentine_probe_N21_full.lock}
...
# 347 ADDS: 5 = mode 1 plus serpentine (the candidate), 6 = mode 2 plus
# serpentine. Both build their own shaped bin on first use.
...
LOGDIR="${LOG_ROOT%/}/347Py_serpentine_probe_logs_N21_full_once_${TS}"
...
echo "[start] 347 serpentine validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 347 serpentine SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '347 serpentine' "$SRC"; then
  printf 'source_version_tag	347 serpentine	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	347 serpentine	missing	FAIL
...
record_check source_chunkshape148_iter_sort_max 6 "${IS_MAX:-missing}" || static_failures=$((static_failures+1))
...
if grep -q 'if (mode==5 or mode==6) and iter_len>=1:' "$SRC_NODOC" \
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 347 source static checks failed" >&2
  exit 66
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 347 serpentine (kernel-identical to 346/345/344/343/338/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT; 347 adds modes 5 and 6, the serpentine layouts, and leaves the 346 default of 1 untouched. Mode 1 should reproduce the 346 measurement of 402.258s with cache reuse; mode 5 is the candidate and is predicted to beat it by roughly 1 percent; mode 6 is predicted to match mode 5 and to shrink or remove the unexplained chunk2 direction dependence seen in 345 under mode 2. To reach the 344 baseline at any time, run with CHUNKSHAPE148_ITER_SORT=0"
