...
# =============================================================================
# 307 N=21 full validation harness (K44-fine-probe)
# Parent: 306 K56-sweep (351.534s, -0.464s vs 304 K48=351.070s, flat/noise).
#   K survey complete: K=32(353.671s) K=40(353.587s) K=48(351.070s BEST)
#   K=56(351.534s flat) K=64(353.896s). Optimum zone is K=48-56 (flat).
#   Fine-probing K=44 (between the K=40 descent and K=48 peak) to
#   check whether the optimum starts exactly at K=48 or between 44-48.
#   EXPECTED_CHUNKS=ceil(2025282/(32*484*44))=ceil(2025282/681472)=3.
#   All kernel logic unchanged from 296/304.
# Safety: K_PER_THREAD_MAXD14=44, EXPECTED_CHUNKS=3.
# NOTE: this validation script has not yet been executed on real hardware.
# =============================================================================

SRC=${SRC:-./307Py_K44_fine_probe.py}
CAND=${CAND:-./307Py_K44_fine_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/307Py_K44_fine_probe_N21_full.lock}
...
EXPECTED_K_PER_THREAD_MAXD14=44
...
LOGDIR="${LOG_ROOT%/}/307Py_K44_fine_logs_N21_full_once_${TS}"
...
echo "[start] 307 K44-fine-probe validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 307 K44-fine-probe SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '307 K44-fine-probe' "$SRC"; then
  printf 'source_version_tag	307 K44-fine-probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	307 K44-fine-probe	missing	FAIL
...
# 304: K48-sweep shape checks (296 kernel logic, K=48).
    ...
    checks.append(('source_K44_fine_probe_shape',
                    '296 kernel shape (stack/save_sp/next_depth/cur_depth/stack_ptr) + K=44',
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 307 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 307 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 307 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 307 K44-fine-probe mode31 split145"
...
echo "[validation-ok] 307 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=44 (3 dispatch/progress rows)"
