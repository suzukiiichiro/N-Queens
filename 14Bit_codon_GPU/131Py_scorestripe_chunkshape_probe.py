...
[131Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/131Py_scorestripe_chunkshape_probe.py)
[131Py1-6.sh](sandbox:/mnt/data/131Py1-6.sh)
...
130Py:
  ...
  special kernel 追加なし
  mode30 / mode31 generic-only harness 継承
  chunkshape131_scorestripe_v2 を追加

129 の問題:
  score bucket が b7 に全寄せ

130 の対策:
  絶対 threshold 方式をやめる
  score key histogram から percentile threshold を作る
  b0..b7 に分布するように host-side bucket 分類を修正
```

`131Py1-6.sh` は初回検証用なので、129 と同じく mode30 の `partial_total` は固定値にしていません。まず `partial_total=` の出力確認、worker 0/4 の `expected_total=314666222712` 確認、full の `314666222712.*ok` 確認にしています。

実行は以下です。

```bash
codon build -release 131Py_scorestripe_chunkshape_probe.py
./131Py1-6.sh
```

長い full run を省く場合は以下です。

```bash
RUN_FULL=0 ./131Py1-6.sh
```

確認していただきたいログ行は、特にこの 2 つです。

```text
[chunkshape131-score-key] ...
[chunkshape131-score-buckets] ... b0=... b1=... ... b7=...
```

ここで 129 のように `b7=2025282` 全寄せになっていなければ、130 の bucket 退化修正はまず効いています。こちらでは `131Py1-6.sh` の bash 構文確認と、130 側の主要シンボル整合確認まで行っています。


2026-06-15 08:48:56 作成の cand131 ログを確認しました。結論は、**130Py は correctness PASS、generic-only 制約 PASS、かつ 128/129 より明確に速いので、130 を current candidate baseline に昇格**でよいです。

130 full correctness は、`records=2025282`、`chunks=131`、`executed_chunks=131` を全処理し、`source_records=2025282`、`base_reordered_records=2025282`、`shaped_records=2025282` を保ったまま、最終 `total=314666222712`、`0:21:10.100 ok` で完走しています。

generic-only 条件も通っています。ログ上は `version : 130 scorestripe chunkshape probe - mode30/mode31 generic kernel, host-side percentile score-aware shaping`、`chunkshape131_params: version=scorestripe_v2 ... generic kernel unchanged` で、実行 bucket も `generic=15488 ... split_mode=0 specialized=0` です。

129 の問題だった score bucket 全寄せも解消されています。129 は `b0=0 ... b7=2025282` でしたが、130 は `b0=253565 b1=253486 b2=253336 b3=254326 b4=252093 b5=252292 b6=253809 b7=252375` で、ほぼ 8 等分に分散しています。
...
130 は 128 初回より **30.145 秒速く、約 2.32% 改善**、129 より **30.096 秒速く、約 2.31% 改善**です。127 比では **2分54.644秒速く、約 12.09% 改善**です。128 の full は `0:21:40.245 ok`、130 の full は `0:21:10.100 ok` なので、今回は実行揺れではなく、`scorestripe_v2` の改善として扱ってよい差です。

mode30 / worker の locked 値は以下で確定できます。
...
01〜04 の selected chunk はそれぞれ `split131-probe` として正常終了しています。 worker 0/4 も `partial_total=79323742766`、`expected_total=314666222712`、`0:05:23.119` で正常です。
...
130 generic-only constraint:
  PASS

130 permutation/count safety:
  PASS
...
baseline decision:
  130 を current candidate baseline に昇格
  128 は previous stable candidate fallback
  129 は correctness-passing だが superseded probe
```

あわせて、今回の確定値で locked script と記録用メモを作成しました。

[131Py1-6_locked.sh](sandbox:/mnt/data/131Py1-6_locked.sh)
[137_130_full_result_memo.txt](sandbox:/mnt/data/137_130_full_result_memo.txt)

はい、**実行したほうが良い**です。

理由は、今回の `131Py1-6.sh` 初回検証ではすでに **correctness PASS / generic-only PASS / 128 より約30秒改善**まで確認できていますが、`131Py1-6_locked.sh` はその次段階の **再現性確認**だからです。130 full は `total=314666222712`、`0:21:10.100 ok` で完走しており、`source_records=base_reordered_records=shaped_records=2025282` も保っています。

ただし目的は「新しい correctness 探索」ではなく、以下の固定値 regression です。

```text
131Py1-6_locked.sh の目的:
  1. mode30 selected chunks の partial_total が再現するか
  2. worker 0/4 の partial_total が再現するか
  3. full total=314666222712 ok が再現するか
  4. elapsed が 0:21:10 前後で安定するか
```
...
130 の `scorestripe_v2` は、129 の `b7=2025282` 全寄せを解消して、bucket がほぼ 8 等分に分散していました。ここは 130 採用判断の重要点なので、locked script で再度通す価値があります。

実行順は以下でよいです。

```bash
chmod +x 131Py1-6_locked.sh
./131Py1-6_locked.sh
```

時間を節約したい場合でも、今回は **full まで実行推奨**です。130 は current candidate baseline に昇格させる候補なので、`locked` の full 再現ログを 1 本残しておくと、次の 131/132 で比較基準がぶれません。

判定はこのように見ます。

```text
全 case OK:
  130 を current candidate baseline として固定

partial は OK、full だけ elapsed が数秒揺れる:
  問題なし
  total=314666222712 ok なら PASS

partial_total が 1つでも不一致:
  要確認
  chunkshape / 入力 cache / 実行 binary の取り違えを疑う
...
したがって、**はい、131Py1-6_locked.sh は実行してください。** 結果ログが出たら、前回 130 初回ログとの差分まで見て、130 baseline 固定でよいか最終確認します。

確認しました。**129Py_locked.tar / 130Py_locked.tar ともに locked validation は PASS** です。

特に重要な 130 locked は、6 ケースすべて `OK / exit_code=0` でした。
...
130 の full は、`records=2025282`、`chunks=131`、`executed_chunks=131`、`total=314666222712` で correctness を満たす構成です。130 側の同名ログでも `chunkshape=scorestripe_v2`、`source_records=2025282`、`base_reordered_records=2025282`、`shaped_records=2025282`、`total=314666222712` が確認できます。

generic-only 条件も問題ありません。130 full のログ冒頭は `host-side percentile score-aware shaping` で、`chunkshape131_params: version=scorestripe_v2 ... generic kernel unchanged`、実行 bucket も `split_mode=0 specialized=0` です。

129 locked も 6 ケースすべて OK でした。

```text
129Py_locked:

01 standard 7chunk:
  partial_total=16770333494
  elapsed=0:01:10.865
  OK

02 heavy band:
  partial_total=19239439100
  elapsed=0:01:20.195
  OK

03 d2base14-density:
  partial_total=16823602934
  elapsed=0:01:09.678
  OK

04 late/tail:
  partial_total=16316779188
  elapsed=0:01:08.595
  OK

05 worker 0/4:
  partial_total=79339432716
  elapsed=0:05:29.045
  OK
...
比較すると、今回の locked tar 同士では **130 が 129 より 29.745 秒速く、約 2.29% 短縮**です。
...
前回の 130 初回 full は `0:21:10.100` でしたが、今回の locked full は `0:21:10.758` なので、差は **+0.658 秒**だけです。これは再現性として問題ありません。前回の 128 locked full は `0:21:40.293`、129 full は `0:21:40.196` だったため、130 の優位は維持されています。

判定はこれで確定でよいです。

```text
129 locked validation:
  PASS
  ただし 130 に superseded

130 locked validation:
  PASS

130 correctness:
  PASS

130 fixed partial_total regression:
  PASS
...
130 reproducibility:
  PASS
  初回 0:21:10.100 → locked 0:21:10.758
  差 +0.658 秒のみ

baseline decision:
  130 を current candidate baseline として固定
  128 は previous stable fallback
  129 は correctness-passing experimental branch
  127 は pre-chunkshape fallback
  115 は production fallback
```

記録用メモも作成しました。

[138_129_130_locked_tar_review_memo.txt](sandbox:/mnt/data/138_129_130_locked_tar_review_memo.txt)
...
VERSION_TAG:str="131 scorestripe chunkshape probe - mode30/mode31 generic kernel, 130 baseline carry-forward, host-side percentile score-aware shaping"
...
####################################################################################################
# 131 reorder chunkshape harness
#
...
####################################################################################################
# 131 score-aware chunk-shape layer
# 131 is an isolated-cache carry-forward of the validated 130 scorestripe_v2 algorithm.
#
# 127 proved that the split/candidate harness has essentially zero full-run overhead when all tasks
# stay on the proven 115 generic kernel. 128 proved that host-side cross-chunk shaping can improve
# N21 while preserving correctness. 130 keeps that GPU/kernel path intact and changes only the
# second-stage permutation after the 114/115 broadmarktail reorder bin has been built.
#
# scorestripe_v2 is a host-side permutation over 16-byte constellation records:
#   - every input record is emitted exactly once
#   - no solution/kernel state is changed
#   - records are first classified into coarse static work buckets from build_soa_for_range()
#   - each output chunk receives a proportional heavy/light bucket mixture
#   - within each bucket, records retain the proven 128 cross-stripe traversal order
####################################################################################################

CHUNKSHAPE131_REORDER_VERSION:str="scorestripe_v2"
CHUNKSHAPE131_DEFAULT_REASON:str="host-side percentile score-aware quota stripe over broadmarktail v4; generic kernel unchanged"

def chunkshape131_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape131_{CHUNKSHAPE131_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}.bin"

CHUNKSHAPE131_SCORE_KEY_MAX:int=32767


def chunkshape131_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  130 score key for host-side shaping only.

  129 used absolute score thresholds and all N21 records landed in bucket 7.
  130 keeps the same safety boundary but changes the classification to a percentile-friendly
  score key:
    - raw component: funcid risk + free-popcount + remaining depth + mark distances
    - tie component: deterministic 0..31 lane from global index / ijkl / fid
  ...
  # Coarse static cost estimate.  It intentionally avoids a single pc*depth product threshold
  # because N21/preset7 made that product too large and collapsed 129 into one bucket.
  raw:int=0
  raw+=pc*12
  raw+=depth*7
  raw+=row_to_end*3
  ...
  # Deterministic tie lane.  This is important when raw scores are very narrow: it lets the
  # histogram percentile split still create multiple buckets while preserving a stable permutation.
  tie:int=(global_idx*13 + soa.ijkl_arr[idx]*7 + fid*5 + pc*3 + depth) & 31
  key:int=raw*32 + tie
  if key<0:
    key=0
  if key>CHUNKSHAPE131_SCORE_KEY_MAX:
    key=CHUNKSHAPE131_SCORE_KEY_MAX
  return key


def chunkshape131_build_thresholds_from_hist(hist:List[int],total_records:int)->List[int]:
  """Build 8 percentile thresholds from the score-key histogram."""
  thresholds:List[int]=[0]*8
  if total_records<=0:
    thresholds[7]=CHUNKSHAPE131_SCORE_KEY_MAX
    return thresholds

  b:int=0
  cum:int=0
  k:int=0
  while k<=CHUNKSHAPE131_SCORE_KEY_MAX:
    cum+=hist[k]
    while b<7:
  ...
  while b<7:
    thresholds[b]=CHUNKSHAPE131_SCORE_KEY_MAX
    b+=1
  thresholds[7]=CHUNKSHAPE131_SCORE_KEY_MAX
  return thresholds


def chunkshape131_bucket_from_key(key:int,thresholds:List[int])->int:
...
def chunkshape131_make_quotas(rem_counts:List[int],total_remaining:int,m_target:int)->List[int]:
...
def build_chunkshape131_reordered_bin(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  """
  130 host-side score-aware chunk shaping.

  Input:
    fname is normally the already-built broadmarktail reorder bin.

  Output:
    a second bin whose records are a permutation of fname.  The builder first assigns each
    record to a coarse score bucket using funcid/free-popcount/depth features from the existing
    build_soa_for_range() host logic, then writes every output chunk with approximately the same
    score-bucket mixture.  Within each bucket, records retain the 128 cross-stripe order.
  ...
  out_fname:str=chunkshape131_reorder_output_fname(N,preset_queens,BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_chunkshape131_{CHUNKSHAPE131_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)}_sim.tsv"

  if gpu_log_level>=1:
    print(f"[chunkshape131-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE131_DEFAULT_REASON}")

  with open(progress_fname,"w") as pf:
    pf.write("N\tpreset\tchunk\tout_records\ttotal_records\telapsed\telapsed_ms\tb0\tb1\tb2\tb3\tb4\tb5\tb6\tb7\n")
  ...
  if len(data)<expected_bytes:
    print(f"[chunkshape131-warning] source byte count shorter than expected: bytes={len(data)} expected={expected_bytes}")
  ...
  key_hist:List[int]=[0]*(CHUNKSHAPE131_SCORE_KEY_MAX+1)
  off:int=0
  classify_chunk:int=0
  key_min:int=CHUNKSHAPE131_SCORE_KEY_MAX
  key_max:int=0
  while off<total_records:
    ...
    i=0
    while i<m:
      key:int=chunkshape131_score_key_from_soa(soa,i,off+i)
      if key<0:
        key=0
      if key>CHUNKSHAPE131_SCORE_KEY_MAX:
        key=CHUNKSHAPE131_SCORE_KEY_MAX
    ...
    if gpu_log_level>=2:
      print(f"[chunkshape131-classify-chunk] N={N} chunk={classify_chunk} off={off} m={m}")
    off+=m
    classify_chunk+=1

  score_thresholds:List[int]=chunkshape131_build_thresholds_from_hist(key_hist,total_records)
  bucket_counts:List[int]=[0]*8
  idx_count:int=0
  while idx_count<total_records:
    b0:int=chunkshape131_bucket_from_key(score_key_by_idx[idx_count],score_thresholds)
    bucket_counts[b0]+=1
    idx_count+=1

  if gpu_log_level>=1:
    print(f"[chunkshape131-score-key] N={N} records={total_records} key_min={key_min} key_max={key_max} t0={score_thresholds[0]} t1={score_thresholds[1]} t2={score_thresholds[2]} t3={score_thresholds[3]} t4={score_thresholds[4]} t5={score_thresholds[5]} t6={score_thresholds[6]}")
    print(f"[chunkshape131-score-buckets] N={N} records={total_records} b0={bucket_counts[0]} b1={bucket_counts[1]} b2={bucket_counts[2]} b3={bucket_counts[3]} b4={bucket_counts[4]} b5={bucket_counts[5]} b6={bucket_counts[6]} b7={bucket_counts[7]}")

  # ------------------------------------------------------------------
  # Stage 2: keep the proven 128 cross-stripe order inside each score bucket.
  # ------------------------------------------------------------------
  bucket_indices:List[List[int]]=[]
  b:int=0
  while b<8:
    one:List[int]=[]
    bucket_indices.append(one)
    b+=1
      ...
      while src_ch<n_chunks:
        ...
        if within<STEPS:
          ...
          if idx<total_records:
            b=chunkshape131_bucket_from_key(score_key_by_idx[idx],score_thresholds)
  ...
  if staged_total!=total_records:
    print(f"[chunkshape131-warning] staged count mismatch: staged={staged_total} source={total_records}")

  # ------------------------------------------------------------------
  # Stage 3: write output chunks with proportional score-bucket quotas.
  # ------------------------------------------------------------------
  bucket_pos:List[int]=[0]*8
  bucket_rem:List[int]=[0]*8
  b=0
  while b<8:
    bucket_rem[b]=len(bucket_indices[b])
    b+=1

  # Heavy/low interleave.  Rotate by out_ch to avoid identical intra-chunk phase.
  ...
  with open(out_fname,"ab") as out:
    while out_ch<n_chunks:
      ...
      quotas:List[int]=chunkshape131_make_quotas(bucket_rem,total_remaining,m_target)
      ...
      while written_chunk<m_target:
        ...
        while oi<8:
          ...
          if quotas[b]>0:
            idx:int=bucket_indices[b][bucket_pos[b]]
            p:int=idx*16
            out.write(data[p:p+16])
            bucket_pos[b]+=1
            bucket_rem[b]-=1
            quotas[b]-=1
      ...
      with open(progress_fname,"a") as pf:
        pf.write(f"{N}\t{preset_queens}\t{out_ch}\t{written_chunk}\t{total_records}\t{elapsed_text}\t{elapsed_ms}\t{written_by_bucket[0]}\t{written_by_bucket[1]}\t{written_by_bucket[2]}\t{written_by_bucket[3]}\t{written_by_bucket[4]}\t{written_by_bucket[5]}\t{written_by_bucket[6]}\t{written_by_bucket[7]}\n")
      if gpu_log_level>=2:
        print(f"[chunkshape131-build-chunk] N={N} chunk={out_ch} written={written_chunk} total_written={written_total} b0={written_by_bucket[0]} b1={written_by_bucket[1]} b2={written_by_bucket[2]} b3={written_by_bucket[3]} b4={written_by_bucket[4]} b5={written_by_bucket[5]} b6={written_by_bucket[6]} b7={written_by_bucket[7]}")
      out_ch+=1
  ...
  if written_total!=total_records:
    print(f"[chunkshape131-warning] record count mismatch: written={written_total} source={total_records}")
  if reordered_records!=written_total:
    print(f"[chunkshape131-warning] output count mismatch: file_records={reordered_records} written={written_total}")
  if gpu_log_level>=1:
    print(f"[chunkshape131-build-summary] N={N} records={reordered_records} chunks={n_chunks} output={out_fname} progress={progress_fname} valid={1 if validate_bin_file(out_fname) else 0}")
...
def exec_solutions_gpu_chunk_split131(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  if gpu_log_level>=1:
    print(f"[split131-buckets] N={N} m={m} generic={m} d2base14={d2base14_m} d0={d0_m} rest=0 split_mode={split_mode} specialized=0")
...
def exec_solutions_gpu_bin_stream_split131(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split131_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split131_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split131_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split131_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split131-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split131-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split131-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split131-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split131(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split131-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split131-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split131 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split131 full run / optional multi-GPU worker
      # Examples:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split131-generic-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split131-generic worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split131_generic_harness_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split131_generic_harness_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  if gpu_log_level>=1 and (bench_mode==30 or bench_mode==31):
    print(f"chunkshape131_params: version={CHUNKSHAPE131_REORDER_VERSION} reason={CHUNKSHAPE131_DEFAULT_REASON}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split131-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split131-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      shaped_fname:str=chunkshape131_reorder_output_fname(N,preset_queens,gpu_block,gpu_max_blocks)
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape131-probe-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape131-probe-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape131_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split131_probe_{CHUNKSHAPE131_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split131(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split131-probe-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE131_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split131-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split131-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split131-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape131-full-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape131-full-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape131_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split131_full_{CHUNKSHAPE131_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split131(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
