...
"""
**357の設計です。** 356は`save_sp`のビット幅を狭めて3チャンク一貫
改善(−0.83%〜−2.00%)を得て採用されたが、355が特定した
`branch_resolving`60.2%の主犯——BSYNC隣接の2本の無条件BRA(スタック
プッシュ直後の再収束ジャンプと、ポップループ脱出直後の再収束
ジャンプ)——自体には触れていない。357は**コードを一切変更せず**、
356採用後の状態でncuを取り直し、この2命令の実測値が356後どう
動いた(あるいは動いていない)かを確認する、355と同型の計測専用
リビジョンである。

## 355との違い: 手続き上の1点のみ

355はN=21側の動的計測に`bench_mode=31`(フルgpu)+`--launch-count 1`
を使い、計装対象は1launchに絞れても**プログラム全体が最後まで
実行され**(実測24分、通常398秒の約3.5倍)、時間を浪費した。357は
`bench_mode=30`(split145 probe/cache-build。`debug_chunk_start`/
`debug_chunk_count`引数でチャンク境界に区切れる)を使い、chunk0
(`debug_chunk_start=0 debug_chunk_count=1`)だけを実行させて止める。
N=18側(`SourceCounters --page source`、約9秒)の手順は355と同一。

## なぜコードを変えないのか

356適用後に何がどう変わった/変わっていないかを実測せずに次の
介入(BRA/BSYNC再収束構造そのものへの手入れ)を設計するのは、
355以前に戻って推測ベースで進めることになる。355の教訓——
「waitの単独最大寄与命令を狙う」という具体的仮説が実測で
裏付けられた——を踏まえ、次の一手も実測を先に置く。

## このリビジョンにオラクルはない

355と同じ理由。コードを変えていないので、正解値も壁時計比較も
存在しない。**唯一の正しさの基準は、356とのコード差分がゼロ
(VERSION_TAGを除く)であること**であり、`.sh`はこれを実行前に
静的に証明する(`source_code_identical_to_356`、docstring剥離+
VERSION_TAG行除外後のsha256指紋照合)。chunk0のみを実行するため
`314666222712`という全体正当性オラクルもこの回では確認できない
(部分実行であり、確認すべきは「382で確立した既知のストール内訳
に対して356後の実測がどう動くか」のみ)。

## ncu出力を自動でパースしない

355で確立した方針を継続する。出力フォーマットへの依存を「知って
いるつもり」で書くのは345 r1・349 r1・354で実際に踏んだ「地の文を
コードと誤認する」失敗と同種の危険を持つ。`.sh`はセクション見出し
の存在だけを緩く確認し、**生ログを丸ごと保存する**。数値の読み取り
と解釈は、実機で取れたログを見てから行う。
...
**権限確認を最初に置く。** `sudo -n true`が通らない場合、ビルドも
ncuも一切実行せずにそこで止まる(352で14分を無駄にした教訓)。

## 357の後はどこから始めるか

357の結果次第で分岐する。`branch_resolving`比率が356後も実質
変わっていなければ、2命令自体(BRA/BSYNC再収束構造)への設計に
...
VERSION_TAG:str="357 branch-resolving-recheck-ncu: NO CODE CHANGE, SECOND CONSECUTIVE MEASUREMENT-ONLY REVISION AFTER 356. Kernel is byte identical to 356 (save_sp remains u32 at all four sites); only this VERSION_TAG line differs, verified by source_code_identical_to_356, a sha256 fingerprint of the docstring-stripped source with the VERSION_TAG line itself excluded. 356 narrowed save_sp from Codons 64 bit int to u32 and measured a consistent 3 chunk improvement of -0.83, -0.93 and -2.00 percent against the 352 anchor, but did not touch the two unconditional BRA instructions 355 identified as responsible for 60.2 percent of the measured stall_branch_resolving total, both adjacent to BSYNC reconvergence markers in the stack push and pop machinery. 357 reruns 355s ncu procedure, N18 K1 SourceCounters at page source for the instruction level breakdown and N21 SchedulerStats plus WarpStateStats for the production K48 shape, against the 356 kernel instead of the 352 kernel, to see whether narrowing save_sp moved branch_resolving at all before any further SASS level intervention is designed. The one procedural change from 355: N21 dynamic profiling uses bench_mode 30, split145 probe slash cache build, with debug_chunk_start 0 and debug_chunk_count 1, instead of 355s bench_mode 31 with launch-count 1, because bench_mode 31 still ran the full three-chunk program to completion under instrumentation, roughly 24 minutes, while bench_mode 30 stops after the requested chunk count. Like 355, ncu output is never auto-parsed; the script only checks that expected section headings are present in the console dump and preserves raw logs and report files for manual reading. Like 355, there is no full-run correctness oracle in this revision because only chunk0 executes and no code changed; the sole correctness criterion is byte-for-byte source identity with 356 outside VERSION_TAG. 358s kernel resumes from 356 in either case, since 357 does not touch it."
