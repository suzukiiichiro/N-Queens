...
# =============================================================================
# 293 N=21 full validation harness (DUAL_LANE_MAXD14 probe)
# Parent: 292 kbatch-gridstride (K_PER_THREAD_MAXD14=32, confirmed -13.4% vs 291,
#   final total 314666222712, N=21 full once = 367.413s). Experiment: within the
#   already-adopted grid-stride outer loop, kernel_dfs_iter_gpu_maxd14 now
#   processes 2 constellations per outer-loop pass (idx and idx+stride) back to
#   back in program order, each with its own independent stack arrays
#   (ld/rd/col/avail vs ld_b/rd_b/col_b/avail_b), so the two computations have
#   no data dependency between them and the compiler/scheduler has independent
#   instruction streams to interleave -- targeting the ~48% "Stall Wait"
#   (fixed-latency dependency-chain) cost that ncu showed was essentially
#   unchanged by K-batching alone (292 K=2 ncu re-profile).
# Safety: hot-loop internals (schedule decode + iterative DFS) are NOT
#   rewritten; each lane is a mechanical continue->break transform of the
#   verbatim 292 body (see 293 notes in README). A DUAL_LANE_MAXD14 static
#   flag keeps a byte-identical copy of 292's single-lane body as a verified
#   fallback (DUAL_LANE_MAXD14=0) in case this experiment regresses or
#   misbehaves -- same single-tuning-knob-with-fallback pattern as
#   K_PER_THREAD_MAXD14 in 292.
# Known open risk (flagged, not yet resolved): doubling the live per-thread
#   stack-array footprint (ld/rd/col/avail x2) could reduce effective L1/L2
#   residency versus 292's single-lane footprint; 292 profiling showed near-100%
#   L1/L2 hit rates but at half the per-thread footprint this experiment adds.
#   ncu re-profiling (SpeedOfLight/Occupancy/WarpStateStats, chunk 40) after a
#   correctness-confirmed run is required before drawing conclusions either way.
# Expected: mode31 split145+chunkshape148, required=14, selected MAXD14,
#           schedule_words=0, stack=208 bytes/thread (per lane; unchanged from
#           292 since MAXD14_ANCESTOR is unchanged, lane B is a separate array),
#           K_PER_THREAD_MAXD14=32, DUAL_LANE_MAXD14=1 -> still 5 dispatch/
#           progress rows (K-batch chunk shape is untouched by this experiment).
# NOTE: this validation script has not yet been executed on real hardware as of
#   this handoff -- it is a first-pass static+build+run harness parallel to how
#   292's script was handed off before its first real run. Please run
#   `STATIC_ONLY=1 bash 293Py_dual_lane_maxd14_validate_N21_full_once.sh` first,
#   and only then the full run, per the usual 293 checklist below.
# =============================================================================

SRC=${SRC:-./293Py_dual_lane_maxd14_probe.py}
CAND=${CAND:-./293Py_dual_lane_maxd14_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/293Py_dual_lane_maxd14_probe_N21_full.lock}
...
EXPECTED_K_PER_THREAD_MAXD14=32
EXPECTED_DUAL_LANE_MAXD14=1
...
LOGDIR="${LOG_ROOT%/}/293Py_dual_lane_maxd14_logs_N21_full_once_${TS}"
...
echo "[start] 293 dual-lane-maxd14 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 293 dual-lane-maxd14 SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '293 dual-lane-maxd14 probe' "$SRC"; then
  printf 'source_version_tag	293 dual-lane-maxd14 probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	293 dual-lane-maxd14 probe	missing	FAIL
...
set +e
python3 - "$SRC" "$SUMMARY" "$EXPECTED_K_PER_THREAD_MAXD14" "$EXPECTED_DUAL_LANE_MAXD14" <<'PYCHECK'
import re, sys
src, summary = sys.argv[1], sys.argv[2]
EXPECTED_K_PER_THREAD_MAXD14_PY = sys.argv[3]
EXPECTED_DUAL_LANE_MAXD14_PY = sys.argv[4]
...
# Check MAXD14 generic DFS loop normal-default nld/nrd + ncol-only + block_code-late
# shape, in EACH of the 3 code regions 293 now contains: lane A, lane B (renamed
# with a "_b" suffix), and the DUAL_LANE_MAXD14=0 fallback (renamed idx->idx_f,
# thread_total->thread_total_f only -- everything else byte-identical to 292/291).
# Root-preroll (pr_block_code etc.) must remain untouched in all 3 regions.
...
if not m14:
    checks.append(('source_generic_normaldefault', 'MAXD14 generic normal-default nld/nrd (laneA/laneB/fallback)', 'MAXD14 not found', False))
    checks.append(('source_blockcode_late', 'block_code scalar only inside special branch (laneA/laneB/fallback)', 'MAXD14 not found', False))
    checks.append(('source_dual_lane_shape', 'DUAL_LANE_MAXD14 if/else + lane B arrays + 2x stride advance', 'MAXD14 not found', False))
else:
    body14 = m14.group(0)

    def region(start_marker, end_marker):
        i = body14.find(start_marker)
        if i < 0:
            return None
        j = body14.find(end_marker, i)
        return body14[i:j] if j >= 0 else body14[i:]

    laneA = region('while True:', 'thread_total+=lane_total\n')
    laneB = region('lane_total_b:u64=u64(0)', 'thread_total+=lane_total_b')
    fallback = region('thread_total_f:u64=u64(0)', 'results[tid]=thread_total_f')

    def check_normal_blockcode(tag, region_text):
        if region_text is None:
            return (False, False, 'region not found')
        idx = region_text.find('nibble_op')
        tail = region_text[idx:] if idx >= 0 else region_text
        suf = '_b' if tag == 'laneB' else ''
        expected_normal = (
            'nld{s}:u32=(cur_ld{s}|bit{s})<<u32(1)\n'
            '        nrd{s}:u32=(cur_rd{s}|bit{s})>>u32(1)\n'
            '        ncol{s}:u32=cur_col{s}|bit{s}\n'
            '        if (nibble_op{s}&u32(7))!=u32(0):\n'
            '          block_code{s}:u32=nibble_op{s}&u32(7)\n'
            '          stepu{s}:u32='
        ).format(s=suf)
        # indentation is relative (region_text keeps the source file's absolute
        # indentation), so the above literal (with 8/10-space continuation
        # indent baked in via triple-quote-equivalent formatting) only matches
        # when nesting is exactly right for the region it's checked against;
        # normalize by stripping a common per-region base indent instead.
        lines = [l for l in tail.split('\n') if l.strip()]
        base_indent = min((len(l) - len(l.lstrip(' ')) for l in lines if 'nld' + suf + ':u32=' in l), default=None)
        ok_normal = False
        ok_late = False
        if base_indent is not None:
            norm = re.sub(r'\n {%d}' % base_indent, '\n', tail)
            expected_normal_relative = (
                'nld{s}:u32=(cur_ld{s}|bit{s})<<u32(1)\n'
                'nrd{s}:u32=(cur_rd{s}|bit{s})>>u32(1)\n'
                'ncol{s}:u32=cur_col{s}|bit{s}\n'
                'if (nibble_op{s}&u32(7))!=u32(0):\n'
            ...
            old_early_blockcode_relative = (
                'block_code{s}:u32=nibble_op{s}&u32(7)\n\nbit{s}:u32=cur_avail{s}&(u32(0)-cur_avail{s})'
            ).format(s=suf)
            nf_default_bad = (('nf{s}:u32=bm&~(nld{s}|nrd{s}|ncol{s})\nif (nibble_op{s}&u32(7))!=u32(0):'.format(s=suf) in norm)
                               or ('nf{s}:u32=bm&~(nld{s}|nrd{s}|ncol{s})\nif block_code{s}!=u32(0):'.format(s=suf) in norm))
            ok_normal = expected_normal_relative in norm and not nf_default_bad
            ok_late = old_early_blockcode_relative not in norm and expected_normal_relative in norm
        return (ok_normal, ok_late, 'checked')

    results3 = {tag: check_normal_blockcode(tag, r) for tag, r in (('laneA', laneA), ('laneB', laneB), ('fallback', fallback))}
    all_normal_ok = all(v[0] for v in results3.values())
    all_late_ok = all(v[1] for v in results3.values())
    detail_normal = ','.join('%s=%s' % (k, 'OK' if v[0] else 'FAIL') for k, v in results3.items())
    detail_late = ','.join('%s=%s' % (k, 'OK' if v[1] else 'FAIL') for k, v in results3.items())
    checks.append(('source_generic_normaldefault', 'MAXD14 generic normal-default nld/nrd+ncol-only (laneA/laneB/fallback all OK)', detail_normal, all_normal_ok))
    checks.append(('source_blockcode_late', 'block_code scalar only inside special branch (laneA/laneB/fallback all OK)', detail_late, all_late_ok))

    # 293: dual-lane structural shape checks.
    ok_stride_param = 'stride:int,' in body14.split(')->None:')[0]
    ok_dual_flag_branch = 'if DUAL_LANE_MAXD14!=0:' in body14
    ok_lane_b_arrays = all(('%s=__array__[u32](MAXD14_ANCESTOR)' % nm) in body14 for nm in ('ld_b', 'rd_b', 'col_b', 'avail_b'))
    ok_gridstride_loop = 'while idx<m:' in body14 and 'while idx_f<m:' in body14
    ok_double_advance = 'idx+=stride+stride' in body14
    ok_single_advance_fallback = re.search(r'idx_f\+=stride\b(?!\+)', body14) is not None
    ok_single_writeback_dual = 'results[tid]=thread_total\n' in body14
    ok_single_writeback_fallback = 'results[tid]=thread_total_f' in body14
    ok_no_percall_writeback = 'results[idx]=' not in body14 and 'results[idx_b]=' not in body14 and 'results[idx_f]=' not in body14
    ok_kbatch_shape = (ok_stride_param and ok_dual_flag_branch and ok_lane_b_arrays and ok_gridstride_loop
                        and ok_double_advance and ok_single_advance_fallback and ok_single_writeback_dual
                        and ok_single_writeback_fallback and ok_no_percall_writeback)
    checks.append(('source_dual_lane_shape',
                    'DUAL_LANE_MAXD14 if/else + lane B arrays + 2x stride advance (dual) / 1x (fallback) + single results[tid] writeback each',
                    'present' if ok_kbatch_shape else (
                        'missing (stride=%s dual_flag=%s lane_b_arrays=%s gridstride=%s dbl_adv=%s fallback_adv=%s writeback_dual=%s writeback_fb=%s no_percall=%s)'
                        % (ok_stride_param, ok_dual_flag_branch, ok_lane_b_arrays, ok_gridstride_loop, ok_double_advance,
                           ok_single_advance_fallback, ok_single_writeback_dual, ok_single_writeback_fallback, ok_no_percall_writeback)
                    ),
                    ok_kbatch_shape))
...
dl_match = re.search(r'^DUAL_LANE_MAXD14:Static\[int\]=(\d+)', s, re.M)
dl_value = dl_match.group(1) if dl_match else 'missing'
checks.append(('source_dual_lane_maxd14_flag', str(EXPECTED_DUAL_LANE_MAXD14_PY), dl_value, dl_value == str(EXPECTED_DUAL_LANE_MAXD14_PY)))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 293 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 293 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 293 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 293 dual-lane-maxd14 mode31 split145"
...
echo "[validation-ok] 293 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32, DUAL_LANE_MAXD14=1 (5 dispatch/progress rows)"
