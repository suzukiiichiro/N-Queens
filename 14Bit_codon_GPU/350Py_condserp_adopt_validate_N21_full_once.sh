...
SRC=${SRC:-./350Py_condserp_adopt.py}
CAND=${CAND:-./350Py_condserp_adopt}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/350Py_condserp_adopt_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/350Py_condserp_adopt_logs_N21_full_once_${TS}"
...
echo "[start] 350 condserp-adopt validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 350 condserp-adopt SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '350 condserp-adopt' "$SRC"; then
  printf 'source_version_tag	350 condserp-adopt	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	350 condserp-adopt	missing	FAIL
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 350 source static checks failed" >&2
  exit 66
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 350 condserp-adopt (kernel-identical to 349/348/347/346/345/344/338/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT; 350 is the ADOPTION revision -- the source change is two default constants, CHUNKSHAPE148_ITER_SORT and A10G_FINAL_DEFAULT_CHUNKSHAPE148_ITER_SORT, both 1 to 9. Kernels and dispatcher are untouched. This run should reproduce the 349 mode 9 measurement of 398.988s with cache reuse, which is -0.824 percent against the 349 mode 1 anchor of 402.301s and -12.510 percent against the 456.036s baseline at the time SoA was adopted in 328. The conditional serpentine rule is EMPIRICAL: the partial-launch penalty it avoids is reproduced but not explained. To reach 346 run with CHUNKSHAPE148_ITER_SORT=1, and to reach 344 run with CHUNKSHAPE148_ITER_SORT=0"
