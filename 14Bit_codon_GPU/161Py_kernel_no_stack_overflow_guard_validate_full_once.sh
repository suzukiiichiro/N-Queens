...
# 161 single-pass validation harness
#
...
#
# Candidate 161 retains the validated 160 root-pop loop termination and every
# earlier GPU representation/DFS/order change.  Its sole kernel hot-path
# experiment removes the unreachable push-time `if sp>=MAXD` branch.  MAXD=21,
# all five stack arrays, root-pop control flow, host representation, task
# order/cache, DFS transitions, and solution arithmetic remain unchanged from
# validated 160.  GPU execution is host-limited to the proven N=5..27 range.

SRC=${SRC:-./161Py_kernel_no_stack_overflow_guard_probe.py}
CAND=${CAND:-./161Py_kernel_no_stack_overflow_guard_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/161Py_kernel_no_stack_overflow_guard_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 160..149 full runs and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 161 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/161Py_kernel_no_stack_overflow_guard_logs_N21_full_once_${TS}"
...
if [[ -f "$SRC" ]]; then
  ...
  # 161 sole hot-path experiment: push is immediately followed by child-frame
  # stores, with no MAXD comparison or early partial-result return.
  ...
  grep -Fq '161 no-stack-overflow-guard GPU candidate supports N=5..27 only' "$SRC" || source_failures=$((source_failures+1))
  grep -Fq 'VERSION_TAG:str="161 no stack overflow guard over 160 root-pop termination' "$SRC" || source_failures=$((source_failures+1))
  ...
  if (( source_failures != 0 )); then
    printf 'source_no_stack_overflow_guard_shape\trequested layout\t%d check failures\tFAIL\n' "$source_failures" >> "$SUMMARY"
    echo "[error] source does not match the requested 161 no-stack-overflow-guard experiment" >&2
    exit 65
  fi
  ...
  printf 'source_no_stack_overflow_guard_shape\t0 push guards + direct child stores\tverified\tOK\n' >> "$SUMMARY"
...
echo "[validation-ok] 161 one full run reproduced cases 01-06"
