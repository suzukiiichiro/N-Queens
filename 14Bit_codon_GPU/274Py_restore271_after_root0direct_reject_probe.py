...
# =============================================================================
# NQ_UPDATE_MEMO
# 274: restore271-after-root0direct-reject.
#      273 root_action==0 direct MAXD14 kernel は正当性OKだが、N=21 fullで
#      271より約1.052秒退行したため採用せず、271 final-fastest-complete
#      相当へ戻す安全復帰版。CUDA MAXD14/16/18/20/21 kernel本文、
#      rootrestlate、futuremask、no-sibling、split145/chunkshape148、
#      broadmarktail、cache生成、worker splitは271相当で変更しない。
#      -c/-g range、GPU N=5..27、N25..N27 dynamic preset=8、
#      mode28/29/30/31、MAXD fallback、CPU dfs_iter pathを維持。
...
"""
274ログは問題ありません。正当性OKで271相当へ戻っています。

```text
274 elapsed: 0:07:07.758
final total: 314666222712 OK
dispatch/progress/MAXD14/stack 208: OK
```

275は予定通り、**速度候補ではなく d2base14 / d0 の split145 bucket 分布診断版**として作成しました。CUDA kernel本文、1 chunk 1 launch、271/274本線は維持しています。`STATIC_ONLY=1` は通しています。

ファイルはこちらです。

* [275Py_split145_bucket_distribution_diag_probe.py](sandbox:/mnt/data/275Py_split145_bucket_distribution_diag_probe.py)
* [275Py_split145_bucket_distribution_diag_validate_N21_full_once.sh](sandbox:/mnt/data/275Py_split145_bucket_distribution_diag_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [275Py_split145_bucket_distribution_diag_sha256.txt](sandbox:/mnt/data/275Py_split145_bucket_distribution_diag_sha256.txt)
* [275Py_split145_bucket_distribution_diag_bundle_20260709.tar](sandbox:/mnt/data/275Py_split145_bucket_distribution_diag_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./275Py_split145_bucket_distribution_diag_probe

STATIC_ONLY=1 bash 275Py_split145_bucket_distribution_diag_validate_N21_full_once.sh

bash 275Py_split145_bucket_distribution_diag_validate_N21_full_once.sh
```

確認したい行です。

```bash
grep -E "split145-bucket-summary|split145_bucket_|^21:|validation-ok" \
  275Py_split145_bucket_distribution_diag_logs_N21_full_once_*/full_once.log
```

summaryでは `d2base14_tasks`、`d0_tasks`、`d0_or_d2_chunks`、`both_chunks`、`other_tasks` を見て、次に分解する価値があるかを判断します。
...
VERSION_TAG:str="274 restore271-after-root0direct-reject: 273 root_action==0 direct MAXD14 kernel rejected for speed; restored 271 final-fastest-complete baseline; generic N5..27; bare -g split145 mode31; unused recursive CPU dfs fallback remains removed; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved; CUDA kernels unchanged"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split274_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split274_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split274-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split274-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split274_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
