...
作成しました。101Py は **Step 5 の追加診断: funcid単体 profile** 用です。
100Py の `bench_mode=20` は `all;5;7;23;5+23;7+23;other` の group 単位でしたが、101Py では `bench_mode=21` を追加して、選択 chunk を **funcid=0..27 の単体ごと**に分解します。100Py の mode20 は group 指定で診断する構造でした。

ソースはこちらです。

[101Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsingle.py](sandbox:/mnt/data/101Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsingle.py)

差分はこちらです。

[100_to_101_funcidsingle.patch](sandbox:/mnt/data/100_to_101_funcidsingle.patch)
...
対象 chunk はデフォルトで、100Py と同じ重い代表6 chunkです。

```text
94,1587,1625,1644,1772,1854
```

対象 funcid はデフォルトで全28種です。
...
kernel / DFS 本体は変更していません。
chunkを読み、SoAで funcid を分類し、指定 funcid の record だけを抽出して既存 kernel に投入する診断モードです。
...
codon build -release \
  101Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsingle.py
```

## まず smoke test

最初は chunk 94 の注目 funcid だけで確認してください。

```bash
./101Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsingle \
  -g 22 22 32 484 1 0 7 21 8 7 0 94 1 "-" "5,7,23"
```
...
まずは `other` 再分解に必要な funcid を中心に走らせるのがよいです。

```bash
./101Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsingle \
  -g 22 22 32 484 1 0 7 21 8 7 0 0 1 \
  "94,1587,1625,1644,1772,1854" \
  "0,4,5,7,12,16,17,18,19,20,21,22,23,24,26,27"
```

全 funcid を見るなら、末尾の funcid list を省略してもよいです。

```bash
./101Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_funcidsingle \
  -g 22 22 32 484 1 0 7 21
```

この場合は、デフォルトで以下になります。

```text
chunk_list  = 94,1587,1625,1644,1772,1854
funcid_list = 0..27
```
...
```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_funcidsingle.tsv
```
...
`stage_classify_ms` は source chunk から SoA を作って funcid を分類する時間です。
`stage_filter_ms` は、その分類済み SoA から指定 funcid の record だけを抜き出す時間です。
...
```bash
PROG=progress_N22_7_stream_funcid_reorder_v2_w8_j7_funcidsingle.tsv
...
}
{
  f=$(c["target_funcid"])
  rows[f]++
  records[f]+=$(c["target_m"])
  total[f]+=$(c["chunk_total"])
  elapsed[f]+=$(c["elapsed_ms"])
  kernel[f]+=$(c["stage_kernel_ms"])
}
END {
  for (f in rows) {
    if (records[f] > 0) {
      printf "fid=%s rows=%d records=%d chunk_total=%d elapsed=%.3f sec kernel=%.3f sec kernel_ms_per_record=%.6f kernel_ms_per_chunk_total=%.9f\n", \
        f, rows[f], records[f], total[f], elapsed[f]/1000.0, kernel[f]/1000.0, \
        kernel[f]/records[f], kernel[f]/total[f]
    }
  }
}
' "$PROG" | sort -k7,7nr
...
}
{
  if ($(c["target_m"]) > 0) {
    print $(c["stage_kernel_ms"]), \
          ...
          "kernel_ms_per_record=" ($(c["stage_kernel_ms"]) / $(c["target_m"]))
  }
}
...
今回の101Pyで確認したいのは、主にこの3点です。

```text
1. funcid_7 が本当に少数 tail の主犯か
2. funcid_5 が総量として重いのか、recordあたりでも重いのか
3. 100Py の other に含まれていた重い funcid がどれか
```
...
100Py では `group=7` が record あたり突出しており、`other` も無視できない重さでした。101Py はその `other` を funcid 単位で割るための診断版です。

こちらでは Codon 実機ビルドまでは確認していません。まず smoke test の `chunk=94 / funcid=5,7,23` で build と起動確認をお願いします。


ログ確認しました。**101Py / funcid単体 profile は成功**です。
そして、今回でボトルネック候補がかなり絞れました。

## 101Py の実行結果

まず、smoke test は成功しています。chunk 94 に対して `fid=5,7,23` を単体実行し、`cases=3`、`partial_total=1097288600`、実行時間 `0:00:17.986` で完了しています。
...
summary でも `cases=168`、`total=9068181302`、`funcidsingle-done` が出ています。

## 一番重要な発見

**前回の `other` の主犯は、かなりの部分が `funcid_4` と `funcid_0` です。**

前回までは `funcid_5 / funcid_7 / funcid_23` を疑っていましたが、101Py の単体分解では、総 kernel 時間の上位がこう見えています。

```text
fid=5   kernel=47.300 sec  records=24529  ms/record=1.928
fid=4   kernel=42.699 sec  records=4084   ms/record=10.455
fid=7   kernel=36.859 sec  records=3837   ms/record=9.606
fid=0   kernel=27.176 sec  records=2148   ms/record=12.652
fid=23  kernel=20.679 sec  records=11800  ms/record=1.752
```

`funcid_5` は総量では最大ですが、record あたりではそれほど異常ではありません。むしろ、**`funcid_4 / funcid_7 / funcid_0` が少ない record 数で大きな kernel 時間を使っている**のが重要です。

## 優先順位の更新

現時点の最適化優先度は、次のように更新した方がよいです。

```text
1. funcid_4
   予想外の大物。records=4084 で kernel=42.699 sec。
   total寄与も record単価も大きい。

2. funcid_7
   records=3837 で kernel=36.859 sec。
   少数 tail が強い。前回の疑いどおり。

3. funcid_0
   records=2148 で kernel=27.176 sec。
   record単価が高く、otherの中身として無視できない。

4. funcid_5
   kernel総量は最大。
   ただし records=24529 と多く、record単価は中程度。
   専用高速化より、量の多い標準pathとして扱う候補。

5. funcid_23
   records=11800 で kernel=20.679 sec。
   総量はあるが、record単価は低め。
   主犯ではない。
```

つまり、**次の本命は `funcid_4 / funcid_7 / funcid_0` の heavy-tail 分離**です。`funcid_5` は大口ですが、tail の主犯というより「量が多いので総時間が大きい」タイプに見えます。
...
```text
chunk=1587 fid=5  kernel=9238 ms  target_m=5013
chunk=1625 fid=5  kernel=9050 ms  target_m=7059
chunk=94   fid=5  kernel=8670 ms  target_m=4168
chunk=1644 fid=0  kernel=8611 ms  target_m=393
chunk=1644 fid=4  kernel=8479 ms  target_m=1157
chunk=1772 fid=4  kernel=8213 ms  target_m=1486
chunk=1587 fid=4  kernel=8114 ms  target_m=975
chunk=1772 fid=7  kernel=7503 ms  target_m=1473
```

ここで注目すべきは、`fid=0` や `fid=4` が **target_m が小さいのに 8秒級**になっていることです。これは単なる records 数ではなく、DFS tail / branch divergence / 少数長時間 thread の影響が強い可能性を示しています。

特に極端な例として、`chunk=1625 fid=7` は `target_m=47` しかないのに `kernel_ms=5963` です。`chunk=1854 fid=7` も `target_m=643` で `kernel_ms=7061` です。これは `funcid_7` が「少数でも長く残る tail」を作っている強い証拠です。

## `funcid_23` の扱い

`funcid_23` は、以前疑っていたほど主犯ではありません。

```text
fid=23
  records=11800
  kernel=20.679 sec
  kernel_ms_per_record=1.752
```

record 数は多いですが、`funcid_4 / 7 / 0` と比べると record あたりの重さは低いです。したがって、次の最適化対象としては `funcid_23` より **`funcid_4 / 7 / 0` を優先**した方がよいです。
...
判定:
  funcid_5 は総量最大。
  funcid_4 / funcid_7 / funcid_0 が heavy-tail 本命。
  funcid_23 は主犯ではない。
```
...
```text
旧:
  funcid_5 / funcid_7 / funcid_23 が本命

新:
  funcid_4 / funcid_7 / funcid_0 が tail本命
  funcid_5 は大口処理本命
  funcid_23 は優先度を下げる
```
...
次は **102Py** として、いきなり DFS ロジックを変えるより、まず「重い funcid を別キュー・別 kernel に分けたら速くなるか」を試すのが安全です。
...
medium:
  1,12,15,18,19,20,22,24

light:
  16,17,21,23,25,26,27
```
...
E. split total = B + C + D
   分離実行の合計が all より改善するか
```

もし `split total < all` になれば、**重い funcid の分離スケジューリングが有効**です。
逆に `split total > all` なら、分離 kernel の起動回数増加や低 occupancy の方が悪いので、DFS内部の専用最適化へ進むべきです。

なお、以前アップロードされた一部ファイルは検索側で期限切れになっていますが、今回の追加ログは確認できています。過去のソースや古いログを再参照したい場合は、再アップロードをお願いします。
...
VERSION_TAG:str="101 funcid-single auto w8_j7 stream funcid risk balanced reorder v2 GPU runner from 100"
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if bench_mode<0 or (bench_mode>11 and bench_mode!=13 and bench_mode!=14 and bench_mode!=15 and bench_mode!=16 and bench_mode!=17 and bench_mode!=18 and bench_mode!=19 and bench_mode!=20 and bench_mode!=21):
    ...
    if bench_mode>=8:
      ...
      preset_queens_arg=5
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21:
        if argc >= 14:
  ...
  if bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and not (bench_mode==8 or bench_mode==9 or bench_mode==10 or bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21):
      ijkl_list,subconst_cache,stream_records,preset_queens,stream_fname=ensure_constellations_bin_stream(N,ijkl_list,subconst_cache,preset_queens,gpu_log_level)
