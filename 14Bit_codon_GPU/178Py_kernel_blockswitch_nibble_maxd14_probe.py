...
167Pyの計算結果は正常です。131チャンク、最終合計`314666222712`が一致し、実行時間は`10:33.039`でした。

`dispatch_max_required`のFAILは、シェルが必要深度を16と誤って期待したためです。実測は`required_maxd=14`、選択容量は`MAXD=16`であり、安全性に問題はありません。168Pyのシェルでは修正済みです。

* [168Py_kernel_packed_opcode_schedule_probe.py](sandbox:/mnt/data/168Py_kernel_packed_opcode_schedule_probe.py)
* [168Py_kernel_packed_opcode_schedule_validate_full_once.sh](sandbox:/mnt/data/168Py_kernel_packed_opcode_schedule_validate_full_once.sh)

168Pyは167Pyを親に、ctrl metadataを6bit opcode化して4深度分を1個のu32へ格納します。N=21では局所stackを320→272 bytes/threadへ縮小します。

静的検査、506,604フレーム等価性、12,000件のDFSモデル、模擬131チャンク再生は通過済みです。Codon/CUDA実機実行は未実施です。

はい、概ねその理解で正しいです。ただし、**Nに比例して自動的に1ずつ増えるのではありません**。

168Pyでは各GPUチャンクのタスクを調べ、必要深度`required_maxd`に応じて静的カーネルを選択します。

```text
required_maxd <= 16 → MAXD=16
required_maxd <= 18 → MAXD=18
required_maxd <= 20 → MAXD=20
required_maxd <= 21 → MAXD=21
required_maxd >  21 → GPUを起動せずエラー
```

したがって、N=22でも必要深度が16以下ならMAXD=16のままです。Nの増加に伴って探索深度が増えれば、16→18→20→21と自動的に切り替わります。選択はN単位ではなく**チャンク単位**です。

現在の対応範囲はN=5～27で、必要深度が21を超えた場合は配列外アクセスを起こさず、明示的に停止します。N=28以上もGPU実行前に拒否されます。

つまり正確には、**「Nに応じて増える」ではなく、「各タスク群の実際の必要深度に応じて、最小の静的MAXDへ自動dispatchする」**仕組みです。
[168Pyソース](sandbox:/mnt/data/168Py_kernel_packed_opcode_schedule_probe.py)

その目標設定は妥当です。ただし、**N=21で2分は「N=26への第一関門」**であり、同時にN増加時の倍率も改善する必要があります。

現在の最速167Pyは約`10:33`なので、2分達成にはさらに**約5.28倍**の高速化が必要です。旧C/CUDAのN=21比では、すでに約1.78倍高速です。
...
です。仮にN=21を2分にしても、この倍率が変わらなければN=25は約15日、N=26は概算で約160～170日になります。

したがって今後は、次の2指標で評価するのが重要です。

```text
1. N=21の絶対時間：目標 2分
2. N=22 / N=21の増加倍率：現在想定の約9.6倍を下げる
```
...
178Py host-precomputed版のログ確認しました。これは **実行結果不一致ではなく、CUDA JIT時点の `CUDA_ERROR_INVALID_PTX`** なので、旧178Pyは不採用にします。
...
を `TaskSoA` と MAXD14 kernel引数へ追加した点です。Codonのビルドは通っても、GPU実行時のPTX JITで落ちています。

そのため、**178Pyを差し替え**ます。
新しい178Pyは、最速基準の **175Pyへ戻し**、host-precomputed scheduleは完全に撤回しました。追加GPU引数も追加SoA配列もありません。

ファイルはこちらです。

* [178Py_kernel_blockswitch_nibble_maxd14_probe.py](sandbox:/mnt/data/178Py_kernel_blockswitch_nibble_maxd14_probe.py)
* [178Py_kernel_blockswitch_nibble_maxd14_validate_full_once.sh](sandbox:/mnt/data/178Py_kernel_blockswitch_nibble_maxd14_validate_full_once.sh)

今回の差し替え178Pyの内容です。

```text
親:
  175Py nibble schedule MAXD14

撤回:
  176Py current-nibble register
  177Py terminal-first
  旧178Py host-precomputed schedule

新178Py:
  175Pyの4-bit nibble scheduleは維持
  schedule生成はGPU thread内のまま
  追加SoA配列なし
  追加kernel引数なし

変更点:
  MAXD14 hot loop内の block_code decode を
  mask decode から direct constant switch へ変更
```

175Pyでは block path で以下を毎回mask decodeしていました。

```text
stepu = 2 + ((OP_STEP3_MASK >> block_code) & 1)
addvu = (OP_ADD1_MASK >> block_code) & 1
bLiu  = ...
ktu   = ...
bKu   = ...
```

新178Pyでは、`block_code` を直接分岐します。
...
block_code == 1:
  step2, add0, blockL0, K=n3

block_code == 2 or 5:
  step2, low-bit OR 1, K=0

block_code == 3:
  step3, low-bit OR 1, K=n4

block_code == 4:
  step3, low-bit OR 2, K=n3
```
...
ancestor stack = u32[13] x 4
current frame register = 維持
```

こちらでは `STATIC_ONLY=1` の静的・モデル検証を通しています。

```text
kernel_variant_count                         5 / 5 OK
kernel_fallback_normalized_equivalence       1 / 1 OK
maxd14_blockswitch_nibble_schedule            1 / 1 OK
block_opcode_mapping                         19 cases OK
nibble_opcode_classes                        7 classes OK
frame_opcode_equivalence                     506604 cases OK
dfs_164_vs_178_blockswitch_nibble_equivalence 12000 cases OK
n21_depth14_scalar_schedule_cases            3415 cases OK
n21_depth14_terminal_parent                  13 / 13 OK
schedule_words_MAXD14                        0 OK
stack_bytes_MAXD14                           208 OK
```

GPU環境では、旧 `178Py_kernel_host_precomputed_nibble...` ではなく、こちらを使ってください。

```bash
chmod +x 178Py_kernel_blockswitch_nibble_maxd14_validate_full_once.sh
./178Py_kernel_blockswitch_nibble_maxd14_validate_full_once.sh
```

この差し替え178Pyは、PTX JITで落ちたhost-precomputed経路を完全に外しているので、まずJITが通るかの確認対象としてはこちらが安全です。


"""

# 178 GPU block-switch nibble experiment over validated 175 MAXD14:
#   Branch from the validated 171 current-frame-register build.  Host-side depth
...
#
#   178 keeps the fastest 175 layout:
#     - MAXD14 scalar 4-bit nibble schedule
#     - current frame in registers
#     - 13-slot ancestor stack
#     - schedule_words=0, stack_bytes/thread=208
#
#   The only kernel experiment is inside the MAXD14 block path.  175 decoded
#   block metadata with several mask shifts from block_code.  178 instead uses
#   direct constant cases for block_code 1..5.  The normal step1 path, future
#   pruning, child-jmark handling, terminal handling, host layout, task order,
#   cache, DFS order, bitboard arithmetic, and solution arithmetic are unchanged.
#   MAXD16/18/20/21 keep the validated byte-opcode local schedule fallback.
#   The supported GPU range remains N=5..27.
...
# 178 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 178 MAXD14 uses scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="178 block-switch nibble schedule for MAXD14 over validated 175 nibble baseline - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 178 inherits the exact 175/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """178 MAXD14 nibble schedule with direct block-code switch plus register-held current frame and 13-slot ancestor stack."""
    ...
    # high-bit u64 variable shifts.  Compared with 172's 3-bit compact op, the
    # 4-bit nibble uses power-of-two shifts and decodes block/future directly.
    # 178 keeps the 175 layout but replaces block-code mask decoding in the
    # MAXD14 hot loop with direct constant cases for codes 1..5.
    ...
    # 178 keeps the validated 175 current-frame register cache plus ancestor stack shrink.  The active
    ...
    while True:
      ...
      nld:u32=u32(0)
      nrd:u32=u32(0)
      # 178: MAXD14 block-code switch.  175 decoded block metadata with
      # several bit-mask shifts for every block candidate.  The nibble already
      # carries the direct block code, so use constant cases in the hot loop.
      # code 2 and code 5 are arithmetically identical: step2, low-bit OR 1, K=0.
      if block_code==u32(0):
        nld=(cur_ld|bit)<<u32(1)
        nrd=(cur_rd|bit)>>u32(1)
      elif block_code==u32(1):
        nld=(cur_ld|bit)<<u32(2)
        nrd=((cur_rd|bit)>>u32(2))|n3
      elif block_code==u32(2) or block_code==u32(5):
        nld=((cur_ld|bit)<<u32(2))|u32(1)
        nrd=(cur_rd|bit)>>u32(2)
      elif block_code==u32(3):
        nld=((cur_ld|bit)<<u32(3))|u32(1)
        nrd=((cur_rd|bit)>>u32(3))|n4
      else:
        nld=((cur_ld|bit)<<u32(3))|u32(2)
        nrd=((cur_rd|bit)>>u32(3))|n3
...
####################################################################################################
# 178 block-switch nibble MAXD14 + static dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 175 scorestripe_v9 task order/cache and MAXD14 nibble schedule; 178 switches block-code decode to direct constants"
...
def main()->None:
  ...
  # 178 branches from validated 175.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 178 block-switch nibble GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split178_nibble_schedule_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split178_nibble_schedule_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split178-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split178-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split178_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split178-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split178-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split178_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
