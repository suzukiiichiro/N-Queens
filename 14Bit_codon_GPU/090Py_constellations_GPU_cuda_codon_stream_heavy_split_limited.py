...
[90Py_constellations_GPU_cuda_codon_stream_heavy_split_limited.py](sandbox:/mnt/data/90Py_constellations_GPU_cuda_codon_stream_heavy_split_limited.py)

90は、89の **heavy split は正しいが、splitしすぎて遅い** という結果を受けて、split対象をかなり絞った版です。89ではN21で入力 `2,025,282` records に対し、GPU投入taskが `8,107,197` まで増えていました。
...
```python
HEAVY_SPLIT_FACTOR_DEFAULT:int=2
HEAVY_SPLIT_FREE_MIN_DEFAULT:int=8
HEAVY_SPLIT_DEPTH_MIN_DEFAULT:int=12
HEAVY_SPLIT_SCORE_MIN_DEFAULT:int=96
```
...
```text
split_factor=2
  1 chunkあたり最大出力task数を STEPS*2 に制限

free_min=8
  free候補が8以上あるtaskだけsplit対象

depth_min=12
  残り深さが12以上あるtaskだけsplit対象

score_min=96
  free_popcount * depth >= 96 のtaskだけsplit対象
```

89では `split_factor=4, free_min=4, depth_min=8` だったため、ほぼ毎chunk `out_m≈61952` まで膨らんでいました。90ではこれを抑えます。
...
```bash
codon build -release 90Py_constellations_GPU_cuda_codon_stream_heavy_split_limited.py

./90Py_constellations_GPU_cuda_codon_stream_heavy_split_limited \
...
```text
21:      314666222712                0      ...    heavy-split-limited-ok
```

## 出力ファイル

```text
progress_N21_6_heavy_split_limited.tsv
```
...
```bash
tail -20 progress_N21_6_heavy_split_limited.tsv
```
...
88 N21:
  0:34:57.160 bc-kernel-ok

89 N21:
  0:44:16.469 heavy-split-ok
```

84はN21〜N23で安定しており、N21は `0:25:38.459` でした。
90は、まず **89より速くなるか**、次に **84に近づくか** を見ます。

## 期待するログ傾向

89では多くのchunkでこうなっていました。
...
`out_m` がほぼ `15488` のままならsplitが少なすぎます。
`out_m` が毎回 `30976` 付近なら上限いっぱいまでsplitしています。

この環境ではCodonの実ビルドはできていませんが、Pythonの `tokenize` による字句レベル確認は通しています。


確認しました。**90は正解一致していますが、速度改善としてはまだ84に負けています。**

90の最終結果は以下です。

```text
90 heavy split limited N21:
21:      314666222712                0          0:31:26.233    heavy-split-limited-ok
```

`total=314666222712` で正解一致、`records=2025282`、`chunks=131`、`executed_chunks=131` なので、計算自体は正常です。
...
```text
84 stream N21             : 0:25:38.459  ok
88 BC kernel N21          : 0:34:57.160  bc-kernel-ok
89 heavy split N21        : 0:44:16.469  heavy-split-ok
90 heavy split limited N21: 0:31:26.233  heavy-split-limited-ok
```

84は `0:25:38.459` でN21を完走しています。
88は `0:34:57.160`、89は `0:44:16.469` でした。

したがって90は、**89よりはかなり改善**しています。
...
90ではsplit制限を入れましたが、それでもGPUへ渡すtask数が約2倍になっています。

```text
records        = 2,025,282
total_out      = 4,057,517
split_roots    = 250,030
split_children = 2,282,265
kept_roots     = 1,775,252
```

つまり、元の2,025,282 taskに対し、最終的に4,057,517 taskをGPUへ投げています。

各chunkもほぼ上限いっぱいです。例えばchunk 0では、
...
つまり、90は「splitしすぎ問題」を89の4倍膨張から2倍膨張まで抑えましたが、**まだ膨張分のコストが勝っている**状態です。

## 判断

**90はN22へ進めなくてよいです。**

理由は、N21で84に負けているためです。N22以降でも、task数を増やす方式はさらに不利になる可能性が高いです。
...
88:
  BC group kernel化は正しいが遅い

89:
  heavy splitは正しいがsplitしすぎで遅い

90:
  heavy splitを制限して改善したが、まだ84より遅い
```

## 次にやるべきこと

次は、91の新規改造に進む前に、**84のまま `max_blocks` を増やすテスト**をおすすめします。

90のログを見ると、split後は `grid=968` で動いています。
しかし90はtask数を2倍に増やしてから `grid=968` にしているため、遅くなっています。

そこで、**task数を増やさずに84で `max_blocks=968` を試す**のが良いです。

```bash
./84Py_constellations_GPU_cuda_codon_dynamic_p8_stream \
  -g 21 21 32 968 1 0 5 0
```
...
となり、chunk数が約半分になります。90のようにtaskを増やさないため、もしかすると84の `32x484` より速くなる可能性があります。

比較対象はこれです。

```text
84 32x484 N21: 0:25:38.459
```

`32x968` がこれより速ければ、ソース改造なしで前進できます。
...
# もし速ければ 32x1452
./84Py_constellations_GPU_cuda_codon_dynamic_p8_stream \
  -g 21 21 32 1452 1 0 5 0
```

ただし、`max_blocks` を大きくしすぎると、1 kernel内のtail effectが大きくなって逆に遅くなる可能性があります。まずは `968` だけで十分です。

## sort_mode=4は封印

念のため再確認ですが、`sort_mode=4` は使わない方がよいです。ログでは `chunk_total=0` が続き、最終的に `ng(0!=314666222712)` になっています。

## 結論

**90は正しいが、84より遅いです。N22には進めず打ち切りでよいです。**
...
これで84のままchunkサイズを広げた場合の速度を見ましょう。 taskを増やさずkernel投入回数を減らせるので、これまでの85〜90より筋が良い可能性があります。

**明日の開始段階で添付するソースコードは、84がよいです。**

理由は明確です。

**84は現時点の本線・最速・安定版**です。N21〜N23まで正解一致しており、GPU実行でも N21 `0:25:38.459`、N22 `3:18:42.963`、N23 `1 day, 6:08:25.451` で `ok` です。

一方、**90は実験版**です。heavy split limited は正解一致しましたが、N21で `0:31:26.233` となり、84の `0:25:38.459` より遅いです。90は「89よりは改善したが、84を超えられなかった検証ブランチ」という位置づけです。
...
必要なら補足資料として:
  90Py_constellations_GPU_cuda_codon_stream_heavy_split_limited.py
  88/89/90の比較ログ
```

特に、明日の議論を継続しやすくするなら、84を添付したうえで、冒頭にこのメモを付けるのがよいと思います。

```text
現時点の本線は84。
84 stream 32x484 sort_mode=0 が最速・安定。
88/89/90は正解一致したが84より遅いため検証ブランチ扱い。
sort_mode=4はchunk_total=0となるため封印。
32x968も正解一致したが32x484より遅い。
```

`sort_mode=4` は `chunk_total=0` が続き、最終的に `ng(0!=314666222712)` になっているため、明日の開始点にはしない方がよいです。

`32x968` も正解一致はしていますが、N21で `0:36:24.367` と遅く、84標準の `32x484` より不利です。

結論として、**明日は84を添付してください。90は参考として残す程度で、本線にはしないのがよいです。**
...
VERSION_TAG:str="90 stream heavy split limited GPU runner from 84 dynamic preset P8"
CROSS_STRIPE_SAFE_DEFAULT:bool=False

# 90 HEAVY SPLIT LIMITED:
#   89 proved the host-side one-step split is correct, but it expanded too many
#   tasks: N21 grew from about 2.0M roots to about 8.1M GPU tasks and slowed down.
#   90 keeps the 84 stream + single generic GPU kernel path, but limits splitting:
#     - split_factor=2 caps each chunk at about 2x the original GPU batch
#     - free_min/depth_min/score_min split only truly heavy-looking safe roots
#   This is a measurement build.  If it regresses, keep 84 as the production path.
HEAVY_SPLIT_FACTOR_DEFAULT:int=2
HEAVY_SPLIT_FREE_MIN_DEFAULT:int=8
HEAVY_SPLIT_DEPTH_MIN_DEFAULT:int=12
HEAVY_SPLIT_SCORE_MIN_DEFAULT:int=96
...
####################################################
#
# 90 heavy split limited GPU stream runner
#
####################################################

"""90 heavy split limited: TaskSoA の1件を別 TaskSoA へコピーする。"""
def copy_soa_task(src:TaskSoA,src_w:List[u64],i:int,dst:TaskSoA,dst_w:List[u64],o:int)->None:
  dst.ld_arr[o]=src.ld_arr[i]
  dst.rd_arr[o]=src.rd_arr[i]
  dst.col_arr[o]=src.col_arr[i]
  dst.row_arr[o]=src.row_arr[i]
  dst.free_arr[o]=src.free_arr[i]
  dst.jmark_arr[o]=src.jmark_arr[i]
  dst.end_arr[o]=src.end_arr[i]
  dst.mark1_arr[o]=src.mark1_arr[i]
  dst.mark2_arr[o]=src.mark2_arr[i]
  dst.funcid_arr[o]=src.funcid_arr[i]
  dst.ijkl_arr[o]=src.ijkl_arr[i]
  dst_w[o]=src_w[i]

"""90 heavy split limited: 現在行で通常 +1 遷移だけを安全にhost側展開できるか判定する。"""
def heavy_split_is_safe(fid:int,row:int,jmark:int,endmark:int,mark1:int,mark2:int)->bool:
  # Same bit masks as kernel_dfs_iter_gpu().  Avoid per-task list allocation.
  META_AVAIL_MASK:int=69226252
  IS_BASE_MASK:int=69222408
  IS_JMARK_MASK:int=4
  IS_MARK_MASK:int=199209203
  IS_P5_MASK:int=(1<<8)|(1<<9)|(1<<10)|(1<<11)
  SEL2_MASK:int=(1<<1)|(1<<6)|(1<<13)|(1<<17)|(1<<20)|(1<<25)

  if fid<0 or fid>=28:
    return False

  # base row must be handled by the GPU kernel; do not split it on host.
  if ((IS_BASE_MASK>>fid)&1)==1 and row==endmark:
    return False

  # jmark special row modifies avail and ld.  Keep it inside the kernel.
  if ((IS_JMARK_MASK>>fid)&1)==1 and row==jmark:
    return False

  # P5 does a same-row transition to a different fid at mark1.
  if ((IS_P5_MASK>>fid)&1)==1 and row==mark1:
    return False

  # mark special rows use step=2/3 and blockK/blockL.  Keep them inside kernel.
  if ((IS_MARK_MASK>>fid)&1)==1:
    if ((SEL2_MASK>>fid)&1)==1:
      if row==mark2:
        return False
    else:
      if row==mark1:
...
"""90 heavy split limited: fid の avail/future flag を返す。"""
def heavy_split_use_future(fid:int)->bool:
  META_AVAIL_MASK:int=69226252
  if fid<0 or fid>=28:
    return False
  return ((META_AVAIL_MASK>>fid)&1)==1

"""90 heavy split limited: 重いが安全な root task だけを1段展開し、同じ汎用 kernel へ渡す。"""
def build_heavy_split_soa(
  N:int,
  src:TaskSoA,
  src_w:List[u64],
  m:int,
  ...
  out_count:int=0
  split_roots:int=0
  emitted_children:int=0
  kept_roots:int=0

  for i in range(m):
    fid:int=src.funcid_arr[i]
    row:int=src.row_arr[i]
    endmark:int=src.end_arr[i]
    free:int=src.free_arr[i]
    depth:int=endmark-row
    if depth<0:
      depth=0
    pc:int=popcount_int(free)
    score:int=pc*depth

    # Reserve room for all remaining roots if they need to be kept unsplit.
    remaining_after:int=m-i-1
    can_split:bool=True
    if pc<free_min:
      can_split=False
    if depth<depth_min:
    ...
    if score<score_min:
      can_split=False
    if not heavy_split_is_safe(fid,row,src.jmark_arr[i],endmark,src.mark1_arr[i],src.mark2_arr[i]):
      can_split=False
    if out_count+pc+remaining_after>max_out:
      can_split=False
    ...
    if can_split:
      ...
      child_count:int=0
      row1:int=row+1
      use_future:bool=heavy_split_use_future(fid)
      while avail:
        bit:int=avail&-avail
        avail&=avail-1

        nld:int=(src.ld_arr[i]|bit)<<1
        nrd:int=(src.rd_arr[i]|bit)>>1
        ncol:int=src.col_arr[i]|bit
        nf:int=board_mask&~(nld|nrd|ncol)
        if nf==0:
          continue

        # Match the 84 GPU kernel's generic future check exactly.
        if use_future and row1<endmark:
          if (board_mask&~((nld<<1)|(nrd>>1)|ncol))==0:
            continue

        dst.ld_arr[out_count]=nld
        dst.rd_arr[out_count]=nrd
        dst.col_arr[out_count]=ncol
        dst.row_arr[out_count]=row1
        dst.free_arr[out_count]=nf
        dst.jmark_arr[out_count]=src.jmark_arr[i]
        dst.end_arr[out_count]=endmark
        dst.mark1_arr[out_count]=src.mark1_arr[i]
        dst.mark2_arr[out_count]=src.mark2_arr[i]
        dst.funcid_arr[out_count]=fid
        dst.ijkl_arr[out_count]=src.ijkl_arr[i]
        dst_w[out_count]=src_w[i]
      ...
      copy_soa_task(src,src_w,i,dst,dst_w,out_count)
      out_count+=1
      kept_roots+=1

  return out_count,split_roots,emitted_children,kept_roots

"""90 heavy split limited progress を TSV に追記する。"""
def append_heavy_split_progress(progress_fname:str,N:int,preset_queens:int,chunk_index:int,off:int,in_m:int,out_m:int,split_roots:int,split_children:int,kept_roots:int,chunk_total:int,gpu_total:int)->None:
  with open(progress_fname,"a") as f:
    f.write(f"{N}\t{preset_queens}\t{chunk_index}\t{off}\t{in_m}\t{out_m}\t{split_roots}\t{split_children}\t{kept_roots}\t{chunk_total}\t{gpu_total}\n")

"""bin をstream読みし、安全な重いtaskだけhost側で1段展開して同じGPU kernelへ投入する。"""
def exec_solutions_gpu_bin_stream_heavy_split(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  debug_chunk_start:int=0,
  debug_chunk_count:int=1,
  split_factor:int=HEAVY_SPLIT_FACTOR_DEFAULT,
  free_min:int=HEAVY_SPLIT_FREE_MIN_DEFAULT,
  depth_min:int=HEAVY_SPLIT_DEPTH_MIN_DEFAULT,
  score_min:int=HEAVY_SPLIT_SCORE_MIN_DEFAULT
)->int:
  ...
  if split_factor<1:
    split_factor=1
  split_steps:int=STEPS*split_factor
  if split_steps<STEPS:
    split_steps=STEPS

  total_records:int=count_constellations_bin_records(fname)
  progress_fname:str=f"progress_N{N}_{preset_queens}_heavy_split_limited.tsv"
  with open(progress_fname,"w") as pf:
    pf.write("N\tpreset\tchunk\toff\tin_m\tout_m\tsplit_roots\tsplit_children\tkept_roots\tchunk_total\tgpu_total\n")
  ...
  if gpu_log_level>=1:
    print(f"[heavy-split-limited-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} split_factor={split_factor} split_steps={split_steps} free_min={free_min} depth_min={depth_min} score_min={score_min} chunk_only={1 if chunk_only else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} progress={progress_fname}")

  raw_soa:TaskSoA=TaskSoA(STEPS)
  raw_w:List[u64]=[u64(0)]*STEPS
  work_soa:TaskSoA=TaskSoA(split_steps)
  work_w:List[u64]=[u64(0)]*split_steps
  results:List[u64]=[u64(0)]*split_steps
  meta_next:List[u8]=[
    u8(1),u8(2),u8(3),u8(3),u8(2),u8(6),u8(2),u8(2),
    u8(0),u8(4),u8(5),u8(7),u8(13),u8(14),u8(14),u8(14),
    u8(17),u8(14),u8(14),u8(20),u8(21),u8(21),u8(21),u8(25),
    u8(21),u8(21),u8(26),u8(26)
  ]
  board_mask:int=(1<<N)-1
  n3:int=1<<(N-3)
  n4:int=1<<(N-4)
  ...
  executed_chunks:int=0
  total_out:int=0
  total_split_roots:int=0
  total_split_children:int=0
  total_kept_roots:int=0
  _read_uint32_le=read_uint32_le
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[heavy-split-limited-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m
          chunk_index+=1
          continue
      ...
      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[heavy-split-limited-chunk-start] N={N} chunk={chunk_index} off={off} in_m={m}")

      build_soa_for_range(N,chunk_constellations,0,m,raw_soa,raw_w)
      t1=datetime.now()
      out_m,split_roots,split_children,kept_roots=build_heavy_split_soa(N,raw_soa,raw_w,m,work_soa,work_w,split_steps,free_min,depth_min,score_min)
      t2=datetime.now()

      chunk_total:int=0
      if out_m>0:
        GRID:int=(out_m+BLOCK-1)//BLOCK
        kernel_dfs_iter_gpu(
          gpu.raw(work_soa.ld_arr),gpu.raw(work_soa.rd_arr),gpu.raw(work_soa.col_arr),
          gpu.raw(work_soa.row_arr),gpu.raw(work_soa.free_arr),
          gpu.raw(work_soa.jmark_arr),gpu.raw(work_soa.end_arr),
          gpu.raw(work_soa.mark1_arr),gpu.raw(work_soa.mark2_arr),
          gpu.raw(work_soa.funcid_arr),gpu.raw(work_w),
          gpu.raw(meta_next),
          gpu.raw(results),
          out_m,board_mask,
          n3,n4,
      ...
      executed_chunks+=1
      total_out+=out_m
      total_split_roots+=split_roots
      total_split_children+=split_children
      total_kept_roots+=kept_roots
      append_heavy_split_progress(progress_fname,N,preset_queens,chunk_index,off,m,out_m,split_roots,split_children,kept_roots,chunk_total,gpu_total)
      t4=datetime.now()

      if gpu_log_level>=1:
        print(f"[heavy-split-limited-chunk-end] N={N} chunk={chunk_index} off={off} in_m={m} out_m={out_m} split_roots={split_roots} split_children={split_children} kept={kept_roots} grid={GRID} build={str(t1-t0)[:-3]} split={str(t2-t1)[:-3]} kernel={str(t3-t2)[:-3]} sum={str(t4-t3)[:-3]} chunk_total={chunk_total} gpu_total={gpu_total}")

      off+=m
      chunk_index+=1

  if gpu_log_level>=1:
    print(f"[heavy-split-limited-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} total_out={total_out} split_roots={total_split_roots} split_children={total_split_children} kept_roots={total_kept_roots} progress={progress_fname}")
...
def main()->None:
  ...
  if use_gpu:
    ...
    if bench_mode==12:
      print(f"heavy_split_limited_stream: mode={bench_mode} preset={preset_queens_arg} factor={HEAVY_SPLIT_FACTOR_DEFAULT} free_min={HEAVY_SPLIT_FREE_MIN_DEFAULT} depth_min={HEAVY_SPLIT_DEPTH_MIN_DEFAULT} score_min={HEAVY_SPLIT_SCORE_MIN_DEFAULT}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and not (bench_mode==8 or bench_mode==9 or bench_mode==10):
      ...
      if bench_mode==12:
        total:int=exec_solutions_gpu_bin_stream_heavy_split(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count)
      else:
      ...
      if bench_mode==12:
        status="heavy-split-limited-ok" if expected[N]==total else f"heavy-split-limited-ng({total}!={expected[N]})"
