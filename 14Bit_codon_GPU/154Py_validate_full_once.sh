...
# 154 single-pass validation harness
#
...
# Candidate 154 retains the validated 153 DFS logic and changes only GPU input representation:
# ld/rd/col/free SoA arrays plus board_mask/n3/n4 are passed as u32.
# A real flock guard prevents two copies of this harness from competing.

SRC=${SRC:-./154Py_gpu_input_u32_probe.py}
CAND=${CAND:-./154Py_gpu_input_u32_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/154Py_nqueens_gpu_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 153/152/151/150/149 full runs and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 154 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/154Py_logs_N21_full_once_${TS}"
...
if [[ -f "$SRC" ]]; then
  ...
  grep -Fq 'ld_arr:Ptr[u32],rd_arr:Ptr[u32],col_arr:Ptr[u32]' "$SRC" || source_failures=$((source_failures+1))
  ...
  grep -Fq 'board_mask_gpu:u32=u32(board_mask)' "$SRC" || source_failures=$((source_failures+1))
  grep -Fq 'n3_gpu:u32=' "$SRC" || source_failures=$((source_failures+1))
  grep -Fq 'n4_gpu:u32=' "$SRC" || source_failures=$((source_failures+1))
  if grep -Fq 'ld[0]=u32(ld_arr[i])' "$SRC" || \
     grep -Fq 'rd[0]=u32(rd_arr[i])' "$SRC" || \
     grep -Fq 'col[0]=u32(col_arr[i])' "$SRC" || \
     grep -Fq 'free0:u32=u32(free_arr[i])' "$SRC" || \
     grep -Fq 'bm:u32=u32(board_mask)' "$SRC" || \
     grep -Fq 'n3u:u32=u32(n3)' "$SRC" || \
     grep -Fq 'n4u:u32=u32(n4)' "$SRC"; then
    source_failures=$((source_failures+1))
  fi
  if (( source_failures != 0 )); then
    printf 'source_u32_shape\trequested layout\t%d check failures\tFAIL\n' "$source_failures" >> "$SUMMARY"
    echo "[error] source does not match the requested 154 u32 GPU-input experiment" >&2
    exit 65
  fi
  printf 'source_u32_shape\trequested layout\tverified\tOK\n' >> "$SUMMARY"
fi
...
echo "[validation-ok] 154 one full run reproduced cases 01-06"
