...
- elapsed = **351.070s** (0:05:51.070)
- vs 296 (353.671s) → **+2.601s (+0.735%) 改善**
- vs 292基準 (367.413s) → 累計 **+16.343s (+4.448%) 改善**

K=48 が K=32/64 よりわずかに速い。Kスイープの最適点が K=32→K=48 にシフトしています。## 304 結果まとめと 305 の考え方

**304 採用確定**: K=48 → 351.070s。vs 296(K=32)比 **+2.601s (+0.735%)** 改善。

---

## 現在の K スイープ全体像（296 カーネルベース）

| K | elapsed | vs 292 |
|---|---|---|
| 32 | 353.671s (296) | −3.74% |
| **48** | **351.070s (304)** | **−4.45%** |
| 64 | 353.896s (299) | −3.67% |

K=48 が明確に最良点です。左側(K=40)・右側(K=56)で絞り込みを行います。

## 305 の着眼点: K=40（左側確認）

K=48 より小さい K=40 を測定します。CHUNKS=4（K=32の5・K=48の3の中間）。

```bash
STATIC_ONLY=1 bash 305Py_K40_sweep_validate_N21_full_once.sh
bash 305Py_K40_sweep_validate_N21_full_once.sh
```
"""
...
K_PER_THREAD_MAXD14:Static[int]=40


VERSION_TAG:str="305 K40-sweep probe: parent 304 K48-sweep (adopted, +2.601s/+0.735% vs 296=353.671s, 351.070s); K=48 better than K=32 and K=64; probing K=40 to bracket the optimum (K=32: 353.671s, K=48: 351.070s, K=64: 353.896s); EXPECTED_CHUNKS=ceil(2025282/(32*484*40))=ceil(2025282/619520)=4; all kernel logic unchanged from 296; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
