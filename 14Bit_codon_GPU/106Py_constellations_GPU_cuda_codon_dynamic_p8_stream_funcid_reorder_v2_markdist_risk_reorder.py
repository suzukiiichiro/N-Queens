...
106Py のソースコードを作成しました。

[106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder.py](sandbox:/mnt/data/106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder.py)

差分も付けておきます。

[105_to_106_markdist_risk_reorder.patch](sandbox:/mnt/data/105_to_106_markdist_risk_reorder.patch)

今回の 106Py は、105Py の結論どおり **`fid × gap × d1`** を risk key にしています。`d2` は `d2 = d1 + gap` なので risk key からは外しました。105Py では exact mark-distance profile により `fid=5` の many exact tail islands などが確認されていたため、その結果を split execution ではなく reorder に使う実装にしています。

追加したモードは以下です。

```text
bench_mode=26
  exact mark-distance risk reorder simulation / build-only

bench_mode=27
  106 risk-reordered bin を GPU 実行
```
...
codon build -release \
  106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder.py
```

まずは build-only の 26 でよいと思います。

```bash
./106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder \
...
```text
constellations_N22_7_markdist_risk_reorder_v1_w8_j7.bin
progress_N22_7_stream_markdist_risk_reorder_v1_w8_j7_sim.tsv
```

GPU full run は 27 です。
...
105Py の最終版ソースをベースに、kernel / DFS 本体は変更せず、stream reorder 側だけを追加しています。

この環境では `codon build` は未実行です。
...
VERSION_TAG:str="106 exact mark-distance risk reorder auto w8_j7 stream funcid gap_d1 risk balanced GPU runner from 105"
