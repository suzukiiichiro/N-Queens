...
# =============================================================================
# NQ_UPDATE_MEMO
# 215: 更新メモ: 214Py rootfirstxor は正当性OKだが大きく退行したため不採用。215Pyは204に最も近かった211Py rootfirstlate形へ戻し、MAXD14 kernelのみroot-preroll内の pr_bit alias を削り、root_first を直接 bit として使う微差実験。root_rest clear-lowbit / root_after_second XOR / futuremask / no-sibling / root one-two preroll は維持する。
...
したがって、214Pyは**正当性OKだが不採用**です。

215Pyは、214の `root_first = cur_avail ^ root_rest` を撤回し、204に最も近かった211Pyの `root_first` 遅延形へ戻したうえで、MAXD14 kernel内だけの小差分にしました。
...
```text
bash -n 215Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 215Py...sh OK
source_rootbitdirect                  OK
source_split_tag split145 split215    OK
```

ファイルはこちらです。

* [215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_probe.py)
* [215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_validate_full_once.sh

bash 215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 215 rootbitdirect memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  215 keeps the narrow root fast-start only when
...
VERSION_TAG:str="215 roottwo-rootbitdirect: 211 near-best late-root_first shape, root-preroll uses root_first directly without pr_bit alias - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """210 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate root_after_second and-not predicate."""
    ...
    # correct but slower.  215 keeps 211's late root_first extraction and 204's
    # clear-lowbit root_rest + root_after_second XOR predicate, but uses root_first
    # directly in the preroll state updates instead of copying it to an extra bit scalar.
    ...
    if root_after_second==u32(0):
      ...
      if pr_block_code!=u32(0):
        ...
        pr_nld=((cur_ld|root_first)<<pr_stepu)|pr_addvu|pr_bLiu
        pr_nrd=((cur_rd|root_first)>>pr_stepu)|pr_bKu
      else:
        pr_nld=(cur_ld|root_first)<<u32(1)
        pr_nrd=(cur_rd|root_first)>>u32(1)
      pr_ncol:u32=cur_col|root_first
...
CHUNKSHAPE148_DEFAULT_REASON:str="215 rootbitdirect reuses validated 204 scorestripe_v9 task order/cache; MAXD14 uses root_first directly inside root-preroll and removes pr_bit alias"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split215_roottwo_rootbitdirect_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split215_roottwo_rootbitdirect_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split215-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split215-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split215_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split215-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split215-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split215_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
