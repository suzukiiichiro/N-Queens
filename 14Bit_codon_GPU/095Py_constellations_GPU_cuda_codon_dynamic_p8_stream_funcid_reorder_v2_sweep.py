...
添付の **94Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2.py** と、前回までのメモ **20260524ChatGPT.txt** は受け取っています。

今日の続きは、現状の流れだと **94本線のN22 mode15結果確認 → N23/N24への展開 → 必要なら95 parameter sweep** の順で進めるのが自然です。

はい、確認すると **N22 mode15 の肝心な結果ログは保存されていません**。

添付された `run94_N22_mode15_gpu.log` は、`date`、`hostname`、`pwd`、`nvidia-smi` までは残っていますが、`GPU mode selected` や `[funcid-reorder-v2-gpu-summary]`、`[funcid-reorder-v2-gpu-done]`、最終の `22: ... ok` がありません。最後は `EXIT=130` なので、実行は **Ctrl-C / SIGINT で中断**されています。

一方、`mode14` は正常です。`constellations_N22_7_funcid_reorder_v2.bin` は `records=28719035`、`chunks=1855`、`valid=1` まで作成・検証できています。
つまり **reorder済み `.bin` はOK、N22のGPU計算結果だけが未確定** です。
...
で、mode15 は実行後に `[funcid-reorder-v2-gpu-done]` と最終 `22: ... ok/ng` を出す設計です。

まずサーバー上で、`progress_N22_7_stream_funcid_reorder_v2.tsv` から復元できるか確認してください。

```bash
ls -lh progress_N22_7_stream_funcid_reorder_v2.tsv run94_N22_mode15_gpu.log

grep -E "funcid-reorder-v2-gpu-summary|funcid-reorder-v2-gpu-done|^22:" \
  run94_N22_mode15_gpu.log || echo "NO_FINAL_IN_LOG"
...
END {
  ...
  if (last_chunk==1854 && last_done==28719035 && last_remaining==0 && last_gpu_total==2691008701644) {
...
が出れば、**ログ最終行は失われていますが、progress TSV からN22成功は復元できます**。その場合は保存用にこれを作ってください。

```bash
{
  echo "===== recovered from progress_N22_7_stream_funcid_reorder_v2.tsv ====="
  awk -F'\t' '
  NR==1 {
    for (i=1; i<=NF; i++) c[$i]=i
    next
  ...
  END {
    ...
    if (last_chunk==1854 && last_done==28719035 && last_remaining==0 && last_gpu_total==2691008701644) {
      print "22:     2691008701644                0      recovered-from-progress    ok"
    } else {
      print "22:     " last_gpu_total "                0      recovered-from-progress    incomplete-or-ng"
    }
  }
  ' progress_N22_7_stream_funcid_reorder_v2.tsv
} | tee recovered_run94_N22_mode15_result.txt
```

もし `RECOVERED_N22_OK` にならない場合は、**N22 mode15 は再実行が必要**です。`mode14` の `.bin` は正常なので、再実行対象は mode15 だけで大丈夫です。

次はログを上書きせず、タイムスタンプ付きで確実に保存する形にします。

```bash
mkdir -p logs

RUN=run94_N22_mode15_gpu_$(date +%Y%m%d_%H%M%S)
LOG="logs/${RUN}.log"
PROG="progress_N22_7_stream_funcid_reorder_v2.tsv"

# 既存progressがあれば退避
if [ -f "$PROG" ]; then
  cp -av "$PROG" "${PROG}.${RUN}.bak"
fi
...
set +e
stdbuf -oL -eL ./94Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2 \
  -g 22 22 32 484 1 0 5 15 2>&1 | tee -a "$LOG"
STATUS=${PIPESTATUS[0]}
set -e
...
```text
[funcid-reorder-v2-gpu-summary] N=22 ... total=2691008701644 ...
[funcid-reorder-v2-gpu-done] N=22 source_records=28719035 reordered_records=28719035 chunks=1855 ...
...
```text
mode14 simulation:
  成功。reordered .bin 作成済み、valid=1。

mode15 GPU:
  添付ログ上は未完了。
  EXIT=130 なので中断扱い。
  N22結果は run94_N22_mode15_gpu.log からは確認不能。

次の一手:
  1. progress_N22_7_stream_funcid_reorder_v2.tsv の最終行で復元確認
  2. 復元できなければ mode15 のみ再実行
```

今回の `sort -k11,11nr ... | head -20` は「重いchunk上位」を見る解析なので、**完走判定には使えません**。完走判定は必ず `tail -1` 相当、つまり `last_chunk=1854 / remaining_records=0 / gpu_total=2691008701644` で見ます。

はい、これは **N22 mode15 成功で確定** でよいです。
...
が揃っているので、**N22の正解値 `2691008701644` に到達して全recordsを処理済み**です。
最後の `last_m=4283` も正常です。

```text
15488 * 1854 + 4283 = 28719035
```

なので、chunk `1854` が端数最終chunkであることも一致しています。

ただし注意点として、`run94_N22_mode15_gpu.log` は保存記録としては使えません。`NO_FINAL_IN_LOG` なので、**成果は progress TSV からの復元結果**として保存するのが正しいです。

そのまま以下を作ってください。

```bash
mkdir -p logs/recovered

REC="logs/recovered/recovered_run94_N22_mode15_from_progress_$(date +%Y%m%d_%H%M%S).txt"

{
  echo "===== recovered N22 mode15 result from progress TSV ====="
  date
  hostname
  pwd
  echo
  ...
  echo
  echo "===== checksums ====="
  sha256sum progress_N22_7_stream_funcid_reorder_v2.tsv 2>/dev/null || true
  sha256sum constellations_N22_7_funcid_reorder_v2.bin 2>/dev/null || true
} | tee "$REC"

echo "saved: $REC"
```

これで、保存上の扱いは次のようにできます。
...
N22 mode15:
  run94_N22_mode15_gpu.log は最終行欠損
  ただし progress TSV から成功復元済み
  total=2691008701644
  done_records=28719035
  remaining_records=0
  status=recovered ok
...
一点だけベンチマーク上の注意です。`sum_chunk_elapsed=13290.452 sec` は、

```text
13290.452 秒 = 約 3:41:30.452
```

ですが、これは progress TSV の chunk elapsed 合計です。最終ログの `22: ... hh:mm:ss.ms ok` が残っていないため、**正確な wall-clock ベンチマーク値としては弱い**です。
ただし、**正しさ確認・N22通過判定としては十分**です。

結論です。

```text
N22は再実行不要。
ただし「正式ログ」は recovered-from-progress として保存する。
次は N23/N24へ進めてよい。
```

2026-05-25の添付ログを見る限り、**進捗できる項目はあります**。結論としては、**N22の正しさは確定、ただし94をそのままN23/N24本線に昇格するかは性能面で一段確認した方がよい**です。

## 進捗できる項目

### 1. N22 mode15 は「recovered ok」として確定保存できる

`run94_N22_mode15_gpu.log` 自体は `EXIT=130` で終わっており、正式な `22: ... ok` 行は残っていません。つまり、このログ単体は正式完走ログとしては使えません。

ただし、貼っていただいた progress TSV の値は、
...
なので、**計算結果としてはN22成功で確定**です。これはまず進捗項目1つ目です。

保存上の扱いは次でよいです。

```text
N22 mode15:
  status = recovered-from-progress ok
  total  = 2691008701644
  records processed = 28719035 / 28719035
  chunks = 1855
  final chunk = 1854
  final m = 4283
```

### 2. N22 mode14 simulation-only は完了済みとして扱える

`run94_N22_mode14_sim.log` では、`constellations_N22_7.bin` を読み、`records=28719035`、`steps=15488`、`chunks=1855` の reordered `.bin` を作成し、`valid=1` まで確認されています。

特にここが重要です。

```text
[funcid-reorder-v2-build-summary]
N=22 records=28719035 chunks=1855
output=constellations_N22_7_funcid_reorder_v2.bin
valid=1
```

したがって、**94 v2 の reordered input 生成は成功済み**です。これはN23/N24へ展開するための前提確認として有効です。

### 3. risk bucket の均一化は成功している

simulation TSV の比率確認では、通常chunkでほぼ一貫して、
...
になっています。ログでも先頭側のchunk比率が連続して同じ形になっており、94の狙いだった **risk bucket の均一化** は機能しています。

つまり次に見るべき問題は、単純な risk 比率ではなく、

```text
同じ比率でも、bucket内部の重さ分布がまだ偏っていないか
phase_jump / window_mult がN22に合っているか
```

です。

### 4. N22の重いchunk・低効率chunkの候補が取れている

`sort -k11,11nr` の結果では、N22 94v2 の重いchunk上位が出ています。例えば、chunk `2` が `10725 ms`、chunk `64` が `10696 ms`、chunk `1634` が `10537 ms`、chunk `1636` が `10493 ms` です。

また、`chunk_total / elapsed_ms` の低い順では、chunk `1364〜1376` 周辺、`1216〜1220` 周辺、最終chunk `1854` などが候補として出ています。

ここから進められる分析項目はこれです。

```text
A. elapsed_ms 上位chunk
  → 純粋に遅いchunk候補

B. chunk_total / elapsed_ms が低いchunk
  → 時間の割に解数が出ない低効率chunk候補

C. chunk番号の集中帯
  → phase/stripe sampling がまだ同じ重い領域を拾っている可能性
```

### 5. 94は「正しい」が、N22では性能評価を保留した方がよい

94のソースコメント上、94 v2 は kernel / DFS logic を変えず、stream-side reordering だけを変えた版です。つまり、正解が一致した場合、差分評価は主に **並べ替えの良し悪し** として見られます。

一方で、貼っていただいたN22の progress 合計は、
...
過去の84 stream 32x484 のN22ログには、

```text
22: 2691008701644 ... 3:18:42.963 ok
```

という記録があります。

これは厳密には `progress TSVのchunk elapsed合計` と `最終ログのwall-clock` の比較なので完全に同一指標ではありませんが、少なくとも **N22では94 v2が84より明確に速いとはまだ言えません**。むしろ、今回の数字だけ見ると **N21では94が良かったが、N22では再調整が必要** という判断が自然です。

### 6. 95 parameter sweep に進める根拠が揃った

メモ上でも、95候補は「94 parameter sweep」で、候補値は以下になっています。

```text
window_mult = 8,16,32
phase_jump  = 5,7,11
目的: 94の 16/7 が最適に近いか確認する
```

今回のN22ログから見ると、これはかなり妥当です。
理由は、94の現行 `16/7` は **risk比率の均一化には成功**している一方、**N22のelapsed面では84を上回ったとは言い切れない**からです。
...
```text
1. N22 recovered result を正式保存
2. N22 94v2 の chunk 統計を1枚にまとめる
3. 95 parameter sweep を作る
4. N21で軽く全組み合わせ確認
5. N22では有望な2〜3組だけmode15実測
6. 勝った設定でN23へ進む
```

N23へすぐ進むこと自体は可能です。N22の正しさは確認できています。
ただ、**速度改善が目的なら、N23へ長時間投入する前に95でN22向けの最適値を探した方がよい**です。

## 追加で作るとよいログ項目

今後の進捗管理用に、94/95ではこの項目を必ず残すとよいです。
...
p95_chunk_ms
p99_chunk_ms
top20_elapsed_chunks
bottom20_solution_per_ms_chunks
```

特に、今回は最終ログが欠けたので、**progress TSVから自動で result summary を吐く仕組み**を94または95に入れる価値があります。
...
確定できるもの:
  N22 mode14 reordered .bin valid=1
  N22 mode15 recovered ok
  N22 total=2691008701644
  94 v2 risk ratio balancing は成功

次にやるべきもの:
  95 parameter sweep
  window_mult / phase_jump の再調整
  N22 chunk統計の保存
...
私の判断では、**「N22は通過、ただし94の16/7をN23本線に固定する前に95 sweep」** が一番堅いです。


はい。**95ソースコード**を作成しました。94をベースに、`window_mult / phase_jump` を切り替えられる **parameter sweep版**です。

* ソース本体: [95Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_sweep.py](sandbox:/mnt/data/95Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_sweep.py)
* 94からの差分パッチ: [95_from_94.patch](sandbox:/mnt/data/95_from_94.patch)

95の方針は、前回メモの **「94 parameter sweep / window_mult = 8,16,32 / phase_jump = 5,7,11」** に沿っています。
また、94側の方針どおり、**kernel / DFS logic は変更せず、stream-side reordering だけを変更**しています。

主な変更点です。

```text
VERSION:
  95 stream funcid risk balanced reorder v2 parameter sweep GPU runner from 94
...
追加:
  bench_mode=16
    8,16,32 × 5,7,11 の9条件を simulation-only で作成

変更:
  bench_mode=14 / 15 で window_mult / phase_jump を引数指定可能

出力ファイル:
  constellations_N22_7_funcid_reorder_v2_w16_j7.bin
  progress_N22_7_stream_funcid_reorder_v2_w16_j7_sim.tsv
  progress_N22_7_stream_funcid_reorder_v2_w16_j7.tsv
```

ビルドはこれです。

```bash
codon build -release 95Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_sweep.py
```

まず **N22の9条件 simulation-only sweep** はこれで回せます。

```bash
./95Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_sweep \
  -g 22 22 32 484 1 0 5 16 \
  2>&1 | tee run95_N22_mode16_sweep_sim.log
```

これはGPU計算はせず、9条件ぶんの reordered `.bin` と simulation TSV を作ります。N22では1本あたり約439MBになるので、9本で約4GB程度を見てください。

個別条件だけ作る場合は、`bench_mode=14` の後ろに `window_mult phase_jump` を渡します。

```bash
# 例: window_mult=8, phase_jump=5
./95Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_sweep \
  -g 22 22 32 484 1 0 5 14 8 5 \
  2>&1 | tee run95_N22_w8_j5_mode14_sim.log
```

GPU実測は `bench_mode=15` です。
...
その後、simulation TSV を見て、候補を2〜3個に絞って `mode15` GPU実測に進むのが安全です。
こちらの環境ではCodon実行環境がないため、`codon build` までは実行していません。
...
VERSION_TAG:str="95 stream funcid risk balanced reorder v2 parameter sweep GPU runner from 94"
CROSS_STRIPE_SAFE_DEFAULT:bool=False

# 95 FUNCID REORDER V2 PARAMETER SWEEP:
...
"""95 funcid reorder: reorder 済み bin 名。window/phase を含めて sweep で上書きを避ける。"""
...
FUNCID_REORDER_V2_WINDOW_MULT:int=16
...
def main()->None:
  ...
  if reorder_window_mult<=0:
    print(f"[warning] reorder_window_mult={reorder_window_mult} is invalid; using 16")
    reorder_window_mult=16
  ...
  if use_gpu:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==16:
      print(f"funcid_reorder_v2_params: window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} param={funcid_reorder_param_tag()}")
    ...
    if bench_mode==16:
      ...
      while wi<len(sweep_windows):
        ...
        while pj_i<len(sweep_phases):
          ...
          print(f"[funcid-reorder-v2-sweep-sim] N={N} preset_queens={preset_queens} records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} valid={1 if validate_bin_file(reorder_fname) else 0} elapsed={one_text}")
          sweep_count+=1
          pj_i+=1
        wi+=1
      time_elapsed=datetime.now()-start_time
