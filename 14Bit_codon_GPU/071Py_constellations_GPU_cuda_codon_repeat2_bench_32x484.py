...
auto:
  N=20      -> sort_mode=9   # cross stripe safe only
  otherwise -> sort_mode=0
...
suzuki@cudacodon$ ./64Py_constellations_GPU_cuda_codon_balanced_stripe_auto_32x484 -g 5 20 32 484 0 -1
GPU mode selected
...
12:             14200                0          0:00:00.013    ok
13:             73712                0          0:00:00.016    ok
14:            365596                0          0:00:00.031    ok
15:           2279184                0          0:00:00.060    ok
16:          14772512                0          0:00:00.141    ok
17:          95815104                0          0:00:00.510    ok
18:         666090624                0          0:00:02.813    ok
19:        4968057848                0          0:00:20.765    ok
20:       39029188884                0          0:02:42.602    ok
suzuki@cudacodon$ 
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  # sort_mode auto:
  #   64 BALANCED STRIPE AUTO:
  #     N=20 は sort_mode=7（balanced stripe only）を採用。
  #     N!=20 は従来どおり sort_mode=0。
  #   sort_mode=5（stripe + sort4）と sort_mode=8（balanced stripe + sort4）は
  #   実測で悪化したため auto には入れない。
  if gpu_sort_mode < 0:
    gpu_sort_mode = 9 if N == 20 else 0
  ...
  if use_gpu:
    ...
    if stripe_chunks:
      ...
      if gpu_sort_mode==5 or gpu_sort_mode==8 or gpu_sort_mode==10:
        ...
        local_sort_mode=0
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk68={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} local_sort={local_sort_mode} stripe={1 if stripe_chunks else 0} balanced={1 if balanced_stripe else 0} cross={1 if cross_stripe else 0} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
    ...
    if gpu_log_level>=1:
      print(f"[gpu-summary] N={N} constellations={m_all} chunks={chunks} direct_total=1 stripe={1 if stripe_chunks else 0} balanced={1 if balanced_stripe else 0} cross={1 if cross_stripe else 0}")
    return
  ##########
  # CPU 
  ##########
...
def main()->None:
  ...
  # 71 REPEAT2 BENCH: preset_queens は 5 固定。preset=6 は 48 で正解には直せたが速度面で不採用。
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_block gpu_max_blocks log_level sort_mode] [preset_queens] [bench_mode]")
      return
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 70 REPEAT SWEEP; using 5")
      preset_queens_arg=5
    if argc >= 10:
  ...
  for N in range(nmin,nmax):
    ...
    if use_solution_cache:
        ...
        if bench_mode==1 and use_gpu and N==20:
          ...
          exec_solutions(N,constellations,use_gpu,gpu_block,gpu_max_blocks,0,gpu_sort_mode)
          ...
          # 71 REPEAT2 BENCH: run1 warmup, run2 measured.
          if gpu_log_level>=1:
            print(f"[constellation-config] N={N} preset_queens={preset_queens} constellations={len(constellations)}")
          ...
          for run_no in range(1,3):
            ...
            exec_solutions(N,constellations,use_gpu,gpu_block,gpu_max_blocks,run_log,gpu_sort_mode)
          ...
          for PN in range(pre_start_N,20):
            ...
            exec_solutions(PN,pre_constellations,use_gpu,gpu_block,gpu_max_blocks,0,gpu_sort_mode)
