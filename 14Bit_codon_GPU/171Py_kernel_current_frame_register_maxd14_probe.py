...
selected_MAXD=14
schedule_words=0
stack_bytes_per_thread=224
warning/error/mismatch なし
```

したがって、**170Pyを現時点の最速基準**として扱ってよいです。

次の171Pyは予定どおり、170Pyを親にして **MAXD14の current frame をレジスタ保持**する版にしました。170Pyの scalar u32-pair schedule は維持し、DFSループ内だけを変更しています。

ファイルはこちらです。

* [171Py_kernel_current_frame_register_maxd14_probe.py](sandbox:/mnt/data/171Py_kernel_current_frame_register_maxd14_probe.py)
* [171Py_kernel_current_frame_register_maxd14_validate_full_once.sh](sandbox:/mnt/data/171Py_kernel_current_frame_register_maxd14_validate_full_once.sh)

171Pyの主な変更点です。

```text
170Py:
  現在frameも local stack 上で処理
  候補bitごとに avail[sp] 更新
  ld[sp] / rd[sp] / col[sp] を候補bitごとに参照

171Py:
  現在frameを scalar register に保持
    cur_ld
    cur_rd
    cur_col
    cur_avail

  local u32[14] 配列は ancestor frame の保存・復元用に限定

  descend時:
    親の current frame を ld[sp]/rd[sp]/col[sp]/avail[sp] へ一度だけspill
    子frameを cur_* に入れて続行

  backtrack時:
    spを戻し、ancestor frameを cur_* にrestore
```

期待される効果は、同一frame内での候補bit反復時に、

```text
avail[sp] = a ^ bit
ld[sp] / rd[sp] / col[sp] の再load
```

を減らすことです。

stack footprint は今回まだ変えていません。

```text
MAXD14:
  4 state stacks * 4 bytes * 14
  = 224 bytes/thread
...
maxd14_current_frame_register_schedule  1 / 1 OK
frame_opcode_equivalence                506604 cases OK
dfs_164_vs_171_register_equivalence     12000 cases OK
n21_depth14_scalar_schedule_cases       3415 cases OK
schedule_words_MAXD14                   0 OK
stack_bytes_MAXD14                      224 OK
```

GPU環境では、以下で実行してください。

```bash
chmod +x 171Py_kernel_current_frame_register_maxd14_validate_full_once.sh
./171Py_kernel_current_frame_register_maxd14_validate_full_once.sh
```

この171Pyで速くなれば次は予定どおり **172Py = ancestor stack 1段縮小** に進めます。N=21のMAXD14では active frame をレジスタに逃がせたので、次段で local stack を `14 → 13` 相当にできる可能性があります。

"""

# 171 GPU current-frame-register experiment over 170 scalar-u32pair MAXD14:
#   Branch from the validated 170 scalar-u32pair MAXD14 static-dispatch build.
#   Host-side depth computation and dispatch remain 14/16/18/20/21.  The
#   MAXD14 schedule stays in PTX-safe scalar u32 fields, and the DFS hot loop
#   now keeps the current frame in scalar registers instead of repeatedly
#   loading/storing ld[sp]/rd[sp]/col[sp]/avail[sp] for every candidate bit.
#
...
#
#   MAXD14 local storage remains four u32[14] ancestor/state arrays
#   (224 bytes/thread).  171 is intentionally not the ancestor-stack shrink step;
#   it isolates current-frame register caching.  MAXD16/18/20/21 keep the
...
# 171 static stack specializations.  MAXD remains the number of stored state
# frames.  MAXD14 uses scalar u32 compact schedule fields, no local u32 schedule
# array, and a register-held current frame in the DFS loop.  Larger fallbacks
# keep the byte-opcode local schedule.
#   MAXD14 bytes/thread = 4 state stacks * 4 bytes * 14 = 224.
...
SCHED_WORDS14:Static[int]=0  # 171 MAXD14 uses scalar u32 schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="171 current-frame register cache for MAXD14 over 170 scalar-u32pair schedule - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 171 inherits the exact 170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """171 MAXD14 scalar schedule plus register-held current DFS frame."""
    ...
    ld=__array__[u32](MAXD14)
    rd=__array__[u32](MAXD14)
    col=__array__[u32](MAXD14)
    avail=__array__[u32](MAXD14)
    bm:u32=board_mask
    ...
    # 171 current-frame register cache.  The local arrays hold ancestors only.
    # The active frame lives in scalar variables, so rejected/terminal candidate
    # bits do not write avail[sp], and ld/rd/col for the active frame are not
    # reloaded from local memory for every bit.  Before descending, the parent
    # register frame is spilled once so backtracking can restore it.
    sp:int=0
    cur_ld:u32=root_ld
    cur_rd:u32=root_rd
    ...
    while True:
      ...
      compact_op:u32=u32(0)
      if sp<10:
        compact_op=(schedule_lo>>u32(sp*3))&u32(7)
      else:
        compact_op=(schedule_hi>>u32((sp-10)*3))&u32(7)
      block_code:u32=u32(0)
      future_check:u32=u32(0)
      if compact_op==u32(1):
        future_check=u32(1)
      elif compact_op>=u32(2):
        block_code=compact_op-u32(1)
...
def kernel_dfs_iter_gpu_maxd16(
    ...
    """171 inherited byte-opcode schedule fallback, static MAXD=16."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd18(
    ...
    """171 inherited byte-opcode schedule fallback, static MAXD=18."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd20(
    ...
    """171 inherited byte-opcode schedule fallback, static MAXD=20."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd21(
    ...
    """171 inherited byte-opcode schedule fallback, static MAXD=21."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
####################################################################################################
# 171 current-frame-register MAXD14 + scalar-u32pair schedule harness
#
# This build defines five capacity-specialized packed-schedule @gpu.kernel variants.
# It keeps the proven 169 MAXD14 dispatch and split/chunk-shape paths,
# changing only the MAXD14 hot loop to keep the current frame in registers.
# The 170 scalar-u32pair schedule representation and all host dispatch logic are retained.
####################################################################################################
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164/167/168/169/170/171: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 170 scorestripe_v9 task order/cache and scalar-u32pair MAXD14 schedule; 171 keeps current DFS frame in registers"
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/167/168/169/170/171: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 171 branches from validated 170.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 171 current-frame register GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split171_current_frame_register_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split171_current_frame_register_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split171-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split171-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split171_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split171-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split171-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split171_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
