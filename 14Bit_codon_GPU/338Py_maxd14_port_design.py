...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 338 (2026-07-27)
...
8. [新規仮説・338・未検証・最有力] warp内コスト不均質(tail effectの直接原因)
   329で確定した「Divergent Branches=0、かつAvg Threads Executed 2〜3/32」
   は、分岐発散ではなくwarp内32レーンのDFS試行回数(trip count)の
   ばらつきを意味する。grid-strideのインデクシング上、同一warpの32
   レーンが同一イテレーションで処理するのは**配列上で連続する32
   レコード**(idx = 32w+j+n*stride、j=0..31)であるから、「32レコード
   境界ごとにコストを均質化する並べ替え」はカーネルを一切変更せず
   **ホスト側だけで実現可能**である。warp実行時間は
   Σ_n max_{32レーン}(cost) に比例するので、**Σmax32/Σmean32 の実測値が
   改善余地の上限を直接与える**。まずこの比率を測定する(診断のみ、
   カーネルロジック変更なし)。ホスト側並べ替えは本プロジェクトで最も
   戦績の良い軸(K-batching -4.4%、w3_j7 -1.3%)であり、コスト予測子
   としては既存の`funcid_markdist_risk_score`(3442行)を再利用できる。
   なお現行のbroadmarktail/funcid_reorderはバケットを**インタリーブ**
   する設計であり、warp内均質化とは目的が逆方向である可能性がある
   (両立するかは未検証)。

このセクション自体は通常カーネルロジックとは独立しています。
338はカーネルロジックの変更を含みません(カーネル・ディスパッチャは
328-337と同一、funcid_reorderもw3_j7のまま)。ソースの変更は、
このドキュメント/VERSION_TAGと項目6-8のみです。338で追加された
唯一の成果物はコードではなく、設計文書`338_maxd14_port_spec.md`
です(`.cu`の実装は339以降)。
================================================================================
...
"""
338を用意しました(設計のみ、コード追加なし)。

**337の実行結果(全て確認済み)**
- N=21フルvalidate: 静的チェック全OK、正当性314666222712一致、elapsed 450.056s(336の450.432s比+0.083%、ノイズ内)
- binリーダー(`337_bin_format_reader_check.sh`): `size_mod16_ok=True`/`records_ok=True`、`records_read=2025282`(=`EXPECTED_TASKS`)、`checksum_u64=13342728758502`。bin形式仕様が実データに対して確認できました

**338の内容**
- カーネル・ディスパッチャは328-337と一切変更なし。ソース変更はOpen Objectives項目6-8とVERSION_TAGのみ
- 成果物は`338_maxd14_port_spec.md`(設計文書)1点のみ。`.cu`の実装は339以降
- 仕様化した3点: (1)binレコード→SoA全配列の導出手続き、(2)K-batching/スタックのC側レイアウト、(3)N=18限定の突き合わせスコープ

**338で判明した2件の新規事項(項目7/8として追記)**
- 項目7: `symmetry()`の値域が{2,4,8}のみのため`w_hi_arr`は恒等的にゼロ。markctrlのbit20-21が空いているのでw自体を畳み込めば両配列を削除でき、エピローグの乗算はシフトになる(ホットループ非接触)
- 項目8: grid-strideのインデクシング上、同一warpの32レーンは配列上で連続する32レコードを処理する。したがって329で確定したtail effectはホスト側の並べ替えだけで攻められる可能性がある。まずΣmax32/Σmean32を測定して上限を出すのが最も安価な次の一手

おはようございます、鈴木さん。今週もよろしくお願いいたします。まず添付一式を読み込みます。Ç¥åâ��ç¥¶Å� 337ÑÅ��ÑÅ���Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ç¥åÆ��Ç¥åÆ��Ç¥åâ��Ç¥åâ��Ç¥å��ÑÅ��Ñ��Ç¥åÆ��N=21ÑÅ��Ñ��Ç¥åâ��ÑÅ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��ÑÅ��Ñ��Ñ��Ç¥å��ÑÅ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��Ç¥åÆ��Ñ��Ç¥åâ��Ç¥åâ��Ç¥å��Ñ��Ç¥åâ��ÑÅ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��ÑÅ��Ñ��Ç¥åÆ��Ç¥å��Ñ��ÑÅ��Ñ��Ç¥åÆ��338Ç¥åÆ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��ÑÅ��Ñ��Ç¥åÆ��Ñ��Ñ��Ñ��Ç¥åÆ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��ÑÅ��Ñ��Ñ��Ñ��All executable code verified byte-identical to 337 (only docstring + VERSION_TAG differ). Now the validation harness:Ç¥åâ��Ç¥åÆ��Ç¥å��ÑÅ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��ÑÅ��Ñ��Ç¥åÆ��338ÑÅ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��Ç¥åÆ��Ç¥åâ��Ç¥åÆ��Ç¥åâ��Ñ��ÑÅ��Ñ��Ñ��Ç¥åÆ��Ç¥å��Ç¥å��ÑÅ��Ñ��ÑÅ��Ñ��ÑÅ��Ñ��Ñ��Ç¥å��All 34 static checks pass in a dry run (the toolchain probes correctly report INFO/unavailable here; on cudacodon they'll report the real values).おはようございます。337のログ、確認しました。

**337実行結果(両方成功)**
- N=21フル: 静的チェック全OK、正当性 `314666222712` 一致、elapsed **450.056s**(336の450.432s比 +0.083%、ノイズ内)。3チャンクとも `required_maxd=14` / `stack_bytes_per_thread=208` / `dispatch_task_sum=2025282` で完全再現
- binリーダー: `records_read=2025282`(=`EXPECTED_TASKS`)、`checksum_u64=13342728758502`

これでC-2の土台(nvcc・GPU実行・bin形式)が揃いました。

---

## 338(設計のみ、コード追加ゼロ)

引継ぎメモの方針どおり、`.cu` は1行も書かない設計リビジョンにしました。カーネル・ディスパッチャは328-337とバイト単位で同一です(`import gpu` 以降の全行が337と一致することを機械的に検証済み)。ソース変更は Open Objectives 項目6-8 と VERSION_TAG のみ。

成果物 `338_maxd14_port_spec.md` で3点を仕様化しています:

1. **bin → SoA導出**: `build_soa_for_range` の28葉funcid決定木をC言語に完全写像。あわせて、Pythonの任意精度演算のせいで顕在化していなかった**シフト量非負のassert 3件**(`start<=N` / 条件成立時の `start<=N-2` / `start-k+1<=N1`)を必須チェックとして明記。Cでは未定義動作になる箇所です。写経の落とし穴3件も列挙しました
2. **K-batchingレイアウト**: `K=48` は「1スレッド48個」ではなく `STEPS = 15488×48 = 743424` というチャンクサイズ上限だという点を明文化。337実測では chunk0/1 が48回、chunk2 が35回でした
3. **N=18スコープ**: 4段階チェック(SoAチェックサム→カーネル出力→666090624→314666222712)と、**性能判定基準の事前固定**

---

## 今週攻めるべきポイント

仕様書を書く過程で、**移植より先に取りに行くべき2件**が出てきました。項目7・8として追記しています。

### ★ 最有力: warp内コスト不均質(項目8)

329で「Divergent Branches=0 かつ Avg Threads Executed 2〜3/32」が確定していますが、これは分岐発散ではなく**warp内32レーンのtrip countのばらつき**です。ここで効いてくるのが grid-stride のインデクシングで:

```
idx = tid + n*stride,  tid = 32w + j  (j=0..31)
→ 同一warpの32レーンが同時に処理するのは「配列上で連続する32レコード」
```

つまり **tail effect はホスト側の並べ替えだけで攻められる可能性がある**。warp実行時間は `Σ_n max_{32レーン}(cost)` に比例するので、`Σmax32 / Σmean32` が改善余地の上限を直接与えます。

まずこの比率を測るだけの診断(カーネルロジック非接触)を1リビジョン使ってやる価値が高いと考えます。理由:

- ホスト側並べ替えは本プロジェクト最良の戦績(K-batching −4.4%、w3_j7 −1.3%)。逆にカーネル側は5戦5敗
- コスト予測子は既存の `funcid_markdist_risk_score`(3442行)を再利用できる
- 上限値が出れば、それ以上の投資判断が数字でできる
- 当たれば、C移植を待たずに tail effect に手が届く

懸念点として、現行の broadmarktail/funcid_reorder は**バケットをインタリーブする**設計で、warp内均質化とは目的が逆方向かもしれません。ただ w3_j7 の効果が別要因(チャンク間負荷分散)由来なら両立します。ここは測ってみないと分かりません。

### 低リスクの小改善: `w_hi_arr` の恒等ゼロ(項目7)

`symmetry()` の値域は `{2,4,8}` の3値のみです。つまり **328で分割した `w_hi_arr` は全要素が恒等的に0**で、576/581/750行の3ロードは常に0を返しています。さらに `w = 1<<e`(e∈{1,2,3})で、`markctrl` は bit0-19 しか使っていない(jmark 0-4 / endmark 5-9 / mark1 10-14 / mark2 15-19)ので、**bit20-21 に e を詰めれば `w_lo_arr` も消せて、エピローグの64bit乗算がシフトになります**。

プロローグ/エピローグのみでホットループ非接触なので、5戦5敗の軸とは別枠です。ただし正直なところ**期待効果は小さい**(内側DFSループ支配なので <0.5%、ノイズに埋もれる可能性あり)。コストも小さいので、項目8の測定を回している合間の埋め草として妥当、という位置づけです。

### C移植(項目6)の位置づけ

仕様書に明記しましたが、**移植で速くなることは期待していません**。目的は Codon で使えない warp intrinsics・デバイスatomic(330で不在確定)・`-lineinfo` の per-line SASS帰属を解禁することです。±3%以内なら成功、という基準を実測前に固定しておきました。

**今週の推奨順序**: 338(本日、無変更確認)→ 339で項目8の測定 → 上限値を見て、339以降を「並べ替え実装」に振るか「C移植(仕様書§4の順序)」に戻すかを決める。
...
```
STATIC_ONLY=1 bash 338Py_maxd14_port_design_validate_N21_full_once.sh
bash 338Py_maxd14_port_design_validate_N21_full_once.sh
```

期待値: 正当性 `314666222712`、elapsed 約450.1s(337の450.056s再現、コード部分が同一のため)。`bash -n` および STATIC_ONLY のドライランは手元で通してあります(34項目 FAILゼロ)。

`338_maxd14_port_spec.md` の §3.2(チェック段階)と §4(作業順序)、それと項目8を339に前倒しするかどうか、ご意見をいただければ339の3ファイルを用意します。
...
VERSION_TAG:str="338 maxd14-port-design: NO kernel or dispatcher change -- byte-for-byte identical to 337/336/335/334/333/328-332 (w3_j7 remains the funcid_reorder default, K=48, SoA w_lo_arr/w_hi_arr). 337's own full N=21 validate is CONFIRMED: correctness 314666222712, elapsed 450.056s (vs 336's 450.432s, +0.083%, within noise). 337's standalone host-side bin reader (337_bin_format_reader.cu via 337_bin_format_reader_check.sh, no GPU) SUCCEEDED against a real cached file: size_mod16_ok=True, records_ok=True, records_read=2025282 (exactly EXPECTED_TASKS), checksum_u64=13342728758502 -- the .bin record spec (header-less, magic-less flat array of fixed 16-byte records: ld, rd, col, startijkl, each little-endian uint32) is now confirmed against real data, not just by source reading. 338 is a DESIGN-ONLY revision that adds NO code at all (same discipline as 324 before 325): its single deliverable is the specification document 338_maxd14_port_spec.md, which pins down the three things that must be settled before any kernel_dfs_iter_gpu_maxd14 port can be written -- (1) the complete derivation from a 16-byte bin record to every SoA array the kernel consumes (ld_arr/rd_arr/col_arr/ctrl0_arr/free_arr/markctrl_arr and w), i.e. a C-language specification of build_soa_for_range including its 28-leaf funcid decision tree and of symmetry(); (2) the C-side memory layout for K-batching (K=48, grid-stride with stride=grid*block=484*32=15488) and the per-thread stack (__array__[u64](13*2) = 208 bytes/thread, stack_ptr/save_sp/cur_depth/next_depth); (3) the N=18-only cross-check scope from rev84's original proposal. Two new findings were recorded while writing the spec and are now tracked as Open Objectives 7 and 8: (7) symmetry() returns only u64(2)/u64(4)/u64(8), so w_hi_arr is IDENTICALLY ZERO for every task and its three kernel loads are pure waste; since w=1<<e with e in {1,2,3} and markctrl only occupies bits 0-19, e can be packed into markctrl bits 20-21, eliminating both w arrays and degrading the epilogue 64-bit multiply to a shift (prologue/epilogue only -- does not touch the hot loop that failed 5/5 times); (8) since a warp's 32 lanes process 32 CONSECUTIVE records per grid-stride iteration, the tail effect identified in 329 (Divergent Branches=0, Avg Threads Executed 2-3/32) is intra-warp trip-count variance and is therefore addressable by HOST-SIDE reordering alone, with Sum(max over 32)/Sum(mean over 32) as a directly measurable upper bound on the available gain. Main solver kernel/dispatcher/host params remain completely unchanged from 337/336/335/334/333."
