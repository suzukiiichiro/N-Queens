...
# 153 single-pass validation harness
#
...
# Candidate 153 retains 152 and derives the default future flag from the complement of IS_MARK_MASK.
# A real flock guard prevents two copies of this harness from competing.

SRC=${SRC:-./153Py_future_mark_complement_probe.py}
CAND=${CAND:-./153Py_future_mark_complement_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/153Py_nqueens_gpu_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 152/151/150/149 full runs and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 153 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/153Py_logs_N21_full_once_${TS}"
...
echo "[validation-ok] 153 one full run reproduced cases 01-06"
