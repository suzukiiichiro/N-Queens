...
9. [解決・346で採用] ソートのスコープ -- CHUNKSHAPE148_ITER_SORT=1
   ...
   **最終成績(N=21)**: 402.460s。アンカー431.983s比 -6.834%、
   344 RUN=1対照450.067s比 -10.578%、328(SoA採用時)456.036s比で
   **-11.748%**。全9ラン正当性 `314666222712` 一致・FAILゼロ、
   build回4件すべてで group_sizes=743424,743424,538434 一致。
...
10. [347候補] 蛇行(serpentine)配置 -- mode 1 の残存不均衡

   mode 1 にはまだ系統的な残存不均衡がある。warp w は毎ストラタムで
   順位 `[32*w, 32*w+32)` を取るので、**warp 0 は常に各ストラタムの底、
   warp 483 は常に天井**を引く。makespanを決める最重warpが固定される。

   奇数番目のストラタム(15488件スライス)だけ順序を反転すれば、各warpが
   半分のストラタムで天井、半分で底を引くので総コストが均等化する。
   32整列は保たれる。合成分布での試算では full launch で約 -1.4%、
   partial launch で約 -0.9%。実装は数行である。

   chunk2の方向依存(項目9の未解明観測)も、蛇行を入れると挙動が
   変わるはずなので、同時に情報が得られる。

このセクション自体は通常カーネルロジックとは独立しています。
346は**採用リビジョン**です(333がw3_j7を、344がbucket_runを採用したのと
同じ位置づけ)。ソース変更は既定値2個のみ:
...
w3_j7)も、344で採用した CHUNKSHAPE148_BUCKET_RUN=2048 も、345で入れた
バッファ書き出し段の本体も、345とバイト一致です。
`CHUNKSHAPE148_ITER_SORT=0` を明示的に渡せば344の順序とキャッシュ
ファイル名をそのまま再現できるため、旧ベースラインはいつでも到達可能です。
既定実行は既存の `..._isort1.bin` を再利用するので構築は走りません。
================================================================================
...
346(採用リビジョン)の3ファイルをお渡しします。

## ソース変更は既定値2個のみ

`import gpu` 以降の実行コード差分を機械照合したところ、**+9 / −4行**、内訳は既定値2個と `VERSION_TAG`・`CHUNKSHAPE148_ITER_SORT_REASON` の文字列のみでした。
...
カーネル・ディスパッチャ・K=48・SoA・w3_j7・`BUCKET_RUN=2048`・345のバッファ書き出し段は、すべて345とバイト一致です。
...
| チェック | 内容 |
|---|---|
| `..._default_adopted` | モジュール既定値が1 |
| `..._default_old_absent` | 旧値0が消えている |
| `..._a10g_default` | A10G既定値が1 |
| `..._defaults_agree` | **2つの既定値が一致** |
| `..._zero_reaches_344` | **`iter_sort=0` が空suffixを返す設計の維持** |

最後の1件は旧ベースラインへの到達手段を守るためのものです。`chunkshape148_iter_sort_tag()` の `if m==0: return ""` が失われると、`CHUNKSHAPE148_ITER_SORT=0` を指定しても344のキャッシュファイルに戻れなくなるので、ゲートにしました。345で入れた membership guard と `SRC_NODOC` 経由の検査もすべて継承しています。

**既定実行はキャッシュ再構築が走りません。** 345スイープで構築済みの `..._run2048_isort1.bin` をそのまま再利用するので、`cache_state=reuse` で即座に402.5s近傍が出るはずです。

```
STATIC_ONLY=1 bash 346Py_iter_sort_adopt_validate_N21_full_once.sh
bash 346Py_iter_sort_adopt_validate_N21_full_once.sh
CHUNKSHAPE148_ITER_SORT=0 bash 346Py_iter_sort_adopt_validate_N21_full_once.sh   # 任意: 344相当の対照点
```

## 記録について

READMEには、確定した知見(スコープが機構であること、予測4件の的中、ノイズ床の確定)と、**未確定の観測**(mode 2 の chunk2 分裂)を明確に分けて書きました。後者は「部分launchとの相互作用を疑ったが合成分布では再現できなかったので機構は未確定、仮説として記録する」という形にしてあります。343の自己批判の件で学んだとおり、**過小にも過大にも書かない**のが記録の正確さだと考えています。

347の候補(蛇行配置、および全件一括ソート)もOpen Objectives項目10として残してあります。
...
VERSION_TAG:str="346 iter-sort-adopt: ADOPTION REVISION for the 345 sort-scope campaign, in the same role that 333 played for w3_j7 and 344 played for bucket_run. THE SOURCE CHANGE IS TWO DEFAULT CONSTANTS: CHUNKSHAPE148_ITER_SORT and A10G_FINAL_DEFAULT_CHUNKSHAPE148_ITER_SORT both go from 0 to 1. Kernels, dispatcher, K=48, the SoA w_lo_arr/w_hi_arr split, w3_j7, the adopted CHUNKSHAPE148_BUCKET_RUN=2048 and the buffered emit stage added in 345 are all byte-for-byte identical to 345. Passing CHUNKSHAPE148_ITER_SORT=0 explicitly still reproduces the 344 emit order and its cached shaped-bin filename, so the old baseline stays reachable. WHAT 345 ESTABLISHED: the thing that matters is SORT SCOPE, not sort granularity. Mode 1 and mode 3 run exactly the same sort, a stable ascending sort on key>>5, and differ only in scope. Mode 1 sorts a whole 743424 record launch group; mode 3 sorts inside a single 15488 record grid-stride iteration. Measured on N=21: mode 1 at 402.460s, mode 3 at 503.038s, against a mode 0 anchor of 431.983s. That is a 24.991 percent spread produced by one structural variable with the granularity held fixed. The reason is that a grid-stride launch hands warp w the ranks 15488*j + 32*w + lane, so sorting the whole launch group makes each warp draw exactly one sample from each of 48 equal width rank strata and balances the warps structurally, while sorting inside one iteration gives warp 0 the lightest 32 records and warp 483 the heaviest 32 records in all 48 iterations and maximises the imbalance. ALL FOUR PRE-REGISTERED PREDICTIONS HELD: mode 1 improved, mode 3 regressed, mode 2 at 403.331s matched mode 1 because the stratification makes direction close to symmetric, and mode 4 at 402.758s matched mode 1 because the low 5 lane bits only reorder records inside one score class. Modes 1, 2 and 4 sit inside a 0.216 percent band, above the measured noise floor of 0.009 percent on the total and 0.04 percent per chunk, so mode 1 being best is a real if small difference. Final standing on N=21: 402.460s, which is -6.834 percent from the 431.983s anchor, -10.578 percent from the 344 RUN=1 control of 450.067s, and -11.748 percent cumulatively from the 456.036s baseline at the time SoA was adopted in 328. THE CAMPAIGN STILL HAS NOT TOUCHED THE KERNEL. Correctness was 314666222712 on all nine runs with zero FAIL, and every build run reported group sizes of 743424, 743424 and 538434, matching the three GPU launches exactly, which is the runtime membership guard. ONE OBSERVATION REMAINS UNEXPLAINED: under mode 2 the per-chunk split diverges, with chunk0 at -8.736 percent and chunk1 at -6.843 percent beating mode 1 while chunk2 falls to -3.372 percent. chunk2 is the only partial launch at 538434 records or 34.76 iterations, against exactly 48 for chunk0 and chunk1, so an interaction with the ragged final iteration is the natural suspicion, but a synthetic simulation does not reproduce any direction asymmetry, so the mechanism is NOT established and this is recorded as a hypothesis only. NEXT AXIS for 347: mode 1 still leaves a systematic residual imbalance, because warp w takes ranks 32*w to 32*w+31 inside every stratum, so warp 0 always draws the floor of each stratum and warp 483 always draws the ceiling, which fixes which warp is heaviest. Reversing the order inside every odd numbered stratum, a serpentine or boustrophedon layout, makes each warp draw the ceiling half the time and the floor the other half and should equalise the warp totals while keeping the 32 alignment intact. A synthetic estimate puts it at about -1.4 percent on a full launch and -0.9 percent on the partial one."
...
CHUNKSHAPE148_ITER_SORT_REASON:str="346 ADOPTED the sort scope knob at 1, changed from the 345 source default of 0. 345 established that what matters is SORT SCOPE and not sort granularity. Mode 1 and mode 3 use exactly the same sort, a stable ascending sort on key>>5, and differ only in scope: mode 1 sorts a whole 743424 record launch group while mode 3 sorts inside a single 15488 record grid-stride iteration. Mode 1 measured 402.460s and mode 3 measured 503.038s against a mode 0 anchor of 431.983s, a spread of 24.991 percent from one structural variable. Mode 2, the descending variant, came in at 403.331s and mode 4, which sorts on the full key including the low 5 lane bits, at 402.758s, so direction and the lane bits are both close to irrelevant and modes 1, 2 and 4 sit inside a 0.216 percent band. All four pre-registered predictions held. Membership is preserved by construction, and every build run reported group sizes of 743424, 743424 and 538434, matching the three GPU launches exactly. Passing 0 explicitly still reproduces the 344 order and its cache filename."
