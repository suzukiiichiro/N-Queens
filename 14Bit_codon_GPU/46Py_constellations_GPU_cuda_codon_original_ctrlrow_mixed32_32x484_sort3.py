...
preset_q   = 省略時 5。末尾引数で 6 などを指定可能
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk46={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
