...
# =============================================================================
# NQ_UPDATE_MEMO
# 228: 更新メモ: 227Py generic normal-first は正当性OKだが217/225を上回らなかったため不採用。
#      228Pyは225Pyのgeneric ncol-earlyを親に、MAXD14 root-preroll内でも
#      pr_ncol=cur_col|pr_bit を pr_nld/pr_nrd 計算より前へ移動する微差実験。
#      rootrestlate / futuremask / no-sibling / root one-two preroll / generic ncol-early は維持する。
...
227Pyのnormal-firstは226Pyより戻しましたが、225Py/223Py/217Pyを上回らないため**不採用**です。

228Pyは227のnormal-firstを撤回して225Pyのgeneric ncol-earlyへ戻し、今度はroot-preroll側だけで `pr_ncol` earlyを試す版にしました。
...
STATIC_ONLY=1 bash 228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物です。

* [228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_probe.py)
* [228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_bundle_20260703.tar)

"""

# 228 rootpr-ncol-early memo over validated 217 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="228 rootpr-ncol-early: 227 normal-first withdrawn; 225 generic ncol-early plus root-preroll ncol early - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    if root_after_second==u32(0):
      ...
      # 228 rootpr-ncol-early: pr_ncol is independent of pr_block_code and
      # pr_nld/pr_nrd, mirroring the validated 225 generic ncol-early shape.
      pr_ncol:u32=cur_col|pr_bit
    ...
    while True:
      ...
      # 228 rootpr-ncol-early: ncol is independent of block_code and nld/nrd.
...
CHUNKSHAPE148_DEFAULT_REASON:str="228 rootpr-ncol-early reuses validated 225 generic ncol-early / 217 rootrestlate task order/cache; MAXD14 scalar root_after_second predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split228_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split228_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split228-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split228-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split228_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split228-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split228-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split228_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
