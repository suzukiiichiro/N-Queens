...
# =============================================================================
# 345 N=21 full validation harness (iter-sort: SORT SCOPE measurement)
#
# 345 IS A SINGLE-VARIABLE MEASUREMENT REVISION on top of the adopted 344.
# Kernels, dispatcher, K=48, the SoA w_lo/w_hi split, w3_j7 and the adopted
# CHUNKSHAPE148_BUCKET_RUN=2048 are byte-for-byte identical to 344. The only
# source change is the WRITE-OUT STAGE of build_chunkshape148_reordered_bin,
# gated by the new CHUNKSHAPE148_ITER_SORT knob whose default is 0.
#
# 344 IS CONFIRMED (2026-07-27):
#     adopted RUN=2048          431.944s
#     in-session RUN=1 control  450.067s      -4.027%
#     chunk0 -3.810%   chunk1 -4.523%   chunk2 -3.824%
#     +0.062% from 343 saturated 431.677s; the control is within -0.026% of
#     the 340 control of 450.183s; both inside the noise band.
#     cumulative from 328 (456.036s, when SoA was adopted): -5.283%
#     correctness 314666222712, 45 static checks with zero FAIL, cache reuse.
#
# A STRUCTURAL FACT CLARIFIED WHILE CHECKING 344 -- this is what 345 is about.
# There are TWO different STEPS in this source and they are not the same:
#     build_chunkshape148_reordered_bin   STEPS = BLOCK*MAX_BLOCKS
#                                               = 15488   (output chunk)
#     exec_solutions_gpu_bin_stream_split145
#                     STEPS = BLOCK*MAX_BLOCKS*K_PER_THREAD_MAXD14
#                                               = 743424  (GPU launch)
# So ONE SHAPING OUTPUT CHUNK IS ONE GRID-STRIDE ITERATION and ONE GPU LAUNCH
# IS 48 OF THEM. The 131 output chunks split 48 + 48 + 35 and line up exactly
# with the three launches of 743424 / 743424 / 538434 records. All 484 blocks
# of 32 threads are resident, so there is no block dispatch wave; the makespan
# is the worst warp total of sum over the 48 iterations of the maximum lane
# cost inside that warp.
#
# THEREFORE THE OPEN QUESTION IS SORT SCOPE, NOT SORT GRANULARITY:
#   - Sorting inside ONE 15488 iteration hands warp 0 the lightest 32 records
#     and warp 483 the heaviest 32 records in every one of the 48 iterations.
#     That maximises warp imbalance.
#   - Sorting across ONE WHOLE 743424 LAUNCH GROUP puts rank r at position r,
#     so warp w receives ranks 15488*j + 32*w + lane and draws exactly one
#     sample from each of 48 equal-width rank strata. Warps balance
#     structurally, and this absorbs the role that the order_pos =
#     (oi+out_ch)%8 rotation played up to 344.
#
# MEMBERSHIP IS PRESERVED BY CONSTRUCTION. The selection rules of the emit
# loop -- quotas, the lane scan, phase_seed, written_by_bucket -- are
# untouched. Picked indices are buffered and reordered before being written,
# and group boundaries are pinned to CHUNKSHAPE148_SORT_GROUP = 48 output
# chunks = one GPU launch, so per-launch record counts and chunk boundaries
# stay identical to 344.
#
# MODES (CHUNKSHAPE148_ITER_SORT):
#     0  off. Byte-identical to 344, same cache filename, so the existing
#        344 shaped bin is reused and this is the timing anchor.
#     1  whole launch group ascending by key>>5, stable.   <- candidate
#     2  same, descending.
#     3  inside each 15488 iteration only, ascending by key>>5, stable.
#        <- control, predicted to REGRESS
#     4  whole launch group ascending by the FULL key, low 5 lane bits included.
#
# PRE-REGISTERED PREDICTIONS:
#     mode 1 improves; mode 3 REGRESSES; mode 2 matches mode 1 because the
#     stratification makes direction symmetric; mode 4 matches mode 1 because
#     the low 5 bits only reorder records inside one score class.
#     If mode 1 improves AND mode 3 regresses, the mechanism is warp
#     stratification rather than granularity as such. Modes 1, 2 and 3 key on
#     key>>5 with a stable sort, so the 344 lane-diffused order survives
#     inside each score class; only mode 4 breaks it.
#
# MANDATORY VALIDATION ORDER:
#   1. STATIC_ONLY=1 first.
#   2. CHUNKSHAPE148_ITER_SORT=0 -- the anchor. Cache MUST report reuse and
#      the elapsed time must land near 431.9s. If it does not, stop: nothing
#      downstream is comparable.
#   3. Then modes 1 and 3 (the discriminating pair), then 2 and 4.
#   4. Confirm 314666222712 FIRST on every run, before any timing.
#
# CACHE WARNING: every non-zero mode gets its OWN shaped bin
# (..._isortN.bin) and therefore BUILDS it on first use. A run that reports
# chunkshape148_cache_state=build is NOT timing-comparable. Run each mode
# TWICE and take the second run. The harness records the cache state so this
# cannot be missed.
#
#     STATIC_ONLY=1 bash 345Py_iter_sort_probe_validate_N21_full_once.sh
#     CHUNKSHAPE148_ITER_SORT=0 bash 345Py_iter_sort_probe_validate_N21_full_once.sh
#     CHUNKSHAPE148_ITER_SORT=1 bash 345Py_iter_sort_probe_validate_N21_full_once.sh
#     CHUNKSHAPE148_ITER_SORT=3 bash 345Py_iter_sort_probe_validate_N21_full_once.sh
...
SRC=${SRC:-./345Py_iter_sort_probe.py}
CAND=${CAND:-./345Py_iter_sort_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/345Py_iter_sort_probe_N21_full.lock}
...
# 345: sort scope knob. 0 = off = byte-identical to 344 and the timing anchor.
# 1 = launch-group ascending (candidate), 2 = launch-group descending,
# 3 = iteration-local ascending (control, predicted to regress),
# 4 = launch-group ascending on the full key including the low 5 lane bits.
CHUNKSHAPE148_ITER_SORT=${CHUNKSHAPE148_ITER_SORT:-0}
...
LOGDIR="${LOG_ROOT%/}/345Py_iter_sort_probe_logs_N21_full_once_${TS}"
...
echo "[start] 345 iter-sort validation script (r2)"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 345 iter-sort r2 SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '345 iter-sort' "$SRC"; then
  printf 'source_version_tag	345 iter-sort	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	345 iter-sort	missing	FAIL
...
IS_DEFAULT=$(grep -c '^CHUNKSHAPE148_ITER_SORT:int=0$' "$SRC_NODOC" || true)
record_check source_chunkshape148_iter_sort_default_zero 1 "$IS_DEFAULT" || static_failures=$((static_failures+1))
...
record_check source_chunkshape148_iter_sort_a10g_default 0 "${IS_A10G:-missing}" || static_failures=$((static_failures+1))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 345 r2 source static checks failed" >&2
  exit 66
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 344 bucket-run-adopt (kernel-identical to 338/337/336/335/334/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN"
