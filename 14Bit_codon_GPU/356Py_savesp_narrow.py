...
"""
**356の設計です。** 352(=351)以来はじめてカーネルに触るリビジョン
である。355のncu実測で、`branch_resolving`の60%を占める2本の無条件
BRA(いずれもBSYNC隣接)の出所を、SASSの前後文脈から特定した——
`save_sp==0`の判定と、その前後のプッシュ/ポップである。

## なぜ過去5敗と違うと言えるか

240・266-269・273・326はいずれも**カーネル分解**(funcid別の専用
カーネル分岐、ホットループの複製)だった。326(ホットループ複製)は
-13.7%と最悪の結果だった。356は**分解もループ複製もしない**。
制御フローの形は一切変えず、`save_sp`という1つのスカラー変数の
**ビット幅だけ**を変える。

## 何を変えたか

`save_sp:int=0` — Codonの`int`は64bit — に対する `if save_sp==0:`
が、64bit比較(`ISETP.NE.U32.AND`+`ISETP.NE.AND.EX`の2命令)にコンパイル
されていた。355のCSVで確認した`@!P2 BREAK B2`(全wait実測の6.4%を
単独で占める最大の寄与命令)は、まさにこの比較の直後にある。

`save_sp`は`0..MAXD14_ANCESTOR`程度の範囲しか取らず、u32で十分。
4箇所を変更した(宣言1・比較1・減算1・加算2箇所)。
...
**`stack_ptr`には触れていない。** 配列添字に使われており、Codonが
u32添字を素直に受け付けるか確認できないため、リスクを避けて`int`の
まま残した。`stack_ptr`と`save_sp`は296のコメントにある通り常に
`stack_ptr=save_sp*2`の関係にあるが、今回は片方だけを狭める。

実行コード差分は5行のみ(354/355と同じ機械照合を適用)。maxd16/18/
20/21・ディスパッチャは1バイトも変わらない。

## 期待値は控えめに置く

項目12(カーネル微修正はくじ引きである)がまだ有効な教訓である。
`save_sp==0`比較はポップループの毎回の入り口で評価されるホット
パス上の命令だが、そこから2命令減る程度の変更が壁時計に与える
影響は、351/352で確立した「0.2%前後の変動は原因を特定できない」
という経験則の範囲に収まる可能性が高い。**それでも実測する価値は
ある**——355が特定した最大の単一寄与命令を狙った、初めての具体的な
仮説だからである。

## 判定基準(実行前に固定)

351の手順をそのまま踏襲する。**判定はchunk0/chunk1の比較で行う。**
elapsedのセッション間比較は0.1%級の効果を誤判定することが352で
確立している。

| 実測(352アンカー比、chunk0/chunk1) | 判定 |
|---|---|
| +0.10%以内(改善・中立とも) | 採用。357で次の一手を検討 |
| +0.10%超の退行 | 差し戻し。355のSASS特定作業に戻る |
...
VERSION_TAG:str="356 savesp-narrow: FIRST KERNEL EDIT SINCE 352. 355 ncu SourceCounters at N18 (K1) identified two unconditional BRA instructions, both adjacent to BSYNC reconvergence markers, accounting for 60.2 percent of the measured stall_branch_resolving total (121083 plus 41143 out of 269490 samples), and confirmed the aggregate stall composition matches the previously known figures exactly: wait 45.44 percent, selected 22.47 percent, branch_resolving 20.10 percent, long_sb 5.01 percent, no_inst 4.81 percent. Disassembly context around the two BRAs shows they belong to the stack push and pop machinery, STL.64 to a computed stack pointer followed by BSYNC/BRA, then a 64 bit not equal zero compare, ISETP.NE.U32.AND plus ISETP.NE.AND.EX, feeding a conditional BREAK, then LDL.64 pop and a second BSYNC/BRA pair. That 64 bit compare is save_sp==0: save_sp is declared as Codons 64 bit int though it only ever ranges 0 to MAXD14_ANCESTOR, and the single instruction that alone accounts for 6.4 percent of the entire measured wait total, at-not-P2 BREAK B2, sits immediately after that compare. THIS IS NOT KERNEL DECOMPOSITION OR LOOP DUPLICATION, the two failure modes that lost five times running, 240, 266-269, 273 and 326, the worst of which, 326s hot loop duplication, cost -13.7 percent; the control flow shape is completely unchanged, only the bit width of one scalar changes. save_sp becomes u32 at declaration, at its equality-to-zero test, at its decrement, and at both of its increments, four sites, five lines of diff total. stack_ptr is deliberately left as int because it indexes the stack array and Codons acceptance of a u32 index was not verified, so risk is avoided there even though 296 notes stack_ptr always equals save_sp times two. maxd16, maxd18, maxd20 and maxd21 and the dispatcher are untouched, confirmed by the same kernel-region byte-identity check 353 and 354 introduced. Item 12 still applies: a hot-path instruction-count reduction of this size may land inside the roughly 0.2 percent lottery band 351/352 established, so the expectation is modest, but this is the first edit aimed at a SASS instruction 355 actually named as the single largest individual contributor to a stall reason, rather than a guess. Judged on chunk0/chunk1 against the 352 anchor, not session elapsed, per the noise-floor lesson from 352; adopt within plus 0.10 percent, revert and return to 355s SASS work otherwise; correctness 314666222712 checked before any timing is read."
