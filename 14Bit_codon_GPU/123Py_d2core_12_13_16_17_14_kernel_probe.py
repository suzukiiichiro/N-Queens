...
今回のログでは、`d2mark2=813 / 345 / 1049` が実際に専用 kernel に入り、`partial_total=7287470230` で基準値と一致しています。時間も `1:21.636` なので、119 の `1:18.796` には少し届きませんが、120/121 の約 `1:30` 台よりかなり改善しています。

次に進むため、123 を作成しました。

[123Py_d2core_12_13_16_17_14_kernel_probe.py](sandbox:/mnt/data/123Py_d2core_12_13_16_17_14_kernel_probe.py)

123 では、122 の成功 kernel に **d2 P1 入口の fid12 / fid16** を追加しています。
...
つまり、d2 全体 `12..18` まではまだ戻していません。118 で一括 d2 kernel が `CUDA_ERROR_INVALID_PTX` になったため、123 では P3 系の `fid15/18` をまだ汎用 kernel 側に残しています。

実行はまず同じ 3 chunk でお願いします。

```bash
codon build -release 123Py_d2core_12_13_16_17_14_kernel_probe.py
```

```bash
./123Py_d2core_12_13_16_17_14_kernel_probe \
...
```text
[split123-buckets] N=21 m=15488 d2core=... d2fid12=... d2fid13=... d2fid16=... d2fid17=... d2base14=... d2p3_rest=... rest=...
```
...
CUDA_ERROR_INVALID_PTX
  → fid12/fid16 を同居させたことで PTX が壊れた可能性。
     次は fid12-only / fid16-only に分けます。

PTX は通るが partial_total 不一致
  → fid12 または fid16 の mark1 遷移差分。
     fid12: row==mark1 -> step=2, blockK=n3, then fid13
     fid16: row==mark1 -> step=2, add1=1, then fid17
     を再点検します。

PTX が通り、partial_total=7287470230
  → d2 の P1/P2/base 統合は成功。
     速度が 122 の 1:21.636 より良ければ採用候補です。
```

もし `d2fid12=0` かつ `d2fid16=0` の場合は、今回の 3 chunk では新規追加分の検証が弱いです。その場合だけ、少し広げてください。
...
まだ N21 full は不要です。123 の目的は **fid12/fid16 を足しても PTX が通るか**、そして **total が壊れないか**の確認です。
...
VERSION_TAG:str="123 d2core(fid12+fid13+fid16+fid17+base14) single-kernel probe - rest uses proven 115 GPU kernel"
...
"""123: d2 core(fid12/fid13/fid16/fid17/base14) を 1 本の専用 kernel にまとめ、その他は 115 汎用 kernel に流す。"""
def exec_solutions_gpu_chunk_split123(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  """
  123 d2 core combined split:
    - bucket funcid 12, 13, 16, 17, and 14 together
    - run that bucket through one combined specialized kernel
    - keep P3 fid15/fid18 and all non-d2 tasks in the proven 115 generic rest kernel

  This extends the successful 122 mark2/base kernel by adding the P1 entry pair fid12/fid16.
  """
  board_mask:int=(1<<N)-1
  ...
  d2core_soa:TaskSoA=TaskSoA(STEPS)
  rest_soa:TaskSoA=TaskSoA(STEPS)
  d2core_w:List[u64]=[u64(0)]*STEPS
  ...
  while i<m:
    fid:int=soa.funcid_arr[i]
    if use_d2_specialized and (fid==12 or fid==13 or fid==16 or fid==17 or fid==14):
      copy_soa_record_116(soa,w_arr,i,d2core_soa,d2core_w,d2core_m)
      d2core_m+=1
      if fid==12:
        d2fid12_m+=1
      elif fid==13:
  ...
  if gpu_log_level>=1:
    print(f"[split123-buckets] N={N} m={m} d2core={d2core_m} d2fid12={d2fid12_m} d2fid13={d2fid13_m} d2fid16={d2fid16_m} d2fid17={d2fid17_m} d2base14={d2base_m} d2p3_rest={d2p3_m} rest={rest_m} split_mode={split_mode} d2_specialized={1 if use_d2_specialized else 0}")

  meta_next:List[u8]=[u8(1),u8(2),u8(3),u8(3),u8(2),u8(6),u8(2),u8(2),u8(0),u8(4),u8(5),u8(7),u8(13),u8(14),u8(14),u8(14),u8(17),u8(14),u8(14),u8(20),u8(21),u8(21),u8(21),u8(25),u8(21),u8(21),u8(26),u8(26)]

  # Specialized bucket first: PTX/JIT failures appear before generic rest work.
  if d2core_m>0:
    GRID=(d2core_m+BLOCK-1)//BLOCK
    kernel_dfs_d2core_12_13_16_17_14_gpu(gpu.raw(d2core_soa.ld_arr),gpu.raw(d2core_soa.rd_arr),gpu.raw(d2core_soa.col_arr),gpu.raw(d2core_soa.row_arr),gpu.raw(d2core_soa.free_arr),gpu.raw(d2core_soa.jmark_arr),gpu.raw(d2core_soa.end_arr),gpu.raw(d2core_soa.mark1_arr),gpu.raw(d2core_soa.mark2_arr),gpu.raw(d2core_soa.funcid_arr),gpu.raw(d2core_w),gpu.raw(results),d2core_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<d2core_m:
      chunk_total+=int(results[i])
      i+=1
  ...
  elapsed_ms:int=stage_no_read_ms
  stages:List[int]=[stage_soa_ms,stage_stats_ms,stage_split_ms,stage_kernel_reduce_ms,stage_compute_ms,stage_no_read_ms,d2core_m,d2fid12_m,d2fid13_m,d2fid16_m,d2fid17_m,d2base_m,d2p3_m,rest_m]
  return chunk_total,stats,stages,elapsed_text,elapsed_ms

def exec_solutions_gpu_bin_stream_split123(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split123_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split123_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split123_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split123_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split123-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split123-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split123-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split123-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split123(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split123-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split123-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split123 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split123 full run / optional multi-GPU worker
      # Examples:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split123-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split123 worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split123_d2core_12_13_16_17_14_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split123_d2core_12_13_16_17_14_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split123-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split123-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split123_probe_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split123(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split123-probe-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split123-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split123-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split123-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split123_full_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split123(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split123-full-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
