...
"""
270 schedule-precompute-rootpre2flag note: 267 root0countcache/precompute-bestを親に、268 root0future0 と269 root0child0 は速度退行のため採用せず撤回。root_action==0 root0 kernel/fallback構造、ctrl0/markctrl/free repack、root_action_nonzero_count cacheは維持する。追加差分は root_after_second/root_second 判定をhost側で root_pre2_flag として事前計算し、sched_mask bit28へpackすること。MAXD14 kernelでは root_pre2_flag を見て1/2bit root-prerollへ入るため、root_second/root_after_second計算を削る。mode28/29/30/31、cache生成、MAXD fallback、worker splitは保持。


269ログ確認しました。**269は正当性OKですが267より遅いため不採用**です。
270は予定通り、**267へ戻して `root_after_second` 判定をhost precompute化する版**にしました。
...
270では、`root_second/root_after_second` をkernel内で毎回作らず、host側で `root_pre2_flag` を作って `sched_mask` の bit28 にpackします。

こちらで `STATIC_ONLY=1` は通しています。

```text
source_schedule_precompute_rootpre2flag      OK
source_rootaction_zero_kernel                OK
source_rootaction_zero_dispatch              OK
source_host_precompute_rootpre2flag_arrays   OK
release_build_policy                         OK
```

ファイルはこちらです。

* [270Py_schedule_precompute_rootpre2flag_probe.py](sandbox:/mnt/data/270Py_schedule_precompute_rootpre2flag_probe.py)
* [270Py_schedule_precompute_rootpre2flag_validate_N21_full_once.sh](sandbox:/mnt/data/270Py_schedule_precompute_rootpre2flag_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [270Py_schedule_precompute_rootpre2flag_sha256.txt](sandbox:/mnt/data/270Py_schedule_precompute_rootpre2flag_sha256.txt)
* [270Py_schedule_precompute_rootpre2flag_bundle_20260707.tar](sandbox:/mnt/data/270Py_schedule_precompute_rootpre2flag_bundle_20260707.tar)

実行は以下でお願いします。

```bash
rm -f ./270Py_schedule_precompute_rootpre2flag_probe

STATIC_ONLY=1 bash 270Py_schedule_precompute_rootpre2flag_validate_N21_full_once.sh

bash 270Py_schedule_precompute_rootpre2flag_validate_N21_full_once.sh
```

こちらではCodon release build/full runは実行できていません。最初の確認点は **JIT成立 → 正当性OK → 267の `0:07:08.056` から戻るか** です。
...
VERSION_TAG:str="270 schedule-precompute-rootpre2flag: parent 267 root0countcache precompute-best; rejected 268 root0future0 and 269 root0child0; single launch per chunk kept; root_action==0 root0 kernel/fallback dispatch preserved; host precomputes root_pre2_flag from post-root-action root availability and packs it into sched_mask bit28, removing root_second/root_after_second compute in MAXD14 kernels; root_action pack/mask decode still rejected; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 270: keep 262's safe terminal_depth+terminal_base14 pack and 244 r4 maskpack.
    ...
    self.sched_mask_arr:List[u32]=[u32(0)]*m  # bits0..13 child_jmark, bits14..27 future_check, bit28 root_pre2_flag
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 270: host-side clone of the former MAXD14 kernel schedule builder.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 270: free_arr is repacked to sched_mask for selected MAXD14 launch.
    ...
    # 270: schedule fields are precomputed on host. For MAXD14 launch,
    ...
    root_pre2_flag:u32=(sched_mask>>u32(28))&u32(1)
    ...
    # 270: root 1/2-candidate predicate is host-precomputed in sched_mask bit28.
    # This avoids root_second/root_after_second computation for 3+ candidate roots.
    if root_pre2_flag!=u32(0):
...
def kernel_dfs_iter_gpu_maxd14_root0(
    ...
    terminal_depth:int=int(sched_terminal&u32(15))
    terminal_base14:u32=(sched_terminal>>u32(4))&u32(1)
    # 270 root0 kernel: root_action is statically zero for all tasks in this launch.
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    # 270: selected_MAXD14 is known here, so original ctrl0/markctrl/free
    ...
    while ri<m:
      ...
      ri+=1
    if root_action_nonzero_count==0:
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        root_pre2_avail:u32=soa.free_arr[t]
        if root_action==u32(1):
          root_pre2_avail=root_pre2_avail&(~u32(1))
        root_pre2_rest:u32=root_pre2_avail&(root_pre2_avail-u32(1))
        root_pre2_tail:u32=root_pre2_rest&(root_pre2_rest-u32(1))
        root_pre2_flag:u32=u32(1) if root_pre2_tail==u32(0) else u32(0)
        sched_mask=sched_mask|(root_pre2_flag<<u32(28))
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split270_rootpre2flag_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split270_rootpre2flag_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split270-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split270-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split270_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
