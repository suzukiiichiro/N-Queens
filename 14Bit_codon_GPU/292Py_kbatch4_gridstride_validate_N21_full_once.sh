...
# =============================================================================
# 292 N=21 full validation harness
# Parent: 291 generic-normaldefault-blockcodelate. Experiment: kernel_dfs_iter_gpu_maxd14
#   wrapped in a grid-stride loop (K_PER_THREAD_MAXD14 constellations per GPU
#   thread instead of exactly one) to raise per-warp active-thread ratio
#   (ncu measured 4.88/32 on 279; 6.28/32 confirmed at K=2). Launch grid/block
#   (32x484), hot-loop internals, and results reduction math are unchanged.
#   K swept 1/2/4/8/16/32/64 by wall-clock N=21 full run; 32 and 64 both land
#   at ~-13.4% vs 291 baseline with no measurable gap between them, so 32 is
#   adopted as the smaller-footprint choice (K=64 needs 2x the per-chunk
#   host-side buffers for the same GPU-side benefit).
# Expected: mode31 split145+chunkshape148, required=14, selected MAXD14,
#           schedule_words=0, stack=208 bytes/thread,
#           K_PER_THREAD_MAXD14=32 -> 5 dispatch/progress rows (not 131).
# =============================================================================

SRC=${SRC:-./292Py_kbatch4_gridstride_probe.py}
CAND=${CAND:-./292Py_kbatch4_gridstride_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/292Py_kbatch4_gridstride_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/292Py_kbatch4_gridstride_logs_N21_full_once_${TS}"
...
echo "[start] 292 kbatch-gridstride validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 292 kbatch-gridstride SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '292 kbatch-gridstride' "$SRC"; then
  printf 'source_version_tag	292 kbatch-gridstride	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	292 kbatch-gridstride	missing	FAIL
...
# Check MAXD14 generic DFS loop normal-default nld/nrd + ncol-only + block_code-late shape. Keep root-preroll untouched.
# 292: the whole per-task body (including this hot loop) sits one indent level
# deeper than in 291 because it is now nested inside "while idx<m:" (the
# grid-stride K-batch loop), so the expected literal indentation here is 8/10
# spaces, not 291's 6/8 spaces. The DFS logic itself is byte-identical to 291.
...
if not m14:
    ...
    body = m14.group(0)
    # Focus on generic DFS loop after nibble_op decode; root-preroll uses pr_block_code and must remain untouched.
    idx = body.find('nibble_op:u32=u32(0)')
    tail = body[idx:] if idx >= 0 else body
    ...
    nf_default_bad = '''nf:u32=bm&~(nld|nrd|ncol)
        if (nibble_op&u32(7))!=u32(0):''' in tail or '''nf:u32=bm&~(nld|nrd|ncol)
        if block_code!=u32(0):''' in tail
    ok_normal = expected_normal in tail and not nf_default_bad
    ok_late = old_early_blockcode not in tail and expected_normal in tail
...
# 292: kbatch/grid-stride shape checks (new vs 291).
if m14:
    body14 = m14.group(0)
    ok_stride_param = 'stride:int,' in body14.split(')->None:')[0]
    ok_gridstride_loop = 'while idx<m:' in body14
    ok_single_writeback = 'results[tid]=thread_total' in body14
    ok_no_percall_writeback = 'results[idx]=' not in body14
    ok_kbatch_shape = ok_stride_param and ok_gridstride_loop and ok_single_writeback and ok_no_percall_writeback
    checks.append(('source_kbatch_gridstride_shape', 'stride param + grid-stride idx loop + single results[tid] writeback', 'present' if ok_kbatch_shape else 'missing (stride_param=%s gridstride_loop=%s single_writeback=%s no_percall_writeback=%s)' % (ok_stride_param, ok_gridstride_loop, ok_single_writeback, ok_no_percall_writeback), ok_kbatch_shape))
else:
    checks.append(('source_kbatch_gridstride_shape', 'stride param + grid-stride idx loop + single results[tid] writeback', 'MAXD14 not found', False))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 287 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 287 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 287 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 292 kbatch-gridstride mode31 split145"
...
echo "[validation-ok] 292 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
