...
# =============================================================================
# 259 N=21 full validation harness
...
#             separate. Decode root_action as (sched_root_action_arr[i]) & u32(3),
#             per user suggestion, to test whether explicit parentheses change
#             the Codon GPU IR/JIT behavior versus 257.
...
SRC=${SRC:-./259Py_schedule_precompute_rootaction_parenmask_probe.py}
CAND=${CAND:-./259Py_schedule_precompute_rootaction_parenmask_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/259Py_schedule_precompute_rootaction_parenmask_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/259Py_schedule_precompute_rootaction_parenmask_logs_N21_full_once_${TS}"
...
echo "[start] 259 schedule-precompute-rootaction-parenmask validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 259 schedule-precompute-rootaction-parenmask user-parenthesized-mask SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '259 schedule-precompute-rootaction-parenmask probe' "$SRC"; then
  printf 'source_version_tag\t259 schedule-precompute-rootaction-parenmask probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t259 schedule-precompute-rootaction-parenmask probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split259_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s and 'split244_' not in s and 'split255_' not in s and 'split256_' not in s and 'split257_' not in s and 'split258_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split259 active; split240/241/242/243/244/255/256/257/258 runtime tags absent', 'split259', True))
else:
    checks.append(('source_split_tag', 'split259 active; split240/241/242/243/244/255/256/257/258 runtime tags absent', 'old split runtime tag present or split259 missing', False))
...
if not m14:
    ...
    ok = (
        ...
        'root_action:u32 = (sched_root_action_arr[i]) & u32(3)' in body and
        'schedule_raw:u32=ctrl0_arr[i]' not in body and
        'schedule_fu:u32=' not in body and
        'sched_ctrl_arr:Ptr[u32]' not in body
    )
    actual = 'rootaction parenthesized-mask precompute active' if ok else 'old schedule interpreter present or parenthesized root_action decode missing'
    checks.append(('source_schedule_precompute_rootaction_parenmask', 'MAXD14 precompute rootaction parenthesized-mask active', actual, ok))
...
checks.append(('source_host_precompute_rootaction_parenmask_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
echo "[static-ok] 259 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 259 schedule-precompute-rootaction-parenmask mode31 split145"
...
echo "[validation-ok] 259 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
