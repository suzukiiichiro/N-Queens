...
# 176 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 176 inherits the
# validated fastest 175 nibble-schedule MAXD14 static dispatch.  Its single
# kernel experiment keeps the 175 4-bit nibble schedule, register-held active
# frame, and 13-slot ancestor stack, but caches the current frame nibble in
# a scalar so the same frame does not shift/extract its schedule for every candidate bit.
#
# For N=21 all launches must remain required_maxd=14, select MAXD14, report
# schedule_words=0 and 208 local stack bytes/thread.

SRC=${SRC:-./176Py_kernel_current_nibble_register_maxd14_probe.py}
CAND=${CAND:-./176Py_kernel_current_nibble_register_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/176Py_kernel_current_nibble_register_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 176 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/176Py_kernel_current_nibble_register_maxd14_logs_N21_full_once_${TS}"
...
assert '176 current-nibble-register GPU candidate supports N=5..27 only' in s
...
# schedule fields, the 175 register-held active frame, and the 176 current-nibble register hot path.
...
def nibble_from_byte(op):
    # 176 keeps the 175 byte-opcode low nibble directly:
...
print('MAXD14_CURRENT_NIBBLE_REGISTER=1')
...
PYMODEL
then
  printf 'source_current_nibble_register_shape\trequested layout\tmodel/static failure\tFAIL\n' >> "$SUMMARY"
...
printf 'source_current_nibble_register_shape\tMAXD14 current-nibble register + scalar nibble schedule + register-held current frame + 13-slot ancestor stack\tverified\tOK\n' >> "$SUMMARY"
...
printf 'maxd14_current_nibble_register\t1\t%s\tOK\n' "$MAXD14_CURRENT_NIBBLE_REGISTER" >> "$SUMMARY"
...
printf 'dfs_164_vs_176_current_nibble_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 176 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 176 current-nibble schedule + current-nibble register + ancestor13 stack"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 176 keeps the validated 175 kernel structure, then derives chunk-tail diagnostics from
...
echo "[validation-ok] 176 current-nibble register reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
