...
149Pyは問題ありません。全131チャンク、固定部分和01〜05、最終合計`314666222712`が一致し、欠落・重複・警告・エラーはありません。実行時間は`0:16:16.932`で、148平均との差は測定誤差範囲です。

### 150Py

[150Py_blockk_branchless_u32_probe.py](sandbox:/mnt/data/150Py_blockk_branchless_u32_probe.py)

149の`u32`デコードを維持し、`blockK`選択の`if/elif`だけを分岐なしの`u32`マスク式へ変更しました。

```python
bKu:u32=(n3u&(u32(0)-(ktu&u32(1))))|(n4u&(u32(0)-(ktu>>u32(1))))
```

`ktu=0/1/2`について149の処理と等価であることを確認しています。並び替えやキャッシュ形式は変更せず、148の検証済みbinを再利用します。

### 実行シェル

[150Py_validate_full_once.sh](sandbox:/mnt/data/150Py_validate_full_once.sh)

```bash
chmod +x 150Py_validate_full_once.sh
./150Py_validate_full_once.sh
```

全131チャンクを1回だけ実行し、そのprogress TSVから01〜05を再計算して、06の全件結果と併せて検証します。`flock`、自動ビルド、欠落・重複・エラー検査も含みます。

この環境ではCodon/CUDA実機コンパイルは未実施です。シェル構文、模擬131チャンクによる全検証処理、分岐なし選択式の等価性は確認済みです。


"""

# 150 GPU kernel experiment:
#   Retain the fully validated 149 ctrl-metadata u32 decode, 148 task order,
#   MAXD=21, five u32 DFS stacks, and nld/nrd full-u32 arithmetic.
#   The sole kernel change is the blockK selector: replace the ktype 1/2
#   if/elif with a branchless u32 mask select.  ktype is invariantly 0, 1,
#   or 2 because MASK_K_N3 and MASK_K_N4 are disjoint.
...
VERSION_TAG:str="150 branchless blockK u32 select over 149 ctrl decode - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    # 150: fixed-width operands used by the u32 diagonal, ctrl-decode, and branchless blockK hot path.
    n3u:u32=u32(n3)
    n4u:u32=u32(n4)
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148/149/150
      ...
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
####################################################################################################
# 150 branchless blockK u32 select over the validated 149/148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 149/148; 150 changes only blockK selection inside kernel_dfs_iter_gpu
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 149/148 scorestripe_v9 task order/cache unchanged; 150 kernel experiment selects blockK with branchless u32 masks"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  150 reuses the validated 149/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 150 experiment keeps the 149 ctrl-u32 path and changes only blockK selection inside the GPU kernel.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  150 host-side score-aware lanephase chunk shaping reused unchanged from validated 149/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 150 experiment changes only blockK selection; the 148 host-shaping cache is reused unchanged.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split150_blockk_branchless_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split150_blockk_branchless_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split150-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split150-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split150_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split150-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split150-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split150_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
