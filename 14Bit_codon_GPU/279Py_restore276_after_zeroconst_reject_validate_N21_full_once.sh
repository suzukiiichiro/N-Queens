...
# =============================================================================
# 279 N=21 full validation harness
# Parent: 276 restore274-after-bucketdiag-reject current baseline.
# Experiment: reject 278 zero-const assign for speed and restore the
#             276 kernel/execution baseline. No depth_u child/save change,
#             no zero-const variable, no root0 direct kernel, no bucket diag,
#             no schedule precompute, no fid14/rest launch split.
...
SRC=${SRC:-./279Py_restore276_after_zeroconst_reject_probe.py}
CAND=${CAND:-./279Py_restore276_after_zeroconst_reject_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/279Py_restore276_after_zeroconst_reject_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/279Py_restore276_after_zeroconst_reject_logs_N21_full_once_${TS}"
...
echo "[start] 279 restore276-after-zeroconst-reject validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 279 restore276-after-zeroconst-reject SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '279 restore276-after-zeroconst-reject' "$SRC"; then
  printf 'source_version_tag\t279 restore276-after-zeroconst-reject\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t279 restore276-after-zeroconst-reject\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split279' in s and 'split278' not in s and 'split277' not in s and 'split275' not in s and 'split274' not in s and 'split273' not in s and 'rootaction0-direct' not in s and 'kernel_dfs_iter_gpu_maxd14_root0' not in s and 'split270_' not in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split279 active; 278/277/275/273/270/240 rejected', 'split279', True))
else:
    checks.append(('source_split_tag', 'split279 active; 278/277/275/273/270/240 rejected', 'old/depthu/zero/bucket/root0 split runtime present or split279 missing', False))
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 279 restore276-after-zeroconst-reject mode31 split145"
