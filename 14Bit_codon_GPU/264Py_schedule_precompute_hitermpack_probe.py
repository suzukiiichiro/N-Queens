...
# =============================================================================
# NQ_UPDATE_MEMO
# 264: schedule-precompute-hitermpack probe。263 reusectrl JIT成立/正当性OKを受け、
#      root_action は危険候補として別配列素通しへ固定したまま、schedule_hi と
#      terminal_depth/base14 を sched_hi_arr の高位bitへ同居させる切り分け版。
#      241 restore239本線を親系統に、単一launchを維持し、child/future maskpackと
#      terminal_depth+base14 pack は保持する。
...
"""
264 schedule-precompute-hitermpack note: 263 reusectrl JIT成立/正当性OKを受け、root_actionは別配列素通し固定のまま、schedule_hi と terminal_depth/base14 を sched_hi_arr の高位bitへ同居させる切り分けprobe。selected_MAXD14直前に ctrl0_arr のみ schedule_lo へ再利用し、markctrl_arr はMAXD14 kernelでは読まない。


了解です。264として作成しました。
263を親にした **schedule_hi + terminal_depth/base14 pack 切り分け版**です。
...
264では、`root_action` は257/259/260でJIT不成立だったため、引き続き別配列素通しに固定しています。変更点は、`schedule_hi` と `terminal_depth/base14` だけを同じcarrierへpackすることです。

```text
MAXD14 launch直前:
  ctrl0_arr    = schedule_lo
  markctrl_arr = schedule_hi | (terminal_depth << 24) | (terminal_base14 << 28)

MAXD14 kernel側:
  schedule_lo     = ctrl0_arr[i]
  schedule_hi     = markctrl_arr[i] & 0xFFFFFF
  terminal_depth  = (markctrl_arr[i] >> 24) & 15
  terminal_base14 = (markctrl_arr[i] >> 28) & 1
  root_action     = root_action_arr[i]
```

つまり、263から `sched_hi_arr` の追加loadを削れるかを見る版です。
...
source_schedule_precompute_hitermpack  OK
source_host_precompute_hitermpack_arrays OK
release_build_policy                   OK
```

ファイルはこちらです。

* [264Py_schedule_precompute_hitermpack_probe.py](sandbox:/mnt/data/264Py_schedule_precompute_hitermpack_probe.py)
* [264Py_schedule_precompute_hitermpack_validate_N21_full_once.sh](sandbox:/mnt/data/264Py_schedule_precompute_hitermpack_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [264Py_schedule_precompute_hitermpack_sha256.txt](sandbox:/mnt/data/264Py_schedule_precompute_hitermpack_sha256.txt)
* [264Py_schedule_precompute_hitermpack_bundle_20260707.tar](sandbox:/mnt/data/264Py_schedule_precompute_hitermpack_bundle_20260707.tar)

sha256です。

```text
43e96e48c98b92307fe74bb55b48a5e12d7e68a1d2e05160ee37828efc6c8878  264Py_schedule_precompute_hitermpack_probe.py
2dcb2154e16fc5f6b884c950a4380ea9f5d67d77fd516efa9cf9efc2a0e3fbd9  264Py_schedule_precompute_hitermpack_validate_N21_full_once.sh
bb44cc8370879fdbe18903f5b6fcbb1b101547b4e698cec2080ccc190965c92c  README.md
```

実行は以下でお願いします。

```bash
rm -f ./264Py_schedule_precompute_hitermpack_probe

STATIC_ONLY=1 bash 264Py_schedule_precompute_hitermpack_validate_N21_full_once.sh

bash 264Py_schedule_precompute_hitermpack_validate_N21_full_once.sh
```

まず見るべき順番は、**JIT成立 → 正当性OK → 263の `0:07:09.417` から改善するか** です。
...
VERSION_TAG:str="264 schedule-precompute-hitermpack probe: parent 263 reusectrl JIT-ok / 262 terminalbasepack JIT-ok / 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; MAXD14 host precompute retained; ctrl0_arr is repacked to schedule_lo only after selected_MAXD14; sched_hi_arr packs schedule_hi plus terminal_depth/base14; sched_mask/root_action remain separate; root_action stays pass-through after 257/259/260 CUDA_ERROR_INVALID_PTX; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 264: keep 262's safe terminal_depth+terminal_base14 pack and 244 r4 maskpack.
    # 264: keep 244 r4 child/future maskpack and pack schedule_hi with
    # terminal_depth/base14. sched_lo_arr and sched_hi_term_arr are host
    # intermediates only: after selected_MAXD14 they are repacked into existing
    # ctrl0_arr / markctrl_arr to reduce separate schedule array arguments.
    self.sched_lo_arr:List[u32]=[u32(0)]*m
    self.sched_hi_term_arr:List[u32]=[u32(0)]*m  # bits0..23 schedule_hi, bits24..27 terminal_depth, bit28 terminal_base14
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 264: host-side clone of the former MAXD14 kernel schedule builder.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    sched_mask_arr:Ptr[u32],root_action_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 264: schedule fields are precomputed on host. For MAXD14 launch,
    # ctrl0_arr has been repacked to schedule_lo and markctrl_arr to
    # schedule_hi + terminal_depth/base14. child/future masks are packed;
    # root_action remains separate pass-through because root_action mask decode
    # triggered CUDA_ERROR_INVALID_PTX in 257/259/260.
    schedule_lo:u32=ctrl0_arr[i]
    sched_hi_term:u32=markctrl_arr[i]
    schedule_hi:u32=sched_hi_term&u32(16777215)
    terminal_depth:int=int((sched_hi_term>>u32(24))&u32(15))
    terminal_base14:u32=(sched_hi_term>>u32(28))&u32(1)
    sched_mask:u32=sched_mask_arr[i]
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    # 264: selected_MAXD14 is known here, so original ctrl0/markctrl are no
    # longer needed by the MAXD14 kernel. Reuse them for schedule_lo and
    # schedule_hi+terminal_depth/base14, reducing separate schedule array loads.
    ri:int=0
    while ri<m:
      soa.ctrl0_arr[ri]=soa.sched_lo_arr[ri]
      soa.markctrl_arr[ri]=soa.sched_hi_term_arr[ri]
      ri+=1
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.sched_mask_arr),gpu.raw(soa.root_action_arr),
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        soa.sched_hi_term_arr[t]=(sched_hi&u32(16777215))|((terminal_depth_u&u32(15))<<u32(24))|((sched_base14&u32(1))<<u32(28))
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        for p in range(m):
          ...
          sort_soa.sched_hi_term_arr[p]=soa.sched_hi_term_arr[q]
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split264_hitermpack_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split264_hitermpack_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split263-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split263-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split264_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
