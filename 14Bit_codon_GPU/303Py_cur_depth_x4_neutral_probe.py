...
**302 = 635.928s — 依然壊滅的、不採用。** バグ修正後でも 296 比 −282s (−80%) 悪化が残っています。

301 と 302 の差が +12s しかなく、preroll の修正は正答性を改善しませんでした（正当性 OK = 314666222712 一致）。ということは **根本問題は preroll の `cur_depth_x4=4` の欠如ではなく別の何か**です。

もしくは `cur_depth_x4-32` が負の値になるケースがあります。`cur_depth_x4` が `cur_depth<<2` で更新されるとき、`cur_depth=0` の pop 後は `cur_depth_x4=0`、then `else` ブランチで `cur_depth_x4-32 = -32` → u32 にキャストされると `4294967264` となり、巨大なシフト量になります。2箇所（preroll push と main DFS push）。root_after_second==0 の追加 push がもう1箇所あると思っていましたが 2 箇所が正解です。チェックの期待値を修正します。全項目OK。検証スクリプトと README を仕上げます。## 301/302 結果まとめと 303 の考え方

**301/302 ともに不採用**（−283〜294s、−80%〜83%）。根本原因は**レジスタ圧迫によるスピル**でした。hot DFS ループはすでに ~28変数がレジスタに収まっており、`cur_depth_x4` を追加するとレジスタが溢れて local memory への書き出しが発生。

---

## 303 の設計原則: レジスタ中立

`cur_depth` を **完全に** `cur_depth_x4` で置き換えます（追加ではなく置換）：

| 箇所 | 変更前 | 変更後 |
|---|---|---|
| init | `cur_depth:int=0` | `cur_depth_x4:int=0` |
| hot nibble decode | `cur_depth*4` / `(cur_depth-8)*4` | `cur_depth_x4` / `cur_depth_x4-32` |
| hot terminal check | `cur_depth==terminal_depth` | `cur_depth_x4==terminal_depth_x4` |
| hot avail pack | `u32(cur_depth)<<u32(27)` | `u32(cur_depth_x4)<<u32(25)` |
| hot child_jmark | `>>u32(cur_depth)` | `>>u32(cur_depth_x4>>2)` |

**ネット変化**: レジスタ 0 増減。乗算 2 本削除、シフト 1 本追加（child_jmark）。

```bash
STATIC_ONLY=1 bash 303Py_cur_depth_x4_neutral_validate_N21_full_once.sh
bash 303Py_cur_depth_x4_neutral_validate_N21_full_once.sh
```
"""
...
VERSION_TAG:str="303 cur-depth-x4-neutral probe: parent 302 (rejected, -282s due to register spilling); register-neutral replacement: cur_depth eliminated and replaced by cur_depth_x4=cur_depth*4 throughout the kernel, including terminal_depth->terminal_depth_x4 and avail packing uses <<25 instead of <<27; child_jmark still uses >>(cur_depth_x4>>2); hot decode: cur_depth_x4<32 / >>cur_depth_x4 / >>cur_depth_x4-32; net register change: 0 (cur_depth removed, cur_depth_x4 added); removes 2 multiplies per DFS node at cost of 1 extra shift for child_jmark; schedule_lo/hi, stack_ptr/save_sp from 296 unchanged; K_PER_THREAD_MAXD14=32; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while idx<m:
      ...
      terminal_parent_depth_x4:int=0
      ...
      while True:
        ...
        if schedule_depth==0:
          ...
          if frame_action==u32(1):
            ...
            terminal_parent_depth_x4=parent_depth<<2
      ...
      terminal_depth_x4:int=terminal_parent_depth_x4
      ...
      if root_after_second==u32(0):
        ...
        if pr_descend!=u32(0):
          if cur_avail!=u32(0):
            ...
            stack[stack_ptr+1]=u64(cur_col)|(u64(cur_avail|(u32(cur_depth_x4)<<u32(25)))<<u64(32))
            stack_ptr+=2
            save_sp+=1
          cur_ld=pr_nld
          cur_rd=pr_nrd
      ...
      while True:
        if cur_avail==u32(0):
          ...
          cur_depth_x4=int(saved_avail>>u32(27))<<2
          continue
        ...
        if cur_depth_x4==terminal_depth_x4:
          if terminal_base14==u32(0):
            total+=u64(1)
          else:
        ...
        child_jmark:u32=(child_jmark_mask>>u32(cur_depth_x4>>2))&u32(1)
