...
codon build -release \
  110Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid7_f4f13_gh_tail_smoothing.py

#CPU
./110Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid7_f4f13_gh_tail_smoothing \
  -c 5 22
#GPU
./110Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid7_f4f13_gh_tail_smoothing \
...
確認しました。109Py のソースでは、mode 29 は **kernel/DFS本体は触らず**、107Py の broad×markdist reorder の内側で `fid==17` と `G_H` を 3分割する weak tertiary smoothing になっています。該当箇所は `broad_markdist_tail_bucket()` と tail summary 周辺です。

N22 full run では、残 tail の主信号は `funcid_17` ではなく `funcid_7_count` が最上位で、p95 slow 側では `funcid_4_count` / `funcid_13_count` / `w4_count` も増えていました。 そのため、109Py を直接ベースにして **110Py 候補**を作りました。

作成したものはこちらです。

[110Py ソース](sandbox:/mnt/data/110Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid7_f4f13_gh_tail_smoothing.py)

[109Py→110Py 差分パッチ](sandbox:/mnt/data/109Py_to_110Py_funcid7_f4f13_gh_tail.patch)
...
109Py:
  tail0 = fid==17
  tail1 = broad==G && markrisk==H
  tail2 = rest

110Py:
  tail0 = fid==7
  tail1 = fid==4 or fid==13 or (broad==G && markrisk==H)
  tail2 = rest
```

`BROAD_MARKDIST_TAIL_REORDER_VERSION` は `v3` に上げてあります。これで 109Py の `v2` reorder bin と衝突しません。GPU progress suffix も `broadmarktail_f7f413gh_gpu` に変えたので、109Py の GPU TSV を上書きしません。

まずは N21 debug でお願いします。
...
./110Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid7_f4f13_gh_tail_smoothing \
  -g 21 21 32 484 1 0 6 29 \
  2>&1 | tee 110Py_N21_p6_broadmarktail_f7f413gh_gpu_full.log
```
...
python3 aggregate_107py_broadmark_gpu.py \
  --gpu progress_N21_6_stream_funcid_reorder_v2_w8_j7_broadmarktail_f7f413gh_gpu.tsv \
  --sim progress_N21_6_stream_broadmarktail_reorder_v3_w8_j7_sim.tsv \
  --out 110Py_N21_p6_broadmarktail_f7f413gh \
  --top 100 \
  --run-threshold p95 \
  --log 110Py_N21_p6_broadmarktail_f7f413gh_gpu_full.log
```

N21 が `status=ok` なら、次に N22 full run です。

```bash
./110Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid7_f4f13_gh_tail_smoothing \
  -g 22 22 32 484 1 0 7 29 \
  2>&1 | tee 110Py_N22_p7_broadmarktail_f7f413gh_gpu_full.log
```
...
```bash
python3 aggregate_107py_broadmark_gpu.py \
  --gpu progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmarktail_f7f413gh_gpu.tsv \
  --sim progress_N22_7_stream_broadmarktail_reorder_v3_w8_j7_sim.tsv \
  --baseline-summary 107Py_N22_p7_broadmark_summary.tsv \
  --baseline-label 107Py \
  --out 110Py_N22_p7_broadmarktail_f7f413gh \
  --top 100 \
  --run-threshold p95 \
  --log 110Py_N22_p7_broadmarktail_f7f413gh_gpu_full.log
```

こちらの環境には Codon が無いのでビルド実行までは確認できていません。ただし変更範囲は stream-side reorder の tail bucket / progress 名 / version tag に限定しており、GPU kernel と DFS ロジックは触っていません。


はい、Thinkingで十分続けられます。添付ログを確認しました。

結論から言うと、**110Py は正しさはOKですが、性能採用は見送り**がよいです。

## 110Py N22 の判定

110Py は、109Py の `funcid_17 / G_H` 分離をやめ、`funcid_7` と `funcid_4 / funcid_13 / G_H` を弱く3分割する版でした。ソース上も、変更は `broad_markdist_tail_bucket()` など stream-side reorder に限定され、GPU kernel / DFS 本体は変えていません。

N22 full run の正しさは問題ありません。

```text
final_gpu_total : 2691008701644
expected_total  : 2691008701644
status          : ok
chunks          : 1855
records         : 28719035
```

レポート上も `status: ok`、`log_status: ok` です。

ただし性能は、107Py比ではほぼ横ばい、109Py比では悪化です。

| 指標        |                 107Py比 |                 109Py比 |
| --------- | ---------------------: | ---------------------: |
| elapsed合計 | 107Pyより **4.514秒だけ速い** |  109Pyより **67.667秒遅い** |
| 平均chunk   |             6981.19 ms | 109Pyの 6944.71 ms より悪化 |
| p95       |              8729.8 ms |  109Pyの 8689.0 ms より悪化 |
| p99       |             9499.74 ms |  109Pyの 9346.9 ms より悪化 |
| max       |               10706 ms |   109Pyの 10307 ms より悪化 |

110Py は、`elapsed_vs_chunk_total` の相関は下がっています。これは良い兆候です。

```text
109Py pearson  : 0.499558
110Py pearson  : 0.346780
```

しかし、相関は改善しても実時間が伸びています。つまり、**chunk_total依存の偏りは薄まったが、tail分散の副作用で重いchunkそのものは減らせなかった**という読みです。
...
したがって、次に進むなら **111Pyでさらにtailをいじるより、multi-GPU worker 化に進む方がよい**です。

## g5.12xlarge向けの次方針

ここからは、`g5.12xlarge` の A10G 4枚を使う方向が一番筋が良いです。

110Py の `joined.tsv` から、仮に `chunk % 4` で4GPUに分散した場合の chunk elapsed 合計を計算すると、以下のようになります。
...
最大 worker が約 **3362.6秒 = 56分02秒** です。単純合計が 12950.1秒なので、理論上は **約3.85倍** の短縮余地があります。

109Pyでも同じ計算をすると、最大 worker は約 **3344.9秒 = 55分45秒** です。つまり、multi-GPU化のベースは **110Pyではなく109Py** の方がよいです。
...
分散方式:
  chunk_index % worker_count == worker_id の chunk だけ実行

progress:
  worker_id ごとに別TSVへ出力

最終:
  4本の partial_total を合算
  expected_total と照合
```
...
```bash
CUDA_VISIBLE_DEVICES=0 ./111Py_... -g 22 22 32 484 1 0 7 29 0 4 \
  2>&1 | tee 111Py_N22_worker0.log &

CUDA_VISIBLE_DEVICES=1 ./111Py_... -g 22 22 32 484 1 0 7 29 1 4 \
  2>&1 | tee 111Py_N22_worker1.log &

CUDA_VISIBLE_DEVICES=2 ./111Py_... -g 22 22 32 484 1 0 7 29 2 4 \
  2>&1 | tee 111Py_N22_worker2.log &

CUDA_VISIBLE_DEVICES=3 ./111Py_... -g 22 22 32 484 1 0 7 29 3 4 \
  2>&1 | tee 111Py_N22_worker3.log &

wait
```

この場合、`chunk_index % 4` の cyclic 分散がよいです。110Py/109Pyとも slow chunk は連続島として出ているため、連続範囲で4分割すると、遅い島を担当したGPUだけが最後に残りやすくなります。110Pyのslow runでも `1642-1652`、`70-76`、`1574-1578`、`86-90` のように島状に残っています。
...
次にやる価値が高いもの:
  111Py = 109Py + multi-GPU worker mode

分散方式:
  chunk_index % worker_count

期待:
  g5.12xlarge 4GPUで、N22 full run の実行時間を
  約3時間半 → 約1時間前後まで短縮できる可能性
```

なので、次は **「111Py multi-GPU worker版」** を作るのがよいと思います。
...
VERSION_TAG:str="110 broadmark funcid7/F4F13/G_H weak tertiary smoothing auto w8_j7 stream GPU runner from 107 type-explicit"
...
# 110 BROAD+MARKDIST+WEAK TAIL TERTIARY REORDER:
#   Keep 107's broad funcid primary quota and mark-distance secondary quota.
#   109Py targeted funcid_17/G_H; N22 full run showed the remaining tail is instead
#   dominated by funcid_7 globally, with local p95 islands enriched for funcid_4/13.
#   This version changes only stream ordering / .bin layout; GPU kernel and DFS logic remain unchanged.
BROAD_MARKDIST_TAIL_REORDER_VERSION:str="v3"
BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON:str="110 broadmark + funcid7/F4F13/G_H weak tertiary smoothing"
BROAD_MARKDIST_TAIL_WINDOW_BOOST:int=1
BROAD_MARKDIST_TAIL_PHASE_SALT:int=31
...
####################################################
#
# 110 broadmark + funcid7 / F4F13 / G_H weak tertiary smoothing reorder
#
####################################################

"""110 broadmark tail: tertiary tail bucket label。0=funcid7, 1=funcid4/13 or cell_G_H, 2=rest。"""
...
"""110 broadmark tail: 25x3 subcell bin 名。"""
...
"""110 broadmark tail: reorder 済み bin 名。"""
def broad_markdist_tail_reorder_output_fname(N:int,preset_queens:int)->str:
  return f"constellations_N{N}_{preset_queens}_broadmarktail_reorder_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{funcid_reorder_param_tag()}.bin"

"""110 broadmark tail: subcell index を 0..74 に pack。"""
def broad_markdist_tail_subcell_index(broad:int,risk:int,tail:int)->int:
  return (broad*5+risk)*3+tail

"""110 broadmark tail: 75 個の constellation buffer を作る。"""
...
"""110 broadmark tail: counts[75] から tail bucket 合計を取る。"""
def sum_broadmarktail_subcell_tail_counts(counts:List[int],tail:int)->int:
  total:int=0
  subcell:int=0
  while subcell<75:
    if (subcell%3)==tail:
...
"""110 broadmark tail: fid / broad / risk から weak tertiary tail bucket を返す。"""
def broad_markdist_tail_bucket(fid:int,broad:int,risk:int)->int:
  # N22 full run の最上位相関は funcid_7_count。107 broadmark を壊さないよう、同じ broad×markdist cell の内側でだけ弱く分離する。
  if fid==7:
    return 0
  # p95 slow island で増えた funcid_4/13 と、従来の残 tail signal である cell_G_H を第2 tail として弱く分離する。
  if fid==4 or fid==13 or (broad==3 and risk==2):
    return 1
  return 2

"""110 broadmark tail: 75 subcell stats。subcell=(broad*5+risk)*3+tail。"""
...
"""110 broadmark tail: summary stats。0=funcid7,1=funcid4/13 or cell_G_H,2=markrisk_H,3=proxy_sum。"""
...
"""110 broadmark tail: chunk constellations から tail summary を作る。"""
...
"""110 broadmark tail progress header。107 broadmark header の後ろに weak tail smoothing 指標を追加。"""
def stream_broad_markdist_tail_reorder_progress_header()->str:
  h:str=stream_broad_markdist_reorder_progress_header().strip()
  h+="\ttail_funcid7_count"
  h+="\ttail_funcid4_13_GH_count"
...
"""110 broadmark tail: tail summary suffix。"""
def stream_broad_markdist_tail_summary_suffix(tail_stats:List[int],m:int)->str:
  ...
  if len(tail_stats)>3:
    proxy=tail_stats[3]
  return f"\t{f7}\t{f413gh}\t{hcnt}\t{proxy}\t{format_ratio_3(proxy,m)}"

"""110 broadmark tail reorder progress を TSV に追記する。"""
...
"""110 broadmark tail: 元 bin を broad x markdist x tail 75 subcell bin へ分配する。"""
def build_broad_markdist_tail_reorder_subcell_bins(N:int,fname:str,preset_queens:int,BLOCK:int,MAX_BLOCKS:int,gpu_log_level:int=0)->List[int]:
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if gpu_log_level>=2:
        ...
        print(f"[broadmarktail-reorder-subcell-chunk] chunk={chunk_index} m={m} funcid7={tail_stats[0]} funcid4_13_GH={tail_stats[1]} markrisk_H={tail_stats[2]} proxy_avg={format_ratio_3(tail_stats[3],m)}")
      chunk_index+=1
  ...
  if gpu_log_level>=1:
    ...
    while subcell<75:
      total+=counts[subcell]
      subcell+=1
    print(f"[broadmarktail-reorder-subcell-summary] N={N} records={total} funcid7_tail={sum_broadmarktail_subcell_tail_counts(counts,0)} funcid4_13_GH_tail={sum_broadmarktail_subcell_tail_counts(counts,1)}")

  return counts

"""110 broadmark tail: 75 subcell rem_counts から broad primary / markrisk secondary / weak tail tertiary quota を作る。"""
...
"""110 broadmark tail: one 107 cell の F7/F413GH/R parts を弱く interleave する。"""
def interleave_broad_markdist_tail_subparts(part_f7:List[Dict[str,int]],part_f413gh:List[Dict[str,int]],part_r:List[Dict[str,int]],m_target:int)->List[Dict[str,int]]:
  out:List[Dict[str,int]]=[]
  i7:int=0
  i413:int=0
  ir:int=0
  while len(out)<m_target:
    ...
    if ir<len(part_r):
      ...
      ir+=1
      progressed=True
    if i413<len(part_f413gh):
      out.append(part_f413gh[i413])
      i413+=1
      progressed=True
    if ir<len(part_r):
      out.append(part_r[ir])
...
"""110 broadmark tail: 107 broadmark base + funcid7/F4F13/G_H weak tertiary smoothing で reordered bin を作る。"""
def build_broad_markdist_tail_reordered_bin(
  ...
  reorder_fname:str=broad_markdist_tail_reorder_output_fname(N,preset_queens)
  truncate_plain_bin(reorder_fname)

  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_broadmarktail_reorder_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{funcid_reorder_param_tag()}_sim.tsv"
  with open(progress_fname,"w") as pf:
  ...
  if gpu_log_level>=1:
    print(f"[broadmarktail-reorder-build-config] N={N} records={total_records} steps={STEPS} output={reorder_fname} progress={progress_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} weak_tail_window_boost={BROAD_MARKDIST_TAIL_WINDOW_BOOST} reason={BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON}")
  ...
  while total_remaining>0:
    ...
    while subcell<75:
      ...
      if q>0:
        ...
        group_id:int=subcell
        if tail==0 or tail==1:
          group_id+=BROAD_MARKDIST_TAIL_PHASE_SALT
    ...
    while cell<25:
      ...
      cell_parts[cell]=interleave_broad_markdist_tail_subparts(parts[cell*3],parts[cell*3+1],parts[cell*3+2],qcell)
      cell+=1
    ...
    if gpu_log_level>=2:
      print(f"[broadmarktail-reorder-build-chunk] chunk={chunk_index} off={off} m={m} B={stats[18+26]+stats[18+27]} A={stats[18+19]+stats[18+22]+stats[18+23]+stats[18+24]} C={stats[18+20]+stats[18+21]} G={stats[18+0]+stats[18+4]+stats[18+5]+stats[18+12]+stats[18+16]+stats[18+17]+stats[18+18]} O={m-(stats[18+26]+stats[18+27]+stats[18+19]+stats[18+22]+stats[18+23]+stats[18+24]+stats[18+20]+stats[18+21]+stats[18+0]+stats[18+4]+stats[18+5]+stats[18+12]+stats[18+16]+stats[18+17]+stats[18+18])} X={risk_stats[0]} T={risk_stats[1]} H={risk_stats[2]} M={risk_stats[3]} R={risk_stats[4]} funcid7={tail_stats[0]} funcid4_13_GH={tail_stats[1]} tail_proxy_avg={format_ratio_3(tail_stats[3],m)}")
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29:
      ...
      #   mode29:    ... [preset_queens] 29   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe]  # broadmarktail GPU full run
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 28        # broad funcid primary + mark-distance secondary + funcid7/F4F13/G_H weak tertiary reorder simulation/build only
      #   -g 22 22 32 484 1 0 7 29        # broadmark + funcid7/F4F13/G_H weak-tail-smoothed reordered GPU full run
      if argc >= 11:
  ...
  for N in range(nmin,nmax):
    ...
    if bench_mode==28:
      ...
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[broadmarktail-reorder-sim-only] N={N} preset_queens={preset_queens} records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} valid={1 if validate_bin_file(reorder_fname) else 0}")
    ...
    if use_gpu and N>=21 and bench_mode==29:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[broadmarktail-reorder-gpu-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()}")
      else:
        if gpu_log_level>=1:
        ...
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      total:int=exec_solutions_gpu_bin_stream_funcid_reorder(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"","broadmarktail_f7f413gh_gpu")
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      print(f"[broadmarktail-reorder-gpu-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
