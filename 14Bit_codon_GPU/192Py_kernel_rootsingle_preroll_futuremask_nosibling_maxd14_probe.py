...
# =============================================================================
# NQ_UPDATE_MEMO
# 192: 更新メモ: 191は正当性OKだが188比で0.007秒の微小退行となったため不採用。188のfuture_check_mask guard/no-sibling spill elisionへ戻し、MAXD14 kernelのみ root_avail が単一bitのthreadでroot frameをDFS loop前に1段prerollする単独実験を導入。
...
ただし速度は `0:07:10.144` で、188Pyの `0:07:10.137` より `0.007秒`、`0.002%` 遅く、ほぼ同等ですが速度更新ではありません。したがって、191Pyは**正当性OKだが不採用**とし、192Pyは191を親にせず、**188Pyを親に戻して**作成しました。

作成物はこちらです。

* [192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_probe.py)
* [192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/192Py_kernel_rootsingle_preroll_futuremask_nosibling_maxd14_bundle_20260630.tar)

192Pyの変更点は、188Pyの `future_check_mask` guard / no-sibling spill elision を維持したまま、MAXD14 kernelに **root-singleton preroll** を追加した点です。

```text
188Py:
  root frameもgeneric DFS loop内で処理

192Py:
  post-root-action の cur_avail が単一bitの場合だけ、
  depth 0をDFS loop前に1段処理してから depth 1でgeneric loopへ入る

multi-candidate root:
  188 generic pathのまま
```

191Pyのplace-XOR変更、190Pyの `block_check_mask` guard、189Pyの forced-chain fast path、187Pyの child_jmark guard、185/186Pyのterminal系変更は入れていません。192Pyは188Py親の単独実験です。

手元では以下を確認済みです。

```text
bash -n 192Py...sh                            OK
STATIC_ONLY=1 192Py...sh                      OK
...
source_split_tag split192 only                OK
abstract root-singleton preroll equivalence   200000 cases OK
```

こちらではCodon/CUDA実機build/full実行はしていません。192Pyの採用可否は、添付の `192Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に188比のtimingを見て判断してください。

"""

# 192 root-singleton preroll revision memo over validated 188 futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  192 adds one narrow fast-start: when the
...
# 192 keeps the validated 188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 192 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="192 rootsingle-preroll: 188 futuremask/no-sibling MAXD14 baseline plus root singleton fast-start - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """192 MAXD14 nibble schedule plus futuremask/no-sibling and root-singleton preroll."""
    ...
    # 192 keeps the validated 188 current-frame register cache, 13-slot
...
CHUNKSHAPE148_DEFAULT_REASON:str="192 rootsingle-preroll reuses validated 188 scorestripe_v9 task order/cache and changes only MAXD14 root single-candidate fast-start"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split192_rootsingle_preroll_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split192_rootsingle_preroll_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split192-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split192-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split192_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split192-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split192-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split192_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
