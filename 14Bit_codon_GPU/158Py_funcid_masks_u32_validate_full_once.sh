...
# 158 single-pass validation harness
#
...
#
# Candidate 158 retains the validated 157 markctrl/row-step-u32 layout and all
# earlier DFS/order changes.  Its sole kernel experiment is:
#   - keep current function-id as fu:u32
#   - keep base/mark/jmark/P5 and select/step/block classification flags in u32
#   - execute every classification-mask shift/selection with u32 operands
# int conversion remains only at the three lazy meta_next pointer indexes.
# Host representation, task order/cache, and solution arithmetic are unchanged.

SRC=${SRC:-./158Py_funcid_masks_u32_probe.py}
CAND=${CAND:-./158Py_funcid_masks_u32_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/158Py_funcid_masks_u32_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 157..149 full runs and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 158 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/158Py_funcid_masks_u32_logs_N21_full_once_${TS}"
...
if [[ -f "$SRC" ]]; then
  ...
  # 158 function-id, classification flags, and every classification mask use u32.
  for expr in \
    'fu:u32=cv&u32(31)' \
  ...
  if ! classification_cases=$(awk 'BEGIN {
      ...
      for (f=0; f<28; f++) {
        ...
        cases++
        oldkt=0
        if ((int(masks[7]/p)%2)==1) oldkt=1
        if ((int(masks[8]/p)%2)==1) oldkt=2
        newkt=0
        if ((int((masks[7]%mod32)/p)%2)==1) newkt=1
        if ((int((masks[8]%mod32)/p)%2)==1) newkt=2
  ...
  if (( source_failures != 0 )); then
    printf 'source_funcid_masks_u32_shape\trequested layout\t%d check failures\tFAIL\n' "$source_failures" >> "$SUMMARY"
    echo "[error] source does not match the requested 158 function-id/classification-u32 experiment" >&2
    exit 65
  fi
  printf 'source_funcid_masks_u32_shape\trequested layout\tverified\tOK\n' >> "$SUMMARY"
  printf 'classification_u32_equivalence\t392 cases\t%s cases\tOK\n' "$classification_cases" >> "$SUMMARY"
...
echo "[validation-ok] 158 one full run reproduced cases 01-06"
