...
確認しました。**143Py／143Py locked は問題ありません。**

* 全6ケース終了コード0
* 固定partial_total 01〜05は通常版・locked版で完全一致
* 全件結果：`314666222712`
* 通常：`0:17:33.941`
* locked：`0:17:33.924`
* 2回平均：`0:17:33.933`
* 142平均から約`31.732秒`短縮、約`2.923%`高速化
...
[144Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/144Py_scorestripe_chunkshape_probe.py)
[144Py1-6.sh](sandbox:/mnt/data/144Py1-6.sh)
[144Py1-6_locked.sh](sandbox:/mnt/data/144Py1-6_locked.sh)

144の変更は、143で32ビット化した5本のDFSスタックを維持し、**スタックの静的深さだけを縮小**するものです。
...
変更なし:
  ...
  32-lane抽出
  +11／+1 pair selector
  +29 octet first-pair lock
  constellation投入順
  generic-only mode30／mode31
```

現行の動的preset条件では、N=5〜27の最大スタック添字は20です。

```text
N=5〜17   preset=5  最大sp=13
N=18〜21  preset=6  最大sp=16
N=22〜24  preset=7  最大sp=18
N=25〜27  preset=8  最大sp=20
```

`MAXD=24`は添字0〜23を持つため、最大値20に対して3段の余裕があります。5本のu32静的配列の宣言量は、1スレッド当たり次のように減ります。

```text
143: 5 × 32 × 4 = 640 bytes
144: 5 × 24 × 4 = 480 bytes
```

なお、**144の深さ証明はN=27まで**です。N=28以上へ拡張する場合は、`MAXD`を再検討する必要があります。

01〜05は143の確定値を直接固定しており、locked版も初回144ログから値を学習しません。

```bash
codon build -release 144Py_scorestripe_chunkshape_probe.py
./144Py1-6.sh
./144Py1-6_locked.sh
```

シェル構文、字句解析、144ラベルとキャッシュ名の分離、検証ハーネス全6ケース、143からの意図外コード差分がないことを確認済みです。Codon／GPU実機実行はこの環境では未実施です。
...
# 144 supported-range bound: dynamic presets keep max DFS stack index <=20 for N<=27.
# MAXD=24 exposes indices 0..23 and retains three spare frames.
MAXD:Static[int]=24

VERSION_TAG:str="144 kernel stackdepth24 v3 over scorestripe v9 lanephase32 octetfirstpairlock29 - mode30/mode31 generic kernel"
...
def kernel_dfs_iter_gpu(
    ...
    前提/不変条件:
      ...
      - 現行 dynamic preset の N<=27 では最大 sp=20。MAXD=24 は index 0..23 を確保する。
    ...
    # 144 kernel stackdepth24 v3。
    # 142/143 で ld/rd を u32 化し、ld/rd/col/avail/ctrl の5スタックはすべて u32。
    # 144 は要素型・DFS遷移・解数演算を変えず、静的スタック長だけを 32 -> 24 に縮小する。
    # 現行 dynamic preset と endmark<=N-2 から、expected table の N<=27 では最大 sp=20。
    # MAXD=24 は index 0..23 を持つため3段の余裕があり、静的配列量を 640 -> 480 bytes/thread に減らす。
    # N>27 を有効化する場合は、この深さ境界を再証明して MAXD を見直す。
    ...
    ld=__array__[u32](MAXD)
    rd=__array__[u32](MAXD)
    ...
    while sp>=0:
      ...
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        ...
        if kt==1:
          ...
          bK=n4
        nld=((int(ld[sp])|bit_i)<<stepv)|addv|bLi
        nrd=((int(rd[sp])|bit_i)>>stepv)|bK
      else:
...
####################################################################################################
# 144 stackdepth24 generic-only harness
#
...
####################################################################################################
# 144 inherited score-aware chunk-shape layer
# The complete validated 141 scorestripe_v9_lanephase32_octetfirstpairlock29 permutation is
# retained byte-for-byte in algorithmic terms; only cache/log labels are isolated for 144.
#
# Safety boundary:
#   - score key, percentile thresholds, bucket quotas, 32-lane extraction,
#     within-bucket stride, 140 +11/+1 pair selector, and 141 +29 first-pair lock are unchanged
#   - the host layer only permutes 16-byte constellation records on disk
#   - every input record is emitted exactly once
#   - mode30 / mode31 remain single generic-kernel launches
#   - the sole 144 experiment is MAXD 32 -> 24 for the five existing u32 DFS stacks
#
# Inherited 141 phase rule:
...
# This phase-locks pair (8p,8p+1) to pair (8p+4,8p+5), while retaining
# every second-pair phase and the 140 +11/+1 selector unchanged.
####################################################################################################

CHUNKSHAPE144_REORDER_VERSION:str="scorestripe_v9_lanephase32_octetfirstpairlock29"
CHUNKSHAPE144_DEFAULT_REASON:str="inherit validated 143 scorestripe_v9 task order and five u32 DFS stacks unchanged; 144 changes only MAXD from 32 to 24 under the proven N<=27 dynamic-preset depth bound"


def chunkshape144_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape144_{CHUNKSHAPE144_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}.bin"


CHUNKSHAPE144_SCORE_KEY_MAX:int=32767
CHUNKSHAPE144_LANE_COUNT:int=32
CHUNKSHAPE144_LANE_MASK:int=31


def chunkshape144_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  144 inherited score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 144 experiment is confined
  to reducing the five existing u32 GPU DFS stacks from 32 to 24 entries.
  ...
  if key<0:
    key=0
  if key>CHUNKSHAPE144_SCORE_KEY_MAX:
    key=CHUNKSHAPE144_SCORE_KEY_MAX
  return key


def chunkshape144_build_thresholds_from_hist(hist:List[int],total_records:int)->List[int]:
  """Build 8 percentile thresholds from the score-key histogram."""
  thresholds:List[int]=[0]*8
  if total_records<=0:
    thresholds[7]=CHUNKSHAPE144_SCORE_KEY_MAX
    return thresholds

  b:int=0
  cum:int=0
  k:int=0
  while k<=CHUNKSHAPE144_SCORE_KEY_MAX:
    cum+=hist[k]
    while b<7:
  ...
  while b<7:
    thresholds[b]=CHUNKSHAPE144_SCORE_KEY_MAX
    b+=1
  thresholds[7]=CHUNKSHAPE144_SCORE_KEY_MAX
  return thresholds


def chunkshape144_bucket_from_key(key:int,thresholds:List[int])->int:
...
def chunkshape144_lane_from_key(key:int)->int:
  """Map a score key to a deterministic lane 0..31 inside its score bucket."""
  lane:int=key&CHUNKSHAPE144_LANE_MASK
  if lane<0:
    lane=0
  if lane>=CHUNKSHAPE144_LANE_COUNT:
    lane=lane&CHUNKSHAPE144_LANE_MASK
  return lane


def chunkshape144_make_quotas(rem_counts:List[int],total_remaining:int,m_target:int)->List[int]:
...
def build_chunkshape144_reordered_bin(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  """
  144 host-side score-aware lanephase chunk shaping inherited from validated 143.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 144 experiment is only MAXD 32 -> 24 for the five
  existing u32 kernel DFS stacks.
  ...
  Safety:
    ...
    - kernel_dfs_iter_gpu DFS transitions and all solution logic are unchanged; only MAXD is smaller
  """
  BLOCK:int=gpu_block
  MAX_BLOCKS:int=gpu_max_blocks
  if BLOCK<=0:
  ...
  out_fname:str=chunkshape144_reorder_output_fname(N,preset_queens,BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_chunkshape144_{CHUNKSHAPE144_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)}_sim.tsv"

  if gpu_log_level>=1:
    print(f"[chunkshape144-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE144_DEFAULT_REASON}")

  with open(progress_fname,"w") as pf:
    lane_header:str=""
    lane_i:int=0
    while lane_i<CHUNKSHAPE144_LANE_COUNT:
  ...
  if len(data)<expected_bytes:
    print(f"[chunkshape144-warning] source byte count shorter than expected: bytes={len(data)} expected={expected_bytes}")
  ...
  key_hist:List[int]=[0]*(CHUNKSHAPE144_SCORE_KEY_MAX+1)
  off:int=0
  classify_chunk:int=0
  key_min:int=CHUNKSHAPE144_SCORE_KEY_MAX
  key_max:int=0
  while off<total_records:
    ...
    i=0
    while i<m:
      key:int=chunkshape144_score_key_from_soa(soa,i,off+i)
      if key<0:
        key=0
      if key>CHUNKSHAPE144_SCORE_KEY_MAX:
        key=CHUNKSHAPE144_SCORE_KEY_MAX
    ...
    if gpu_log_level>=2:
      print(f"[chunkshape144-classify-chunk] N={N} chunk={classify_chunk} off={off} m={m}")
    off+=m
    classify_chunk+=1

  score_thresholds:List[int]=chunkshape144_build_thresholds_from_hist(key_hist,total_records)
  bucket_counts:List[int]=[0]*8
  lane_counts:List[int]=[0]*CHUNKSHAPE144_LANE_COUNT
  idx_count:int=0
  while idx_count<total_records:
    b0:int=chunkshape144_bucket_from_key(score_key_by_idx[idx_count],score_thresholds)
    lane0:int=chunkshape144_lane_from_key(score_key_by_idx[idx_count])
    bucket_counts[b0]+=1
    lane_counts[lane0]+=1
    idx_count+=1

  if gpu_log_level>=1:
    print(f"[chunkshape144-score-key] N={N} records={total_records} key_min={key_min} key_max={key_max} t0={score_thresholds[0]} t1={score_thresholds[1]} t2={score_thresholds[2]} t3={score_thresholds[3]} t4={score_thresholds[4]} t5={score_thresholds[5]} t6={score_thresholds[6]}")
    print(f"[chunkshape144-score-buckets] N={N} records={total_records} b0={bucket_counts[0]} b1={bucket_counts[1]} b2={bucket_counts[2]} b3={bucket_counts[3]} b4={bucket_counts[4]} b5={bucket_counts[5]} b6={bucket_counts[6]} b7={bucket_counts[7]}")
    lane_counts_text:str=""
    lane_i:int=0
    while lane_i<CHUNKSHAPE144_LANE_COUNT:
      lane_counts_text+=f" l{lane_i}={lane_counts[lane_i]}"
      lane_i+=1
    print(f"[chunkshape144-lanes] N={N} records={total_records}{lane_counts_text}")
  ...
  flat:int=0
  while flat<(8*CHUNKSHAPE144_LANE_COUNT):
    one:List[int]=[]
      ...
      while src_ch<n_chunks:
        ...
        if within<STEPS:
          ...
          if idx<total_records:
            bucket:int=chunkshape144_bucket_from_key(score_key_by_idx[idx],score_thresholds)
            lane_val:int=chunkshape144_lane_from_key(score_key_by_idx[idx])
            bucket_lane_indices[bucket*CHUNKSHAPE144_LANE_COUNT+lane_val].append(idx)
            staged_total+=1
        src_ch+=1
      base+=1
    out_ch+=1
  ...
  if staged_total!=total_records:
    print(f"[chunkshape144-warning] staged count mismatch: staged={staged_total} source={total_records}")

  # ------------------------------------------------------------------
  # Stage 3: write output chunks with proportional score-bucket quotas.
  #          Inside each bucket, retain the 140 +11/+1 pair bias and retain the inherited 141 complete-even-quartet first-pair +29 lock.
  # ------------------------------------------------------------------
  bucket_lane_pos:List[int]=[0]*(8*CHUNKSHAPE144_LANE_COUNT)
  bucket_lane_rem:List[int]=[0]*(8*CHUNKSHAPE144_LANE_COUNT)
  bucket_rem:List[int]=[0]*8
  b:int=0
  while b<8:
    lane:int=0
    while lane<CHUNKSHAPE144_LANE_COUNT:
      flat=b*CHUNKSHAPE144_LANE_COUNT+lane
  ...
  with open(out_fname,"ab") as out:
    while out_ch<n_chunks:
      ...
      quotas:List[int]=chunkshape144_make_quotas(bucket_rem,total_remaining,m_target)
      written_by_bucket:List[int]=[0]*8
      written_by_lane:List[int]=[0]*CHUNKSHAPE144_LANE_COUNT
      written_chunk:int=0

      # inherited 141 octetfirstpairlock29:
      ...
      while written_chunk<m_target:
        ...
        while oi<8:
          ...
          if quotas[b]>0:
            ...
            phase_seed:int=(out_ch*11 + quartet_index*17 + b*13 + pair_phase_bias + octet_first_pair_phase_bias) & CHUNKSHAPE144_LANE_MASK
            start_lane:int=(phase_seed + written_by_bucket[b]*5) & CHUNKSHAPE144_LANE_MASK
            found:int=-1
            scan:int=0
            while scan<CHUNKSHAPE144_LANE_COUNT:
              lane=(start_lane+scan) & CHUNKSHAPE144_LANE_MASK
              flat=b*CHUNKSHAPE144_LANE_COUNT+lane
              ...
              scan+=1
            if found>=0:
              flat=b*CHUNKSHAPE144_LANE_COUNT+found
      ...
      if gpu_log_level>=2:
        ...
        while lane_i<CHUNKSHAPE144_LANE_COUNT:
          ...
          lane_i+=1
        print(f"[chunkshape144-build-chunk] N={N} chunk={out_ch} written={written_chunk} total_written={written_total} b0={written_by_bucket[0]} b1={written_by_bucket[1]} b2={written_by_bucket[2]} b3={written_by_bucket[3]} b4={written_by_bucket[4]} b5={written_by_bucket[5]} b6={written_by_bucket[6]} b7={written_by_bucket[7]}{lane_debug}")
      out_ch+=1
  ...
  if written_total!=total_records:
    print(f"[chunkshape144-warning] record count mismatch: written={written_total} source={total_records}")
  if reordered_records!=written_total:
    print(f"[chunkshape144-warning] output count mismatch: file_records={reordered_records} written={written_total}")
  if gpu_log_level>=1:
    print(f"[chunkshape144-build-summary] N={N} records={reordered_records} chunks={n_chunks} output={out_fname} progress={progress_fname} valid={1 if validate_bin_file(out_fname) else 0}")
...
def exec_solutions_gpu_chunk_split144(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  if gpu_log_level>=1:
    print(f"[split144-buckets] N={N} m={m} generic={m} d2base14={d2base14_m} d0={d0_m} rest=0 split_mode={split_mode} specialized=0")
...
def exec_solutions_gpu_bin_stream_split144(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split144_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split144_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split144_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split144_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split144-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split144-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split144-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split144-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split144(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split144-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split144-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split144 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split144 full run / optional multi-GPU worker
      # Examples:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split144-generic-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split144-generic worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split144_generic_harness_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split144_generic_harness_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  if gpu_log_level>=1 and (bench_mode==30 or bench_mode==31):
    print(f"chunkshape144_params: version={CHUNKSHAPE144_REORDER_VERSION} reason={CHUNKSHAPE144_DEFAULT_REASON}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split144-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split144-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      shaped_fname:str=chunkshape144_reorder_output_fname(N,preset_queens,gpu_block,gpu_max_blocks)
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape144-probe-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape144-probe-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape144_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split144_probe_{CHUNKSHAPE144_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split144(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split144-probe-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE144_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split144-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split144-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split144-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape144-full-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape144-full-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape144_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split144_full_{CHUNKSHAPE144_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split144(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
