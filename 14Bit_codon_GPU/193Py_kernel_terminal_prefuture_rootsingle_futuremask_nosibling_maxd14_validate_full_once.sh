...
# =============================================================================
# NQ_UPDATE_MEMO
# 193: 更新メモ: 192は正当性OKかつ188比で0.019秒の微小改善。192のroot-singleton preroll/future_check_mask guard/no-sibling spill elisionを親に、MAXD14 terminal hitではfuture-prune判定を先に見ない terminal-before-future ordering を単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 193 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 193 starts from
# validated 192 because 192 was correct and numerically fastest, while 189
# forced-chain, 190 blockmask guard, and 191 place-xor were correct but not adopted.
# 193 changes only the ordering of terminal counting versus future-prune checking.
#
# Kernel arithmetic keeps 192 bitboard/schedule semantics: register-held current
# frame, 13-slot ancestor stack, scalar 4-bit nibble schedule, no-sibling spill
# elision, future_check_mask zero guard, root-singleton preroll, schedule_words=0,
# and 208 local stack bytes/thread. The experiment is deliberately focused: after
# nf is known nonzero, terminal-parent frames count immediately before the
# future_check_mask branch. This is safe because a terminal-parent nibble cannot
# have the future-prune bit set; non-terminal frames keep the validated 188/192
# future-prune condition. Host task order, dispatch, cache, progress verification,
# and README-managed history remain unchanged.
SRC=${SRC:-./193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 193 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/193Py_kernel_terminal_prefuture_rootsingle_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 193Py source
#   has a harmless header/string/layout difference. This 193 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 192 rootsingle/futuremask/no-sibling MAXD14 dispatch; 189 forced-chain, 190 blockmask, and 191 place-xor not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root-singleton preroll + terminal-before-future ordering	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'root_singleton_preroll' "$SRC" && grep -q 'if (cur_avail&(cur_avail-u32(1)))==u32(0):' "$SRC" && grep -q 'pr_nibble_op:u32=schedule_lo&u32(15)' "$SRC"; then
  printf 'source_root_singleton_preroll	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_root_singleton_preroll	present	missing	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
if python3 - "$SRC" <<'PYCHK'
import sys
from pathlib import Path
txt = Path(sys.argv[1]).read_text()
pr = txt.find('# 193 terminal-before-future: at a terminal parent')
loop = txt.find('# 193 terminal-before-future: terminal-parent nibbles')
if pr < 0 or loop < 0:
    raise SystemExit(1)
pr_term = txt.find('if terminal_depth==0:', pr)
pr_future = txt.find('if future_check_mask!=u32(0):', pr)
loop_term = txt.find('if cur_depth==terminal_depth:', loop)
loop_future = txt.find('if future_check_mask!=u32(0):', loop)
if not (0 <= pr < pr_term < pr_future < loop and 0 <= loop < loop_term < loop_future):
    raise SystemExit(1)
PYCHK
then
  printf 'source_terminal_before_future_order	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_terminal_before_future_order	present	missing-or-wrong-order	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split193' "$SRC" && ! grep -q 'split192' "$SRC" && ! grep -q 'split191' "$SRC" && ! grep -q 'split190' "$SRC" && ! grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split193 only	split193 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split193 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 193 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source terminal-prefuture static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 193 terminal-prefuture over 192 rootsingle/futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 192 keeps the validated 188 host/chunk structure, then derives chunk-tail diagnostics from
...
# 192: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 193 terminal-prefuture reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
