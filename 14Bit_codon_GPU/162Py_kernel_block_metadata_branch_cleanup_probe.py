...
* 実行時間 `0:15:05.957`
* 160Py比 `18.699秒（2.022%）`高速

162Pyでは、`frame_blocksu`の後段判定とblockK用2分岐のみを削除しました。

* [162Py_kernel_block_metadata_branch_cleanup_probe.py](sandbox:/mnt/data/162Py_kernel_block_metadata_branch_cleanup_probe.py)
* [162Py_kernel_block_metadata_branch_cleanup_validate_full_once.sh](sandbox:/mnt/data/162Py_kernel_block_metadata_branch_cleanup_validate_full_once.sh)

静的形状検査、metadata等価性888,832ケース、検証シェルの模擬全件再生は通過済みです。Codon/CUDA実機実行は未実施です。

なお、ログ記載の161PyソースSHA-256と、こちらの保管コピーのバイト列SHA-256は一致していません。実行ログの静的検査と全計算結果は正常ですが、バイト完全同一性のみ未確認です。

"""

# 162 GPU block-metadata branch-cleanup experiment:
#   Retain the fully validated 161 no-stack-overflow-guard kernel, 160 root-pop
#   loop termination, 159 kernel-local u32 mask constants, 158 function-id/
#   classification-u32 arithmetic, 157 markctrl-u32 prepack, 156 initialized-
#   ctrl-u32 packing, 155 ctrl0-u32 prepack, 154 GPU-input-u32 representation,
#   and all earlier DFS/order changes.  The sole kernel hot-path experiment
#   generates blockL and blockK metadata directly inside the existing
#   `rowv==markv` branch.  It removes the later `if frame_blocksu` test and
#   replaces the two blockK mask branches with one mutually-exclusive 2-bit
#   mask expression.  Stack layout, host data layout, task order, cache format,
#   DFS transitions, and solution arithmetic remain unchanged from validated
#   161.  The inherited GPU safety envelope remains N=5..27.
...
# MAXD=21 exposes indices 0..20 and retains zero spare frames.  162 inherits
# 161's guard removal, so GPU execution remains host-limited to this proven range.
MAXD:Static[int]=21

VERSION_TAG:str="162 block metadata branch cleanup over 161 no stack overflow guard - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    前提/不変条件:
      ...
      - 162は161のguard除去とGPU N=5..27制限を維持し、block metadata生成だけを整理する。
    ホットパス（ソース引用）:
      bit = a & -a
      avail[sp] = a ^ bit
    """
    ...
    #       162はmark成立時のblock metadata生成分岐だけを整理する単独実験とする。
    ...
    # 162は161と同じくGPU N=5..27に限定する。N>27を有効化する場合は深さ境界を再証明し、
    ...
    # 158/159/160/161/162: function-id, flags, and mask operands stay in u32.
    ...
    while True:
      ...
      if (cv&INIT_MASK)==u32(0):
        # 158/159/160/161/162: function-id and every classification-mask operand stay in u32.
        ...
        if ismu!=u32(0):
          ...
          if rowv==markv:
            frame_blocksu=u32(1)
            # 162: create block metadata only at the mark transition itself.
            # MASK_K_N3 and MASK_K_N4 are disjoint for every valid fid, so their
            # bits encode ktype 0/1/2 directly without two conditional branches.
            frame_bLiu=(
              ((MASK_L_1>>fu)&u32(1))
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154/155/156/157/158/159/160/161/162: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155/156/157/158/159/160/161/162: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154/155/156/157/158/159/160/161/162: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 162 block-metadata branch cleanup over validated 161 no-stack-overflow-guard and the 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 161; 162 changes only kernel block-metadata generation
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 161 scorestripe_v9 task order/cache unchanged; 162 cleans block metadata branches"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  162 reuses the validated 161/160/159/158/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 162 experiment changes only GPU block-metadata generation at mark transitions.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  162 host-side score-aware lanephase chunk shaping is reused unchanged from validated 161/160/159/158/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 162 experiment changes only GPU block-metadata generation; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 162 inherits the 161 safety envelope: the removed kernel guard was proven
  # unreachable only for the existing dynamic-preset range N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 162 block-metadata-cleanup GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split162_block_metadata_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split162_block_metadata_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split162-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split162-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split162_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      ...
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split162-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split162-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split162-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split162_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
