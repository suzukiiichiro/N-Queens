...
- elapsed = **362.782s** (0:06:02.782)
- vs 292確定値 367.413s → **+4.631s (+1.260%) 改善**
- vs 293 367.652s → **+4.870s (+1.325%) 改善**
- 正当性: 314666222712 ✓、全静的チェック OK

push/pop を 4回→2回に減らした効果が出ました。続けて 295 を組みます。コードの全体を把握しました。**294 は +1.26% 改善・採用確定**。次の戦略を考えます。

294で `packed_ldrd` / `packed_colav` を pop 時に定義していますが、`packed_ldrd:u64` という型宣言付き変数は Codon では loop-carried とみなされるか、push/pop が u64 の単一ロードで完結しているはずです。

次の候補: **push/pop のさらなる削減 — `colav` に depth を埋め込んでいるが、`cur_depth` をスタックから分離し scalar で追跡できるか？** いや、depth は push 時に `avail` の上位bits に埋め込まれているので現状で最適。

より有望なのは: **`ldrd`・`colav` の2配列を1本の `u128` 相当（実際は2×u64 を struct 的に持つ）にしてさらにアクセス局所性を上げる**か、あるいは全く別のアプローチとして**`save_sp` のスタック深度が浅い(MAXD14_ANCESTOR=13)ことを利用して、配列アクセスをコンパイラにアンロールさせるため固定サイズを明示する**手があります。

実はここで一番シンプルかつ効果が期待できるのは: **`thread_total` への加算を `idx+=stride` の前後で整理し、ループ終端の `thread_total+=total*w_arr[idx]; idx+=stride` を1 constellation あたり1 u64 ロードに絞る** — これは既に最適な形です。

**295 の方針: `ldrd`/`colav` を1本の `u128` 相当 `stack` 配列（u64×2 の struct ではなく、`__array__[u64](MAXD14_ANCESTOR*2)` で偶数インデックスに ldrd、奇数に colav）にして stride アクセスを改善する。**

具体的には: `stack[save_sp*2] = ldrd_val`、`stack[save_sp*2+1] = colav_val` とすることで、push/pop が連続アドレスになり L1 キャッシュのラインを無駄なく使う可能性があります。## 294 結果まとめと 295 の考え方

**294 採用確定**: 362.782s、vs 292比 **+4.631s (+1.26%) 改善**。push/pop の local memory アクセスを 4→2 に半減した効果が出ました。

---

## 295 の着眼点

294 では `ldrd[save_sp]` と `colav[save_sp]` が**2本の独立した配列**で、同じインデックス `save_sp` でアクセスしていましたが、2本の配列のメモリアドレスは連続していません。

295 では **1本の `stack[MAXD14_ANCESTOR*2]` 配列**に統合し、`sp2 = save_sp*2` として：

| 操作 | 294 | 295 |
|---|---|---|
| push ldrd | `ldrd[sp]` | `stack[sp*2]` |
| push colav | `colav[sp]` | `stack[sp*2+1]` |
| pop ldrd | `ldrd[sp]` | `stack[sp*2]` |
| pop colav | `colav[sp]` | `stack[sp*2+1]` |

隣接アドレスに収まることで、同一 128-bit キャッシュラインに両値が入りやすくなります。footprint は 26エントリ×8B = 208 bytes で変わりません。

```bash
STATIC_ONLY=1 bash 295Py_stack_merge_validate_N21_full_once.sh
bash 295Py_stack_merge_validate_N21_full_once.sh
```
...
VERSION_TAG:str="295 stack-merge probe: parent 294 colav-ldrd-pack (adopted, +1.26% vs 292); kernel_dfs_iter_gpu_maxd14 merges the 2x u64 stack arrays (ldrd, colav) into a single __array__[u64](MAXD14_ANCESTOR*2), with push/pop using interleaved indexing (stack[sp*2]=ldrd_val, stack[sp*2+1]=colav_val) so that both words of a push/pop occupy adjacent cache-line positions, improving L1 locality over the 294 two-separate-array layout; MAXD14_ANCESTOR*2=26 entries x 8B = 208 bytes/thread unchanged; hot-loop DFS logic, schedule decode, root-preroll, K_PER_THREAD_MAXD14=32, and all launch parameters otherwise unchanged from 294; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # stack[sp*2]   = ldrd  (lo32:ld  | hi32:rd)
    # stack[sp*2+1] = colav (lo32:col | hi32:avail|(depth<<27))
    ...
    while idx<m:
      ...
      if root_after_second==u32(0):
        ...
        if pr_descend!=u32(0):
          if cur_avail!=u32(0):
            sp2:int=save_sp*2
            stack[sp2]=u64(cur_ld)|(u64(cur_rd)<<u64(32))
            stack[sp2+1]=u64(cur_col)|(u64(cur_avail|(u32(cur_depth)<<u32(27)))<<u64(32))
            save_sp+=1
          cur_ld=pr_nld
          cur_rd=pr_nrd
          cur_col=pr_ncol
      ...
      while True:
        if cur_avail==u32(0):
          ...
          save_sp-=1
          sp2:int=save_sp*2
          packed_ldrd:u64=stack[sp2]
          packed_colav:u64=stack[sp2+1]
