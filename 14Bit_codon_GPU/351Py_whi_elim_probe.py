...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 351 (2026-07-30)
...
7. [351で実装・測定中] 上位半分は恒等的にゼロ -- 前半を351で実施
   338の仕様化作業中に確認した事実: `symmetry()`(2635行)の戻り値は
   `u64(2)`/`u64(4)`/`u64(8)`の3値のみであり、w_arrの値域は{2,4,8}に
   限られる。したがって328で分割した上位u32配列は**全要素が恒等的に0**
   であり、5カーネルのエピローグが読む各3箇所のロードは常に0を返す
   純粋な無駄である。さらにw=1<<e(e∈{1,2,3})なので、`markctrl`の
   未使用ビット20-21にeを詰めれば下位配列も不要になり、エピローグの
   64bit乗算はシフトに退化する(markctrlはbit0-19のみ使用: jmark(0-4)/
   endmark(5-9)/mark1(10-14)/mark2(15-19))。

   **2段に分ける。** 一度に両方やると、退行した場合に切り分けができない。
     351 = 上位配列の除去のみ。5カーネルのシグネチャからポインタ引数が
           1本消え、ディスパッチャは配列を2本ではなく1本作る。値の意味は
           一切変わらず、エピローグは `total*u64(w_lo_arr[idx])` になる。
           **u64乗算は残したまま**である。
     352 = `markctrl` へのe詰め込み。351が中立以上なら進む。エピローグの
           算術そのものが変わる(乗算 -> シフト)。

   **351は328以来はじめてカーネルに触るリビジョンである。** 339-350は
   すべてホスト側の並べ替えだった。5戦5敗の記録があるのはホットループの
   分解(240/266-269/273/326)であって、プロローグ/エピローグの変更は
   別軸だが、慎重さの水準は上げる。

   **事前登録した予測**: 削減されるのはコンステレーション1件当たり
   u32ロード1回とshift+or 1組であり、全体でも2,025,282件分にすぎない。
   内側DFSループのノード数はこれより桁違いに多い。したがって
   **期待効果は -0.00%〜-0.15% であり、ノイズ床0.03%と同程度、あるいは
   それに埋もれる可能性が高い**。351の価値は(a)証明可能に死んでいる
   作業の除去と(b)352の前提条件を満たすことにある。よって
   **中立(+0.10%以内)であれば採用**とする。有意な退行(+0.10%超)が
   出た場合は「エピローグは無償ではない」という重要な情報であり、
   352の設計を見直す。

   **ホスト側に不変条件のガードを1つ入れた。** ディスパッチャで w_arr
   全要素のORを取り、上位32ビットが非ゼロなら `[whi-error]` を出して
   Falseを返す。351の正しさは「symmetry()が32ビットに収まる」という
   一点に依存しており、N=21の正解値だけに頼ると他のNや将来の変更で
   静かに壊れる。コストは1launch当たり743424回のOR(1ミリ秒未満)で、
   350が2本目の配列を組み立てていた分より安い。
...
351は**測定リビジョン**です(349が条件付き蛇行を測ったのと同じ位置づけ。
中立以上であれば352の前に採用扱いとします)。

**351のソース変更は削除が主体です。** 5カーネルのシグネチャから
ポインタ引数1本、各エピローグ3箇所の再構成式、ディスパッチャの
2本目の配列構築と5箇所の `gpu.raw` 引数が消えます。追加は
ホスト側の不変条件ガード1つのみです。

  5カーネルのシグネチャ        引数1本を削除(計5箇所)
  各カーネルのエピローグ        再構成式 -> 単一ロード(計15箇所)
  ディスパッチャ               2本目の配列構築を削除、ガードを追加
  ディスパッチャの launch      gpu.raw 引数1本を削除(計5箇所)

**ホスト側の並べ替えは350と完全に同一です。** `CHUNKSHAPE148_ITER_SORT=9`
(条件付き蛇行)、`CHUNKSHAPE148_BUCKET_RUN=2048`、w3_j7、K=48、
`BROADMARK_VARIANT=2`、`chunkshape148_reorder_group` の全モード本体は
350とバイト一致です。したがって **shaped bin は350と同一ファイル**
(`..._isort9.bin`)であり、キャッシュは再構築されません
(`cache_state=reuse`)。`CHUNKSHAPE148_ITER_SORT=1` で346相当、`=0` で
344相当の配置にも、キャッシュファイル名まで含めていつでも戻れます。

**アンカーは350のバイナリを同一セッションで回して取ります。** 351には
除去を切り替えるノブがありません(両方のカーネルを残すとレジスタ割当と
コンパイル結果が変わり、単一変数実験でなくなるため)。350は既定値だけの
差分なので再ビルドは安価です。
================================================================================
...
**351の設計です。** 前回の引継ぎメモの論点1-5をすべて反映しました。

## 何を変えるか

`w_hi_arr` の除去のみです。**`markctrl` へのe詰め込みは352に回します。**
一度に両方やると、退行した場合にどちらが原因か切り分けられません。
...
| ディスパッチャ(追加) | 1 | 不変条件ガード |

エピローグは3種類あり、いずれも同じ形で縮みます。
...
**u64乗算はそのまま残ります。** 上位オペランドが常にゼロであることを
コンパイラは知りようがないので、351では乗算は縮みません。乗算がシフトに
退化するのは352(eを`markctrl`のbit20-21に詰める)のときです。

## ガードを1つ追加しました

351の正しさは「`symmetry()` が32ビットに収まる」という一点だけに
依存しています。N=21の正解値`314666222712`はこの前提が破れれば当然
落ちますが、**落ち方が「合計値が違う」という形になり、しかもN=21でしか
効きません**。そこで前提そのものを直接検査します。
...
コストは1launch当たり743424回のORで、1ミリ秒を大きく下回ります。
**350が2本目の配列(743424要素のList[u32]の確保と書き込み)を組み立てて
いた分より安い**ので、ホスト側の総コストは351のほうが下がります。
`[whi-error]` は `.sh` の `error_or_mismatch_hits` に拾われるので、
違反すれば静かに間違えるのではなく検証がFAILします。

## 静的チェックを反転させました

前回の引継ぎメモで赤字にしていた点です。328由来の2件は351では必ず
落ちるので、**「あること」から「ないこと」へ反転**させました。

| 旧(328) | 新(351) |
|---|---|
| `source_warr_soa_split_signatures` | `source_whi_elim_signatures` |
| `source_warr_soa_split_dispatcher` | `source_whi_elim_dispatcher` |

さらに3件を新設しました。

- `source_whi_elim_kernel_bodies` — 5カーネル本体(コメント除去後)に
  `w_hi_arr` が0件、`u64(w_lo_arr[` がちょうど15件。
- `source_whi_zero_guard` — ガードの4要素(ORの初期化、ループ、
  上位32ビットの判定、`return False`)が揃っていること。
- `runtime_whi_elim` — 実行ログに `whi_elim=removed` が出ること。
  350のバイナリが混ざっていないことを実行時に確認します。

**チェックの走査範囲は328と同じ方式で絞っています。** シグネチャ行と
ディスパッチャ本体だけを見て、docstringとコメントは除去した上で数えます。
このファイルは会話ログをdocstringへ貼り込む運用なので、全ファイル
`grep` は原理的に信用できません。上の文章にも `w_hi_arr` が何度も
出てきますが、それがチェックに影響しないようにしてあります。

## 事前登録した予測

**期待効果は -0.00%〜-0.15%、ノイズ床0.03%と同程度かそれに埋もれる
可能性が高い**と考えています。根拠は件数です。

- 削減されるのはコンステレーション1件当たりu32ロード1回とshift+or 1組。
- 全体で2,025,282件。u32ロードにして約8.1MBの転送削減。
- 内側DFSループのノード数はこれより桁違いに多く、そこは1命令も変わりません。

**したがって判定規則を先に決めておきます。**

| 実測 | 判定 |
|---|---|
| 350比 +0.10%以内(改善・中立とも) | **採用**。352へ進む |
| 350比 +0.10%超の退行 | 差し戻し。エピローグが無償ではないという情報として352の設計を見直す |

中立を採用とするのは、この変更が**証明可能に死んでいる作業の除去**で
あり、かつ352の前提条件だからです。逆に退行が出た場合、それは失敗では
なく発見です(レジスタ割当か命令スケジューリングが動いたということ)。

## 実行手順

**アンカーを同一セッションで取ってください。** 351には除去を切り替える
ノブがありません。両方のカーネルを残すとレジスタ割当が変わり、単一変数
実験ではなくなるためです。350は既定値だけの差分なので再ビルドは安価です。

```
STATIC_ONLY=1 bash 351Py_whi_elim_probe_validate_N21_full_once.sh
bash 350Py_condserp_adopt_validate_N21_full_once.sh    # アンカー(reuse)
bash 351Py_whi_elim_probe_validate_N21_full_once.sh    # 候補 1回目
bash 351Py_whi_elim_probe_validate_N21_full_once.sh    # 候補 2回目
```

**キャッシュは影響を受けません。** shaped bin はホスト側の産物であり、
ホスト側の並べ替えは350とバイト一致なので、3ランすべてが既存の
`..._isort9.bin` を再利用します(`cache_state=reuse`)。構築回は不要です。

同一構成の再現性が直近3セッションで0.03%に収まっているので、アンカー
1回・候補2回で0.1%の差は判定できます。アンカーが398.7s近傍から外れた
場合は**そこで止めてください**。下流の比較が成立しません。
...
| 正当性 | `314666222712`(両ラン一致) |
| `cache_state` | reuse(両ラン) |
| FAIL | ゼロ |

349の398.988sに対し **-0.064%**、mode 1アンカーも349の402.301sに対し
402.413sで **+0.028%**。いずれもノイズ床0.03%とほぼ同じ幅で、
**4セッション連続の再現**になりました。
...
| 項目 | 結果 |
|---|---|
| 351ソースの `STATIC_ONLY` | **OK 69 / INFO 6 / FAIL 0** |
| `bash -n` | 通過 |
| `source_str_literal_quote_balance` | 0 |
| 負のテスト(350ソースを食わせる) | 新4件すべてFAIL、診断は `maxd14=high_half_still_present` 等 |
| 実行コード差分(docstring除去後) | 26箇所+ガード+`WHI_ELIM_REASON`+print 1行のみ |
| 新規ログ文字列 vs `error｜mismatch｜ng(` | 衝突なし |
| 新規ログ文字列 vs 既存sed抽出パターン | `variant=` / `tag=` / `bucket_run=` / `iter_sort=` / `sort_group=` / `bin=` / `progress=` いずれとも衝突なし |

反転チェックの4件は `source_whi_elim_signatures` / `source_whi_elim_kernel_bodies` / `source_whi_elim_dispatcher` / `source_whi_zero_guard` として全てOKです。

## README.md について1点お断り

作成の途中で **README.md が500KBを超えたので `README2.md` に分割する**、という判断を私が勝手に入れてしまっていました。**これは元に戻し、単一の `README.md` に統合してあります。** 「1リビジョン = `.py` / `.sh` / `README.md` の3点」という運用はこのプロジェクトの規律なので、私の判断で4ファイル目を増やすべきではありませんでした。

ただし**サイズは実際に問題になり得ます**。現在 518KB / 3881行です。分割するとしても、それは提案してご判断いただくべき話なので、352の作業時に改めてお伺いします(分割案なら「Open Objectives は `README.md` 先頭に残し、年代順ログの古い側を切り出す」形が素直だと思います)。
...
3ランすべて `cache_state=reuse` になるはずです(このリビジョンに構築回は存在しません)。**アンカーが398.7s近傍から外れたらそこで止めてください** — 下流の比較が成立しません。

判定は事前に固定済みです。350アンカー比 **+0.10%以内なら採用**して352へ、**+0.10%超の退行なら差し戻し**。いずれの場合も正当性 `314666222712` を**タイミングを読む前に**確認します。

今日もよろしくお願いいたします。


**351は採用です。** そして**私の事前予測は外れました**——低すぎました。

## 結果(3ラン、正当性 `314666222712` 一致、FAILゼロ、全ラン `cache_state=reuse`)

まず、**328以来はじめてのカーネル変更ビルドが通りました。** `codon build -release` は3ランとも exit 0、コンパイラ出力ゼロ。PTX関連の問題は起きていません。ガードも `whi_zero_guard_trips=0`、実行時マーカーも `whi_elim=removed guard=or_all_high32` で、350のバイナリの混入がないことを確認済みです。
...
GPUクロックは3ランとも1320MHz固定、温度35〜43℃、電力上限300W。**測定条件に差はありません。**

## 判定

事前登録した規則は「350アンカー比 +0.10%以内なら採用」でした。実測は **−0.292%(elapsed)**。改善側に大きく振れており、**採用**です。

## ただし、数字の読み方を整理させてください

**elapsed の −0.292% をそのまま成果として書くべきではない**と考えます。理由は2つあります。

**1つ目。350の同一ソースが、セッションをまたいで +0.117% 動いています。**

| 350(同一ソース) | 0728 | 0730 | 差 |
|---|---:|---:|---:|
| chunk0 | 146125 | 146138 | **+0.0089%** |
| chunk1 | 146068 | 146067 | **−0.0007%** |
| chunk2 | 105558 | 105771 | +0.2018% |
| チャンク外オーバーヘッド | 982ms | 1224ms | **+242ms** |
| elapsed | 398.733s | 399.200s | +0.117% |

**chunk0とchunk1は0.001〜0.009%で再現しています**——驚異的です。ドリフトは chunk2(部分launch)とチャンク外オーバーヘッドに閉じ込められています。

**2つ目。私は「ホスト側のコストが下がる」と書きましたが、これは測定できていません。** オーバーヘッドは同一ソースの350自身が982ms〜1224msと242msも振れており、351の1027msはその範囲内です。**2本目の配列を作らなくなった分の節約を、この計測から取り出すことはできません。** 前回「350より安い」と設計意図に書いたのは妥当ですが、**実測で確認できたとは言えません。**

## 信頼できる中核はここです

両方の350測定に対して並べると、こうなります。
...
**フルlaunch2つに対する −0.19〜0.21% は、独立した2つの350測定に対して0.01ポイント以内で一致します。** しかもその2チャンクは再現性が0.001〜0.009%の部分です。**これが351の効果の実体**だと考えます。chunk2の−0.36%/−0.16%の開きは項目11(未解明の部分launch挙動)の領域なので、そこに寄りかかるべきではありません。

351の2ラン間の再現性は elapsed 35ms(0.0088%)、chunk0 は4ms。セッション内の再現性は相変わらず極めて良好です。

## 私の予測は外れました

**事前登録した予測は −0.00%〜−0.15% でしたが、実測(フルlaunch)は −0.19〜0.21% です。約1.4倍、予測レンジの上端も超えました。**

外した理由も明確です。私は**命令数だけで見積もっていました**——「2,025,282件分のu32ロードとshift+orは、内側DFSループのノード数に対して桁違いに小さい」。これは命令数としては正しいのですが、**実測はそれより大きい**。したがって命令数以外の何かが効いています。

候補は思いつきます(レジスタ数の変化、grid-strideループ本体の縮小によるフェッチ圧の低下、カーネル引数1本とアドレス計算の消滅)。**ただしどれも検証していないので、機構は未確定として記録すべきです。** 347で「機構が判明した」と書いて348で反証された件と同じ轍は踏みません。**再現した事実(−0.19〜0.21%、2つの独立した対照に対して0.01ポイント以内)と、その説明は分けます。**

## ノイズ床の記述を訂正すべきです

READMEの「全体0.03%」は**セッション内の話**であって、**elapsedのセッション間には当てはまりません**(今回0.117%)。chunk0/chunk1のセッション間は0.009%以下で成立しています。この区別を記録に入れておかないと、将来0.1%級の効果を誤判定します。

## 次にできること

352(`markctrl` へのe詰め込み、乗算→シフト)に進む前に、**ncuで registers/thread を350と351で比較する**のが安く価値があります。1kernel launchだけ `--section LaunchStats --section Occupancy` を取れば即座に分かり、レジスタ説が生きているかどうかが決まります。生きていれば352の期待値の見積もりも変わります。

352を先に行くか、このncu確認を挟むか、どちらにしますか。ご指示いただければ、その方向で採用リビジョン(または調査リビジョン)の3ファイルを作成します。
"""
...
VERSION_TAG:str="351 whi-elim: FIRST REVISION SINCE 328 THAT TOUCHES THE KERNEL. It removes the identically zero high half of the SoA w split that 328 introduced. symmetry() returns only u64(2), u64(4) or u64(8), so every element of w_arr is at most 8, the high u32 array was identically zero, and the three loads of it in each of the five kernel epilogues always returned 0. THE CHANGE IS MOSTLY DELETION: one pointer parameter leaves each of the five kernel signatures, fifteen epilogue reconstruction expressions collapse to a single widened u32 load, the dispatcher builds one array instead of two, and one gpu.raw argument leaves each of the five launches. THE ONE ADDITION IS A HOST-SIDE INVARIANT GUARD: the dispatcher ORs every element of w_arr and aborts with a whi-error line if the high half is ever nonzero, because the correctness of the removal rests entirely on symmetry() staying inside 32 bits and the N=21 oracle would catch a violation only as a wrong total and only for N=21. That guard costs one OR pass over 743424 elements per launch, well under a millisecond, and it is cheaper than the second array 350 was building, so host cost goes down rather than up. THE U64 MULTIPLY IS DELIBERATELY LEFT ALONE. The compiler cannot know the high operand is always zero, so nothing about the multiply shrinks here. Folding the exponent into the unused markctrl bits 20-21, which degenerates that multiply into a shift, is 352 and is kept separate on purpose so that a regression in either step can be attributed to that step. Open Objectives item 7 was recorded at 338 and this is its first half. HOST SIDE IS BYTE IDENTICAL TO 350: CHUNKSHAPE148_ITER_SORT stays 9, CHUNKSHAPE148_BUCKET_RUN stays 2048, w3_j7, K=48, BROADMARK_VARIANT=2 and every mode body in chunkshape148_reorder_group are unchanged, so the shaped bin is the same file as 350 and every run reuses it with cache_state reuse. PRE-REGISTERED PREDICTION AND DECISION RULE: the saving is one u32 load and one shift-or pair per constellation, 2025282 of them in total, about 8.1 MB of traffic, against an inner DFS loop whose node count is orders of magnitude larger and which does not change by a single instruction. The expected effect is therefore between 0.00 and -0.15 percent, comparable to or buried inside the 0.03 percent noise floor. A NEUTRAL RESULT IS A PASS, within +0.10 percent of the 350 anchor, because the change removes provably dead work and is the prerequisite for 352. A regression beyond +0.10 percent is not a failure but a finding, namely that the epilogue is not free and register allocation or instruction scheduling moved, and 352 would then be redesigned. THE ANCHOR MUST BE TAKEN IN THE SAME SESSION by rebuilding and running 350, since 351 has no knob to switch the removal off; keeping both kernel variants would change register allocation and stop this being a single-variable experiment. 350 CONFIRMED: 398.733s default against an in-session mode 1 control of 402.413s, -0.914 percent, correctness 314666222712 on both runs, cache_state reuse on both, zero FAIL. That is -0.064 percent from the 349 measurement of 398.988s and the mode 1 control moved +0.028 percent from 402.301s, a fourth consecutive session inside the noise floor. STANDING BEFORE 351: 398.733s, -12.565 percent from the 456.036s baseline at the time SoA was adopted in 328. THE STATIC CHECKS FROM 328 WERE INVERTED, not deleted: source_warr_soa_split_signatures and source_warr_soa_split_dispatcher became source_whi_elim_signatures and source_whi_elim_dispatcher, which now require the parameter to be ABSENT, and three checks were added for the kernel bodies, the guard and a runtime marker. All of them are scoped to signature lines and the dispatcher body with docstrings and comments stripped, because this project pastes chat logs into docstrings and a whole-file grep cannot be trusted."
WHI_ELIM_REASON:str="351 removes the identically zero high half of the SoA w split introduced in 328. symmetry() yields only 2, 4 or 8, so the high 32 bits of every w value were always zero and the three loads of them in each kernel epilogue were pure waste. Five kernel signatures lose one pointer parameter, the dispatcher builds one array instead of two, and each epilogue reads a single u32 and widens it. The u64 multiply is still there on purpose; folding the exponent into the unused markctrl bits 20 and 21 so that the multiply degenerates into a shift is a separate revision, 352, kept apart so a regression can be attributed. A host-side guard ORs every element of w_arr and aborts if the high half is ever nonzero, so the invariant is checked rather than assumed. Host-side reordering is byte identical to 350, so the shaped bin is the same file and is reused. The expected effect is small and plausibly inside the 0.03 percent noise floor; a neutral result is a pass, because the change removes provably dead work and unblocks 352."
