...
# 134 locked validation harness
# - Candidate: 134Py_scorestripe_chunkshape_probe
# - Kernel path remains generic-only; 134 adds scorestripe_v4_lanephase32 host-side 32-lane bucket-internal rotation over the validated 133 lanephase result.
...
CAND=${CAND:-./134Py_scorestripe_chunkshape_probe}
RUN_FULL=${RUN_FULL:-1}

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="134Py_logs_N21_locked_validation_${TS}"
...
# ================================================================
run_case \
  "05_cand134_mode31_worker0of4_quick" \
...
if [ "${RUN_FULL}" = "1" ]; then
  run_case \
    "06_cand134_mode31_full_correctness" \
    "314666222712.*ok" \
    "$CAND" \
