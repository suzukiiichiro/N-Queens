...
# =============================================================================
# NQ_UPDATE_MEMO
# 223: 更新メモ: 222Py MAXD13準備診断は正当性OKだがmax_save_sp=13/save_sp13_count>0でMAXD13単純縮小不可、かつ診断オーバーヘッドで大幅低速化した。224Pyは診断を撤回し、217Py rootrestlate最速基準相当へ戻す安全復帰版。MAXD14 kernelはroot_after_second scalar predicate / root_first late extraction / futuremask / no-sibling / root one-two prerollを維持する。
...
warning/error            0 / 0
N=21 full                0:07:07.750
```

217Py `0:07:07.709` には `0.041秒` 届きませんが、222Py診断版からは正常に速度復帰しています。したがって、予定通り **224Py generic loop clear-lowbit update** に進めます。

224Pyの差分はMAXD14 generic loop内だけです。
...
root-preroll、rootrestlate、futuremask、no-sibling、dispatch、host task orderは変更していません。こちらでは `STATIC_ONLY=1` の静的確認までOKです。

```bash
cd /home/suzuki/Github/N-Queens/13Bit_codon/Archive

STATIC_ONLY=1 bash 224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物です。

* [224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_probe.py)
* [224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/224Py_kernel_generic_clearlowbit_rootrestlate_futuremask_nosibling_maxd14_bundle_20260703.tar)

"""

# 224 generic-clearlowbit memo over validated 223/217 rootrestlate futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="224 generic-clearlowbit: 223 rootrestlate restored baseline plus generic loop cur_avail clear-lowbit update - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 224 rootrestlate parent + generic clear-lowbit update:
...
CHUNKSHAPE148_DEFAULT_REASON:str="224 generic-clearlowbit reuses validated 217 scorestripe_v9 task order/cache; MAXD14 generic loop uses cur_avail&(cur_avail-1) after bit selection"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split224_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split224_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split224-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split224-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split224_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split224-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split224-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split224_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
