...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 330 (2026-07-23)
...
5. [330で着手] funcid_reorder w/jパラメータのN21再スイープ
   現行のw8_j7は94-99時代(2026年6月上旬)にN22で決めた値のまま
   ("N22 measured best baseline w8_j7")。その後のchunkshape148
   (128-148)、nibble schedule(168)、root-preroll(196-232)、
   K-batching(292-308)、variant2(311)、SoA(328)という下流の全面改変を
   経て一度も再検証されていない(95のスイープはw10/12/16/32×j5/7/11で
   w8自体は当時のbaseline由来)。w/jは実行時引数(位置9/10)のため
   カーネル・ソース無変更でスイープ可能。330はJ=7固定でW∈{4,6,8,10,
   12,16}の6点を1セッションで実行し(w8は同一セッション内の対照点)、
   各点で正当性(314666222712)を確認の上、最終行elapsedと
   kernel_reduce_ms合計(ホスト側bin生成コストの影響を受けない
   GPU純粋時間)の両方を記録する。新しいw値では2種のreorder binが
   新規生成される(1値あたり数百MB規模、ディスク残量に注意)。

このセクション自体は通常カーネルロジックとは独立しています。
330はカーネルロジックの変更を含みません(328/329と同一。ソースの
変更はこのドキュメントとVERSION_TAG/reason文字列のみ)。
================================================================================
...
VERSION_TAG:str="330 wjsweep-reorder-retune: kernel logic is BYTE-FOR-BYTE IDENTICAL to 328/329 (SoA w_lo_arr/w_hi_arr split, ADOPTED baseline; 329's own N=21 run reconfirmed 314666222712 / 456.187s, -0.033% vs 328's 456.036s, within noise) -- this revision changes NO kernel code; source changes are documentation/VERSION_TAG/reason-string only. Two things happen in 330. FIRST, it closes the rev292 open question 'does Codon's @gpu.kernel support atomic operations': real-machine investigation (grep of /home/suzuki/.codon/lib/codon/stdlib/gpu.codon and internal/gpu.codon on cudacodon, 2026-07-23) found ZERO CUDA device atomics; the only 2 'atomic' hits are internal.gc.atomic, a GC allocator type-trait check (whether a type contains no pointers and needs no GC tracing), unrelated to device atomicAdd/atomicCAS -- therefore 292's plan-C-1 (persistent kernel + global atomic work queue) is CONFIRMED IMPOSSIBLE in pure Codon and formally CLOSED; the only remaining route to warp-level dynamic work redistribution (the sole principled fix for the tail-effect/occupancy-collapse behind the 65.5%-concentrated stall_branch_resolving confirmed in 329) is the rev84-proposed-but-never-attempted CUDA C runner (constellation generation/reordering/bin caching stays in Codon; only the kernel+launcher is rewritten in .cu reading the same bins), which also unlocks __shfl_sync/__ballot_sync, -lineinfo per-line SASS attribution (closing the 318-321 toolchain dead-end), __launch_bounds__ and shared memory -- recorded as the mid-term candidate. SECOND, 330 executes buried-idea C-3 from the 2026-07-23 archive survey (330_buried_ideas_survey.md): the funcid_reorder w/j parameters are STILL w8_j7 chosen in the rev94-99 era on N22 ('N22 measured best baseline w8_j7'), never re-validated across chunkshape148 (128-148), nibble schedule (168), root-preroll (196-232), K-batching (292-308), variant2 (311) and SoA (328); w/j are runtime args (positions 9/10), so a sweep needs no source change: 330 runs J=7 fixed, W in {4,6,8,10,12,16} (w8 doubles as the in-session control point), one full N=21 run each, correctness (314666222712) checked per point, recording BOTH the final-line elapsed AND the sum of kernel_reduce_ms across the 3 chunks (the latter is immune to first-run reorder-bin generation cost -- each new W value generates 2 new bins, several hundred MB each, so watch disk space); decision rule: adopt a new W only if its kernel_reduce_ms sum beats w8's by clearly more than session noise (reference noise band: 328 vs 329 was 0.03%, 327 vs 319 was 0.12%), otherwise keep w8_j7 and record the sweep as closing this axis for N21"
...
BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON:str="330 wjsweep-reorder-retune: kernel logic identical to 328/329 (adopted SoA baseline, 456.0-456.2s confirmed twice); this revision closes the rev292 Codon-atomic question (no CUDA device atomics in Codon's gpu module -- persistent-kernel work queue impossible in pure Codon, CUDA C runner is the only remaining route to warp-level work redistribution) and re-sweeps the funcid_reorder W parameter (J=7 fixed, W in 4/6/8/10/12/16) for N21, a value frozen since the rev94-99 N22-era tuning; w/j are runtime args so no kernel or source change is involved; adopt a new W only on a clear kernel_reduce_ms improvement beyond session noise, else close this axis for N21"
