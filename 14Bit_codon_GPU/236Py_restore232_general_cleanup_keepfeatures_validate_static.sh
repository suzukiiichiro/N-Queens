...
# =============================================================================
# NQ_UPDATE_MEMO
# 236 static validation: 232Py cleanup-only を親に戻した汎用本線を検査する。
# N21専用化、cache-hot専用化、helper分離は不可。N=5..27、-c/-g bare range、
# A10G既定値、cache生成、MAXD fallback、bench_mode 28/29/31、worker splitを残す。
# =============================================================================

SRC=${SRC:-./236Py_restore232_general_cleanup_keepfeatures_probe.py}
SUMMARY=${SUMMARY:-./236Py_restore232_general_cleanup_keepfeatures_static_summary.tsv}
...
if grep -q '236 restore232-general-cleanup-keepfeatures' "$SRC"; then ok_line source_version_tag '236 restore232-general-cleanup-keepfeatures' present; else fail_line source_version_tag '236 restore232-general-cleanup-keepfeatures' missing; fi
...
python3 - "$SRC" <<'PY' > /tmp/236_static_checks.$$ || true
import sys,re
...
checks={
  ...
  'a10g_best_params':'A10G_FINAL_DEFAULT_BLOCK:int=32' in s and 'A10G_FINAL_DEFAULT_MAX_BLOCKS:int=484' in s and 'A10G_FINAL_DEFAULT_PRESET:int=7' in s and 'A10G_FINAL_DEFAULT_BENCH_MODE:int=29' in s and 'A10G_FINAL_DEFAULT_REORDER_WINDOW_MULT:int=8' in s and 'A10G_FINAL_DEFAULT_REORDER_PHASE_JUMP:int=7' in s and 'A10G_FINAL_DEFAULT_BROADMARK_VARIANT:int=2' in s,
  ...
  'split145_mode31':'bench_mode==31' in s and 'exec_solutions_gpu_bin_stream_split145' in s and 'split236_full' in s,
...
done < /tmp/236_static_checks.$$
rm -f /tmp/236_static_checks.$$
...
sys.exit(0 if 'split236' in tags and 'split232' not in tags and 'split234' not in tags and 'split235' not in tags else 1)
PY
) && split_status=OK || split_status=FAIL
if [[ "$split_status" == OK ]]; then ok_line source_split_tag 'split236 active; no split232/234/235' "$actual_split"; else fail_line source_split_tag 'split236 active; no split232/234/235' "$actual_split"; fi
...
echo "[validation-ok] 236 general cleanup keeps 232 generic CPU/GPU range, cache, fallback kernels, and worker split"
