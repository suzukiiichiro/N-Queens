...
# =============================================================================
# NQ_UPDATE_MEMO
# 189: 更新メモ: 188は正当性OKかつ184比で1.262%高速化したため新最速基準として採用。188を親に、MAXD14 kernelのみ単一候補frameを連続処理するforced-chain fast pathを単独実験として検証する。
# Full update history: see README.md
# =============================================================================

# 189 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 189 starts from
# validated 188 because 188 improved over 184 and became the new fastest N=21
# baseline. 189 changes only the MAXD14 single-candidate forced-chain path.
#
...
# bytes/thread. The experiment is deliberately focused: when the active frame
# has exactly one candidate bit, process consecutive no-sibling frames in a
# small inner loop. Host task order, dispatch, cache, progress verification,
# and README-managed history remain unchanged.
SRC=${SRC:-./189Py_kernel_forcedchain_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./189Py_kernel_forcedchain_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/189Py_kernel_forcedchain_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 189 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/189Py_kernel_forcedchain_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 189Py source
#   has a harmless header/string/layout difference. This 189 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 188 futuremask/no-sibling MAXD14 dispatch; 184 previous baseline; 185/186/187 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + forced-chain fast path	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'forced-chain fast path' "$SRC" && grep -q 'fast_nibble_op:u32=u32(0)' "$SRC" && grep -q 'fast_child_jmark:u32=' "$SRC" && grep -q 'if (cur_avail&(cur_avail-u32(1)))==u32(0):' "$SRC"; then
  printf 'source_forced_chain_fastpath	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_forced_chain_fastpath	present	missing	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split189 only	split189 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split189 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 189 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source forced-chain static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 189 forced-chain futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 189 forced-chain futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
