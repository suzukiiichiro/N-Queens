...
はい。**最有力は function-id と分類マスク周辺**です。
...
さらに、`IS_BASE_MASK`、`IS_MARK_MASK`、`SEL2_MASK`などのマスクも`Static[int]`なので、`(MASK >> f) & 1`はソース上64bit整数領域です。Codonの通常の`int`は64bit符号付き整数です。([Exaloop Docs][1])

次の実験では、ここをまとめて`u32`内へ閉じるのがよいです。

```python
fu:u32=cv&u32(31)

if ((u32(IS_P5_MASK)>>fu)&u32(1))!=u32(0):
  if rowv==mark1:
    fu=u32(meta_next[int(fu)])

isbu:u32=(u32(IS_BASE_MASK)>>fu)&u32(1)
...
```python
ismu:u32=(u32(IS_MARK_MASK)>>fu)&u32(1)
use_futureu:u32=u32(1)-ismu
nextfidu:u32=fu

isju:u32=(u32(IS_JMARK_MASK)>>fu)&u32(1)
```

とできます。

`blockL`と`blockK type`の生成も、まだ一部が`int`式です。

```python
frame_bLiu:u32=(
  ((u32(MASK_L_1)>>fu)&u32(1))
  |(((u32(MASK_L_2)>>fu)&u32(1))<<u32(1))
)
```

`frame_ktu`もまずは分岐構造を維持したまま、判定式だけを`u32`化できます。

```python
if ((u32(MASK_K_N3)>>fu)&u32(1))!=u32(0):
  frame_ktu=u32(1)
if ((u32(MASK_K_N4)>>fu)&u32(1))!=u32(0):
  frame_ktu=u32(2)
```

`meta_next`の添字だけは`int`が必要になる可能性があるため、変換を遷移成立時だけに限定します。
...
`sp`は`-1`を探索終了の番兵として使うため、`u32`にするとアンダーフローして壊れます。
...
将来的に試すなら`u32`ではなく`i32`ですが、スタック配列の添字ごとに`int`変換が入る可能性があるため、別実験にすべきです。

`total`、`results`は解数用なので`u64`を維持します。`w_arr`は値が2・4・8だけなので`u32`化可能ですが、スレッド終了時の1ロードしかなく、効果は小さいと考えられます。

`i`と`m`も値域上は`u32`にできますが、グローバルメモリアドレス計算やポインタ添字で再変換される可能性があり、優先度は低いです。

したがって次は、**158Pyとして「function-id、分類フラグ、全分類マスク演算の完全u32化」だけを実施**するのが適切です。`frame_ktu`の分岐なし生成や`sp:i32`化は、その後に分けた方が効果を判定しやすくなります。

対象ソース：[157Py_markctrl_u32_prepack_probe.py](sandbox:/mnt/data/157Py_markctrl_u32_prepack_probe.py)

[1]: https://docs.exaloop.io/language/lowlevel/ "Low-Level Programming | Codon"


作成しました。

* [158Py_funcid_masks_u32_probe.py](sandbox:/mnt/data/158Py_funcid_masks_u32_probe.py)
* [158Py_funcid_masks_u32_validate_full_once.sh](sandbox:/mnt/data/158Py_funcid_masks_u32_validate_full_once.sh)

158Pyでは、`fu`、`isbu`、`ismu`、`isju`、`use_futureu`と、P5・SEL2・STP3・blockK/Lを含む全分類マスク演算を`u32`内で処理します。`int`変換は`meta_next`の配列添字3か所だけです。`meta_next`の値自体は`u8`から`u32`へ直接変換します。([GitHub][1])

```bash
chmod +x 158Py_funcid_masks_u32_validate_full_once.sh
./158Py_funcid_masks_u32_validate_full_once.sh
```

シェル構文、分類マスク392ケース、既存のmarkctrl・row/step・ctrlパック検査、および157Pyの131チャンクTSVを使った模擬全検証は通過しています。Codon/CUDA実機ビルドはこの環境では未実施です。

[1]: https://raw.githubusercontent.com/exaloop/codon/develop/stdlib/internal/types/int.codon?utm_source=chatgpt.com "raw.githubusercontent.com"


**`[u32]`は必要です。** これは各静的配列の「要素型」を指定しています。
...
Codon公式例も`__array__`のように、`[]`内で要素型を明示しています。([Exaloop Docs][1])
...
は要素型を決定できないため適切ではありません。また、左辺に追加の型指定を付ける必要はありません。`__array__[u32]`の戻り値から配列型が確定します。

整理すると、

* `[u32]`：配列の要素型。**必要**
* `(MAXD)`：配列の要素数。**必要**
* `ld:u32=...`：`ld`はスカラーではなく配列なので**誤り**
* `MAXD:Static[int]=21`：静的配列長なので、そのままでよい

現在の5配列はすべて、要素型が明示された`u32`静的スタック配列になっています。

[1]: https://docs.exaloop.io/language/lowlevel/?utm_source=chatgpt.com "Low-Level Programming - Codon - Exaloop"


**カーネル全域という意味では「いいえ」**です。
ただし、**DFSのビットボード・`ctrl`・分類マスク演算領域は、158Pyでほぼ完全にu32化済み**と見てよいです。

`Static[int]`の分類マスクは、現在すべて使用時に、

```python
(u32(IS_MARK_MASK)>>fu)&u32(1)
```

のように`u32`へ変換されています。そのため、宣言を変更しなくても実際のマスク演算はu32内で閉じています。Codonの通常の`int`は64bit符号付き、`u32`は32bit符号なしですが、158Pyでは演算オペランドを明示的にu32へ揃えています。([Exaloop Docs][1])

一方、次の`int`は意図的に残す必要があります。
...
と、`-1`を探索終了の番兵に使っています。`u32`へ変更すると`0 - 1`が大きな正数へ折り返すため、変更できません。

また、次の3か所に限って`int`変換が残っています。

```python
meta_next[int(fu)]
```

これは配列／ポインタの添字境界です。分類演算そのものはu32のままです。
...
も変更対象ではありません。`MAXD`は静的配列長とスタック境界に使われ、Codonの`__array__`の長さはコンパイル時リテラルである必要があります。([Exaloop Docs][1])

解数側も、オーバーフロー防止のため意図的にu64です。
...
したがって現在の状態は次のように整理できます。

| 領域                                | 状態       |
| --------------------------------- | -------- |
| `ld/rd/col/avail/free/nld/nrd/nf` | u32完了    |
| `ctrl`と全フィールド                     | u32完了    |
| `row/step/mark/end`               | u32完了    |
| function-id・分類フラグ                 | u32完了    |
| 分類マスク演算                           | 実質u32完了  |
| スタック添字`sp`                        | int維持が必須 |
| スレッド／配列添字                         | int維持    |
| 解数                                | u64維持    |

**`Static[int]`をu32宣言へ変更する実験は、速度改善にはほぼ寄与しない可能性が高いため、実施しない方がよいです。** 158Pyの時点で「GPUのDFS状態・制御・マスク計算の32bit化は完了」、ただし「カーネル内の全変数をu32化したわけではなく、そうする必要もない」という判断が正確です。

対象ソース：[158Py_funcid_masks_u32_probe.py](sandbox:/mnt/data/158Py_funcid_masks_u32_probe.py)

[1]: https://docs.exaloop.io/language/lowlevel/?utm_source=chatgpt.com "Low-Level Programming - Codon - Exaloop"

はい。ご指摘のとおり、**ソース上の`u32(...)`は削除できます**。

158Pyは全131チャンク・部分和・最終合計`314666222712`が一致し、実行時間は`0:15:54.673`でした。157Py比で`3.958秒（0.413%）`短縮し、131チャンク中99チャンクが高速化しています。

次は各マスクを、カーネル内で直接`u32`として宣言するのが安全です。
...
cv=INIT_MASK|nextfidu|...
```

ただし、**`Static[u32]`ではなく、カーネル内の通常の`u32`定数として宣言する方が安全**です。Codonで文書化されている固定幅整数は`u32 = UInt[32]`で、コンパイル時整数には主に`Static[int]`／`Literal[int]`が使われています。([Exaloop Docs][1])
...
も`IS_MARK_MASK`がコンパイル時定数なので、生成コードでは変換が定数畳み込みされ、実行時のキャスト命令が既に存在しない可能性があります。Codonの`UInt`変換は異なるビット幅ならLLVMの`trunc`などへloweringされますが、定数に対する変換は最適化対象です。([GitHub][2])

したがって今回の変更は、

* ソース上の型閉包をより明確にする
* 全マスク式を最初から最後まで`u32`にする
* Codonの型推論差による生成コード変化を測る

という意味で、**159Pyの単独実験として実施する価値があります**。

マスクはモジュールのグローバル変数へ移さず、現在と同じくカーネル内に置くべきです。Codon GPUでは、変換式から初期化された非リテラルのグローバル値がデバイス側で正しく見えないという報告があります。([GitHub][3])
...
これは`__array__[u32](MAXD)`のコンパイル時配列長として使うため、`Static[int]`を維持します。また、`u32(0)`や`u32(1)`まで共通変数化する必要はありません。

[1]: https://docs.exaloop.io/language/lowlevel/?utm_source=chatgpt.com "Low-Level Programming - Codon - Exaloop"
[2]: https://raw.githubusercontent.com/exaloop/codon/develop/stdlib/internal/types/int.codon "raw.githubusercontent.com"
[3]: https://github.com/exaloop/codon/issues/781 "GPU kernel cannot use non-literal global variables · Issue #781 · exaloop/codon · GitHub"


"""

# 158 GPU function-id/classification-mask-u32 experiment:
#   Retain the fully validated 157 markctrl-u32 prepack, 156 initialized-ctrl-u32
#   packing, 155 ctrl0-u32 prepack, 154 GPU-input-u32 representation, and all
#   earlier DFS/order changes.  The sole kernel experiment keeps function-id,
#   P5/base/mark/jmark classification flags, future-enable state, and every
#   SEL2/STP3/K/L classification-mask operation in u32.  Function-id converts
#   to int only at the meta_next pointer-index boundary.  Host data layout,
#   task order, cache format, DFS transitions, and solution arithmetic remain
#   unchanged from validated 157.
...
# 145 supported-range exact-bound probe: dynamic presets keep max DFS stack index <=20 for N<=27.
# MAXD=21 exposes indices 0..20 and retains zero spare frames.
MAXD:Static[int]=21

VERSION_TAG:str="158 function-id/classification masks full-u32 over 157 markctrl-u32 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    引数（抜粋）:
      ...
      ctrl0_arr:
        bits0..4=functionid、bits5..9=開始rowを事前パックしたu32。
      markctrl_arr:
        bits0..4=jmark、bits5..9=end、bits10..14=mark1、
        bits15..19=mark2。未使用markの-1は5bit番兵値31として保持する。
      w_arr:
    ...
    #       分岐テーブルを Static[int] のビットマスクとして焼き込む。
    #       158では使用時にu32へ畳み込み、(u32(MASK)>>fu)&u32(1)へ統一する。
    ...
    # 148 は nld/nrd 一時値も u32 に揃え、計算・nf/future判定・pushまで32bitのまま扱う。
    # 149 は block path の ctrl metadata も u32 のまま decode し、残っていた int(cv) を除去する。
    # 150 は ktype 0/1/2 を u32 mask 化し、blockK の if/elif 分岐を除去する。
    # 151 は loop-head の a/cv を展開部まで再利用し、avail/ctrl stack の重複 read を除去する。
    # 152 は meta_next[f] を遷移成立時だけ読み、通常フレーム/基底フレームの不要な lookup を除去する。
    # 153 は valid fid 0..27 で future既定値が mark分類の完全な補集合であることを利用し、
    #      別の META_AVAIL_MASK shift を廃止して use_future=1-ism とする。
    # 154 は ld/rd/col/free の入力配列と board_mask/n3/n4 scalar を u32 で渡し、
    #      kernel entry の u32 cast 7個を除去する。
    # 155 は initial row/function-id を ctrl0_arr:u32 へ事前パックし、kernel入力を
    #      2配列から1配列へ統合して ctrl[0] の shift/or/cast を除去する。
    # 156 は initialized ctrl の8フィールドを u32 で保持し、最終パックを
    #      外側の u32(...) なしの純粋な u32 shift/OR にする。
    # 157 は jmark/end/mark1/mark2 を1個の markctrl_arr:u32 へ事前パックし、
    #      kernel入力を4本から1本へ統合する。rowv/stepvもu32でdecodeし、
    #      row/mark/end比較、future境界、child-row生成を32bit内で閉じる。
    # 158 は function-id、P5/base/mark/jmark分類、future-enable、SEL2/STP3/K/Lの
    #      全分類マスク演算をu32で閉じる。meta_nextの配列添字境界だけintへ変換する。
    # Codon の UInt[32] shift は右辺も UInt[32] を要求するため、stepu/定数シフトも u32 に統一する。
    # 現行 dynamic preset と endmark<=N-2 から、expected table の N<=27 では最大 sp=20。
    # MAXD=21 は index 0..20 を持つため0段の余裕があり、静的配列量を 480 -> 420 bytes/thread に減らす。
    ...
    # 158: function-id and every classification mask/flag stay in u32.
    ...
    while sp>=0:
      ...
      if a==u32(0):
        sp-=1
        continue
      # 151: keep the loop-head ctrl value live through this iteration.
      cv:u32=ctrl[sp]
      if (cv&u32(INIT_MASK))==u32(0):
        # 158: function-id and every classification-mask operand stay in u32.
        # Only meta_next pointer indexing crosses briefly to int.
        fu:u32=cv&u32(31)
        ...
        # 通常状態設定 / is_mark分類
        #
        # valid function-id 0..27では、future既定値を持つ型
        # (P4/P5/P6) が IS_MARK_MASK の厳密な補集合である。
        ismu:u32=(u32(IS_MARK_MASK)>>fu)&u32(1)
        #######################################
        stepv:u32=u32(1)
        frame_addu:u32=u32(0)
        ...
        if ismu!=u32(0):
          ###################
          # 147 single-compare mark selector retained; marks and selector are u32.
          markv:u32=mark2 if ((u32(SEL2_MASK)>>fu)&u32(1))!=u32(0) else mark1
          ###################
          # mark
          ###################
          # a is nonzero here because the loop rejects a==0 before frame initialization.
          if rowv==markv:
            frame_blocksu=u32(1)
            use_futureu=u32(0)
            ###################
            # step
            stepv=u32(3) if ((u32(STP3_MASK)>>fu)&u32(1))!=u32(0) else u32(2)
            ###################
            # add
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154/155/156/157/158: a already holds avail[sp].  The jmark path updates both a and
      # avail[sp], so no second indexed stack read is required here.
      bit:u32=a&(u32(0)-a)
      avail[sp]=a^bit
      #----------------
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155/156/157/158: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154/155/156/157/158: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
        # low bits become all-zero/all-one masks via modular u32 subtraction.
        #   0 -> 0, 1 -> n3, 2 -> n4
        # This removes the divergent ktype if/elif from the block path.
        stepu:u32=(cv>>u32(10))&u32(3)
        addvu:u32=(cv>>u32(12))&u32(1)
        bLiu:u32=(cv>>u32(15))&u32(3)
        ktu:u32=(cv>>u32(17))&u32(3)
...
####################################################################################################
# 145 stackdepth21 generic-only harness
#
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158: convert GPU scalar inputs once per host invocation, not once per GPU thread.
    ...
    while off < m_all:
      ...
      if use_sorted:
        kernel_dfs_iter_gpu(
          ...
          gpu.raw(sort_soa.ctrl0_arr), gpu.raw(sort_soa.free_arr),
          gpu.raw(sort_soa.markctrl_arr), gpu.raw(sort_w_arr),
          gpu.raw(meta_next),
          gpu.raw(results),
          m, board_mask_gpu,
          n3_gpu, n4_gpu,
        ...
        kernel_dfs_iter_gpu(
          ...
          gpu.raw(soa.markctrl_arr), gpu.raw(w_arr),
          gpu.raw(meta_next),
          gpu.raw(results),
          m, board_mask_gpu,
          n3_gpu, n4_gpu,
...
####################################################################################################
# 158 function-id/classification-mask-u32 over validated 157 markctrl-u32 and the 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 157; 158 changes only kernel function-id/classification arithmetic types
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 157 scorestripe_v9 task order/cache unchanged; 158 keeps function-id and classification masks in u32"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  158 reuses the validated 157/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 158 experiment changes only kernel function-id/classification arithmetic types.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  158 host-side score-aware lanephase chunk shaping is reused unchanged from validated 157/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 158 experiment changes only kernel function-id/classification arithmetic types; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split158_funcid_masks_u32_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split158_funcid_masks_u32_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split158-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split158-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split158_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split158-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split158-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split158_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
