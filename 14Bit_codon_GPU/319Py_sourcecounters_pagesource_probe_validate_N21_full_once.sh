...
# =============================================================================
# 319 N=21 full validation harness (sourcecounters-pagesource-probe)
# Parent: 318 sourcecounters-sudo-retry. 318's N=21 run confirmed correct
#   (454.585s, 314666222712, within 0.007% of 316's 454.460s; clock/power
#   checks unchanged: current_sm=1320MHz vs max_sm=1710MHz WARN-CAPPED,
#   power.limit==power.default_limit==300W OK).
#
# 318_ncu.txt SUCCEEDED for the first time at getting real Source Counters /
#   PC-sampling data. Two earlier mistakes had to be corrected first this
#   session: (1) `sudo` was required (316's non-sudo attempt failed with
#   "No metrics to collect found in sections"), and (2) the flag itself was
#   wrong -- `--set SourceCounters` is invalid because `--set` selects a
#   predefined *set* (basic/full/detailed) while `SourceCounters` is a
#   *section* name, selected with `--section`. Once corrected to
#   `sudo ncu --section SourceCounters ...`, the report finally contained a
#   real "Section: Source Counters" block with PC-sampling warp-stall data:
#     # Samples (all)          = 3,670,006
#     stall_wait               = 1,592,467  (43.39%)
#     stall_branch_resolving   =   830,653  (22.63%)
#   (all stall_* categories sum exactly to # Samples -- verified, no
#   dropped buckets.) This corroborates 316's cycle-based Stall Wait
#   (44.1%) / Stall Branch Resolving (~19.6%) and 317's hardware-counter
#   divergence ratio (21.44%) -- three independent measurement methods now
#   converge on the same ~20% Stall Branch Resolving range.
#
# New secondary finding (not part of the branch investigation): ncu's OPT
#   advisor flagged an estimated 11.79% speedup from uncoalesced global
#   memory accesses (189728/1331305 excessive sectors, 14%). Logged as a
#   lower-priority parallel lead, not pursued in this revision.
#
# Still unresolved: per-line attribution. The "Hotspot Locations" section
#   in 318_ncu.txt only repeats kernel-wide aggregates -- no source
#   file/line references appear anywhere. Most likely cause: the command
#   used, `--print-details all`, renders the metric-list ("details") page,
#   not the source-correlated ("source") page.
#
# This revision: ZERO source changes from 318 (variant=2, K=48 unchanged).
#   Proposed next command (NOT auto-executed by this script) re-imports the
#   SAME already-captured 318_ncu.ncu-rep with --page source instead of
#   --print-details all. This is report post-processing only -- it does
#   NOT require re-running the GPU kernel or re-profiling with ncu:
#     /usr/local/cuda/bin/ncu --page source --print-details all \
#       --import 318_ncu.ncu-rep 2>&1 | tee 319_ncu_source.txt
#   - If this succeeds and shows metrics interleaved with source lines:
#     per-line branch attribution finally becomes available, achieving the
#     original goal of handoff priority #2 (identifying which branch
#     drives Stall Branch Resolving).
#   - If it succeeds but only shows SASS instruction addresses (no Python
#     source lines): the compiled kernel binary likely lacks embedded
#     line/debug info, and the next question becomes whether Codon exposes
#     a build flag to embed it (a build-flag change only, not a source
#     change).
#   - If --page source itself errors or reports no source available:
#     per-line attribution is genuinely unavailable in this environment;
#     317's 21.44% divergence ratio and 318's 22.63% stall_branch_resolving
#     become the final quantitative substitutes, and next steps become
#     either a manual source-level review of the hot loop's branches, or
#     pursuing the newly-found uncoalesced-memory-access lead (11.79%).
...
SRC=${SRC:-./319Py_sourcecounters_pagesource_probe.py}
CAND=${CAND:-./319Py_sourcecounters_pagesource_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/319Py_sourcecounters_pagesource_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/319Py_sourcecounters_pagesource_probe_logs_N21_full_once_${TS}"
...
echo "[start] 319 sourcecounters-pagesource-probe validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 319 sourcecounters-pagesource-probe SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '319 sourcecounters-pagesource-probe' "$SRC"; then
  printf 'source_version_tag	319 sourcecounters-pagesource-probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	319 sourcecounters-pagesource-probe	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 319 sourcecounters-pagesource-probe mode31 split145"
...
echo "[validation-ok] 319 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); 318 finally captured real Source Counters/PC-sampling data via 'sudo ncu --section SourceCounters' (stall_wait 43.39%, stall_branch_resolving 22.63% over 3,670,006 samples), corroborating 317's 21.44% divergence ratio; next step is re-importing the existing 318_ncu.ncu-rep with --page source (no rerun) per the header comment"
