...
# =============================================================================
# NQ_UPDATE_MEMO
# 289: generic-normaldefault-ncolonly。287/286のnormaldefault nld/nrdを親に、MAXD14 generic loopで ncol だけを block_code 分岐前へ前倒しする微差実験版。
...
今回は **287を親にした ncol-only early 実験版**です。
288の `nf default` は入れず、MAXD14 generic loop内で `ncol = cur_col | bit` だけを `block_code` 分岐前へ前倒ししています。

こちらで `STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [289Py_generic_normaldefault_ncolonly_probe.py](sandbox:/mnt/data/289Py_generic_normaldefault_ncolonly_probe.py)
* [289Py_generic_normaldefault_ncolonly_validate_N21_full_once.sh](sandbox:/mnt/data/289Py_generic_normaldefault_ncolonly_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [289Py_generic_normaldefault_ncolonly_sha256.txt](sandbox:/mnt/data/289Py_generic_normaldefault_ncolonly_sha256.txt)
* [289Py_generic_normaldefault_ncolonly_bundle_20260709.tar](sandbox:/mnt/data/289Py_generic_normaldefault_ncolonly_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./289Py_generic_normaldefault_ncolonly_probe

STATIC_ONLY=1 bash 289Py_generic_normaldefault_ncolonly_validate_N21_full_once.sh

bash 289Py_generic_normaldefault_ncolonly_validate_N21_full_once.sh
```

採否は、まず正当性OK、そのうえで **286の `0:07:04.033`、287の `0:07:04.486`** を上回るかで判断するのがよいです。
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while True:
      ...
      # 289: keep 287 normal-default nld/nrd and move only ncol before the special branch.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split289_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split289_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split289-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split289-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split289_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
