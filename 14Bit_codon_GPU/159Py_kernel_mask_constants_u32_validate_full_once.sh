...
# 159 single-pass validation harness
#
...
#
# Candidate 159 retains the validated 158 function-id/classification-u32 layout
# and all earlier DFS/order changes.  Its sole kernel experiment is:
#   - declare ten classification masks and INIT_MASK directly as local u32 values
#   - remove every u32(MASK_NAME) wrapper from mask use sites
# MAXD remains Static[int] because it is the compile-time __array__ length.
# Host representation, task order/cache, DFS transitions, and solution arithmetic
# are unchanged from validated 158.

SRC=${SRC:-./159Py_kernel_mask_constants_u32_probe.py}
CAND=${CAND:-./159Py_kernel_mask_constants_u32_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/159Py_kernel_mask_constants_u32_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 158..149 full runs and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 159 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/159Py_kernel_mask_constants_u32_logs_N21_full_once_${TS}"
...
if [[ -f "$SRC" ]]; then
  ...
  if (( source_failures != 0 )); then
    printf 'source_kernel_mask_constants_u32_shape\trequested layout\t%d check failures\tFAIL\n' "$source_failures" >> "$SUMMARY"
    echo "[error] source does not match the requested 159 kernel-mask-constant-u32 experiment" >&2
    exit 65
  fi
  printf 'source_kernel_mask_constants_u32_shape\trequested layout\tverified\tOK\n' >> "$SUMMARY"
...
echo "[validation-ok] 159 one full run reproduced cases 01-06"
