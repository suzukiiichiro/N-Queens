...
# =============================================================================
# NQ_UPDATE_MEMO
# 244: schedule-precompute-pack probe。241 restore239本線を親に、240別launchと
#      242小手先条件整理を採用せず、243のschedule precompute構造だけを継承する。
#      243の7本追加配列loadを、sched_lo / sched_hi_term / sched_mask の3本へpackし、
...
"""
244 schedule-precompute-pack note: 241 restore239本線を親に、240のfid14/rest別launchと242のfuturemask-depthbit微差を採用せず、単一launchのままMAXD14 schedule precompute情報を3 u32へpackする構造probe。

作成しました。244は **241親 / 単一launch維持 / 243 schedule-precompute のpack版**です。

243では7本の追加global loadが入って `0:07:10.524` まで退行したため、244ではprecompute情報を3本へpackしました。
...
sched_hi_term_arr:
  bits  0..23  schedule_hi
  bits 24..27  terminal_depth
  bit     28   terminal_base14
  bits 30..31  root_action
...
source_schedule_precompute_pack     OK
source_host_precompute_pack_arrays  OK
...
* [244Py_schedule_precompute_pack_bundle_20260707.tar](sandbox:/mnt/data/244Py_schedule_precompute_pack_bundle_20260707.tar)

sha256です。

```text
852d617c1f84e1d23c36bdf6af7f1da4384133d08abcf1348b0ef4e5069eef10  244Py_schedule_precompute_pack_probe.py
96edb971e12a4d8e56a82b3a5dc8e9e22c37251ad37783ca284f34b6a3eac14e  244Py_schedule_precompute_pack_validate_N21_full_once.sh
3713f4782cae720ee6e55d6fadead830aec0f37287c9343700da5197841864d5  README.md
```
...
STATIC_ONLY=1 bash 244Py_schedule_precompute_pack_validate_N21_full_once.sh

bash 244Py_schedule_precompute_pack_validate_N21_full_once.sh
```

こちらではCodon build自体は実行できていません。244の採否は、241の `0:07:07.788` と243の `0:07:10.524` の間でどこまで戻せるかを見て判断するのがよいです。
...
VERSION_TAG:str="244 schedule-precompute-pack probe: parent 241 restore239; single launch kept; MAXD14 host precompute fields packed into 3 u32 arrays; 240 fid14 split and 242 condition micro-diff rejected; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 244: bare -g stays split145 mode31; single-launch schedule-precompute-pack probe
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32]:
  ...
  while True:
    ...
    raw=frame_raw
    schedule_depth+=1
  sched_hi_term:u32=(schedule_hi&u32(16777215))|((terminal_parent_depth&u32(15))<<u32(24))|((terminal_is_base14&u32(1))<<u32(28))|((root_action&u32(3))<<u32(30))
  sched_mask:u32=(child_jmark_mask&u32(16383))|((future_check_mask&u32(16383))<<u32(14))
  return schedule_lo,sched_hi_term,sched_mask
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    sched_lo_arr:Ptr[u32],sched_hi_term_arr:Ptr[u32],sched_mask_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 244: packed schedule fields are precomputed on the host in build_soa_for_range().
    # 243 loaded 7 separate arrays; 244 loads 3 u32 fields and unpacks them here.
    schedule_lo:u32=sched_lo_arr[i]
    hi_term:u32=sched_hi_term_arr[i]
    schedule_hi:u32=hi_term&u32(16777215)
    terminal_depth:int=int((hi_term>>u32(24))&u32(15))
    terminal_base14:u32=(hi_term>>u32(28))&u32(1)
    root_action:u32=(hi_term>>u32(30))&u32(3)
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.sched_lo_arr),gpu.raw(soa.sched_hi_term_arr),gpu.raw(soa.sched_mask_arr),
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        soa.ijkl_arr[t]=ijkl
        sched_lo:u32=u32(0)
        sched_hi_term:u32=u32(0)
        sched_mask:u32=u32(0)
        sched_lo,sched_hi_term,sched_mask=precompute_maxd14_schedule_fields(soa.ctrl0_arr[t],soa.markctrl_arr[t],meta_next_sched)
        soa.sched_lo_arr[t]=sched_lo
        soa.sched_hi_term_arr[t]=sched_hi_term
...
CHUNKSHAPE148_DEFAULT_REASON:str="244 schedule-precompute-pack keeps validated 241/239/238 scorestripe_v9 task order/cache; single-launch MAXD14 packed schedule precompute probe"
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 244 schedule-precompute-pack; using 0")
        bench_mode=0
