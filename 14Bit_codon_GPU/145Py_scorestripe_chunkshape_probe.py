...
確認しました。**144Py／144Py locked は問題ありません。**
...
* locked：`0:17:28.347`
* 2回平均：`0:17:28.593`
* 143平均 `0:17:33.933` から約`5.340秒`短縮、約`0.507%`高速化
...
[145Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/145Py_scorestripe_chunkshape_probe.py)
[145Py1-6.sh](sandbox:/mnt/data/145Py1-6.sh)
[145Py1-6_locked.sh](sandbox:/mnt/data/145Py1-6_locked.sh)

145の変更は、144の5本u32スタックを維持したまま、**DFSスタック長をさらに縮小**するものです。
...
145は、N=5〜27の動的presetに対する理論上の最大スタック添字 `20` に合わせた**余裕0段の確認版**です。N=21では余裕がありますが、N=27まで含めると `MAXD=21` がちょうど境界です。
...
5本のu32静的配列の宣言量は、1スレッド当たり次のように減ります。

```text
144: 5 × 24 × 4 = 480 bytes
145: 5 × 21 × 4 = 420 bytes
```

01〜05の期待値は144の確定値を直接固定しており、locked版も初回145ログから値を学習しません。

```bash
codon build -release 145Py_scorestripe_chunkshape_probe.py
./145Py1-6.sh
./145Py1-6_locked.sh
```

注意点として、145は`MAXD=21`の境界確認なので、もしNGや明確な速度低下が出た場合は、余裕3段を持つ144を安全側の最終候補に戻すのがよいです。Codon／GPU実機実行はこの環境では未実施です。
...
VERSION_TAG:str="145 kernel stackdepth21 v4 over scorestripe v9 lanephase32 octetfirstpairlock29 - mode30/mode31 generic kernel"
...
####################################################################################################
# 145 inherited score-aware chunk-shape layer
# The complete validated 141 scorestripe_v9_lanephase32_octetfirstpairlock29 permutation is
# retained byte-for-byte in algorithmic terms; only cache/log labels are isolated for 145.
#
# Safety boundary:
...
#   - the sole 145 experiment is MAXD 24 -> 21 for the five existing u32 DFS stacks
#
...
CHUNKSHAPE145_REORDER_VERSION:str="scorestripe_v9_lanephase32_octetfirstpairlock29"
CHUNKSHAPE145_DEFAULT_REASON:str="inherit validated 144 scorestripe_v9 task order and five u32 DFS stacks unchanged; 145 changes only MAXD from 24 to 21 under the proven N<=27 dynamic-preset depth bound"


def chunkshape145_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape145_{CHUNKSHAPE145_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}.bin"


CHUNKSHAPE145_SCORE_KEY_MAX:int=32767
CHUNKSHAPE145_LANE_COUNT:int=32
CHUNKSHAPE145_LANE_MASK:int=31


def chunkshape145_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  145 inherited score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 145 experiment is confined
  to reducing the five existing u32 GPU DFS stacks from 24 to 21 entries.
  ...
  if key<0:
    key=0
  if key>CHUNKSHAPE145_SCORE_KEY_MAX:
    key=CHUNKSHAPE145_SCORE_KEY_MAX
  return key


def chunkshape145_build_thresholds_from_hist(hist:List[int],total_records:int)->List[int]:
  """Build 8 percentile thresholds from the score-key histogram."""
  thresholds:List[int]=[0]*8
  if total_records<=0:
    thresholds[7]=CHUNKSHAPE145_SCORE_KEY_MAX
    return thresholds

  b:int=0
  cum:int=0
  k:int=0
  while k<=CHUNKSHAPE145_SCORE_KEY_MAX:
    cum+=hist[k]
    while b<7:
  ...
  while b<7:
    thresholds[b]=CHUNKSHAPE145_SCORE_KEY_MAX
    b+=1
  thresholds[7]=CHUNKSHAPE145_SCORE_KEY_MAX
  return thresholds


def chunkshape145_bucket_from_key(key:int,thresholds:List[int])->int:
...
def chunkshape145_lane_from_key(key:int)->int:
  """Map a score key to a deterministic lane 0..31 inside its score bucket."""
  lane:int=key&CHUNKSHAPE145_LANE_MASK
  if lane<0:
    lane=0
  if lane>=CHUNKSHAPE145_LANE_COUNT:
    lane=lane&CHUNKSHAPE145_LANE_MASK
  return lane


def chunkshape145_make_quotas(rem_counts:List[int],total_remaining:int,m_target:int)->List[int]:
...
def build_chunkshape145_reordered_bin(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  """
  145 host-side score-aware lanephase chunk shaping inherited from validated 143.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 145 experiment is only MAXD 24 -> 21 for the five
  ...
  out_fname:str=chunkshape145_reorder_output_fname(N,preset_queens,BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_chunkshape145_{CHUNKSHAPE145_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)}_sim.tsv"

  if gpu_log_level>=1:
    print(f"[chunkshape145-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE145_DEFAULT_REASON}")

  with open(progress_fname,"w") as pf:
    lane_header:str=""
    lane_i:int=0
    while lane_i<CHUNKSHAPE145_LANE_COUNT:
  ...
  if len(data)<expected_bytes:
    print(f"[chunkshape145-warning] source byte count shorter than expected: bytes={len(data)} expected={expected_bytes}")
  ...
  key_hist:List[int]=[0]*(CHUNKSHAPE145_SCORE_KEY_MAX+1)
  off:int=0
  classify_chunk:int=0
  key_min:int=CHUNKSHAPE145_SCORE_KEY_MAX
  key_max:int=0
  while off<total_records:
    ...
    i=0
    while i<m:
      key:int=chunkshape145_score_key_from_soa(soa,i,off+i)
      if key<0:
        key=0
      if key>CHUNKSHAPE145_SCORE_KEY_MAX:
        key=CHUNKSHAPE145_SCORE_KEY_MAX
    ...
    if gpu_log_level>=2:
      print(f"[chunkshape145-classify-chunk] N={N} chunk={classify_chunk} off={off} m={m}")
    off+=m
    classify_chunk+=1

  score_thresholds:List[int]=chunkshape145_build_thresholds_from_hist(key_hist,total_records)
  bucket_counts:List[int]=[0]*8
  lane_counts:List[int]=[0]*CHUNKSHAPE145_LANE_COUNT
  idx_count:int=0
  while idx_count<total_records:
    b0:int=chunkshape145_bucket_from_key(score_key_by_idx[idx_count],score_thresholds)
    lane0:int=chunkshape145_lane_from_key(score_key_by_idx[idx_count])
    bucket_counts[b0]+=1
    lane_counts[lane0]+=1
    idx_count+=1

  if gpu_log_level>=1:
    print(f"[chunkshape145-score-key] N={N} records={total_records} key_min={key_min} key_max={key_max} t0={score_thresholds[0]} t1={score_thresholds[1]} t2={score_thresholds[2]} t3={score_thresholds[3]} t4={score_thresholds[4]} t5={score_thresholds[5]} t6={score_thresholds[6]}")
    print(f"[chunkshape145-score-buckets] N={N} records={total_records} b0={bucket_counts[0]} b1={bucket_counts[1]} b2={bucket_counts[2]} b3={bucket_counts[3]} b4={bucket_counts[4]} b5={bucket_counts[5]} b6={bucket_counts[6]} b7={bucket_counts[7]}")
    lane_counts_text:str=""
    lane_i:int=0
    while lane_i<CHUNKSHAPE145_LANE_COUNT:
      lane_counts_text+=f" l{lane_i}={lane_counts[lane_i]}"
      lane_i+=1
    print(f"[chunkshape145-lanes] N={N} records={total_records}{lane_counts_text}")
  ...
  flat:int=0
  while flat<(8*CHUNKSHAPE145_LANE_COUNT):
    one:List[int]=[]
      ...
      while src_ch<n_chunks:
        ...
        if within<STEPS:
          ...
          if idx<total_records:
            bucket:int=chunkshape145_bucket_from_key(score_key_by_idx[idx],score_thresholds)
            lane_val:int=chunkshape145_lane_from_key(score_key_by_idx[idx])
            bucket_lane_indices[bucket*CHUNKSHAPE145_LANE_COUNT+lane_val].append(idx)
            staged_total+=1
        src_ch+=1
      base+=1
    out_ch+=1
  ...
  if staged_total!=total_records:
    print(f"[chunkshape145-warning] staged count mismatch: staged={staged_total} source={total_records}")
  ...
  # ------------------------------------------------------------------
  bucket_lane_pos:List[int]=[0]*(8*CHUNKSHAPE145_LANE_COUNT)
  bucket_lane_rem:List[int]=[0]*(8*CHUNKSHAPE145_LANE_COUNT)
  bucket_rem:List[int]=[0]*8
  b:int=0
  while b<8:
    lane:int=0
    while lane<CHUNKSHAPE145_LANE_COUNT:
      flat=b*CHUNKSHAPE145_LANE_COUNT+lane
  ...
  with open(out_fname,"ab") as out:
    while out_ch<n_chunks:
      ...
      quotas:List[int]=chunkshape145_make_quotas(bucket_rem,total_remaining,m_target)
      written_by_bucket:List[int]=[0]*8
      written_by_lane:List[int]=[0]*CHUNKSHAPE145_LANE_COUNT
      written_chunk:int=0
      ...
      while written_chunk<m_target:
        ...
        while oi<8:
          ...
          if quotas[b]>0:
            ...
            phase_seed:int=(out_ch*11 + quartet_index*17 + b*13 + pair_phase_bias + octet_first_pair_phase_bias) & CHUNKSHAPE145_LANE_MASK
            start_lane:int=(phase_seed + written_by_bucket[b]*5) & CHUNKSHAPE145_LANE_MASK
            found:int=-1
            scan:int=0
            while scan<CHUNKSHAPE145_LANE_COUNT:
              lane=(start_lane+scan) & CHUNKSHAPE145_LANE_MASK
              flat=b*CHUNKSHAPE145_LANE_COUNT+lane
              ...
              scan+=1
            if found>=0:
              flat=b*CHUNKSHAPE145_LANE_COUNT+found
      ...
      if gpu_log_level>=2:
        ...
        while lane_i<CHUNKSHAPE145_LANE_COUNT:
          ...
          lane_i+=1
        print(f"[chunkshape145-build-chunk] N={N} chunk={out_ch} written={written_chunk} total_written={written_total} b0={written_by_bucket[0]} b1={written_by_bucket[1]} b2={written_by_bucket[2]} b3={written_by_bucket[3]} b4={written_by_bucket[4]} b5={written_by_bucket[5]} b6={written_by_bucket[6]} b7={written_by_bucket[7]}{lane_debug}")
      out_ch+=1
  ...
  if written_total!=total_records:
    print(f"[chunkshape145-warning] record count mismatch: written={written_total} source={total_records}")
  if reordered_records!=written_total:
    print(f"[chunkshape145-warning] output count mismatch: file_records={reordered_records} written={written_total}")
  if gpu_log_level>=1:
    print(f"[chunkshape145-build-summary] N={N} records={reordered_records} chunks={n_chunks} output={out_fname} progress={progress_fname} valid={1 if validate_bin_file(out_fname) else 0}")
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split145_generic_harness_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split145_generic_harness_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  if gpu_log_level>=1 and (bench_mode==30 or bench_mode==31):
    print(f"chunkshape145_params: version={CHUNKSHAPE145_REORDER_VERSION} reason={CHUNKSHAPE145_DEFAULT_REASON}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split145-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split145-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      shaped_fname:str=chunkshape145_reorder_output_fname(N,preset_queens,gpu_block,gpu_max_blocks)
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape145-probe-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape145-probe-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape145_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split145_probe_{CHUNKSHAPE145_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split145-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split145-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape145-full-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape145-full-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape145_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split145_full_{CHUNKSHAPE145_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      ...
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split145-full-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE145_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
