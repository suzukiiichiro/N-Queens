...
169Pyログを確認しました。**計算結果・検証は問題ありません**。
...
なので、**正当性はOK、最速基準はまだ168Pyのまま**という扱いがよいです。169Pyでは全131チャンクが `required_maxd=14 / selected_MAXD=14 / schedule_words=4 / stack_bytes=240` で通っています。

次の170Pyとして、メモの方針どおり **MAXD14だけ schedule配列を廃止し、7種類の演算を3bit化して1個の`u64`へ格納**する版を作成しました。狙いは `240 → 224 bytes/thread` です。

ファイルはこちらです。

* [170Py_kernel_scalar_u64_schedule_maxd14_probe.py](sandbox:/mnt/data/170Py_kernel_scalar_u64_schedule_maxd14_probe.py)
* [170Py_kernel_scalar_u64_schedule_maxd14_validate_full_once.sh](sandbox:/mnt/data/170Py_kernel_scalar_u64_schedule_maxd14_validate_full_once.sh)

170Pyの主な変更点です。

```text
MAXD14:
  local schedule u32[4] を廃止
  scalar u64 schedule_u64 に集約

compact op:
  0    normal step1, futureなし
  1    normal step1, futureあり
  2..6 block_code 1..5

schedule_u64 layout:
  bits  0..41 : 14個 × 3bit compact op
  bits 42..55 : child jmark mask
  bits 56..59 : terminal parent depth
  bit      60 : terminal kind, 0=base, 1=base14
  bits 61..62 : root action
...
MAXD14 scalar-u64 schedule            OK
compact_opcode_classes                7 classes OK
frame_opcode_equivalence              506604 cases OK
dfs_164_vs_170_scalar_equivalence     12000 cases OK
N21 depth14 scalar schedule cases      3415 cases OK
...
```bash
chmod +x 170Py_kernel_scalar_u64_schedule_maxd14_validate_full_once.sh
./170Py_kernel_scalar_u64_schedule_maxd14_validate_full_once.sh
```

で、N=21 full run の `314666222712`、全131チャンクの `selected_MAXD=14`、`schedule_words=0`、`stack_bytes_per_thread=224` を確認する流れです。

"""

# 170 GPU scalar-u32pair compact schedule experiment over 169 MAXD14:
#   Branch from the validated 169 MAXD14 static-dispatch build.  Host-side
#   depth computation and dispatch remain 14/16/18/20/21.  The MAXD14 kernel
#   replaces the local u32[4] byte-opcode schedule with PTX-safe scalar u32 fields.
#
...
#
#   MAXD14 local storage is now only four u32[14] state arrays
#   (224 bytes/thread).  MAXD16/18/20/21 intentionally keep the validated 169
#   packed-byte local schedule fallback for larger required depths.  Host layout,
...
# 170 static stack specializations.  MAXD remains the number of stored state
# frames.  MAXD14 uses scalar u32 compact schedule fields and no local u32 schedule
# array.  Larger fallbacks keep the 169 byte-opcode local schedule.
...
SCHED_WORDS14:Static[int]=0  # 170 MAXD14 uses scalar u32 schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="170 scalar-u32pair 3-bit schedule for MAXD14 over 169 MAXD14 dispatch - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 170 inherits the exact 169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """170 PTX-safe scalar-u32-pair compact 3-bit schedule kernel, static MAXD=14."""
...
def kernel_dfs_iter_gpu_maxd16(
    ...
    """170 inherited 169 packed byte-opcode schedule fallback, static MAXD=16."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd18(
    ...
    """170 inherited 169 packed byte-opcode schedule fallback, static MAXD=18."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd20(
    ...
    """170 inherited 169 packed byte-opcode schedule fallback, static MAXD=20."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd21(
    ...
    """170 inherited 169 packed byte-opcode schedule fallback, static MAXD=21."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
####################################################################################################
# 170 scalar-u32pair compact schedule + MAXD14 static dispatch harness
#
...
# changing only the MAXD14 schedule representation from local u32[4] to PTX-safe scalar u32 fields.
####################################################################################################
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164/167/168/169/170: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 169 scorestripe_v9 task order/cache and MAXD14 dispatch; 170 compacts MAXD14 schedule to PTX-safe scalar u32 fields"
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/167/168/169/170: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 170 branches from validated 169.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 170 scalar-u32pair schedule GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split170_scalar_u32pair_schedule_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split170_scalar_u32pair_schedule_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split170-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split170-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split170_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split170-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split170-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split170_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
