...
# =============================================================================
# NQ_UPDATE_MEMO
# 286: generic-normaldefault-nldnrd。285/283 generic normal-firstを親に、
...
"""
285復帰版と286 normal path対応版を作成しました。

どちらもこちらで `STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

## 285 復帰版

284の `ncol early` を不採用にして、283 `generic normal-first` へ戻す安全復帰版です。

* [285Py_restore283_after_ncol_early_reject_probe.py](sandbox:/mnt/data/285Py_restore283_after_ncol_early_reject_probe.py)
* [285Py_restore283_after_ncol_early_reject_validate_N21_full_once.sh](sandbox:/mnt/data/285Py_restore283_after_ncol_early_reject_validate_N21_full_once.sh)
* [285Py_restore283_after_ncol_early_reject_sha256.txt](sandbox:/mnt/data/285Py_restore283_after_ncol_early_reject_sha256.txt)
* [285Py_restore283_after_ncol_early_reject_bundle_20260709.tar](sandbox:/mnt/data/285Py_restore283_after_ncol_early_reject_bundle_20260709.tar)

```bash
rm -f ./285Py_restore283_after_ncol_early_reject_probe

STATIC_ONLY=1 bash 285Py_restore283_after_ncol_early_reject_validate_N21_full_once.sh

bash 285Py_restore283_after_ncol_early_reject_validate_N21_full_once.sh
```

## 286 normal path対応版

285/283を親に、MAXD14 generic loop内で normal path の `nld/nrd` を既定値として先に作り、`block_code!=0` のspecial pathだけ上書きする実験版です。
...
if block_code != 0:
    # special path だけ nld/nrd を上書き
```

* [286Py_generic_normaldefault_nldnrd_probe.py](sandbox:/mnt/data/286Py_generic_normaldefault_nldnrd_probe.py)
* [286Py_generic_normaldefault_nldnrd_validate_N21_full_once.sh](sandbox:/mnt/data/286Py_generic_normaldefault_nldnrd_validate_N21_full_once.sh)
* [286Py_generic_normaldefault_nldnrd_sha256.txt](sandbox:/mnt/data/286Py_generic_normaldefault_nldnrd_sha256.txt)
* [286Py_generic_normaldefault_nldnrd_bundle_20260709.tar](sandbox:/mnt/data/286Py_generic_normaldefault_nldnrd_bundle_20260709.tar)

```bash
rm -f ./286Py_generic_normaldefault_nldnrd_probe

STATIC_ONLY=1 bash 286Py_generic_normaldefault_nldnrd_validate_N21_full_once.sh

bash 286Py_generic_normaldefault_nldnrd_validate_N21_full_once.sh
```
...
実行順は、まず285で283相当へ戻ることを確認し、その後286を回すのが安全です。
...
VERSION_TAG:str="286 generic-normaldefault-nldnrd: parent 285/283 generic normal-first; MAXD14 generic loop computes normal-path nld/nrd as default and only overwrites them for block_code!=0 special path; no ncol early, no placed_ld/placed_rd commonization, root-preroll and host shaping/cache/dispatch preserved"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while True:
      ...
      # 286: normal path is the dominant path in 281 diagnostics.
      # Compute normal nld/nrd as defaults, then overwrite only for special block_code.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split286_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split286_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split286-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split286-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split286_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
