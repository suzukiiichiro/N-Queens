...
118 の結果、確認しました。
これは **d2 全体 kernel、つまり funcid 12..18 をまとめた kernel の PTX 生成が Codon/CUDA 側で通らない**という切り分けでよいです。

一方で、117 d0-only は chunk `0,40,120` で `partial_total=7287470230` まで正常に通っています。chunk 別にも `2132383228 / 2863018070 / 2292068932` で、116/117 の基準値と一致しています。

次は d2 をさらに小さくし、**SQd2B / funcid 14 だけ**に絞ります。
118 では `12..18` の mark 遷移を全部 1 kernel に入れたため、分岐・ctrl・step=2/3・K=n3/n4・L=1/2 が一気に入りました。119 ではそれを避け、まず `funcid==14` の base DFS だけを専用 kernel にしています。

作成しました。

[119Py_d2base14_kernel_probe.py](sandbox:/mnt/data/119Py_d2base14_kernel_probe.py)
...
目的:
  1. d2 の base 特例 free & ~1 が PTX 的に安全か確認
  2. d2 mark 遷移 12/13/15/16/17/18 はまだ入れない
  3. 118 の INVALID_PTX 原因が「d2全体の複雑化」か「d2B自体」かを分ける
```

今回は、専用 kernel を **rest 汎用 kernel より先に起動**するようにしました。
そのため `d2base14>0` の chunk なら、PTX が駄目な場合は rest 側の長い計算を待たずに落ちるはずです。

まず build です。

```bash
codon build -release 119Py_d2base14_kernel_probe.py
```

最初は同じ chunk list でお願いします。

```bash
./119Py_d2base14_kernel_probe \
...
```text
[split119-buckets] N=21 m=15488 d2base14=... rest=... split_mode=2 d2base14_specialized=1
```

判定は以下です。

```text
d2base14 > 0 で CUDA_ERROR_INVALID_PTX
  → SQd2B/fid14 の最小 kernel でも PTX が壊れる。
     d2 は専用 kernel 化の入口を別方式にする必要あり。

d2base14 > 0 で通る
  → fid14 base は安全。
     次は 120 で d2 mark を 1 本だけ追加、候補は fid13 または fid17。

d2base14 = 0
  → 専用 kernel が起動されていないので、PTX 確認としては不十分。
     別 chunk を選ぶ必要あり。
```

期待 total は、同じ chunk list ならこれです。
...
119 が通った場合でも、まだ N21 full は不要です。
次は `fid14` に接続する mark 側を **1種類ずつ**戻します。候補順はこう見ています。

```text
120: fid13 または fid17 の step2 → fid14
121: fid12 または fid16 の step2 → fid13/17
122: fid15 または fid18 の step3 → fid14
```

118 で `12..18` 一括が落ちたので、ここからは d2 も d0 と同じく **1 kernel ずつ、PTX を通したものだけ採用**で進めるのが安全です。
...
VERSION_TAG:str="119 d2base14-only kernel probe - rest uses proven 115 GPU kernel"
...
####################################################################################################
# 119 d2base14-only kernel probe
#
...
# the proven 115 kernel_dfs_iter_gpu device code, plus one small d2base14-only kernel.
#
# Purpose:
...
#   - add exactly one new specialized kernel at a time in a 119 probe
...
# every bucket still used the generic 115 kernel.  This 119 build adds exactly one new PTX target:
...
####################################################################################################
# 119 d2base14-only split-kernel chunk runner
####################################################################################################

"""119: SoA 1件を別 SoA へコピーする小ヘルパ。"""
...
"""119: d2 base fid14 だけ専用 kernel、その他は 115 汎用 kernel にまとめる。"""
def exec_solutions_gpu_chunk_split119(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  """
  119 d2base14-only split:
    - build one SoA for the input chunk
    - split only funcid 14 into a d2base14 bucket
    - run all other tasks as a single rest bucket through the proven 115 generic kernel
    - run fid14 through kernel_dfs_d2base14_gpu

  This keeps the PTX experiment smaller than the failed 118 d2-wide kernel.  It should be used first with mode 30 probe chunks.
  """
  board_mask:int=(1<<N)-1
  ...
  while i<m:
    fid:int=soa.funcid_arr[i]
    if use_d2_specialized and fid==14:
  ...
  if gpu_log_level>=1:
    print(f"[split119-buckets] N={N} m={m} d2base14={d2_m} rest={rest_m} split_mode={split_mode} d2base14_specialized={1 if use_d2_specialized else 0}")

  meta_next:List[u8]=[u8(1),u8(2),u8(3),u8(3),u8(2),u8(6),u8(2),u8(2),u8(0),u8(4),u8(5),u8(7),u8(13),u8(14),u8(14),u8(14),u8(17),u8(14),u8(14),u8(20),u8(21),u8(21),u8(21),u8(25),u8(21),u8(21),u8(26),u8(26)]

  # Specialized bucket first: this makes PTX/JIT failures appear before the generic rest work.
  if d2_m>0:
    GRID=(d2_m+BLOCK-1)//BLOCK
    kernel_dfs_d2base14_gpu(gpu.raw(d2_soa.ld_arr),gpu.raw(d2_soa.rd_arr),gpu.raw(d2_soa.col_arr),gpu.raw(d2_soa.row_arr),gpu.raw(d2_soa.free_arr),gpu.raw(d2_soa.jmark_arr),gpu.raw(d2_soa.end_arr),gpu.raw(d2_soa.mark1_arr),gpu.raw(d2_soa.mark2_arr),gpu.raw(d2_soa.funcid_arr),gpu.raw(d2_w),gpu.raw(results),d2_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<d2_m:
      chunk_total+=int(results[i])
      i+=1
...
def exec_solutions_gpu_bin_stream_split119(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split119_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split119_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split119_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split119_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split119-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split119-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split119-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split119-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split119(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split119-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split119-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split119 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split119 full run / optional multi-GPU worker
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 30 8 7 0 1222 5 "" 2  # 119 d2base14-only probe chunks 1222..1226
      #   -g 22 22 32 484 1 0 7 31 8 7 0 0 1 2  # 119 d2base14-only full GPU single worker
      if argc >= 11:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split119-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split119 worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split119_d2base14_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split119_d2base14_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split119-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split119-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split119_probe_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split119(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split119-probe-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split119-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split119-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split119-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split119_full_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split119(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split119-full-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
