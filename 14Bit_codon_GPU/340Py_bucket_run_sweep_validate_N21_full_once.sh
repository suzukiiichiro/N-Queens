...
# =============================================================================
# 340 N=21 full validation harness (bucket-run-sweep)
# Base: 339/338/337/336/335/334/333/328-332 lineage.
#
# 340 CONTAINS NO SOURCE CODE CHANGE AT ALL. 340Py_bucket_run_sweep.py is
# byte-for-byte identical to 339's source apart from VERSION_TAG and the
# documentation block -- kernels, dispatcher, K=48, SoA w_lo/w_hi, w3_j7 and
# the host-side build_chunkshape148_reordered_bin emit loop introduced in 339
# are all unchanged.
#
# 339 RESULT (confirmed, POSITIVE -- this is the new reference point):
#   CHUNKSHAPE148_BUCKET_RUN=32, correctness 314666222712 exact,
#   elapsed 447.116s vs 338's 450.329s = -0.713%, with
#   chunkshape148_cache_state=reuse so the timing is directly comparable.
#   Every baseline since 333 lies in 450.056-450.667s (full width 0.136%),
#   so this is about 4.8x the entire historical noise band. Per-chunk kernel
#   elapsed: chunk0 +0.244%, chunk1 -0.745%, chunk2 -1.856%, total -0.666%.
#   First measured win against the 329 tail effect, kernel untouched.
#
# THIS SCRIPT is the ordinary single-point validate harness. Its default is
# CHUNKSHAPE148_BUCKET_RUN=32 (the 339 result), so running it bare
# reproduces 339. The actual 340 experiment is the SWEEP, which is driven by
# the standalone companion script:
#
#     bash 340_bucket_run_sweep_driver.sh
#
# The driver calls THIS script once per sweep point with a different
# CHUNKSHAPE148_BUCKET_RUN, handles the build/cached two-pass protocol, and
# aggregates the results into one table. It is separate for the same reason
# 337's bin-format check was separate: this harness stays a clean, unchanged
# single-run validate.
#
# WHY THESE SWEEP POINTS (the driver's default: 1 32 16 48 64 32):
#   The question is not "is more better" but "why does 32 work". Three
#   discriminating points answer it:
#     RUN=16  below warp width; one warp-iteration straddles two buckets.
#             Model predicts WORSE than 32.
#     RUN=48  LARGER than 32 but not a multiple of it, so warp boundaries
#             and bucket boundaries misalign. If a larger value still
#             regresses, the mechanism is warp-width ALIGNMENT, not run
#             length. This is the decisive point.
#     RUN=64  a multiple of 32: intra-warp homogeneity is preserved while
#             inter-warp mixing coarsens. Model predicts ~equal to 32.
#   RUN=1 (control) and RUN=32 (anchor) are re-measured in the same session
#   so the comparison does not depend on cross-session stability, and RUN=32
#   is measured first AND last so session drift is itself visible.
#
# ADOPTION IS DEFERRED TO 341. 340 is a pure measurement revision; the
# source default CHUNKSHAPE148_BUCKET_RUN stays 1.
#
# TIMING CAVEAT: a BUCKET_RUN value with no cached shaped bin makes the
# first run rebuild it, and that rebuild lands inside the N=21 elapsed line.
# chunkshape148_cache_state=build|reuse records which happened; only reuse
# rows are comparable. The driver handles this automatically.
#
# MANDATORY VALIDATION ORDER:
#   1. STATIC_ONLY=1 first (checks are unchanged from 339, including the
#      339 r2 quote-balance guard).
#   2. Full N=21 run; confirm 314666222712 FIRST, before any timing.
#   3. Then the sweep via 340_bucket_run_sweep_driver.sh.
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./340Py_bucket_run_sweep.py}
CAND=${CAND:-./340Py_bucket_run_sweep}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/340Py_bucket_run_sweep_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/340Py_bucket_run_sweep_logs_N21_full_once_${TS}"
...
echo "[start] 340 bucket-run-sweep validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 340 bucket-run-sweep SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '340 bucket-run-sweep' "$SRC"; then
  printf 'source_version_tag	340 bucket-run-sweep	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	340 bucket-run-sweep	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN")
{
  ...
  echo "validation: one N=21 full run through 340 bucket-run-sweep (kernel-identical to 338/337/336/335/334/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN"
...
echo "[validation-ok] 340 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); 340 changes NO source code at all -- identical to 339 apart from VERSION_TAG and documentation. This run used CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN (339 measured 447.116s at 32, -0.713% vs 338). CHECK chunkshape148_cache_state IN THE SUMMARY BEFORE COMPARING TIMING: only reuse rows are comparable. The 340 experiment proper is the sweep -- run bash 340_bucket_run_sweep_driver.sh"
