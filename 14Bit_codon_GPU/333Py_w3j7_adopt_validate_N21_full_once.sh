...
# =============================================================================
# 333 N=21 full validation harness (w3j7-adopt)
# Base: 328-332 lineage (adopted SoA baseline; kernel logic byte-for-byte
#   identical throughout). This revision is the FIRST source change since
#   328, but it is HOST-SIDE PARAMETER ONLY: the funcid_reorder defaults
#   A10G_FINAL_DEFAULT_REORDER_WINDOW_MULT and FUNCID_REORDER_V2_WINDOW_MULT
#   change 8 -> 3 (PHASE_JUMP stays 7), plus the reason string. The kernel
#   and dispatcher are untouched.
#
# BASIS (330-332 three-stage sweep, buried-idea C-3 of the archive survey):
#   330: W in {4,6,8,10,12,16} at J=7 -> W=4 candidate (-0.523%).
#   331: W in {2,3,4,5,8} -> W=3 interior optimum, BRACKETED by worse
#        W=2 (450840) and W=4 (452288); kernel_reduce_ms 448789 = -1.294%
#        vs W=8's 454673.
#   332: J side at W=3 -> (3,3)/(3,5)/(3,11) all clearly worse
#        (462440/458032/458036); (1,7)=454315 (-0.08%, within noise)
#        rules out a low-side second minimum. J=7 confirmed.
#   Metric reliability: (8,7) control reproduced across FOUR sessions at
#   454672/454674/454673/454677 (spread 5ms = 0.001%); (3,7) anchor
#   reproduced to 2ms; cached-bin elapsed predictions hit exactly
#   (W=4: ~453.6s predicted, 453.623s measured; W=3: ~450.1s predicted,
#   450.113s measured).
#
# EXPECTED RESULT OF THIS RUN:
#   - Correctness 314666222712 UNCHANGED. The w/j parameters only REORDER
#     the identical task set; any total deviation means a real bug and is
#     an immediate abort (do not rationalize it as a side effect).
#   - elapsed ~450.1s (~-1.3% vs the 456.0s w8_j7 baseline);
#     kernel_reduce_ms sum ~448.8s. The w3_j7 bins are already cached
#     from 331/332, so no bin-generation cost applies.
# MANDATORY VALIDATION ORDER (unchanged discipline):
#   1. STATIC_ONLY=1 first (includes the new w3/j7 constant checks).
#   2. Full N=21 run; confirm 314666222712 FIRST, before timing.
#   3. Compare timing against the w3_j7 sweep measurement (450.113s) and
#      the old w8_j7 baseline (456.036s/456.187s).
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./333Py_w3j7_adopt.py}
CAND=${CAND:-./333Py_w3j7_adopt}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/333Py_w3j7_adopt_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/333Py_w3j7_adopt_logs_N21_full_once_${TS}"
...
echo "[start] 333 w3j7-adopt validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 333 w3j7-adopt SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '333 w3j7-adopt' "$SRC"; then
  printf 'source_version_tag	333 w3j7-adopt	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	333 w3j7-adopt	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 333 w3j7-adopt (kernel-identical to 328) mode31 split145"
...
echo "[validation-ok] 333 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); w3_j7 formally ADOPTED as funcid_reorder default (replacing rev94-99-era N22-tuned w8_j7; basis: 330-332 three-stage sweep, kernel_reduce_ms -1.294%, metric reproducible to 0.001% across four sessions); kernel/dispatcher identical to 328-332; expected elapsed ~450.1s vs old 456.0s baseline -- if timing does not land near 332's direct (3,7) measurement 450.113s, investigate before proceeding"
