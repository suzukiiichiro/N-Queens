...
# =============================================================================
# NQ_UPDATE_MEMO
# 195: 更新メモ: 194は正当性OKかつ192比1.830秒高速で新最速基準として採用。194のfuture_check_mask guard/no-sibling/root one/two-candidate prerollを親に、MAXD14 kernelのみ root_avail が1bit/2bit/3bitのthreadでdepth0先頭候補をDFS loop前に1段prerollする単独実験を導入。
...
194Pyログを確認しました。**正当性・実行結果ともに問題ありません。**
...
速度は `0:07:08.288` でした。
192Pyの `0:07:10.118` より `1.830秒`、`0.425%` 高速です。差は大きくはありませんが、数値上は明確に更新しているため、**194Pyを新最速基準として採用**しました。

作成物はこちらです。

* [195Py_kernel_rootthree_preroll_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/195Py_kernel_rootthree_preroll_futuremask_nosibling_maxd14_probe.py)
* [195Py_kernel_rootthree_preroll_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/195Py_kernel_rootthree_preroll_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/195Py_kernel_rootthree_preroll_futuremask_nosibling_maxd14_bundle_20260630.tar)

195Pyは194Pyを親にしています。194の `root one/two-candidate preroll`、188の `future_check_mask guard`、184の `no-sibling spill elision` は維持し、MAXD14 kernelだけ **root one/two/three-candidate preroll** へ拡張しました。

```text
194Py:
  root_avail が1bitまたは2bitの場合だけ、
  depth 0の先頭候補をDFS loop前に1段preroll

195Py:
  root_avail が1bit/2bit/3bitの場合、
  depth 0の先頭候補をDFS loop前に1段preroll

root_avail == 1bit / 2bit:
  194Py相当

root_avail == 3bit:
  先頭候補をpreroll
  first childがdescendする場合だけ、
  残り2候補を持つroot frameをsave_sp/cur_depth規則で保存

root_avail >= 4bit:
  194Py generic pathのまま
```
...
source_split_tag split195 only             OK
abstract root<=3 preroll equivalence       200000 cases OK
```

こちらではCodon/CUDA実機build/full実行はしていません。195Pyの採用可否は、添付の `195Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に194比と192/188比のtimingを見て判断してください。


"""

# 195 root<=3 preroll revision memo over validated 194 root-two futuremask/no-sibling MAXD14:
#   194 was correct and numerically faster than 192/188, so this branch adopts
#   the validated 194 root one/two-candidate preroll, 188 future_check_mask guard,
#   and 184 no-sibling tailcall build.
...
#   only restorable ancestor frames.  195 extends the narrow root fast-start: when the
#   post-root-action root availability has one, two, or three bits, process the first
#   depth-0 candidate before entering the generic DFS loop.  In the three-candidate
#   case the remaining two root siblings are saved only if the first child descends;
#   roots with four or more candidates preserve the exact 194 generic loop.  The
#   no-sibling rule and depth packing in avail[] bits 27..31 are unchanged, so MAXD14 remains four
...
# 195 keeps the validated 194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 195 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="195 rootthree-preroll: 194 root one/two-candidate futuremask/no-sibling MAXD14 baseline plus root three-candidate fast-start - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """195 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two/three-candidate preroll."""
    ...
    # 195 keeps the validated 194 current-frame register cache, 13-slot
    # ancestor storage, no-sibling tail-call descent, future-prune guard, and
    # root one/two/three-candidate preroll. It extends only the root fast-start: when the root
    # active frame has one, two, or three available bits after root-action handling,
    ...
    # candidates are left as the active root frame. Roots with four or more
    ...
    # 195 root_three_preroll:
    # If the root frame has one, two, or three candidates, process the first
    # root candidate before the generic DFS loop.  The one/two-candidate cases
    # are the validated 194 behavior.  The three-candidate case preserves the
    # two remaining root siblings by saving the root frame with its remaining
    # cur_avail when the first child descends.  If the first candidate dies or
    # terminates, the remaining root candidates stay active at depth 0 and the
    # generic loop handles them normally.
    ...
    if root_rest==u32(0):
      ...
      root_rest_tail:u32=root_rest&(root_rest-u32(1))
      if (root_rest_tail&(root_rest_tail-u32(1)))==u32(0):
        root_preroll=u32(1)
...
CHUNKSHAPE148_DEFAULT_REASON:str="195 rootthree-preroll reuses validated 194 scorestripe_v9 task order/cache and changes only MAXD14 root one/two/three-candidate fast-start"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split195_rootthree_preroll_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split195_rootthree_preroll_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split195-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split195-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split195_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split195-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split195-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split195_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
