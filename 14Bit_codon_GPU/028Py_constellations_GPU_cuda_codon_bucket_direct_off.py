...
""" CUDA GPU 用 DFS カーネル関数: 入力配列を base offset 付きで読む版 """
@gpu.kernel
def kernel_dfs_iter_gpu_off(
    ld_arr,rd_arr,col_arr,row_arr,free_arr,
    jmark_arr,end_arr,mark1_arr,mark2_arr,
    funcid_arr,w_arr,
    meta_next:Ptr[u8],
    results,
    base:int,m:int,board_mask:int,
    n3:int,n4:int,
):
    """
    機能:
    ...
    mark2=mark2_arr[src]
    sp=0
    fid[0]=funcid_arr[src]
    ld[0]=ld_arr[src]
    rd[0]=rd_arr[src]
    col[0]=col_arr[src]
    row[0]=row_arr[src]

    free0=free_arr[src]&board_mask
    if free0==0:
      results[i]=u64(0)
      return
    avail[0]=free0
...
def exec_solutions(
  ...
  if use_gpu:
    ...
    # 28: direct offset kernel では gpu_soa/w_task/owner_batch の staging buffer は不要。
    ...
    print(f"[gpu-config] N={N} bucketized=1 par_expand=1 direct_off=1 split_row={GPU_SPLIT_ROW} bucket_mode={GPU_BUCKET_MODE} block={BLOCK} owner_blocks={MAX_BLOCKS} owner_steps={STEPS} task_blocks={GPU_TASK_BLOCKS} task_steps={GPU_TASK_STEPS} log_level={GPU_LOG_LEVEL}")
    ...
    while off < m_all:
      ...
      for bucket_id in range(GPU_BUCKETS):
        ...
        while pos<bucket_m:
          ...
          src_base:int=bucket_base+pos

          GRID = (cm + BLOCK - 1) // BLOCK
          # 28: bucket内は expanded 上で連続配置済みなので、gpu_soa へ詰め替えず
          #     offset付きkernelで expanded[src_base + thread] を直接読む。
          kernel_dfs_iter_gpu_off(
            gpu.raw(expanded.ld_arr), gpu.raw(expanded.rd_arr), gpu.raw(expanded.col_arr),
            gpu.raw(expanded.row_arr), gpu.raw(expanded.free_arr),
            gpu.raw(expanded.jmark_arr), gpu.raw(expanded.end_arr),
            gpu.raw(expanded.mark1_arr), gpu.raw(expanded.mark2_arr),
            gpu.raw(expanded.funcid_arr), gpu.raw(expanded.w_arr),
            gpu.raw(meta_next),
            gpu.raw(results),
            src_base, cm, board_mask,
            n3, n4,
            grid=GRID, block=BLOCK
          )

          # 同一 owner に複数 task が戻るため、atomic はまだ使わずCPU側で安全に加算する。
          # owner_batch への詰め替えも削除し、expanded.owner_arr を offset で直接参照する。
          for ii in range(cm):
            results_all[expanded.owner_arr[src_base+ii]]+=results[ii]
          pos+=cm
