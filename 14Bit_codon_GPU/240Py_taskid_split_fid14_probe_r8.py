...
# =============================================================================
# NQ_UPDATE_MEMO
# 240 r8 taskid-split-fid14 buildfix: 239Py n27coretrim-keepfeaturesを親に、
#      cleanupで維持した汎用本線を保ったまま、split145実行時だけ
#      fid=14(SQd2B/base14) と rest を別launchへ分ける第一段probe。
#      CUDA MAXD14/16/18/20/21 kernel本文はまだ変更せず、まずは
#      task/id splitのlaunch overhead、load balance、dispatch影響を測る。
#      mode28/29/30/31、cache生成、chunkshape148、broadmarktail、worker split、
#      fallback kernels、-c/-g range、GPU N=5..27 は保持する。
...
"""
240 taskid-split-fid14 r8 r8 note: 239のkeepfeatures本線を親に、split145実行時だけfid14とrestを別launchへ分けるprobe。CUDA kernel本文は変更せず、まずlaunch分割の損益を測る。

作成しました。239は **238親の cleanup-only** です。
CUDA kernel本文、MAXD fallback、cache生成、mode28/29/30/31、worker split は触らず、CPU側の未使用再帰 `dfs()` fallback と到達不能分岐だけを削っています。

こちらで `STATIC_ONLY=1` は通しました。Codon full run はこの環境では実行していません。

```text
238 source : 5674行 / 196443 bytes / 132 defs
239 source : 5436行 / 189259 bytes / 131 defs
...
* [239Py_n27coretrim_keepfeatures_probe.py](sandbox:/mnt/data/239Py_n27coretrim_keepfeatures_probe.py)
* [239Py_n27coretrim_keepfeatures_validate_N21_full_once.sh](sandbox:/mnt/data/239Py_n27coretrim_keepfeatures_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

念のため、sha256とbundleも置いてあります。

* [239Py_n27coretrim_keepfeatures_sha256.txt](sandbox:/mnt/data/239Py_n27coretrim_keepfeatures_sha256.txt)
* [239Py_n27coretrim_keepfeatures_bundle_20260707.tar](sandbox:/mnt/data/239Py_n27coretrim_keepfeatures_bundle_20260707.tar)
...
VERSION_TAG:str="240 taskid-split-fid14 r8 r8 buildfix: 239 generic N5..27; bare -g split145 mode31; split145 chunks launch fid14 and rest separately; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved; CUDA kernel bodies unchanged"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 240: bare -g stays split145 mode31; split145 uses fid14 launch-split probe
...
# 240: keep runtime globals required by broadmarktail/chunkshape148/split145
...
# 239/240: recursive CPU dfs fallback removed. CPU path uses dfs_iter only.
...
# 240: these helpers are used by the kept broadmarktail/split145
...
CHUNKSHAPE148_DEFAULT_REASON:str="240 taskid-split-fid14 r8 r8 keeps validated 238/231/217 scorestripe_v9 task order/cache; CUDA kernel bodies unchanged"
...
def exec_solutions_gpu_chunk_split145(
  ...
  # 240 r8 buildfix: split145 dispatch uses only rest/d2/gen-specific
  # maxd locals. No stale single-dispatch reqmaxd/selmaxd names
  # remain inside this function.

  if m>0:
    if split_mode==14 and d2base14_m>0 and d2base14_m<m:
      rest_m:int=m-d2base14_m
      if gpu_log_level>=1:
        print(f"[split145-buckets] N={N} m={m} generic={rest_m} d2base14={d2base14_m} d0={d0_m} rest={rest_m} split_mode={split_mode} specialized=1 split=fid14_launch")
      d2_soa:TaskSoA=TaskSoA(STEPS)
      rest_soa:TaskSoA=TaskSoA(STEPS)
      d2_w_arr:List[u64]=[u64(0)]*STEPS
      rest_w_arr:List[u64]=[u64(0)]*STEPS
      d2_results:List[u64]=[u64(0)]*STEPS
      rest_results:List[u64]=[u64(0)]*STEPS
      d2_i:int=0
      rest_i:int=0
      i=0
      while i<m:
        if soa.funcid_arr[i]==14:
          d2_soa.ld_arr[d2_i]=soa.ld_arr[i]
          d2_soa.rd_arr[d2_i]=soa.rd_arr[i]
          d2_soa.col_arr[d2_i]=soa.col_arr[i]
          d2_soa.ctrl0_arr[d2_i]=soa.ctrl0_arr[i]
          d2_soa.free_arr[d2_i]=soa.free_arr[i]
          d2_soa.markctrl_arr[d2_i]=soa.markctrl_arr[i]
          d2_w_arr[d2_i]=w_arr[i]
          d2_i+=1
        else:
          rest_soa.ld_arr[rest_i]=soa.ld_arr[i]
          rest_soa.rd_arr[rest_i]=soa.rd_arr[i]
          rest_soa.col_arr[rest_i]=soa.col_arr[i]
          rest_soa.ctrl0_arr[rest_i]=soa.ctrl0_arr[i]
          rest_soa.free_arr[rest_i]=soa.free_arr[i]
          rest_soa.markctrl_arr[rest_i]=soa.markctrl_arr[i]
          rest_w_arr[rest_i]=w_arr[i]
          rest_i+=1
        i+=1

      if rest_i>0:
        rest_GRID:int=(rest_i+BLOCK-1)//BLOCK
        rmaxd:int=max_schedule_depth_of_tasks(rest_soa,rest_i,meta_next)
        rselmaxd:int=select_static_maxd(rmaxd)
        if gpu_log_level>=1:
          print("[maxd-dispatch] N="+str(N)+" scope=split145-rest m="+str(rest_i)+" reqmaxd="+str(rmaxd)+" selMAXD="+str(rselmaxd)+" schedule_words="+str(packed_schedule_words_for_maxd(rselmaxd))+" stack_bytes_per_thread="+str(packed_stack_bytes_per_thread(rselmaxd))+" capacity_check=OK")
        if not launch_kernel_dfs_iter_gpu_static_maxd(rselmaxd,rest_soa,rest_w_arr,meta_next,rest_results,rest_i,board_mask_gpu,n3_gpu,n4_gpu,rest_GRID,BLOCK):
          print("[maxd-error] unsupported reqmaxd="+str(rmaxd)+"; supported maximum is 21")
          error_stages:List[int]=[0]*10
          return 0,stats,error_stages,"0:00:00.000",0
        i=0
        while i<rest_i:
          chunk_total+=int(rest_results[i])
          i+=1

      if d2_i>0:
        d2_GRID:int=(d2_i+BLOCK-1)//BLOCK
        d2maxd:int=max_schedule_depth_of_tasks(d2_soa,d2_i,meta_next)
        d2selmaxd:int=select_static_maxd(d2maxd)
        if gpu_log_level>=1:
          print("[maxd-dispatch] N="+str(N)+" scope=split145-fid14 m="+str(d2_i)+" reqmaxd="+str(d2maxd)+" selMAXD="+str(d2selmaxd)+" schedule_words="+str(packed_schedule_words_for_maxd(d2selmaxd))+" stack_bytes_per_thread="+str(packed_stack_bytes_per_thread(d2selmaxd))+" capacity_check=OK")
        if not launch_kernel_dfs_iter_gpu_static_maxd(d2selmaxd,d2_soa,d2_w_arr,meta_next,d2_results,d2_i,board_mask_gpu,n3_gpu,n4_gpu,d2_GRID,BLOCK):
          print("[maxd-error] unsupported reqmaxd="+str(d2maxd)+"; supported maximum is 21")
          error_stages=[0]*10
          return 0,stats,error_stages,"0:00:00.000",0
        i=0
        while i<d2_i:
          chunk_total+=int(d2_results[i])
          i+=1
    else:
      if gpu_log_level>=1:
        print(f"[split145-buckets] N={N} m={m} generic={m} d2base14={d2base14_m} d0={d0_m} rest=0 split_mode={split_mode} specialized=0")
      gen_GRID:int=(m+BLOCK-1)//BLOCK
      gmaxd:int=max_schedule_depth_of_tasks(soa,m,meta_next)
      gselmaxd:int=select_static_maxd(gmaxd)
      if gpu_log_level>=1:
        print("[maxd-dispatch] N="+str(N)+" scope=split145 m="+str(m)+" reqmaxd="+str(gmaxd)+" selMAXD="+str(gselmaxd)+" schedule_words="+str(packed_schedule_words_for_maxd(gselmaxd))+" stack_bytes_per_thread="+str(packed_stack_bytes_per_thread(gselmaxd))+" capacity_check=OK")
      if not launch_kernel_dfs_iter_gpu_static_maxd(gselmaxd,soa,w_arr,meta_next,results,m,board_mask_gpu,n3_gpu,n4_gpu,gen_GRID,BLOCK):
        print("[maxd-error] unsupported reqmaxd="+str(gmaxd)+"; supported maximum is 21")
        error_stages:List[int]=[0]*10
        return 0,stats,error_stages,"0:00:00.000",0
      i=0
      while i<m:
...
def main()->None:
  ...
  if argc == 1:
    ...
    print("[240-default] no arguments: CPU N22 default; use -g for A10G range mode")
  elif argc >= 2:
    arg=sys.argv[1]
    if arg == "-c":
      use_gpu=False
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 240 taskid-split-fid14 r8 r8; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split240_keepfeatures_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split240_keepfeatures_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split240-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split240-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split240_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      chunk_only:bool=(bench_mode==30)
      total:int=exec_solutions_gpu_bin_stream_split145(N,shaped_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,chunk_only,debug_chunk_start,debug_chunk_count,split_probe_chunk_list_spec if chunk_only else "",progress_suffix,worker_id if bench_mode==31 else 0,worker_count if bench_mode==31 else 1,14)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if bench_mode==30:
