...
# =============================================================================
# NQ_PY_UPDATE_HISTORY
# 181: 更新メモ: 180の高速・正当性OKを受け、kernelは180/175相当のまま、更新履歴ヘッダーを正式導入。sh側にtail平坦性メトリクスを追加。
# =============================================================================
...
180Pyログ確認しました。**問題ありません。高速です。**

```text
180Py : 0:07:51.191
175Py : 0:07:51.106
差分  : 180Pyが +0.085秒、約0.018%だけ遅い
```
...
```text
chunk elapsed sum : 470236 ms
min               : 3367 ms @ chunk99
max               : 3965 ms @ chunk13
p50 / p90 / p95   : 3578 / 3732 / 3807 ms
top5 share        : 41/1000
top10 share       : 81/1000
slow top chunks   : 13,48,37,38,1,29,3,46,128,94,...
```

top chunk群の dominant funcid はほぼ `fid=5` ですが、top5でも全体の約4.1%、top10でも約8.1%なので、現状は「巨大な単一tail island」ではなく、かなり平坦に分散できています。したがって181Pyでは危険なkernel変更には進まず、まずご提案どおり **更新履歴ヘッダーを正式導入**しました。

作成したファイルはこちらです。

* [NQ_Py_update_history_020_to_181_20260629.txt](sandbox:/mnt/data/NQ_Py_update_history_020_to_181_20260629.txt)
* [181Py_kernel_history_tailflat_nibble_maxd14_probe.py](sandbox:/mnt/data/181Py_kernel_history_tailflat_nibble_maxd14_probe.py)
* [181Py_kernel_history_tailflat_nibble_maxd14_validate_full_once.sh](sandbox:/mnt/data/181Py_kernel_history_tailflat_nibble_maxd14_validate_full_once.sh)

181Pyの位置づけです。

```text
親:
  180Py / 175Py相当の validated nibble MAXD14 kernel

GPU kernel:
  変更なし
  current frame register 維持
  ancestor13 stack 維持
  4-bit nibble schedule 維持
  schedule_words = 0
  stack_bytes_per_thread = 208

181Pyで追加:
  .py ヘッダーに NQ_PY_UPDATE_HISTORY を追加
  20Py付近〜181Pyまで、可能な範囲で更新内容を一行ずつ列挙
  181行に「更新メモ」を追加

181Py .shで追加:
  NQ_SH_UPDATE_HISTORY を追加
  source側の更新履歴ヘッダー存在確認
  tail_range_ms
  tail_p95_over_p50_permille
```

今回の更新履歴では、20Py〜47Py付近については現在手元に残っている資料だけでは詳細を断定できないため、基礎整備期として範囲記載にしています。49Py以降は、ソース内コメントとこれまでの実行ログから安全に復元できる範囲で整理しました。

直近の履歴抜粋です。

```text
168: packed byte opcode schedule。MAXD16 local storage 320→272 bytes/thread。
169: MAXD14静的kernel追加。240 bytes/thread化したが168より遅い。
170: schedule配列廃止。u64版はPTX JIT失敗、u32-pair版で224 bytes/thread化。
171: current frame register保持。大幅高速化。
172: ancestor stack 13段化。208 bytes/thread。
173: frame entry metadata decode。register圧増大で不採用。
174: compact_op fast path。正当性OKだが遅く不採用。
175: 4-bit nibble schedule。7:51.106で新最速基準。
176: current nibble register。遅く不採用。
177: terminal-first。遅く不採用。
178: host-precomputed / block-switch。CUDA_ERROR_INVALID_PTXで不採用。
179: block-switch no-or。CUDA_ERROR_INVALID_PTXで不採用。
180: 175相当kernelへ安全復帰 + tail diagnostics。7:51.191で175同等。
181: 更新履歴ヘッダー正式導入 + tail平坦性メトリクス追加。kernelは180/175相当。
```

こちらで `STATIC_ONLY=1` の静的・モデル検証は通しています。

```text
source_update_history_header             OK
source_update_history_181                OK
kernel_variant_count                     5 / 5 OK
maxd14_nibble_schedule                   OK
frame_opcode_equivalence                 506604 cases OK
dfs_164_vs_181_nibble_equivalence        12000 cases OK
...
```bash
chmod +x 181Py_kernel_history_tailflat_nibble_maxd14_validate_full_once.sh
./181Py_kernel_history_tailflat_nibble_maxd14_validate_full_once.sh
```

181Pyはkernel本体を変えていないので、速度は180Py/175Py近辺になる想定です。今回から、以後の `182Py` 以降もヘッダーに **「更新メモ」一行追記** の形で残していきます。



"""

# 181 update-history + tail-flatness rebound over validated 175/180 nibble-schedule MAXD14:
...
# 181 keeps the validated 175/180 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 181 MAXD14 uses scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="181 update-history tailflat: validated 175/180 nibble MAXD14 kernel unchanged - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 181 inherits the exact 172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """181 MAXD14 nibble schedule plus register-held current frame and 13-slot ancestor stack."""
    ...
    # 181 keeps the validated 172 current-frame register cache plus ancestor stack shrink.  The active
...
####################################################################################################
# 181 nibble-schedule MAXD14 + scalar-u32pair dispatch harness
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="181 history-tailflat reuses validated 175/180 nibble MAXD14 kernel and scorestripe_v9 task order/cache"
...
def main()->None:
  ...
  # 181 branches from validated 180/175.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 181 history-header tailflat GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split181_history_tailflat_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split181_history_tailflat_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split181-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split181-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split181_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split181-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split181-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split181_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
