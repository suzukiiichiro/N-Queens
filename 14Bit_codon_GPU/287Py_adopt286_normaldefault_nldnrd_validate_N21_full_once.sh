...
# =============================================================================
# 287 N=21 full validation harness
# Parent: 286 generic-normaldefault-nldnrd. Experiment: adoption/fixed baseline, CUDA kernel logic unchanged from 286.
...
SRC=${SRC:-./287Py_adopt286_normaldefault_nldnrd_probe.py}
CAND=${CAND:-./287Py_adopt286_normaldefault_nldnrd_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/287Py_adopt286_normaldefault_nldnrd_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/287Py_adopt286_normaldefault_nldnrd_logs_N21_full_once_${TS}"
...
echo "[start] 287 adopt286-normaldefault-nldnrd validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 287 adopt286-normaldefault-nldnrd SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '287 adopt286-normaldefault-nldnrd' "$SRC"; then
  printf 'source_version_tag	287 adopt286-normaldefault-nldnrd	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	287 adopt286-normaldefault-nldnrd	missing	FAIL
...
old_split_markers = ['split286', 'split285', 'split284', 'split282', 'split281', 'split280', 'split278', 'split277', 'split275', 'split273', 'split270_', 'split240', 'split239_', 'kernel-blockdiag', 'block_count_diag', 'rootaction0-direct', 'ncol_early']
old_present = [x for x in old_split_markers if x in s]
if 'split287' in s and not old_present:
    checks.append(('source_split_tag', 'split287 active; rejected runtime tags absent', 'split287', True))
else:
    checks.append(('source_split_tag', 'split287 active; rejected runtime tags absent', 'present=' + ','.join(old_present) if old_present else 'split287 missing', False))
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 287 adopt286-normaldefault-nldnrd mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "286adopt:$BASELINE_286_ADOPT_SECONDS" "285restore:$BASELINE_285_RESTORE_SECONDS" "284rejected:$BASELINE_284_REJECT_SECONDS" "283normalfirst:$BASELINE_283_NORMALFIRST_SECONDS" "276current:427.672" "271fastest:427.705" "239:427.703" "217:427.709"; do
...
echo "[validation-ok] 287 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
