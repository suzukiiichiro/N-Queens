...
# =============================================================================
# NQ_UPDATE_MEMO
# 214: 更新メモ: 213Py rootfirstlate inlinexor は正当性OKだが204最速を上回らなかったため不採用。214Pyは211Pyのlate-root-first形を親に、MAXD14 kernelのみroot_firstを cur_avail ^ root_rest で復元する微差実験を検証する。
...
# progress TSV and verifies the complete total. Candidate 214 returns to
...
#   - rootfirstxor source check is whitespace/comment robust.
SRC=${SRC:-./214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 214 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/214Py_kernel_roottwo_rootfirstxor_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 214Py source
#   has a harmless header/string/layout difference. This 214 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second XOR scalar if + late root_first xor restoration	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
rf_pos=root_compact.find('root_first:u32=cur_avail^root_rest')
...
checks = {
    ...
    'root_first_xor_late_present': rf_pos > if_pos >= 0,
    'root_first_lowbit_absent': 'root_first:u32=cur_avail&(u32(0)-cur_avail)' not in root_compact,
    ...
    # 214 keeps 204/197 save payload; 203 depth0-save is not adopted.
    ...
    # 214 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 214 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 214 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_rootfirstxor_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootfirstxor\tpresent_late_root_first_xor\tpresent_late_root_first_xor\tOK
' >> "$SUMMARY"
else
  printf 'source_rootfirstxor\tpresent_late_root_first_xor\tmissing_or_old_rootfirstxor_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split214'}
bad=sorted(tags-allowed)
if 'split214' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split214 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split214 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 214 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 214 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootfirstxor static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 210 roottwo-rootfirstxor futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 210 roottwo-rootfirstxor futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
