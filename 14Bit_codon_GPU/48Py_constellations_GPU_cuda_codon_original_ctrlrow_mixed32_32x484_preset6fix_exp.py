...
Python/codon Ｎクイーン コンステレーション版 CUDA 高速ソルバ 48 trunk候補 32x484 preset6-fix 実験版（ctrlrow mixed32 tuned）
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1)->None:
  ...
  if use_gpu:
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk48={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
...
def set_pre_queens(N:int,ijkl_list:Set[int],subconst_cache:Set[Tuple[int,int,int,int,int,int,int,int,int,int,int]],ld:int,rd:int,col:int,k:int,l:int,row:int,queens:int,LD:int,RD:int,counter:list,constellations:List[Dict[str,int]],preset_queens:int,visited:Set[int],constellation_signatures:Set[Tuple[int,int,int,int,int,int]],zobrist_hash_tables: Dict[int, Dict[str, List[u64]]])->Tuple[Set[int], Set[Tuple[int,int,int,int,int,int,int,int,int,int,int]], List[Dict[str,int]], int]:
  ...
  if queens == preset_queens:
    # preset_queens=6 でも同一 sc 内の完全同一状態は同じ未来を持つため dedup する。
    # 46 では <=5 のみ dedup していたが、6 診断で合計不一致が出たため実験的に一般化。
...
def gen_constellations(
  ...
  for sc in ijkl_list:
    ...
    # 46 の preset_queens=6 では subconst_cache が sc をまたいで効き、
    # base(i,j,k,l) が異なるタスクまで落とす可能性があるため、48 実験版では sc ごとに clear する。
    subconst_cache.clear()
...
def main()->None:
  ...
  if argc == 1:
    ...
    if arg == "-c":
      ...
      print("Usage: nqueens [-c | -g] [nmin nmax] [gpu_block gpu_max_blocks log_level sort_mode preset_queens]")
      return
    ...
    if argc >= 9:
      preset_queens_arg=int(sys.argv[8])
    if argc > 9:
