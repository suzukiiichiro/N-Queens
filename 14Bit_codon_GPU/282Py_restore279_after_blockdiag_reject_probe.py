...
# =============================================================================
# NQ_UPDATE_MEMO
# 282: restore279-after-blockdiag-reject。281 kernel block count診断版は
#      N=21 full onceで正当性OK、診断値も取得できたが、診断配列書き込みにより
#      速度候補ではないため採用せず撤回する。
#      279/276 restore系の本線へ戻す安全復帰版。CUDA MAXD14/16/18/20/21
#      kernel本文、rootrestlate、futuremask、no-sibling、split145/chunkshape148、
#      broadmarktail、cache生成、worker split、mode28/29/30/31、MAXD fallback、
#      CPU dfs_iter pathは279/276相当で維持する。
#      281/280 block count診断、278 zero-const、277 depth_u、275 bucket診断、
#      273 root0 direct kernel、270 schedule precompute、240 fid14/rest別launch は入れない。
...
VERSION_TAG:str="282 restore279-after-blockdiag-reject: 281/280 kernel block count diagnostics rejected as diagnostic-only; restored 279/276/274/271 final-fastest-complete baseline; generic N5..27; bare -g split145 mode31; unused recursive CPU dfs fallback remains removed; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved; CUDA kernels unchanged"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split282_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split282_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split282-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split282-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split282_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
