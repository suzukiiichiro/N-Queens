...
codon build -release \
112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune.py


# 112Py 変更点
# - GPU kernel / DFS / broadmarktail reorder の分類ロジックは 111Py から変更なし
# - mode28/mode29 の reordered bin 名に b{block}_m{max_blocks}_s{steps} を追加
# - mode29 の progress TSV 名にも b{block}_m{max_blocks}_s{steps} を追加
# - 32x484 と 32x968 の実行結果・TSV・bin が衝突しないようにする計測整理版

# GPUシングル実行
./112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune \
...
CUDA_VISIBLE_DEVICES=0 ./112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune \
  -g 22 22 32 484 1 0 7 29 8 7 0 0 4 \
  2>&1 | tee 111Py_N22_worker0of4.log &

CUDA_VISIBLE_DEVICES=1 ./112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune \
  -g 22 22 32 484 1 0 7 29 8 7 0 1 4 \
  2>&1 | tee 111Py_N22_worker1of4.log &

CUDA_VISIBLE_DEVICES=2 ./112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune \
  -g 22 22 32 484 1 0 7 29 8 7 0 2 4 \
  2>&1 | tee 111Py_N22_worker2of4.log &

CUDA_VISIBLE_DEVICES=3 ./112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune \
...
[112Py ソースコードをダウンロード](sandbox:/mnt/data/112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune.py)

今回の 112Py は **111Py の kernel / DFS / broadmarktail 分類ロジックは変更なし**です。変更点は計測整理に限定しています。

```text
112Py の変更点

1. VERSION_TAG を 112 single-GPU measurement-safe に変更
2. mode28/mode29 の broadmarktail reordered bin 名に chunk tag を追加
   例:
   constellations_N22_7_broadmarktail_reorder_v2_w8_j7_b32_m484_s15488.bin

3. mode29 の progress TSV 名にも chunk tag を追加
   例:
   progress_N22_7_stream_funcid_reorder_v2_w8_j7_b32_m484_s15488_broadmarktail_gpu.tsv

4. 32x484 と 32x968 の bin / progress TSV が衝突しないように修正
```
...
./112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune \
  -g 22 22 32 484 1 0 7 29 \
  2>&1 | tee 112Py_N22_single_32x484.log
```

比較用に 32x968 を再確認する場合も、今度はファイル名が衝突しません。

```bash
./112Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_single_tune \
  -g 22 22 32 968 1 0 7 29 \
  2>&1 | tee 112Py_N22_single_32x968.log
```

ただし、前回ログから判断すると、現時点の本命は **32x484 固定**です。
...
VERSION_TAG:str="112 single-GPU measurement-safe broadmark funcid17/G_H weak tertiary smoothing auto w8_j7 stream GPU runner from 111"
