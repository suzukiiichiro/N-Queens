...
# =============================================================================
# NQ_UPDATE_MEMO
# 280: kernel-block-count diagnostic。279/276本線を親に、速度候補ではなく
...
今回は **279/276本線を親にした kernel block count 診断版**です。速度候補ではなく、MAXD14 generic DFS loop内の通過回数をchunkごとに集計します。`STATIC_ONLY=1` は通っています。

ファイルはこちらです。

* [280Py_kernel_block_count_diag_probe.py](sandbox:/mnt/data/280Py_kernel_block_count_diag_probe.py)
* [280Py_kernel_block_count_diag_validate_N21_full_once.sh](sandbox:/mnt/data/280Py_kernel_block_count_diag_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [280Py_kernel_block_count_diag_sha256.txt](sandbox:/mnt/data/280Py_kernel_block_count_diag_sha256.txt)
* [280Py_kernel_block_count_diag_bundle_20260709.tar](sandbox:/mnt/data/280Py_kernel_block_count_diag_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./280Py_kernel_block_count_diag_probe

STATIC_ONLY=1 bash 280Py_kernel_block_count_diag_validate_N21_full_once.sh

bash 280Py_kernel_block_count_diag_validate_N21_full_once.sh
```
...
```bash
grep -E "kernel-blockdiag|validation-ok|^21:" \
  280Py_kernel_block_count_diag_logs_N21_full_once_*/full_once.log
```

280は診断オーバーヘッドがあるため、速度ではなく、`normal/special`、`future_check/future_kill`、`save/restore/descend` の比率を見て281以降の候補を決める用途です。
...
VERSION_TAG:str="280 kernel-block-count-diagnostic: parent 279/276 baseline; adds MAXD14 generic DFS block counters into separate diagnostic arrays and progress columns; calculation, task order, one-launch-per-chunk split145, rootrestlate, futuremask, no-sibling, cache generation, MAXD fallbacks, worker split preserved; 278 zero const, 277 depth_u, 275 bucket diag, 273 root0 direct, 270 precompute, 240 fid14 launch split rejected"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split280_blockdiag_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split280_blockdiag_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split280-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split280-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split280_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
