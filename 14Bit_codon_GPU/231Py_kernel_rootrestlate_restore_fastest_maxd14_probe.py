...
# =============================================================================
# NQ_UPDATE_MEMO
# 231: 更新メモ: 230Py root-preroll placed_ld/placed_rd 共通化は正当性OKだが大きく退行したため不採用。231Pyは添付段取りどおり217Py rootrestlate最速基準相当へ復帰する。MAXD14 kernelの探索ロジックは217相当へ戻し、230 rootpr place-or と225 generic ncol-early を撤回する。futuremask、no-sibling、root one/two preroll、host task order、dispatch、stack 208 bytes/thread は維持する。
...
"""
230Pyログ確認メモ:
  final total 314666222712、progress rows 131、dispatch task sum 2025282、
  required/selected MAXD は全chunk 14、schedule_words=0、stack=208、
  warning/error=0。正当性はOKだが N=21 full 0:07:08.848 で、
  217Py 0:07:07.709 から大きく退行したため不採用。

231Pyは 230Py の root-preroll place-or と 225Py の generic ncol-earlyを撤回し、
217Py rootrestlate / futuremask / no-sibling / root one-two preroll の最速基準へ戻す。
"""
"""
作成しました。添付メモでは、230Pyは正当性OKながら `0:07:08.848` で大きく退行し不採用、次は217Py基準へ戻す方針になっていました。 またREADME上でも217Pyは `0:07:07.709` の新最速基準として記録されています。

ご入力は「213Py」に見えましたが、段取りメモと連番から **231Py = 217Py rootrestlate restore版** として作成しています。213PyはREADME上では不採用側の履歴なので、最速復帰版の名前としては231Pyにしました。

`STATIC_ONLY=1` は実行済みで、以下を確認しています。

```text
source_version_tag        OK
source_future_check_mask  OK
source_nosibling_parent   OK
source_217_restore_shape  OK
source_split230_removed   OK
source_split_tag          split145 split231 OK
```

作成物はこちらです。

* [231Py_kernel_rootrestlate_restore_fastest_maxd14_probe.py](sandbox:/mnt/data/231Py_kernel_rootrestlate_restore_fastest_maxd14_probe.py)
* [231Py_kernel_rootrestlate_restore_fastest_maxd14_validate_full_once.sh](sandbox:/mnt/data/231Py_kernel_rootrestlate_restore_fastest_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/231Py_kernel_rootrestlate_restore_fastest_maxd14_bundle_20260706.tar)

"""
# 231 restore memo over validated 217 rootrestlate/futuremask/no-sibling MAXD14:
#   217 was the current fastest N=21 full baseline.  This branch backs out
#   the 224..230 rejected probes and keeps the validated root one/two-
#   candidate preroll, 217 rootrestlate placement of cur_avail=root_rest,
#   188 future_check_mask guard, and 184 no-sibling tailcall build.
...
#   loop.  The root<=2 predicate is the 217 second-lowbit scalar predicate,
#   and cur_avail=root_rest remains delayed until after the first-child checks.
...
VERSION_TAG:str="231 rootrestlate-restore: validated 217 fastest baseline restored; 224-230 rejected probes backed out - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # 231 rootrestlate restore:
    # 230 root-preroll place-or and 225 generic ncol-early are withdrawn.
    # Keep the validated 217 rootrestlate scalar predicate.  The root fast-start
    # scope remains only one/two-candidate roots; 3-bit and wider roots stay
    # generic.  If the first child dies or is terminal, the generic loop still
    # sees root_rest as the active root remainder.  If the first child descends,
    # the same root_rest is saved as a sibling.
    ...
    if root_after_second==u32(0):
      ...
      # 231 restore: use the 217/225 root-preroll block-first shape directly.
      # Do not keep the 230 placement-common scalars.
      pr_nld:u32=u32(0)
      pr_nrd:u32=u32(0)
...
CHUNKSHAPE148_DEFAULT_REASON:str="231 restore reuses validated 217 scorestripe_v9 task order/cache; MAXD14 scalar root_after_second predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split231_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split231_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split231-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split231-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split231_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split231-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split231-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split231_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
