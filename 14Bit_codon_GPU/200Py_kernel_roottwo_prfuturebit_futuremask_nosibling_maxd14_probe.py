...
# =============================================================================
# NQ_UPDATE_MEMO
# 200: 更新メモ: 199は正当性OKだが197比0.091秒遅いため不採用。197 root<=2 direct-if preroll/future_check_mask guard/no-siblingを親に戻し、MAXD14 kernelのみroot preroll内のfuture-prune判定から冗長なfuture_check_mask外側guardを外し、pr_nibble_op bit8を直接見る微差実験を導入。
...
200Py note:
  199Py was correct but did not beat the 197 direct-if baseline.  This branch
  returns to 197 as the numerical baseline.  It keeps the same validated
  root fast-start scope: only post-root-action root_avail with one or two bits
  is prerolled, and 3-bit or wider roots stay on the generic loop.  The only
  new micro-experiment is inside the root-preroll body: for the prerolled
  depth-0 candidate, test pr_nibble_op bit8 directly instead of first checking
  the aggregate future_check_mask.  The generic DFS loop keeps the validated
...
"""
199Pyログを確認しました。**正当性・実行結果は問題ありません。**
...
```text
199Py  0:07:08.293
197Py  0:07:08.202  → 199が 0.091秒 / 0.021% 遅い
198Py  0:07:08.239  → 199が 0.054秒 / 0.013% 遅い
196Py  0:07:08.239  → 199が 0.054秒 / 0.013% 遅い
194Py  0:07:08.288  → 199が 0.005秒 / 0.001% 遅い
195Py  0:07:08.738  → 199が 0.445秒 / 0.104% 高速
188Py  0:07:10.137  → 199が 1.844秒 / 0.429% 高速
```

そのため、**199Pyは正当性OKだが197Pyを上回らないため不採用**としました。
新最速基準は引き続き **197Py** です。

200Pyは199を親にせず、197Pyへ戻しています。root fast-startの対象は197/196/194と同じく **root_avail 1bit/2bitのみ**です。195の3bit root preroll、198のtail-test、199の `root_rest==root_second` 判定は採用していません。

200Pyの差分はMAXD14 kernel内だけです。

```text
197Py:
  root-preroll内のfuture-prune判定

  if pr_descend != 0:
      if future_check_mask != 0:
          if pr_nibble_op & 8 != 0:
              future prune

200Py:
  root-preroll内だけ future_check_mask 外側guardを削除
...
意味は同じです。depth0の `pr_nibble_op` にfuture bit8が立つ場合、schedule生成上 `future_check_mask` は必ず非zeroです。
generic DFS loop側の `future_check_mask==0` schedule軽量guardは197/188のまま残しています。

手元確認結果です。

```text
bash -n 200Py...sh        OK
STATIC_ONLY=1 200Py...sh  OK

source_future_check_mask_guard   OK
source_nosibling_parent          OK
source_rootpr_futurebit          OK
source_195_rootthree_removed     OK
...
こちらではCodon/CUDA実機build/full実行はしていません。cudacodon側では、以下でN=21 fullのcases 01-06、dispatch、final total、warning/error、特に197/199/198/196比のtimingを見て採用可否を判断してください。

```bash
STATIC_ONLY=1 bash 200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_validate_full_once.sh
bash 200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物はこちらです。

* [200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_probe.py)
* [200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/200Py_kernel_roottwo_prfuturebit_futuremask_nosibling_maxd14_bundle_20260701.tar)

"""


# 200 root-preroll future-bit memo over validated 197 root-two futuremask/no-sibling MAXD14:
#   199 and 198 were both correct but slower than 197, so this branch returns
#   to 197 as the numerical baseline.  It keeps the validated root one/two-
#   candidate direct-if preroll, 188 future_check_mask guard, and 184 no-sibling
#   tailcall build.
...
#   only restorable ancestor frames.  200 keeps the narrow root fast-start only
#   when post-root-action root availability has one or two bits, matching the
#   validated 197/196/194 behavior.  Three-candidate roots explicitly stay on
#   the generic loop.  The 197 second-lowbit direct-if predicate is restored.
#   In the root-preroll body only, the future-prune test skips the aggregate
#   future_check_mask guard and uses pr_nibble_op bit8 directly; if bit8 is set,
#   future_check_mask is necessarily nonzero for depth 0.  The generic loop
#   keeps the validated future_check_mask guard.
#   The no-sibling rule and depth packing in avail[] bits 27..31 are unchanged,
#   so MAXD14 remains four u32[13] arrays = 208 bytes/thread.  The supported
#   GPU range remains N=5..27.
...
# 200 keeps the validated 197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 200 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="200 roottwo-prfuturebit: 197 root<=2 direct-if futuremask/no-sibling MAXD14 baseline with root-preroll future-bit direct test - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """197 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate direct-if preroll predicate."""
    ...
    # 200 rootpr_futurebit:
    # If the root frame has one or two candidates, process the first root
    # candidate before the generic DFS loop, matching the validated 197/196/194
    # scope. 195's three-candidate case remains intentionally removed because it
    # was correct but slower. The predicate is the 197 second-lowbit direct-if
    # form. Only the root-preroll future-prune guard shape changes: depth-0
    # pr_nibble_op bit8 is tested directly, while the generic DFS loop keeps
    # the aggregate future_check_mask zero guard.
...
CHUNKSHAPE148_DEFAULT_REASON:str="200 roottwo-prfuturebit reuses validated 197/196/194 scorestripe_v9 task order/cache; only MAXD14 root-preroll future-bit guard shape changes"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split200_roottwo_prfuturebit_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split200_roottwo_prfuturebit_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split200-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split200-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split200_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split200-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split200-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split200_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
