...
# =============================================================================
# 313 N=21 full validation harness (clock-cap-diagnosis)
# Parent: 312 thermal-repro-check. Zero source changes from 311, yet
#   reproduced 311's elapsed almost exactly: 454.417s vs 311's 454.422s,
#   delta +0.005s (+0.001%). Such tight reproduction across independent runs
#   argues AGAINST transient thermal drift (which would show run-to-run
#   variance or gradual recovery) and FOR a stable, persistent state.
# GPU telemetry from 312 (gpu_telemetry.csv, 91 samples @ 5s over the
#   ~7.5min run) settled the question:
#     - temperature.gpu stayed 32C (pre-run idle) to 39C (peak) -- far below
#       the ~83-90C range where datacenter/workstation GPUs typically begin
#       thermal throttling.
#     - clocks.current.sm was pinned at exactly 1320 MHz for the ENTIRE
#       capture window, including the pre-run idle snapshot -- i.e. it never
#       boosted under 100% utilization AND never dropped under thermal load,
#       because it never moved at all.
#     - clocks.current.memory was likewise flat at 6251 MHz throughout.
#     - clocks_event_reasons.active was 0x0 (no active throttle reason bits)
#       for the entire compute portion of the run (only 0x1/Idle at the
#       very first idle sample).
#   Conclusion: this is NOT thermal throttling. A GPU that is cool and
#   reports no active throttle reasons but never varies its clock under load
#   is consistent with a persistent clock lock/cap (e.g. `nvidia-smi -lgc`
#   applied, or persistence-mode application clocks pinned below boost) that
#   most likely predates 309 and does not depend on this session's thermal
#   history -- which explains why 309/310/311/312 all cluster in the
#   ~450-481s range regardless of variant or re-run, while 304/308 (recorded
#   351.070s/351.675s in an earlier session) were faster.
# This revision: ZERO source changes from 312 (same as 311/312: variant=2,
#   K=48). The validate script instead extends the pre-run nvidia-smi
#   snapshot to also query clocks.max.sm / clocks.max.memory /
#   clocks.applications.sm / clocks.applications.memory, and prints an
#   explicit warning if the observed running clock is capped well below the
#   max supported clock -- this is the smoking-gun check for a clock lock.
# Remediation (NOT auto-executed by this script -- requires manual
#   confirmation, may need sudo, and could affect other jobs on shared
#   hardware): if a clock lock is confirmed, the standard reset commands are
#     nvidia-smi -rgc   # reset GPU clocks to default/auto-boost
#     nvidia-smi -rac   # reset application clocks to default
#   Re-run this validation after resetting clocks to see whether timing
#   recovers toward 304/308's ~351s range.
# Stall Branch Resolving ncu work (handoff priority #2) remains deferred
#   until the clock-cap question is resolved, since ncu profiling under a
#   locked/capped clock would not be comparable to 304's baseline profile.
...
#           to again reproduce ~454s under the current (still-locked, most
#           likely) clock state; the clock-cap diagnostic output is the
#           primary new information from this run, not the timing itself.
...
SRC=${SRC:-./313Py_clock_cap_diagnosis.py}
CAND=${CAND:-./313Py_clock_cap_diagnosis}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/313Py_clock_cap_diagnosis_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/313Py_clock_cap_diagnosis_logs_N21_full_once_${TS}"
...
echo "[start] 313 clock-cap-diagnosis validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 313 clock-cap-diagnosis SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '313 clock-cap-diagnosis' "$SRC"; then
  printf 'source_version_tag	313 clock-cap-diagnosis	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	313 clock-cap-diagnosis	missing	FAIL
...
TELEMETRY_PID=""
NVSMI_FIELDS="timestamp,temperature.gpu,clocks.current.sm,clocks.current.memory,clocks.max.sm,clocks.max.memory,clocks.applications.sm,clocks.applications.memory,power.draw,utilization.gpu,clocks_event_reasons.active"
if [[ "$CAPTURE_TELEMETRY" == "1" ]] && command -v nvidia-smi >/dev/null 2>&1; then
  ...
  # 313: clock-cap smoking-gun check. Compare the currently-reported SM clock
  # against the max supported SM clock; a large, persistent gap with the GPU
  # cool and idle is the signature of a clock lock/cap, not thermal throttling.
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 313 clock-cap-diagnosis mode31 split145"
...
echo "[validation-ok] 313 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); see gpu_clock_cap_check in summary.tsv and gpu_pre_run_snapshot.csv/gpu_telemetry.csv for clock-cap diagnosis"
