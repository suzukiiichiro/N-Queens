...
# =============================================================================
# NQ_UPDATE_MEMO
# 198: 更新メモ: 197は正当性OKかつ196比0.037秒高速で数値上の新最速基準。197のroot<=2 direct-if preroll/future_check_mask guard/no-siblingを親に、MAXD14 kernelのみroot<=2判定をsecond-lowbit直後判定からroot_rest&(root_rest-1)==0のtail-testへ置き換える微差実験を導入。
...
198Py note:
  197Py was correct and numerically faster than 196/194/195.  This branch keeps
  the same validated MAXD14 root fast-start scope: only post-root-action
  root_avail with one or two bits is prerolled, and 3-bit or wider roots stay on
  the generic loop.  The only new micro-experiment is the predicate shape: remove
  the second-lowbit helper scalar and guard the preroll body with the equivalent
  power-of-two tail test root_rest & (root_rest - 1) == 0.
...
"""
197Pyログを確認しました。**正当性・実行結果ともに問題ありません。**
...
```text
197Py  0:07:08.202
196Py  0:07:08.239  → 197が 0.037秒 / 0.009% 高速
194Py  0:07:08.288  → 197が 0.086秒 / 0.020% 高速
195Py  0:07:08.738  → 197が 0.536秒 / 0.125% 高速
192Py  0:07:10.118  → 197が 1.916秒 / 0.445% 高速
188Py  0:07:10.137  → 197が 1.935秒 / 0.450% 高速
```

差は非常に小さいですが、197Pyは196を再更新しているため、**197Pyを新最速基準として採用**しました。

198Pyは197Pyを親にしています。root fast-startの対象は197/196/194と同じく **root_avail 1bit/2bitのみ**です。195の3bit root prerollは戻していません。

198Pyの差分はMAXD14 kernel内だけです。
...
198Py:
  root_first      = cur_avail の先頭bit
  root_rest       = root_first除去後
  root_rest_more  = root_rest & (root_rest - 1)
...
198 root_rest tail-test predicate
equivalence OK: 66529 cases
```

こちらではCodon/CUDA実機build/full実行はしていません。cudacodon側では、以下でN=21 fullのcases 01-06、dispatch、final total、warning/error、特に197/196/194比のtimingを見て採用可否を判断してください。

```bash
STATIC_ONLY=1 bash 198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_validate_full_once.sh
bash 198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物はこちらです。

* [198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_probe.py)
* [198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_bundle_20260701.tar)

"""


# 198 root<=2 tail-test memo over validated 197 root-two futuremask/no-sibling MAXD14:
#   197 was correct and 0.037 seconds faster than 196, so this branch adopts
#   197 as the new numerical baseline.  It keeps the validated root one/two-
#   candidate preroll, 188 future_check_mask guard, and 184 no-sibling tailcall
#   build.
...
#   only restorable ancestor frames.  198 keeps the narrow root fast-start only when
...
#   The root<=2 predicate is changed from the second-lowbit direct-if test
#   to the equivalent root_rest & (root_rest - 1) tail test, removing one scalar.
...
# 198 keeps the validated 197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 198 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="198 roottwo-tailtest: 197 root<=2 direct-if futuremask/no-sibling MAXD14 baseline with root_rest tail-test predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """198 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate root_rest tail-test preroll predicate."""
    ...
    # 198 keeps the validated 197 current-frame register cache, 13-slot
    ...
    # correct but slower.  Replace the 197 second-lowbit direct-if predicate
    # with the equivalent power-of-two tail test on root_rest, so 1-bit roots
    # (root_rest==0) and 2-bit roots (root_rest has one bit) enter the preroll body
    # while 3-bit and wider roots stay on the generic loop.
    root_first:u32=cur_avail&(u32(0)-cur_avail)
    root_rest:u32=cur_avail^root_first
    root_rest_more:u32=root_rest&(root_rest-u32(1))

    if root_rest_more==u32(0):
...
CHUNKSHAPE148_DEFAULT_REASON:str="198 roottwo-tailtest reuses validated 197/196/194 scorestripe_v9 task order/cache; only MAXD14 root<=2 predicate scalar shape changes"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split198_roottwo_tailtest_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split198_roottwo_tailtest_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split198-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split198-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split198_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split198-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split198-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split198_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
