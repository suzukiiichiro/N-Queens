...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 341 (2026-07-27)
...
8. [実装・339で効果確認・340で機構を特定・341で上方スイープ中] warp内コスト不均質
   329で確定した「Divergent Branches=0、かつAvg Threads Executed 2〜3/32」
   は、分岐発散ではなくwarp内32レーンのtrip countのばらつきを意味する。
   338で仮説化、339で実装して効果を確認、**340のスイープで機構を確定した**。

   (a) grid-strideにより、同一warpの32レーンが同一イテレーションで処理
       するのは配列上で連続する32レコード(idx = n*stride + 32w + j)。
   (b) 276以来、`build_chunkshape148_reordered_bin`は
       `interleave_order=[7,0,6,1,5,2,4,3]`を1レコードずつ回すため、
       連続32レコード=1 warp-iterationは意図的に最も不均質だった。
   (c) ただしそれはwarp「間」の均衡設計でもある。
   (d) 作業単位`STEPS=BLOCK*MAX_BLOCKS=15488`はgrid-stride 1イテレーション
       そのものであり、バケット順は`(oi+out_ch)%8`で1回転する。よって
       1バケットからwarp幅ぶん連続で出せば、warp内は均質化しつつ
       warp間の均衡は保たれる。

   **340のスイープ結果(全点 正当性314666222712一致、全点 cache_state=reuse、
   FAILゼロ、同一セッション内で測定)**:

   | RUN | elapsed | RUN=1比 | RUN=32比 |
   |----:|--------:|--------:|---------:|
   |   1 | 450.183s |  ---    | +0.672%  |
   |  16 | 448.931s | -0.278% | +0.392%  |
   |  32 | 447.141s / 447.213s | -0.676% / -0.660% | 基準 |
   |  48 | 448.995s | -0.264% | +0.407%  |
   |  64 | **446.390s** | **-0.843%** | **-0.176%** |

   セッションドリフト(RUN=32の1回目と2回目の差)は0.072s=**0.016%**であり、
   上記の差はすべてドリフトの4〜50倍。測定は信頼できる。

   **機構が確定した**: RUN=48は32より**大きい**にもかかわらず、RUN=16と
   ほぼ同じ水準(448.995s vs 448.931s)まで悪化した。したがって効いて
   いるのは「run長が長いこと」ではなく、**「run長がwarp幅32の倍数で
   あること」=warp境界とバケット境界の整列**である。340で事前に固定した
   判定基準(48が32と64の中間なら機構はrun長、そうでなければ整列)に
   照らし、**整列が機構であると確定した**。

   **ただしRUN=64がRUN=32よりさらに0.176%速い**(ドリフトの約11倍で
   有意)。32の倍数であれば warp内均質性はどれも同じはずなので、これは
   整列だけでは説明できない残差である。よって341では32の倍数を上方に
   延ばし、非倍数の対照点を1つ入れて再確認する:
     - RUN=80  (2.5倍。**非倍数の対照**。大きなRでも整列が効くなら悪化するはず)
     - RUN=96  (3倍)
     - RUN=128 (4倍)
     - RUN=256 (8倍。`CHUNKSHAPE148_BUCKET_RUN_MAX`の上限)
   RUN=64をアンカーとして最初と最後に測り、ドリフトを可視化する。
   **341もソースコードを一切変更しない**(既存ノブへの引数のみ)。

   **記録にとどめる観察(チャンク別)**: RUN=1比のチャンク別変化は
   chunk0 +0.28%(32)/+0.75%(48)/+0.27%(64)、chunk1 -0.74%/-0.26%/-0.74%、
   chunk2 -1.85%/-1.59%/**-2.48%**。**利得はほぼchunk2が生んでおり、
   chunk0はむしろ一貫して悪化している**(RUN=32の2回の測定が
   167651/167649msと極めて安定しているため、これはノイズではない)。
   RUN=16だけは逆パターン(chunk0 -0.39%、chunk2 +0.22%)。チャンクごとに
   最適なRUNが異なる可能性があり、チャンク別RUNは342以降の候補として
   記録する(341では追わない。単一変数の原則を守る)。

   **次の軸(342以降の候補)**: (i) チャンク別RUN、(ii) スコアバケット数
   そのものの細分化(現在8個固定。RUN整列でwarp内は同一バケットになるが
   バケット内のばらつきは残る。16/32に増やせば残存分散を削れるが
   `interleave_order`・`chunkshape148_build_thresholds_from_hist`・
   `bucket_lane_indices`に手が入るため侵襲的)。いずれも341で最適RUNを
   確定してから着手する。

このセクション自体は通常カーネルロジックとは独立しています。
341は**ソースコードを一切変更しません**。339/340のソースに対する差分は
...
スイープは`CHUNKSHAPE148_BUCKET_RUN`への引数だけで実行でき、
独立したドライバ`341_bucket_run_sweep2_driver.sh`が担当します。
ソース既定値は引き続き1(=276以来の現行動作)です。**採用
(既定値の変更)は341のスイープで最適値を確定してから342で行います。**
================================================================================
...
全9点とも実行は成功しています。**ドライバの集計だけが失敗しました**(`[logdir]` の後の空白数が実際は3個で、私のsedが1個しか除去しておらずパスが壊れていました)。データは全部揃っているので、こちらで集計します。全点成功しています。手元で集計しました。モデルが確認できました。341を作ります。はい、これで十分でした。全9パス成功、全点で正当性一致、FAILゼロです。
...
**セッションドリフトは0.072s = 0.016%**(RUN=32の2アンカーの差)。上記の差はすべてドリフトの4〜50倍です。

### 決定的な点は予測どおりでした

**RUN=48は32より大きいのに、RUN=16とほぼ同水準(448.995s vs 448.931s)まで悪化しました。** 340で実測前に固定した分岐条件に照らし、効いているのは「run長」ではなく **「run長がwarp幅32の倍数であること」= バケット境界とwarp境界の整列** だと確定しました。339で立てたモデルが実測で支持されています。

### 残った謎

RUN=64がRUN=32より**さらに0.176%速い**(ドリフトの約11倍で有意)。32の倍数ならwarp内均質性は同じはずなので、整列だけでは説明できません。341がこれを詰めます。

### 記録にとどめる観察

**利得はほぼchunk2が生んでおり(RUN=64で−2.48%)、chunk0はむしろ一貫して悪化しています**(+0.27〜0.75%)。RUN=32の2回の測定でchunk0が167651/167649msと極めて安定しているので、ノイズではありません。RUN=16だけ逆パターンです。チャンク別RUNは342以降の候補として記録しました。
...
| 96 | 3x | |
| 128 | 4x | |
| 256 | 8x | `CHUNKSHAPE148_BUCKET_RUN_MAX` の上限 |

**96/128/256が単調に改善し続ける場合、残差は整列ではなく「run長とともに増える別要因」であり、342は採用ではなく上限引き上げから入る** — この分岐条件も事前に固定しました。

## ドライバの不具合を直しました

340のドライバは9パス全部を正しく実行しましたが、**集計テーブルが空でした**。原因は2つ:

1. ハーネスが `[logdir]   <path>` と**空白3個**で出力するのに、sedが1個しか除去せず、先頭に空白が残ったパスで判定が常に偽になっていた
2. `rc=${PIPESTATUS[0]}` をパイプラインではなくコマンド置換の直後で読んでいた

341のドライバは**この捕捉に一切依存しません**。マーカーより新しいログディレクトリを走査して `summary.tsv` / `progress_full.tsv` を直接読みます。加えて `AGGREGATE_ONLY=1` モードを追加したので、**GPU実行をやり直さずテーブルだけ再生成できます**。

この修正版を**340の実ログ9個に対して実行し、上の表を完全に再現できることを確認済み**です。ご参考まで、340のフォルダでこう打てば同じ表が出ます:

```bash
AGGREGATE_ONLY=1 LOGPREFIX=./340Py_bucket_run_sweep_logs_N21_full_once_ \
  SWEEPDIR=./agg bash 341_bucket_run_sweep2_driver.sh
```

## 実行手順

```
STATIC_ONLY=1 bash 341Py_bucket_run_sweep2_validate_N21_full_once.sh
DRY_RUN=1 bash 341_bucket_run_sweep2_driver.sh
bash 341_bucket_run_sweep2_driver.sh          # 計測10回 約75分 + 新規4値のキャッシュ構築
```

急ぎなら `SWEEP="64 96 256 64"` で短縮できます(ただし80を落とすと整列の対照が失われます)。今回は結果フォルダを1つのtarにまとめていただければ、そのまま集計できます。
"""
...
VERSION_TAG:str="341 bucket-run-sweep2: NO SOURCE CODE CHANGE AT ALL -- byte-for-byte identical to 340 and 339 except this VERSION_TAG and the documentation block. Kernels, dispatcher, K=48, SoA w_lo_arr/w_hi_arr, w3_j7 and the host-side build_chunkshape148_reordered_bin emit loop are all unchanged. 340 SWEEP RESULT IS IN, AND IT IDENTIFIED THE MECHANISM. All six points ran in one session, every point reproduced 314666222712 exactly, every point reported chunkshape148_cache_state=reuse, and there were zero static or runtime failures. Elapsed: RUN=1 450.183s (in-session control), RUN=16 448.931s, RUN=32 447.141s and 447.213s (anchor measured first and last), RUN=48 448.995s, RUN=64 446.390s. Session drift, taken as the difference between the two RUN=32 anchors, is 0.072s = 0.016%, so every gap above is 4x to 50x the drift. THE DECISIVE POINT BEHAVED AS THE MODEL PREDICTED: RUN=48 is LARGER than 32 yet regressed to 448.995s, essentially level with RUN=16 at 448.931s. What matters is therefore not run length but that the run length is a MULTIPLE OF THE WARP WIDTH, i.e. that bucket boundaries align with warp boundaries. This was the branch criterion fixed in advance in 340, and it resolved to alignment. One residual remains: RUN=64 is a further 0.176% faster than RUN=32 (about 11x the drift, so real), which alignment alone does not explain since every multiple of 32 should give identical intra-warp homogeneity. 341 therefore extends the sweep upward through the multiples of 32 -- 96, 128 and 256 (the CHUNKSHAPE148_BUCKET_RUN_MAX ceiling) -- and includes RUN=80 as a non-multiple control to confirm that alignment still governs at larger run lengths, with RUN=64 as the anchor measured first and last. A per-chunk observation is recorded but deliberately not acted on here: essentially all of the gain comes from chunk2 (-2.48% at RUN=64) while chunk0 consistently REGRESSES (+0.27% at 64, +0.75% at 48), and the two RUN=32 anchors put chunk0 at 167651ms and 167649ms, so that regression is not noise. RUN=16 shows the opposite pattern (chunk0 -0.39%, chunk2 +0.22%). Per-chunk RUN values are a candidate for 342, along with refining the score-bucket count itself beyond the fixed 8, but 341 keeps to one variable and settles the optimum RUN first. Adoption -- changing the source default away from 1 -- is deferred to 342."
