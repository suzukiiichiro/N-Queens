...
# =============================================================================
# 291 N=21 full validation harness
# Parent: 289 generic-normaldefault-ncolonly. Experiment: delay block_code scalar to the special branch only in MAXD14 generic loop; nf remains after branch.
...
SRC=${SRC:-./291Py_generic_normaldefault_blockcodelate_probe.py}
CAND=${CAND:-./291Py_generic_normaldefault_blockcodelate_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/291Py_generic_normaldefault_blockcodelate_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/291Py_generic_normaldefault_blockcodelate_logs_N21_full_once_${TS}"
...
echo "[start] 291 generic-normaldefault-blockcodelate validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 291 generic-normaldefault-blockcodelate SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '291 generic-normaldefault-blockcodelate' "$SRC"; then
  printf 'source_version_tag	291 generic-normaldefault-blockcodelate	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	291 generic-normaldefault-blockcodelate	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 291 generic-normaldefault-blockcodelate mode31 split145"
...
echo "[validation-ok] 291 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
