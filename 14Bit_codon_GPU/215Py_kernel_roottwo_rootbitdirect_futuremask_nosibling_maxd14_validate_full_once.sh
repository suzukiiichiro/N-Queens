...
# =============================================================================
# NQ_UPDATE_MEMO
# 215: 更新メモ: 214Py rootfirstxor は正当性OKだが204/211より大きく退行したため不採用。215Pyは211Py rootfirstlate形へ戻し、MAXD14 kernelのみroot-preroll内の pr_bit alias を削り root_first を直接使う微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 215 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 215 returns to near-best 211 late-root_first shape because 214 rootfirstxor regressed.
# 198/199/200/201/202/203/205/207/208/209/210/212/213/214 were correct but not adopted.
#
...
# 204 clear-lowbit root_rest plus root_after_second=root_rest^root_second scalar predicate, with root_first computed only inside the preroll branch and used directly without an alias.
...
#   - rootbitdirect source check is whitespace/comment robust.
SRC=${SRC:-./215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 215 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/215Py_kernel_roottwo_rootbitdirect_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second XOR scalar if + late root_first direct bit use	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
checks = {
    ...
    'pr_bit_alias_removed': 'pr_bit:u32=' not in root_compact and '|pr_bit' not in root_compact and '>>pr_bit' not in root_compact,
    'root_first_direct_block_path': 'pr_nld=((cur_ld|root_first)<<pr_stepu)|pr_addvu|pr_bLiu' in root_compact and 'pr_nrd=((cur_rd|root_first)>>pr_stepu)|pr_bKu' in root_compact,
    'root_first_direct_normal_path': 'pr_nld=(cur_ld|root_first)<<u32(1)' in root_compact and 'pr_nrd=(cur_rd|root_first)>>u32(1)' in root_compact,
    'root_first_direct_col_path': 'pr_ncol:u32=cur_col|root_first' in root_compact,
    ...
    # 215 keeps 204/197 save payload; 203 depth0-save is not adopted.
    ...
    # 215 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 215 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 215 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_rootbitdirect_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootbitdirect\tpresent_late_root_first_direct_bit\tpresent_late_root_first_direct_bit\tOK
' >> "$SUMMARY"
else
  printf 'source_rootbitdirect\tpresent_late_root_first_direct_bit\tmissing_or_old_rootbitdirect_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split215'}
bad=sorted(tags-allowed)
if 'split215' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split215 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split215 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 215 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 215 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootbitdirect static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 215 roottwo-rootbitdirect futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 215 roottwo-rootbitdirect futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
