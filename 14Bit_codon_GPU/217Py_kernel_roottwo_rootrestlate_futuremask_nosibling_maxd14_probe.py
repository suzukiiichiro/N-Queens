...
# =============================================================================
# NQ_UPDATE_MEMO
# 217: 更新メモ: 216Py rootprblockdirect は正当性OKだが204最速を上回らなかったため不採用。217Pyは211Pyのroot_first遅延形を親に、MAXD14 kernelのみroot-preroll内の cur_avail=root_rest 代入をfirst-child検証後へ遅延する最後のroot-preroll微差実験。root_rest clear-lowbit / root_after_second XOR / futuremask / no-sibling / root one-two preroll は維持する。
...
217Py note:
  ...
  This is a safety rollback from 210/209/208/207 to the 204/206 root predicate:
  keep the 204 clear-lowbit root_rest expression and the validated XOR
  root_after_second scalar.  The micro-experiment delays root_first lowbit
  extraction until the root<=2 predicate is known to pass.
...
"""
217Pyを作成しました。方針通り、**211Py近傍を最後に1本だけ確認する版**です。217Pyが204Pyを上回らなければ、root-preroll微差分探索はいったん一区切りにして、次はMAXD13準備診断へ移る前提です。

217Pyの差分はMAXD14 kernel内だけです。
...
意味は211Pyと同じです。first childが死ぬ/terminalになる場合も、branch終了後にgeneric loopが `root_rest` をactive root remainderとして処理します。first childがdescendする場合も、同じ `root_rest` を保存します。

手元確認は以下です。

```text
bash -n 217Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 217Py...sh OK
source_rootrestlate                   OK
source_split_tag split145 split217    OK
```

ファイルはこちらです。

* [217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_probe.py)
* [217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 217Py_kernel_roottwo_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 217 rootrestlate memo over validated 211 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="217 roottwo-rootrestlate: 211 rootrestlate/204 rootclearrest futuremask/no-sibling MAXD14 baseline with cur_avail root_rest assignment delayed in root-preroll - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # correct but slower.  217 keeps 211's late root_first extraction and 204's
    # root_after_second XOR scalar predicate, but delays cur_avail=root_rest until
    # after the first root child has passed the initial nf/future/terminal/jmark
    # checks.  If the first child dies or is terminal, the generic loop still sees
    # root_rest as the active root remainder.  If the first child descends, the same
    # root_rest is saved as a sibling exactly as before.
...
CHUNKSHAPE148_DEFAULT_REASON:str="217 rootrestlate reuses validated 204 scorestripe_v9 task order/cache; MAXD14 delays cur_avail=root_rest assignment within root-preroll"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split217_roottwo_rootrestlate_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split217_roottwo_rootrestlate_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split217-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split217-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split217_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split217-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split217-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split217_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
