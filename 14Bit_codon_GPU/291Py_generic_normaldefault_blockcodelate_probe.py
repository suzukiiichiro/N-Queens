...
# =============================================================================
# NQ_UPDATE_MEMO
# 291: generic-normaldefault-blockcodelate。289のnormaldefault nld/nrd + ncol-only earlyを親に、MAXD14 generic loopで block_code scalar をspecial側だけに遅延する微差実験版。
...
今回は **289を親にした `block_code` scalar遅延実験版**です。
289の `normaldefault nld/nrd + ncol-only early` は維持し、MAXD14 generic loop内で `block_code:u32 = nibble_op & 7` をnormal pathでは作らず、special branch内だけで作るようにしました。288の `nf default`、290のfast path複製は入れていません。

こちらで `STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [291Py_generic_normaldefault_blockcodelate_probe.py](sandbox:/mnt/data/291Py_generic_normaldefault_blockcodelate_probe.py)
* [291Py_generic_normaldefault_blockcodelate_validate_N21_full_once.sh](sandbox:/mnt/data/291Py_generic_normaldefault_blockcodelate_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [291Py_generic_normaldefault_blockcodelate_sha256.txt](sandbox:/mnt/data/291Py_generic_normaldefault_blockcodelate_sha256.txt)
* [291Py_generic_normaldefault_blockcodelate_bundle_20260709.tar](sandbox:/mnt/data/291Py_generic_normaldefault_blockcodelate_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./291Py_generic_normaldefault_blockcodelate_probe

STATIC_ONLY=1 bash 291Py_generic_normaldefault_blockcodelate_validate_N21_full_once.sh

bash 291Py_generic_normaldefault_blockcodelate_validate_N21_full_once.sh
```

採否は、まず正当性OK、そのうえで **289の `0:07:04.097`、287の `0:07:04.486`、286単発最速の `0:07:04.033`** と比較して判断するのがよいです。
...
VERSION_TAG:str="291 generic-normaldefault-blockcodelate: parent 289 normaldefault-nldnrd + ncol-only early; MAXD14 generic loop delays block_code scalar creation to the special branch only by testing nibble_op&7 directly; nf remains after the branch and is not defaulted; no nfdefault, no placed_ld/placed_rd commonization, root-preroll and host shaping/cache/dispatch preserved"
