...
172Pyログを確認しました。**問題ありません**。

```text
172Py : 0:08:11.190
171Py : 0:08:11.231
差分  : 172Pyが 0.041秒高速、約0.008%改善
```
...
131チャンク完走
重複・欠落なし
最終合計 314666222712 一致
selected_MAXD=14
schedule_words=0
stack_bytes_per_thread=208
warning/error/mismatch なし
```

速度差は誤差レベルですが、正当性は通っており、数値上は **172Pyを現時点の最速基準**として扱えます。

次の173Pyは予定どおり、172Pyを親にして **MAXD14のframe metadataをframe entry時に一度だけdecode** する版にしました。stack構成は172Pyと同じく `208 bytes/thread` のままです。

ファイルはこちらです。

* [173Py_kernel_frame_entry_decode_maxd14_probe.py](sandbox:/mnt/data/173Py_kernel_frame_entry_decode_maxd14_probe.py)
* [173Py_kernel_frame_entry_decode_maxd14_validate_full_once.sh](sandbox:/mnt/data/173Py_kernel_frame_entry_decode_maxd14_validate_full_once.sh)

173Pyの主な変更点です。

```text
172Py:
  current frame は register保持
  ただし候補bit処理ごとに、
    compact_op
    block_code
    future_check
    step/add/blockL/blockK
  をdecode

173Py:
  frameがcurrentになった時点で一度だけdecode
    root entry
    descend後のchild frame entry
    backtrack restore後のancestor frame entry

  候補bit反復中はdecode済みscalarを再利用
    cur_block_code
    cur_future_check
    cur_stepu
    cur_addvu
...
狙いは、同じframeに複数候補bitがある場合の反復decode削減です。

```text
stack_bytes_per_thread = 208
schedule_words         = 0
MAXD14 ancestor stack  = u32[13] x 4
```

`STATIC_ONLY=1` の静的・モデル検証は通しています。

```text
kernel_variant_count                    5 / 5 OK
kernel_fallback_normalized_equivalence  1 / 1 OK
maxd14_frame_entry_decode_schedule       1 / 1 OK
frame_opcode_equivalence                506604 cases OK
dfs_164_vs_173_entrydecode_equivalence  12000 cases OK
...
```bash
chmod +x 173Py_kernel_frame_entry_decode_maxd14_validate_full_once.sh
./173Py_kernel_frame_entry_decode_maxd14_validate_full_once.sh
```

この173Pyはdecode命令削減が目的ですが、scalar register数が増えるため、速くなるか遅くなるかは実測判断になります。



"""

# 173 GPU frame-entry decode experiment over 172 ancestor13-stack MAXD14:
#   Branch from the validated 172 ancestor13-stack build.  Host-side depth
...
#   registers, and local arrays still hold only saved ancestor frames.
#
...
#   Therefore MAXD14 local storage remains four u32[13] arrays: 208 bytes/thread.
#   The 173 change decodes each frame's compact operation once when that frame
#   becomes current, keeping step/add/block/future metadata in scalar registers
#   while iterating over all candidate bits in the same frame.  MAXD16/18/20/21
#   keep the validated byte-opcode local schedule fallback for larger required
#   depths.  Host layout, task order, cache, DFS order, bitboard arithmetic, and
#   solution arithmetic are unchanged.  The supported GPU range remains N=5..27.
...
# 173 static stack specializations.  MAXD remains the logical number of stored
# schedule frames.  MAXD14 uses scalar u32 compact schedule fields and a
...
SCHED_WORDS14:Static[int]=0  # 173 MAXD14 uses scalar u32 schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="173 frame-entry metadata decode for MAXD14 over 172 ancestor13 stack - scorestripe v9 lanephase32 octetfirstpairlock29"
...
# 173 inherits the exact 172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """173 MAXD14 frame-entry decoded scalar schedule plus register-held current frame and 13-slot ancestor stack."""
    ...
    # MAXD14 PTX-safe scalar schedule layout:
    #   schedule_lo bits  0..29: depths 0..9, 10 x 3-bit compact op
    #   schedule_hi bits  0..11: depths 10..13, 4 x 3-bit compact op
    ...
    # The local packed_schedule u32[4] array is removed for MAXD14, but all
    # shifts remain 32-bit to avoid Codon/CUDA invalid-PTX generation from
    # high-bit u64 variable shifts.
    ...
    # Generate the deterministic compact schedule once.  The current-frame
    # operation is one of seven variants and occupies 3 bits.  Using two u32
    # scalar fields keeps the schedule off local memory without relying on u64
    # high-bit packing inside the GPU kernel.
    ...
    while True:
      ...
      if schedule_isbu!=u32(0) and schedule_rowv==endm:
        ...
        if schedule_block_code==u32(0):
          frame_compact=schedule_fcvu
        else:
          frame_compact=schedule_block_code+u32(1)
    ...
    # 173 current-frame register cache with frame-entry metadata decode.  The
    # active frame state and its decoded operation metadata live in scalar
    # variables.  Local arrays hold only restorable ancestor states.  A depth-14
    # schedule can make sp==13 active, but that terminal frame is never spilled;
    # the deepest saved parent is index 12.  Decoding compact_op on frame entry
    # avoids repeating the same metadata decode for every candidate bit in a
    ...
    cur_compact_op:u32=schedule_lo&u32(7)
    cur_block_code:u32=u32(0)
    cur_future_check:u32=u32(0)
    cur_stepu:u32=u32(1)
    cur_addvu:u32=u32(0)
    cur_bLiu:u32=u32(0)
    cur_ktu:u32=u32(0)
    cur_bKu:u32=u32(0)
    if cur_compact_op==u32(1):
      cur_future_check=u32(1)
    elif cur_compact_op>=u32(2):
      cur_block_code=cur_compact_op-u32(1)
      cur_stepu=u32(2)+((OP_STEP3_MASK>>cur_block_code)&u32(1))
      cur_addvu=(OP_ADD1_MASK>>cur_block_code)&u32(1)
      cur_bLiu=(
        ((OP_BL1_MASK>>cur_block_code)&u32(1))
        |(((OP_BL2_MASK>>cur_block_code)&u32(1))<<u32(1))
      )
      cur_ktu=(
        ((OP_KN3_MASK>>cur_block_code)&u32(1))
        |(((OP_KN4_MASK>>cur_block_code)&u32(1))<<u32(1))
      )
      cur_bKu=(n3&(u32(0)-(cur_ktu&u32(1))))|(n4&(u32(0)-(cur_ktu>>u32(1))))
    ...
    while True:
      if cur_avail==u32(0):
        ...
        cur_avail=avail[sp]
        if sp<10:
          cur_compact_op=(schedule_lo>>u32(sp*3))&u32(7)
        else:
          cur_compact_op=(schedule_hi>>u32((sp-10)*3))&u32(7)
        cur_block_code=u32(0)
        cur_future_check=u32(0)
        cur_stepu=u32(1)
        cur_addvu=u32(0)
        cur_bLiu=u32(0)
        cur_ktu=u32(0)
      ...
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if cur_block_code!=u32(0):
        nld=((cur_ld|bit)<<cur_stepu)|cur_addvu|cur_bLiu
        nrd=((cur_rd|bit)>>cur_stepu)|cur_bKu
      else:
...
def kernel_dfs_iter_gpu_maxd16(
    ...
    """173 inherited byte-opcode schedule fallback, static MAXD=16."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd18(
    ...
    """173 inherited byte-opcode schedule fallback, static MAXD=18."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd20(
    ...
    """173 inherited byte-opcode schedule fallback, static MAXD=20."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
def kernel_dfs_iter_gpu_maxd21(
    ...
    """173 inherited byte-opcode schedule fallback, static MAXD=21."""
    IS_BASE_MASK:u32=u32(69222408)
    IS_JMARK_MASK:u32=u32(4)
    IS_MARK_MASK:u32=u32(199209203)
    IS_P5_MASK:u32=u32(3840)
...
####################################################################################################
# 173 frame-entry decode MAXD14 + scalar-u32pair schedule harness
#
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161/162/164/167/168/169/170/171/172/173: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 172 scorestripe_v9 task order/cache and ancestor13 MAXD14; 173 decodes frame metadata on entry"
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161/162/164/167/168/169/170/171/172/173: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 173 branches from validated 172.  The inherited host depth model rejects any launch
  # above MAXD21; the established bit-width/range envelope remains N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 173 frame-entry decode GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split173_frame_entry_decode_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split173_frame_entry_decode_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split173-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split173-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split173_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      ...
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split173-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split173-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split173-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split173_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
