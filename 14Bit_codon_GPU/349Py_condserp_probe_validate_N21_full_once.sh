...
SRC=${SRC:-./349Py_condserp_probe.py}
CAND=${CAND:-./349Py_condserp_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/349Py_condserp_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/349Py_condserp_probe_logs_N21_full_once_${TS}"
...
echo "[start] 349 condserp validation script (r2)"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 349 condserp SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '349 condserp' "$SRC"; then
  printf 'source_version_tag	349 condserp	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	349 condserp	missing	FAIL
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 349 source static checks failed" >&2
  exit 66
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 349 condserp (kernel-identical to 348/347/346/345/344/338/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT; 349 adds mode 9, conditional serpentine, applied only when the group length is an exact multiple of iter_len, and leaves the 346 default of 1 untouched. Mode 9 is element-for-element mode 5 on the two full launches and mode 1 on the partial one, so its shaped bin must be the byte concatenation of theirs and the harness checks that with cmp before any timing. Expected total is about 397.8s, roughly -1.12 percent against the mode 1 anchor of 402.296s. To reach the 344 baseline at any time, run with CHUNKSHAPE148_ITER_SORT=0"
