...
156Pyは問題ありません。全131チャンク、部分和01〜05、最終合計`314666222712`が一致し、欠落・重複・警告・エラーはありません。実行時間は`0:16:09.213`です。155Py比では`4.349秒（0.451%）`遅くなっています。

### 157Py

[157Py_markctrl_u32_prepack_probe.py](sandbox:/mnt/data/157Py_markctrl_u32_prepack_probe.py)

GPU入力を次の20bitへ事前パックしました。
...
`mark1`／`mark2`の`-1`は`31`として格納します。対応範囲N≤27ではrowが31にならないため、比較不一致を維持する番兵値になります。

カーネルでは4本の`Ptr[int]`を1本の`Ptr[u32]`へ統合し、`rowv`、`stepv`、mark/end比較、future境界判定、child-row生成をすべて`u32`内で処理します。CPU・診断用のint配列と、148のタスク順序・キャッシュは維持しています。

### 実行シェル

[157Py_markctrl_u32_validate_full_once.sh](sandbox:/mnt/data/157Py_markctrl_u32_validate_full_once.sh)

```bash
chmod +x 157Py_markctrl_u32_validate_full_once.sh
./157Py_markctrl_u32_validate_full_once.sh
```

シェル構文、markctrlの984,064組合せ、row/stepの3,596組合せ、ctrlの258,048組合せ、および模擬131チャンクによる全検証処理は通過しています。この環境ではCodon/CUDA実機コンパイルは未実施です。
...
# 157 GPU mark/end-u32 prepack experiment:
#   Retain the fully validated 156 initialized-ctrl-u32 packing, 155 ctrl0-u32
#   prepack, 154 GPU-input-u32 representation, and all earlier DFS/order changes.
#   The sole coordinated GPU experiment replaces four Ptr[int] mark/end inputs
#   (jmark/end/mark1/mark2) with one markctrl_arr:Ptr[u32], packed as four 5-bit
#   fields on the host.  The kernel decodes rowv and stepv as u32 and performs
#   every row/mark/end comparison and child-row calculation in u32.
#   Host int arrays remain available for CPU DFS, diagnostics, and sorting keys;
#   the validated 148 host-side task order/cache is unchanged.
...
VERSION_TAG:str="157 markctrl-u32 prepack + row/step u32 over 156 ctrl-fields-u32 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    while sp>=0:
      ...
      if (cv&u32(INIT_MASK))==u32(0):
        # 157: function-id remains int only for table/mask indexing; row is u32.
        f:int=int(cv&u32(31))
        ...
        #######################################
        # 157: row/step and all initialized-ctrl fields stay in u32.
        stepv:u32=u32(1)
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained; 157 selects packed u32 marks.
          markv:u32=mark2 if ((SEL2_MASK>>f)&1)==1 else mark1
          ###################
          # mark
          ###################
          ...
          if rowv==markv:
            ...
            ###################
            # step
            stepv=u32(3) if ((STP3_MASK>>f)&1) else u32(2)
            ###################
            # add
            frame_addu=u32(1) if f==20 else u32(0)
            ###################
            nextfidu=u32(int(meta_next[f]))
        #######################################
        # is_jmark
        ...
        frame_ktu:u32=u32(0)
        if frame_blocksu==u32(1):
          frame_bLiu=u32(((MASK_L_1>>f)&1)|(((MASK_L_2>>f)&1)<<1))
          if ((MASK_K_N3>>f)&1)==1:
            frame_ktu=u32(1)
          if ((MASK_K_N4>>f)&1)==1:
            frame_ktu=u32(2)
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154/155/156/157: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155/156/157: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154/155/156/157: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 157 markctrl-u32 prepack over the validated 156/155/154/153/152/151/150/149/148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 156/155/154/153/152/151/150/149/148; 157 changes only GPU mark/end representation
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 156/155/154/153/152/151/150/149/148 scorestripe_v9 task order/cache unchanged; 157 packs GPU mark/end fields in one u32"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  157 reuses the validated 156/155/154/153/152/151/150/149/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 157 experiment changes only the GPU mark/end input representation and row/step types.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  157 host-side score-aware lanephase chunk shaping is reused unchanged from validated 156/155/154/153/152/151/150/149/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 157 experiment changes only GPU mark/end packing and row/step types; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split157_markctrl_u32_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split157_markctrl_u32_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split157-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split157-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split157_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      ...
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split157-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split157-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split157-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split157_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
