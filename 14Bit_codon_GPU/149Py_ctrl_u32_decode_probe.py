...
お待たせしました。148の通常版・locked版は実行内容が同一で、ログ名だけが異なるため、149では1本に統合しました。

### 149Py

[149Py_ctrl_u32_decode_probe.py](sandbox:/mnt/data/149Py_ctrl_u32_decode_probe.py)

148の正解済み構成を維持し、GPUホットパスに残っていた`int(cv)`を除去しました。
...
`step`、`add1`、`blockL`、`blockK type`のデコードをすべて`u32`内で閉じる、単独の低リスク実験です。148で検証済みの並び替えbinは、そのまま再利用します。

### 統合実行シェル

[149Py_validate_full_once.sh](sandbox:/mnt/data/149Py_validate_full_once.sh)

旧シェル2本の候補実行回数は合計12回でした。149シェルはN=21の全131チャンクを**1回だけ**実行し、生成されたprogress TSVから旧01〜05を再計算します。

* 01〜04：指定チャンクの`chunk_total`を再集計
* 05：`chunk_index % 4 == 0`を集計し、worker 0-of-4を再現
* 06：全131チャンクの合計を検証
* 重複チャンク、欠落チャンク、エラー、mismatchも検査
* 実際の`flock`を使用し、同時GPU実行を防止
* バイナリがない、またはソースより古い場合だけ自動ビルド
* 148の平均`976.825秒`との差を情報表示

実行は次の1コマンドです。

```bash
chmod +x 149Py_validate_full_once.sh
./149Py_validate_full_once.sh
```

148は通常・lockedとも全6ケース一致し、実行差も0.023秒だけだったため、独立した2回目の同一検証は省略可能と判断しました。

### 一式

[149Py一式 tar.gz](sandbox:/mnt/data/149Py_ctrl_u32_decode_bundle.tar.gz)

この環境ではCodon/CUDA実機コンパイルは行えていません。シェルは`bash -n`と、131チャンクの模擬TSVを使った全検証処理を通過しています。

"""

# 149 GPU kernel experiment:
#   Retain the fully validated 148 task order, MAXD=21, five u32 DFS stacks,
#   and nld/nrd full-u32 arithmetic.  The sole kernel change is to decode
#   step/add/blockL/blockK metadata from ctrl (cv) entirely as u32, removing
#   the remaining int(cv) conversion from the expanded DFS hot path.
...
VERSION_TAG:str="149 ctrl-metadata full-u32 decode over 148 nld/nrd full-u32 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    # 149: fixed-width operands used by the u32 diagonal and ctrl-decode hot path.
    n3u:u32=u32(n3)
    n4u:u32=u32(n4)
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148/149
      ...
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 149: ctrl metadata decode is closed in u32; no int(cv) in this hot path.
        stepu:u32=(cv>>u32(10))&u32(3)
        addvu:u32=(cv>>u32(12))&u32(1)
        bLiu:u32=(cv>>u32(15))&u32(3)
        ktu:u32=(cv>>u32(17))&u32(3)
...
####################################################################################################
# 149 ctrl-metadata full-u32 decode over the validated 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 148; 149 changes only ctrl metadata decode inside kernel_dfs_iter_gpu
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 148 scorestripe_v9 task order/cache unchanged; 149 kernel experiment decodes ctrl metadata in u32 end-to-end"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  149 reuses the validated 148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 149 experiment keeps the 148 nld/nrd path and changes only ctrl metadata decoding inside the GPU kernel.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  149 host-side score-aware lanephase chunk shaping reused unchanged from validated 148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 149 experiment changes only ctrl metadata decoding; the 148 host-shaping cache is reused unchanged.
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split149_ctrl_u32_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split149_ctrl_u32_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split149-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split149-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split149_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      ...
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split149-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split149-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split149-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split149_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
