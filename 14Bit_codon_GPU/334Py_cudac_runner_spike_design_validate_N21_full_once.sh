...
# =============================================================================
# 334 N=21 full validation harness (cudac-runner-spike-design)
# Base: 333/328-332 lineage (adopted SoA baseline + w3_j7 reorder default;
#   kernel logic and dispatcher byte-for-byte identical throughout). 334
#   makes NO source change to the main solver at all -- kernel, dispatcher,
#   and host reorder parameters (w3_j7, K=48) are all unchanged from 333.
#   The only content is a design-stage doc update (Open Objectives #5
#   result confirmation + new #6: CUDA C runner / buried-idea C-2 spike
#   design) plus, in THIS harness only, a new NON-GATING environment probe.
#
# 333 RESULT (now confirmed, used as this run's expected-match baseline):
#   correctness 314666222712, elapsed 450.667s (332's direct measurement
#   450.113s, -0.123%, within noise), kernel_reduce_ms sum 448920ms
#   (within +-0.03% of 331/332's 448789/448791), +1.18% vs the old w8_j7
#   baseline (456.036/456.187s). Buried-idea C-3 fully closed.
#
# 334's NEW ENVIRONMENT PROBE (design prerequisite for buried-idea C-2,
# the rev84 CUDA C runner proposal -- the sole remaining route to Open
# Objective #3 / tail effect now that plan C-1 is closed per 330):
#   Runs `nvcc --version` and `cuobjdump --version` if present on PATH
#   and records the result (or absence) to summary.tsv as INFO. This
#   NEVER fails the run (unlike static source checks) -- it is pure
#   reconnaissance for a 335+ decision on whether a standalone .cu build
#   is feasible on cudacodon. No spike code is written in 334.
#
# MANDATORY VALIDATION ORDER (unchanged discipline):
#   1. STATIC_ONLY=1 first (no new gating source checks in 334; the nvcc
#      probe also runs under STATIC_ONLY=1 since it does not touch the GPU).
#   2. Full N=21 run; confirm 314666222712 FIRST, before timing.
#   3. Compare timing against 333 (450.667s) -- expect reproduction within
#      noise, since nothing in the solver changed.
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./334Py_cudac_runner_spike_design.py}
CAND=${CAND:-./334Py_cudac_runner_spike_design}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/334Py_cudac_runner_spike_design_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/334Py_cudac_runner_spike_design_logs_N21_full_once_${TS}"
...
echo "[start] 334 cudac-runner-spike-design validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 334 cudac-runner-spike-design SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '334 cudac-runner-spike-design' "$SRC"; then
  printf 'source_version_tag	334 cudac-runner-spike-design	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	334 cudac-runner-spike-design	missing	FAIL
...
# ---- 334: CUDA C runner (buried-idea C-2) spike prerequisite recon ----
...
# toolchain (nvcc + cuobjdump) before any 335+ spike code is written.
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 334 cudac-runner-spike-design (kernel-identical to 333/328) mode31 split145"
...
echo "[validation-ok] 334 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); NO solver source change in 334 (kernel/dispatcher/host params identical to 333/328-332, w3_j7 remains default); expected elapsed ~450.7s matching 333s own measurement 450.667s -- if timing drifts well outside noise, investigate before proceeding; see cudac_toolchain_probe rows in summary.tsv for the new nvcc/cuobjdump environment reconnaissance (INFO only, non-gating)"
