...
ご指摘ありがとうございます。これは **121 側の作成ミス**です。

`exec_solutions_gpu_chunk_split121()` の最後で、120/118 系の変数名 `d2_m` が残っていました。121 では bucket 名が以下に変わっているため、`d2_m` は存在しません。
...
[121Py_d2fid17_14_kernel_probe_fix.py](sandbox:/mnt/data/121Py_d2fid17_14_kernel_probe_fix.py)
...
# 修正前
stages:List[int]=[
  stage_soa_ms,stage_stats_ms,stage_split_ms,stage_kernel_reduce_ms,
  stage_compute_ms,stage_no_read_ms,d2_m,rest_m
]

# 修正後
stages:List[int]=[
  stage_soa_ms,stage_stats_ms,stage_split_ms,stage_kernel_reduce_ms,
  stage_compute_ms,stage_no_read_ms,fid17_m,d2base_m,rest_m
]
```

お手元で既存ファイルを直接直す場合は、以下でも同じです。

```bash
sed -i 's/stage_no_read_ms,d2_m,rest_m/stage_no_read_ms,fid17_m,d2base_m,rest_m/' \
  121Py_d2fid17_14_kernel_probe.py
```

修正版の実行は以下でお願いします。

```bash
codon build -release 121Py_d2fid17_14_kernel_probe_fix.py
```

```bash
./121Py_d2fid17_14_kernel_probe_fix \
...
今回のエラーは PTX/JIT ではなく、**host 側の未定義変数による Codon build error**です。したがって、この修正版で build が通った後に初めて `fid17` kernel の PTX/JIT 判定へ進めます。
...
VERSION_TAG:str="121 d2fid17+base14 kernel probe fix1 - rest uses proven 115 GPU kernel"
...
def exec_solutions_gpu_chunk_split121(
  ...
  elapsed_ms:int=stage_no_read_ms
  stages:List[int]=[stage_soa_ms,stage_stats_ms,stage_split_ms,stage_kernel_reduce_ms,stage_compute_ms,stage_no_read_ms,fid17_m,d2base_m,rest_m]
