...
# =============================================================================
# 262 N=21 full validation harness
...
SRC=${SRC:-./262Py_schedule_precompute_terminalbasepack_probe.py}
CAND=${CAND:-./262Py_schedule_precompute_terminalbasepack_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/262Py_schedule_precompute_terminalbasepack_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/262Py_schedule_precompute_terminalbasepack_logs_N21_full_once_${TS}"
...
echo "[start] 262 schedule-precompute-terminalbasepack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 262 schedule-precompute-terminalbasepack terminal-depth-only SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '262 schedule-precompute-terminalbasepack probe' "$SRC"; then
  printf 'source_version_tag\t262 schedule-precompute-terminalbasepack probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t262 schedule-precompute-terminalbasepack probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split262_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s and 'split244_' not in s and 'split255_' not in s and 'split256_' not in s and 'split257_' not in s and 'split258_' not in s and 'split259_' not in s and 'split260_' not in s and 'split261_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split262 active; split240/241/242/243/244/255/256/257/258/259/260/261 runtime tags absent', 'split262', True))
else:
    checks.append(('source_split_tag', 'split262 active; split240/241/242/243/244/255/256/257/258/259/260/261 runtime tags absent', 'old split runtime tag present or split262 missing', False))
...
if not m14:
    ...
    ok = (
        ...
        'sched_terminal_arr:Ptr[u32]' in body and
        ...
        'sched_terminal:u32=sched_terminal_arr[i]' in body and
    ...
    actual = 'terminalbasepack precompute active' if ok else 'old schedule interpreter present or terminalbasepack loads missing or root_action/action/ctrl pack still present'
    checks.append(('source_schedule_precompute_terminalbasepack', 'MAXD14 precompute terminalbasepack active', actual, ok))
...
host_ok = (
    ...
    'gpu.raw(soa.sched_terminal_arr)' in s and
...
checks.append(('source_host_precompute_terminalbasepack_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 262 schedule-precompute-terminalbasepack mode31 split145"
