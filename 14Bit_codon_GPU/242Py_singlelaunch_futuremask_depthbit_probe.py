...
# =============================================================================
# NQ_UPDATE_MEMO
# 242: singlelaunch-futuremask-depthbit probe。241 restore239 を親に、
#      240のfid14/rest別launchは採用せず、単一launchを維持する。
#      MAXD14 kernel 内だけ future-prune 判定を depth bit 参照へ寄せ、
#      root-preroll と generic loop の future 判定分岐を小さく減らす。
#      CUDA launch数、MAXD16/18/20/21 fallback、cache生成、mode28/29/30/31、
#      worker split、CPU dfs_iter path、N=5..27 range は維持する。
...
作成しました。242は **241親 / 単一launch維持 / MAXD14 future-prune条件整理 probe** です。

240の fid14/rest 別launch は採用せず、dispatch rows は従来通り `131` 前提です。
変更は MAXD14 kernel 内だけで、future-prune 判定を `nibble_op & 8` ではなく `future_check_mask` の depth bit から直接見る形にしています。
...
source_future_depthbit_probe    OK
release_build_policy            OK
```

ファイルはこちらです。

* [242Py_singlelaunch_futuremask_depthbit_probe.py](sandbox:/mnt/data/242Py_singlelaunch_futuremask_depthbit_probe.py)
* [242Py_singlelaunch_futuremask_depthbit_validate_N21_full_once.sh](sandbox:/mnt/data/242Py_singlelaunch_futuremask_depthbit_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [242Py_singlelaunch_futuremask_depthbit_sha256.txt](sandbox:/mnt/data/242Py_singlelaunch_futuremask_depthbit_sha256.txt)
* [242Py_singlelaunch_futuremask_depthbit_bundle_20260707.tar](sandbox:/mnt/data/242Py_singlelaunch_futuremask_depthbit_bundle_20260707.tar)

sha256です。

```text
e2039be9ce15b5a624da925ef9b7df512f54318f29a72526e0e3aec2ab24a565  242Py_singlelaunch_futuremask_depthbit_probe.py
b7152b0adc112ec7963ce59f71e7ae4f3f4604146b5ad11b049eb5ae88bfc326  242Py_singlelaunch_futuremask_depthbit_validate_N21_full_once.sh
47ae3793b3189792d980a2e47dc2e86c5d4cb7ea0e1b9ba234227abf0967a805  README.md
```

実行はいつも通りです。

```bash
rm -f ./242Py_singlelaunch_futuremask_depthbit_probe

STATIC_ONLY=1 bash 242Py_singlelaunch_futuremask_depthbit_validate_N21_full_once.sh

bash 242Py_singlelaunch_futuremask_depthbit_validate_N21_full_once.sh
```

Codon build はこの環境では実行できていないため、cudacodon側ではまず `STATIC_ONLY=1` の表示が上記と一致するか確認してください。


おっしゃる通りです。
そのレベルの変更をこちらが「次の最適化案」として出すのは間違いでした。

ここまでの結果から、もう結論は出ています。
...
やらない:
  ...
  条件式の畳み込み
  一時変数を消す / 増やす
  nibble を見るか mask bit を見るか
  root-preroll の局所的な順序変更
  generic loop の微差分岐変更
  小さい task を別 launch に逃がすだけの分割
```

これは、実際にこの一週間でほぼ全部やって、速くならなかったものです。
ご指摘の通り、そこは Copilot 程度でも出せるし、NQの次段として価値が低いです。

こちらが次に出すべきなのは、そういう小手先ではなく、**実行構造そのものから GPU kernel 内の仕事量を削る案**です。

現時点で意味がある候補は、かなり絞れます。

```text
1. kernel内 schedule interpreter の撤去
   kernel冒頭の schedule_raw 解析 while を host 側で precompute して、
   kernelには schedule_lo / schedule_hi / future_check_mask /
   child_jmark_mask / terminal_depth / terminal_base14 / root_action
   を渡す。

2. task shape ごとの kernel生成
   ただし別launch乱発ではなく、まずは N=21/N=22 の実データで
   schedule pattern の種類数と上位頻度を出す。
   上位数個だけ専用化する価値があるかを見る。

3. GPU kernel のPTX/SASS観点の確認
   register数、local memory、branch数、命令数を見て、
   どこが本当に詰まっているかを確認する。
   体感のif整理ではなく、コンパイル結果で判断する。

4. host precompute と global load 増加の損益確認
   schedule生成を消す代わりに配列loadが増えるので、
   これは full 実装前に mode30 の数chunkで見る。
```

特に 1 は、単なる if 整理ではありません。
今の MAXD14 kernel は、各 thread が探索前に小さな schedule interpreter を毎回走らせています。そこには `IS_BASE_MASK`、`IS_MARK_MASK`、`IS_JMARK_MASK`、`IS_P5_MASK`、`BLOCK_CODE_*`、`meta_next`、mark/end 比較があり、これは DFS 本体とは別の前処理です。

ここを host 側へ逃がせれば、kernel内からまとまった条件分岐群が消えます。
失敗する可能性もあります。理由は、代わりに global load が増えるからです。ただ、これは「ifの向き」ではなく、**GPUで毎thread実行していた小インタプリタを消す**話なので、試す価値があります。
...
次に作るなら:
  242Py_schedule_precompute_probe.py

目的:
  単一launch維持
  fid14 splitなし
  kernel内 schedule生成ブロックをhost precomputeへ移す
  MAXD14 kernelだけ差分
  mode28/29/30/31、cache生成、worker split、fallbackは維持

採否:
  正当性OK
  dispatch rows 131維持
  241の 0:07:07.788 を上回るか
```

こちらの出す案がまた「if/elseの向き」レベルになったら、それはやる価値がないので止めます。次は構造変更だけに限定します。
...
VERSION_TAG:str="242 singlelaunch-futuremask-depthbit: 241 restore239 parent; 240 fid14 split rejected; single launch kept; MAXD14 future-prune branch uses future_check_mask depth bit; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
A10G_FINAL_DEFAULT_BENCH_MODE:int=31  # 242: bare -g stays split145 mode31; no extra launch split
...
# 242: keep runtime globals required by broadmarktail/chunkshape148/split145
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    if root_after_second==u32(0):
      ...
      if pr_nf==u32(0):
        pr_descend=u32(0)
      if pr_descend!=u32(0):
        # 242: depth-bit future-prune check.  Equivalent to the depth-0
        # nibble future bit, but avoids the extra future_check_mask/nibble
        # pair of conditionals in the root-preroll path.
        if (future_check_mask&u32(1))!=u32(0):
    ...
    while True:
      ...
      if nf==u32(0):
        continue
      # 242: use the packed future_check_mask depth bit directly.
      # This keeps the single launch path and replaces the nested
      # future_check_mask / nibble_op future-bit conditionals with one
      # depth-specific branch.
      if ((future_check_mask>>u32(cur_depth))&u32(1))!=u32(0):
...
# 242: recursive CPU dfs fallback remains removed. CPU path uses dfs_iter only.
...
# 242: these helpers are used by the kept broadmarktail/split145
...
CHUNKSHAPE148_DEFAULT_REASON:str="242 futuredepthbit keeps validated 239/238/231/217 scorestripe_v9 task order/cache; single launch kept; 240 fid14 launch split rejected"
...
def main()->None:
  ...
  if argc == 1:
    ...
    print("[242-default] no arguments: CPU N22 default; use -g for A10G range mode")
  elif argc >= 2:
    arg=sys.argv[1]
    if arg == "-c":
      use_gpu=False
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 242 futuredepthbit/coretrim; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split242_futuredepth_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split242_futuredepth_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split242-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split242-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split242_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
