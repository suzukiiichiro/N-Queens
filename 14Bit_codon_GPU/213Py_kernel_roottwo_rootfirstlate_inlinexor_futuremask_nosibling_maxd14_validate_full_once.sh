...
# =============================================================================
# NQ_UPDATE_MEMO
# 213: 更新メモ: 212Py rootfirstlate+andnot は正当性OKだが204最速を上回れなかったため未採用。213Pyは211のroot_first lateを維持し、205で試した root_after_second scalar削除を late-root-first 形に限定して再検証する。
...
# progress TSV and verifies the complete total. Candidate 213 returns to
...
#   - rootfirstlate_inlinexor source check is whitespace/comment robust.
SRC=${SRC:-./213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 213 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/213Py_kernel_roottwo_rootfirstlate_inlinexor_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 213Py source
#   has a harmless header/string/layout difference. This 213 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + inline XOR root predicate + late root_first extraction	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
marker='# 213 rootfirstlate_inlinexor:'
...
checks = {
    ...
    'root_inline_xor_if_present': if_pos >= 0,
    ...
    # 213 keeps 204/197 save payload; 203 depth0-save is not adopted.
    ...
    # 213 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 213 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 213 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_rootfirstlate_inlinexor_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootfirstlate_inlinexor\tpresent_late_root_first_inlinexor_predicate\tpresent_late_root_first_inlinexor_predicate\tOK
' >> "$SUMMARY"
else
  printf 'source_rootfirstlate_inlinexor\tpresent_late_root_first_inlinexor_predicate\tmissing_or_old_rootfirst_inlinexor_shape\tFAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split213'}
bad=sorted(tags-allowed)
if 'split213' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split213 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split213 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 213 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 213 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootfirstlate_inlinexor static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 213 roottwo-rootfirstlate_inlinexor futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 213 roottwo-rootfirstlate_inlinexor futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
