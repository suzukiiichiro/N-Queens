...
codon build -release \
111Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_multigpu_worker.py

# GPUシングル実行
./111Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_multigpu_worker \
  -g 22 22 32 484 1 0 7 29 \
  2>&1 | tee 111Py_N22_single_gpu_full.log
...
./111Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_multigpu_worker \
  ...
  2>&1 | tee 111Py_N22_broadmarktail_sim_build.log


CUDA_VISIBLE_DEVICES=0 ./111Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_multigpu_worker \
  -g 22 22 32 484 1 0 7 29 8 7 0 0 4 \
  2>&1 | tee 111Py_N22_worker0of4.log &

CUDA_VISIBLE_DEVICES=1 ./111Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_multigpu_worker \
  -g 22 22 32 484 1 0 7 29 8 7 0 1 4 \
  2>&1 | tee 111Py_N22_worker1of4.log &

CUDA_VISIBLE_DEVICES=2 ./111Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_multigpu_worker \
  -g 22 22 32 484 1 0 7 29 8 7 0 2 4 \
  2>&1 | tee 111Py_N22_worker2of4.log &

CUDA_VISIBLE_DEVICES=3 ./111Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_multigpu_worker \
  -g 22 22 32 484 1 0 7 29 8 7 0 3 4 \
  2>&1 | tee 111Py_N22_worker3of4.log &

wait

# 4本の合算確認は添付したスクリプトでできます。
python3 sum_111py_worker_totals.py 111Py_N22_worker*of4.log
...
109Py をベースに、**111Py multi-GPU worker版**を作成しました。109Py側は mode 29 の broadmarktail GPU full run を持っているので、ここに worker 分割だけを足しています。

作成ファイルはこちらです。

[111Py ソース](sandbox:/mnt/data/111Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_funcid17_gh_multigpu_worker.py)

[109Py→111Py 差分パッチ](sandbox:/mnt/data/109Py_to_111Py_multigpu_worker.patch)

[worker合算スクリプト](sandbox:/mnt/data/sum_111py_worker_totals.py)
...
追加:
  mode 29 に worker_id / worker_count を追加
...
multi:
  worker_count=4
  worker_id=0,1,2,3 がそれぞれ担当chunkだけ実行
```
...
従来どおり single GPU で動かす場合は、これまでと同じです。
...
各workerの最後に以下のような行が出ます。

```text
[worker-done] N=22 worker=0/4 partial_total=... expected_total=2691008701644
```
...
```text
grand_total=2691008701644
expected_total=2691008701644
status=ok
```

multi実行時のprogress TSVは worker ごとに分かれます。

```text
progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu_worker0of4.tsv
progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu_worker1of4.tsv
progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu_worker2of4.tsv
progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu_worker3of4.tsv
```

こちらの環境では Codon ビルド実行まではできていません。変更は **mode 29 のstream実行部、CLI引数、worker別progress名、partial_totalログ**に限定し、GPU kernel / DFS / reorder ロジックは触っていません。
...
VERSION_TAG:str="111 broadmark funcid17/G_H weak tertiary smoothing + optional multi-GPU worker split auto w8_j7 stream GPU runner from 109"
...
def exec_solutions_gpu_bin_stream_funcid_reorder(
  ...
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_funcid_reorder_v2_{funcid_reorder_param_tag()}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_funcid_reorder_v2_{funcid_reorder_param_tag()}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
...
def main()->None:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==29:
      ...
      print(f"[broadmarktail-reorder-gpu-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} worker={worker_id}/{worker_count} total={total}")
      if worker_count>1:
