...
# =============================================================================
# 337 N=21 full validation harness (bin-format-reader-design)
# Base: 336/335/334/333/328-332 lineage (adopted SoA baseline + w3_j7
#   reorder default; kernel logic and dispatcher byte-for-byte identical
#   throughout). 337 makes NO source change to the main solver at all --
#   kernel, dispatcher, and host reorder parameters (w3_j7, K=48) are all
#   unchanged from 336/335/334/333. The only .py content is an Open
#   Objectives #6 update. The new deliverable this revision is a
#   STANDALONE artifact outside this harness: 337_bin_format_reader.cu +
#   337_bin_format_reader_check.sh (see those files; not part of this
#   N=21 validate at all).
#
# 336 RESULT (now confirmed, used as this run's expected-match baseline):
#   correctness 314666222712, elapsed 450.432s (335's 450.218s, -0.048%,
#   within noise). All static checks OK. 336's standalone smoke test
#   (336_cudac_smoke_test.cu / _check.sh) SUCCEEDED: build_exit=0,
#   run_exit=0, gpu_sum=cpu_sum=22898104320, match=True -- the first
#   confirmed nvcc -arch=sm_86 compile/link/execute round-trip on
#   cudacodon, months after rev84 first proposed a CUDA C runner.
#
# 337's NEW ARTIFACT (second concrete step of buried-idea C-2): a bin
# file format spec, pinned down by direct source reading rather than
# assumption. Every .bin write in this codebase (base generation,
# broadmarktail subcell buckets, and the final broadmarktail/
# chunkshape148-reordered output) routes through the single function
# append_constellations_bin, and the record encoding is uniform
# throughout: a header-less, magic-number-less flat array of fixed
# 16-byte records (four little-endian uint32 fields: ld, rd, col,
# startijkl). Reordering stages only change record SEQUENCE, never the
# encoding. 337_bin_format_reader.cu is a minimal HOST-ONLY (no GPU
# kernel, no DFS logic) C++ reader that opens an actual .bin file
# passed as a CLI argument, validates size%16==0, and folds every
# record's four fields into a uint64 checksum for manual cross-
# reference against the Codon side. Run it via
# `bash 337_bin_format_reader_check.sh <path-to-a-.bin-file>`
# (separate from this harness; does not touch the GPU at all). If it
# fails or the checksum does not make sense, that is itself valuable
# design information and does not affect the N=21 validation below in
# any way.
#
# MANDATORY VALIDATION ORDER (unchanged discipline):
#   1. STATIC_ONLY=1 first for this harness (no new gating source
#      checks in 337).
#   2. Full N=21 run; confirm 314666222712 FIRST, before timing.
#   3. Compare timing against 336 (450.432s) -- expect reproduction
#      within noise, since nothing in the solver changed.
#   4. Separately (order independent, does not gate the above):
#      `bash 337_bin_format_reader_check.sh <bin-file>` to exercise the
#      new bin-format reader against a real cached .bin file (e.g. one
#      of the constellations_N21_6*.bin files already on disk from
#      earlier revisions).
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./337Py_bin_format_reader_design.py}
CAND=${CAND:-./337Py_bin_format_reader_design}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/337Py_bin_format_reader_design_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/337Py_bin_format_reader_design_logs_N21_full_once_${TS}"
...
echo "[start] 337 bin-format-reader-design validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 337 bin-format-reader-design SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '337 bin-format-reader-design' "$SRC"; then
  printf 'source_version_tag	337 bin-format-reader-design	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	337 bin-format-reader-design	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 337 bin-format-reader-design (kernel-identical to 336/335/334/333/328) mode31 split145"
...
echo "[validation-ok] 337 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); NO solver source change in 337 (kernel/dispatcher/host params identical to 336/335/334/333/328-332, w3_j7 remains default); expected elapsed ~450.4s matching 336s own measurement 450.432s -- see the standalone 337_bin_format_reader.cu / 337_bin_format_reader_check.sh (run separately, NOT part of this harness, no GPU used) for the bin-format spec confirmation and checksum reader"
