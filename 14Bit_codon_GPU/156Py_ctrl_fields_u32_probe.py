...
155Pyは問題ありません。全131チャンク、部分和01〜05、最終合計`314666222712`が一致し、欠落・重複・警告・エラーはありません。実行時間は`0:16:04.864`でした。

### 156Py

[156Py_ctrl_fields_u32_probe.py](sandbox:/mnt/data/156Py_ctrl_fields_u32_probe.py)

初期化済み`ctrl`へ格納する次の8フィールドを`u32`化しました。
...
最終パック式も、外側の`u32(...)`変換を使わない純粋な`u32`のshift／OR式に変更しています。

`rowv`、`stepv`、`jmark/end/mark1/mark2`は今回`int`のまま維持し、157Pyの事前パック実験と分離しています。

### 実行シェル

[156Py_ctrl_fields_u32_validate_full_once.sh](sandbox:/mnt/data/156Py_ctrl_fields_u32_validate_full_once.sh)

```bash
chmod +x 156Py_ctrl_fields_u32_validate_full_once.sh
./156Py_ctrl_fields_u32_validate_full_once.sh
```

全131チャンクを1回実行し、部分和01〜05と全件結果を再検証します。併せて、`ctrl`の全258,048フィールド組合せについてpack/decode検査を行い、155Pyとの速度比較も表示します。

この環境ではCodon/CUDA実機コンパイルは未実施です。シェル構文と、155Pyのprogress TSVを用いた全検証処理は通過しています。


"""

# 156 initialized-ctrl field-u32 packing experiment:
#   Retain the fully validated 155 ctrl0-u32 prepack, 154 GPU-input-u32
#   representation, 153 future-from-mark complement, 152 lazy next-fid lookup,
#   151 frame-state reload elision, 150 branchless blockK selector, 149 ctrl-u32
#   decode, and the validated 148 task order / MAXD=21 / five-u32-stack kernel.
#   The sole GPU experiment keeps all eight fields written into an initialized
#   ctrl word as u32 before the final shift/OR: next-fid, child-row, step, add1,
#   use-blocks, future-check, blockL, and blockK type.  rowv/stepv and mark/end
#   comparisons intentionally remain int; their coordinated u32 conversion is
#   reserved for the next experiment.
#   The 155 ctrl0_arr:u32 input and 148 host-side ordering/cache are unchanged.
...
VERSION_TAG:str="156 initialized-ctrl fields u32 pack over 155 ctrl0-u32 - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    # 156 は initialized ctrl の8フィールドを frame_*:u32 で保持し、
    #      最終パックを外側の u32(...) なしの純粋な u32 shift/OR にする。
    #      rowv/stepv と mark/end 比較値は int のまま維持する。
    ...
    while sp>=0:
      ...
      if (cv&u32(INIT_MASK))==u32(0):
        ...
        #######################################
        # 156: initialized-ctrl fields are u32 from their first assignment.
        # stepv stays int only for the still-int row/end comparison in this probe.
        stepv:int=1
        frame_stepu:u32=u32(1)
        ...
        if ism==1:
          ###################
          # 147 single-compare mark selector retained by 148/149/150/151/152/153/154/155/156
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154/155/156: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155/156: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154/155/156: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 156 ctrl-fields-u32 pack over the validated 155/154/153/152/151/150/149/148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 155/154/153/152/151/150/149/148; 156 changes only initialized ctrl field types
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 155/154/153/152/151/150/149/148 scorestripe_v9 task order/cache unchanged; 156 packs initialized ctrl fields in u32"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  156 reuses the validated 155/154/153/152/151/150/149/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 156 experiment retains the validated 155 input layout and changes only initialized ctrl field types.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  156 host-side score-aware lanephase chunk shaping is reused unchanged from validated 155/154/153/152/151/150/149/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 156 experiment changes only initialized ctrl field types; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split156_ctrl_fields_u32_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split156_ctrl_fields_u32_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split156-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split156-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split156_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split156-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split156-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split156_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
