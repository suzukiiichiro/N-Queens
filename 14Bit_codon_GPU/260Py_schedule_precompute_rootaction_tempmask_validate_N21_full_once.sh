...
# =============================================================================
# 260 N=21 full validation harness
# Parent: 256 base14pack JIT-ok / 244 r4 maskpack / 241 restore239.
# Experiment: keep single launch, child/future maskpack, and terminal_base14
#             separate. Split root_action decode into raw load then mask, to test whether
#             Codon GPU IR/JIT rejects only the direct index-and-mask expression
#             seen in 257/259.
...
SRC=${SRC:-./260Py_schedule_precompute_rootaction_tempmask_probe.py}
CAND=${CAND:-./260Py_schedule_precompute_rootaction_tempmask_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/260Py_schedule_precompute_rootaction_tempmask_N21_full.lock}
...
BASELINE_256_SECONDS=${BASELINE_256_SECONDS:-430.754}
...
LOGDIR="${LOG_ROOT%/}/260Py_schedule_precompute_rootaction_tempmask_logs_N21_full_once_${TS}"
...
echo "[start] 260 schedule-precompute-rootaction-tempmask validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 260 schedule-precompute-rootaction-tempmask user-tempmask SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '260 schedule-precompute-rootaction-tempmask probe' "$SRC"; then
  printf 'source_version_tag\t260 schedule-precompute-rootaction-tempmask probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t260 schedule-precompute-rootaction-tempmask probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split260_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s and 'split244_' not in s and 'split255_' not in s and 'split256_' not in s and 'split257_' not in s and 'split258_' not in s and 'split259_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split260 active; split240/241/242/243/244/255/256/257/258/259 runtime tags absent', 'split260', True))
else:
    checks.append(('source_split_tag', 'split260 active; split240/241/242/243/244/255/256/257/258/259 runtime tags absent', 'old split runtime tag present or split260 missing', False))
...
if not m14:
    ...
    ok = (
        ...
        'sched_base14_arr:Ptr[u32]' in body and
        'sched_root_action_arr:Ptr[u32]' in body and
        ...
        'terminal_base14:u32=sched_base14_arr[i]&u32(1)' in body and
        'root_action_raw:u32=sched_root_action_arr[i]' in body and
        'root_action:u32=root_action_raw&u32(3)' in body and
        'schedule_raw:u32=ctrl0_arr[i]' not in body and
        'schedule_fu:u32=' not in body and
        'sched_ctrl_arr:Ptr[u32]' not in body
    )
    actual = 'rootaction tempmask precompute active' if ok else 'old schedule interpreter present or tempmask root_action decode missing'
    checks.append(('source_schedule_precompute_rootaction_tempmask', 'MAXD14 precompute rootaction tempmask active', actual, ok))
...
host_ok = (
    ...
    'self.sched_base14_arr:List[u32]' in s and
    'self.sched_root_action_arr:List[u32]' in s and
    'sched_root_action:u32=root_action&u32(3)' in s and
    'gpu.raw(soa.sched_mask_arr)' in s and
    'gpu.raw(soa.sched_base14_arr)' in s and
    'gpu.raw(soa.sched_root_action_arr)' in s and
    'self.child_jmark_mask_arr' not in s and
    'self.future_check_mask_arr' not in s and
    'sched_ctrl_arr' not in s
)
checks.append(('source_host_precompute_rootaction_tempmask_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
echo "[static-ok] 260 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 260 schedule-precompute-rootaction-tempmask mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "241parent:$BASELINE_241_SECONDS" "239:$BASELINE_239_SECONDS" "242rejected:$BASELINE_242_REJECT_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 260 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
