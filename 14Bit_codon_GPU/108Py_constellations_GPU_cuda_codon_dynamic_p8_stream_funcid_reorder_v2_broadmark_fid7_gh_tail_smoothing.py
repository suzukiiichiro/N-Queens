...
codon build -release \
  108Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_fid7_gh_tail_smoothing.py

#CPU
./108Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_fid7_gh_tail_smoothing \
  -c 5 22
#GPU
./108Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_fid7_gh_tail_smoothing \
...
作成しました。107Py の **broad bucket primary + mark-distance secondary** を維持し、その内側に **fid7 / cell_G_H tertiary tail smoothing** を追加した 108Py 版です。107Py 側の broad×markrisk 25 cell reorder を土台にしつつ、メモで 108Py の追加制御対象とされた `funcid_7_count` / `cell_G_H_count` を 75 subcell、つまり `5 broad × 5 markrisk × 3 tail(F7/GH/R)` に分けて散らす形にしました。

こちらです。

[108Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_fid7_gh_tail_smoothing.py](sandbox:/mnt/data/108Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_broadmark_fid7_gh_tail_smoothing.py)

主な変更点は以下です。

```text
VERSION_TAG:
  108 broadmark fid7/G_H tertiary tail smoothing auto w8_j7 stream GPU runner from 107 type-explicit

mode 28:
  broadmarktail reorder simulation/build only

mode 29:
  broadmarktail reordered GPU full run
```

実行コマンドは従来どおり `mode=29` でいけます。
...
この環境には `codon` が入っていなかったため、こちらではコンパイル実行まではできていません。ソース生成後、`VERSION_TAG`、108Py 追加関数群、mode 28/29 の接続、出力ファイル名の切替は確認済みです。

実行完了、ありがとうございます。**108Py は正しく完走しています。**
N=22 の `final_gpu_total` は期待値 `2691008701644` と一致し、status は **ok** です。集計上の kernel elapsed 合計は **12983.119 sec = 3:36:23.119**、実行ログ上の wall clock は **3:39:28.536 ok** でした。

結論から言うと、**108Py は「max chunk の改善」には成功しましたが、「総 elapsed」では 107Py に負けています。したがって採用本線はまだ 107Py、108Py は tail smoothing 実験版扱いがよいです。**

## 107Py 比較

| 指標          |         107Py |         108Py | 判定                         |
| ----------- | ------------: | ------------: | -------------------------- |
| elapsed sum | 12954.626 sec | 12983.119 sec | **108Py が +28.493 sec 遅い** |
| mean/chunk  |   6983.626 ms |   6998.986 ms | 108Py が +15.36 ms 遅い       |
| median      |       6920 ms |       6923 ms | ほぼ同等                       |
| p95         |     8671.3 ms |     8735.6 ms | 108Py が悪化                  |
| p99         |    9352.46 ms |    9486.84 ms | 108Py が悪化                  |
| max         |      10743 ms |      10299 ms | **108Py が -444 ms 改善**     |
| stdev       |   1005.183 ms |    979.670 ms | 108Py が改善                  |
| CV          |      0.143934 |      0.139973 | 108Py が改善                  |

つまり、**最大値とばらつきは改善**しましたが、**総時間・p95・p99 が悪化**しています。108Py は 107Py 比で `current_over_baseline=1.002199`、つまり約 **0.22% 遅い**です。

旧 v2 / 106Py との関係では、108Py もまだ十分速いです。旧 v2 `w8_j7` 比では約 **1.88% 改善**、106Py 比では約 **3.64% 改善**です。ただし、前回メモでは 107Py が旧 v2 比 **2.10% 改善**、106Py 比 **3.85% 改善**だったので、総合順位は **107Py > 108Py > 旧 v2 > 106Py** です。

## 108Py の良かった点

108Py は、狙いどおり **worst chunk の天井を下げました**。

107Py では `elapsed_max_ms=10743` が残っていましたが、108Py では `10299` まで下がっています。これは **-444 ms / 約 4.13% 改善**です。

また、`elapsed_vs_chunk_total_pearson` も `0.424220 → 0.392571` に下がっており、chunk_total 依存の重さは少し薄まっています。

`funcid_7_count` については、108Py の tertiary smoothing が効いた可能性が高いです。107Py では `funcid_7_count`, `cell_G_H_count`, `markrisk_h_ratio` が次の制御対象として挙がっていましたが、108Py 側では上位相関から `funcid_7_count` が消え、代わりに `cell_G_H_count`, `markrisk_h_ratio`, `funcid_17_count` が目立っています。

## 108Py の悪かった点

108Py は tail を下げた一方で、**p95/p99 の厚みを増やしてしまいました**。

特に slow run はまだ残っています。

| slow region | chunks |         avg |          max |
| ----------- | -----: | ----------: | -----------: |
| 1633-1642   |     10 | 9258.500 ms |      9752 ms |
| 1644-1650   |      7 | 9417.571 ms |      9805 ms |
| 1572-1578   |      7 | 9205.000 ms |      9961 ms |
| 78-83       |      6 | 9331.167 ms |      9758 ms |
| 66-70       |      5 | 9744.200 ms | **10299 ms** |

107Py で問題だった `1644-1654` や `1633-1636` の天井は下がっていますが、**chunk 66-70 が新しい最大領域として残りました**。

また、108Py の top correlation ではまだ `cell_G_H_count` が Spearman `0.379927`、`markrisk_h_ratio` が `0.377388`、`funcid_17_count` が `0.343329` で残っています。特に `funcid_17_count` は slow p95 で `all_mean=431.548 → slow_mean=743.699`、約 **1.72倍** まで増えており、次の本命候補です。
...
107Py:
  採用候補のまま
  総 elapsed 最速
  p95/p99 も 108Py より良い

108Py:
  不採用寄りの実験版
  max chunk は改善
  ただし総 elapsed が +28.493 sec 悪化
  p95/p99 も悪化
```

したがって、現時点の本線は **107Py を維持**でよいです。

## 次の 109Py 方針

108Py の結果から、`fid7` はかなり均された一方、次の支配要因が `funcid_17_count` と `G_H / markrisk_h_ratio` に移ったように見えます。

次は、108Py をそのまま強化するよりも、**107Py をベースに戻して、もっと軽い tertiary 制御を入れる**のがよいです。

```text
109Py-A:
  107Py broadmark をベース
  tertiary は funcid_17 のみ
  tail_window_boost は使わない、または 1 にする

109Py-B:
  107Py broadmark をベース
  tertiary は funcid_17 + cell_G_H
  fid7 は外すか、弱く扱う

狙い:
  108Py の max 改善だけ取り込みつつ、
  p95/p99 と総 elapsed の悪化を避ける
```

短縮確認なら、まず `66-70`, `78-83`, `1572-1578`, `1633-1656` 付近を中心に見るのがよいです。ここで 107Py より悪ければ full run は不要です。

詳細メモも作成しました。

[108Py_N22_p7_broadmarktail_analysis_memo.md](sandbox:/mnt/data/108Py_N22_p7_broadmarktail_analysis_memo.md)

一点だけ補足です。今回こちらで見えている raw GPU progress は `progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmark_gpu.tsv` 名でヘッダのみでした。分析は `report`, `summary`, `joined.tsv` から可能でしたが、次回の再現確認用には、ログに出ている `progress_N22_7_stream_funcid_reorder_v2_w8_j7_broadmarktail_gpu.tsv` も添付対象に入れると安全です。
...
VERSION_TAG:str="108 broadmark fid7/G_H tertiary tail smoothing auto w8_j7 stream GPU runner from 107 type-explicit"
...
# 108 BROAD+MARKDIST+TAIL TERTIARY REORDER:
#   Keep 107's broad funcid primary quota and mark-distance secondary quota.
#   Add a third in-cell split for the two observed tail signals from the 107Py
#   report: fid7 and cell_G_H.  This changes only stream ordering / .bin layout;
#   GPU kernel and DFS logic remain unchanged.
BROAD_MARKDIST_TAIL_REORDER_VERSION:str="v1"
BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON:str="108 broadmark + fid7/G_H tertiary tail smoothing"
BROAD_MARKDIST_TAIL_WINDOW_BOOST:int=2
BROAD_MARKDIST_TAIL_PHASE_SALT:int=17
...
####################################################
#
# 108 broadmark + fid7 / G_H tertiary tail smoothing reorder
#
####################################################

"""108 broadmark tail: tertiary tail bucket label。0=fid7, 1=cell_G_H, 2=rest。"""
...
"""108 broadmark tail: 25x3 subcell bin 名。"""
...
"""108 broadmark tail: reorder 済み bin 名。"""
...
"""108 broadmark tail: subcell index を 0..74 に pack。"""
def broad_markdist_tail_subcell_index(broad:int,risk:int,tail:int)->int:
  return (broad*5+risk)*3+tail

"""108 broadmark tail: 75 個の constellation buffer を作る。"""
...
"""108 broadmark tail: fid / broad / risk から tertiary tail bucket を返す。"""
def broad_markdist_tail_bucket(fid:int,broad:int,risk:int)->int:
  # fid7 は 107Py の slow correlation で単独 signal として見えていたため最優先で分離する。
  if fid==7:
    return 0
...
"""108 broadmark tail: 75 subcell stats。subcell=(broad*5+risk)*3+tail。"""
...
"""108 broadmark tail: summary stats。0=fid7,1=cell_G_H,2=markrisk_H,3=proxy_sum。"""
...
"""108 broadmark tail: chunk constellations から tail summary を作る。"""
...
"""108 broadmark tail progress header。107 broadmark header の後ろに tail smoothing 指標を追加。"""
...
"""108 broadmark tail: tail summary suffix。"""
def stream_broad_markdist_tail_summary_suffix(tail_stats:List[int],m:int)->str:
  ...
  if len(tail_stats)>3:
    proxy=tail_stats[3]
  return f"\t{f7}\t{gh}\t{hcnt}\t{proxy}\t{format_ratio_3(proxy,m)}"

"""108 broadmark tail reorder progress を TSV に追記する。"""
...
"""108 broadmark tail: 元 bin を broad x markdist x tail 75 subcell bin へ分配する。"""
def build_broad_markdist_tail_reorder_subcell_bins(N:int,fname:str,preset_queens:int,BLOCK:int,MAX_BLOCKS:int,gpu_log_level:int=0)->List[int]:
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if gpu_log_level>=2:
        ...
        print(f"[broadmarktail-reorder-subcell-chunk] chunk={chunk_index} m={m} fid7={tail_stats[0]} cell_G_H={tail_stats[1]} markrisk_H={tail_stats[2]} proxy_avg={format_ratio_3(tail_stats[3],m)}")
      chunk_index+=1
  ...
  if gpu_log_level>=1:
    ...
    while subcell<75:
      ...
      subcell+=1
    print(f"[broadmarktail-reorder-subcell-summary] N={N} records={total} fid7={counts[broad_markdist_tail_subcell_index(4,0,0)]+counts[broad_markdist_tail_subcell_index(4,1,0)]+counts[broad_markdist_tail_subcell_index(4,2,0)]+counts[broad_markdist_tail_subcell_index(4,3,0)]+counts[broad_markdist_tail_subcell_index(4,4,0)]} cell_G_H={counts[broad_markdist_tail_subcell_index(3,2,1)]}")

  return counts

"""108 broadmark tail: 75 subcell rem_counts から broad primary / markrisk secondary / tail tertiary quota を作る。"""
...
"""108 broadmark tail: one 107 cell の F7/GH/R parts を、tail が隣接しにくい順で interleave する。"""
def interleave_broad_markdist_tail_subparts(part_f7:List[Dict[str,int]],part_gh:List[Dict[str,int]],part_r:List[Dict[str,int]],m_target:int)->List[Dict[str,int]]:
  out:List[Dict[str,int]]=[]
  i7:int=0
  ig:int=0
  ir:int=0
...
"""108 broadmark tail: broad primary + markdistance secondary + fid7/G_H tertiary smoothing で reordered bin を作る。"""
def build_broad_markdist_tail_reordered_bin(
  ...
  if gpu_log_level>=1:
    print(f"[broadmarktail-reorder-build-config] N={N} records={total_records} steps={STEPS} output={reorder_fname} progress={progress_fname} param={funcid_reorder_param_tag()} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} tail_window_boost={BROAD_MARKDIST_TAIL_WINDOW_BOOST} reason={BROAD_MARKDIST_TAIL_REORDER_DEFAULT_REASON}")
  ...
  while total_remaining>0:
    ...
    if gpu_log_level>=2:
      print(f"[broadmarktail-reorder-build-chunk] chunk={chunk_index} off={off} m={m} B={stats[18+26]+stats[18+27]} A={stats[18+19]+stats[18+22]+stats[18+23]+stats[18+24]} C={stats[18+20]+stats[18+21]} G={stats[18+0]+stats[18+4]+stats[18+5]+stats[18+12]+stats[18+16]+stats[18+17]+stats[18+18]} O={m-(stats[18+26]+stats[18+27]+stats[18+19]+stats[18+22]+stats[18+23]+stats[18+24]+stats[18+20]+stats[18+21]+stats[18+0]+stats[18+4]+stats[18+5]+stats[18+12]+stats[18+16]+stats[18+17]+stats[18+18])} X={risk_stats[0]} T={risk_stats[1]} H={risk_stats[2]} M={risk_stats[3]} R={risk_stats[4]} fid7={tail_stats[0]} cell_G_H={tail_stats[1]} tail_proxy_avg={format_ratio_3(tail_stats[3],m)}")
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29:
      ...
      #   -g 22 22 32 484 1 0 7 28        # broad funcid primary + mark-distance secondary + fid7/G_H tertiary reorder simulation/build only
      #   -g 22 22 32 484 1 0 7 29        # broadmark + fid7/G_H tail-smoothed reordered GPU full run
      if argc >= 11:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==29:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        ...
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      total:int=exec_solutions_gpu_bin_stream_funcid_reorder(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"","broadmarktail_gpu")
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
