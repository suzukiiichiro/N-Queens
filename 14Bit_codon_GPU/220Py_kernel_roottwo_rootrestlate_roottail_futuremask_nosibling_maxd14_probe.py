...
# =============================================================================
# NQ_UPDATE_MEMO
# 220: 更新メモ: 219Py rootrestlate_prblockdirect は正当性OKだが217最速を上回らなかったため不採用。220Pyは217Py rootrestlate新最速基準を親に、MAXD14 kernelのみroot<=2判定をroot_after_second XORからroot_tail clear-lowbit判定へ置き換える再結合実験。rootrestlate / root_first late / futuremask / no-sibling / root one-two preroll は維持する。
...
219Pyは204Pyよりは速いですが、217Pyより `0.067秒` 遅いため**不採用**です。予定通り、220Pyは **217 + root_tail 判定** にしました。
...
```text id="f8vxfp"
bash -n 220Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 220Py...sh OK
source_rootrestlate_roottail          OK
source_split_tag split145 split220    OK
```

ファイルはこちらです。

* [220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_probe.py)
* [220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash id="eid8ek"
STATIC_ONLY=1 AUTO_BUILD=0 bash 220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_validate_full_once.sh

bash 220Py_kernel_roottwo_rootrestlate_roottail_futuremask_nosibling_maxd14_validate_full_once.sh
```
...
VERSION_TAG:str="220 roottwo-rootrestlate-roottail: 217 rootrestlate futuremask/no-sibling MAXD14 baseline with root_tail clear-lowbit predicate - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """220 MAXD14 nibble schedule plus futuremask/no-sibling, rootrestlate, and root_tail predicate."""
    ...
    # 220 rootrestlate-roottail:
    # Keep 217's rootrestlate shape: process the first root candidate before the
    # generic DFS loop, but delay cur_avail=root_rest until after the first child
    # viability checks.  220 changes only the root<=2 predicate.  Instead of
    # computing root_second and root_after_second with XOR, use a clear-lowbit
    # tail test over root_rest.  root_tail==0 selects exactly 1bit/2bit roots;
    # three-candidate and wider roots stay on the generic loop.
    root_rest:u32=cur_avail&(cur_avail-u32(1))
    root_tail:u32=root_rest&(root_rest-u32(1))
...
CHUNKSHAPE148_DEFAULT_REASON:str="220 rootrestlate-roottail reuses validated 217 scorestripe_v9 task order/cache; MAXD14 keeps rootrestlate and changes only root<=2 predicate to root_tail"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split220_roottwo_rootrestlate_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split220_roottwo_rootrestlate_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split220-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split220-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split220_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split220-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split220-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split220_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
