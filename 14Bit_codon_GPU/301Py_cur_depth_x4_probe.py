...
**300 = 375.613s — 不採用。** vs 296比で −21.942s (−6.204%) 悪化、292より遅い。

schedule_lo/hi → u64 統合によるブランチ除去は逆効果でした。`cur_depth<8` のブランチは実際はコストにならず、むしろ schedule_lo と schedule_hi を別々の u32 レジスタに持つことで並列アクセスが効いていたようです。u64 シフトに統合すると依存関係が増えた可能性があります。

**296 確定ベース継続。** 変数を「消す」「統合する」系の試みは軒並み逆効果（297, 298, 300）。次は別方向へ転換します。## 300 結果まとめと 301 の考え方

**300 不採用**: 375.613s — vs 296比 **−21.942s (−6.2%) 悪化**、292より遅い。schedule_lo/hi の u64 統合は schedule_lo と schedule_hi が独立した u32 レジスタで並列アクセスできていた利点を壊しました。
...
| Rev | 変更 | 結果 |
|---|---|---|
| **296** | **乗算(save_sp*2)→カウンタ** | **+9.1s 改善** ✓ |
| 297 | 変数削除(save_sp) | −9.0s 悪化 |
| 298 | 変数削除(next_depth) | −62.8s 悪化 |
| 299 | K=64 | flat |
| 300 | 2u32→1u64 統合 | −21.9s 悪化 |

## 301 の着眼点

**296 の成功パターンを再現**：乗算命令の排除。

hot DFS ループで `cur_depth*4` と `(cur_depth-8)*4` がまだ残っています。296 が `save_sp*2` を `stack_ptr+=2` カウンタで置き換えたのと全く同じ手法で、`cur_depth_x4` カウンタを並列維持します：

```python
# 301の変更:
cur_depth_x4:int=0          # init
...
cur_depth_x4+=4             # push後（descend）
...
cur_depth_x4=cur_depth<<2   # pop後（シフトのみ）

# hot decode (乗算ゼロ):
nibble_op=(schedule_lo>>u32(cur_depth_x4))&u32(15)      # depth<8
nibble_op=(schedule_hi>>u32(cur_depth_x4-32))&u32(15)   # depth>=8
```

```bash
STATIC_ONLY=1 bash 301Py_cur_depth_x4_validate_N21_full_once.sh
bash 301Py_cur_depth_x4_validate_N21_full_once.sh
```
"""
...
VERSION_TAG:str="301 cur-depth-x4 probe: parent 300 schedule-u64 (rejected, -21.9s regression); reverted to 296 (353.671s) as base; maintains cur_depth_x4:int=cur_depth*4 as a running counter alongside cur_depth to eliminate the cur_depth*4 and (cur_depth-8)*4 multiplies in the hot DFS loop nibble_op decode; analogous to 296 which eliminated save_sp*2 multiply; cur_depth_x4 init=0, +4 on descend, =cur_depth<<2 on pop; schedule_lo/hi restored from 296 (300 showed u64 merge is counterproductive); stack_ptr/save_sp/next_depth unchanged from 296; K_PER_THREAD_MAXD14=32; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
