...
# =============================================================================
# 322 N=21 full validation harness (manualreview-reconverge-loopback)
# Parent: 321 debugbuild-lineinfo-attempt. 321's diagnostic -debug build
#   crashed at kernel launch with CUDA_ERROR_INVALID_PTX (Codon's -debug
#   codegen produces PTX the driver's JIT rejects outright) -- not just a
#   risk of divergent SASS as flagged, but a hard failure. Neither 320's
#   nor 321's own N=21 -release run has been executed; 319 (455.116s,
#   314666222712) remains the most recent confirmed timing baseline.
#
# CONCLUSION on tooling: per-line (.py file:line) attribution for Stall
#   Branch Resolving is unavailable in this Codon+ncu toolchain. Every
#   avenue has now been exhausted: `--set`/`--section SourceCounters`
#   (318, eventually succeeded but kernel-aggregate only), `--page
#   source` (319, succeeded and gave per-SASS-instruction attribution,
#   but Source column is disassembly text only, no source lines), and
#   `-debug` build for line info (321, crashes at launch). The user
#   chose to close this track and move to manual source review, using
#   319's address-level finding as the map.
#
# MANUAL REVIEW RESULT (this revision, no source changes): the kernel
#   source for kernel_dfs_iter_gpu_maxd14 was read directly. The main DFS
#   backtracking loop (`while True:`, starting line ~783) has exactly the
#   structural signature that would produce 319's SASS pattern:
#     - FIVE separate `continue` statements (dead-end/backtrack paths:
#       cur_avail==0 backtrack-pop; nf==0; future_check dead end;
#       terminal_depth reached; child_jmark forced dead end)
#     - PLUS one implicit fallthrough (push+descend) that loops back to
#       the same point
#   All six paths reconverge at the single loop head (`if
#   cur_avail==u32(0):`). Different warp lanes take different paths on
#   every iteration depending on each lane's own board state -- textbook
#   SIMT divergence -- and the compiler must insert a BSYNC to
#   reconverge all lanes before the single backward branch to the loop
#   head can issue. This matches 319's finding exactly: one dominant
#   unconditional BRA (50.8% of all stall_branch_resolving) sitting
#   right after a BSYNC, while the actual divergent conditional branches
#   (nf==0 checks etc.) individually rank far lower.
#
# This closes the investigation opened around 312 with a source-level
#   (if not exact-line-verified) answer: the cost is not any single
#   conditional branch, but the reconvergence overhead of the loop's
#   *shared* back-edge, driven by unpredictable per-lane DFS depth/
#   branching (subtree size varies per constellation). This is the same
#   root cause already identified via Avg Active Threads Per Warp (~15%)
#   back in 292/316 -- 319/322 now show precisely *where* in the compiled
#   code that cost is paid, without new root-cause information.
#
# NO SOURCE CHANGE IS PROPOSED IN THIS REVISION. Given the rev-189
#   regression precedent (+108% slowdown from an earlier inner-loop
#   restructuring attempt) and the fact this loop has already been
#   through many careful iterations (K-batching/292, stack merge/295,
#   stack_ptr/296, save_sp elim/297, next_depth elim/298...), any attempt
#   to restructure this reconvergence point is HIGH RISK and warrants a
#   dedicated, carefully-scoped future session with its own incremental
#   validation plan -- not an incidental change here. This revision is
#   documentation/analysis only: ZERO changes to kernel logic (variant=2,
#   K=48 unchanged).
...
#           to again land near ~454-455s under the accepted clock state.
...
SRC=${SRC:-./322Py_manualreview_reconverge_loopback.py}
CAND=${CAND:-./322Py_manualreview_reconverge_loopback}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/322Py_manualreview_reconverge_loopback_N21_full.lock}
...
# NOTE: neither 320's nor 321's own N=21 full run has been executed yet
# (320 went straight to `codon build --help`; 321's -debug build crashed
# with CUDA_ERROR_INVALID_PTX before any N=21 run was attempted). No
# fabricated BASELINE_320_*/BASELINE_321_* values are added here; 319
# remains the most recent confirmed timing baseline until this 322 run
# (or a future one) produces a newer one.
...
LOGDIR="${LOG_ROOT%/}/322Py_manualreview_reconverge_loopback_logs_N21_full_once_${TS}"
...
echo "[start] 322 manualreview-reconverge-loopback validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 322 manualreview-reconverge-loopback SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '322 manualreview-reconverge-loopback' "$SRC"; then
  printf 'source_version_tag	322 manualreview-reconverge-loopback	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	322 manualreview-reconverge-loopback	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 322 manualreview-reconverge-loopback mode31 split145"
...
echo "[validation-ok] 322 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); 321's -debug build crashed with CUDA_ERROR_INVALID_PTX, closing the tooling-based per-line-attribution track; manual review of kernel_dfs_iter_gpu_maxd14's main while-True DFS loop (~line 783) found five continue statements plus one fallthrough all reconverging at the loop head, matching 319's SASS finding (one dominant BRA next to BSYNC = 50.8% of stall_branch_resolving); NO source change proposed here per the header comment -- documentation/analysis only"
