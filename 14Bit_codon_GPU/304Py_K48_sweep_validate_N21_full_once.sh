...
# =============================================================================
# 304 N=21 full validation harness (K48-sweep probe)
...
# EXPECTED_CHUNKS=3, K=32 (unchanged from 296).
...
SRC=${SRC:-./304Py_K48_sweep_probe.py}
CAND=${CAND:-./304Py_K48_sweep_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/304Py_K48_sweep_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/304Py_K48_sweep_logs_N21_full_once_${TS}"
...
echo "[start] 304 K48-sweep validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 304 K48-sweep SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '304 K48-sweep probe' "$SRC"; then
  printf 'source_version_tag	304 K48-sweep probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	304 K48-sweep probe	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 304 K48-sweep mode31 split145"
...
echo "[validation-ok] 304 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows)"
