...
# =============================================================================
# NQ_UPDATE_MEMO
# 211: 更新メモ: 210Py rootafterandnot は正当性OKで206より速いが204最速を上回らなかったため不採用。211Pyは204/206相当のroot_after_second XOR predicateへ戻し、MAXD14 kernelのみroot_first lowbit抽出をroot<=2 predicate成立後へ遅延する微差実験。root_rest clear-lowbit / futuremask / no-sibling / root one-two preroll は維持する。
...
210Pyは206/209/197より速く、かなり良い結果でした。ただし204Pyを `0.079秒` 上回れなかったため、**正当性OKだが新最速としては不採用**にします。

211Pyは、210の `root_after_second = root_rest & (~root_second)` を撤回し、204/206相当のXOR predicateへ戻したうえで、MAXD14 kernel内だけの小差分にしました。
...
3bit以上rootではgeneric loopへ戻るため、その場合はpreludeで `root_first` を作らない、という微差実験です。

手元確認は以下です。

```text
bash -n 211Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 211Py...sh OK
source_rootfirstlate                  OK
source_split_tag split145 split211    OK
```

ファイルはこちらです。

* [211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_probe.py)
* [211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 211Py_kernel_roottwo_rootfirstlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 211 rootfirstlate memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="211 roottwo-rootfirstlate: 204 rootclearrest futuremask/no-sibling MAXD14 baseline with root_first computed only inside root<=2 preroll - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    # correct but slower.  211 keeps 204's clear-lowbit root_rest construction and restores
...
CHUNKSHAPE148_DEFAULT_REASON:str="211 rootfirstlate reuses validated 204 scorestripe_v9 task order/cache; MAXD14 delays root_first lowbit extraction until root<=2 predicate passes"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split211_roottwo_rootfirstlate_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split211_roottwo_rootfirstlate_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split211-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split211-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split211_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split211-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split211-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split211_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
