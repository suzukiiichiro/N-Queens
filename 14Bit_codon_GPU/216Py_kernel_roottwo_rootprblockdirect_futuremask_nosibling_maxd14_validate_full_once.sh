...
# =============================================================================
# NQ_UPDATE_MEMO
# 216: 更新メモ: 215Py rootbitdirect は正当性OKだが204/211より遅いため不採用。216Pyは211Py rootfirstlate形へ戻し、MAXD14 kernelのみroot-preroll内の pr_block_code を schedule_lo&7 から直接取得する微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 216 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 216 returns to near-best 211 late-root_first shape because 215 rootbitdirect did not beat 204/211.
# 198/199/200/201/202/203/205/207/208/209/210/212/213/214/215 were correct but not adopted.
#
...
# schedule_words=0, and 208 local stack bytes/thread. The rollback is
...
# 204 clear-lowbit root_rest plus root_after_second=root_rest^root_second scalar predicate, with root_first computed only inside the preroll branch and pr_block_code read directly from schedule_lo&7.
# Host task order, dispatch, cache, progress verification, and README-managed
# history remain unchanged.
#
# Shell policy:
#   - STATIC_ONLY=1 does not take the full-run lock.
#   - source static-check failure prints the summary before exiting.
#   - rootprblockdirect source check is whitespace/comment robust.
SRC=${SRC:-./216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 216 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/216Py_kernel_roottwo_rootprblockdirect_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 215Py source
#   has a harmless header/string/layout difference. This 215 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root_rest clear-lowbit + root_after_second XOR scalar if + late root_first plus pr_block_code=schedule_lo&7	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
root_compact=''.join(root_block.split())
generic_block=s[gen_pos if gen_pos >= 0 else 0:]
generic_compact=''.join(generic_block.split())
...
checks = {
    ...
    'root_first_lowbit_shape_present': 'root_first:u32=cur_avail&(u32(0)-cur_avail)' in root_compact,
    'pr_bit_alias_present': 'pr_bit:u32=root_first' in root_compact,
    'pr_block_code_direct_present': 'pr_block_code:u32=schedule_lo&u32(7)' in root_compact,
    'pr_block_code_from_nibble_absent': 'pr_block_code:u32=pr_nibble_op&u32(7)' not in root_compact,
    'pr_nibble_for_future_present': 'pr_nibble_op:u32=schedule_lo&u32(15)' in root_compact,
    ...
    # 216 keeps 204/197 save payload; 203 depth0-save is not adopted.
    ...
    # 216 keeps 204/197 payload source; 202 root_second payload is not adopted.
    ...
    # 216 keeps 204/197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 216 keeps 204/197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_rootprblockdirect_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootprblockdirect	present_direct_root_pr_block_code	present_direct_root_pr_block_code	OK
' >> "$SUMMARY"
else
  printf 'source_rootprblockdirect	present_direct_root_pr_block_code	missing_or_old_rootprblockdirect_shape	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split216'}
bad=sorted(tags-allowed)
if 'split216' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split216 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split216 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 216 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 216 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootprblockdirect static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 216 roottwo-rootprblockdirect futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 216 roottwo-rootprblockdirect futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
