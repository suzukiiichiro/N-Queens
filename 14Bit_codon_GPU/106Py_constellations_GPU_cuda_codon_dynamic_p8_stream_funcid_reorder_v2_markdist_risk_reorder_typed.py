...
106Py update:
  ...
  Risk key is fid, gap=(mark2-mark1), d1=(mark1-row); d2 is not used because d2=d1+gap.
...
はい、106Py をベースに **型明示を強化した版** を作成しました。
実行中の GPU full run には影響しないよう、元ファイルは上書きせず **別名ファイル** にしています。

修正版はこちらです。

[106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder_typed.py](sandbox:/mnt/data/106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder_typed.py)

差分はこちらです。

[106_to_106_typed_annotations.patch](sandbox:/mnt/data/106_to_106_typed_annotations.patch)

主な修正点です。

```text
1. @gpu.kernel の引数を Ptr[int] / Ptr[u64] / Ptr[u8] で明示
2. kernel_dfs_iter_gpu() に ->None を追加
3. TaskSoA.__init__() に ->None を追加
4. dfs_iter() の meta / blockK / blockL / board_mask を明示
5. build_soa_for_range() の N:int を追加
6. exec_solutions_gpu_chunk_profile() の戻り型を Tuple[int,List[int],List[int],str,int] に明示
7. file handle helper は T:type generic で型を受ける形に変更
8. raw read buffer / datetime 計測変数 / kernel 内 hot path 変数の一部を明示
9. module-level の use_visited_prune を bool 明示
```

Codon の `gpu.raw()` は `Ptr` を返す仕様なので、GPU kernel 側の raw array 引数は `Ptr[int]`, `Ptr[u64]`, `Ptr[u8]` に寄せました。([docs.exaloop.io][1])
また、file handle の実体型をこちらで勝手に固定すると危ないため、`append_one_constellation_from_file()` と `fill_constellation_buffer_from_file()` は Codon の generic type parameter 形式で `f:T, T:type` としています。Codon 公式ドキュメントでも generic function の型パラメータはこの形で扱えると説明されています。([docs.exaloop.io][2])

確認として、Codon 固有の `@par` 行だけを一時的に除外して Python AST で構文チェックし、以下を確認しました。

```text
OK  syntax parse
OK  function / method parameter annotations
OK  function / method return annotations
OK  module-level unannotated assignment count = 0
```

ビルド確認は、こちらの環境に `codon` がないため未実行です。そちらでは GPU 実行が終わった後に、まず build だけ通すのがよいです。

```bash
cd /home/suzuki/Github/N-Queens/13Bit_codon

codon build -release \
  106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder_typed.py
```

build が通ったら、まず 26 の reuse/sim 側で確認できます。

```bash
./106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder_typed \
  -g 22 22 32 484 1 0 7 26
```
...
```bash
./106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder_typed \
  -g 22 22 32 484 1 0 7 27
```
完了時に見るべき行はこれです。

[markdist-risk-reorder-gpu-done] ...
22:     2691008701644                0     ...    ok

特に確認したい出力ファイルはこれです。

progress_N22_7_stream_funcid_reorder_v2_w8_j7_markdistrisk_gpu.tsv
106Py_N22_p7_markdistrisk_gpu_full.log


[1]: https://docs.exaloop.io/parallel/gpu/ "GPU Programming | Codon"
[2]: https://docs.exaloop.io/language/generics/?utm_source=chatgpt.com "Generics | Codon - Exaloop"


添付 sim を確認しました。結論として、**GPU Full はこのまま継続で正解**です。

`bench_mode=26` は正常に完了しており、元 records と reorder 後 records が一致しています。
...
ログ上も `markdist-risk-reorder-build-summary` と `markdist-risk-reorder-sim-only` が `valid=1` で通っています。
106Py の mode 26/27 は、ちょうどこの `markdist-risk-reorder-sim-only` と `markdist-risk-reorder-gpu` 用に入っているので、現在の GPU Full は流れとして問題ありません。

## sim の判定

今回の 106Py は、105Py の結論どおり `fid × gap × d1` を risk score にして、`X/T/H/M/O` の 5 bucket に分けています。`d2` は `d2 = d1 + gap` なので使っていません。
進捗 TSV には `markrisk_x_count` から `markrisk_score_max` までの exact risk 統計列が追加されています。
...
full chunk、つまり最後の端数 chunk を除いた 1,854 chunks では、件数配分はかなりきれいです。

```text
X count: 251〜252 / chunk
T count: 191〜192 / chunk
H count: 588〜589 / chunk
M count: 6846〜6847 / chunk
O count: 7608〜7612 / chunk
```

これは **exact risk bucket の均等分散としては成功**です。
特に `X/T/H` の極端 tail 系が chunk ごとにほぼ同数になっているので、106Py の第一目的は達成しています。

一方で、`markrisk_score_avg` はまだ揺れています。
...
つまり、**bucket 件数は均等だが、bucket 内の exact key 強度までは完全には均されていない**です。これは想定内です。今回の 106Py は 5 bucket 方式なので、たとえば同じ `X` でも `score=368385` と `score=229048` が同格に扱われます。

## GPU Full で特に見るべき chunk

sim 上で高 risk が残っていそうな範囲は以下です。

```text
markrisk_score_avg > 14000 の連続区間:
  chunk 145-148
  chunk 528-550
  chunk 555-568
  chunk 587-589
...
また、最大 exact score `368385` を含む chunk はこの範囲に偏りがあります。
...
GPU Full の `elapsed_ms` がこのあたりでまだ山を作るなら、次の 107Py では **X bucket をさらに細分化**する価値があります。

## 旧 funcid reorder v2 との比較

添付の旧 `progress_N22_7_stream_funcid_reorder_v2_w32_j7_sim.tsv` は、 broad funcid bucket の均等化は非常にきれいでした。

```text
risky_a: 2926〜2927 / chunk
risky_b: 2069〜2070 / chunk
risky_c:  913〜914  / chunk
good   : 7253〜7257 / chunk
other  : 2323〜2324 / chunk
```

一方、106Py は exact risk を優先したため、broad funcid bucket はかなり崩れます。

```text
106Py broad bucket range:
  risky_a: 0〜9009
  risky_b: 0〜7608
  risky_c: 0〜7552
  good   : 778〜13650
...
```text
旧方式:
  broad funcid balance は非常に良い
  ただし exact tail islands は見えていない

106Py:
  exact risk bucket balance は非常に良い
  ただし broad funcid balance は犠牲になっている
```

したがって GPU Full の判定は、単純な total time だけでなく、**chunk elapsed の山が exact risk に従うのか、broad funcid imbalance に従うのか**を見る必要があります。

## 次の判断

GPU Full の結果が良ければ、106Py は採用候補です。

GPU Full で tail が残る場合は、107Py はこの方針がよいです。

```text
107Py candidate:
  1. X bucket を score 別に分割
       X0: score >= 300000
       X1: score >= 250000
       X2: score >= 200000

  2. exact score quota を chunk ごとに均す

  3. broad funcid bucket も secondary quota として軽く残す
       exact risk primary
       funcid balance secondary
```

今回の sim だけで見ると、**106Py は実験する価値あり**です。GPU Full は止めずに最後まで取ってください。

集計ファイルも作っておきました。

[106Py_sim_analysis_summary.tsv](sandbox:/mnt/data/106Py_sim_analysis_summary.tsv)
[106Py_markdist_risk_top50_chunks.tsv](sandbox:/mnt/data/106Py_markdist_risk_top50_chunks.tsv)
[106Py_markdist_risk_runs.tsv](sandbox:/mnt/data/106Py_markdist_risk_runs.tsv)

なお、添付されていた typed 版ソースは、先頭 docstring に過去回答文が残っていました。実行上は docstring なので致命的ではありませんが、後で使うなら clean header 版の方が安全です。

[106Py typed clean header 版](sandbox:/mnt/data/106Py_constellations_GPU_cuda_codon_dynamic_p8_stream_funcid_reorder_v2_markdist_risk_reorder_typed_clean.py)
[clean header 差分](sandbox:/mnt/data/106_typed_to_typed_clean_header.patch)


はい、**GPU Full 完了後に集計スクリプトを実行して、その結果を添付していただくのが最善**です。

106Py の `bench_mode=27` は、risk-reordered bin を GPU 実行し、progress suffix として `markdistrisk_gpu` を使う実装になっています。
一方、exact risk の `markrisk_x_count` / `markrisk_score_avg` などは sim/build 側 TSV に入っているので、集計スクリプトでは **GPU Full の elapsed/chunk_total TSV と sim TSV を chunk 番号で join** して解析します。

スクリプトはこちらです。

[aggregate_106py_markdistrisk_gpu.py](sandbox:/mnt/data/aggregate_106py_markdistrisk_gpu.py)

GPU Full が終わったら、同じディレクトリで以下を実行してください。

```bash
cd /home/suzuki/Github/N-Queens/13Bit_codon

python3 aggregate_106py_markdistrisk_gpu.py \
  --gpu progress_N22_7_stream_funcid_reorder_v2_w8_j7_markdistrisk_gpu.tsv \
  --sim progress_N22_7_stream_markdist_risk_reorder_v1_w8_j7_sim.tsv \
  --old-sim progress_N22_7_stream_funcid_reorder_v2_w32_j7_sim.tsv \
  --log 106Py_N22_p7_markdistrisk_gpu_full.log \
  --out 106Py_N22_p7_markdistrisk
```

出力されるファイルは以下です。

```text
106Py_N22_p7_markdistrisk_report.md
106Py_N22_p7_markdistrisk_summary.tsv
106Py_N22_p7_markdistrisk_top_elapsed.tsv
106Py_N22_p7_markdistrisk_risk_correlation.tsv
106Py_N22_p7_markdistrisk_elapsed_runs.tsv
106Py_N22_p7_markdistrisk_bucket_balance.tsv
106Py_N22_p7_markdistrisk_joined.tsv
```

添付していただきたい優先順は以下です。

```text
1. 106Py_N22_p7_markdistrisk_report.md
2. 106Py_N22_p7_markdistrisk_summary.tsv
3. 106Py_N22_p7_markdistrisk_top_elapsed.tsv
4. 106Py_N22_p7_markdistrisk_risk_correlation.tsv
5. 106Py_N22_p7_markdistrisk_elapsed_runs.tsv
6. progress_N22_7_stream_funcid_reorder_v2_w8_j7_markdistrisk_gpu.tsv
7. 106Py_N22_p7_markdistrisk_gpu_full.log
```

`joined.tsv` は全 chunk の結合表なので、サイズが大きければ省略して大丈夫です。こちらで追加確認が必要になった場合だけ添付してください。

このスクリプトで見たいポイントは主に以下です。

```text
status:
  final_gpu_total == 2691008701644 か

elapsed distribution:
  max / p95 / p99 / CV が旧方式より改善したか

top elapsed chunks:
  遅い chunk がどこに残ったか

risk correlation:
  GPU elapsed が markrisk_score_avg / score_max / X count に従うか
  それとも broad funcid imbalance に従うか

elapsed runs:
  遅い chunk が連続区間として残っているか
```

特に今回の判断で重要なのは、**106Py の exact risk balance は成功しているが、GPU 実測の山が exact risk 由来なのか、funcid broad imbalance 由来なのか**です。105Py の結論では tail の説明変数は `fid × gap × d1` が最小セットで、`d2` は `d1 + gap` なので不要、という整理でした。



結果確認しました。
結論から言うと、**106Py は正解性は完全に OK ですが、速度改善版としては採用しない方がよい**です。今回は「診断としては成功、reorder 方針としては要修正」です。
...
集計スクリプト側でも `final_gpu_total == expected_total`、`status=ok` になっています。

したがって、**106Py の bin reorder 自体に欠落・重複・集計破綻はありません**。ここは合格です。
...
```text
elapsed_sum_sec = 13,473.847 sec
mean            = 7,263.529 ms / chunk
p95             = 8,792.800 ms
p99             = 9,497.600 ms
max             = 10,706.000 ms
CV              = 0.122540
...
過去メモ内の 84Py GPU run では N=22 が `3:18:42.963 ok` なので、今回の `3:44:49.084` はかなり遅いです。
同一条件の完全な apples-to-apples 比較ではありませんが、少なくとも **106Py risk reorder は高速化方向には出ていない** と判断してよいです。

## 3. exact risk の均等化は成功しています

ここは 106Py の良いところです。bucket balance を見ると、mark-distance risk bucket は非常にきれいに揃っています。

```text
markrisk_x_count CV = 0.016872
markrisk_t_count CV = 0.016877
markrisk_h_count CV = 0.016804
markrisk_m_count CV = 0.016802
markrisk_o_count CV = 0.016813
```

一方で broad funcid bucket は大きく崩れています。

```text
risky_a_count CV = 0.851146
risky_b_count CV = 1.558922
risky_c_count CV = 1.589365
good_count    CV = 0.332351
other_count   CV = 0.965774
```

つまり、**exact risk を均すことには成功したが、その代償として funcid broad balance をかなり壊している**、という状態です。

## 4. GPU elapsed は markdist risk に従っていません

ここが一番重要です。

elapsed との相関を見ると、`markrisk_score_avg` はむしろ負方向です。

```text
good_count          pearson =  0.303396, spearman =  0.341162
depth_avg           pearson =  0.290933, spearman =  0.254044
markrisk_score_avg  pearson = -0.186186, spearman = -0.217237
markrisk_x_count    pearson =  0.023454, spearman =  0.147757
markrisk_t_count    pearson =  0.025121, spearman =  0.125714
```

したがって今回の Full run では、**GPU elapsed の山は `fid × gap × d1` の risk score では説明しきれていません**。

特に top elapsed chunk を見ると、上位 chunk の `markrisk_x_count / t_count / h_count` はほぼ固定です。

```text
chunk 176: X=251, T=191, H=588, elapsed=10706 ms
chunk 174: X=251, T=191, H=588, elapsed=10501 ms
chunk 168: X=251, T=191, H=588, elapsed=10445 ms
chunk 170: X=251, T=191, H=588, elapsed=10390 ms
chunk 166: X=251, T=191, H=588, elapsed=10352 ms
```

これは **106Py の exact-risk quota が効きすぎて、上位 chunk 間で risk count が同じになっている**ことを意味します。にもかかわらず elapsed に差が出ているので、残っている山の主因は別です。

## 5. 残った最大の山は chunk 166〜181

slow run の最大連続区間はここです。

```text
chunk 166-181
chunks          = 16
elapsed_sum_ms  = 156,484
elapsed_avg_ms  = 9,780.250
elapsed_max_ms  = 10,706.000
```

この区間の `markrisk_score_avg_mean` は `11819.978750` で、全体平均 `11701.512075` と比べて極端に高いわけではありません。

つまり、**166〜181 の山は exact mark-distance risk の山ではない**です。

私が添付ログを再集計したところ、GPU elapsed と `chunk_total` の相関はかなり強めでした。

```text
elapsed_ms vs chunk_total:
  pearson  ≒ 0.565
  spearman ≒ 0.519
```

また p95 以上の slow chunk は、全体平均より `chunk_total` が約 29% 大きいです。

```text
all chunks chunk_total mean  ≒ 1.451e9
slow chunks chunk_total mean ≒ 1.875e9
ratio                       ≒ 1.293
```

`chunk_total` は実行後にしか分からない値なので直接 reorder key には使えませんが、少なくとも **今回の elapsed は exact risk bucket 数よりも、実際の探索展開量・解数規模側に強く引っ張られている**と見てよいです。
...
exact mark-distance risk bucket balance:
  OK
...
結論:
  106Py は最終候補ではなく、107Py のための診断版として扱う
```

105Py の時点では `fid × gap × d1` は heavy tail の説明変数として有効でした。106Py の結果で分かったのは、**その exact tail を均等に配っても、Full GPU elapsed の主因はそれだけではなかった**ということです。

## 7. 次の 107Py 方針

107Py は、106Py のように exact risk を primary にしすぎるのではなく、**旧 funcid reorder v2 の broad balance を primary に戻す**べきです。
...
107Py:
  ...
  depth / score_avg / predicted chunk_total proxy を tertiary
```
...
reorder unit:
  broad_bucket × markrisk_bucket の 5×5 quota
```
...
```text
broad funcid を絶対に崩さない
その中で markrisk X/T/H/M/O を薄く散らす
```
...
集計スクリプトには次を追加した方がよいです。

```text
elapsed_ms vs chunk_total correlation
slow p95 chunk の feature mean / all mean ratio
slow run ごとの chunk_total_avg
broad bucket × markrisk bucket の 25-cell balance
```

今回の結果を見る限り、次に見るべき主指標は `markrisk_score_avg` ではなく、
...
depth_avg
score_avg
broad funcid imbalance
```

です。

次は **107Py: broad-preserving markdist-secondary reorder** に進むのがよいです。
...
VERSION_TAG:str="106 exact mark-distance risk reorder auto w8_j7 stream funcid gap_d1 risk balanced GPU runner from 105 type-explicit"
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode>=8:
      ...
      preset_queens_arg=5
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27:
  ...
  if bench_mode==14 or bench_mode==15 or bench_mode==16 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27:
