...
# =============================================================================
# 316 N=21 full validation harness (env-accept-ncu-prep)
# Parent: 315 telemetry-fieldname-fix. `nvidia-smi -q -d POWER` ruled out a
#   power cap: Current/Requested/Default Power Limit were all 300.00W
#   (Min=100.00W, Max=300.00W), and 312's telemetry showed peak compute power
#   draw of only ~95.33W (~32% of the 300W budget) -- nowhere near the limit.
#   `sudo nvidia-persistenced` failed to initialize (a symptom commonly seen
#   in virtualized/restricted GPU environments where the persistence daemon
#   cannot get the host-level access it needs). `sudo nvidia-smi
#   --auto-boost-default=0` reported "not supported for GPU", consistent
#   with the earlier "Auto Boost: N/A" finding.
#
# Investigation summary (311-316), all avenues now exhausted on the guest
#   side:
#     - code regression                  -> ruled out (311: byte-identical
#                                            to 304, still slow)
#     - thermal throttling                -> ruled out (312: cool temps,
#                                            no throttle reason bits)
#     - guest -lgc clock lock             -> ruled out (-rgc had no effect)
#     - -ac application clock override    -> ruled out (Applications Clocks
#                                            == Default Applications Clocks)
#     - power limit cap                   -> ruled out (this revision:
#                                            limit == default, well under
#                                            budget)
#     - Auto Boost setting                -> not applicable to this GPU gen
#     - persistence mode                  -> daemon fails to initialize,
#                                            suggestive of a virtualized/
#                                            restricted environment
#   The most consistent remaining explanation is a host/hypervisor-level
#   clock ceiling on a virtualized/shared GPU instance, invisible to and
#   uncontrollable by any guest-side nvidia-smi command.
#
# Decision (per the pragmatic stance set out in 314): the environment
#   investigation is now formally closed. ~454s under the current 1320MHz
#   SM clock is adopted as this session's practical working baseline.
#   Stall Branch Resolving isolation (handoff plan priority #2) resumes:
#   the recommended next command -- to be run manually, NOT invoked by this
#   script -- is a single-launch, SourceCounters-only ncu profile:
#     ncu --launch-count 1 --set SourceCounters -o 316_sourcecounters_chunk0 \
#         ./316Py_env_accept_ncu_prep -g 21 21 32 484 1 0 7 31 8 7 0 0 1 2
#   Absolute timing comparison to 304's profile (recorded at 1710MHz) is no
#   longer valid, but ncu's Stall Wait / Stall Branch Resolving percentages
#   reflect architectural properties (dependency-chain length, branch
#   execution frequency) that should remain informative under the current
#   clock state for identifying which branch is responsible.
#
# This revision: ZERO source changes from 315 (variant=2, K=48 unchanged).
#   GPU telemetry capture is left enabled (low cost, useful passive
#   monitoring) but is no longer a blocking/primary concern of this script.
...
#           to again land near ~454s under the current (accepted) clock
#           state; gpu_clock_cap_check/gpu_power_cap_check should both now
#           run against real data given 315's field-name fix.
...
SRC=${SRC:-./316Py_env_accept_ncu_prep.py}
CAND=${CAND:-./316Py_env_accept_ncu_prep}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/316Py_env_accept_ncu_prep_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/316Py_env_accept_ncu_prep_logs_N21_full_once_${TS}"
...
echo "[start] 316 env-accept-ncu-prep validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 316 env-accept-ncu-prep SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '316 env-accept-ncu-prep' "$SRC"; then
  printf 'source_version_tag	316 env-accept-ncu-prep	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	316 env-accept-ncu-prep	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 316 env-accept-ncu-prep mode31 split145"
...
echo "[validation-ok] 316 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); environment investigation closed, ~454s accepted as practical baseline; next step is manual ncu SourceCounters profiling per the header comment"
