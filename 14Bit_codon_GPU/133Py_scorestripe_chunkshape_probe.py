...
[133Py_scorestripe_chunkshape_probe.py](sandbox:/mnt/data/133Py_scorestripe_chunkshape_probe.py)
[133Py1-6.sh](sandbox:/mnt/data/133Py1-6.sh)
...
133Py:
  132 から分岐
  scorestripe_v3_lanephase
  kernel / DFS / generic-only path は変更なし
  score bucket の percentile key は 130/131/132 v2 を継承
  bucket 内だけ 16-lane phase rotation に変更

LOGDIR:
  133Py_logs_N21_validation_${TS}
...
cache / label:
  split133
  chunkshape133
  CHUNKSHAPE133_REORDER_VERSION="scorestripe_v3_lanephase"
```

実行は以下です。

```bash
codon build -release 133Py_scorestripe_chunkshape_probe.py
./133Py1-6.sh
```
...
```bash
RUN_FULL=0 ./133Py1-6.sh
```

こちらでは Codon 実機ビルドまでは実行していません。ファイル生成、133 ラベル化、ログディレクトリ命名、chunkshape133 の参照整合まで確認しています。
...
VERSION_TAG:str="133 scorestripe v3 lanephase probe - mode30/mode31 generic kernel, host-side score-bucket lane-phase shaping"
...
####################################################################################################
# 133 reorder chunkshape harness
#
...
####################################################################################################
# 133 score-aware chunk-shape layer
# 133 is a minimal host-side experiment over the validated 130/131/132 scorestripe_v2 baseline.
#
# Safety boundary:
...
#
# scorestripe_v3_lanephase keeps the v2 percentile score bucket calculation, then changes only the
# bucket-internal extraction order:
#   v2: bucket[b] FIFO in the proven 128 cross-stripe order
#   v3: bucket[b][lane0..lane15], with output chunk / bucket dependent lane start rotation
####################################################################################################

CHUNKSHAPE133_REORDER_VERSION:str="scorestripe_v3_lanephase"
CHUNKSHAPE133_DEFAULT_REASON:str="host-side percentile score-aware quota stripe with deterministic 16-lane bucket phase rotation; generic kernel unchanged"


def chunkshape133_reorder_output_fname(N:int,preset_queens:int,block:int=32,max_blocks:int=484)->str:
  return f"constellations_N{N}_{preset_queens}_chunkshape133_{CHUNKSHAPE133_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(block,max_blocks)}.bin"


CHUNKSHAPE133_SCORE_KEY_MAX:int=32767


def chunkshape133_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  133 score key for host-side shaping only.
  ...
  v3 behavior uses the low 4 bits of the deterministic tie component as a bucket-internal lane.
  ...
  # Deterministic tie lane.  Percentile buckets use the full key; v3 bucket lanes use key&15.
  tie:int=(global_idx*13 + soa.ijkl_arr[idx]*7 + fid*5 + pc*3 + depth) & 31
  key:int=raw*32 + tie
  if key<0:
    key=0
  if key>CHUNKSHAPE133_SCORE_KEY_MAX:
    key=CHUNKSHAPE133_SCORE_KEY_MAX
  return key


def chunkshape133_build_thresholds_from_hist(hist:List[int],total_records:int)->List[int]:
  """Build 8 percentile thresholds from the score-key histogram."""
  thresholds:List[int]=[0]*8
  if total_records<=0:
    thresholds[7]=CHUNKSHAPE133_SCORE_KEY_MAX
    return thresholds

  b:int=0
  cum:int=0
  k:int=0
  while k<=CHUNKSHAPE133_SCORE_KEY_MAX:
    cum+=hist[k]
    while b<7:
  ...
  while b<7:
    thresholds[b]=CHUNKSHAPE133_SCORE_KEY_MAX
    b+=1
  thresholds[7]=CHUNKSHAPE133_SCORE_KEY_MAX
  return thresholds


def chunkshape133_bucket_from_key(key:int,thresholds:List[int])->int:
...
def chunkshape133_lane_from_key(key:int)->int:
  """Map a score key to a deterministic lane 0..15 inside its score bucket."""
  lane:int=key&15
  if lane<0:
    lane=0
  if lane>15:
...
def chunkshape133_make_quotas(rem_counts:List[int],total_remaining:int,m_target:int)->List[int]:
...
def build_chunkshape133_reordered_bin(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  """
  133 host-side score-aware lanephase chunk shaping.
  ...
  Output:
    ...
    cross-stripe traversal order, but splits each bucket into 16 deterministic lanes and rotates
  ...
  out_fname:str=chunkshape133_reorder_output_fname(N,preset_queens,BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_chunkshape133_{CHUNKSHAPE133_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}_{funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)}_sim.tsv"

  if gpu_log_level>=1:
    print(f"[chunkshape133-build-config] N={N} source={fname} records={total_records} chunks={n_chunks} steps={STEPS} output={out_fname} progress={progress_fname} reason={CHUNKSHAPE133_DEFAULT_REASON}")

  with open(progress_fname,"w") as pf:
    pf.write("N\tpreset\tchunk\tout_records\ttotal_records\telapsed\telapsed_ms\tb0\tb1\tb2\tb3\tb4\tb5\tb6\tb7\tl0\tl1\tl2\tl3\tl4\tl5\tl6\tl7\tl8\tl9\tl10\tl11\tl12\tl13\tl14\tl15\n")
  ...
  if len(data)<expected_bytes:
    print(f"[chunkshape133-warning] source byte count shorter than expected: bytes={len(data)} expected={expected_bytes}")
  ...
  key_hist:List[int]=[0]*(CHUNKSHAPE133_SCORE_KEY_MAX+1)
  off:int=0
  classify_chunk:int=0
  key_min:int=CHUNKSHAPE133_SCORE_KEY_MAX
  key_max:int=0
  while off<total_records:
    ...
    i=0
    while i<m:
      key:int=chunkshape133_score_key_from_soa(soa,i,off+i)
      if key<0:
        key=0
      if key>CHUNKSHAPE133_SCORE_KEY_MAX:
        key=CHUNKSHAPE133_SCORE_KEY_MAX
    ...
    if gpu_log_level>=2:
      print(f"[chunkshape133-classify-chunk] N={N} chunk={classify_chunk} off={off} m={m}")
    off+=m
    classify_chunk+=1

  score_thresholds:List[int]=chunkshape133_build_thresholds_from_hist(key_hist,total_records)
  bucket_counts:List[int]=[0]*8
  lane_counts:List[int]=[0]*16
  idx_count:int=0
  while idx_count<total_records:
    b0:int=chunkshape133_bucket_from_key(score_key_by_idx[idx_count],score_thresholds)
    lane0:int=chunkshape133_lane_from_key(score_key_by_idx[idx_count])
    bucket_counts[b0]+=1
    lane_counts[lane0]+=1
    idx_count+=1

  if gpu_log_level>=1:
    print(f"[chunkshape133-score-key] N={N} records={total_records} key_min={key_min} key_max={key_max} t0={score_thresholds[0]} t1={score_thresholds[1]} t2={score_thresholds[2]} t3={score_thresholds[3]} t4={score_thresholds[4]} t5={score_thresholds[5]} t6={score_thresholds[6]}")
    print(f"[chunkshape133-score-buckets] N={N} records={total_records} b0={bucket_counts[0]} b1={bucket_counts[1]} b2={bucket_counts[2]} b3={bucket_counts[3]} b4={bucket_counts[4]} b5={bucket_counts[5]} b6={bucket_counts[6]} b7={bucket_counts[7]}")
    print(f"[chunkshape133-lanes] N={N} records={total_records} l0={lane_counts[0]} l1={lane_counts[1]} l2={lane_counts[2]} l3={lane_counts[3]} l4={lane_counts[4]} l5={lane_counts[5]} l6={lane_counts[6]} l7={lane_counts[7]} l8={lane_counts[8]} l9={lane_counts[9]} l10={lane_counts[10]} l11={lane_counts[11]} l12={lane_counts[12]} l13={lane_counts[13]} l14={lane_counts[14]} l15={lane_counts[15]}")

  # ------------------------------------------------------------------
  # Stage 2: keep the proven 128 cross-stripe order, but split each score bucket into 16 lanes.
  # ------------------------------------------------------------------
      ...
      while src_ch<n_chunks:
        ...
        if within<STEPS:
          ...
          if idx<total_records:
            bucket:int=chunkshape133_bucket_from_key(score_key_by_idx[idx],score_thresholds)
            lane_val:int=chunkshape133_lane_from_key(score_key_by_idx[idx])
            bucket_lane_indices[bucket*16+lane_val].append(idx)
            staged_total+=1
        src_ch+=1
      base+=1
    out_ch+=1
  ...
  if staged_total!=total_records:
    print(f"[chunkshape133-warning] staged count mismatch: staged={staged_total} source={total_records}")

  # ------------------------------------------------------------------
  # Stage 3: write output chunks with proportional score-bucket quotas.
  #          Inside each bucket, rotate the 16-lane start phase per chunk and bucket.
  # ------------------------------------------------------------------
  bucket_lane_pos:List[int]=[0]*128
  bucket_lane_rem:List[int]=[0]*128
  ...
  with open(out_fname,"ab") as out:
    while out_ch<n_chunks:
      ...
      quotas:List[int]=chunkshape133_make_quotas(bucket_rem,total_remaining,m_target)
      written_by_bucket:List[int]=[0]*8
      written_by_lane:List[int]=[0]*16
      written_chunk:int=0
      ...
      while written_chunk<m_target:
        ...
        while oi<8:
          ...
          if quotas[b]>0:
            start_lane:int=(out_ch*5 + b*3 + written_by_bucket[b]) & 15
            found:int=-1
            scan:int=0
            while scan<16:
              lane=(start_lane+scan) & 15
              flat=b*16+lane
      ...
      with open(progress_fname,"a") as pf:
        pf.write(f"{N}\t{preset_queens}\t{out_ch}\t{written_chunk}\t{total_records}\t{elapsed_text}\t{elapsed_ms}\t{written_by_bucket[0]}\t{written_by_bucket[1]}\t{written_by_bucket[2]}\t{written_by_bucket[3]}\t{written_by_bucket[4]}\t{written_by_bucket[5]}\t{written_by_bucket[6]}\t{written_by_bucket[7]}\t{written_by_lane[0]}\t{written_by_lane[1]}\t{written_by_lane[2]}\t{written_by_lane[3]}\t{written_by_lane[4]}\t{written_by_lane[5]}\t{written_by_lane[6]}\t{written_by_lane[7]}\t{written_by_lane[8]}\t{written_by_lane[9]}\t{written_by_lane[10]}\t{written_by_lane[11]}\t{written_by_lane[12]}\t{written_by_lane[13]}\t{written_by_lane[14]}\t{written_by_lane[15]}\n")
      if gpu_log_level>=2:
        print(f"[chunkshape133-build-chunk] N={N} chunk={out_ch} written={written_chunk} total_written={written_total} b0={written_by_bucket[0]} b1={written_by_bucket[1]} b2={written_by_bucket[2]} b3={written_by_bucket[3]} b4={written_by_bucket[4]} b5={written_by_bucket[5]} b6={written_by_bucket[6]} b7={written_by_bucket[7]} l0={written_by_lane[0]} l1={written_by_lane[1]} l2={written_by_lane[2]} l3={written_by_lane[3]} l4={written_by_lane[4]} l5={written_by_lane[5]} l6={written_by_lane[6]} l7={written_by_lane[7]} l8={written_by_lane[8]} l9={written_by_lane[9]} l10={written_by_lane[10]} l11={written_by_lane[11]} l12={written_by_lane[12]} l13={written_by_lane[13]} l14={written_by_lane[14]} l15={written_by_lane[15]}")
      out_ch+=1
  ...
  if written_total!=total_records:
    print(f"[chunkshape133-warning] record count mismatch: written={written_total} source={total_records}")
  if reordered_records!=written_total:
    print(f"[chunkshape133-warning] output count mismatch: file_records={reordered_records} written={written_total}")
  if gpu_log_level>=1:
    print(f"[chunkshape133-build-summary] N={N} records={reordered_records} chunks={n_chunks} output={out_fname} progress={progress_fname} valid={1 if validate_bin_file(out_fname) else 0}")
...
def exec_solutions_gpu_chunk_split133(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  if gpu_log_level>=1:
    print(f"[split133-buckets] N={N} m={m} generic={m} d2base14={d2base14_m} d0={d0_m} rest=0 split_mode={split_mode} specialized=0")
...
def exec_solutions_gpu_bin_stream_split133(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split133_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split133_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split133_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split133_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split133-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split133-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split133-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split133-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split133(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split133-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split133-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split133 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split133 full run / optional multi-GPU worker
      # Examples:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split133-generic-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split133-generic worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split133_generic_harness_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split133_generic_harness_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  if gpu_log_level>=1 and (bench_mode==30 or bench_mode==31):
    print(f"chunkshape133_params: version={CHUNKSHAPE133_REORDER_VERSION} reason={CHUNKSHAPE133_DEFAULT_REASON}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split133-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split133-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      shaped_fname:str=chunkshape133_reorder_output_fname(N,preset_queens,gpu_block,gpu_max_blocks)
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape133-probe-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape133-probe-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape133_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split133_probe_{CHUNKSHAPE133_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split133(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split133-probe-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE133_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split133-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split133-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split133-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      if shaped_records==base_reorder_records and shaped_done==base_reorder_records and validate_bin_file(shaped_fname):
        if gpu_log_level>=1:
          print(f"[chunkshape133-full-reuse] N={N} records={shaped_records} chunks={shaped_chunks} bin={shaped_fname} source_bin={base_reorder_fname}")
      else:
        if gpu_log_level>=1:
          print(f"[chunkshape133-full-build] N={N} source_records={base_reorder_records} existing_records={shaped_records} done_count={shaped_done} bin={shaped_fname}")
        shaped_fname,shaped_records,shaped_chunks=build_chunkshape133_reordered_bin(N,base_reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level)
      reorder_fname=shaped_fname
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split133_full_{CHUNKSHAPE133_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split133(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,0)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split133-full-done] N={N} source_records={stream_records} base_reordered_records={base_reorder_records} shaped_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} base_bin={base_reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} chunkshape={CHUNKSHAPE133_REORDER_VERSION} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
