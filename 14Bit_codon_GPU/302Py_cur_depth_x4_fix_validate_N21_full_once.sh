...
# =============================================================================
# 302 N=21 full validation harness (cur-depth-x4-fix probe)
...
SRC=${SRC:-./302Py_cur_depth_x4_fix_probe.py}
CAND=${CAND:-./302Py_cur_depth_x4_fix_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/302Py_cur_depth_x4_fix_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/302Py_cur_depth_x4_fix_logs_N21_full_once_${TS}"
...
echo "[start] 302 cur-depth-x4-fix validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 302 cur-depth-x4-fix SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '302 cur-depth-x4-fix probe' "$SRC"; then
  printf 'source_version_tag	302 cur-depth-x4-fix probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	302 cur-depth-x4-fix probe	missing	FAIL
...
# 302: cur-depth-x4-fix shape checks (3 update sites for cur_depth_x4).
    ...
    ok_no_depth_mul = 'cur_depth*4' not in body14 and '(cur_depth-8)*4' not in body14
    ok_x4_init = 'cur_depth_x4:int=0' in body14
    ok_x4_preroll = 'cur_depth_x4=4\n' in body14
    ok_x4_pop = 'cur_depth_x4=cur_depth<<2' in body14
    ok_x4_incr = body14.count('cur_depth_x4+=4') == 1
    ok_x4_decode = ('schedule_lo>>u32(cur_depth_x4))' in body14 and
    ...
    ok_shape = (ok_stride_param and ok_schedule_lo_hi and ok_no_depth_mul
                 and ok_x4_init and ok_x4_preroll and ok_x4_pop and ok_x4_incr and ok_x4_decode
                 and ok_save_sp and ok_next_depth and ok_stack_ptr_incr
                 and ok_gridstride_loop and ok_single_writeback and ok_push)
    checks.append(('source_cur_depth_x4_fix_shape',
                    'no cur_depth*4 mul + x4 init/preroll=4/pop<<2/incr+4(inside while) + hot decode x4/x4-32 + 296 stack',
                    'present' if ok_shape else (
                        'missing (stride=%s sched=%s no_mul=%s x4_init=%s x4_pre=%s x4_pop=%s x4_incr=%s x4_dec=%s sp=%s nd=%s ptr=%s gs=%s wb=%s push=%s)'
                        % (ok_stride_param, ok_schedule_lo_hi, ok_no_depth_mul, ok_x4_init, ok_x4_preroll,
                           ok_x4_pop, ok_x4_incr, ok_x4_decode, ok_save_sp, ok_next_depth, ok_stack_ptr_incr,
                           ok_gridstride_loop, ok_single_writeback, ok_push)
                    ),
                    ok_shape))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 302 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 302 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 302 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 302 cur-depth-x4-fix mode31 split145"
...
echo "[validation-ok] 302 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
