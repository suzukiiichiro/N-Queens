...
# =============================================================================
# 275 r2 N=21 full validation harness
# Parent: 274 restore271-after-root0direct-reject / 271 final-fastest-complete.
# Experiment: diagnostic-only split145 bucket summary for d2base14(fid14)
#             and d0(fid26/27). No CUDA kernel body change, no schedule
#             precompute, no root0 direct kernel, no extra launch split.
...
# Diagnostic: [split145-bucket-summary] plus progress-derived bucket counts.
# =============================================================================

SRC=${SRC:-./275Py_split145_bucket_distribution_diag_probe.py}
CAND=${CAND:-./275Py_split145_bucket_distribution_diag_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/275Py_split145_bucket_distribution_diag_N21_full.lock}
...
BASELINE_275_PARENT_274_SECONDS=${BASELINE_275_PARENT_274_SECONDS:-427.758}
...
LOGDIR="${LOG_ROOT%/}/275Py_split145_bucket_distribution_diag_logs_N21_full_once_${TS}"
...
echo "[start] 275 r2 split145-bucket-distribution diagnostic validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 275 split145-bucket-distribution-diag SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '275 split145-bucket-distribution-diag' "$SRC"; then
  printf 'source_version_tag\t275 split145-bucket-distribution-diag\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t275 split145-bucket-distribution-diag\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split275' in s and 'split274_' not in s and 'split273_' not in s and 'kernel_dfs_iter_gpu_maxd14_root0' not in s and 'split270_' not in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split275 active; 274/273/270/240 runtime tags rejected', 'split275', True))
else:
    checks.append(('source_split_tag', 'split275 active; 274/273/270/240 runtime tags rejected', 'old/root0 split runtime present or split275 missing', False))
...
if 'kernel_dfs_iter_gpu_maxd14_root0' in active or 'root0-dispatch' in active or 'rootaction0-direct' in active:
    checks.append(('source_root0_direct_rejected', 'active absent', 'active root0 direct kernel marker present', False))
else:
    checks.append(('source_root0_direct_rejected', 'active absent', 'active absent', True))

if '[split145-bucket-summary]' in s and 'bucket_d0_tasks' in s and 'bucket_d2base14_tasks' in s:
    checks.append(('source_split145_bucket_diag', 'present', 'present', True))
else:
    checks.append(('source_split145_bucket_diag', 'present', 'missing', False))
fail = 0
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 275 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 275 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 275 split145-bucket-distribution diagnostic mode31 split145"
...
# ---- 275 split145 bucket diagnostic checks ----
if [[ -s "${PROGRESS_COPY:-}" ]]; then
  awk -F '	' '
    NR==1 {
      for(i=1;i<=NF;i++){
        if($i=="m") mcol=i
        else if($i=="funcid_14_count") f14=i
        else if($i=="funcid_26_count") f26=i
        else if($i=="funcid_27_count") f27=i
      }
      next
    }
    {
      ...
      if(d2>0)d2chunks++
      if(d0>0)d0chunks++
      if(d0+d2>0)d0ord2chunks++
      if(d0>0 && d2>0)bothchunks++
    }
    END {
      cols=(mcol && f14 && f26 && f27)?"present":"missing"
      other=total-d0ord2; if(other<0)other=0
      printf "BUCKET_COLS=%s
BUCKET_ROWS=%.0f
BUCKET_TOTAL=%.0f
BUCKET_D2_TASKS=%.0f
BUCKET_D2_CHUNKS=%.0f
BUCKET_D0_TASKS=%.0f
BUCKET_D0_CHUNKS=%.0f
BUCKET_D0ORD2_TASKS=%.0f
BUCKET_D0ORD2_CHUNKS=%.0f
BUCKET_BOTH_CHUNKS=%.0f
BUCKET_OTHER_TASKS=%.0f
", cols, rows, total, d2tasks, d2chunks, d0tasks, d0chunks, d0ord2, d0ord2chunks, bothchunks, other
    }
  ' "$PROGRESS_COPY" > "$LOGDIR/bucket_progress.env"
  source "$LOGDIR/bucket_progress.env"
  record_check split145_bucket_progress_columns present "${BUCKET_COLS:-missing}" || failures=$((failures+1))
  record_check split145_bucket_progress_rows "$EXPECTED_CHUNKS" "${BUCKET_ROWS:-missing}" || failures=$((failures+1))
  record_check split145_bucket_progress_task_sum "$EXPECTED_TASKS" "${BUCKET_TOTAL:-missing}" || failures=$((failures+1))
  bucket_diag_actual="d2base14_tasks=${BUCKET_D2_TASKS:-missing} d2base14_chunks=${BUCKET_D2_CHUNKS:-missing} d0_tasks=${BUCKET_D0_TASKS:-missing} d0_chunks=${BUCKET_D0_CHUNKS:-missing} d0_or_d2_tasks=${BUCKET_D0ORD2_TASKS:-missing} d0_or_d2_chunks=${BUCKET_D0ORD2_CHUNKS:-missing} both_chunks=${BUCKET_BOTH_CHUNKS:-missing} other_tasks=${BUCKET_OTHER_TASKS:-missing}"
  printf 'split145_bucket_progress_summary	diagnostic	%s	INFO
' "$bucket_diag_actual" >> "$SUMMARY"
fi

BUCKET_SUMMARY_LINE=$(grep -E '^\[split145-bucket-summary\]' "$RUN_LOG" | tail -n1 || true)
if [[ -n "$BUCKET_SUMMARY_LINE" ]]; then
  record_check split145_bucket_summary_line present present || failures=$((failures+1))
  awk '
    /^\[split145-bucket-summary\]/ {
      for(i=1;i<=NF;i++){
        split($i,a,"=")
        if(a[1]=="total_tasks") total=a[2]
        else if(a[1]=="d2base14_tasks") d2=a[2]
        else if(a[1]=="d2base14_chunks") d2c=a[2]
        else if(a[1]=="d0_tasks") d0=a[2]
        else if(a[1]=="d0_chunks") d0c=a[2]
        else if(a[1]=="d0_or_d2_tasks") d0d2=a[2]
        else if(a[1]=="d0_or_d2_chunks") d0d2c=a[2]
        else if(a[1]=="both_chunks") both=a[2]
        else if(a[1]=="other_tasks") other=a[2]
      }
    }
    END {
      printf "LOG_BUCKET_TOTAL=%s
LOG_BUCKET_D2_TASKS=%s
LOG_BUCKET_D2_CHUNKS=%s
LOG_BUCKET_D0_TASKS=%s
LOG_BUCKET_D0_CHUNKS=%s
LOG_BUCKET_D0ORD2_TASKS=%s
LOG_BUCKET_D0ORD2_CHUNKS=%s
LOG_BUCKET_BOTH_CHUNKS=%s
LOG_BUCKET_OTHER_TASKS=%s
", total,d2,d2c,d0,d0c,d0d2,d0d2c,both,other
    }
  ' <<< "$BUCKET_SUMMARY_LINE" > "$LOGDIR/bucket_log.env"
  source "$LOGDIR/bucket_log.env"
  record_check split145_bucket_log_task_sum "$EXPECTED_TASKS" "${LOG_BUCKET_TOTAL:-missing}" || failures=$((failures+1))
  if [[ -n "${BUCKET_D2_TASKS:-}" ]]; then
    record_check split145_bucket_log_vs_progress_d2_tasks "$BUCKET_D2_TASKS" "${LOG_BUCKET_D2_TASKS:-missing}" || failures=$((failures+1))
    record_check split145_bucket_log_vs_progress_d0_tasks "$BUCKET_D0_TASKS" "${LOG_BUCKET_D0_TASKS:-missing}" || failures=$((failures+1))
    record_check split145_bucket_log_vs_progress_d0ord2_tasks "$BUCKET_D0ORD2_TASKS" "${LOG_BUCKET_D0ORD2_TASKS:-missing}" || failures=$((failures+1))
  fi
else
  record_check split145_bucket_summary_line present missing || failures=$((failures+1))
fi
...
echo "[validation-ok] 275 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread and bucket diagnostic summary"
