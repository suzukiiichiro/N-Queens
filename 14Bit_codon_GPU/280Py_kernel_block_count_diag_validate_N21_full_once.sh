...
# =============================================================================
# 280 N=21 full diagnostic validation harness
...
SRC=${SRC:-./280Py_kernel_block_count_diag_probe.py}
CAND=${CAND:-./280Py_kernel_block_count_diag_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/280Py_kernel_block_count_diag_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/280Py_kernel_block_count_diag_logs_N21_full_once_${TS}"
...
echo "[start] 280 kernel-block-count diagnostic validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 280 kernel-block-count-diagnostic SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '280 kernel-block-count-diagnostic' "$SRC"; then
  printf 'source_version_tag\t280 kernel-block-count-diagnostic\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t280 kernel-block-count-diagnostic\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if 'split280' in s and 'split278' not in s and 'split277' not in s and 'split275' not in s and 'split274' not in s and 'split273' not in s and 'rootaction0-direct' not in s and 'kernel_dfs_iter_gpu_maxd14_root0' not in s and 'split270_' not in s and 'split240' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split280 active; 278/277/275/273/270/240 rejected', 'split280', True))
else:
    checks.append(('source_split_tag', 'split280 active; 278/277/275/273/270/240 rejected', 'old/depthu/zero/bucket/root0 split runtime present or split280 missing', False))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 280 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 280 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 280 kernel-block-count-diagnostic mode31 split145"
...
echo "[validation-ok] 280 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread and kernel block diagnostics"
