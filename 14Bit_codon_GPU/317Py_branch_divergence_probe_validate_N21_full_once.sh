...
# =============================================================================
# 317 N=21 full validation harness (branch-divergence-probe)
# Parent: 316 env-accept-ncu-prep. 315/316 both confirmed correct (315=
#   454.779s, 316=454.460s, 314666222712), and 315's field-name fix now
#   works: gpu_clock_cap_check reports current_sm=1320MHz vs max_sm=1710MHz
#   (WARN-CAPPED, as expected), gpu_power_cap_check reports
#   power.limit=300.00W == power.default_limit=300.00W (OK, power was never
#   the constraint).
#
# 316_ncu.txt analysis (the requested `ncu --launch-count 1 --set
#   SourceCounters` capture):
#   (1) Architectural ratios matched 304's original 1710MHz-era profile
#       almost exactly despite the ~23% lower clock:
#         Avg Active Threads Per Warp : 6.34  vs 304's 6.34 (EXACT)
#         Achieved Occupancy          : 11.03% vs 304's 11.04% (noise)
#         Stall Wait                  : 44.09% vs 304's 44.1% (noise)
#         Stall Branch Resolving      : 19.62% vs 304's ~19.6% (noise)
#       This confirms the 316 assumption that these ratios reflect
#       architectural properties independent of absolute clock frequency.
#   (2) BUT the requested per-line branch attribution was not obtained. The
#       captured file contains only 5 lightweight sections (SpeedOfLight /
#       Scheduler Statistics / Warp State Statistics / Launch Statistics /
#       Occupancy) -- no Source Counters section -- and includes:
#         WRN  The optional metric smsp__pcsamp_sample_count could not be
#              found.
#       i.e. PC-sampling-based profiling is unavailable in this environment.
#       Combined with 316's nvidia-persistenced initialization failure, this
#       is a second independent signal that low-level/privileged driver
#       features are blocked here -- consistent with a virtualized/shared
#       GPU environment.
#
# This revision: ZERO source changes from 316 (variant=2, K=48 unchanged).
#   Pivoting away from PC-sampling: the next command to try manually (NOT
#   invoked by this script) uses hardware-counter-based branch divergence
#   aggregates instead, which do not require PC sampling:
#     ncu --launch-count 1 \
#       --metrics smsp__sass_branch_targets.sum,smsp__sass_branch_targets_threads_divergent.sum,smsp__sass_branch_targets_threads_uniform.sum \
#       -o 317_branchdiv_chunk0 \
#       ./317Py_branch_divergence_probe -g 21 21 32 484 1 0 7 31 8 7 0 0 1 2
#   This gives kernel-wide branch divergence magnitude (uniform vs divergent
#   branch target counts), not per-line attribution, but is a real signal if
#   PC sampling itself is the blocked feature rather than ncu profiling
#   entirely. If this also fails, treat both per-line and counter-based ncu
#   branch profiling as unavailable in this environment and fall back to a
#   manual source-level review of the hot loop's continuation/termination
#   branches instead of further ncu attempts.
...
SRC=${SRC:-./317Py_branch_divergence_probe.py}
CAND=${CAND:-./317Py_branch_divergence_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/317Py_branch_divergence_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/317Py_branch_divergence_probe_logs_N21_full_once_${TS}"
...
echo "[start] 317 branch-divergence-probe validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 317 branch-divergence-probe SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '317 branch-divergence-probe' "$SRC"; then
  printf 'source_version_tag	317 branch-divergence-probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	317 branch-divergence-probe	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 317 branch-divergence-probe mode31 split145"
...
echo "[validation-ok] 317 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); PC-sampling-based ncu profiling unavailable in this environment, next step is the hardware-counter branch divergence command in the header comment"
