...
# =============================================================================
# 308 N=21 full validation harness (K52-final-sweep)
# Parent: 307 K44-fine-probe (351.240s, -0.170s vs 304 K48=351.070s, flat/noise).
#   K=44 is within noise of K=48; flat zone extends from K=44 to K=56.
#   K survey so far: K=32(353.671s) K=40(353.587s) K=44(351.240s flat)
#   K=48(351.070s BEST) K=56(351.534s flat) K=64(353.896s).
#   Final K-sweep data point: K=52 to complete the curve between K=48 and K=56.
#   EXPECTED_CHUNKS=ceil(2025282/(32*484*52))=ceil(2025282/805376)=3.
#   All kernel logic unchanged from 296/304.
# Safety: K_PER_THREAD_MAXD14=52, EXPECTED_CHUNKS=3.
# NOTE: this validation script has not yet been executed on real hardware.
# =============================================================================

SRC=${SRC:-./308Py_K52_final_sweep.py}
CAND=${CAND:-./308Py_K52_final_sweep}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/308Py_K52_final_sweep_N21_full.lock}
...
EXPECTED_K_PER_THREAD_MAXD14=52
...
BASELINE_307_K44_FINE_SECONDS=${BASELINE_307_K44_FINE_SECONDS:-351.240}
...
LOGDIR="${LOG_ROOT%/}/308Py_K52_final_sweep_logs_N21_full_once_${TS}"
...
echo "[start] 308 K52-final-sweep validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 308 K52-final-sweep SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '308 K52-final-sweep' "$SRC"; then
  printf 'source_version_tag	308 K52-final-sweep	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	308 K52-final-sweep	missing	FAIL
...
# 304: K48-sweep shape checks (296 kernel logic, K=48).
    ...
    checks.append(('source_K52_final_sweep_shape',
                    '296 kernel shape (stack/save_sp/next_depth/cur_depth/stack_ptr) + K=52',
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 308 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 308 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 308 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 308 K52-final-sweep mode31 split145"
...
echo "[validation-ok] 308 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=52 (3 dispatch/progress rows)"
