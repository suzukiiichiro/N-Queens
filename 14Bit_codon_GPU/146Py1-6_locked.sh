...
# 146 locked validation harness
# - Candidate: 146Py_scorestripe_chunkshape_probe
...
CAND=${CAND:-./146Py_scorestripe_chunkshape_probe}
RUN_FULL=${RUN_FULL:-1}
...
LOGDIR="146Py_logs_N21_locked_validation_${TS}"
...
if [ "${RUN_FULL}" = "1" ]; then
  run_case \
    "06_cand146_mode31_full_correctness" \
    "${FULL_TOTAL}.*ok" \
    "$CAND" \
