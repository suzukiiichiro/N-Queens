...
# =============================================================================
# NQ_UPDATE_MEMO
# 271: final-fastest-complete baseline.
#      270Py rootpre2flag は正当性OKだが 0:07:08.824 へ退行したため採用せず、
#      241Py restore239-after-fid14split-reject r3 を完成版として固定する。
#      CUDA MAXD14/16/18/20/21 kernel本文、rootrestlate、futuremask、no-sibling、
#      split145/chunkshape148、broadmarktail、cache生成、worker splitは241相当で変更しない。
#      -c/-g range、GPU N=5..27、N25..N27 dynamic preset=8、mode28/29/30/31、
#      MAXD fallback、CPU dfs_iter pathを維持。CPU再帰dfs削除は239/241同様に維持。
...
"""
271 final-fastest note: 270のrootpre2flagは正当性OKだがN21で退行したため採用せず、241 r3 restore239-after-fid14split-reject相当を完成版として固定する。CUDA kernel本文とGPU本線機能は変更しない。
...
速度も、診断版としては問題ありません。271の `0:07:07.705` より `0.023秒` だけ遅いだけなので、追加したroot_action診断のオーバーヘッドはほぼ無視できます。

重要なのはここです。

```text
root0_kernel_chunks = 131
fallback_chunks     = 0

root_action0 = 2025282
root_action1 = 0
root_action2 = 0
root_action3 = 0
nonzero      = 0
...
つまり **N=21 / mode31 / 現在のtask配置では、全chunk・全taskが root_action==0** です。
...
は、少なくともN=21最速化では **候補から外してよい** です。対象taskが0なので、専用kernelを作っても実行されません。

次の一手は、むしろこちらです。

```text
273Py = root_action==0 前提のMAXD14 kernel
```

ただし、267/270のようなhost precompute系には戻しません。271の本線構造を維持したまま、

```text
1 chunk 1 launch は維持
host側で root_action_nonzero_count==0 を確認
全chunk root0なら root_action処理を持たない MAXD14 kernel へdispatch
schedule precompute配列や追加global loadは入れない
```

という形がよさそうです。

狙いは、271のMAXD14 kernel内に残っている以下を消すことです。
...
if root_action==3
if root_action==1
root_action==1 時の root_a / root_ld 補正
```

272で **全task root_action==0** と確認できたため、この分岐群はN=21では死に枝です。ここを消せれば、271より少しでも前に出る可能性があります。

README追記候補は以下です。

```text
- **272確認結果**: 272Py root_action_distribution_diag は `N=21 full once` で final total `314666222712` 一致、progress rows `131`、duplicate/missing `0/0`、dispatch task sum `2025282`、required_maxd/selected_MAXD は全chunk `14/MAXD14`、schedule_words `0`、stack_bytes_per_thread `208`、error/mismatch `0`。速度は `0:07:07.728` で、271Py `0:07:07.705` より `0.023秒` 遅いだけで診断オーバーヘッドは小さい。root_action分布は `root_action0=2025282`、`root_action1=0`、`root_action2=0`、`root_action3=0`、`root0_kernel_chunks=131`、`fallback_chunks=0`。したがってN=21では root_action==1/2/3 専用化は不要。次は271本線を維持しつつ、schedule precomputeを入れない root_action==0 専用MAXD14 kernel を273で試す。
```

結論として、**272は診断版として成功**です。次は **273Py rootaction0専用MAXD14 kernel、ただしprecomputeなし** が最優先です。


273一式を作成しました。

今回は **271を親にした速度候補**として、272の診断列は入れず、`root_action_nonzero_count==0` のchunkだけ **root_action分岐なしMAXD14 root0 kernel** にdispatchする形にしています。schedule precompute / repack 系には戻していません。

こちらで `STATIC_ONLY=1` は通しています。Codon release build / N=21 full run はこの環境では実行していません。

ファイルはこちらです。

* [273Py_rootaction0_direct_kernel_probe.py](sandbox:/mnt/data/273Py_rootaction0_direct_kernel_probe.py)
* [273Py_rootaction0_direct_kernel_validate_N21_full_once.sh](sandbox:/mnt/data/273Py_rootaction0_direct_kernel_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [273Py_rootaction0_direct_kernel_sha256.txt](sandbox:/mnt/data/273Py_rootaction0_direct_kernel_sha256.txt)
* [273Py_rootaction0_direct_kernel_bundle_20260709.tar](sandbox:/mnt/data/273Py_rootaction0_direct_kernel_bundle_20260709.tar)

実行は以下でお願いします。

```bash
rm -f ./273Py_rootaction0_direct_kernel_probe

STATIC_ONLY=1 bash 273Py_rootaction0_direct_kernel_validate_N21_full_once.sh

bash 273Py_rootaction0_direct_kernel_validate_N21_full_once.sh
```

確認したい追加ポイントはこの行です。

```bash
grep -E "root0-dispatch|maxd-dispatch|^21:|validation-ok" \
  273Py_rootaction0_direct_kernel_logs_N21_full_once_*/full_once.log
```

期待は以下です。

```text
root0_dispatch_rows        131
root0_dispatch_task_sum    2025282
root0_dispatch_nonzero_sum 0
root0_dispatch_root0_rows  131
root0_dispatch_fallback_rows 0
```

採否は、まず正当性OK、そのうえで **271の 0:07:07.705 を上回るか** で判断するのがよいです。
...
VERSION_TAG:str="273 rootaction0-direct-kernel: parent 271 final-fastest-complete; 272 diagnostic showed N21 mode31 all tasks have root_action==0; schedule precompute remains rejected; if cached root_action_nonzero_count is zero for a selected MAXD14 chunk, dispatch a root_action-free MAXD14 root0 kernel, otherwise fallback to 271 MAXD14; generic N5..27; bare -g split145 mode31; mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
def root_action_for_task(ctrl0:u32,markctrl:u32,meta_next:List[u8])->int:
  # 273: host-side depth-0 action classifier. 0=normal, 1=jmark,
  # 2=terminal, 3=base14. Used only to select root0/fallback kernel.
  IS_BASE_MASK_I:int=69222408
  IS_JMARK_MASK_I:int=4
  IS_P5_MASK_I:int=3840
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    if soa.root_action_nonzero_count==0:
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        soa.ijkl_arr[t]=ijkl
        root_action:int=root_action_for_task(soa.ctrl0_arr[t],soa.markctrl_arr[t],meta_next_sched)
        if root_action!=0:
...
CHUNKSHAPE148_DEFAULT_REASON:str="273 keeps validated 271/241/239/238/231/217 scorestripe_v9 task order/cache; adds direct root_action==0 MAXD14 kernel dispatch without schedule precompute; 270 rootpre2flag and 240 fid14 launch split rejected"
...
def exec_solutions_gpu_chunk_split145(
  ...
  if m>0:
    ...
    if gpu_log_level>=1:
      ...
      root0_kernel:int=1 if selected_maxd==14 and soa.root_action_nonzero_count==0 else 0
      fallback_kernel:int=1 if selected_maxd==14 and soa.root_action_nonzero_count!=0 else 0
      print(f"[root0-dispatch] N={N} scope=split145 m={m} selected_MAXD={selected_maxd} root_action_nonzero_count={soa.root_action_nonzero_count} root0_kernel={root0_kernel} fallback_kernel={fallback_kernel}")
    if not launch_kernel_dfs_iter_gpu_static_maxd(selected_maxd,soa,w_arr,meta_next,results,m,board_mask_gpu,n3_gpu,n4_gpu,GRID,BLOCK):
      print(f"[maxd-error] unsupported required_maxd={required_maxd}; supported maximum is 21")
...
def main()->None:
  ...
  if argc == 1:
    ...
    if argc >= 10:
      ...
      if not (bench_mode==0 or bench_mode==11 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31):
        print(f"[warning] bench_mode={bench_mode} was removed in 273 rootaction0-direct-kernel; using 0")
        bench_mode=0
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split273_final_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split273_final_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split273-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split273-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split273_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
