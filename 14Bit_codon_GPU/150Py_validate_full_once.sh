...
# 150 single-pass validation harness
#
...
# Candidate 150 changes only the u32 blockK selector inside the GPU kernel.
# A real flock guard prevents two copies of this harness from competing.

SRC=${SRC:-./150Py_blockk_branchless_u32_probe.py}
CAND=${CAND:-./150Py_blockk_branchless_u32_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
LOCK_FILE=${LOCK_FILE:-/tmp/150Py_nqueens_gpu_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
# Direct validated 149 full run and historical 148 mean (informational only).
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 150 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/150Py_logs_N21_full_once_${TS}"
...
echo "[validation-ok] 150 one full run reproduced cases 01-06"
