...
# =============================================================================
# 282 N=21 full validation harness
# Parent: 279/276 restored final-fastest baseline.
# Experiment: reject 281/280 kernel block-count diagnostics and restore the 279/276 baseline. No diagnostic arrays, no zero-const variable, no depth_u child/save, no root0 direct kernel, no bucket diag, no schedule precompute.
...
SRC=${SRC:-./282Py_restore279_after_blockdiag_reject_probe.py}
CAND=${CAND:-./282Py_restore279_after_blockdiag_reject_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/282Py_restore279_after_blockdiag_reject_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/282Py_restore279_after_blockdiag_reject_logs_N21_full_once_${TS}"
...
echo "[start] 282 restore279-after-blockdiag-reject validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 282 restore279-after-blockdiag-reject SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '282 restore279-after-blockdiag-reject' "$SRC"; then
  printf 'source_version_tag\t282 restore279-after-blockdiag-reject\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t282 restore279-after-blockdiag-reject\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
old_split_markers = ['split281', 'split280', 'split278', 'split277', 'split275', 'split273', 'split270_', 'split240', 'split239_', 'kernel-blockdiag', 'block_count_diag', 'rootaction0-direct']
old_present = [x for x in old_split_markers if x in s]
if 'split282' in s and not old_present:
    checks.append(('source_split_tag', 'split282 active; rejected runtime tags absent', 'split282', True))
else:
    checks.append(('source_split_tag', 'split282 active; rejected runtime tags absent', 'present=' + ','.join(old_present) if old_present else 'split282 missing', False))
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 282 restore279-after-blockdiag-reject mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "281diag:574.374" "279restore:427.728" "278rejected:429.603" "276current:427.672" "277depthu:427.717" "271fastest:427.705" "239:427.703" "217:427.709"; do
