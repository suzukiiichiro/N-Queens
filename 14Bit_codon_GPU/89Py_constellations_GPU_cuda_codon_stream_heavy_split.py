...
[89Py_constellations_GPU_cuda_codon_stream_heavy_split.py](sandbox:/mnt/data/89Py_constellations_GPU_cuda_codon_stream_heavy_split.py)

89は、**84 streamをベースにして、kernelは1本のまま維持**し、GPU投入前に「安全に1段展開できる重いtask」だけをhost側でsplitする版です。
...
89:
  重い task だけ host側で通常 +1 遷移を1段展開
  複数の lighter subtask にしてから
  同じ kernel_dfs_iter_gpu へ投入
```

85/86/88のように複数kernelを増やす方向ではありません。
`sort_mode=4` も使っていません。
...
```text
heavy_split_is_safe()
heavy_split_use_future()
build_heavy_split_soa()
exec_solutions_gpu_bin_stream_heavy_split()
```
...
```bash
codon build -release 89Py_constellations_GPU_cuda_codon_stream_heavy_split.py

./89Py_constellations_GPU_cuda_codon_stream_heavy_split \
...
```text
21:      314666222712                0      ...    heavy-split-ok
```

## 出力ファイル

```text
progress_N21_6_heavy_split.tsv
```
...
```python
HEAVY_SPLIT_FACTOR_DEFAULT:int=4
HEAVY_SPLIT_FREE_MIN_DEFAULT:int=4
HEAVY_SPLIT_DEPTH_MIN_DEFAULT:int=8
```
...
```text
split_factor = 4
  1 chunk 15488件を最大 61952 subtask まで展開可能

free_min = 4
  freeのpopcountが4以上のtaskだけsplit候補

depth_min = 8
  残り深さが8以上のtaskだけsplit候補
```

## 安全条件

host側でsplitするのは、GPU kernel内で特殊遷移に入らないtaskだけです。
...
mark1 / mark2 special transition
```

つまり、host側では通常の `+1` 遷移だけを再現します。
これにより、まず正しさを優先しています。
...
89がN21で正解一致し、かつ84に近いか速ければ、次はN22へ進める価値があります。
...
この環境にはCodonがないため、実ビルド確認はできていません。Pythonのtokenizeによる字句レベル確認は通しています。

ログ確認しました。**89 heavy split は正解一致しましたが、速度面では失敗です。**

結果は以下です。

```text
89 heavy split N21:
21:      314666222712                0          0:44:16.469    heavy-split-ok
```

正解値 `314666222712` には一致しています。したがって、host側1段splitのロジック自体は、少なくともN21では壊れていません。

ただし、速度はかなり悪化しています。

```text
84 stream N21        : 0:25:38.459
88 BC kernel N21     : 0:34:57.160
89 heavy split N21   : 0:44:16.469
```

84の通常streamがN21を `0:25:38.459 ok` で完走しているので、89は84より約 **1.73倍遅い** です。

## 89が遅くなった理由

89のsummaryを見ると、原因はかなり明確です。

```text
records        = 2,025,282
total_out      = 8,107,197
split_roots    = 767,709
split_children = 6,849,624
kept_roots     = 1,257,573
```

つまり、入力2,025,282件に対して、GPUへ渡すtaskが8,107,197件に増えています。約 **4.0倍** です。

各chunkでも、`in_m=15488` に対して `out_m=61950` 前後まで膨らんでいます。例えばchunk 0では `in_m=15488`、`out_m=61952`、kernel時間 `18.782秒`、chunk 1では `out_m=61951`、kernel時間 `25.336秒` です。

84のN21 32x484では、1chunkあたり概ね10〜15秒台で進んでいました。89はsplitによって不均一を減らす前に、**GPUへ渡す総task数を増やしすぎている**状態です。
...
89:
  正しいがさらに遅い。
  heavy splitはsplitしすぎ。
```

`sort_mode=4` についても、今回のログで改めて `chunk_total=0` が続き、最終的に `ng(0!=314666222712)` になっています。これは比較対象ではなく、封印でよいです。

## 次にやるなら 90 は「制限付き heavy split」

89の考え方自体は完全に捨てなくてよいです。正解一致したので、split遷移は安全に実装できています。問題は **split対象が多すぎる** ことです。

90では、次のように制限するのが良いと思います。
...
が緩すぎます。N21では毎chunkほぼ上限の `out_m≈61952` まで膨らんでいます。
...
89のように毎回61,952件まで膨らませるのはやりすぎです。

## もう一つの改善案：split budget制

さらに良いのは、`free_min/depth_min` だけでなく、**chunkごとのsplit予算**を入れることです。

```text
1 chunkの最大出力数 = STEPS * 2
つまり 15,488 * 2 = 30,976件まで
```

このようにして、重いtaskから順にsplitし、予算に達したら残りはsplitせずそのままGPUへ渡します。

これなら89の安全性を保ちつつ、task数爆発を防げます。

## おすすめ

次は 90 として、以下の方針が良いです。
...
変更点:
  ...
  free_min = 7 or 8
  depth_min = 12
  chunkごとの out_m 上限を STEPS*2 に制限
  split候補は score の高いものだけ
```

89は「正しいがsplitしすぎ」と分かったので、次は **heavy split limited** が本筋です。
...
VERSION_TAG:str="89 stream heavy split GPU runner from 84 dynamic preset P8"
CROSS_STRIPE_SAFE_DEFAULT:bool=False

# 89 HEAVY SPLIT:
#   Keep the 84 stream + single generic GPU kernel path, but split only safe
#   heavy root tasks on the host by one normal +1 transition before launching
#   the same kernel.  This avoids the multi-kernel overhead observed in 85/86/88.
HEAVY_SPLIT_FACTOR_DEFAULT:int=4
HEAVY_SPLIT_FREE_MIN_DEFAULT:int=4
HEAVY_SPLIT_DEPTH_MIN_DEFAULT:int=8
...
####################################################
#
# 89 heavy split GPU stream runner
#
####################################################

"""89 heavy split: TaskSoA の1件を別 TaskSoA へコピーする。"""
...
"""89 heavy split: 現在行で通常 +1 遷移だけを安全にhost側展開できるか判定する。"""
...
"""89 heavy split: fid の avail/future flag を返す。"""
...
"""89 heavy split: 重いが安全な root task だけを1段展開し、同じ汎用 kernel へ渡す。"""
...
"""89 heavy split progress を TSV に追記する。"""
...
def exec_solutions_gpu_bin_stream_heavy_split(
  ...
  total_records:int=count_constellations_bin_records(fname)
  progress_fname:str=f"progress_N{N}_{preset_queens}_heavy_split.tsv"
  with open(progress_fname,"w") as pf:
  ...
  if gpu_log_level>=1:
    print(f"[heavy-split-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} split_factor={split_factor} split_steps={split_steps} free_min={free_min} depth_min={depth_min} chunk_only={1 if chunk_only else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[heavy-split-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m
          chunk_index+=1
          continue
      ...
      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[heavy-split-chunk-start] N={N} chunk={chunk_index} off={off} in_m={m}")

      build_soa_for_range(N,chunk_constellations,0,m,raw_soa,raw_w)
      t1=datetime.now()
      out_m,split_roots,split_children,kept_roots=build_heavy_split_soa(N,raw_soa,raw_w,m,work_soa,work_w,split_steps,free_min,depth_min)
      t2=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[heavy-split-chunk-end] N={N} chunk={chunk_index} off={off} in_m={m} out_m={out_m} split_roots={split_roots} split_children={split_children} kept={kept_roots} grid={GRID} build={str(t1-t0)[:-3]} split={str(t2-t1)[:-3]} kernel={str(t3-t2)[:-3]} sum={str(t4-t3)[:-3]} chunk_total={chunk_total} gpu_total={gpu_total}")

      off+=m
      chunk_index+=1

  if gpu_log_level>=1:
    print(f"[heavy-split-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} total_out={total_out} split_roots={total_split_roots} split_children={total_split_children} kept_roots={total_kept_roots} progress={progress_fname}")
...
def main()->None:
  ...
  if use_gpu:
    ...
    if bench_mode==12:
      print(f"heavy_split_stream: mode={bench_mode} preset={preset_queens_arg} factor={HEAVY_SPLIT_FACTOR_DEFAULT} free_min={HEAVY_SPLIT_FREE_MIN_DEFAULT} depth_min={HEAVY_SPLIT_DEPTH_MIN_DEFAULT}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and not (bench_mode==8 or bench_mode==9 or bench_mode==10):
      ...
      if bench_mode==12:
        status="heavy-split-ok" if expected[N]==total else f"heavy-split-ng({total}!={expected[N]})"
