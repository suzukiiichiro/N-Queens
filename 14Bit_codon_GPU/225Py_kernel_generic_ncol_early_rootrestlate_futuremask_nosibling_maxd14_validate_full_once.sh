...
SRC=${SRC:-./225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 225 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/225Py_kernel_generic_ncol_early_rootrestlate_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
if root_pos < 0:
    root_pos=s.find('# 225 generic-ncol-early:')
if root_pos < 0:
...
if all(checks.values()):
    sys.exit(0)
print('source_generic_ncol_early_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_generic_ncol_early\trootrestlate_plus_generic_ncol_early\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_generic_ncol_early\trootrestlate_plus_generic_ncol_early\tmissing_or_old_shape\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split225'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split225' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit225 active preferred\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit225 active preferred\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
echo "[validation-ok] 225 generic ncol-early reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
