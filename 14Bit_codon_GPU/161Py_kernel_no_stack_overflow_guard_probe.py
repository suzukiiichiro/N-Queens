...
160Pyログは問題ありません。全131チャンク、部分和01～05、最終合計、欠落・重複、警告・エラーがすべて一致しています。

161Pyでは`if sp>=MAXD`のみを除去し、GPU実行範囲をN=5～27に制限しました。静的検査と検証シェルの再生試験は通過済みです。Codon/CUDA実機実行は未実施です。

* [161Py_kernel_no_stack_overflow_guard_probe.py](sandbox:/mnt/data/161Py_kernel_no_stack_overflow_guard_probe.py)
* [161Py_kernel_no_stack_overflow_guard_validate_full_once.sh](sandbox:/mnt/data/161Py_kernel_no_stack_overflow_guard_validate_full_once.sh)


"""

# 161 GPU no-stack-overflow-guard experiment:
#   Retain the fully validated 160 root-pop loop termination, 159 kernel-local
#   u32 mask constants, 158 function-id/classification-u32 arithmetic, 157
#   markctrl-u32 prepack, 156 initialized-ctrl-u32 packing, 155 ctrl0-u32
#   prepack, 154 GPU-input-u32 representation, and all earlier DFS/order
#   changes.  The sole kernel hot-path experiment removes the unreachable
#   `if sp>=MAXD` branch after push.  MAXD remains Static[int]=21 and all five
#   stack arrays remain unchanged.  GPU requests are limited on the host to the
#   proven N=5..27 range.  Host data layout, task order, cache format, DFS
#   transitions, and solution arithmetic remain unchanged from validated 160.
...
# MAXD=21 exposes indices 0..20 and retains zero spare frames.  161 removes the
# runtime overflow branch, so GPU execution is host-limited to this proven range.
MAXD:Static[int]=21

VERSION_TAG:str="161 no stack overflow guard over 160 root-pop termination - scorestripe v9 lanephase32 octetfirstpairlock29"
...
def kernel_dfs_iter_gpu(
    ...
    前提/不変条件:
      ...
      - 161ではpush後のruntime overflow guardを除去し、GPU呼出し側をN=5..27へ制限する。
    ホットパス（ソース引用）:
      bit = a & -a
      avail[sp] = a ^ bit
    """
    ...
    #       160でDFS終了判定をroot pop時へ移動済み。161はpush後の到達不能な
    #       stack overflow guardだけを除去する単独実験とする。
    ...
    # 161はGPU N=5..27に限定する。N>27を有効化する場合は深さ境界を再証明し、
    ...
    # 158/159/160/161: function-id, flags, and mask operands stay in u32.
    ...
    while True:
      ...
      if (cv&INIT_MASK)==u32(0):
        # 158/159/160/161: function-id and every classification-mask operand stay in u32.
        ...
        if frame_blocksu!=u32(0):
          ...
          if ((MASK_K_N3>>fu)&u32(1))!=u32(0):
            frame_ktu=u32(1)
          if ((MASK_K_N4>>fu)&u32(1))!=u32(0):
            frame_ktu=u32(2)
        cv=(
          INIT_MASK
          |nextfidu
      ...
      # 1bit 展開
      #----------------
      # 151/152/153/154/155/156/157/158/159/160/161: a already holds avail[sp].  The jmark path updates both a and
      ...
      # 次状態計算（2値選択はそのまま）
      #----------------
      # 151/152/153/154/155/156/157/158/159/160/161: cv is the initialized ctrl value (or the already-initialized
      # loop-head value), so no second ctrl[sp] read is required.
      nld:u32=u32(0)
      nrd:u32=u32(0)
      if (cv&u32(8192))!=u32(0):  # use_blocks bit13
        # 150/151/152/153/154/155/156/157/158/159/160/161: ctrl metadata remains u32.  ktu is 0/1/2 only; the two
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False,chunk_only:bool=False,debug_chunk_start:int=0,debug_chunk_count:int=1)->None:
  ...
  if use_gpu:
    ...
    # 154/155/156/157/158/159/160/161: convert GPU scalar inputs once per host invocation, not once per GPU thread.
...
####################################################################################################
# 161 no-stack-overflow-guard over validated 160 root-pop termination and the 148 score-aware chunk-shape layer
...
#   - host shaping is byte-equivalent to validated 160; 161 removes only the unreachable GPU push overflow branch
#
...
CHUNKSHAPE148_DEFAULT_REASON:str="reuse validated 160 scorestripe_v9 task order/cache unchanged; 161 removes unreachable push overflow guard"
...
def chunkshape148_score_key_from_soa(soa:TaskSoA,idx:int,global_idx:int)->int:
  """
  161 reuses the validated 160/159/158/148 score key for host-side shaping only.

  The raw score, percentile thresholds, 32-lane tie extraction, 140 +11/+1 pair selector,
  and 141 +29 first-pair lock are carried forward unchanged.  The 161 experiment changes only removal of the unreachable GPU push overflow branch.
...
def build_chunkshape148_reordered_bin(
  ...
  """
  161 host-side score-aware lanephase chunk shaping is reused unchanged from validated 160/159/158/148.

  No score-key, quota, bucket-count, lane-extraction, within-bucket stride, +11/+1 pair-selector,
  or +29 octet first-pair-lock change.  The 161 experiment removes only the unreachable GPU push overflow branch; the 148 host-shaping cache is reused unchanged.
...
def exec_solutions_gpu_chunk_profile(
  ...
  # 154/155/156/157/158/159/160/161: fixed-width GPU scalar arguments are prepared once on the host.
  board_mask_gpu:u32=u32(board_mask)
  n3_gpu:u32=u32(1)<<u32(N-3)
  n4_gpu:u32=u32(1)<<u32(N-4)
...
def main()->None:
  ...
  # 161 safety envelope: the removed kernel guard was proven unreachable only
  # for the existing dynamic-preset range N=5..27.
  if use_gpu and (nmin<5 or nmax>28):
    print(f"[error] 161 no-stack-overflow-guard GPU candidate supports N=5..27 only; requested N={nmin}..{nmax-1}")
    return
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split161_no_stack_guard_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split161_no_stack_guard_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split161-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split161-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split161_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split161-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split161-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split161_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
