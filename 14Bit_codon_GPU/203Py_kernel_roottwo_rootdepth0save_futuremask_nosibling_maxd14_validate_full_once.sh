...
# =============================================================================
# NQ_UPDATE_MEMO
# 202: 更新メモ: 201は正当性OKだが197比で遅いため不採用。197 root<=2 direct-if prerollを親に戻し、MAXD14 kernelのみroot-prerollの残りroot sibling保存をroot_secondから行う微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 203 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 203 starts from
# validated 197 because 198/199/200/201/202 were correct but did not beat the
# 197 direct-if baseline.
#
...
# and wider roots remain generic. The root<=2 predicate stays the 197
# second-lowbit direct-if form. Only the root-preroll save of the remaining
# root sibling changes: cur_depth is known to be 0 there, so the saved avail
# payload is just cur_avail instead of cur_avail | (cur_depth << 27). Generic
# DFS saves keep the validated packed-depth payload. Host task order, dispatch,
# cache, progress verification, and README-managed history remain unchanged.
#
# Shell policy:
#   - STATIC_ONLY=1 does not take the full-run lock.
#   - source static-check failure prints the summary before exiting.
#   - root-depth0-save source check is whitespace/comment robust.
SRC=${SRC:-./203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 203 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/203Py_kernel_roottwo_rootdepth0save_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 203Py source
#   has a harmless header/string/layout difference. This 203 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 197 root one/two-candidate direct-if futuremask/no-sibling MAXD14 dispatch; 198 tailtest, 199 eqsecond, 200 prfuturebit, 201 prjmarkdirect, and 202 rootsecondsave correct but not adopted; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate direct-if predicate + root-preroll depth0 save payload	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
marker='203root_depth0_save:'
# The marker appears in a comment; use it only to split the source around the active root-preroll body.
root_pos=compact.find(marker)
if root_pos < 0:
    root_pos=compact.find('root_after_second:u32=root_rest^root_second')
...
checks = {
    'root_after_second_code': 'root_after_second:u32=root_rest^root_second' in compact,
    'root_direct_if': 'ifroot_after_second==u32(0):' in compact,
    'root_preroll_depth0_payload': 'avail[save_sp]=cur_avail' in root_block,
    'root_preroll_old_depth_pack_absent': 'avail[save_sp]=cur_avail|(u32(cur_depth)<<u32(27))' not in root_block,
    'generic_depth_pack_present': 'avail[save_sp]=cur_avail|(u32(cur_depth)<<u32(27))' in generic_block,
    # 203 returns to 197 payload source; 202 root_second payload is not adopted.
    ...
    # 203 returns to 197 root-preroll child_jmark scalar; 201 direct bit0 is not adopted.
    ...
    # 203 returns to 197 root-preroll future guard; 200 direct pr_nibble bit8 is not adopted.
...
if all(checks.values()):
    sys.exit(0)
print('source_rootdepth0_save_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootdepth0_save	present_depth0_save_payload	present_depth0_save_payload	OK
' >> "$SUMMARY"
else
  printf 'source_rootdepth0_save	present_depth0_save_payload	missing_or_old_depth_pack	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
allowed={'split145','split203'}
bad=sorted(tags-allowed)
if 'split203' in tags and not bad:
    sys.exit(0)
...
PYSPLIT2
  ...
  printf 'source_split_tag	split203 active preferred	%s	OK
...
PYSPLIT3
  ...
  printf 'source_split_tag	split203 active preferred	%s	FAIL
' "$actual_split" >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 203 runtime-only harness disabled source/model assertions"
...
if (( static_failures != 0 )); then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 203 runtime-only harness disabled source/model assertions"
  echo "[error] source roottwo-rootdepth0save static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 201 roottwo-prjmarkdirect futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 203 roottwo-rootdepth0save futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
