...
# 168 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 168 inherits the
# validated 167 static MAXD 16/18/20/21 host dispatch.  Its sole kernel change
# replaces u32 ctrl[MAXD] with four-byte-per-u32 packed opcodes generated once
# before DFS.  For N=21 all launches must remain required_maxd=14, select
# MAXD16, use four schedule words, and report 272 local stack bytes/thread.
#
# The 167 log's summary row "dispatch_max_required expected=16 actual=14" was
# a validation-harness expectation error, not a kernel/capacity error.  This
# harness checks the observed exact requirement (14) separately from the
# selected static capacity (16).

SRC=${SRC:-./168Py_kernel_packed_opcode_schedule_probe.py}
CAND=${CAND:-./168Py_kernel_packed_opcode_schedule_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/168Py_kernel_packed_opcode_schedule_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
EXPECTED_SELECTED_MAXD=16
EXPECTED_SCHEDULE_WORDS=4
EXPECTED_STACK_BYTES=272
EXP01=16879968420
EXP02=19219113480
EXP03=16935522136
EXP04=16230260724
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 168 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/168Py_kernel_packed_opcode_schedule_logs_N21_full_once_${TS}"
...
# ---------------- structural checks ----------------
word_counts={16:4,18:5,20:5,21:6}
...
assert '168 packed-opcode-schedule GPU candidate supports N=5..27 only' in s
...
def words_for(md):
    return {16:4,18:5,20:5,21:6}.get(md,0)
...
assert {md:stack_bytes(md) for md in word_counts}=={16:272,18:308,20:340,21:360}
...
printf 'source_parent_baseline\tvalidated fastest 167 static MAXD dispatch\tverified\tOK\n' >> "$SUMMARY"
printf 'source_packed_opcode_schedule_shape\t4 static kernels + packed byte schedule\tverified\tOK\n' >> "$SUMMARY"
...
printf 'dfs_164_vs_168_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 168 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD16, schedule_words=4, stack=272 bytes/thread"
  echo "================================================================"
} | tee "$RUN_LOG"
...
record_check "dispatch_non_272_bytes" "0" "$DISPATCH_NON272" || failures=$((failures+1))
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "167:$BASELINE_167_SECONDS" "166:$BASELINE_166_SECONDS" "165:$BASELINE_165_SECONDS" "164:$BASELINE_164_SECONDS" "162:$BASELINE_162_SECONDS"; do
...
echo "[validation-ok] 168 packed opcode schedule reproduced cases 01-06 with required=14, MAXD16, 272 bytes/thread"
