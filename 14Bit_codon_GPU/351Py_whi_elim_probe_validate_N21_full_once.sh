...
# =============================================================================
# 351 N=21 full validation harness (w_hi_arr elimination)
#
# 351 IS A SINGLE-VARIABLE MEASUREMENT REVISION on top of the adopted 350, and
# it is THE FIRST REVISION SINCE 328 THAT TOUCHES THE KERNEL. Every revision
# from 339 to 350 changed host-side reordering only.
#
# WHAT IT CHANGES. 328 split the u64 w array into two densely-packed u32 arrays
# at the dispatch boundary, which took L2 Theoretical Sectors Global Excessive
# on the w read from about 25000 to zero. But symmetry() returns only u64(2),
# u64(4) or u64(8), so every element of w_arr is at most 8 and the HIGH array
# was identically zero: the three loads of it in each of the five kernel
# epilogues always returned 0. 351 deletes it.
#
#   5 kernel signatures        one pointer parameter removed
#   15 epilogue expressions    reconstruction collapses to one widened u32 load
#   dispatcher                 builds one array instead of two
#   5 launch argument lists    one gpu.raw argument removed
#   dispatcher (added)         host-side invariant guard, see below
#
# THE U64 MULTIPLY IS DELIBERATELY LEFT ALONE. The compiler cannot know the
# high operand is always zero, so nothing about the multiply shrinks here.
# Folding the exponent into the unused markctrl bits 20-21, which degenerates
# that multiply into a shift, is 352. THE TWO STEPS ARE KEPT SEPARATE ON PURPOSE
# so that a regression in either one can be attributed to that one.
#
# THE ONE ADDITION IS AN INVARIANT GUARD. The correctness of the removal rests
# entirely on symmetry() staying inside 32 bits. The N=21 oracle would catch a
# violation only as a wrong total, and only for N=21. So the dispatcher ORs
# every element of w_arr and, if the high half is ever nonzero, prints a
# whi-error line and returns False -- which error_or_mismatch_hits below turns
# into a validation FAIL. One OR pass over 743424 elements costs well under a
# millisecond against a ~400s run, and it is CHEAPER than the second array 350
# was building, so host cost goes down rather than up.
#
# 350 IS CONFIRMED (2026-07-28):
#     default (mode 9)          398.733s
#     in-session mode 1 control 402.413s      -0.914%
#     correctness 314666222712 on both runs, cache_state=reuse on both,
#     zero FAIL. That is -0.064% from the 349 measurement of 398.988s, and the
#     mode 1 control moved +0.028% from 402.301s -- a fourth consecutive
#     session inside the 0.03% noise floor.
#
# PRE-REGISTERED PREDICTION. The saving is one u32 load and one shift-or pair
# per constellation, 2025282 of them in total, about 8.1 MB of traffic, against
# an inner DFS loop whose node count is orders of magnitude larger and which
# does not change by a single instruction. The expected effect is therefore
# between 0.00 and -0.15 percent: COMPARABLE TO OR BURIED INSIDE THE 0.03
# PERCENT NOISE FLOOR.
#
# PRE-REGISTERED DECISION RULE.
#   within +0.10% of the 350 anchor (improvement or neutral) -> ADOPT, go to 352
#   regression beyond +0.10%                                 -> revert, and
#     treat it as the finding that the epilogue is not free and that register
#     allocation or instruction scheduling moved; redesign 352 accordingly.
# Neutral counts as a pass because the change removes provably dead work and is
# the prerequisite for 352.
#
# THE ANCHOR MUST BE TAKEN IN THE SAME SESSION, by rebuilding and running 350.
# 351 HAS NO KNOB to switch the removal off: keeping both kernel variants in one
# binary would change register allocation and stop this being a single-variable
# experiment. 350 differs from 349 by two default constants only, so rebuilding
# it is cheap.
#
# CACHE IS NOT AFFECTED. The shaped bin is a host-side product and host-side
# reordering is byte-identical to 350 (CHUNKSHAPE148_ITER_SORT=9,
# CHUNKSHAPE148_BUCKET_RUN=2048, w3_j7, K=48, BROADMARK_VARIANT=2, every mode
# body in chunkshape148_reorder_group). All runs reuse the existing
# ..._isort9.bin and must report chunkshape148_cache_state=reuse. THERE IS NO
# BUILD RUN in this revision, so every run is timing-comparable.
#
# STATIC CHECKS: THE TWO 328 CHECKS WERE INVERTED, NOT DELETED.
#   source_warr_soa_split_signatures -> source_whi_elim_signatures
#   source_warr_soa_split_dispatcher -> source_whi_elim_dispatcher
# Both now require the parameter to be ABSENT. Three checks were added:
#   source_whi_elim_kernel_bodies  no reference in any kernel body, and exactly
#                                  15 single-load epilogue sites
#   source_whi_zero_guard          the four elements of the guard are present
#   runtime_whi_elim               the run log carries whi_elim=removed, which
#                                  proves a 350 binary did not slip in
# Every one of these is scoped to signature lines and the dispatcher body with
# docstrings AND comments stripped. This project pastes chat logs into
# docstrings, so a whole-file grep for an identifier cannot be trusted -- the
# 345 r1 and 349 r1 failures were both of exactly that kind.
#
# MANDATORY VALIDATION ORDER:
#   1. STATIC_ONLY=1 first.
#   2. The 350 anchor, in this session. Cache MUST report reuse and elapsed
#      must land near 398.7s. If it does not, stop: nothing downstream is
#      comparable.
#   3. Then 351 twice. Cache MUST report reuse on both.
#   4. Confirm 314666222712 FIRST on every run, before any timing.
#
#     STATIC_ONLY=1 bash 351Py_whi_elim_probe_validate_N21_full_once.sh
#     bash 350Py_condserp_adopt_validate_N21_full_once.sh
#     bash 351Py_whi_elim_probe_validate_N21_full_once.sh
#     bash 351Py_whi_elim_probe_validate_N21_full_once.sh
#
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./351Py_whi_elim_probe.py}
CAND=${CAND:-./351Py_whi_elim_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/351Py_whi_elim_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/351Py_whi_elim_probe_logs_N21_full_once_${TS}"
...
echo "[start] 351 whi-elim validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 351 whi-elim SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '351 whi-elim' "$SRC"; then
  printf 'source_version_tag	351 whi-elim	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	351 whi-elim	missing	FAIL
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 351 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 351 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 351 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 351 whi-elim (host-side reordering byte-identical to 350/349/348/347/346/345/344/338/333) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT; 351 is the FIRST REVISION SINCE 328 THAT TOUCHES THE KERNEL. It removes the identically zero high half of the SoA w split: symmetry() returns only 2, 4 or 8, so the high u32 array was always zero and the three loads of it in each of the five kernel epilogues always returned 0. One pointer parameter leaves each signature, fifteen reconstruction expressions collapse to a single widened u32 load, the dispatcher builds one array instead of two, and a host-side guard ORs all of w_arr and aborts if the high half is ever nonzero. The u64 multiply is deliberately untouched; folding the exponent into markctrl bits 20-21 is 352. Expected 0.00 to -0.15 percent against the in-session 350 anchor of 398.733s, comparable to or buried inside the 0.03 percent noise floor; neutral within +0.10 percent counts as a pass. The shaped bin is the same file as 350 and must be reused, so there is no build run in this revision"
...
echo "[validation-ok] 351 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged), CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN and CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT (both unchanged from 350). 351 removes the identically zero high half of the 328 SoA w split from all five kernels and the dispatcher, and adds a host-side guard that aborts if symmetry() ever leaves 32 bits. Compare against the in-session 350 anchor, not against any earlier session: adopt within +0.10 percent, revert beyond it. The u64 multiply is still there on purpose -- 352 folds the exponent into markctrl bits 20-21 and turns it into a shift"
