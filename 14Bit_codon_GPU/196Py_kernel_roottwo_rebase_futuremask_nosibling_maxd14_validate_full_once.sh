...
# =============================================================================
# NQ_UPDATE_MEMO
# 196: 更新メモ: 195は正当性OKだが194比0.450秒遅く不採用。194 root one/two-candidate prerollへ戻し、MAXD14 kernelのみroot<=2判定をsecond-lowbit方式に変更する復帰・微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 196 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 196 starts from
# validated 194 because 195 root three-candidate preroll was correct but 0.450s
# slower than the 194 fastest baseline.
#
...
# before the generic DFS loop. 3-bit and wider roots return to the generic 194
# loop. The root<=2 predicate is implemented as a second-lowbit test.
# Host task order, dispatch, cache, progress verification, and README-managed history remain unchanged.
SRC=${SRC:-./196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 196 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/196Py_kernel_roottwo_rebase_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 196Py source
#   has a harmless header/string/layout difference. This 196 harness validates the
...
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate preroll rebase	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'root_two_predicate_rebase' "$SRC" && grep -q 'root_second:u32=root_rest&(u32(0)-root_rest)' "$SRC" && grep -q 'root_after_second:u32=root_rest\^root_second' "$SRC" && grep -q 'if root_after_second==u32(0):' "$SRC"; then
  printf 'source_root_two_predicate_rebase	present	present	OK
' >> "$SUMMARY"
else
  printf 'source_root_two_predicate_rebase	present	missing	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split196' "$SRC" && ! grep -q 'split195' "$SRC" && ! grep -q 'split194' "$SRC" && ! grep -q 'split193' "$SRC" && ! grep -q 'split192' "$SRC" && ! grep -q 'split191' "$SRC" && ! grep -q 'split190' "$SRC" && ! grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split196 only	split196 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split196 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 196 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source roottwo-rebase static checks failed" >&2
  exit 66
fi
...
echo "[validation-ok] 196 roottwo-rebase futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
