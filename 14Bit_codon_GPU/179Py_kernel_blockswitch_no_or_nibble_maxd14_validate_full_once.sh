...
# 179 single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total.  Candidate 179 inherits the
# validated fastest 175 nibble MAXD14 static dispatch.  Its single
# kernel experiment keeps the 172 register-held active frame and 13-slot
# ancestor stack, backs out the slower 176 current-nibble register and 177 terminal-first changes,
# then changes only the MAXD14 block-code decode to a direct constant switch.
#
# For N=21 all launches must remain required_maxd=14, select MAXD14, report
# schedule_words=0 and 208 local stack bytes/thread.

SRC=${SRC:-./179Py_kernel_blockswitch_no_or_nibble_maxd14_probe.py}
CAND=${CAND:-./179Py_kernel_blockswitch_no_or_nibble_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/179Py_kernel_blockswitch_no_or_nibble_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
BASELINE_177_SECONDS=${BASELINE_177_SECONDS:-587.077}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 179 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/179Py_kernel_blockswitch_no_or_nibble_maxd14_logs_N21_full_once_${TS}"
...
assert '179 no-or block-switch nibble GPU candidate supports N=5..27 only' in s
...
# schedule fields, the 172 register-held active frame, and the 179 no-or block-switch nibble hot path.
...
for marker in (
    ...
    '# 179: MAXD14 block-code switch, PTX-safe no-or form.',
    'if block_code==u32(0):',
...
def nibble_from_byte(op):
    # 175 keeps the byte-opcode low nibble directly:
...
print('MAXD14_NO_OR_BLOCKSWITCH_NIBBLE_SCHEDULE=1')
...
PYMODEL
then
  printf 'source_no_or_blockswitch_nibble_shape\trequested layout\tmodel/static failure\tFAIL\n' >> "$SUMMARY"
...
printf 'source_no_or_blockswitch_nibble_shape\tMAXD14 scalar nibble schedule + direct block-code switch + register-held current frame + 13-slot ancestor stack\tverified\tOK\n' >> "$SUMMARY"
...
printf 'maxd14_no_or_blockswitch_nibble_schedule\t1\t%s\tOK\n' "$MAXD14_NO_OR_BLOCKSWITCH_NIBBLE_SCHEDULE" >> "$SUMMARY"
...
printf 'dfs_164_vs_179_no_or_blockswitch_nibble_equivalence\t>=10000 cases\t%s cases\tOK\n' "$DFS_EQUIVALENCE_CASES" >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-ok] 179 source/model checks passed"
  exit 0
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 175 nibble schedule + no-or direct block-code switch + register cache + ancestor13 stack"
  echo "================================================================"
} | tee "$RUN_LOG"
...
# 179 keeps the validated 175 kernel structure, then derives chunk-tail diagnostics from
...
echo "[validation-ok] 179 no-or block-switch nibble schedule reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
