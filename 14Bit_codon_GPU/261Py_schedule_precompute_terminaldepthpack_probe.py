...
# =============================================================================
# NQ_UPDATE_MEMO
# 261: schedule-precompute-terminaldepthpack probe。256 terminaldepthpack JIT成立と
...
#      261では terminal_depth だけを単独 low-bit 配列 sched_sched_terminal_depth_arr から
...
"""
261 schedule-precompute note: 256 terminaldepthpack JIT成立と257/259/260 root_action mask系PTX/JIT不成立を受け、root_actionは別配列素通しへ固定し、terminal_depthだけをsched_sched_terminal_depth_arrからlow-bit decodeするprobe。


作成しました。261は **terminal_depth 単独low-bit decode 切り分け版**です。

260で `root_action` の mask decode 系がJIT不成立だったため、261では **root_action は別配列素通しへ固定**し、`terminal_depth` だけを単独でpack/decodeします。

```text
261:
  child/future maskpack       維持
  terminal_base14 pack        維持
  root_action                 別配列素通しへ固定
  terminal_depth              sched_terminal_depth_arr[i] & 15 で単独decode
```
...
source_schedule_precompute_terminaldepthpack OK
source_host_precompute_terminaldepthpack_arrays OK
release_build_policy                       OK
```

ファイルはこちらです。

* [261Py_schedule_precompute_terminaldepthpack_probe.py](sandbox:/mnt/data/261Py_schedule_precompute_terminaldepthpack_probe.py)
* [261Py_schedule_precompute_terminaldepthpack_validate_N21_full_once.sh](sandbox:/mnt/data/261Py_schedule_precompute_terminaldepthpack_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [261Py_schedule_precompute_terminaldepthpack_sha256.txt](sandbox:/mnt/data/261Py_schedule_precompute_terminaldepthpack_sha256.txt)
* [261Py_schedule_precompute_terminaldepthpack_bundle_20260707.tar](sandbox:/mnt/data/261Py_schedule_precompute_terminaldepthpack_bundle_20260707.tar)

sha256です。

```text
fde02c22839c2c4731de56c66ec896f5c8649aed32c8b420329d64dcf7e41b26  261Py_schedule_precompute_terminaldepthpack_probe.py
fb1f889dbc5c12a33b2a20642b2f5889b0c255c6cdddf8f8f6bd0d5553be1573  261Py_schedule_precompute_terminaldepthpack_validate_N21_full_once.sh
99f753ce7b0297bf933ac7f4d8281915ed6be8639e2af59e1226afa5e5645c07  README.md
```

実行は以下でお願いします。

```bash
rm -f ./261Py_schedule_precompute_terminaldepthpack_probe

STATIC_ONLY=1 bash 261Py_schedule_precompute_terminaldepthpack_validate_N21_full_once.sh

bash 261Py_schedule_precompute_terminaldepthpack_validate_N21_full_once.sh
```

261がJIT成立すれば、`terminal_depth` 単独decodeは安全、JIT不成立源はほぼ `root_action` 側に限定できます。
...
VERSION_TAG:str="261 schedule-precompute-terminaldepthpack probe: parent 256 terminaldepthpack JIT-ok / 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; child/future maskpack and terminal_base14 pack retained; 257/259/260 root_action mask decodes hit CUDA_ERROR_INVALID_PTX so root_action stays separate pass-through; only terminal_depth is decoded from sched_sched_terminal_depth_arr & 15; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 261: 256 terminaldepthpack JIT-ok shape plus terminal_depth-only low-bit decode
    # from sched_sched_terminal_depth_arr. root_action remains separate pass-through
    # because 257/259/260 root_action mask variants hit CUDA_ERROR_INVALID_PTX.
    self.sched_lo_arr:List[u32]=[u32(0)]*m
    self.sched_hi_arr:List[u32]=[u32(0)]*m
    self.sched_mask_arr:List[u32]=[u32(0)]*m
    self.sched_terminal_depth_arr:List[u32]=[u32(0)]*m
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 261: host-side clone of the former MAXD14 kernel schedule builder.
  # Keeps 244 r4 maskpack and 256 terminal_base14 pack. Changes only
  # terminal_depth storage to sched_sched_terminal_depth_arr; root_action remains separate.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    sched_terminal_depth_arr:Ptr[u32],sched_base14_arr:Ptr[u32],root_action_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 256: schedule fields are precomputed on the host in build_soa_for_range().
    # child/future masks are packed; terminal_base14 is decoded from its own
    # low-bit arrays; root_action remains separate pass-through.
    ...
    terminal_depth:int=int(sched_terminal_depth_arr[i]&u32(15))
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.sched_terminal_depth_arr),gpu.raw(soa.sched_base14_arr),gpu.raw(soa.root_action_arr),
...
def build_soa_for_range(
    ...
    for t in range(m):
        ...
        soa.sched_terminal_depth_arr[t]=terminal_depth_u&u32(15)
    ...
    while off < m_all:
      ...
      if use_sorted:
        ...
        for p in range(m):
          ...
          sort_soa.sched_terminal_depth_arr[p]=soa.sched_terminal_depth_arr[q]
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split261_schedpre_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split261_schedpre_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      progress_suffix:str=f"split261_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
