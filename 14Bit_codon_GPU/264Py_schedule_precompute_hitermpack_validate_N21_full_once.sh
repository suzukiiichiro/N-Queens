...
# =============================================================================
# 264 N=21 full validation harness
# Parent: 263 hitermpack JIT-ok / 262 terminalbasepack JIT-ok / 244 r4 maskpack / 241 restore239.
# Experiment: keep root_action separate and pack schedule_hi with terminal_depth/base14
#             into sched_hi_arr high bits while retaining child/future maskpack and
#             single-launch mode31 split145. This tests whether high-bit hi+terminal
#             pack is JIT-safe without root_action.
...
SRC=${SRC:-./264Py_schedule_precompute_hitermpack_probe.py}
CAND=${CAND:-./264Py_schedule_precompute_hitermpack_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/264Py_schedule_precompute_hitermpack_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/264Py_schedule_precompute_hitermpack_logs_N21_full_once_${TS}"
...
echo "[start] 264 schedule-precompute-hitermpack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 264 schedule-precompute-hitermpack ctrl0-reuse-hi-terminal-pack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '264 schedule-precompute-hitermpack probe' "$SRC"; then
  printf 'source_version_tag\t264 schedule-precompute-hitermpack probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t264 schedule-precompute-hitermpack probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
old_split_tags = ['split240_','split241_','split242_','split243_','split244_','split255_','split256_','split257_','split258_','split259_','split260_','split261_','split262_','split263_']
runtime_split_ok = ('split264_' in s and not any(tag in s for tag in old_split_tags))
if runtime_split_ok:
    checks.append(('source_split_tag', 'split264 active; old split runtime tags absent', 'split264', True))
else:
    checks.append(('source_split_tag', 'split264 active; old split runtime tags absent', 'old split runtime tag present or split264 missing', False))
...
if not m14:
    checks.append(('source_schedule_precompute_hitermpack', 'MAXD14 schedule_hi+terminal precompute pack active', 'MAXD14 kernel not found', False))
else:
    body = m14.group(0)
    ok = (
        ...
        'sched_hi_arr:Ptr[u32]' not in body and
        'sched_lo_arr:Ptr[u32]' not in body and
        'sched_terminal_arr:Ptr[u32]' not in body and
        'schedule_lo:u32=ctrl0_arr[i]' in body and
        'sched_hi_term:u32=markctrl_arr[i]' in body and
        'schedule_hi:u32=sched_hi_term&u32(16777215)' in body and
        'terminal_depth:int=int((sched_hi_term>>u32(24))&u32(15))' in body and
        'terminal_base14:u32=(sched_hi_term>>u32(28))&u32(1)' in body and
        ...
        'sched_root_action_arr:Ptr[u32]' not in body
    )
    actual = 'hitermpack active' if ok else 'hitermpack missing/unsafe'
    checks.append(('source_schedule_precompute_hitermpack', 'MAXD14 schedule_hi+terminal precompute pack active', actual, ok))

host_ok = (
    has_def('precompute_maxd14_schedule_fields') and
    'self.sched_lo_arr:List[u32]' in s and
    'self.sched_hi_term_arr:List[u32]' in s and
    'self.sched_mask_arr:List[u32]' in s and
    'self.root_action_arr:List[u32]' in s and
    'soa.sched_hi_term_arr[t]=(sched_hi&u32(16777215))|((terminal_depth_u&u32(15))<<u32(24))|((sched_base14&u32(1))<<u32(28))' in s and
    'soa.ctrl0_arr[ri]=soa.sched_lo_arr[ri]' in s and
    'soa.markctrl_arr[ri]=soa.sched_hi_term_arr[ri]' in s and
    'gpu.raw(soa.sched_hi_arr)' not in s and
    'gpu.raw(soa.sched_mask_arr)' in s and
    'gpu.raw(soa.root_action_arr)' in s and
    'gpu.raw(soa.sched_lo_arr)' not in s and
    'gpu.raw(soa.sched_hi_term_arr)' not in s and
    'self.sched_terminal_arr' not in s and
    'sched_ctrl_arr' not in s
)
checks.append(('source_host_precompute_hitermpack_arrays', 'host sched_hi+terminal pack + root_action pass-through', 'host hitermpack present' if host_ok else 'host hitermpack missing/unsafe', host_ok))
fail = 0
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 264 schedule-precompute-hitermpack mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "241parent:$BASELINE_241_SECONDS" "263:$BASELINE_263_SECONDS" "262:$BASELINE_262_SECONDS" "239:$BASELINE_239_SECONDS" "242rejected:$BASELINE_242_REJECT_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 264 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
