...
"""
267 schedule-precompute-root0countcache note: 266 root0probe JIT成立/正当性OK/precompute系最良を親に、root_action_nonzero_count を build_soa_for_range 側で集計して TaskSoA に保持する。MAXD14 launch直前のrepack loopでは root_action_arr を再走査せず、既存の root0/fallback dispatchを維持する。root_action pack/mask decode は引き続き入れない。mode28/29/30/31、cache生成、MAXD fallback、worker splitは保持。


266ログは問題ありません。**266はprecompute系最良として、次版の親にしてよい**です。
...
266は265から **1.120秒改善**しており、`root_action==0` 専用kernelの方向は有効です。ただし241の `0:07:07.788` にはまだ `0.325秒` 届かないため、本線採用基準は241のままです。

267は、266のroot0/fallback構造を維持したまま、`root_action_nonzero_count` を `build_soa_for_range()` 側で集計して `TaskSoA` に保持する版にしました。MAXD14 launch直前のrepack loopでは `root_action_arr` を再走査せず、cached countで root0/fallback を選びます。
...
source_schedule_precompute_root0countcache OK
...
* [267Py_schedule_precompute_root0countcache_probe.py](sandbox:/mnt/data/267Py_schedule_precompute_root0countcache_probe.py)
* [267Py_schedule_precompute_root0countcache_validate_N21_full_once.sh](sandbox:/mnt/data/267Py_schedule_precompute_root0countcache_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [267Py_schedule_precompute_root0countcache_sha256.txt](sandbox:/mnt/data/267Py_schedule_precompute_root0countcache_sha256.txt)
* [267Py_schedule_precompute_root0countcache_bundle_20260707.tar](sandbox:/mnt/data/267Py_schedule_precompute_root0countcache_bundle_20260707.tar)

sha256です。

```text
e47d1a642e6e57e94580772b36d3028dbdb20ff06a4f120b1f5f9270056d5b4b  267Py_schedule_precompute_root0countcache_probe.py
a0acf5f5dc8f9873dfb46fe489675eee98a7527204a2782c303a3a6d665bee61  267Py_schedule_precompute_root0countcache_validate_N21_full_once.sh
27daf9c83e9fc00fb86453b9f1d94dec882d980bf6c93538ea5e8c66a2c34134  README.md
```

実行は以下でお願いします。

```bash
rm -f ./267Py_schedule_precompute_root0countcache_probe

STATIC_ONLY=1 bash 267Py_schedule_precompute_root0countcache_validate_N21_full_once.sh

bash 267Py_schedule_precompute_root0countcache_validate_N21_full_once.sh
```
...
VERSION_TAG:str="267 schedule-precompute-root0countcache: parent 266 root0probe precompute-best; single launch per chunk kept; TaskSoA caches root_action_nonzero_count during host schedule precompute, so MAXD14 repack loop no longer rescans root_action_arr; root0 kernel/fallback dispatch preserved; root_action pack/mask decode still rejected; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
def kernel_dfs_iter_gpu_maxd14_root0(
    ...
    terminal_depth:int=int(sched_terminal&u32(15))
    terminal_base14:u32=(sched_terminal>>u32(4))&u32(1)
    # 267 root0 kernel: root_action is statically zero for all tasks in this launch.
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    # 267: selected_MAXD14 is known here, so original ctrl0/markctrl/free
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split267_root0countcache_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split267_root0countcache_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      if not (reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname)):
        if gpu_log_level>=1:
          print(f"[split267-base-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      elif gpu_log_level>=1:
        print(f"[split267-base-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      ...
      progress_suffix:str=f"split267_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
