...
# =============================================================================
# NQ_UPDATE_MEMO
# 199: 更新メモ: 198は正当性OKだが197比0.037秒遅いため不採用。197 root<=2 direct-if prerollを親に、MAXD14 kernelのみroot<=2判定をroot_rest==root_second比較へ変更する微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 199 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 199 starts from
# validated 197 because 198 was correct but did not beat the 197 baseline.
#
...
# and wider roots remain generic. The root<=2 predicate keeps the second-lowbit
# helper but replaces the 197 after-second-XOR comparison with the equivalent
# root_rest==root_second comparison, removing one XOR scalar from the predicate.
# Host task order, dispatch, cache, progress verification, and README-managed
# history remain unchanged.
SRC=${SRC:-./199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 199 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/199Py_kernel_roottwo_eqsecond_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 199Py source
#   has a harmless header/string/layout difference. This 199 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 197 root one/two-candidate direct-if futuremask/no-sibling MAXD14 dispatch; 198 tailtest correct but not adopted; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate root_rest==root_second preroll predicate	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'root_two_eqsecond' "$SRC" && grep -q 'root_second:u32=root_rest&(u32(0)-root_rest)' "$SRC" && grep -q 'if root_rest==root_second:' "$SRC" && ! grep -q 'root_after_second:u32' "$SRC" && ! grep -q 'root_rest_more:u32' "$SRC" && ! grep -q 'root_preroll:u32' "$SRC" && ! grep -q 'if root_preroll' "$SRC"; then
  printf 'source_root_two_eqsecond	present_no_aftersecond_xor	present_no_aftersecond_xor	OK
' >> "$SUMMARY"
else
  printf 'source_root_two_eqsecond	present_no_aftersecond_xor	missing_or_old_predicate	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if grep -q 'split199' "$SRC" && ! grep -q 'split198' "$SRC" && ! grep -q 'split197' "$SRC" && ! grep -q 'split196' "$SRC" && ! grep -q 'split195' "$SRC" && ! grep -q 'split194' "$SRC" && ! grep -q 'split193' "$SRC" && ! grep -q 'split192' "$SRC" && ! grep -q 'split191' "$SRC" && ! grep -q 'split190' "$SRC" && ! grep -q 'split189' "$SRC" && ! grep -q 'split188' "$SRC" && ! grep -q 'split187' "$SRC" && ! grep -q 'split186' "$SRC" && ! grep -q 'split185' "$SRC" && ! grep -q 'split184' "$SRC"; then
  printf 'source_split_tag	split199 only	split199 only	OK
' >> "$SUMMARY"
else
  printf 'source_split_tag	split199 only	check source	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 199 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source roottwo-eqsecond static checks failed" >&2
  exit 66
fi
...

{
  ...
  echo "dispatch  : required=14, selected MAXD14, schedule_words=0, stack=208 bytes/thread, 199 roottwo-eqsecond futuremask/no-sibling kernel + README history policy + tail-flatness metrics"
  echo "================================================================"
} | tee "$RUN_LOG"
...
echo "[validation-ok] 199 roottwo-eqsecond futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
