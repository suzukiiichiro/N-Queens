...
おはようございます。116Py を作成しました。

[116Py_kernel_split_probe.py](sandbox:/mnt/data/116Py_kernel_split_probe.py)

今回の 116 は、115Py の CPU 経路・既存 `kernel_dfs_iter_gpu`・broadmarktail / reorder 系を残したまま、GPU kernel 分割の実験経路を追加した版です。115Py は N>=21 で A10G 向けの `block=32 max_blocks=484 preset=7 bench_mode=29 w8_j7 variant=2` を既定にしているため、その流し込み設計は壊さず、kernel/DFS 側だけに手を入れています。

添付メモにあった本日の三点、つまり `funcid 0,4,7,5` の heavy 系専用 kernel 化、`B/d2/d1/d0` の大分類 kernel 化、そして kernel 内の `P5/base/mark/jmark` 判定削減に沿って実装しました。 13Py の `SQ*` 群は「枝刈り条件だけが違い、核心は共通」という構造なので、再帰関数をそのまま移植せず、115Py の `MAXD` stack 方式へ落とし込んでいます。

追加した主な kernel は以下です。

```text
kernel_dfs_heavy0475_gpu : funcid 0,4,5,7 heavy 系
kernel_dfs_B_gpu         : funcid 0..11, SQB 系
kernel_dfs_d2_gpu        : funcid 12..18, SQd2 系
kernel_dfs_d1_gpu        : funcid 19..25, SQd1 系
kernel_dfs_d0_gpu        : funcid 26..27, SQd0 系
```

実行モードも追加しています。

```text
mode 30 : split116 probe
          broadmarktail reorder bin を使い、選択 chunk だけ split kernel で実行

mode 31 : split116 full GPU
          broadmarktail reorder bin 全体を split kernel hybrid で実行
```

まずは probe から確認してください。

```bash
codon build -release 116Py_kernel_split_probe.py

# N22 の選択 chunk probe
./116Py_kernel_split_probe \
  -g 22 22 32 484 1 0 7 30 8 7 0 1222 5 "" 2
```
...
```bash
./116Py_kernel_split_probe \
  -g 22 22 32 484 1 0 7 31 8 7 0 0 1 2
```
...
重要な注意点として、こちらの環境には Codon/GPU 実行環境がないため、`codon build` と実機 GPU での `ok` 照合は未実施です。まず `mode 30` の数 chunk で total 不整合がないことを確認し、その後 `mode 31` で N22 全体比較へ進めるのが安全です。昨日の添付ログでは 115 系で CPU N22 と GPU N22 はいずれも `ok` まで確認されているので、116 の初期判定は 115 mode29 との比較が基準になります。

はい、これは **116 で追加した専用 `@gpu.kernel` 側の PTX 生成問題**と見てよいです。

`CUDA_ERROR_INVALID_PTX` は探索中の total 不一致ではなく、CUDA が Codon 生成 PTX を JIT compile する段階で拒否しています。115Py の GPU 経路は N21/N22 で `ok` 済みで、N>=21 では A10G 向けの `block=32 max_blocks=484 preset=7 bench_mode=29 w8_j7 variant=2` を使う設計になっているため、既存の `kernel_dfs_iter_gpu` は基準として信頼できます。

今回の 116 初版では、専用 kernel を一気に 5 本追加しました。

```text
kernel_dfs_heavy0475_gpu
kernel_dfs_B_gpu
kernel_dfs_d2_gpu
kernel_dfs_d1_gpu
kernel_dfs_d0_gpu
...
このどれか、または複数 kernel を同時に持つ構成が Codon/CUDA の PTX 生成に刺さった可能性が高いです。

対処として、**PTX 安定版の 116** を作りました。
新規専用 `@gpu.kernel` はすべて外し、GPU device code は 115 で実績のある `kernel_dfs_iter_gpu` のみに戻しています。ただし、host 側の `heavy0475 / B / d2 / d1 / d0 / rest` 分割、mode30/mode31、進捗 TSV、reordered bin 読み込みは残しています。

[116Py_ptx_safe_split_scheduler.py](sandbox:/mnt/data/116Py_ptx_safe_split_scheduler.py)

まずこちらで JIT が通るかを確認してください。

```bash
codon build -release 116Py_ptx_safe_split_scheduler.py
```

最初は N21 の probe 1 chunk だけで十分です。これは速度評価ではなく、**PTX JIT が通るか**と **bucket 分割後の total 集計が壊れていないか**を見るためです。

```bash
./116Py_ptx_safe_split_scheduler \
  -g 21 21 32 484 1 0 7 30 8 7 0 0 1
```
...
比較用の 115 相当経路は従来どおりです。

```bash
./116Py_ptx_safe_split_scheduler \
  -g 21 21 32 484 1 0 7 29 8 7 0 0 1 2
```

この PTX-safe 版は、**速度改善版ではありません**。5 bucket に分けてはいますが、各 bucket の実行 kernel はすべて 115 の汎用 `kernel_dfs_iter_gpu` です。したがって、launch 回数が増える分、mode31 は mode29 より遅くなる可能性があります。

目的は以下です。

```text
1. CUDA_ERROR_INVALID_PTX を止める
2. N21 で split scheduler / bin / progress / total を確認する
3. その後、117 で専用 kernel を 1 本ずつ追加する
```

次の専用 kernel 化は、いきなり 5 本ではなく、まず **d0 だけ**、または **d2 だけ**が安全です。今回のような PTX JIT 失敗を避けるため、117 では「追加 kernel 1 本 → N21 probe → N21 full → 次の kernel」という順に進めるのがよいです。
...
VERSION_TAG:str="116 kernel split probe - heavy0475 + B/d2/d1/d0 specialized GPU kernels from 115"
...
####################################################################################################
# 116 GPU kernel split probe
# 115 の汎用 kernel_dfs_iter_gpu を残したまま、funcid-heavy / B,d2,d1,d0 専用 kernel を追加する。
####################################################################################################

@gpu.kernel
def kernel_dfs_heavy0475_gpu(
    ld_arr:Ptr[int],rd_arr:Ptr[int],col_arr:Ptr[int],row_arr:Ptr[int],free_arr:Ptr[int],
    jmark_arr:Ptr[int],end_arr:Ptr[int],mark1_arr:Ptr[int],mark2_arr:Ptr[int],
    funcid_arr:Ptr[int],w_arr:Ptr[u64],
    results:Ptr[u64],
    ...
    """116: initial funcid 0/4/5/7 heavy 専用。到達し得る B 後続 1/2/3/6 だけを処理。"""
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        addv:int=0
        use_blocks:int=0
        use_future:int=1 if (f==2 or f==3) else 0
        nextfv:int=f
        bLv:int=0
        ktype:int=0

        at_mark:int=0
        if f==0 or f==5 or f==4 or f==7:
          if rowv==mark1:
            at_mark=1
        elif f==1 or f==6:
          if rowv==mark2:
        ...
        if at_mark==1 and a!=u32(0):
          ...
          if f==0:
            ...
            nextfv=2
          if f==1 or f==4 or f==5:
            bLv=1
          elif f==7:
            bLv=2
          if f==0 or f==6 or f==7:
            ktype=1
          elif f==4:
            ktype=2

        if f==2 and rowv==jmark:
          a&=~u32(1)
          avail[sp]=a
          if a==u32(0):
            sp-=1
...
def kernel_dfs_B_gpu(
    ...
    """116: B 大分類 funcid 0..11 専用。d2/d1/d0 判定を削除。"""
    INIT_MASK:Static[int]=524288
    ld=__array__[int](MAXD)
    rd=__array__[int](MAXD)
    col=__array__[u32](MAXD)
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        addv:int=0
        use_blocks:int=0
        use_future:int=1 if (f==2 or f==3 or f==8 or f==9 or f==10 or f==11) else 0
        nextfv:int=f
        bLv:int=0
        ktype:int=0
...
def kernel_dfs_d2_gpu(
    ...
    """116: d2 大分類 funcid 12..18 専用。SQd2B(fid=14) の free&~1 特例つき。"""
    INIT_MASK:Static[int]=524288
    ld=__array__[int](MAXD)
    rd=__array__[int](MAXD)
    col=__array__[u32](MAXD)
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        addv:int=0
        use_blocks:int=0
        use_future:int=1 if f==14 else 0
        nextfv:int=f
        bLv:int=0
        ktype:int=0

        at_mark:int=0
        if f==12 or f==15 or f==16 or f==18:
          if rowv==mark1:
            at_mark=1
        elif f==13 or f==17:
          if rowv==mark2:
        ...
        if at_mark==1 and a!=u32(0):
          ...
          if f==12:
            ...
            nextfv=14
          if f==13 or f==15 or f==16:
            bLv=1
          elif f==18:
            bLv=2
          if f==12 or f==17 or f==18:
            ktype=1
          elif f==15:
            ktype=2
...
def kernel_dfs_d1_gpu(
    ...
    """116: d1 大分類 funcid 19..25 専用。fid=20 の add1 特例を直接化。"""
    INIT_MASK:Static[int]=524288
    ld=__array__[int](MAXD)
    rd=__array__[int](MAXD)
    col=__array__[u32](MAXD)
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        addv:int=0
        use_blocks:int=0
        use_future:int=1 if f==21 else 0
        nextfv:int=f
        bLv:int=0
        ktype:int=0

        at_mark:int=0
        if f==19 or f==22 or f==23 or f==24:
          if rowv==mark1:
            at_mark=1
        elif f==20 or f==25:
          if rowv==mark2:
...
def kernel_dfs_d0_gpu(
    ...
    while sp>=0:
      ...
      if (cv0&u32(INIT_MASK))==u32(0):
        ...
        addv:int=0
        use_blocks:int=0
        use_future:int=1 if f==26 else 0
        nextfv:int=f
        bLv:int=0
        ktype:int=0
...
####################################################################################################
# 116 split-kernel chunk runner
####################################################################################################

"""116: SoA 1件を別 SoA へコピーする小ヘルパ。"""
...
"""116: split-kernel hybrid。heavy0475 を先に抜き、残りを B/d2/d1/d0 へ分ける。"""
def exec_solutions_gpu_chunk_split116(
  N:int,
  chunk_constellations:List[Dict[str,int]],
  gpu_block:int=32,
  gpu_max_blocks:int=484,
  ...
  h_soa:TaskSoA=TaskSoA(STEPS)
  b_soa:TaskSoA=TaskSoA(STEPS)
  d2_soa:TaskSoA=TaskSoA(STEPS)
  d1_soa:TaskSoA=TaskSoA(STEPS)
  d0_soa:TaskSoA=TaskSoA(STEPS)
  x_soa:TaskSoA=TaskSoA(STEPS)
  h_w:List[u64]=[u64(0)]*STEPS
  b_w:List[u64]=[u64(0)]*STEPS
  d2_w:List[u64]=[u64(0)]*STEPS
  d1_w:List[u64]=[u64(0)]*STEPS
  d0_w:List[u64]=[u64(0)]*STEPS
  x_w:List[u64]=[u64(0)]*STEPS
  ...
  d0_m:int=0
  x_m:int=0
  use_heavy:bool=(split_mode!=0)

  i:int=0
  while i<m:
    fid:int=soa.funcid_arr[i]
    if use_heavy and (fid==0 or fid==4 or fid==5 or fid==7):
      copy_soa_record_116(soa,w_arr,i,h_soa,h_w,h_m)
      h_m+=1
    elif fid>=0 and fid<=11:
      copy_soa_record_116(soa,w_arr,i,b_soa,b_w,b_m)
      b_m+=1
    elif fid>=12 and fid<=18:
      copy_soa_record_116(soa,w_arr,i,d2_soa,d2_w,d2_m)
      d2_m+=1
    elif fid>=19 and fid<=25:
      copy_soa_record_116(soa,w_arr,i,d1_soa,d1_w,d1_m)
      d1_m+=1
    elif fid==26 or fid==27:
      copy_soa_record_116(soa,w_arr,i,d0_soa,d0_w,d0_m)
      d0_m+=1
    else:
      copy_soa_record_116(soa,w_arr,i,x_soa,x_w,x_m)
      x_m+=1
    i+=1
  ...
  if gpu_log_level>=2:
    print(f"[split116-buckets] N={N} m={m} heavy0475={h_m} B={b_m} d2={d2_m} d1={d1_m} d0={d0_m} rest={x_m} split_mode={split_mode}")

  if h_m>0:
    GRID=(h_m+BLOCK-1)//BLOCK
    kernel_dfs_heavy0475_gpu(gpu.raw(h_soa.ld_arr),gpu.raw(h_soa.rd_arr),gpu.raw(h_soa.col_arr),gpu.raw(h_soa.row_arr),gpu.raw(h_soa.free_arr),gpu.raw(h_soa.jmark_arr),gpu.raw(h_soa.end_arr),gpu.raw(h_soa.mark1_arr),gpu.raw(h_soa.mark2_arr),gpu.raw(h_soa.funcid_arr),gpu.raw(h_w),gpu.raw(results),h_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<h_m:
      chunk_total+=int(results[i]); i+=1

  if b_m>0:
    GRID=(b_m+BLOCK-1)//BLOCK
    kernel_dfs_B_gpu(gpu.raw(b_soa.ld_arr),gpu.raw(b_soa.rd_arr),gpu.raw(b_soa.col_arr),gpu.raw(b_soa.row_arr),gpu.raw(b_soa.free_arr),gpu.raw(b_soa.jmark_arr),gpu.raw(b_soa.end_arr),gpu.raw(b_soa.mark1_arr),gpu.raw(b_soa.mark2_arr),gpu.raw(b_soa.funcid_arr),gpu.raw(b_w),gpu.raw(results),b_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<b_m:
      chunk_total+=int(results[i]); i+=1

  if d2_m>0:
    GRID=(d2_m+BLOCK-1)//BLOCK
    kernel_dfs_d2_gpu(gpu.raw(d2_soa.ld_arr),gpu.raw(d2_soa.rd_arr),gpu.raw(d2_soa.col_arr),gpu.raw(d2_soa.row_arr),gpu.raw(d2_soa.free_arr),gpu.raw(d2_soa.jmark_arr),gpu.raw(d2_soa.end_arr),gpu.raw(d2_soa.mark1_arr),gpu.raw(d2_soa.mark2_arr),gpu.raw(d2_soa.funcid_arr),gpu.raw(d2_w),gpu.raw(results),d2_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<d2_m:
      chunk_total+=int(results[i]); i+=1

  if d1_m>0:
    GRID=(d1_m+BLOCK-1)//BLOCK
    kernel_dfs_d1_gpu(gpu.raw(d1_soa.ld_arr),gpu.raw(d1_soa.rd_arr),gpu.raw(d1_soa.col_arr),gpu.raw(d1_soa.row_arr),gpu.raw(d1_soa.free_arr),gpu.raw(d1_soa.jmark_arr),gpu.raw(d1_soa.end_arr),gpu.raw(d1_soa.mark1_arr),gpu.raw(d1_soa.mark2_arr),gpu.raw(d1_soa.funcid_arr),gpu.raw(d1_w),gpu.raw(results),d1_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<d1_m:
      chunk_total+=int(results[i]); i+=1

  if d0_m>0:
    GRID=(d0_m+BLOCK-1)//BLOCK
    kernel_dfs_d0_gpu(gpu.raw(d0_soa.ld_arr),gpu.raw(d0_soa.rd_arr),gpu.raw(d0_soa.col_arr),gpu.raw(d0_soa.row_arr),gpu.raw(d0_soa.free_arr),gpu.raw(d0_soa.jmark_arr),gpu.raw(d0_soa.end_arr),gpu.raw(d0_soa.mark1_arr),gpu.raw(d0_soa.mark2_arr),gpu.raw(d0_soa.funcid_arr),gpu.raw(d0_w),gpu.raw(results),d0_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<d0_m:
      chunk_total+=int(results[i]); i+=1
  ...
  if x_m>0:
    meta_next:List[u8]=[u8(1),u8(2),u8(3),u8(3),u8(2),u8(6),u8(2),u8(2),u8(0),u8(4),u8(5),u8(7),u8(13),u8(14),u8(14),u8(14),u8(17),u8(14),u8(14),u8(20),u8(21),u8(21),u8(21),u8(25),u8(21),u8(21),u8(26),u8(26)]
    GRID=(x_m+BLOCK-1)//BLOCK
    kernel_dfs_iter_gpu(gpu.raw(x_soa.ld_arr),gpu.raw(x_soa.rd_arr),gpu.raw(x_soa.col_arr),gpu.raw(x_soa.row_arr),gpu.raw(x_soa.free_arr),gpu.raw(x_soa.jmark_arr),gpu.raw(x_soa.end_arr),gpu.raw(x_soa.mark1_arr),gpu.raw(x_soa.mark2_arr),gpu.raw(x_soa.funcid_arr),gpu.raw(x_w),gpu.raw(meta_next),gpu.raw(results),x_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<x_m:
      chunk_total+=int(results[i]); i+=1
  ...
  elapsed_ms:int=stage_no_read_ms
  stages_inner:List[int]=[stage_soa_ms,stage_stats_ms,stage_split_ms,stage_kernel_reduce_ms,0,stage_compute_ms,stage_no_read_ms]
  return chunk_total,stats,stages_inner,elapsed_text,elapsed_ms

"""116: broadmarktail 等の reordered bin を split-kernel hybrid で実行する。"""
def exec_solutions_gpu_bin_stream_split116(
  N:int,
  fname:str,
  preset_queens:int,
  gpu_block:int=32,
  ...
  total_records:int=count_constellations_bin_records(fname)
  run_param_tag:str=funcid_reorder_run_param_tag(BLOCK,MAX_BLOCKS)
  progress_fname:str=f"progress_N{N}_{preset_queens}_stream_split116_{run_param_tag}.tsv"
  if progress_suffix!="":
    progress_fname=f"progress_N{N}_{preset_queens}_stream_split116_{run_param_tag}_{progress_suffix}.tsv"
  if worker_count>1:
    if progress_suffix!="":
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split116_{run_param_tag}_{progress_suffix}_worker{worker_id}of{worker_count}.tsv"
    else:
      progress_fname=f"progress_N{N}_{preset_queens}_stream_split116_{run_param_tag}_worker{worker_id}of{worker_count}.tsv"
  with open(progress_fname,"w") as pf:
    pf.write(stream_funcid_reorder_progress_header())
  ...
  if gpu_log_level>=1:
    print(f"[split116-gpu-config] N={N} records={total_records} bin={fname} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} sort_mode={gpu_sort_mode} chunk_only={1 if (chunk_only or use_chunk_list) else 0} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={chunk_list_to_string(selected_chunks)} worker={worker_id}/{worker_count} split_mode={split_mode} progress={progress_fname}")
  ...
  with open(fname,"rb") as f:
    while True:
      ...
      if chunk_only or use_chunk_list:
        ...
        if not run_this_chunk:
          if gpu_log_level>=2:
            print(f"[split116-gpu-chunk-skip] N={N} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue
      if worker_count>1:
        run_worker_chunk:bool=((chunk_index % worker_count)==worker_id)
        if not run_worker_chunk:
          if gpu_log_level>=2:
            print(f"[split116-gpu-worker-skip] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
          off+=m; chunk_index+=1; continue

      t0=datetime.now()
      if gpu_log_level>=1:
        print(f"[split116-gpu-chunk-start] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m}")
      chunk_total:int=0
      stats:List[int]=[0]*46
      stages_inner:List[int]=[0,0,0,0,0,0,0]
      elapsed_text:str="0:00:00.000"
      elapsed_ms:int=0
      chunk_total,stats,stages_inner,elapsed_text,elapsed_ms=exec_solutions_gpu_chunk_split116(N,chunk_constellations,BLOCK,MAX_BLOCKS,gpu_sort_mode,cross_stripe_safe,split_mode,0 if gpu_log_level<2 else gpu_log_level)
      gpu_total+=chunk_total
      executed_chunks+=1
      t1=datetime.now()
      ...
      if gpu_log_level>=1:
        print(f"[split116-gpu-chunk-end] N={N} worker={worker_id}/{worker_count} chunk={chunk_index} off={off} m={m} elapsed={elapsed_outer_text} inner={elapsed_text} elapsed_ms={elapsed_outer_ms} chunk_total={chunk_total} gpu_total={gpu_total} soa_ms={stages_inner[0]} stats_ms={stages_inner[1]} split_ms={stages_inner[2]} kernel_reduce_ms={stages_inner[3]}")
      off+=m
      chunk_index+=1
  if gpu_log_level>=1:
    print(f"[split116-gpu-summary] N={N} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} total={gpu_total} split_mode={split_mode} progress={progress_fname}")
    if worker_count>1:
      print(f"[worker-summary] N={N} worker={worker_id}/{worker_count} records={total_records} chunks={chunk_index} executed_chunks={executed_chunks} partial_total={gpu_total} progress={progress_fname}")
  return gpu_total
...
def main()->None:
  ...
  if argc == 1:
    ...
    if bench_mode==14 or bench_mode==15 or bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==26 or bench_mode==27 or bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
      ...
      #   mode30:    ... [preset_queens] 30   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]  # split116 selected chunks
      #   mode31:    ... [preset_queens] 31   [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]  # split116 full run / optional multi-GPU worker
      # Examples:
      ...
      #   -g 22 22 32 484 1 0 7 30 8 7 0 1222 5 "" 2  # 116 split-kernel probe chunks 1222..1226
      #   -g 22 22 32 484 1 0 7 31 8 7 0 0 1 2  # 116 split-kernel full GPU single worker
      if argc >= 11:
      ...
      if bench_mode==17 or bench_mode==18 or bench_mode==19 or bench_mode==20 or bench_mode==21 or bench_mode==22 or bench_mode==23 or bench_mode==24 or bench_mode==25 or bench_mode==30:
        ...
        if bench_mode==30 and argc > 17:
          print("Too many arguments")
          print("Usage split116-probe: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens 30 [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [chunk_start] [chunk_count] [chunk_list] [broadmark_tail_variant]")
          return
      else:
        if bench_mode==29 or bench_mode==31:
          if argc > 16:
            print("Too many arguments")
            print("Usage broadmarktail/split116 worker: nqueens -g nmin nmax block max_blocks log_level sort_mode preset_queens mode[29|31] [reorder_window_mult] [reorder_phase_jump] [cross_stripe_safe] [worker_id] [worker_count] [broadmark_tail_variant]")
            return
        elif bench_mode==28:
          if argc > 14:
            print("Too many arguments")
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split116_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split116_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split116-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split116-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split116_probe_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split116(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,True,debug_chunk_start,debug_chunk_count,microbench_chunk_list_spec,progress_suffix,0,1,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      print(f"[split116-probe-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec} partial_total={total}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    split116-probe")
      continue
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split116-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split116-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      progress_suffix:str=f"split116_full_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
      total:int=exec_solutions_gpu_bin_stream_split116(N,reorder_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode,cross_stripe_safe,False,debug_chunk_start,debug_chunk_count,"",progress_suffix,worker_id,worker_count,2)
      time_elapsed=datetime.now()-start_time
      text=str(time_elapsed)[:-3]
      status:str="ok" if expected[N]==total else f"ng({total}!={expected[N]})"
      if worker_count>1:
        status=f"partial-worker-{worker_id}-of-{worker_count}"
      if gpu_log_level>=1:
        print(f"[split116-full-done] N={N} source_records={stream_records} reordered_records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)} window_mult={FUNCID_REORDER_V2_WINDOW_MULT} phase_jump={FUNCID_REORDER_V2_PHASE_JUMP} variant={broad_markdist_tail_variant_tag()} worker={worker_id}/{worker_count} total={total}")
        if worker_count>1:
          print(f"[worker-done] N={N} worker={worker_id}/{worker_count} partial_total={total} expected_total={expected[N]}")
      print(f"{N:2d}:{total:18d}{0:17d}{text:>21s}    {status}")
      continue
