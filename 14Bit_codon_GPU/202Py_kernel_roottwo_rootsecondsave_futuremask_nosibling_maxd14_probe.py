...
# =============================================================================
# NQ_UPDATE_MEMO
# 202: 更新メモ: 201は正当性OKだが197比で遅いため不採用。197 root<=2 direct-if prerollを親に戻し、MAXD14 kernelのみroot-prerollで残りroot siblingをcur_availではなくroot_secondから保存する微差実験を検証する。
...
202Py note:
  201Py was correct but slower than the validated 197 direct-if baseline.
  This branch returns to 197's MAXD14 root fast-start scope: only
  post-root-action root_avail with one or two bits is prerolled, and 3-bit or
  wider roots stay on the generic loop.  The only new micro-experiment is the
  root-preroll save payload: under the existing root_after_second==0 predicate,
  the remaining root sibling is saved from root_second instead of cur_avail.
...
"""
201Pyログを確認しました。**正当性・実行結果は問題ありません。**
...
```text
201Py  0:07:08.383
197Py  0:07:08.202  → 201が 0.181秒 / 0.042% 遅い
200Py  0:07:09.520  → 201が 1.137秒 高速
199Py  0:07:08.293  → 201が 0.090秒 遅い
198Py  0:07:08.239  → 201が 0.144秒 遅い
196Py  0:07:08.239  → 201が 0.144秒 遅い
194Py  0:07:08.288  → 201が 0.095秒 遅い
```

そのため、**201Pyは正当性OKだが197Pyを上回らないため不採用**としました。
新最速基準は引き続き **197Py** です。

202Pyは201を親にせず、197Pyへ戻しています。

202Pyの差分はMAXD14 kernel内だけです。

```text
197Py:
  root-prerollで残りroot候補を保存するとき、
  cur_avail を見て save し、cur_avail を payload に入れる

202Py:
  root-prerollで残りroot候補を保存するとき、
  root_second を見て save し、root_second を payload に入れる
```
...
```text
root_second save payload equivalence OK
```

こちらではCodon/CUDA実機build/full実行はしていません。cudacodon側では以下で確認してください。

```bash
STATIC_ONLY=1 bash 202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_validate_full_once.sh
bash 202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物はこちらです。

* [202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_probe.py)
* [202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/202Py_kernel_roottwo_rootsecondsave_futuremask_nosibling_maxd14_bundle_20260701.tar)

"""

# 202 root<=2 root-second-save memo over validated 197 direct-if futuremask/no-sibling MAXD14:
#   201 was correct but 0.181 seconds slower than 197, so this branch adopts
#   197 as the active numerical baseline.  It keeps the validated root one/two-
...
#   only restorable ancestor frames.  202 keeps the narrow root fast-start only when
...
#   The root<=2 predicate remains the 197 second-lowbit direct-if form.  Only the
#   root-preroll save of the remaining root sibling changes: inside the predicate
#   the remaining sibling is represented by root_second, so the save condition and
#   saved avail payload use root_second directly rather than the mutable cur_avail.
...
# 202 keeps the validated 197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 202 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="202 roottwo-rootsecondsave: 197 root<=2 direct-if futuremask/no-sibling MAXD14 baseline with root-second save payload - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """202 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate root-second-save preroll."""
    ...
    # 202 keeps the validated 197 current-frame register cache, 13-slot
    ...
    # correct but slower.  The predicate stays the 197 second-lowbit direct-if
    # shape.  Only the remaining root sibling save changes: inside the predicate,
    # root_second is zero for one-candidate roots and the only remaining root
    # candidate for two-candidate roots.  Therefore 3-bit and wider roots stay
    # on the generic loop, while the saved root avail uses root_second directly.
    ...
    if root_after_second==u32(0):
      ...
      if pr_descend!=u32(0):
        if root_second!=u32(0):
          ld[save_sp]=cur_ld
          rd[save_sp]=cur_rd
          col[save_sp]=cur_col
          avail[save_sp]=root_second|(u32(cur_depth)<<u32(27))
          save_sp+=1
        cur_ld=pr_nld
        cur_rd=pr_nrd
        cur_col=pr_ncol
...
CHUNKSHAPE148_DEFAULT_REASON:str="202 rootsecondsave reuses validated 197 scorestripe_v9 task order/cache; only MAXD14 root-preroll save payload changes"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split202_roottwo_rootsecondsave_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split202_roottwo_rootsecondsave_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split202-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split202-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split202_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split202-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split202-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split202_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
