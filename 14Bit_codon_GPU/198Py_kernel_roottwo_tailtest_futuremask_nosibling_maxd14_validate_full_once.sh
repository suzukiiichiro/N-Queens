...
# =============================================================================
# NQ_UPDATE_MEMO
# 198: 更新メモ: 197は正当性OKかつ196比0.037秒高速で数値上の新最速基準。197 root<=2 direct-if prerollを親に、MAXD14 kernelのみroot<=2判定をroot_rest&(root_rest-1)==0 tail-testへ変更する微差実験を検証する。
# Full update history: see README.md
# =============================================================================

# 198 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 198 starts from
# validated 197 because 197 was correct and 0.037s faster than the 196 baseline.
#
...
# and wider roots remain generic. The root<=2 predicate changes from the 197
# second-lowbit direct-if shape to the equivalent root_rest&(root_rest-1)==0
# tail-test, removing the helper scalars used only by the predicate. Host task
# order, dispatch, cache, progress verification, and README-managed history remain
# unchanged.
SRC=${SRC:-./198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 198 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/198Py_kernel_roottwo_tailtest_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 198Py source
#   has a harmless header/string/layout difference. This 198 harness validates the
...
' >> "$SUMMARY"
printf 'source_parent_baseline	validated 197 root one/two-candidate direct-if futuremask/no-sibling MAXD14 dispatch; 195 rootthree and 193/189/190/191 not adopted	runtime-only validation	INFO
' >> "$SUMMARY"
printf 'kernel_shape_expectation	MAXD14 nibble schedule + current frame register + ancestor13 stack + no-sibling spill elision + future_check_mask zero guard + root one/two-candidate root_rest tail-test preroll predicate	runtime dispatch/progress checked	INFO
' >> "$SUMMARY"
...
if grep -q 'root_two_tailtest' "$SRC" && grep -q 'root_rest_more:u32=root_rest&(root_rest-u32(1))' "$SRC" && grep -q 'if root_rest_more==u32(0):' "$SRC" && ! grep -q 'root_second:u32' "$SRC" && ! grep -q 'root_after_second:u32' "$SRC" && ! grep -q 'root_preroll:u32' "$SRC" && ! grep -q 'if root_preroll' "$SRC"; then
  printf 'source_root_two_tailtest	present_no_secondlowbit_helper	present_no_secondlowbit_helper	OK
' >> "$SUMMARY"
else
  printf 'source_root_two_tailtest	present_no_secondlowbit_helper	missing_or_helper_retained	FAIL
' >> "$SUMMARY"
  static_failures=$((static_failures+1))
fi
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 198 runtime-only harness disabled source/model assertions"
  if (( static_failures != 0 )); then exit 1; fi
  exit 0
fi

if (( static_failures != 0 )); then
  echo "[error] source roottwo-tailtest static checks failed" >&2
  exit 66
fi
...
echo "[validation-ok] 198 roottwo-tailtest futuremask/no-sibling reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
