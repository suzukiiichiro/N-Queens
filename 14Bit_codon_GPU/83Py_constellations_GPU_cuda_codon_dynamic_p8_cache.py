...
VERSION_TAG:str="83 cache-only validation from 82 dynamic preset P8"
...
"""bin キャッシュのレコード数を返す（1 record = 16 bytes）。"""
...
"""84 stream と共有する完了マーカーを書く。"""
def write_constellations_done_count(fname:str,count:int)->None:
...
def main()->None:
  ...
  if use_gpu:
    ...
    if bench_mode==8 or bench_mode==9 or bench_mode==10 or bench_mode==11:
      if bench_mode==11:
        print(f"cache_only    : mode={bench_mode} preset={preset_queens_arg}")
      else:
  ...
  for N in range(nmin,nmax):
    ...
    if bench_mode==11:
      ...
      # use_constellation_cache=True なので build_constellations_dynamicK() 内で
      # .bin は読み込み、または生成済み。念のため False に戻した場合だけ保存する。
      if not use_constellation_cache:
        save_constellations_bin(N,fname,constellations)
      records:int=count_constellations_bin_records(fname)
      write_constellations_done_count(fname+".done",records)
      print(f"[constellation-cache-only] N={N} preset_queens={preset_queens} constellations={len(constellations)} bin={fname} records={records} valid={1 if validate_bin_file(fname) else 0}")
