...
#      228Pyは225Py generic ncol-earlyを親に、MAXD14 root-preroll内でも
#      pr_ncol=cur_col|pr_bit を pr_nld/pr_nrd 計算より前へ移動する微差実験を検証する。
# =============================================================================

SRC=${SRC:-./228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 228 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/228Py_kernel_rootpr_ncol_early_rootrestlate_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
root_pos=s.find('# 228 rootpr-ncol-early:')
if root_pos < 0:
...
pr_nld_pos=root_compact.find('pr_nld:u32=u32(0)')
...
checks={
  ...
  'rootpr_ncol_present': pr_ncol_pos >= 0,
  'rootpr_ncol_before_nld': 0 <= pr_ncol_pos < pr_nld_pos,
  'rootpr_nf_uses_ncol': pr_nf_pos >= 0,
  'rootpr_ncol_after_block_absent': root_compact.find('else:pr_nld=(cur_ld|pr_bit)<<u32(1)pr_nrd=(cur_rd|pr_bit)>>u32(1)pr_ncol:u32=cur_col|pr_bit') < 0,
...
if all(checks.values()):
    sys.exit(0)
print('source_rootpr_ncol_early_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootpr_ncol_early	rootrestlate_plus_rootpr_ncol_early	present	OK\n' >> "$SUMMARY"
else
  printf 'source_rootpr_ncol_early	rootrestlate_plus_rootpr_ncol_early	missing_or_old_shape	FAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split228'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split228' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit228 active preferred\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit228 active preferred\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if (( static_failures != 0 )); then
  echo "================================================================"; echo "[static-summary]"; cat "$SUMMARY"; echo "[logdir] $LOGDIR"
  echo "[error] 228 source static checks failed" >&2
  exit 66
fi
