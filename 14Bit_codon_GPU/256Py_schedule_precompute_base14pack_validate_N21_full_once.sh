...
# =============================================================================
# 256 N=21 full validation harness
# Parent: 244 r4 maskpack JIT-ok / 241 restore239-after-fid14split-reject.
# Experiment: keep single launch and child/future maskpack, but pack only
#             terminal_base14 into a separate low-bit sched_base14_arr.
#             terminal_depth and root_action stay separate. 255 actionpack
#             hit CUDA_ERROR_INVALID_PTX even with the correct mode31 command.
...
SRC=${SRC:-./256Py_schedule_precompute_base14pack_probe.py}
CAND=${CAND:-./256Py_schedule_precompute_base14pack_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/256Py_schedule_precompute_base14pack_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/256Py_schedule_precompute_base14pack_logs_N21_full_once_${TS}"
...
echo "[start] 256 schedule-precompute-base14pack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 256 schedule-precompute-base14pack terminal-base14-only SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '256 schedule-precompute-base14pack probe' "$SRC"; then
  printf 'source_version_tag\t256 schedule-precompute-base14pack probe\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t256 schedule-precompute-base14pack probe\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
runtime_split_ok = ('split256_' in s and 'split240_' not in s and 'split241_' not in s and 'split242_' not in s and 'split243_' not in s and 'split244_' not in s and 'split255_' not in s)
if runtime_split_ok:
    checks.append(('source_split_tag', 'split256 active; split240/241/242/243/244/255 runtime tags absent', 'split256', True))
else:
    checks.append(('source_split_tag', 'split256 active; split240/241/242/243/244/255 runtime tags absent', 'old split runtime tag present or split256 missing', False))
...
if not m14:
    ...
    ok = (
        ...
        'sched_action_arr:Ptr[u32]' not in body
    )
    actual = 'base14pack precompute active' if ok else 'old schedule interpreter present or base14pack loads missing or action/ctrl pack still present'
    checks.append(('source_schedule_precompute_base14pack', 'MAXD14 precompute base14pack active', actual, ok))
...
checks.append(('source_host_precompute_base14pack_arrays', 'present', 'present' if host_ok else 'missing', host_ok))
fail = 0
...
echo "[static-ok] 256 source checks passed; proceeding to release build/run"
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 256 schedule-precompute-base14pack mode31 split145"
...
echo "[validation-ok] 256 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
