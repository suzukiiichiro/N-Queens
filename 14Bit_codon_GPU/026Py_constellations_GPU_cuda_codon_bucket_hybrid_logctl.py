...
def exec_solutions(
  ...
  if use_gpu:
    ...
    print(f"[gpu-config] N={N} bucketized=1 split_row={GPU_SPLIT_ROW} bucket_mode={GPU_BUCKET_MODE} block={BLOCK} owner_blocks={MAX_BLOCKS} owner_steps={STEPS} task_blocks={GPU_TASK_BLOCKS} task_steps={GPU_TASK_STEPS} log_level={GPU_LOG_LEVEL}")
    ...
    while off < m_all:
      ...
      # CPU浅展開 → owner付き task + bucket index
      expand_start=datetime.now()
      expanded:ExpandedTasks=ExpandedTasks()
      bucket_indices:List[List[int]]=make_bucket_indices(GPU_BUCKETS)
      expand_iter_to_row(
        func_meta,
        ...
        off,m,
        GPU_SPLIT_ROW,
        expanded,bucket_indices,
        GPU_BUCKET_MODE
      )
      expand_dt=datetime.now()-expand_start
      task_count:int=len(expanded.ld_arr)
      expand_total+=task_count
      if GPU_LOG_EXPAND:
        print(f"[gpu-expand] N={N} chunk={input_chunks} owner={off}-{off+m-1} base={m} tasks={task_count} split_row={GPU_SPLIT_ROW} time={str(expand_dt)[:-3]}")

      # bucket ごとに compact → kernel 起動。atomic は使わず、戻ってから owner に逐次加算。
      for bucket_id in range(GPU_BUCKETS):
        idxs:List[int]=bucket_indices[bucket_id]
        bucket_m:int=len(idxs)
        if bucket_m==0:
          continue
