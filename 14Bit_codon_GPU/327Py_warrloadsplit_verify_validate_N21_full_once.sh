...
# =============================================================================
# 327 N=21 full validation harness (warrloadsplit-verify)
# Base: 325 (kernel logic byte-for-byte identical to 319-325's unified
#   hot loop -- 327 is NOT a revert-record of 326; the user already has
#   325 saved locally under its own filename, so no separate rollback
#   revision was needed). 326 (future_check_mask in-kernel specialization)
#   was tried and REJECTED: correctness OK but elapsed=517.563s, -13.7%
#   slower than 319's 455.116s baseline -- the worst of 5 failed
#   GPU-side specialization attempts (240/266/267/268-269/273/326). That
#   direction is set aside; this revision moves to Open Objectives #2.
#
# THIS REVISION: verification only, before any kernel change (326's
#   lesson: a plausible hypothesis can still regress badly in practice).
#   Adds a STANDALONE diagnostic probe, with ZERO changes to the main
#   solver, to test the 323 hypothesis that `w_arr[idx]` (the only
#   Ptr[u64] array read via a single idx index, formerly line ~839:
#   `thread_total+=total*w_arr[idx]`) compiles to two 32-bit LDG.E
#   instructions instead of one 64-bit load:
#     327_w_arr_loadsplit_probe.py       -- a minimal @gpu.kernel that
#       reproduces the exact access shape (grid-stride while loop,
#       `thread_total+=total*w_arr[idx]`) without any DFS machinery.
#     327_w_arr_loadsplit_probe_check.sh -- builds it and runs
#       `cuobjdump --dump-sass` (no GPU execution needed) to check for
#       LDG.E.64 (single load) vs paired LDG.E (32-bit x2), falling back
#       to `sudo ncu --section SourceCounters` + `--page source` if
#       cuobjdump is unavailable.
#   These two files are NOT tracked by this N=21 validation harness --
#   they test an isolated toy kernel, not the solver. Run with:
#     bash 327_w_arr_loadsplit_probe_check.sh
#   Interpreting the result: two 32-bit LDG.E confirms the split-load
#   pattern is a general Codon/NVPTX code-generation behavior for this
#   access shape (matching 319_ncu_source.txt), and isolated source-level
#   variations can be tried in this same probe before touching the real
#   kernel; a single LDG.E.64 means something else in the real kernel
#   (surrounding code, register pressure, aliasing) is responsible, and
#   the 323 hypothesis needs revisiting before any fix is attempted.
#
# This revision: ZERO changes to kernel logic in the main solver (same
#   as 311-325: variant=2, K=48). The probe files above are entirely
#   separate from kernel_dfs_iter_gpu_maxd14 and do not affect the N=21
#   correctness/timing path this script validates.
...
SRC=${SRC:-./327Py_warrloadsplit_verify.py}
CAND=${CAND:-./327Py_warrloadsplit_verify}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/327Py_warrloadsplit_verify_N21_full.lock}
...
# NOTE: 327 is based directly on 325 (kernel logic byte-for-byte
# identical to 319-325's unified hot loop). 326 (future_check_mask
# specialization) was tried and REJECTED: correctness OK but
...
# baseline for reference, but this 327 lineage does not build on 326's
# code. 319 (455.116s) remains the most recent confirmed-good timing
# baseline.
...
LOGDIR="${LOG_ROOT%/}/327Py_warrloadsplit_verify_logs_N21_full_once_${TS}"
...
echo "[start] 327 warrloadsplit-verify validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 327 warrloadsplit-verify SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '327 warrloadsplit-verify' "$SRC"; then
  printf 'source_version_tag	327 warrloadsplit-verify	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	327 warrloadsplit-verify	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 327 warrloadsplit-verify mode31 split145"
...
echo "[validation-ok] 327 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); 326's future_check_mask specialization was rejected (517.563s, -13.7% slower than 319); this revision adds a standalone 327_w_arr_loadsplit_probe.py + 327_w_arr_loadsplit_probe_check.sh (unrelated to this harness) to verify the 323 w_arr[idx] uncoalesced-load hypothesis before attempting any real kernel change; NO change to the main solver in this revision"
