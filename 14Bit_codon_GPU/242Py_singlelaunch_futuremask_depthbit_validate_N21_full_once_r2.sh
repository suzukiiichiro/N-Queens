...
# =============================================================================
# 242 N=21 full validation harness
# Parent: 241 restore239-after-fid14split-reject.
# Experiment: keep the 241/239 single launch path and reduce one MAXD14
#             future-prune condition pair by using future_check_mask depth bits.
#             No fid/task launch split; keep -c/-g, N5..27, cache generation,
#             broadmarktail 28/29, split145 30/31, MAXD fallbacks, and
...
SRC=${SRC:-./242Py_singlelaunch_futuremask_depthbit_probe.py}
CAND=${CAND:-./242Py_singlelaunch_futuremask_depthbit_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
# 242: visible startup + force release rebuild by default so a previous non-release
# `codon build` executable is not reused and cannot trigger CUDA_ERROR_INVALID_PTX.
# 242: print start/status early and prevent static pycheck from failing silently under set -e.
FORCE_REBUILD=${FORCE_REBUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/242Py_singlelaunch_futuremask_depthbit_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/242Py_singlelaunch_futuremask_depthbit_logs_N21_full_once_${TS}"
...
echo "[start] 242 r2 validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 242 r2 singlelaunch-futuremask-depthbit SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '242 singlelaunch-futuremask-depthbit' "$SRC"; then
  printf 'source_version_tag\t242 singlelaunch-futuremask-depthbit\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_version_tag\t242 singlelaunch-futuremask-depthbit\tmissing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
def active_code_without_comments_and_strings(text):
    # Static checks below should inspect executable tokens, not old memo text.
    out = []
    try:
...
active_s = active_code_without_comments_and_strings(s)
checks = []
def has_def(name):
...
use_itter_active = re.search(r'\buse_itter\b', active_s) is not None
checks.append(('removed_use_itter_branch', 'active branch absent', 'active present' if use_itter_active else 'active absent', not use_itter_active))
...
if 'split242' in s and 'split240' not in s and 'split241' not in s and 'split239_' not in s:
    checks.append(('source_split_tag', 'split242 active; split240/241 rejected', 'split242', True))
else:
    checks.append(('source_split_tag', 'split242 active; split240/241 rejected', 'old split runtime present or split242 missing', False))
...
# 242: ensure the active MAXD14 future-prune probe is present and the
# old nested future_check_mask/nibble future branch is absent from MAXD14.
m = re.search(r'^@gpu\.kernel\s*\n^def\s+kernel_dfs_iter_gpu_maxd14\b(?P<body>.*?)(?=^@gpu\.kernel\s*\n^def\s+kernel_dfs_iter_gpu_maxd16\b)', s, re.M | re.S)
if not m:
    checks.append(('source_future_depthbit_probe', 'MAXD14 future depth-bit probe', 'MAXD14 body missing', False))
else:
    body = m.group('body')
    ok = ('future_check_mask&u32(1)' in body and
          'future_check_mask>>u32(cur_depth)' in body and
          'if future_check_mask!=u32(0):' not in body and
          '(nibble_op&u32(8))' not in body and
          '(pr_nibble_op&u32(8))' not in body)
    actual = 'depthbit present' if ok else 'missing_depthbit_or_old_future_branch_present'
    checks.append(('source_future_depthbit_probe', 'MAXD14 future depth-bit probe', actual, ok))
fail = 0
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 242 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 242 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 242 singlelaunch future-depthbit mode31 split145"
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "241parent:$BASELINE_241_SECONDS" "239parent:$BASELINE_239_SECONDS" "240rejected:$BASELINE_240_REJECT_SECONDS" "238:$BASELINE_238_SECONDS" "237:$BASELINE_237_SECONDS" "232:$BASELINE_232_SECONDS" "217:$BASELINE_217_SECONDS"; do
...
echo "[validation-ok] 242 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread"
