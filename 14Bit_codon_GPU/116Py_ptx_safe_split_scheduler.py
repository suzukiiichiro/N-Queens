...
VERSION_TAG:str="116 PTX-safe split scheduler - host buckets + proven 115 GPU kernel"
...
####################################################################################################
# 116 PTX-safe split scheduler
#
# The first 116 build added several new @gpu.kernel definitions.  On Codon/CUDA this can fail at
# runtime with CUDA_ERROR_INVALID_PTX on some driver/compiler combinations.  This safe build keeps
# only the proven 115 kernel_dfs_iter_gpu device code, and performs the 116 bucket split on the host.
#
# Purpose:
...
def exec_solutions_gpu_chunk_split116(
  ...
  if gpu_log_level>=2:
    print(f"[split116-buckets] N={N} m={m} heavy0475={h_m} B={b_m} d2={d2_m} d1={d1_m} d0={d0_m} rest={x_m} split_mode={split_mode} ptx_safe=1")

  # PTX-safe mode: use only the proven 115 generic kernel.
  # The host-side buckets remain useful for validating the scheduler and for measuring launch/split overhead.
  meta_next:List[u8]=[u8(1),u8(2),u8(3),u8(3),u8(2),u8(6),u8(2),u8(2),u8(0),u8(4),u8(5),u8(7),u8(13),u8(14),u8(14),u8(14),u8(17),u8(14),u8(14),u8(20),u8(21),u8(21),u8(21),u8(25),u8(21),u8(21),u8(26),u8(26)]

  if h_m>0:
    GRID=(h_m+BLOCK-1)//BLOCK
    kernel_dfs_iter_gpu(gpu.raw(h_soa.ld_arr),gpu.raw(h_soa.rd_arr),gpu.raw(h_soa.col_arr),gpu.raw(h_soa.row_arr),gpu.raw(h_soa.free_arr),gpu.raw(h_soa.jmark_arr),gpu.raw(h_soa.end_arr),gpu.raw(h_soa.mark1_arr),gpu.raw(h_soa.mark2_arr),gpu.raw(h_soa.funcid_arr),gpu.raw(h_w),gpu.raw(meta_next),gpu.raw(results),h_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<h_m:
  ...
  if b_m>0:
    ...
    kernel_dfs_iter_gpu(gpu.raw(b_soa.ld_arr),gpu.raw(b_soa.rd_arr),gpu.raw(b_soa.col_arr),gpu.raw(b_soa.row_arr),gpu.raw(b_soa.free_arr),gpu.raw(b_soa.jmark_arr),gpu.raw(b_soa.end_arr),gpu.raw(b_soa.mark1_arr),gpu.raw(b_soa.mark2_arr),gpu.raw(b_soa.funcid_arr),gpu.raw(b_w),gpu.raw(meta_next),gpu.raw(results),b_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<b_m:
      chunk_total+=int(results[i]); i+=1
  ...
  if d2_m>0:
    ...
    kernel_dfs_iter_gpu(gpu.raw(d2_soa.ld_arr),gpu.raw(d2_soa.rd_arr),gpu.raw(d2_soa.col_arr),gpu.raw(d2_soa.row_arr),gpu.raw(d2_soa.free_arr),gpu.raw(d2_soa.jmark_arr),gpu.raw(d2_soa.end_arr),gpu.raw(d2_soa.mark1_arr),gpu.raw(d2_soa.mark2_arr),gpu.raw(d2_soa.funcid_arr),gpu.raw(d2_w),gpu.raw(meta_next),gpu.raw(results),d2_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<d2_m:
      chunk_total+=int(results[i]); i+=1
  ...
  if d1_m>0:
    ...
    kernel_dfs_iter_gpu(gpu.raw(d1_soa.ld_arr),gpu.raw(d1_soa.rd_arr),gpu.raw(d1_soa.col_arr),gpu.raw(d1_soa.row_arr),gpu.raw(d1_soa.free_arr),gpu.raw(d1_soa.jmark_arr),gpu.raw(d1_soa.end_arr),gpu.raw(d1_soa.mark1_arr),gpu.raw(d1_soa.mark2_arr),gpu.raw(d1_soa.funcid_arr),gpu.raw(d1_w),gpu.raw(meta_next),gpu.raw(results),d1_m,board_mask,n3,n4,grid=GRID,block=BLOCK)
    i=0
    while i<d1_m:
      chunk_total+=int(results[i]); i+=1
