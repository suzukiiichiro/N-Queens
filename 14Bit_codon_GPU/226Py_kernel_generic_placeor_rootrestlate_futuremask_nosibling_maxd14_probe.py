...
225Pyは217Pyには `0.076秒` 届きませんが、204Pyより `0.010秒` 高速で、223/219とほぼ同等です。新最速基準にはしませんが、generic ncol-early は大きな退行なしと見てよいです。

226Pyは225Pyを親にし、MAXD14 generic loopだけをさらに1差分進めました。
...
# 226Py
ncol:u32 = cur_col | bit
placed_ld:u32 = cur_ld | bit
placed_rd:u32 = cur_rd | bit

if block_code != u32(0):
    nld = (placed_ld << stepu) | addvu | bLiu
    nrd = (placed_rd >> stepu) | bKu
else:
    nld = placed_ld << u32(1)
    nrd = placed_rd >> u32(1)
```
...
STATIC_ONLY=1 bash 226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh

bash 226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh
```

作成物です。

* [226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_probe.py)
* [226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/226Py_kernel_generic_placeor_rootrestlate_futuremask_nosibling_maxd14_bundle_20260703.tar)

"""

# 226 generic-placeor memo over validated 217 rootrestlate/204 root-clearrest futuremask/no-sibling MAXD14:
...
VERSION_TAG:str="226 generic-placeor: 222 MAXD13 diagnostics withdrawn; validated 217 rootrestlate scalar predicate restored - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while True:
      ...
      # 226 generic-placeor: ncol is independent of block_code and nld/nrd.
...
CHUNKSHAPE148_DEFAULT_REASON:str="226 generic-placeor reuses validated 217 scorestripe_v9 task order/cache; MAXD14 scalar root_after_second predicate with delayed cur_avail=root_rest"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split226_rootrestlate_restore_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split226_rootrestlate_restore_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split226-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split226-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split226_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split226-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split226-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split226_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
