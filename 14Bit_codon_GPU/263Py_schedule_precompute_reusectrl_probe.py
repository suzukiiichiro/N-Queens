...
# =============================================================================
# NQ_UPDATE_MEMO
# 261: schedule-precompute-reusectrl probe。256 reusectrl JIT成立と
...
"""
263 schedule-precompute-reusectrl note: 262 terminal_depth+terminal_base14 pack JIT成立/正当性OKを受け、5配列安全形は維持しつつMAXD14 dispatch直前に既存 ctrl0_arr / markctrl_arr を schedule_lo / sched_terminal へ再利用するprobe。root_actionはJIT危険のため別配列素通し固定。


262の結果を受けて、263を作成しました。

263は **262親 / 5配列安全形維持 / MAXD14直前に `ctrl0_arr`・`markctrl_arr` を再利用するprobe** です。

262で確認できたこと：

```text
262 terminalbasepack
JIT成立
正当性OK
elapsed 0:07:09.447
terminal_depth + terminal_base14 pack は安全
root_action decode 系はJIT危険
```

263では、`root_action` は引き続き別配列素通しのまま固定し、`selected_maxd==14` が確定した後だけ以下のrepackを行います。
...
MAXD14 kernel側ではこう読みます。

```python
schedule_lo = ctrl0_arr[i]
sched_terminal = markctrl_arr[i]
```

これにより、MAXD14 kernelへ `sched_lo_arr` / `sched_terminal_arr` を別引数として渡さず、既存SoA配列を再利用します。MAXD16/18/20/21 fallbackを壊さないよう、repackは **selected_MAXD14 launch直前だけ** に限定しています。
...
source_schedule_precompute_reusectrl OK
source_host_precompute_reusectrl_arrays OK
release_build_policy                 OK
```

ファイルはこちらです。

* [263Py_schedule_precompute_reusectrl_probe.py](sandbox:/mnt/data/263Py_schedule_precompute_reusectrl_probe.py)
* [263Py_schedule_precompute_reusectrl_validate_N21_full_once.sh](sandbox:/mnt/data/263Py_schedule_precompute_reusectrl_validate_N21_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)

補助ファイルです。

* [263Py_schedule_precompute_reusectrl_sha256.txt](sandbox:/mnt/data/263Py_schedule_precompute_reusectrl_sha256.txt)
* [263Py_schedule_precompute_reusectrl_bundle_20260707.tar](sandbox:/mnt/data/263Py_schedule_precompute_reusectrl_bundle_20260707.tar)

sha256です。

```text
39b063b69396d51e45d370f7a24fe928cb8c2c74c45bde17ac6c1b4a2988e1ee  263Py_schedule_precompute_reusectrl_probe.py
3bc6075270eaa7cba43267050b1540f1dc6119108368f13499facc5ed7815cab  263Py_schedule_precompute_reusectrl_validate_N21_full_once.sh
9cf41667d67b7c78d0e76855c9e473ad277059045ff70131d315082ca03fc038  README.md
```

実行は以下でお願いします。

```bash
rm -f ./263Py_schedule_precompute_reusectrl_probe

STATIC_ONLY=1 bash 263Py_schedule_precompute_reusectrl_validate_N21_full_once.sh

bash 263Py_schedule_precompute_reusectrl_validate_N21_full_once.sh
```

採否は、まず **JIT成立 → 正当性OK → 262の `0:07:09.447` から戻るか → 241の `0:07:07.788` に近づくか** で見るのがよいです。
...
VERSION_TAG:str="263 schedule-precompute-reusectrl probe: parent 262 terminalbasepack JIT-ok / 244 r4 maskpack JIT-ok / 241 restore239; single launch kept; MAXD14 host precompute retained; ctrl0_arr is repacked to schedule_lo and markctrl_arr to terminal_depth/base14 only after selected_MAXD14; sched_hi/sched_mask/root_action remain separate; root_action stays pass-through after 257/259/260 CUDA_ERROR_INVALID_PTX; generic N5..27, bare -g split145 mode31, mode28/29/30/31, cache generation, MAXD fallbacks, worker split preserved"
...
class TaskSoA:
  def __init__(self,m:int)->None:
    ...
    # 263: keep 262's safe terminal_depth+terminal_base14 pack and 244 r4 maskpack.
    # sched_lo_arr / sched_terminal_arr are host intermediates only: for MAXD14
    # selected chunks they are repacked into existing ctrl0_arr / markctrl_arr
    # immediately before launch, reducing separate schedule array arguments.
...
def precompute_maxd14_schedule_fields(ctrl0:u32,markctrl:u32,meta_next:List[u8])->Tuple[u32,u32,u32,u32,u32,u32]:
  # 263: host-side clone of the former MAXD14 kernel schedule builder.
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    sched_hi_arr:Ptr[u32],sched_mask_arr:Ptr[u32],root_action_arr:Ptr[u32],
    w_arr:Ptr[u64],
    meta_next:Ptr[u8],
    results:Ptr[u64],
    ...
    # 263: schedule fields are precomputed on host. For MAXD14 launch,
    # ctrl0_arr has been repacked to schedule_lo and markctrl_arr to sched_terminal.
    # child/future masks are packed; root_action remains separate pass-through.
...
def launch_kernel_dfs_iter_gpu_static_maxd(
  ...
  if selected_maxd==14:
    # 263: selected_MAXD14 is known here, so original ctrl0/markctrl are no
    # longer needed by the MAXD14 kernel. Reuse them for precomputed schedule
    # fields instead of passing two additional schedule arrays to the kernel.
    ri:int=0
    while ri<m:
      soa.ctrl0_arr[ri]=soa.sched_lo_arr[ri]
      soa.markctrl_arr[ri]=soa.sched_terminal_arr[ri]
    ...
    kernel_dfs_iter_gpu_maxd14(
      ...
      gpu.raw(soa.sched_hi_arr),gpu.raw(soa.sched_mask_arr),gpu.raw(soa.root_action_arr),
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split263_reusectrl_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={split_probe_chunk_list_spec}")
    if bench_mode==31:
      print(f"split263_reusectrl_full_gpu: mode={bench_mode} preset={preset_queens_arg} worker={worker_id}/{worker_count}")
    if bench_mode==28 or bench_mode==29 or bench_mode==30 or bench_mode==31:
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and (bench_mode==30 or bench_mode==31):
      ...
      progress_suffix:str=f"split263_{'probe' if bench_mode==30 else 'full'}_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
