...
結果が明確です。297 = **362.707s vs 296 = 353.671s、差 −9.036s (−2.555%) — 大幅悪化、不採用**。

`save_sp` 削除が逆効果でした。296の `save_sp` は冗長に見えたが、実際にはコンパイラが `save_sp` と `stack_ptr` を協調してレジスタ最適化していた可能性があります。また `if save_sp==0:` → `if stack_ptr==0:` の置換により、比較対象が `0` から `0` へと同じでも、コンパイラへのヒントが変わった可能性があります。

**296が確定ベース**。`save_sp` は 296 に戻す。次は別の角度から攻めます。## 297 結果まとめと 298 の考え方

**297 不採用**: 362.707s vs 296 = 353.671s → **−9.036s (−2.555%) 悪化**。`save_sp` 削除は逆効果でした。コンパイラが `save_sp` と `stack_ptr` の2変数を協調スケジューリングに活用していたと考えられます。**296 が確定ベース**。

---

## 現在の累計進捗（292基準）

| Rev | 変更 | elapsed | vs 292 |
|---|---|---|---|
| 292 | K=32 grid-stride | 367.413s | 基準 |
| 294 | 4xu32→2xu64 pack | 362.782s | −1.26% |
| **296** | **sp2乗算→stack_ptr** | **353.671s** | **−3.74%** |
| 297 | save_sp削除(不採用) | 362.707s | +1.28% 悪化 |

## 298 の着眼点

297 で `save_sp` 削除が裏目に出たため、温存します。代わりに **`next_depth:int=cur_depth+1`** 一時変数を排除します。

```python
# 296/297の形:
next_depth:int = cur_depth + 1
if cur_avail != u32(0):
    stack[stack_ptr] = ...  # cur_depth を詰める
    stack_ptr += 2
    save_sp += 1
cur_depth = next_depth      ← ここをなくす

# 298の形:
if cur_avail != u32(0):
    stack[stack_ptr] = ...  # cur_depth を詰める（変わらず）
    stack_ptr += 2
    save_sp += 1
cur_depth += 1              ← インライン化
```

`save_sp` は保持、変更は `next_depth` の排除のみ。

```bash
STATIC_ONLY=1 bash 298Py_next_depth_elim_validate_N21_full_once.sh
bash 298Py_next_depth_elim_validate_N21_full_once.sh
```
"""
...
VERSION_TAG:str="298 next-depth-elim probe: parent 297 save-sp-elim (rejected, +9.036s regression vs 296); reverted to 296 as base (353.671s); eliminates next_depth:int=cur_depth+1 temp variable from the main DFS push/descend sequence, replacing cur_depth=next_depth with cur_depth+=1 inline after the push block; reduces live integer register count by 1 in the hot loop; save_sp retained from 296 (297 showed save_sp removal regresses); stack array (MAXD14_ANCESTOR*2) and stack_ptr counter from 296 unchanged; K_PER_THREAD_MAXD14=32, all launch params unchanged; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
