...
CPU
$ ./74Py_constellations_GPU_cuda_codon_n21_crossstripe_fix_debug -c

GPU
$ codon build -release 74Py_constellations_GPU_cuda_codon_n21_crossstripe_fix_debug.py
$ ./74Py_constellations_GPU_cuda_codon_n21_crossstripe_fix_debug -g 5 20 32 484 0 -1

N=20 単体ベンチ
$ ./74Py_constellations_GPU_cuda_codon_n21_crossstripe_fix_debug -g 20 20 32 484 0 -1

ログ確認
$ ./74Py_constellations_GPU_cuda_codon_n21_crossstripe_fix_debug -g 20 20 32 484 2 -1 5 5

# ============================================================
# 73 STABLE FINAL BENCH + CROSS STRIPE SAFE CLEANUP
# 74 N21 CROSS STRIPE FIX + REORDER DEBUG
# ============================================================
# Base:
...
VERSION_TAG:str="74 N21 CROSS STRIPE FIX + REORDER DEBUG"
...
"""74 の auto sort 方針。N=20 だけ sort_mode=9、それ以外は安全側で sort_mode=0。"""
def auto_sort_mode(N:int)->int:
  if N==20:
    return 9
  return 0
...
def exec_solutions(N:int,constellations:List[Dict[str,int]],use_gpu:bool,gpu_block:int=32,gpu_max_blocks:int=484,gpu_log_level:int=0,gpu_sort_mode:int=-1,cross_stripe_safe:bool=CROSS_STRIPE_SAFE_DEFAULT,reorder_only:bool=False)->None:
  """
  機能:
  ...
  if use_gpu:
    ...
    if stripe_chunks:
      ...
      if gpu_sort_mode==5 or gpu_sort_mode==8 or gpu_sort_mode==10:
        ...
        local_sort_mode=0
    if gpu_log_level>=1:
      print(f"[gpu-config] N={N} original=1 mixed32=1 hotpath=1 trunk74={BLOCK}x{MAX_BLOCKS} sort_mode={gpu_sort_mode} local_sort={local_sort_mode} stripe={1 if stripe_chunks else 0} balanced={1 if balanced_stripe else 0} cross={1 if cross_stripe else 0} cross_safe={1 if cross_stripe_safe else 0} reorder_only={1 if reorder_only else 0} block={BLOCK} max_blocks={MAX_BLOCKS} steps={STEPS} log_level={gpu_log_level}")
...
def main()->None:
  ...
  bench_mode:int=0  # 0:normal, 1:N=20 warmup repeat, 2:N19 preheat, 3:N18+N19 preheat, 4:N20 repeat3 sweep, 5:N20 repeat2 benchmark, 6:reorder-only debug
  ...
  if argc == 1:
    ...
    if argc >= 9:
      ...
      if requested_preset!=5:
        print(f"[warning] preset_queens={requested_preset} is disabled in 74 N21 CROSS STRIPE FIX; using 5")
      preset_queens_arg=5
    if argc >= 10:
      bench_mode=int(sys.argv[9])
      if bench_mode<0 or bench_mode>6:
