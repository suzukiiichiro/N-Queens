...
結果明確です。295 = **362.588s vs 294 = 362.782s、差 +0.194s (+0.053%) — 誤差級、flat**。

stack merge による局所性改善は効果なし。294 の `ldrd`/`colav` 2配列レイアウトと実質同等でした。これは local memory が既にコンパイラによって十分最適化されていたか、2配列が元々近接配置されていたためと考えられます。

295 は不採用。**次の方針**を考えます。push が2箇所あり `stack_ptr+=2` も2回、正しいです。push site 1（preroll）では `save_sp+=1` の前に `stack_ptr+=2` があるので計2回。チェックの期待値を修正します。## 295 結果と 296 の考え方

**295 不採用**: 362.588s vs 294の362.782s → **+0.194s (+0.053%)、誤差級**。隣接インデックスでの L1 局所性改善は効果なし。

---

## 進捗の整理

| リビジョン | 変更内容 | elapsed | vs 292 |
|---|---|---|---|
| 292 | K=32 grid-stride | 367.413s | 基準 |
| 293 | dual-lane (不採用) | 367.652s | flat |
| **294** | **4xu32→2xu64 pack** | **362.782s** | **−1.26%** |
| 295 | 2配列→1配列 (不採用) | 362.588s | flat |

## 296 の着眼点

295 の `sp2:int=save_sp*2` は push/pop のたびに整数乗算が走ります（push 2箇所 + pop 1箇所 = constellation あたり最大数回）。**`stack_ptr` カウンタを並列管理**して常に `save_sp*2` の値を保持することで、この乗算をシフト/インクリメントに置き換えます：

```
# push (296):
stack[stack_ptr] = ldrd_val
stack[stack_ptr+1] = colav_val
stack_ptr += 2        ← 乗算なし
save_sp += 1
...
save_sp -= 1
stack_ptr -= 2        ← 乗算なし
ldrd = stack[stack_ptr]
colav = stack[stack_ptr+1]
```

```bash
STATIC_ONLY=1 bash 296Py_stack_ptr_validate_N21_full_once.sh
bash 296Py_stack_ptr_validate_N21_full_once.sh
```
"""
...
VERSION_TAG:str="296 stack-ptr probe: parent 295 stack-merge (rejected, flat +0.053% vs 294); kernel_dfs_iter_gpu_maxd14 replaces the sp2=save_sp*2 multiply on each push/pop (3 multiplies per constellation-iteration) with a direct stack_ptr counter incremented/decremented by 2, eliminating integer multiplies from the push/pop path; single stack array (MAXD14_ANCESTOR*2) from 295 retained; save_sp still maintained in parallel for the save_sp==0 break check; hot-loop DFS logic, schedule decode, root-preroll, K_PER_THREAD_MAXD14=32, and all launch parameters otherwise unchanged from 295; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
