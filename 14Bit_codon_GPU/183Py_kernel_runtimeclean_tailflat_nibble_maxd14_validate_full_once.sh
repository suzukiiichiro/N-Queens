...
# =============================================================================
# NQ_UPDATE_MEMO
# 183: 更新メモ: 182のN=21 full検証OK・0:07:51.064を受け、GPU kernelは182/181/180/175相当のまま維持。182で実運用確認済みのruntime-only検証を正式版として継承し、source/model Assertion検証は引き続き非強制。
# Full update history: see README.md
# =============================================================================

# 183 runtime-only single-pass validation harness
#
# One N=21 full GPU run reconstructs cases 01-05 from the same 131-row
# progress TSV and verifies the complete total. Candidate 183 keeps the validated 182/181/180/175 nibble MAXD14 kernel after the 178/179
...
# and 208 local stack bytes/thread. The experiment is deliberately safe: keep the validated kernel path, keep
...
# GPU arithmetic and task order remain the validated 182/181/180/175 path.
SRC=${SRC:-./183Py_kernel_runtimeclean_tailflat_nibble_maxd14_probe.py}
CAND=${CAND:-./183Py_kernel_runtimeclean_tailflat_nibble_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/183Py_kernel_runtimeclean_tailflat_nibble_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 183 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/183Py_kernel_runtimeclean_tailflat_nibble_maxd14_logs_N21_full_once_${TS}"
...
#   can fail before the real Codon/CUDA validation starts if the local 183Py source
#   has a harmless header/string/layout difference.  This 183 harness validates the
...
printf 'source_parent_baseline\tvalidated 182/181/180/175 nibble MAXD14 dispatch\truntime-only validation\tINFO\n' >> "$SUMMARY"
...
if [[ "$STATIC_ONLY" == "1" ]]; then
  ...
  echo "[logdir] $LOGDIR"
  echo "[static-validation-skipped] 183 runtime-only harness disabled source/model assertions"
  exit 0
fi
...
# 183 keeps the validated 180/175 kernel structure, then derives chunk-tail diagnostics from
...
# 183: richer tail diagnostics from the already-validated progress TSV.
...
echo "[validation-ok] 183 runtime-clean tailflat reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
