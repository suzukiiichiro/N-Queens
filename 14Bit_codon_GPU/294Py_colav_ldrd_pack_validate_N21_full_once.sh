...
# =============================================================================
# 294 N=21 full validation harness (colav-ldrd-pack probe)
...
#           schedule_words=0, stack=208 bytes/thread (unchanged: MAXD14_ANCESTOR=13
#           x 16B/entry; u64 packing changes word width not count),
#           K_PER_THREAD_MAXD14=32 -> still 5 dispatch/progress rows.
...
SRC=${SRC:-./294Py_colav_ldrd_pack_probe.py}
CAND=${CAND:-./294Py_colav_ldrd_pack_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/294Py_colav_ldrd_pack_probe_N21_full.lock}
...
LOGDIR="${LOG_ROOT%/}/294Py_colav_ldrd_pack_logs_N21_full_once_${TS}"
...
echo "[start] 294 colav-ldrd-pack validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 294 colav-ldrd-pack SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '294 colav-ldrd-pack probe' "$SRC"; then
  printf 'source_version_tag	294 colav-ldrd-pack probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	294 colav-ldrd-pack probe	missing	FAIL
...
# 294: colav-ldrd pack shape checks (replaces 293 dual-lane checks).
    ok_stride_param = 'stride:int,' in body14.split(')->None:')[0]
    ok_ldrd_array = 'ldrd=__array__[u64](MAXD14_ANCESTOR)' in body14
    ok_colav_array = 'colav=__array__[u64](MAXD14_ANCESTOR)' in body14
    ok_no_u32_stack = ('ld=__array__[u32]' not in body14 and 'rd=__array__[u32]' not in body14
                        and 'col=__array__[u32]' not in body14 and 'avail=__array__[u32]' not in body14)
    ok_no_lane_b = 'ld_b=__array__' not in body14 and 'DUAL_LANE' not in body14
    ok_gridstride_loop = 'while idx<m:' in body14
    ok_single_writeback = 'results[tid]=thread_total\n' in body14
    ok_no_percall_writeback = 'results[idx]=' not in body14
    ok_push_pack = body14.count('ldrd[save_sp]=u64(cur_ld)') >= 1 and body14.count('colav[save_sp]=u64(cur_col)') >= 1
    ok_pop_pack = 'packed_ldrd:u64=ldrd[save_sp]' in body14 and 'packed_colav:u64=colav[save_sp]' in body14
    ok_colav_shape = (ok_stride_param and ok_ldrd_array and ok_colav_array and ok_no_u32_stack
                       and ok_no_lane_b and ok_gridstride_loop and ok_single_writeback
                       and ok_no_percall_writeback and ok_push_pack and ok_pop_pack)
    checks.append(('source_colav_ldrd_shape',
                    'ldrd/colav u64 arrays + u32 stack absent + push/pop packed + stride + single writeback',
                    'present' if ok_colav_shape else (
                        'missing (stride=%s ldrd=%s colav=%s no_u32_stk=%s no_lane_b=%s gridstride=%s writeback=%s no_percall=%s push=%s pop=%s)'
                        % (ok_stride_param, ok_ldrd_array, ok_colav_array, ok_no_u32_stack, ok_no_lane_b,
                           ok_gridstride_loop, ok_single_writeback, ok_no_percall_writeback, ok_push_pack, ok_pop_pack)
                    ),
                    ok_colav_shape))
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 294 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 294 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 294 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 294 colav-ldrd-pack mode31 split145"
...
echo "[validation-ok] 294 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=32 (5 dispatch/progress rows)"
