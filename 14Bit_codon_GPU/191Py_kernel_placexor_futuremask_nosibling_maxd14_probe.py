...
# =============================================================================
# NQ_UPDATE_MEMO
# 191: 更新メモ: 190は正当性OKだが188比で退行したため不採用。188を親に戻し、MAXD14 kernelのみfree候補bitがld/rd/colとdisjointである前提を使い、queen配置のORをXORへ置換する単独実験版。
...
ただし速度は `0:07:53.357` で、188Pyの `0:07:10.137` より `43.220秒`、`10.048%` 遅くなっています。したがって、190Pyは**正当性OKだが不採用**とし、191Pyは190を親にせず、**188Pyを親に戻して**作成しました。

作成物はこちらです。

* [191Py_kernel_placexor_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/191Py_kernel_placexor_futuremask_nosibling_maxd14_probe.py)
* [191Py_kernel_placexor_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/191Py_kernel_placexor_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/191Py_kernel_placexor_futuremask_nosibling_maxd14_bundle_20260630.tar)

191Pyの変更点は、188Pyの `future_check_mask` guard / no-sibling spill elision を維持したまま、MAXD14 kernelのqueen配置演算だけを **ORからXORへ置換**した点です。
...
```text
cur_avail = bm & ~(cur_ld | cur_rd | cur_col)
```

から取り出すため、board内では `cur_ld / cur_rd / cur_col` とdisjointです。そのため、この配置部分では `state | bit` と `state ^ bit` は等価です。狙いは、hot placement updateの演算形がCodon/CUDA生成コード上で少しでも軽くなるかを見ることです。

191Pyには、190Pyの `block_check_mask` guard、189Pyの forced-chain fast path、187Pyの child_jmark guard、185/186Pyのterminal系変更は入れていません。188Py親の単独実験です。

手元では以下を確認済みです。

```text
bash -n 191Py...sh                         OK
STATIC_ONLY=1 191Py...sh                   OK
source_future_check_mask_guard             OK
source_nosibling_parent                    OK
source_disjoint_place_xor                  OK
source_190_block_guard_removed             OK
source_189_forced_chain_removed            OK
source_187_jmark_guard_removed             OK
source_terminal13_literal_removed          OK
source_split_tag split191 only             OK
abstract disjoint place-xor equivalence    200000 cases OK
```

こちらではCodon/CUDA実機build/full実行はしていません。191Pyの採用可否は、添付の `191Py...sh` をcudacodon側で実行し、N=21 fullのcases 01-06、dispatch、final total、warning/error、特に188比のtimingを見て判断してください。


"""

# 191 place-XOR revision memo over validated 188 futuremask/no-sibling MAXD14:
#   190 block_check_mask guard was correct but slower, so this branch returns to
#   the validated 188 baseline.  Host-side depth computation, scorestripe/chunkshape148
#   task order, cache naming, dispatch range 14/16/18/20/21, bitboard arithmetic
#   meaning, and solution arithmetic are unchanged.
#
...
#   parent depth, terminal base14 bit, root action, and the 188 future_check_mask.
#   MAXD16/18/20/21 keep the validated byte-opcode local schedule fallback for
#   larger required depths.
#
#   The active DFS frame remains in scalar registers and local arrays still hold
#   only restorable ancestor frames. 191 changes only the MAXD14 queen-placement
#   expression. The selected bit is always taken from cur_avail, which was formed
#   as bm & ~(cur_ld|cur_rd|cur_col), so within the board it is disjoint from
#   cur_ld, cur_rd, and cur_col. Therefore (state | bit) and (state ^ bit) are
#   equivalent for the placed queen. 191 uses XOR in the hot placement update to
#   test whether avoiding OR dependencies helps code generation. The no-sibling
#   save_sp/cur_depth policy and future_check_mask zero guard remain exactly 188-like;
#   stack storage remains four u32[13] arrays = 208 bytes/thread.
...
# 191 keeps the validated 188 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 191 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="191 place-xor: 188 futuremask/no-sibling MAXD14 baseline plus disjoint placement XOR - README-managed history"
...
# 191 inherits the exact 188/184/183/172/171/170/169/168/167 host-side control-schedule depth model.  It mirrors only the row/fid
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """191 MAXD14 nibble schedule plus no-sibling/futuremask and disjoint placement XOR."""
    ...
    # 191 keeps the validated 188 current-frame register cache, 13-slot
    # ancestor storage, no-sibling tail-call descent, and future_check_mask guard.
    # It changes only queen placement in the MAXD14 hot loop: since bit is selected
    # from cur_avail, it is disjoint from cur_ld/cur_rd/cur_col inside bm, so
    # (state|bit) is equivalent to (state^bit).
    #
    ...
    while True:
      ...
      if nf==u32(0):
        continue
      # 191 keeps the 188 future_check_mask guard: if no depth in this schedule
      # carries the future-prune bit, avoid testing nibble bit3 for every candidate.
...
CHUNKSHAPE148_DEFAULT_REASON:str="191 place-xor reuses validated 188 scorestripe_v9 task order/cache and changes only MAXD14 placement operator"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split191_placexor_futuremask_guard_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split191_placexor_futuremask_guard_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split191-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split191-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split191_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split191-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split191-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split191_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
