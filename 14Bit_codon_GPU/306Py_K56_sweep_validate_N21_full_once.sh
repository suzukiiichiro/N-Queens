...
# =============================================================================
# 306 N=21 full validation harness (K56-sweep probe)
# Parent: 305 K40-sweep (rejected: 353.587s, flat vs 296=353.671s, -2.517s vs 304=351.070s).
#   K=40 falls back to ~K=32 level; K=48 is the peak. K survey:
#     K=32: 353.671s, K=40: 353.587s, K=48: 351.070s (BEST), K=64: 353.896s
#   Probing K=56 to confirm right side drops off from K=48.
#   EXPECTED_CHUNKS=ceil(2025282/(32*484*56))=ceil(2025282/867328)=3.
#   All kernel logic unchanged from 296.
# Safety: K_PER_THREAD_MAXD14=56, EXPECTED_CHUNKS=3.
# NOTE: this validation script has not yet been executed on real hardware.
# =============================================================================

SRC=${SRC:-./306Py_K56_sweep_probe.py}
CAND=${CAND:-./306Py_K56_sweep_probe}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/306Py_K56_sweep_probe_N21_full.lock}
...
EXPECTED_K_PER_THREAD_MAXD14=56
...
LOGDIR="${LOG_ROOT%/}/306Py_K56_sweep_logs_N21_full_once_${TS}"
...
echo "[start] 306 K56-sweep validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 306 K56-sweep SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '306 K56-sweep probe' "$SRC"; then
  printf 'source_version_tag	306 K56-sweep probe	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	306 K56-sweep probe	missing	FAIL
...
# 304: K48-sweep shape checks (296 kernel logic, K=48).
    ...
    checks.append(('source_K56_sweep_shape',
                    '296 kernel shape (stack/save_sp/next_depth/cur_depth/stack_ptr) + K=56',
...
if (( static_failures != 0 )); then
  ...
  cat "$SUMMARY"
  echo "[logdir] $LOGDIR"
  echo "[error] 306 source static checks failed" >&2
  exit 66
fi
echo "[static-ok] 306 source checks passed; proceeding to release build/run"

if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 306 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT")
{
  ...
  echo "validation: one N=21 full run through 306 K56-sweep mode31 split145"
...
echo "[validation-ok] 306 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=56 (3 dispatch/progress rows)"
