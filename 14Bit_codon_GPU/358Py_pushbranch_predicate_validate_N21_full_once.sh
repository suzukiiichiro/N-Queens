...
# =============================================================================
# 358 N=21 full validation harness (hot-loop push guard: if -> unconditional
# store + predicated ternary update -- FIRST CONTROL-FLOW-SHAPE CHANGE SINCE
# THE 240/266-269/273/326 LOSSES)
#
# WHAT 358 IS. 355 and 357 -- two independent measurements, N=18 static
# SourceCounters and N=21 dynamic SchedulerStats+WarpStateStats, taken
# before and after 356 -- confirmed that stall_branch_resolving's two
# culprit BRA instructions (both adjacent to BSYNC reconvergence markers)
# remain essentially untouched by 356's bit-width narrowing: 60.2% then
# 60.45% combined, and ncu's own divergence-driven local speedup estimate
# stayed at 79.68% then 79.29%. 356-style narrowing does not reach this
# structure; 358 changes approach.
#
# THE CHANGE (one site only): the hot-loop push guard,
#     if cur_avail!=u32(0):
#       stack[stack_ptr]=...; stack[stack_ptr+1]=...
#       stack_ptr+=2; save_sp+=u32(1)
# -- culprit-1, 45.16% of stall_branch_resolving on its own -- becomes
#     stack[stack_ptr]=...; stack[stack_ptr+1]=...          # unconditional
#     push_inc:int=2 if cur_avail!=u32(0) else 0
#     stack_ptr+=push_inc
#     save_sp+=(u32(1) if cur_avail!=u32(0) else u32(0))
# removing the `if` shape itself so the compiler is nudged toward predicated
# select instructions instead of a branch + BSYNC reconvergence point.
#
# THIS IS A CONTROL-FLOW-SHAPE CHANGE, unlike 356. It is scoped to exactly
# one site: the once-per-constellation preamble push (same guard pattern,
# structurally identical, outside the hot loop) is deliberately left
# touched-not (still `if cur_avail!=u32(0):`), and the pop path (culprit-2,
# 15.30%) is untouched -- an unconditional load at stack_ptr-2 would
# underflow when save_sp==0, unlike the push side.
#
# MEMORY SAFETY: stack is sized MAXD14_ANCESTOR*2 = 26 u64 words. save_sp
# resets to 0 per constellation and never reaches MAXD14_ANCESTOR while
# correctness holds at 314666222712 (an invariant every prior revision's
# oracle has verified), so the pre-increment stack_ptr at this site is
# always <=24; a discarded speculative write here is never read back, since
# loads only ever occur at positions a genuine push actually advanced
# save_sp/stack_ptr to.
#
# THIS IS NOT KERNEL DECOMPOSITION OR LOOP DUPLICATION -- the two failure
# modes that lost five times running (240, 266-269, 273, 326; the worst,
# 326's hot-loop duplication, cost -13.7%). No loop is duplicated, no
# kernel is split; exactly one guard's shape changes.
#
# JUDGE ON CHUNK0/CHUNK1 AGAINST THE 356 ANCHOR (144590ms / 144473ms), NOT
# SESSION ELAPSED -- 357 made no code change, so 356 remains the anchor.
#
#     STATIC_ONLY=1 bash 358Py_pushbranch_predicate_validate_N21_full_once.sh
#     bash 356Py_savesp_narrow_validate_N21_full_once.sh      # anchor (reuse)
#     bash 358Py_pushbranch_predicate_validate_N21_full_once.sh   # candidate
#
# DECISION RULE (fixed before running): within +0.10% of the 356 anchor on
# chunk0/chunk1 (improvement or neutral) -> ADOPT, proceed to 359. Beyond
# +0.10% regression -> REVERT, return to 355/357's SASS-identification
# work. Correctness 314666222712 is checked before any timing is read.
#
# FOLLOW-UP (regardless of adoption): rerun N=18 SourceCounters to check
# whether culprit-1's sample count actually moved -- mechanism and
# wall-clock outcome are separate questions (item 12's lesson).
#
# EXPECTED_CHUNKS=3, K=48 (unchanged). Cache is reused (..._isort9.bin).
# =============================================================================

SRC=${SRC:-./358Py_pushbranch_predicate.py}
CAND=${CAND:-./358Py_pushbranch_predicate}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/358Py_pushbranch_predicate_N21_full.lock}
...
# 358 judges chunk0/chunk1 against the 356 anchor per the noise-floor lesson
# (357 made no code change, so 356 remains the most recent code anchor).
BASELINE_356_SECONDS=${BASELINE_356_SECONDS:-393.404}
...
LOGDIR="${LOG_ROOT%/}/358Py_pushbranch_predicate_logs_N21_full_once_${TS}"
...
echo "[start] 358 pushbranch-predicate validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 358 pushbranch-predicate SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '358 pushbranch-predicate' "$SRC"; then
  printf 'source_version_tag	358 pushbranch-predicate	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	358 pushbranch-predicate	missing	FAIL
...
# ---- 358: prove the diff against 356 is EXACTLY the push-guard change ----
# Same two-proof technique 356 established against 352, now against 356.
#
# PROOF 1: maxd16/maxd18/maxd20/maxd21 and the dispatcher's launch signatures
# are BYTE IDENTICAL to 356 -- sliced from "def kernel_dfs_iter_gpu_maxd16("
# to "def launch_kernel_dfs_iter_gpu_static_maxd(" and compared verbatim.
#
# PROOF 2: after docstrings and the two prose constants are stripped, the
# rest of the file (including maxd14) differs from 356 by EXACTLY the
# push-guard rewrite (21 added / 5 removed lines, including the explanatory
# comment) -- the sha256 fingerprint of the sorted diff. Any unrelated
# change anywhere FAILs this, including a stray edit to the preamble push
# or the pop path.
#
# Both FAIL rather than skip when 356 is absent, per every prior identity
# check in this project.
REF356=${REF356:-./356Py_savesp_narrow.py}
EXPECTED_PUSHBRANCH_DIFF="added=21 removed=5 sha256=6d988f60ddedbe5e48ee8527899cfe5eb6b5c9b47aa85c6e1f4fd161195d6724"
if [[ -f "$REF356" ]]; then
  PUSHBRANCH_IDENT_AND_DIFF=$(python3 - "$REF356" "$SRC" <<'PYPUSHBRANCHIDENT'
import re, sys, difflib, hashlib

def other_kernels_slice(p):
    s = open(p, encoding='utf-8').read()
    a = s.index('def kernel_dfs_iter_gpu_maxd16(')
...
k_ref, k_src = other_kernels_slice(sys.argv[1]), other_kernels_slice(sys.argv[2])
other_status = 'identical' if k_ref == k_src else 'DIFFERS_%d_vs_%d_bytes' % (len(k_ref), len(k_src))
...
print('other_kernels=%s' % other_status)
print(diff_status)
PYPUSHBRANCHIDENT
)
  OTHER_STATUS=$(echo "$PUSHBRANCH_IDENT_AND_DIFF" | sed -n '1p')
  DIFF_STATUS=$(echo "$PUSHBRANCH_IDENT_AND_DIFF" | sed -n '2p')
  record_check source_other_kernels_identical_to_356 "other_kernels=identical" "${OTHER_STATUS:-compare_failed}" || static_failures=$((static_failures+1))
  record_check source_code_identical_to_356_except_pushbranch "$EXPECTED_PUSHBRANCH_DIFF" "${DIFF_STATUS:-compare_failed}" || static_failures=$((static_failures+1))
else
  printf 'source_other_kernels_identical_to_356\tother_kernels=identical\t356 source not found at %s\tFAIL\n' "$REF356" >> "$SUMMARY"
  printf 'source_code_identical_to_356_except_pushbranch\t%s\t356 source not found at %s\tFAIL\n' "$EXPECTED_PUSHBRANCH_DIFF" "$REF356" >> "$SUMMARY"
  static_failures=$((static_failures+2))
fi

# ---- 358: the push-guard rewrite itself, checked site by site ----
# NEGATIVE TEST (the established procedure): feed 356's own source to this
# script (SRC=REF=356) and SIX checks correctly FAIL: source_version_tag
# (missing), source_pushbranch_ternary_present and
# source_pushbranch_store_unconditional (0 instead of 1, the rewrite isn't
# there), source_hotloop_push_if_absent (reads PRESENT, 356 still has the
# `if`), source_K48_sweep_shape (ptr=False, old literal), and
# source_code_identical_to_356_except_pushbranch (356 vs itself diffs by
# 0/0, which does not match the expected 21/5 -- a 0-diff is exactly as
# wrong an answer as any other unexpected diff would be):
#     SRC=./356Py_savesp_narrow.py STATIC_ONLY=1 bash 358Py_pushbranch_predicate_validate_N21_full_once.sh
#
# 1) the new pattern must be present, exactly once
PUSH_TERNARY=$(grep -c '^        push_inc:int=2 if cur_avail!=u32(0) else 0$' "$SRC_NODOC" || true)
record_check source_pushbranch_ternary_present 1 "$PUSH_TERNARY" || static_failures=$((static_failures+1))
PUSH_STORE_UNCOND=$(grep -c '^        stack\[stack_ptr\]=u64(cur_ld)|(u64(cur_rd)<<u64(32))$' "$SRC_NODOC" || true)
record_check source_pushbranch_store_unconditional 1 "$PUSH_STORE_UNCOND" || static_failures=$((static_failures+1))
# 2) the hot-loop `if cur_avail!=u32(0):` guard (10-space indent, the one
# directly preceding `next_depth:int=cur_depth+1` in source) must be GONE --
# but the structurally identical preamble guard (12-space indent, inside the
# once-per-constellation root_after_second block) must still be there, so a
# naive count-based check would be blind to "changed the wrong one."
HOTLOOP_GUARD_GONE=$(awk '/^        next_depth:int=cur_depth\+1$/{getline; if ($0=="        if cur_avail!=u32(0):") print "PRESENT"; else print "ABSENT"}' "$SRC_NODOC")
record_check source_hotloop_push_if_absent ABSENT "${HOTLOOP_GUARD_GONE:-CHECK_FAILED}" || static_failures=$((static_failures+1))
PREAMBLE_GUARD_COUNT=$(grep -c '^          if cur_avail!=u32(0):$' "$SRC_NODOC" || true)
record_check source_preamble_push_if_unchanged 1 "$PREAMBLE_GUARD_COUNT" || static_failures=$((static_failures+1))
# 3) the pop path (culprit-2, untouched) must still read exactly as before
POP_GUARD=$(grep -c '^          if save_sp==u32(0):$' "$SRC_NODOC" || true)
POP_DEC=$(grep -c '^          save_sp-=u32(1)$' "$SRC_NODOC" || true)
POP_SITES="guard=$POP_GUARD dec=$POP_DEC"
record_check source_pop_site_unchanged "guard=1 dec=1" "$POP_SITES" || static_failures=$((static_failures+1))
# 4) save_sp stays u32 everywhere (356's narrowing is not reverted by 358)
SAVESP_DECL=$(grep -c '^      save_sp:u32=u32(0)$' "$SRC_NODOC" || true)
record_check source_savesp_still_u32 1 "$SAVESP_DECL" || static_failures=$((static_failures+1))

# stack_ptr must remain untouched (still plain int, no u32 cast anywhere on it)
if grep -q 'stack_ptr:int=0' "$SRC_NODOC" && ! grep -q 'stack_ptr:u32' "$SRC_NODOC" && ! grep -q 'stack_ptr-=u32' "$SRC_NODOC"; then
  printf 'source_stack_ptr_unchanged\tstill int, no u32\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_stack_ptr_unchanged\tstill int, no u32\tmodified\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
# stack array must still be sized MAXD14_ANCESTOR*2 -- the memory-safety
# argument for the unconditional push store depends on this exact sizing.
if grep -q 'stack=__array__\[u64\](MAXD14_ANCESTOR\*2)' "$SRC_NODOC"; then
  printf 'source_stack_array_size_unchanged\tMAXD14_ANCESTOR*2\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_stack_array_size_unchanged\tMAXD14_ANCESTOR*2\tmodified or missing\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
# 304: K48-sweep shape checks (296 kernel logic, K=48).
    ...
    ok_save_sp = 'if save_sp==u32(0):' in body14  # 356: save_sp narrowed to u32; literal updated to match
    ok_next_depth = 'next_depth:int=cur_depth+1' in body14
    ok_cur_depth = 'cur_depth:int=0' in body14
    # 358: hot-loop push increment changed from a literal `stack_ptr+=2`
    # inside an `if` to `stack_ptr+=push_inc` (ternary-fed, unconditional
    # statement). The preamble push (untouched) still reads `stack_ptr+=2`
    # literally. Updated to match, same policy 356 used for the save_sp
    # literal above.
    ok_stack_ptr_incr = (body14.count('stack_ptr+=2') == 1
                          and 'push_inc:int=2 if cur_avail!=u32(0) else 0' in body14
                          and 'stack_ptr+=push_inc' in body14)
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN" "$CHUNKSHAPE148_ITER_SORT")
{
  ...
  echo "validation: one N=21 full run through 358 pushbranch-predicate. FIRST CONTROL-FLOW-SHAPE CHANGE SINCE 240/266-269/273/326: the hot-loop push guard (if cur_avail!=u32(0): stores+increments) becomes unconditional stores plus a predicated ternary stack_ptr/save_sp update (21 added / 5 removed lines vs 356, source_code_identical_to_356_except_pushbranch proves it statically), targeting culprit-1 of stall_branch_resolving (45.16 percent of it, per 355/357 SourceCounters). Preamble push and pop path (culprit-2) are untouched; stack_ptr, maxd16/18/20/21 and the dispatcher are unchanged. mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT, all unchanged from 356, shaped bin reused. Judge on chunk0/chunk1 against the 356 anchor (144590ms/144473ms), not session elapsed. Adopt within +0.10 percent, revert beyond it; correctness 314666222712 checked first either way"
...
echo "[validation-ok] 358 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2, CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN and CHUNKSHAPE148_ITER_SORT=$CHUNKSHAPE148_ITER_SORT, all unchanged from 356. Hot-loop push guard is unconditional-store+ternary at the one site 355/357 measured; preamble push, pop path, stack_ptr and the other 4 kernels are byte identical to 356. Read the chunk0/chunk1 comparison against the 356 anchor, not session elapsed, before deciding adopt vs revert per the fixed +0.10 percent rule"
