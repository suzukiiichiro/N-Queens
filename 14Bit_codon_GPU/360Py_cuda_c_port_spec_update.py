...
================================================================================
## 現在の未解決課題 (Open Objectives) -- 最終更新: 360 (2026-08-24)
...
6. [進行中・360で338からの差分を反映] CUDA Cランナー(案C-2)スパイク設計
   ...
   **334-337で土台が全て揃った(確定)**: nvcc=`/usr/local/cuda/bin/nvcc`
   ...
   450.056sで再現済み。
   **338はコードを一切追加しない設計リビジョンだった**(324→325と同じ
   パターン)。移植仕様を`338_maxd14_port_spec.md`に文書化: (1)bin
   レコードからSoA全配列を導出する完全な手続き(`build_soa_for_range`と
   `symmetry`のC言語仕様への写像、28分岐のfuncid決定木を含む)、
   ...
   (3)N=18限定の突き合わせスコープ、(4)±3%以内なら成功という判定基準
   (移植は速度向上が目的ではなく、Codonでは使えないwarp intrinsics・
   デバイスatomic・`-lineinfo`のper-line SASS帰属の解禁が目的)。

   **338以降、カーネルに2件の変更が入り、338 specは337時点のソースを
   前提にしていたため陳腐化していた**: (i) 351でw_hi_arr(上位半分の
   SoA配列)が完全削除され、SoA配列は7本(ld_arr/rd_arr/col_arr/
   ctrl0_arr/free_arr/markctrl_arr/w_lo_arr)に減った。338が仕様化した
   旧576/581/750行のw_hi_arrロード3箇所はもう存在しない。(ii) 356で
   save_spがint→u32にnarrowingされ(393.404秒へ採用)、C側では
   `save_sp`を`uint32_t`、`stack_ptr`は据え置きで`int`(配列添字の
   未検証な幅変更リスクを避けるため356が意図的に見送った判断)として
   仕様化する必要がある。

   **360はこの2件の差分を338 specへ反映する設計パッチである**
   (`360_maxd14_port_spec_update.md`。360自身もコード変更ゼロ、338と
   同じ規律)。あわせて、±3%判定基準の分母を337の450.056秒から
   **356の393.404秒**へ更新した。358・359で確定した「制御フロー形状
   変更はこのカーネルで危険」という知見は、移植そのもの(振る舞い等価
   性が目的)には直接影響しないが、移植後にC側で最適化を試す段になっ
   たら参照すべきリスクノートとして記録した(Codonバックエンド固有の
   挙動である可能性があり、nvccへの外挿は未検証)。実装(`.cu`)は
   361以降。
...
2026年  9月  2日 水曜日 02:31:26 UTC
21:      314666222712                0          0:06:33.109    ok
...
"""
360を用意しました(設計のみ、コード追加なし)。

**前回セッションからの重要な更新**: 356Pyの実物を受領し、前回の
「359から再構成した356相当コード」と実物356Pyのコード領域(docstring
剥離・VERSION_TAG除外後)をバイト単位でdiffした結果、**完全一致**を
確認しました。再構成は正しかったことが実証されています。360は今回、
この実物356Pyをそのまま土台として組み直しています。

**359までの状態(README.mdより)**
- 現行採用: 356(save_sp narrowing、393.404秒、chunk0/1/2=144,590/
  144,473/103,271ms)
- 357: ncu再計測(コード変更なし)。356がbranch_resolving構造に
  効いていないことを実測で確定
- 358: pushガード完全分岐除去 → 退行(+3.47〜5.51%)、差し戻し
- 359: pushガード部分無条件化 → さらに壊滅的な退行(+65.0〜66.8%)、
  差し戻し。post-mortemでコンパイラの命令スケジューリング崩壊が示唆
- **pushガード軸(制御フロー形状変更)はここで確定的にクローズ。
  残る対策経路は項目6(CUDA Cランナー)のみ**

**360の内容**
- カーネル・ディスパッチャは356/357とバイト単位で完全一致(実物356Py
  から直接組み立てたため、今回は再構成の不確実性なし)
- 成果物は`360_maxd14_port_spec_update.md`(338からの差分パッチ文書)
  1点。`.cu`の実装は361以降
- 338 specとの差分2件を反映: (1)w_hi_arr削除(351)によりSoA配列が
  8本→7本へ、旧576/581/750行のロード3箇所が消滅、(2)save_spの
  int→u32 narrowing(356)をC側`uint32_t`として仕様化、`stack_ptr`は
  `int`のまま据え置き
- ±3%判定基準の分母を337の450.056秒から356の393.404秒へ更新

**静的チェックの参照ハッシュについて**
`.sh`の`source_code_identical_to_356`チェックは、実物の356Pyから今回
このセッションで直接算出したsha256(docstring2ブロックをtriple-quote
splitで除去、VERSION_TAG行を除外)を参照値としています。357自身が
保持していた過去の参照値(`6db7de43...`、5607行)とは一致しません
でしたが、これは357のdocstring剥離スクリプトが本ハーネスと異なる
実装(おそらく行数の数え方や空白行の扱いが微妙に違う)を使っていた
ためと推定され、コード内容の相違ではありません。実際、実物356Pyと
今回組み立てた360Pyのコード領域を直接diffして完全一致を確認済みです
(下記の`.sh`にはこの直接diffの結果も自己検査として含めています)。
...
```
STATIC_ONLY=1 bash 360Py_cuda_c_port_spec_update_validate_N21_full_once.sh
bash 356Py_savesp_narrow_validate_N21_full_once.sh          # アンカー(reuse)
bash 360Py_cuda_c_port_spec_update_validate_N21_full_once.sh   # 候補(コード同一のため393.4秒近辺を期待)
```

期待値: 正当性`314666222712`、elapsed 約393.404秒(356の再現。コード
部分が実物356Pyとバイト単位で同一であることを確認済みのため、今回は
高い確度で期待できます)。

`360_maxd14_port_spec_update.md`の内容と、Open Objectives項目6の
記述で合意が取れれば、361で`.cu`スケルトンの作成に進みます。
"""
...
VERSION_TAG:str="360 cuda_c_port_spec_update_356_delta: DESIGN-ONLY REVISION, NO KERNEL OR DISPATCHER CHANGE. Executable code (import gpu onward) is byte-identical to 356/357: this file was assembled directly from the literal 356Py_savesp_narrow.py that Suzuki supplied this session (docstring-stripped body sha256=793eef693d8f5af43ca6f131fcdd37d000efb785be5c295e54188885f5625fd4, 5601 lines, VERSION_TAG line excluded), not from a reconstruction. A prior session had reconstructed 356 from 359Py_pushbranch_partial.py by reverting its one documented push-guard hunk back to the 356/357 anchor form quoted in the project README; that reconstruction was cross-checked this session against the real 356Py via direct line-for-line diff and found byte-for-byte identical, confirming the earlier method was sound, but this file uses the real 356Py directly regardless. 360s actual deliverable is a specification-update patch on top of 338s CUDA C runner port spec (338_maxd14_port_spec.md), reflecting two kernel-level deltas that landed after 338 was written: (1) 351 removed w_hi_arr entirely (identically zero, since symmetry() only returns 2, 4 or 8), so the C port now derives exactly 7 SoA arrays -- ld_arr/rd_arr/col_arr/ctrl0_arr/free_arr/markctrl_arr/w_lo_arr -- not 8, and the three w_hi_arr load sites 338 catalogued at old lines 576/581/750 no longer exist; (2) 356 narrowed save_sp from int to u32 (5-line diff, adopted after a clean -0.83/-0.93/-2.00 percent chunk-level win, 393.404s), so the C port must declare save_sp as uint32_t while stack_ptr stays a signed int/int32_t exactly as before, per 356s own explicit decision to leave stack_ptr untouched to avoid unverified array-index-width risk. The plus-or-minus 3 percent equivalence success criterion fixed in 338 (the C port is not expected to be faster; its purpose is unlocking warp intrinsics, device atomics, and -lineinfo per-line SASS attribution that Codon cannot provide) is retained unchanged, but its performance denominator is updated from 337s 450.056s to 356s 393.404s (chunk0/chunk1/chunk2 = 144590/144473/103271 ms), since 356 is the current adopted baseline after the 357-359 campaign closed the push-guard control-flow-shape axis (358 and 359 both regressed, 359 catastrophically at plus 65 to 67 percent, with post-mortem ncu analysis attributing this to compiler instruction-scheduling collapse rather than the source-level branch-elimination hypothesis). This closure is recorded as a risk note for future C-side kernel optimization work, not as a constraint on the initial straight port itself, since the initial port targets behavioral equivalence rather than restructuring, and the Codon-backend-specific finding may not generalize to nvcc."
