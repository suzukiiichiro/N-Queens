...
**305 (K=40) = 353.587s** — vs 304(K=48) 比で −2.517s (−0.717%) 悪化、296(K=32) とほぼ同等(+0.084s)。K=40 は K=48 に劣ります。

K=48 が左側のピーク。次は右側 **K=56** を確認します。## 305 結果まとめと 306 の考え方

**305 (K=40) = 353.587s — 不採用**。K=32 とほぼ同等、K=48 には届かず。

---

## K スイープ現況（296 カーネルベース）

```
K=32:  353.671s  ──┐
K=40:  353.587s    │  なだらかな傾き
K=48:  351.070s  ← 最良点
K=64:  353.896s  ──┘  右側も傾く
```

K=48 が明確なピークです。右側の K=56 を確認して曲線の形を把握します。

```bash
STATIC_ONLY=1 bash 306Py_K56_sweep_validate_N21_full_once.sh
bash 306Py_K56_sweep_validate_N21_full_once.sh
```
"""
...
K_PER_THREAD_MAXD14:Static[int]=56


VERSION_TAG:str="306 K56-sweep probe: parent 305 K40-sweep (rejected: 353.587s, flat vs 296=353.671s, -2.517s vs 304=351.070s); K survey: K=32(353.671s) K=40(353.587s) K=48(351.070s BEST) K=64(353.896s); probing K=56 to confirm right side of K=48 optimum; EXPECTED_CHUNKS=ceil(2025282/(32*484*56))=ceil(2025282/867328)=3; all kernel logic unchanged from 296; kernel_dfs_iter_gpu_maxd16/18/20/21 unchanged"
