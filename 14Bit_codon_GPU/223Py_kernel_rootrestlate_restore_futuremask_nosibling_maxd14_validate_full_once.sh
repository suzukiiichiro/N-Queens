...
SRC=${SRC:-./223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 223 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/223Py_kernel_rootrestlate_restore_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
marker='# 223 rootrestlate-restore:'
...
if all(checks.values()):
    sys.exit(0)
print('source_rootrestlate_restore_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootrestlate_restore\tscalar_predicate_restored\tpresent\tOK\n' >> "$SUMMARY"
else
  printf 'source_rootrestlate_restore\tscalar_predicate_restored\tmissing_or_old_shape\tFAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split223'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split223' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit223 active preferred\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit223 active preferred\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if (( static_failures != 0 )); then
  echo "================================================================"; echo "[static-summary]"; cat "$SUMMARY"; echo "[logdir] $LOGDIR"
  echo "[error] 223 source static checks failed" >&2
  exit 66
fi
...
if [[ -n "$ELAPSED_TEXT" ]]; then
  ...
  for pair in "222:$BASELINE_222_SECONDS" "221:$BASELINE_221_SECONDS" "220:$BASELINE_220_SECONDS" "219:$BASELINE_219_SECONDS" "218:$BASELINE_218_SECONDS" "217:$BASELINE_217_SECONDS" "204:$BASELINE_204_SECONDS" "197:$BASELINE_197_SECONDS" "188:$BASELINE_188_SECONDS" "184:$BASELINE_184_SECONDS" "175:$BASELINE_175_SECONDS"; do
...
echo "[validation-ok] 223 rootrestlate restore reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
