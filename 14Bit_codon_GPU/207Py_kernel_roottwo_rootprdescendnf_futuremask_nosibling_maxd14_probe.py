...
# =============================================================================
# NQ_UPDATE_MEMO
# 207: 更新メモ: 206Pyは正当性OK。207Pyは204/206のroot_after_second scalar形を維持し、MAXD14 kernelのみroot-preroll内のpr_descend初期化をpr_nf由来にして、pr_nf==0分岐を削る微差実験。root_rest clear-lowbit / futuremask / no-sibling / root one-two preroll は維持する。
...
```text
204Py  0:07:07.795
206Py  0:07:07.908  204比 +0.113秒
205Py  0:07:08.074  206は205比 -0.166秒
197Py  0:07:08.202  206は197比 -0.294秒
```

したがって、206Pyは**正当性OK**です。ただし204Py最速値はまだ上回っていないため、207Pyは204/206の安全形を維持した小差分にしました。

207Pyの差分はMAXD14 kernel内だけです。
...
# 207
pr_descend:u32 = pr_nf
```

`pr_descend` は以降 `!= 0` の継続flagとしてしか使っていないため、意味は同じです。`pr_nf==0` の初期分岐だけを削る微差実験です。

手元確認は以下です。

```text
bash -n 207Py...sh                    OK
STATIC_ONLY=1 AUTO_BUILD=0 207Py...sh OK
source_rootprdescendnf                OK
source_split_tag split145 split207    OK
```

ファイルはこちらです。

* [207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_probe.py](sandbox:/mnt/data/207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_probe.py)
* [207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_validate_full_once.sh](sandbox:/mnt/data/207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_validate_full_once.sh)
* [README.md](sandbox:/mnt/data/README.md)
* [3点まとめtar](sandbox:/mnt/data/207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_bundle_20260702.tar)

実行は以下でお願いします。

```bash
STATIC_ONLY=1 AUTO_BUILD=0 bash 207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_validate_full_once.sh

bash 207Py_kernel_roottwo_rootprdescendnf_futuremask_nosibling_maxd14_validate_full_once.sh
```

"""

# 207 rootprdescendnf memo over validated 204 root-clearrest futuremask/no-sibling MAXD14:
...
#   only restorable ancestor frames.  207 keeps the narrow root fast-start only when
...
# 207 keeps the validated 204/197/196/194/192/188/184 static stack specializations.  MAXD remains the logical number of stored
...
SCHED_WORDS14:Static[int]=0  # 207 MAXD14 keeps scalar u32 nibble schedule fields, not local u32 schedule words
SCHED_WORDS16:Static[int]=4
SCHED_WORDS18:Static[int]=5
SCHED_WORDS20:Static[int]=5
SCHED_WORDS21:Static[int]=6

VERSION_TAG:str="207 roottwo-rootprdescendnf: 204/206 rootclearrest futuremask/no-sibling MAXD14 baseline with pr_descend initialized from pr_nf - README-managed history"
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    """207 MAXD14 nibble schedule plus futuremask/no-sibling and root one/two-candidate restored root_after_second predicate."""
    ...
    # correct but slower.  207 keeps 204's clear-lowbit root_rest construction and restores
    ...
    if root_after_second==u32(0):
      ...
      # 207 rootprdescendnf:
      #   pr_descend is used only as a nonzero/zero continuation flag.
      #   Initialize it directly from pr_nf and let later kill conditions set it
      #   to zero, avoiding the separate pr_nf==0 branch.
      pr_descend:u32=pr_nf
      if pr_descend!=u32(0):
...
CHUNKSHAPE148_DEFAULT_REASON:str="207 rootprdescendnf reuses validated 204 scorestripe_v9 task order/cache; MAXD14 root_after_second scalar shape restored after 205 was not adopted"
...
def main()->None:
  ...
  if use_gpu and gpu_log_level>=1:
    ...
    if bench_mode==30:
      print(f"split207_roottwo_rootprdescendnf_probe: mode={bench_mode} preset={preset_queens_arg} chunk_start={debug_chunk_start} chunk_count={debug_chunk_count} chunk_list={microbench_chunk_list_spec}")
    if bench_mode==31:
      print(f"split207_roottwo_rootprdescendnf_full_gpu: mode={bench_mode} preset={preset_queens_arg}")
  ...
  for N in range(nmin,nmax):
    ...
    if use_gpu and N>=21 and bench_mode==30:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split207-probe-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split207-probe-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      ...
      progress_suffix:str=f"split207_probe_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
    ...
    if use_gpu and N>=21 and bench_mode==31:
      ...
      if reorder_records==stream_records and done_count==stream_records and validate_bin_file(reorder_fname):
        if gpu_log_level>=1:
          print(f"[split207-full-reuse] N={N} records={reorder_records} chunks={reorder_chunks} bin={reorder_fname} param={funcid_reorder_run_param_tag(gpu_block,gpu_max_blocks)}")
      else:
        if gpu_log_level>=1:
          print(f"[split207-full-build] N={N} stream_records={stream_records} existing_records={reorder_records} done_count={done_count} bin={reorder_fname}")
        reorder_fname,reorder_records,reorder_chunks=build_broad_markdist_tail_reordered_bin(N,stream_fname,preset_queens,gpu_block,gpu_max_blocks,gpu_log_level,gpu_sort_mode)
      base_reorder_fname:str=reorder_fname
      base_reorder_records:int=reorder_records
      base_reorder_chunks:int=reorder_chunks
      ...
      reorder_records=shaped_records
      reorder_chunks=shaped_chunks
      progress_suffix:str=f"split207_full_{CHUNKSHAPE148_REORDER_VERSION}_{BROAD_MARKDIST_TAIL_REORDER_VERSION}_{broad_markdist_tail_variant_tag()}"
