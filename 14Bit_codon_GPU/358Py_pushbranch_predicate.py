...
"""
**358の設計です。** 355・357(N=18静的・N=21動的、356前後2回とも
独立に実測)が確定させた事実: `branch_resolving`の主犯2本のBRA/BSYNC
(合算60%超)は356後もほぼ無傷、発散由来のncu見積り改善余地は79%
前後で完全に安定。356型の「制御フローの形を変えないビット幅
narrowing」ではこの構造に届かないことが、推測ではなく2回の実測で
確定した。358は**この方針を転換し、犯人①(プッシュ直後の再収束
BRA、branch_resolving全体の45.16%)を狙って制御フローの形そのもの
に手を入れる**、過去5敗(240, 266-269, 273, 326)以来はじめての
「制御フロー形状変更」に踏み込むリビジョンである。

## 何を変えたか(ホットループのpushガードのみ、1箇所)

対象はホットループ内(`while True:`内)のpushガード、355/357が
SASSアドレスまで特定した犯人①そのものである:
...
# 変更後
stack[stack_ptr]=...              # 無条件ストアに
stack[stack_ptr+1]=...            # 無条件ストアに
push_inc:int=2 if cur_avail!=u32(0) else 0
stack_ptr+=push_inc               # 述語実行向けの三項演算に
save_sp+=(u32(1) if cur_avail!=u32(0) else u32(0))
```

**`if`という分岐の形自体をソースから消した。** ストア2本を無条件化
し、ポインタ/カウンタの更新だけを三項演算にすることで、Codonの
バックエンドが分岐(BSSY/BSYNC/BRA)ではなく述語実行(select系
命令)にコンパイルすることを狙う。

## メモリ安全性の根拠

`stack=__array__[u64](MAXD14_ANCESTOR*2)`(26要素)。`save_sp`/
`stack_ptr`は各コンステレーションごとに0リセットされ、`save_sp`が
`MAXD14_ANCESTOR`(13)を超えないことは、`314666222712`という正当性
オラクルが全リビジョンを通じて裏付けてきた設計不変条件である。
つまりこの無条件ストアが実行される時点の`stack_ptr`(インクリメント
**前**の値)は常に24以下であり、`stack[stack_ptr]`・
`stack[stack_ptr+1]`(最大24,25)への書き込みは配列範囲(0-25)を
一度も超えない。`cur_avail==u32(0)`のときに書かれる値は「本物の
プッシュ」で上書きされるまで一切読まれない(読み出しは`save_sp`/
`stack_ptr`が実際に進んだ位置でのみ発生する)ため、正当性には
影響しない。

## 変更範囲を1箇所に絞った理由(単一変数の原則)

同じpushパターンはホットループ外に1箇所(1コンステレーションに
つき1回だけ通るdescend前のプリアンブル)あるが、355/357が実測で
名指ししたのはホットループ側のみなので、**そちらだけ**を変更した。
プリアンブル側の`if cur_avail!=u32(0):`はそのまま残っている。

**pop側(犯人②、15.30%)には同じ手法を使わなかった。** pop側は
`stack[stack_ptr-2]`を読む前に`save_sp==u32(0)`を判定する必要があり、
無条件ロードにすると`stack_ptr=0`のとき配列範囲外(負のインデック
ス)を読みに行く。push側のような「安全に無条件化できる」根拠が
ないため、今回は対象外とした。

## 過去5敗との違い、そして今回のリスク

240・266-269・273・326はいずれも**カーネル分解**(funcid別の専用
カーネル分岐、ホットループの複製)だった。358は**分解もループ複製
もしない**——変更は1箇所のガード構造のみで、ループの本数・カーネル
の分割数は一切変わらない。ただし356とは異なり、**今回は意図的に
制御フローの形を変えている**。項目12(カーネル微修正はくじ引き)の
リスクは356より高いことを認識した上で進める。
...
VERSION_TAG:str="358 pushbranch-predicate: FIRST CONTROL-FLOW-SHAPE CHANGE SINCE THE 240/266-269/273/326 LOSSES. 355 and 357 (two independent measurements, N18 static SourceCounters and N21 dynamic SchedulerStats plus WarpStateStats, taken before and after 356) confirmed that stall_branch_resolvings two culprit BRA instructions, both adjacent to BSYNC reconvergence markers, remain essentially untouched by 356s bit-width narrowing: 60.2 percent then 60.45 percent combined, and ncus own divergence-driven local speedup estimate stayed at 79.68 then 79.29 percent. 358 targets culprit one directly, the hot-loop push guard, 45.16 percent of stall_branch_resolving on its own. The guard if cur_avail not equal zero around the two stack stores plus the stack_ptr and save_sp increments is rewritten: the two stores execute unconditionally at the current, pre-increment stack_ptr, and only the pointer and counter updates stay conditional, expressed as ternaries rather than an if, so the compiler is nudged toward predicated select instructions instead of an actual branch and its BSYNC reconvergence point. This IS a control-flow-shape change, unlike 356, and is scoped to exactly one site: the once-per-constellation preamble push (the same guard pattern, structurally identical, outside the hot loop) is deliberately left untouched, and so is the pop path (culprit two, 15.30 percent), because an unconditional load at stack_ptr minus two would underflow when save_sp is zero, unlike the push side. Memory safety for the unconditional stores rests on an existing, oracle-verified invariant: stack is sized MAXD14_ANCESTOR times two, 26 u64 words, save_sp resets to zero per constellation and never reaches MAXD14_ANCESTOR while correctness holds at 314666222712, so the pre-increment stack_ptr at this site is always at most 24, and a discarded speculative write here is never read back since loads only ever occur at positions a genuine push actually advanced save_sp and stack_ptr to. Judged on chunk0slash chunk1 against the 356 anchor (144590ms slash 144473ms), not session elapsed; adopt within plus 0.10 percent, revert and return to 355 slash 357s SASS-identification work otherwise; correctness 314666222712 checked before any timing is read either way. Regardless of adoption, a follow-up N18 SourceCounters rerun is planned to check whether culprit ones sample count actually moved, independent of whether wall clock moved, per item 12s lesson that mechanism and wall-clock outcome are separate questions."
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while idx<m:
      ...
      while True:
        ...
        # 358: hot-loop push guard converted from an `if` (BSYNC/BRA
        # reconvergence, culprit-1 in 355/357's SourceCounters, 45.16% of
        # stall_branch_resolving) to an unconditional store + predicated
        # (ternary) pointer/counter update. Safe because stack_ptr's
        # pre-increment value here is always <= (MAXD14_ANCESTOR-1)*2 = 24
        # (stack is sized MAXD14_ANCESTOR*2 = 26; save_sp never reaches
        # MAXD14_ANCESTOR, an invariant the 314666222712 oracle has held
        # across every prior revision), so writing stack[stack_ptr] and
        # stack[stack_ptr+1] here is always in-bounds even when cur_avail==0
        # and the write is about to be discarded (never reached by a load,
        # since loads only ever occur at stack_ptr/save_sp positions a real
        # push actually advanced to). The once-per-constellation preamble
        # push (the other `if cur_avail!=u32(0):` site, untouched) and the
        # pop path (untouched -- an unconditional load at stack_ptr-2 would
        # underflow when save_sp==0) are deliberately left alone; this is a
        # single, isolated change at the one site 355/357 measured.
        stack[stack_ptr]=u64(cur_ld)|(u64(cur_rd)<<u64(32))
        stack[stack_ptr+1]=u64(cur_col)|(u64(cur_avail|(u32(cur_depth)<<u32(27)))<<u64(32))
        push_inc:int=2 if cur_avail!=u32(0) else 0
        stack_ptr+=push_inc
