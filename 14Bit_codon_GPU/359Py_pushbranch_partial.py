...
"""
**359の設計です。** 358(ホットループpushガードを完全に分岐除去)
は3チャンクとも一貫した退行(+3.47%〜+5.51%)で差し戻された。事後
のN=18 SourceCounters再測定で、仮説は**部分的に正しかった**ことが
判明した: BSYNC/BRA再収束は確かに縮んだ(旧犯人①のBRAアドレスは
上位から消滅、BSYNC自体は5.85%まで縮小)。しかし`stall_long_sb`
(メモリレイテンシ系のスコアボード待ち)が5.6%→17.8%へ激増し、
差し引きで負けた——**ストア2本を無条件化したことで、元は`if`を
スキップしていたレーンも含め全レーンが常にストア命令を発行する
ようになった**ことが原因と考えられる(機構は断定しない)。

359は**このコストを半分に抑える**: 無条件化するストアを2本から
1本(ldrd側のみ)に減らし、colav側のストアと`stack_ptr`/`save_sp`の
更新は元の`if`の中に残す(4文→3文のガードに縮小)。
...
`if`という分岐の形自体は残す(358とはここが違う)。358ほど積極的
ではないが、ガードの中身が4文から3文に減ることで分岐コスト自体も
いくらか軽くなることを期待しつつ、無条件ストアによるメモリ負荷の
増分は半分に抑える狙い。

## 不確実性を正直に記録する

**`if`が残るため、コンパイラが依然として分岐(BSYNC/BRA)を生成する
可能性が高い。** その場合、356比で「分岐削減効果はほぼゼロ+ストア
1本分のコスト増」という、356より悪い結果になるリスクがある。これは
358の教訓(部分的な無条件化が必ずしも比例して効くとは限らない)を
踏まえた上での、あくまで実測で判定すべき仮説である。
...
356/358の手順を踏襲。**357・358はどちらもコード変更なし/差し戻し
のため、356が実質的なアンカーである。** chunk0/chunk1を356実測値
...
バランスが358とどう違うか(分岐削減効果が半分程度に縮んだか、
...
VERSION_TAG:str="359 pushbranch-partial: SECOND CONTROL-FLOW-SHAPE ATTEMPT AFTER 358S REJECTION. 358 hoisted BOTH stack stores unconditional in the hot-loop push guard and was REVERTED: all 3 chunks regressed +3.47 to +5.51 percent against the 356 anchor despite correctness holding and every static check passing. 358s post-mortem N18 SourceCounters rerun showed the branch-elimination hypothesis was PARTIALLY correct: BSYNC reconvergence for this guard did shrink, old culprit-ones BRA addresses dropped out of the top branch_resolving contributors and BSYNC itself fell to 5.85 percent of it, but stall_long_sb, memory-latency scoreboard waits, exploded from roughly 5.6 to 17.77 percent, because both stores became unconditional and every lane now issues them regardless of whether it used to skip the guard under 356s original branch. 359 halves that cost: only the ldrd store, stack index stack_ptr, is hoisted unconditional; the colav store at stack_ptr plus one, and the stack_ptr slash save_sp updates, stay inside a smaller if, three statements instead of 356s four. The if shape itself remains, unlike 358, so it is NOT certain the compiler generates less branch cost than 356s baseline at all; that uncertainty is exactly what this revision measures, honestly acknowledged rather than assumed away. Memory safety is identical to 358s argument: stack is sized MAXD14_ANCESTOR times two, 26 u64 words, save_sp never reaches MAXD14_ANCESTOR while correctness holds at 314666222712, so the pre-increment stack_ptr here is always at most 24, and the unconditional ldrd store, when cur_avail is zero, is simply discarded and never read back. Judged on chunk0 slash chunk1 against the 356 anchor 144590ms slash 144473ms, not session elapsed, since 357 made no code change and 358 was reverted; adopt within plus 0.10 percent, revert and close this axis in favor of item 6 the CUDA C runner otherwise; correctness 314666222712 checked before any timing is read either way. Regardless of adoption, a follow-up N18 SourceCounters rerun is planned to compare branch_resolving and long_sb against both 356 and 358, to see whether halving the unconditional-store count roughly halves long_sb's growth as hypothesized."
...
def kernel_dfs_iter_gpu_maxd14(
    ...
    while idx<m:
      ...
      while True:
        ...
        # 359: 358 was REJECTED (+3.5%..+5.5% regression, all 3 chunks).
        # 358's N=18 post-mortem showed the branch-elimination hypothesis
        # was PARTIALLY right: BSYNC/BRA reconvergence for this guard did
        # shrink (culprit-1's old BRA addresses dropped out of the top
        # branch_resolving contributors, BSYNC share fell to 5.85%), but
        # stall_long_sb (memory-latency scoreboard waits) exploded from
        # ~5.6% to 17.77%, because BOTH stores became unconditional and
        # every lane now issues them regardless of whether it used to skip
        # the guard. 359 halves that cost: only the ldrd store is hoisted
        # unconditional; the colav store and the stack_ptr/save_sp updates
        # stay inside a (now 3-statement, down from 358's would-be-4) `if`.
        # This keeps the `if` shape -- unlike 358 -- so it is NOT certain
        # the compiler generates less branch cost than 356's baseline; that
        # is exactly what this revision measures. Safe for the same reason
        # 358 was: stack_ptr's pre-increment value here is always <=24 (see
        # 358's memory-safety note), so the unconditional ldrd store is
        # always in-bounds and, when cur_avail==0, is simply discarded
        # (never read back, since loads only occur at positions a genuine
        # push actually advanced save_sp/stack_ptr to).
        stack[stack_ptr]=u64(cur_ld)|(u64(cur_rd)<<u64(32))
        if cur_avail!=u32(0):
          stack[stack_ptr+1]=u64(cur_col)|(u64(cur_avail|(u32(cur_depth)<<u32(27)))<<u64(32))
          stack_ptr+=2
