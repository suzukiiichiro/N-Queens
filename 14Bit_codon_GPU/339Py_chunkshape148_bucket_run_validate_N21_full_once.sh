...
# =============================================================================
# 339 N=21 full validation harness (chunkshape148-bucket-run)
# Base: 338/337/336/335/334/333/328-332 lineage.
#
# NO KERNEL OR DISPATCHER CHANGE. All five @gpu.kernel bodies,
# launch_kernel_dfs_iter_gpu_static_maxd, K_PER_THREAD_MAXD14=48, the SoA
# w_lo_arr/w_hi_arr split and the w3_j7 funcid_reorder defaults are
# byte-for-byte identical to 338. 339's only code change is HOST-SIDE, in
# build_chunkshape148_reordered_bin.
#
# 338 RESULT (confirmed, used as this run's baseline): correctness
#   314666222712, elapsed 450.329s (vs 337's 450.056s, -0.061%, within
#   noise), zero static-check failures.
#
# WHAT 339 CHANGES AND WHY (Open Objectives #8):
#   329 measured the tail effect as Divergent Branches=0 with Avg Threads
#   Executed 2-3/32 -- i.e. intra-warp trip-count variance, not branch
#   divergence. Three facts from source reading:
#     (a) grid-stride indexing means a warp's 32 lanes process 32
#         CONSECUTIVE records per iteration: idx = n*stride + 32w + j.
#     (b) since rev276, build_chunkshape148_reordered_bin emits ONE record
#         per score bucket while cycling interleave_order=[7,0,6,1,5,2,4,3]
#         (highest score, lowest, second-highest, ...). Every aligned
#         32-record group therefore spans all 8 buckets four times over --
#         the current order MAXIMISES intra-warp cost heterogeneity.
#     (c) that same interleave is what keeps WARPS balanced against each
#         other, so a naive global cost sort would just trade one
#         imbalance for another.
#   Both can hold at once: build_chunkshape148_reordered_bin works in units
#   of STEPS = BLOCK*MAX_BLOCKS = 15488, which is exactly ONE grid-stride
#   iteration, and it already rotates the bucket order by out_ch each
#   iteration (order_pos=(oi+out_ch)%8). Emitting 32 consecutive records
#   from one bucket gives warp w the bucket interleave_order[(w+out_ch)%8]
#   at iteration out_ch, so every warp still sees all 8 buckets
#   near-uniformly across the 131 iterations of an N=21 run, while each
#   individual warp-iteration becomes cost-homogeneous.
#
# THE KNOB: CHUNKSHAPE148_BUCKET_RUN = records emitted consecutively from
#   one bucket before rotating. SOURCE DEFAULT IS 1 = the exact 276-338
#   emit order, and chunkshape148_bucket_run_tag() returns "" for 1 so the
#   already-cached shaped bin FILENAME is unchanged and the 333-338
#   baseline needs no rebuild. THIS HARNESS PASSES 32 EXPLICITLY.
#
# CORRECTNESS IS STRUCTURAL: reordering is a permutation, so the total is
#   invariant by construction. 314666222712 must still hold; if it does
#   not, the reorder dropped or duplicated records and dispatch_task_sum /
#   full_chunk_sum will say so.
#
# TIMING CAVEAT -- READ BEFORE COMPARING:
#   A new BUCKET_RUN value has no cached shaped bin, so the FIRST run
#   rebuilds it and the N=21 elapsed line INCLUDES that rebuild. This
#   harness records chunkshape148_cache_state=build|reuse so the summary
#   says for itself whether the timing row is comparable. Protocol,
#   the same one used for the 332/333 w3_j7 measurements:
#     run 1 -> correctness + cache build (timing NOT comparable)
#     run 2 -> cached; THIS is the number to compare against 450.329s
#
# MANDATORY VALIDATION ORDER:
#   1. STATIC_ONLY=1 first. Four new gating checks are added
#      (bucket_run source default =1, filename tag helper, the run-length
#      emit loop, and the argv[16] plumbing).
#   2. Full N=21 run; confirm 314666222712 FIRST, before any timing.
#   3. Second full N=21 run (cached) for the timing comparison.
#   4. To reproduce the 338 baseline exactly at any time:
#      CHUNKSHAPE148_BUCKET_RUN=1 bash <this script>
# EXPECTED_CHUNKS=3, K=48 (unchanged).
# =============================================================================

SRC=${SRC:-./339Py_chunkshape148_bucket_run.py}
CAND=${CAND:-./339Py_chunkshape148_bucket_run}
...
LOG_ROOT=${LOG_ROOT:-.}
LOCK_FILE=${LOCK_FILE:-/tmp/339Py_chunkshape148_bucket_run_N21_full.lock}
...
CHUNKSHAPE148_BUCKET_RUN=${CHUNKSHAPE148_BUCKET_RUN:-32}
...
LOGDIR="${LOG_ROOT%/}/339Py_chunkshape148_bucket_run_logs_N21_full_once_${TS}"
...
echo "[start] 339 chunkshape148-bucket-run validation script"
echo "[source] $SRC"
echo "[candidate] $CAND"
echo "[logdir] $LOGDIR"
trap 'rc=$?; if [[ $rc -ne 0 ]]; then echo "[abort] rc=$rc logdir=${LOGDIR:-unknown}" >&2; fi' EXIT
echo "[validation-start] 339 chunkshape148-bucket-run SRC=$SRC CAND=$CAND STATIC_ONLY=$STATIC_ONLY FORCE_REBUILD=$FORCE_REBUILD LOGDIR=$LOGDIR"
...
if grep -q '339 chunkshape148-bucket-run' "$SRC"; then
  printf 'source_version_tag	339 chunkshape148-bucket-run	present	OK
' >> "$SUMMARY"
else
  printf 'source_version_tag	339 chunkshape148-bucket-run	missing	FAIL
...
CMD=("$CAND" -g "$N" "$N" "$BLOCK" "$MAX_BLOCKS" "$LOG_LEVEL" "$SORT_MODE" "$PRESET_QUEENS" "$BENCH_MODE" "$REORDER_WINDOW_MULT" "$REORDER_PHASE_JUMP" "$CROSS_STRIPE_SAFE" "$WORKER_ID" "$WORKER_COUNT" "$BROADMARK_VARIANT" "$CHUNKSHAPE148_BUCKET_RUN")
{
  ...
  echo "validation: one N=21 full run through 339 chunkshape148-bucket-run (kernel-identical to 338/337/336/335/334/333/328) mode31 split145 CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN"
...
echo "[validation-ok] 339 N=21 mode31 split145 full reproduced total with required=14, MAXD14, 208 bytes/thread, K_PER_THREAD_MAXD14=48 (3 dispatch/progress rows), BROADMARK_VARIANT=2 (rotate_only, unchanged); NO kernel or dispatcher change in 339 -- the only code change is host-side, the CHUNKSHAPE148_BUCKET_RUN knob in build_chunkshape148_reordered_bin (source default 1 = the exact 276-338 emit order and filename; this run used CHUNKSHAPE148_BUCKET_RUN=$CHUNKSHAPE148_BUCKET_RUN). Correctness is structural because reordering is a permutation. CHECK chunkshape148_cache_state IN THE SUMMARY BEFORE COMPARING TIMING: if it says build, this run also constructed the shaped bin and its elapsed is NOT comparable to 338s 450.329s -- re-run once more to get a cached number"
