...
# =============================================================================
# NQ_UPDATE_MEMO
# 230: 更新メモ: 229Py root-preroll normal-first は正当性OKだが217/225を上回らなかったため不採用。
#      230Pyは225Py generic ncol-earlyを親に、MAXD14 root-preroll内だけ
#      (cur_ld|pr_bit)/(cur_rd|pr_bit) を pr_placed_ld/pr_placed_rd として
#      共通化する rootpr place-or 微差実験を検証する。
# =============================================================================

SRC=${SRC:-./230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_probe.py}
CAND=${CAND:-./230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_probe}
AUTO_BUILD=${AUTO_BUILD:-1}
STATIC_ONLY=${STATIC_ONLY:-0}
LOCK_FILE=${LOCK_FILE:-/tmp/230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_validation.lock}
LOG_ROOT=${LOG_ROOT:-.}
...
if [[ "$STATIC_ONLY" != "1" ]] && command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[error] another 230 validation holds: $LOCK_FILE" >&2
    exit 75
  fi
fi

TS=$(date +%Y%m%d_%H%M%S)
LOGDIR="${LOG_ROOT%/}/230Py_kernel_rootpr_placeor_rootrestlate_futuremask_nosibling_maxd14_logs_N21_full_once_${TS}"
...
root_pos=s.find('# 230 rootpr-placeor:')
if root_pos < 0:
    root_pos=s.find('pr_placed_ld:u32=cur_ld|pr_bit')
...
placed_ld_pos=root_compact.find('pr_placed_ld:u32=cur_ld|pr_bit')
placed_rd_pos=root_compact.find('pr_placed_rd:u32=cur_rd|pr_bit')
if_pos=root_compact.find('ifpr_block_code!=u32(0):')
else_pos=root_compact.find('else:', if_pos)
block_ld_pos=root_compact.find('pr_nld=(pr_placed_ld<<pr_stepu)|pr_addvu|pr_bLiu')
block_rd_pos=root_compact.find('pr_nrd=(pr_placed_rd>>pr_stepu)|pr_bKu')
norm_ld_pos=root_compact.find('pr_nld=pr_placed_ld<<u32(1)')
norm_rd_pos=root_compact.find('pr_nrd=pr_placed_rd>>u32(1)')
pr_ncol_pos=root_compact.find('pr_ncol:u32=cur_col|pr_bit')
pr_nf_pos=root_compact.find('pr_nf:u32=bm&~(pr_nld|pr_nrd|pr_ncol)')
gen_ncol_pos=generic_compact.find('ncol:u32=cur_col|bit')
gen_nld_pos=generic_compact.find('nld:u32=u32(0)')
checks={
  'rootpr_marker_present': root_pos >= 0,
  'rootpr_placed_ld_present': placed_ld_pos >= 0,
  'rootpr_placed_rd_present': placed_rd_pos > placed_ld_pos >= 0,
  'rootpr_blockfirst_kept': if_pos > placed_rd_pos,
  'rootpr_normalfirst_absent': 'ifpr_block_code==u32(0):' not in root_compact,
  'rootpr_block_uses_placed': block_ld_pos > if_pos and block_rd_pos > block_ld_pos,
  'rootpr_normal_uses_placed': norm_ld_pos > else_pos and norm_rd_pos > norm_ld_pos,
  'rootpr_old_cur_or_absent': 'pr_nld=((cur_ld|pr_bit)<<pr_stepu)' not in root_compact and 'pr_nrd=((cur_rd|pr_bit)>>pr_stepu)' not in root_compact and 'pr_nld=(cur_ld|pr_bit)<<u32(1)' not in root_compact and 'pr_nrd=(cur_rd|pr_bit)>>u32(1)' not in root_compact,
  'rootpr_ncol_after_branch_kept': pr_ncol_pos > norm_rd_pos and pr_nf_pos > pr_ncol_pos,
  'generic_ncol_early_kept': 0 <= gen_ncol_pos < gen_nld_pos,
  'generic_xor_clear_kept': 'cur_avail=cur_avail^bit' in generic_compact,
  ...
  'generic_placed_absent': 'placed_ld:u32=' not in generic_compact and 'placed_rd:u32=' not in generic_compact,
}
if all(checks.values()):
    sys.exit(0)
print('source_rootpr_placeor_pycheck_failed=' + ','.join(k for k,v in checks.items() if not v))
sys.exit(1)
PYCHECK
then
  printf 'source_rootpr_placeor	rootrestlate_plus_rootpr_placeor	present	OK\n' >> "$SUMMARY"
else
  printf 'source_rootpr_placeor	rootrestlate_plus_rootpr_placeor	missing_or_old_shape	FAIL\n' >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
bad=sorted(set(tags)-{'split145','split230'})
print(' '.join(tags) if tags else 'none')
sys.exit(0 if 'split230' in tags and not bad else 1)
PYSPLIT
) && split_ok=1 || split_ok=0
if [[ "$split_ok" == 1 ]]; then
  printf 'source_split_tag\tsplit230 active preferred\t%s\tOK\n' "$actual_split" >> "$SUMMARY"
else
  printf 'source_split_tag\tsplit230 active preferred\t%s\tFAIL\n' "$actual_split" >> "$SUMMARY"; static_failures=$((static_failures+1))
fi
...
if (( static_failures != 0 )); then
  echo "================================================================"; echo "[static-summary]"; cat "$SUMMARY"; echo "[logdir] $LOGDIR"
  echo "[error] 230 source static checks failed" >&2
  exit 66
fi
...
echo "[validation-ok] 228 root-preroll ncol-early reproduced cases 01-06 with required=14, MAXD14, 208 bytes/thread"
