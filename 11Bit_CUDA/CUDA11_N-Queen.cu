...
14:       365596           45752  00:00:00:00.53
15:      2279184          285053  00:00:00:02.34
16:     14772512         1846955  00:00:00:14.77
17:     95815104        11977939  00:00:01:56.55
*/
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
      ...
      if((bit&mask)!=0){//置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          if(totalCond==steps){
            ...
            /***11 BOUND1,BOUND2,SIDEMASK,LASTMASK追加*********************/
...
{
  /***11　から枝刈りするのでTOPBIT,ENDBIT,LASTMASK,SIDEMASK,BOUND1,BOUND2を使用*********************/
  int TOPBIT;
  int ENDBIT;
  int LASTMASK;
  int SIDEMASK;
...
//
/***11 枝刈りをするので引数を追加*********************/
//void backTrack2(int size,int mask, int row,int h_left,int h_down,int h_right,unsigned int* aBoard)
...
void backTrackR2(int size,int mask, int row,int left,int down,int right,int BOUND1,int BOUND2,int SIDEMASK,int LASTMASK,unsigned int* aBoard)
{
 ...
 if(row==sizeE){
    ...
    while(bitmap){
      ...
      backTrackR2(size,mask,row+1,(left|bit)<<1, down|bit,(right|bit)>>1,BOUND1,BOUND2,SIDEMASK,LASTMASK,aBoard);
    }
  }
}
//
...
void NQueenR(int size,int mask)
{
  ...
  for(int col=1,col2=size-2;col<col2;col++,col2--){
      ...
      backTrackR2(size,mask,1,bit<<1,bit,bit>>1,BOUND1,BOUND2,SIDEMASK,LASTMASK,aBoard);
      /***11 LASTMASK,ENDBIT*********************/
      LASTMASK|=LASTMASK>>1|LASTMASK<<1;
      ENDBIT>>=1;
  }
...
//通常版
/***11 枝刈りする*****/ 
void backTrack2D_NR(int size,int mask,int row,int left,int down,int right,int BOUND1,int BOUND2,int SIDEMASK,int LASTMASK,unsigned int* aBoard){
  int bitmap,bit;
  int b[100], *p=b;
  int sizeE=size-1;
...
void NQueenD(int size,int mask){
  ...
  for(BOUND1=1,BOUND2=size-2;BOUND1<BOUND2;BOUND1++,BOUND2--){
    ...
    backTrack2D_NR(size,mask,1,bit<<1,bit,bit>>1,BOUND1,BOUND2,SIDEMASK,LASTMASK,aBoard);
    LASTMASK|=LASTMASK>>1|LASTMASK<<1;
    ENDBIT>>=1;
  }
}
...
// 通常版
/***09 backTrack登場メソッド名だけ枝刈りはまだしない*****/  
void backTrack2D(int size,int mask,int row,int left,int down,int right,int BOUND1,int BOUND2,int SIDEMASK,int LASTMASK,unsigned int* aBoard){
  int bit;
  ...
  if(row==size-1){
    ...
    /***11 【枝刈り】 最下段枝刈り*****/
      ...
      while(bitmap){
        ...
        backTrack2D(size,mask,row+1,(left|bit)<<1,down|bit,(right|bit)>>1,BOUND1,BOUND2,SIDEMASK,LASTMASK,aBoard);
      }
    }
  }
}
...
void NQueenDR(int size,int mask)
{
  ...
  for(BOUND1=1,BOUND2=size-2;BOUND1<BOUND2;BOUND1++,BOUND2--){
    aBoard[0]=bit=(1<<BOUND1);
    backTrack2D(size,mask,1,bit<<1,bit,bit>>1,BOUND1,BOUND2,SIDEMASK,LASTMASK,aBoard);
    LASTMASK|=LASTMASK>>1|LASTMASK<<1;
    ENDBIT>>=1;
  }
}
