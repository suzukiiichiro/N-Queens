...
// ビット操作関数プロトタイプ
int checkRotations(GHashTable* set, int i, int j, int k, int l);
...
int checkRotations(GHashTable* ijklList, int i, int j, int k, int l) {
    int rot90 = ((N-1-k) << 15) + ((N-1-l) << 10) + (j << 5) + i;
    int rot180 = ((N-1-j) << 15) + ((N-1-i) << 10) + ((N-1-l) << 5) + (N-1-k);
    int rot270 = (l << 15) + (k << 10) + ((N-1-i) << 5) + (N-1-j);

    if (g_hash_table_contains(ijklList, &rot90)) {
        return 1; // true
    }
    if (g_hash_table_contains(ijklList, &rot180)) {
        return 1; // true
    }
    if (g_hash_table_contains(ijklList, &rot270)) {
        return 1; // true
    }
    return 0; // false
}
...
void genConstellations() {
    ...
    GHashTable* ijklListJasmin = g_hash_table_new_full(g_int_hash, g_int_equal, free, NULL);
    GHashTableIter iter;
    gpointer key, value;

    g_hash_table_iter_init(&iter, ijklList);
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        int startConstellation = *(int*)key;
        int* jasminKey = malloc(sizeof(int));
        *jasminKey = jasmin(startConstellation);
        g_hash_table_add(ijklListJasmin, jasminKey);
    }

    g_hash_table_destroy(ijklList);

    ijklList = ijklListJasmin;

    //int i, j, k, l, ld, rd, col, currentSize = 0;
     for (GList* keys = g_hash_table_get_keys(ijklList); keys != NULL; keys = keys->next) {
        int sc = *(int*)keys->data;
        int i = geti(sc);
        int j = getj(sc);
        int k = getk(sc);
        int l = getl(sc);
...
int main(int argc, char** argv) {
  ...
  for(unsigned int size=min;size<=targetN;++size){
    ...
    ijklList = g_hash_table_new_full(g_int_hash, g_int_equal, free, NULL);
