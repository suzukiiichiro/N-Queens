...
__global__
void BitBoard_cuda_kernel_b1(const unsigned int size,unsigned int mark,unsigned int* _down,unsigned int* _left,unsigned int* _right,unsigned long* _total,unsigned long* _unique,unsigned long _cond,unsigned int _row,struct local* l)
{
  ...
  __shared__ unsigned long sum[THREAD_NUM];
  __shared__ unsigned long usum[THREAD_NUM];
  //余分なスレッドは動かさない 
...
__global__
void BitBoard_cuda_kernel_b2(const unsigned int size,unsigned int mark,unsigned int* _down,unsigned int* _left,unsigned int* _right,unsigned long* _total,unsigned long* _unique,unsigned long _cond,unsigned int* board,unsigned int _row,struct local* l)
{
  const unsigned int mask=(1<<size)-1;
  unsigned long total=0;
  unsigned long unique=0;
...
void BitBoard_backTrack2G(const unsigned int size,unsigned int row,unsigned int _left,unsigned int _down,unsigned int _right,struct local* l)
{
  ...
  cudaMallocHost((void**) &hostTotal,sizeof(long)*l->STEPS);
  unsigned long* hostUnique;
  cudaMallocHost((void**) &hostUnique,sizeof(long)*l->STEPS);
