...
//1. void BitBoard_cuda_kernel_b2(const unsigned int size,unsigned int mark,unsigned int* _total,unsigned int* _unique,unsigned long _cond,unsigned int* board,unsigned int _row,struct local* l)
{
...
// 5.void BitBoard_cuda_kernel_b1(const unsigned int size,unsigned int mark,unsigned int* _total,unsigned int* _unique,unsigned long _cond,unsigned int _row,struct local* l)
{
  const unsigned int mask=(1<<size)-1;
  unsigned long total=0;
  unsigned int unique=0;
  ...
  __shared__ unsigned int down[THREAD_NUM][10];
  down[tid][row]=_down[idx];
  //6 down[tid][row]=l->DOWN[idx];
  __shared__ unsigned int left[THREAD_NUM][10];
  left[tid][row]=_left[idx];
  //7 left[tid][row]=l->LEFT[idx];
  __shared__ unsigned int right[THREAD_NUM][10];
  right[tid][row]=_right[idx];
  //8 right[tid][row]=l->RIGHT[idx];
  __shared__ unsigned int bitmap[THREAD_NUM][10];
  bitmap[tid][row] =mask&~(down[tid][row]|left[tid][row]|right[tid][row]);
  __shared__ unsigned int sum[THREAD_NUM];
  __shared__ unsigned int usum[THREAD_NUM];
...
void BitBoard_backTrack2G(const unsigned int size,unsigned int row,unsigned int _left,unsigned int _down,unsigned int _right,struct local* l)
{
  ...
  cudaMallocHost((void**) &hostDown,
      sizeof(int)*l->STEPS);
  unsigned int* hostLeft;
  cudaMallocHost((void**) &hostLeft,
      sizeof(int)*l->STEPS);
  unsigned int* hostRight;
  cudaMallocHost((void**) &hostRight,
      sizeof(int)*l->STEPS);
  unsigned int* hostTotal;
  cudaMallocHost((void**) &hostTotal,
      sizeof(long)*l->STEPS);
  unsigned int* hostUnique;
  cudaMallocHost((void**) &hostUnique,
      sizeof(long)*l->STEPS);
  unsigned int* hostBoard;
  cudaMallocHost((void**) &hostBoard,
      sizeof(int)*l->STEPS*mark);
  //device
  //10 
  unsigned int* deviceDown;
  cudaMalloc((void**) &deviceDown,
      sizeof(int)*l->STEPS);
  unsigned int* deviceLeft;
  cudaMalloc((void**) &deviceLeft,
      sizeof(int)*l->STEPS);
  unsigned int* deviceRight;
  cudaMalloc((void**) &deviceRight,
      sizeof(int)*l->STEPS);
  unsigned int* deviceTotal;
  cudaMalloc((void**) &deviceTotal,
      sizeof(long)*l->STEPS/THREAD_NUM);
  unsigned int* deviceUnique;
  cudaMalloc((void**) &deviceUnique,
      sizeof(long)*l->STEPS/THREAD_NUM);
  unsigned int* deviceBoard;
  cudaMalloc((void**) &deviceBoard,
      sizeof(int)*l->STEPS*mark);
  struct local* hostLocal;
  cudaMallocHost((void**) &hostLocal,
      sizeof(struct local)*l->STEPS);
  struct local* deviceLocal;
  cudaMallocHost((void**) &deviceLocal,
      sizeof(struct local)*l->STEPS);
  ...
  for(int i=0;i<MAX;i++){
    ...
    hostLocal[0].DOWN[i]=l->DOWN[i];
    hostLocal[0].RIGHT[i]=l->RIGHT[i];
    hostLocal[0].LEFT[i]=l->LEFT[i];
  }
    ...
    if(bitmap[row]==0){ 
      ...
      if((bit&mask)!=0){          //置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          //ン置き終わったらGPU並列実行
          //totalCond がthreadIdになる 
          // 各スレッドに down,left,right情報を渡す
          //row=2(13行目以降は増えていく。
          // 例えばn15だとrow=5)の情報を
          ...
          */
          //11 
          hostLocal[0].DOWN[totalCond]=down[row];
          hostLocal[0].LEFT[totalCond]=left[row];
          hostLocal[0].RIGHT[totalCond]=right[row];
          for(int i=0;i<mark;i++){
            hostBoard[totalCond*mark+i]=l->board[i];
          }
          
          totalCond++; //スレッド数をインクリメントする
          //最大GPU数に達してしまったら一旦ここでGPUを実行する。STEPSはGPUの同時並行稼働数を制御Ｎの数が少ないうちはtotalCondがSTEPSを超えることはないがnの数が増えて行くと超えるようになる。
          //ここではtotalCond==STEPSの場合だけこの中へ         
          if(totalCond==l->STEPS){
            //matched=trueの時にCOUNT追加 
            //GPU内でカウントしているので、
            // GPUから出たらmatched=trueになってる
            if(matched){
              cudaMemcpy(hostTotal,deviceTotal,
                  sizeof(long)*l->STEPS/THREAD_NUM,
                  cudaMemcpyDeviceToHost);
              cudaMemcpy(hostUnique,deviceUnique,
                  sizeof(long)*l->STEPS/THREAD_NUM,
                  cudaMemcpyDeviceToHost);
            ...
            //12 
            cudaMemcpy(deviceDown,hostDown,
                sizeof(int)*totalCond,
                cudaMemcpyHostToDevice);
            cudaMemcpy(deviceLeft,hostLeft,
                sizeof(int)*totalCond,
                cudaMemcpyHostToDevice);
            cudaMemcpy(deviceRight,hostRight,
                sizeof(int)*totalCond,
                cudaMemcpyHostToDevice);
            cudaMemcpy(deviceBoard,hostBoard,
                sizeof(int)*totalCond*mark,
                cudaMemcpyHostToDevice);
            ...
            //13 BitBoard_cuda_kernel_b2<<<l->STEPS/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceTotal,deviceUnique,totalCond,deviceBoard,row,deviceLocal);
            // STEPS数の数だけマルチスレッドで起動するのだが、
            // 実際に計算が行われるのはtotalCondの数だけで
            // それ以外は空回しになる
            // GPU内でカウントしているので、GPUから出たら
            // matched=trueになってる
            matched=true;
            // totalCond==STEPSルートでGPUを実行したら
            // スレッドをまた0から開始する
            // (これによりなんどもSTEPS数分だけGPUを起動できる)
            totalCond=0;           
          }
          //hostDown,hostLeft,hostRightに情報を格納したら
          // 1行上に上がる
          //これを繰り返すことにより row=2で可能な場所全てに
          // クイーンを置いてhostDown,hostLeft,hostRightに
          // 情報を格納する
          row--;
        }
      }else{
        //置く場所がなければ上に上がる。
        // row==mark行に達するまではCPU側で普通にnqueenをやる
        row--;
      }
    }
  }
  //matched=trueの時にCOUNT追加 
  //GPU内でカウントしているのでGPUから出たらmatched=trueになってる
  if(matched){
    cudaMemcpy(hostTotal,deviceTotal,
        sizeof(long)*l->STEPS/THREAD_NUM,
        cudaMemcpyDeviceToHost);
  ...
  //15  BitBoard_cuda_kernel_b2<<<l->STEPS/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceTotal,deviceUnique,totalCond,deviceBoard,mark,deviceLocal);
  cudaMemcpy(hostTotal,deviceTotal,
      sizeof(long)*l->STEPS/THREAD_NUM,
      cudaMemcpyDeviceToHost);
  cudaMemcpy(hostUnique,deviceUnique,
...
void BitBoard_backTrack1G(const unsigned int size,unsigned int row,unsigned int _left,unsigned int _down,unsigned int _right,struct local* l)
{
      ...
      if((bit&mask)!=0){//置く場所があれば先に進む
        ...
        if(row==mark){
          ...
          */
          ...
          //スレッド数をインクリメントする
          totalCond++;
          // 最大GPU数に達してしまったら一旦ここでGPUを実行する。
          // STEPSはGPUの同
          //時並行稼働数を制御
          //nの数が少ないうちはtotalCondがSTEPSを超えることはないが
          // nの数が増えて行くと超えるようになる。
          //ここではtotalCond==STEPSの場合だけこの中へ         
          if(totalCond==l->STEPS){
            //matched=trueの時にCOUNT追加 
            ...
            */
            //cudaMemcpy(deviceLocal,hostLocal,sizeof(struct local)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
            //cudaMemcpy(deviceLocal,hostLocal,sizeof(struct local)*l->STEPS/THREAD_NUM,cudaMemcpyDeviceToHost);
            cudaMemcpy(deviceLocal,hostLocal,
                sizeof(struct local)*l->STEPS,
                cudaMemcpyDeviceToHost);
            ...
            //15 BitBoard_cuda_kernel_b1<<<l->STEPS/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceTotal,deviceUnique,totalCond,row,deviceLocal);
            //STEPS数の数だけマルチスレッドで起動するのだが、
            // 実際に計算が行われるのはtotalCondの数だけで
            // それ以外は空回しになる
            //GPU内でカウントしているので、GPUから出たら
          ...
          //hostDown,hostLeft,hostRightに情報を格納したら
          // 1行上に上がる。これを繰り返すことにより 
          // row=2で可能な場所全てにクイーンを置いて
  ...
  */
  ...
  // BitBoard_cuda_kernel_b1<<<l->STEPS/THREAD_NUM,THREAD_NUM >>>(size,size-mark,deviceTotal,deviceUnique,totalCond,mark,deviceLocal);
  cudaMemcpy(hostTotal,deviceTotal,
      sizeof(long)*l->STEPS/THREAD_NUM,
      cudaMemcpyDeviceToHost);
  cudaMemcpy(hostUnique,deviceUnique,
