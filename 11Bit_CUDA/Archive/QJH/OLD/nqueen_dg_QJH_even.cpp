...
int Solver()
{
  long tick1,tick2,t1,t2;
  long Tans,Tcnt,Mans,Mcnt;
  int  etime;
  long  AnsAll[] = { 0L,0L,0L,0L,2L, 10L, 4L, 40L, 92L, 352L, 724L, 2680L, 14200L,
    73712L, 365596L, 2279184L, 14772512L, 95815104L, 666090624L, 4968057848L, 39029188884L, 314666222712L, 2691008701644L,  24233937684440L, 227514171973736L, 2207893435808352L, 22317699616364044L,  };
  long  AnsUnq[] = { 0L,0L,0L,0L,1L,  2L, 1L,  6L, 12L,  46L,  92L,  341L,  1787L, 9233L,  45752L,  285053L,  1846955L,  11977939L,  83263591L, 621012754L,  4878666808L, 39333324973L, 336376244042, 3029242658210L, 28439272956934L, 275986683743434L, 2789712466510289L,    };
  tick1 = clock();
  N2 = N * 2;
  init(N);
  Tans = 0;
  ...
  for (M=1; M<=N/2; M++) {
    ...
    t2 = clock();
    etime = (int)(t2-t1);
    fprintf(logid,"----- M=%2d   %12ld   %12ld    %7d        Mj : %9ld  Mn : %9ld  /   ABcnt : %9ld   CDcnt : %9ld\n", M,Mans,Mcnt,etime,Mjcnt,Mncnt,TABcnt,TCDcnt);
    fflush(logid);
  }
  clear();
  tick2 = clock();
  ...
  if ((Tans != AnsUnq[N2]) || (Tcnt != AnsAll[N2])) 
    fprintf(logid,"?????? Ans Error  %14ld  %14ld\n",AnsUnq[N2],AnsAll[N2]);
  fflush(logid);
  return 0;
}
...
//-------------------------------------------------------------------------
int  QCache(int c,PARTSJ *QIXP[],PARTSJ **Qdtp)
{
  int QClen,x,X,xy;
  PARTSJ *qdtp;
  PARTS  *dtp;
  ...
  for (x=0; x<Bit[N]; x++) {
    ...
    for (dtp=MnIXP[xy]; dtp<MnIXP[xy+1]; dtp++) {
      ...
      // (lldb)run
      printf("##### DEBUG %d\n",(MnIXP[xy]->v << N) + (MnIXP[xy]->u & MASKJ));
      //qdtp->jkey = (dtp->v << N) + (dtp->u & MASKJ);  
      QIXP[x]->jkey = (MnIXP[xy]->v << N) + (MnIXP[xy]->u & MASKJ);  
      qdtp++;
    }
  }
  return 0;
...
void QJoin(int M)
{
  ...
  for (ia=0; ia<Mjcnt; ia++) {
    ...
    if (QCache(A^MASK,DIXP,&Ddtp) < 0)         continue;
    ...
    for (ic=0; ic<Mjcnt; ic++) {
      ...
      if (QCache(C^MASK,BIXP,&Bdtp) < 0)     continue;
...
int PartsGen(int N,int M,int *IXC,PARTS *IXP[],PARTS **Dtp) 
{
  int k;
...
void QStore(int X,int Y,int U,int V) 
{
...
//-------------------------------------------------------------------------
void printNQ2(char ID[],long POS,int N)
{
  int k,n,pos[32];
...
//-------------------------------------------------------------------------
void printAns(char ID[],long A,long B,long C,long D,int N)
{
  int pos[32] = { 0 };
  int n,k;
  long w;
