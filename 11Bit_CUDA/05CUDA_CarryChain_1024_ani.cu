...
{
  unsigned int size;
  unsigned int pres_a[960]; 
  unsigned int pres_b[960];
  // uint64_t COUNTER[3];      
  // //カウンター配列
  // unsigned int COUNT2;
  // unsigned int COUNT4;
...
void carryChain_build_nodeLayer(int){
  ...
  cudaMalloc((void**) &pres_a_Cuda,sizeof(int)*960);
  cudaMalloc((void**) &pres_b_Cuda,sizeof(int)*960);
  cudaMalloc((void**) &resultsCuda,sizeof(int)*960/THREAD_NUM);
  int totalCond=g.size/2*(g.size-3);
  cudaMemcpy(pres_a_Cuda,g.pres_a,
      sizeof(int)*960,cudaMemcpyHostToDevice);
  cudaMemcpy(pres_b_Cuda,g.pres_b,
      sizeof(int)*960,cudaMemcpyHostToDevice);
  unsigned int* results=new unsigned int[960];
  cuda_kernel<<<960/THREAD_NUM,THREAD_NUM
    >>>(pres_a_Cuda,pres_b_Cuda,resultsCuda,totalCond,g.size);
  cudaMemcpy(results,resultsCuda,
      sizeof(int)*960/THREAD_NUM,cudaMemcpyDeviceToHost);
  for(int col=0;col<960/THREAD_NUM;col++){TOTAL+=results[col];}	
}
//メイン
