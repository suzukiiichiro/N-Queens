...
void dim_nodeLayer(unsigned int size,long* nodes,long* solutions,unsigned int numElements,struct local* l)
{
  ...
  dl[i].BOUND1=l[i].BOUND1;
  dl[i].BOUND2=l[i].BOUND2;
  dl[i].TOPBIT=l[i].TOPBIT;
  dl[i].ENDBIT=l[i].ENDBIT;
  dl[i].SIDEMASK=l[i].SIDEMASK;
  dl[i].LASTMASK=l[i].LASTMASK;
  dl[i].COUNT2=l[i].COUNT2;
  dl[i].COUNT4=l[i].COUNT4;
  dl[i].COUNT8=l[i].COUNT8;
  dl[i].TYPE=l[i].TYPE;
  for(int j=0;j<MAX;j++){ dl[j].board[j]=l->board[j]; }
