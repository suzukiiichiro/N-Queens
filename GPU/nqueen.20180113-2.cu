...
long long solve_nqueen_internal(int n,unsigned int mask,unsigned int l_mask,unsigned int r_mask,unsigned int t_mask) {
	if(mask==t_mask){ return 1; }
	unsigned int m=(mask|l_mask|r_mask);
	if((m&t_mask)==t_mask){ return 0; }
	long long total=0;
...
long long solve_nqueen_nonRecursive(int n){
  unsigned int mask[32]; unsigned int l_mask[32]; unsigned int r_mask[32]; unsigned int m[32];
  if(n<=0||n>32){ return 0; }
  const unsigned int t_mask=(1<<n)-1; long long total=0; long long upper_total=0;
  int i=0; int j=0; unsigned int index;
  mask[0]=0; l_mask[0]=0; r_mask[0]=0; m[0]=0;
  for(j=0;j<(n+1)/2;j++){
    index=(1<<j);
    m[0]|=index; mask[1]=index; l_mask[1]=index<<1; r_mask[1]=index >> 1;
    m[1]=(mask[1]|l_mask[1]|r_mask[1]);
    i=1;
    if(n%2==1 && j==(n+1)/2-1){ upper_total=total; total=0; }
    while(i>0){
      if((m[i]&t_mask)==t_mask){ i--; }
      else{
        index=((m[i]+1) ^ m[i])&~m[i];
        m[i]|=index;
        if((index&t_mask)!=0){
  ...
  else{ return upper_total * 2+total; }
}
/** CUDA 非再帰バージョン 
  CPUイテレータから複数の初期条件を受け取り、カウント
  1. バックトラック
  2. ビットマップ
14:            365596                 0          00:00:00:00.08
15:           2279184                 0          00:00:00:00.52
*/
__global__ void solve_nqueen_cuda_kernel_bt_bm(int n,int mark,unsigned int* total_masks,unsigned int* total_l_masks,unsigned int* total_r_masks,unsigned int* results,int total_conditions){
  ...
  int total=0; int i=0; unsigned int index;
  ...
  if(tid==0){ results[bid]=sum[0]; }
}
long long solve_nqueen_cuda(int n,int steps) {
  unsigned int mask[32]; unsigned int l_mask[32]; unsigned int r_mask[32];
  unsigned int m[32]; unsigned int index;
  if(n<=0||n>32){ return 0; }
  ...
  for(j=0;j<n/2;j++){
    index=(1<<j);
    m[0]|=index;
    mask[1]=index; l_mask[1]=index<<1; r_mask[1]=index >> 1;
    m[1]=(mask[1]|l_mask[1]|r_mask[1]);
    i=1;
    while(i>0){
      if((m[i]&t_mask)==t_mask){ i--; }
      else{
        index=(m[i]+1)&~m[i]; m[i]|=index;
        if((index&t_mask)!=0){
          mask[i+1]=mask[i]|index; l_mask[i+1]=(l_mask[i]|index)<<1; r_mask[i+1]=(r_mask[i]|index) >> 1;
          m[i+1]=(mask[i+1]|l_mask[i+1]|r_mask[i+1]);
          i++;
          if(i==mark){
            ...
            if(total_conditions==steps){
              ...
              /** backTrack+bitmap*/
              solve_nqueen_cuda_kernel_bt_bm<<<steps/THREAD_NUM,THREAD_NUM>>>(n,n-mark,masks_cuda,l_masks_cuda,r_masks_cuda,results_cuda,total_conditions);
              computed=true;
              total_conditions=0;
            }
            i--;
  ...
  cudaFree(masks_cuda); cudaFree(l_masks_cuda); cudaFree(r_masks_cuda); cudaFree(results_cuda);
  delete[] total_masks; delete[] total_l_masks; delete[] total_r_masks; delete[] results;
  return total;
}
/** CUDA 初期化 **/
bool InitCUDA(){
