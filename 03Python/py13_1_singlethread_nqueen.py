...
#
#  実行
#  $ python py13_1_singlethread_nqueen.py
#
#
# １３−１．シングルスレッドの構築
#       マルチスレッドの構築の準備としてシングルスレッドを構築します
#       シングルスレッドは一つのコア（CPU）を使っているに過ぎません。
  ...
  def nqueen(self):
    ...
    if self.child is None:
      if self.nmore > 0:
        ...
        self.bound1 = 1
        while (self.bound1 > 0) \
        and (self.bound2 < self.size - 1) \
        and (self.bound1 < self.bound2): # pylint: disable=C0301
          self.rec_bound2(self.bound1, self.bound2)
          self.bound1 += 1
          self.bound2 -= 1
#end WorkingEngine() class
#
def main():
  """ main() """
  nmax = 16
