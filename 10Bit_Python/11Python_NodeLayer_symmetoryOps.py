...
class NQueens21:
    ...
    def kLayer_nodeLayer_backtrack(self,size: int, nodes: list, k: int, 
      ...
      while bitmap:
        ...
        bitmap ^= bit
        local.board[row] = bit
        counter +=  self.kLayer_nodeLayer_backtrack(
    ...
    def kLayer_nodeLayer_backtrack_corner(self,size: int, nodes: list, k: int, 
                                        left: int, down: int, right: int, local: Local,types: list, local_list: list[Local]) -> int:
      counter: int = 0
      mask: int = (1 << size) - 1
      bitmap: int = mask & ~(left | down | right)
      bit : int=0
      ...
      while bitmap:
        ...
        bitmap ^= bit
        local.board[row] = bit
        counter +=  self.kLayer_nodeLayer_backtrack_corner(
            size, nodes, k, (left | bit) << 1, down | bit, (right | bit) >> 1,
            local,types, local_list
        )
      return counter
