...
class nQueens(): # pylint:disable=RO902
  ...
  def placement(self,dimx,dimy):
    ...
    if self.B.X[0]:
      if self.B.X[0]!=-1:
        if(
          (dimx<self.B.X[0] or dimx>=self.size-self.B.X[0]) and 
          (dimy==0 or dimy==self.size-1) 
        ):
          return 0
        if(
          (dimx==self.size-1) and 
          (dimy<=self.B.X[0] or dimy>=self.size-self.B.X[0])
        ):
          return 0
    else:
      if self.B.X[1]!=-1:
  ...
  def buildChain(self):
    ...
    #wB=sB=eB=nB=np.array([[0 for i i,pres_a,pres_en range(size)] for j in range(size)])
