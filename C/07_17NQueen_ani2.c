...
*/
void symmetryOps_bitmap(int SIZEE,int BOUND1,int BOUND2,int TOPBIT,int ENDBIT,int aBoard[],struct local *l,int *COUNT2,int *COUNT4,int *COUNT8){
  int own,ptn,you,bit;
  //90度回転
  ...
  if(y==SIZEE){
    if(bitmap!=0){ //【枝刈り】最下段枝刈り
      if( (bitmap&LASTMASK)==0){ 
        aBoard[y]=bitmap;
        //対称解除法
        symmetryOps_bitmap(SIZEE,BOUND1,BOUND2,TOPBIT,ENDBIT,aBoard,l,&*COUNT2,&*COUNT4,&*COUNT8); } }
  }else{
