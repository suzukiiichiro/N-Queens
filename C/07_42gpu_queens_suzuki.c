...
*/
cl_mem d_A,d_B,d_C;
struct queenState *inProgress_A;
struct queenState *inProgress_B;
struct queenState *inProgress_C;
int makeInProgress(int si){
	inProgress_A=(struct queenState *)malloc(si * sizeof(struct queenState));
	inProgress_B=(struct queenState *)malloc(si * sizeof(struct queenState));
	inProgress_C=(struct queenState *)malloc(si * sizeof(struct queenState));
  for(int i=0;i<si;i++){
  ...
  for(int i=0;i<si;i++){
  //for(int i=0;i<1;i++){ //Single
    inProgress_A[i].si=si;
    inProgress_A[i].id=i;
    for (int m=0;m<si;m++){ inProgress_A[i].aB[m]=m;}
    inProgress_A[i].lTotal=0;
    inProgress_A[i].lUnique=0;
    inProgress_A[i].step=0;
    inProgress_A[i].y=0;
    inProgress_A[i].bend=0;
    inProgress_A[i].rflg=0;
    for (int m=0;m<si;m++){ 
      inProgress_A[i].fA[m]=0;
      inProgress_A[i].fB[m]=0;
      inProgress_A[i].fC[m]=0;
      inProgress_A[i].aT[m]=0;
      inProgress_A[i].aS[m]=0;
    }
    for (int m=0;m<si;m++){ 
      inProgress_A[i].stParam.param[m].Y=0;
      inProgress_A[i].stParam.param[m].I=0;
    }
    inProgress_A[i].stParam.current=0;
  }
  for(int i=0;i<si;i++){
  //for(int i=0;i<1;i++){ //Single
    inProgress_B[i].si=si;
    inProgress_B[i].id=i;
    for (int m=0;m<si;m++){ inProgress_B[i].aB[m]=m;}
    inProgress_B[i].lTotal=0;
    inProgress_B[i].lUnique=0;
    inProgress_B[i].step=0;
    inProgress_B[i].y=0;
    inProgress_B[i].bend=0;
    inProgress_B[i].rflg=0;
    for (int m=0;m<si;m++){ 
      inProgress_B[i].fA[m]=0;
      inProgress_B[i].fB[m]=0;
      inProgress_B[i].fC[m]=0;
      inProgress_B[i].aT[m]=0;
      inProgress_B[i].aS[m]=0;
    }
    for (int m=0;m<si;m++){ 
      inProgress_B[i].stParam.param[m].Y=0;
      inProgress_B[i].stParam.param[m].I=0;
    }
    inProgress_B[i].stParam.current=0;
  }
  for(int i=0;i<si;i++){
  //for(int i=0;i<1;i++){ //Single
    inProgress_C[i].si=si;
    inProgress_C[i].id=i;
    for (int m=0;m<si;m++){ inProgress_C[i].aB[m]=m;}
    inProgress_C[i].lTotal=0;
    inProgress_C[i].lUnique=0;
    inProgress_C[i].step=0;
    inProgress_C[i].y=0;
    inProgress_C[i].bend=0;
    inProgress_C[i].rflg=0;
    for (int m=0;m<si;m++){ 
      inProgress_C[i].fA[m]=0;
      inProgress_C[i].fB[m]=0;
      inProgress_C[i].fC[m]=0;
      inProgress_C[i].aT[m]=0;
      inProgress_C[i].aS[m]=0;
    }
    for (int m=0;m<si;m++){ 
      inProgress_C[i].stParam.param[m].Y=0;
      inProgress_C[i].stParam.param[m].I=0;
    }
    inProgress_C[i].stParam.current=0;
  }
  ...
  d_A = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(inProgress_A), NULL, &status);
  d_B = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(inProgress_B), NULL, &status);
  d_C = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(inProgress_C), NULL, &status);
  clRetainMemObject(d_A);
  clRetainMemObject(d_B);
  clRetainMemObject(d_C);
  if(USE_DEBUG>0) if(status!=CL_SUCCESS){ printf("Couldn't create buffer.\n"); return 14; }
  /**
   *
   */
  ...
  //if(USE_DEBUG>0) if(status!=CL_SUCCESS){ printf("Couldn't enque write buffer command."); return 16; }
	status=clEnqueueWriteBuffer(cmd_queue,d_A,CL_FALSE,0,sizeof(inProgress_A),inProgress_A,0,NULL,NULL); 
	status=clEnqueueWriteBuffer(cmd_queue,d_B,CL_FALSE,0,sizeof(inProgress_B),inProgress_B,0,NULL,NULL); 
  if(USE_DEBUG>0) if(status!=CL_SUCCESS){ printf("Couldn't enque write buffer command."); return 16; }
  /**
   *
   */
  ...
  status=clSetKernelArg(kernel,3,sizeof(int),&si);
