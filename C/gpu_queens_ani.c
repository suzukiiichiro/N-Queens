...
int main(){
  ...
  */
  ...
  struct queenState inProgress[spread*spread]={ 0 };
  ...
  for(int i=0; i < spread; i++){
  for(int j=0; j < spread; j++){
    printf("%d: %llu\n",inProgress[i*spread+j].id,inProgress[i*spread+j].lTotal);
    lGTotal+=inProgress[i*spread+j].lTotal;
	}
	}
  printf("lGTotal:%ld\n",lGTotal);
