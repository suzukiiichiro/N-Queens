...
　（こちらが絶対的におすすめ）
 ...
 コンパイル
 $ nvcc 07_13N-Queen.cu -o 07_13N-Queen
...
*/
long long nqInternal(int n,unsigned int left,unsigned int down,unsigned int right) {
  ...
  if(down==msk){return 1;}
	...
	while((bit&msk)!=0){
		total+=nqInternal(n,(left|bit)<<1,down|bit,(right|bit)>>1);
		bm|=bit;
		bit=(bm+1)&~bm;
	}
	return total;
