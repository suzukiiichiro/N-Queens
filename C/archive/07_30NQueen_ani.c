  ...
  //B1 と B2を初期化
    for(int k=0;k<si;k++){
      ...
      for(int j=0;j<si;j++){
        ...
        l[B1-1][k][j].B2=B2;     
        for(int kj4=0;kj4<si;kj4++){
          l3[B1-1][k][j][kj4].B1=B1;
          l3[B1-1][k][j][kj4].B2=B2;
        }
      }
    }
    //aB[]の初期化
    for(int i=0;i<siE;i++){ 
      for(int k=0;k<si;k++){
        for(int j=0;j<si;j++){
          l[B1-1][k][j].aB[i]=i;
          for(int kj4=0;kj4<si;kj4++){
            l3[B1-1][k][j][kj4].aB[i]=i;  // 上から３行目のスレッドに使う構造体aB[]の初期化
          }
        }
      }
    } 
   //カウンターの初期化
    for(int k=0;k<si;k++){
      for(int j=0;j<si;j++){
        l[B1-1][k][j].C2[B1][0]=
        l[B1-1][k][j].C4[B1][0]= 
        l[B1-1][k][j].C8[B1][0]=0;	
        for(int kj4=0;kj4<si;kj4++){
          l3[B1-1][k][j][kj4].C2[B1][1]= 
          l3[B1-1][k][j][kj4].C4[B1][1]= 
          l3[B1-1][k][j][kj4].C8[B1][1]=0;	
        }
      }
    }
    ...
    for(int k=0;k<si;k++){
      for(int j=0;j<si;j++){
        ...
        for(int kj4=0;kj4<si;kj4++){
          l3[B1-1][k][j][kj4].k=k;
          l3[B1-1][k][j][kj4].j=j;
          l3[B1-1][k][j][kj4].kj4=kj4;
          pthread_create(&pt3[B1-1][k][j][kj4],NULL,&run3,(void*)&l3[B1-1][k][j][kj4]);// チルドスレッドの生成
        }
        for(int kj4=0;kj4<si;kj4++){
          pthread_join(pt3[B1-1][k][j][kj4],NULL); 
        }
        pthread_create(&pt1[B1-1][k][j],NULL,&run,(void*)&l[B1-1][k][j]);// チルドスレッドの生成
      }
      for(int j=0;j<si;j++){
        pthread_join(pt1[B1-1][k][j],NULL); 
      }
  ...
  */
  ...
  for(int B1=1;B1<siE;B1++){
    for(int k=0;k<si;k++){
      ...
      for(int j=0;j<si;j++){
        lTotal+=
          l[B1-1][k][j].C2[B1][0]*2+
          l[B1-1][k][j].C4[B1][0]*4+
          l[B1-1][k][j].C8[B1][0]*8;
        lUnique+=
          l[B1-1][k][j].C2[B1][0]+
          l[B1-1][k][j].C4[B1][0]+
          l[B1-1][k][j].C8[B1][0]; 
        for(int kj4=0;kj4<si;kj4++){
          //backtrack2の集計
          lTotal+=
            l3[B1-1][k][j][kj4].C2[B1][1]*2+
            l3[B1-1][k][j][kj4].C4[B1][1]*4+
            l3[B1-1][k][j][kj4].C8[B1][1]*8;
          lUnique+=
            l3[B1-1][k][j][kj4].C2[B1][1]+
            l3[B1-1][k][j][kj4].C4[B1][1]+
            l3[B1-1][k][j][kj4].C8[B1][1]; 
          }
      }
    }
  }
